---
summary: "CLI reference for `oriro directory` (self, peers, groups)"
read_when:
...
- `directory` is meant to help you find IDs you can paste into other commands (especially `oriro message send --target ...`).
...
```bash
oriro directory peers list --channel slack --query "U0"
oriro message send --channel slack --target user:U012ABCDEF --message "hello"
```
...
```bash
oriro directory peers list --channel zalouser
oriro directory peers list --channel zalouser --query "name"
oriro directory peers list --channel zalouser --limit 50
```

## Groups

```bash
oriro directory groups list --channel zalouser
