...
vi.mock("../infra/path-env.js", () => ({
  ensureOriroCliOnPath: ensurePathMock,
}));
...
describe("runCli exit behavior", () => {
  beforeEach(() => {
    ...
    delete process.env.ORIRO_DISABLE_CLI_STARTUP_HELP_FAST_PATH;
    delete process.env.ORIRO_HIDE_BANNER;
  ...
  it("does not force process.exit after successful routed command", async () => {
    ...
    expect(tryRouteCliMock).toHaveBeenCalledWith(["node", "oriro", "status"]);
  ...
  it("disposes registered harnesses after full CLI command completion", async () => {
    ...
    await runCli(["node", "oriro", "agent", "--local"]);

    expect(parseAsync).toHaveBeenCalledWith(["node", "oriro", "agent", "--local"]);
  ...
  it("shows the standard spinner while loading the full CLI", async () => {
    ...
    await runCli(["node", "oriro", "config"]);
  ...
  it("suppresses startup progress for json output commands before full CLI parsing", async () => {
    ...
    await runCli(["node", "oriro", "sessions", "--json", "--limit", "all"]);
  ...
  it("pauses non-tty stdin after full CLI command completion", async () => {
    ...
    try {
      await runCli(["node", "oriro", "channels"]);

      expect(parseAsync).toHaveBeenCalledWith(["node", "oriro", "channels"]);
  ...
  it("emits the startup banner before gateway foreground fast-path startup", async () => {
    await runCli(["node", "oriro", "gateway", "--force"]);
  ...
  it("rechecks the effective guarded config before automatic startup migrations", async () => {
    ...
    await runCli(["node", "oriro", "gateway"]);
    const hooks = addGatewayRunCommandMock.mock.calls[0]?.[1] as
      | { beforeRun?: (opts: { reset?: boolean }) => Promise<void> }
      | undefined;
    await hooks?.beforeRun?.({});
  ...
  it.each([
    ...
    const previousMarker = process.env.ORIRO_SERVICE_MARKER;
    const previousOverride = process.env.ORIRO_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS;
    if (params.marker) {
      process.env.ORIRO_SERVICE_MARKER = params.marker;
    } else {
      delete process.env.ORIRO_SERVICE_MARKER;
    }
    if (params.override) {
      process.env.ORIRO_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS = params.override;
    } else {
      delete process.env.ORIRO_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS;
    }
    const errorSpy = vi.spyOn(console, "error").mockImplementation(() => {});
    const exitSpy = vi.spyOn(process, "exit").mockImplementation(((code?: number) => {
      throw new Error(`exit:${String(code)}`);
    ...
    try {
      await expect(runCli(["node", "oriro", "gateway", ...params.flags])).rejects.toThrow(
      ...
      if (params.marker) {
        expect(process.env.ORIRO_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS).toBeUndefined();
      }
    } finally {
      exitSpy.mockRestore();
      errorSpy.mockRestore();
      if (previousMarker === undefined) {
        ...
        process.env.ORIRO_SERVICE_MARKER = previousMarker;
      }
      if (previousOverride === undefined) {
        delete process.env.ORIRO_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS;
      } else {
        process.env.ORIRO_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS = previousOverride;
      }
    }
  });
  ...
  it("blocks and revokes the destructive override when selected config declares service mode", async () => {
    readConfigFileSnapshotMock.mockResolvedValue({
      ...
      valid: true,
      sourceConfig: {
        env: { vars: { ORIRO_SERVICE_MARKER: "gateway" } },
    ...
    try {
      await withEnvAsync(
        {
          ORIRO_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS: "1",
          ORIRO_SERVICE_MARKER: undefined,
        },
        async () => {
          await expect(runCli(["node", "oriro", "gateway"])).rejects.toThrow("exit:78");
          expect(process.env.ORIRO_SERVICE_MARKER).toBeUndefined();
          expect(process.env.ORIRO_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS).toBeUndefined();
          expect(ensureCliExecutionBootstrapMock).not.toHaveBeenCalled();
        },
      );
  ...
  it("ignores service mode declared by an invalid selected config", async () => {
    ...
    await withEnvAsync(
      ...
      async () => {
        ...
        expect(process.env.ORIRO_SERVICE_MARKER).toBeUndefined();
        expect(process.env.ORIRO_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS).toBe("1");
      },
    );
  });

  it("guards the config selected by trusted global dotenv before the default config", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-gateway-global-selection-"));
    const stateDir = path.join(homeDir, ".oriro");
    const selectedConfigPath = path.join(stateDir, "selected.json");
    await fs.mkdir(stateDir, { recursive: true });
    await fs.writeFile(
      path.join(stateDir, ".env"),
      [
        `ORIRO_CONFIG_PATH=${selectedConfigPath}`,
        "ORIRO_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1",
        "",
      ].join("\n"),
    );
    try {
      await withEnvAsync(
        {
          HOME: homeDir,
          ORIRO_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS: undefined,
        ...
        async () => {
          readConfigFileSnapshotMock.mockImplementation(async () =>
            process.env.ORIRO_CONFIG_PATH === selectedConfigPath
              ? {
                  exists: true,
                  valid: true,
                  sourceConfig: { gateway: { mode: "local" } },
          ...
          expect(process.env.ORIRO_CONFIG_PATH).toBe(selectedConfigPath);
          expect(process.env.ORIRO_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS).toBeUndefined();
  ...
  it("loads state dotenv before a custom config-root fallback", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-gateway-config-env-"));
    const stateDir = path.join(homeDir, ".oriro");
    const configDir = path.join(homeDir, "profile");
    const configPath = path.join(configDir, "oriro.json");
    await fs.mkdir(stateDir, { recursive: true });
    await fs.mkdir(configDir, { recursive: true });
    await fs.writeFile(path.join(stateDir, ".env"), "ORIRO_GATEWAY_TOKEN=state-token\n");
    await fs.writeFile(
      path.join(configDir, ".env"),
      [
        "ORIRO_GATEWAY_PASSWORD=config-root-password",
        "ORIRO_GATEWAY_TOKEN=config-root-token",
        "",
      ].join("\n"),
    );
    try {
      await withEnvAsync(
        {
          HOME: homeDir,
          ORIRO_CONFIG_PATH: configPath,
          ORIRO_GATEWAY_PASSWORD: undefined,
          ORIRO_GATEWAY_TOKEN: undefined,
          ORIRO_HOME: homeDir,
          ORIRO_STATE_DIR: undefined,
        },
        async () => {
          ...
          expect(process.env.ORIRO_GATEWAY_TOKEN).toBe("state-token");
          expect(process.env.ORIRO_GATEWAY_PASSWORD).toBe("config-root-password");
        },
      );
    } finally {
      await fs.rm(homeDir, { recursive: true, force: true });
  ...
  it("loads and repins a legacy state dotenv after automatic state migration", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-gateway-legacy-env-"));
    const legacyStateDir = path.join(homeDir, ".oriro");
    const newStateDir = path.join(homeDir, ".oriro");
    await fs.mkdir(legacyStateDir, { recursive: true });
    await fs.writeFile(path.join(legacyStateDir, ".env"), "ORIRO_GATEWAY_TOKEN=legacy-token\n");
    try {
      await withEnvAsync(
        {
          HOME: homeDir,
        ...
        async () => {
          ...
          expect(process.env.ORIRO_GATEWAY_TOKEN).toBe("legacy-token");
  ...
  it("blocks a future-config recovery candidate before destructive gateway reset", async () => {
    ...
    try {
      await runCli(["node", "oriro", "gateway", "--dev", "--reset"]);
      const hooks = addGatewayRunCommandMock.mock.calls[0]?.[1] as
        | { beforeRun?: (opts: { reset?: boolean }) => Promise<void> }
        | undefined;
  ...
  it("re-guards config env path selection until the gateway config is stable", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-gateway-selection-"));
    try {
      await withEnvAsync(
        {
          HOME: homeDir,
        ...
        async () => {
          readConfigFileSnapshotMock.mockImplementation(async () => {
            if (process.env.ORIRO_CONFIG_PATH === "/tmp/oriro-chain-c.json") {
              return {
                exists: true,
                valid: true,
                sourceConfig: { meta: { lastTouchedVersion: "9999.1.1" } },
            ...
            if (process.env.ORIRO_STATE_DIR === "/tmp/oriro-chain-b") {
              return {
                exists: true,
                valid: true,
                sourceConfig: {
                  env: { vars: { ORIRO_CONFIG_PATH: "/tmp/oriro-chain-c.json" } },
                  gateway: { mode: "local" },
                },
              };
            }
            return {
              ...
              valid: true,
              sourceConfig: {
                env: { vars: { ORIRO_STATE_DIR: "/tmp/oriro-chain-b" } },
                gateway: { mode: "local" },
              },
            };
          });
          ...
          try {
            await expect(runCli(["node", "oriro", "gateway"])).rejects.toThrow("exit:1");
            expect(errorSpy).toHaveBeenCalledWith(
              expect.stringContaining("run automatic gateway startup migrations"),
            );
            expect(ensureCliExecutionBootstrapMock).not.toHaveBeenCalled();
  ...
  it("drops credentials from configs superseded during state selection", async () => {
    await withEnvAsync(
      ...
      async () => {
        readConfigFileSnapshotMock.mockImplementation(async () =>
          process.env.ORIRO_STATE_DIR === "/tmp/oriro-selected-state"
            ? {
                exists: true,
                valid: true,
                sourceConfig: {
                  env: { vars: { ORIRO_GATEWAY_TOKEN: "selected-token" } },
                  gateway: { mode: "local" },
                },
              }
            : {
                ...
                sourceConfig: {
                  env: {
                    vars: {
                      ...
                      ORIRO_STATE_DIR: "/tmp/oriro-selected-state",
                    },
                  },
                  gateway: { mode: "local" },
                },
        ...
        expect(process.env.ORIRO_STATE_DIR).toBe("/tmp/oriro-selected-state");
        expect(process.env.ORIRO_GATEWAY_TOKEN).toBe("selected-token");
  ...
  it("re-guards config selection from a newly selected state dotenv", async () => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-gateway-dotenv-"));
    const futureConfigPath = path.join(stateDir, "future.json");
    await fs.writeFile(
      path.join(stateDir, ".env"),
      [
        `ORIRO_CONFIG_PATH=${futureConfigPath}`,
        "ORIRO_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1",
        "",
      ].join("\n"),
    );
    try {
      await withEnvAsync(
        ...
        async () => {
          readConfigFileSnapshotMock.mockImplementation(async () => {
            if (process.env.ORIRO_CONFIG_PATH === futureConfigPath) {
              return {
                exists: true,
                valid: true,
                sourceConfig: { meta: { lastTouchedVersion: "9999.1.1" } },
            ...
            return {
              ...
              valid: true,
              sourceConfig: {
                env: { vars: { ORIRO_STATE_DIR: stateDir } },
                gateway: { mode: "local" },
              },
            };
          });
  ...
  it("re-inspects recovery after recovery changes config selection", async () => {
    await withEnvAsync({ ORIRO_CONFIG_PATH: undefined }, async () => {
      const selectedConfigPath = "/tmp/oriro-recovered-selection.json";
      const currentSnapshot = {
        exists: true,
        valid: true,
        sourceConfig: { gateway: { mode: "local" } },
      ...
      readConfigFileSnapshotMock.mockImplementation(async (options) => {
        ...
        if (recoveryReads === 1) {
          const recoveredSnapshot = {
            ...
            valid: true,
            sourceConfig: {
              env: { vars: { ORIRO_CONFIG_PATH: selectedConfigPath } },
              gateway: { mode: "local" },
            },
          };
          await options.allowSuspiciousRecovery?.(
  ...
  it("discards env from a config replaced by suspicious recovery", async () => {
    await withEnvAsync(
      { ORIRO_GATEWAY_TOKEN: undefined, ORIRO_PROXY_ACTIVE: undefined },
      async () => {
        const clobberedSnapshot = {
          exists: true,
          valid: true,
          sourceConfig: {
            env: { vars: { ORIRO_GATEWAY_TOKEN: "discarded-token" } },
            gateway: { mode: "local" },
          },
          hash: "clobbered",
          path: "/tmp/oriro.json",
        ...
        process.env.ORIRO_PROXY_ACTIVE = "1";
        const hooks = addGatewayRunCommandMock.mock.calls[0]?.[1] as
          | { beforeRun?: (opts: { force?: boolean }) => Promise<void> }
          | undefined;
        await hooks?.beforeRun?.({});

        expect(process.env.ORIRO_GATEWAY_TOKEN).toBeUndefined();
        expect(process.env.ORIRO_PROXY_ACTIVE).toBe("1");
        expect(ensureCliExecutionBootstrapMock).toHaveBeenCalledOnce();
      },
    );
  });
  ...
  it("does not apply environment variables from invalid config snapshots", async () => {
    await withEnvAsync({ ORIRO_INCLUDE_ROOTS: undefined }, async () => {
      readConfigFileSnapshotMock.mockResolvedValue({
        exists: true,
        valid: false,
        sourceConfig: {
          env: { vars: { ORIRO_INCLUDE_ROOTS: "/tmp/oriro-includes" } },
          gateway: { mode: "local" },
        },
      });
      ...
      expect(process.env.ORIRO_INCLUDE_ROOTS).toBeUndefined();
      expect(readConfigFileSnapshotMock.mock.calls).toEqual([
        [{ isolateEnv: true, observe: false }],
        [{ isolateEnv: true, observe: false }],
      ]);
  ...
  it("loads selected state dotenv before config env and environment normalization", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-gateway-selected-env-"));
        ...
        async () => {
          readConfigFileSnapshotMock.mockResolvedValue({
            ...
            sourceConfig: {
              env: {
                vars: {
                  ORIRO_GATEWAY_TOKEN: "config-token",
          ...
          normalizeEnvMock.mockImplementation(() => {
            tokenAtNormalize = process.env.ORIRO_GATEWAY_TOKEN;
          });
  ...
  it("drops credentials from a trusted dotenv superseded by state selection", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-gateway-dotenv-hop-"));
    const defaultStateDir = path.join(homeDir, ".oriro");
    ...
    try {
      await withEnvAsync(
        ...
        async () => {
          ...
          expect(process.env.ORIRO_STATE_DIR).toBe(selectedStateDir);
          expect(process.env.ORIRO_GATEWAY_TOKEN).toBe("selected-token");
        },
      );
    } finally {
  ...
  it("drops gateway.env selectors when the default state dotenv selects a custom state", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-gateway-fallback-hop-"));
    const defaultStateDir = path.join(homeDir, ".oriro");
    const selectedStateDir = path.join(homeDir, "selected-state");
    const gatewayEnvDir = path.join(homeDir, ".config", "oriro");
    await fs.mkdir(defaultStateDir, { recursive: true });
    await fs.mkdir(selectedStateDir, { recursive: true });
    ...
    await fs.writeFile(
      ...
      [
        "ORIRO_GATEWAY_TOKEN=selected-token",
        "ORIRO_INCLUDE_ROOTS=/tmp/untrusted-include-root",
    ...
    try {
      await withEnvAsync(
        ...
        async () => {
          ...
          expect(process.env.ORIRO_STATE_DIR).toBe(selectedStateDir);
          expect(process.env.ORIRO_CONFIG_PATH).toBeUndefined();
          expect(process.env.ORIRO_GATEWAY_TOKEN).toBe("selected-token");
        },
      );
    } finally {
  ...
  it("preserves gateway.env selectors when the compatibility fallback selects the target", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-gateway-fallback-select-"));
    const selectedStateDir = path.join(homeDir, "selected-state");
    const gatewayEnvDir = path.join(homeDir, ".config", "oriro");
    await fs.mkdir(selectedStateDir, { recursive: true });
    await fs.mkdir(gatewayEnvDir, { recursive: true });
  ...
  it("drops old state dotenv credentials when config selects another state", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-gateway-config-state-hop-"));
    const defaultStateDir = path.join(homeDir, ".oriro");
    const selectedStateDir = path.join(homeDir, "selected-state");
    await fs.mkdir(defaultStateDir, { recursive: true });
    await fs.mkdir(selectedStateDir, { recursive: true });
      ...
      await withEnvAsync(
        ...
        async () => {
          readConfigFileSnapshotMock.mockImplementation(async () => ({
            ...
            valid: true,
            sourceConfig:
              process.env.ORIRO_STATE_DIR === selectedStateDir
                ? { gateway: { mode: "local" } }
                : {
                    env: { vars: { ORIRO_STATE_DIR: selectedStateDir } },
                    gateway: { mode: "local" },
                  },
          }));
  ...
  it("drops early target credentials when a later guard selects another state", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-gateway-late-state-hop-"));
    const defaultStateDir = path.join(homeDir, ".oriro");
    const selectedStateDir = path.join(homeDir, "selected-state");
    await fs.mkdir(defaultStateDir, { recursive: true });
    await fs.mkdir(selectedStateDir, { recursive: true });
      ...
      await withEnvAsync(
        ...
        async () => {
          ...
          readConfigFileSnapshotMock.mockImplementation(async () => ({
            ...
            valid: true,
            sourceConfig:
              selectLateState && process.env.ORIRO_STATE_DIR !== selectedStateDir
                ? {
                    env: { vars: { ORIRO_STATE_DIR: selectedStateDir } },
                    gateway: { mode: "local" },
                  }
          ...
          await runCli(["node", "oriro", "gateway"]);
          expect(process.env.ORIRO_GATEWAY_TOKEN).toBe("early-token");
  ...
  it("does not let gateway.env authorize automatic mutations of a selected future config", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-gateway-global-env-"));
    const gatewayEnvDir = path.join(homeDir, ".config", "oriro");
    ...
    try {
      await withEnvAsync(
        ...
        async () => {
          readConfigFileSnapshotMock.mockImplementation(async () =>
            process.env.ORIRO_CONFIG_PATH === futureConfigPath
              ? {
                  exists: true,
                  valid: true,
                  sourceConfig: { meta: { lastTouchedVersion: "9999.1.1" } },
              ...
              : {
                  ...
                  valid: true,
                  sourceConfig: {
                    env: { vars: { ORIRO_CONFIG_PATH: futureConfigPath } },
                    gateway: { mode: "local" },
                  },
                },
          );
  ...
  it("does not treat gateway option values as bootstrap command paths", async () => {
    await runCli(["node", "oriro", "gateway", "--raw-stream-path", "status"]);
  ...
  it("retains selected config paths and invocation reset targets", async () => {
    await withEnvAsync(
      {
        ORIRO_CONFIG_PATH: "/tmp/oriro-invocation/oriro.json",
        ORIRO_GATEWAY_TOKEN: undefined,
        ORIRO_HOME: "/tmp/oriro-invocation-home",
        ORIRO_INCLUDE_ROOTS: undefined,
        ORIRO_PROFILE: undefined,
        ORIRO_STATE_DIR: "/tmp/oriro-invocation-state",
        ORIRO_TEST_FAST: "1",
        ORIRO_WORKSPACE_DIR: "/tmp/oriro-invocation-workspace",
      },
      async () => {
        readConfigFileSnapshotMock.mockResolvedValue({
          exists: true,
          ...
          sourceConfig: {
            env: {
              vars: {
                ORIRO_CONFIG_PATH: "/tmp/oriro-reset/oriro.json",
                ORIRO_GATEWAY_TOKEN: "old-token",
                ORIRO_HOME: "/tmp/oriro-reset-home",
                ORIRO_INCLUDE_ROOTS: "/tmp/oriro-reset-includes",
                ORIRO_PROFILE: "config-dev",
                ORIRO_STATE_DIR: "/tmp/oriro-reset",
                ORIRO_TEST_FAST: "0",
                ORIRO_WORKSPACE_DIR: "/tmp/oriro-reset-workspace",
              },
            },
            gateway: { mode: "local" },
          },
        ...
        expect(process.env.ORIRO_CONFIG_PATH).toBe("/tmp/oriro-invocation/oriro.json");
        expect(process.env.ORIRO_HOME).toBe("/tmp/oriro-invocation-home");
        expect(process.env.ORIRO_PROFILE).toBeUndefined();
        expect(process.env.ORIRO_STATE_DIR).toBe("/tmp/oriro-invocation-state");
        expect(process.env.ORIRO_TEST_FAST).toBe("1");
        expect(process.env.ORIRO_WORKSPACE_DIR).toBe("/tmp/oriro-invocation-workspace");
        expect(process.env.ORIRO_GATEWAY_TOKEN).toBeUndefined();
        expect(process.env.ORIRO_INCLUDE_ROOTS).toBeUndefined();
        expect(ensureCliExecutionBootstrapMock).not.toHaveBeenCalled();
      },
  ...
  it("honors banner suppression on the gateway foreground fast path", async () => {
    process.env.ORIRO_HIDE_BANNER = "1";

    await runCli(["node", "oriro", "gateway"]);

    expect(tryRouteCliMock).not.toHaveBeenCalled();
    expect(emitCliBannerMock).not.toHaveBeenCalled();
    expect(commanderParseAsyncMock).toHaveBeenCalledWith(["node", "oriro", "gateway"]);
  });
  ...
  it("renders secrets help from startup metadata without building the full program", async () => {
    ...
    await runCli(["node", "oriro", "secrets", "--help"]);
  ...
  it("renders nodes help from startup metadata without building the full program", async () => {
    ...
    await runCli(["node", "oriro", "nodes", "--help"]);
  ...
  it("defers nodes help startup metadata when plugin config can change command metadata", async () => {
    const argv = ["node", "oriro", "nodes", "--help"];
    const parseAsync = vi.fn().mockResolvedValueOnce(undefined);
    const program = {
  ...
  it("renders selected subcommand help from startup metadata without building the full program", async () => {
    ...
    await runCli(["node", "oriro", "doctor", "--help"]);
  ...
  it("renders setup/onboard/configure help without building the full program", async () => {
    await runCli(["node", "oriro", "setup", "--help"]);
  ...
  it("renders root help without building the full program", async () => {
    ...
    expect(maybeRunCliInContainerMock).toHaveBeenCalledWith(["node", "oriro", "--help"]);
    expect(tryRouteCliMock).not.toHaveBeenCalled();
    expect(loadRootHelpRenderOptionsForConfigSensitivePluginsMock).toHaveBeenCalledTimes(1);
    expect(outputPrecomputedRootHelpTextMock).toHaveBeenCalledTimes(1);
  ...
  it.each([
    ["gateway runtime", ["node", "oriro", "gateway", "run"]],
    ["bare gateway runtime", ["node", "oriro", "gateway"]],
    ["node runtime", ["node", "oriro", "node", "run"]],
    ["local agent runtime", ["node", "oriro", "agent", "--local"]],
    ["provider inference", ["node", "oriro", "infer", "web", "fetch", "https://example.com"]],
    ["model command", ["node", "oriro", "models", "auth", "login", "openai"]],
    ["plugin command", ["node", "oriro", "plugins", "marketplace", "list"]],
    ["skill command", ["node", "oriro", "skills", "search", "browser"]],
    ["update command", ["node", "oriro", "update", "check"]],
    ["channel probe", ["node", "oriro", "channels", "status", "--probe"]],
    ["channel capabilities probe", ["node", "oriro", "channels", "capabilities"]],
    ["directory plugin command", ["node", "oriro", "directory", "peers", "list"]],
    ["message plugin command", ["node", "oriro", "message", "send", "--to", "demo"]],
    ["metadata-owned plugin command", ["node", "oriro", "googlemeet", "login"]],
  ])("starts managed proxy routing for %s", (_name, argv) => {
    expect(shouldStartProxyForCli(argv)).toBe(true);
  });

  it.each([
    ["root help", ["node", "oriro", "--help"]],
    ["root version", ["node", "oriro", "--version"]],
    ["gateway help", ["node", "oriro", "gateway", "--help"]],
    ["gateway run help", ["node", "oriro", "gateway", "run", "--help"]],
    ["status", ["node", "oriro", "status"]],
    ["health", ["node", "oriro", "health"]],
    ["gateway status", ["node", "oriro", "gateway", "status"]],
    ["gateway health", ["node", "oriro", "gateway", "health"]],
    ["remote agent control-plane", ["node", "oriro", "agent", "run"]],
    ["chat control-plane", ["node", "oriro", "chat"]],
    ["terminal control-plane", ["node", "oriro", "terminal"]],
    ["config", ["node", "oriro", "config", "get", "proxy.enabled"]],
    ["channels parent help", ["node", "oriro", "channels"]],
    ["completion", ["node", "oriro", "completion", "zsh"]],
    ["debug proxy cli", ["node", "oriro", "proxy", "start"]],
    ["agents list", ["node", "oriro", "agents", "list"]],
    ["models list", ["node", "oriro", "models", "list"]],
    ["models status without live probe", ["node", "oriro", "models", "status"]],
    ["skills check", ["node", "oriro", "skills", "check"]],
    ["skills info", ["node", "oriro", "skills", "info", "weather"]],
    ["skills list", ["node", "oriro", "skills", "list"]],
    ["tasks list", ["node", "oriro", "tasks", "list"]],
    ["legacy singular tool namespace", ["node", "oriro", "tool", "image_generate"]],
    ["gateway tools namespace typo", ["node", "oriro", "tools", "effective"]],
    ["migrate", ["node", "oriro", "migrate"]],
  ...
  it("starts the managed proxy for network-capable commands by default", async () => {
    ...
    await runCli(["node", "oriro", "plugins", "marketplace", "list"]);

    expect(startProxyMock).toHaveBeenCalledWith(undefined);
  });

  it.each([
    ["fast path", ["node", "oriro", "gateway", "run"]],
    [
      "full Commander path with root options",
      ["node", "oriro", "--log-level", "debug", "gateway", "run"],
    ],
  ...
  it("validates the runtime before selecting gateway config", async () => {
    await runCli(["node", "oriro", "gateway", "run"]);
  ...
  it("selects gateway config env before starting its managed proxy", async () => {
    await withEnvAsync({ ORIRO_TEST_PROXY_SELECTION: undefined }, async () => {
      readConfigFileSnapshotMock.mockResolvedValue({
        exists: true,
        valid: true,
        sourceConfig: {
          env: { vars: { ORIRO_TEST_PROXY_SELECTION: "selected" } },
          gateway: { mode: "local" },
        },
      });
      loadConfigMock.mockImplementationOnce(() => ({
        proxy: { selected: process.env.ORIRO_TEST_PROXY_SELECTION },
      }));
  ...
  it("starts the managed proxy for metadata-owned plugin commands by default", async () => {
    ...
    await runCli(["node", "oriro", "googlemeet", "login"]);

    expect(startProxyMock).toHaveBeenCalledWith(undefined);
  });

  it("rejects unowned command roots before proxy and plugin runtime registration", async () => {
    await expect(runCli(["node", "oriro", "foo"])).rejects.toThrow(
  ...
  it("suggests close known commands for unowned command roots before proxy startup", async () => {
    await expect(runCli(["node", "oriro", "upate"])).rejects.toThrow(
      "Did you mean this?\n  oriro update",
    );
  ...
  it("rejects unowned command roots even when --help is appended (regression for #81077)", async () => {
    await expect(runCli(["node", "oriro", "foo", "--help"])).rejects.toThrow(
      'No built-in command or plugin CLI metadata owns "foo"',
    );
  ...
  it("rejects unowned command roots even when --version is appended", async () => {
    await expect(runCli(["node", "oriro", "foo", "--version"])).rejects.toThrow(
      'No built-in command or plugin CLI metadata owns "foo"',
    );
  ...
  it("does not suggest plugins.allow for unknown command roots before proxy startup", async () => {
    ...
    let error: unknown;
    try {
      await runCli(["node", "oriro", "totally-unknown"]);
    } catch (caught) {
      error = caught;
    }
    expect(error).toBeInstanceOf(Error);
  ...
  it("preserves plugins.allow diagnostics for roots owned only by CLI metadata", async () => {
    ...
    await expect(runCli(["node", "oriro", "qa"])).rejects.toThrow(
  ...
  it("reports plugin tool command mistakes before proxy startup", async () => {
    ...
    await expect(runCli(["node", "oriro", "lcm_recent"])).rejects.toThrow(
      '"lcm_recent" is an agent tool available from the "lossless-oriro" plugin',
    );
  ...
  it("does not install the env proxy dispatcher for bypassed skills inspection commands", async () => {
    ...
    await runCli(["node", "oriro", "skills", "check"]);
  ...
  it.each([
    ["auth", ["node", "oriro", "auth", "--help"]],
    ["tool", ["node", "oriro", "tool", "image_generate"]],
    ["tools", ["node", "oriro", "tools", "effective"]],
  ...
  it("routes lazy plugin registration logs to stderr only during --json registration", async () => {
    ...
    await runCli(["node", "oriro", "memory", "search", "query", "--json"]);
  ...
  it("does not route lazy plugin registration logs for pass-through --json after terminator", async () => {
    ...
    await runCli(["node", "oriro", "memory", "--", "--json"]);
  ...
  it("fails protected commands when managed proxy activation fails", async () => {
    ...
    await expect(runCli(["node", "oriro", "gateway", "run"])).rejects.toThrow(
  ...
  it("stops the managed proxy and exits after SIGINT", async () => {
    ...
    try {
      const runPromise = runCli(["node", "oriro", "plugins", "marketplace", "list"]);
      await vi.waitFor(() => {
        expect(
  ...
  it("starts onboarding for bare root invocations when config only has metadata", async () => {
    readConfigFileSnapshotMock.mockResolvedValueOnce({
      ...
      valid: true,
      sourceConfig: {
        $schema: "https://oriro.ai/config.json",
  ...
  it("points noninteractive fresh bare root invocations to onboarding automation", async () => {
    ...
    try {
      ...
      expect(errorSpy).toHaveBeenCalledWith(
        "Onboarding needs an interactive TTY. Use `oriro onboard --non-interactive --accept-risk ...` for automation.",
      );
  ...
  it("bootstraps env proxy before modern onboard Crestodian startup", async () => {
    ...
    await runCli(["node", "oriro", "onboard", "--modern", "--json"]);
  ...
  it("does not fail the command when memory cleanup is unavailable", async () => {
    ...
    await expect(runCli(["node", "oriro", "status"])).resolves.toBeUndefined();
  ...
  it("returns after a handled container-target invocation", async () => {
    ...
    await runCli(["node", "oriro", "--container", "demo", "status"]);
  ...
  it("swallows Commander parse exits after recording the exit code", async () => {
    ...
    expect(registerSubCliByNameMock.mock.calls).toEqual([
      [program, "status", ["node", "oriro", "status"]],
    ]);
    expect(process.exitCode).toBe(1);
    process.exitCode = exitCode;
  });
  ...
  it("loads the real primary command before rendering command help", async () => {
    ...
    expect(registerCoreCliByNameMock.mock.calls).toEqual([
      [program, ctx, "doctor", ["node", "oriro", "doctor", "--help"]],
    ]);
    expect(registerSubCliByNameMock.mock.calls).toEqual([
      [program, "doctor", ["node", "oriro", "doctor", "--help"]],
    ]);
  });
  ...
  it("does not exit for transient uncaught CLI exceptions", async () => {
    ...
    try {
      ...
      expect(consoleWarnSpy.mock.calls).toEqual([
        ["[oriro] Non-fatal uncaught exception (continuing):", hostUnreachable.stack],
      ]);
