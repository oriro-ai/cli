...
vi.mock("./channel-bridge.js", () => ({
  OriroChannelBridge: class MockOriroChannelBridge {
...
describe("serveOriroChannelMcp shutdown", () => {
  ...
  it("does not leak unhandled rejections when shutdown close fails", async () => {
    ...
    const { serveOriroChannelMcp } = await import("./channel-server.js");

    const servePromise = serveOriroChannelMcp({ verbose: false });
