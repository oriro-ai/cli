...
export function printDaemonStatus(status: DaemonStatus, opts: { json: boolean; deep?: boolean }) {
  ...
  if (service.configAudit?.issues.length) {
    ...
    defaultRuntime.error(
      warnText(
        `Recommendation: run "${formatCliCommand("oriro doctor")}" (or "${formatCliCommand("oriro doctor --repair")}").`,
      ),
    );
  }
  ...
  if (status.config) {
    ...
    if (status.config.mismatch) {
      ...
      defaultRuntime.error(
        errorText(
          `Fix: rerun \`${formatCliCommand("oriro gateway install --force")}\` from the same --profile / ORIRO_STATE_DIR you expect.`,
        ),
      );
    }
    spacer();
  ...
  if (
    ...
    defaultRuntime.error(
      errorText(
        `Fix: run ${formatCliCommand("oriro gateway restart")} and re-check with ${formatCliCommand("oriro gateway status --deep")}.`,
      ),
    );
    spacer();
  }
  ...
  if (service.runtime?.cachedLabel) {
    ...
    const labelValue = resolveGatewayLaunchAgentLabel(env.ORIRO_PROFILE);
    defaultRuntime.error(
      errorText(
    ...
    defaultRuntime.error(
      errorText(`Then reinstall: ${formatCliCommand("oriro gateway install")}`),
    );
    spacer();
  }
  ...
  if (service.staleUpdateLaunchdJobs?.length) {
    ...
    defaultRuntime.error(
      errorText(
        `Fix after confirming no update is running: launchctl remove <label>, then run ${formatCliCommand("oriro gateway restart")}.`,
      ),
    );
    spacer();
  }
  ...
  if (
    ...
    if (process.platform === "linux") {
      const unit = resolveGatewaySystemdServiceName(serviceEnv.ORIRO_PROFILE);
      defaultRuntime.error(
  ...
  if (drift && drift.drifts.length > 0) {
    ...
    if (opts.deep) {
      ...
      defaultRuntime.log(
        `${label("Fix:")} ${formatCliCommand("oriro plugins update <plugin-id>")} for each drifted plugin, then ${formatCliCommand("oriro gateway restart")}.`,
      );
    } else {
      defaultRuntime.log(
        infoText(
          `Run ${formatCliCommand("oriro gateway status --deep")} for affected plugin ids and fix commands.`,
        ),
      );
    }
    spacer();
  ...
  defaultRuntime.log(`${label("Troubles:")} run ${formatCliCommand("oriro status")}`);
  defaultRuntime.log(`${label("Troubleshooting:")} https://docs.oriro.ai/troubleshooting`);
}
