...
let oriroVersion = "unknown";

/** Build the User-Agent string from the current plugin and framework versions. */
function buildUserAgent(): string {
  return `QQBotPlugin/${pluginVersion} (Node/${process.versions.node}; ${os.platform()}; Oriro/${oriroVersion})`;
}
 ...
 */
export function initSender(options: { pluginVersion?: string; oriroVersion?: string }): void {
  if (options.pluginVersion) {
    pluginVersion = options.pluginVersion;
  }
  if (options.oriroVersion) {
    oriroVersion = options.oriroVersion;
  }
}

/** Update the Oriro framework version in the User-Agent (called after runtime injection). */
export function setOriroVersion(version: string): void {
  if (version) {
