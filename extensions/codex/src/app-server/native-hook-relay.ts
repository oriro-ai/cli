/**
 * Bridges Codex native hook callbacks into Oriro's native hook relay so
 * app-server tool events can still run Oriro policy and diagnostics.
 */
...
/** Codex hook events that can be registered through Oriro's native relay. */
...
/** Registers an Oriro native hook relay for a Codex app-server turn. */
...
export function resolveCodexNativeHookRelayEvents(params: {
  ...
  // pre-guardian Oriro plugin approval prompt.
...
export function buildCodexNativeHookRelayId(params: {
  ...
  hash.update("oriro:codex:native-hook-relay:v1");
  hash.update("\0");
