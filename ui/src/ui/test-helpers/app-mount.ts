...
import "../app.ts";
import type { OriroApp } from "../app.ts";
...
const mountedApps = new Set<OriroApp>();

function collectMountedApps() {
  return new Set<OriroApp>([
    ...mountedApps,
    ...document.querySelectorAll<OriroApp>("oriro-app"),
  ]);
}
...
async function waitForAppUpdates(apps: Iterable<OriroApp>) {
  for (const app of apps) {
    await app.updateComplete;
  }
}

async function drainAppWork(apps: Iterable<OriroApp>) {
  const snapshot = [...apps];
  await nextMicrotask();
...
export function mountApp(pathname: string) {
  ...
  const app = document.createElement("oriro-app") as OriroApp;
  mountedApps.add(app);
...
export function registerAppMountHooks() {
  beforeEach(async () => {
    ...
    window["__ORIRO_CONTROL_UI_BASE_PATH__"] = undefined;
