...
enum GatewayDiscoverySelectionSupport {
        ...
        state: AppState)
    {
        ...
        if preferredTransport == .direct {
            OriroConfigFile.setRemoteGatewayTransport(AppState.RemoteTransport.direct.rawValue)
            if !state.remoteUrl.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty {
                OriroConfigFile.setRemoteGatewayUrlString(state.remoteUrl)
            } else {
                OriroConfigFile.clearRemoteGatewayUrl()
            }
        } else {
            OriroConfigFile.setRemoteGatewayTransport(AppState.RemoteTransport.ssh.rawValue)
            OriroConfigFile.setRemoteGatewayUrlString(state.remoteUrl)
        }
    }
