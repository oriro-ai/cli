...
function shouldRequireBrokeredAws(commandArgs, providerName) {
  if (process.env.ORIRO_CRABBOX_ALLOW_DIRECT_AWS === "1") {
    return false;
  }
...
function remoteGitBootstrapForChangedGate(changedGateBase) {
  ...
  return [
    "oriro_changed_gate_base=${ORIRO_CHANGED_GATE_BASE:-" + quotedBase + "};",
    'if ! command -v git >/dev/null 2>&1; then echo "git is required for Oriro remote changed-gate sync" >&2; exit 2; fi;',
    ...
    "git add -A;",
    "if ! git diff --cached --quiet; then git -c user.name=Oriro -c user.email=ci@oriro.local commit -q --no-gpg-sign -m remote-changed-gate-tree; fi;",
    "fi",
  ].join(" ");
}
...
function remoteWindowsHydratedNodeModulesBootstrap() {
  return [
    "$oriroModulesDir = $env:PNPM_CONFIG_MODULES_DIR",
    "if ($oriroModulesDir) {",
    'if (-not (Test-Path $oriroModulesDir)) { throw "PNPM_CONFIG_MODULES_DIR does not exist: $oriroModulesDir" }',
    '$oriroWorkspaceModules = Join-Path (Get-Location).Path "node_modules"',
    '$oriroSelfModules = Join-Path $oriroModulesDir "node_modules"',
    'if (-not (Test-Path $oriroSelfModules)) { cmd /c mklink /J "$oriroSelfModules" "$oriroModulesDir" | Out-Host; if ($LASTEXITCODE -ne 0) { throw "failed to link hydrated pnpm node_modules" } }',
    'if (-not (Test-Path $oriroWorkspaceModules)) { cmd /c mklink /J "$oriroWorkspaceModules" "$oriroModulesDir" | Out-Host; if ($LASTEXITCODE -ne 0) { throw "failed to link workspace node_modules" } }',
    "}",
  ].join("; ");
}
...
function remoteAwsMacosJsBootstrap({ packageManager = false, bun = false } = {}) {
  const nodeVersion = process.env.ORIRO_CRABBOX_MACOS_NODE_VERSION?.trim() || "24.15.0";
  const bootstrap = [
    "oriro_crabbox_bootstrap_macos_js() {",
    'tool_root="${ORIRO_CRABBOX_MACOS_TOOLCHAIN_DIR:-$HOME/.oriro-crabbox-toolchain}";',
    ...
    'macos_locale="${ORIRO_CRABBOX_MACOS_LOCALE:-en_US.UTF-8}";',
    ...
    'ready_marker="$node_dir/.oriro-crabbox-node-ready";',
    ...
    "oriro_env_args=();",
    "oriro_env_ignore=0;",
    "oriro_env_path_seen=0;",
    'while [ "$#" -gt 0 ]; do',
    'case "$1" in',
    '-i|--ignore-environment) oriro_env_ignore=1; oriro_env_args+=("$1"); shift ;;',
    '-S|--split-string|-S*|--split-string=*) command env "${oriro_env_args[@]}" "$@"; return ;;',
    '-[!-]*i*) oriro_env_ignore=1; oriro_env_args+=("$1"); shift ;;',
    '-u|--unset|-C|--chdir) oriro_env_args+=("$1"); shift; if [ "$#" -gt 0 ]; then oriro_env_args+=("$1"); shift; fi ;;',
    '--unset=*|--chdir=*) oriro_env_args+=("$1"); shift ;;',
    'PATH=*) if [ "$oriro_env_ignore" = "1" ]; then oriro_env_args+=("PATH=$PATH:${1#PATH=}"); else oriro_env_args+=("$1"); fi; oriro_env_path_seen=1; shift ;;',
    '[A-Za-z_]*=*) oriro_env_args+=("$1"); shift ;;',
    '--) oriro_env_args+=("--"); shift; break ;;',
    "*) break ;;",
    "esac;",
    "done;",
    'if [ "$oriro_env_ignore" = "1" ] && [ "$oriro_env_path_seen" = "0" ]; then oriro_env_args+=("PATH=$PATH"); fi;',
    'command env "${oriro_env_args[@]}" "$@";',
    "};",
  ];
  if (packageManager) {
    bootstrap.push(
  ...
  if (bun) {
    bootstrap.push(
      ...
      'bun_ready_marker="$bun_root/.oriro-crabbox-bun-ready";',
  ...
  bootstrap.push("};", "oriro_crabbox_bootstrap_macos_js");
...
function scopedAwsMacosEnvCommand(commandArgs) {
  ...
  return {
    ...
    shellCommand: `oriro_crabbox_env ${shellJoin(commandArgs.slice(1))}`,
  };
}
...
function prepareAwsMacosScriptStdinBootstrap(commandArgs, providerName) {
  ...
  const scriptRoot = mkdtempSync(resolve(tmpdir(), "oriro-crabbox-macos-script-"));
...
function createAwsMacosScriptStdinWrapper(script) {
  ...
  return [
    ...
    'tmp_script="$(mktemp "${TMPDIR:-/tmp}/oriro-crabbox-script.XXXXXX")" || exit $?',
    'cleanup_oriro_crabbox_script() { rm -f "$tmp_script"; }',
    "trap cleanup_oriro_crabbox_script EXIT",
...
function uniqueHereDocDelimiter(script) {
  let index = 0;
  for (;;) {
    const delimiterLocal = `ORIRO_CRABBOX_SCRIPT_${index}`;
...
function defaultFullCheckoutSyncRoot() {
  const home = homedir();
  if (home) {
    return resolve(home, ".cache", "oriro", "crabbox-sync");
  }
  return resolve(tmpdir(), "oriro-crabbox-sync");
}

function fullCheckoutSyncRoot() {
  const configured = process.env.ORIRO_CRABBOX_SYNC_TMPDIR?.trim();
...
function assertFullCheckoutSyncDisk(root) {
  const requiredBytes = parseNonNegativeIntegerEnv(
    "ORIRO_CRABBOX_SYNC_MIN_FREE_BYTES",
    1024 * 1024 * 1024,
    "byte count",
  );
  if (requiredBytes === 0) {
  ...
  throw new Error(
    [
      ...
      "set ORIRO_CRABBOX_SYNC_TMPDIR to a roomier filesystem or lower ORIRO_CRABBOX_SYNC_MIN_FREE_BYTES if you know this checkout fits",
    ].join("; "),
  );
}
...
function prepareFullCheckoutForSync(options = {}) {
  ...
  const dir = mkdtempSync(resolve(syncRoot, "oriro-crabbox-sync-"));
  let active = false;
...
function fullCheckoutKeepaliveIntervalMs() {
  return parseNonNegativeIntegerEnv(
    "ORIRO_CRABBOX_SYNC_KEEPALIVE_MS",
    5000,
  ...
  if (!satisfiesMinimumCrabboxVersion(version.text, minimumBlacksmithCrabboxVersion)) {
    console.error(
      [
        ...
        "[crabbox] if using ../crabbox, rebuild it: version=$(git -C ../crabbox describe --tags --always --dirty | sed 's/^v//') && go build -C ../crabbox -trimpath -ldflags \"-s -w -X github.com/oriro/crabbox/internal/cli.version=${version}\" -o bin/crabbox ./cmd/crabbox",
      ].join("\n"),
    );
    process.exit(2);
  }
...
if (
  ...
  if (isAwsMacosRemoteTarget(normalizedArgs, provider)) {
    ...
    console.error(
      `[crabbox] warning: provider=aws raw boxes may lack Node/Corepack/pnpm/Bun for ${runtimeEntrypoint}; hydrate first (${hydrate}) or pass --provider blacksmith-testbox for Oriro CI-like proof; not switching providers automatically`,
    );
  }
}
...
if (
  ...
  console.error(
    "[crabbox] provider=docker enabling host Docker socket pass-through for Oriro Docker tests",
  );
}
if (
  isLocalContainerProvider(provider) &&
  ...
  childEnv.CRABBOX_LOCAL_CONTAINER_WORK_ROOT = "/tmp/oriro-crabbox-docker-work";
  console.error(
