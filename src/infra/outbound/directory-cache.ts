...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: OriroConfig | null = null;
   ...
   */
  get(key: string, cfg: OriroConfig): T | undefined {
   ...
   */
  set(key: string, value: T, cfg: OriroConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: OriroConfig): void {
