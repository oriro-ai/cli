...
# build the runner/functional images with the prepared Oriro package tarball.
...
docker_e2e_resolve_image() {
  ...
  if [ -n "${ORIRO_DOCKER_E2E_IMAGE:-}" ]; then
    printf '%s\n' "$ORIRO_DOCKER_E2E_IMAGE"
    return 0
  fi
...
docker_e2e_build_or_reuse() {
  ...
  if [ "${ORIRO_SKIP_DOCKER_BUILD:-0}" = "1" ] || [ "$skip_build" = "1" ]; then
    ...
    if ! docker_e2e_docker_cmd image inspect "$image_name" >/dev/null 2>&1; then
      ...
      if docker_build_on_missing_enabled; then
        echo "Docker image not available; building because ORIRO_DOCKER_BUILD_ON_MISSING/ORIRO_TESTBOX allows fallback."
      else
  ...
  if [ "$target" = "functional" ]; then
    ...
    if [ -z "${ORIRO_CURRENT_PACKAGE_TGZ:-}" ]; then
    ...
    build_args+=(--build-context "oriro_package=$package_context")
  fi
...
docker_e2e_test_state_function_b64() {
  node "$ROOT_DIR/scripts/lib/oriro-test-state.mjs" shell-function |
    base64 |
    tr -d '\n'
}
