...
// Translates OpenAI chat requests to Oriro agent runs and SSE/JSON responses.
...
function resolveAgentResponseText(result: unknown): string {
  ...
  if (!Array.isArray(payloads) || payloads.length === 0) {
    return "No response from Oriro.";
  }
  const content = payloads
    .map((p) => (typeof p.text === "string" ? p.text : ""))
    .filter(Boolean)
    .join("\n\n");
  return content || "No response from Oriro.";
}
...
export async function handleOpenAiHttpRequest(
  ...
  const model = typeof payload.model === "string" ? payload.model : "oriro";
