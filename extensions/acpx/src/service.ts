...
import { finiteSecondsToTimerSafeMilliseconds } from "oriro/plugin-sdk/number-runtime";
import type {
...
const ENABLE_STARTUP_PROBE_ENV = "ORIRO_ACPX_RUNTIME_STARTUP_PROBE";
const SKIP_RUNTIME_PROBE_ENV = "ORIRO_SKIP_ACPX_RUNTIME_PROBE";
  ...
  async function resolveRuntime(): Promise<AcpxRuntimeLike> {
    ...
    runtimePromise ??= loadRuntimeModule().then((module) => {
      runtime = new module.AcpxRuntime({
        ...
        oriroGatewayInstanceId: params.gatewayInstanceId,
        oriroProcessLeaseStore: params.processLeaseStore,
        oriroWrapperRoot: params.wrapperRoot,
...
function resolveAllowedAgentsProbeAgent(ctx: OriroPluginServiceContext): string | undefined {
...
async function reapOpenAcpxProcessLeases(params: {
  ...
  for (const lease of leases) {
    if (lease.rootPid <= 0) {
      ...
      if (!result) {
        result = await reapStaleOriroOwnedAcpxOrphans({
...
export function createAcpxRuntimeService(
  ...
  return {
    id: "acpx-runtime",
    async start(ctx: OriroPluginServiceContext): Promise<void> {
      if (process.env.ORIRO_SKIP_ACPX_RUNTIME === "1") {
        ctx.logger.info("skipping embedded acpx runtime backend (ORIRO_SKIP_ACPX_RUNTIME=1)");
        return;
      }
    ...
    async stop(_ctx: OriroPluginServiceContext): Promise<void> {
      lifecycleRevision += 1;
