...
export async function readServiceStatusSummary(
  ...
  try {
    ...
    const managedByOriro = state.installed;
    // A running unmanaged process still counts as installed for status display.
    const externallyManaged = !managedByOriro && state.running;
    const installed = managedByOriro || externallyManaged;
