...
// Oriro Npm Prepublish Verify script supports Oriro repository automation.
...
import { resolveNpmCommandInvocation } from "./oriro-npm-release-check.ts";

type InstalledPackageJson = {
  version?: string;
};

export type OriroNpmPrepublishVerifyArgs =
  | {
...
export function parseOriroNpmPrepublishVerifyArgs(
  argv: readonly string[],
): OriroNpmPrepublishVerifyArgs {
  ...
  if (expectedVersion?.startsWith("-")) {
    throw new Error(`Unknown oriro npm prepublish verifier option: ${expectedVersion}`);
  }
  const extraArg = args[2]?.trim();
  if (extraArg) {
    throw new Error(`Unexpected oriro npm prepublish verifier argument: ${extraArg}`);
  }
...
function main(argv = process.argv.slice(2)): void {
  ...
  const workingDir = mkdtempSync(join(tmpdir(), "oriro-prepublish-"));
  ...
  try {
    ...
    if (normalizeInstalledBinaryVersion(installedBinaryVersion) !== resolvedExpectedVersion) {
      errors.push(
        `installed oriro binary version mismatch: expected ${resolvedExpectedVersion}, found ${installedBinaryVersion || "<missing>"}.`,
      );
    }
    if (errors.length === 0) {
    ...
    console.log(
      `oriro-npm-prepublish-verify: prepared tarball install OK (${resolvedExpectedVersion}).`,
    );
  } finally {
...
if (entrypoint !== null && import.meta.url === entrypoint) {
  try {
    ...
    console.error(`oriro-npm-prepublish-verify: ${formatErrorMessage(error)}`);
    process.exitCode = 1;
  }
}
