...
import { formatEnvelopeTimestamp } from "oriro/plugin-sdk/channel-test-helpers";
...
const CONFIG_KEY = Symbol.for("oriro:testConfigMock");
const SOURCE_CONFIG_KEY = Symbol.for("oriro:testSourceConfigMock");
...
vi.mock("./auto-reply/monitor/message-line.runtime.js", () => ({
  ...
  resolveMessagePrefix: (
    ...
    return params?.hasAllowFrom === true ? "" : "[oriro]";
  },
}));

vi.mock("./auth-store.runtime.js", () => ({
  resolveOAuthDir: () => "/tmp/oriro-oauth",
}));

vi.mock("./session.runtime.js", () => {
  const created = createMockBaileys();
  (globalThis as Record<PropertyKey, unknown>)[Symbol.for("oriro:lastSocket")] =
    created.lastSocket;
  return {
    ...created.mod,
  };
...
export function getLastSocket(): MockBaileysSocket {
  const getter = (globalThis as Record<PropertyKey, unknown>)[Symbol.for("oriro:lastSocket")];
  if (typeof getter === "function") {
