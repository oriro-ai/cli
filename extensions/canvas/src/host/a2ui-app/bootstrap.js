...
import { themeContext } from "@oriro/a2ui-theme-context";
...
class OriroA2UIHost extends LitElement {
  static properties = {
    surfaces: { state: true },
  ...
  connectedCallback() {
    ...
    globalThis.oriroA2UI = api;
    this.addEventListener("a2uiaction", (evt) => this.#handleA2UIAction(evt));
    this.#statusListener = (evt) => this.#handleActionStatus(evt);
    for (const eventName of ["oriro:a2ui-action-status"]) {
  ...
  #handleA2UIAction(evt) {
    ...
    const handler =
      globalThis.webkit?.messageHandlers?.oriroCanvasA2UIAction ??
      globalThis.oriroCanvasA2UIAction;
    if (handler?.postMessage) {
      try {
        // WebKit message handlers support structured objects; Android's JS interface expects strings.
        if (handler === globalThis.oriroCanvasA2UIAction) {
...
if (!customElements.get("oriro-a2ui-host")) {
  customElements.define("oriro-a2ui-host", OriroA2UIHost);
}
