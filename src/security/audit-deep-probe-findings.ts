...
export function collectDeepProbeFindings(params: {
  ...
  if (params.deep?.gateway?.attempted && !params.deep.gateway.ok) {
    findings.push({
      ...
      remediation: `Run "${formatCliCommand("oriro status --all")}" to debug connectivity/auth, then re-run "${formatCliCommand("oriro security audit --deep")}".`,
    });
  }
  if (params.authWarning) {
    findings.push({
      ...
      remediation: `Set ORIRO_GATEWAY_TOKEN/ORIRO_GATEWAY_PASSWORD in this shell or resolve the external secret provider, then re-run "${formatCliCommand("oriro security audit --deep")}".`,
    });
  }
  return findings;
}
