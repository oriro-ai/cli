...
describe("proxy cli", () => {
  ...
  it.each([
    ...
    expect(() => program.parse(["node", "oriro", ...args])).toThrow(expected);
  });

  it("normalizes signed decimal numeric options through the shared parser", async () => {
    const program = createProgram();

    await program.parseAsync(["node", "oriro", "proxy", "start", "--port", "+08080"]);
    await program.parseAsync(["node", "oriro", "proxy", "validate", "--timeout-ms", "+01000"]);
    await program.parseAsync(["node", "oriro", "proxy", "sessions", "--limit", "+05"]);
