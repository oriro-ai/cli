...
function resolveStartupEntryPath(env: Record<string, string>, extension = "cmd") {
  const taskName = env.ORIRO_WINDOWS_TASK_NAME ?? "Oriro Gateway";
  return path.join(
    env.APPDATA,
    "Microsoft",
    "Windows",
...
async function writeNodeScript(env: Record<string, string>, port = "18789") {
  ...
  await fs.writeFile(
    ...
    [
      "@echo off",
      `set "ORIRO_SERVICE_KIND=node"`,
      `set "ORIRO_GATEWAY_PORT=${port}"`,
      `"C:\\bin\\oriro.cmd" node run --host 127.0.0.1 --port ${port}`,
      "",
    ].join("\r\n"),
    "utf8",
  );
...
function makeNodeServiceEnv(env: Record<string, string>): Record<string, string> {
  return {
    ...
    ORIRO_WINDOWS_TASK_NAME: "Oriro Node",
  };
}
    ...
    if (
      ...
      return makeSpawnSyncResult({
        stdout: JSON.stringify([
          {
            ProcessId: processId,
            CommandLine: "C:\\bin\\oriro.cmd node run --host 127.0.0.1 --port 18789",
          },
        ]),
      });
    }
...
function expectStartupFallbackSpawn() {
  ...
  expect((options.env as Record<string, string> | undefined)?.ORIRO_GATEWAY_PORT).toBe("18789");
...
function installNodeScheduledTask(env: Record<string, string>, stdout = new PassThrough()) {
  return installScheduledTask({
    ...
    stdout,
    programArguments: ["node", "oriro", "node", "run", "--host", "127.0.0.1", "--port", "18789"],
    environment: {
      ORIRO_SERVICE_KIND: "node",
...
describe("Windows startup fallback", () => {
  it("falls back to a Startup-folder launcher when schtasks create is denied", async () => {
    await withWindowsEnv("oriro-win-startup-", async ({ env }) => {
      addStartupFallbackMissingResponses([
        ...
        if (
          ...
          return {
            ...
            stdout: JSON.stringify([
              {
                ProcessId: 5151,
                CommandLine: "node oriro node run --host 127.0.0.1 --port 18789",
              },
            ]),
            stderr: "",
            status: 0,
  ...
  it("does not report a node task as running from a gateway listener", async () => {
    await withWindowsEnv("oriro-win-startup-", async ({ env }) => {
      env.ORIRO_SERVICE_KIND = "node";
      env.ORIRO_WINDOWS_TASK_NAME = "Oriro Node";
      findVerifiedGatewayListenerPidsOnPortSync.mockReturnValue([4242]);
      schtasksResponses.push(
        { code: 0, stdout: "", stderr: "" },
        { code: 0, stdout: notYetRunTaskQueryOutput(), stderr: "" },
        ...
        if (
          ...
          return {
            ...
            stdout: JSON.stringify([
              {
                ProcessId: 4242,
                CommandLine: "C:\\manual\\oriro.cmd node run --host 127.0.0.1 --port 18789",
              },
              {
                ProcessId: 5151,
                CommandLine: "C:\\bin\\oriro.cmd node run --host 127.0.0.1 --port 18789",
  ...
  it("kills the Startup fallback runtime even when the CLI env omits the gateway port", async () => {
    await withWindowsEnv("oriro-win-startup-", async ({ env }) => {
      ...
      delete envWithoutPort.ORIRO_GATEWAY_PORT;
