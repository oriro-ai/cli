...
import { resolveChannelTtsVoiceDelivery } from "oriro/plugin-sdk/channel-targets";
import type {
  OriroConfig,
  ResolvedTtsPersona,
  TtsAutoMode,
...
import { redactSensitiveText } from "oriro/plugin-sdk/logging-core";
import { transcodeAudioBuffer } from "oriro/plugin-sdk/media-runtime";
import { clampTimerTimeoutMs } from "oriro/plugin-sdk/number-runtime";
import {
...
import { isVerbose, logVerbose } from "oriro/plugin-sdk/runtime-env";
import { tempWorkspaceSync, resolvePreferredOriroTmpDir } from "oriro/plugin-sdk/sandbox";
import { privateFileStoreSync } from "oriro/plugin-sdk/security-runtime";
import {
  normalizeLowercaseStringOrEmpty,
  normalizeOptionalLowercaseString,
  normalizeOptionalString,
} from "oriro/plugin-sdk/string-coerce-runtime";
import { stripMarkdown } from "oriro/plugin-sdk/text-chunking";
import { resolveConfigDir, resolveUserPath } from "oriro/plugin-sdk/text-utility-runtime";
import {
...
function resolveTtsPrefsPathValue(prefsPath: string | undefined): string {
  ...
  const envPath = process.env.ORIRO_TTS_PREFS?.trim();
  if (envPath) {
...
function resolveConfiguredSpeechVoiceModelRefs(cfg: OriroConfig | undefined): VoiceModelRef[] {
...
function sortSpeechProvidersForAutoSelection(cfg?: OriroConfig) {
...
function resolveTtsRuntimeConfig(cfg: OriroConfig): OriroConfig {
  return (
    selectApplicableRuntimeConfig({
      inputConfig: cfg,
...
export function resolveTtsProviderOrder(primary: TtsProvider, cfg?: OriroConfig): TtsProvider[] {
  const effectiveCfg = cfg ? resolveTtsRuntimeConfig(cfg) : undefined;
