// Oriro plugin module implements oriro ajv cache behavior.
import { createHash } from "node:crypto";
import { createRequire } from "node:module";
import { pathToFileURL } from "node:url";

const installedSymbol = Symbol.for("oriro.oriro.ajv-compile-cache.installed");
const cacheSymbol = Symbol.for("oriro.oriro.ajv-compile-cache.entries");
...
async function resolveOriroAjvConstructor(packageEntryPath: string): Promise<AjvConstructor> {
  const oriroRequire = createRequire(packageEntryPath);
...
export async function installOriroAjvCompileCache(packageEntryPath: string) {
  let AjvCtor: AjvConstructor;
  try {
    AjvCtor = await resolveOriroAjvConstructor(packageEntryPath);
  } catch {
    return;
  }
