#!/usr/bin/env bash
# Rootless Oriro in Podman: run after one-time setup.
#
# One-time setup (from repo root): ./scripts/podman/setup.sh
# Then:
#   ./scripts/run-oriro-podman.sh launch        # Start gateway
#   ./scripts/run-oriro-podman.sh launch setup  # Onboarding wizard
#
# Manage the running container from the host CLI:
#   oriro --container oriro dashboard --no-open
#   oriro --container oriro channels login
#
...
run_podman_detached() {
  oriro_host_timeout_cmd "$PODMAN_RUN_TIMEOUT" podman run "$@"
}
...
load_podman_env_file() {
  ...
  while IFS= read -r line <&9 || [[ -n "$line" ]]; do
    ...
    case "$key" in
      ORIRO_GATEWAY_TOKEN|ORIRO_PODMAN_CONTAINER|ORIRO_PODMAN_IMAGE|ORIRO_IMAGE|ORIRO_PODMAN_PULL|ORIRO_PODMAN_RUN_TIMEOUT|ORIRO_PODMAN_GATEWAY_HOST_PORT|ORIRO_GATEWAY_PORT|ORIRO_PODMAN_BRIDGE_HOST_PORT|ORIRO_BRIDGE_PORT|ORIRO_GATEWAY_BIND|ORIRO_PODMAN_USERNS|ORIRO_BIND_MOUNT_OPTIONS|ORIRO_PODMAN_PUBLISH_HOST)
        ;;
      *)
        continue
        ;;
...
if [[ "$(id -u)" -eq 0 ]]; then
  fail "Run run-oriro-podman.sh as your normal user so Podman stays rootless."
fi
...
CONFIG_DIR="${ORIRO_CONFIG_DIR:-$EFFECTIVE_HOME/.oriro}"
ENV_FILE="${ORIRO_PODMAN_ENV:-$CONFIG_DIR/.env}"
...
CONFIG_DIR="${ORIRO_CONFIG_DIR:-$EFFECTIVE_HOME/.oriro}"
ENV_FILE="${ORIRO_PODMAN_ENV:-$CONFIG_DIR/.env}"
WORKSPACE_DIR="${ORIRO_WORKSPACE_DIR:-$CONFIG_DIR/workspace}"
CONTAINER_NAME="${ORIRO_PODMAN_CONTAINER:-oriro}"
ORIRO_IMAGE="${ORIRO_PODMAN_IMAGE:-${ORIRO_IMAGE:-oriro:local}}"
PODMAN_PULL="${ORIRO_PODMAN_PULL:-never}"
PODMAN_RUN_TIMEOUT="${ORIRO_PODMAN_RUN_TIMEOUT:-600s}"
HOST_GATEWAY_PORT="${ORIRO_PODMAN_GATEWAY_HOST_PORT:-${ORIRO_GATEWAY_PORT:-18789}}"
HOST_BRIDGE_PORT="${ORIRO_PODMAN_BRIDGE_HOST_PORT:-${ORIRO_BRIDGE_PORT:-18790}}"
PUBLISH_HOST="${ORIRO_PODMAN_PUBLISH_HOST:-127.0.0.1}"
...
validate_single_line_value "image name" "$ORIRO_IMAGE"
...
resolve_config_gateway_bind() {
  local config_dir="$1"
  if ! command -v oriro >/dev/null 2>&1; then
    return 0
  fi
  ORIRO_CONTAINER="" ORIRO_CONFIG_DIR="$config_dir" \
    oriro config get gateway.bind 2>/dev/null || true
}
...
# ORIRO_GATEWAY_BIND first, then gateway.bind in local config.
CONFIG_GATEWAY_BIND="$(resolve_config_gateway_bind "$CONFIG_DIR")"
GATEWAY_BIND="${ORIRO_GATEWAY_BIND:-${CONFIG_GATEWAY_BIND:-lan}}"
...
sync_local_control_ui_origins_via_cli() {
  ...
  if ! command -v oriro >/dev/null 2>&1; then
    echo "Warning: oriro not found; unable to sync gateway.controlUi.allowedOrigins in $file." >&2
    return 0
  fi
  if ! command -v python3 >/dev/null 2>&1; then
    ORIRO_CONTAINER="" ORIRO_CONFIG_DIR="$config_dir" \
      oriro config set gateway.controlUi.allowedOrigins \
  ...
  allowed_json="$(
    ORIRO_CONTAINER="" ORIRO_CONFIG_DIR="$config_dir" \
      oriro config get gateway.controlUi.allowedOrigins --json 2>/dev/null || true
  )"
...
PY
  ...
  ORIRO_CONTAINER="" ORIRO_CONFIG_DIR="$config_dir" \
    oriro config set gateway.controlUi.allowedOrigins "$merged_json" --strict-json >/dev/null
}
...
if [[ -z "${ORIRO_GATEWAY_TOKEN:-}" ]]; then
  export ORIRO_GATEWAY_TOKEN="$(generate_token_hex_32)"
  mkdir -p "$(dirname "$ENV_FILE")"
  ensure_safe_existing_dir "env file directory" "$(dirname "$ENV_FILE")"
  upsert_env_var "$ENV_FILE" "ORIRO_GATEWAY_TOKEN" "$ORIRO_GATEWAY_TOKEN"
  echo "Generated ORIRO_GATEWAY_TOKEN and wrote it to $ENV_FILE." >&2
fi

CONFIG_JSON="$CONFIG_DIR/oriro.json"
...
PODMAN_USERNS="${ORIRO_PODMAN_USERNS:-keep-id}"
USERNS_ARGS=()
RUN_USER_ARGS=()
case "$PODMAN_USERNS" in
  ...
  *)
    echo "Unsupported ORIRO_PODMAN_USERNS=$PODMAN_USERNS (expected: keep-id, auto, host)." >&2
    exit 2
    ;;
esac
...
if [[ "$PODMAN_USERNS" == "keep-id" ]]; then
  ...
  echo "Starting container without --user (ORIRO_PODMAN_USERNS=$PODMAN_USERNS), mounts may require ownership fixes." >&2
fi

SELINUX_MOUNT_OPTS=""
if [[ -z "${ORIRO_BIND_MOUNT_OPTIONS:-}" ]]; then
  ...
  SELINUX_MOUNT_OPTS="${ORIRO_BIND_MOUNT_OPTIONS#:}"
  [[ -n "$SELINUX_MOUNT_OPTS" ]] && SELINUX_MOUNT_OPTS=",$SELINUX_MOUNT_OPTS"
fi

if [[ "$RUN_SETUP" == true ]]; then
  TOKEN_ENV_FILE="$(create_token_env_file "$ENV_FILE" "$ORIRO_GATEWAY_TOKEN")"
  podman run --pull="$PODMAN_PULL" --rm -it \
    --init \
    "${USERNS_ARGS[@]}" "${RUN_USER_ARGS[@]}" \
    -e HOME=/home/node -e TERM=xterm-256color -e BROWSER=echo \
    -e NPM_CONFIG_CACHE=/home/node/.oriro/.npm \
    -e ORIRO_NO_RESPAWN=1 \
    --env-file "$TOKEN_ENV_FILE" \
    -v "$CONFIG_DIR:/home/node/.oriro:rw${SELINUX_MOUNT_OPTS}" \
    -v "$WORKSPACE_DIR:/home/node/.oriro/workspace:rw${SELINUX_MOUNT_OPTS}" \
    "$ORIRO_IMAGE" \
...
echo "podman exec -it $CONTAINER_NAME oriro dashboard --no-open"
echo "podman exec -it $CONTAINER_NAME oriro devices approve --latest  # if pairing required"
