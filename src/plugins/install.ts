...
import {
  ...
  readOriroManagedNpmRootOverrides,
  repairManagedNpmRootOriroPeer,
...
import { installedPackageNeedsOriroPeerLinkRepair } from "../infra/package-update-utils.js";
import {
...
function formatUnresolvedOriroPeerLinkError(packageName: string): string {
  return `Installed plugin ${packageName} declares oriro as a peer dependency, but Oriro could not create a plugin-local node_modules/oriro link. Run from a packaged Oriro install or reinstall Oriro, then retry.`;
}
...
const MISSING_EXTENSIONS_ERROR =
  'package.json missing oriro.extensions; update the plugin package to include oriro.extensions (for example ["./dist/index.js"]). See https://docs.oriro.ai/help/troubleshooting#plugin-install-fails-with-missing-oriro-extensions';
...
const MANAGED_NPM_PACK_ARCHIVE_DIR = "_oriro-pack-archives";
const MANAGED_NPM_PROJECT_QUARANTINE_DIR = "_oriro-quarantined-npm-projects";
...
export const PLUGIN_INSTALL_ERROR_CODE = {
  ...
  MISSING_ORIRO_EXTENSIONS: "missing_oriro_extensions",
  MISSING_PLUGIN_MANIFEST: "missing_plugin_manifest",
  EMPTY_ORIRO_EXTENSIONS: "empty_oriro_extensions",
  INVALID_ORIRO_EXTENSIONS: "invalid_oriro_extensions",
...
function validateOriroPackageCompatibility(params: {
  pluginId: string;
  currentHostVersion: string;
  packageMetadata?: OriroPackageManifest;
  ...
  if (!pluginApiRangeCheck.ok) {
    return {
      ok: false,
      error: `invalid package.json oriro.compat.pluginApi: ${pluginApiRangeCheck.error}`,
...
function validateOriroPackageInstallCompatibility(params: {
  ...
  if (!minHostVersionCheck.ok) {
    if (minHostVersionCheck.kind === "invalid") {
      return {
        ok: false,
        error: `invalid package.json oriro.install.minHostVersion: ${minHostVersionCheck.error}`,
  ...
  return validateOriroPackageCompatibility({
...
function ensureOriroExtensions(params: { manifest: PackageManifest }):
  | {
      ok: true;
      entries: string[];
    }
  ...
  if (resolved.status === "missing") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.MISSING_ORIRO_EXTENSIONS,
    };
  }
  if (resolved.status === "empty") {
    return {
      ok: false,
      error: "package.json oriro.extensions is empty",
      code: PLUGIN_INSTALL_ERROR_CODE.EMPTY_ORIRO_EXTENSIONS,
    };
  }
  if (resolved.status === "invalid") {
    return {
      ...
      code: PLUGIN_INSTALL_ERROR_CODE.INVALID_ORIRO_EXTENSIONS,
    };
  }
  return {
    ok: true,
...
function compareNpmSemver(a: string, b: string): number {
  const releaseCmp = compareOriroReleaseVersions(a, b);
  if (releaseCmp !== null) {
    return releaseCmp;
  }
...
async function resolveTrustedOfficialPrereleaseResolution(params: {
  ...
  if (!params.spec.name.startsWith("@oriro/")) {
    return null;
  }
...
function validateNpmResolutionCompatibility(params: {
  ...
  return validateOriroPackageInstallCompatibility({
    runtime: params.runtime,
    pluginId: params.expectedPluginId ?? params.resolution.name ?? params.parsedSpec.name,
    packageMetadata: params.resolution.packageOriro as OriroPackageManifest | undefined,
  });
}
...
async function rollbackManagedNpmPluginInstall(params: {
  ...
  if (params.snapshot) {
    try {
      ...
      await relinkOriroPeerDependenciesInManagedNpmRoot({
        npmRoot: params.npmRoot,
  ...
  if (params.packageName !== "oriro") {
    try {
      await repairManagedNpmRootOriroPeer({
        npmRoot: params.npmRoot,
        timeoutMs: params.timeoutMs,
        logger: params.logger,
      });
      ...
      params.logger.warn?.(
        `Failed to repair managed npm oriro peer after rollback: ${String(error)}`,
      );
    }
  }
  try {
...
async function createManagedNpmPluginInstallRollbackSnapshot(params: {
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-npm-plugin-rollback-"));
...
async function listManagedNpmRootPackageNames(npmRoot: string): Promise<Set<string>> {
  ...
  for (const entry of entries.toSorted((left, right) => left.name.localeCompare(right.name))) {
    if (entry.name === ".bin" || entry.name === "oriro") {
      continue;
    }
...
function resolveRequiredPlatformPackageNames(
  ...
  if (!Array.isArray(raw)) {
    return {
      ok: false,
      error: "package.json oriro.install.requiredPlatformPackages must be an array",
    };
  }
  const packageNames = new Set<string>();
  for (const value of raw) {
    if (typeof value !== "string") {
      return {
        ok: false,
        error:
          "package.json oriro.install.requiredPlatformPackages must contain only npm package names",
      };
    }
    ...
    if (specError || !parsed || parsed.selectorKind !== "none") {
      return {
        ok: false,
        error: `package.json oriro.install.requiredPlatformPackages contains invalid package name: ${value}`,
      };
    }
...
async function installPluginFromManagedNpmRoot(
  ...
  const runManagedNpmInstall = async (
    ...
    if (params.packageName !== "oriro") {
      const repairedOriroPeer = await repairManagedNpmRootOriroPeer({
        npmRoot,
        timeoutMs,
        logger,
      });
      if (repairedOriroPeer) {
        logger.info?.(`Repaired stale oriro peer dependency in ${npmRoot}`);
      }
    }
    let preInstallRootPackageNames = await listManagedNpmRootPackageNames(npmRoot);
    const managedOverrides = await readOriroManagedNpmRootOverrides();
    ...
    if (omittedPlatformPackages.length > 0) {
      ...
      try {
        freshCacheDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-npm-cache-"));
    ...
    if (params.packageName !== "oriro") {
      ...
      if (repairedOriroPeer) {
        logger.info?.(`Repaired stale oriro peer dependency in ${npmRoot} after npm install`);
      }
    }
    try {
      await relinkOriroPeerDependenciesInManagedNpmRoot({
      ...
      return await rollbackFailedManagedNpmInstall({
        ok: false,
        error: `Failed to repair oriro peer links after npm install: ${String(error)}`,
      });
    }
    if (installedPackageNeedsOriroPeerLinkRepair(installRoot)) {
      return await rollbackFailedManagedNpmInstall({
        ok: false,
        error: formatUnresolvedOriroPeerLinkError(params.packageName),
      });
    }
...
> {
  ...
  try {
    ...
    backupTempDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-npm-pack-archive-"));
...
async function runInstallSourceScan(params: {
  ...
  try {
    ...
    return {
      ok: false,
      error: `${params.subject} installation blocked: code safety scan failed (${String(err)}). Run "oriro security audit --deep" for details.`,
      code: PLUGIN_INSTALL_ERROR_CODE.SECURITY_SCAN_FAILED,
    };
  }
}
...
async function installBundleFromSourceDir(
  ...
  const packageMetadata = packageManifestResult.manifest
    ? runtime.getPackageManifestMetadata(packageManifestResult.manifest)
    : undefined;
  const compatibilityError = validateOriroPackageInstallCompatibility({
    runtime,
    pluginId,
    packageMetadata,
  });
...
async function detectNativePackageInstallSource(packageDir: string): Promise<boolean> {
  ...
  try {
    ...
    return ensureOriroExtensions({ manifest }).ok;
  } catch {
    return false;
  }
}
...
> {
  ...
  if (!ocManifestResult.ok && params.requirePluginManifest) {
    return {
      ok: false,
      error: `package missing valid oriro.plugin.json: ${ocManifestResult.error}`,
  ...
  const extensionsResult = ensureOriroExtensions({
    manifest,
  });
...
async function scanAndLinkInstalledPackage(params: {
  ...
  if (peerLinkRepair.skipped > 0) {
    return {
      ok: false,
      error: formatUnresolvedOriroPeerLinkError(params.pluginId),
    };
  }
  return null;
}
...
export async function installPluginFromNpmSpec(
  ...
  const policyTempDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-npm-policy-"));
  try {
...
export async function installPluginFromPath(
  ...
  return {
    ...
    error:
      "Plain file plugin installs are not supported. Install a plugin directory or archive that contains oriro.plugin.json, or list standalone plugin files in plugins.load.paths.",
  };
}
