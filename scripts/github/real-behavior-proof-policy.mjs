...
/** OriroSweeper-owned labels that Oriro preserves but does not mutate. */
...
export const ORIROSWEEPER_PROOF_VERDICT_STATUS = "orirosweeper_exact_head_pass";
const ORIROSWEEPER_BOT_LOGINS = new Set(["orirosweeper[bot]", "oriro-orirosweeper[bot]"]);
...
function result(status, reason, details = {}) {
  return {
    ...
    passed: ["passed", "skipped", ORIROSWEEPER_PROOF_VERDICT_STATUS].includes(status),
    ...details,
  };
}
...
export function hasOriroSweeperExactHeadProof({ pullRequest, comments = [] } = {}) {
  ...
  for (const comment of comments) {
    if (!isTrustedOriroSweeperComment(comment)) {
      continue;
    }
    const body = String(comment?.body ?? "");
    const markers = body.match(/<!--\s*orirosweeper-verdict:pass\b[\s\S]*?-->/gi) ?? [];
...
export function evaluateOriroSweeperExactHeadProof({ pullRequest, comments = [] } = {}) {
  if (hasOriroSweeperExactHeadProof({ pullRequest, comments })) {
    return result(
      ORIROSWEEPER_PROOF_VERDICT_STATUS,
      "OriroSweeper accepted the PR evidence for the exact PR head.",
    );
  }
  return result("insufficient", "No exact-head OriroSweeper proof verdict was found.");
}
