...
export function registerHeadCommand(program: Command): void {
  program
    ...
    .option(
      "--code",
      "Reverse-engineer the target page into working code (uses your configured model)",
      false,
    )
    .option(
      "--yaml",
      "Reverse-engineer the target page into a structured YAML build spec for a coder",
      false,
    )
      ...
      async (
        ...
        if (opts.code) {
          try {
            ...
            const out = await urlToCode(target, headModelsFromConfig(getRuntimeConfig()), {
              stack: opts.stack,
            });
            const codePath =
              (opts.out || join(homedir(), ".oriro", "head", `code-${Date.now()}`)).replace(
                /\.html$/,
                "",
              ) + ".txt";
        ...
        // ── url → YAML build spec: a coder-grade reverse-engineering a developer can
        //    build from (design tokens, layout, component tree, data model, interactions). ──
        if (opts.yaml) {
          try {
            const { urlToSpec } = await import("../../head/video-to-code.js");
            const { headModelsFromConfig } = await import("../../head/model.js");
            const { getRuntimeConfig } = await import("../../config/io.js");
            process.stdout.write(
              `\n  🧭 ORIRO Head — capturing ${target} and reverse-engineering to a YAML spec…\n`,
            );
            const out = await urlToSpec(target, headModelsFromConfig(getRuntimeConfig()), {
              goal: opts.stack ? `Intended rebuild stack: ${opts.stack}` : undefined,
            });
            const specPath =
              (opts.out || join(homedir(), ".oriro", "head", `spec-${Date.now()}`)).replace(
                /\.(html|yaml|yml)$/,
                "",
              ) + ".yaml";
            mkdirSync(join(homedir(), ".oriro", "head"), { recursive: true });
            writeFileSync(specPath, out.spec, "utf8");
            process.stdout.write(`\n${out.spec}\n\n  Spec → ${specPath}\n\n`);
          } catch (err) {
            process.stderr.write(
              `  url→spec needs Playwright + a model: ${theme.accent("npm i -D playwright && npx playwright install chromium")}, then connect a model (oriro onboard).\n  (${(err as Error).message})\n`,
            );
          }
          return;
        }
        ...
        if (opts.json) {
          ...
          if (report.missing?.length) {
            process.stdout.write(`\n  Missing on ${target}:\n`);
            for (const g of report.missing.slice(0, 10))
              process.stdout.write(`   • ${g.label} (${g.priority}) — ${g.recommendation}\n`);
          }
          if (report.actionItems?.length) {
            process.stdout.write(`\n  Action items:\n`);
            for (const a of report.actionItems.slice(0, 10))
              process.stdout.write(
                `   → ${a.title} [${a.priority}/${a.effort}] — ${a.rationale}\n`,
              );
          }
        }
        ...
        const outPath =
          ...
          join(
            ...
            ".oriro",
            "head",
            `report-${report.target?.title ? report.target.title.replace(/\W+/g, "-").slice(0, 24) : "inspect"}.html`,
          );
        try {
          mkdirSync(join(homedir(), ".oriro", "head"), { recursive: true });
