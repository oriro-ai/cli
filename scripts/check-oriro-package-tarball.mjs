...
function usage() {
  return "Usage: node scripts/check-oriro-package-tarball.mjs <oriro.tgz>";
}
...
function runPhase(label, action) {
  ...
  try {
    ...
    if (phaseTimingsEnabled) {
      ...
      console.error(`check-oriro-package-tarball: ${label} completed in ${durationMs}ms`);
    }
  }
}
...
const extractDir = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-package-tarball-"));
try {
...
if (!entrySet.has("npm-shrinkwrap.json")) {
  ...
  try {
    ...
    if (shrinkwrap.name !== "oriro") {
      errors.push("npm-shrinkwrap.json root name must be oriro");
    }
    ...
    if (!rootPackage || rootPackage.name !== "oriro") {
      errors.push("npm-shrinkwrap.json packages root must name oriro");
    }
