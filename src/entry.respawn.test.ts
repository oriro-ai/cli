...
const ORIRO_NODE_EXTRA_CA_CERTS_READY = "ORIRO_NODE_EXTRA_CA_CERTS_READY";
const ORIRO_NODE_OPTIONS_READY = "ORIRO_NODE_OPTIONS_READY";
...
describe("buildCliRespawnPlan", () => {
  ...
  it("adds NODE_EXTRA_CA_CERTS and warning suppression in one respawn", () => {
    ...
    expect(respawnPlan.env[ORIRO_NODE_EXTRA_CA_CERTS_READY]).toBe("1");
    expect(respawnPlan.env[ORIRO_NODE_OPTIONS_READY]).toBe("1");
  ...
  it.each(["tui", "terminal", "chat"] as const)(
    ...
    (command) => {
      ...
      const respawnPlan = expectCliRespawnPlan(plan);
      expect(respawnPlan.argv).toEqual(["oriro", command]);
      expect(respawnPlan.env.NODE_EXTRA_CA_CERTS).toBe("/etc/ssl/certs/ca-certificates.crt");
      expect(respawnPlan.env[ORIRO_NODE_EXTRA_CA_CERTS_READY]).toBe("1");
      expect(respawnPlan.env[ORIRO_NODE_OPTIONS_READY]).toBeUndefined();
  ...
  it("keeps bare-root startup respawns attached to the terminal", () => {
    ...
    const respawnPlan = expectCliRespawnPlan(plan);
    expect(respawnPlan.argv).toEqual([EXPERIMENTAL_WARNING_FLAG, "oriro"]);
    expect(respawnPlan.detachForProcessTree).toBe(false);
  });
  ...
  it("does not respawn interactive commands for warning suppression only", () => {
    expect(
      buildCliRespawnPlan({
        argv: ["node", "oriro", "tui"],
        env: { [ORIRO_NODE_EXTRA_CA_CERTS_READY]: "1" },
        execArgv: [],
  ...
  it("returns null when both respawn guards are already satisfied", () => {
    expect(
      buildCliRespawnPlan({
        argv: ["node", "oriro", "status"],
        env: {
          [ORIRO_NODE_EXTRA_CA_CERTS_READY]: "1",
          [ORIRO_NODE_OPTIONS_READY]: "1",
        },
  ...
  it("adds a larger V8 stack size on Windows", () => {
    ...
    expect(respawnPlan.env[ORIRO_NODE_EXTRA_CA_CERTS_READY]).toBeUndefined();
    expect(respawnPlan.env[ORIRO_NODE_OPTIONS_READY]).toBeUndefined();
    expect(respawnPlan.detachForProcessTree).toBe(false);
  });
  ...
  it("respawns Volta shims through node so the shim is not called directly", () => {
    const plan = buildCliRespawnPlan({
      argv: ["/home/alice/.volta/bin/volta-shim", "/usr/local/bin/oriro", "status"],
    ...
    runCliRespawnPlan(
      {
        ...
        argv: ["/repo/oriro/dist/entry.js", "status"],
        env: { ORIRO_NODE_OPTIONS_READY: "1" },
  ...
  it("force-kills a signaled respawn child that does not exit", () => {
    ...
    try {
      runCliRespawnPlan(
        {
          command: "/usr/bin/node",
          argv: ["/repo/oriro/dist/entry.js", "tui"],
          env: {},
