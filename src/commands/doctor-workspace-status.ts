...
function noteFlowRecoveryHints() {
  ...
  note(
    [
      ...
      `Inspect: ${formatCliCommand("oriro tasks flow show <flow-id>")}`,
      `Cancel: ${formatCliCommand("oriro tasks flow cancel <flow-id>")}`,
    ]
  ...
  const lines = [
    ...
    singleDrift
      ? `Fix: ${formatCliCommand(
          `oriro plugins update ${singleDrift.pluginId}`,
        )} && ${formatCliCommand("oriro gateway restart")}.`
      : `Fix: ${formatCliCommand(
          "oriro plugins update <plugin-id>",
        )} for each drifted plugin, then ${formatCliCommand("oriro gateway restart")}.`,
  ];
