...
const STEP_LABELS: Record<string, string> = {
  ...
  "oriro doctor entry": "Checking doctor entrypoint",
  "oriro doctor": "Running doctor checks",
...
export function inferUpdateFailureHints(result: UpdateRunResult): string[] {
  ...
  if (isGlobalPackageInstallStep && stderr.includes("eacces")) {
    ...
    hints.push("Example: npm config set prefix ~/.local && npm i -g oriro@latest");
    hints.push(
      "System install outline: oriro gateway stop -> sudo <system-npm> i -g oriro@latest -> oriro gateway install --force -> oriro gateway restart.",
    );
  }
  ...
  if (
    ...
    hints.push("If it still fails: npm i -g oriro@latest --omit=optional");
  }
