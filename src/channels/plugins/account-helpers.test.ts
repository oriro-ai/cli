...
function cfg(accounts?: Record<string, unknown> | null, defaultAccount?: string): OriroConfig {
  if (accounts === null) {
    return {
      channels: {
  ...
  if (accounts === undefined && !defaultAccount) {
    return {} as unknown as OriroConfig;
  }
  return {
    channels: {
      testchannel: {
...
function expectResolvedAccountIdsCase(params: {
  resolve: (cfg: OriroConfig) => string[];
  input: OriroConfig;
  expected: string[];
}) {
...
function expectResolvedDefaultAccountCase(input: OriroConfig, expected: string) {
