  ...
  it("flags wildcard re-exports from plugin-sdk subpaths", () => {
    expect(
      findPluginSdkWildcardReexports(
        [
          'export * from "oriro/plugin-sdk/foo";',
          'export type * from "oriro/plugin-sdk/bar";',
          'export { named } from "oriro/plugin-sdk/foo";',
        ].join("\n"),
      ),
    ).toEqual([
      { line: 1, text: 'export * from "oriro/plugin-sdk/foo";' },
      { line: 2, text: 'export type * from "oriro/plugin-sdk/bar";' },
    ]);
  });
  ...
  it("allows explicit SDK exports and local wildcard barrels", () => {
    expect(
      findPluginSdkWildcardReexports(
        [
          'export { named } from "oriro/plugin-sdk/foo";',
          'export type { Named } from "oriro/plugin-sdk/foo";',
