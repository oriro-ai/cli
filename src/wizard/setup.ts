// Setup wizard orchestrates onboarding prompts and generated Oriro config.
...
async function writeWizardConfigFile(
  configInput: OriroConfig,
  opts: {
    allowConfigSizeDrop?: boolean;
    migrationBaseConfig?: OriroConfig;
...
export async function runSetupWizard(
  ...
  let baseConfig: OriroConfig = snapshot.valid
    ? snapshot.exists
  ...
  let pendingPluginInstallMigrationBaseConfig: OriroConfig | undefined = baseConfig;
  ...
  if (snapshot.exists && !snapshot.valid) {
    ...
    await prompter.outro(
      `Config invalid. Run \`${formatCliCommand("oriro doctor")}\` to repair it, then re-run setup.`,
    );
    runtime.exit(1);
    return;
  }
    ...
    await prompter.note(
      [
        ...
        ...(compatibilityNotices.length > 4
          ...
          : []),
        "",
        `Review: ${formatCliCommand("oriro doctor")}`,
        `Inspect: ${formatCliCommand("oriro plugins inspect --all")}`,
      ].join("\n"),
  ...
  const quickstartHint = t("wizard.setup.flowQuickstartHint", {
    command: formatCliCommand("oriro configure"),
  });
  ...
  if (
    ...
    runtime.error(
      "Invalid --flow. Use quickstart, manual, advanced, or import. Example: oriro onboard --flow quickstart",
    );
    runtime.exit(1);
    return;
  }
  ...
  let localGatewayToken = process.env.ORIRO_GATEWAY_TOKEN;
  try {
  ...
  let localGatewayPassword = process.env.ORIRO_GATEWAY_PASSWORD;
  try {
  ...
  let nextConfig: OriroConfig = applyLocalSetupWorkspaceConfig(baseConfig, workspaceDir);
  if (opts.skipBootstrap) {
    nextConfig = applySkipBootstrapConfig(nextConfig);
  }
