...
describe("collectWhatsAppStatusIssues", () => {
  beforeEach(() => {
    vi.stubEnv("ORIRO_CONTAINER_HINT", "");
    vi.stubEnv("ORIRO_PROFILE", "");
  });
    ...
    expect(issues).toEqual([
      {
        ...
        fix: "Run: oriro channels login (scan QR on the gateway host).",
      },
    ]);
  });
    ...
    expect(issues).toEqual([
      {
        ...
        fix: "Run: oriro doctor (or restart the gateway). If it persists, relink via channels login and check logs.",
      },
    ]);
  });
    ...
    expect(issues).toEqual([
      {
        ...
        fix: "Watch: oriro logs --follow and run oriro channels status --probe if disconnects continue. If it keeps flapping, restart the gateway or relink via channels login.",
      },
    ]);
  });
