...
vi.mock("../../plugins/manifest-metadata-scan.js", () => ({
  listOriroPluginManifestMetadata: manifestMocks.listOriroPluginManifestMetadata,
}));
...
function setManifestPlugins(plugins: unknown[]) {
  ...
  manifestMocks.listOriroPluginManifestMetadata.mockReturnValue(
  ...
  manifestMocks.loadPluginManifest.mockImplementation((pluginDir: string) => {
    ...
    return plugin
      ...
      : { ok: false, error: "missing manifest", manifestPath: `${pluginDir}/oriro.plugin.json` };
  });
}
...
beforeEach(() => {
  manifestMocks.listOriroPluginManifestMetadata.mockReset();
...
describe("resolveBundledStaticCatalogModel", () => {
  it("reuses one manifest scan across prepared lookups", () => {
    ...
    expect(manifestMocks.listOriroPluginManifestMetadata).toHaveBeenCalledTimes(1);
  });
