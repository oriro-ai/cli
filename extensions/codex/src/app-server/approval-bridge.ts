...
} from "oriro/plugin-sdk/agent-harness-exec-review-runtime";
/**
 * Bridges Codex app-server approval requests into Oriro policy hooks and
 * plugin approval UX.
 */
import {
...
export async function handleCodexAppServerApprovalRequest(params: {
  ...
  try {
    const policyOutcome = await runOriroToolPolicyForApprovalRequest({
      method: params.method,
      requestParams,
      paramsForRun: params.paramsForRun,
      context,
    ...
    // Native hook/model policy did not decide; fall back to the Oriro
...
/** Converts an Oriro approval outcome into the app-server method response. */
...
async function runOriroToolPolicyForApprovalRequest(params: {
  method: string;
  requestParams: JsonObject | undefined;
  paramsForRun: EmbeddedRunAttemptParams;
  context: ApprovalContext;
  ...
  const policyRequest = buildOriroToolPolicyRequest(params.method, params.requestParams);
  if (!policyRequest) {
    return undefined;
  }
...
function buildOriroToolPolicyRequest(
  method: string,
  requestParams: JsonObject | undefined,
