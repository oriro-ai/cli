...
import { resolveAccountEntry } from "oriro/plugin-sdk/account-resolution";
...
export function listLineAccountIds(cfg: OriroConfig): string[] {
  const lineConfig = cfg.channels?.line as LineConfig | undefined;
  const accounts = lineConfig?.accounts;
...
export function resolveDefaultLineAccountId(cfg: OriroConfig): string {
