...
const thirdPartyExtensionMessage =
  "Please publish this as a third-party plugin on [OriroHub](https://orirohub.ai) instead of adding it to the core repo. Docs: https://docs.oriro.ai/plugin and https://docs.oriro.ai/orirohub";
...
const rules = [
  {
    ...
    close: true,
    message:
      "Thanks for the contribution! New skills should be published on [OriroHub](https://orirohub.ai) for everyone to use. We’re keeping the core lean on skills, so I’m closing this out.",
  },
  {
    label: "r: support",
    close: true,
    message:
      "Please use [our support server](https://discord.gg/orirod) and ask in #help or #users-helping-users to resolve this, or follow the stuck FAQ at https://docs.oriro.ai/help/faq#im-stuck-whats-the-fastest-way-to-get-unstuck.",
  },
  {
    label: "r: false-positive",
    close: true,
    message:
      "Closing this because it looks like a false positive or reclassification-only report rather than an actionable ORIRO bug. If this is still a real issue, please open a fresh report with concrete reproduction steps and current-version details.",
  },
  {
    label: "r: no-ci-pr",
    close: true,
  ...
  },
  {
    ...
    message:
      "BlueBubbles is deprecated and no longer ships as a bundled Oriro channel. Use iMessage via `imsg` instead: https://docs.oriro.ai/channels/imessage. If this needs to stay BlueBubbles-backed, publish it as a third-party plugin on OriroHub instead of adding it back to core.",
  },
  {
    label: "r: moltbook",
    close: true,
    ...
    message:
      "ORIRO is not affiliated with Moltbook, and issues related to Moltbook should not be submitted here.",
  },
];
...
export const managedLabelSpecs = {
  "r: skill": {
    color: "5319E7",
    description: "Auto-close: skills should be published on OriroHub, not added to core.",
  },
  "r: support": {
    color: "0E8A16",
  ...
  "r: third-party-extension": {
    color: "5319E7",
    description: "Auto-close: third-party plugins/capabilities belong on OriroHub.",
  },
  "r: bluebubbles": {
    color: "D93F0B",
    description: "Auto-close: BlueBubbles is deprecated; use iMessage via imsg or OriroHub.",
  },
  "r: moltbook": {
    color: "B60205",
    description: "Auto-close and lock: Moltbook is off-topic for ORIRO.",
  },
  "r: spam": {
    color: "B60205",
  ...
  "triage: external-plugin-candidate": {
    color: "C5DEF5",
    description: "Candidate: plugin/capability may belong on OriroHub.",
  },
};
...
const pingWarningMessage =
  "Please don’t spam-ping multiple maintainers at once. Be patient, or join our community Discord for help: https://discord.gg/orirod";
...
const candidateActionRules = [
  ...
  },
  {
    ...
    close: true,
    message:
      "Closing this PR because docs discoverability and community-plugin listing changes should go through OriroHub or a maintainer-owned docs plan, not drive-by core churn.",
  },
  {
    label: candidateLabels.lowSignalDocs,
    close: true,
    message:
      "Closing this PR because the docs-only change is too low-signal for the core repo. Please reopen or resubmit with a concrete ORIRO docs gap and linked context.",
  },
  {
  ...
  },
  {
    ...
    close: true,
    message:
      "Closing this PR because the template is mostly blank and does not describe a concrete ORIRO problem, fix, or test plan. Please reopen or resubmit with the missing context filled in.",
  },
];

const normalizeLogin = (login) => login.toLowerCase();
const automationPrHeadPrefixes = ["orirosweeper/", "clownfish/"];
...
function hasLinkedReference(text) {
  return /(?:#\d+|github\.com\/oriro\/oriro\/(?:issues|pull)\/\d+)/i.test(text);
}
...
export function classifyPullRequestCandidateLabels(pullRequest, files) {
  ...
  if (
    ...
    !linkedReference &&
    (discoverabilityDocs ||
      /\b(community plugin|plugin listing|discoverability|showcase|orirohub)\b/i.test(text))
  ) {
  ...
  const addsPluginManifest = files.some(
    (file) =>
      file.status === "added" && /^extensions\/[^/]+\/oriro\.plugin\.json$/i.test(file.filename),
  );
  if (
    !clearDesignContext &&
    (addsPluginManifest ||
      /\b(third[- ]party|external plugin|community plugin|orirohub)\b/i.test(lowerText))
  ) {
...
function isOriroSweeperOwnedLabel(label) {
  return label === "orirosweeper" || label.startsWith("orirosweeper:");
}
...
function isOriroSweeperProofSufficientLabelEvent(context) {
  ...
  return (
    ...
    /orirosweeper/i.test(senderLogin)
  );
}
...
async function removeLabels(github, context, issueNumber, labels, labelSet) {
  for (const label of labels) {
    ...
    if (isOriroSweeperOwnedLabel(label)) {
      continue;
    }
    try {
      await github.rest.issues.removeLabel({
...
export async function runBarnacleAutoResponse({ github, context, core = console }) {
  ...
  if (pullRequest) {
    ...
    if (isOriroSweeperProofSufficientLabelEvent(context)) {
      core.info(
        `Skipping PR auto-response checks for #${pullRequest.number} because OriroSweeper owns ${PROOF_SUFFICIENT_LABEL}.`,
      );
      return;
    }
