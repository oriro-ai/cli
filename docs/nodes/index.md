...
`oriro nodes …` works against this Mac). In remote gateway mode, browser
automation is handled by the CLI node host (`oriro node run` or the
...
oriro devices reject <requestId>
oriro nodes status
...
`oriro devices list` before approving.
...
- `node.pair.*` (CLI: `oriro nodes pending/approve/reject/remove/rename`) is a separate gateway-owned
  node pairing store; it does **not** gate the WS `connect` handshake.
- `oriro nodes remove --node <id|name|ip>` removes a node pairing. For a
...
- **Approvals**: enforced on the node host via `~/.oriro/exec-approvals.json`.

Approval note:

- Approval-backed node runs bind exact request context.
- For direct shell/runtime file executions, Oriro also best-effort binds one concrete local
  file operand and denies the run if that file changes before execution.
- If Oriro cannot identify exactly one concrete local file for an interpreter/runtime command,
...
```bash
oriro node run --host <gateway-host> --port 18789 --display-name "Build Node"
```
...
export ORIRO_GATEWAY_TOKEN="<gateway-token>"
oriro node run --host 127.0.0.1 --port 18790 --display-name "Build Node"
```

Notes:

- `oriro node run` supports token or password auth.
- Env vars are preferred: `ORIRO_GATEWAY_TOKEN` / `ORIRO_GATEWAY_PASSWORD`.
...
- Node-host auth resolution only honors `ORIRO_GATEWAY_*` env vars.

### Start a node host (service)

```bash
oriro node install --host <gateway-host> --port 18789 --display-name "Build Node"
oriro node start
oriro node restart
```
...
If the node retries with changed auth details, re-run `oriro devices list`
and approve the current `requestId`.

Naming options:

- `--display-name` on `oriro node run` / `oriro node install` (persists in `~/.oriro/node.json` on the node).
- `oriro nodes rename --node <id|name|ip> --name "Build Node"` (gateway override).
...
```bash
oriro approvals allowlist add --node <id|name|ip> "/usr/bin/uname"
oriro approvals allowlist add --node <id|name|ip> "/usr/bin/sw_vers"
```

Approvals live on the node host at `~/.oriro/exec-approvals.json`.
...
```bash
oriro config set tools.exec.host node
oriro config set tools.exec.security allowlist
oriro config set tools.exec.node "<id-or-name>"
```
...
```bash
oriro nodes invoke --node <idOrNameOrIp> --command canvas.eval --params '{"javaScript":"location.href"}'
```
...
```bash
oriro nodes canvas snapshot --node <idOrNameOrIp> --format png
oriro nodes canvas snapshot --node <idOrNameOrIp> --format jpg --max-width 1200 --quality 0.9
```

### Canvas controls

```bash
oriro nodes canvas present --node <idOrNameOrIp> --target https://example.com
oriro nodes canvas hide --node <idOrNameOrIp>
oriro nodes canvas navigate https://example.com --node <idOrNameOrIp>
oriro nodes canvas eval --node <idOrNameOrIp> --js "document.title"
```
...
```bash
oriro nodes canvas a2ui push --node <idOrNameOrIp> --text "Hello"
oriro nodes canvas a2ui push --node <idOrNameOrIp> --jsonl ./payload.jsonl
oriro nodes canvas a2ui reset --node <idOrNameOrIp>
```
...
```bash
oriro nodes camera list --node <idOrNameOrIp>
oriro nodes camera snap --node <idOrNameOrIp>            # default: both facings (2 MEDIA lines)
oriro nodes camera snap --node <idOrNameOrIp> --facing front
```

Video clips (`mp4`):

```bash
oriro nodes camera clip --node <idOrNameOrIp> --duration 10s
oriro nodes camera clip --node <idOrNameOrIp> --duration 3000 --no-audio
```
...
```bash
oriro nodes screen record --node <idOrNameOrIp> --duration 10s --fps 10
oriro nodes screen record --node <idOrNameOrIp> --duration 10s --fps 10 --no-audio
```
...
```bash
oriro nodes location get --node <idOrNameOrIp>
oriro nodes location get --node <idOrNameOrIp> --accuracy precise --max-age 15000 --location-timeout 10000
```
...
```bash
oriro nodes invoke --node <idOrNameOrIp> --command sms.send --params '{"to":"+15555550123","message":"Hello from Oriro"}'
```
...
```bash
oriro nodes invoke --node <idOrNameOrIp> --command device.status --params '{}'
oriro nodes invoke --node <idOrNameOrIp> --command device.apps --params '{"limit":10}'
oriro nodes invoke --node <idOrNameOrIp> --command notifications.list --params '{}'
oriro nodes invoke --node <idOrNameOrIp> --command photos.latest --params '{"limit":1}'
```
...
```bash
oriro nodes notify --node <idOrNameOrIp> --title "Ping" --body "Gateway ready"
oriro nodes invoke --node <idOrNameOrIp> --command system.which --params '{"name":"git"}'
```
...
- On headless node host, `system.run` is gated by exec approvals (`~/.oriro/exec-approvals.json`).
...
```bash
oriro config set tools.exec.node "node-id-or-name"
```
...
```bash
oriro config unset tools.exec.node
oriro config unset 'agents.list[0].tools.exec.node'
```
...
Oriro can run a **headless node host** (no UI) that connects to the Gateway
...
```bash
oriro node run --host <gateway-host> --port 18789
```

Notes:

- Pairing is still required (the Gateway will show a device pairing prompt).
- The node host stores its node id, token, display name, and gateway connection info in `~/.oriro/node.json`.
- Exec approvals are enforced locally via `~/.oriro/exec-approvals.json`
  (see [Exec approvals](/tools/exec-approvals)).
- On macOS, the headless node host executes `system.run` locally by default. Set
  `ORIRO_NODE_EXEC_HOST=app` to route `system.run` through the companion app exec host; add
  `ORIRO_NODE_EXEC_FALLBACK=0` to require the app host and fail closed if it is unavailable.
- Add `--tls` / `--tls-fingerprint` when the Gateway WS uses TLS.

## Mac node mode

- The macOS menubar app connects to the Gateway WS server as a node (so `oriro nodes …` works against this Mac).
