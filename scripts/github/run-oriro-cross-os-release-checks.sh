...
tsx_version="${ORIRO_RELEASE_TSX_VERSION:-${TSX_VERSION:-4.21.0}}"
script_path="${ORIRO_RELEASE_CHECKS_SCRIPT:-workflow/scripts/oriro-cross-os-release-checks.ts}"
...
temp_root="${ORIRO_RELEASE_TSX_TOOL_ROOT:-${RUNNER_TEMP:-${TMPDIR:-/tmp}}}"
if command -v cygpath >/dev/null 2>&1; then
  temp_root="$(cygpath -u "${temp_root}")"
fi

tool_dir="${ORIRO_RELEASE_TSX_TOOL_DIR:-${temp_root}/oriro-release-tsx-${tsx_version}}"
