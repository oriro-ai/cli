...
/** Runs a test callback with an isolated Oriro home for cron tests. */
export async function withTempCronHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "oriro-cron-" });
}
...
export async function writeSessionStoreEntries(
  ...
  const dir = path.join(home, ".oriro", "sessions");
...
export function makeCfg(
  home: string,
  storePath: string,
  overrides: Partial<OriroConfig> = {},
): OriroConfig {
  const base: OriroConfig = {
    agents: {
      defaults: {
