...
vi.mock("oriro/plugin-sdk/runtime-env", async (importOriginal) => {
  const actual = await importOriginal<typeof import("oriro/plugin-sdk/runtime-env")>();
  const makeLogger = () => {
    const logger = {
      subsystem: "test",
      isEnabled: () => true,
...
vi.mock("oriro/plugin-sdk/system-event-runtime", () => ({
  enqueueSystemEvent: (...args: unknown[]) => messageQueueMock(...args),
}));
vi.mock("oriro/plugin-sdk/system-event-runtime.js", () => ({
  enqueueSystemEvent: (...args: unknown[]) => messageQueueMock(...args),
}));
