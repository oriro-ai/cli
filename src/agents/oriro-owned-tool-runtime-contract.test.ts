// Verifies Oriro-owned tool hooks preserve adjusted params and telemetry.
import type { AgentTool } from "oriro/plugin-sdk/agent-core";
import {
  installOriroOwnedToolHooks,
  resetOriroOwnedToolHooks,
  textToolResult,
} from "oriro/plugin-sdk/agent-runtime-test-contracts";
import type { ExtensionContext } from "oriro/plugin-sdk/agent-sessions";
...
describe("Oriro-owned tool runtime contract - embedded agent adapter", () => {
  afterEach(() => {
    resetOriroOwnedToolHooks();
  });
  ...
  it("preserves partially adjusted before_tool_call params through execution and after_tool_call", async () => {
    ...
    const hooks = installOriroOwnedToolHooks({ adjustedParams });
  ...
  it("commits successful embedded agent messaging text, media, and target telemetry", async () => {
    const hooks = installOriroOwnedToolHooks();
    ...
    expect(ctx.state.messagingToolSentMediaUrls).toEqual(["/tmp/oriro-reply.png"]);
    expect(
  ...
  it("fails closed when before_tool_call blocks an embedded agent dynamic tool", async () => {
    const hooks = installOriroOwnedToolHooks({ blockReason: "blocked by policy" });
