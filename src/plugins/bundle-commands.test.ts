...
function resolveBundlePluginRoot(homeDir: string, pluginId: string) {
  return path.join(homeDir, ".oriro", "extensions", pluginId);
}
...
describe("loadEnabledClaudeBundleCommands", () => {
  it("loads enabled Claude bundle markdown commands and skips disabled-model-invocation entries", async () => {
    const env = captureEnv(["HOME", "USERPROFILE", "ORIRO_HOME", "ORIRO_STATE_DIR"]);
    try {
      const homeDir = await createTempDir("oriro-bundle-commands-home-");
      const workspaceDir = await createTempDir("oriro-bundle-commands-workspace-");
