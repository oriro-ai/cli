...
describe("resolveSafeExternalUrl", () => {
  const baseHref = "https://oriro.ai/chat";
  ...
  it("allows relative URLs resolved against the current origin", () => {
    expect(resolveSafeExternalUrl("/assets/pic.png", baseHref)).toBe(
      "https://oriro.ai/assets/pic.png",
    );
  });

  it("allows blob URLs", () => {
    expect(resolveSafeExternalUrl("blob:https://oriro.ai/abc-123", baseHref)).toBe(
      "blob:https://oriro.ai/abc-123",
    );
  });
