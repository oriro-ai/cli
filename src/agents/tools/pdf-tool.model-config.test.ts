...
const TEST_AGENT_DIR = "/tmp/oriro-pdf-model-config";
...
vi.mock("./model-config.helpers.js", () => ({
  ...
  hasProviderAuthForTool: ({ provider, cfg }: { provider: string; cfg?: OriroConfig }) => {
  ...
  resolveDefaultModelRef: (cfg?: OriroConfig) => {
