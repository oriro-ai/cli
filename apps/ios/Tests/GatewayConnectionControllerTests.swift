...
@Suite(.serialized) struct GatewayConnectionControllerTests {
    ...
    @Test @MainActor func currentCapsReflectToggles() {
        withUserDefaults([
            ...
            "camera.enabled": true,
            "location.enabledMode": OriroLocationMode.always.rawValue,
            ...
            #expect(caps.contains(OriroCapability.canvas.rawValue))
            #expect(caps.contains(OriroCapability.screen.rawValue))
            #expect(caps.contains(OriroCapability.camera.rawValue))
            #expect(caps.contains(OriroCapability.location.rawValue))
            #expect(caps.contains(OriroCapability.voiceWake.rawValue))
            #expect(caps.contains(OriroCapability.talk.rawValue))
        }
    }
    ...
    @Test @MainActor func currentCommandsIncludeLocationWhenEnabled() {
        withUserDefaults([
            "node.instanceId": "ios-test",
            "location.enabledMode": OriroLocationMode.whileUsing.rawValue,
        ]) {
            let appModel = NodeAppModel()
            let controller = GatewayConnectionController(appModel: appModel, startDiscovery: false)
            let commands = Set(controller._test_currentCommands())

            #expect(commands.contains(OriroLocationCommand.get.rawValue))
        }
    }
    ...
    @Test @MainActor func currentCommandsExcludeDangerousSystemExecCommands() {
        withUserDefaults([
            ...
            #expect(commands.contains(OriroSystemCommand.notify.rawValue))
            #expect(!commands.contains(OriroSystemCommand.run.rawValue))
            #expect(!commands.contains(OriroSystemCommand.which.rawValue))
            #expect(!commands.contains(OriroSystemCommand.execApprovalsGet.rawValue))
            #expect(!commands.contains(OriroSystemCommand.execApprovalsSet.rawValue))
        }
    }
    ...
    @Test @MainActor func loadLastConnectionReadsSavedValues() {
        let prior = KeychainStore.loadString(service: "ai.orirofoundation.app.gateway", account: "lastConnection")
        defer {
            if let prior {
                _ = KeychainStore.saveString(
                    prior,
                    service: "ai.orirofoundation.app.gateway",
                    account: "lastConnection")
            } else {
                _ = KeychainStore.delete(service: "ai.orirofoundation.app.gateway", account: "lastConnection")
            }
        }
        _ = KeychainStore.delete(service: "ai.orirofoundation.app.gateway", account: "lastConnection")
