---
summary: "Host Oriro on Oracle Cloud's Always Free ARM tier"
read_when:
  - Setting up Oriro on Oracle Cloud
  - Looking for free VPS hosting for Oriro
  - Want 24/7 Oriro on a small server
title: "Oracle Cloud"
---

Run a persistent Oriro Gateway on Oracle Cloud's **Always Free** ARM tier (up to 4 OCPU, 24 GB RAM, 200 GB storage) at no cost.
...
<Steps>
  ...
  <Step title="Configure user and hostname">
    ```bash
    sudo hostnamectl set-hostname oriro
    sudo passwd ubuntu
  ...
  <Step title="Install Tailscale">
    ...
    sudo tailscale up --ssh --hostname=oriro
    ```

    From now on, connect via Tailscale: `ssh ubuntu@oriro`.
  ...
  <Step title="Configure the gateway">
    ...
    ```bash
    oriro config set gateway.bind loopback
    oriro config set gateway.auth.mode token
    oriro doctor --generate-gateway-token
    oriro config set gateway.tailscale.mode serve
    oriro config set gateway.trustedProxies '["127.0.0.1"]'
  ...
  <Step title="Verify">
    ...
    ```
    https://oriro.<tailnet-name>.ts.net/
    ```
...
- `chmod 700 ~/.oriro` to restrict credential file permissions.
- `oriro security audit` for an Oriro-specific posture check.
...
The Always Free tier is ARM (`aarch64`). Most Oriro features work fine; a small number of native binaries need ARM builds:
...
- `~/.oriro/` — `oriro.json`, per-agent `auth-profiles.json`, channel/provider state, and session data.
- `~/.oriro/workspace/` — the agent workspace (SOUL.md, memory, artifacts).
...
```bash
ssh -L 18789:127.0.0.1:18789 ubuntu@oriro
```
...
**Tailscale will not connect** -- Run `sudo tailscale up --ssh --hostname=oriro --reset` to re-authenticate.

**Gateway will not start** -- Run `oriro doctor --non-interactive` and check logs with `journalctl --user -u oriro-gateway.service -n 50`.
