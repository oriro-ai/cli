...
read_when:
  - You want to place an outbound voice call from Oriro
...
Voice calls for Oriro via a plugin. Supports outbound notifications,
...
<Steps>
  <Step title="Install the plugin">
    <Tabs>
      <Tab title="From npm">
        ```bash
        oriro plugins install @oriro/voice-call
        ```
      </Tab>
...
<AccordionGroup>
  ...
  <Accordion title="Legacy config migrations">
    ...
    `streaming.*` OpenAI keys are rewritten by `oriro doctor --fix`.
    Runtime fallback still accepts the old voice-call keys for now, but
    the rewrite path is `oriro doctor --fix` and the compat shim is
    temporary.
...
- Voice Call exposes the shared `oriro_agent_consult` realtime tool by default. The realtime model can call it when the caller asks for deeper reasoning, current information, or normal Oriro tools.
- `realtime.consultPolicy` optionally adds guidance for when the realtime model should call `oriro_agent_consult`.
...
configured Oriro agent without paying a full agent-consult round trip on
...
`oriro_agent_consult` still run the full Oriro agent and should be used
          ...
          "voice-call": {
            config: {
              ...
              realtime: {
                enabled: true,
                provider: "google",
                instructions: "Speak briefly. Call oriro_agent_consult before using deeper tools.",
                toolPolicy: "safe-read-only",
                consultPolicy: "substantive",
...
- Legacy `tts.<provider>` keys inside plugin config (`openai`, `elevenlabs`, `microsoft`, `edge`) are repaired by `oriro doctor --fix`; committed config should use `tts.providers.<provider>`.
...
oriro voicecall start --to "+15555550123"   # alias for call
oriro voicecall continue --call-id <id> --message "Any questions?"
oriro voicecall speak --call-id <id> --message "One moment"
oriro voicecall dtmf --call-id <id> --digits "ww123456#"
oriro voicecall end --call-id <id>
oriro voicecall status --call-id <id>
oriro voicecall tail
oriro voicecall latency                      # summarize turn latency from logs
oriro voicecall expose --mode funnel
```
...
Provider signatures are checked against the public URL Oriro reconstructs
...
Use `oriro logs --follow` for the live phase trace. A healthy Twilio Meet
...
`oriro voicecall tail` still shows persisted call records; it is useful for
...
- `oriro logs --follow` shows realtime TwiML served, the realtime bridge
