...
import { closeActiveMemorySearchManager } from "oriro/plugin-sdk/memory-host-search";
import {
  asDateTimestampMs,
...
} from "oriro/plugin-sdk/plugin-config-runtime";
import { definePluginEntry, type OriroPluginApi } from "oriro/plugin-sdk/plugin-entry";
import { parseAgentSessionKey, parseThreadSessionSuffix } from "oriro/plugin-sdk/routing";
import { isPathInside } from "oriro/plugin-sdk/security-runtime";
import {
...
import { tempWorkspace, resolvePreferredOriroTmpDir } from "oriro/plugin-sdk/temp-path";
...
function resolveDefaultToolsAllow(cfg: OriroConfig | undefined): string[] {
  return cfg?.plugins?.slots?.memory === "memory-lancedb"
    ? [...LANCEDB_ACTIVE_MEMORY_TOOLS_ALLOW]
    : [...DEFAULT_ACTIVE_MEMORY_TOOLS_ALLOW];
}

function resolveToolsAllow(params: { pluginToolsAllow: unknown; cfg?: OriroConfig }): string[] {
  return (
...
function resolvePersistentTranscriptBaseDir(api: OriroPluginApi, agentId: string): string {
  return path.join(
...
function openActiveMemoryToggleStore(api: OriroPluginApi) {
...
function isActiveMemoryGloballyEnabled(cfg: OriroConfig): boolean {
...
function resolveActiveMemoryCleanupConfig(api: OriroPluginApi): OriroConfig | undefined {
  try {
    return (
      (api.runtime.config?.current?.() as OriroConfig | undefined) ??
      (api.config as OriroConfig | undefined)
    );
  } catch {
    return api.config as OriroConfig | undefined;
  }
}
...
export default definePluginEntry({
  ...
  register(api: OriroPluginApi) {
    const readCurrentConfig = (): OriroConfig | undefined => {
      try {
        return (
          (api.runtime.config?.current?.() as OriroConfig | undefined) ??
          (api.config as OriroConfig | undefined)
    ...
    api.registerCommand({
      ...
      handler: async (ctx) => {
        ...
        if (isGlobal) {
          const currentConfig = api.runtime.config.current() as OriroConfig;
          if (action === "status") {
            return {
