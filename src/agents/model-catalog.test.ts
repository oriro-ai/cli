...
let prepareOriroModelsJsonSourceMock: ReturnType<typeof vi.fn>;
...
describe("loadModelCatalog", () => {
  beforeAll(async () => {
    ...
    prepareOriroModelsJsonSourceMock = vi.fn().mockResolvedValue({
      agentDir: "/tmp/oriro",
    ...
    vi.doMock("./models-config.js", () => ({
      ...
      prepareOriroModelsJsonSource: prepareOriroModelsJsonSourceMock,
    }));
    ...
    vi.doMock("./agent-scope.js", () => ({
      resolveAgentWorkspaceDir: (cfg: OriroConfig, agentId: string) => {
        const entry = cfg.agents?.list?.find((entryEntry) => entryEntry.id === agentId);
        return entry?.workspace ?? cfg.agents?.defaults?.workspace ?? "/tmp/oriro-workspace";
      },
      resolveDefaultAgentDir: () => "/tmp/oriro",
      resolveDefaultAgentId: (cfg: OriroConfig) =>
  ...
  beforeEach(() => {
    ...
    prepareOriroModelsJsonSourceMock.mockReset();
    prepareOriroModelsJsonSourceMock.mockResolvedValue({
      agentDir: "/tmp/oriro",
      fingerprint: "source-fingerprint",
      workspaceDir: "/tmp/oriro-workspace",
      wrote: false,
  ...
  it("uses the state cached catalog before runtime discovery", async () => {
    ...
    const result = await loadModelCatalog({ config: {} as OriroConfig });
    ...
    expect(prepareOriroModelsJsonSourceMock).not.toHaveBeenCalled();
  ...
  it("bypasses the state cached catalog when a refresh is requested", async () => {
    ...
    const result = await loadModelCatalog({ config: {} as OriroConfig, useCache: false });
  ...
  it("does not publish a catalog load from an invalidated generation", async () => {
    ...
    const staleLoad = loadModelCatalog({ config: {} as OriroConfig });
    resetModelCatalogCache();
    await expect(loadModelCatalog({ config: {} as OriroConfig })).resolves.toBe(freshCatalog);
  ...
  it("misses the state cached catalog when source freshness changes", async () => {
    ...
    await expect(loadModelCatalog({ config: {} as OriroConfig })).resolves.toEqual([
      { id: "cached-stale", name: "Cached Stale", provider: "openai" },
    ]);
    resetModelCatalogCacheForTest();
    mockAgentDiscoveryModels([{ id: "fresh-fast", name: "Fresh Fast", provider: "openai" }]);
  ...
  it("reloads dynamic registry entries after clearing the cache", async () => {
    ...
    const first = await loadModelCatalog({ config: {} as OriroConfig });
    expect(first).toStrictEqual([
      {
        id: "existing",
        name: "Existing",
    ...
    const second = await loadModelCatalog({ config: {} as OriroConfig });
  ...
  it("filters suppressed built-ins from persisted read-only catalog rows", async () => {
    ...
    const result = await loadModelCatalog({ config: {} as OriroConfig, readOnly: true });
  ...
  it("loads generated plugin catalog rows in read-only mode", async () => {
    const catalogPath = "/tmp/oriro/plugins/read-only-shard/catalog.json";
    mkdirSync("/tmp/oriro/plugins/read-only-shard", { recursive: true });
    ...
    try {
      ...
      rmSync("/tmp/oriro/plugins/read-only-shard", { recursive: true, force: true });
    }
  });
  ...
  it("loads manifest catalog rows from the current metadata snapshot without provider runtime", () => {
    ...
    const result = loadManifestModelCatalog({ config: {} as OriroConfig });
  ...
  it("passes explicit env when checking current manifest catalog snapshot compatibility", () => {
    const env = { HOME: "/tmp/oriro-model-catalog-env" } as NodeJS.ProcessEnv;
