...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(
    path.join(fs.realpathSync(os.tmpdir()), "oriro-doctor-contract-load-paths-"),
  );
  tempDirs.push(dir);
  return dir;
}
...
function createDoctorPluginConfig(pluginRoot: string, pluginId: string): OriroConfig {
  return {
    plugins: {
...
function readPluginLlmPolicy(config: OriroConfig, pluginId: string): Record<string, unknown> {
