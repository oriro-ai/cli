...
const testConfigStore = new Map<string, OriroConfig>();
...
function resolveTestConfigPath() {
  const override = process.env.ORIRO_CONFIG_PATH?.trim();
  if (override) {
    return override;
  }
  const stateDir = process.env.ORIRO_STATE_DIR?.trim();
  if (!stateDir) {
    throw new Error("ORIRO_STATE_DIR must be set before config IO in this test");
  }
  return path.join(stateDir, "oriro.json");
}
...
function readTestConfig<T = OriroConfig>(): T {
...
vi.mock("../config/config.js", () => ({
  ...
  resolveGatewayPort: (cfg: OriroConfig) => cfg.gateway?.port ?? 18789,
}));
...
vi.mock("./onboard-helpers.js", () => {
  ...
  return {
    DEFAULT_WORKSPACE: "/tmp/oriro-workspace",
    ...
    runNonInteractiveSetup(
      {
        nonInteractive: true,
        mode: "local",
        workspace: path.join(stateDir, "oriro"),
        authChoice: "skip",
        skipSkills: true,
        skipHealth: false,
        installDaemon: true,
...
describe("onboard (non-interactive): gateway and remote auth", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("oriro-onboard-");
  ...
  it("preserves existing agents.list and bindings on onboard rerun (oriro#84692)", async () => {
    await withStateDir("state-preserve-agents-", async (stateDir) => {
      const workspace = path.join(stateDir, "oriro");
      const seededAgents = [
  ...
  it("preserves existing agents.list and bindings on remote onboard rerun (oriro#84692)", async () => {
  ...
  it("emits structured JSON diagnostics when daemon health fails", async () => {
    await withStateDir("state-local-daemon-health-json-fail-", async (stateDir) => {
      ...
      expect(parsed.hints).toContain("Run `oriro gateway status --deep` for more detail.");
  ...
  it("classifies daemon health ECONNREFUSED failures with a recovery command", async () => {
    await withStateDir("state-local-daemon-health-refused-", async (stateDir) => {
      ...
      expect(parsed.hints).toContain("Fix: run `oriro gateway restart`.");
    });
  }, 60_000);
  ...
  it("auto-generates token auth when binding LAN and persists the token", async () => {
    ...
    await withStateDir("state-lan-", async (stateDir) => {
      process.env.ORIRO_STATE_DIR = stateDir;
      process.env.ORIRO_CONFIG_PATH = path.join(stateDir, "oriro.json");
