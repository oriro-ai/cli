...
const TMUX_CWD_ENV_KEY = "ORIRO_GATEWAY_WATCH_CWD";
const TMUX_CWD_OPTION_KEY = "@oriro.gateway_watch.cwd";
const TMUX_CHILD_ENV_KEYS = [
  "NODE_OPTIONS",
  "ORIRO_CONFIG_PATH",
  "ORIRO_DIAGNOSTICS",
  "ORIRO_DIAGNOSTICS_EVENT_LOOP",
  "ORIRO_DIAGNOSTICS_TIMELINE_PATH",
  "ORIRO_GATEWAY_PORT",
  "ORIRO_GATEWAY_RESTART_TRACE",
  "ORIRO_GATEWAY_STARTUP_TRACE",
  "ORIRO_HOME",
  "ORIRO_PROFILE",
...
const resolveGatewayWatchBenchmarkArgs = ({ args = [], env = process.env } = {}) => {
  ...
  if (benchmarkFlagSeen) {
    ...
    nextEnv.ORIRO_TRACE_SYNC_IO ??= "0";
    if (nextEnv.ORIRO_TRACE_SYNC_IO === "1") {
...
export const resolveGatewayWatchTmuxSessionName = ({ args = [], env = process.env } = {}) => {
  ...
  const port = env.ORIRO_GATEWAY_PORT || readArgValue(args, "--port");
  const parts = [
    "oriro",
    "gateway",
    "watch",
...
export const buildGatewayWatchTmuxCommand = ({
  ...
  const childEnv = [
    "env",
    ...colorEnv.options,
    `ORIRO_GATEWAY_WATCH_TMUX_CHILD=1`,
    `ORIRO_GATEWAY_WATCH_SESSION=${sessionName}`,
...
const shouldAttachTmux = ({ env, stdinIsTTY, stdoutIsTTY }) => {
  const raw = String(env.ORIRO_GATEWAY_WATCH_ATTACH ?? "").toLowerCase();
...
export const runGatewayWatchTmuxMain = (params = {}) => {
  ...
  if (TMUX_DISABLE_VALUES.has((deps.env.ORIRO_GATEWAY_WATCH_TMUX ?? "").toLowerCase())) {
  ...
  if (deps.env.ORIRO_GATEWAY_WATCH_TMUX_CHILD === "1") {
    return runForegroundWatcher({
      args: deps.args,
      cwd: deps.cwd,
      env: deps.env,
