// Applies persisted state migrations across Oriro config files.
...
import {
  detectOriroStateDatabaseSchemaMigrations,
  repairOriroStateDatabaseSchema,
...
type LegacyPluginStateImportDatabase = Pick<OriroStateKyselyDatabase, "plugin_state_entries">;
type LegacyVoiceWakeImportDatabase = Pick<
  OriroStateKyselyDatabase,
  "voicewake_routing_config" | "voicewake_routing_routes" | "voicewake_triggers"
>;
type LegacyUpdateCheckImportDatabase = Pick<OriroStateKyselyDatabase, "update_check_state">;
type LegacyConfigHealthImportDatabase = Pick<OriroStateKyselyDatabase, "config_health_entries">;
...
function resolveDefaultExecApprovalsStateDir(
  ...
  return path.join(resolveRequiredHomeDir(env, homedir), ".oriro");
}
...
function detectLegacyExecApprovalsMigration(params: {
  ...
  return {
    ...
    targetPath,
    hasLegacy:
      Boolean(params.env.ORIRO_STATE_DIR?.trim()) &&
...
async function migrateLegacyTaskRunsSidecar(params: {
  ...
  try {
    ...
    runOriroStateWriteTransaction(
      ...
      { env: { ...process.env, ORIRO_STATE_DIR: params.stateDir } },
    );
    if (importedTasks > 0) {
      changes.push(
...
const DEFAULT_VOICEWAKE_TRIGGERS = ["oriro", "claude", "computer"];
...
function migrateLegacyVoiceWakeSettings(params: {
  ...
  const changes: string[] = [];
  const warnings: string[] = [];
  const env = { ...process.env, ORIRO_STATE_DIR: params.stateDir };
...
async function withPluginStateImportEnv<T>(
  ...
  process.env.ORIRO_STATE_DIR = plan.stateDir;
  try {
    return await run();
  } finally {
...
export async function autoMigrateLegacyStateDir(params: {
  ...
  const warnings: string[] = [];
  const changes: string[] = [];
  const hasCustomStateDir = Boolean(env.ORIRO_STATE_DIR?.trim());
  ...
  try {
    ...
    try {
      ...
      try {
        ...
        warnings.push(
          `Rollback failed; set ORIRO_STATE_DIR=${targetDir} to avoid split state: ${String(rollbackErr)}`,
        );
...
export async function detectLegacyStateMigrations(params: {
  ...
  const stateSchemaMigrations = detectOriroStateDatabaseSchemaMigrations({
    env: { ...env, ORIRO_STATE_DIR: stateDir },
  });
...
async function migrateLegacySessions(
  ...
  if (!targetReadable) {
    if (options.recoverCorruptTargetStore) {
      ...
      warnings.push(
        `Target sessions store unreadable; left untouched to avoid overwriting at ${detected.sessions.targetStorePath}. Run oriro doctor --fix to archive it and retry the legacy merge.`,
      );
    }
  }
...
function migrateLegacyStateSchema(detected: LegacyStateDetection): {
  ...
  return repairOriroStateDatabaseSchema({
    env: { ...process.env, ORIRO_STATE_DIR: detected.stateDir },
  });
}
...
export async function autoMigrateLegacyState(params: {
  ...
  const stateSchema = repairOriroStateDatabaseSchema({
    env: { ...env, ORIRO_STATE_DIR: stateDir },
  });
  ...
  const hasCustomAgentDir = env.ORIRO_AGENT_DIR?.trim() || env.PI_CODING_AGENT_DIR?.trim();
