...
function listAgents(cfg: OriroConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

type AgentLookupCache = {
  agentsRef: OriroConfig["agents"] | undefined;
  byNormalizedId: Map<string, string>;
  fallbackDefaultAgentId: string;
};

const agentLookupCacheByCfg = new WeakMap<OriroConfig, AgentLookupCache>();

function resolveAgentLookupCache(cfg: OriroConfig): AgentLookupCache {
...
export function pickFirstExistingAgentId(cfg: OriroConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: OriroConfig["bindings"];
...
const evaluatedBindingsCacheByCfg = new WeakMap<OriroConfig, EvaluatedBindingsCache>();
const MAX_EVALUATED_BINDINGS_CACHE_KEYS = 2000;
const resolvedRouteCacheByCfg = new WeakMap<
  OriroConfig,
  {
    bindingsRef: OriroConfig["bindings"];
    agentsRef: OriroConfig["agents"];
    sessionRef: OriroConfig["session"];
...
function resolveRouteCacheForConfig(cfg: OriroConfig): Map<string, ResolvedAgentRoute> {
