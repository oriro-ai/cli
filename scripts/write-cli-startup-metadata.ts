// Write Cli Startup Metadata script supports Oriro repository automation.
...
export function readBundledChannelCatalog(
  ...
  for (const dirEntry of readdirSync(extensionsDirOverride, { withFileTypes: true })) {
    ...
    try {
      ...
      const id = parsed.oriro?.channel?.id;
      if (typeof id !== "string" || !id.trim()) {
        continue;
      }
      const orderRaw = parsed.oriro?.channel?.order;
      const labelRaw = parsed.oriro?.channel?.label;
      entries.push({
        id: id.trim(),
...
function createIsolatedRootHelpRenderContext(
  ...
  const stateDir = path.join(rootDir, ".oriro-build-root-help");
  const workspaceDir = path.join(stateDir, "workspace");
  const homeDir = path.join(stateDir, "home");
  const env: NodeJS.ProcessEnv = {
    HOME: homeDir,
    LOGNAME: process.env.LOGNAME ?? process.env.USER ?? "oriro-build",
    USER: process.env.USER ?? process.env.LOGNAME ?? "oriro-build",
...
async function renderSourceBrowserHelpText(
  ...
  const inlineModule = [
    ...
    `registerBrowserCli(program, ["node", "oriro", "browser", "--help"]);`,
...
async function renderSourceCommandHelpText(
  ...
  return await spawnText(["oriro.mjs", command, "--help"], {
    cwd: rootDir,
    env: {
      ...renderContext.env,
      ORIRO_DISABLE_CLI_STARTUP_HELP_FAST_PATH: "1",
