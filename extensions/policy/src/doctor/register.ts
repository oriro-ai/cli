...
async function evaluatePolicyUncached(ctx: HealthCheckContext): Promise<PolicyEvaluation> {
  ...
  if (
    ...
    findings.push({
      ...
      requirement: "oc://oriro.config/plugins/entries/policy/config/expectedHash",
  ...
  return [
    {
      ...
      requirement: "oc://oriro.config/plugins/entries/policy/config/expectedAttestationHash",
...
function isGroupIngressDisabled(
  ...
  const channelDefaultsParent = "oc://oriro.config/channels/defaults";
    ...
    if (invalidIndex >= 0) {
      findings.push(
        policyShapeFinding(
          ...
          "Use non-empty source names such as env, file, exec, or oriro.",
        ),
      );
    }
  }
...
function resolvePolicyArtifactHomeDir(): string | undefined {
  const explicitHome = normalizedEnvValue(process.env.ORIRO_HOME);
  ...
  return [
    ...new Set(
      findings
        ...
        .map((finding) => finding.ocPath?.match(/^oc:\/\/oriro\.config\/channels\/(.+)$/)?.[1])
...
function canonicalExecApprovalsPath(): string {
  return "~/.oriro/exec-approvals.json";
}
...
function execApprovalsArtifactLocation(ctx: HealthCheckContext): {
  ...
  const stateDir = normalizedEnvValue(process.env.ORIRO_STATE_DIR);
