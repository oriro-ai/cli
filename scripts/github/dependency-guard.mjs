...
export const dependencyChangeMarker = "<!-- oriro:dependency-guard -->";
export const dependencyGraphGuardMarker = "<!-- oriro:dependency-graph-guard -->";
...
export function githubApi(token, options = {}) {
  const api = createGitHubApi(token, { ...options, userAgent: "oriro-dependency-guard" });
  return {
    ...api,
...
async function main() {
  ...
  const autoscrubToken = process.env.ORIRO_DEPENDENCY_GUARD_AUTOSCRUB_TOKEN;
  ...
  const trustedCommentAuthors = dependencyGuardCommentAuthors(
    process.env.ORIRO_DEPENDENCY_GUARD_COMMENT_BOTS,
  );
  ...
  const mode = process.env.ORIRO_DEPENDENCY_GUARD_MODE ?? "enforce";
