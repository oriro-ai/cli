...
read_when:
  - You want to use Cloudflare AI Gateway with Oriro
  - You need the account ID, gateway ID, or API key env var
---

Cloudflare AI Gateway sits in front of provider APIs and lets you add analytics, caching, and controls. For Anthropic, Oriro uses the Anthropic Messages API through your Gateway endpoint.
...
When thinking is enabled for Anthropic Messages models, Oriro strips trailing
...
```bash
oriro plugins install @oriro/cloudflare-ai-gateway-provider
...
<Steps>
  <Step title="Set the provider API key and Gateway details">
    ...
    ```bash
    oriro onboard --auth-choice cloudflare-ai-gateway-api-key
    ```
  ...
  <Step title="Set a default model">
    Add the model to your Oriro config:
  ...
  <Step title="Verify the model is available">
    ```bash
    oriro models list --provider cloudflare-ai-gateway
    ```
  </Step>
</Steps>
...
<AccordionGroup>
  ...
  <Accordion title="Environment note">
    ...
    <Warning>
    A key exported only in an interactive shell will not help a launchd/systemd daemon unless that environment is imported there as well. Set the key in `~/.oriro/.env` or via `env.shellEnv` to ensure the gateway process can read it.
    </Warning>
