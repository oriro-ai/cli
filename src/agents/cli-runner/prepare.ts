...
const prepareDeps = {
  ...
  resolveOriroReferencePaths: async (
    params: Parameters<typeof import("../docs-path.js").resolveOriroReferencePaths>[0],
  ) => (await import("../docs-path.js")).resolveOriroReferencePaths(params),
...
export async function prepareCliRunContext(
  ...
  const preparedBackend = await prepareCliBundleMcpConfig({
    ...
    env: mcpLoopbackRuntime
      ? {
          ORIRO_MCP_TOKEN: prepareDeps.resolveMcpLoopbackBearerToken(
            mcpLoopbackRuntime,
            params.senderIsOwner === true,
          ),
          ORIRO_MCP_AGENT_ID: sessionAgentId ?? "",
          ORIRO_MCP_ACCOUNT_ID: params.agentAccountId ?? "",
          ORIRO_MCP_SESSION_KEY: params.sessionKey ?? "",
          ORIRO_MCP_SESSION_ID: params.sessionId,
          ORIRO_MCP_MESSAGE_CHANNEL: params.messageChannel ?? params.messageProvider ?? "",
          ORIRO_MCP_CURRENT_CHANNEL_ID: params.currentChannelId ?? "",
          ORIRO_MCP_CURRENT_THREAD_TS: params.currentThreadTs ?? "",
          ORIRO_MCP_CURRENT_MESSAGE_ID:
            params.currentMessageId != null ? String(params.currentMessageId) : "",
          ORIRO_MCP_CURRENT_INBOUND_AUDIO: params.currentInboundAudio === true ? "true" : "",
          ORIRO_MCP_INBOUND_EVENT_KIND: params.currentInboundEventKind ?? "",
          ORIRO_MCP_SOURCE_REPLY_DELIVERY_MODE: params.sourceReplyDeliveryMode ?? "",
          ORIRO_MCP_REQUIRE_EXPLICIT_MESSAGE_TARGET: requireExplicitMessageTarget ? "true" : "",
  ...
  if (!isSideQuestion) {
    ...
    try {
      const hookResult = await resolvePromptBuildHookResult({
        ...
        messages: await loadOriroHistoryMessages(),
        hookCtx: {
          runId: params.runId,
          agentId: sessionAgentId,
          sessionKey: params.sessionKey,
