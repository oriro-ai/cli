...
final class RealtimeTalkRelaySession {
    private static let agentControlToolName = "oriro_agent_control"
    ...
    private let logger = Logger(subsystem: "ai.orirofoundation.app", category: "RealtimeTalkRelay")
    ...
    private func handleToolCall(_ payload: [String: AnyCodable]) async {
        ...
        do {
            ...
            let result: [String: Any] = completion.failed
                ? ["error": "Oriro tool call failed"]
                : ["text": completion.text ?? "Oriro finished with no text."]
                ...
                for await event in stream {
                    ...
                    if chatEvent.state == "final" {
                        return ChatCompletionResult(
                            text: OriroChatEventText.assistantText(from: chatEvent),
                            failed: false)
                    }
