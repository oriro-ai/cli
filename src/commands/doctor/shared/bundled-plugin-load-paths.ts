...
function resolveBundledWorkspaceDir(cfg: OriroConfig): string | undefined {
  return resolveAgentWorkspaceDir(cfg, resolveDefaultAgentId(cfg)) ?? undefined;
}

function isOriroNodeModulesPackageRoot(packageRoot: string): boolean {
  ...
  return packageDir === "oriro" && parentDir === "node_modules";
}

/** Find configured plugin load paths that alias bundled plugins already shipped by Oriro. */
  ...
  for (const rawPath of rawPaths) {
    ...
    if (!match) {
      ...
      const oldPackageMatch =
        // Only rewrite paths rooted in the installed Oriro package; user plugin paths stay intact.
        oldPackageRoot && oldBundledLeaf && isOriroNodeModulesPackageRoot(oldPackageRoot)
...
export function collectBundledPluginLoadPathWarnings(params: {
  ...
  const lines = params.hits.map(
    (hit) =>
      `- ${hit.pathLabel}: bundled plugin path "${hit.fromPath}" still aliases ${hit.pluginId}; ORIRO loads the packaged bundled plugin from "${hit.toPath}".`,
  );
