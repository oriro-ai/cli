...
read_when:
  - You want to use Volcano Engine or Doubao models with Oriro
...
<Steps>
  <Step title="Set the API key">
    ...
    ```bash
    oriro onboard --auth-choice volcengine-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    oriro models list --provider volcengine
    oriro models list --provider volcengine-plan
    ```
  </Step>
</Steps>
...
For voice-note targets, Oriro asks Volcengine for provider-native
...
<AccordionGroup>
  <Accordion title="Default model after onboarding">
    `oriro onboard --auth-choice volcengine-api-key` currently sets
  ...
  <Accordion title="Model picker fallback behavior">
    ...
    loaded yet, Oriro falls back to the unfiltered catalog instead of showing an
  ...
  <Accordion title="Environment variables for daemon processes">
    ...
    `~/.oriro/.env` or via `env.shellEnv`).
  </Accordion>
</AccordionGroup>

<Warning>
When running Oriro as a background service, environment variables set in your
...
<CardGroup cols={2}>
  ...
  <Card title="FAQ" href="/help/faq" icon="circle-question">
    Frequently asked questions about Oriro setup.
  </Card>
</CardGroup>
