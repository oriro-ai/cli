...
describe("normalizePluginsConfig", () => {
  ...
  it.each([
    [{}, undefined],
    [{ slots: { contextEngine: "lossless-oriro" } }, "lossless-oriro"],
  ...
  it("normalizes unknown plugin ids without consulting discovery", async () => {
    const discoverPlugins = vi.spyOn(discovery, "discoverOriroPlugins");
  ...
  it("does not consult discovery or manifests for alias lookup", async () => {
    const discoverPlugins = vi.spyOn(discovery, "discoverOriroPlugins").mockReturnValue({
      candidates: [
        {
          idHint: "anthropic",
          source: "/tmp/oriro-bundled-anthropic/index.js",
          rootDir: "/tmp/oriro-bundled-anthropic",
          origin: "bundled",
          bundledManifest: {
            id: "anthropic",
            configSchema: {},
        ...
        },
        {
          ...
          source: "/tmp/oriro-global-anthropic/index.js",
          rootDir: "/tmp/oriro-global-anthropic",
          origin: "global",
        },
      ],
      diagnostics: [],
    ...
    const loadManifest = vi.spyOn(manifest, "loadPluginManifest").mockReturnValue({
      ok: true,
      manifestPath: "/tmp/oriro-global-anthropic/oriro.plugin.json",
      manifest: {
...
describe("resolveEnableState", () => {
  ...
  it("keeps an explicitly selected workspace context engine enabled when omitted from plugins.allow", () => {
    expectNormalizedEnableState({
      ...
      config: {
        allow: ["telegram"],
        slots: { contextEngine: "lossless-oriro" },
      },
      expected: {
        enabled: true,
      },
