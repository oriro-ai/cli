        ...
        resolvedSkills: [
          {
            ...
            filePath: "/tmp/oriro-old-package/skills/legacy-heavy-skill-cache/SKILL.md",
          },
        ],
      },
    },
...
function getScenario() {
  const scenario = process.env.ORIRO_UPGRADE_SURVIVOR_SCENARIO || "base";
  assert(SCENARIOS.has(scenario), `unknown upgrade survivor scenario: ${scenario}`);
  return scenario;
}

function getConfig() {
  return readJson(requireEnv("ORIRO_CONFIG_PATH"));
}

function getCoverage() {
  const file = process.env.ORIRO_UPGRADE_SURVIVOR_CONFIG_COVERAGE_JSON;
...
function seedState() {
  const stateDir = requireEnv("ORIRO_STATE_DIR");
  const workspace = requireEnv("ORIRO_TEST_WORKSPACE_DIR");
  ...
  for (const plugin of ["discord", "telegram", "whatsapp"]) {
    writeJson(path.join(runtimeRoot, plugin, ".oriro-runtime-deps-stamp.json"), {
      version: 0,
      plugin,
      stale: true,
    });
  ...
  if (scenario === "versioned-runtime-deps") {
    const version = process.env.ORIRO_UPGRADE_SURVIVOR_BASELINE_VERSION || "2026.4.24";
    for (const plugin of ["discord", "feishu", "telegram", "whatsapp"]) {
      writeJson(
        path.join(
          runtimeRoot,
          `oriro-${version}-${plugin}`,
          ".oriro-runtime-deps-stamp.json",
        ),
        {
...
function assertConfigSurvived() {
  ...
  if (hasCoverage(coverage) && acceptsIntent(coverage, "logging")) {
    assert(
      config.logging?.file === "~/oriro-upgrade-survivor/gateway.jsonl",
...
function assertStateSurvived() {
  ...
  const workspace = requireEnv("ORIRO_TEST_WORKSPACE_DIR");
  const scenario = getScenario();
  const stage = process.env.ORIRO_UPGRADE_SURVIVOR_ASSERT_STAGE || "survival";
  ...
  if (scenario === "stale-source-plugin-shadow") {
    const staleRoot = path.join(stateDir, "extensions", "opik-oriro");
    assert(
  ...
  if (scenario === "versioned-runtime-deps") {
    ...
    const staleVersionedRoots = fs.existsSync(runtimeRoot)
      ? fs.readdirSync(runtimeRoot).filter((entry) => entry.startsWith(`oriro-${version}-`))
      : [];
    assert(
...
function readMigratedSessionStore(stateDir, targetStorePath) {
  ...
  const dbPath = path.join(stateDir, "agents", "main", "agent", "oriro-agent.sqlite");
...
function assertExternalPluginInstall(records, pluginId, packageName) {
  ...
  const installedFromOfficialOriroHubNpmPack =
    record.source === "orirohub" &&
    record.orirohubChannel === "official" &&
    record.artifactKind === "npm-pack";
  assert(
    installedFromNpm || installedFromOfficialOriroHubNpmPack,
    `configured external ${pluginId} plugin must be installed from npm or official OriroHub npm-pack, got: ${record.source}`,
  );
  ...
  assert(
    record.orirohubPackage === packageName,
    `configured external ${pluginId} OriroHub package changed: ${record.orirohubPackage}`,
  );
  const extensionsRoot = path.join(requireEnv("ORIRO_STATE_DIR"), "extensions");
  assert(
    isPathInside(extensionsRoot, installPath),
    `configured external ${pluginId} OriroHub install path outside managed extensions root: ${installPath}`,
  );
}
...
function assertConfiguredPluginInstalls() {
  ...
  assertOptionalConfiguredPluginIndex(records, index.plugins ?? [], {
    packageName: "@oriro/brave-plugin",
    pluginId: "brave",
  });
