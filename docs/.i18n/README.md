...
- repo: `oriro/docs`
- local checkout: `~/Projects/oriro-docs`

## Source of truth

- English docs are authored in `oriro-ai/cli`.
...
1. Edit English docs in `oriro-ai/cli`.
2. Push to `main`.
3. `oriro-ai/cli/.github/workflows/docs-sync-publish.yml` mirrors the docs tree into `oriro/docs`.
...
5. `oriro/docs/.github/workflows/translate-all.yml` waits for `main` to settle, translates only stale or missing locale pages, and uploads per-locale artifacts.
...
- Docs translation workflows generate the same non-English locale set in `oriro/docs`.
...
- Do not treat missing `th` or `fa` entries in generated `docs/docs.json` as a pipeline failure. Verify their generated folders in `oriro/docs` instead.
...
- Sync metadata is written to `.oriro-sync/source.json` in the publish repo.
- Source repo secret: `ORIRO_DOCS_SYNC_TOKEN`
- Publish repo secret: `ORIRO_DOCS_I18N_OPENAI_API_KEY`
- If locale output looks stale, check the `Translate All` workflow in `oriro/docs` first.
