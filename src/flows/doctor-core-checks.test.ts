...
function createSkill(overrides: Partial<SkillStatusEntry> = {}): SkillStatusEntry {
  return {
    ...
    source: "workspace",
    bundled: false,
    filePath: "/tmp/oriro-test-workspace/skills/missing-tool/SKILL.md",
    baseDir: "/tmp/oriro-test-workspace/skills/missing-tool",
    ...
    requirements: {
      bins: ["oriro-test-missing-skill-bin"],
      anyBins: [],
      env: [],
      config: [],
      os: [],
...
describe("CORE_HEALTH_CHECKS", () => {
  ...
  it("converts unavailable skills into repair-capable health findings", async () => {
    ...
    const cfg: OriroConfig = {
      agents: {
        defaults: {
          workspace: "/tmp/oriro-test-workspace",
          skills: ["missing-tool"],
        },
      },
    };
  ...
  it("skips gateway auth warning when SecretRef-managed token resolves in lint checks", async () => {
    ...
    await withEnvAsync({ ORIRO_TEST_GATEWAY_TOKEN: "resolved-test-token" }, async () => {
      const findings = await check?.detect({
        mode: "lint",
  ...
  it("reports unresolved SecretRefs even when ORIRO_GATEWAY_TOKEN is set", async () => {
    const check = CORE_HEALTH_CHECKS.find((entry) => entry.id === "core/doctor/gateway-auth");
    await withEnvAsync(
      {
        ORIRO_GATEWAY_TOKEN: "fallback-token",
        ORIRO_MISSING_GATEWAY_REF_TOKEN: undefined,
      },
      async () => {
        const findings = await check?.detect({
          mode: "lint",
  ...
  it("does not execute or warn for valid exec SecretRefs during default gateway auth lint checks", async () => {
    tmp = await fs.mkdtemp(join(tmpdir(), "oriro-health-exec-ref-"));
  ...
  it("reports exec SecretRef failures when gateway auth lint explicitly allows exec checks", async () => {
    ...
    const findings = await withEnvAsync({ ORIRO_GATEWAY_TOKEN: "fallback-token" }, async () => {
    ...
    expect(findings).toContainEqual(
      expect.objectContaining({
        ...
        message: expect.stringContaining("Gateway token SecretRef could not be resolved:"),
        fixHint:
          "Run `oriro doctor --allow-exec` to verify exec SecretRefs during doctor, or `oriro secrets audit --allow-exec` to audit all exec SecretRefs.",
      }),
    );
  });
