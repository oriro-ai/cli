...
function resolveControlUiBuildId(): string {
  const explicit =
    process.env.ORIRO_CONTROL_UI_BUILD_ID?.trim() || process.env.ORIRO_VERSION?.trim();
  if (explicit) {
...
function controlUiServiceWorkerBuildIdPlugin(buildId: string): Plugin {
  return {
    ...
    closeBundle() {
      ...
      const placeholder = '"__ORIRO_CONTROL_UI_BUILD_ID__"';
...
export default function controlUiViteConfig(): UserConfig {
  const envBase = process.env.ORIRO_CONTROL_UI_BASE_PATH?.trim();
  ...
  return {
    base,
    define: {
      ORIRO_CONTROL_UI_BUILD_ID: JSON.stringify(controlUiBuildId),
    },
