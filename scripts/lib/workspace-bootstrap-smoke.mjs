// Verifies installed packages can bootstrap the default Oriro workspace files.
...
export function runInstalledWorkspaceBootstrapSmoke(params) {
  const tempRoot = mkdtempSync(join(tmpdir(), "oriro-workspace-bootstrap-smoke-"));
  ...
  try {
    try {
      execFileSync(
        process.execPath,
        [
          join(params.packageRoot, "oriro.mjs"),
          "agent",
          "--message",
    ...
    const workspaceDir = join(homeDir, ".oriro", "workspace");
