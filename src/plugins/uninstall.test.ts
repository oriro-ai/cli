...
type PluginConfig = NonNullable<OriroConfig["plugins"]>;
...
function createSinglePluginWithEmptySlotsConfig(): OriroConfig {
...
function createSingleNpmInstallConfig(installPath: string): OriroConfig {
  return createPluginConfig({
    entries: createSinglePluginEntries(),
    installs: {
...
describe("removePluginFromConfig", () => {
  ...
  it("removes absolute load path for a workspace-relative install source path", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(tempRoot, "oriro-uninstall-portable-source-"));
    try {
    ...
    },
    {
      ...
      config: createPluginConfig({
        ...
        channels: {
          ...
          timbot: { sdkAppId: "123" },
        } as unknown as OriroConfig["channels"],
      }),
      pluginId: "timbot",
      expectedChannels: {
        defaults: { groupPolicy: "opt-in" },
...
describe("uninstallPlugin", () => {
      ...
      config: createPluginConfig({
        installs: {
          "missing-linked-plugin": createPathInstallRecord(
            "/missing/oriro/plugin",
            "/missing/oriro/plugin",
          ),
        },
        loadPaths: ["/missing/oriro/plugin", "/keep/this/plugin"],
      }),
      expectedActions: {
        entry: false,
        install: true,
  ...
  it("uninstalls npm-managed packages through npm before deleting the package directory", async () => {
    ...
    const pluginDir = path.join(npmRoot, "node_modules", "@oriro", "kitchen-sink");
    ...
    const plan = planPluginUninstall({
      config: createPluginConfig({
        entries: createSinglePluginEntries("oriro-kitchen-sink-fixture"),
        installs: {
          "oriro-kitchen-sink-fixture": {
            source: "npm",
            spec: "@oriro/kitchen-sink@1.0.0",
            installPath: pluginDir,
          },
        },
      }),
    ...
    expect(plan.directoryRemoval).toEqual({
      ...
      cleanup: {
        kind: "npm",
        npmRoot,
        packageName: "@oriro/kitchen-sink",
      },
    });
    ...
    expect(applied).toEqual({ directoryRemoved: true, warnings: [] });
    expectNpmUninstallCommand({ packageName: "@oriro/kitchen-sink", npmRoot });
    await expectPathAccessState(pluginDir, "missing");
  });
  ...
  it("repairs remaining npm plugin oriro peer links after npm uninstall prunes them", async () => {
    const stateDir = path.join(tempDir, "state");
    const npmRoot = path.join(stateDir, "npm");
    ...
    const peerLink = path.join(peerPluginDir, "node_modules", "oriro");
    ...
    runCommandWithTimeoutMock.mockImplementationOnce(async (argv: string[]) => {
      ...
      if (!argv.includes("--legacy-peer-deps")) {
        await fs.mkdir(path.join(npmRoot, "node_modules", "oriro"), { recursive: true });
      }
      return {
        code: 0,
        stdout: "",
    ...
    await expectPathAccessState(path.join(npmRoot, "node_modules", "oriro"), "missing");
  ...
  it("prunes managed peer dependencies after their owning npm plugin is uninstalled", async () => {
    ...
    expect(rootManifest.oriro?.managedPeerDependencies ?? []).not.toContain("runtime-peer");
  ...
  it("deletes managed OriroHub install directories", async () => {
    const stateDir = path.join(tempDir, "state");
    const extensionsDir = path.join(stateDir, "extensions");
    const installPath = resolvePluginInstallDir("oriropack-demo", extensionsDir);
    await fs.mkdir(installPath, { recursive: true });
    await fs.writeFile(path.join(installPath, "index.js"), "// orirohub plugin");

    const result = await uninstallPlugin({
      config: createPluginConfig({
        entries: createSinglePluginEntries("oriropack-demo"),
        installs: {
          "oriropack-demo": {
            source: "orirohub",
            spec: "orirohub:oriropack-demo@2026.5.1-beta.2",
            installPath,
            orirohubUrl: "https://orirohub.ai",
            orirohubPackage: "oriropack-demo",
            ...
            npmTarballName: "oriropack-demo-2026.5.1-beta.2.tgz",
...
describe("resolveUninstallDirectoryTarget", () => {
  ...
  it("returns managed install path for copied path installs", () => {
    const extensionsDir = path.join(os.tmpdir(), "oriro-uninstall-safe");
  ...
  it("falls back to default path when configured installPath is untrusted", () => {
    ...
    const target = resolveUninstallDirectoryTarget({
      ...
      installRecord: {
        ...
        installPath: "/tmp/not-oriro-plugin-install/my-plugin",
      },
      extensionsDir,
    });
  ...
  it("uses configured installPath when npm installed it under the managed npm root", () => {
    const stateDir = path.join(os.tmpdir(), "oriro-uninstall-safe");
    const extensionsDir = path.join(stateDir, "extensions");
    const installPath = path.join(stateDir, "npm", "node_modules", "@oriro", "kitchen-sink");
  ...
  it("uses configured installPath when npm installed it under a managed npm project", () => {
    ...
    const installPath = path.join(npmRoot, "node_modules", "@oriro", "kitchen-sink");
  ...
  it("uses configured installPath when OriroHub installed it under the managed extensions root", () => {
    const stateDir = path.join(os.tmpdir(), "oriro-uninstall-safe");
    const extensionsDir = path.join(stateDir, "extensions");
    const installPath = resolvePluginInstallDir("oriropack-demo", extensionsDir);
    ...
    expect(
      resolveUninstallDirectoryTarget({
        ...
        installRecord: {
          source: "npm",
          spec: "@oriro/kitchen-sink@latest",
          installPath: path.join(os.tmpdir(), "npm", "node_modules", "@oriro", "kitchen-sink"),
        },
        extensionsDir,
      }),
    ).toBe(resolvePluginInstallDir("oriro-kitchen-sink-fixture", extensionsDir));
  });

  it("uses configured installPath when it is under the recorded managed extensions root", () => {
    const currentExtensionsDir = path.join(os.tmpdir(), "oriro-uninstall-current", "extensions");
    const recordedExtensionsDir = path.join(
      os.tmpdir(),
      "oriro-uninstall-recorded",
      "extensions",
    );
    const installPath = resolvePluginInstallDir("my-plugin", recordedExtensionsDir);
