// Oriro test instance tests cover spawned test instance lifecycle.
import fs from "node:fs/promises";
import path from "node:path";
import { describe, expect, it, vi } from "vitest";
import { createOriroTestInstance, testing } from "./oriro-test-instance.js";
...
describe("oriro test instance", () => {
  ...
  it("signals test instance process groups on POSIX", () => {
    ...
    testing.signalOriroTestProcess(child, "SIGKILL", killProcess);
  ...
  it("creates isolated config and spawn env without mutating process env", async () => {
    ...
    const inst = await createOriroTestInstance({
      name: "instance-unit",
    ...
    try {
      ...
      expect(inst.stateDir).toBe(path.join(inst.homeDir, ".oriro"));
      expect(inst.configPath).toBe(path.join(inst.stateDir, "oriro.json"));
      expect(inst.env.HOME).toBe(inst.homeDir);
      expect(inst.env.ORIRO_STATE_DIR).toBe(inst.stateDir);
      expect(inst.env.ORIRO_CONFIG_PATH).toBe(inst.configPath);
      expect(inst.env.ORIRO_SKIP_CRON).toBe("0");
