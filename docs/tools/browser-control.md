---
summary: "Oriro browser control API, CLI reference, and scripting actions"
read_when:
  - Scripting or debugging the agent browser via the local control API
  - Looking for the `oriro browser` CLI reference
...
This page is the reference for the local control HTTP API, the `oriro browser`
...
`ORIRO_EAGER_BROWSER_CONTROL_SERVER=1` in the gateway service environment
...
that override because Oriro does not launch those browser processes.
...
- `x-oriro-password: <gateway password>` or HTTP Basic auth with that password
...
- Role-style accessibility snapshots (`--interactive`, `--compact`,
  ...
  action engine.
- Page screenshots for the managed `oriro` browser when a per-tab CDP
...
Oriro, then restart the gateway. For Docker, also install the Chromium
...
```bash
ORIRO_INSTALL_BROWSER=1 ./scripts/docker/setup.sh
```
...
`ORIRO_HOME_VOLUME` or a bind mount. Oriro auto-detects the persisted
...
oriro browser start
oriro browser start --headless # one-shot local managed headless launch
oriro browser stop            # also clears emulation on attach-only/remote CDP
oriro browser tabs
oriro browser tab             # shortcut for current tab
oriro browser tab new
...
oriro browser screenshot --ref 12        # or --ref e12
oriro browser screenshot --labels
oriro browser snapshot
oriro browser snapshot --format aria --limit 200
oriro browser snapshot --interactive --compact --depth 6
oriro browser snapshot --efficient
oriro browser snapshot --labels
oriro browser snapshot --urls
oriro browser snapshot --selector "#main" --interactive
oriro browser snapshot --frame "iframe#main" --interactive
oriro browser console --level error
oriro browser errors --clear
oriro browser requests --filter api --clear
oriro browser pdf
oriro browser responsebody "**/api" --max-chars 5000
```
...
oriro browser click 12 --double           # or e12 for role refs
oriro browser click-coords 120 340        # viewport coordinates
oriro browser type 23 "hello" --submit
oriro browser press Enter
oriro browser hover 44
oriro browser scrollintoview e12
oriro browser drag 10 11
oriro browser select 9 OptionA OptionB
oriro browser download e12 report.pdf
oriro browser waitfordownload report.pdf
oriro browser upload /tmp/oriro/uploads/file.pdf
oriro browser upload media://inbound/file.pdf
oriro browser fill --fields '[{"ref":"1","type":"text","value":"Ada"}]'
oriro browser dialog --accept
oriro browser dialog --dismiss --dialog-id d1
oriro browser wait --text "Done"
oriro browser wait "#main" --url "**/dash" --load networkidle --fn "window.ready===true"
...
oriro browser highlight e12
oriro browser trace start
oriro browser trace stop
```
...
oriro browser storage local set theme dark
oriro browser storage session clear
oriro browser set offline on
oriro browser set headers --headers-json '{"X-Debug":"1"}'
oriro browser set credentials user pass            # --clear to remove
oriro browser set geo 37.7749 -122.4194 --origin "https://example.com"
oriro browser set media dark
oriro browser set timezone America/New_York
oriro browser set locale en-US
...
- `upload` and `dialog` are **arming** calls; run them before the click/press that triggers the chooser/dialog. If an action opens a modal, the action response includes `blockedByDialog` and `browserState.dialogs.pending`; pass that `dialogId` to respond directly. Dialogs handled outside Oriro appear under `browserState.dialogs.recent`.
...
- Download and trace paths are constrained to Oriro temp roots: `/tmp/oriro{,/downloads}` (fallback: `${os.tmpdir()}/oriro/...`).
- `upload` accepts files from the Oriro temp uploads root and
  Oriro-managed inbound media. Managed inbound media can be referenced as
...
Stable tab ids and labels survive Chromium raw-target replacement when Oriro
...
- `--format aria`: accessibility tree with `axN` refs. When Playwright is available, Oriro binds refs with backend DOM ids to the live page so follow-up actions can use them; otherwise treat the output as inspection-only.
...
Oriro supports two "snapshot" styles:

- **AI snapshot (numeric refs)**: `oriro browser snapshot` (default; `--format ai`)
  - Output: a text snapshot that includes numeric refs.
  - Actions: `oriro browser click 12`, `oriro browser type 23 "hello"`.
  - Internally, the ref is resolved via Playwright's `aria-ref`.

- **Role snapshot (role refs like `e12`)**: `oriro browser snapshot --interactive` (or `--compact`, `--depth`, `--selector`, `--frame`)
  - Output: a role-based list/tree with `[ref=e12]` (and optional `[nth=1]`).
  - Actions: `oriro browser click e12`, `oriro browser highlight e12`.
...
- **ARIA snapshot (ARIA refs like `ax12`)**: `oriro browser snapshot --format aria`
  - Output: the accessibility tree as structured nodes.
  - Actions: `oriro browser click ax12` works when the snapshot path can bind
...
- Wait for URL (globs supported by Playwright):
  - `oriro browser wait --url "**/dash"`
- Wait for load state:
  - `oriro browser wait --load networkidle`
  - Supported on managed `oriro` and raw/remote CDP profiles. The `user` and `existing-session` profiles reject `networkidle`; use `--url`, `--text`, a selector, or `--fn` waits there.
- Wait for a JS predicate:
  - `oriro browser wait --fn "window.ready===true"`
- Wait for a selector to become visible:
  - `oriro browser wait "#main"`
...
1. `oriro browser snapshot --interactive`
2. Use `click <ref>` / `type <ref>` (prefer role refs in interactive mode)
3. If it still fails: `oriro browser highlight <ref>` to see what Playwright is targeting
...
5. For deep debugging: record a trace:
   - `oriro browser trace start`
   - reproduce the issue
   - `oriro browser trace stop` (prints `TRACE:<path>`)
...
```bash
oriro browser status --json
oriro browser snapshot --interactive --json
oriro browser requests --filter api --json
oriro browser cookies --json
```
...
- The oriro browser profile may contain logged-in sessions; treat it as sensitive.
- `browser act kind=evaluate` / `oriro browser evaluate` and `wait --fn`
  execute arbitrary JavaScript in the page context. Prompt injection can steer
  this. Disable it with `browser.evaluateEnabled=false` if you do not need it.
- `oriro browser evaluate --fn` accepts a function source, an expression, or
