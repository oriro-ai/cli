...
x64 and ARM64 installers on Oriro releases.

Download the latest stable installer from the [Oriro releases page](https://github.com/oriro-ai/cli/releases):

- [OriroCompanion-Setup-x64.exe](https://github.com/oriro-ai/cli/releases/download/v2026.6.5/OriroCompanion-Setup-x64.exe)
- [OriroCompanion-Setup-arm64.exe](https://github.com/oriro-ai/cli/releases/download/v2026.6.5/OriroCompanion-Setup-arm64.exe)
- [Checksums](https://github.com/oriro-ai/cli/releases/download/v2026.6.5/OriroCompanion-SHA256SUMS.txt)

If a download link above returns a 404, visit the [releases page](https://github.com/oriro-ai/cli/releases) and look for the `OriroCompanion-Setup-*` assets on the latest release.

After install, launch **Oriro Companion** from the Start menu or the system
...
`OriroGateway` WSL distro, installs the Gateway inside it, and pairs the app.
...
Windows Hub can register as a first-class Oriro node. The agent can then use
...
```powershell
oriro devices list
oriro devices approve <request-id>
oriro nodes status
```
...
Windows capabilities without a running Oriro Gateway.
...
For terminal-first use, install Oriro from PowerShell:
...
```powershell
oriro onboard --non-interactive --skip-health
oriro gateway run
```
...
Then install Oriro inside WSL with the Linux quickstart:
...
```bash
systemctl --user is-enabled oriro-gateway.service
systemctl --user status oriro-gateway.service --no-pager
```
...
Check Task Manager for `Oriro.Tray.WinUI.exe`. If it is running, open the
hidden tray-icons area and pin it. If it is not running, launch **Oriro
...
```powershell
notepad "$env:LOCALAPPDATA\OriroTray\Logs\Setup\easy-setup-latest.txt"
```
