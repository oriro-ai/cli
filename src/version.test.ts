...
const versionFixtureRoot = createSuiteTempRootTracker({ prefix: "oriro-version-" });
...
describe("version resolution", () => {
  it("keeps bundled version injection as a direct define identifier", async () => {
    ...
    expect(source).toContain("typeof __ORIRO_VERSION__");
    expect(source).toContain("? __ORIRO_VERSION__");
  });

  it("resolves package version from nested dist/plugin-sdk module URL", async () => {
    await withVersionFixtureDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "oriro", version: "1.2.3" });
  ...
  it("ignores unrelated nearby package.json files", async () => {
    await withVersionFixtureDir(async (root) => {
      await writeJsonFixture(root, "package.json", { name: "oriro", version: "2.3.4" });
  ...
  it("ignores non-oriro package and blank build-info versions", async () => {
    await withVersionFixtureDir(async (root) => {
  ...
  it("prefers ORIRO_VERSION over service and package versions", () => {
    expect(
      resolveRuntimeServiceVersion({
        ORIRO_VERSION: "9.9.9",
        ORIRO_SERVICE_VERSION: "2.2.2",
  ...
  it("prefers runtime VERSION over stale ORIRO_VERSION for compatibility checks", () => {
    const previous = process.env.ORIRO_VERSION;
    const previousService = process.env.ORIRO_SERVICE_VERSION;
    const previousPackage = process.env.npm_package_version;
    try {
      process.env.ORIRO_VERSION = "2026.3.25";
      process.env.ORIRO_SERVICE_VERSION = "2026.3.25-service";
      process.env.npm_package_version = "2026.3.25-package";
      expect(resolveCompatibilityHostVersion()).toBe(VERSION);
    } finally {
      restoreEnvValue("ORIRO_VERSION", previous);
      restoreEnvValue("ORIRO_SERVICE_VERSION", previousService);
  ...
  it("keeps explicit env-object overrides for compatibility checks in tests", () => {
    expect(
      resolveCompatibilityHostVersion({
        ORIRO_VERSION: "2026.3.99",
        ORIRO_SERVICE_VERSION: "2026.3.98",
  ...
  it("prefers explicit compatibility host overrides over runtime and stale env versions", () => {
    expect(
      resolveCompatibilityHostVersion({
        ORIRO_COMPATIBILITY_HOST_VERSION: "2026.4.8",
        ORIRO_VERSION: "2026.3.99",
        ORIRO_SERVICE_VERSION: "2026.3.98",
        npm_package_version: "2026.3.97",
      }),
