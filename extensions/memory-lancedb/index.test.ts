...
async function withMockedOpenAiMemoryPlugin<T>(params: {
  ...
  vi.resetModules();
  vi.doMock("oriro/plugin-sdk/runtime-env", () => ({
  ...
  try {
    ...
    vi.doUnmock("oriro/plugin-sdk/runtime-env");
    vi.doUnmock("openai");
...
describe("memory plugin e2e", () => {
  const { getDbPath, getTmpDir } = installTmpDirHarness({ prefix: "oriro-memory-test-" });
  ...
  test("uses provider adapter auth when embedding apiKey is omitted", async () => {
    ...
    vi.resetModules();
    vi.doMock("oriro/plugin-sdk/memory-core-host-engine-embeddings", () => ({
    ...
    try {
      ...
      expect(providerOptions.agentDir).toBe("/tmp/oriro-agent");
      ...
      vi.doUnmock("oriro/plugin-sdk/memory-core-host-engine-embeddings");
      vi.doUnmock("openai");
      vi.doUnmock("./lancedb-runtime.js");
      vi.resetModules();
    }
    ...
    await withMockedOpenAiMemoryPlugin({
      ...
      run: async (dynamicMemoryPlugin) => {
        ...
        try {
          ...
          await program.parseAsync(["node", "oriro", "ltm", "list", "--limit", "+03"]);
  ...
  test("sanitizeForMemoryCapture strips sender label from real room-label envelope shapes", () => {
    ...
    expect(
      sanitizeForMemoryCapture(
        "[Discord Oriro #dev channel id:456 +5m] Alice: I prefer dark mode",
      ),
    ).toBe("I prefer dark mode");
    expect(sanitizeForMemoryCapture("[Telegram Oriro id:-100] Alice: I prefer dark mode")).toBe(
      "I prefer dark mode",
    );
  ...
  test("escapeMemoryForPrompt strips media attached annotations before escaping", () => {
    expect(
      escapeMemoryForPrompt(
        "User sent image [media attached: /Users/alex/.oriro/media/photo.jpg (image/jpeg)] and said hello",
      ),
