...
import {
  ...
  requestDiscord,
} from "@oriro/discord/api.js";
import { DEFAULT_EMOJIS } from "oriro/plugin-sdk/channel-feedback";
...
import { writeExternalFileWithinRoot } from "oriro/plugin-sdk/security-runtime";
import { uniqueStrings } from "oriro/plugin-sdk/string-coerce-runtime";
...
const DISCORD_QA_CAPTURE_CONTENT_ENV = "ORIRO_QA_DISCORD_CAPTURE_CONTENT";
const DISCORD_QA_CAPTURE_UI_METADATA_ENV = "ORIRO_QA_DISCORD_CAPTURE_UI_METADATA";
const DISCORD_QA_KEEP_THREADS_ENV = "ORIRO_QA_DISCORD_KEEP_THREADS";
const QA_REDACT_PUBLIC_METADATA_ENV = "ORIRO_QA_REDACT_PUBLIC_METADATA";
const DISCORD_QA_ENV_KEYS = [
  ...
  "ORIRO_QA_DISCORD_SUT_APPLICATION_ID",
] as const;
...
function resolveDiscordQaRuntimeEnv(env: NodeJS.ProcessEnv = process.env): DiscordQaRuntimeEnv {
  const voiceChannelId = env.ORIRO_QA_DISCORD_VOICE_CHANNEL_ID?.trim();
  const runtimeEnv = {
    guildId: resolveEnvValue(env, "ORIRO_QA_DISCORD_GUILD_ID"),
    channelId: resolveEnvValue(env, "ORIRO_QA_DISCORD_CHANNEL_ID"),
    driverBotToken: resolveEnvValue(env, "ORIRO_QA_DISCORD_DRIVER_BOT_TOKEN"),
    sutBotToken: resolveEnvValue(env, "ORIRO_QA_DISCORD_SUT_BOT_TOKEN"),
    sutApplicationId: resolveEnvValue(env, "ORIRO_QA_DISCORD_SUT_APPLICATION_ID"),
    ...(voiceChannelId ? { voiceChannelId } : {}),
  };
  validateDiscordQaRuntimeEnv(runtimeEnv, "ORIRO_QA_DISCORD");
  return runtimeEnv;
}
...
async function resolveDiscordQaVoiceChannel(params: {
  ...
  if (!first) {
    throw new Error(
      "Discord voice auto-join scenario could not find a visible voice/stage channel for the SUT bot. Add voiceChannelId to the Convex discord credential payload or set ORIRO_QA_DISCORD_VOICE_CHANNEL_ID.",
    );
  }
  return first;
}
...
<body>
  <main>
    ...
    <section class="message">
      <div class="author">Oriro Discord SUT</div>
