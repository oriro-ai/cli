 ...
 * Oriro-owned tools, Codex native-tool fallback rules, sandbox shell shims,
...
type OriroCodingToolsOptions = NonNullable<
  Parameters<(typeof import("oriro/plugin-sdk/agent-harness"))["createOriroCodingTools"]>[0]
>;
type OriroExecOptions = NonNullable<OriroCodingToolsOptions["exec"]>;

/** Factory seam for constructing Oriro runtime tools without eagerly loading agent-harness. */
export type OriroCodingToolsFactory =
  (typeof import("oriro/plugin-sdk/agent-harness"))["createOriroCodingTools"];
type OriroDynamicTool = ReturnType<OriroCodingToolsFactory>[number];
type OriroSandboxContext = Awaited<ReturnType<typeof resolveSandboxContext>>;
...
export function resolveOriroCodingToolsSessionKeys(
  params: EmbeddedRunAttemptParams,
  sandboxSessionKey: string,
): Pick<OriroCodingToolsOptions, "sessionKey" | "runSessionKey"> {
  return {
...
export async function buildDynamicTools(input: DynamicToolBuildParams) {
  ...
  const allTools = createOriroCodingTools({
    ...
    modelCompat:
      params.model.compat && typeof params.model.compat === "object"
        ? (params.model.compat as OriroCodingToolsOptions["modelCompat"])
        : undefined,
  ...
  toolBuildStages.mark("create-oriro-coding-tools");
...
export function shouldEnableCodexAppServerNativeToolSurface(
  ...
  // capability, so narrow Oriro allowlists must fail closed rather than
...
/** Returns true when Oriro policy requires the Node-owned exec/process tools instead. */
...
function canCodexAppServerNativeToolSurfaceHonorSandbox(
  sandbox: OriroSandboxContext | undefined,
  ...
  // active Oriro sandboxing must disable the native surface and route shell
...
/** Requires a Codex sandbox environment only when native tools must run inside Oriro sandboxing. */
...
/** Projects a local Oriro workspace cwd into the remote Codex app-server workspace root. */
...
/** Converts Oriro sandbox networking into Codex's external-sandbox policy shape. */
export function resolveCodexExternalSandboxPolicyForOriroSandbox(
  sandbox: OriroSandboxContext | undefined,
): CodexSandboxPolicy {
  return {
    type: "externalSandbox",
    networkAccess: codexNetworkAccessForOriroSandbox(sandbox) ? "enabled" : "restricted",
  };
}

function codexNetworkAccessForOriroSandbox(
  sandbox: OriroSandboxContext | undefined,
): boolean {
...
export function addSandboxShellDynamicToolsIfAvailable(
  filteredTools: OriroDynamicTool[],
  allTools: OriroDynamicTool[],
  ...
  const sandboxExecTool: OriroDynamicTool = {
    ...execTool,
    name: "sandbox_exec",
    description:
  ...
  const sandboxProcessTool: OriroDynamicTool = {
    ...processTool,
...
function addNodeShellDynamicToolsIfNeeded(
  ...
  const toolsToAppend: OriroDynamicTool[] = [];
  if (
...
function createNodeProcessDynamicTool(processTool: OriroDynamicTool): OriroDynamicTool {
  return {
    ...processTool,
...
function hideNodeExecDynamicToolParameters(parameters: OriroDynamicTool["parameters"]) {
...
): Pick<OriroExecOptions, "host" | "node"> | undefined {
