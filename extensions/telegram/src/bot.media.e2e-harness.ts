...
type DispatchReplyWithBufferedBlockDispatcherFn =
  typeof import("oriro/plugin-sdk/reply-runtime").dispatchReplyWithBufferedBlockDispatcher;
type DispatchReplyHarnessParams = Parameters<DispatchReplyWithBufferedBlockDispatcherFn>[0];
type ReadRemoteMediaBufferFn =
  typeof import("oriro/plugin-sdk/media-runtime").readRemoteMediaBuffer;
...
function ensureMediaHarnessStoreRoot(): string {
  mediaHarnessStoreRoot ??= mkdtempSync(path.join(os.tmpdir(), "oriro-telegram-media-e2e-"));
...
const defaultRuntimeConfig = (() =>
  ...
  }) as OriroConfig) as TelegramBotDeps["getRuntimeConfig"];
