...
describe("release candidate checklist", () => {
  ...
  it("runs Parallels against the exact prepared candidate tarball", () => {
    expect(candidateParallelsArgs(".artifacts/preflight/oriro.tgz")).toEqual([
      "test:parallels:npm-update",
      "--",
      "--target-tarball",
      ".artifacts/preflight/oriro.tgz",
      "--json",
    ]);
    expect(
      candidateParallelsShellCommand(
        ".artifacts/preflight/oriro candidate.tgz",
    ...
    ).toContain("'--target-tarball' '.artifacts/preflight/oriro candidate.tgz'");
  });
  ...
  it("requires and carries an exact Windows Node tag for stable release candidates", () => {
    ...
    const options = {
      ...
      windowsNodeInstallerDigests: JSON.stringify({
        "OriroCompanion-Setup-x64.exe": `sha256:${"a".repeat(64)}`,
        "OriroCompanion-Setup-arm64.exe": `sha256:${"b".repeat(64)}`,
      }),
    };

    expect(buildPublishCommand(options)).toContain("'windows_node_tag=v0.6.3'");
    expect(buildPublishCommand(options)).toContain(
      `'windows_node_installer_digests={"OriroCompanion-Setup-x64.exe":"sha256:${"a".repeat(64)}","OriroCompanion-Setup-arm64.exe":"sha256:${"b".repeat(64)}"}'`,
    );
  });
  ...
  it("validates the stable Windows source release and immutable installer digests", async () => {
    const assets = [
      ...
      {
        name: "OriroCompanion-Setup-arm64.exe",
  ...
  it.each([
    ...
    [
      { assets: [] },
      "must contain exactly one required asset OriroCompanion-Setup-x64.exe; found 0",
    ],
    [
      {
        assets: [
      ...
      "must contain exactly one required asset OriroCompanion-Setup-x64.exe; found 2",
    ],
    [
      {
        assets: [
          { name: "OriroCompanion-Setup-x64.exe", digest: "" },
          { name: "OriroCompanion-Setup-arm64.exe", digest: `sha256:${"b".repeat(64)}` },
        ],
      },
      "asset OriroCompanion-Setup-x64.exe is missing its SHA-256 digest",
    ],
  ...
  it("rejects selected plugin publish scope for release candidates", () => {
    ...
    ).toThrow("release candidates publish Oriro with --plugin-publish-scope all-publishable");
  });
  ...
  it("falls back to a single compatible artifact from the same run", () => {
    expect(
      resolveArtifactName(
        [{ name: "oriro-npm-preflight-dba00", expired: false }],
        "oriro-npm-preflight-v2026.5.16-beta.2",
        "oriro-npm-preflight-",
      ),
    ).toBe("oriro-npm-preflight-dba00");
  });
