...
# Oriro under node-A, registers the gateway service pointing at node-A, then
# switches PATH so node-B comes first and runs `oriro update`. Verifies that:
#
...
#
# Requires: Docker, a built oriro-current.tgz (or will build one).
set -euo pipefail
...
IMAGE_NAME="$(docker_e2e_resolve_image "oriro-multi-node-update-e2e" ORIRO_MULTI_NODE_UPDATE_E2E_IMAGE)"
SKIP_BUILD="${ORIRO_MULTI_NODE_UPDATE_E2E_SKIP_BUILD:-0}"
DOCKER_RUN_TIMEOUT="${ORIRO_MULTI_NODE_DOCKER_TIMEOUT:-300s}"
...
PACKAGE_TGZ="$(docker_e2e_prepare_package_tgz multi-node-update "${ORIRO_CURRENT_PACKAGE_TGZ:-}")"
...
echo "oriro binary: $ORIRO_A"
echo "oriro version: $(oriro --version 2>/dev/null || echo unknown)"

# Record the package root for node-A install.
PACKAGE_ROOT_A="$NPM_PREFIX_A/lib/node_modules/oriro"
...
GATEWAY_UNIT_PATH="/root/.config/systemd/user/oriro-gateway.service"
...
if ! oriro gateway install --json >"$ARTIFACTS/gateway-install.json" 2>"$ARTIFACTS/gateway-install.err"; then
...
# but oriro is NOT installed there.
...
echo "which oriro: $(command -v oriro)"
echo "process.execPath will be: $(node -e "console.log(process.execPath)")"

echo ""
echo "── Step 6: Run oriro update (this is the bug) ──"
...
echo "Running oriro update --yes --json..."
UPDATE_EXIT=0
oriro update --yes --json \
  --tag /tmp/oriro-current.tgz \
...
if [ "$UPDATE_EXIT" -ne 0 ] && ! grep -q "gateway" "$ARTIFACTS/update.err" 2>/dev/null; then
  echo "FAIL: oriro update failed before reaching the package install step"
