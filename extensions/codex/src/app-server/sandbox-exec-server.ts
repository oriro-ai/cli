/**
 * Hosts the local Oriro sandbox exec-server that Codex app-server native
...
import { embeddedAgentLog } from "oriro/plugin-sdk/agent-harness-runtime";
import type { SandboxContext } from "oriro/plugin-sdk/sandbox";
...
const SANDBOX_EXEC_SERVERS = new Map<string, Promise<OriroExecServer>>();
  ...
  await Promise.all(
    servers.map(async (entry) => {
      if (entry.status === "fulfilled") {
        ...
        await closeOriroExecServer(entry.value);
      }
    }),
  );
}
...
export async function ensureCodexSandboxExecServerEnvironment(params: {
  ...
  const execServer = await acquireOriroExecServer(params.sandbox);
  try {
    ...
    await releaseOriroExecServer(execServer);
...
export async function releaseCodexSandboxExecServerEnvironment(
  ...
  if (server) {
    await releaseOriroExecServer(server);
  }
}
...
async function acquireOriroExecServer(sandbox: SandboxContext): Promise<OriroExecServer> {
  const key = sandbox.runtimeId;
  while (true) {
    const existing = SANDBOX_EXEC_SERVERS.get(key);
    const promise = existing ?? startAndRememberOriroExecServer(sandbox);
...
function startAndRememberOriroExecServer(sandbox: SandboxContext): Promise<OriroExecServer> {
  const created = startOriroExecServer(sandbox);
  const key = sandbox.runtimeId;
...
async function startOriroExecServer(sandbox: SandboxContext): Promise<OriroExecServer> {
  ...
  const execServer: OriroExecServer = {
    authPath,
    closed: false,
    environmentId,
    refCount: 0,
...
async function releaseOriroExecServer(execServer: OriroExecServer): Promise<void> {
  ...
  await closeOriroExecServer(execServer);
}

async function closeOriroExecServer(execServer: OriroExecServer): Promise<void> {
  if (execServer.closed) {
    return;
  }
...
function buildEnvironmentId(sandbox: SandboxContext): string {
  ...
  return `oriro-sandbox-${hash}`;
}

function isAuthorizedExecServerRequest(
  execServer: OriroExecServer,
...
function handleConnection(execServer: OriroExecServer, socket: WebSocket): void {
