...
export type NodeMatchCandidate = {
  ...
  connected?: boolean;
  /** Client id used to prefer current Oriro nodes over legacy migration ties. */
  clientId?: string;
};
...
function isCurrentOriroClient(clientId: string | undefined): boolean {
...
function pickPreferredLegacyMigrationMatch(
  ...
  const current = matches.filter((match) => isCurrentOriroClient(match.clientId));
  if (current.length !== 1) {
    return undefined;
  }
