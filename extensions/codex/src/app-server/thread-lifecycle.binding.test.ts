...
describe("Codex app-server thread lifecycle bindings", () => {
  ...
  it("starts a fresh Codex thread when context-engine policy metadata changes", async () => {
    ...
    await writeCodexAppServerBinding(sessionFile, {
      ...
      contextEngine: {
        ...
        engineId: "lossless-oriro",
        policyFingerprint:
          '{"schemaVersion":1,"engineId":"lossless-oriro","engineVersion":"1.0.0","ownsCompaction":true,"turnMaintenanceMode":"foreground","citationsMode":"inline","contextTokenBudget":400000,"projectionMaxChars":1000000}',
      },
    });
    const params = createParams(sessionFile, workspaceDir);
    params.contextEngine = {
  ...
  it("starts a new thread when the network proxy config is not active on the binding", async () => {
    ...
    expect(binding?.networkProxyProfileName).toBe("oriro-network");
  ...
  it("keeps native hook relay config as the final thread config patch", async () => {
    ...
    const finalConfigPatch = {
      ...
      "hooks.PreToolUse": [
        {
          hooks: [{ type: "command", command: "oriro-native-hook-relay", timeout: 5 }],
        },
      ],
    };
    const buildPluginThreadConfig = vi.fn(async () => ({
