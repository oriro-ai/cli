...
function stripUnsafeBlockedRunMetadata(messages: AgentMessage[]): AgentMessage[] {
  ...
  const nextMessages = messages.map((message) => {
    const oriro = (message as unknown as Record<string, unknown>)["__oriro"];
    if (!oriro || typeof oriro !== "object") {
      return message;
    }
    const beforeAgentRunBlocked = (oriro as { beforeAgentRunBlocked?: unknown })
    ...
    const nextOriro = {
      ...(oriro as Record<string, unknown>),
