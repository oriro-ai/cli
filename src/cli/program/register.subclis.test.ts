...
describe("registerSubCliCommands", () => {
  ...
  const originalDisableLazySubcommands = process.env.ORIRO_DISABLE_LAZY_SUBCOMMANDS;
  const originalEnablePrivateQaCli = process.env.ORIRO_ENABLE_PRIVATE_QA_CLI;
  ...
  beforeEach(() => {
    if (originalDisableLazySubcommands === undefined) {
      delete process.env.ORIRO_DISABLE_LAZY_SUBCOMMANDS;
    } else {
      process.env.ORIRO_DISABLE_LAZY_SUBCOMMANDS = originalDisableLazySubcommands;
    }
    process.env.ORIRO_ENABLE_PRIVATE_QA_CLI = "1";
  ...
  afterEach(() => {
    ...
    if (originalEnablePrivateQaCli === undefined) {
      delete process.env.ORIRO_ENABLE_PRIVATE_QA_CLI;
    } else {
      process.env.ORIRO_ENABLE_PRIVATE_QA_CLI = originalEnablePrivateQaCli;
    }
  });

  it("registers the primary placeholder plus completion and dispatches", async () => {
    const program = createRegisteredProgram(["node", "oriro", "acp"]);
  ...
  it("registers placeholders for all subcommands when no primary", () => {
    const program = createRegisteredProgram(["node", "oriro"]);

    const names = program.commands.map((cmd) => cmd.name());
    expect(names).toContain("acp");
    expect(names).toContain("gateway");
    expect(names).toContain("orirobot");
  ...
  it("re-parses argv for lazy subcommands", async () => {
    const program = createRegisteredProgram(["node", "oriro", "nodes", "list"], "oriro");
  ...
  it("registers the infer placeholder and dispatches through the capability registrar", async () => {
    const program = createRegisteredProgram(["node", "oriro", "infer"], "oriro");
  ...
  it("replaces placeholder when registering a subcommand by name", async () => {
    const program = createRegisteredProgram(["node", "oriro", "acp", "--help"], "oriro");
  ...
  it("registers only the gateway run surface for gateway startup", async () => {
    const argv = ["node", "oriro", "gateway", "--force"];
    process.argv = argv;
    const program = new Command().name("oriro");
  ...
  it("keeps the full gateway CLI for non-run gateway subcommands", async () => {
    const argv = ["node", "oriro", "gateway", "call", "health"];
    process.argv = argv;
    const program = new Command().name("oriro");
  ...
  it("passes completion context to channel registration", async () => {
    const argv = ["node", "oriro", "completion", "--write-state"];
    const program = new Command().name("oriro");
  ...
  it.each([
    ["plugins update", ["plugins", "update", "lossless-oriro"]],
    ["plugins update --all", ["plugins", "update", "--all"]],
    ["plugins install", ["plugins", "install", "lossless-oriro"]],
    ["plugins list", ["plugins", "list"]],
    ["plugins inspect", ["plugins", "inspect", "lossless-oriro"]],
  ...
  it("does not preload plugin CLI registrations for bare plugin parent help", async () => {
    process.argv = ["node", "oriro", "plugins"];
    const program = new Command().name("oriro");
