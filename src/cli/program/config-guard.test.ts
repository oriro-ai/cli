...
describe("ensureConfigReady", () => {
  ...
  function useTempOriroHome(): string {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-config-guard-"));
    tempRoots.push(root);
    setTestEnvValue("ORIRO_HOME", root);
    deleteTestEnvValue("ORIRO_STATE_DIR");
    return root;
  }

  function writeLegacyTaskSidecarMarker(root: string): void {
    const markerPath = path.join(root, ".oriro", "tasks", "runs.sqlite");
  ...
  function writeStateMarker(root: string, relativePath: string): void {
    const markerPath = path.join(root, ".oriro", relativePath);
    fs.mkdirSync(path.dirname(markerPath), { recursive: true });
    fs.writeFileSync(markerPath, "{}");
  }

  beforeEach(() => {
    envSnapshot = captureEnv(["HOME", "ORIRO_HOME", "ORIRO_STATE_DIR"]);
    vi.clearAllMocks();
  ...
  it("runs doctor flow for legacy sessions without task sidecars", async () => {
    const root = useTempOriroHome();
    fs.mkdirSync(path.join(root, ".oriro", "sessions"), { recursive: true });
  ...
  it("uses shared tilde expansion for ORIRO_HOME in the startup detector", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-config-guard-home-"));
    tempRoots.push(root);
    setTestEnvValue("HOME", root);
    setTestEnvValue("ORIRO_HOME", "~/svc");
    deleteTestEnvValue("ORIRO_STATE_DIR");
  ...
  it("exits for invalid config on non-allowlisted commands", async () => {
    ...
    expect(plainErrorCalls(runtime)).toEqual([
      "Oriro config is invalid",
      "File: /tmp/oriro.json",
      "Problem:",
      "  - channels.quietchat: invalid",
      "",
      `Fix: ${formatCliCommand("oriro doctor --fix")}`,
      `Inspect: ${formatCliCommand("oriro config validate")}`,
  ...
  it("replaces doctor fix advice for plugin packaging-only invalid config", async () => {
    ...
    expect(calls).not.toContain(`Fix: ${formatCliCommand("oriro doctor --fix")}`);
    expect(runtime.exit).toHaveBeenCalledWith(1);
  });
  ...
  it("runs doctor migration flow only once per module instance", async () => {
    writeLegacyTaskSidecarMarker(useTempOriroHome());
