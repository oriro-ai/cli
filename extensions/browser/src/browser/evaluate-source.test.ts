...
describe("normalizeBrowserEvaluateFunctionSource", () => {
  ...
  it("wraps expressions as page functions", () => {
    expect(normalizeBrowserEvaluateFunctionSource("document.title")).toBe(
      [
        "() => {",
        "const __oriroEvaluateExpressionResult = (document.title);",
        'return typeof __oriroEvaluateExpressionResult === "function" ? __oriroEvaluateExpressionResult() : __oriroEvaluateExpressionResult;',
        "}",
      ].join("\n"),
    );
  });
  ...
  it("preserves function-valued expression invocation", () => {
    expect(normalizeBrowserEvaluateFunctionSource("extractTitle")).toBe(
      [
        "() => {",
        "const __oriroEvaluateExpressionResult = (extractTitle);",
        'return typeof __oriroEvaluateExpressionResult === "function" ? __oriroEvaluateExpressionResult() : __oriroEvaluateExpressionResult;',
        "}",
      ].join("\n"),
    );
    expect(normalizeBrowserEvaluateFunctionSource("extractText", { argumentName: "el" })).toBe(
      [
        "(el) => {",
        "const __oriroEvaluateExpressionResult = (extractText);",
        'return typeof __oriroEvaluateExpressionResult === "function" ? __oriroEvaluateExpressionResult(el) : __oriroEvaluateExpressionResult;',
        "}",
      ].join("\n"),
    );
  });
