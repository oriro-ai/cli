...
let configOverride: OriroConfig = {
  session: {
    mainKey: "main",
    scope: "per-sender",
  },
...
function setConfigOverride(next: OriroConfig): void {
  configOverride = next;
...
describe("subagent announce formatting", () => {
  ...
  beforeAll(async () => {
    ...
    // See: https://github.com/oriro-ai/cli/issues/31298
    previousFastTestEnv = process.env.ORIRO_TEST_FAST;
  ...
  afterAll(() => {
    ...
    process.env.ORIRO_TEST_FAST = previousFastTestEnv;
  });
  ...
  beforeEach(() => {
    vi.useRealTimers();
    // ORIRO_TEST_FAST is set in beforeAll before module import
  ...
  it("sends instructional message to main agent with status and findings", async () => {
    ...
    expect(msg).toContain("Oriro runtime context (internal):");
