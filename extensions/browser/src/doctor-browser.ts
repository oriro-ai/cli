...
import { DEFAULT_ORIRO_BROWSER_PROFILE_NAME, resolveBrowserConfig } from "./browser/config.js";
...
const LEGACY_ORIROD_BROWSER_PROFILE_NAME = "orirod";
...
/** Legacy managed orirod profile paths that can be archived by doctor --fix. */
...
function collectChromeMcpProfiles(cfg: OriroConfig): ExistingSessionProfile[] {
...
function collectManagedProfiles(cfg: OriroConfig): ManagedProfile[] {
  const browser = asRecord(cfg.browser);
  if (!browser) {
    return [];
  }
  ...
  for (const [profileName, rawProfile] of Object.entries(configuredProfiles)) {
    const profile = asRecord(rawProfile);
    const driver = normalizeOptionalString(profile?.driver) ?? "oriro";
    if (driver !== "existing-session") {
...
function isLegacyOrirodProfileConfigured(cfg: OriroConfig, legacyProfileDir: string): boolean {
  const browser = asRecord(cfg.browser);
  if (!browser) {
    return false;
  }
  if (normalizeOptionalString(browser.defaultProfile) === LEGACY_ORIROD_BROWSER_PROFILE_NAME) {
    return true;
  }
  ...
  if (Object.hasOwn(configuredProfiles, LEGACY_ORIROD_BROWSER_PROFILE_NAME)) {
    return true;
  }
...
/** Detects unmanaged legacy orirod browser profile residue on disk. */
export function detectLegacyOrirodBrowserProfileResidue(
  cfg: OriroConfig,
  ...
  const legacyProfileDir = resolveManagedBrowserProfileDir(
    configDir,
    LEGACY_ORIROD_BROWSER_PROFILE_NAME,
  );
  ...
  if (isLegacyOrirodProfileConfigured(cfg, legacyProfileDir)) {
    return null;
  }
  ...
  if (
    resolved.defaultProfile !== DEFAULT_ORIRO_BROWSER_PROFILE_NAME ||
...
function formatLegacyOrirodBrowserProfileResidueNote(
  residue: LegacyOrirodBrowserProfileResidue,
): string {
  return [
...
export async function noteChromeMcpBrowserReadiness(
  ...
  const resolved = resolveBrowserConfig(cfg.browser, cfg);
  const legacyOrirodResidue = detectLegacyOrirodBrowserProfileResidue(cfg, {
    configDir: deps?.configDir,
    pathExists: deps?.pathExists,
  });
  if (legacyOrirodResidue) {
    noteFn(formatLegacyOrirodBrowserProfileResidueNote(legacyOrirodResidue), "Browser");
  }
...
/** Archives legacy orirod browser profile residue when doctor --fix is requested. */
export async function maybeArchiveLegacyOrirodBrowserProfileResidue(
  cfg: OriroConfig,
  deps?: BrowserDoctorFilesystemDeps,
): Promise<{ changes: string[]; warnings: string[] }> {
  const residue = detectLegacyOrirodBrowserProfileResidue(cfg, deps);
  if (!residue) {
  ...
  try {
    ...
    return {
      changes: [
        [
          "Archived legacy orirod managed browser profile residue.",
    ...
    return {
      changes: [],
      warnings: [`Legacy orirod browser profile residue could not be archived: ${message}`],
    };
  }
}
