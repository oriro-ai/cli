...
async function withPluginAssetFixture(run: (rootDir: string) => Promise<void>) {
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-plugin-assets-"));
  try {
    ...
    fs.writeFileSync(
      path.join(rootDir, "extensions", "canvas", "oriro.plugin.json"),
  ...
  it("discovers plugin-owned asset scripts by manifest id", async () => {
    await withPluginAssetFixture(async (rootDir) => {
      ...
      expect(hooks).toEqual([
        {
          aliases: ["@oriro/canvas-plugin", "canvas", "canvas-plugin"],
          command: "node scripts/bundle-a2ui.mjs",
          packageName: "@oriro/canvas-plugin",
          phase: "build",
