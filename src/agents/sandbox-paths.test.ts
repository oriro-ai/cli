...
async function withManagedMediaRoot<T>(run: (ctx: { stateDir: string }) => Promise<T>) {
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-managed-media-"));
  try {
    return await withEnvAsync({ ORIRO_STATE_DIR: stateDir }, async () => {
...
describe("resolveSandboxedMediaSource", () => {
  ...
  it.each([
    ...
    {
      name: "absolute paths under host tmp outside oriro tmp root",
      media: path.join(os.tmpdir(), "outside-oriro", "passwd"),
      expected: /sandbox/i,
    },
    {
  ...
  it("rejects symlinked Oriro tmp paths escaping tmp root", async () => {
  ...
  it("rejects hardlinked Oriro tmp paths to outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
  ...
  it("rejects symlinked Oriro tmp paths to hardlinked outside files", async () => {
    if (process.platform === "win32") {
      return;
    }
    await withOutsideHardlinkInOriroTmp(
