...
import { collectConfiguredModelRefs } from "@oriro/model-catalog-core/configured-model-refs";
...
function canReuseUnscopedCurrentPluginMetadataSnapshot(config: OriroConfig): boolean {
...
function isProviderConfigured(cfg: OriroConfig, providerId: string): boolean {
...
function hasPluginOwnedWebSearchConfig(cfg: OriroConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
  return isRecord(pluginConfig) && isRecord(pluginConfig.webSearch);
}

function hasPluginOwnedWebFetchConfig(cfg: OriroConfig, pluginId: string): boolean {
  const pluginConfig = cfg.plugins?.entries?.[pluginId]?.config;
...
function hasPluginOwnedToolConfig(cfg: OriroConfig, plugin: PluginManifestRecord): boolean {
...
function hasConfiguredWebSearchPluginEntry(cfg: OriroConfig): boolean {
...
function hasConfiguredWebSearchProviderSelection(cfg: OriroConfig): boolean {
...
function hasConfiguredWebFetchPluginEntry(cfg: OriroConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    Boolean(entries) &&
    typeof entries === "object" &&
...
function hasConfiguredSpeechProviderSelection(cfg: OriroConfig): boolean {
  return collectConfiguredSpeechProviderIds(cfg).size > 0;
}

function hasConfiguredPluginConfigEntry(cfg: OriroConfig): boolean {
  const entries = cfg.plugins?.entries;
  return (
    Boolean(entries) &&
    typeof entries === "object" &&
...
function hasBrowserToolReference(cfg: OriroConfig): boolean {
...
function collectConfiguredPluginEntryIds(cfg: OriroConfig): string[] {
  const entries = cfg.plugins?.entries;
...
function hasOwnPluginEntry(cfg: OriroConfig, pluginId: string): boolean {
  const entries = cfg.plugins?.entries;
  return Boolean(entries) && typeof entries === "object" && Object.hasOwn(entries, pluginId);
}

function isPluginEntryExplicitlyDisabled(cfg: OriroConfig, pluginId: string): boolean {
  return cfg.plugins?.entries?.[pluginId]?.enabled === false;
}

function hasNonDisabledPluginEntry(cfg: OriroConfig, pluginId: string): boolean {
...
function hasBrowserSetupAutoEnableRelevantConfig(cfg: OriroConfig): boolean {
...
function hasAcpxSetupAutoEnableRelevantConfig(cfg: OriroConfig): boolean {
...
function hasXaiSetupAutoEnableRelevantConfig(cfg: OriroConfig): boolean {
...
function resolveRelevantSetupAutoEnablePluginIds(cfg: OriroConfig): string[] {
...
function hasSetupAutoEnableRelevantConfig(cfg: OriroConfig): boolean {
  return (
...
function hasPluginEntries(cfg: OriroConfig): boolean {
  const entries = cfg.plugins?.entries;
  return Boolean(entries) && typeof entries === "object" && Object.keys(entries).length > 0;
}

function hasPluginAllowlistWithMaterialEntries(cfg: OriroConfig): boolean {
  if (
...
function hasConfiguredProviderModelOrHarness(cfg: OriroConfig, env: NodeJS.ProcessEnv): boolean {
...
function arePluginsGloballyDisabled(cfg: OriroConfig): boolean {
  return cfg.plugins?.enabled === false;
}

function configMayNeedPluginManifestRegistry(cfg: OriroConfig, env: NodeJS.ProcessEnv): boolean {
...
function isPluginExplicitlyDisabled(cfg: OriroConfig, pluginId: string): boolean {
...
function isPluginDenied(cfg: OriroConfig, pluginId: string): boolean {
  const deny = cfg.plugins?.deny;
  return Array.isArray(deny) && deny.includes(pluginId);
}

function isPluginExplicitlySelected(cfg: OriroConfig, pluginId: string): boolean {
...
function isBuiltInChannelAlreadyEnabled(cfg: OriroConfig, channelId: string): boolean {
  const channels = cfg.channels as Record<string, unknown> | undefined;
