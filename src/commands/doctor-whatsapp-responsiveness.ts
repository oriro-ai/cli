...
function isLocalTuiCommand(command: string): boolean {
  ...
  if (executable === "oriro-tui") {
    return true;
  }
  return executable === "oriro" && LOCAL_TUI_SUBCOMMANDS.has(argv[1] ?? "");
}
...
/** Lists local Oriro TUI processes that can contend with gateway responsiveness. */
...
function hasWhatsappEnabled(cfg: OriroConfig): boolean {
...
export async function noteWhatsappResponsivenessHealth(params: {
  ...
  if (gatewayDegraded && tuiProcesses.length > 0) {
    ...
    if (params.shouldRepair) {
      ...
      warnings.push(
        `Fix: close those TUI sessions, or run ${formatCliCommand("oriro doctor --fix")}.`,
      );
    }
  }
