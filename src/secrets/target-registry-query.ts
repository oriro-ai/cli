...
const compiledChannelOriroTargets = new Map<string, CompiledTargetRegistryEntry[] | null>();
...
function getCompiledCoreOriroTargetState() {
...
function getCompiledChannelOriroTargets(
  channelId: string,
  ...
  if (compiledChannelOriroTargets.has(normalizedChannelId)) {
    return compiledChannelOriroTargets.get(normalizedChannelId) ?? null;
  }
  const compiledEntries =
    loadChannelSecretContractApi({
      ...
      ?.secretTargetRegistryEntries?.filter((entry) => entry.configFile === "oriro.json")
      .map(compileTargetRegistryEntry) ?? null;
  compiledChannelOriroTargets.set(normalizedChannelId, compiledEntries);
...
export function resolvePlanTargetAgainstRegistry(candidate: {
  ...
  const coreEntries = getCompiledCoreOriroTargetState().targetsByType.get(candidate.type);
  if (coreEntries) {
...
export function resolveConfigSecretTargetByPath(pathSegments: string[]): ResolvedPlanTarget | null {
  ...
  const explicitChannelEntries = explicitChannelId
    ? getCompiledChannelOriroTargets(explicitChannelId)
    : null;
...
/**
 * Discovers configured secret-bearing values in oriro.json using the full registry.
 */
...
/**
 * Discovers configured oriro.json targets, optionally limited to selected registry ids.
 */
export function discoverConfigSecretTargetsByIds(
  ...
  const registryState =
    ...
    Array.from(allowedTargetIds).every((targetId) =>
      getCompiledCoreOriroTargetState().knownTargetIds.has(targetId),
    )
      ? getCompiledCoreOriroTargetState()
