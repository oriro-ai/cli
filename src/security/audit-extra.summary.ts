...
function summarizeGroupPolicy(cfg: OriroConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function hasWebSearchKey(cfg: OriroConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: OriroConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: OriroConfig): boolean {
...
function isBrowserEnabled(cfg: OriroConfig): boolean {
  return cfg.browser?.enabled !== false;
}

/** Produce a concise inventory of major security-relevant surfaces. */
export function collectAttackSurfaceSummaryFindings(cfg: OriroConfig): SecurityAuditFinding[] {
