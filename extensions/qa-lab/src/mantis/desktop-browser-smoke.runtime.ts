...
const DEFAULT_BROWSER_URL = "https://oriro.ai";
...
const BROWSER_PROFILE_ARCHIVE_ENV = "ORIRO_MANTIS_BROWSER_PROFILE_TGZ_B64";
const BROWSER_PROFILE_DIR_ENV = "ORIRO_MANTIS_BROWSER_PROFILE_DIR";
...
if [ -n "$profile_archive_b64" ]; then
  profile_archive="$profile/oriro-mantis-browser-profile.tgz"
...
export async function runMantisDesktopBrowserSmoke(
  ...
  const browserProfileArchiveEnv =
    ...
    trimToValue(env.ORIRO_MANTIS_BROWSER_PROFILE_ARCHIVE_ENV) ??
  ...
  const remoteOutputDir = `/tmp/oriro-mantis-desktop-${startedAt
    .toISOString()
