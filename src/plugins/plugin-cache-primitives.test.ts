...
describe("createConfigScopedPromiseLoader", () => {
  ...
  it("caches loads by config object", async () => {
    const firstConfig = { plugins: { load: { disabled: true } } } as OriroConfig;
    const secondConfig = { plugins: { load: { disabled: false } } } as OriroConfig;
    const load = vi.fn(async (config?: OriroConfig) =>
  ...
  it("clears default and config-scoped entries", async () => {
    ...
    const loader = createConfigScopedPromiseLoader(
      async (owner?: OriroConfig) => `${owner ? "config" : "default"}-${++calls}`,
    );
