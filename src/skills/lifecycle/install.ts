...
function resolveDefaultNodeInstallStateDir({
  ...
  if (platform !== "win32" && getuid?.() === 0) {
    return path.join(path.parse(cwd).root, "var", "lib", "oriro");
  }
  return path.join(homedir(), ".oriro");
}
...
export async function installSkill(params: SkillInstallRequest): Promise<SkillInstallResult> {
  ...
  const scanResult = await evaluateSkillInstallPolicy({
    ...
    source:
      skillSource === "oriro-bundled"
        ? { kind: "bundled", authority: "oriro", mutable: false, network: false }
        : skillSource === "oriro-managed" || skillSource === "oriro-extra"
          ? { kind: "managed", authority: "oriro", mutable: false, network: false }
  ...
  const trustedInstallSources = new Set(["oriro-bundled", "oriro-managed", "oriro-extra"]);
