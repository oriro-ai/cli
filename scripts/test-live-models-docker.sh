...
LIVE_MAX_MODELS="${ORIRO_LIVE_MAX_MODELS:-}"
if [[ -n "$LIVE_MAX_MODELS" && ! "$LIVE_MAX_MODELS" =~ ^\+?[0-9]+$ ]]; then
  echo "invalid ORIRO_LIVE_MAX_MODELS: $LIVE_MAX_MODELS" >&2
  exit 2
fi
LIVE_MODEL_TIMEOUT_MS="${ORIRO_LIVE_MODEL_TIMEOUT_MS:-}"
if [[ -n "$LIVE_MODEL_TIMEOUT_MS" ]]; then
  LIVE_MODEL_TIMEOUT_MS="$(oriro_live_read_positive_int_env ORIRO_LIVE_MODEL_TIMEOUT_MS "$LIVE_MODEL_TIMEOUT_MS")"
fi
TEMP_DIRS=()
DOCKER_HOME_MOUNT=()
cleanup_temp_dirs() {
...
if oriro_live_truthy "${ORIRO_DOCKER_PROFILE_ENV_ONLY:-}"; then
  CONFIG_DIR="$(mktemp -d)"
  WORKSPACE_DIR="$(mktemp -d)"
  TEMP_DIRS+=("$CONFIG_DIR" "$WORKSPACE_DIR")
  ORIRO_DOCKER_AUTH_DIRS=none
else
...
elif [[ -n "${ORIRO_LIVE_PROVIDERS:-}" || -n "${ORIRO_LIVE_GATEWAY_PROVIDERS:-}" ]]; then
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
  done < <(
    {
      oriro_live_collect_auth_dirs_from_csv "${ORIRO_LIVE_PROVIDERS:-}"
      oriro_live_collect_auth_dirs_from_csv "${ORIRO_LIVE_GATEWAY_PROVIDERS:-}"
    } | awk '!seen[$0]++'
  )
  while IFS= read -r auth_file; do
    [[ -n "$auth_file" ]] || continue
    ...
    {
      oriro_live_collect_auth_files_from_csv "${ORIRO_LIVE_PROVIDERS:-}"
      oriro_live_collect_auth_files_from_csv "${ORIRO_LIVE_GATEWAY_PROVIDERS:-}"
    } | awk '!seen[$0]++'
  )
else
  while IFS= read -r auth_dir; do
...
echo "==> Profile env only: ${ORIRO_DOCKER_PROFILE_ENV_ONLY:-0}"
echo "==> Profile file: $PROFILE_STATUS"
echo "==> External auth dirs: ${AUTH_DIRS_CSV:-none}"
echo "==> External auth files: ${AUTH_FILES_CSV:-none}"
DOCKER_RUN_ARGS=()
oriro_live_init_docker_run_args DOCKER_RUN_ARGS "${ORIRO_LIVE_MODELS_DOCKER_RUN_TIMEOUT:-2100s}"
DOCKER_RUN_ARGS+=(--rm -t \
  ...
  -e ORIRO_LIVE_TEST=1 \
  -e ORIRO_LIVE_MODELS="${ORIRO_LIVE_MODELS:-modern}" \
  -e ORIRO_LIVE_PROVIDERS="${ORIRO_LIVE_PROVIDERS:-}" \
  -e ORIRO_LIVE_MAX_MODELS="$LIVE_MAX_MODELS" \
  -e ORIRO_LIVE_MODEL_TIMEOUT_MS="$LIVE_MODEL_TIMEOUT_MS" \
  -e ORIRO_LIVE_REQUIRE_PROFILE_KEYS="${ORIRO_LIVE_REQUIRE_PROFILE_KEYS:-}" \
  -e ORIRO_LIVE_GATEWAY_MODELS="${ORIRO_LIVE_GATEWAY_MODELS:-}" \
  -e ORIRO_LIVE_GATEWAY_PROVIDERS="${ORIRO_LIVE_GATEWAY_PROVIDERS:-}" \
  -e ORIRO_LIVE_GATEWAY_MAX_MODELS="${ORIRO_LIVE_GATEWAY_MAX_MODELS:-}" \
