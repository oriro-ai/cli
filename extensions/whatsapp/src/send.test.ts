...
import { MEDIA_FFMPEG_MAX_AUDIO_DURATION_SECS } from "oriro/plugin-sdk/media-runtime";
...
let resetLogger: typeof import("oriro/plugin-sdk/runtime-env").resetLogger;
let setLoggerOverride: typeof import("oriro/plugin-sdk/runtime-env").setLoggerOverride;

const WHATSAPP_TEST_CFG: OriroConfig = {
  channels: { whatsapp: {} },
};
...
describe("web outbound", () => {
  ...
  beforeAll(async () => {
    ...
    ({ resetLogger, setLoggerOverride } = await import("oriro/plugin-sdk/runtime-env"));
  });
  ...
  it("redacts recipients and poll text in outbound logs", async () => {
    const logPath = path.join(os.tmpdir(), `oriro-outbound-${crypto.randomUUID()}.log`);
