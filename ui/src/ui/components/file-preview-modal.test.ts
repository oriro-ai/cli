...
import type { OriroFilePreviewModal } from "./file-preview-modal.ts";
...
async function renderPreview(query = "") {
  ...
  const modal = container.querySelector<OriroFilePreviewModal>("oriro-file-preview-modal");
...
function shadowText(modal: OriroFilePreviewModal): string {
  return modal.shadowRoot?.textContent ?? "";
}

describe("oriro-file-preview-modal", () => {
  beforeEach(() => {
