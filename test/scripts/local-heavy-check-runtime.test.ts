...
function makeEnv(overrides: Record<string, string | undefined> = {}) {
  ...
  if (!Object.hasOwn(overrides, "ORIRO_LOCAL_CHECK_MODE")) {
    delete env.ORIRO_LOCAL_CHECK_MODE;
  }
  return env;
}
...
describe("local-heavy-check-runtime", () => {
  it("reenables local heavy-check policy for local wrapper entrypoints", () => {
    expect(resolveLocalHeavyCheckEnv({ ORIRO_LOCAL_CHECK: "0", PATH: "/usr/bin" })).toEqual({
      ORIRO_LOCAL_CHECK: "1",
      PATH: "/usr/bin",
    });
    expect(resolveLocalHeavyCheckEnv({ ORIRO_LOCAL_CHECK: "false", PATH: "/usr/bin" })).toEqual({
      ORIRO_LOCAL_CHECK: "1",
      PATH: "/usr/bin",
    });
  });
  ...
  it("skips declaration transforms for no-emit tsgo checks", () => {
    const { args } = applyLocalTsgoPolicy([], makeEnv({ ORIRO_LOCAL_CHECK: "0" }), ROOMY_HOST);
  ...
  it("keeps explicit tsgo flags and Go env overrides intact when throttled", () => {
    const { args, env } = applyLocalTsgoPolicy(
      ...
      makeEnv({
        GOGC: "80",
        GOMEMLIMIT: "5GiB",
        ORIRO_TSGO_PPROF_DIR: "/tmp/profile",
      }),
      CONSTRAINED_HOST,
    );
  ...
  it("keeps explicit tsgo declaration flags intact", () => {
    const env = makeEnv({ ORIRO_LOCAL_CHECK_MODE: "full" });
  ...
  it("uses the configured local tsgo build info file", () => {
    const { args } = applyLocalTsgoPolicy(
      [],
      makeEnv({
        ORIRO_LOCAL_CHECK_MODE: "full",
        ORIRO_TSGO_BUILD_INFO_FILE: ".artifacts/custom/tsgo.tsbuildinfo",
      }),
      ROOMY_HOST,
    );
  ...
  it("avoids incremental cache reuse for ad hoc tsgo runs", () => {
    const { args } = applyLocalTsgoPolicy(
      ["--extendedDiagnostics"],
      makeEnv({ ORIRO_LOCAL_CHECK_MODE: "full" }),
      ROOMY_HOST,
    );
  ...
  it("allows forcing the tsgo lock back on", () => {
    expect(
      shouldAcquireLocalHeavyCheckLockForTsgo(
        ["--help"],
        makeEnv({ ORIRO_TSGO_FORCE_LOCK: "1" }),
      ),
    ).toBe(true);
  });
  ...
  it("skips the heavy-check lock for explicit oxlint file targets", () => {
    const cwd = createTempDir("oriro-oxlint-lock-skip-");
  ...
  it("keeps the heavy-check lock for directory targets and broad oxlint runs", () => {
    const cwd = createTempDir("oriro-oxlint-lock-keep-");
  ...
  it("allows forcing the oxlint lock back on", () => {
    const cwd = createTempDir("oriro-oxlint-lock-force-");
    ...
    expect(
      shouldAcquireLocalHeavyCheckLockForOxlint(["--type-aware", "--", "sample.ts"], {
        cwd,
        env: makeEnv({ ORIRO_OXLINT_FORCE_LOCK: "1" }),
      }),
    ).toBe(true);
  });

  it("reclaims stale local heavy-check locks from dead pids", () => {
    const cwd = createTempDir("oriro-local-heavy-check-");
    const commonDir = path.join(cwd, ".git");
    const lockDir = path.join(commonDir, "oriro-local-checks", "heavy-check.lock");
  ...
  it("uses a worktree-local heavy-check lock when explicitly requested", () => {
    const repoRoot = createTempDir("oriro-local-heavy-check-worktree-");
    execFileSync("git", ["init"], { cwd: repoRoot, stdio: "ignore" });
    const cwd = path.join(repoRoot, "nested", "tooling");
    fs.mkdirSync(cwd, { recursive: true });
    const commonLockDir = path.join(repoRoot, ".git", "oriro-local-checks", "heavy-check.lock");
    ...
    const nestedLockDir = path.join(cwd, ".artifacts", "oriro-local-checks", "heavy-check.lock");

    const release = acquireLocalHeavyCheckLockSync({
      cwd,
      env: makeEnv({ ORIRO_HEAVY_CHECK_LOCK_SCOPE: "worktree" }),
  ...
  it("rejects malformed heavy-check lock timing env values", () => {
    const cwd = createTempDir("oriro-local-heavy-check-malformed-env-");

    expect(() =>
      acquireLocalHeavyCheckLockSync({
        cwd,
        env: makeEnv({ ORIRO_HEAVY_CHECK_LOCK_TIMEOUT_MS: "10ms" }),
        toolName: "oxlint",
      }),
    ).toThrow("ORIRO_HEAVY_CHECK_LOCK_TIMEOUT_MS must be a positive integer; got: 10ms");
    expect(() =>
      acquireLocalHeavyCheckLockSync({
        cwd,
        env: makeEnv({ ORIRO_HEAVY_CHECK_LOCK_POLL_MS: "0" }),
        toolName: "oxlint",
      }),
    ).toThrow("ORIRO_HEAVY_CHECK_LOCK_POLL_MS must be a positive integer; got: 0");
  });

  it("cleans up stale legacy test locks when acquiring the shared heavy-check lock", () => {
    const cwd = createTempDir("oriro-local-heavy-check-legacy-");
    const commonDir = path.join(cwd, ".git");
    const locksDir = path.join(commonDir, "oriro-local-checks");
