...
import * as bundledLlm from "oriro/plugin-sdk/llm";
...
import { installOriroInternalCorePackageNativeResolver } from "../../../plugins/plugin-sdk-native-resolver.js";
import {
...
const VIRTUAL_MODULES: Record<string, unknown> = {
  ...
  "oriro/plugin-sdk/agent-core": bundledAgentCore,
  "@oriro/plugin-sdk/agent-core": bundledAgentCore,
  "oriro/plugin-sdk/llm": bundledLlm,
  "@oriro/plugin-sdk/llm": bundledLlm,
  "oriro/plugin-sdk/agent-sessions": bundledAgentSessions,
  "@oriro/plugin-sdk/agent-sessions": bundledAgentSessions,
};
...
const EXTENSION_LOADER_ALIAS_IMPORT_PATTERN =
  /(?:@oriro\/plugin-sdk|oriro\/plugin-sdk|@sinclair\/typebox|typebox)(?:\/[A-Za-z0-9_-]+)?/u;
...
function getExtensionLoaderAliases(): Record<string, string> {
  ...
  aliases = {
    ...
    "oriro/plugin-sdk/agent-sessions": agentSessionsEntry,
    "@oriro/plugin-sdk/agent-sessions": agentSessionsEntry,
...
async function loadExtensionSourceTransformModule(
  ...
  if (!extensionSourceTransformLoader) {
    installOriroInternalCorePackageNativeResolver({ moduleUrl: import.meta.url });
...
function readResourceManifest(packageJsonPath: string): ResourceManifest | null {
  try {
    ...
    if (pkg.oriro && typeof pkg.oriro === "object") {
      return pkg.oriro as ResourceManifest;
    }
    return null;
  } catch {
    return null;
 ...
 *
 * Checks for:
 * 1. package.json with "oriro.extensions" field -> returns declared paths
 * 2. index.ts or index.js -> returns the index file
 *
 * Returns resolved paths or null if no entry points found.
 */
function resolveExtensionEntries(dir: string): string[] | null {
  // Check for package.json with "oriro" field first
 ...
 * 3. Subdirectory with package.json: `extensions/* /package.json` with "oriro" field → load what it declares
 *
...
export async function discoverAndLoadExtensions(
  ...
  for (const p of configuredPaths) {
    ...
    if (fs.existsSync(resolved) && fs.statSync(resolved).isDirectory()) {
      // Check for package.json with Oriro manifest or index.ts
