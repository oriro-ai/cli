...
import { peekSystemEventEntries } from "oriro/plugin-sdk/system-event-runtime";
...
export async function runShortTermDreamingPromotionIfTriggered(params: {
  ...
  config: ShortTermPromotionDreamingConfig;
  logger: Logger;
  subagent?: OriroPluginApi["runtime"]["subagent"];
...
export function registerShortTermPromotionDreaming(api: OriroPluginApi): void {
  ...
  const resolveCurrentConfig = (): OriroConfig =>
    (api.runtime.config?.current?.() ?? api.config) as OriroConfig;
