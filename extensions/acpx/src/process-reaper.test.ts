...
import {
  ...
  isOriroOwnedAcpxProcessCommand,
  reapStaleOriroOwnedAcpxOrphans,
  type AcpxProcessInfo,
} from "./process-reaper.js";

const WRAPPER_ROOT = "/tmp/oriro-state/acpx";
const CODEX_WRAPPER_COMMAND = `node ${WRAPPER_ROOT}/codex-acp-wrapper.mjs`;
const CODEX_WRAPPER_COMMAND_WITH_LEASE = `${CODEX_WRAPPER_COMMAND} ${ORIRO_ACPX_LEASE_ID_ARG} lease-1 ${ORIRO_GATEWAY_INSTANCE_ID_ARG} gateway-1`;
...
describe("process reaper", () => {
  ...
  it("only treats generated wrappers as launch-lease aware", () => {
    ...
    expect(
      isOriroLeaseAwareAcpxProcessCommand({ command: LOCAL_NODE_MODULES_CODEX_COMMAND }),
    ).toBe(false);
    expect(isOriroLeaseAwareAcpxProcessCommand({ command: PLUGIN_DEPS_CODEX_COMMAND })).toBe(
      false,
    );
  });

  it("recognizes Oriro plugin-runtime-deps ACP adapter children", () => {
    expect(isOriroOwnedAcpxProcessCommand({ command: PLUGIN_DEPS_CODEX_COMMAND })).toBe(true);
    expect(isOriroOwnedAcpxProcessCommand({ command: "npx @zed-industries/codex-acp" })).toBe(
      false,
    );
  });

  it("recognizes plugin-local ACP adapter package paths without trusting arbitrary installs", () => {
    expect(isOriroOwnedAcpxProcessCommand({ command: LOCAL_NODE_MODULES_CODEX_COMMAND })).toBe(
      true,
    );
    expect(
      isOriroOwnedAcpxProcessCommand({
    ...
    const { deps, killed } = cleanupDeps([
      {
        pid: 113,
        ppid: 1,
        command: `${CODEX_WRAPPER_COMMAND} ${ORIRO_ACPX_LEASE_ID_ARG} other-lease ${ORIRO_GATEWAY_INSTANCE_ID_ARG} gateway-1`,
      },
    ]);
  ...
  it("does not kill a reused pid when the live command is not Oriro-owned", async () => {
  ...
  it("skips non-owned recorded process trees", async () => {
    ...
    expect(result.skippedReason).toBe("not-oriro-owned");
    expect(killed).toStrictEqual([]);
  });

  it("reaps stale Oriro-owned wrapper and adapter orphans on startup", async () => {
    const { deps, killed } = cleanupDeps([
    ...
    const result = await reapStaleOriroOwnedAcpxOrphans({
      wrapperRoot: WRAPPER_ROOT,
      deps,
    });
