...
function clickClackHttpTimeoutMs() {
  return readPositiveInt(
    process.env.ORIRO_RELEASE_USER_JOURNEY_HTTP_TIMEOUT_MS,
    5000,
    "ORIRO_RELEASE_USER_JOURNEY_HTTP_TIMEOUT_MS",
  );
}

function clickClackHttpBodyMaxBytes() {
  return readPositiveInt(
    process.env.ORIRO_RELEASE_USER_JOURNEY_HTTP_BODY_MAX_BYTES,
    1024 * 1024,
...
function configPath() {
  return (
    ...
    path.join(process.env.HOME ?? "", ".oriro", "oriro.json")
  );
}
...
function assertOnboard() {
  ...
  assert(fs.existsSync(configPath()), "onboard did not write oriro.json");
  const stateRaw =
    fs.readFileSync(configPath(), "utf8") +
    (fs.existsSync(authPath) ? fs.readFileSync(authPath, "utf8") : "");
  assert(
    !stateRaw.includes("sk-oriro-release-user-journey"),
