...
const providerMonitorTestMocks: ProviderMonitorTestMocks = vi.hoisted(() => {
  ...
  return {
    ...
    getAcpSessionStatusMock: vi.fn(
      async (_params: { cfg: OriroConfig; sessionKey: string; signal?: AbortSignal }) => ({
        state: "idle",
      }),
    ),
...
export const baseConfig = (): OriroConfig =>
  ({
    channels: {
      discord: {
        accounts: {
...
vi.mock("oriro/plugin-sdk/acp-runtime", async () => {
  const actual = await vi.importActual<typeof import("oriro/plugin-sdk/acp-runtime")>(
    "oriro/plugin-sdk/acp-runtime",
  );
  return {
    ...actual,
...
vi.mock("oriro/plugin-sdk/native-command-config-runtime", async () => {
  const actual = await vi.importActual<
    typeof import("oriro/plugin-sdk/native-command-config-runtime")
  >("oriro/plugin-sdk/native-command-config-runtime");
  return {
    ...actual,
...
vi.mock("oriro/plugin-sdk/error-runtime", async () => {
  const actual = await vi.importActual<typeof import("oriro/plugin-sdk/error-runtime")>(
    "oriro/plugin-sdk/error-runtime",
  );
  return {
    ...actual,
