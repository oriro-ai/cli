...
export type ConfigScopedRuntimeCache<T> = WeakMap<OriroConfig, Map<string, T>>;
...
export type ConfigScopedPromiseLoader<T> = {
  load(config?: OriroConfig): Promise<T>;
  clear(): void;
};
...
export function createConfigScopedPromiseLoader<T>(
  load: (config?: OriroConfig) => T | Promise<T>,
): ConfigScopedPromiseLoader<T> {
  let defaultPromise: Promise<T> | undefined;
  let promisesByConfig = new WeakMap<OriroConfig, Promise<T>>();

  const createPromise = (config?: OriroConfig): Promise<T> => {
  ...
  return {
    async load(config?: OriroConfig): Promise<T> {
      if (!config) {
    ...
    clear(): void {
      defaultPromise = undefined;
      promisesByConfig = new WeakMap<OriroConfig, Promise<T>>();
    },
  };
}
