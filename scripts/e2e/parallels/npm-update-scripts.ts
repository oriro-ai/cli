// Npm Update Scripts script supports Oriro repository automation.
...
const windowsStalePostSwapImportRegex = String.raw`node_modules\\oriro\\dist\\[^\\]+-[A-Za-z0-9_-]+\.js`;
const macosGuestPath =
  "/opt/homebrew/bin:/opt/homebrew/opt/node/bin:/usr/local/bin:/usr/local/sbin:/opt/homebrew/sbin:/usr/bin:/bin:/usr/sbin:/sbin";
const macosOriroCommand = '"$ORIRO_BIN"';
...
function posixPrintLogTailFunction(): string {
  ...
  log_file="$1"
  max_bytes="\${ORIRO_PARALLELS_NPM_UPDATE_LOG_TAIL_BYTES:-262144}"
  case "$max_bytes" in
...
for attempt in 1 2; do
  ...
  rm -f "$HOME/.oriro/agents/main/sessions/$session_id.jsonl"
  output_file="$(mktemp)"
  set +e
  ORIRO_ALLOW_ROOT="\${ORIRO_ALLOW_ROOT:-}" ${input.auth.apiKeyEnv}=${shellQuote(input.auth.apiKeyValue)} ${command} agent --local --agent main --session-id "$session_id" --message 'Reply with exact ASCII text OK only.' --thinking off --timeout ${resolveParallelsModelTimeoutSeconds(platform)} --json >"$output_file" 2>&1
  rc=$?
  set -e
...
if [ "$agent_ok" != true ]; then
  echo "oriro agent finished without OK response" >&2
  exit 1
fi`;
}

function windowsUpdateWithBundledPluginsDisabled(input: NpmUpdateScriptInput): string {
  return `$script:OriroUpdateExit = 0
$updateOutput = Invoke-WithScopedEnv @{ ORIRO_DISABLE_BUNDLED_PLUGINS = '1'; ORIRO_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS = '1' } {
  Invoke-Oriro update --tag ${psSingleQuote(input.updateTarget)} --yes --json --no-restart 2>&1
  $script:OriroUpdateExit = $LASTEXITCODE
}
$updateExit = $script:OriroUpdateExit
$updateOutput`;
}

function windowsGatewayReadyScript(): string {
  return `function Wait-OriroGateway {
  $deadline = (Get-Date).AddSeconds(180)
  $attempt = 0
  while ((Get-Date) -lt $deadline) {
    Invoke-Oriro gateway status --deep --require-rpc --timeout 15000
    if ($LASTEXITCODE -eq 0) { return }
    $attempt += 1
    if ($attempt -eq 4) {
      Invoke-Oriro gateway start *>&1 | Out-Host
    }
    Start-Sleep -Seconds 5
  }
  throw "gateway did not become ready after update"
}
Invoke-Oriro gateway restart *>&1 | Out-Host
...
${windowsCodexPlatformPackageRepairFunction()}
$sessionPath = Join-Path $env:USERPROFILE '.oriro\\agents\\main\\sessions\\parallels-npm-update-windows.jsonl'
...
for ($attempt = 1; $attempt -le 2; $attempt++) {
  ...
  $sessionsDir = Join-Path $env:USERPROFILE '.oriro\\agents\\main\\sessions'
  $sessionPath = Join-Path $sessionsDir "$sessionId.jsonl"
  Remove-Item $sessionPath -Force -ErrorAction SilentlyContinue
  $output = Invoke-Oriro agent --local --agent main --session-id $sessionId --model ${psSingleQuote(input.auth.modelId)} --message 'Reply with exact ASCII text OK only.' --thinking off --timeout ${resolveParallelsModelTimeoutSeconds("windows")} --json 2>&1
...
if (-not $agentOk) { throw 'oriro agent finished without OK response' }`;
}
...
ORIRO_BIN="$(resolve_required_command oriro)"
scrub_future_plugin_entries() {
  python3 - <<'PY'
import json
from pathlib import Path
path = Path.home() / ".oriro" / "oriro.json"
if not path.exists():
    raise SystemExit(0)
try:
...
stop_oriro_gateway_processes() {
  ORIRO_DISABLE_BUNDLED_PLUGINS=1 "$ORIRO_BIN" gateway stop || true
  pkill -f 'oriro.*gateway' >/dev/null 2>&1 || true
...
wait_for_gateway() {
  ...
  while [ "$SECONDS" -lt "$deadline" ]; do
    if "$ORIRO_BIN" gateway status --deep --require-rpc --timeout 15000; then
      return
    fi
    sleep 2
  done
  print_log_tail /tmp/oriro-parallels-macos-gateway.log >&2
  echo "gateway did not become ready after update" >&2
  exit 1
}
scrub_future_plugin_entries
stop_oriro_gateway_processes
ORIRO_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1 ORIRO_DISABLE_BUNDLED_PLUGINS=1 "$ORIRO_BIN" update --tag ${shellQuote(input.updateTarget)} --yes --json --no-restart
${posixVersionCheck(macosOriroCommand, input.expectedNeedle)}
start_oriro_gateway
wait_for_gateway
"$ORIRO_BIN" models set ${shellQuote(input.auth.modelId)}
${posixModelProviderConfigCommands(macosOriroCommand, input.auth.modelId, "macos")}
"$ORIRO_BIN" config set agents.defaults.skipBootstrap true --strict-json
"$ORIRO_BIN" config set tools.profile minimal
${posixAgentWorkspaceScript("Parallels npm update smoke test assistant.")}
${posixAssertAgentOkScript(macosOriroCommand, input, "macos", "parallels-npm-update-macos")}`;
}
...
function Remove-FuturePluginEntries {
  $configPath = Join-Path $env:USERPROFILE '.oriro\\oriro.json'
  if (-not (Test-Path $configPath)) { return }
  try { $config = Get-Content $configPath -Raw | ConvertFrom-Json } catch { return }
  $plugins = Get-OriroJsonProperty $config 'plugins'
  if ($null -eq $plugins) { return }
  $entries = Get-OriroJsonProperty $plugins 'entries'
  if ($null -ne $entries) {
    foreach ($pluginId in @('feishu', 'whatsapp', 'openai')) {
      Remove-OriroJsonProperty $entries $pluginId
    }
  }
  $allow = Get-OriroJsonProperty $plugins 'allow'
  if ($allow -is [array]) {
    Set-OriroJsonProperty $plugins 'allow' @($allow | Where-Object { $_ -notin @('feishu', 'whatsapp', 'openai') })
  }
  $config | ConvertTo-Json -Depth 100 | Set-Content -Path $configPath -Encoding UTF8
}
function Get-OriroJsonProperty {
...
function Set-OriroJsonProperty {
...
function Remove-OriroJsonProperty {
  param([object]$Object, [string]$Name)
...
function Stop-OriroGatewayProcesses {
  Invoke-Oriro gateway stop *>&1 | Out-Host
  Get-CimInstance Win32_Process -ErrorAction SilentlyContinue |
    Where-Object { $_.CommandLine -match 'oriro.*gateway' } |
...
if ($updateExit -ne 0) {
  ...
  if (-not $stalePostSwapImport) { throw "oriro update failed with exit code $updateExit" }
  Write-Host "oriro update returned a stale post-swap module import; continuing to post-update health checks"
}
...
export ORIRO_ALLOW_ROOT=1
${posixPrintLogTailFunction()}
scrub_future_plugin_entries() {
  node - <<'JS'
const fs = require("node:fs");
const path = require("node:path");
const configPath = path.join(process.env.HOME || "/root", ".oriro", "oriro.json");
...
stop_oriro_gateway_processes() {
  ORIRO_DISABLE_BUNDLED_PLUGINS=1 ORIRO_ALLOW_ROOT=1 oriro gateway stop || true
  pkill -f 'oriro.*gateway' >/dev/null 2>&1 || true
}
start_oriro_gateway() {
...
wait_for_gateway() {
  deadline=$((SECONDS + 240))
  while [ "$SECONDS" -lt "$deadline" ]; do
    if oriro gateway status --deep --require-rpc --timeout 15000; then
      return
    fi
    sleep 2
  done
  print_log_tail /tmp/oriro-parallels-linux-gateway.log >&2
  echo "gateway did not become ready after update" >&2
  exit 1
}
scrub_future_plugin_entries
stop_oriro_gateway_processes
ORIRO_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1 ORIRO_DISABLE_BUNDLED_PLUGINS=1 oriro update --tag ${shellQuote(input.updateTarget)} --yes --json --no-restart
${posixVersionCheck("oriro", input.expectedNeedle)}
start_oriro_gateway
wait_for_gateway
oriro models set ${shellQuote(input.auth.modelId)}
${posixModelProviderConfigCommands("oriro", input.auth.modelId, "linux")}
oriro config set agents.defaults.skipBootstrap true --strict-json
oriro config set tools.profile minimal
${posixAgentWorkspaceScript("Parallels npm update smoke test assistant.")}
${posixAssertAgentOkScript("oriro", input, "linux", "parallels-npm-update-linux")}`;
}
...
while ($true) {
  $version = Invoke-Oriro --version
  $version
  if ($LASTEXITCODE -eq 0) { break }
  if ((Get-Date) -ge $versionDeadline) { throw "oriro --version failed with exit code $LASTEXITCODE" }
  Start-Sleep -Seconds 2
}`;
  }
...
while ($true) {
  ...
  if ((Get-Date) -ge $versionDeadline) {
    if ($LASTEXITCODE -ne 0) { throw "oriro --version failed with exit code $LASTEXITCODE" }
    throw ${mismatch}
  }
  Start-Sleep -Seconds 2
}`;
