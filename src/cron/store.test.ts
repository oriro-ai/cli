...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-cron-store-"));
});
...
describe("resolveCronStorePath", () => {
  ...
  it("uses ORIRO_HOME for tilde expansion", () => {
    vi.stubEnv("ORIRO_HOME", "/srv/oriro-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/oriro-home", "cron", "jobs.json"));
  });
});
