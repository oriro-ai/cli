...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.ORIRO_GATEWAY_PORT).toBe("18789");
    expect(env.ORIRO_GATEWAY_TOKEN).toBeUndefined();
    expect(env.ORIRO_SERVICE_MARKER).toBe("oriro");
    expect(env.ORIRO_SERVICE_KIND).toBe("gateway");
    expect(typeof env.ORIRO_SERVICE_VERSION).toBe("string");
    expect(env.ORIRO_SYSTEMD_UNIT).toBe("oriro-gateway.service");
    expect(env.ORIRO_WINDOWS_TASK_NAME).toBe("Oriro Gateway");
    if (process.platform === "darwin") {
      expect(env.ORIRO_LAUNCHD_LABEL).toBe("ai.oriro.gateway");
    }
  });

  it("sets the Oriro-owned launchd marker for macOS gateway services", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/Users/user" },
      port: 18789,
      platform: "darwin",
  ...
  it("passes through ORIRO_WRAPPER for gateway services", () => {
    const env = buildServiceEnvironment({
      env: {
        HOME: "/home/user",
        ORIRO_WRAPPER: " /usr/local/bin/oriro-doppler ",
      },
      port: 18789,
    });

    expect(env.ORIRO_WRAPPER).toBe("/usr/local/bin/oriro-doppler");
  });
  ...
  it("uses a durable state temp directory for macOS LaunchAgents", () => {
    ...
    expect(env.TMPDIR).toBe(path.join("/Users/user", ".oriro", "tmp"));
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", ORIRO_PROFILE: "work" },
      port: 18789,
    });
    expect(env.ORIRO_SYSTEMD_UNIT).toBe("oriro-gateway-work.service");
    expect(env.ORIRO_WINDOWS_TASK_NAME).toBe("Oriro Gateway (work)");
    if (process.platform === "darwin") {
      expect(env.ORIRO_LAUNCHD_LABEL).toBe("ai.oriro.work");
    }
  });
  ...
  it("preserves explicit systemd unit overrides", () => {
    const env = buildServiceEnvironment({
      env: {
        HOME: "/home/user",
        ORIRO_PROFILE: "work",
        ORIRO_SYSTEMD_UNIT: "oriro-gateway-maintenance",
      },
      port: 18789,
      platform: "linux",
    });

    expect(env.ORIRO_SYSTEMD_UNIT).toBe("oriro-gateway-maintenance.service");
  });
  ...
  it("preserves explicit systemd unit overrides with service suffix", () => {
    const env = buildServiceEnvironment({
      env: {
        HOME: "/home/user",
        ORIRO_SYSTEMD_UNIT: "oriro-gateway-maintenance.service",
      },
      port: 18789,
      platform: "linux",
    });
  ...
  it("sets a profile-specific launchd marker for macOS gateway services", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/Users/user", ORIRO_PROFILE: "work" },
      port: 18789,
      platform: "darwin",
    });
  ...
  it("forwards proxy URL env fallback for installed gateway services", () => {
    const env = buildServiceEnvironment({
      env: {
        HOME: "/home/user",
        ORIRO_PROXY_URL: " http://127.0.0.1:3128 ",
      },
      port: 18789,
    });

    expect(env.ORIRO_PROXY_URL).toBe("http://127.0.0.1:3128");
  });
...
describe("buildNodeServiceEnvironment", () => {
  ...
  it("sets the Oriro-owned launchd marker for macOS node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/Users/user" },
      platform: "darwin",
    });

    expect(env.ORIRO_LAUNCHD_LABEL).toBe("ai.oriro.node");
  });

  it("passes through ORIRO_GATEWAY_TOKEN for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", ORIRO_GATEWAY_TOKEN: " node-token " },
    });
    expect(env.ORIRO_GATEWAY_TOKEN).toBe("node-token");
  });

  it("passes through ORIRO_ALLOW_INSECURE_PRIVATE_WS for node services", () => {
    const env = buildNodeServiceEnvironment({
      env: { HOME: "/home/user", ORIRO_ALLOW_INSECURE_PRIVATE_WS: " 1 " },
    });
    expect(env.ORIRO_ALLOW_INSECURE_PRIVATE_WS).toBe("1");
  });

  it("omits ORIRO_GATEWAY_TOKEN when the env var is empty", () => {
    const env = buildNodeServiceEnvironment({
      env: {
        HOME: "/home/user",
        ORIRO_GATEWAY_TOKEN: "   ",
  ...
  it("marks Windows node tasks for hidden launcher startup", () => {
    ...
    expect(env.ORIRO_WINDOWS_TASK_NAME).toBe("Oriro Node");
    expect(env.ORIRO_WINDOWS_TASK_HIDDEN_LAUNCHER).toBe("1");
    expect(env.ORIRO_TASK_SCRIPT_NAME).toBe("node.cmd");
  });
});
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".oriro"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", ORIRO_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".oriro-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", ORIRO_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".oriro"));
  });

  it("uses ORIRO_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", ORIRO_STATE_DIR: "/var/lib/oriro" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/oriro"));
  });

  it("expands ~ in ORIRO_STATE_DIR", () => {
    const env = { HOME: "/Users/test", ORIRO_STATE_DIR: "~/oriro-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/oriro-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { ORIRO_STATE_DIR: "C:\\State\\oriro" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\oriro");
  });
});
