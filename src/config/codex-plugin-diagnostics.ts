...
function isOpenAiCodexDefaultRuntimeSelection(params: {
  ...
  // Custom OpenAI-compatible base URLs stay on the Oriro runtime path.
...
function codexPluginEntryEnabled(cfg: OriroConfig): boolean | undefined {
...
function openAiProviderRuntimePolicy(cfg: OriroConfig): AgentRuntimePolicyConfig | undefined {
...
function listConfiguredAgentIds(cfg: OriroConfig): Array<string | undefined> {
...
function openAiHasCodexDefaultRuntimePolicy(cfg: OriroConfig): boolean {
  for (const [providerId, providerConfig] of Object.entries(cfg.models?.providers ?? {})) {
    if (normalizeProviderId(providerId) !== OPENAI_PROVIDER_ID) {
      continue;
    }
    ...
    // A model-scoped explicit "auto"/"default" overrides provider-wide PI/Oriro
...
function openAiDefaultRouteKeepsCodexUnavailable(cfg: OriroConfig): boolean {
  const policy = openAiDefaultRouteRuntimePolicy(cfg);
  if (!policy?.id?.trim()) {
    // With no explicit runtime policy, the OpenAI route only needs Codex on the
    // official OpenAI endpoint. OpenAI-compatible proxies stay on Oriro.
    return !openAIProviderUsesCodexRuntimeByDefault({
      provider: OPENAI_PROVIDER_ID,
      config: cfg,
    });
 ...
 */
export function configExplicitlyKeepsCodexUnavailableForOpenAi(cfg: OriroConfig): boolean {
 ...
 * all default routes would otherwise stay on the Oriro runtime.
 */
export function shouldSuppressMissingCodexPluginDiagnostics(cfg: OriroConfig): boolean {
