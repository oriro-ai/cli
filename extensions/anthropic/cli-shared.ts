 ...
 * thinking levels, and keeps Oriro-managed CLI runs isolated from shell env.
 */
import type {
  CliBackendConfig,
...
// steer Oriro-managed Claude CLI runs toward a different provider,
// endpoint, token source, plugin/config tree, or telemetry bootstrap mode.
/** Environment variables removed before launching Oriro-managed Claude CLI runs. */
...
function isOriroRequestedYolo(context?: CliBackendNormalizeConfigContext): boolean {
...
/** Resolve Claude permission mode from Oriro exec security settings. */
export function resolveClaudePermissionMode(context?: CliBackendNormalizeConfigContext): {
  mode?: string;
  overrideExisting: boolean;
} {
  return isOriroRequestedYolo(context)
...
/** Map Oriro thinking levels to Claude CLI effort flags for a model id. */
