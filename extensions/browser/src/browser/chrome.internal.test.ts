...
vi.mock("oriro/plugin-sdk/ssrf-runtime-internal", () => ({
...
vi.mock("../infra/tmp-oriro-dir.js", () => ({
  resolvePreferredOriroTmpDir: () => "/tmp/oriro-browser-test",
}));
...
import {
  buildOriroChromeLaunchArgs,
  getChromeWebSocketUrl,
  isChromeCdpReady,
  isChromeReachable,
  launchOriroChrome,
  resolveOriroUserDataDir,
  stopOriroChrome,
} from "./chrome.js";
 ...
 * not exercise: launchOriroChrome (with child_process.spawn mocked),
 * canRunCdpHealthCommand all branches, canOpenWebSocket failure,
 * stopOriroChrome SIGKILL fallback, fs.exists() catch, default
 * profile name, buildOriroChromeLaunchArgs branches, and friends.
 */
...
describe("chrome.ts internal", () => {
  ...
  describe("resolveOriroUserDataDir", () => {
    it("falls back to the default profile name when none is supplied", () => {
      const dir = resolveOriroUserDataDir();
      expect(dir.endsWith(path.join("oriro", "user-data"))).toBe(true);
    });

    it("respects an explicit profile name", () => {
      const dir = resolveOriroUserDataDir("my-profile");
      expect(dir.endsWith(path.join("my-profile", "user-data"))).toBe(true);
    });
  });

  describe("buildOriroChromeLaunchArgs branches", () => {
    ...
    it("toggles headless args", () => {
      const args = buildOriroChromeLaunchArgs({
  ...
  describe("fs.exists() catch branch", () => {
    it("treats a throwing fs.existsSync (for prefs files) as non-existent to force bootstrap", async () => {
      ...
      // exists() invocation inside launchOriroChrome.
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          ...
          const running = await launchOriroChrome(resolved, profile);
  ...
  describe("launchOriroChrome", () => {
    let tmpDir = "";

    beforeEach(async () => {
      tmpDir = await fsp.mkdtemp(path.join(os.tmpdir(), "oriro-launch-"));
    });
    ...
    it("rejects a remote profile before attempting to spawn", async () => {
      ...
      await expect(launchOriroChrome(makeResolved(), profile)).rejects.toThrow(
    ...
    it.each([
      ...
      async ({ cdpUrl, configuredProbeHost }) => {
        ...
        await expect(launchOriroChrome(makeResolved(), profile)).rejects.toThrow(portBusy);
    ...
    it("completes successfully when Chrome reports /json/version and CDP is reachable", async () => {
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          ...
          const running = await launchOriroChrome(makeResolved(), profile);
          expect(running.pid).toBe(4242);
    ...
    it("accepts a ready CDP diagnostic after the launch HTTP probe expires", async () => {
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          const port = new URL(baseUrl).port;
          const profile = makeProfile(Number(port));
          const running = await launchOriroChrome(
    ...
    it("clears stale singleton locks and retries once after profile-in-use launch failure", async () => {
      const configPath = path.join(tmpDir, "oriro.json");
      await fsp.writeFile(
        configPath,
        JSON.stringify({
          logging: {
      ...
      await withMockChromeCdpServer({
        ...
        run: async (baseUrl) => {
          ...
          const userDataDir = resolveOriroUserDataDir(profile.name);
        ...
        for (const testCase of [
          ...
          await withMockChromeCdpServer({
            ...
            run: async (baseUrl) => {
              ...
              const userDataDir = resolveOriroUserDataDir(`${profile.name}-${testCase.pid}`);
              ...
              try {
                await expect(
                  launchOriroChrome(makeResolved(), profileWithUniqueName),
    ...
    it("throws with stderr hint + sandbox hint when CDP never becomes reachable", async () => {
      ...
      try {
        ...
        await expect(launchOriroChrome(resolved, profile)).rejects.toThrow(
  ...
  describe("stopOriroChrome SIGKILL fallback", () => {
    it("escalates to SIGKILL when CDP keeps reporting reachable past the deadline", async () => {
      ...
      await stopOriroChrome(
        { proc, cdpPort: 12345 } as unknown as Parameters<typeof stopOriroChrome>[0],
        1,
      );
  ...
  describe("launchOriroChrome remaining branches", () => {
    it("skips decoration entirely when the profile is already decorated", async () => {
      ...
      const stageDir = await fsp.mkdtemp(path.join(os.tmpdir(), "oriro-decorated-"));
      try {
        const profileName = path.basename(stageDir);
        const colorHex = "#FF4500";
        const colorInt = ((0xff << 24) | 0xff4500) >> 0;
        const userDataDir = path.join(resolveOriroUserDataDir(profileName));
        ...
        const staged = resolveOriroUserDataDir(path.basename(stageDir));
    ...
    it("falls back to the default color when profile.color is undefined", async () => {
      // Covers the `profile.color ?? DEFAULT_ORIRO_BROWSER_COLOR` coalescing.
      vi.spyOn(fs, "existsSync").mockImplementation((p) => {
        const s = String(p);
        if (
          s.includes("Google Chrome") ||
    ...
    it("buffers stderr chunks when Chrome emits diagnostics while CDP comes up", async () => {
      ...
      const configDir = await fsp.mkdtemp(path.join(os.tmpdir(), "oriro-redact-off-"));
      ...
      let message = "";
      try {
        await launchOriroChrome(resolved, profile);
      } catch (err) {
    ...
    it("logs a warning when decorateOriroProfile throws and still returns a running Chrome", async () => {
      // Covers the decoration catch branch (log.warn).
      const { decorateOriroProfile } = await import("./chrome.profile-decoration.js");
      vi.spyOn(fs, "existsSync").mockImplementation((p) => {
        const s = String(p);
        if (
          s.includes("Google Chrome") ||
      ...
      const decorationSpy = vi
        .spyOn({ decorateOriroProfile }, "decorateOriroProfile")
        .mockImplementation(() => {
      ...
      const writeSpy = vi.spyOn(fs, "writeFileSync").mockImplementation((p) => {
        const s = String(p);
        if (s.endsWith(".oriro-profile-decorated") || s.endsWith("Preferences")) {
  ...
  describe("launchOriroChrome managed-proxy CDP bypass", () => {
    ...
    it("surfaces loopbackMode=block as BrowserProfileUnavailableError without spawning Chrome", async () => {
      ...
      await expect(launchOriroChrome(makeResolved(), profile)).rejects.toBeInstanceOf(
    ...
    it("does not register a bypass for a remote attachOnly CDP URL (loopback gate)", async () => {
      stubExecutableAndPrefsExist();
      // For this test we want launchOriroChrome to reject before any
      ...
      await expect(launchOriroChrome(makeResolved(), remoteProfile)).rejects.toThrow(
        /is remote; cannot launch local Chrome/,
      );
