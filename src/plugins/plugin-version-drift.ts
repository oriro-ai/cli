...
import {
  resolveTrustedSourceLinkedOfficialOriroHubInstall,
...
function isPluginEnabled(config: OriroConfig | undefined, pluginId: string): boolean {
...
function shouldCompareOfficialInstallToGateway(params: {
  ...
  const officialOriroHubInstall = resolveTrustedSourceLinkedOfficialOriroHubInstall(params);
  if (officialOriroHubInstall) {
    if (officialOriroHubInstall.orirohubSpec) {
      return !parseOriroHubPluginSpec(officialOriroHubInstall.orirohubSpec)?.version;
    }
    return (
      parseRegistryNpmSpec(officialOriroHubInstall.npmSpec ?? "")?.selectorKind !== "exact-version"
    );
  }
  return false;
}
 ...
 *   `version` field of the installed oriro package.json).
 ...
 * @param params.config The merged daemon-side OriroConfig (optional).
