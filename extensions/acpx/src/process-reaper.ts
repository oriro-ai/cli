 ...
 * Oriro-owned wrapper trees after validating paths, packages, and lease ids.
 */
...
const ORIRO_PLUGIN_DEPS_MARKER = "/plugin-runtime-deps/";
...
export type AcpxProcessCleanupResult = {
  ...
  skippedReason?: "missing-root" | "not-oriro-owned" | "unverified-root";
};
...
function resolveOriroInstallRoot(pluginRoot: string): string {
  if (
...
/** Check whether a command references an Oriro-generated ACPX wrapper path. */
export function isOriroLeaseAwareAcpxProcessCommand(params: {
...
function liveCommandMatchesLeaseIdentity(params: {
  ...
  return (
    commandOptionEquals(parts, ORIRO_ACPX_LEASE_ID_ARG, params.expectedLeaseId) &&
    commandOptionEquals(parts, ORIRO_GATEWAY_INSTANCE_ID_ARG, params.expectedGatewayInstanceId)
  );
}

/** Check whether a command is owned by Oriro ACPX runtime packages or wrappers. */
export function isOriroOwnedAcpxProcessCommand(params: {
  command: string | undefined;
  wrapperRoot?: string;
}): boolean {
  const command = params.command?.trim();
  ...
  if (!normalized.includes(ORIRO_PLUGIN_DEPS_MARKER)) {
    return false;
  }
...
/** Terminate one validated Oriro-owned ACPX wrapper process tree. */
export async function cleanupOriroOwnedAcpxProcessTree(params: {
  rootPid?: number;
  rootCommand?: string;
  expectedLeaseId?: string;
  expectedGatewayInstanceId?: string;
  ...
  // that this PID still belongs to an Oriro-owned wrapper, fail closed to
...
/** Reap orphaned Oriro-owned ACPX wrapper trees during runtime startup. */
export async function reapStaleOriroOwnedAcpxOrphans(params: {
  wrapperRoot: string;
  deps?: AcpxProcessCleanupDeps;
