#!/usr/bin/env bun
// Discord Acp Plain Language Smoke script supports Oriro repository automation.
...
type DriverMode = "token" | "webhook" | "oriro";
...
const DEFAULT_ORIRO_CLI_TIMEOUT_MS = 60_000;
...
function resolveStateDir(): string {
  const override = process.env.ORIRO_STATE_DIR?.trim();
  if (override) {
    if (override === "~") {
  ...
  const home = process.env.ORIRO_HOME?.trim() || process.env.HOME || "";
  return path.join(home, ".oriro");
}
...
function parseDriverMode(raw: string): DriverMode {
  ...
  if (normalized === "token" || normalized === "webhook" || normalized === "oriro") {
    return normalized;
  }
  throw new Error(
    `Invalid --driver value ${JSON.stringify(raw)}; expected token, webhook, or oriro.`,
  );
}
...
function usage(): string {
  return (
    ...
    "  --driver <token|webhook|oriro> Driver transport mode (default: token)\n" +
    ...
    "  ORIRO_DISCORD_SMOKE_CHANNEL_ID\n" +
    "  ORIRO_DISCORD_SMOKE_DRIVER\n" +
    "  ORIRO_DISCORD_SMOKE_DRIVER_TOKEN\n" +
    "  ORIRO_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX\n" +
    "  ORIRO_DISCORD_SMOKE_BOT_TOKEN\n" +
    "  ORIRO_DISCORD_SMOKE_BOT_TOKEN_PREFIX\n" +
    "  ORIRO_DISCORD_SMOKE_AGENT\n" +
    "  ORIRO_DISCORD_SMOKE_MENTION_USER_ID\n" +
    "  ORIRO_DISCORD_SMOKE_TIMEOUT_MS\n" +
    "  ORIRO_DISCORD_SMOKE_POLL_MS\n" +
    "  ORIRO_STATE_DIR\n" +
    "  ORIRO_DISCORD_SMOKE_ORIRO_BIN"
  );
}
...
function parseArgs(argv = process.argv.slice(2)): Args {
  ...
  const channelId =
    resolveArg("--channel", argv) || process.env.ORIRO_DISCORD_SMOKE_CHANNEL_ID || "";
  const driverModeRaw =
    resolveArg("--driver", argv) || process.env.ORIRO_DISCORD_SMOKE_DRIVER || "token";
  const driverMode = parseDriverMode(driverModeRaw);
  const driverToken =
    resolveArg("--token", argv) || process.env.ORIRO_DISCORD_SMOKE_DRIVER_TOKEN || "";
  const driverTokenPrefix =
    resolveArg("--token-prefix", argv) ||
    process.env.ORIRO_DISCORD_SMOKE_DRIVER_TOKEN_PREFIX ||
    "Bot";
  const botToken =
    resolveArg("--bot-token", argv) ||
    process.env.ORIRO_DISCORD_SMOKE_BOT_TOKEN ||
    process.env.DISCORD_BOT_TOKEN ||
    "";
  const botTokenPrefix =
    resolveArg("--bot-token-prefix", argv) ||
    process.env.ORIRO_DISCORD_SMOKE_BOT_TOKEN_PREFIX ||
    "Bot";
  const targetAgent =
    resolveArg("--agent", argv) || process.env.ORIRO_DISCORD_SMOKE_AGENT || "codex";
  const mentionUserId =
    resolveArg("--mention", argv) ||
    process.env.ORIRO_DISCORD_SMOKE_MENTION_USER_ID ||
    undefined;
  const instruction =
    resolveArg("--instruction", argv) ||
    process.env.ORIRO_DISCORD_SMOKE_INSTRUCTION ||
    undefined;
  const timeoutMs = parseNumber(
    resolveArg("--timeout-ms", argv) || process.env.ORIRO_DISCORD_SMOKE_TIMEOUT_MS,
    240_000,
    "--timeout-ms",
  );
  const pollMs = parseNumber(
    resolveArg("--poll-ms", argv) || process.env.ORIRO_DISCORD_SMOKE_POLL_MS,
    1_500,
    "--poll-ms",
  );
  const stateDir = path.resolve(resolveArg("--state-dir", argv) || resolveStateDir());
  const oriroBin =
    resolveArg("--oriro-bin", argv) ||
    process.env.ORIRO_DISCORD_SMOKE_ORIRO_BIN ||
    "oriro";
...
async function oriroCliJson<T>(params: {
  oriroBin: string;
  args: string[];
  timeoutMs?: number;
}): Promise<T> {
  const result = await execFileAsync(params.oriroBin, params.args, {
    maxBuffer: 8 * 1024 * 1024,
    env: process.env,
    timeout: params.timeoutMs ?? DEFAULT_ORIRO_CLI_TIMEOUT_MS,
    killSignal: "SIGKILL",
  });
  const stdout = (result.stdout || "").trim();
  if (!stdout) {
    throw new Error(`oriro ${params.args.join(" ")} returned empty stdout`);
  }
  return JSON.parse(stdout) as T;
}

async function readMessagesWithOriro(params: {
  oriroBin: string;
  target: string;
  limit: number;
  timeoutMs?: number;
}): Promise<DiscordMessage[]> {
  const response = await oriroCliJson<{
    payload?: {
      messages?: DiscordMessage[];
    };
  }>({
    oriroBin: params.oriroBin,
    args: [
      "message",
      "read",
      "--channel",
...
async function readThreadBindings(stateDir: string): Promise<ThreadBindingRecord[]> {
  const store = createPluginStateKeyedStore<ThreadBindingRecord>("discord", {
    ...
    env: { ...process.env, ORIRO_STATE_DIR: stateDir },
  });
...
async function loadParentRecentMessages(params: {
  ...
  if (params.args.driverMode === "oriro") {
    return await readMessagesWithOriro({
      oriroBin: params.args.oriroBin,
  ...
  try {
    if (args.driverMode === "token") {
      ...
      const webhook = await discordApi<{ id: string; token?: string | null }>({
        ...
        timeoutMs: remainingTimeoutMs(deadline),
        body: {
          name: `oriro-acp-smoke-${smokeId.slice(-8)}`,
        },
      });
      ...
      if (!sentMessageId) {
        throw new Error("oriro message send did not return payload.result.messageId");
      }
    }
  } catch (err) {
    ...
    while (Date.now() < deadline && !ackMessage) {
      try {
        const threadMessages =
          args.driverMode === "oriro"
            ? await readMessagesWithOriro({
                oriroBin: args.oriroBin,
                target: threadId,
                limit: 50,
                timeoutMs: remainingTimeoutMs(deadline),
