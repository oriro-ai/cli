...
type TaskFlowRegistryTestDatabase = Pick<OriroStateKyselyDatabase, "flow_runs">;
...
async function withFlowRegistryTempDir<T>(run: (root: string) => Promise<T>): Promise<T> {
  return await withOriroTestState(
    {
      layout: "state-only",
      prefix: "oriro-task-flow-store-",
    },
    async (state) => {
      const root = state.stateDir;
      resetTaskFlowRegistryForTests();
      try {
        return await withEnvAsync({ ORIRO_STATE_DIR: root }, async () => await run(root));
      } finally {
        resetTaskFlowRegistryForTests();
      }
    },
