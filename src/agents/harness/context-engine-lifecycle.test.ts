...
import { ORIRO_RUNTIME_CONTEXT_CUSTOM_TYPE } from "../internal-runtime-context.js";
import {
...
function runtimeContextMessage(content: string, timestamp: number): AgentMessage {
  ...
  return {
    role: "custom",
    customType: ORIRO_RUNTIME_CONTEXT_CUSTOM_TYPE,
    content,
    display: false,
...
describe("harness context engine lifecycle", () => {
  ...
  describe("assembleHarnessContextEngine result validation", () => {
    ...
    it("rejects an assemble result missing the messages array (lossless-oriro shape)", async () => {
