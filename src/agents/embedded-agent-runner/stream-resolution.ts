...
function isDefaultOriroStreamFnForModel(
...
function resolveOriroNativeCodexResponsesStreamFn(params: {
  model: EmbeddedRunAttemptParams["model"];
...
export function describeEmbeddedAgentStreamStrategy(params: {
  ...
  if (
    resolveOriroNativeCodexResponsesStreamFn({
      model: params.model,
      currentStreamFn: params.currentStreamFn,
    })
  ) {
    return "oriro-native-codex-responses";
  }
  if (isDefaultOriroStreamFnForModel(params.model, params.currentStreamFn)) {
...
export function resolveEmbeddedAgentStreamFn(params: {
  ...
  if (
    isDefaultOriroStreamFnForModel(params.model, params.currentStreamFn) ||
    hasResolvedRuntimeApiKey(params.resolvedApiKey)
  ) {
    const boundaryAwareStreamFn = createBoundaryAwareStreamFnForModel(params.model);
    if (boundaryAwareStreamFn) {
      // Some Oriro session factories return a provider-specific stream wrapper
      // once runtime auth is resolved. Keep transport-supported APIs on
      // Oriro's HTTP transport so provider-specific auth/header semantics
