...
describe("scripts/mantis/publish-pr-evidence", () => {
  it("renders a manifest-driven PR comment with inline screenshots and video links", () => {
    ...
    expect(body).toContain(
      '<td width="50%" align="center"><img src="https://qa.oriro.ai/mantis/discord/pr-1/run-1/baseline.png" width="100%"',
    );
    expect(body).toContain(
      "[Baseline change MP4](https://qa.oriro.ai/mantis/discord/pr-1/run-1/baseline-change.mp4)",
    );
  ...
  it("uploads manifest artifacts to R2-compatible object storage", async () => {
    ...
    const published = await publishArtifactFiles({
      ...
      storageConfig: {
        ...
        publicBaseUrl: "https://qa.oriro.ai",
        region: "auto",
    ...
    expect(published).toEqual({
      artifactRoot: "mantis/discord/pr-1/run-1",
      rawBase: "https://qa.oriro.ai/mantis/discord/pr-1/run-1",
      treeUrl: "https://qa.oriro.ai/mantis/discord/pr-1/run-1/index.json",
    });
    ...
    expect(String(requests[4]?.body)).toContain(
      '"url": "https://qa.oriro.ai/mantis/discord/pr-1/run-1/baseline.png"',
    );
  });
  ...
  it("allows failure manifests to omit optional visual artifacts", () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        ...
        scenario: "slack-oriro-desktop-smoke",
        comparison: {
          candidate: {
