---
summary: "Use Ollama Cloud directly with Oriro"
read_when:
...
Ollama Cloud is Ollama's hosted model API. It lets Oriro call Ollama-hosted
...
`/api/chat` style, not the OpenAI-compatible `/v1` route. Oriro registers it
...
- You want the same native Ollama chat API shape Oriro uses for local
...
Oriro discovers Ollama Cloud models from the live hosted catalog. Commonly
...
```bash
oriro models list --provider ollama-cloud
oriro models set ollama-cloud/kimi-k2.6
```
...
ORIRO_LIVE_OLLAMA_MODEL=kimi-k2.6 \
...
- Unknown model errors: run `oriro models list --provider ollama-cloud` and
