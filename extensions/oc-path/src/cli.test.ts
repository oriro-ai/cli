/**
 * Smoke tests for the `oriro path` CLI handlers.
 *
...
describe("oriro path CLI", () => {
  ...
  describe("set", () => {
    ...
    it("CLI-S08 sets slash-deep JSONC paths and parsed JSON values", async () => {
      const filePath = join(workspaceDir, "oriro.json");
      writeFileSync(
        filePath,
      ...
      await pathSetCommand(
        "oc://oriro.json/gateway/auth/token",
      ...
      await pathSetCommand(
        "oc://oriro.json/agents/list/0/tools/exec/security",
        "allowlist",
        { cwd: workspaceDir, json: true },
        rt2,
      );
