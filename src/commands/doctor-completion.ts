...
export type ShellCompletionStatus = {
  ...
  cacheExists: boolean;
  cachePath: string;
  /** True if profile uses slow dynamic pattern like `source <(oriro completion ...)` */
    ...
    return [
      {
        ...
        path: pathLocal,
        fixHint: "Run `oriro doctor --fix` to upgrade to cached completion.",
      },
    ];
  }
    ...
    return [
      {
        ...
        path: pathLocal,
        fixHint: `Run \`oriro completion --write-state\` or \`oriro doctor --fix\` to regenerate ${status.cachePath}.`,
      },
    ];
  }
  return [];
