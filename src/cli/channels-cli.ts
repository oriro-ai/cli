...
export async function registerChannelsCli(
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["oriro channels list", "List configured channels."],
          ["oriro channels list --all", "Show configured, bundled, and installable channels."],
          ["oriro channels add", "Open guided channel setup."],
          ["oriro channels status --probe", "Run channel status checks and probes."],
          [
            "oriro channels add --channel telegram --token <token>",
            "Add or update a channel account non-interactively.",
          ],
          ["oriro channels login --channel whatsapp", "Link a WhatsApp Web account."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink(
          "/cli/channels",
          "docs.oriro.ai/cli/channels",
        )}\n`,
    );
  ...
  const addCommand = channels
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["oriro channels add", "Open guided setup for available chat channels."],
          [
            "oriro channels add --channel telegram --token <token>",
            "Add or update Telegram non-interactively.",
          ],
          ["oriro channels list --all", "Find channel ids before using --channel."],
        ])}\n`,
    )
    .option("--channel <name>", `Channel (${channelNames})`)
