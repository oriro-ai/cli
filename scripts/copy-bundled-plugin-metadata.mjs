...
function shouldCopyBundledPluginMetadata(id, env, buildablePluginDirs) {
  ...
  return env.ORIRO_BUILD_PRIVATE_QA === "1";
}
...
export function copyBundledPluginMetadata(params = {}) {
  ...
  for (const dirent of fs.readdirSync(extensionsRoot, { withFileTypes: true })) {
    ...
    const distManifestPath = path.join(distPluginDir, "oriro.plugin.json");
    ...
    if (packageJson.oriro && "extensions" in packageJson.oriro) {
      packageJson.oriro = {
        ...packageJson.oriro,
        extensions: rewritePackageExtensions(packageJson.oriro.extensions),
        ...(typeof packageJson.oriro.setupEntry === "string"
          ? { setupEntry: rewritePackageEntry(packageJson.oriro.setupEntry) }
          : {}),
      };
    }
