...
Oriro gives agents tools to work across sessions, inspect status, and
...
After the target responds, Oriro can run a **reply-back loop** where the
...
the caller's current session; visible client labels such as `oriro-tui` are
not session keys.
...
- `deliveryContext` is the persisted delivery route stored on the session,
  which Oriro can reuse for later delivery even when the active surface
  differs.
...
`subagents` is the visibility helper for already spawned Oriro
...
the completion origin only identifies a channel Oriro can still reuse the
