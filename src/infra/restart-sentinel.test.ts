...
async function withRestartSentinelStateDir(run: () => Promise<void>): Promise<void> {
  await withTempDir({ prefix: "oriro-sentinel-" }, async (tempDir) => {
    try {
      await withEnvAsync({ ORIRO_STATE_DIR: tempDir }, run);
    } finally {
      closeOriroStateDatabaseForTest();
    }
  });
...
describe("restart sentinel", () => {
  ...
  it("imports a legacy file sentinel into sqlite once", async () => {
    await withRestartSentinelStateDir(async () => {
      ...
      const legacyPath = path.join(process.env.ORIRO_STATE_DIR ?? "", "restart-sentinel.json");
...
describe("restart sentinel message dedup", () => {
  ...
  it("formats the non-interactive doctor command as actionability guidance", () => {
    expect(formatDoctorNonInteractiveHint({ PATH: "/usr/bin:/bin" })).toBe(
      "Recommended follow-up: run oriro doctor --non-interactive in a terminal or approvals-capable Oriro surface.",
    );
  });
  ...
  it("keeps profile-aware doctor guidance actionable outside constrained delivery surfaces", () => {
    expect(
      formatDoctorNonInteractiveHint({
        ORIRO_PROFILE: "isolated",
        PATH: "/usr/bin:/bin",
      }),
    ).toBe(
      "Recommended follow-up: run oriro --profile isolated doctor --non-interactive in a terminal or approvals-capable Oriro surface.",
    );
  });
});
