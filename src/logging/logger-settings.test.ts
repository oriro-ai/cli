...
beforeEach(() => {
  envSnapshot = captureEnv(["ORIRO_TEST_FILE_LOG", "ORIRO_LOG_LEVEL"]);
  delete process.env.ORIRO_TEST_FILE_LOG;
...
describe("getResolvedLoggerSettings", () => {
  ...
  it("reads logging config when test file logging is explicitly enabled", () => {
    ...
    expect(settings.file).toBe("/tmp/oriro-configured.log");
