...
export function resolveOpenAITransportTurnState(
  ...
  return {
    headers: {
      ...sessionHeaders,
      "x-oriro-turn-id": turnId,
      "x-oriro-turn-attempt": attempt,
    },
    metadata: {
      oriro_session_id: sessionHeaders["x-oriro-session-id"] ?? "",
      oriro_turn_id: turnId,
      oriro_turn_attempt: attempt,
      oriro_transport: ctx.transport,
    },
  };
}
