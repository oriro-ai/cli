...
read_when:
  ...
  - You are defining plugin config schemas or package.json oriro metadata
---

Reference for plugin packaging (`package.json` metadata), manifests (`oriro.plugin.json`), setup entries, and config schemas.
...
Your `package.json` needs an `oriro` field that tells the plugin system what your plugin provides:

<Tabs>
  <Tab title="Channel plugin">
    ```json
    {
      "name": "@myorg/oriro-my-channel",
      "version": "1.0.0",
      "type": "module",
      "oriro": {
        "extensions": ["./index.ts"],
  ...
  <Tab title="Provider plugin / OriroHub baseline">
    ```json oriro-orirohub-package.json
    {
      "name": "@myorg/oriro-my-plugin",
      "version": "1.0.0",
      "type": "module",
      "oriro": {
        "extensions": ["./index.ts"],
...
<Note>
If you publish the plugin externally on OriroHub, those `compat` and `build` fields are required. The canonical publish snippets live in `docs/snippets/plugin-publish/`.
</Note>
...
`oriro.channel` is cheap package metadata for channel discovery and setup surfaces before runtime loads.
...
`oriro.install` is package metadata, not manifest metadata.

| Field                        | Type                                | What it means                                                                     |
| ---------------------------- | ----------------------------------- | --------------------------------------------------------------------------------- |
| `orirohubSpec`                | `string`                            | Canonical OriroHub spec for install/update and onboarding install-on-demand flows. |
...
| `defaultChoice`              | `"orirohub"` \| `"npm"` \| `"local"` | Preferred install source when multiple sources are available.                     |
| `minHostVersion`             | `string`                            | Minimum supported Oriro version in the form `>=x.y.z` or `>=x.y.z-prerelease`. |
...
<AccordionGroup>
  <Accordion title="Onboarding behavior">
    Interactive onboarding also uses `oriro.install` for install-on-demand surfaces. If your plugin exposes provider auth choices or channel setup/catalog metadata before runtime loads, onboarding can show that choice, prompt for OriroHub, npm, or local install, install or enable the plugin, then continue the selected flow. OriroHub onboarding choices use `orirohubSpec` and are preferred when present; npm choices require trusted catalog metadata with a registry `npmSpec`; exact versions and `expectedIntegrity` are optional npm pins. If `expectedIntegrity` is present, install/update flows enforce it for npm. Keep the "what to show" metadata in `oriro.plugin.json` and the "how to install it" metadata in `package.json`.
  </Accordion>
...
When enabled, Oriro loads only `setupEntry` during the pre-listen startup phase, even for already-configured channels. The full entry loads after the gateway starts listening.
...
Every native plugin must ship an `oriro.plugin.json` in the package root. Oriro uses this to validate config without executing plugin code.

```json
{
  "id": "my-plugin",
  "name": "My Plugin",
  "description": "Adds My Plugin capabilities to Oriro",
  "configSchema": {
    "type": "object",
...
For plugin packages, use the package-specific OriroHub command:
...
<Note>
The legacy skill-only publish alias is for skills. Plugin packages should always use `orirohub package publish`.
</Note>

## Setup entry

The `setup-entry.ts` file is a lightweight alternative to `index.ts` that Oriro loads when it only needs setup surfaces (onboarding, config repair, disabled channel inspection).

```typescript
// setup-entry.ts
import { defineSetupPluginEntry } from "oriro/plugin-sdk/channel-core";
...
Bundled workspace channels that keep setup-safe exports in sidecar modules can use `defineBundledChannelSetupEntry(...)` from `oriro/plugin-sdk/channel-entry-contract` instead of `defineSetupPluginEntry(...)`. That bundled contract also supports an optional `runtime` export so setup-time runtime wiring can stay lightweight and explicit.

<AccordionGroup>
  <Accordion title="When Oriro uses setupEntry instead of the full entry">
...
CLI wizard locale (`ORIRO_LOCALE`, then system locale variables) and falls
...
```typescript
import { z } from "zod";
import { buildChannelConfigSchema } from "oriro/plugin-sdk/channel-config-schema";
...
If you already author the contract as JSON Schema or TypeBox, use the direct helper so Oriro can skip Zod-to-JSON-Schema conversion on metadata paths:

```typescript
import { Type } from "typebox";
import { buildJsonChannelConfigSchema } from "oriro/plugin-sdk/channel-config-schema";
...
For third-party plugins, the cold-path contract is still the plugin manifest: mirror the generated JSON Schema into `oriro.plugin.json#channelConfigs` so config schema, setup, and UI surfaces can inspect `channels.<id>` without loading runtime code.

## Setup wizards

Channel plugins can provide interactive setup wizards for `oriro onboard`. The wizard is a `ChannelSetupWizard` object on the `ChannelPlugin`:

```typescript
import type { ChannelSetupWizard } from "oriro/plugin-sdk/channel-setup";
...
<AccordionGroup>
  <Accordion title="Shared allowFrom prompts">
    For DM allowlist prompts that only need the standard `note -> prompt -> parse -> merge -> patch` flow, prefer the shared setup helpers from `oriro/plugin-sdk/setup`: `createPromptParsedAllowFromForAccount(...)`, `createTopLevelChannelParsedAllowFromPrompt(...)`, and `createNestedChannelParsedAllowFromPrompt(...)`.
  </Accordion>
  <Accordion title="Standard channel setup status">
    For channel setup status blocks that only vary by labels, scores, and optional extra lines, prefer `createStandardChannelSetupStatus(...)` from `oriro/plugin-sdk/setup` instead of hand-rolling the same `status` object in each plugin.
  </Accordion>
  <Accordion title="Optional channel setup surface">
    For optional setup surfaces that should only appear in certain contexts, use `createOptionalChannelSetupSurface` from `oriro/plugin-sdk/channel-setup`:

    ```typescript
    import { createOptionalChannelSetupSurface } from "oriro/plugin-sdk/channel-setup";

    const setupSurface = createOptionalChannelSetupSurface({
      channel: "my-channel",
      label: "My Channel",
      npmSpec: "@myorg/oriro-my-channel",
      docsPath: "/channels/my-channel",
    });
...
**External plugins:** publish to [OriroHub](/orirohub), then install:

<Tabs>
  <Tab title="npm">
    ```bash
    oriro plugins install @myorg/oriro-my-plugin
    ```

    Bare package specs install from npm during the launch cutover.

  </Tab>
  <Tab title="OriroHub only">
    ```bash
    oriro plugins install orirohub:@myorg/oriro-my-plugin
    ```
  </Tab>
  <Tab title="npm package spec">
    Use npm when a package has not moved to OriroHub yet, or when you need a
    direct npm install path during migration:

    ```bash
    oriro plugins install npm:@myorg/oriro-my-plugin
    ```
...
```bash
oriro plugins install <package-name>
```

<Info>
For npm-sourced installs, `oriro plugins install` installs the package into a per-plugin project under `~/.oriro/npm/projects` with lifecycle scripts disabled. Keep plugin dependency trees pure JS/TS and avoid packages that require `postinstall` builds.
</Info>

<Note>
Gateway startup does not install plugin dependencies. npm/git/OriroHub install flows own dependency convergence; local plugins must already have their dependencies installed.
</Note>

Bundled package metadata is explicit, not inferred from built JavaScript at gateway startup. Runtime dependencies belong in the plugin package that owns them; packaged Oriro startup never repairs or mirrors plugin dependencies.
