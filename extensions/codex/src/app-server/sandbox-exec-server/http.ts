 ...
 * access through the active Oriro sandbox backend.
 */
...
import { SsrFBlockedError, isBlockedHostnameOrIp } from "oriro/plugin-sdk/ssrf-runtime";
...
import type { HttpHeader, OriroExecServer } from "./types.js";
...
tmp=$(mktemp "$TMPDIR/oriro-http.XXXXXX.py" 2>/dev/null || mktemp "/tmp/oriro-http.XXXXXX.py") || exit 1
trap 'rm -f "$tmp"' EXIT
cat > "$tmp" <<'PY'
import base64
import json
