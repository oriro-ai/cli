...
Kimi K2.7 Code always uses native thinking. Oriro exposes only the `on`
thinking state for this model and omits outbound `thinking` and
`reasoning_effort` controls, as required by Moonshot. Oriro also omits
...
<Tabs>
    ...
    **Best for:** Kimi K2 models via the Moonshot Open Platform.
    ...
    <Steps>
      ...
      <Step title="Run onboarding">
        ```bash
        oriro onboard --auth-choice moonshot-api-key
        ```

        Or for the China endpoint:

        ```bash
        oriro onboard --auth-choice moonshot-api-key-cn
        ```
      </Step>
      ...
      <Step title="Verify models are available">
        ```bash
        oriro models list --provider moonshot
        ```
      </Step>
      <Step title="Run a live smoke test">
        ...
        ```bash
        ORIRO_CONFIG_PATH=/tmp/oriro-kimi/oriro.json \
        ORIRO_STATE_DIR=/tmp/oriro-kimi \
        oriro agent --local \
  ...
  <Tab title="Kimi Coding">
    ...
    ```bash
    oriro plugins install @oriro/kimi-provider
    ...
    **Best for:** code-focused tasks via the Kimi Coding endpoint.
    ...
    <Steps>
      <Step title="Run onboarding">
        ```bash
        oriro onboard --auth-choice kimi-code-api-key
        ```
      </Step>
      <Step title="Set a default model">
        ```json5
      ...
      <Step title="Verify the model is available">
        ```bash
        oriro models list --provider kimi
        ```
      </Step>
    </Steps>
...
Oriro also ships **Kimi** as a `web_search` provider, backed by Moonshot web
search.
...
<AccordionGroup>
  <Accordion title="Native thinking mode">
    ...
    omit the `thinking` field for this model, so Oriro exposes only `on` and
    ignores stale `off` settings. K2.7 also fixes `temperature`, `top_p`, `n`,
    `presence_penalty`, and `frequency_penalty`; Oriro omits configured
    ...
    Oriro maps runtime `/think` levels for those models:
    ...
    <Warning>
    When Moonshot thinking is enabled, `tool_choice` must be `auto` or `none`. Oriro normalizes incompatible values to `auto`. This includes Kimi K2.7 Code, whose thinking mode cannot be disabled to preserve a pinned tool choice.
    </Warning>
    ...
    default strategy. Oriro only forwards `thinking.keep` for
    `moonshot/kimi-k2.6` and strips it from other models. Kimi K2.7 Code
    preserves full reasoning history by default while Oriro omits the entire
    `thinking` field.
  ...
  <Accordion title="Tool call id sanitization">
    Moonshot Kimi serves native tool_call ids shaped like `functions.<name>:<index>`. For the OpenAI-completions transport, Oriro preserves the first occurrence of each native Kimi id and rewrites later duplicates to deterministic OpenAI-style `call_*` ids. Matching tool results are remapped with the same id so replay remains unique without stripping Kimi's first native id.
  ...
  <Accordion title="Streaming usage compatibility">
    ...
    shared `openai-completions` transport. Oriro keys that off endpoint
