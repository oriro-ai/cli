...
import type { OriroConfig, SecurityConfig } from "../config/types.oriro.js";
...
export type InstallPolicySource = {
  kind:
    ...
    | "upload"
    | "workspace";
  authority: "oriro" | "official" | "third-party" | "unknown" | "user";
  mutable: boolean;
  network: boolean;
};
...
export async function runInstallPolicy(params: {
  ...
  const input = JSON.stringify({
    protocolVersion: 1,
    oriroVersion: resolveRuntimeServiceVersion(params.env ?? process.env),
    ...params.request,
  });
