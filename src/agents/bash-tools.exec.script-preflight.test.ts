...
import { __setFsSafeTestHooksForTest } from "@oriro/fs-safe/test-hooks";
...
const parseOriroChannelsLoginShellCommand = testing.parseOriroChannelsLoginShellCommand;
...
async function expectSymlinkSwapDuringPreflightToAvoidErrors(params: {
  ...
  await withTempDir("oriro-exec-preflight-open-race-", async (parent) => {
...
describe("exec interactive Oriro channel login guard", () => {
  it("recognizes direct and package-runner channel login commands before execution", () => {
    expect(
      parseOriroChannelsLoginShellCommand("oriro channels login --channel whatsapp"),
    ).toBe(true);
    expect(
      parseOriroChannelsLoginShellCommand(
        "pnpm exec oriro channels login --channel whatsapp --verbose",
      ),
    ).toBe(true);
    expect(parseOriroChannelsLoginShellCommand("oriro channels status --deep")).toBe(false);
  });
  ...
  it("blocks interactive channel login commands from exec", async () => {
    ...
    await expect(
      tool.execute("call-oriro-channel-login", {
        command: "oriro channels login --channel whatsapp --verbose",
      }),
    ).rejects.toThrow(/exec cannot run interactive Oriro channel login commands/);
    await expect(
      tool.execute("call-wrapped-oriro-channel-login", {
        command: "sudo -u oriro bash -lc 'oriro channels login --channel whatsapp'",
      }),
    ).rejects.toThrow(/exec cannot run interactive Oriro channel login commands/);
    await expect(
      tool.execute("call-clustered-sudo-channel-login", {
        command: "sudo -EH bash -lc 'oriro channels login --channel whatsapp'",
      }),
    ).rejects.toThrow(/exec cannot run interactive Oriro channel login commands/);
    await expect(
      tool.execute("call-deep-env-channel-login", {
        command: "env env env env env env oriro channels login --channel whatsapp",
      }),
    ).rejects.toThrow(/exec cannot run interactive Oriro channel login commands/);
    await expect(
      tool.execute("call-env-s-trailing-channel-login", {
        command: "env -S 'oriro channels' login --channel whatsapp",
      }),
    ).rejects.toThrow(/exec cannot run interactive Oriro channel login commands/);
  });
});
...
describeNonWin("exec script preflight", () => {
  it("blocks shell env var injection tokens in python scripts before execution", async () => {
    await withTempDir("oriro-exec-preflight-", async (tmp) => {
  ...
  it("skips preflight file reads for script paths outside the workdir", async () => {
    await withTempDir("oriro-exec-preflight-parent-", async (parent) => {
      const outsidePath = path.join(parent, "outside.js");
      const workdir = path.join(parent, "workdir");
      await fs.mkdir(workdir, { recursive: true });
  ...
  it("opens preflight script reads with O_NONBLOCK to avoid FIFO stalls", async () => {
    await withTempDir("oriro-exec-preflight-nonblock-", async (tmp) => {
...
describeWin("exec script preflight on windows path syntax", () => {
  it("preserves windows-style python relative path separators during script extraction", async () => {
    await withTempDir("oriro-exec-preflight-win-", async (tmp) => {
      await fs.writeFile(path.join(tmp, "bad.py"), "payload = $DM_JSON", "utf-8");
...
describe("exec interpreter heuristics ReDoS guard", () => {
  it("does not hang on long commands with VAR=value assignments and whitespace-heavy text", async () => {
    ...
    const command = `ACCESS_TOKEN=$(__oriro_missing_redos_guard__)\ncat > /tmp/out.html << 'EOF'\n${htmlBlock}\nEOF`;
