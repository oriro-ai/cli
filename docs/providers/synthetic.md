---
summary: "Use Synthetic's Anthropic-compatible API in Oriro"
read_when:
...
Oriro registers it as the `synthetic` provider and uses the Anthropic
Messages API.
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    oriro onboard --auth-choice synthetic-api-key
    ```
  </Step>
...
<Warning>
Oriro's Anthropic client appends `/v1` to the base URL automatically, so use
...
`oriro models list --provider synthetic` to see all models available on your
account.
</Tip>
...
<AccordionGroup>
  ...
  <Accordion title="Base URL override">
    ...
    Remember that Oriro appends `/v1` automatically.
