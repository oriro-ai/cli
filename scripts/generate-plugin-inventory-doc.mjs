...
const MANUAL_SECTION_START = "<!-- oriro-plugin-reference:manual-start -->";
const MANUAL_SECTION_END = "<!-- oriro-plugin-reference:manual-end -->";
...
function resolveDocs({ dirName, manifest, packageJson }) {
  ...
  const channelDoc = normalizeDocPath(packageJson.oriro?.channel?.docsPath);
  if (channelDoc) {
...
function resolveInstallRoute(packageJson, status) {
  ...
  const release = packageJson.oriro?.release;
  const orirohubSpec =
    typeof install?.orirohubSpec === "string" ? `: \`${install.orirohubSpec}\`` : "";
  const npmSpec =
    typeof install?.npmSpec === "string" && install.npmSpec !== packageJson.name
      ? `: \`${install.npmSpec}\``
      : "";
  if (release?.publishToOriroHub === true && release?.publishToNpm === true) {
    if (install?.defaultChoice === "orirohub") {
      return orirohubSpec ? `OriroHub${orirohubSpec}; npm${npmSpec}` : `OriroHub + npm${npmSpec}`;
    }
    return orirohubSpec ? `npm${npmSpec}; OriroHub${orirohubSpec}` : `npm${npmSpec}; OriroHub`;
  }
  if (release?.publishToOriroHub === true) {
    return `OriroHub${orirohubSpec || npmSpec}`;
  }
...
function resolveStatus({ dirName, packageJson, excludedDirs }) {
  ...
  const hasInstallSpec =
    typeof packageJson.oriro?.install?.orirohubSpec === "string" ||
    typeof packageJson.oriro?.install?.npmSpec === "string";
  if (!excludedDirs.has(dirName)) {
    return "core";
  }
  if (release?.publishToOriroHub === true || release?.publishToNpm === true || hasInstallSpec) {
    return "external";
  }
  return "source";
}
...
\`oriro.plugin.json\`. Regenerate it with:
...
function collectPluginSourceEntries() {
  ...
  for (const dirName of fs
    ...
    const manifestPath = path.join(EXTENSIONS_DIR, dirName, "oriro.plugin.json");
