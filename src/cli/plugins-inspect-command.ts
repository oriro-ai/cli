// `oriro plugins inspect`: renders plugin registry shape, capabilities, policy, diagnostics, and install records.
...
/** Options accepted by `oriro plugins inspect`. */
...
function formatInstallLines(install: PluginInstallRecord | undefined): string[] {
  ...
  if (install.orirohubPackage) {
    lines.push(`OriroHub package: ${install.orirohubPackage}`);
  }
  if (install.orirohubChannel) {
    lines.push(`OriroHub channel: ${install.orirohubChannel}`);
  }
  ...
  if (install.oriropackSha256) {
    lines.push(`OriroPack sha256: ${install.oriropackSha256}`);
  }
  if (install.oriropackSpecVersion !== undefined) {
    lines.push(`OriroPack spec: ${install.oriropackSpecVersion}`);
  }
  if (install.oriropackManifestSha256) {
    lines.push(`OriroPack manifest sha256: ${install.oriropackManifestSha256}`);
  }
  if (install.oriropackSize !== undefined) {
    lines.push(`OriroPack size: ${install.oriropackSize} bytes`);
  }
...
export async function runPluginsInspectCommand(
  ...
  if (!inspect) {
    defaultRuntime.error(
      formatMissingPluginMessage({ id, listCommand: "oriro plugins list --json" }),
    );
    return defaultRuntime.exit(1);
  }
  ...
  lines.push(`${theme.muted("Format:")} ${inspect.plugin.format ?? "oriro"}`);
