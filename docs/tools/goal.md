...
read_when:
  - You want Oriro to keep one objective visible across a long session
...
A **goal** is one durable objective attached to the current Oriro session.
...
Oriro waits for the next fresh session token snapshot and uses that as the
...
cost reporting, and context-window behavior still use the normal Oriro
usage and model controls.

## Model tools

Oriro exposes three core goal tools to agent harnesses:
...
The `/goal` command works in command-capable Oriro sessions, including the
...
token snapshot yet. Usage refreshes as Oriro records session usage and
