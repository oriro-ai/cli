...
const suiteRootTracker = createSuiteTempRootTracker({ prefix: "oriro-provider-auth-suite-" });
...
describe("resolveProviderAuths key normalization", () => {
  ...
  async function withSuiteHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
    ...
    const stateDir = path.join(base, ".oriro");
  ...
  function agentDirForHome(home: string): string {
    return path.join(home, ".oriro", "agents", "main", "agent");
  }
