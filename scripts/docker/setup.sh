...
EXTRA_MOUNTS="${ORIRO_EXTRA_MOUNTS:-}"
HOME_VOLUME_NAME="${ORIRO_HOME_VOLUME:-}"
RAW_SANDBOX_SETTING="${ORIRO_SANDBOX:-}"
SANDBOX_ENABLED=""
DOCKER_SOCKET_PATH="${ORIRO_DOCKER_SOCKET:-}"
TIMEZONE="${ORIRO_TZ:-}"
RAW_SKIP_ONBOARDING="${ORIRO_SKIP_ONBOARDING:-}"
SKIP_ONBOARDING=""
DOCKER_PULL_TIMEOUT="${ORIRO_DOCKER_SETUP_PULL_TIMEOUT:-600s}"
OFFLINE_MODE=""
DEFAULT_SANDBOX_IMAGE="oriro-sandbox:bookworm-slim"
DEFAULT_SANDBOX_BROWSER_IMAGE="oriro-sandbox-browser:bookworm-slim"
...
read_config_gateway_token() {
  local config_path="$ORIRO_CONFIG_DIR/oriro.json"
...
read_env_gateway_token() {
  ...
  while IFS= read -r line || [[ -n "$line" ]]; do
    line="${line%$'\r'}"
    if [[ "$line" == ORIRO_GATEWAY_TOKEN=* ]]; then
      token="${line#ORIRO_GATEWAY_TOKEN=}"
    fi
  done <"$env_path"
  if [[ -n "$token" ]]; then
    printf '%s' "$token"
...
sync_gateway_config() {
  ...
  if [[ "${ORIRO_GATEWAY_BIND}" != "loopback" ]]; then
    allowed_origin_json="$(printf '["http://localhost:%s","http://127.0.0.1:%s"]' "$ORIRO_GATEWAY_PORT" "$ORIRO_GATEWAY_PORT")"
  ...
  batch_json="$(printf '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"%s"}' "$ORIRO_GATEWAY_BIND")"
  ...
  echo "Pinned gateway.mode=local and gateway.bind=$ORIRO_GATEWAY_BIND for Docker setup."
  if [[ -n "$allowed_origin_json" ]]; then
...
run_prestart_cli() {
  # During setup, avoid the shared-network oriro-cli service because it
  ...
  # Host ORIRO_* paths are Compose bind-mount sources. Setup-time CLI writes
...
run_runtime_cli() {
  ...
  docker compose "${compose_args[@]}" "${run_args[@]}" oriro-cli "$@"
}
...
run_gateway_up() {
  ...
  docker compose "${compose_args[@]}" "${up_args[@]}" oriro-gateway
}
...
resolve_offline_sandbox_images() {
  ...
  printf '%s' "$agents_json" | run_prestart_gateway \
    -T --entrypoint node oriro-gateway -e '
const fs = require("node:fs");
...
const matchesBrowser = (rawPattern) => {
  ...
  if (pattern === "group:oriro" || pattern === "group:ui") {
    return true;
  }
  if (!pattern) {
    return false;
...
validate_offline_sandbox_prerequisites() {
      ...
      browser)
        if ! browser_contract="$(
          docker --host "unix://$DOCKER_SOCKET_PATH" image inspect \
            -f '{{ index .Config.Labels "org.oriro.sandbox-browser.contract" }}' \
  ...
  if ! run_compose_one_off --rm --entrypoint docker oriro-gateway --version >/dev/null 2>&1; then
...
validate_named_volume() {
  ...
  if [[ ! "$value" =~ ^[A-Za-z0-9][A-Za-z0-9_.-]*$ ]]; then
    fail "ORIRO_HOME_VOLUME must match [A-Za-z0-9][A-Za-z0-9_.-]* when using a named volume."
  fi
}
...
validate_mount_spec() {
  ...
  if contains_disallowed_chars "$mount"; then
    fail "ORIRO_EXTRA_MOUNTS entries cannot contain control characters."
  fi
...
ORIRO_CONFIG_DIR="${ORIRO_CONFIG_DIR:-$HOME/.oriro}"
ORIRO_WORKSPACE_DIR="${ORIRO_WORKSPACE_DIR:-$HOME/.oriro/workspace}"
ORIRO_AUTH_PROFILE_SECRET_DIR="${ORIRO_AUTH_PROFILE_SECRET_DIR:-$HOME/.oriro-auth-profile-secrets}"

validate_mount_path_value "ORIRO_CONFIG_DIR" "$ORIRO_CONFIG_DIR"
validate_mount_path_value "ORIRO_WORKSPACE_DIR" "$ORIRO_WORKSPACE_DIR"
validate_mount_path_value "ORIRO_AUTH_PROFILE_SECRET_DIR" "$ORIRO_AUTH_PROFILE_SECRET_DIR"
if [[ -n "$HOME_VOLUME_NAME" ]]; then
  if [[ "$HOME_VOLUME_NAME" == *"/"* ]]; then
    validate_mount_path_value "ORIRO_HOME_VOLUME" "$HOME_VOLUME_NAME"
  else
    validate_named_volume "$HOME_VOLUME_NAME"
  fi
fi
if contains_disallowed_chars "$EXTRA_MOUNTS"; then
  fail "ORIRO_EXTRA_MOUNTS cannot contain control characters."
fi
if [[ -n "$SANDBOX_ENABLED" ]]; then
  validate_mount_path_value "ORIRO_DOCKER_SOCKET" "$DOCKER_SOCKET_PATH"
fi
if [[ -n "$TIMEZONE" ]]; then
  if contains_disallowed_chars "$TIMEZONE"; then
    fail "ORIRO_TZ contains unsupported control characters."
  fi
  if [[ ! "$TIMEZONE" =~ ^[A-Za-z0-9/_+\-]+$ ]]; then
    fail "ORIRO_TZ must be a valid IANA timezone string (e.g. Asia/Shanghai)."
  fi
  if ! is_valid_timezone "$TIMEZONE"; then
    fail "ORIRO_TZ must match a timezone in /usr/share/zoneinfo (e.g. Asia/Shanghai)."
  fi
fi

mkdir -p "$ORIRO_CONFIG_DIR"
mkdir -p "$ORIRO_WORKSPACE_DIR"
mkdir -p "$ORIRO_AUTH_PROFILE_SECRET_DIR"
# Seed directory tree eagerly so bind mounts work even on Docker Desktop/Windows
# where the container (even as root) cannot create new host subdirectories.
mkdir -p "$ORIRO_CONFIG_DIR/identity"
mkdir -p "$ORIRO_CONFIG_DIR/agents/main/agent"
mkdir -p "$ORIRO_CONFIG_DIR/agents/main/sessions"

export ORIRO_CONFIG_DIR
export ORIRO_WORKSPACE_DIR
export ORIRO_AUTH_PROFILE_SECRET_DIR
export ORIRO_GATEWAY_PORT="${ORIRO_GATEWAY_PORT:-18789}"
export ORIRO_BRIDGE_PORT="${ORIRO_BRIDGE_PORT:-18790}"
export ORIRO_GATEWAY_BIND="${ORIRO_GATEWAY_BIND:-lan}"
export ORIRO_DISABLE_BONJOUR="${ORIRO_DISABLE_BONJOUR:-}"
export ORIRO_IMAGE="$IMAGE_NAME"
export ORIRO_IMAGE_APT_PACKAGES="${ORIRO_IMAGE_APT_PACKAGES-${ORIRO_DOCKER_APT_PACKAGES:-}}"
export ORIRO_IMAGE_PIP_PACKAGES="${ORIRO_IMAGE_PIP_PACKAGES:-}"
export ORIRO_EXTENSIONS="${ORIRO_EXTENSIONS:-}"
export ORIRO_INSTALL_BROWSER="${ORIRO_INSTALL_BROWSER:-}"
export ORIRO_EXTRA_MOUNTS="$EXTRA_MOUNTS"
export ORIRO_HOME_VOLUME="$HOME_VOLUME_NAME"
export ORIRO_ALLOW_INSECURE_PRIVATE_WS="${ORIRO_ALLOW_INSECURE_PRIVATE_WS:-}"
export ORIRO_SANDBOX="$SANDBOX_ENABLED"
export ORIRO_DOCKER_SOCKET="$DOCKER_SOCKET_PATH"
export ORIRO_DOCKER_SETUP=1
export ORIRO_TZ="$TIMEZONE"
...
export ORIRO_OTEL_PRELOADED="${ORIRO_OTEL_PRELOADED:-}"
export ORIRO_SKIP_ONBOARDING="$SKIP_ONBOARDING"
...
if [[ -z "${ORIRO_GATEWAY_TOKEN:-}" ]]; then
  EXISTING_CONFIG_TOKEN="$(read_config_gateway_token || true)"
  if [[ -n "$EXISTING_CONFIG_TOKEN" ]]; then
    ORIRO_GATEWAY_TOKEN="$EXISTING_CONFIG_TOKEN"
    echo "Reusing gateway token from $ORIRO_CONFIG_DIR/oriro.json"
  else
    DOTENV_GATEWAY_TOKEN="$(read_env_gateway_token "$ROOT_DIR/.env" || true)"
    if [[ -n "$DOTENV_GATEWAY_TOKEN" ]]; then
      ORIRO_GATEWAY_TOKEN="$DOTENV_GATEWAY_TOKEN"
      echo "Reusing gateway token from $ROOT_DIR/.env"
    elif command -v openssl >/dev/null 2>&1; then
      ORIRO_GATEWAY_TOKEN="$(openssl rand -hex 32)"
    else
      ORIRO_GATEWAY_TOKEN="$(python3 - <<'PY'
import secrets
...
YAML
  ...
  if [[ -n "$home_volume" ]]; then
    ...
    gateway_config_mount="${ORIRO_CONFIG_DIR}:/home/node/.oriro"
    gateway_workspace_mount="${ORIRO_WORKSPACE_DIR}:/home/node/.oriro/workspace"
    gateway_auth_profile_secret_mount="${ORIRO_AUTH_PROFILE_SECRET_DIR}:/home/node/.config/oriro"
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  if [[ -z "${ORIRO_INSTALL_DOCKER_CLI:-}" ]]; then
    export ORIRO_INSTALL_DOCKER_CLI=1
  fi
fi
...
upsert_env "$ENV_FILE" \
  ORIRO_CONFIG_DIR \
  ORIRO_WORKSPACE_DIR \
  ORIRO_AUTH_PROFILE_SECRET_DIR \
  ORIRO_GATEWAY_PORT \
  ORIRO_BRIDGE_PORT \
  ORIRO_GATEWAY_BIND \
  ORIRO_DISABLE_BONJOUR \
  ORIRO_GATEWAY_TOKEN \
  ORIRO_IMAGE \
  ORIRO_EXTRA_MOUNTS \
  ORIRO_HOME_VOLUME \
  ORIRO_IMAGE_APT_PACKAGES \
  ORIRO_IMAGE_PIP_PACKAGES \
  ORIRO_EXTENSIONS \
  ORIRO_INSTALL_BROWSER \
  ORIRO_SANDBOX \
  ORIRO_DOCKER_SOCKET \
  DOCKER_GID \
  ORIRO_INSTALL_DOCKER_CLI \
  ORIRO_ALLOW_INSECURE_PRIVATE_WS \
  ORIRO_TZ \
  ...
  ORIRO_OTEL_PRELOADED \
  ORIRO_SKIP_ONBOARDING

if [[ -n "$OFFLINE_MODE" ]]; then
  require_local_docker_image "$IMAGE_NAME"
  echo "==> Using preloaded Docker image: $IMAGE_NAME"
elif [[ "$IMAGE_NAME" == "oriro:local" ]]; then
  echo "==> Building Docker image: $IMAGE_NAME"
  run_docker_build \
    --build-arg "ORIRO_IMAGE_APT_PACKAGES=${ORIRO_IMAGE_APT_PACKAGES}" \
    --build-arg "ORIRO_IMAGE_PIP_PACKAGES=${ORIRO_IMAGE_PIP_PACKAGES}" \
    --build-arg "ORIRO_EXTENSIONS=${ORIRO_EXTENSIONS}" \
    --build-arg "ORIRO_INSTALL_BROWSER=${ORIRO_INSTALL_BROWSER}" \
    --build-arg "ORIRO_INSTALL_DOCKER_CLI=${ORIRO_INSTALL_DOCKER_CLI:-}" \
    -t "$IMAGE_NAME" \
...
# After fixing the config dir, only the Oriro metadata subdirectory
# (.oriro/) inside the workspace gets chowned, not the user's project files.
run_prestart_gateway --user root --entrypoint sh oriro-gateway -c \
  'find /home/node/.oriro -xdev -exec chown node:node {} +; \
   chown node:node /home/node/.config; \
   find /home/node/.config/oriro -xdev -exec chown node:node {} +; \
   [ -d /home/node/.oriro/workspace/.oriro ] && chown -R node:node /home/node/.oriro/workspace/.oriro || true'

echo ""
if [[ -n "$SKIP_ONBOARDING" ]]; then
  echo "==> Skipping onboarding (ORIRO_SKIP_ONBOARDING is set)"
else
  echo "==> Onboarding (interactive)"
  echo "Docker setup pins Gateway mode to local."
  echo "Gateway runtime bind comes from ORIRO_GATEWAY_BIND (default: lan)."
  echo "Current runtime bind: $ORIRO_GATEWAY_BIND"
  if is_truthy_value "$ORIRO_DISABLE_BONJOUR"; then
    echo "Bonjour/mDNS advertising: force disabled (ORIRO_DISABLE_BONJOUR=$ORIRO_DISABLE_BONJOUR)."
  elif [[ -z "$ORIRO_DISABLE_BONJOUR" ]]; then
    echo "Bonjour/mDNS advertising: auto (disabled inside the Gateway container unless explicitly enabled)."
  else
    echo "Bonjour/mDNS advertising: explicitly enabled (ORIRO_DISABLE_BONJOUR=$ORIRO_DISABLE_BONJOUR)."
  fi
  ...
  run_prestart_cli onboard \
    ...
    --no-install-daemon \
    --gateway-auth token \
    --gateway-token-ref-env ORIRO_GATEWAY_TOKEN \
    --skip-ui \
...
echo "WhatsApp (QR):"
echo "  ${COMPOSE_HINT} run --rm oriro-cli channels login"
echo "Telegram (bot token):"
echo "  ${COMPOSE_HINT} run --rm oriro-cli channels add --channel telegram --token <token>"
echo "Discord (bot token):"
echo "  ${COMPOSE_HINT} run --rm oriro-cli channels add --channel discord --token <token>"
echo "Docs: https://docs.oriro.ai/channels"
...
# --- Sandbox setup (opt-in via ORIRO_SANDBOX=1) ---
if [[ -n "$SANDBOX_ENABLED" ]]; then
  echo ""
  echo "==> Sandbox setup"
  ...
  if [[ -z "$OFFLINE_MODE" && ! -S "$DOCKER_SOCKET_PATH" ]]; then
    echo "WARNING: ORIRO_SANDBOX enabled but Docker socket not found at $DOCKER_SOCKET_PATH." >&2
  ...
  if [[ -n "$SANDBOX_ENABLED" && -z "$OFFLINE_MODE" ]] &&
    ! run_compose_one_off --rm --entrypoint docker oriro-gateway --version >/dev/null 2>&1; then
    echo "WARNING: Docker CLI not found inside the container image." >&2
    echo "  Sandbox requires Docker CLI. Rebuild with --build-arg ORIRO_INSTALL_DOCKER_CLI=1" >&2
    echo "  or use a local build (ORIRO_IMAGE=oriro:local). Skipping sandbox setup." >&2
    SANDBOX_ENABLED=""
  fi
fi
...
if [[ -n "$SANDBOX_ENABLED" ]]; then
  # Enable sandbox in Oriro config.
  ...
  if [[ "$sandbox_config_ok" == true ]]; then
    ...
    echo "Docs: https://docs.oriro.ai/gateway/sandboxing"
...
echo "Config: $ORIRO_CONFIG_DIR"
echo "Workspace: $ORIRO_WORKSPACE_DIR"
echo "Token: stored in Docker environment/config (not printed)."
echo ""
echo "Commands:"
echo "  ${COMPOSE_HINT} logs -f oriro-gateway"
echo "  ${COMPOSE_HINT} exec oriro-gateway sh -lc 'node dist/index.js health --token \"\$ORIRO_GATEWAY_TOKEN\"'"
