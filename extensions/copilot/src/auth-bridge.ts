 ...
 *     use cases.
 *   - Falls back to ORIRO_GITHUB_TOKEN, COPILOT_GITHUB_TOKEN,
 ...
 *   4. ORIRO_GITHUB_TOKEN, then COPILOT_GITHUB_TOKEN, then
...
function resolveCopilotHome(args: {
  ...
  // "copilot" subdir so it cannot collide with Oriro's own files
...
function readEnvTokenFallback(
  ...
  // ORIRO_GITHUB_TOKEN is the harness-specific override and stays at
  ...
  const candidates: Array<{ name: string; value: string | undefined }> = [
    { name: "ORIRO_GITHUB_TOKEN", value: readString(env.ORIRO_GITHUB_TOKEN) },
