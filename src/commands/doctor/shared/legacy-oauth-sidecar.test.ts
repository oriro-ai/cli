...
async function writeLegacySidecarThatNeedsKeychain(): Promise<{
  ...
  const state = await createOriroTestState({
    layout: "state-only",
    prefix: "oriro-legacy-oauth-keychain-warn-",
    env: {
      ORIRO_AGENT_DIR: undefined,
      ORIRO_AUTH_PROFILE_SECRET_KEY: undefined,
    },
  });
  states.push(state);
...
describe("loadLegacyOAuthSidecarMaterial keychain-only headless warning", () => {
  ...
  function envWithoutVitestSignals(state: OriroTestState): NodeJS.ProcessEnv {
  ...
  it("emits a single doctor-pointer warning when only Keychain can decrypt and prompts are disabled", async () => {
    ...
    expect(String(firstMessage)).toContain("oriro doctor --fix");
