// Generates setup codes used to pair external channels with Oriro.
...
function resolvePairingSetupAuthLabel(
  ...
  const envToken = normalizeOptionalString(env.ORIRO_GATEWAY_TOKEN);
  const envPassword = normalizeOptionalString(env.ORIRO_GATEWAY_PASSWORD);
  const token =
...
export async function resolvePairingSetupFromConfig(
  ...
  const cfgForAuth = await materializeGatewayAuthSecretRefs({
    ...
    hasTokenCandidate: Boolean(normalizeOptionalString(env.ORIRO_GATEWAY_TOKEN)),
    hasPasswordCandidate: Boolean(normalizeOptionalString(env.ORIRO_GATEWAY_PASSWORD)),
  });
