// Check Oriro Package Tarball tests cover check oriro package tarball script behavior.
...
const CHECK_SCRIPT = "scripts/check-oriro-package-tarball.mjs";
...
function withTarball(
  ...
  const root = mkdtempSync(join(tmpdir(), "oriro-package-tarball-test-"));
  try {
    const packageRoot = join(root, "package");
    mkdirSync(join(packageRoot, "dist"), { recursive: true });
    writeFileSync(join(packageRoot, "package.json"), JSON.stringify({ name: "oriro", version }));
    ...
    const tarFiles =
      options.includeControlUi === false
        ? files
        : {
            "dist/control-ui/index.html": "<!doctype html><oriro-app></oriro-app>",
    ...
    const tarball = join(root, "oriro.tgz");
...
describe("check-oriro-package-tarball", () => {
  ...
  it("rejects option-like and extra arguments before tar inspection", () => {
    ...
    expect(unknown.stderr).toContain("Unknown Oriro package tarball check option: --tag");
    expect(unknown.stderr).not.toContain("Oriro package tarball does not exist");

    const extra = spawnSync("node", [CHECK_SCRIPT, "oriro.tgz", "extra"], {
      encoding: "utf8",
    });

    expect(extra.status).not.toBe(0);
    expect(extra.stderr).toContain("Unexpected Oriro package tarball check argument: extra");
    expect(extra.stderr).not.toContain("Oriro package tarball does not exist");
  });

  it.runIf(process.platform !== "win32")(
    "removes the extract dir when tar extraction fails",
    () => {
      const root = mkdtempSync(join(tmpdir(), "oriro-package-tarball-extract-fail-"));
      try {
        ...
        writeFileSync(
          ...
          [
            ...
            "fs.writeFileSync(process.env.ORIRO_TEST_EXTRACT_DIR_FILE, outputDir);",
        ...
        const result = spawnSync("node", [CHECK_SCRIPT, tarball], {
          ...
          env: {
            ...process.env,
            ORIRO_TEST_EXTRACT_DIR_FILE: extractDirFile,
  ...
  it("allows legacy private QA inventory entries omitted from shipped tarballs through 2026.4.25", () => {
    withTarball(
      ...
      (tarball) => {
        ...
        expect(result.stdout).toContain("Oriro package tarball integrity passed.");
      },
      "2026.4.25-beta.10",
    );
  });
