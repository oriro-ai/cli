#!/usr/bin/env bash
# Reset Oriro like Trimmy: kill running instances, rebuild, repackage, relaunch, verify.

set -euo pipefail

ROOT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
source "${ROOT_DIR}/scripts/lib/restart-mac-gateway.sh"
APP_BUNDLE="${ORIRO_APP_BUNDLE:-}"
APP_EXECUTABLE_RELATIVE_PATH="Contents/MacOS/Oriro"
DEBUG_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/debug/Oriro"
LOCAL_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build-local/debug/Oriro"
RELEASE_PROCESS_PATTERN="${ROOT_DIR}/apps/macos/.build/release/Oriro"
LAUNCH_AGENT="${HOME}/Library/LaunchAgents/ai.oriro.mac.plist"
LOCK_KEY="$(printf '%s' "${ROOT_DIR}" | shasum -a 256 | cut -c1-8)"
LOCK_DIR="${TMPDIR:-/tmp}/oriro-restart-${LOCK_KEY}"
LOCK_PID_FILE="${LOCK_DIR}/pid"
WAIT_FOR_LOCK=0
LOG_PATH="${ORIRO_RESTART_LOG:-${TMPDIR:-/tmp}/oriro-restart-${LOCK_KEY}.log}"
NO_SIGN=0
SIGN=0
AUTO_DETECT_SIGNING=1
GATEWAY_WAIT_SECONDS="${ORIRO_GATEWAY_WAIT_SECONDS:-0}"
LAUNCHAGENT_DISABLE_MARKER="${HOME}/.oriro/disable-launchagent"
ATTACH_ONLY=1
...
canonicalize_app_bundle() {
  ...
  if [[ ! -d "${APP_BUNDLE}" ]]; then
    fail "ORIRO_APP_BUNDLE does not exist: ${APP_BUNDLE}"
  fi
...
for arg in "$@"; do
  case "${arg}" in
    ...
    --help|-h)
      ...
      log ""
      log "Env:"
      log "  ORIRO_GATEWAY_WAIT_SECONDS=0  Wait time before gateway port check (unsigned only)"
      log ""
      log "Unsigned recovery:"
      log "  node oriro.mjs daemon install --force --runtime node"
      log "  node oriro.mjs daemon restart"
      log ""
      log "Reset unsigned overrides:"
      log "  rm ~/.oriro/disable-launchagent"
      log ""
...
kill_all_oriro() {
  ...
  [[ -z "$(oriro_process_pids)" ]]
}

known_oriro_executables() {
  if [[ -n "${APP_BUNDLE}" ]]; then
    printf '%s\n' "${APP_BUNDLE}/${APP_EXECUTABLE_RELATIVE_PATH}"
  fi
  printf '%s\n' \
    "${ROOT_DIR}/dist/Oriro.app/${APP_EXECUTABLE_RELATIVE_PATH}" \
    "/Applications/Oriro.app/${APP_EXECUTABLE_RELATIVE_PATH}" \
...
oriro_process_pids() {
  ...
  done < <(known_oriro_executables) | sort -u
}
...
stop_launch_agent() {
  launchctl bootout gui/"$UID"/ai.oriro.mac 2>/dev/null || true
}
...
choose_app_bundle() {
  ...
  if [[ -d "${ROOT_DIR}/dist/Oriro.app" ]]; then
    APP_BUNDLE="$(cd "${ROOT_DIR}/dist/Oriro.app" && pwd -P)"
    if [[ ! -d "${APP_BUNDLE}/Contents/Frameworks/Sparkle.framework" ]]; then
      fail "dist/Oriro.app missing Sparkle after packaging"
    fi
    return 0
  fi

  if [[ -d "/Applications/Oriro.app" ]]; then
    APP_BUNDLE="$(cd "/Applications/Oriro.app" && pwd -P)"
    return 0
  fi
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "install gateway launch agent (unsigned)" bash -lc "cd '${ROOT_DIR}' && node oriro.mjs daemon install --force --runtime node"
  run_step "restart gateway daemon (unsigned)" bash -lc "cd '${ROOT_DIR}' && node oriro.mjs daemon restart"
  ...
  GATEWAY_PORT="$(
    node -e '
      ...
      try {
        const raw = fs.readFileSync(path.join(process.env.HOME, ".oriro", "oriro.json"), "utf8");
...
if [ "$NO_SIGN" -eq 1 ] && [ "$ATTACH_ONLY" -ne 1 ]; then
  run_step "show gateway launch agent args (unsigned)" bash -lc "/usr/bin/plutil -p '${HOME}/Library/LaunchAgents/ai.oriro.gateway.plist' | head -n 40 || true"
fi
