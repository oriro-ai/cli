...
struct GatewayProcessManagerTests {
    ...
    @Test func `attaches to existing gateway without spawning launchd`() async throws {
        let port = 19097
        try await TestIsolation.withEnvValues(["ORIRO_GATEWAY_PORT": "\(port)"]) {
            let healthData = Data(
                """
                {
                  "ok": true,
            ...
            let descriptor = PortGuardian.Descriptor(
                pid: 4242,
                command: "oriro-gateway",
                executablePath: "/tmp/oriro-gateway")
            ...
            do {
                ...
                #expect(details.contains("pid 4242 oriro-gateway @ /tmp/oriro-gateway"))
                await cleanup()
            } catch {
                await cleanup()
                throw error
