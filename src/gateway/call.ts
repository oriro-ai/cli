...
import { isLoopbackIpAddress } from "@oriro/net-policy/ip";
import { redactSensitiveUrlLikeString } from "@oriro/net-policy/redact-sensitive-url";
...
type GatewayRuntimeConfigLoader = () => OriroConfig | Promise<OriroConfig>;
const defaultGetRuntimeConfig = async (): Promise<OriroConfig> =>
...
async function loadGatewayConfig(): Promise<OriroConfig> {
  const loadConfigFn =
...
function resolveGatewayPortValue(config?: OriroConfig, env?: NodeJS.ProcessEnv): number {
...
function resolveGatewayCallAuth(config: OriroConfig) {
...
function resolveGatewayCallTimeout(
  ...
  const hasEnvHandshakeTimeout =
    Boolean(process.env.ORIRO_HANDSHAKE_TIMEOUT_MS) ||
    Boolean(process.env.VITEST && process.env.ORIRO_TEST_HANDSHAKE_TIMEOUT_MS);
...
async function resolveGatewayCallContext(
  ...
  const envUrlOverride = cliUrlOverride
    ? undefined
    : trimToUndefined(process.env.ORIRO_GATEWAY_URL);
  ...
  const config =
    opts.config ?? (canSkipConfigLoad ? ({} as OriroConfig) : await loadGatewayConfig());
...
function formatGatewayCloseError(
  ...
  if (code === 1006) {
    message +=
      ...
      "\nRun `oriro doctor` for diagnostics.";
  }
  return message;
}
