...
const GATEWAY_TEST_CONFIG_ROOT_KEY = Symbol.for("oriro.gatewayTestHelpers.configRoot");
...
const gatewayTestHoisted = vi.hoisted(() => {
  const key = Symbol.for("oriro.gatewayTestHelpers.hoisted");
...
export const testConfigRoot = resolveGlobalSingleton(GATEWAY_TEST_CONFIG_ROOT_KEY, () => ({
  value: path.join(os.tmpdir(), `oriro-gateway-test-${process.pid}-${crypto.randomUUID()}`),
}));

/** Updates the config root used by gateway config-module mocks. */
export function setTestConfigRoot(root: string): void {
  testConfigRoot.value = root;
  process.env.ORIRO_CONFIG_PATH = path.join(root, "oriro.json");
}
