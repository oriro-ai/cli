...
import type { StreamFn } from "oriro/plugin-sdk/agent-core";
...
afterEach(() => {
  testing.resetOriroNativeCodexResponsesStreamFnForTest();
});
...
describe("describeEmbeddedAgentStreamStrategy", () => {
  ...
  it("describes default Codex fallback as Oriro native", () => {
    expect(
      describeEmbeddedAgentStreamStrategy({
        currentStreamFn: undefined,
        model: {
    ...
    ).toBe("oriro-native-codex-responses");
  });
...
describe("resolveEmbeddedAgentStreamFn", () => {
  ...
  it("routes Codex responses fallbacks through Oriro native transport", async () => {
    // Codex OAuth models use the Oriro native transport, with prompt-cache
    ...
    testing.setOriroNativeCodexResponsesStreamFnForTest(nativeStreamFn as never);
    const streamFn = resolveEmbeddedAgentStreamFn({
      currentStreamFn: undefined,
      sessionId: "session-1",
      model: {
  ...
  it("routes Oriro native OpenAI-compatible provider streams through boundary-aware transports", async () => {
  ...
  it("injects the resolved run api key into the Oriro native Codex Responses fallback", async () => {
  ...
  it("falls back to authStorage when no resolved api key is available for Oriro native fallback", async () => {
    const nativeStreamFn = vi.fn(async (_model, _context, options) => options);
    const authStorage = {
  ...
  it("forwards the run abort signal into the Oriro native fallback when callers omit one", async () => {
    const nativeStreamFn = vi.fn(async (_model, _context, options) => options);
    const runSignal = new AbortController().signal;
    testing.setOriroNativeCodexResponsesStreamFnForTest(nativeStreamFn as never);
    const streamFn = resolveEmbeddedAgentStreamFn({
  ...
  it("does not overwrite an explicit signal on the Oriro native fallback path", async () => {
    const nativeStreamFn = vi.fn(async (_model, _context, options) => options);
    const runSignal = new AbortController().signal;
    const explicitSignal = new AbortController().signal;
    testing.setOriroNativeCodexResponsesStreamFnForTest(nativeStreamFn as never);
  ...
  it("forwards the run signal on the sync Oriro native fallback path without auth credentials", async () => {
    const nativeStreamFn = vi.fn(async (_model, _context, options) => options);
    const runSignal = new AbortController().signal;
    testing.setOriroNativeCodexResponsesStreamFnForTest(nativeStreamFn as never);
    const streamFn = resolveEmbeddedAgentStreamFn({
  ...
  it("strips cache boundary markers on the Oriro native fallback path", async () => {
