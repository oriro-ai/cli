...
describe("local media roots", () => {
  function withStateDir<T>(stateDir: string, run: () => T): T {
    return withEnv({ ORIRO_STATE_DIR: stateDir }, run);
  }
  ...
  it.each([
    {
      ...
      stateDir: path.join("/tmp", "oriro-media-roots-state"),
    ...
    },
    {
      ...
      stateDir: path.join("/tmp", "oriro-agent-media-roots-state"),
  ...
  it.each([
    {
      ...
      stateDir: path.join("/tmp", "oriro-flexible-media-roots-state"),
      cfg: {},
    ...
    },
    {
      ...
      stateDir: path.join("/tmp", "oriro-messaging-media-roots-state"),
    ...
    },
    {
      ...
      stateDir: path.join("/tmp", "oriro-messaging-fs-media-roots-state"),
      cfg: {
        tools: {
          profile: "messaging",
    ...
    },
    {
      ...
      stateDir: path.join("/tmp", "oriro-messaging-read-media-roots-state"),
      cfg: {
        tools: {
          profile: "messaging",
  ...
  it("keeps the config-dir media cache root when state and config paths differ", () => {
    const stateDir = path.join("/tmp", "oriro-legacy-state");
    const configDir = path.join("/tmp", "oriro-current-config");
