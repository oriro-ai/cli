...
const stateDir = "/opt/oriro/state";
...
describe("isVolatileBackupPath", () => {
  ...
  it("does not match paths that escape the anchor via `..`", () => {
    // `/opt/oriro/state/sessions/../config.jsonl` resolves to
    // `/opt/oriro/state/config.jsonl`, which is NOT inside sessions/.
  ...
  it("normalizes Windows-style separators before anchor checks", () => {
    const winStateDir = "C:\\oriro\\state";
  ...
  it("matches tar filter paths when node-tar omits the leading slash", () => {
    expect(
      isVolatileBackupPath("opt/oriro/state/agents/main/sessions/transcript.jsonl", plan),
    ).toBe(true);
  });
