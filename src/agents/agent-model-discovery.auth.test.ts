...
async function createAgentDir(): Promise<string> {
  return await fs.mkdtemp(path.join(os.tmpdir(), "oriro-agent-auth-storage-"));
}
...
describe("discoverAuthStorage", () => {
  ...
  it("includes env-backed provider auth when no auth profile exists", () => {
    ...
    const previousDisableBundledPlugins = process.env.ORIRO_DISABLE_BUNDLED_PLUGINS;
    ...
    try {
      ...
      if (previousDisableBundledPlugins === undefined) {
        ...
        process.env.ORIRO_DISABLE_BUNDLED_PLUGINS = previousDisableBundledPlugins;
      }
    }
  });
