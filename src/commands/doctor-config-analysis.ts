...
import { resolvePrimaryStringValue } from "@oriro/normalization-core/string-coerce";
...
function isUpdateInProgress(): boolean {
  const value = process.env.ORIRO_UPDATE_IN_PROGRESS;
 ...
 */
export function stripUnknownConfigKeys(config: OriroConfig): {
  config: OriroConfig;
  removed: string[];
} {
  ...
  const parsed = OriroSchema.safeParse(config);
  if (parsed.success) {
    return { config, removed: [] };
  }
...
export function noteOpencodeProviderOverrides(cfg: OriroConfig): void {
...
export function collectImplicitFallbackClobberWarnings(cfg: OriroConfig): string[] {
...
export function noteImplicitFallbackClobberWarnings(cfg: OriroConfig): void {
