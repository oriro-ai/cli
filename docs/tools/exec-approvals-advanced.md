...
- allowlist entries live in the host-local approvals file under `agents.<id>.allowlist` (or via Control UI / `oriro approvals allowlist ...`).
- `oriro security audit` warns with `tools.exec.safe_bins_interpreter_unprofiled` when interpreter/runtime bins appear in `safeBins` without explicit profiles.
- `oriro doctor --fix` can scaffold missing custom `safeBinProfiles.<bin>` entries as `{}` (review and tighten afterward). Interpreter/runtime bins are not auto-scaffolded.
...
If you explicitly opt `jq` into `safeBins`, Oriro still rejects the `env` builtin in safe-bin
...
session, Oriro also resumes that session with an internal followup so the agent observes that
the command did not run instead of later repairing a missing result.

### Followup delivery behavior

After an approved async exec finishes, Oriro sends a followup `agent` turn to the same session.
...
the originating channel, Oriro keeps the local deterministic `/approve`
prompt visible. If the native runtime is active and attempts delivery but no
target receives the card, Oriro sends a same-chat fallback notice with the
...
- Matrix native prompts include `com.oriro.approval` custom event content on the first prompt
  event so Oriro-aware Matrix clients can read structured approval state while stock clients
...
owner routing for approval prompts and final results. Oriro first tries a private route on the
...
topics, Oriro preserves the topic for the approval prompt and the post-approval follow-up.
