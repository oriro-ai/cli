...
async function withTempSessionLockFile(
  ...
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-lock-"));
  try {
...
describe("acquireSessionWriteLock", () => {
  ...
  it("reports live Oriro-owned stale locks without removing them", async () => {
    await withTempSessionLockFile(async ({ sessionFile, lockPath }) => {
      const owner = spawn(process.execPath, ["-e", "setInterval(() => {}, 1000)", "oriro"], {
        stdio: "ignore",
      });
      if (!owner.pid) {
  ...
  it("lets session write-lock env override config for emergency tuning", () => {
    expect(
      resolveSessionWriteLockOptions(
        ...
        {
          env: {
            ORIRO_SESSION_WRITE_LOCK_ACQUIRE_TIMEOUT_MS: "120000",
            ORIRO_SESSION_WRITE_LOCK_STALE_MS: "60000",
            ORIRO_SESSION_WRITE_LOCK_MAX_HOLD_MS: "50000",
          },
        },
      ),
    ).toEqual({
  ...
  it("ignores non-decimal and unsafe session write-lock env values", () => {
    expect(
      resolveSessionWriteLockOptions(
        ...
        {
          env: {
            ORIRO_SESSION_WRITE_LOCK_ACQUIRE_TIMEOUT_MS: "1e3",
            ORIRO_SESSION_WRITE_LOCK_STALE_MS: "0x1000",
            ORIRO_SESSION_WRITE_LOCK_MAX_HOLD_MS: "9007199254740993",
          },
        },
      ),
    ).toEqual({
  ...
  it("allows explicit infinite acquire timeout but not infinite stale policy", () => {
    expect(
      resolveSessionWriteLockOptions(undefined, {
        env: {
          ORIRO_SESSION_WRITE_LOCK_ACQUIRE_TIMEOUT_MS: "Infinity",
          ORIRO_SESSION_WRITE_LOCK_STALE_MS: "Infinity",
        },
      }),
    ).toMatchObject({
  ...
  it("uses resolved stale policy when cleaning stale lock files", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-lock-policy-"));
  ...
  it("does not clean live Oriro locks just because holder max hold expired", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-lock-policy-"));
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });
    const nowMs = Date.now();
  ...
  it("cleans old live .jsonl lock files owned by non-Oriro processes", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-lock-"));
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });
    const nowMs = Date.now();
    const lockPath = path.join(sessionsDir, "old-non-oriro.jsonl.lock");
    ...
    try {
      ...
      expect(lockCleanupRecords(result.cleaned)).toEqual([
        {
          name: "old-non-oriro.jsonl.lock",
          removed: true,
          stale: true,
          staleReasons: ["too-old", "non-oriro-owner"],
        },
      ]);
      await expectPathMissing(lockPath);
    } finally {
  ...
  it("cleans fresh live .jsonl lock files owned by a non-Oriro process", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-lock-"));
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });
  ...
  it("cleans fresh live .jsonl lock files owned by generic non-Oriro entrypoints", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-lock-"));
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });
  ...
  it("keeps fresh live .jsonl lock files with Oriro or unknown owners", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-lock-"));
    const sessionsDir = path.join(root, "sessions");
    await fs.mkdir(sessionsDir, { recursive: true });

    const nowMs = Date.now();
    const oriroLock = path.join(sessionsDir, "oriro-live.jsonl.lock");
    ...
    try {
      ...
      const oriroResult = await cleanStaleLockFiles({
        sessionsDir,
        staleMs: 30_000,
        nowMs,
        removeStale: true,
      ...
      await expect(fs.access(oriroLock)).resolves.toBeUndefined();

      await fs.rm(oriroLock, { force: true });
      await fs.writeFile(
        gatewayLock,
        JSON.stringify({
          pid: process.pid,
  ...
  it("removes held locks on termination signals", async () => {
    ...
    try {
      for (const signal of signals) {
        const root = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-lock-cleanup-"));
        try {
          const sessionFile = path.join(root, "sessions.json");
          const lockPath = `${sessionFile}.lock`;
