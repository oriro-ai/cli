...
import type { AnyAgentTool, SandboxContext } from "oriro/plugin-sdk/agent-harness-runtime";
import { afterEach, describe, expect, it, vi } from "vitest";
import {
  createCopilotToolBridge,
  convertOriroToolToSdkTool,
  supportsModelTools,
} from "./tool-bridge.js";
...
describe("supportsModelTools", () => {
  it("returns true for github-copilot and false otherwise", () => {
    ...
    expect(supportsModelTools("oriro")).toBe(false);
...
describe("createCopilotToolBridge", () => {
  it("returns empty arrays for unsupported providers without calling the seam", async () => {
    const createOriroCodingTools = vi.fn(async () => [makeTool()]);
    ...
    expect(createOriroCodingTools).toHaveBeenCalledTimes(0);
  });

  it("forwards supported fields to injected createOriroCodingTools", async () => {
    ...
    expect(createOriroCodingTools).toHaveBeenCalledTimes(1);
    ...
    expect(createOriroCodingTools).toHaveBeenCalledWith(
  ...
  it("returns sdkTools and sourceTools with matching lengths", async () => {
    ...
    const result = await createCopilotToolBridge({
      agentId: "agent-1",
      createOriroCodingTools: async () => sourceTools,
      modelId: "gpt-4o",
      modelProvider: "github-copilot",
      sessionId: "session-1",
    });
  ...
  it("throws when createOriroCodingTools returns a non-array", async () => {
    await expect(
      createCopilotToolBridge({
        agentId: "agent-1",
        createOriroCodingTools: async () => ({ tools: [] }) as never,
        modelId: "gpt-4o",
        modelProvider: "github-copilot",
        sessionId: "session-1",
      }),
    ).rejects.toThrow("createOriroCodingTools must return an array");
  });

  it("throws when createOriroCodingTools rejects and includes the cause", async () => {
    await expect(
      createCopilotToolBridge({
        agentId: "agent-1",
        createOriroCodingTools: async () => {
  ...
  // F6: PI-parity tool context. The bridged Oriro tools register
  ...
  describe("PI-parity attempt context (F6)", () => {
    function captureCall() {
      ...
      return {
        createOriroCodingTools,
        getOpts: () =>
          (createOriroCodingTools.mock.calls[0] as unknown[] | undefined)?.[0] as Record<
            string,
            unknown
          >,
      };
    ...
    it("forwards identity, owner/policy, and channel/routing fields from attemptParams", async () => {
      const { createOriroCodingTools, getOpts } = captureCall();
    ...
    it("prefers the unscoped toolAuthProfileStore when building Oriro tools", async () => {
      const { createOriroCodingTools, getOpts } = captureCall();
  ...
  describe("sandbox forwarding (PR #86155 [P1])", () => {
    ...
    it("defaults sandbox to undefined and derives spawnWorkspaceDir from workspaceDir when no sandbox is passed (back-compat)", async () => {
      ...
      const opts = (createOriroCodingTools.mock.calls[0] as unknown[] | undefined)?.[0] as {
        sandbox?: unknown;
    ...
    it("forwards an explicit sandbox and spawnWorkspaceDir verbatim to createOriroCodingTools", async () => {
  ...
  // Oriro attempt would suppress. These tests pin the contract.
  describe("tool-surface gating (PR #86155 [P1] round-6)", () => {
    it("short-circuits when attemptParams.disableTools is true and never calls createOriroCodingTools", async () => {
      const createOriroCodingTools = vi.fn(async () => [makeTool()]);
      const result = await createCopilotToolBridge({
        agentId: "agent-1",
    ...
    it("filters constructed tools to exactly the allowlist when toolsAllow is narrow", async () => {
      const createOriroCodingTools = vi.fn(async () => [
        makeTool({ name: "read" }),
        makeTool({ name: "edit" }),
    ...
    it("leaves the tool list unchanged when toolsAllow is undefined", async () => {
      ...
      const createOriroCodingTools = vi.fn(async () => tools);
      const result = await createCopilotToolBridge({
        agentId: "agent-1",
  ...
  describe("tool-name aliases (PR #86155 [P1] round-7)", () => {
    ...
    it("keeps core tools available for glob allowlists", async () => {
      ...
      const options = (createOriroCodingTools.mock.calls[0] as unknown[] | undefined)?.[0] as {
        toolConstructionPlan?: { includeOriroTools?: boolean };
      };
      expect(options?.toolConstructionPlan?.includeOriroTools).toBe(true);
    });
...
describe("convertOriroToolToSdkTool", () => {
  it("throws on empty and non-string names", () => {
    expect(() => convertOriroToolToSdkTool(makeTool({ name: "" as never }), {})).toThrow(
      "tool name must be a non-empty string",
    );
    expect(() => convertOriroToolToSdkTool(makeTool({ name: 42 as never }), {})).toThrow(
      "tool name must be a non-empty string",
    );
  });

  it("throws on non-function execute", () => {
    expect(() => convertOriroToolToSdkTool(makeTool({ execute: "nope" as never }), {})).toThrow(
  ...
  it("preserves name, description, and parameters exactly", () => {
    ...
    const result = convertOriroToolToSdkTool(sourceTool, {});
  ...
  it("sets skipPermission: true so Oriro's wrapped-tool internal enforcement handles permission decisions (PI-parity model)", () => {
    // Per the harness docs: every bridged Oriro tool comes from
    // `createOriroCodingTools`, which already wraps each tool with
  ...
  it("marks every bridged tool as overridesBuiltInTool so Oriro owns names that collide with Copilot CLI built-ins (edit/read/write/bash/...)", () => {
    // Real-world dogfood found that oriro's createOriroCodingTools
    ...
    for (const name of ["edit", "read", "write", "bash", "live_echo"]) {
      const result = convertOriroToolToSdkTool(makeTool({ name }), {}) as SdkTool & {
  ...
  it("returns a failure result when the signal is already aborted", async () => {
    ...
    controller.abort();
    const sourceTool = makeTool();
    const sdkTool = convertOriroToolToSdkTool(sourceTool, { abortSignal: controller.signal });
  ...
  it("calls beforeExecute with the invocation context before execute", async () => {
    ...
    const sourceTool = makeTool();
    const sdkTool = convertOriroToolToSdkTool(sourceTool, { beforeExecute });
  ...
  it("returns a failure result when beforeExecute throws", async () => {
    ...
    const sourceTool = makeTool();
    const sdkTool = convertOriroToolToSdkTool(sourceTool, {
  ...
  it("calls prepareArguments and passes the prepared args and toolCallId to execute", async () => {
    ...
    const sdkTool = convertOriroToolToSdkTool(sourceTool, { onToolCompleted });
  ...
  it("converts single text content to an exact textResultForLlm", async () => {
    ...
    const sdkTool = convertOriroToolToSdkTool(makeTool({}, sourceResult), { onAgentToolResult });
  ...
  it("reports terminal tool results to the harness lifecycle bridge", async () => {
    ...
    const sdkTool = convertOriroToolToSdkTool(makeTool({}, sourceResult), {
      onToolCompleted,
    });
  ...
  it("reports thrown tool failures to the private result observer", async () => {
    ...
    const onAgentToolResult = vi.fn();
    const sdkTool = convertOriroToolToSdkTool(
      makeTool({
        execute: vi.fn(async () => {
          throw error;
        }),
  ...
  it("reports returned Oriro error results to both tool observers", async () => {
    const onAgentToolResult = vi.fn();
    const onToolCompleted = vi.fn();
    const sourceResult = {
