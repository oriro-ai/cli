...
function runProbe(command: string, payload: unknown): void {
  const root = mkdtempSync(path.join(tmpdir(), "oriro-plugin-update-probe-"));
...
describe("plugin update unchanged Docker E2E", () => {
  it("seeds current plugin install ledger state before checking config stability", () => {
    ...
    expect(probe).toContain("writeJson(process.env.ORIRO_CONFIG_PATH, { plugins: {} });");
    expect(probe).not.toContain(
      "writeJson(process.env.ORIRO_CONFIG_PATH, { plugins: { installs",
    );
    expect(probe).toContain("installRecords: {");
    expect(probe).toContain('"lossless-oriro": {');
  });

  it("bounds the update command and prints diagnostics on hangs", () => {
    const script = readFileSync(PLUGIN_UPDATE_SCENARIO_SCRIPT, "utf8");

    expect(script).toContain("ORIRO_PLUGIN_UPDATE_TIMEOUT_SECONDS");
    expect(script).toContain(
      "oriro_e2e_read_positive_int_env ORIRO_PLUGIN_UPDATE_TIMEOUT_SECONDS 180",
    );
    expect(script).toContain(
      'oriro_e2e_maybe_timeout "${plugin_update_timeout_seconds}s" node "$entry" plugins update',
    );
    expect(script).not.toContain(
      'plugin_update_timeout_seconds="${ORIRO_PLUGIN_UPDATE_TIMEOUT_SECONDS:-180}"',
    );
    ...
    expect(script).toContain("oriro_e2e_print_log /tmp/plugin-update-output.log");
    expect(script).toContain("oriro_e2e_print_log /tmp/oriro-e2e-registry.log");
    expect(script).not.toContain("cat /tmp/plugin-update-output.log");
    expect(script).not.toContain("cat /tmp/oriro-e2e-registry.log");
  });
  ...
  it("detects unexpected download output before a large log tail", () => {
    ...
    try {
      writeFileSync(
        logPath,
        [
          "Downloading @example/lossless-oriro",
          "filler line\n".repeat(12 * 1024),
          "lossless-oriro is up to date (0.9.0).",
        ].join("\n"),
        "utf8",
      );
  ...
  it("waits for the local registry process during cleanup", () => {
    ...
    expect(script).toContain('oriro_e2e_stop_process "${registry_pid:-}"');
  ...
  it("bounds corrupt plugin update commands and prints diagnostics on hangs", () => {
    ...
    expect(script).toContain("ORIRO_UPDATE_CORRUPT_PLUGIN_TIMEOUT_SECONDS");
    expect(script).toContain(
      "oriro_e2e_read_positive_int_env ORIRO_UPDATE_CORRUPT_PLUGIN_TIMEOUT_SECONDS 900",
    );
    expect(script).not.toContain(
      'update_timeout_seconds="${ORIRO_UPDATE_CORRUPT_PLUGIN_TIMEOUT_SECONDS:-900}"',
    );
    expect(
      script.match(/oriro_e2e_maybe_timeout "\$\{update_timeout_seconds\}s" \\/gu)?.length,
    ).toBe(2);
    expect(script).toContain("--channel beta");
    expect(script).toContain("ORIRO_UPDATE_POST_CORE=1");
    expect(script).not.toContain(
      'node "$entry" update --channel beta --tag "${ORIRO_CURRENT_PACKAGE_TGZ',
    );
    expect(script).toContain(
      "oriro update failed or timed out after ${update_timeout_seconds}s",
    );
    expect(script).toContain(
      "updated Oriro entry failed or timed out after ${update_timeout_seconds}s",
    );
    expect(script.match(/oriro_e2e_print_log \/tmp\/oriro-update-corrupt-/g)).toHaveLength(8);
    expect(script).not.toContain("cat /tmp/oriro-update-corrupt-");
  });
        ...
        outcomes: [
          {
            ...
            status: "skipped",
            message: `Disabled "${CORRUPT_PLUGIN_ID}" after plugin update failure; Oriro will continue without it. Failed to update ${CORRUPT_PLUGIN_ID}: registry timeout`,
          },
        ],
      },
    };
        ...
        warnings: [
          {
            ...
            message:
              ...
              `Run oriro plugins inspect ${CORRUPT_PLUGIN_ID} --runtime --json for details.`,
          },
        ],
      }),
    ).not.toThrow();
