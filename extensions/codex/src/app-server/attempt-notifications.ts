 ...
 */
import { asBoolean } from "oriro/plugin-sdk/string-coerce-runtime";
import {
...
/** Detects completion for an Oriro dynamic tool result still awaited by Codex. */
export function isPendingOriroDynamicToolCompletionNotification(
  notification: CodexServerNotification,
  pendingOriroDynamicToolCompletionIds: ReadonlySet<string>,
): boolean {
  if (notification.method !== "item/completed" || !isJsonObject(notification.params)) {
    return false;
  }
  const itemId = readNotificationItemId(notification);
  if (!itemId || !pendingOriroDynamicToolCompletionIds.has(itemId)) {
    return false;
  }
  const item = isJsonObject(notification.params.item) ? notification.params.item : undefined;
