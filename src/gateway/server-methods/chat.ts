...
export function buildOversizedHistoryPlaceholder(message?: unknown): Record<string, unknown> {
  ...
  const rawMetadata =
    message && typeof message === "object"
      ? (message as Record<string, unknown>)["__oriro"]
      : undefined;
  const metadata =
...
async function findSourceReplyTranscriptMirrorByIdempotencyKey(
  ...
  if (found?.message.provider !== "oriro" || found.message.model !== "delivery-mirror") {
    return null;
  }
  return found;
}
...
function sendGlobalAwareNodeChatPayload(params: {
  ...
  const deliveryKeys = resolveGlobalAwareNodeChatDeliveryKeys({
    cfg: params.context.getRuntimeConfig?.() ?? ({} as OriroConfig),
    sessionKey: params.sessionKey,
    agentId: params.agentId,
  });
...
function readChatHistoryMessageId(message: unknown): string | undefined {
  const metadata = asOptionalRecord(asOptionalRecord(message)?.["__oriro"]);
...
async function handleChatHistoryRequest({
  ...
  const requestedAgentId = resolveRequestedChatAgentId({
    cfg: (context as { getRuntimeConfig?: () => OriroConfig }).getRuntimeConfig?.(),
      ...
      try {
        await measureDiagnosticsTimelineSpan(
          ...
          async () => {
            ...
            // attachments even when the parent Oriro session model is text-only.
        ...
        .then(async () => {
          ...
          await measureDiagnosticsTimelineSpan(
            ...
            async () => {
              ...
              // through Oriro runtime's SessionManager; this dispatcher only owns live delivery payloads.
                  ...
                  if (shouldAppendAssistantTranscript) {
                    ...
                    if (appended.ok) {
                      ...
                      message = {
                        ...
                        timestamp: nowValue,
                        ...(ttsSupplementMarker
                          ? { oriroTtsSupplement: ttsSupplementMarker }
                          : {}),
