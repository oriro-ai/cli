// Update Orirotributors script supports Oriro repository automation.
import { execFileSync, execSync } from "node:child_process";
import { readFileSync, writeFileSync } from "node:fs";
import { resolve } from "node:path";
import type { ApiContributor, Entry, MapConfig, User } from "./update-orirotributors.types.js";
...
const ORIROTRIBUTORS_START = "<!-- orirotributors:start -->";
const ORIROTRIBUTORS_END = "<!-- orirotributors:end -->";
const ORIROTRIBUTORS_HIDDEN_START = "<!-- orirotributors:hidden:start";
const ORIROTRIBUTORS_HIDDEN_END = "orirotributors:hidden:end -->";

const mapPath = resolve("scripts/orirotributors-map.json");
...
const block = `${ORIROTRIBUTORS_START}\n${markdownLines.join("\n")}\n${ORIROTRIBUTORS_END}`;
...
const readmeWithoutMeta = hiddenRange
  ...
  : currentReadme;
const range = findOrirotributorsRange(readmeWithoutMeta);

if (!range) {
  throw new Error("README.md missing orirotributors block");
}
...
console.log(
  `Updated README orirotributors: ${visibleEntries.length} visible (${entries.length - visibleEntries.length} default-avatar entries hidden)`,
);
  ...
  try {
    return await withAvatarProbeTimeout(login, async ({ signal, timeoutPromise }) => {
      const response = await fetch(`https://github.com/${login}.png?size=${AVATAR_PROBE_SIZE}`, {
        headers: { "user-agent": "oriro-orirotributors" },
        signal,
      });
      if (!response.ok) {
        return false;
...
function parseReadmeEntries(
  ...
  const rangeValue = findOrirotributorsRange(content);
  if (!rangeValue) {
    return [];
  }
...
function buildHiddenReadmeBlock(entriesLocal: Entry[], visibleEntriesLocal: Entry[]): string {
  ...
  if (hiddenLogins.length === 0) {
    return `${ORIROTRIBUTORS_HIDDEN_START}\n${notice}\n${ORIROTRIBUTORS_HIDDEN_END}\n`;
  }
  return `${ORIROTRIBUTORS_HIDDEN_START}\n${notice}\n${hiddenLogins.join("\n")}\n${ORIROTRIBUTORS_HIDDEN_END}\n`;
}

function findOrirotributorsRange(content: string): { start: number; end: number } | null {
  const markerStart = content.indexOf(ORIROTRIBUTORS_START);
  const markerEnd = content.indexOf(ORIROTRIBUTORS_END, markerStart);
  if (markerStart !== -1 && markerEnd !== -1) {
    return {
      start: markerStart,
      end: markerEnd + ORIROTRIBUTORS_END.length,
    };
  }
...
function findHiddenReadmeRange(content: string): { start: number; end: number } | null {
  const markerStart = content.indexOf(ORIROTRIBUTORS_HIDDEN_START);
  const markerEnd = content.indexOf(ORIROTRIBUTORS_HIDDEN_END, markerStart);
  if (markerStart === -1 || markerEnd === -1) {
    return null;
  }
  return {
    start: markerStart,
    end: markerEnd + ORIROTRIBUTORS_HIDDEN_END.length,
  };
}
