---
summary: "CLI reference for `oriro memory` (status/index/search/promote/promote-explain/rem-harness)"
read_when:
...
```bash
oriro memory status
oriro memory status --deep
oriro memory status --fix
oriro memory index --force
oriro memory search "meeting notes"
oriro memory search --query "deployment" --max-results 20
oriro memory promote --limit 10 --min-score 0.75
oriro memory promote --apply
oriro memory promote --json --min-recall-count 0 --min-unique-queries 0
oriro memory promote-explain "router vlan"
oriro memory promote-explain "router vlan" --json
oriro memory rem-harness
oriro memory rem-harness --json
oriro memory status --json
oriro memory status --deep --index
oriro memory status --deep --index --verbose
oriro memory status --agent main
oriro memory index --agent main --verbose
```
...
```bash
oriro memory promote [--apply] [--limit <n>] [--include-promoted]
```
...
- When dreaming is enabled, `memory-core` auto-manages one cron job that runs a full sweep (`light -> REM -> deep`) in the background (no manual `oriro cron add` required).
- `--agent <id>`: scope to a single agent (default: the default agent).
...
```bash
oriro memory promote-explain <selector> [--agent <id>] [--include-promoted] [--json]
```
...
```bash
oriro memory rem-harness [--agent <id>] [--include-promoted] [--json]
```
