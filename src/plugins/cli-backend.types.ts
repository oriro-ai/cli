...
export type PluginTextTransforms = {
  ...
  /** Rewrites applied to inbound assistant text before Oriro consumes it. */
...
export type CliBackendPlugin = {
  ...
  /**
   * Whether Oriro should inject bundle MCP config for this backend.
   *
   * Keep this opt-in. Only backends that explicitly consume Oriro's bundle
   ...
   *
   * Use this when the backend should consume a canonical Oriro auth profile
   ...
   * `profile-only` treats the selected Oriro auth profile as the sole auth
   ...
   * as static config, such as mapping Oriro thinking levels to a backend's
   * native effort flag.
   */
  resolveExecutionArgs?: CliBackendResolveExecutionArgs;
  /**
   * Whether this CLI backend can expose native tools outside Oriro's tool
