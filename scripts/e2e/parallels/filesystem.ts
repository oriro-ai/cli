// Filesystem script supports Oriro repository automation.
...
const ORIRO_VERSION_PATTERN = /Oriro\s+([0-9][^\s]*)/gi;
...
export async function extractLastOriroVersionFromLog(
  logPath: string,
  pattern = ORIRO_VERSION_PATTERN,
  maxBytes = DEFAULT_TEXT_FILE_TAIL_BYTES,
): Promise<string> {
...
export async function makeTempDir(prefix: string): Promise<string> {
  const root =
    process.env.ORIRO_PARALLELS_ARTIFACT_ROOT || path.join(repoRoot, ".artifacts", "parallels");
