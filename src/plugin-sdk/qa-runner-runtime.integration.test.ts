...
const ORIGINAL_ENV = {
  ORIRO_DISABLE_BUNDLED_PLUGINS: process.env.ORIRO_DISABLE_BUNDLED_PLUGINS,
  ORIRO_CONFIG_PATH: process.env.ORIRO_CONFIG_PATH,
  ORIRO_STATE_DIR: process.env.ORIRO_STATE_DIR,
  ORIRO_TEST_FAST: process.env.ORIRO_TEST_FAST,
} as const;
...
describe("plugin-sdk qa-runner-runtime linked plugin smoke", () => {
  ...
  it("loads an activated qa runner from a linked plugin path without a bundled install fallback", async () => {
    const stateDir = makeTempDir("oriro-qa-runner-state-");
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@oriro/qa-linked",
        type: "module",
        oriro: {
          extensions: ["./index.js"],
          install: {
            npmSpec: "@oriro/qa-linked",
          },
        },
      }),
      "utf8",
