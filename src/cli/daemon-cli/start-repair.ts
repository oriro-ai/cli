...
export async function repairLoadedGatewayServiceForStart(params: {
  ...
  const wrapperPath = await resolveOriroWrapperPath(installEnv[ORIRO_WRAPPER_ENV_KEY]);
  ...
  return {
    result: "started",
    message:
      "Gateway service definition repaired and started. Reopen the Control UI with `oriro dashboard` or copy a fresh auth URL with `oriro dashboard --no-open`.",
