...
export function setupCliBundleMcpTestHarness(): void {
  beforeAll(async () => {
    ...
    envSnapshot = captureEnv(["ORIRO_BUNDLED_PLUGINS_DIR"]);
    bundleProbeHomeDir = await tempHarness.createTempDir("oriro-cli-bundle-mcp-home-");
    bundleProbeWorkspaceDir = await tempHarness.createTempDir("oriro-cli-bundle-mcp-workspace-");
    const emptyBundledDir = await tempHarness.createTempDir("oriro-cli-bundle-mcp-bundled-");
    setTestEnvValue("ORIRO_BUNDLED_PLUGINS_DIR", emptyBundledDir);
...
function createEnabledBundleProbeConfig(): OriroConfig {
  return {
    plugins: {
      entries: {
