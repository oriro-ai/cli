...
The ACPX extension is a thin Oriro wrapper around the published `acpx` package. Keep reusable ACP runtime logic in `oriro/acpx`, not in this extension.
...
Use this flow when Oriro needs unreleased ACPX changes before the ACPX version is published.

1. Make the ACPX code change in the `oriro/acpx` repo first.
2. In Oriro, temporarily point `extensions/acpx/package.json` at the ACPX GitHub commit you need.
...
6. Rebuild Oriro and restart the gateway before doing live ACP validation.
...
- Do not rely on a globally installed `acpx` binary for Oriro ACP validation.
...
If a change feels like shared ACP runtime behavior instead of Oriro-specific glue, move it to `oriro/acpx` and consume it from here instead of re-implementing it inside `extensions/acpx`.
