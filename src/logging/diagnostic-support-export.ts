...
const SUPPORT_EXPORT_PREFIX = "oriro-diagnostics-";
...
function resolveBonjourEnvOverride(
  ...
  const raw = env.ORIRO_DISABLE_BONJOUR?.trim().toLowerCase();
  if (!raw) {
    return "unset";
  }
  switch (raw) {
...
function readStabilityBundle(
  ...
  if (target === false) {
    return { status: "missing", dir: "$ORIRO_STATE_DIR/logs/stability" };
  }
...
function renderSummary(params: {
  ...
  return [
    ...
    "- `logs/oriro-sanitized.jsonl`: sanitized log summaries and metadata",
