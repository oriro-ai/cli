...
describe("gateway auth", () => {
  ...
  it("resolves token/password from ORIRO gateway env vars", () => {
    const auth = resolveGatewayAuth({
      authConfig: {},
      env: {
        ORIRO_GATEWAY_TOKEN: "env-token",
        ORIRO_GATEWAY_PASSWORD: "env-password",
  ...
  it("accepts password mode when env provides ORIRO_GATEWAY_PASSWORD", () => {
...
describe("trusted-proxy auth", () => {
  ...
  it.each([
    ...
    },
    {
      ...
      env: {
        ORIRO_GATEWAY_TOKEN: "shared-secret",
      } as NodeJS.ProcessEnv,
    },
