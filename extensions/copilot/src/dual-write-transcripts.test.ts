...
import type { AgentMessage } from "oriro/plugin-sdk/agent-harness-runtime";
import {
...
async function createTempSessionFile() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-copilot-mirror-"));
  tempDirs.push(dir);
...
describe("mirrorCopilotTranscript", () => {
  it("mirrors user, assistant, and tool result messages into the Oriro transcript", async () => {
  ...
  it("creates the transcript directory on first mirror", async () => {
    const root = await makeRoot("oriro-copilot-mirror-missing-dir-");
  ...
  it("uses attached identity instead of content fingerprint when provided", async () => {
    ...
    await mirrorCopilotTranscript({
      sessionFile,
      messages: [tagged],
      idempotencyScope: "copilot:oriro-session-1",
    });

    const raw = await fs.readFile(sessionFile, "utf8");
    expect(raw).toContain(
      '"idempotencyKey":"copilot:oriro-session-1:sdk-session-1:assistant:0"',
    );
