...
const ensureCodexRuntimePluginForModelSelection = vi.hoisted(() =>
  vi.fn(
    async ({ cfg }: { cfg: OriroConfig }): Promise<CodexRuntimePluginInstallResult> => ({
      cfg,
      required: false,
      installed: false,
    }),
...
const ensureCopilotRuntimePluginForModelSelection = vi.hoisted(() =>
  vi.fn(
    async ({ cfg }: { cfg: OriroConfig }): Promise<CopilotRuntimePluginInstallResult> => ({
      cfg,
      required: false,
      installed: false,
    }),
...
describe("applyNonInteractivePluginProviderChoice", () => {
  it("loads plugin providers for provider-plugin auth choices", async () => {
    ...
    const result = await applyNonInteractivePluginProviderChoice({
      nextConfig: { agents: { defaults: {} } } as OriroConfig,
      authChoice: "provider-plugin:vllm:custom",
      opts: {} as never,
      runtime: runtime as never,
      baseConfig: { agents: { defaults: {} } } as OriroConfig,
      resolveApiKey: vi.fn(),
  ...
  it("installs an official catalog provider before applying a cold auth choice", async () => {
    const runtime = createRuntime();
    const runNonInteractive = vi.fn(async ({ config }: { config: OriroConfig }) => ({
      ...config,
      agents: {
        defaults: {
