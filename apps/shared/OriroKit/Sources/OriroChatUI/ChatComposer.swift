...
public struct OriroChatTalkControl {
...
@MainActor
struct OriroChatComposer: View {
    @Bindable var viewModel: OriroChatViewModel
    let style: OriroChatView.Style
    let showsSessionSwitcher: Bool
    let userAccent: Color?
    let assistantName: String?
    let assistantAvatarText: String?
    let assistantAvatarTint: Color?
    let composerChrome: OriroChatView.ComposerChrome
    let isComposerEnabled: Bool
    let messagePlaceholder: String?
    let talkControl: OriroChatTalkControl?
            ...
            if self.composerChrome == .full {
                ...
                if self.style == .standard {
                    let shape = UnevenRoundedRectangle(
                        ...
                        style: .continuous)
                    shape
                        .fill(OriroChatTheme.composerBackground)
                        .overlay(shape.strokeBorder(OriroChatTheme.composerBorder, lineWidth: 1))
    ...
    private var modelPicker: some View {
        ...
        {
            Text(self.viewModel.defaultModelLabel).tag(OriroChatViewModel.defaultModelSelectionID)
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \OriroPendingAttachment.id)
                { (att: OriroPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            OriroPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var fullEditor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 14, style: .continuous)
                .fill(OriroChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 14, style: .continuous)
                        .strokeBorder(OriroChatTheme.composerBorder)))
    ...
    private func talkButton(_ talkControl: OriroChatTalkControl) -> some View {
        Button {
    ...
    private func compactTalkButton(_ talkControl: OriroChatTalkControl) -> some View {
        Button {
            talkControl.toggle(self.viewModel.sessionKey)
        } label: {
            Image(systemName: talkControl.isEnabled ? "stop.fill" : "waveform")
    ...
    private func talkButtonFill(_ talkControl: OriroChatTalkControl) -> AnyShapeStyle {
        if talkControl.isEnabled {
            return AnyShapeStyle(OriroChatTheme.userBubble)
        }
        if !talkControl.isGatewayConnected {
            return AnyShapeStyle(Color.secondary.opacity(0.12))
        }
        return OriroChatTheme.subtleCard
    }

    private func talkButtonStroke(_ talkControl: OriroChatTalkControl) -> Color {
        if talkControl.isEnabled {
            return Color.white.opacity(0.18)
        }
        return OriroChatTheme.composerBorder
    }

    private func talkAccessibilityValue(_ talkControl: OriroChatTalkControl) -> String {
    ...
    private func talkHelpText(_ talkControl: OriroChatTalkControl) -> String {
    ...
    private var connectionPill: some View {
        ...
        .background {
            if self.composerChrome == .full {
                Capsule()
                    .fill(OriroChatTheme.subtleCard)
            }
        }
    }
    ...
    private var sendButtonFill: Color {
        self.userAccent ?? OriroChatTheme.userBubble
    }
