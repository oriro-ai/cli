...
const createOriroTransportStreamFnForModel = vi.fn();
...
describe("prepareModelForSimpleCompletion", () => {
  ...
  beforeEach(() => {
    ...
    createOriroTransportStreamFnForModel.mockReset();
    ...
    createOriroTransportStreamFnForModel.mockReturnValue(undefined);
  ...
  it("routes provider-owned simple-completion wrappers through an internal API alias", () => {
    ...
    expect(result.api).toBe(
      "oriro-provider-simple:moonshot:kimi-k2.7-code:moonshot-simple-source:https%3A%2F%2Fapi.moonshot.ai%2Fv1",
    );
  ...
  it("uses a custom api alias for Anthropic Vertex simple completions", () => {
    ...
    expect(ensureCustomApiRegistered).toHaveBeenCalledWith(
      "oriro-anthropic-vertex-simple:https%3A%2F%2Fus-central1-aiplatform.googleapis.com",
      "vertex-stream",
    );
    expect(result).toEqual({
      ...model,
      api: "oriro-anthropic-vertex-simple:https%3A%2F%2Fus-central1-aiplatform.googleapis.com",
    });
  });
  ...
  it("uses a transport-aware custom api alias when llm request transport overrides are present", () => {
    ...
    expect(ensureCustomApiRegistered).toHaveBeenCalledWith(
      "oriro-openai-responses-transport",
      "transport-stream",
    );
    expect(result).toEqual({
      ...model,
  ...
  it("uses the Google simple-completion sanitizer alias after transport checks pass through", () => {
    ...
    prepareGoogleSimpleCompletionModel.mockImplementationOnce((m: unknown) => ({
      ...
      api: "oriro-google-generative-ai-simple",
    }));
    resolveProviderStreamFn.mockReturnValueOnce(undefined);
  ...
  it.each([
    ...
    "uses Oriro transport for OpenAI Codex-response simple completions with baseUrl %s",
    (baseUrl, expectedBaseUrl) => {
      ...
      resolveProviderStreamFn.mockReturnValueOnce(undefined);
      createOriroTransportStreamFnForModel.mockReturnValueOnce("codex-transport-stream");
      resolveTransportAwareSimpleApi.mockReturnValueOnce("oriro-openai-responses-transport");
      ...
      expect(createOriroTransportStreamFnForModel).toHaveBeenCalledWith(
        {
          ...model,
          baseUrl: expectedBaseUrl,
        },
