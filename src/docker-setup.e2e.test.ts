...
const sandboxRootTracker = createSuiteTempRootTracker({ prefix: "oriro-docker-setup-" });
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    ORIRO_GATEWAY_TOKEN: "test-token",
    ORIRO_CONFIG_DIR: join(sandbox.rootDir, "config"),
    ORIRO_WORKSPACE_DIR: join(sandbox.rootDir, "oriro"),
    ORIRO_AUTH_PROFILE_SECRET_DIR: join(sandbox.rootDir, "auth-profile-secrets"),
  };
...
function isGatewayStartLine(line: string) {
  return line.includes("compose") && line.includes(" up -d") && line.includes("oriro-gateway");
}
...
describe("scripts/docker/setup.sh", () => {
  ...
  it("handles env defaults, home-volume mounts, and Docker build args", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      ORIRO_DOCKER_APT_PACKAGES: "curl wget",
      ORIRO_EXTRA_MOUNTS: undefined,
      ORIRO_HOME_VOLUME: "oriro-home",
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("ORIRO_IMAGE_APT_PACKAGES=curl wget");
    expect(envFile).toContain("ORIRO_EXTRA_MOUNTS=");
    expect(envFile).toContain("ORIRO_HOME_VOLUME=oriro-home"); // pragma: allowlist secret
    expect(envFile).toContain("ORIRO_DISABLE_BONJOUR=");
    expect(envFile).toContain(
      `ORIRO_AUTH_PROFILE_SECRET_DIR=${join(activeSandbox.rootDir, "auth-profile-secrets")}`,
    );
    const extraCompose = await readFile(
      join(activeSandbox.rootDir, "docker-compose.extra.yml"),
      "utf8",
    );
    expect(extraCompose).toContain("oriro-home:/home/node");
    expect(extraCompose).toContain(
      `${join(activeSandbox.rootDir, "auth-profile-secrets")}:/home/node/.config/oriro`,
    );
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("oriro-home:");
    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("--build-arg ORIRO_IMAGE_APT_PACKAGES=curl wget");
    expect(log).toContain(
      `run --rm --no-deps ${prestartContainerEnvFlags} --entrypoint node oriro-gateway dist/index.js onboard --mode local --no-install-daemon --gateway-auth token --gateway-token-ref-env ORIRO_GATEWAY_TOKEN --skip-ui --suppress-gateway-token-output`,
    );
    ...
    expect(log).toContain(
      `run --rm --no-deps ${prestartContainerEnvFlags} --entrypoint node oriro-gateway dist/index.js config set --batch-json [{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"},{"path":"gateway.controlUi.allowedOrigins","value":["http://localhost:18789","http://127.0.0.1:18789"]}]`,
    );
    expect(log).not.toContain("run --rm oriro-cli onboard --mode local --no-install-daemon");
  });
  ...
  it("allows ordinary spaces in host persistence paths and quotes generated mounts", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      ORIRO_CONFIG_DIR: configDir,
      ORIRO_WORKSPACE_DIR: workspaceDir,
      ORIRO_AUTH_PROFILE_SECRET_DIR: authProfileSecretDir,
      ORIRO_HOME_VOLUME: homeVolumeDir,
      ORIRO_EXTRA_MOUNTS: `${extraMountSource}:/mnt/extra data:ro`,
    });

    expect(result.status).toBe(0);
    expect(result.stderr).not.toContain("cannot contain whitespace");
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain(`ORIRO_CONFIG_DIR=${configDir}`);
    expect(envFile).toContain(`ORIRO_WORKSPACE_DIR=${workspaceDir}`);
    expect(envFile).toContain(`ORIRO_AUTH_PROFILE_SECRET_DIR=${authProfileSecretDir}`);
    ...
    expect(extraCompose).toContain(`"${configDir}:/home/node/.oriro"`);
    expect(extraCompose).toContain(`"${workspaceDir}:/home/node/.oriro/workspace"`);
    expect(extraCompose).toContain(`"${authProfileSecretDir}:/home/node/.config/oriro"`);
  ...
  it("persists explicit Docker Bonjour opt-in overrides", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      ORIRO_DISABLE_BONJOUR: "0",
    });

    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("ORIRO_DISABLE_BONJOUR=0");
  });

  it("normalizes legacy ORIRO_DOCKER_APT_PACKAGES into ORIRO_IMAGE_APT_PACKAGES", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);
    ...
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("ORIRO_IMAGE_APT_PACKAGES=curl wget");
    expect(envFile).not.toContain("ORIRO_DOCKER_APT_PACKAGES");

    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("--build-arg ORIRO_IMAGE_APT_PACKAGES=curl wget");
    expect(log).not.toContain("--build-arg ORIRO_DOCKER_APT_PACKAGES");
  });

  it("prefers ORIRO_IMAGE_APT_PACKAGES over legacy ORIRO_DOCKER_APT_PACKAGES", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);

    const result = runDockerSetup(activeSandbox, {
      ORIRO_IMAGE_APT_PACKAGES: "curl wget httpie",
      ORIRO_DOCKER_APT_PACKAGES: "curl wget",
    });
    expect(result.status).toBe(0);

    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("ORIRO_IMAGE_APT_PACKAGES=curl wget httpie");
    expect(envFile).not.toContain("ORIRO_DOCKER_APT_PACKAGES");

    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("--build-arg ORIRO_IMAGE_APT_PACKAGES=curl wget httpie");
    expect(log).not.toMatch(/--build-arg ORIRO_IMAGE_APT_PACKAGES=curl wget(?! httpie)/);
  });

  it("explicitly empty ORIRO_IMAGE_APT_PACKAGES suppresses legacy fallback", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);

    const result = runDockerSetup(activeSandbox, {
      ORIRO_IMAGE_APT_PACKAGES: "",
      ORIRO_DOCKER_APT_PACKAGES: "curl wget",
    });
    expect(result.status).toBe(0);

    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("ORIRO_IMAGE_APT_PACKAGES=");
    expect(envFile).not.toContain("curl wget");

    const log = await readDockerLog(activeSandbox);
    expect(log).not.toContain("--build-arg ORIRO_IMAGE_APT_PACKAGES=curl wget");
  });

  it("avoids shared-network oriro-cli before the gateway is started", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    const prestartCliRunLines = collectMatchingLines(prestartLines, (line) =>
      /\bcompose\b.*\brun\b.*\boriro-cli\b/.test(line),
    );
  ...
  it("forces BuildKit for local and sandbox docker builds", async () => {
    ...
    await withUnixSocket(socketPath, async () => {
      const result = runDockerSetup(activeSandbox, {
        ORIRO_SANDBOX: "1",
        ORIRO_DOCKER_SOCKET: socketPath,
      });
  ...
  it("offline sandbox stays disabled when its configured image is missing", async () => {
    ...
    await withUnixSocket(socketPath, async () => {
      const defaultImage = "registry.example/oriro-sandbox:approved";
      const agentImage = " registry.example/oriro-sandbox:agent ";
      const result = runDockerSetup(
        activeSandbox,
        {
          ORIRO_SANDBOX: "1",
  ...
  it("offline sandbox validates only effective Docker and browser images", async () => {
    ...
    await withUnixSocket(socketPath, async () => {
      const defaultImage = "registry.example/oriro-sandbox:default";
      const browserImage = "registry.example/oriro-sandbox-browser:default";
      const ignoredImages = [
        "registry.example/oriro-sandbox:ssh",
        "registry.example/oriro-sandbox:shared-agent",
        "registry.example/oriro-sandbox-browser:shared-agent",
        "registry.example/oriro-sandbox:off",
        "registry.example/oriro-sandbox-browser:denied",
      ];
      const result = runDockerSetup(
        activeSandbox,
        {
  ...
  it("offline sandbox rejects an incompatible browser image", async () => {
    ...
    await withUnixSocket(socketPath, async () => {
      const browserImage = "registry.example/oriro-sandbox-browser:stale";
      const result = runDockerSetup(
        activeSandbox,
        {
          ORIRO_SANDBOX: "1",
  ...
  it("writes ORIRO_TZ into .env when given a real IANA timezone", async () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      ORIRO_TZ: "Asia/Shanghai",
    });

    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("ORIRO_TZ=Asia/Shanghai");
  });
  ...
  it("precreates agent data dirs to avoid EACCES in container", async () => {
    ...
    expect(log).toContain("run --rm --no-deps --user root --entrypoint sh oriro-gateway -c");
  ...
  it("precreates auth profile secret key dir outside the mounted state dir", async () => {
    ...
    const result = runDockerSetup(activeSandbox, {
      ORIRO_CONFIG_DIR: configDir,
      ORIRO_WORKSPACE_DIR: workspaceDir,
      ORIRO_AUTH_PROFILE_SECRET_DIR: secretDir,
    });
    ...
    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("find /home/node/.config/oriro -xdev");
  });

  it("reuses existing config token when ORIRO_GATEWAY_TOKEN is unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("ORIRO_GATEWAY_TOKEN=config-token-123"); // pragma: allowlist secret
  });

  it("reuses existing .env token when ORIRO_GATEWAY_TOKEN and config token are unset", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await writeFile(
      join(activeSandbox.rootDir, ".env"),
      "ORIRO_GATEWAY_TOKEN=dotenv-token-123\nORIRO_GATEWAY_PORT=18789\n", // pragma: allowlist secret
    );
    const { result, envFile } = await runDockerSetupWithUnsetGatewayToken(
      activeSandbox,
      "dotenv-token-reuse",
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("ORIRO_GATEWAY_TOKEN=dotenv-token-123"); // pragma: allowlist secret
  ...
  it("reuses the last non-empty .env token and strips CRLF without truncating '='", async () => {
    ...
    await writeFile(
      ...
      [
        "ORIRO_GATEWAY_TOKEN=",
        "ORIRO_GATEWAY_TOKEN=first-token",
        "ORIRO_GATEWAY_TOKEN=last=token=value\r", // pragma: allowlist secret
      ].join("\n"),
    );
    const { result, envFile } = await runDockerSetupWithUnsetGatewayToken(
      activeSandbox,
    ...
    expect(result.status).toBe(0);
    expect(envFile).toContain("ORIRO_GATEWAY_TOKEN=last=token=value"); // pragma: allowlist secret
    expect(envFile).not.toContain("ORIRO_GATEWAY_TOKEN=first-token");
    expect(envFile).not.toContain("\r");
  });

  it("treats ORIRO_SANDBOX=0 as disabled", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);
    ...
    expect(result.status).toBe(0);
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("ORIRO_SANDBOX=");

    const log = await readDockerLog(activeSandbox);
    expect(log).toContain("--build-arg ORIRO_INSTALL_DOCKER_CLI=");
    expect(log).not.toContain("--build-arg ORIRO_INSTALL_DOCKER_CLI=1");
  ...
  it("resets stale sandbox mode and overlay when sandbox is not active", async () => {
    ...
    await withUnixSocket(socketPath, async () => {
      const result = runDockerSetup(activeSandbox, {
        ORIRO_SANDBOX: "1",
        ORIRO_DOCKER_SOCKET: socketPath,
        DOCKER_STUB_FAIL_MATCH: "--entrypoint docker oriro-gateway --version",
      });
  ...
  it("keeps offline policy when sandbox config writes fail and the gateway rolls back", async () => {
    ...
    await withUnixSocket(socketPath, async () => {
      ...
      expect(log).toContain(
        "run --pull never --rm --no-deps oriro-cli config set agents.defaults.sandbox.mode non-main",
      );
      expect(log).toContain("config set agents.defaults.sandbox.mode off");
      const forceRecreateLine = log
        .split("\n")
        .find((line) => line.includes("--force-recreate oriro-gateway"));
      expect(forceRecreateLine).toBe(
        `compose compose -f ${join(activeSandbox.rootDir, "docker-compose.yml")} up -d --pull never --no-build --force-recreate oriro-gateway`,
      );
      expect(forceRecreateLine).not.toContain("docker-compose.sandbox.yml");
      expect(log).toContain(
        `image inspect oriro-sandbox:bookworm-slim host=unix://${socketPath}`,
      );
      expectOfflineComposePolicy(lines);
      await expectMissingPath(join(activeSandbox.rootDir, "docker-compose.sandbox.yml"));
    });
  ...
  it("rejects injected multiline ORIRO_EXTRA_MOUNTS values", () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      ORIRO_EXTRA_MOUNTS: "/tmp:/tmp\n  evil-service:\n    image: alpine",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("ORIRO_EXTRA_MOUNTS cannot contain control characters");
  });

  it("rejects invalid ORIRO_EXTRA_MOUNTS mount format", () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      ORIRO_EXTRA_MOUNTS: "bad mount spec",
    });
  ...
  it("rejects invalid ORIRO_HOME_VOLUME names", () => {
    const activeSandbox = requireSandbox(sandbox);

    const result = runDockerSetup(activeSandbox, {
      ORIRO_HOME_VOLUME: "bad name",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("ORIRO_HOME_VOLUME must match");
  });

  it("rejects ORIRO_TZ values that are not present in zoneinfo", () => {
    const activeSandbox = requireSandbox(sandbox);
    ...
    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("ORIRO_TZ must match a timezone in /usr/share/zoneinfo");
  });

  it("skips onboarding when ORIRO_SKIP_ONBOARDING is set", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);
    ...
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("ORIRO_SKIP_ONBOARDING=1");
  });

  it("treats ORIRO_SKIP_ONBOARDING=0 as disabled and runs onboarding", async () => {
    const activeSandbox = requireSandbox(sandbox);
    await resetDockerLog(activeSandbox);

    const result = runDockerSetup(activeSandbox, {
      ORIRO_SKIP_ONBOARDING: "0",
    });

    expect(result.status).toBe(0);
    const log = await readDockerLog(activeSandbox);
    expect(log).toContain(
      "onboard --mode local --no-install-daemon --gateway-auth token --gateway-token-ref-env ORIRO_GATEWAY_TOKEN --skip-ui --suppress-gateway-token-output",
    );
    const envFile = await readFile(join(activeSandbox.rootDir, ".env"), "utf8");
    expect(envFile).toMatch(/ORIRO_SKIP_ONBOARDING=\n/);
  });
  ...
  it("keeps docker-compose gateway Bonjour advertising in auto mode by default", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(
      compose.match(/ORIRO_DISABLE_BONJOUR: \$\{ORIRO_DISABLE_BONJOUR:-\}/g),
    ).toHaveLength(1);
  });

  it("keeps docker-compose CLI network namespace settings in sync", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose).toContain('network_mode: "service:oriro-gateway"');
    expect(compose).toContain("depends_on:\n      - oriro-gateway");
  });

  it("keeps docker-compose gateway token env defaults aligned across services", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose.match(/ORIRO_GATEWAY_TOKEN: \$\{ORIRO_GATEWAY_TOKEN:-\}/g)).toHaveLength(
      2,
    );
  });
  ...
  it("keeps docker-compose auth profile secret key source durable outside state", async () => {
    ...
    expect(
      compose.split(
        '"${ORIRO_AUTH_PROFILE_SECRET_DIR:-${HOME:-/tmp}/.oriro-auth-profile-secrets}:/home/node/.config/oriro"',
      ),
    ).toHaveLength(3);
  });
  ...
  it("keeps docker-compose timezone env defaults aligned across services", async () => {
    const compose = await readFile(join(repoRoot, "docker-compose.yml"), "utf8");
    expect(compose.match(/TZ: \$\{ORIRO_TZ:-UTC\}/g)).toHaveLength(2);
  });
  ...
  it("pins container-side state, workspace, and config dirs on both services so host .env paths cannot leak (#77436)", async () => {
    ...
    expect(compose.match(/ORIRO_HOME: \/home\/node$/gm)).toHaveLength(2);
    expect(compose.match(/ORIRO_STATE_DIR: \/home\/node\/\.oriro$/gm)).toHaveLength(2);
    expect(
      compose.match(/ORIRO_CONFIG_PATH: \/home\/node\/\.oriro\/oriro\.json$/gm),
    ).toHaveLength(2);
    expect(compose.match(/ORIRO_CONFIG_DIR: \/home\/node\/\.oriro$/gm)).toHaveLength(2);
    expect(
      compose.match(/ORIRO_WORKSPACE_DIR: \/home\/node\/\.oriro\/workspace$/gm),
    ).toHaveLength(2);
  });

  it("Dockerfile ARG ORIRO_IMAGE_APT_PACKAGES must not have a default value", async () => {
    // If the ARG has a default (e.g. ARG ORIRO_IMAGE_APT_PACKAGES=""), Docker treats it as
    // "set" even when no --build-arg is passed. That breaks the RUN fallback expression
    // ${ORIRO_IMAGE_APT_PACKAGES-$ORIRO_DOCKER_APT_PACKAGES} because the variable is
    // never truly unset, so legacy-only callers using --build-arg ORIRO_DOCKER_APT_PACKAGES
    ...
    const argLine = dockerfile
      .split("\n")
      .find((line) => line.startsWith("ARG ORIRO_IMAGE_APT_PACKAGES"));
    expect(argLine).toBeDefined();
    // Must be bare `ARG ORIRO_IMAGE_APT_PACKAGES` with no default assignment
    expect(argLine).toBe("ARG ORIRO_IMAGE_APT_PACKAGES");
  });
});
