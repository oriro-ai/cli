...
async function verifyRuntimeContextTranscriptShape(root: string) {
  const sessionFile = path.join(root, ".oriro", "agents", "main", "sessions", "runtime.jsonl");
  ...
  assert(
    runtimeContextMessage.customType === "oriro.runtime-context",
  ...
  assert(
    !userText.includes("ORIRO_INTERNAL_CONTEXT") && !userText.includes("secret docker context"),
...
async function main() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-session-runtime-context-"));
  process.env.HOME = root;
  process.env.ORIRO_STATE_DIR = path.join(root, ".oriro");
  process.env.ORIRO_CONFIG_PATH = path.join(process.env.ORIRO_STATE_DIR, "oriro.json");
  try {
    ...
    if (process.env.ORIRO_SESSION_RUNTIME_CONTEXT_KEEP_ARTIFACTS !== "1") {
