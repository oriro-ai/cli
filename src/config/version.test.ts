...
import {
  compareOriroVersions,
  isSameOriroStableFamily,
  parseOriroVersion,
  shouldWarnOnTouchedVersion,
} from "./version.js";

describe("parseOriroVersion", () => {
  it("parses stable, correction, and beta forms", () => {
    expect(parseOriroVersion("2026.3.23")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: null,
    ...
    expect(parseOriroVersion("2026.3.23-1")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: 1,
    ...
    expect(parseOriroVersion("2026.3.23-beta.1")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: null,
    ...
    expect(parseOriroVersion("v2026.3.23.beta.2")).toEqual({
      major: 2026,
      minor: 3,
      patch: 23,
      revision: null,
  ...
  it("rejects invalid versions", () => {
    expect(parseOriroVersion("2026.3")).toBeNull();
    expect(parseOriroVersion("latest")).toBeNull();
  });
});

describe("compareOriroVersions", () => {
  it("treats correction publishes as newer than the base stable release", () => {
    expect(compareOriroVersions("2026.3.23", "2026.3.23-1")).toBe(-1);
    expect(compareOriroVersions("2026.3.23-1", "2026.3.23")).toBe(1);
    expect(compareOriroVersions("2026.3.23-2", "2026.3.23-1")).toBe(1);
  });

  it("treats stable as newer than beta and compares beta identifiers", () => {
    expect(compareOriroVersions("2026.3.23", "2026.3.23-beta.1")).toBe(1);
    expect(compareOriroVersions("2026.3.23-beta.2", "2026.3.23-beta.1")).toBe(1);
    expect(compareOriroVersions("2026.3.23.beta.1", "2026.3.23-beta.2")).toBe(-1);
  });
});

describe("isSameOriroStableFamily", () => {
  it("treats same-base stable and correction versions as one family", () => {
    expect(isSameOriroStableFamily("2026.3.23", "2026.3.23-1")).toBe(true);
    expect(isSameOriroStableFamily("2026.3.23-1", "2026.3.23-2")).toBe(true);
    expect(isSameOriroStableFamily("2026.3.23", "2026.3.24")).toBe(false);
    expect(isSameOriroStableFamily("2026.3.23-beta.1", "2026.3.23")).toBe(false);
  });
});
