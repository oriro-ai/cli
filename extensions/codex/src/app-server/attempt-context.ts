...
import { resolveAgentWorkspaceDir } from "oriro/plugin-sdk/agent-runtime";
...
export async function buildCodexWorkspaceBootstrapContext(params: {
  ...
  try {
    ...
    const developerInstructionFiles = shouldInjectCodexOriroPromptContext(params.params)
      ? selectCodexWorkspaceInheritedDeveloperInstructionFiles(contextFiles)
      : [];
    const turnScopedDeveloperInstructionFiles = shouldInjectCodexOriroPromptContext(
      params.params,
    )
    ...
    return {
      ...
      memoryCollaborationInstructions: shouldInjectCodexOriroPromptContext(params.params)
...
/**
 * Builds Oriro-provided workspace prompt context for the current Codex turn.
 */
export function buildCodexOriroPromptContext(params: {
  params: EmbeddedRunAttemptParams;
  workspacePromptContext?: string;
}): string | undefined {
  if (!shouldInjectCodexOriroPromptContext(params.params)) {
    return undefined;
  }
  const sections = [
...
function shouldInjectCodexOriroPromptContext(params: EmbeddedRunAttemptParams): boolean {
...
/** Renders loaded Oriro skill prompts as Codex collaboration instructions. */
export function renderCodexSkillsCollaborationInstructions(params: {
  attempt: EmbeddedRunAttemptParams;
  skillsPrompt?: string;
}): string | undefined {
  if (!shouldInjectCodexOriroPromptContext(params.attempt)) {
    return undefined;
  }
...
/**
 * Prepends Oriro context while preserving leading delivery metadata as
 * routing guidance instead of user request text.
 */
export function prependCodexOriroPromptContext(
  prompt: string,
