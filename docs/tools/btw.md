...
It is modeled after Claude Code's `/btw` behavior, but adapted to Oriro's
...
instead of falling back to a direct provider call. Oriro seeds sanitized
conversation context into a fresh one-shot CLI invocation, disables Oriro MCP
...
If the main run is currently active, Oriro snapshots the current message
