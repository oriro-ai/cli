...
import Foundation
import Testing
@testable import OriroDiscovery
...
struct WideAreaGatewayDiscoveryTests {
    @Test func `discovers beacon from tailnet dns sd fallback`() {
        let originalWideAreaDomain = getenv("ORIRO_WIDE_AREA_DOMAIN").map { String(cString: $0) }
        setenv("ORIRO_WIDE_AREA_DOMAIN", "oriro.internal", 1)
        defer {
            if let originalWideAreaDomain {
                setenv("ORIRO_WIDE_AREA_DOMAIN", originalWideAreaDomain, 1)
            } else {
                unsetenv("ORIRO_WIDE_AREA_DOMAIN")
            }
        }
        let statusJson = """
        {
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.100.100.100" {
                        return "steipetacstudio-gateway._oriro-gw._tcp.oriro.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
        ...
        let expectedDisplay = "Peter\u{2019}s Mac Studio (Oriro)"
            ...
            dig: { args, _ in
                ...
                if recordType == "PTR" {
                    if nameserver == "@100.64.0.2" {
                        return "evil._oriro-gw._tcp.oriro.internal.\n"
                    }
                    return ""
                }
                if recordType == "SRV" {
