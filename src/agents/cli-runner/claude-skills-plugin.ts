/**
 * Materializes selected Oriro skills as a temporary Claude CLI plugin.
 */
...
const ORIRO_CLAUDE_PLUGIN_NAME = "oriro-skills";
...
export async function prepareClaudeCliSkillsPlugin(params: {
  ...
  const tempDir = await fs.mkdtemp(
    path.join(resolvePreferredOriroTmpDir(), "oriro-claude-skills-"),
  );
  const pluginDir = path.join(tempDir, ORIRO_CLAUDE_PLUGIN_NAME);
