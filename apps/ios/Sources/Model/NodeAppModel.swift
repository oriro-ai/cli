...
final class NodeAppModel {
    ...
    private let deepLinkLogger = Logger(subsystem: "ai.orirofoundation.app", category: "DeepLink")
    ...
    private let pushWakeLogger = Logger(subsystem: "ai.orirofoundation.app", category: "PushWake")
    private let pendingActionLogger = Logger(subsystem: "ai.orirofoundation.app", category: "PendingAction")
    private let locationWakeLogger = Logger(subsystem: "ai.orirofoundation.app", category: "LocationWake")
    private let watchReplyLogger = Logger(subsystem: "ai.orirofoundation.app", category: "WatchReply")
    private let watchExecApprovalLogger = Logger(subsystem: "ai.orirofoundation.app", category: "WatchExecApproval")
    private let execApprovalNotificationLogger = Logger(
        subsystem: "ai.orirofoundation.app",
    ...
    func makeChatTransport() -> any OriroChatTransport {
    ...
    private func handleCanvasA2UIAction(body: [String: Any]) async {
        ...
        guard let name = OriroCanvasA2UIAction.extractActionName(userAction) else { return }
        let actionId: String = {
        ...
        let contextJSON = OriroCanvasA2UIAction.compactJSON(userAction["context"])
        let sessionKey = self.mainSessionKey

        let messageContext = OriroCanvasA2UIAction.AgentMessageContext(
            actionName: name,
            session: .init(key: sessionKey, surfaceId: surfaceId),
            component: .init(id: sourceComponentId, host: host, instanceId: instanceId),
            contextJSON: contextJSON)
        let message = OriroCanvasA2UIAction.formatAgentMessage(messageContext)
        ...
        let js = OriroCanvasA2UIAction.jsDispatchA2UIActionStatus(actionId: actionId, ok: ok, error: errorText)
        do {
    ...
    func requestLocationPermissions(mode: OriroLocationMode) async -> Bool {
            ...
            check: { [weak self] in
                ...
                do {
                    let data = try await self.operatorGateway.request(
                        ...
                        paramsJSON: nil,
                        timeoutSeconds: 6)
                    guard let decoded = try? JSONDecoder().decode(OriroGatewayHealthOK.self, from: data) else {
                        return false
                    }
        ...
        do {
            ...
            case .unknownCommand:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: OriroNodeError(code: .invalidRequest, message: "INVALID_REQUEST: unknown command"))
            case .handlerUnavailable:
                return BridgeInvokeResponse(
                    id: req.id,
                    ok: false,
                    error: OriroNodeError(code: .unavailable, message: "node handler unavailable"))
            }
        } catch {
            ...
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: OriroNodeError(code: .unavailable, message: error.localizedDescription))
        }
    }
    ...
    private func handleLocationInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        ...
        let params = (try? Self.decodeParams(OriroLocationGetParams.self, from: req.paramsJSON)) ??
            OriroLocationGetParams()
        ...
        let payload = OriroLocationPayload(
    ...
    private func handleCanvasInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case OriroCanvasCommand.present.rawValue:
            // iOS ignores placement hints; canvas always fills the screen.
            let params = (try? Self.decodeParams(OriroCanvasPresentParams.self, from: req.paramsJSON)) ??
                OriroCanvasPresentParams()
        ...
        case OriroCanvasCommand.hide.rawValue:
            self.screen.hideCanvas()
            return BridgeInvokeResponse(id: req.id, ok: true)
        case OriroCanvasCommand.navigate.rawValue:
            let params = try Self.decodeParams(OriroCanvasNavigateParams.self, from: req.paramsJSON)
            let trimmedURL = params.url.trimmingCharacters(in: .whitespacesAndNewlines)
            self.screen.present(urlString: trimmedURL)
            return BridgeInvokeResponse(id: req.id, ok: true)
        case OriroCanvasCommand.evalJS.rawValue:
            let params = try Self.decodeParams(OriroCanvasEvalParams.self, from: req.paramsJSON)
            let result = try await self.screen.eval(javaScript: params.javaScript)
            let payload = try Self.encodePayload(["result": result])
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case OriroCanvasCommand.snapshot.rawValue:
            let params = try? Self.decodeParams(OriroCanvasSnapshotParams.self, from: req.paramsJSON)
    ...
    private func handleCanvasA2UIInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let command = req.command
        switch command {
        case OriroCanvasA2UICommand.reset.rawValue:
            ...
            (() => {
              const host = globalThis.oriroA2UI;
              if (!host) return JSON.stringify({ ok: false, error: "missing oriroA2UI" });
              return JSON.stringify(host.reset());
            })()
            """)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)

        case OriroCanvasA2UICommand.push.rawValue, OriroCanvasA2UICommand.pushJSONL.rawValue:
            let messages: [OriroKit.AnyCodable]
            if command == OriroCanvasA2UICommand.pushJSONL.rawValue {
                let params = try Self.decodeParams(OriroCanvasA2UIPushJSONLParams.self, from: req.paramsJSON)
                messages = try OriroCanvasA2UIJSONL.decodeMessagesFromJSONL(params.jsonl)
            } else {
                do {
                    let params = try Self.decodeParams(OriroCanvasA2UIPushParams.self, from: req.paramsJSON)
            ...
            let messagesJSON = try OriroCanvasA2UIJSONL.encodeMessagesJSONArray(messages)
            let js = """
            (() => {
              try {
                const host = globalThis.oriroA2UI;
    ...
    private func handleCameraInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case OriroCameraCommand.list.rawValue:
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case OriroCameraCommand.snap.rawValue:
            self.showCameraHUD(text: "Taking photo…", kind: .photo)
            self.triggerCameraFlash()
            let params = (try? Self.decodeParams(OriroCameraSnapParams.self, from: req.paramsJSON)) ??
                OriroCameraSnapParams()
            ...
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: payload)
        case OriroCameraCommand.clip.rawValue:
            let params = (try? Self.decodeParams(OriroCameraClipParams.self, from: req.paramsJSON)) ??
                OriroCameraClipParams()
    ...
    private func handleScreenRecordInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(OriroScreenRecordParams.self, from: req.paramsJSON)) ??
            OriroScreenRecordParams()
    ...
    private func handleSystemNotify(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(OriroSystemNotifyParams.self, from: req.paramsJSON)
        ...
        if title.isEmpty, body.isEmpty {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: OriroNodeError(code: .invalidRequest, message: "INVALID_REQUEST: empty notification"))
        }
        ...
        guard Self.isNotificationAuthorizationAllowed(status) else {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: OriroNodeError(code: .unavailable, message: "NOT_AUTHORIZED: notifications"))
        }
        ...
        if case let .failure(error) = addResult {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: OriroNodeError(code: .unavailable, message: "NOTIFICATION_FAILED: \(error.message)"))
        }
        return BridgeInvokeResponse(id: req.id, ok: true)
    }

    private func handleChatPushInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = try Self.decodeParams(OriroChatPushParams.self, from: req.paramsJSON)
        let text = params.text.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !text.isEmpty else {
            return BridgeInvokeResponse(
                id: req.id,
                ok: false,
                error: OriroNodeError(code: .invalidRequest, message: "INVALID_REQUEST: empty chat.push text"))
        }
        ...
        let payload = OriroChatPushPayload(messageId: messageId)
        let json = try Self.encodePayload(payload)
        return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
    }
    ...
    private func handleDeviceInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case OriroDeviceCommand.status.rawValue:
            let payload = try await self.deviceStatusService.status()
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case OriroDeviceCommand.info.rawValue:
    ...
    private func handlePhotosInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        let params = (try? Self.decodeParams(OriroPhotosLatestParams.self, from: req.paramsJSON)) ??
            OriroPhotosLatestParams()
    ...
    private func handleContactsInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case OriroContactsCommand.search.rawValue:
            let params = (try? Self.decodeParams(OriroContactsSearchParams.self, from: req.paramsJSON)) ??
                OriroContactsSearchParams()
            let payload = try await self.contactsService.search(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case OriroContactsCommand.add.rawValue:
            let params = try Self.decodeParams(OriroContactsAddParams.self, from: req.paramsJSON)
    ...
    private func handleCalendarInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case OriroCalendarCommand.events.rawValue:
            let params = (try? Self.decodeParams(OriroCalendarEventsParams.self, from: req.paramsJSON)) ??
                OriroCalendarEventsParams()
            let payload = try await self.calendarService.events(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case OriroCalendarCommand.add.rawValue:
            let params = try Self.decodeParams(OriroCalendarAddParams.self, from: req.paramsJSON)
    ...
    private func handleRemindersInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case OriroRemindersCommand.list.rawValue:
            let params = (try? Self.decodeParams(OriroRemindersListParams.self, from: req.paramsJSON)) ??
                OriroRemindersListParams()
            let payload = try await self.remindersService.list(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case OriroRemindersCommand.add.rawValue:
            let params = try Self.decodeParams(OriroRemindersAddParams.self, from: req.paramsJSON)
    ...
    private func handleMotionInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case OriroMotionCommand.activity.rawValue:
            let params = (try? Self.decodeParams(OriroMotionActivityParams.self, from: req.paramsJSON)) ??
                OriroMotionActivityParams()
            let payload = try await self.motionService.activities(params: params)
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case OriroMotionCommand.pedometer.rawValue:
            let params = (try? Self.decodeParams(OriroPedometerParams.self, from: req.paramsJSON)) ??
                OriroPedometerParams()
    ...
    private func handleTalkInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case OriroTalkCommand.pttStart.rawValue:
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case OriroTalkCommand.pttStop.rawValue:
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case OriroTalkCommand.pttCancel.rawValue:
            let payload = await self.talkMode.cancelPushToTalk()
            self.voiceWake.resumeAfterExternalAudioCapture(wasSuspended: self.pttVoiceWakeSuspended)
            self.pttVoiceWakeSuspended = false
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case OriroTalkCommand.pttOnce.rawValue:
            self.pttVoiceWakeSuspended = self.voiceWake.suspendForExternalAudioCapture()
            defer {
                self.voiceWake.resumeAfterExternalAudioCapture(wasSuspended: self.pttVoiceWakeSuspended)
                self.pttVoiceWakeSuspended = false
...
extension NodeAppModel {
    ...
    private func buildCapabilityRouter() -> NodeCapabilityRouter {
        ...
        register([OriroLocationCommand.get.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleLocationInvoke(req)
        }

        register([
            OriroCanvasCommand.present.rawValue,
            OriroCanvasCommand.hide.rawValue,
            OriroCanvasCommand.navigate.rawValue,
            OriroCanvasCommand.evalJS.rawValue,
            OriroCanvasCommand.snapshot.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            OriroCanvasA2UICommand.reset.rawValue,
            OriroCanvasA2UICommand.push.rawValue,
            OriroCanvasA2UICommand.pushJSONL.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([OriroScreenCommand.record.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleScreenRecordInvoke(req)
        }

        register([OriroSystemCommand.notify.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleSystemNotify(req)
        }

        register([OriroChatCommand.push.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            OriroWatchCommand.status.rawValue,
            OriroWatchCommand.notify.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
            return try await self.handleWatchInvoke(req)
        }

        register([OriroPhotosCommand.latest.rawValue]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
        ...
        register([
            OriroRemindersCommand.list.rawValue,
            OriroRemindersCommand.add.rawValue,
        ]) { [weak self] req in
            guard let self else { throw NodeCapabilityRouter.RouterError.handlerUnavailable }
    ...
    private func handleWatchInvoke(_ req: BridgeInvokeRequest) async throws -> BridgeInvokeResponse {
        switch req.command {
        case OriroWatchCommand.status.rawValue:
            let status = await self.watchMessagingService.status()
            let payload = OriroWatchStatusPayload(
            ...
            let json = try Self.encodePayload(payload)
            return BridgeInvokeResponse(id: req.id, ok: true, payloadJSON: json)
        case OriroWatchCommand.notify.rawValue:
            let params = try Self.decodeParams(OriroWatchNotifyParams.self, from: req.paramsJSON)
            ...
            do {
                ...
                let payload = OriroWatchNotifyPayload(
    ...
    private func locationMode() -> OriroLocationMode {
        let raw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        return OriroLocationMode(rawValue: raw) ?? .off
    }
...
extension NodeAppModel {
    ...
    private func legacyClientIdFallback(currentClientId: String, error: Error) -> String? {
        ...
        guard normalizedClientId == "oriro-ios" else { return nil }
...
extension NodeAppModel {
    ...
    func runSharePipelineSelfTest() async {
        ...
        let payload = SharedContentPayload(
            title: "Oriro Share Self-Test",
            url: URL(string: "https://oriro.ai/share-self-test"),
    ...
    private static func makeWatchExecApprovalItem(from prompt: ExecApprovalPrompt) -> OriroWatchExecApprovalItem {
        let decisions = prompt.allowedDecisions.compactMap { decision in
            ...
            return OriroWatchExecApprovalDecision(rawValue: normalizedDecision)
        }
        let preview = Self.trimmedOrNil(prompt.commandPreview) ?? Self.trimmedOrNil(prompt.commandText)
        return OriroWatchExecApprovalItem(
            id: prompt.id,
    ...
    private func publishWatchExecApprovalPrompt(_ prompt: ExecApprovalPrompt, reason: String) async {
        let message = OriroWatchExecApprovalPromptMessage(
        ...
        source: String) async
    {
        ...
        let message = OriroWatchExecApprovalResolvedMessage(
    ...
    private func publishWatchExecApprovalExpired(
        approvalId: String,
        reason: OriroWatchExecApprovalCloseReason) async
    {
        let normalizedApprovalID = approvalId.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !normalizedApprovalID.isEmpty else { return }
        self.removeWatchExecApprovalPrompt(normalizedApprovalID)
        let message = OriroWatchExecApprovalExpiredMessage(
            approvalId: normalizedApprovalID,
            reason: reason,
    ...
    private func syncWatchExecApprovalSnapshot(reason: String) async {
        ...
        let message = OriroWatchExecApprovalSnapshotMessage(
            approvals: approvals,
            sentAtMs: Int(Date().timeIntervalSince1970 * 1000),
    ...
    private func makeWatchChatPreview() async -> WatchChatPreview {
        do {
            let payload: OriroChatHistoryPayload
            if self.isAppleReviewDemoModeEnabled {
    ...
    private nonisolated static func decodeWatchChatMessage(
        _ raw: OriroKit.AnyCodable) -> OriroChatMessage?
    {
        guard let data = try? JSONEncoder().encode(raw) else { return nil }
        return try? JSONDecoder().decode(OriroChatMessage.self, from: data)
    }

    private nonisolated static func makeWatchChatItems(
        from raw: [OriroKit.AnyCodable]) -> [OriroWatchChatItem]
    {
        var readableMessages: [(OriroChatMessage, String)] = []
    ...
    private nonisolated static func watchChatText(from message: OriroChatMessage) -> String {
        let parts = message.content.compactMap { content -> String? in
            ...
            if let dict = content.content?.value as? [String: OriroKit.AnyCodable],
    ...
    private func makeWatchAppSnapshot(
        chatPreview: WatchChatPreview? = nil) -> OriroWatchAppSnapshotMessage
    {
        self.pruneExpiredWatchExecApprovalPrompts()
        ...
        return OriroWatchAppSnapshotMessage(
    ...
    func handleSilentPushWake(_ userInfo: [AnyHashable: Any]) async -> Bool {
        ...
        let pushKind = Self.oriroPushKind(userInfo)
        let receivedMessage =
    ...
    private static func oriroPushKind(_ userInfo: [AnyHashable: Any]) -> String {
        ...
        if let payload = userInfo["oriro"] as? [AnyHashable: Any],
           let kind = payload["kind"] as? String
        {
            let trimmed = kind.trimmingCharacters(in: .whitespacesAndNewlines)
            if !trimmed.isEmpty { return trimmed }
        ...
        sourceReason: String? = nil) async -> ExecApprovalResolutionOutcome
    {
        ...
        guard connected else {
            ...
            return .failed(message: "Oriro couldn't connect to the gateway operator session.")
        }
        ...
        do {
            ...
            await self.publishWatchExecApprovalResolved(
                approvalId: normalizedApprovalID,
                decision: OriroWatchExecApprovalDecision(rawValue: normalizedDecision),
                source: "iphone")
            return .resolved
        } catch {
            if Self.isApprovalNotificationStaleError(error) {
            ...
            return .failed(
                message: "Oriro couldn't resolve this approval right now. Try again.")
        }
    }
...
extension NodeAppModel {
    ...
    nonisolated static func _test_makeWatchChatItems(from raw: [OriroKit.AnyCodable]) -> [OriroWatchChatItem] {
