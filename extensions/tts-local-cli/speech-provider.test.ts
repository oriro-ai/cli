...
import type { SpeechProviderConfig, SpeechSynthesisRequest } from "oriro/plugin-sdk/speech-core";
...
const TEST_CFG = {} as OriroConfig;

function createCliFixture(): { dir: string; script: string } {
  const dir = mkdtempSync(path.join(os.tmpdir(), "oriro-cli-tts-test-"));
...
describe("buildCliSpeechProvider", () => {
  ...
  it("can synthesize through a real local CLI fixture and ffmpeg", async () => {
    if (process.env.ORIRO_LIVE_TEST !== "1") {
      return;
    }
    const fixture = createCliFixture();
    const rawFfmpeg = await vi.importActual<typeof import("oriro/plugin-sdk/media-runtime")>(
