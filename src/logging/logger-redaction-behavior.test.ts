...
const logPathTracker = createSuiteLogPathTracker("oriro-log-redaction-");
...
describe("file log redaction", () => {
  ...
  it("uses logging.file from the active config path", () => {
    ...
    withEnv({ ORIRO_CONFIG_PATH: configPath, ORIRO_TEST_FILE_LOG: "1" }, () => {
  ...
  it("expands leading tilde in logging.file", () => {
    ...
    withEnv({ HOME: home }, () => {
      expect(loggerTest.resolveActiveLogFile("~/custom-oriro.log")).toBe(
        path.join(home, "custom-oriro.log"),
      );
    });
  });
