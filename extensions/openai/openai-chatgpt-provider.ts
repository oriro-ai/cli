...
import { CODEX_CLI_PROFILE_ID, type OAuthCredential } from "oriro/plugin-sdk/provider-auth";
...
import { fetchCodexUsage } from "oriro/plugin-sdk/provider-usage";
import {
 ...
 * entirely when they were written by older Oriro versions. When that row wins
...
async function runOpenAICodexDeviceCode(ctx: ProviderAuthContext) {
  ...
  try {
    ...
    await ctx.prompter.note(
      "Trouble with device code login? See https://docs.oriro.ai/start/faq",
      "OAuth help",
    );
    throw error;
  }
...
function buildOpenAICodexAuthDoctorHint(ctx: { profileId?: string }) {
  ...
  return "Deprecated profile. Run `oriro models auth login --provider openai` or `oriro configure`.";
}
