...
import type { OriroPackageManifest } from "../../plugins/manifest.js";
...
export type ChannelPluginCatalogInstall = PluginPackageInstall &
  ({ orirohubSpec: string } | { npmSpec: string });
...
const ENV_CATALOG_PATHS = ["ORIRO_PLUGIN_CATALOG_PATHS", "ORIRO_MPM_CATALOG_PATHS"];
...
function resolveOfficialCatalogPaths(options: CatalogOptions): string[] {
  ...
  const packageRoots = uniqueStrings(
    [
      resolveOriroPackageRootSync({ cwd: process.cwd() }),
      resolveOriroPackageRootSync({ moduleUrl: import.meta.url }),
...
function toChannelMeta(params: {
  channel: NonNullable<OriroPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function resolveInstallInfo(params: {
  ...
  const orirohubSpec = normalizeOptionalString(params.install?.orirohubSpec);
  let npmSpec =
  ...
  const defaultChoice: NonNullable<PluginPackageInstall["defaultChoice"]> =
    requestedDefaultChoice === "orirohub" && orirohubSpec
      ? "orirohub"
