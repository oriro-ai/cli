...
function isOriroCommandBasename(basename: string, platform: NodeJS.Platform): boolean {
  if (basename === "oriro") {
    return true;
  }
  if (platform === "win32") {
    return (
      basename === "oriro.cmd" || basename === "oriro.ps1" || basename === "oriro.exe"
    );
  }
  return false;
}
...
/** Resolve the Oriro CLI binary directory from argv/PATH for daemon PATH. */
export function resolveDaemonOriroBinDir(
  params: {
    argv?: string[];
    env?: Record<string, string | undefined>;
  ...
  if (
    ...
    isOriroCommandBasename(path.basename(argv1), platform)
  ) {
  ...
  for (const rawSegment of (env.PATH ?? "").split(path.delimiter)) {
    ...
    const candidate = path.join(segment, platform === "win32" ? "oriro.cmd" : "oriro");
...
/** Merge Node and Oriro binary directories for the daemon service PATH. */
export function resolveDaemonServicePathDirs(params: {
  ...
  for (const dir of resolveDaemonOriroBinDir(params) ?? []) {
    addUniquePathDir(dirs, dir);
  }
  return dirs.length > 0 ? dirs : undefined;
}
