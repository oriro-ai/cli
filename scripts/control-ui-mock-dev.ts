// Control Ui Mock Dev script supports Oriro repository automation.
...
function createChatPickerScenario(): ControlUiMockGatewayScenario {
  ...
  const workspaceListCases = ["main", "alpha", "oriro-mock"].map((agentId) => ({
    match: { agentId },
    response: {
      agentId,
  ...
  const workspaceFileCases = ["main", "alpha", "oriro-mock"].flatMap((agentId) =>
...
function createMockGatewayPlugin(scenario: ControlUiMockGatewayScenario): Plugin {
  ...
  return {
    ...
    name: "oriro-control-ui-mock-gateway",
    transformIndexHtml(html) {
      return html.replace(
        "</head>",
        `    <script data-oriro-control-ui-mock-gateway>\n${initScript}\n    </script>\n  </head>`,
      );
    },
  };
}
...
const server = await createServer({
  ...
  define: {
    ORIRO_CONTROL_UI_BUILD_ID: JSON.stringify("mock"),
  },
  logLevel: "error",
  optimizeDeps: {
