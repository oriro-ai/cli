...
import { pathExists } from "oriro/plugin-sdk/security-runtime";
...
const CRABBOX_BIN_ENV = "ORIRO_MANTIS_CRABBOX_BIN";
const CRABBOX_PROVIDER_ENV = "ORIRO_MANTIS_CRABBOX_PROVIDER";
const CRABBOX_CLASS_ENV = "ORIRO_MANTIS_CRABBOX_CLASS";
const CRABBOX_MARKET_ENV = "ORIRO_MANTIS_CRABBOX_MARKET";
const CRABBOX_LEASE_ID_ENV = "ORIRO_MANTIS_CRABBOX_LEASE_ID";
const CRABBOX_KEEP_ENV = "ORIRO_MANTIS_KEEP_VM";
const CRABBOX_IDLE_TIMEOUT_ENV = "ORIRO_MANTIS_CRABBOX_IDLE_TIMEOUT";
const CRABBOX_TTL_ENV = "ORIRO_MANTIS_CRABBOX_TTL";
const HYDRATE_MODE_ENV = "ORIRO_MANTIS_HYDRATE_MODE";
const SLACK_URL_ENV = "ORIRO_MANTIS_SLACK_URL";
const SLACK_CHANNEL_ID_ENV = "ORIRO_MANTIS_SLACK_CHANNEL_ID";
...
function buildCrabboxEnv(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  if (!trimToValue(next.ORIRO_LIVE_OPENAI_KEY) && trimToValue(next.OPENAI_API_KEY)) {
    next.ORIRO_LIVE_OPENAI_KEY = next.OPENAI_API_KEY;
  }
  if (!trimToValue(next.ORIRO_MANTIS_SLACK_BOT_TOKEN) && trimToValue(next.SLACK_BOT_TOKEN)) {
    next.ORIRO_MANTIS_SLACK_BOT_TOKEN = next.SLACK_BOT_TOKEN;
  }
  if (
    !trimToValue(next.ORIRO_MANTIS_SLACK_BOT_TOKEN) &&
    trimToValue(next.ORIRO_QA_SLACK_SUT_BOT_TOKEN)
  ) {
    next.ORIRO_MANTIS_SLACK_BOT_TOKEN = next.ORIRO_QA_SLACK_SUT_BOT_TOKEN;
  }
  if (!trimToValue(next.ORIRO_MANTIS_SLACK_APP_TOKEN) && trimToValue(next.SLACK_APP_TOKEN)) {
    next.ORIRO_MANTIS_SLACK_APP_TOKEN = next.SLACK_APP_TOKEN;
  }
  if (
    !trimToValue(next.ORIRO_MANTIS_SLACK_APP_TOKEN) &&
    trimToValue(next.ORIRO_QA_SLACK_SUT_APP_TOKEN)
  ) {
    next.ORIRO_MANTIS_SLACK_APP_TOKEN = next.ORIRO_QA_SLACK_SUT_APP_TOKEN;
  }
  if (
    !trimToValue(next.ORIRO_MANTIS_SLACK_CHANNEL_ID) &&
    trimToValue(next.ORIRO_QA_SLACK_CHANNEL_ID)
  ) {
    next.ORIRO_MANTIS_SLACK_CHANNEL_ID = next.ORIRO_QA_SLACK_CHANNEL_ID;
  }
  return next;
}

function resolveSlackGatewayEnvPayload(env: NodeJS.ProcessEnv): SlackGatewayCredentialPayload {
  const channelId = trimToValue(env.ORIRO_QA_SLACK_CHANNEL_ID);
  const sutBotToken = trimToValue(env.ORIRO_QA_SLACK_SUT_BOT_TOKEN);
  const sutAppToken = trimToValue(env.ORIRO_QA_SLACK_SUT_APP_TOKEN);
  if (!channelId || !sutBotToken || !sutAppToken) {
    throw new Error(
      "Gateway setup requires ORIRO_QA_SLACK_CHANNEL_ID, ORIRO_QA_SLACK_SUT_BOT_TOKEN, and ORIRO_QA_SLACK_SUT_APP_TOKEN when using --credential-source env.",
    );
  }
  return {
    channelId,
...
async function prepareGatewayCredentialEnv(params: {
  ...
  if (
    trimToValue(params.env.ORIRO_MANTIS_SLACK_BOT_TOKEN) &&
    trimToValue(params.env.ORIRO_MANTIS_SLACK_APP_TOKEN)
  ) {
    return {};
  }
  ...
  params.env.ORIRO_MANTIS_SLACK_BOT_TOKEN = payload.sutBotToken;
  params.env.ORIRO_MANTIS_SLACK_APP_TOKEN = payload.sutAppToken;
  params.env.ORIRO_MANTIS_SLACK_CHANNEL_ID =
    trimToValue(params.env.ORIRO_MANTIS_SLACK_CHANNEL_ID) ?? payload.channelId;
  params.env.ORIRO_QA_SLACK_CHANNEL_ID =
    trimToValue(params.env.ORIRO_QA_SLACK_CHANNEL_ID) ?? payload.channelId;
  params.env.ORIRO_QA_SLACK_SUT_BOT_TOKEN =
    trimToValue(params.env.ORIRO_QA_SLACK_SUT_BOT_TOKEN) ?? payload.sutBotToken;
  params.env.ORIRO_QA_SLACK_SUT_APP_TOKEN =
    trimToValue(params.env.ORIRO_QA_SLACK_SUT_APP_TOKEN) ?? payload.sutAppToken;
  return {
    credentialLease,
    leaseHeartbeat,
  };
...
remote_command_timeout_seconds="\${ORIRO_MANTIS_REMOTE_COMMAND_TIMEOUT_SECONDS:-600}"
if [ -z "\${ORIRO_QA_SLACK_CHANNEL_ID:-}" ] && [ -n "$slack_channel_id" ]; then
  export ORIRO_QA_SLACK_CHANNEL_ID="$slack_channel_id"
fi
case "$remote_command_timeout_seconds" in
  ''|*[!0-9]*)
    echo "ORIRO_MANTIS_REMOTE_COMMAND_TIMEOUT_SECONDS must be an integer number of seconds." >&2
    exit 2
    ;;
esac
if [ "$remote_command_timeout_seconds" -le 0 ]; then
  echo "ORIRO_MANTIS_REMOTE_COMMAND_TIMEOUT_SECONDS must be greater than zero." >&2
  exit 2
fi
rm -rf "$out"
mkdir -p "$out"
export DISPLAY="\${DISPLAY:-:99}"
if [ -n "\${ORIRO_LIVE_OPENAI_KEY:-}" ] && [ -z "\${OPENAI_API_KEY:-}" ]; then
  export OPENAI_API_KEY="$ORIRO_LIVE_OPENAI_KEY"
fi
...
team_id="\${ORIRO_QA_SLACK_TEAM_ID:-}"
auth_test_token="\${ORIRO_QA_SLACK_SUT_BOT_TOKEN:-\${ORIRO_MANTIS_SLACK_BOT_TOKEN:-}}"
...
const token = process.env.ORIRO_QA_SLACK_SUT_BOT_TOKEN || process.env.ORIRO_MANTIS_SLACK_BOT_TOKEN;
...
if [ -z "$slack_url" ] && [ -n "$team_id" ] && [ -n "\${ORIRO_QA_SLACK_CHANNEL_ID:-}" ]; then
  slack_url="https://app.slack.com/client/$team_id/$ORIRO_QA_SLACK_CHANNEL_ID"
fi
profile="\${ORIRO_MANTIS_SLACK_BROWSER_PROFILE_DIR:-$HOME/.config/oriro-mantis/slack-chrome-profile}"
mkdir -p "$profile"
if [ "$setup_gateway" = "1" ]; then
  export SLACK_BOT_TOKEN="\${ORIRO_MANTIS_SLACK_BOT_TOKEN:-\${SLACK_BOT_TOKEN:-}}"
  export SLACK_APP_TOKEN="\${ORIRO_MANTIS_SLACK_APP_TOKEN:-\${SLACK_APP_TOKEN:-}}"
  if [ -z "$SLACK_BOT_TOKEN" ] || [ -z "$SLACK_APP_TOKEN" ]; then
    echo "Gateway setup requires ORIRO_MANTIS_SLACK_BOT_TOKEN and ORIRO_MANTIS_SLACK_APP_TOKEN." >&2
    exit 2
  fi
...
run_mantis_remote_body() {
  ...
  if [ "$setup_gateway" = "1" ]; then
    export ORIRO_HOME="$HOME/.oriro-mantis/slack-oriro"
    mkdir -p "$ORIRO_HOME"
...
MANTIS_SLACK_PATCH
    pnpm oriro config patch --file "$out/slack.socket.patch.json5" --dry-run
    pnpm oriro config patch --file "$out/slack.socket.patch.json5"
    nohup pnpm oriro gateway run --dev --allow-unconfigured --port 38973 --cli-backend-logs </dev/null >"$out/oriro-gateway.log" 2>&1 &
    gateway_pid="$!"
    echo "$gateway_pid" >"$out/oriro-gateway.pid"
    sleep 12
    if ! kill -0 "$gateway_pid" >/dev/null 2>&1; then
      echo "Oriro gateway exited during startup." >&2
    ...
    qa_args=(oriro qa slack --repo-root . --output-dir "$slack_qa_output_dir" --provider-mode "$provider_mode" --model "$primary_model" --alt-model "$alternate_model" --credential-source "$credential_source" --credential-role "$credential_role")
    if [ "$fast_mode" = "1" ]; then
      qa_args+=(--fast)
    fi
    if [ "$approval_checkpoints" = "1" ]; then
      ...
      export ORIRO_QA_SLACK_APPROVAL_CHECKPOINT_DIR="$checkpoint_dir"
      export ORIRO_QA_SLACK_APPROVAL_CHECKPOINT_TIMEOUT_MS="\${ORIRO_QA_SLACK_APPROVAL_CHECKPOINT_TIMEOUT_MS:-120000}"
      export ORIRO_MANTIS_APPROVAL_CHECKPOINT_SCENARIOS_JSON="$approval_checkpoint_scenarios_json"
      export ORIRO_MANTIS_APPROVAL_BROWSER_BIN="$browser_bin"
      cat >"$out/approval-checkpoint-watcher.mjs" <<'MANTIS_APPROVAL_WATCHER'
	import { spawn } from "node:child_process";
	import fs from "node:fs/promises";
	import path from "node:path";

const checkpointDir = process.env.ORIRO_QA_SLACK_APPROVAL_CHECKPOINT_DIR;
const timeoutMs = Number.parseInt(
  process.env.ORIRO_QA_SLACK_APPROVAL_CHECKPOINT_TIMEOUT_MS || "120000",
  10,
);
	const scenarioIds = JSON.parse(
	  process.env.ORIRO_MANTIS_APPROVAL_CHECKPOINT_SCENARIOS_JSON || "[]",
	);
	const browserBin = process.env.ORIRO_MANTIS_APPROVAL_BROWSER_BIN;

if (!checkpointDir) {
  throw new Error("ORIRO_QA_SLACK_APPROVAL_CHECKPOINT_DIR is required.");
}
if (!Number.isFinite(timeoutMs) || timeoutMs <= 0) {
  throw new Error("ORIRO_QA_SLACK_APPROVAL_CHECKPOINT_TIMEOUT_MS must be a positive integer.");
}
  ...
  throw new Error(\`Timed out waiting for approval checkpoint: \${filePath}\`);
	...
	function renderCheckpointHtml(checkpoint) {
	  ...
	  return '<!doctype html><html><head><meta charset="utf-8">' +
	    ...
	    '<div class="meta"><span class="name">oriro</span><span class="app">APP</span><span class="state">' +
	...
	async function captureScreenshot(screenshotPath, checkpoint) {
	  if (!browserBin) {
	    throw new Error("ORIRO_MANTIS_APPROVAL_BROWSER_BIN is required to render approval checkpoint screenshots.");
	  }
...
cat >"$out/remote-metadata.json" <<MANTIS_REMOTE_METADATA
{
  ...
  "gatewayAlive": $(if [ "$setup_gateway" = "1" ] && [ -f "$out/oriro-gateway.pid" ] && kill -0 "$(cat "$out/oriro-gateway.pid")" >/dev/null 2>&1; then echo true; else echo false; fi),
  "gatewayPid": "$(if [ -f "$out/oriro-gateway.pid" ]; then cat "$out/oriro-gateway.pid"; fi)",
  "gatewayPort": 38973,
...
export async function runMantisSlackDesktopSmoke(
  ...
  const slackChannelId =
    ...
    trimToValue(env.ORIRO_QA_SLACK_CHANNEL_ID) ??
  ...
  const remoteOutputDir = `/tmp/oriro-mantis-slack-desktop-${startedAt
    .toISOString()
  ...
  try {
    ...
    if (gatewaySetup && !gatewaySetupCompleted) {
      throw new Error("Slack desktop gateway setup did not report a live Oriro gateway.");
    }
