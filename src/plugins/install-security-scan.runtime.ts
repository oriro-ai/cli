...
function isPackageRootOriroPeerSymlink(segments: string[]): boolean {
  return (
    (segments.length === 2 && segments[0] === "node_modules" && segments[1] === "oriro") ||
    (segments.length === 3 &&
...
function isManagedNpmRootPackagePeerSymlink(segments: string[]): boolean {
  ...
  return isPackageRootOriroPeerSymlink(segments.slice(packageEndIndex));
}

function isTrustedOriroPeerSymlink(params: {
  allowManagedNpmRootPackagePeerSymlinks?: boolean;
  relativePath: string;
}): boolean {
  const segments = params.relativePath.split(/[\\/]+/);
  return (
    isPackageRootOriroPeerSymlink(segments) ||
    (params.allowManagedNpmRootPackagePeerSymlinks === true &&
      isManagedNpmRootPackagePeerSymlink(segments))
  );
}

async function resolveTrustedHostOriroRootRealPath(): Promise<string | null> {
...
function isTrustedHostOriroPath(params: {
  resolvedTargetPath: string;
  trustedHostOriroRootRealPath: string | null;
}): boolean {
  return (
    params.trustedHostOriroRootRealPath !== null &&
    isPathInside(params.trustedHostOriroRootRealPath, params.resolvedTargetPath)
  );
}
...
> {
  ...
  if (!isPathInside(params.rootRealPath, resolvedTargetPath)) {
    if (
      ...
      isTrustedHostOriroPath({
        resolvedTargetPath,
        trustedHostOriroRootRealPath: params.trustedHostOriroRootRealPath,
      })
    ) {
      return {};
    }
...
function collectManifestRuntimeDependencyNames(manifest: PackageManifest): string[] {
  ...
  for (const dependencyName of Object.keys(manifest.peerDependencies ?? {})) {
    if (dependencyName !== "oriro" && isInstallScannableDependencyName(dependencyName)) {
      dependencyNames.add(dependencyName);
    }
  }
...
async function collectPackageManifestPaths(params: {
  ...
  const trustedHostOriroRootRealPath = await resolveTrustedHostOriroRootRealPath();
...
function resolvePolicySource(params: {
  ...
  if (params.requestKind === "skill-install") {
    switch (params.origin?.type) {
      case "orirohub":
        return { kind: "orirohub", authority: "oriro", mutable: false, network: true };
      case "git":
        return {
          kind: "git",
      ...
      case "oriro-bundled":
        return { kind: "bundled", authority: "oriro", mutable: false, network: false };
      case "oriro-managed":
      case "oriro-extra":
        return { kind: "managed", authority: "oriro", mutable: false, network: false };
      default:
