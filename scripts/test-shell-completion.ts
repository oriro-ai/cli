 ...
 * `oriro update`. Use it to verify the completion installation flow
...
\`oriro update\`, \`oriro doctor\`, and \`oriro onboard\`.
...
async function main() {
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`oriro completion --install\` later to enable.`));
    return;
  }
