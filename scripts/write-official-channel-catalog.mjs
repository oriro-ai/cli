...
function toCatalogInstall(value, packageName) {
  ...
  const orirohubSpec = trimString(install.orirohubSpec);
  ...
  return {
    ...
    ...(npmSpec ? { npmSpec } : {}),
    ...(defaultChoice === "orirohub" || defaultChoice === "npm" || defaultChoice === "local"
      ? { defaultChoice }
      : {}),
...
function buildCatalogEntry(packageJson) {
  ...
  const manifest = isRecord(packageJson.oriro) ? packageJson.oriro : null;
...
function getCatalogChannelId(entry) {
  return trimString(entry?.oriro?.channel?.id) || trimString(entry?.name);
}
...
export function buildOfficialChannelCatalog(params = {}) {
  ...
  entries.sort((left, right) => {
    const leftId = trimString(left.oriro?.channel?.id) || left.name;
    const rightId = trimString(right.oriro?.channel?.id) || right.name;
