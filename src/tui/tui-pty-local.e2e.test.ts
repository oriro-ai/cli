  ...
  return {
    ...
    agents: {
      defaults: {
        ...
        models: {
          "tui-pty-mock/gpt-5.5": { agentRuntime: { id: "oriro" } },
        },
        skills: [],
        skipBootstrap: true,
      },
...
async function startLocalModeTui() {
  ...
  const tempDir = await mkdtemp(path.join(tmpdir(), "oriro-tui-pty-local-"));
  ...
  const script = [
    ...
    `program.parseAsync([process.execPath, "oriro", "tui", "--local"], { from: "node" }).catch((error) => {`,
    `  console.error(error);`,
    `  process.exit(1);`,
    `});`,
  ].join("\n");
