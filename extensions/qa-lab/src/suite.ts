...
import { disposeRegisteredAgentHarnesses } from "oriro/plugin-sdk/agent-harness";
...
} from "oriro/plugin-sdk/qa-runtime";
...
function shouldLogQaSuiteProgress(env: NodeJS.ProcessEnv = process.env) {
  const override = parseQaSuiteBooleanEnv(env.ORIRO_QA_SUITE_PROGRESS);
...
async function runScenario(name: string, steps: QaSuiteStep[]): Promise<QaSuiteScenarioResult> {
  ...
  for (const step of steps) {
    try {
      if (process.env.ORIRO_QA_DEBUG === "1") {
...
function buildRuntimeParityScenarioResult(params: {
  ...
  const oriroCell = params.result.cells.oriro;
  return {
    ...
    steps: [
      {
        name: oriroCell.runtime,
        status:
          oriroCell.runtimeErrorClass || oriroCell.transportErrorClass ? "fail" : "pass",
        details: formatRuntimeParityCellDetails(oriroCell),
      },
      {
...
function buildQaRuntimeEnvPatch(params: {
  ...
  if (params.forcedRuntime) {
    patch.ORIRO_BUILD_PRIVATE_QA = "1";
    patch.ORIRO_QA_FORCE_RUNTIME = params.forcedRuntime;
  }
  if (params.forcedRuntime !== "codex" || params.providerMode !== "mock-openai") {
  ...
  patch.ORIRO_CODEX_APP_SERVER_ARGS = `app-server -c openai_base_url=${mockBaseUrl}/v1 --listen stdio://`;
...
function shouldCaptureGatewayHeapCheckpoints(env: NodeJS.ProcessEnv = process.env) {
  return parseQaSuiteBooleanEnv(env.ORIRO_QA_GATEWAY_HEAP_CHECKPOINTS) === true;
}
...
export async function runQaFlowSuite(params?: QaSuiteRunParams): Promise<QaSuiteResult> {
  ...
  const enabledPluginIds = [
    ...new Set([
      ...
      ...(params?.forcedRuntime && params.forcedRuntime !== "oriro"
  ...
  const gateway = await startQaGatewayChild({
    ...
    mutateConfig: gatewayConfigPatch
      ? (cfg) => applyQaMergePatch(cfg, gatewayConfigPatch) as OriroConfig
      : undefined,
  ...
  try {
    ...
    const keepTemp = process.env.ORIRO_QA_KEEP_TEMP === "1" || false;
    await gateway.stop({
      keepTemp,
