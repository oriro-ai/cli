...
vi.mock("../config/paths.js", () => ({
  resolveStateDir: () => process.env.ORIRO_STATE_DIR ?? "/tmp/oriro-state",
}));
...
vi.mock("../agents/auth-profiles/profiles.js", async () => {
  ...
  const upsert = (params: { profileId: string; credential: unknown; agentDir?: string }) => {
    const stateDir = process.env.ORIRO_STATE_DIR ?? "/tmp/oriro-state";
...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under the default agent dir", async () => {
    const env = await setupAuthTestEnv("oriro-oauth-");
  ...
  it("writes OAuth credentials to all sibling agent dirs when syncSiblingAgents=true", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-oauth-sync-"));
    ...
    process.env.ORIRO_AGENT_DIR = kidAgentDir;
  ...
  it("writes OAuth credentials only to target dir by default", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-oauth-nosync-"));
    process.env.ORIRO_STATE_DIR = tempStateDir;
  ...
  it("syncs siblings from explicit agentDir outside ORIRO_STATE_DIR", async () => {
    tempStateDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-oauth-external-"));
    process.env.ORIRO_STATE_DIR = tempStateDir;

    // Create standard-layout agents tree *outside* ORIRO_STATE_DIR
...
describe("upsertApiKeyProfile secret refs", () => {
  ...
  it("handles plaintext, ref mode, and inline env-ref provider keys", async () => {
    const env = await setupAuthTestEnv("oriro-onboard-auth-credentials-");
    lifecycle.setStateDir(env.stateDir);
  ...
  it("stores provider-specific env refs and metadata in ref mode", async () => {
    const env = await setupAuthTestEnv("oriro-onboard-auth-credentials-provider-ref-");
    lifecycle.setStateDir(env.stateDir);
...
describe("upsertApiKeyProfile", () => {
  const lifecycle = createAuthTestLifecycle(["ORIRO_STATE_DIR", "ORIRO_AGENT_DIR"]);
  ...
  it("writes to the default agent dir", async () => {
    const env = await setupAuthTestEnv("oriro-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
    const defaultAgentDir = path.join(env.stateDir, "agents", "main", "agent");
