// Verifies OpenAI model selections route between Oriro and Codex runtimes.
...
describe("OpenAI runtime routing policy", () => {
  ...
  it("keeps explicit Oriro plus Codex auth profile under the unified OpenAI provider", () => {
    // OpenAI auth now stays canonical even when the runtime is not Codex.
    expect(
      listOpenAIAuthProfileProvidersForAgentRuntime({
        provider: "openai",
        harnessRuntime: "oriro",
      }),
    ).toEqual(["openai"]);
    expect(
      resolveOpenAIRuntimeProvider({
  ...
  it("keeps explicit OpenAI Oriro API-key auth order ahead of Codex backups", () => {
    const config = {
      auth: {
        order: {
  ...
  it("does not route custom OpenAI-compatible Oriro configs through Codex auth order", () => {
    const config = {
      models: {
        providers: {
          openai: {
