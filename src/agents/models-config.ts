...
import { planOriroModelsJson } from "./models-config.plan.js";
import {
...
export type PreparedOriroModelsJsonSource = ModelsJsonReadyResult & {
  fingerprint: string;
  workspaceDir?: string;
};

type EnsureOriroModelsJsonOptions = {
...
function resolveModelsConfigInput(config?: OriroConfig): {
  config: OriroConfig;
  sourceConfigForSecrets: OriroConfig;
} {
...
export async function prepareOriroModelsJsonSource(
  config?: OriroConfig,
  agentDirOverride?: string,
  options: EnsureOriroModelsJsonOptions = {},
): Promise<PreparedOriroModelsJsonSource> {
  const resolved = resolveModelsConfigInput(config);
  const cfg = resolved.config;
  ...
  const pending: Promise<ModelsJsonReadyState> = withModelsJsonWriteLock(targetPath, async () => {
    ...
    const plan = await planOriroModelsJson({
      cfg,
      sourceConfigForSecrets: resolved.sourceConfigForSecrets,
      agentDir,
      env,
...
/** Ensures models.json and plugin catalog sidecars are current for an agent. */
export async function ensureOriroModelsJson(
  config?: OriroConfig,
  agentDirOverride?: string,
  options: EnsureOriroModelsJsonOptions = {},
): Promise<ModelsJsonReadyResult> {
  const prepared = await prepareOriroModelsJsonSource(config, agentDirOverride, options);
