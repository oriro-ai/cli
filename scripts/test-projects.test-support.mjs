...
const TOOLING_ISOLATED_TEST_TARGET = "test/scripts/oriro-e2e-instance.test.ts";
...
const INCLUDE_FILE_ENV_KEY = "ORIRO_VITEST_INCLUDE_FILE";
const FS_MODULE_CACHE_PATH_ENV_KEY = "ORIRO_VITEST_FS_MODULE_CACHE_PATH";
...
const BROAD_CHANGED_FALLBACK_PATTERNS = [
  /^package\.json$/u,
  /^pnpm-lock\.yaml$/u,
  /^test\/setup(?:\.shared|\.extensions|-oriro-runtime)?\.ts$/u,
...
const GITHUB_WORKFLOW_OWNER_TEST_TARGETS = new Map([
  ...
  [
    ...
    [
      ...
      "test/scripts/oriro-cross-os-release-workflow.test.ts",
      "test/scripts/package-acceptance-workflow.test.ts",
    ],
  ],
  [
  ...
  [
    ".github/workflows/oriro-performance.yml",
    ["test/scripts/oriro-performance-workflow.test.ts"],
  ],
  [
    ".github/workflows/oriro-release-checks.yml",
    [
      "test/scripts/oriro-cross-os-release-checks.test.ts",
      "test/scripts/package-acceptance-workflow.test.ts",
      "test/scripts/plugin-prerelease-test-plan.test.ts",
  ...
  [
    ".github/workflows/oriro-scheduled-live-checks.yml",
    ["test/scripts/package-acceptance-workflow.test.ts"],
  ],
  [
    ".github/workflows/oriro-stable-main-closeout.yml",
    ["test/scripts/package-acceptance-workflow.test.ts"],
  ],
  [
  ...
  [
    ".github/workflows/plugin-orirohub-new.yml",
    ["test/scripts/package-acceptance-workflow.test.ts"],
  ],
  [
...
const TOOLING_SOURCE_TEST_TARGETS = new Map([
  ...
  ["scripts/orirotributors-map.json", ["test/scripts/update-orirotributors.test.ts"]],
  ...
  [
    "scripts/github/resolve-oriro-ref.sh",
    ["test/scripts/resolve-oriro-ref.test.ts"],
  ],
  ["scripts/ci-hydrate-testbox-env.sh", ["test/scripts/ci-hydrate-testbox-env.test.ts"]],
  [
    "scripts/github/run-oriro-cross-os-release-checks.sh",
    ["test/scripts/oriro-cross-os-release-workflow.test.ts"],
  ],
  ...
  ["scripts/lib/oriro-test-state.mjs", ["test/scripts/oriro-test-state.test.ts"]],
  [
  ...
  ["scripts/oriro-npm-prepublish-verify.ts", ["test/oriro-npm-prepublish-verify.test.ts"]],
  ["scripts/oriro-npm-postpublish-verify.ts", ["test/oriro-npm-postpublish-verify.test.ts"]],
  ...
  [
    "scripts/check-oriro-package-tarball.mjs",
    ["test/scripts/check-oriro-package-tarball.test.ts"],
  ],
  ...
  [
    ...
    ["test/scripts/docker-build-helper.test.ts", "test/scripts/oriro-test-state.test.ts"],
  ],
  [
  ...
  ["scripts/e2e/lib/skills/orirohub-install-proof.sh", ["test/scripts/e2e-shell-tempfiles.test.ts"]],
  [
  ...
  [
    "scripts/oriro-performance-source-summary.mjs",
    ["test/scripts/oriro-performance-source-summary.test.ts"],
  ],
...
const TOOLING_TEST_TARGETS = new Map([
  ...
  ["test/scripts/oriro-test-state.test.ts", ["test/scripts/oriro-test-state.test.ts"]],
...
const SOURCE_TEST_TARGETS = new Map([
  ...
  ["src/test-utils/oriro-test-state.ts", ["src/test-utils/oriro-test-state.test.ts"]],
  [
...
const BROAD_CHANGED_ENV_KEY = "ORIRO_TEST_CHANGED_BROAD";
const VITEST_NO_OUTPUT_TIMEOUT_ENV_KEY = "ORIRO_VITEST_NO_OUTPUT_TIMEOUT_MS";
const VITEST_NO_OUTPUT_HEARTBEAT_ENV_KEY = "ORIRO_VITEST_NO_OUTPUT_HEARTBEAT_MS";
const VITEST_NO_OUTPUT_RETRY_ENV_KEY = "ORIRO_VITEST_NO_OUTPUT_RETRY";
...
export function buildFullSuiteVitestRunPlans(args, cwd = process.cwd()) {
  ...
  const parallelShardCount = parsePositiveInt(
    process.env.ORIRO_TEST_PROJECTS_PARALLEL,
    "ORIRO_TEST_PROJECTS_PARALLEL",
  );
...
export function shouldUseLocalFullSuiteParallelByDefault(env = process.env) {
  ...
  return (
    env.ORIRO_TEST_PROJECTS_SERIAL !== "1" && env.CI !== "true" && env.GITHUB_ACTIONS !== "true"
  );
}
...
function hasConservativeVitestWorkerBudget(env) {
  const workerBudget = parsePositiveInt(
    env.ORIRO_VITEST_MAX_WORKERS ?? env.ORIRO_TEST_WORKERS,
    env.ORIRO_VITEST_MAX_WORKERS === undefined
      ? "ORIRO_TEST_WORKERS"
...
export function resolveParallelFullSuiteConcurrency(specCount, envInput, hostInfo) {
  ...
  const override = parsePositiveInt(
    env.ORIRO_TEST_PROJECTS_PARALLEL,
    "ORIRO_TEST_PROJECTS_PARALLEL",
  );
  if (override !== null) {
    return Math.min(override, specCount);
  }
  if (env.ORIRO_TEST_PROJECTS_SERIAL === "1") {
    return 1;
  }
  if (isCiLikeEnv(env)) {
    return 1;
  ...
  if (
    env.ORIRO_TEST_PROJECTS_LEAF_SHARDS !== "1" &&
  ...
  return plans.map((plan, index) => {
    const includeFilePath = plan.includePatterns
      ? path.join(
          ...
          `oriro-vitest-include-${randomUUID()}-${index}.json`,
        )
      : null;
    return {
      config: plan.config,
...
export function shouldAcquireLocalHeavyCheckLock(runSpecs, env = process.env) {
  if (env.ORIRO_TEST_HEAVY_CHECK_LOCK_HELD === "1") {
    return false;
  }

  if (env.ORIRO_TEST_PROJECTS_FORCE_LOCK === "1") {
    return true;
  }
