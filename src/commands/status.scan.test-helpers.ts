...
export function createStatusScanSharedMocks(configPathLabel: string): StatusScanSharedMocks {
  return {
    resolveConfigPath: vi.fn(() => `/tmp/oriro-${configPathLabel}-missing-${process.pid}.json`),
...
export function createStatusScanConfig<T extends object = OriroConfig>(
  overrides: T = {} as T,
): OriroConfig & T {
  return {
    session: {},
...
export function createStatusMemorySearchConfig(): OriroConfig {
