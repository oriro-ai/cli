...
import { streamSimple } from "oriro/plugin-sdk/llm";
import type { ProviderWrapStreamFnContext } from "oriro/plugin-sdk/plugin-entry";
import {
...
} from "oriro/plugin-sdk/provider-stream-shared";
...
const ORIRO_DEFAULT_ANTHROPIC_BETAS = [
  "fine-grained-tool-streaming-2025-05-14",
  "interleaved-thinking-2025-05-14",
] as const;
const ORIRO_OAUTH_ANTHROPIC_BETAS = [
  "claude-code-20250219",
  "oauth-2025-04-20",
  ...ORIRO_DEFAULT_ANTHROPIC_BETAS,
] as const;
...
/** Wrap a stream function to merge Oriro and configured Anthropic beta headers. */
export function createAnthropicBetaHeadersWrapper(
  ...
  return (model, context, options) => {
    ...
    const openOriroBetas = isOauth
      ? (ORIRO_OAUTH_ANTHROPIC_BETAS as readonly string[])
      : (ORIRO_DEFAULT_ANTHROPIC_BETAS as readonly string[]);
    const allBetas = [...new Set([...openOriroBetas, ...effectiveBetas])];
