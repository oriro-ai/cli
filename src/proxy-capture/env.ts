...
export const ORIRO_DEBUG_PROXY_ENABLED = "ORIRO_DEBUG_PROXY_ENABLED";
export const ORIRO_DEBUG_PROXY_URL = "ORIRO_DEBUG_PROXY_URL";
/** @deprecated Capture storage now lives in the shared state database. */
export const ORIRO_DEBUG_PROXY_DB_PATH = "ORIRO_DEBUG_PROXY_DB_PATH";
/** @deprecated Capture payloads now live in the shared state database. */
export const ORIRO_DEBUG_PROXY_BLOB_DIR = "ORIRO_DEBUG_PROXY_BLOB_DIR";
export const ORIRO_DEBUG_PROXY_CERT_DIR = "ORIRO_DEBUG_PROXY_CERT_DIR";
export const ORIRO_DEBUG_PROXY_SESSION_ID = "ORIRO_DEBUG_PROXY_SESSION_ID";
export const ORIRO_DEBUG_PROXY_REQUIRE = "ORIRO_DEBUG_PROXY_REQUIRE";
...
export function resolveDebugProxySettings(
  ...
  const enabled = isTruthy(env[ORIRO_DEBUG_PROXY_ENABLED]);
  const explicitSessionId = env[ORIRO_DEBUG_PROXY_SESSION_ID]?.trim() || undefined;
  ...
  return {
    enabled,
    required: isTruthy(env[ORIRO_DEBUG_PROXY_REQUIRE]),
    proxyUrl: env[ORIRO_DEBUG_PROXY_URL]?.trim() || undefined,
    dbPath: env[ORIRO_DEBUG_PROXY_DB_PATH]?.trim() || resolveDebugProxyDbPath(env),
    blobDir: env[ORIRO_DEBUG_PROXY_BLOB_DIR]?.trim() || resolveDebugProxyBlobDir(env),
    certDir: env[ORIRO_DEBUG_PROXY_CERT_DIR]?.trim() || resolveDebugProxyCertDir(env),
    sessionId,
    sourceProcess: "oriro",
  };
}
...
export function applyDebugProxyEnv(
  ...
  const baseEnv = { ...env };
  delete baseEnv.ORIRO_DEBUG_PROXY_DB_PATH;
  delete baseEnv.ORIRO_DEBUG_PROXY_BLOB_DIR;
  return {
    ...baseEnv,
    [ORIRO_DEBUG_PROXY_ENABLED]: "1",
    [ORIRO_DEBUG_PROXY_REQUIRE]: "1",
    [ORIRO_DEBUG_PROXY_URL]: params.proxyUrl,
    [ORIRO_DEBUG_PROXY_CERT_DIR]: params.certDir ?? resolveDebugProxyCertDir(env),
    [ORIRO_DEBUG_PROXY_SESSION_ID]: params.sessionId,
