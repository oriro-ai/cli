...
function makeTempDir(): string {
  const root = mkdtempSync(path.join(tmpdir(), "oriro-ios-log-pull-"));
  tempDirs.push(root);
  return root;
}
...
function runWithFakeXcrun(
  ...
  return spawnSync("bash", [scriptPath, "device-udid", "ai.oriro.ios.dev", destPath], {
    cwd: process.cwd(),
    encoding: "utf8",
    env: {
      ...process.env,
...
describe("scripts/dev/ios-pull-gateway-log.sh", () => {
  it("fails when the copied gateway log is empty", () => {
    ...
    const destPath = path.join(root, "oriro-gateway.log");
