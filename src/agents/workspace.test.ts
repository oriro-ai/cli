...
let testState: OriroTestState | undefined;

beforeEach(async () => {
  testState = await createOriroTestState({
    layout: "state-only",
    prefix: "oriro-workspace-state-",
  });
});
...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses ORIRO_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      ORIRO_HOME: "/srv/oriro-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/oriro-home"), ".oriro", "workspace"));
  });

  it("prefers ORIRO_WORKSPACE_DIR for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      ORIRO_WORKSPACE_DIR: "/srv/oriro-workspace",
      ORIRO_HOME: "/srv/oriro-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.resolve("/srv/oriro-workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = ["oriro-workspace-state.json"] as const;
const LEGACY_WORKSPACE_STATE_PATH_SEGMENTS = [".oriro", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("oriro-workspace-");
  ...
  it("does not overwrite a foreign root workspace-state.json file", async () => {
    ...
    const foreignState = "not oriro state\n";
  ...
  it("refuses to re-seed a recently attested workspace after only generated git metadata survives", async () => {
    ...
    await fs.mkdir(path.join(tempDir, ".oriro"), { recursive: true });
  ...
  it("does not overwrite a sibling file that is not an Oriro attestation marker", async () => {
    const tempDir = await makeTempWorkspace("oriro-workspace-");
...
describe("loadWorkspaceBootstrapFiles", () => {
  ...
  it("treats hardlinked bootstrap aliases as missing", async () => {
    ...
    const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-workspace-hardlink-"));
    try {
