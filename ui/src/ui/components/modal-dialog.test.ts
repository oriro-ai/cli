...
import type { OriroModalDialog } from "./modal-dialog.ts";
...
function expectShadowElement(modal: OriroModalDialog, id: string): HTMLElement {
...
describe("oriro-modal-dialog", () => {
  ...
  it("opens a labelled modal dialog with an optional description", async () => {
    ...
    expect(labelId).toBe("oriro-modal-dialog-label");
    expect(descriptionId).toBe("oriro-modal-dialog-description");
    expect(dialog.getRootNode()).toBe(modal.shadowRoot);
    expect(dialog.ownerDocument.querySelector(`#${labelId}`)).toBeNull();
    expect(expectShadowElement(modal, "oriro-modal-dialog-label").textContent).toBe(
      "Confirm action",
    );
    expect(expectShadowElement(modal, "oriro-modal-dialog-description").textContent).toBe(
