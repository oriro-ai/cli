...
describe("isGatewayArgv", () => {
  ...
  it("matches the oriro executable but gates the gateway binary behind the opt-in flag", () => {
    expect(isGatewayArgv(["C:\\bin\\oriro.cmd", "gateway"])).toBe(true);
    expect(isGatewayArgv(["/usr/local/bin/oriro-gateway", "gateway"])).toBe(false);
    expect(
      isGatewayArgv(["/usr/local/bin/oriro-gateway", "gateway"], {
        allowGatewayBinary: true,
      }),
    ).toBe(true);
    expect(
      isGatewayArgv(["C:\\bin\\oriro-gateway.EXE", "gateway"], {
        allowGatewayBinary: true,
      }),
    ).toBe(true);
  });
  ...
  it("rejects unknown gateway argv even when the token is present", () => {
    expect(isGatewayArgv(["node", "/srv/oriro/custom.js", "gateway"])).toBe(false);
