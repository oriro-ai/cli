...
```bash
ORIRO_DIAGNOSTICS=telegram.http,telegram.payload
```
...
`ORIRO_DIAGNOSTICS=0` is a process-level disable override: it disables
...
```bash
ORIRO_DIAGNOSTICS=profiler oriro gateway run
```

Enable only reply-dispatch profiler spans:

```bash
ORIRO_DIAGNOSTICS=reply.profiler oriro gateway run
```

Enable only Codex app-server startup/tool/thread profiler spans:

```bash
ORIRO_DIAGNOSTICS=codex.profiler oriro gateway run
```
...
```bash
ORIRO_DIAGNOSTICS=0 oriro gateway run
```
...
ORIRO_DIAGNOSTICS_TIMELINE_PATH=/tmp/oriro-timeline.jsonl \
oriro gateway run
```
...
`ORIRO_DIAGNOSTICS_TIMELINE_PATH`. When `timeline` is enabled only from
config, the earliest config-loading spans are not emitted because Oriro has
not read config yet; subsequent startup spans use the config flag.

`ORIRO_DIAGNOSTICS=1`, `ORIRO_DIAGNOSTICS=all`, and
`ORIRO_DIAGNOSTICS=*` also enable the timeline because they enable every
diagnostics flag. Prefer `timeline` when you only want the JSONL timing
artifact.

Timeline records use the `oriro.diagnostics.v1` envelope. Events can include
...
```bash
ls -t /tmp/oriro-ai/cli-*.log | head -n 1
```

Filter for Telegram HTTP diagnostics:

```bash
rg "telegram http error" /tmp/oriro-ai/cli-*.log
```

Filter for Brave Search HTTP diagnostics:

```bash
rg "brave http" /tmp/oriro-ai/cli-*.log
```

Or tail while reproducing:

```bash
tail -f /tmp/oriro-ai/cli-$(date +%F).log | rg "telegram http error"
```

For remote gateways, you can also use `oriro logs --follow` (see [/cli/logs](/cli/logs)).
