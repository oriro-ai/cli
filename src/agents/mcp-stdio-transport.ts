/**
 * Oriro stdio transport wrapper for MCP server subprocesses.
 */
...
type OriroStdioServerParameters = {
  command: string;
  args?: string[];
...
export class OriroStdioClientTransport implements Transport {
  onclose?: () => void;
  ...
  constructor(private readonly serverParams: OriroStdioServerParameters) {
  ...
  async start(): Promise<void> {
    if (this.process) {
      throw new Error(
        "OriroStdioClientTransport already started; Client.connect() starts transports automatically.",
      );
    }
