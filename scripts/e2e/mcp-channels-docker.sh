...
IMAGE_NAME="$(docker_e2e_resolve_image "oriro-mcp-channels-e2e" ORIRO_IMAGE)"
PORT="18789"
TOKEN="mcp-e2e-$(date +%s)-$$"
CONTAINER_NAME="oriro-mcp-e2e-$$"
CLIENT_LOG="$(mktemp -t oriro-mcp-client-log.XXXXXX)"
...
ORIRO_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 mcp-channels empty)"
...
docker_e2e_run_with_harness \
  ...
  bash -lc "set -euo pipefail
    ...
    mock_pid=\"\$(oriro_e2e_start_mock_openai \"\$mock_port\" /tmp/mcp-channels-mock-openai.log)\"
    gateway_pid=
    cleanup_inner() {
    ...
    gateway_pid=\"\$(oriro_e2e_start_gateway \"\$entry\" $PORT /tmp/mcp-channels-gateway.log)\"
    oriro_e2e_wait_gateway_ready \"\$gateway_pid\" /tmp/mcp-channels-gateway.log 480 $PORT
