...
read_when:
  - Running Oriro Gateway in WSL2 while Chrome lives on Windows
...
In the common split-host setup, Oriro Gateway runs inside WSL2, Chrome runs on Windows, and browser control must cross the WSL2 and Windows boundary. The layered failure pattern from [issue #39369](https://github.com/oriro-ai/cli/issues/39369) means several independent problems can show up at once, which makes the wrong layer look broken first.
...
- Oriro and Chrome are on the same machine
...
- Oriro points a browser profile at the address that is reachable from WSL2
...
If this fails on Windows, Oriro is not the problem yet.
...
Fix that before touching Oriro config.

### Layer 3: Configure the correct browser profile

For raw remote CDP, point Oriro at the address that is reachable from WSL2:
...
- use HTTP(S) when you want Oriro to discover `/json/version`
- use WS(S) only when the browser provider gives you a direct DevTools socket URL
- test the same URL with `curl` before expecting Oriro to succeed
...
```bash
oriro browser open https://example.com --browser-profile remote
oriro browser tabs --browser-profile remote
```

Good result:

- the tab opens in Windows Chrome
- `oriro browser tabs` returns the target
...
- stale viewport / dark-mode / locale / offline overrides after a remote session
  - run `oriro browser stop --browser-profile remote`
...
3. Oriro config: does `browser.profiles.<name>.cdpUrl` use that exact WSL2-reachable address?
...
- only then debug Oriro config or Control UI auth
