...
function createGatewayCliStartupTrace() {
  const enabled = isTruthyEnvValue(process.env.ORIRO_GATEWAY_STARTUP_TRACE);
...
async function maybeLogPendingControlUiBuild(cfg: OriroConfig): Promise<void> {
...
function getGatewayStartGuardErrors(params: {
  ...
  if (!params.configExists) {
    return [
      `Missing config. Run \`${formatCliCommand("oriro setup")}\` or set gateway.mode=local (or pass --allow-unconfigured).`,
    ];
  }
  if (params.mode === undefined) {
    return [
      [
        ...
        `Re-run \`${formatCliCommand("oriro onboard --mode local")}\` or \`${formatCliCommand("oriro setup")}\`, set gateway.mode=local manually, or pass --allow-unconfigured.`,
      ].join(" "),
...
async function readGatewayStartupConfig(params: {
  ...
  let blockedRecoveryConfig: OriroConfig | null = null;
...
export async function runGatewayCommand(opts: GatewayRunOpts, hooks: GatewayRunRuntimeHooks = {}) {
  ...
  const isDevProfile = normalizeOptionalLowercaseString(process.env.ORIRO_PROFILE) === "dev";
  ...
  if (opts.cliBackendLogs || opts.claudeCliLogs) {
    ...
    process.env.ORIRO_CLI_BACKEND_LOG_OUTPUT = "1";
  }
  ...
  if (opts.rawStream) {
    process.env.ORIRO_RAW_STREAM = "1";
  }
  const rawStreamPath = toOptionString(opts.rawStreamPath);
  if (rawStreamPath) {
    process.env.ORIRO_RAW_STREAM_PATH = rawStreamPath;
  }
  ...
  if (snapshot) {
    ...
    const finalConfigEnteredServiceMode = Boolean(process.env.ORIRO_SERVICE_MARKER?.trim());
  ...
  if (process.env.ORIRO_SERVICE_MARKER?.trim()) {
  ...
  if (opts.force) {
    try {
      ...
      defaultRuntime.error(
        `Could not free port ${port}: ${formatErrorMessage(err)}. Run ${formatCliCommand("oriro gateway status --deep")} to inspect the listener.`,
      );
      defaultRuntime.exit(1);
      return;
    }
  ...
  if (resolvedAuthMode === "password" && !passwordConfigured) {
    defaultRuntime.error(
      [
        ...
        "Set gateway.auth.password (or ORIRO_GATEWAY_PASSWORD), or pass --password.",
        ...authHints,
      ]
        .filter(Boolean)
        .join("\n"),
      ...
      [
        ...
        ...(isContainerEnvironment()
          ? [
              ...
              "Set ORIRO_GATEWAY_TOKEN or ORIRO_GATEWAY_PASSWORD, or pass --token/--password to start with auth.",
            ]
          : [
              "Set gateway.auth.token/password (or ORIRO_GATEWAY_TOKEN/ORIRO_GATEWAY_PASSWORD) or pass --token/--password.",
            ]),
        ...authHints,
      ]
        .filter(Boolean)
  ...
  const deferStartupSidecars =
    isTruthyEnvValue(process.env.ORIRO_SKIP_CHANNELS) ||
    isTruthyEnvValue(process.env.ORIRO_SKIP_PROVIDERS);
  ...
  try {
    ...
    if (isGatewayLockError(err)) {
      ...
      defaultRuntime.error(
        `Gateway failed to start: ${errMessage}\nIf the gateway is supervised, stop it with: ${formatCliCommand("oriro gateway stop")}`,
      );
      try {
    ...
    defaultRuntime.error(
      `Gateway failed to start: ${formatErrorMessage(err)}. Run ${formatCliCommand("oriro gateway status --deep")} for diagnostics.`,
    );
    defaultRuntime.exit(1);
  }
}
