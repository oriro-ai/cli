...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["oriroPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["oriro", "pollId"]) ??
    readNestedString(value, ["oriro", "poll", "id"]) ??
    readNestedString(value, ["data", "oriroPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "oriro", "pollId"]) ??
    // Action.Execute (Universal Action Model) payload shape: value.action.data
    readNestedString(value, ["action", "data", "oriroPollId"]) ??
