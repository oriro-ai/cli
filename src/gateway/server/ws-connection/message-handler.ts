...
/**
 * Lazily resolve the local node host's nodeId from ~/.oriro/node.json.
 * Process-stable: only changes on `oriro node install`, which requires restart.
 */
...
function resolveTrustedProxyControlUiScopes(params: {
  ...
  const rawHeader = firstHeaderValue(params.upgradeReq.headers["x-oriro-scopes"]);
  ...
  const handleMessage = async (data: RawData) => {
    ...
    try {
      ...
      if (!client) {
        ...
        // matching instanceId against ~/.oriro/node.json nodeId). SSH-tunneled remote
