 ...
 * returning control to the caller (typically `triggerOriroRestart` →
...
function parsePidsFromLsofOutput(
  ...
  for (const entry of parseLsofEntries(stdout)) {
    ...
    if (entry.cmd && normalizeLowercaseStringOrEmpty(entry.cmd).includes("oriro")) {
      pids.push(entry.pid);
      continue;
    }
...
/**
 * Windows: find listening PIDs on the port, then verify each is an oriro
...
function findGatewayPidsOnPortWithProtectedPidSync(
  ...
  if (process.platform === "win32") {
    ...
    // command-line verification to find only oriro gateway processes.
 ...
 * Returns only PIDs that belong to oriro gateway processes (not the current process).
 */
