...
function createRelayApnsSendFixture(params: {
  ...
  return {
    ...
    relayConfig: {
      baseUrl: "https://relay.oriro.test",
      timeoutMs: 2_500,
    },
    gatewayIdentity: {
...
describe("push APNs send semantics", () => {
  ...
  it("sends alert pushes with alert headers and payload", async () => {
    ...
    const oriroPayload = requireRecord(payload.oriro, "oriro payload");
    expectRecordFields(oriroPayload, {
      kind: "push.test",
      nodeId: "ios-node-alert",
    });
    expect(typeof oriroPayload.ts).toBe("number");
  ...
  it("routes direct APNs HTTP/2 requests through the active managed proxy", async () => {
    ...
    try {
      ...
      expect(request?.headers["apns-topic"]).toBe("ai.oriro.ios");
  ...
  it("sends exec approval alert pushes with generic modal-only metadata", async () => {
    ...
    expect(payload.aps).toEqual({
      alert: {
        ...
        body: "Open Oriro to review this request.",
      },
      sound: "default",
      category: "oriro.exec-approval",
      "content-available": 1,
    });
    const oriroPayload = requireRecord(payload.oriro, "oriro payload");
    expectRecordFields(oriroPayload, {
    ...
    expect(typeof oriroPayload.ts).toBe("number");
    expectNoProperties(oriroPayload, [
      "host",
      "nodeId",
      "agentId",
      "commandText",
  ...
  it("defaults background wake reason when not provided", async () => {
    ...
    expectRecordFields(requireRecord(payload.oriro, "oriro payload"), {
      kind: "node.wake",
      reason: "node.invoke",
      nodeId: "ios-node-wake-default-reason",
    });
