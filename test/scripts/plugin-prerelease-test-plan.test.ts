...
describe("scripts/lib/plugin-prerelease-test-plan.mjs", () => {
  ...
  it("uses kitchen-sink npm and OriroHub scenarios as the registry install canary", () => {
    ...
    expect(lane).toEqual({
      command: "ORIRO_SKIP_DOCKER_BUILD=1 pnpm test:docker:kitchen-sink-plugin",
    ...
    expect(script).toContain("npm:@oriro/kitchen-sink@latest");
    expect(script).toContain("npm-latest-conformance");
    expect(script).toContain("npm-latest-adversarial");
    expect(script).toContain("npm:@oriro/kitchen-sink@beta");
    expect(script).toContain("orirohub:@oriro/kitchen-sink@latest");
    expect(script).toContain("orirohub:@oriro/kitchen-sink@beta");
    expect(script).toContain("ORIRO_KITCHEN_SINK_PLUGIN_MAX_MEMORY_MIB");
    expect(script).toContain(
      "npm-to-orirohub|orirohub:@oriro/kitchen-sink@latest|oriro-kitchen-sink-fixture|orirohub|success|basic||${KITCHEN_SINK_NPM_SPEC}",
    );
    ...
    expect(sweepScript).toContain("ORIRO_KITCHEN_SINK_PERSONALITY");
    ...
    expect(assertionsScript).toContain("record.orirohubPackage !== packageName");
    expect(assertionsScript).toContain("record.oriropackSha256");
    expect(assertionsScript).toContain("record.artifactKind");
    expect(assertionsScript).toContain("record.npmIntegrity");
    expect(assertionsScript).toContain("assertOriroHubExternalInstallContract");
    ...
    expect(readFileSync("scripts/e2e/lib/orirohub-fixture-server.cjs", "utf8")).toContain(
      'from "oriro/plugin-sdk/plugin-entry"',
    );
    expect(readFileSync("scripts/e2e/lib/orirohub-fixture-server.cjs", "utf8")).toContain(
      "X-OriroHub-Artifact-Sha256",
    );
  ...
  it("keeps kitchen-sink RPC coverage package-backed and resource-guarded", () => {
    ...
    expect(script).toContain("ORIRO_ENTRY=/app/oriro.mjs");
    expect(script).toContain("ORIRO_KITCHEN_SINK_COMMAND_MAX_RSS_MIB");
    expect(script).toContain("docker_e2e_sample_stats_until_exit");
    expect(script).toContain("scripts/e2e/lib/docker-stats/assert-resource-ceiling.mjs");
    ...
    expect(walkScript).toContain("usesBuiltOriroEntry(runner)");
  ...
  it("keeps the generic plugin Docker lane as an external install contract canary", () => {
    ...
    const orirohubScript = readFileSync("scripts/e2e/lib/plugins/orirohub.sh", "utf8");
    const assertionsScript = readFileSync("scripts/e2e/lib/plugins/assertions.mjs", "utf8");
    const fixtureServer = readFileSync("scripts/e2e/lib/orirohub-fixture-server.cjs", "utf8");
    const prereleasePlan = createPluginPrereleaseTestPlan();

    expect(lane).toEqual({
      command: "ORIRO_SKIP_DOCKER_BUILD=1 pnpm test:docker:plugins",
      e2eImageKind: "functional",
      live: false,
      name: "plugins",
    ...
    expect(sweepScript).toContain("run_plugins_orirohub_scenario");
    expect(orirohubScript).toContain('plugins install "$ORIROHUB_PLUGIN_SPEC"');
    expect(assertionsScript).toContain("assertOriroHubExternalInstallContract");
    expect(assertionsScript).toContain('node_modules", "oriro');
    expect(fixtureServer).toContain('"is-number": "7.0.0"');
    expect(fixtureServer).toContain('oriro: ">=2026.4.11"');
  ...
  it("wires the full plugin prerelease plan into its release workflow", () => {
    ...
    expect(manifestEnv).toEqual({
      ORIRO_CI_CHECKOUT_REVISION: "${{ steps.checkout_ref.outputs.sha }}",
      ORIRO_CI_DOCS_CHANGED:
      ...
      ORIRO_CI_EVENT_NAME: "${{ github.event_name }}",
      ORIRO_CI_REPOSITORY: "${{ github.repository }}",
      ORIRO_CI_RUN_ANDROID:
    ...
    expect(inspectorRun.env).toEqual({
      ORIRO_PLUGIN_INSPECTOR_ROOT: ".artifacts/plugin-inspector",
      ORIRO_PLUGIN_INSPECTOR_VERSION: "0.3.10",
    });
    expect(inspectorRun.run).toContain("extensions/");
    expect(inspectorRun.run).toContain(
      'npm exec --yes "@oriro/plugin-inspector@${ORIRO_PLUGIN_INSPECTOR_VERSION}" -- ci',
    );
  ...
  it("keeps release-check reruns independent while cancelling superseded umbrella runs", () => {
    const releaseChecksWorkflow = parse(
      readFileSync(".github/workflows/oriro-release-checks.yml", "utf8"),
    );
    const fullReleaseWorkflow = readFullReleaseValidationWorkflow();

    expect(releaseChecksWorkflow.concurrency).toEqual({
      group:
        "oriro-release-checks-${{ inputs.expected_sha || inputs.ref }}-${{ inputs.rerun_group }}",
      "cancel-in-progress": "${{ startsWith(github.ref, 'refs/heads/tideoriro/alpha/') }}",
    });
    expect(fullReleaseWorkflow.concurrency).toEqual({
      group: "full-release-validation-${{ inputs.ref }}-${{ inputs.rerun_group }}",
      "cancel-in-progress":
        "${{ (inputs.ref == 'main' && inputs.rerun_group == 'all') || startsWith(inputs.ref, 'tideoriro/alpha/') }}",
    });
    ...
    expect(dockerPreflightStep?.run).toContain(
      '--build-arg ORIRO_EXTENSIONS="diagnostics-otel,codex"',
    );
    expect(
    ...
    expect(runtimeToolCoverage.steps).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          ...
          run: expect.stringContaining("pnpm oriro qa coverage"),
        }),
      ]),
    );
    expect(runtimeToolCoverage.steps).toEqual(
  ...
  it("keeps the live-ish availability check redacted", () => {
    ...
    expect(output).toContain("channel-discord: present (DISCORD_TOKEN, ORIRO_DISCORD_TOKEN)");
