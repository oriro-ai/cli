...
import { normalizeModelCatalog } from "@oriro/model-catalog-core/model-catalog-normalize";
...
const ORIRO_PROVIDER_INDEX_VERSION = 1;
...
function normalizeInstall(value: unknown): OriroProviderIndexPluginInstall | undefined {
  if (!isRecord(value)) {
    return undefined;
  }
  const orirohubSpec = normalizeOptionalString(value.orirohubSpec);
  const parsedOriroHub = orirohubSpec ? parseOriroHubPluginSpec(orirohubSpec) : null;
  ...
  if (!parsedOriroHub && !parsedNpm) {
    return undefined;
  }
  const defaultChoice =
    value.defaultChoice === "orirohub" && parsedOriroHub
      ? "orirohub"
...
function normalizePlugin(value: unknown): OriroProviderIndexPlugin | undefined {
  if (!isRecord(value)) {
    return undefined;
  }
...
function normalizeOnboardingScopes(
  value: unknown,
): OriroProviderIndexProviderAuthChoice["onboardingScopes"] | undefined {
...
function normalizeAssistantVisibility(
  value: unknown,
): OriroProviderIndexProviderAuthChoice["assistantVisibility"] | undefined {
...
function normalizeAuthChoice(params: {
  ...
  providerName: string;
  value: unknown;
}): OriroProviderIndexProviderAuthChoice | undefined {
...
function normalizeAuthChoices(params: {
  ...
  providerName: string;
  value: unknown;
}): readonly OriroProviderIndexProviderAuthChoice[] | undefined {
  if (!Array.isArray(params.value)) {
    return undefined;
  }
  const choices = params.value
    .map((value) => normalizeAuthChoice({ ...params, value }))
    .filter((choice): choice is OriroProviderIndexProviderAuthChoice => Boolean(choice));
...
function normalizeProvider(
  ...
  value: unknown,
): OriroProviderIndexProvider | undefined {
  if (!isRecord(value)) {
    return undefined;
  }
...
export function normalizeOriroProviderIndex(value: unknown): OriroProviderIndex | undefined {
  if (!isRecord(value) || value.version !== ORIRO_PROVIDER_INDEX_VERSION) {
    return undefined;
  }
  if (!isRecord(value.providers)) {
    return undefined;
  }
  const providers: Record<string, OriroProviderIndexProvider> = {};
  ...
  return {
    version: ORIRO_PROVIDER_INDEX_VERSION,
