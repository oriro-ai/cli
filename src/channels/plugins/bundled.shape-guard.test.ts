...
function restoreBundledPluginsDir(previousBundledPluginsDir: string | undefined) {
  if (previousBundledPluginsDir === undefined) {
    ...
    process.env.ORIRO_BUNDLED_PLUGINS_DIR = previousBundledPluginsDir;
  }
}
  ...
  return fs
    ...
    .filter(
      (entryPath) =>
        ...
        fs.existsSync(path.join(entryPath, "oriro.plugin.json")),
    );
}
...
function listGitSourceBundledPluginRoots(extensionsDir: string): string[] | null {
  const result = spawnSync(
    "git",
    ["ls-files", "--", "extensions/*/package.json", "extensions/*/oriro.plugin.json"],
    {
      cwd: process.cwd(),
      encoding: "utf8",
...
afterEach(() => {
  delete (globalThis as { __oriroBundledChannelReenter?: () => void })[
    "__oriroBundledChannelReenter"
  ];
  vi.resetModules();
...
describe("bundled channel entry shape guards", () => {
  ...
  it("fills sparse bundled channel plugin metadata from package metadata", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-bundled-metadata-"));
    const previousBundledPluginsDir = process.env.ORIRO_BUNDLED_PLUGINS_DIR;
    ...
    try {
      process.env.ORIRO_BUNDLED_PLUGINS_DIR = path.join(tempRoot, "dist", "extensions");
  ...
  it("uses the active bundled plugin root override for channel entry loading", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-bundled-override-"));
    const previousBundledPluginsDir = process.env.ORIRO_BUNDLED_PLUGINS_DIR;
    const pluginDir = path.join(tempRoot, "dist", "extensions", "alpha");
    fs.mkdirSync(pluginDir, { recursive: true });
    fs.writeFileSync(
  ...
  it("falls back through the cached loader for package-local dist entries needing SDK aliases", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-bundled-package-dist-"));
  ...
  it("falls back through the cached loader for direct override dist entries needing SDK aliases", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-bundled-direct-dist-"));
    const previousBundledPluginsDir = process.env.ORIRO_BUNDLED_PLUGINS_DIR;
    ...
    try {
      process.env.ORIRO_BUNDLED_PLUGINS_DIR = pluginsRoot;
  ...
  it("treats direct bundled plugin-tree overrides as scan roots", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-bundled-direct-override-"));
    const previousBundledPluginsDir = process.env.ORIRO_BUNDLED_PLUGINS_DIR;
  ...
  it("partitions bundled channel lazy caches by active bundled root without re-importing", async () => {
    const rootA = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-bundled-root-a-"));
    const rootB = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-bundled-root-b-"));
    const previousBundledPluginsDir = process.env.ORIRO_BUNDLED_PLUGINS_DIR;
    const testGlobal = globalThis as typeof globalThis & {
    ...
    try {
      ...
      process.env.ORIRO_BUNDLED_PLUGINS_DIR = path.join(rootA, "dist", "extensions");
      ...
      process.env.ORIRO_BUNDLED_PLUGINS_DIR = path.join(rootB, "dist", "extensions");
  ...
  it("uses dist-runtime as the boundary root for packaged setup entries", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-bundled-runtime-root-"));
  ...
  it("loads setup-entry feature plugins without loading the main channel entry", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-bundled-setup-only-"));
    const previousBundledPluginsDir = process.env.ORIRO_BUNDLED_PLUGINS_DIR;
    ...
    try {
      process.env.ORIRO_BUNDLED_PLUGINS_DIR = path.join(root, "dist", "extensions");
  ...
  it("swallows and caches bundled plugin and setup load failures", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-bundled-load-failure-"));
    const previousBundledPluginsDir = process.env.ORIRO_BUNDLED_PLUGINS_DIR;
    const pluginDir = path.join(root, "dist", "extensions", "alpha");
    const testGlobal = globalThis as typeof globalThis & {
  ...
  it("caches undefined bundled plugin loads as unavailable", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-bundled-null-load-"));
    const previousBundledPluginsDir = process.env.ORIRO_BUNDLED_PLUGINS_DIR;
    const pluginDir = path.join(root, "dist", "extensions", "alpha");
    const testGlobal = globalThis as typeof globalThis & {
  ...
  it("keeps channel entrypoints on the dedicated entry-contract SDK surface", () => {
    const offenders = collectBundledChannelEntrypointOffenders(
      bundledPluginRoots,
      (source) =>
        !source.includes('from "oriro/plugin-sdk/channel-entry-contract"') ||
        source.includes('from "oriro/plugin-sdk/core"') ||
        source.includes('from "oriro/plugin-sdk/channel-core"'),
    );
  ...
  it("keeps setup-entry legacy feature hints mirrored in package metadata", () => {
    ...
    for (const extensionDir of bundledPluginRoots) {
      ...
      for (const feature of ["legacyStateMigrations", "legacySessionSurfaces"]) {
        ...
        const hasHint = packageJson.oriro?.setupFeatures?.[feature] === true;
  ...
  it("keeps channel implementations off the broad core SDK surface", () => {
    ...
    for (const extensionDir of bundledPluginRoots) {
      for (const relativePath of ["src/channel.ts", "src/plugin.ts"]) {
        ...
        if (source.includes('from "oriro/plugin-sdk/core"')) {
          offenders.push(path.relative(process.cwd(), filePath));
        }
      }
    }
  ...
  it("keeps bundled hot runtime barrels off the broad core SDK surface", () => {
    const offenders = [
      ...
      fs.readFileSync(path.resolve(filePath), "utf8").includes("oriro/plugin-sdk/core"),
    );
  ...
  it("keeps bundled doctor surfaces off the broad runtime barrel", () => {
    const offenders = [
      ...
      fs
        ...
        .includes('from "oriro/plugin-sdk/runtime"'),
    );
  ...
  it("breaks reentrant bundled channel discovery cycles with an empty fallback", async () => {
    const pluginDir = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-bundled-reentrant-"));
    const modulePath = path.join(pluginDir, "index.cjs");
    fs.writeFileSync(
      modulePath,
      `
const reenter = globalThis["__oriroBundledChannelReenter"];
...
`,
    ...
    let reentered = false;
    (globalThis as { __oriroBundledChannelReenter?: () => void })[
      "__oriroBundledChannelReenter"
    ] = () => {
      if (!reentered) {
        reentered = true;
