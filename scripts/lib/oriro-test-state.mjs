#!/usr/bin/env node
// Creates isolated Oriro test HOME/state directories and shell snippets.
...
function usage() {
  return `Usage:
  node scripts/lib/oriro-test-state.mjs -- create [--label <name>] [--scenario <name>] [--env-file <path>] [--json]
  node scripts/lib/oriro-test-state.mjs shell [--label <name>] [--scenario <name>]
  node scripts/lib/oriro-test-state.mjs shell-function
...
function scenarioConfig(scenario, options = {}) {
  ...
  if (scenario === "upgrade-survivor") {
    return {
      ...
      skills: {
        allowBundled: ["memory", "oriro-testing"],
        limits: {
          maxSkillsInPrompt: 8,
  ...
  if (scenario === "gateway-loopback") {
    return {
      gateway: {
        ...
        auth: {
          mode: "token",
          token: options.token || "oriro-test-token",
        },
        controlUi: {
          enabled: false,
        },
...
function renderAuthProfileSecretKeyExport() {
  return [
    'ORIRO_AUTH_PROFILE_SECRET_KEY_FILE="$ORIRO_TEST_STATE_HOME/.oriro-test-auth-profile-secret-key"',
    'if [ -s "$ORIRO_AUTH_PROFILE_SECRET_KEY_FILE" ]; then',
    '  ORIRO_AUTH_PROFILE_SECRET_KEY="$(cat "$ORIRO_AUTH_PROFILE_SECRET_KEY_FILE")"',
    "else",
    '  ORIRO_AUTH_PROFILE_SECRET_KEY="$(od -An -N 32 -tx1 /dev/urandom | tr -d " \\n")"',
    '  ( umask 077; printf "%s\\n" "$ORIRO_AUTH_PROFILE_SECRET_KEY" > "$ORIRO_AUTH_PROFILE_SECRET_KEY_FILE" )',
    "fi",
    'if [ -z "$ORIRO_AUTH_PROFILE_SECRET_KEY" ]; then',
    '  echo "failed to generate ORIRO_AUTH_PROFILE_SECRET_KEY" >&2',
    "  return 1 2>/dev/null || exit 1",
    "fi",
    "export ORIRO_AUTH_PROFILE_SECRET_KEY",
  ];
}
...
function renderConfigWrite(configPathExpression, config) {
  ...
  return [
    `cat > ${configPathExpression} <<'ORIRO_TEST_STATE_JSON'`,
    json,
    "ORIRO_TEST_STATE_JSON",
  ].join("\n");
}
...
function buildCreatePlan(options = {}) {
  ...
  const env = {
    ...
    ORIRO_AUTH_PROFILE_SECRET_KEY: generateAuthProfileSecretKey(),
...
export function renderShellSnippet(options = {}) {
  ...
  const homeTemplate = `oriro-${label}-${scenario}-home.XXXXXX`;
  const lines = [
    'ORIRO_TEST_STATE_TMP_ROOT="${ORIRO_TEST_STATE_TMPDIR:-${TMPDIR:-/tmp}}"',
    'ORIRO_TEST_STATE_TMP_ROOT="${ORIRO_TEST_STATE_TMP_ROOT%/}"',
    '[ -n "$ORIRO_TEST_STATE_TMP_ROOT" ] || ORIRO_TEST_STATE_TMP_ROOT="/tmp"',
    "export ORIRO_TEST_STATE_TMP_ROOT",
    'mkdir -p "$ORIRO_TEST_STATE_TMP_ROOT"',
    `ORIRO_TEST_STATE_HOME="$(mktemp -d "$ORIRO_TEST_STATE_TMP_ROOT/${homeTemplate}")"`,
    'export HOME="$ORIRO_TEST_STATE_HOME"',
    'export USERPROFILE="$ORIRO_TEST_STATE_HOME"',
    'export ORIRO_HOME="$ORIRO_TEST_STATE_HOME"',
    'export ORIRO_STATE_DIR="$ORIRO_TEST_STATE_HOME/.oriro"',
    'export ORIRO_CONFIG_PATH="$ORIRO_STATE_DIR/oriro.json"',
    ...renderAuthProfileSecretKeyExport(),
    'export ORIRO_TEST_WORKSPACE_DIR="$ORIRO_TEST_STATE_HOME/workspace"',
    'mkdir -p "$ORIRO_STATE_DIR" "$ORIRO_TEST_WORKSPACE_DIR"',
  ];
  for (const [key, value] of Object.entries(env)) {
    lines.push(`export ${key}=${shellQuote(value)}`);
  }
  const configWrite = renderConfigWrite('"$ORIRO_CONFIG_PATH"', config);
  if (configWrite) {
...
export function renderShellFunction() {
  ...
  case "$raw_label" in
    /*)
      ORIRO_TEST_STATE_HOME="$raw_label"
      mkdir -p "$ORIRO_TEST_STATE_HOME"
      ;;
    *)
      label="$(printf "%s" "$label" | tr -cs "A-Za-z0-9_.-" "-" | sed -e "s/^-*//" -e "s/-*$//")"
      [ -n "$label" ] || label="state"
      local tmp_root="\${ORIRO_TEST_STATE_TMPDIR:-\${TMPDIR:-/tmp}}"
      tmp_root="\${tmp_root%/}"
      [ -n "$tmp_root" ] || tmp_root="/tmp"
      mkdir -p "$tmp_root"
      ORIRO_TEST_STATE_HOME="$(mktemp -d "$tmp_root/oriro-$label-$scenario-home.XXXXXX")"
      ;;
  esac
  export HOME="$ORIRO_TEST_STATE_HOME"
  export USERPROFILE="$ORIRO_TEST_STATE_HOME"
  ...
  export ORIRO_TEST_WORKSPACE_DIR="$ORIRO_TEST_STATE_HOME/workspace"
  unset ORIRO_AGENT_DIR
  unset ORIRO_SERVICE_REPAIR_POLICY
  mkdir -p "$ORIRO_STATE_DIR" "$ORIRO_TEST_WORKSPACE_DIR"
  case "$scenario" in
    minimal)
      cat > "$ORIRO_CONFIG_PATH" <<'ORIRO_TEST_STATE_JSON'
{}
ORIRO_TEST_STATE_JSON
      ;;
    update-stable)
...
ORIRO_TEST_STATE_JSON
      ;;
    external-service)
      export ORIRO_SERVICE_REPAIR_POLICY="external"
      cat > "$ORIRO_CONFIG_PATH" <<'ORIRO_TEST_STATE_JSON'
{}
ORIRO_TEST_STATE_JSON
      ;;
