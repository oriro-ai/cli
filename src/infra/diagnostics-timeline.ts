...
const ORIRO_DIAGNOSTICS_TIMELINE_SCHEMA_VERSION = "oriro.diagnostics.v1";
...
export function isDiagnosticsTimelineEnabled(options: DiagnosticsTimelineOptions = {}): boolean {
  ...
  return (
    (isDiagnosticFlagEnabled("timeline", config, env) ||
      ...
      isTruthyEnvValue(env.ORIRO_DIAGNOSTICS)) &&
    typeof env.ORIRO_DIAGNOSTICS_TIMELINE_PATH === "string" &&
    env.ORIRO_DIAGNOSTICS_TIMELINE_PATH.trim().length > 0
  );
}
...
function serializeTimelineEvent(event: DiagnosticsTimelineEvent, env: NodeJS.ProcessEnv): string {
  const normalized = {
    schemaVersion: ORIRO_DIAGNOSTICS_TIMELINE_SCHEMA_VERSION,
    type: event.type,
    timestamp: event.timestamp ?? new Date().toISOString(),
    name: event.name,
    ...(env.ORIRO_DIAGNOSTICS_RUN_ID ? { runId: env.ORIRO_DIAGNOSTICS_RUN_ID } : {}),
    ...(env.ORIRO_DIAGNOSTICS_ENV ? { envName: env.ORIRO_DIAGNOSTICS_ENV } : {}),
    pid: process.pid,
...
export function emitDiagnosticsTimelineEvent(
  ...
  const path = env.ORIRO_DIAGNOSTICS_TIMELINE_PATH?.trim();
  if (!path) {
    return;
  }
