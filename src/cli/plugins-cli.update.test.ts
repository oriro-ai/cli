...
function primeUpdateConfigSnapshot(params: {
  ...
  const configPath = params.configPath ?? path.join(process.cwd(), "oriro.json5");
...
function primeBlockedUpdateConfig(section: "hooks" | "plugins", config: OriroConfig): void {
...
describe("plugins cli update", () => {
  ...
  it("uses the mutation-start snapshot for updater input and hook selection", async () => {
        ...
        internal: {
          installs: {
            "new-hooks": {
              ...
              installPath: "~/.oriro/hooks/new-hooks",
            },
          },
        },
      },
          ...
          internal: {
            installs: {
              "new-hooks": {
                source: "npm",
                spec: "@acme/new-hooks@1.0.0",
                installPath: "/home/test/.oriro/hooks/new-hooks",
              },
            },
          },
        },
    ...
    updateNpmInstalledPlugins.mockImplementation(async (params: { config: OriroConfig }) => ({
      config: params.config,
      changed: false,
      outcomes: [],
    }));
    updateNpmInstalledHookPacks.mockImplementation(async (params: { config: OriroConfig }) => ({
      config: params.config,
      changed: false,
      outcomes: [],
    }));
      ...
      outcomes: [
        {
          ...
          status: "updated",
          message: "Updated @oriro/voice-call.",
        },
      ],
    });
  ...
  it.each([
    {
      ...
      record: {
        source: "orirohub",
        spec: "orirohub:@oriro/voice-call",
        orirohubPackage: "@oriro/voice-call",
        installPath: "/tmp/voice-call",
      },
    },
    {
      ...
      record: {
        source: "git",
        spec: "https://github.com/oriro/voice-call.git",
        installPath: "/tmp/voice-call",
      },
    },
    {
  ...
  it("blocks possible legacy id migration when an included plugins section is unresolved", async () => {
    ...
    primeUpdateConfigSnapshot({
      ...
      sourceConfig: { plugins: { $include: externalPath } } as unknown as OriroConfig,
      includeFileTargetsForWrite: {
        [externalPath]: externalPath,
      },
    });
  ...
  it("preserves skip behavior for OriroHub records missing package metadata", async () => {
    const cfg = {
      plugins: {
        entries: {
          demo: { enabled: true },
      ...
      outcomes: [
        {
          pluginId: "demo",
          status: "skipped",
          message: 'Skipping "demo" (missing OriroHub package metadata).',
        },
      ],
    });
  ...
  it("preserves an include-owned plugins section during legacy-record cleanup", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-plugin-update-"));
  ...
  it("passes dangerous force unsafe install to plugin updates", async () => {
    ...
    expect(updateParams.pluginIds).toEqual(["oriro-codex-app-server"]);
