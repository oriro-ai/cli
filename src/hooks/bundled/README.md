...
This directory contains hooks that ship with Oriro. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
**Output**: `<workspace>/memory/YYYY-MM-DD-slug.md` (defaults to `~/.oriro/workspace`)
...
homepage: https://docs.oriro.ai/automation/hooks#my-hook
metadata:
...
- **Managed hooks**: `~/.oriro/hooks/` (shared across workspaces)
...
2. Restart gateway: `pkill -9 -f 'oriro.*gateway' && pnpm oriro gateway`
3. Enable the hook: `oriro hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.oriro.ai/automation/hooks
