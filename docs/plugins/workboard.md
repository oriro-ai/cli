...
Oriro Gateway; it is not a replacement for GitHub Issues, Linear, Jira, or
...
state directory and move with the rest of that Gateway's Oriro state.
...
under the Oriro state directory. Boards, cards, labels, lifecycle events,
...
`oriro doctor --fix` to migrate the shipped legacy plugin-state namespaces
...
system processes; normal Oriro subagent sessions still own execution. The
...
Oriro records the orchestration intent and exposes it in worker context; the
...
oriro workboard create "Fix stale card lifecycle" --priority high --labels bug,workboard
oriro workboard show <card-id>
oriro workboard dispatch
```

`oriro workboard dispatch` calls the running Gateway so worker starts use the
...
```bash
oriro plugins disable workboard
oriro gateway restart
```
