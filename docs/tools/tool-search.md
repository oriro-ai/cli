---
summary: "Tool Search: compact large Oriro tool catalogs behind search, describe, and call"
title: "Tool Search"
read_when:
  - You want Oriro agents to use a large tool catalog without adding every tool schema to the prompt
  - You want Oriro tools, MCP tools, and client tools exposed through one compact runtime surface
  - You are implementing or debugging tool discovery for Oriro runs
---

Tool Search is an experimental Oriro agent runtime feature. It gives agents one
...
This page documents Oriro Tool Search. It is not the Codex-native tool
...
When enabled for Oriro runs, the model receives one `tool_search_code` tool
by default. That tool runs a short JavaScript body in an isolated Node
subprocess with an `oriro.tools` bridge:

```js
const hits = await oriro.tools.search("create a GitHub issue");
const tool = await oriro.tools.describe(hits[0].id);
return await oriro.tools.call(tool.id, {
  title: "Crash on startup",
  body: "Steps to reproduce...",
});
```

The catalog can include Oriro tools, plugin tools, MCP tools, and
...
needs the exact schema, and calls that tool through Oriro.

Codex harness runs do not receive these experimental Oriro Tool Search
controls. Oriro passes product capabilities to Codex as dynamic tools, and
Codex owns the stable native code mode, native tool search, deferred dynamic
tools, and nested tool calls.

## How a turn runs

At planning time the Oriro embedded runner builds the effective catalog for the
run:

1. Resolve the active tool policy for the agent, profile, sandbox, and session.
2. List eligible Oriro and plugin tools.
...
6. Expose the Oriro code bridge, the structured fallback tools, or the
   compact directory surface to the model.

At execution time every real tool call returns to Oriro. The isolated Node
runtime does not hold plugin implementations, MCP client objects, or secrets.
`oriro.tools.call(...)` crosses the bridge back into the Gateway, where the
...
- `directory`: exposes `tool_search`, `tool_describe`, and `tool_call` plus a
  ...
  providers that should see tool names without every full schema. Oriro can
  also expose a small bounded set of likely or required tool schemas directly
  for the current turn.

All modes use the same policy-filtered catalog and normal Oriro execution
...
for the current run while Oriro tools, plugin tools, and MCP tools can be
...
All modes are experimental. Prefer direct tool exposure for small Oriro tool
...
catalog includes eligible Oriro, MCP, and client tools after normal policy
filtering.
...
`oriro.tools.search(query, options?)`

Searches the effective catalog for the current run. Results are compact and safe
to put back into prompt context.

```js
const hits = await oriro.tools.search("calendar event", { limit: 5 });
```

`oriro.tools.describe(id)`

Loads full metadata for one search result, including the exact input schema.

```js
const calendarCreate = await oriro.tools.describe("mcp:calendar:create_event");
```

`oriro.tools.call(id, args)`

Calls a selected tool through Oriro.

```js
await oriro.tools.call(calendarCreate.id, {
  summary: "Planning",
...
the model requests an exact hidden directory tool name directly, Oriro
hydrates it from the authorized catalog before normal execution.
Directory-mode client tool names must not collide with Oriro, plugin, or MCP
...
network grants, and no child-process or worker grants. Oriro enforces a
...
- `oriro.tools.search`
- `oriro.tools.describe`
- `oriro.tools.call`

Normal Oriro behavior still applies to final calls:
...
Enable Tool Search for Oriro runs with the default code bridge:

```bash
oriro config set tools.toolSearch true
```
...
Use the structured fallback tools instead for Oriro runs:
...
Use the compact directory surface instead for Oriro runs:
...
- final tool calls executed through Oriro
...
- whether the result came from Oriro, MCP, or a client tool

## E2E validation

The gateway E2E runner proves both paths with the Oriro runtime:
