...
vi.mock("../../skills/lifecycle/orirohub.js", () => ({
  ...
  searchSkillsFromOriroHub: vi.fn(),
...
vi.mock("../../skills/security/orirohub-verdicts.js", () => ({
  collectOriroHubVerdictTargets: vi.fn(() => []),
  fetchOriroSkillSecurityVerdicts: vi.fn(),
}));
...
describe("skills proposal gateway handlers", () => {
  beforeEach(async () => {
    testState = await createOriroTestState({
      layout: "state-only",
      prefix: "oriro-skills-proposals-gateway-state-",
    });
    ...
    mocks.workspaceDir = await tempDirs.make("oriro-skills-proposals-gateway-");
  ...
  it("scopes list and inspect to the resolved agent workspace", async () => {
    ...
    const secondWorkspaceDir = await tempDirs.make("oriro-skills-proposals-gateway-second-");
