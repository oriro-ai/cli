 ...
 */
import { formatCliCommand, parseDurationMs } from "oriro/plugin-sdk/cli-runtime";
import { resolveExpiresAtMsFromDurationMs } from "oriro/plugin-sdk/number-runtime";
import type {
  OriroPluginApi,
  ProviderAuthContext,
...
import {
  ...
  type OriroConfig as ProviderAuthConfig,
...
import { fetchClaudeUsage } from "oriro/plugin-sdk/provider-usage";
...
function buildAnthropicAuthDoctorHint(params: {
  ...
  return [
    ...
    `Fix: run "${formatCliCommand("oriro doctor --yes")}"`,
  ].join("\n");
}
...
export function registerAnthropicPlugin(api: OriroPluginApi): void {
