// Debug Claude Usage script supports Oriro repository automation.
...
const loadAuthProfiles = (agentId: string) => {
  const stateRoot = process.env.ORIRO_STATE_DIR?.trim() || path.join(os.homedir(), ".oriro");
...
const resolveFetchTimeoutMs = (raw = process.env.ORIRO_DEBUG_CLAUDE_USAGE_FETCH_TIMEOUT_MS) => {
  return parseStrictIntegerOption({
    fallback: DEFAULT_FETCH_TIMEOUT_MS,
    label: "ORIRO_DEBUG_CLAUDE_USAGE_FETCH_TIMEOUT_MS",
    min: 1,
    raw,
  });
};
