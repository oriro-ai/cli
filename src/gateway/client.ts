// Oriro Gateway client facade.
// Wraps the shared gateway-client package with Oriro host dependencies.
import {
...
function createOriroGatewayClientHostDeps(
  overrides?: GatewayClientHostDeps,
): GatewayClientHostDeps {
  return {
    // This wrapper is the only place the package reaches into Oriro runtime
...
export class GatewayClient {
  ...
  constructor(opts: GatewayClientOptions) {
    ...
    // Oriro device identity, token storage, proxy routing, and logging.
    this.#client = new BaseGatewayClient({
      ...opts,
      clientVersion: opts.clientVersion ?? VERSION,
      hostDeps: createOriroGatewayClientHostDeps(opts.hostDeps),
    });
  }
