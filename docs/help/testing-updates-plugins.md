---
summary: "How Oriro validates update paths, package migrations, and plugin install/update behavior"
read_when:
  - Changing Oriro update, doctor, package acceptance, or plugin install behavior
...
- A user can move from an older published package to the candidate package
  ...
  channel config.
- `oriro doctor --fix --non-interactive` owns legacy cleanup and repair
...
- `test:docker:plugins` validates plugin install smoke, local folder installs,
  ...
  local OriroHub fixture installs and update no-ops, marketplace update behavior,
  and Claude-bundle enable/inspect. Set `ORIRO_PLUGINS_E2E_ORIROHUB=0` to
  keep the OriroHub block hermetic/offline.
...
- `test:docker:plugin-update` validates that an unchanged installed plugin does
  not reinstall or lose install metadata during `oriro plugins update`.
...
- `test:docker:published-upgrade-survivor` first installs a published baseline,
  configures it through a baked `oriro config set` recipe, updates it to the
...
- `test:docker:update-restart-auth` installs the candidate package, starts a
  ...
  `oriro update --yes --json`, and requires the candidate update command to
...
```bash
ORIRO_UPGRADE_SURVIVOR_BASELINE_SPEC=oriro@2026.4.23 \
ORIRO_UPGRADE_SURVIVOR_SCENARIO=versioned-runtime-deps \
pnpm test:docker:published-upgrade-survivor

ORIRO_UPGRADE_SURVIVOR_BASELINE_SPEC=oriro@latest \
ORIRO_UPGRADE_SURVIVOR_SCENARIO=bootstrap-persona \
pnpm test:docker:published-upgrade-survivor
```
...
`ORIRO_UPGRADE_SURVIVOR_SCENARIOS=reported-issues` expands to all reported
...
- `source=npm`: validate `oriro@beta`, `oriro@latest`, or an exact
  published version.
...
- `source=trusted-url`: validate an HTTPS tarball with required
  ...
  `ORIRO_TRUSTED_PACKAGE_TOKEN` secret.
...
`package_acceptance_package_spec=oriro@YYYY.M.PATCH` so the same upgrade matrix
...
`last-stable-4` resolves to the four latest stable npm-published Oriro
...
- Registry/package source behavior: `test:docker:plugins` fixture or OriroHub
  fixture server.
