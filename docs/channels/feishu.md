...
<Note>
Requires Oriro 2026.5.29 or above. Run `oriro --version` to check. Upgrade with `oriro update`.
</Note>

<Steps>
  <Step title="Run the channel setup wizard">
  ```bash
  oriro channels login --channel feishu
  ```
...
```bash
oriro pairing list feishu
oriro pairing approve feishu <CODE>
```
...
4. Check logs: `oriro logs --follow`
...
5. Ensure the gateway is running: `oriro gateway status`
6. Check logs: `oriro logs --follow`

### QR setup does not react in the Feishu mobile app

1. Rerun setup: `oriro channels login --channel feishu`
...
3. Restart the gateway: `oriro gateway restart`
...
If a named account created an unscoped dynamic agent on an older release, that legacy agent still counts toward `maxAgents`. Confirm that it is not used by the default account before removing it, or temporarily increase `maxAgents`; Oriro cannot safely infer which account owns ambiguous legacy state.
</Note>
  ...
  channels: {
    feishu: {
      ...
      dynamicAgentCreation: {
        enabled: true,
        workspaceTemplate: "~/.oriro/workspace-{agentId}",
        agentDirTemplate: "~/.oriro/agents/{agentId}/agent",
      },
    },
  },
  session: {
...
| `channels.feishu.dynamicAgentCreation.workspaceTemplate` | Path template for dynamic agent workspaces | `~/.oriro/workspace-{agentId}`    |
| `channels.feishu.dynamicAgentCreation.agentDirTemplate`  | Agent directory name template              | `~/.oriro/agents/{agentId}/agent` |
...
workspace: /Users/you/.oriro/workspace-feishu-ou_xxxxxx
...
of raw `file_key` JSON. When `tools.media.audio` is configured, Oriro
...
conversion fails, Oriro falls back to a file attachment and logs the reason.
...
topic session key. If a native topic starter event omits `thread_id`, Oriro
hydrates it from Feishu before routing the turn. Normal group replies that
Oriro turns into threads keep using the reply root message ID (`om_*`) so the
