...
- During heartbeat runs, Oriro treats `HEARTBEAT_OK` as an ack when it appears at the **start or end** of the reply. The token is stripped and the reply is dropped if the remaining content is **≤ `ackMaxChars`** (default: 300).
...
- Explicit session key (copy from `oriro sessions --json` or the [sessions CLI](/cli/sessions)).
...
<AccordionGroup>
  ...
  <Accordion title="Visibility and skip behavior">
    ...
    - If only alert delivery is disabled, Oriro can still run the heartbeat, update due-task timestamps, restore the session idle timestamp, and suppress the outward alert payload.
    - If the resolved heartbeat target supports typing, Oriro shows typing while the heartbeat run is active. This uses the same target the heartbeat would send chat output to, and it is disabled by `typingMode: "never"`.
...
If **all three** are false, Oriro skips the heartbeat run entirely (no model call).
...
<AccordionGroup>
  <Accordion title="Behavior">
    - Oriro parses the `tasks:` block and checks each task against its own `interval`.
...
```bash
oriro system event --text "Check for urgent follow-ups" --mode now
```
