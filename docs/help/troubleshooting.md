---
summary: "Symptom first troubleshooting hub for Oriro"
read_when:
  - Oriro is not working and you need the fastest path to a fix
...
- `oriro status` → shows configured channels and no obvious auth errors.
- `oriro status --all` → full report is present and shareable.
- `oriro gateway probe` → expected gateway target is reachable (`Reachable: yes`). `Capability: ...` tells you what auth level the probe could prove, and `Read probe: limited - missing scope: operator.read` is degraded diagnostics, not a connect failure.
- `oriro gateway status` → `Runtime: running`, `Connectivity probe: ok`, and a plausible `Capability: ...` line. Use `--require-rpc` if you need read-scope RPC proof too.
- `oriro doctor` → no blocking config/service errors.
- `oriro channels status --probe` → reachable gateway returns live per-account
...
- `oriro logs --follow` → steady activity, no repeating fatal errors.
...
and run `oriro status --all` again. See [Tools](/tools) for the profile
...
## Local OpenAI-compatible backend works directly but fails in Oriro

If your local or self-hosted `/v1` backend answers small direct
`/v1/chat/completions` probes but fails on `oriro infer model run` or normal
agent turns:

1. If the error mentions `messages[].content` expecting a string, set
   `models.providers.<provider>.models[].compat.requiresStringContent: true`.
2. If the backend still fails only on Oriro agent turns, set
   `models.providers.<provider>.models[].compat.supportsTools: false` and retry.
3. If tiny direct calls still work but larger Oriro prompts crash the
...
## Plugin install fails with missing oriro extensions

If install fails with `package.json missing oriro.extensions`, the plugin package
is using an old shape that Oriro no longer accepts.

Fix in the plugin package:

1. Add `oriro.extensions` to `package.json`.
2. Point entries at built runtime files (usually `./dist/index.js`).
3. Republish the plugin and run `oriro plugins install <package>` again.

Example:

```json
{
  "name": "@oriro/my-plugin",
  "version": "1.2.3",
  "oriro": {
...
Install policy runs on plugin installs and updates. Oriro-owned plugin
versions normally move with the Oriro release, so an Oriro update can
also need matching `@oriro/*` plugin updates during post-update sync.

Avoid these broad policy shapes unless you also maintain the matching upgrade
rule:

- Freezing Oriro-owned plugins to one exact old version, such as allowing
  only `@oriro/*@2026.5.3`.
...
- Approving plugin versions without considering the policy request's
  `oriroVersion` and the plugin candidate metadata.

Safer policy rules allow trusted Oriro-owned plugin updates when the
candidate is compatible with the current Oriro host, instead of pinning a
single release forever. If you block npm by default, make a narrow exception
for the trusted `@oriro/*` plugin packages or plugin ids you use. If you
...
If the policy is intentionally strict, relax it for the trusted Oriro upgrade
window, rerun `oriro plugins update --all`, then restore the stricter rule.
...
If `oriro doctor`, setup, or startup warnings show:
...
them. Do not remove the plugin config. Fix the file ownership or run Oriro as
...
```mermaid
flowchart TD
  A[Oriro is not working] --> B{What breaks first}
  B --> C[No replies]
...
<AccordionGroup>
  ...
  <Accordion title="Dashboard or Control UI will not connect">
    ...
    - `Dashboard: http://...` is shown in `oriro gateway status`
    - `Connectivity probe: ok`
    - `Capability: read-only`, `write-capable`, or `admin-capable`
  ...
  <Accordion title="Exec suddenly asks for approval">
    ```bash
    oriro config get tools.exec.host
    oriro config get tools.exec.security
    oriro config get tools.exec.ask
  ...
  <Accordion title="Browser tool fails">
    ...
    - `oriro` starts, or `user` can see local Chrome tabs.
    ...
    - stale viewport / dark-mode / locale / offline overrides on attach-only or remote CDP profiles → run `oriro browser stop --browser-profile <name>` to close the active control session and release emulation state without restarting the gateway.
