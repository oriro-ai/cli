---
summary: "Use NovitaAI's OpenAI-compatible API with Oriro"
read_when:
  - You want to run Oriro with NovitaAI models
...
API. In Oriro it is a bundled model provider, so the provider id is
...
This provider uses Novita's OpenAI-compatible endpoint. Oriro handles
...
```bash
oriro onboard --auth-choice novita-api-key
```
...
The catalog is a starting point for Oriro model selection. Your account,
...
```bash
oriro models list --provider novita
```

## Troubleshooting

- `401` or `403`: verify the key in Novita's key management page and re-run
  `oriro onboard --auth-choice novita-api-key` if the stored profile is
  stale.
