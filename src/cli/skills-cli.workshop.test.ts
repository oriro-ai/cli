...
vi.mock("../terminal/links.js", () => ({
  formatDocsLink: () => "docs.oriro.ai/cli/skills",
}));
...
describe("skills workshop cli", () => {
  ...
  beforeEach(async () => {
    testState = await createOriroTestState({
      layout: "state-only",
      prefix: "oriro-skills-cli-workshop-state-",
    });
    mocks.workspaceDir = await tempDirs.make("oriro-skills-cli-workshop-");
  ...
  it("scopes list and inspect to the selected workspace", async () => {
    ...
    mocks.workspaceDir = await tempDirs.make("oriro-skills-cli-workshop-second-");
    await runCommand(["skills", "workshop", "list"]);
