...
// Bridges /v1/embeddings requests to configured Oriro memory providers.
...
// OpenAI-compatible `/v1/embeddings` bridge. It maps Oriro agent/model
...
export async function handleOpenAiEmbeddingsHttpRequest(
  ...
  if (requestModel !== ORIRO_MODEL_ID && !resolveAgentIdFromModel(requestModel, cfg)) {
    sendJson(res, 400, {
      error: {
        message: "Invalid `model`. Use `oriro` or `oriro/<agentId>`.",
  ...
  const overrideModel =
    normalizeOptionalString(getHeader(req, "x-oriro-model")) ||
