---
summary: "Get Oriro installed and run your first chat in minutes."
read_when:
...
Install Oriro, run onboarding, and chat with your AI assistant — all in
...
```bash
mkdir -p "$HOME/.oriro/control-ui-custom"
...
```json
{
  "gateway": {
    "controlUi": {
      "enabled": true,
      "root": "$HOME/.oriro/control-ui-custom"
    }
  }
}
```
...
<Accordion title="Advanced: environment variables">
  If you run Oriro as a service account or want custom paths:

- `ORIRO_HOME` — home directory for internal path resolution
- `ORIRO_STATE_DIR` — override the state directory
- `ORIRO_CONFIG_PATH` — override the config file path
