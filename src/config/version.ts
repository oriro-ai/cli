...
/** Parses stable, prerelease, and legacy dot-beta Oriro versions. */
export function parseOriroVersion(raw: string | null | undefined): OriroVersion | null {
  if (!raw) {
    return null;
  }
...
export function normalizeOriroVersionBase(raw: string | null | undefined): string | null {
  const parsed = parseOriroVersion(raw);
  if (!parsed) {
    return null;
  }
...
export function isSameOriroStableFamily(
  a: string | null | undefined,
  b: string | null | undefined,
): boolean {
  const parsedA = parseOriroVersion(a);
  const parsedB = parseOriroVersion(b);
  if (!parsedA || !parsedB) {
    return false;
  }
...
export function compareOriroVersions(
  a: string | null | undefined,
  b: string | null | undefined,
): number | null {
  const parsedA = parseOriroVersion(a);
...
export function shouldWarnOnTouchedVersion(
  ...
  const parsedCurrent = parseOriroVersion(current);
  const parsedTouched = parseOriroVersion(touched);
  if (
    parsedCurrent &&
    parsedTouched &&
  ...
  if (isSameOriroStableFamily(current, touched)) {
    return false;
  }
  const cmp = compareOriroVersions(current, touched);
  return cmp !== null && cmp < 0;
}

function releaseRank(version: OriroVersion): number {
