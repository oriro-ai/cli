...
import { spawn as startOriroCliProcess, spawnSync } from "node:child_process";
...
export function formatMatrixQaCliCommand(args: string[]) {
  return `oriro ${redactMatrixQaCliArgs(args).join(" ")}`;
}

export function resolveMatrixQaOriroCliEntryPath(cwd: string): string {
...
function killMatrixQaCliChild(
  child: ReturnType<typeof startOriroCliProcess>,
...
export function startMatrixQaOriroCli(params: {
  ...
  const distEntryPath = resolveMatrixQaOriroCliEntryPath(cwd);
  const stdout: Buffer[] = [];
  const stderr: Buffer[] = [];
  let closed = false;
  ...
  const child = startOriroCliProcess(process.execPath, [distEntryPath, ...params.args], {
    cwd,
...
export async function runMatrixQaOriroCli(params: {
  allowNonZero?: boolean;
  args: string[];
  cwd?: string;
  env: NodeJS.ProcessEnv;
  ...
  return await startMatrixQaOriroCli(params).wait();
}
...
export async function createMatrixQaOriroCliRuntime(params: {
  accountId: string;
  accessToken: string;
  ...
  const rootDir = await mkdtemp(
    path.join(resolvePreferredOriroTmpDir(), "oriro-matrix-cli-qa-"),
  );
