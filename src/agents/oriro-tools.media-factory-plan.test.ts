...
import { resolveOptionalMediaToolFactoryPlan } from "./oriro-tools.media-factory-plan.js";
...
type CreateOriroToolsOptions = Parameters<
  typeof import("./oriro-tools.js").createOriroTools
>[0];
let createOriroToolsForTestModule: typeof import("./oriro-tools.js").createOriroTools;
let legacyComfyToolNames: string[];

async function createOriroToolsForTest(options?: CreateOriroToolsOptions) {
  return createOriroToolsForTestModule(options);
}
...
function createPlugin(params: {
  ...
  return {
    ...
    manifestPath: `/plugins/${params.id}/oriro.plugin.json`,
    channels: [],
    providers: [],
    cliBackends: [],
    skills: [],
...
function legacyModelProviderConfig(provider: Record<string, unknown>): OriroConfig {
  return {
    models: {
      providers: {
...
describe("optional media tool factory planning", () => {
  beforeAll(async () => {
    ({ createOriroTools: createOriroToolsForTestModule } =
      await import("./oriro-tools.js"));
  ...
  it("applies global tool policy before optional media factories run", () => {
    const config: OriroConfig = { tools: { deny: ["pdf"] } };
    installSnapshot(config, [
      createPlugin({
        id: "media-owner",
        contracts: { mediaUnderstandingProviders: ["anthropic"] },
  ...
  it("defers PDF model resolution from the tool-prep hot path", async () => {
    ...
    const tools = await createOriroToolsForTest({
      config,
      agentDir: "/tmp/oriro-agent-main",
  ...
  it("does not register the image tool without cheap vision availability evidence", async () => {
    const config: OriroConfig = {};
    const workspaceDir = "/tmp/oriro-workspace";
