...
function resolveAvailableKnownChannel(params: {
  ...
  // been eagerly loaded (e.g. `oriro agent --local`). Already-loaded and
...
export function isConfiguredChannel(cfg: OriroConfig, channelId: string): boolean {
...
function formatMissingOfficialExternalChannelsMessage(
  ...
  return `Configured official external channels ${labels} are missing their plugins. Run: oriro doctor --fix, or install individually: ${installCommands}.`;
}

function formatNoConfiguredChannelsMessage(): string {
  return [
    "Channel is required (no configured channels detected).",
    "Run oriro channels add to configure one, or pass --channel <channel> after enabling a channel.",
    "Use oriro channels list --all to see available channel ids.",
  ].join(" ");
}
...
async function isPluginConfigured(plugin: ChannelPlugin, cfg: OriroConfig): Promise<boolean> {
