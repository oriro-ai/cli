...
function isRuntimeDependencyMarkerName(name: string): boolean {
  return (
    name === ".oriro-runtime-deps.json" ||
    name === ".oriro-runtime-deps-stamp.json" ||
    name.startsWith(".oriro-runtime-deps-")
  );
}

function isInstallStageDebrisName(name: string): boolean {
  return /^\.oriro-install-stage(?:-.+)?$/u.test(name);
}

function isLegacyDependencyDebrisName(name: string): boolean {
  return (
    isRuntimeDependencyMarkerName(name) ||
    name === ".oriro-pnpm-store" ||
    name === ".oriro-install-backups" ||
...
function collectExplicitStageTargets(env: NodeJS.ProcessEnv): CleanupTarget[] {
  return splitPathList(env.ORIRO_PLUGIN_STAGE_DIR).map((entry) => ({
    kind: "explicit-stage",
...
async function hasOriroRenameResidue(root: string): Promise<boolean> {
  const nodeModulesRoot = path.join(root, "node_modules");
  if (await isFile(path.join(nodeModulesRoot, ".oriro-rename-tmp"))) {
    return true;
  }
  ...
  for (const entry of entries) {
    ...
    if (await isFile(path.join(entryPath, ".oriro-rename-tmp"))) {
      return true;
    }
    ...
    for (const scopedEntry of scopedEntries) {
      ...
      if (await isFile(path.join(entryPath, scopedEntry.name, ".oriro-rename-tmp"))) {
        return true;
      }
    }
  }
...
async function hasExplicitStageDebrisProof(root: string): Promise<boolean> {
  ...
  return await hasOriroRenameResidue(root);
}
...
/** Remove legacy plugin dependency state under trusted Oriro cleanup roots. */
