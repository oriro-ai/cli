...
```bash
oriro plugins install @openclaw/lobster
```
...
- This plugin integrates Lobster with Oriro _without core changes_.
...
## Using `oriro.invoke` (Lobster → Oriro tools)

Some Lobster pipelines may include a `oriro.invoke` step to call back into Oriro tools/plugins (for example: `gog` for Google Workspace, `gh` for GitHub, `message.send`, etc.).
...
- Runs Lobster in process via the published `@openclaw/lobster/core` runtime.
- Does not manage OAuth/tokens.
- Uses timeouts, stdout caps, and strict JSON envelope parsing.

## Docs

- https://docs.oriro.ai/tools/lobster
