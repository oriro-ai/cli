...
import type { MessageReceipt } from "oriro/plugin-sdk/channel-outbound";
import { mediaKindFromMime } from "oriro/plugin-sdk/media-mime";
import { MEDIA_FFMPEG_MAX_AUDIO_DURATION_SECS, runFfmpeg } from "oriro/plugin-sdk/media-runtime";
import { saveMediaBuffer, saveMediaStream, type SavedMedia } from "oriro/plugin-sdk/media-store";
import { readRegularFile, writeExternalFileWithinRoot } from "oriro/plugin-sdk/security-runtime";
...
async function saveMessageResourceWithType(params: {
  ...
  const saved = await saveFeishuResponseMedia({
    response,
    tmpDirPrefix: "oriro-feishu-resource-",
...
async function transcodeToFeishuVoiceOpus(params: {
  ...
  return await withTempWorkspace(
    { rootDir: resolvePreferredOriroTmpDir(), prefix: "feishu-voice-" },
    async (workspace) => {
      const ext = normalizeLowercaseStringOrEmpty(path.extname(params.fileName));
