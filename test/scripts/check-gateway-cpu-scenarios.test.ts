  ...
  it("prebuilds private QA dist before running QA scenarios when it is missing", async () => {
    ...
    const result = await testing.runGatewayCpuScenarios(options, {
      ...
      spawnSync: (_command: string, args: string[], opts?: { env?: Record<string, string> }) => {
        ...
        if (args.includes("oriro") && args.includes("qa")) {
    ...
    expect(calls[0]?.env).toMatchObject({
      ...
      ORIRO_CONFIG_PATH: path.join(outputDir, "qa-state-root", "state", "oriro.json"),
      ORIRO_ENABLE_PRIVATE_QA_CLI: "1",
      ORIRO_HOME: path.join(outputDir, "qa-state-root", "home"),
      ORIRO_RUN_NODE_SKIP_DTS_BUILD: "1",
      ORIRO_STATE_DIR: path.join(outputDir, "qa-state-root", "state"),
      ORIRO_TEST_DISABLE_UPDATE_CHECK: "1",
      USERPROFILE: path.join(outputDir, "qa-state-root", "home"),
    });
    expect(calls[0]?.env?.ORIRO_BUNDLED_PLUGIN_BUILD_IDS).toBeUndefined();
  });
