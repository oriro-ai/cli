// Implements guided and non-interactive `oriro channels add` account setup.
...
async function resolveCatalogChannelEntry(raw: string, cfg: OriroConfig | null) {
...
async function channelsAddCommandImpl(
  ...
  const cfg = (configSnapshot.sourceConfig ?? configSnapshot.config) as OriroConfig;
  ...
  if (!channel) {
    const hint = catalogEntry
      ? `Plugin ${catalogEntry.meta.label} could not be loaded after install. Run oriro doctor --fix, then retry oriro channels add.`
  ...
  if (!plugin?.setup?.applyAccountConfig) {
    runtime.error(
      ...
      })} Run ${formatCliCommand("oriro channels add")} with no flags for guided setup.`,
    );
    runtime.exit(1);
    return;
  }
