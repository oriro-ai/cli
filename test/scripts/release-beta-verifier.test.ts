  ...
  it("parses child run IDs and repair flags", () => {
    expect(
      parseReleaseVerifyBetaArgs([
        ...
        "--workflow-ref",
        "release/2026.5.10",
        "--orirohub-workflow-ref",
        "v2026.5.10-beta.3",
        "--plugins",
        "@oriro/plugin-a,@oriro/plugin-b",
      ...
      oriroHubWorkflowRef: "v2026.5.10-beta.3",
      pluginSelection: ["@oriro/plugin-a", "@oriro/plugin-b"],
      evidenceOut: ".artifacts/release-evidence.json",
      skipPostpublish: true,
      skipGitHubRelease: true,
      skipOriroHub: true,
      rerunFailedOriroHub: true,
      workflowRuns: {
        fullReleaseValidation: "10",
        oriroNpm: "11",
        pluginNpm: "22",
        pluginOriroHub: "33",
        pluginOriroHubBootstrap: "34",
        npmTelegram: "44",
      },
    });
  });
    ...
    expect(
      parseNpmViewFields(
        JSON.stringify({
          ...
          "dist.tarball": "https://registry.example/oriro.tgz",
        }),
        "beta",
      ),
    ).toEqual({
      ...
      tarball: "https://registry.example/oriro.tgz",
    });
  });
...
describe("runNpmViewWithRetry", () => {
  it("retries transient registry failures with online metadata reads", async () => {
    ...
    await expect(
      runNpmViewWithRetry(["view", "oriro@2026.5.10-beta.3", "version", "--json"], {
        attempts: 3,
        delay: async (delayMs) => {
          delays.push(delayMs);
        },
...
describe("fetchStatusWithRetry", () => {
  it("cancels retryable and returned GET response bodies", async () => {
    ...
    const status = fetchStatusWithRetry("https://orirohub.test/api/v1/package", "GET");
...
describe("readBoundedJsonResponse", () => {
  it("parses JSON bodies within the release verifier limit", async () => {
    await expect(
      readBoundedJsonResponse(new Response('{"ok":true}'), "OriroHub package", 64),
  ...
  it("rejects oversized JSON bodies by content length", async () => {
    ...
    ).rejects.toThrow("OriroHub package response body exceeded 64 bytes.");
  });

  it("rejects oversized streamed JSON bodies", async () => {
    await expect(
      readBoundedJsonResponse(new Response('{"padding":"too-large"}'), "OriroHub package", 8),
    ).rejects.toThrow("OriroHub package response body exceeded 8 bytes.");
  });

  it("keeps OriroHub request timeouts active while reading JSON bodies", async () => {
    let canceled = false;
    ...
    const json = readBoundedJsonResponse(response, "OriroHub package", 64, {
    ...
    abortController.abort(new Error("OriroHub body timed out"));

    await expect(json).rejects.toThrow("OriroHub body timed out");
