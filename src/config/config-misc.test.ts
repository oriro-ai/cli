...
import { buildWebSearchProviderConfig, withTempHome, writeOriroConfig } from "./test-helpers.js";
...
describe("boolean config validation", () => {
  it.each(nonBooleanConfigCases)("rejects non-boolean values for $name", ({ config }) => {
    const result = OriroSchema.safeParse(config);
...
describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    ...
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://oriro.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = OriroSchema.safeParse({ $schema: 123 });
    expect(result.success).toBe(false);
  });
  ...
  it("preserves $schema through validateConfigObject round-trip", () => {
    ...
    if (res.ok) {
      expect(res.config.$schema).toBe("https://oriro.ai/config.json");
    }
  });
});
...
describe("diagnostics.otel.captureContent", () => {
  it("accepts supported OTEL log exporters and rejects unknown values", () => {
    ...
    const invalid = OriroSchema.safeParse({
      diagnostics: {
        otel: {
          logs: true,
          logsExporter: "stderr",
...
describe("config identity/materialization regressions", () => {
  it("keeps explicit responsePrefix and group mention patterns", () => {
    ...
    if (res.ok) {
      ...
      expect(res.config.agents?.list?.[0]?.groupChat?.mentionPatterns).toEqual(["@oriro"]);
    }
  });
...
describe("config strict validation", () => {
  ...
  it("rejects resolved-only gateway.bind aliases as invalid schema values, not legacy", async () => {
    await withTempHome(async (home) => {
      await writeOriroConfig(home, {
        gateway: { bind: "${ORIRO_BIND}" },
      });

      const prev = process.env.ORIRO_BIND;
      process.env.ORIRO_BIND = "0.0.0.0";
      try {
        const snap = await readConfigFileSnapshot();
        expect(snap.valid).toBe(false);
        ...
        if (prev === undefined) {
          delete process.env.ORIRO_BIND;
        } else {
          process.env.ORIRO_BIND = prev;
        }
      }
    });
  });
