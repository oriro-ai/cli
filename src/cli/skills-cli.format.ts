// Formatting layer for `oriro skills` commands; keeps discovery data separate from terminal UI.
...
function appendOriroHubHint(output: string, json?: boolean): string {
  if (json) {
    return output;
  }
  return `${output}\n\nTip: use \`oriro skills search\`, \`oriro skills install\`, and \`oriro skills update\` for OriroHub-backed skills.`;
}
...
export function formatSkillsList(report: SkillStatusReport, opts: SkillsListOptions): string {
  ...
  if (skills.length === 0) {
    const message = opts.eligible
      ? `No eligible skills found. Run \`${formatCliCommand("oriro skills list")}\` to see all skills.`
      : "No skills found.";
    return appendOriroHubHint(message, opts.json);
  }
  ...
  return appendOriroHubHint(lines.join("\n"), opts.json);
}
...
export function formatSkillInfo(
  ...
  if (!skill) {
    ...
    return appendOriroHubHint(
      `Skill "${safeRequestedName}" not found. Run \`${formatCliCommand("oriro skills list")}\` to see available skills.`,
      opts.json,
    );
  }
