...
function parseExternalCatalogChannelEntries(raw: unknown): ExternalCatalogChannelEntry[] {
  ...
  for (const entry of list) {
    if (!isRecord(entry) || !isRecord(entry.oriro) || !isRecord(entry.oriro.channel)) {
      continue;
    }
    const channel = entry.oriro.channel;
...
export function shouldSkipPreferredPluginAutoEnable(params: {
  ...
  env: NodeJS.ProcessEnv;
  registry: PluginManifestRegistry;
  isPluginDenied: (config: OriroConfig, pluginId: string) => boolean;
  isPluginExplicitlyDisabled: (config: OriroConfig, pluginId: string) => boolean;
