...
describe("buildStatusMessage", () => {
  it("summarizes agent readiness and context usage", () => {
    ...
    expect(normalized).toContain("Oriro");
    ...
    expect(normalized).toContain("Runtime: Oriro Default");
  ...
  it("shows the default Oriro harness as the model runtime", () => {
    const text = buildStatusMessage({
      agent: {
        model: "openai/gpt-5.4",
      },
      sessionEntry: {
        sessionId: "oriro-harness",
        updatedAt: 0,
        fastMode: true,
      },
      sessionKey: "agent:main:main",
    ...
    expect(normalized).toContain("Fast");
    expect(normalized).toContain("Runtime: Oriro Default");
    expect(normalized).not.toContain("· oriro");
  });
