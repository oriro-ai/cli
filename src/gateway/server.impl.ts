...
function createGatewayStartupTrace() {
  const logEnabled = isTruthyEnvValue(process.env.ORIRO_GATEWAY_STARTUP_TRACE);
  let timelineConfig: OriroConfig | undefined;
  ...
  const eventLoopTimelineEnabled = () =>
    ...
    isTruthyEnvValue(process.env.ORIRO_DIAGNOSTICS_EVENT_LOOP);
...
function formatRuntimeGatewayAuthTokenWarning(): string {
  ...
  if (!isNixMode) {
    return `${base} Persist one with \`oriro config set gateway.auth.mode token\` and \`oriro config set gateway.auth.token <token>\`.`;
  }
  return [
    base,
...
export type GatewayServerOptions = {
   ...
   * reparsing oriro.json during server startup.
   */
...
export async function startGatewayServer(
  ...
  const minimalTestGateway =
    isVitestRuntimeEnv() && process.env.ORIRO_TEST_MINIMAL_GATEWAY === "1";

  // Ensure all default port derivations (browser/canvas) see the actual runtime port.
  process.env.ORIRO_GATEWAY_PORT = String(port);
  logAcceptedEnvOption({
    key: "ORIRO_RAW_STREAM",
    description: "raw stream logging enabled",
  });
  logAcceptedEnvOption({
    key: "ORIRO_RAW_STREAM_PATH",
  ...
  const resolveSharedGatewaySessionGenerationForConfig = (config: OriroConfig) =>
    ...
    const gatewayRequestContext = await startupTrace.measure(
      ...
      async () => {
        ...
        return createGatewayRequestContext({
          ...
          clients,
          enforceSharedGatewayAuthGenerationForConfigWrite: (nextConfig: OriroConfig) => {
            enforceSharedGatewaySessionGenerationForConfigWrite({
