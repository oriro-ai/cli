...
struct CommandCenterTab: View {
    ...
    @State private var defaultChatSessionEntry: OriroChatSessionEntry?
    @State private var recentChatSessions: [OriroChatSessionEntry] = []
    var headerTitle: String = "Oriro"
    var headerLeadingAction: OriroSidebarHeaderAction?
        ...
        NavigationStack {
            GeometryReader { geometry in
                ZStack {
                    ...
                    .safeAreaPadding(.bottom, OriroProMetric.bottomScrollInset)
                }
            }
    ...
    private var header: some View {
            ...
            subtitleLineLimit: 1)
        {
            ...
            {
                OriroProMark(size: 28, shadowRadius: 5)
            }
        } accessory: {
        ...
        HStack(spacing: 8) {
            ...
            if let badgeValue {
                Text(badgeValue)
                    ...
                    .padding(.vertical, 3)
                    .background(OriroBrand.accentHot, in: Capsule())
            }
            Spacer(minLength: 8)
            if let value {
                if let action {
    ...
    private var gatewayStatusColor: Color {
        self.gatewayConnected ? OriroBrand.ok : .secondary
    }
    ...
    private var defaultChatWorkItem: WorkItem {
        ...
        return WorkItem(
            ...
            trailing: "chat",
            color: isOpen ? OriroBrand.accent : OriroBrand.ok,
            progress: nil,
            route: .chat(nil))
    }
    ...
    private static func sessionChoices(
        _ sessions: [OriroChatSessionEntry],
        currentSessionKey: String,
        defaultSessionKey: String) -> [OriroChatSessionEntry]
    {
    ...
    static func sessionWorkItem(
        for session: OriroChatSessionEntry,
        currentSessionKey: String) -> WorkItem
    {
        ...
        return WorkItem(
            ...
            trailing: "chat",
            color: isCurrent ? OriroBrand.accent : OriroBrand.ok,
            progress: nil,
            route: .chat(session.key))
    }

    fileprivate static func sessionTitle(_ session: OriroChatSessionEntry) -> String {
    ...
    fileprivate static func sessionDetail(_ session: OriroChatSessionEntry) -> String {
...
struct CommandSessionsScreen: View {
    ...
    @State private var sessions: [OriroChatSessionEntry] = []
    @State private var isLoading = false
    @State private var loadErrorText: String?
    let headerLeadingAction: OriroSidebarHeaderAction?
    let openChat: () -> Void

    init(headerLeadingAction: OriroSidebarHeaderAction? = nil, openChat: @escaping () -> Void) {
