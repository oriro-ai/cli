...
async function writeDefaultConfigFile(config: OriroConfig): Promise<void> {
...
async function readConfigFileRaw(configPath: string): Promise<{
  ...
  try {
    ...
    return { exists: true, parsed: (parsed ?? {}) as OriroConfig };
  } catch {
...
export async function setupCommand(
  ...
  runtime.log(`Next guided path: ${formatCliCommand("oriro onboard")}.`);
  runtime.log(
    `Next targeted changes: ${formatCliCommand("oriro configure")} for models, channels, Gateway, plugins, skills, and health checks.`,
  );
  runtime.log(`Add a chat channel later: ${formatCliCommand("oriro channels add")}.`);
}
