---
summary: "CLI reference for `oriro webhooks` (Gmail Pub/Sub setup and runner)"
read_when:
  - You want to wire Gmail Pub/Sub events into Oriro
...
```bash
oriro webhooks gmail setup --account <email> [...]
oriro webhooks gmail run   [--account <email>] [...]
```

| Subcommand    | Description                                                                                  |
| ------------- | -------------------------------------------------------------------------------------------- |
| `gmail setup` | Configure Gmail watch, Pub/Sub topic/subscription, and the Oriro webhook delivery target. |
| `gmail run`   | Run `gog watch serve` plus the watch auto-renew loop.                                        |

## `webhooks gmail setup`

Configure Gmail watch, Pub/Sub, and Oriro webhook delivery.

```bash
oriro webhooks gmail setup --account you@example.com
oriro webhooks gmail setup --account you@example.com --project my-gcp-project --json
oriro webhooks gmail setup --account you@example.com --hook-url https://gateway.example.com/hooks/gmail
```
...
### Oriro delivery options

| Flag                   | Default | Description                                |
| ---------------------- | ------- | ------------------------------------------ |
| `--hook-url <url>`     | (none)  | Oriro webhook URL.                      |
| `--hook-token <token>` | (none)  | Oriro webhook token.                    |
...
```bash
oriro webhooks gmail run --account you@example.com
```

`run` accepts the same `gog watch serve`, Oriro delivery, Pub/Sub, and Tailscale flags as `setup`, except:
...
| Oriro delivery | `--hook-url`, `--hook-token`, `--push-token`                                     |
