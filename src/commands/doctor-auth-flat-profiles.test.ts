...
async function makeTestState(): Promise<OriroTestState> {
  const state = await createOriroTestState({
    layout: "state-only",
    prefix: "oriro-doctor-flat-auth-",
    env: {
...
describe("maybeMigrateAuthProfileJsonStoresToSqlite", () => {
  ...
  it("infers config credential provider and mode before stripping config", async () => {
    const cases: Array<{ profileId: string; cfg: OriroConfig; now: number }> = [
      {
...
describe("maybeRepairOpenAICodexAuthConfig", () => {
  it("renames legacy OpenAI Codex config profiles and merges auth order", () => {
    ...
    const migrated = result.config as OriroConfig & {
      agents?: {
        defaults?: {
