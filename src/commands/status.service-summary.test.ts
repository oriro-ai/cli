...
describe("readServiceStatusSummary", () => {
  it("marks Oriro-managed services as installed", async () => {
    const summary = await readServiceStatusSummary(
      createService({
        isLoaded: vi.fn(async () => true),
        readCommand: vi.fn(async () => ({ programArguments: ["oriro", "gateway", "run"] })),
    ...
    expect(summary.managedByOriro).toBe(true);
  ...
  it("marks running unmanaged services as externally managed", async () => {
    ...
    expect(summary.installed).toBe(true);
    expect(summary.managedByOriro).toBe(false);
  ...
  it("passes command environment to runtime and loaded checks", async () => {
    const isLoaded = vi.fn(async ({ env }: GatewayServiceEnvArgs) => {
      return env?.ORIRO_GATEWAY_PORT === "18789";
    });
    const readRuntime = vi.fn(async (env?: NodeJS.ProcessEnv) => ({
      status: env?.ORIRO_GATEWAY_PORT === "18789" ? ("running" as const) : ("unknown" as const),
    }));
    ...
    const summary = await readServiceStatusSummary(
      createService({
        ...
        readCommand: vi.fn(async () => ({
          programArguments: ["oriro", "gateway", "run", "--port", "18789"],
    ...
    expect(loadedArgs?.env?.ORIRO_GATEWAY_PORT).toBe("18789");
    const runtimeEnv = requireMockArg(readRuntime, "readRuntime") as NodeJS.ProcessEnv;
    expect(runtimeEnv?.ORIRO_GATEWAY_PORT).toBe("18789");
    expect(summary.installed).toBe(true);
  ...
  it("includes service layout diagnostics and flags source checkout entrypoints", async () => {
    await withTempDir({ prefix: "oriro-status-service-layout-" }, async (root) => {
      ...
      const serviceFile = path.join(root, "oriro-gateway.service");
      ...
      expect(layout.sourcePathReal).toBe(path.join(realRoot, "oriro-gateway.service"));
