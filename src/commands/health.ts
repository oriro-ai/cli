...
const debugHealth = (...args: unknown[]) => {
  if (isTruthyEnvValue(process.env.ORIRO_DEBUG_HEALTH)) {
...
const resolveHeartbeatSummary = (cfg: OriroConfig, agentId: string) =>
  resolveHeartbeatSummaryForAgent(cfg, agentId);

const resolveAgentOrder = (cfg: OriroConfig) => {
...
/** Runs the `oriro health` command against the gateway and renders JSON or text. */
export async function healthCommand(
  ...
  if (opts.json) {
    ...
    const debugEnabled = isTruthyEnvValue(process.env.ORIRO_DEBUG_HEALTH);
    const rich = isRich();
    if (opts.verbose) {
...
async function readBestEffortHealthConfig(): Promise<OriroConfig> {
  const { readBestEffortConfig } = await loadConfigRuntime();
  return await readBestEffortConfig();
}

async function readRuntimeHealthConfig(): Promise<OriroConfig> {
