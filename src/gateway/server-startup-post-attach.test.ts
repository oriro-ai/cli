...
const hoisted = vi.hoisted(() => {
  ...
  const ensureOriroModelsJson = vi.fn(async () => {});
...
vi.mock("../agents/models-config.js", () => ({
  ensureOriroModelsJson: hoisted.ensureOriroModelsJson,
}));
...
describe("startGatewayPostAttachRuntime", () => {
  beforeEach(() => {
    ...
    hoisted.ensureOriroModelsJson.mockReset();
    hoisted.ensureOriroModelsJson.mockResolvedValue(undefined);
  ...
  it("defers qmd memory backend startup refresh until the idle delay elapses", async () => {
    ...
    try {
      await startGatewaySidecars({
        ...
        defaultWorkspaceDir: "/tmp/oriro-workspace",
        deps: {} as never,
  ...
  it("cleans startup session locks with bounded concurrency", async () => {
    ...
    let maxActive = 0;
    const cleanedLock = {
      lockPath: "/tmp/oriro-state/agents/main/sessions/a.jsonl.lock",
      pid: null,
      pidAlive: false,
      createdAt: null,
      ageMs: null,
  ...
  it("starts channels when channel startup is enabled", async () => {
    await withEnvAsync(
      {
        ORIRO_SKIP_CHANNELS: undefined,
        ORIRO_SKIP_PROVIDERS: undefined,
      },
      async () => {
  ...
  it("starts and reports plugin services after channel startup completes", async () => {
    await withEnvAsync(
      { ORIRO_SKIP_CHANNELS: undefined, ORIRO_SKIP_PROVIDERS: undefined },
      async () => {
  ...
  it("emits a startup trace span when channel startup is skipped", async () => {
    ...
    await withEnvAsync(
      { ORIRO_SKIP_CHANNELS: "1", ORIRO_SKIP_PROVIDERS: undefined },
      async () => {
        await startGatewaySidecars({
          cfg: { hooks: { internal: { enabled: false } } } as never,
          pluginRegistry: createPostAttachParams().pluginRegistry,
