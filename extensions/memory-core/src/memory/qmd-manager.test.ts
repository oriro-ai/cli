...
import { withMockedWindowsPlatform } from "oriro/plugin-sdk/test-node-mocks";
...
const MEMORY_EMBEDDING_PROVIDERS_KEY = Symbol.for("oriro.memoryEmbeddingProviders");
const MCPORTER_STATE_KEY = Symbol.for("oriro.mcporterState");
const QMD_EMBED_QUEUE_KEY = Symbol.for("oriro.qmdEmbedQueueTail");
...
vi.mock("oriro/plugin-sdk/memory-core-host-engine-foundation", async () => {
  const actual = await vi.importActual<
    typeof import("oriro/plugin-sdk/memory-core-host-engine-foundation")
  >("oriro/plugin-sdk/memory-core-host-engine-foundation");
  return {
    ...actual,
...
vi.mock("oriro/plugin-sdk/file-lock", async () => {
  const actual = await vi.importActual<typeof import("oriro/plugin-sdk/file-lock")>(
    "oriro/plugin-sdk/file-lock",
  );
  return {
    ...actual,
...
import type { OriroConfig } from "oriro/plugin-sdk/memory-core-host-engine-foundation";
import {
  requireNodeSqlite,
...
describe("QmdMemoryManager", () => {
  ...
  it("rebinds a stale in-container collection root to the host workspace (sandbox-mode transition)", async () => {
    ...
    // /home/node/.oriro/... stored that in-container path as the collection root. Resolved
    ...
    const containerRoot = "/home/node/.oriro/teams/x/workspace";
  ...
  it("parseShownCollection extracts path and pattern from qmd collection show output", async () => {
    ...
    const sampleOutput = [
      ...
      "  Path:     /home/node/.oriro/teams/example-team/workspace-example/memory",
      "  Pattern:  **/*.md",
      "  Include:  yes (default)",
    ].join("\n");

    const result = parser(sampleOutput);
    expect(result.path).toBe("/home/node/.oriro/teams/example-team/workspace-example/memory");
