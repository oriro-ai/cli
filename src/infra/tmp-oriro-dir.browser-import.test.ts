...
describe("tmp-oriro-dir browser-safe import", () => {
  it("loads when a browser fs shim omits constants", async () => {
    const resultKey = `__oriroTmpDirBrowserImport_${crypto.randomUUID().replaceAll("-", "_")}`;
    ...
    const bundled = await build({
      ...
      stdin: {
        contents: `
          import { POSIX_ORIRO_TMP_DIR, resolvePreferredOriroTmpDir } from "./src/infra/tmp-oriro-dir.ts";
          globalThis.${resultKey} = {
            posixTmpDir: POSIX_ORIRO_TMP_DIR,
            resolverType: typeof resolvePreferredOriroTmpDir,
          };
        `,
        loader: "ts",
        resolveDir: process.cwd(),
        sourcefile: "tmp-oriro-dir-browser-entry.ts",
      },
      write: false,
    });
