...
function isOriroLoopbackMcpServer(name: string, server: BundleMcpServerConfig): boolean {
  return (
    name === "oriro" &&
...
export function normalizeCodexMcpServerConfig(
  ...
  } else if (isOriroLoopbackMcpServer(name, server)) {
    // Oriro's loopback MCP exposes local tools; Codex should ask for approval
