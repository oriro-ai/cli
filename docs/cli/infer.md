...
read_when:
  - Adding or modifying `oriro infer` commands
  - Designing stable headless capability automation
title: "Inference CLI"
---

`oriro infer` is the canonical headless surface for provider-backed inference workflows.
...
```text
Read https://docs.oriro.ai/cli/infer, then create a skill that routes my common workflows to `oriro infer`.
...
- prefer `oriro infer ...` in examples and suggestions
...
- `oriro infer audio transcribe`
- `oriro infer tts convert`
- `oriro infer web search`
- `oriro infer embedding create`

## Why use infer

`oriro infer` provides one consistent CLI for provider-backed inference tasks inside Oriro.

Benefits:

- Use the providers and models already configured in Oriro instead of wiring up one-off wrappers for each backend.
...
- Prefer a first-party Oriro surface when the task is fundamentally "run inference."
- Use the normal local path without requiring the gateway for most infer commands.

For end-to-end provider checks, prefer `oriro infer ...` once lower-level
...
| Task                          | Command                                                                                       | Notes                                                 |
| ----------------------------- | --------------------------------------------------------------------------------------------- | ----------------------------------------------------- |
| Run a text/model prompt       | `oriro infer model run --prompt "..." --json`                                              | Uses the normal local path by default                 |
| Run a model prompt on images  | `oriro infer model run --prompt "Describe this" --file ./image.png --model provider/model` | Repeat `--file` for multiple image inputs             |
| Generate an image             | `oriro infer image generate --prompt "..." --json`                                         | Use `image edit` when starting from an existing file  |
| Describe an image file or URL | `oriro infer image describe --file ./image.png --prompt "..." --json`                      | `--model` must be an image-capable `<provider/model>` |
| Transcribe audio              | `oriro infer audio transcribe --file ./memo.m4a --json`                                    | `--model` must be `<provider/model>`                  |
| Synthesize speech             | `oriro infer tts convert --text "..." --output ./speech.mp3 --json`                        | `tts status` is gateway-oriented                      |
| Generate a video              | `oriro infer video generate --prompt "..." --json`                                         | Supports provider hints such as `--resolution`        |
| Describe a video file         | `oriro infer video describe --file ./clip.mp4 --json`                                      | `--model` must be `<provider/model>`                  |
| Search the web                | `oriro infer web search --query "..." --json`                                              |                                                       |
| Fetch a web page              | `oriro infer web fetch --url https://example.com --json`                                   |                                                       |
| Create embeddings             | `oriro infer embedding create --text "..." --json`                                         |                                                       |

## Behavior

- `oriro infer ...` is the primary CLI surface for these workflows.
...
```bash
oriro infer model run --prompt "Reply with exactly: smoke-ok" --json
oriro infer model run --prompt "Summarize this changelog entry" --model openai/gpt-5.4 --json
oriro infer model run --prompt "Describe this image in one sentence" --file ./photo.jpg --model google/gemini-2.5-flash --json
oriro infer model run --prompt "Use more reasoning here" --thinking high --json
oriro infer model providers --json
oriro infer model inspect --name gpt-5.5 --json
```
...
```bash
oriro infer model run --local --model anthropic/claude-sonnet-4-6 --prompt "Reply with exactly: pong" --json
oriro infer model run --local --model cerebras/zai-glm-4.7 --prompt "Reply with exactly: pong" --json
oriro infer model run --local --model google/gemini-2.5-flash --prompt "Reply with exactly: pong" --json
oriro infer model run --local --model groq/llama-3.1-8b-instant --prompt "Reply with exactly: pong" --json
oriro infer model run --local --model mistral/mistral-medium-3-5 --prompt "Reply with exactly: pong" --json
oriro infer model run --local --model mistral/mistral-small-latest --prompt "Reply with exactly: pong" --json
oriro infer model run --local --model openai/gpt-5.5 --prompt "Reply with exactly: pong" --json
oriro infer model run --local --model ollama/qwen2.5vl:7b --prompt "Describe this image." --file ./photo.jpg --json
```
...
- Codex Responses local probes are the narrow exception: Oriro adds a minimal system instruction so the transport can populate its required `instructions` field, without adding full agent context, tools, memory, or session transcript.
...
- `model run --file` is best when you want to test the selected multimodal text model directly. Use `infer image describe` when you want Oriro's image-understanding provider selection and default image-model routing.
...
- Use `model run --gateway` when you need to test Gateway routing, agent-runtime setup, or Gateway-managed provider state while keeping the model input raw. Use `oriro agent` or chat surfaces when you want the full agent context, tools, memory, and session transcript.
...
```bash
oriro infer image generate --prompt "friendly oriro illustration" --json
oriro infer image generate --prompt "cinematic product photo of headphones" --json
oriro infer image generate --model openai/gpt-image-1.5 --output-format png --background transparent --prompt "simple red circle sticker on a transparent background" --json
oriro infer image generate --model openai/gpt-image-2 --quality low --openai-moderation low --prompt "low-cost draft poster" --json
oriro infer image generate --prompt "slow image backend" --timeout-ms 180000 --json
oriro infer image edit --file ./logo.png --model openai/gpt-image-1.5 --output-format png --background transparent --prompt "keep the logo, remove the background" --json
oriro infer image edit --file ./poster.png --prompt "make this a vertical story ad" --size 2160x3840 --aspect-ratio 9:16 --resolution 4K --json
oriro infer image describe --file ./photo.jpg --json
oriro infer image describe --file https://example.com/photo.png --json
oriro infer image describe --file ./receipt.jpg --prompt "Extract the merchant, date, and total" --json
oriro infer image describe-many --file ./before.png --file ./after.png --prompt "Compare the screenshots and list visible UI changes" --json
oriro infer image describe --file ./ui-screenshot.png --model openai/gpt-5.4-mini --json
oriro infer image describe --file ./photo.jpg --model ollama/qwen2.5vl:7b --prompt "Describe the image in one sentence" --timeout-ms 300000 --json
```
...
```bash
oriro infer audio transcribe --file ./memo.m4a --json
oriro infer audio transcribe --file ./team-sync.m4a --language en --prompt "Focus on names and action items" --json
oriro infer audio transcribe --file ./memo.m4a --model openai/whisper-1 --json
```
...
```bash
oriro infer tts convert --text "hello from oriro" --output ./hello.mp3 --json
oriro infer tts convert --text "Your build is complete" --output ./build-complete.mp3 --json
oriro infer tts providers --json
oriro infer tts status --json
```
...
```bash
oriro infer video generate --prompt "cinematic sunset over the ocean" --json
oriro infer video generate --prompt "slow drone shot over a forest lake" --resolution 768P --duration 6 --json
oriro infer video describe --file ./clip.mp4 --json
oriro infer video describe --file ./clip.mp4 --model openai/gpt-5.4-mini --json
```
...
```bash
oriro infer web search --query "Oriro docs" --json
oriro infer web search --query "Oriro infer web providers" --json
oriro infer web fetch --url https://docs.oriro.ai/cli/infer --json
oriro infer web providers --json
```
...
```bash
oriro infer embedding create --text "friendly oriro" --json
oriro infer embedding create --text "customer support ticket: delayed shipment" --model openai/text-embedding-3-large --json
oriro infer embedding providers --json
```
...
For generated media commands, `outputs` contains files written by Oriro. Use
...
```bash
# Bad
oriro infer media image generate --prompt "friendly oriro"

# Good
oriro infer image generate --prompt "friendly oriro"
```

```bash
# Bad
oriro infer audio transcribe --file ./memo.m4a --model whisper-1 --json
...
- `oriro capability ...` is an alias for `oriro infer ...`.
