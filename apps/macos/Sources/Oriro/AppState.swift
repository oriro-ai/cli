...
final class AppState {
    private static let logger = Logger(subsystem: "ai.oriro", category: "app-state")
    ...
    init(preview: Bool = false) {
        ...
        let configRoot = OriroConfigFile.loadDict()
    ...
    private static func sshTunnelGatewayUrl(existingUrl: String?, expectedRemoteHost: String?) -> String {
        ...
        let preservePort: Bool = if LoopbackHost.isLoopbackHost(host) {
            ...
            OriroConfigFile.canonicalHostForComparison(host) ==
                OriroConfigFile.canonicalHostForComparison(expectedRemoteHost)
        } else {
            false
        }
    ...
    private func startConfigWatcher() {
        let configUrl = OriroConfigFile.url()
    ...
    func syncGatewayConfigNow() {
        ...
        let synced = Self.syncedGatewayRoot(
            currentRoot: OriroConfigFile.loadDict(),
            draft: .init(
        ...
        guard OriroConfigFile.saveDict(synced.root) else {
...
extension AppState {
    static var preview: AppState {
        ...
        state.remoteProjectRoot = "~/Projects/oriro"
        state.remoteCliPath = ""
        return state
    }
}
