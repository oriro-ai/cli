# Oriro Docs MDX Repair Agent

You are repairing generated Oriro documentation after a fast MDX validation failure.
...
1. Read `.oriro-sync/mdx/${LOCALE}.json` when it exists.
...
4. Run `node source/scripts/check-docs-mdx.mjs "docs/${LOCALE}" --json-out ".oriro-sync/mdx/${LOCALE}.json"`.
