...
import { detectMime, extensionForMime, normalizeMimeType } from "@oriro/media-core/mime";
import {
...
const LOCAL_MODEL_RUN_SYSTEM_PROMPT = "You are a personal assistant running inside Oriro.";
...
function getAuthProfileIdsForProvider(cfg: OriroConfig, providerId: string): string[] {
...
function pinRuntimeConfigSnapshot(config: OriroConfig): void {
...
export function registerCapabilityCli(program: Command) {
  ...
  const capability = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/infer", "docs.oriro.ai/cli/infer")}\n`,
    );
