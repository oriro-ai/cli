...
Oriro agents can generate videos from text prompts, reference images, or
...
Oriro treats video generation as three runtime modes:
...
<Steps>
  ...
  <Step title="Pick a default model (optional)">
    ```bash
    oriro config set agents.defaults.videoGenerationModel.primary "google/veo-3.1-fast-generate-preview"
    ```
  </Step>
  <Step title="Ask the agent">
    > Generate a 5-second cinematic video of a friendly oriro surfing at sunset.
...
1. Oriro submits the request to the provider and immediately returns a task id.
...
3. When the video is ready, Oriro wakes the same session with an internal completion event.
4. The agent tells the user through the session's normal visible-reply mode:
   ...
   completion reply, Oriro sends an idempotent direct fallback with only the
   missing video.
...
generation. Use `oriro tasks list` or `oriro tasks show <taskId>` to
...
Generated video files are saved under Oriro-managed media storage when
...
larger renders. When a provider also returns a hosted output URL, Oriro
...
```bash
oriro tasks list
oriro tasks show <taskId>
oriro tasks cancel <taskId>
```
...
<ParamField path="aspectRatio" type="string">
  Aspect-ratio hint such as `1:1`, `16:9`, `9:16`, `adaptive`, or a provider-specific value. Oriro normalizes or ignores unsupported values per provider.
</ParamField>
<ParamField path="resolution" type="string">Resolution hint such as `480P`, `720P`, `768P`, `1080P`, `4K`, or a provider-specific value. Oriro normalizes or ignores unsupported values per provider.</ParamField>
...
<ParamField path="timeoutMs" type="number">Optional provider operation timeout in milliseconds. When omitted, Oriro uses `agents.defaults.videoGenerationModel.timeoutMs` if configured, otherwise the plugin-authored provider default when one exists.</ParamField>
...
<Note>
Not all providers support all parameters. Oriro normalizes duration to
...
Oriro resolves the model in this order:
...
<AccordionGroup>
  ...
  <Accordion title="BytePlus Seedance 1.5">
    Requires the [`@oriro/byteplus-modelark`](https://www.npmjs.com/package/@oriro/byteplus-modelark)
  ...
  <Accordion title="fal">
    Uses a queue-backed flow for long-running jobs. Oriro waits up to 20
  ...
  <Accordion title="OpenRouter">
    Uses OpenRouter's asynchronous `/videos` API. Oriro submits the
...
```bash
ORIRO_LIVE_TEST=1 pnpm test:live -- extensions/video-generation-providers.live.test.ts
```
...
- One-second oriro prompt.
- Per-provider operation cap from
  `ORIRO_LIVE_VIDEO_GENERATION_TIMEOUT_MS` (`180000` by default).
...
Set `ORIRO_LIVE_VIDEO_GENERATION_FULL_MODES=1` to also run declared
...
Set the default video-generation model in your Oriro config:
...
```bash
oriro config set agents.defaults.videoGenerationModel.primary "qwen/wan2.6-t2v"
```
