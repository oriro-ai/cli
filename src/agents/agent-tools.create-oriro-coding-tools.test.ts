/**
 * Broad coverage for createOriroCodingTools.
...
import "./test-helpers/fast-oriro-tools.js";
...
function expectNoSubagentControlTools(tools: ReturnType<typeof createOriroCodingTools>) {
...
type OriroCodingTool = ReturnType<typeof createOriroCodingTools>[number];
type OriroToolsOptions = NonNullable<Parameters<typeof createOriroTools>[0]>;

function toolNameList(tools: readonly { name: string }[]): string[] {
  return tools.map((tool) => tool.name);
}

function requireTool(tools: OriroCodingTool[], name: string): OriroCodingTool {
...
function requireToolExecute(tool: OriroCodingTool): NonNullable<OriroCodingTool["execute"]> {
  if (!tool.execute) {
...
function latestCreateOriroToolsOptions(): OriroToolsOptions {
  const calls = vi.mocked(createOriroTools).mock.calls;
  const lastCall = calls.at(-1);
  const options = lastCall?.[0];
  if (!options) {
    throw new Error("expected createOriroTools call");
  }
  return options;
}
...
function cronCreatorToolNames(
  list: OriroToolsOptions["cronCreatorToolAllowlist"] | undefined,
): string[] | undefined {
  return list?.map((entry) => (typeof entry === "string" ? entry : entry.name));
}

describe("createOriroCodingTools", () => {
  const testConfig: OriroConfig = {};
  ...
  it("exposes gateway config and restart actions to owner sessions", () => {
    const tools = createOriroCodingTools({ config: testConfig });
  ...
  it("passes explicit hook channel ids to wrapped tool hooks", async () => {
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-hook-channel-"));
  ...
  it("re-wraps existing before_tool_call hooks once with the current context", async () => {
    ...
    vi.mocked(createOriroTools).mockReturnValueOnce([wrapped as never]);

    const tools = createOriroCodingTools({ agentId: "main", sessionId: "session-new" });
  ...
  it("keeps Tool Search controls when core Oriro tools are not materialized", () => {
    ...
    expect(createOriroToolsMock).not.toHaveBeenCalled();
    expect(names.has("tool_search_code")).toBe(true);
    expect(names.has("tool_search")).toBe(true);
    expect(names.has("tool_describe")).toBe(true);
    expect(names.has("tool_call")).toBe(true);
  ...
  it("uses runtime toolsAllow when materializing plugin tools", () => {
    ...
    const options = latestCreateOriroToolsOptions();
  ...
  it("preserves runtime allowlist groups containing message through restrictive profiles", () => {
    for (const runtimeToolAllowlist of [["group:messaging"], ["group:oriro"], ["*"]]) {
      const tools = createOriroCodingTools({
        config: { tools: { profile: "minimal" } },
  ...
  it("passes source reply delivery mode to Oriro tool construction", () => {
    const createOriroToolsMock = vi.mocked(createOriroTools);
    createOriroToolsMock.mockClear();
    ...
    expect(createOriroToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateOriroToolsOptions().sourceReplyDeliveryMode).toBe("message_tool_only");
  });
  ...
  it("passes plugin suppression into Oriro tool construction plans", () => {
    const createOriroToolsMock = vi.mocked(createOriroTools);
    createOriroToolsMock.mockClear();
    ...
    expect(createOriroToolsMock).toHaveBeenCalledTimes(1);
    expect(latestCreateOriroToolsOptions().disablePluginTools).toBe(true);
  });

  it("keeps plugin-only construction off the Oriro core factory", () => {
    const createOriroToolsMock = vi.mocked(createOriroTools);
    createOriroToolsMock.mockClear();
  ...
  it("passes explicit denylist entries to Oriro tool factory planning", () => {
    const createOriroToolsMock = vi.mocked(createOriroTools);
    createOriroToolsMock.mockClear();
    ...
    expect(createOriroToolsMock).toHaveBeenCalledTimes(1);
    expectListIncludes(latestCreateOriroToolsOptions().pluginToolDenylist, ["pdf"]);
  });

  it("passes inherited allowlist entries to Oriro plugin discovery", async () => {
    const createOriroToolsMock = vi.mocked(createOriroTools);
    createOriroToolsMock.mockClear();
    const agentId = `inherited-allow-${Date.now()}-${Math.random().toString(16).slice(2)}`;
    const storeTemplate = path.join(
      os.tmpdir(),
      `oriro-session-store-${agentId}-{agentId}.json`,
    );
    ...
    expect(createOriroToolsMock).toHaveBeenCalledTimes(1);
    expectListIncludes(latestCreateOriroToolsOptions().pluginToolAllowlist, [
      "custom_plugin_tool",
      "sessions_spawn",
    ]);
  });
  ...
  it("passes effective allow-list-restricted tool surface to spawned sessions", () => {
    ...
    expect(createOriroToolsMock).toHaveBeenCalledTimes(1);
    const inheritedAllow = latestCreateOriroToolsOptions().inheritedToolAllowlist;
  ...
  it("passes group-restricted tool surface to cron-created agent turns", () => {
    ...
    expect(createOriroToolsMock).toHaveBeenCalledTimes(1);
    const cronAllow = latestCreateOriroToolsOptions().cronCreatorToolAllowlist;
  ...
  it("lets embedded attempts refresh a caller-owned cron creator tool surface", () => {
    ...
    const cronCreatorToolAllowlistRef: NonNullable<
      OriroToolsOptions["cronCreatorToolAllowlist"]
    > = [];
  ...
  it("records core tool-prep stages for hot-path diagnostics", () => {
    ...
    expect(stages.indexOf("oriro-tools:test-helper")).toBeLessThan(
      stages.indexOf("oriro-tools"),
    );
    expect(stages.indexOf("schema-normalization")).toBeLessThan(stages.indexOf("tool-hooks"));
  });

  it("preserves action enums in normalized schemas", () => {
    const defaultTools = createOriroCodingTools({ config: testConfig });
  ...
  it("enforces apply_patch availability and canonical names across model/provider constraints", () => {
    ...
    const openAiTools = createOriroCodingTools({
      config: testConfig,
    ...
    const codexTools = createOriroCodingTools({
      config: testConfig,
      modelProvider: "openai",
      modelId: "gpt-5.4",
    });
    ...
    const disabledConfig: OriroConfig = {
      tools: {
        exec: {
    ...
    const disabledOpenAiTools = createOriroCodingTools({
    ...
    const anthropicTools = createOriroCodingTools({
      config: disabledConfig,
    ...
    const allowModelsConfig: OriroConfig = {
      tools: {
        exec: {
    ...
    const allowed = createOriroCodingTools({
    ...
    const denied = createOriroCodingTools({
      config: allowModelsConfig,
      modelProvider: "openai",
    ...
    const oauthTools = createOriroCodingTools({
      config: testConfig,
      modelProvider: "anthropic",
      modelAuthMode: "oauth",
    });
  ...
  it("does not expose provider-specific message tools", () => {
    const tools = createOriroCodingTools({ messageProvider: "discord" });
    const names = new Set(tools.map((tool) => tool.name));
  ...
  it("separates the canonical message provider from transport tool policy", () => {
    vi.mocked(createOriroTools).mockClear();
    ...
    expect(latestCreateOriroToolsOptions().agentChannel).toBe("discord");
  });
  ...
  it("uses stored spawnDepth to apply leaf tool policy for flat depth-2 session keys", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-depth-policy-"));
    try {
  ...
  it("applies subagent tool policy to ACP children spawned under a subagent envelope", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-acp-subagent-policy-"));
    try {
      const storeTemplate = path.join(tmpDir, "sessions-{agentId}.json");
      await writeSessionStore(storeTemplate, "main", {
  ...
  it("applies leaf tool policy for cross-agent subagent sessions when spawnDepth is missing", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-cross-agent-subagent-"));
    try {
      const storeTemplate = path.join(tmpDir, "sessions-{agentId}.json");
      await writeSessionStore(storeTemplate, "main", {
  ...
  it("keeps browser out of coding-profile subagents unless profile-stage alsoAllow adds it", () => {
    ...
    const codingSubagent = createOriroCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: baseConfig,
    });
    ...
    const subagentAllowOnly = createOriroCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: {
        ...baseConfig,
        tools: {
    ...
    const profileStageAlsoAllow = createOriroCodingTools({
      sessionKey: "agent:main:subagent:test",
      config: {
        ...baseConfig,
  ...
  it("can keep message available when a cron route needs it under the coding profile", () => {
    const codingTools = createOriroCodingTools({
      config: { tools: { profile: "coding" } },
    });
    expect(toolNameList(codingTools)).not.toContain("message");

    const cronTools = createOriroCodingTools({
      config: { tools: { profile: "coding" } },
      forceMessageTool: true,
    });
  ...
  it("can keep message available when a cron route needs it under a provider coding profile", () => {
    const providerProfileTools = createOriroCodingTools({
  ...
  it("removes unsupported JSON Schema keywords for Cloud Code Assist API compatibility", () => {
    const googleTools = createOriroCodingTools({
  ...
  it("applies xai model compat for direct Grok tool cleanup", () => {
    const xaiTools = createOriroCodingTools({
      modelProvider: "xai",
      modelCompat: {
  ...
  it("returns image-aware read metadata for images and text-only blocks for text files", async () => {
    const defaultTools = createOriroCodingTools();
    const readTool = requireTool(defaultTools, "read");
    const readExecute = requireToolExecute(readTool);

    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-read-"));
    try {
      ...
      const contents = "Hello from oriro read tool.";
  ...
  it("filters tools by sandbox policy", () => {
    const sandboxDir = path.join(os.tmpdir(), "oriro-sandbox");
    const sandbox = createAgentToolsSandboxContext({
      workspaceDir: sandboxDir,
      agentWorkspaceDir: path.join(os.tmpdir(), "oriro-workspace"),
    ...
    const tools = createOriroCodingTools({ sandbox });
  ...
  it("accepts canonical parameters for read/write/edit", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-canonical-"));
    try {
      const tools = createOriroCodingTools({ workspaceDir: tmpDir });
  ...
  it("roots memory flush append-only writes in the workspace when cwd differs", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-memory-workspace-"));
    const taskCwd = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-memory-cwd-"));
  ...
  it("rejects legacy alias parameters", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-legacy-alias-"));
    try {
      const tools = createOriroCodingTools({ workspaceDir: tmpDir });
      const { readTool, writeTool, editTool } = expectReadWriteEditTools(tools);
  ...
  it("rejects structured content blocks for write", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-structured-write-"));
    try {
      const tools = createOriroCodingTools({ workspaceDir: tmpDir });
      ...
      await expect(
        writeExecute("tool-structured-write", {
          ...
          content: [
            ...
            { type: "input_text", text: "const root = path.join(process.env.HOME, 'orirod');\n" },
          ],
        }),
  ...
  it("rejects structured edit payloads", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-structured-edit-"));
    try {
