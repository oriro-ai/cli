...
private let chatUILogger = Logger(subsystem: "ai.oriro", category: "OriroChatUI")

@MainActor
@Observable
// swiftlint:disable:next type_body_length
public final class OriroChatViewModel {
    public static let defaultModelSelectionID = "__default__"
    static let maxAttachmentBytes = 5_000_000

    public private(set) var messages: [OriroChatMessage] = []
    public var input: String = ""
    public private(set) var thinkingLevel: String
    public private(set) var thinkingLevelOptions: [OriroChatThinkingLevelOption]
    public private(set) var modelSelectionID: String = "__default__"
    public private(set) var modelChoices: [OriroChatModelChoice] = []
    public private(set) var isLoading = false
    public private(set) var isSending = false
    public private(set) var isAborting = false
    public var errorText: String?
    public var attachments: [OriroPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [OriroChatPendingToolCall] = []
    public private(set) var sessions: [OriroChatSessionEntry] = []
    private let transport: any OriroChatTransport
    private var sessionDefaults: OriroChatSessionsDefaults?
    ...
    private var pendingToolCallsById: [String: OriroChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(
        sessionKey: String,
        transport: any OriroChatTransport,
    ...
    public var sessionChoices: [OriroChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }
        let mainSessionKey = self.resolvedMainSessionKey

        var result: [OriroChatSessionEntry] = []
    ...
    private static let baseThinkingLevelOptions: [OriroChatThinkingLevelOption] = [
        OriroChatThinkingLevelOption(id: "off", label: "off"),
        OriroChatThinkingLevelOption(id: "minimal", label: "minimal"),
        OriroChatThinkingLevelOption(id: "low", label: "low"),
    ...
    public func removeAttachment(_ id: OriroPendingAttachment.ID) {
    ...
    private func applyHistoryPayload(
        _ payload: OriroChatHistoryPayload,
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [OriroChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: OriroChatMessage.self))
                .map { Self.stripInboundMetadata(from: $0) }
        }
        return Self.dedupeMessages(decoded)
    }

    private static func stripInboundMetadata(from message: OriroChatMessage) -> OriroChatMessage {
        guard message.role.lowercased() == "user" else {
            return message
        }

        let sanitizedContent = message.content.map { content -> OriroChatMessageContent in
            guard let text = content.text else { return content }
            let cleaned = ChatMarkdownPreprocessor.preprocess(markdown: text).cleaned
            return OriroChatMessageContent(
                type: content.type,
                text: cleaned,
    ...
    private static func messageContentFingerprint(for message: OriroChatMessage) -> String {
    ...
    private static func messageIdentityKey(for message: OriroChatMessage) -> String? {
    ...
    private static func userRefreshIdentityKey(for message: OriroChatMessage) -> String? {
        let role = message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
    ...
    private func adoptPendingLocalUserEcho(incoming: OriroChatMessage) -> Bool {
        ...
        updated[matchIndex] = OriroChatMessage(
            id: existing.id,
            role: incoming.role,
    ...
    private static func reconcileMessageIDs(
        previous: [OriroChatMessage],
        incoming: [OriroChatMessage]) -> [OriroChatMessage]
    {
    ...
    private static func reconcileRunRefreshMessages(
        previous: [OriroChatMessage],
        incoming: [OriroChatMessage],
        pendingLocalUserEchoIDs: Set<UUID>) -> [OriroChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [OriroChatMessage]) -> [OriroChatMessage] {
        var result: [OriroChatMessage] = []
    ...
    private static func dedupeKey(for message: OriroChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [OriroChatMessageContent] = [
            OriroChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> OriroChatAttachmentPayload in
            OriroChatAttachmentPayload(
                type: att.type,
    ...
    private static func isUnsupportedCreateSessionError(_ error: Error) -> Bool {
        ...
        return nsError.domain == "OriroChatTransport"
    ...
    private func resolvedThinkingLevelOptions(
        for currentSession: OriroChatSessionEntry?) -> [OriroChatThinkingLevelOption]
    {
    ...
    private static func sessionModelMatchesDefaults(
        _ session: OriroChatSessionEntry,
        defaults: OriroChatSessionsDefaults?) -> Bool
    {
    ...
    private static func normalizedThinkingLevelOptions(
        _ levels: [OriroChatThinkingLevelOption]?) -> [OriroChatThinkingLevelOption]?
    {
        ...
        return Self.dedupedThinkingOptions(
            levels.compactMap { level in
                ...
                return OriroChatThinkingLevelOption(id: id, label: label.isEmpty ? id : label)
            })
    }

    private static func thinkingOptions(from labels: [String]?) -> [OriroChatThinkingLevelOption]? {
        guard let labels else { return nil }
        return Self.dedupedThinkingOptions(
            labels.compactMap { label in
                guard let id = Self.normalizedThinkingLevel(label) else { return nil }
                let trimmed = label.trimmingCharacters(in: .whitespacesAndNewlines)
                return OriroChatThinkingLevelOption(id: id, label: trimmed.isEmpty ? id : trimmed)
            })
    }

    private static func withCurrentThinkingOption(
        _ options: [OriroChatThinkingLevelOption],
        current: String) -> [OriroChatThinkingLevelOption]
    {
        guard !options.contains(where: { $0.id == current }) else { return options }
        return options + [OriroChatThinkingLevelOption(id: current, label: current)]
    }

    private static func dedupedThinkingOptions(
        _ options: [OriroChatThinkingLevelOption]) -> [OriroChatThinkingLevelOption]
    {
        var result: [OriroChatThinkingLevelOption] = []
        var seen = Set<String>()
    ...
    private func placeholderSession(key: String) -> OriroChatSessionEntry {
    ...
    private func updateCurrentSessionThinkingLevel(_ thinkingLevel: String?, sessionKey: String) {
        ...
        self.sessions[index] = OriroChatSessionEntry(
            key: current.key,
            kind: current.kind,
    ...
    private func handleTransportEvent(_ evt: OriroChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleSessionMessageEvent(_ payload: OriroSessionMessageEventPayload) {
    ...
    private func handleChatEvent(_ chat: OriroChatEventPayload) {
    ...
    private func appendFinalChatMessageIfPresent(_ chat: OriroChatEventPayload) {
        guard chat.state == "final" else { return }
        guard let text = OriroChatEventText.assistantText(from: chat) else { return }

        let decoded = chat.message.flatMap {
            try? ChatPayloadDecoding.decode($0, as: OriroChatMessage.self)
        }
    ...
    private static func isAssistantMessage(_ message: OriroChatMessage) -> Bool {
        message.role.trimmingCharacters(in: .whitespacesAndNewlines).lowercased() == "assistant"
    }

    private static func messageWithTimestampIfNeeded(_ message: OriroChatMessage) -> OriroChatMessage {
    ...
    private func handleAgentEvent(_ evt: OriroAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = OriroChatPendingToolCall(
    ...
    private func handleAgentLifecycleEvent(_ evt: OriroAgentEventPayload, isPendingRun: Bool) {
    ...
    private static func agentLifecycleErrorMessage(_ evt: OriroAgentEventPayload, aborted: Bool) -> String {
        if aborted {
            return "Run aborted"
        }
    ...
    private static func hasUnansweredLatestUser(in messages: [OriroChatMessage]) -> Bool {
        self.latestUserTurn(in: messages) != nil && !self.hasAssistantMessageAfterLatestUser(in: messages)
    }

    private static func latestUserTurn(in messages: [OriroChatMessage]) -> LatestUserTurn? {
    ...
    private static func hasAnsweredUser(
        ...
        in messages: [OriroChatMessage])
        -> Bool
    {
        guard let refreshKey = user.refreshKey else { return false }
        var occurrence = 0
        var latestMatchingUserIndex: [OriroChatMessage].Index?
    ...
    private static func hasAssistantMessageAfterLatestUser(in messages: [OriroChatMessage]) -> Bool {
        guard let lastUserIndex = messages.lastIndex(where: { $0.role.lowercased() == "user" }) else {
            return false
        }
