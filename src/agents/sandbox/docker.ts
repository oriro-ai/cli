...
import { markOriroExecEnv } from "../../infra/oriro-exec-env.js";
...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "oriro.sandbox=1");
  args.push("--label", `oriro.sessionKey=${params.scopeKey}`);
  args.push("--label", `oriro.createdAtMs=${createdAtMs}`);
  args.push("--label", `oriro.mountFormatVersion=${SANDBOX_MOUNT_FORMAT_VERSION}`);
  if (params.configHash) {
    args.push("--label", `oriro.configHash=${params.configHash}`);
  }
  ...
  for (const [key, value] of Object.entries(markOriroExecEnv(envSanitization.allowed))) {
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "oriro.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`oriro sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`oriro sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("oriro sandbox recreate --all");
}
