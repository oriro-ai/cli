...
const ensureSupportedNodeVersion = () => {
  ...
  process.stderr.write(
    `oriro: Node.js v${MIN_NODE_VERSION}+ is required (current: v${process.versions.node}).\n` +
      "If you use nvm, run:\n" +
...
const runRespawnedChild = (command, args, env) => {
  ...
  child.once("error", (error) => {
    detach();
    process.stderr.write(
      `[oriro] Failed to respawn launcher: ${
...
const respawnWithPackagedCompileCacheIfNeeded = () => {
  ...
  if (process.env.ORIRO_PACKAGED_COMPILE_CACHE_RESPAWNED === "1") {
    return false;
  }
  ...
  const env = {
    ...
    ORIRO_PACKAGED_COMPILE_CACHE_RESPAWNED: "1",
  };
  return runRespawnedChild(
    process.execPath,
    [...process.execArgv, fileURLToPath(import.meta.url), ...process.argv.slice(2)],
...
const buildMissingEntryErrorMessage = async () => {
  const lines = ["oriro: missing dist/entry.(m)js (build output)."];
...
const isHelpFastPathDisabled = () =>
  process.env.ORIRO_DISABLE_CLI_STARTUP_HELP_FAST_PATH === "1";
...
const resolveLauncherHomeDir = () => {
  const explicit = normalizeLauncherHomeValue(process.env.ORIRO_HOME);
  const rawHome =
...
const resolveLauncherConfigPaths = () => {
  const explicit = process.env.ORIRO_CONFIG_PATH?.trim();
  if (explicit) {
...
const shouldDeferRootHelpToRuntimeEntry = () => {
  if (
    process.env.ORIRO_BUNDLED_PLUGINS_DIR?.trim() ||
    process.env.ORIRO_DISABLE_BUNDLED_PLUGINS?.trim()
  ) {
    return true;
  }
...
function tryOutputLauncherVersion(argv) {
  try {
    if (normalizeLauncherMetadataValue(process.env.ORIRO_CONTAINER)) {
      return false;
    }
...
function resolveLauncherVersion() {
  ...
  return normalizeLauncherMetadataValue(process.env.ORIRO_BUNDLED_VERSION) ?? "0.0.0";
}
