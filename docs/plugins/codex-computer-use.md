---
summary: "Set up Codex Computer Use for Codex-mode Oriro agents"
title: "Codex Computer Use"
read_when:
  - You want Codex-mode Oriro agents to use Codex Computer Use
...
Computer Use is a Codex-native MCP plugin for local desktop control. Oriro
...
Use this page when Oriro is already using the native Codex harness. For the
runtime setup itself, see [Codex harness](/plugins/codex-harness).

## Oriro.app and Peekaboo

Oriro.app's Peekaboo integration is separate from Codex Computer Use. The
...
Use [Peekaboo bridge](/platforms/mac/peekaboo) when you want Oriro.app to be
a permission-aware host for Peekaboo CLI automation. Use this page when a
Codex-mode Oriro agent should have Codex's native `computer-use` MCP plugin
...
Instead, the iOS app connects as an Oriro node and exposes mobile
...
Oriro-managed runtimes to call TryCua's driver directly, use the upstream
`cua-driver mcp` server through Oriro's MCP registry instead of the
Codex-specific marketplace flow.

After installing `cua-driver`, either ask it for the Oriro command:

```bash
cua-driver mcp-config --client oriro
```
...
available as a normal Oriro MCP server. Use the Codex Computer Use setup on
...
that its app prompts for, such as Accessibility and Screen Recording. Oriro
...
Computer Use in and lets Oriro install or re-enable it before the turn:
...
With this config, Oriro checks Codex app-server before each Codex-mode turn.
If Computer Use is missing but Codex app-server has already discovered an
installable marketplace, Oriro asks Codex app-server to install or re-enable
the plugin and reload MCP servers. On macOS, when no matching marketplace is
registered and the standard Codex app bundle exists, Oriro also tries to
...
plugin command surface is available. These are Oriro chat/runtime commands,
not `oriro codex ...` CLI subcommands:
...
Oriro uses the same app-server API that Codex itself exposes. The
...
During install, Oriro polls `plugin/list` for up to
`marketplaceDiscoveryTimeoutMs` milliseconds. The default is 60 seconds.

If multiple known marketplaces contain Computer Use, Oriro prefers
...
`computer-use` is registered, Oriro tries to add the standard bundled
...
Oriro reports a stable setup reason internally and formats the user-facing
status for chat:
...
permissions before it can inspect or control apps. If Oriro says Computer Use
...
Oriro intentionally fails closed when `computerUse.enabled` is true. A
...
tool hook could not reach an active Oriro relay through the local bridge or
Gateway fallback. Start a fresh Oriro session with `/new` or `/reset`. If it
works once and then fails again on a later tool call, `/new` is only clearing the
current attempt; restart the Codex app-server or Oriro Gateway so old threads
