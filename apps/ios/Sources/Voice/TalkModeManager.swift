...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__ORIRO_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<OriroTalkPTTStopPayload, Never>?
    ...
    private let logger = Logger(subsystem: "ai.orirofoundation.app", category: "TalkMode")
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = OriroTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> OriroTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = activePTTCaptureId {
            return OriroTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> OriroTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> OriroTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return OriroTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> OriroTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = OriroTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: OriroTalkPTTStopPayload) {
            ...
            group.addTask { [runId] in
                ...
                for await evt in stream {
                    ...
                    if evt.event == "chat" {
                        ...
                        if let text = OriroChatEventText.assistantText(from: chatEvent) {
