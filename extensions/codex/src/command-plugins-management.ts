...
/**
 * Lightweight read/write surface over the Oriro config file. Plugged in by
...
// Plugin lifecycle changes (enable/disable) write to oriro.json
...
export async function handleCodexPluginsSubcommand(
  ...
  if (normalized === "enable" || normalized === "disable") {
    ...
    return {
      text: `${formatCodexDisplayText(target)}: ${wantEnabled ? "enabled" : "disabled"} in oriro.json. ${POLICY_REFRESH_HINT}`,
    };
  }
