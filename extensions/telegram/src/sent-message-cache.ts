...
const TELEGRAM_SENT_MESSAGES_STATE_KEY = Symbol.for("oriro.telegramSentMessagesState");
const TELEGRAM_SENT_MESSAGES_STORE_FOR_TEST_KEY = Symbol.for(
  "oriro.telegramSentMessagesStoreForTest",
);
...
function resolveSentMessageStorePath(cfg?: Pick<OriroConfig, "session">): string {
  return `${resolveStorePath(cfg?.session?.store)}.telegram-sent-messages.json`;
}

function resolveSentMessageScopeKey(cfg?: Pick<OriroConfig, "session">): string {
...
function getSentMessageBucket(cfg?: Pick<OriroConfig, "session">): SentMessageBucket {
...
function getSentMessages(cfg?: Pick<OriroConfig, "session">): SentMessageStore {
