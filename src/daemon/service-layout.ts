/** Summarizes installed service command paths and Oriro package layout. */
...
async function resolveOriroPackageRoot(entrypoint: string): Promise<string | undefined> {
...
export async function summarizeGatewayServiceLayout(
  ...
  const packageRoot = entrypointReal ? await resolveOriroPackageRoot(entrypointReal) : undefined;
