...
describe("scripts/test-docker-all scheduler", () => {
  ...
  it("prints CLI help without a stack trace", () => {
    ...
    expect(result.stdout).toContain("ORIRO_DOCKER_ALL_* env vars");
  });
  ...
  it("rejects loose numeric runner env vars without a stack trace", () => {
    const result = spawnSync(process.execPath, ["scripts/test-docker-all.mjs", "--plan-json"], {
      ...
      env: {
        ...process.env,
        ORIRO_DOCKER_ALL_PARALLELISM: "1e3",
      },
    });

    expect(result.status).toBe(1);
    expect(result.stdout).toBe("");
    expect(result.stderr).toContain("ORIRO_DOCKER_ALL_PARALLELISM must be a positive integer");
    expect(result.stderr).not.toContain("at ");
  });

  it("rejects loose numeric resource limit env vars before scheduling lanes", () => {
    const logDir = mkdtempSync(`${tmpdir()}/oriro-docker-all-`);
    try {
      const result = spawnSync(process.execPath, ["scripts/test-docker-all.mjs"], {
        ...
        env: {
          ...
          ORIRO_DOCKER_ALL_DOCKER_LIMIT: "1e3",
          ORIRO_DOCKER_ALL_DRY_RUN: "1",
          ORIRO_DOCKER_ALL_LOG_DIR: logDir,
          ORIRO_DOCKER_ALL_PREFLIGHT: "0",
          ORIRO_DOCKER_ALL_TIMINGS: "0",
        },
      });

      expect(result.status).toBe(1);
      expect(result.stderr).toContain(
        "ORIRO_DOCKER_ALL_DOCKER_LIMIT must be a positive integer",
      );
      expect(result.stderr).not.toContain("at ");
    } finally {
  ...
  it("rejects release-path configs that schedule zero Docker lanes", () => {
    ...
    try {
      const result = spawnSync(process.execPath, ["scripts/test-docker-all.mjs"], {
        ...
        env: {
          ...process.env,
          ORIRO_DOCKER_ALL_CHUNK: "openwebui",
          ORIRO_DOCKER_ALL_DRY_RUN: "1",
          ORIRO_DOCKER_ALL_INCLUDE_OPENWEBUI: "0",
          ORIRO_DOCKER_ALL_LOG_DIR: logDir,
          ORIRO_DOCKER_ALL_PREFLIGHT: "0",
  ...
  posixIt("writes Docker run artifacts when cleanup smoke fails", () => {
    const root = mkdtempSync(`${tmpdir()}/oriro-docker-all-cleanup-`);
    const logDir = path.join(root, "logs");
    const packageTgz = path.join(root, "oriro-current.tgz");
...
`,
    ...
    try {
      const result = spawnSync(process.execPath, ["scripts/test-docker-all.mjs"], {
        ...
        env: {
          ...process.env,
          ORIRO_CURRENT_PACKAGE_TGZ: packageTgz,
          ORIRO_DOCKER_ALL_BUILD: "0",
          ORIRO_DOCKER_ALL_LIVE_MODE: "skip",
          ORIRO_DOCKER_ALL_LOG_DIR: logDir,
          ORIRO_DOCKER_ALL_PARALLELISM: "16",
          ORIRO_DOCKER_ALL_PREFLIGHT: "0",
          ORIRO_DOCKER_ALL_START_STAGGER_MS: "0",
          ORIRO_DOCKER_ALL_STATUS_INTERVAL_MS: "0",
          ORIRO_DOCKER_ALL_TAIL_PARALLELISM: "16",
          ORIRO_DOCKER_ALL_TIMINGS: "0",
  ...
  it("cleans stale stopped containers from all named Docker E2E lanes", () => {
    expect(
      dockerPreflightContainerNames(`
oriro-gateway-e2e-123 Exited (1) 2 minutes ago
oriro-config-reload-e2e-234 Created
oriro-plugin-binding-command-escape-e2e-345 Dead
oriro-kitchen-sink-rpc-e2e-456 Exited (137) 10 seconds ago
oriro-openwebui-gateway-567 Exited (1) 3 minutes ago
oriro-openwebui-678 Created
oriro-not-an-e2e-container Exited (1) 2 minutes ago
postgres Created
`),
    ).toEqual([
      "oriro-gateway-e2e-123",
      "oriro-config-reload-e2e-234",
      "oriro-plugin-binding-command-escape-e2e-345",
      "oriro-kitchen-sink-rpc-e2e-456",
      "oriro-openwebui-gateway-567",
      "oriro-openwebui-678",
    ]);
  });
  ...
  it("reads bounded lane log tails instead of full noisy logs", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "oriro-docker-all-log-tail-"));
    try {
  ...
  posixIt("kills timed-out shell command groups when the leader exits first", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "oriro-docker-all-timeout-"));
...
`,
  ...
  posixIt("clamps oversized shell command kill grace before scheduling", async () => {
    const root = createTempDir("oriro-docker-all-oversized-grace-");
    const scriptPath = path.join(root, "leader-exits.mjs");
...
`,
  ...
  posixIt("lets timed-out shell command descendants exit during kill grace", async () => {
    const root = createTempDir("oriro-docker-all-grace-");
    const scriptPath = path.join(root, "leader-exits.mjs");
    const donePath = path.join(root, "done");
    const readyPath = path.join(root, "ready");
    const childScript = [
...
`,
  ...
  posixIt("lets timed-out shell capture descendants exit during kill grace", async () => {
    const root = createTempDir("oriro-docker-all-capture-grace-");
    const scriptPath = path.join(root, "leader-exits.mjs");
    const donePath = path.join(root, "done");
    const readyPath = path.join(root, "ready");
    const childScript = [
...
`,
  ...
  posixIt("cleans active shell command groups before parent signal exit", async () => {
    const root = createTempDir("oriro-docker-all-parent-signal-");
