...
- Same path: run the app from a fixed location (for Oriro, `dist/Oriro.app`).
...
Prefer granting Accessibility to Oriro.app, Peekaboo.app, or another signed
...
```bash
sudo tccutil reset Accessibility ai.oriro.mac
sudo tccutil reset ScreenCapture ai.oriro.mac
...
Workaround: move files into the Oriro workspace (`~/.oriro/workspace`) if you want to avoid per-folder grants.
