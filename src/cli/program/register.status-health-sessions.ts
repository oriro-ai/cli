...
export function registerStatusHealthSessionsCommands(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["oriro status", "Show channel health + session summary."],
          ["oriro status --all", "Full diagnosis (read-only)."],
          ["oriro status --json", "Machine-readable output."],
          ["oriro status --usage", "Show model provider usage/quota snapshots."],
          [
            "oriro status --deep",
            "Run channel probes (WA + Telegram + Discord + Slack + Signal).",
          ],
          ["oriro status --deep --timeout 5000", "Tighten probe timeout."],
        ])}`,
    )
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/status", "docs.oriro.ai/cli/status")}\n`,
    )
    .action(async (opts) => {
  ...
  program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/health", "docs.oriro.ai/cli/health")}\n`,
    )
    .action(async (opts) => {
      await runWithVerboseAndTimeout(opts, async ({ verbose, timeoutMs }) => {
  ...
  const sessionsCmd = addSessionsListOptions(
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["oriro sessions", "List all sessions."],
          ["oriro sessions --agent work", "List sessions for one agent."],
          ["oriro sessions --all-agents", "Aggregate sessions across agents."],
          ["oriro sessions --active 120", "Only last 2 hours."],
          ["oriro sessions --limit 25", "Show the newest 25 sessions."],
          ["oriro sessions --json", "Machine-readable output."],
          ["oriro sessions --store ./tmp/sessions.json", "Use a specific session store."],
        ])}\n\n${theme.muted(
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sessions", "docs.oriro.ai/cli/sessions")}\n`,
    )
    .action(async (opts) => {
  ...
  sessionsCmd
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["oriro sessions cleanup --dry-run", "Preview stale/cap cleanup."],
          [
            "oriro sessions cleanup --dry-run --fix-missing",
          ...
          ["oriro sessions cleanup --enforce", "Apply maintenance now."],
          ["oriro sessions cleanup --agent work --dry-run", "Preview one agent store."],
          ["oriro sessions cleanup --all-agents --dry-run", "Preview all agent stores."],
          [
            "oriro sessions cleanup --enforce --store ./tmp/sessions.json",
  ...
  sessionsCmd
    ...
    .option("--output <path>", "Output directory name inside .oriro/trajectory-exports")
  ...
  sessionsCmd
    ...
    .action(async (key: string, opts, command) => {
      ...
      // command, e.g. `oriro sessions --agent work compact <key>`. Merge those
  ...
  const commitmentsCmd = program
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["oriro commitments", "List pending inferred follow-ups."],
          ["oriro commitments --all", "List all inferred follow-ups."],
          ["oriro commitments --agent work", "List one agent's inferred follow-ups."],
          ["oriro commitments dismiss cm_abc123", "Dismiss a follow-up."],
        ])}`,
    )
    .action(async (opts) => {
      await runCommandWithRuntime(defaultRuntime, async () => {
