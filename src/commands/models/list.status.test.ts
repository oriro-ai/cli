...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/oriro-agent"),
    resolveAgentWorkspaceDir: vi.fn().mockReturnValue("/tmp/oriro-agent/workspace"),
    ...
    resolveAuthStorePathForDisplay: vi.fn(
      (agentDir?: string) => `${agentDir ?? "/tmp/oriro-agent"}/auth-profiles.json`,
    ),
    ...
    createConfigIO: vi.fn().mockReturnValue({
      configPath: "/tmp/oriro-dev/oriro.json",
    }),
...
vi.mock("../../agents/workspace.js", () => ({
  resolveDefaultAgentWorkspaceDir: vi.fn().mockReturnValue("/tmp/oriro-agent/workspace"),
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/oriro-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  ...
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(payload.configPath).toBe("/tmp/oriro-dev/oriro.json");
    expect(payload.auth.storePath).toBe("/tmp/oriro-agent/auth-profiles.json");
  ...
  it("honors ORIRO_AGENT_DIR when no --agent override is provided", async () => {
    const localRuntime = createRuntime();
    mocks.resolveAgentDir.mockClear();
    await withEnvAsync({ ORIRO_AGENT_DIR: "/tmp/oriro-isolated-agent" }, async () => {
      await modelsStatusCommand({ json: true }, localRuntime as never);
    });

    expect(mocks.resolveAgentDir).not.toHaveBeenCalled();
    expect(mocks.ensureAuthProfileStore).toHaveBeenCalledWith("/tmp/oriro-isolated-agent");
    const payload = parseFirstJsonLog(localRuntime);
    expect(payload.agentDir).toBe("/tmp/oriro-isolated-agent");
    expect(payload.auth.storePath).toBe("/tmp/oriro-isolated-agent/auth-profiles.json");
  });

  it("honors deprecated PI_CODING_AGENT_DIR when ORIRO_AGENT_DIR is unset", async () => {
    const localRuntime = createRuntime();
    mocks.resolveAgentDir.mockClear();
    await withEnvAsync(
      {
        ORIRO_AGENT_DIR: undefined,
    ...
    expect(mocks.resolveAgentDir).not.toHaveBeenCalled();
    expect(mocks.ensureAuthProfileStore).toHaveBeenCalledWith("/tmp/oriro-legacy-agent");
    const payload = parseFirstJsonLog(localRuntime);
    expect(payload.agentDir).toBe("/tmp/oriro-legacy-agent");
  });
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/oriro-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/oriro-agent-custom");
        ...
        expect(openAiCodex?.effective).toEqual({
          kind: "profiles",
          detail: "/tmp/oriro-agent-custom/auth-profiles.json",
        });
      },
    );
  });
  ...
  it("reports Gemini CLI OAuth for canonical Google text routed through the CLI runtime", async () => {
    ...
    try {
      ...
      expect(
        ...
        kind: "profiles",
        detail: "/tmp/oriro-agent/auth-profiles.json",
      });
      expect(payload.auth.runtimeAuthRoutes).toEqual([
        {
          provider: "google",
  ...
  it("includes auth-evidence-only providers in the auth overview", async () => {
    ...
    mocks.resolveEnvApiKey.mockImplementation(
      (provider: string, _env?: NodeJS.ProcessEnv, options?: { workspaceDir?: string }) =>
        provider === "workspace-cloud" && options?.workspaceDir === "/tmp/oriro-agent/workspace"
          ? {
