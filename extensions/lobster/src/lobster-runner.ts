...
function findLobsterPackageRoot(resolvedEntryPath: string): string {
  ...
  while (true) {
    ...
    try {
      ...
      if (parsed.name === "@openclaw/lobster") {
        return dir;
      }
    } catch {
    ...
    if (parent === dir) {
      throw new Error(`Could not locate @openclaw/lobster package root from ${resolvedEntryPath}`);
    }
    dir = parent;
  }
}
    ...
    if (envelope.status === "needs_input") {
      return {
        ...
        error: {
          ...
          message: "Lobster input requests are not supported by the ORIRO Lobster tool yet",
        },
      };
    }
    return {
...
export async function loadEmbeddedToolRuntimeFromPackage(
  ...
  const packageEntryPath = resolvePackageEntry("@openclaw/lobster");
  await installLobsterAjvCompileCache(packageEntryPath);

  let coreLoadError: unknown;
  try {
    const coreSpecifier = ["@oriro", "lobster", "core"].join("/");
    return toEmbeddedToolRuntime(await importModule(coreSpecifier), "@openclaw/lobster/core");
  } catch (error) {
    coreLoadError = error;
  }
