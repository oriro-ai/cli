...
describe("config io write prepare", () => {
        ...
        sourceConfig: {
          plugins: {
            ...
            installs: {
              "oriro-web-search": {
                source: "npm",
                spec: "@ollama/oriro-web-search",
                installPath: "/tmp/oriro-web-search",
                resolvedName: "@ollama/oriro-web-search",
        ...
        nextConfig: {
          plugins: {
            ...
            installs: {
              "oriro-web-search": {
                source: "npm",
                spec: "@ollama/oriro-web-search@0.2.2",
                installPath: "/tmp/oriro-web-search",
                resolvedName: "@ollama/oriro-web-search",
                resolvedVersion: "0.2.2",
              },
  ...
  it('formats actionable guidance for dmPolicy="open" without wildcard allowFrom', () => {
    ...
    expect(message).toContain("oriro config set channels.telegram.allowFrom '[\"*\"]'");
    expect(message).toContain('oriro config set channels.telegram.dmPolicy "pairing"');
  });
  ...
  it("keeps the read-time env snapshot when writing the same config path", () => {
    ...
    expect(
      resolveWriteEnvSnapshotForPath({
        actualConfigPath: "/tmp/oriro.json",
  ...
  it("keeps runtime-only channel defaults out of the persisted candidate", () => {
    ...
    const nextConfig: OriroConfig = structuredClone(runtimeConfig);
    nextConfig.gateway = {
      ...nextConfig.gateway,
      auth: { mode: "token" },
    };
  ...
  it("rejects writes that would flatten a root include", () => {
    const sourceConfig = {
      $schema: "https://oriro.ai/config-from-include.json",
      gateway: { mode: "local" },
    };
