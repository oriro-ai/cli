...
} from "@oriro/media-core/inline-image-data-url";
...
function redactTranscriptOptions(cfg?: OriroConfig) {
...
function isTranscriptRedactionDisabled(cfg?: OriroConfig): boolean {
  return (cfg?.logging?.redactSensitive ?? readLoggingConfig()?.redactSensitive) === "off";
}

function redactTranscriptText(value: string, cfg?: OriroConfig): string {
...
const OPENAI_RESPONSES_APIS = new Set([
  ...
  "oriro-azure-openai-responses-transport",
]);
...
const ANTHROPIC_REASONING_APIS = new Set([
  ...
  "oriro-anthropic-messages-transport",
]);
const OPENAI_COMPLETIONS_APIS = new Set([
  "openai-completions",
  "oriro-openai-completions-transport",
]);
...
function isGitHubCopilotResponsesRoute(route: TranscriptAssistantRoute | undefined): boolean {
  return (
    (route?.api === "openai-responses" || route?.api === "oriro-openai-responses-transport") &&
...
const OPENAI_REASONING_REPLAY_METADATA_KEY = "__oriro_replay";
...
export function redactTranscriptMessage(message: AgentMessage, cfg?: OriroConfig): AgentMessage {
