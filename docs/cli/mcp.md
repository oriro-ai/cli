---
summary: "Expose Oriro channel conversations over MCP and manage saved MCP server definitions"
read_when:
  - Connecting Codex, Claude Code, or another MCP client to Oriro-backed channels
  - Running `oriro mcp serve`
  - Managing Oriro-saved MCP server definitions
title: "MCP"
sidebarTitle: "MCP"
---

`oriro mcp` has two jobs:

- run Oriro as an MCP server with `oriro mcp serve`
- manage Oriro-managed outbound MCP server definitions with `list`, `show`, `status`, `doctor`, `probe`, `add`, `set`, `configure`, `tools`, `login`, `logout`, `reload`, and `unset`

In other words:

- `serve` is Oriro acting as an MCP server
- the other subcommands are Oriro acting as an MCP client-side registry for MCP servers its runtimes may consume later

<Note>
  `list`, `show`, `set`, and `unset` only read and write Oriro-managed `mcp.servers` entries in Oriro config. They do not include mcporter servers from `config/mcporter.json`; use `mcporter list` for that registry.
</Note>

Use [`oriro acp`](/cli/acp) when Oriro should host a coding harness session itself and route that runtime through ACP.

## Choose the right MCP path

Oriro has several MCP surfaces. Pick the one that matches who owns the agent runtime and who owns the tools.

| Goal                                                                | Use                                                                  | Why                                                                                                             |
| ------------------------------------------------------------------- | -------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------- |
| Let an external MCP client read/send Oriro channel conversations | `oriro mcp serve`                                                 | Oriro is the MCP server and exposes Gateway-backed conversations over stdio.                                 |
| Save third-party MCP servers for Oriro-managed agent runs        | `oriro mcp add`, `set`, `configure`, `tools`, `login`             | Oriro is the MCP client-side registry and later projects those servers into eligible runtimes.               |
| Check a saved server without running an agent turn                  | `oriro mcp status`, `doctor`, `probe`                             | `status` and `doctor` inspect config; `probe` opens a live MCP connection and lists capabilities.               |
...
| Run ACP-hosted harness sessions                                     | [`oriro acp`](/cli/acp) and [ACP Agents](/tools/acp-agents-setup) | ACP bridge mode does not accept per-session MCP server injection; configure gateway/plugin bridges instead.     |

<Tip>
If you are not sure which path you need, start with `oriro mcp status --verbose`. It shows what Oriro has saved without starting any MCP servers.
</Tip>

## Oriro as an MCP server

This is the `oriro mcp serve` path.

### When to use `serve`

Use `oriro mcp serve` when:

- Codex, Claude Code, or another MCP client should talk directly to Oriro-backed channel conversations
- you already have a local or remote Oriro Gateway with routed sessions
- you want one MCP server that works across Oriro's channel backends instead of running separate per-channel bridges

Use [`oriro acp`](/cli/acp) instead when Oriro should host the coding runtime itself and keep the agent session inside Oriro.

### How it works

`oriro mcp serve` starts a stdio MCP server. The MCP client owns that process. While the client keeps the stdio session open, the bridge connects to a local or remote Oriro Gateway over WebSocket and exposes routed channel conversations over MCP.

<Steps>
  <Step title="Client spawns the bridge">
    The MCP client spawns `oriro mcp serve`.
  </Step>
  <Step title="Bridge connects to Gateway">
    The bridge connects to the Oriro Gateway over WebSocket.
  </Step>
...
<AccordionGroup>
  <Accordion title="Important behavior">
    ...
    - one-shot agent entry points such as `oriro agent` and `oriro infer model run` retire any bundled MCP runtimes they open when the reply completes, so repeated scripted runs do not accumulate stdio MCP child processes
    - stdio MCP servers launched by Oriro (bundled or user-configured) are torn down as a process tree on shutdown, so child subprocesses started by the server do not survive after the parent stdio client exits
...
The bridge uses existing Gateway session route metadata to expose channel-backed conversations. A conversation appears when Oriro already has session state with a known route such as:
...
<Tabs>
  ...
  <Tab title="Remote Gateway (token)">
    ```bash
    oriro mcp serve --url wss://gateway-host:18789 --token-file ~/.oriro/gateway.token
    ```
  </Tab>
  <Tab title="Remote Gateway (password)">
    ```bash
    oriro mcp serve --url wss://gateway-host:18789 --password-file ~/.oriro/gateway.password
    ```
  </Tab>
  <Tab title="Verbose / Claude off">
    ```bash
    oriro mcp serve --verbose
    oriro mcp serve --claude-channel-mode off
    ```
  </Tab>
</Tabs>
...
The bridge can also expose Claude-specific channel notifications. This is the Oriro equivalent of a Claude Code channel adapter: standard MCP tools remain available, but live inbound messages can also arrive as Claude-specific MCP notifications.
...
- sender allowlists, pairing, and channel-level trust still belong to the underlying Oriro channel configuration
...
Oriro ships a deterministic Docker smoke for this bridge:
...
- starts a second container that spawns `oriro mcp serve`
...
## Oriro as an MCP client registry

This is the `oriro mcp list`, `show`, `status`, `doctor`, `probe`, `add`, `set`,
`configure`, `tools`, `login`, `logout`, `reload`, and `unset` path.

These commands do not expose Oriro over MCP. They manage Oriro-managed MCP server definitions under `mcp.servers` in Oriro config. They do not read mcporter servers from `config/mcporter.json`.

Those saved definitions are for runtimes that Oriro launches or configures later, such as embedded Oriro and other runtime adapters. Oriro stores the definitions centrally so those runtimes do not need to keep their own duplicate MCP server lists.

<AccordionGroup>
  <Accordion title="Important behavior">
    - these commands only read or write Oriro config
    ...
    - embedded Oriro exposes configured MCP tools in normal `coding` and `messaging` tool profiles; `minimal` still hides them, and `tools.deny: ["bundle-mcp"]` disables them explicitly
    - per-server `toolFilter.include` and `toolFilter.exclude` filter discovered MCP tools before they become Oriro tools
...
Runtime adapters may normalize this shared registry into the shape their downstream client expects. For example, embedded Oriro consumes Oriro `transport` values directly, while Claude Code and Gemini receive CLI-native `type` values such as `http`, `sse`, or `stdio`.

Codex app-server also honors an optional `codex` block on each server. This is
Oriro projection metadata for Codex app-server threads only; it does not
change ACP sessions, generic Codex harness config, or other runtime adapters.
Use non-empty `codex.agents` to project a server only into specific Oriro
...
Oriro strips the `codex` metadata before handing the native `mcp_servers`
config to Codex.

### Saved MCP server definitions

Oriro also stores a lightweight MCP server registry in config for surfaces that want Oriro-managed MCP definitions.

Commands:

- `oriro mcp list`
- `oriro mcp show [name]`
- `oriro mcp status [--verbose]`
- `oriro mcp doctor [name] [--probe]`
- `oriro mcp probe [name]`
- `oriro mcp add <name> [flags]`
- `oriro mcp set <name> <json>`
- `oriro mcp configure <name> [flags]`
- `oriro mcp tools <name> [--include csv] [--exclude csv] [--clear]`
- `oriro mcp login <name> [--code code]`
- `oriro mcp logout <name>`
- `oriro mcp reload`
- `oriro mcp unset <name>`
...
- Use `transport: "streamable-http"` for Streamable HTTP MCP servers. `oriro mcp set` also normalizes CLI-native `type: "http"` to the same canonical config shape for compatibility.
...
```bash
oriro mcp list
oriro mcp show context7 --json
oriro mcp status --verbose
oriro mcp doctor --probe
oriro mcp probe context7 --json
oriro mcp add memory --command npx --arg -y --arg @modelcontextprotocol/server-memory
oriro mcp set context7 '{"command":"uvx","args":["context7-mcp"]}'
oriro mcp tools context7 --include 'resolve-library-id,get-library-docs'
oriro mcp set docs '{"url":"https://mcp.example.com","transport":"streamable-http"}'
oriro mcp configure docs --timeout 20 --connect-timeout 5 --include 'search,read_*'
oriro mcp configure docs --auth oauth --oauth-scope 'docs.read'
oriro mcp login docs
oriro mcp logout docs
oriro mcp unset context7
```

### Common server recipes

These examples save server definitions only. Run `oriro mcp doctor --probe` afterward to prove that the server starts and exposes tools.
...
<Tabs>
  ...
  <Tab title="Memory">
    ```bash
    oriro mcp add memory \
      --command npx \
      --arg -y \
      --arg @modelcontextprotocol/server-memory
    oriro mcp probe memory --json
    ```
  ...
  <Tab title="Desktop/CUA">
    ```bash
    oriro mcp set cua-driver '{"command":"cua-driver","args":["mcp"]}'
    oriro mcp tools cua-driver --include 'list_apps,observe,click,type'
    oriro mcp doctor cua-driver --probe
    ```
...
<AccordionGroup>
  <Accordion title="status --json">
    ```json
    {
      "path": "/home/user/.oriro-ai/cli.json",
      "servers": [
        {
          "name": "docs",
          "configured": true,
      ...
      "servers": [
        {
          ...
          "issues": [
            {
              "level": "error",
              "message": "OAuth credentials are not authorized; run oriro mcp login docs"
            }
          ]
        }
      ]
...
| `auth: "oauth"`                | Use MCP OAuth token storage and `oriro mcp login`             |
...
Sensitive values in `url` (userinfo) and `headers` are redacted in logs and status output. `oriro mcp doctor` warns when sensitive-looking `headers` or `env` entries contain literal values, so operators can move those values out of committed config.
...
<Steps>
  <Step title="Save the server">
    ...
    ```bash
    oriro mcp set docs '{"url":"https://mcp.example.com/mcp","transport":"streamable-http","auth":"oauth","oauth":{"scope":"docs.read"}}'
    ```
  ...
  <Step title="Start login">
    ...
    Oriro prints the authorization URL and stores temporary OAuth verifier state under the Oriro state directory.

  </Step>
  <Step title="Finish with the code">
    After approving in the browser, pass the returned code back to Oriro.

    ```bash
    oriro mcp login docs --code abc123
    ```
...
If the provider rotates tokens or the authorization state gets stuck, run `oriro mcp logout <name>`, then repeat `login`. `logout` can clear credentials for a saved HTTP server even after `auth: "oauth"` has been removed from config, as long as the server name and URL still identify the credential store entry.
...
| ------------------------------ | -------------------------------------------------------------------------------------- |
| `url`                          | HTTP or HTTPS URL of the remote server (required)                                      |
| `transport`                    | Set to `"streamable-http"` to select this transport; when omitted, Oriro uses `sse` |
| `headers`                      | Optional key-value map of HTTP headers (for example auth tokens)                       |
| `connectionTimeoutMs`          | Per-server connection timeout in ms (optional)                                         |
| `connectTimeout`               | Per-server connection timeout in seconds (optional)                                    |
| `timeout` / `requestTimeoutMs` | Per-server MCP request timeout in seconds or ms                                        |
...
Oriro config uses `transport: "streamable-http"` as the canonical spelling. CLI-native MCP `type: "http"` values are accepted when saved through `oriro mcp set` and repaired by `oriro doctor --fix` in existing config, but `transport` is what embedded Oriro consumes directly.
...
Use the page for operator edits and quick inventory. Use `oriro mcp doctor --probe` or `oriro mcp probe` when you need live server proof.
...
7. Run `oriro mcp doctor --probe` when you need live proof that the edited server starts and lists tools.
...
- active runtimes may need `oriro mcp reload`, Gateway config publish, or process restart depending on which process owns the MCP clients
