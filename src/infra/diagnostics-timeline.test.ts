...
async function createTimelineEnv() {
  const dir = await mkdtemp(join(tmpdir(), "oriro-diagnostics-timeline-"));
  tempDirs.push(dir);
  return {
    env: {
      ORIRO_DIAGNOSTICS: "timeline",
      ORIRO_DIAGNOSTICS_RUN_ID: "run-1",
      ORIRO_DIAGNOSTICS_ENV: "env-1",
      ORIRO_DIAGNOSTICS_TIMELINE_PATH: join(dir, "nested", "timeline.jsonl"),
...
describe("diagnostics timeline", () => {
  it("detects when timeline output is enabled", async () => {
    ...
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, ORIRO_DIAGNOSTICS: "1" } })).toBe(true);
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, ORIRO_DIAGNOSTICS: "yes" } })).toBe(
      true,
    );
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, ORIRO_DIAGNOSTICS: "on" } })).toBe(
      true,
    );
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, ORIRO_DIAGNOSTICS: "all" } })).toBe(
      true,
    );
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, ORIRO_DIAGNOSTICS: "*" } })).toBe(true);
    expect(
      isDiagnosticsTimelineEnabled({
        env: { ...env, ORIRO_DIAGNOSTICS: "diagnostics.timeline" },
      }),
    ).toBe(true);
    expect(
      isDiagnosticsTimelineEnabled({ env: { ...env, ORIRO_DIAGNOSTICS: "telegram.http" } }),
    ).toBe(false);
    expect(isDiagnosticsTimelineEnabled({ env: { ...env, ORIRO_DIAGNOSTICS: "0" } })).toBe(
      false,
    );
    expect(
      isDiagnosticsTimelineEnabled({
        env: { ...env, ORIRO_DIAGNOSTICS_TIMELINE_PATH: "" },
      }),
    ).toBe(false);
  });
  ...
  it("honors config diagnostics flags after config is available", async () => {
    ...
    delete envWithoutFlag.ORIRO_DIAGNOSTICS;
    const configWithTimeline = { diagnostics: { flags: ["timeline"] } } as OriroConfig;
    const configWithWildcard = { diagnostics: { flags: ["*"] } } as OriroConfig;
    const configWithoutTimeline = { diagnostics: { flags: ["telegram.http"] } } as OriroConfig;
  ...
  it("writes JSONL diagnostic events with the stable envelope", async () => {
    ...
    expect(event?.schemaVersion).toBe("oriro.diagnostics.v1");
  ...
  it("records span start and end events around successful work", async () => {
    ...
    delete configOnlyEnv.ORIRO_DIAGNOSTICS;

    await expect(
      measureDiagnosticsTimelineSpan("runtimeDeps.stage", () => "ok", {
        phase: "startup",
        attributes: { pluginCount: 3 },
        config: { diagnostics: { flags: ["timeline"] } } as OriroConfig,
        env: configOnlyEnv,
      }),
    ).resolves.toBe("ok");
