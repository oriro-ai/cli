...
import { resolveOriroUserDataDir } from "./chrome.js";
import type { BrowserRouteContext, BrowserServerState } from "./server-context.js";
import { movePathToTrash } from "./trash.js";

const configMocks = vi.hoisted(() => ({
  getRuntimeConfig: vi.fn<() => OriroConfig>(),
  writeConfigFile: vi.fn<(cfg: OriroConfig) => Promise<void>>(async (_cfg) => {}),
  mutateConfigFile: vi.fn(
    async (params: {
      mutate: (draft: OriroConfig, context: { snapshot: { path: string } }) => unknown;
    }) => {
...
vi.mock("../config/config.js", async () => {
  ...
  return {
    ...actual,
    replaceConfigFile: vi.fn(async ({ nextConfig }: { nextConfig: OriroConfig }) => {
...
vi.mock("./chrome.js", () => ({
  resolveOriroUserDataDir: vi.fn(() => "/tmp/oriro-test/oriro/user-data"),
}));
...
describe("BrowserProfilesService", () => {
  ...
  it("creates existing-session profiles with an explicit userDataDir", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join("/tmp", "oriro-profile-"));
  ...
  it("deletes local profiles and moves data to Trash", async () => {
    ...
    vi.mocked(resolveOriroUserDataDir).mockReturnValue(userDataDir);
