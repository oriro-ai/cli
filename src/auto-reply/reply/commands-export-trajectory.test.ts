...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-export-command-"));
  tempDirs.push(dir);
  return dir;
}
    ...
    execute: vi.fn(async (_toolCallId: string, params: unknown) => {
      ...
      return {
        details: {
          ...
          command: "oriro sessions export-trajectory --session-key agent:target:session",
          cwd: "/tmp",
        },
      };
    }),
...
describe("buildExportTrajectoryCommandReply", () => {
  ...
  it("requests per-run exec approval for trajectory exports", async () => {
    ...
    expect(reply.text).toContain("https://docs.oriro.ai/tools/trajectory");
    ...
    expect(execCall.defaults.sessionStore).toBe("/tmp/oriro-sessions.json");
    ...
    expect(command).not.toContain("oriro sessions export-trajectory");
    ...
    expect(String(request.workspace)).toContain("oriro-export-command-");
  });
  ...
  it("routes group trajectory export approval privately", async () => {
    ...
    expect(privateReplies[0]?.text).toContain("oriro sessions export-trajectory");
