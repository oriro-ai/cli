...
homepage: https://docs.oriro.ai/automation/hooks#command-logger
metadata:
  {
    "oriro":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.oriro/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.oriro/logs/commands.log ~/.oriro/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/oriro`:
   ```
   /home/username/.oriro/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
