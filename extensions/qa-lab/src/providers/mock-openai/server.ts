...
import { escapeRegExp } from "oriro/plugin-sdk/text-utility-runtime";
import { readRequestBodyWithLimit } from "oriro/plugin-sdk/webhook-ingress";
...
const QA_GROUP_AUDIO_TRANSCRIPTION_TEXT =
  "oriroqa reply with only this exact marker after group audio preflight: WHATSAPP_QA_GROUP_AUDIO_TRANSCRIPT_OK";
...
function buildQaToolSearchArgs(targetTool: string, failureMode: boolean): Record<string, unknown> {
  ...
  if (targetTool === "web_search") {
    return { query: "Oriro runtime parity fixed query", count: 1 };
  }
...
function buildAssistantText(
  ...
  if (/tool continuity check/i.test(prompt) && toolOutput) {
    return `Protocol note: model switch handoff confirmed on ${model || "the requested model"}. QA mission from QA_KICKOFF_TASK.md still applies: understand this Oriro repo from source + docs before acting.`;
  }
  ...
  if (
    ...
    if (targetTool && hasDeclaredTool(body, "tool_search_code")) {
      return buildToolCallEventsWithArgs("tool_search_code", {
        code: [
          `const hits = await oriro.tools.search(${JSON.stringify(targetTool)}, { limit: 1 });`,
          "const match = hits.find((tool) => tool.name === " + JSON.stringify(targetTool) + ");",
          "if (!match) throw new Error('target tool not found');",
          `return await oriro.tools.call(match.id, ${JSON.stringify(plannedArgs)});`,
        ].join("\n"),
      });
    }
