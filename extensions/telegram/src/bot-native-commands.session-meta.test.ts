...
import type { ResolvedAgentRoute } from "oriro/plugin-sdk/routing";
...
type ResolveConfiguredBindingRouteFn =
  typeof import("oriro/plugin-sdk/conversation-runtime").resolveConfiguredBindingRoute;
type EnsureConfiguredBindingRouteReadyFn =
  typeof import("oriro/plugin-sdk/conversation-runtime").ensureConfiguredBindingRouteReady;
type DispatchReplyWithBufferedBlockDispatcherFn =
  typeof import("oriro/plugin-sdk/reply-dispatch-runtime").dispatchReplyWithBufferedBlockDispatcher;
...
type LoadModelCatalogFn = typeof import("oriro/plugin-sdk/agent-runtime").loadModelCatalog;
...
vi.mock("oriro/plugin-sdk/conversation-runtime", async () => {
  const actual = await vi.importActual<typeof import("oriro/plugin-sdk/conversation-runtime")>(
    "oriro/plugin-sdk/conversation-runtime",
  );
  return {
    ...actual,
...
vi.mock("oriro/plugin-sdk/session-store-runtime", async () => {
  const actual = await vi.importActual<typeof import("oriro/plugin-sdk/session-store-runtime")>(
    "oriro/plugin-sdk/session-store-runtime",
  );
  return {
    ...actual,
...
vi.mock("oriro/plugin-sdk/command-auth-native", async () => {
  const actual = await vi.importActual<typeof import("oriro/plugin-sdk/command-auth-native")>(
    "oriro/plugin-sdk/command-auth-native",
  );
...
vi.mock("oriro/plugin-sdk/plugin-runtime", async () => {
  const actual = await vi.importActual<typeof import("oriro/plugin-sdk/plugin-runtime")>(
    "oriro/plugin-sdk/plugin-runtime",
  );
  return {
    ...actual,
...
describe("registerTelegramNativeCommands — session metadata", () => {
  ...
  beforeEach(() => {
    ...
    sessionMocks.resolveAndPersistSessionFile.mockClear().mockImplementation(async (params) => {
      const sessionFile =
        params.fallbackSessionFile ?? `/tmp/oriro-sessions/${params.sessionId}.jsonl`;
      return {
        sessionFile,
        sessionEntry: {
          ...params.sessionEntry,
    ...
    sessionMocks.resolveStorePath.mockClear().mockReturnValue("/tmp/oriro-sessions.json");
  ...
  it("uses the target session model when building native argument menus", async () => {
    ...
    expect(sessionMocks.loadSessionStore).toHaveBeenCalledWith("/tmp/oriro-sessions.json");
    expectSendMessageCall({
      sendMessage,
      chatId: 100,
  ...
  it("passes a persisted topic session file to plugin commands", async () => {
    sessionMocks.resolveStorePath.mockReturnValue("/tmp/oriro-sessions/sessions.json");
    sessionMocks.loadSessionStore.mockReturnValue({
    ...
    const { handler } = registerAndResolveCommandHandler({
      commandName: "codex",
      cfg: { commands: { allowFrom: { telegram: ["200"] } } } as OriroConfig,
    ...
    expectRecordFields(
      firstMockArg(sessionMocks.resolveAndPersistSessionFile, "resolveAndPersistSessionFile"),
      {
        ...
        storePath: "/tmp/oriro-sessions/sessions.json",
        sessionsDir: "/tmp/oriro-sessions",
        fallbackSessionFile: path.resolve("/tmp/oriro-sessions", "sess-topic-topic-42.jsonl"),
      },
    ...
    expectRecordFields(
      (pluginRuntimeMocks.executePluginCommand.mock.calls as unknown as Array<[unknown]>)[0]?.[0],
      {
        sessionKey: "agent:main:telegram:group:-1001234567890:topic:42",
        sessionId: "sess-topic",
        sessionFile: path.resolve("/tmp/oriro-sessions", "sess-topic-topic-42.jsonl"),
