...
Oriro supports Grok as a `web_search` provider, using xAI web-grounded
...
Oriro reuses it as a fallback for the bundled xAI model provider too.
...
Oriro can use an existing xAI OAuth profile without prompting for a separate
web-search key. If OAuth is not available, it falls back to xAI API-key setup.
Oriro can also show a separate follow-up step to enable `x_search` with the
...
<Steps>
  <Step title="Use xAI OAuth">
    ...
    ```bash
    oriro onboard --auth-choice xai-oauth
...
**Credential alternatives:** sign in with `oriro models auth login
...
route through an operator proxy or xAI-compatible Responses endpoint. Oriro
