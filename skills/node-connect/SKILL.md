...
name: node-connect
description: "Diagnose ORIRO Android, iOS, or macOS node pairing, QR/setup code, route, auth, and connection failures."
---

# Node Connect

Goal: find the one real route from node -> gateway, verify Oriro is advertising that route, then fix pairing/auth.
...
- whether `oriro devices list` shows a pending pairing request

Do not guess from `can't connect`.

## Canonical checks

Prefer `oriro qr --json`. It uses the same setup-code payload Android scans.

```bash
oriro config get gateway.mode
oriro config get gateway.bind
oriro config get gateway.tailscale.mode
oriro config get gateway.remote.url
oriro config get gateway.auth.mode
oriro config get gateway.auth.allowTailscale
oriro config get plugins.entries.device-pair.config.publicUrl
oriro qr --json
oriro devices list
oriro nodes status
```

If this Oriro instance is pointed at a remote gateway, also run:

```bash
oriro qr --remote --json
```
...
If `oriro qr --json` says `Gateway is only bound to loopback`:
...
```bash
oriro devices list
oriro devices approve --latest   # preview only; copy the requestId from output
...
- `oriro devices list` shows pending requests: stop changing network config and approve first.
...
- `The gateway is still loopback-only, so a node on another network can never reach it. Enable Tailscale Serve, restart the gateway, run oriro qr again, rescan, then approve the pending device pairing.`
