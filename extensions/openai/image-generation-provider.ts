...
} from "oriro/plugin-sdk/image-generation";
import {
  parseOpenAiCompatibleImageResponse,
  toImageDataUrl,
} from "oriro/plugin-sdk/image-generation";
import { createSubsystemLogger } from "oriro/plugin-sdk/logging-core";
import { resolveClosestSize } from "oriro/plugin-sdk/media-generation-runtime";
import { extensionForMime } from "oriro/plugin-sdk/media-mime";
import {
  ensureAuthProfileStore,
  isProviderApiKeyConfigured,
  listProfilesForProvider,
  type AuthProfileStore,
} from "oriro/plugin-sdk/provider-auth";
import { resolveApiKeyForProvider } from "oriro/plugin-sdk/provider-auth-runtime";
import {
...
import { isPrivateNetworkOptInEnabled } from "oriro/plugin-sdk/ssrf-runtime";
import {
...
function shouldAllowPrivateImageEndpoint(req: {
  ...
  return process.env.ORIRO_QA_ALLOW_LOCAL_IMAGE_PROVIDER === "1";
}
...
function hasExplicitOpenAIImageApiKeyConfig(cfg: OriroConfig | undefined): boolean {
  const providerConfig = cfg?.models?.providers?.openai;
  return providerConfig?.apiKey !== undefined || providerConfig?.auth === "api-key";
}

function hasExplicitDirectOpenAIImageConfig(cfg: OriroConfig | undefined): boolean {
  const providerConfig = cfg?.models?.providers?.openai;
  if (!providerConfig) {
    return false;
  }
...
function hasChatGPTImageRouteConfig(cfg: OriroConfig | undefined): boolean {
  const providerConfig = cfg?.models?.providers?.openai;
  return (
...
function forceOpenAIImageApiKeyAuth(cfg: OriroConfig | undefined): OriroConfig | undefined {
