...
struct ControlAgentEvent: Codable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: OriroProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.oriro", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: OriroKit.AnyCodable]()) {
                $0[$1.key] = OriroKit.AnyCodable($1.value.base)
            }
        ...
        if let urlError = error as? URLError {
            ...
            case .notConnectedToInternet:
                if Self.isLikelyLocalNetworkPermissionBlock() {
                    return """
                    macOS is blocking Oriro Local Network access.
                    Allow Oriro in System Settings → Privacy & Security → Local Network, then relaunch the app.
                    """
                }
    ...
    private static func bridgeToProtocolArgs(
        _ value: OriroProtocol.AnyCodable?) -> [String: OriroProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: OriroProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: OriroKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: OriroProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("oriro.control.heartbeat")
    static let controlAgentEvent = Notification.Name("oriro.control.agent")
}
