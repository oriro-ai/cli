...
struct SettingsChannelsDestination: View {
    ...
    private var summaryColor: Color {
        ...
        if self.errorText != nil { return OriroBrand.warn }
        return self.channelEntries.contains(where: { $0.running || $0.connected }) ? OriroBrand.ok : OriroBrand
            .accent
    }
...
private struct SettingsChannelEntry: Identifiable {
    ...
    var color: Color {
        if self.connected || self.running { return OriroBrand.ok }
        if self.lastError != nil { return OriroBrand.warn }
        return self.configured ? OriroBrand.accent : .secondary
    }
