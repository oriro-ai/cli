...
import { triggerOriroRestart } from "./restart.js";
...
const PNPM_VERSIONED_ORIRO_ENTRY_PATTERN =
  /^(.*?)([\\/])node_modules\2\.pnpm\2oriro@[^\\/]+\2node_modules\2oriro\2.+$/;

function rewritePnpmVersionedOriroEntryPath(entryPath: string): string {
  // pnpm can expose argv[1] as a versioned realpath that self-update removes.
  // Respawn through the stable Oriro package wrapper instead.
  return entryPath.replace(
    PNPM_VERSIONED_ORIRO_ENTRY_PATTERN,
    "$1$2node_modules$2oriro$2oriro.mjs",
  );
}
...
function spawnDetachedGatewayProcess(opts: GatewayRespawnOptions = {}): {
  ...
  const args = [
    ...process.execArgv,
    ...(entryArg ? [rewritePnpmVersionedOriroEntryPath(entryArg)] : []),
    ...entryArgs,
  ];
 ...
 * - ORIRO_NO_RESPAWN=1: caller should keep in-process restart behavior (tests/dev)
...
export function restartGatewayProcessWithFreshPid(
  ...
  if (isTruthy(process.env.ORIRO_NO_RESPAWN)) {
  ...
  if (supervisor) {
    ...
    if (supervisor === "schtasks") {
      const restart = triggerOriroRestart();
      if (!restart.ok) {
        return {
          mode: "failed",
...
export function respawnGatewayProcessForUpdate(
  ...
  if (isTruthy(process.env.ORIRO_NO_RESPAWN)) {
    return { mode: "disabled", detail: "ORIRO_NO_RESPAWN" };
  }
