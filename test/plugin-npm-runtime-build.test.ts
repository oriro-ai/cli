...
describe("plugin npm runtime build planning", () => {
  it("plans package-local runtime entries for every publishable plugin package", () => {
    ...
    for (const plan of resolvedPlans) {
      ...
      expect(plan.packagePeerMetadata.peerDependencies.oriro).toBe(
        plan.packageJson.oriro.compat.pluginApi,
      );
      expect(plan.packagePeerMetadata.peerDependenciesMeta.oriro.optional).toBe(true);
    }
  });
