...
const PLUGIN_SPEC =
  process.env.ORIRO_KITCHEN_SINK_NPM_SPEC || "npm:@oriro/kitchen-sink@latest";
const PLUGIN_ID = process.env.ORIRO_KITCHEN_SINK_PLUGIN_ID || "oriro-kitchen-sink-fixture";
...
Runs the external Kitchen Sink plugin RPC walk against a built Oriro entry.

Environment:
  ORIRO_ENTRY                         Built Oriro entrypoint. Defaults to dist/index.mjs or dist/index.js.
  ORIRO_KITCHEN_SINK_NPM_SPEC         Plugin package spec. Default: npm:@oriro/kitchen-sink@latest.
  ORIRO_KITCHEN_SINK_PLUGIN_ID        Plugin id. Default: oriro-kitchen-sink-fixture.
  ORIRO_KITCHEN_SINK_PERSONALITY      Plugin fixture personality. Default: conformance.
  ORIRO_KITCHEN_SINK_RPC_PORT         Gateway loopback port. Default: OS-selected free port.
  ORIRO_KITCHEN_SINK_RPC_READY_MS     Gateway readiness timeout.
  ORIRO_KITCHEN_SINK_RPC_COMMAND_MS   Oriro command timeout.
  ORIRO_KITCHEN_SINK_RPC_INSTALL_MS   Plugin install timeout.
  ORIRO_KITCHEN_SINK_RPC_CALL_MS      RPC call timeout.
  ORIRO_KITCHEN_SINK_RPC_FETCH_MS     HTTP readiness probe timeout.
  ORIRO_KITCHEN_SINK_RPC_FETCH_BODY_BYTES  HTTP readiness probe response ceiling.
  ORIRO_KITCHEN_SINK_MAX_RSS_MIB      Gateway RSS ceiling.
  ORIRO_KITCHEN_SINK_COMMAND_MAX_RSS_MIB  Install/CLI command RSS ceiling.
  ORIRO_KITCHEN_SINK_OUTPUT_CAPTURE_CHARS  Per-command stdout/stderr capture ceiling.
  ORIRO_KITCHEN_SINK_KEEP_TMP=1       Preserve the isolated temp home.
`;
}
...
export function resolveKitchenSinkRpcConfig(env = process.env) {
  const commandTimeoutMs = readPositiveInt(
    env.ORIRO_KITCHEN_SINK_RPC_COMMAND_MS,
    DEFAULT_COMMAND_TIMEOUT_MS,
    "ORIRO_KITCHEN_SINK_RPC_COMMAND_MS",
  );
  return {
    commandMaxRssMiB: readPositiveInt(
      env.ORIRO_KITCHEN_SINK_COMMAND_MAX_RSS_MIB,
      DEFAULT_MAX_COMMAND_RSS_MIB,
      "ORIRO_KITCHEN_SINK_COMMAND_MAX_RSS_MIB",
    ),
    commandTimeoutMs,
    fetchBodyMaxBytes: readPositiveInt(
      env.ORIRO_KITCHEN_SINK_RPC_FETCH_BODY_BYTES,
      DEFAULT_FETCH_BODY_MAX_BYTES,
      "ORIRO_KITCHEN_SINK_RPC_FETCH_BODY_BYTES",
    ),
    fetchTimeoutMs: readPositiveInt(
      env.ORIRO_KITCHEN_SINK_RPC_FETCH_MS,
      DEFAULT_FETCH_TIMEOUT_MS,
      "ORIRO_KITCHEN_SINK_RPC_FETCH_MS",
    ),
    installTimeoutMs: readPositiveInt(
      env.ORIRO_KITCHEN_SINK_RPC_INSTALL_MS,
      Math.max(commandTimeoutMs, DEFAULT_INSTALL_TIMEOUT_MS),
      "ORIRO_KITCHEN_SINK_RPC_INSTALL_MS",
    ),
    maxRssMiB: readPositiveInt(
      env.ORIRO_KITCHEN_SINK_MAX_RSS_MIB,
      DEFAULT_MAX_RSS_MIB,
      "ORIRO_KITCHEN_SINK_MAX_RSS_MIB",
    ),
    outputCaptureChars: readPositiveInt(
      env.ORIRO_KITCHEN_SINK_OUTPUT_CAPTURE_CHARS,
      DEFAULT_OUTPUT_CAPTURE_CHARS,
      "ORIRO_KITCHEN_SINK_OUTPUT_CAPTURE_CHARS",
    ),
    readyTimeoutMs: readPositiveInt(
      env.ORIRO_KITCHEN_SINK_RPC_READY_MS,
      DEFAULT_READY_TIMEOUT_MS,
      "ORIRO_KITCHEN_SINK_RPC_READY_MS",
    ),
    rpcTimeoutMs: readPositiveInt(
      env.ORIRO_KITCHEN_SINK_RPC_CALL_MS,
      DEFAULT_RPC_TIMEOUT_MS,
      "ORIRO_KITCHEN_SINK_RPC_CALL_MS",
    ),
  };
}
...
export async function resolveKitchenSinkRpcPort(env = process.env, options = {}) {
  const rawPort = (env.ORIRO_KITCHEN_SINK_RPC_PORT || "").trim();
  if (rawPort) {
    const port = readPositiveInt(rawPort, 0, "ORIRO_KITCHEN_SINK_RPC_PORT");
    if (port > 65535) {
      throw new Error(
        `ORIRO_KITCHEN_SINK_RPC_PORT must be a TCP port from 1 to 65535. Got: ${JSON.stringify(rawPort)}`,
      );
    }
    return port;
  }
...
function resolveOriroRunner() {
  if (process.env.ORIRO_ENTRY) {
    return {
      command: "node",
      baseArgs: [process.env.ORIRO_ENTRY],
      label: process.env.ORIRO_ENTRY,
    };
  }
  ...
  return { pnpm: true, baseArgs: ["oriro"], label: "pnpm oriro" };
}

export function makeEnv() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-kitchen-sink-rpc-"));
  ...
  return {
    ...
    env: {
      ...
      ORIRO_NO_ONBOARD: "1",
      ORIRO_SKIP_PROVIDERS: "0",
      ORIRO_KITCHEN_SINK_PERSONALITY:
        process.env.ORIRO_KITCHEN_SINK_PERSONALITY || "conformance",
    },
  };
}
...
async function runOriro(runner, args, env, options = {}) {
  const config = resolveKitchenSinkRpcConfig(env);
  const command = await resolveOriroCommand(runner, args, env, {
    stdio: ["ignore", "pipe", "pipe"],
  });
...
async function resolveOriroCommand(runner, args, env, options = {}) {
  if (runner.pnpm) {
...
async function rpcCall(method, params, options) {
  ...
  const payload = module
    ? await module.callGateway({
        config: readJson(options.env.ORIRO_CONFIG_PATH),
        configPath: options.env.ORIRO_CONFIG_PATH,
...
export function usesBuiltOriroEntry(runner, cwd = process.cwd(), env = process.env) {
  if (runner?.pnpm || !runner?.baseArgs?.[0]) {
    return false;
  }
  const entry = runner.baseArgs[0];
  if (env.ORIRO_ENTRY && entry === env.ORIRO_ENTRY) {
    return true;
  }
...
function configureKitchenSink(env, port) {
  const configPath = env.ORIRO_CONFIG_PATH;
    ...
    entries: {
      ...
      [PLUGIN_ID]: {
        ...
        config: {
          ...
          personality: env.ORIRO_KITCHEN_SINK_PERSONALITY,
        },
        hooks: {
...
async function startGateway(runner, port, env, logPath) {
  ...
  const command = await resolveOriroCommand(
    runner,
...
async function samplePosixProcessTree(pid, run, commandLineNeedles) {
  ...
  try {
    ...
    const gatewayTitleMatches = descendants.filter((row) =>
      row.command.toLowerCase().includes("oriro-gateway"),
    );
...
export async function main() {
  const config = resolveKitchenSinkRpcConfig();
  let runner = resolveOriroRunner();
  const port = await resolveKitchenSinkRpcPort();
  const { root, env } = makeEnv();
  const logPath = path.join(root, "gateway.log");
  const keepTmp = process.env.ORIRO_KITCHEN_SINK_KEEP_TMP === "1";
  let failed = false;
  let child;
  ...
  try {
    ...
    const inspect = parseJsonOutput(
      (
        await runOriro(runner, ["plugins", "inspect", PLUGIN_ID, "--runtime", "--json"], env, {
          ...commandResourceOptions,
    ...
    assertKitchenSinkUiDescriptors(uiDescriptors, {
      expectDescriptor: env.ORIRO_KITCHEN_SINK_PERSONALITY !== "conformance",
    });
