...
async function makeTempStorePath() {
  tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-doctor-cron-"));
...
function createCronConfig(storePath: string): OriroConfig {
  return {
    cron: {
      store: storePath,
...
describe("maybeRepairLegacyCronStore", () => {
  ...
  it("removes inert legacy notify:true for delivery.mode none when cron.webhook is unset and stops looping (#44460)", async () => {
    ...
    const cfg = { cron: { store: storePath } } as OriroConfig;
    await maybeRepairLegacyCronStore({
      cfg,
      options: {},
      prompter: makePrompter(true),
  ...
  it("rewrites stale managed dreaming jobs to the isolated agentTurn shape", async () => {
    ...
    expect(payload.message).toBe("__oriro_memory_core_short_term_promotion_dream__");
    ...
    const warning = await collectLegacyWhatsAppCrontabHealthWarning({
      ...
      readCrontab: async () => ({
        stdout: [
          ...
          "*/5 * * * * ~/.oriro/bin/ensure-whatsapp.sh >> ~/.oriro/logs/whatsapp-health.log 2>&1",
          "0 9 * * * /usr/bin/true",
          "",
        ].join("\n"),
      }),
  ...
  it("ignores malformed crontab output instead of crashing", async () => {
    ...
    await expect(
      noteLegacyWhatsAppCrontabHealthCheck({
        platform: "linux",
        readCrontab: async () => ({
          stdout: { lines: ["*/5 * * * * ~/.oriro/bin/ensure-whatsapp.sh"] },
        }),
      }),
    ).resolves.toBeUndefined();
