/**
 * Bridges Oriro runtime tools into Codex app-server dynamic tool specs and
 * tool-call responses.
 */
...
import type { ImageContent, TextContent } from "oriro/plugin-sdk/llm";
...
/** Namespace attached to Oriro-owned dynamic tools exposed to Codex. */
export const CODEX_ORIRO_DYNAMIC_TOOL_NAMESPACE = "oriro";

// Keep Oriro session spawning searchable in Codex mode so Codex's native
...
/**
 * Creates dynamic tool specs and a call handler that executes Oriro tools,
...
function createCodexDynamicToolSpecs(params: {
  ...
  if (namespaceTools.length > 0) {
    specs.push({
      type: "namespace",
      name: CODEX_ORIRO_DYNAMIC_TOOL_NAMESPACE,
      description: "",
