---
summary: "Install Oriro - installer script, npm/pnpm/bun, from source, Docker, and more"
read_when:
...
The fastest way to install. It detects your OS, installs Node if needed, installs Oriro, and launches onboarding.
...
<Tabs>
  <Tab title="macOS / Linux / WSL2">
    ```bash
    curl -fsSL https://oriro.ai/install.sh | bash -s -- --no-onboard
    ```
  </Tab>
  <Tab title="Windows (PowerShell)">
    ```powershell
...
Use this when you want Oriro and Node kept under a local prefix such as
`~/.oriro`, without depending on a system-wide Node install:

```bash
curl -fsSL https://oriro.ai/install-cli.sh | bash
```
...
`oriro update --channel dev` and `oriro update --channel stable`. See
...
<Tabs>
  <Tab title="npm">
    ...
    for the Oriro package install. If you install manually with npm, your own
...
Or skip the link and use `pnpm oriro ...` from inside the repo. See [Setup](/start/setup) for full development workflows.
...
```bash
oriro --version      # confirm the CLI is available
oriro doctor         # check for config issues
oriro gateway status # verify the Gateway is running
```

If you want managed startup after install:

- macOS: LaunchAgent via `oriro onboard --install-daemon` or `oriro gateway install`
- Linux/WSL2: systemd user service via the same commands
- Native Windows: Scheduled Task first, with a per-user Startup-folder login item fallback if task creation is denied

## Hosting and deployment

Deploy Oriro on a cloud server or VPS:
...
<CardGroup cols={3}>
  ...
  <Card title="Uninstall" href="/install/uninstall" icon="trash-2">
    Remove Oriro completely.
  </Card>
</CardGroup>

## Troubleshooting: `oriro` not found

If the install succeeded but `oriro` is not found in your terminal:
