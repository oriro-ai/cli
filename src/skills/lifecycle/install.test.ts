...
import { resolveOriroMetadata, resolveSkillInvocationPolicy } from "../loading/frontmatter.js";
...
metadata: {"oriro":{"install":[{"id":"deps","kind":"node","package":"example-package"}]}}
---
...
function loadTestWorkspaceSkillEntries(workspaceDir: string): SkillEntry[] {
  ...
  return skills.map((skill) => {
    ...
    return {
      skill,
      frontmatter,
      metadata: resolveOriroMetadata(frontmatter),
      invocation,
      exposure: {
...
const workspaceSuite = createFixtureSuite("oriro-skills-install-");
...
describe("installSkill before_install hooks", () => {
  beforeEach(() => {
    ...
    skillsInstallTesting.setDepsForTest({
      ...
      resolveNodeInstallStateDir: () => {
        ...
        if (!stateDir) {
          throw new Error("ORIRO_STATE_DIR missing in skills install test");
        }
        return stateDir;
      },
    });
  ...
  it("runs npm node installs with an Oriro-managed user prefix", async () => {
  ...
  it("keeps the default npm prefix out of env-overridden state paths", () => {
    const envSnapshot = captureEnv(["ORIRO_STATE_DIR", "ORIRO_CONFIG_PATH"]);
    try {
      process.env.ORIRO_STATE_DIR = "/tmp/untrusted-state";
      process.env.ORIRO_CONFIG_PATH = "/tmp/untrusted-config/oriro.json";
  ...
  it("surfaces plugin hook findings from before_install", async () => {
    ...
    await withWorkspaceCase(async ({ workspaceDir }) => {
      ...
      expect(payload?.origin).toBe("oriro-workspace");
