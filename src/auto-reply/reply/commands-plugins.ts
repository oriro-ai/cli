  ...
  const lines = [
    ...
    ...report.plugins.map((plugin) => {
      const format = plugin.bundleFormat
        ? `${plugin.format ?? "oriro"}/${plugin.bundleFormat}`
        : (plugin.format ?? "oriro");
...
async function installPluginFromPluginsCommand(params: {
  ...
  const orirohubSpec = parseOriroHubPluginSpec(params.raw);
  if (orirohubSpec) {
    ...
    await persistPluginInstall({
      ...
      install: {
        ...
        installPath: result.targetDir,
        version: result.version,
        integrity: result.orirohub.integrity,
        resolvedAt: result.orirohub.resolvedAt,
        orirohubUrl: result.orirohub.orirohubUrl,
        orirohubPackage: result.orirohub.orirohubPackage,
        orirohubFamily: result.orirohub.orirohubFamily,
        orirohubChannel: result.orirohub.orirohubChannel,
      },
    });
    return { ok: true, pluginId: result.pluginId };
  }
...
export const handlePluginsCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  let committedConfig: OriroConfig;
  try {
