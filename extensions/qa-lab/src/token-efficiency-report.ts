...
export type TokenEfficiencyRow = {
  ...
  oriro: TokenEfficiencyRuntimeUsage;
...
export type TokenEfficiencyReport = {
  ...
  aggregate: {
    oriro: { totalTokens: number; p50PerScenario: number; p90PerScenario: number };
...
const ZERO_AGGREGATE: TokenEfficiencyReport["aggregate"] = {
  oriro: { totalTokens: 0, p50PerScenario: 0, p90PerScenario: 0 },
...
function deltaPercent(oriroTotalTokens: number, codexTotalTokens: number): number {
  if (oriroTotalTokens === 0) {
    return codexTotalTokens === 0 ? 0 : 100;
  }
  return ((codexTotalTokens - oriroTotalTokens) / oriroTotalTokens) * 100;
}
...
function toolNamesForCells(oriro: RuntimeParityCell, codex: RuntimeParityCell): string[] {
  return [
    ...new Set([...oriro.toolCalls, ...codex.toolCalls].map((call) => call.tool)),
...
function buildRow(params: {
  ...
  const oriro = runtimeUsage(params.result.cells.oriro);
  const codex = runtimeUsage(params.result.cells.codex);
  const delta = deltaPercent(oriro.totalTokens, codex.totalTokens);
  ...
  return {
    ...
    classification,
    flagged,
    toolsUsed: toolNamesForCells(params.result.cells.oriro, params.result.cells.codex),
  };
}

function buildAggregate(rows: readonly TokenEfficiencyRow[]): TokenEfficiencyReport["aggregate"] {
  const oriroTotals = rows.map((row) => row.oriro.totalTokens);
  const codexTotals = rows.map((row) => row.codex.totalTokens);
  const oriroTotalTokens = oriroTotals.reduce((sum, value) => sum + value, 0);
  const codexTotalTokens = codexTotals.reduce((sum, value) => sum + value, 0);
  return {
    oriro: {
      totalTokens: oriroTotalTokens,
      p50PerScenario: percentile(oriroTotals, 50),
      p90PerScenario: percentile(oriroTotals, 90),
    },
    codex: {
    ...
    deltaPercent: deltaPercent(oriroTotalTokens, codexTotalTokens),
...
function liveEvidenceFailures(row: TokenEfficiencyRow): string[] {
  ...
  if (row.oriro.totalTokens <= 0) {
    failures.push(`${row.scenarioId} oriro live usage totalTokens=${row.oriro.totalTokens}`);
  }
...
export function buildTokenEfficiencyReport(
  ...
  return {
    ...
    notes: [
      ...
      "Codex savings are reported as savings and do not fail the gate; only positive Codex-over-Oriro live deltas exceed the threshold.",
...
export function renderTokenEfficiencyMarkdownReport(report: TokenEfficiencyReport): string {
  ...
  lines.push(
    ...
    "| --- | ---: | ---: | ---: |",
    `| oriro | ${report.aggregate.oriro.totalTokens} | ${report.aggregate.oriro.p50PerScenario} | ${report.aggregate.oriro.p90PerScenario} |`,
  ...
  if (report.rows.length > 0) {
    ...
    for (const row of report.rows) {
      lines.push(
        `| ${row.scenarioId} | ${row.usageSource} | ${row.oriro.inputTokens}/${row.oriro.outputTokens}/${row.oriro.totalTokens}/${row.oriro.toolCallCount} | ${row.codex.inputTokens}/${row.codex.outputTokens}/${row.codex.totalTokens}/${row.codex.toolCallCount} | ${formatPercent(row.deltaPercent)} | ${row.classification} | ${row.flagged ? "yes" : "no"} | ${row.toolsUsed.join(", ")} |`,
      );
    }
    lines.push("");
  }
