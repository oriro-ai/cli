...
import type { ChannelSetupWizard } from "oriro/plugin-sdk/setup";
import { formatCliCommand, formatDocsLink } from "oriro/plugin-sdk/setup-tools";
import {
...
type WhatsAppConfig = NonNullable<NonNullable<OriroConfig["channels"]>["whatsapp"]>;
...
function shouldWriteDefaultWhatsAppAccountConfigAtAccountScope(cfg: OriroConfig): boolean {
...
function resolveDefaultWhatsAppAccountWriteKey(cfg: OriroConfig): string {
  const accounts = cfg.channels?.whatsapp?.accounts;
  if (!accounts) {
...
function resolveWhatsAppConfigPathPrefix(cfg: OriroConfig, accountId: string): string {
  if (
...
async function detectWhatsAppLinked(cfg: OriroConfig, accountId: string): Promise<boolean> {
  ...
  if (wantsLink) {
    ...
    await params.prompter.note(
      t("wizard.whatsapp.linkLater", {
        command: formatCliCommand("oriro channels login"),
      }),
      "WhatsApp",
    );
  }
