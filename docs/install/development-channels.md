...
Oriro ships three update channels:
...
- **`dev`**: ensures a git checkout (default `~/oriro`, or
  `$ORIRO_HOME/oriro` when `ORIRO_HOME` is set; override with
  `ORIRO_GIT_DIR`), switches to `main`, rebases on upstream, builds, and
...
oriro update --tag 2026.4.1-beta.1
...
oriro update --tag oriro@2026.4.1-beta.1
...
- The tag is not persisted. Your next `oriro update` uses your configured
  channel as usual.
- For package installs, Oriro pre-packs GitHub/git source specs into a
...
- Downgrade protection: if the target version is older than your current version,
  Oriro prompts for confirmation (skip with `--yes`).
...
Preview what `oriro update` would do without making changes:

```bash
oriro update --dry-run
oriro update --channel beta --dry-run
oriro update --tag 2026.4.1-beta.1 --dry-run
oriro update --dry-run --json
```
...
When you switch channels with `oriro update`, Oriro also syncs plugin
sources:
