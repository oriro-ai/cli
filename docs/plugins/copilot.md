---
summary: "Run Oriro embedded agent turns through the external GitHub Copilot SDK harness"
...
read_when:
  ...
  - You are wiring an agent to subscription Copilot (github / oriro / copilot) and want it to run through the Copilot CLI
---

The external `@oriro/copilot` plugin lets Oriro run embedded subscription
...
Oriro still owns chat channels, session files, model selection, Oriro
...
[Side questions (`/btw`)](#side-questions-btw)), and `oriro doctor`.
...
- Oriro with the `@oriro/copilot` plugin installed.
- If your config uses `plugins.allow`, include `copilot` (the manifest
  id declared by the plugin). A restrictive
  allowlist that uses the npm-style `@oriro/copilot` package name
...
- A writable `copilotHome` directory. The harness defaults to
  `~/.oriro/agents/<agentId>/copilot` for full per-agent isolation. The
...
The Copilot runtime is an external plugin so the core `oriro` package does
...
```bash
oriro plugins install @oriro/copilot
```
...
Without the opt-in, oriro uses its built-in GitHub Copilot provider
and never installs the runtime plugin.

The runtime resolves the SDK in this order:

1. `import("@github/copilot-sdk")` from the installed `@oriro/copilot`
   package.
2. The well-known fallback dir `~/.oriro/npm-runtime/copilot/` (the
...
4. **Env-var fallback** for direct CLI / dogfood runs where no auth
   ...
   setup:
   1. `ORIRO_GITHUB_TOKEN` -- harness-specific override; set this
      to pin a token for the Oriro harness without disturbing
...
(isolating SDK state from Oriro's `models.json` / `auth-profiles.json` in
the same directory), or `~/.oriro/agents/<agentId>/copilot` otherwise.
...
- `model` — string or `{ provider, id, api? }`. When omitted, Oriro uses
...
- `reasoningEffort` — `"low" | "medium" | "high" | "xhigh"`. Maps from
  Oriro's `ThinkLevel` / `ReasoningLevel` resolution in
...
- `infiniteSessionConfig` — optional override for the SDK
  ...
  leave as-is.
- `hooksConfig` — optional bridge config exposing Oriro
...
- `permissionPolicy` — optional override for the SDK's
  ...
  invokes any of those kinds because every bridged Oriro tool is
  registered with `overridesBuiltInTool: true` and
  `skipPermission: true` so 100% of tool calls flow through Oriro's
...
Nothing in the rest of Oriro needs to know about these fields. Other
...
The Oriro side transcript mirror (see below) continues to receive the
...
deliberately leaves `harness.runSideQuestion` undefined, so Oriro's `/btw`
...
asserted in
[`extensions/copilot/harness.test.ts`](https://github.com/oriro-ai/cli/blob/main/extensions/copilot/harness.test.ts)
...
that hosts (including `oriro doctor`) can call to verify the environment:
...
- **Interactive `ask_user` is not yet wired.** The SDK's
  ...
  `UserInputRequest`s through the Oriro channel/TUI prompt path.

## Permissions and ask_user

Permission enforcement for bridged Oriro tools happens **inside the
...
`createOriroCodingTools` to every coding tool: loop detection,
...
`createOriroCodingTools` — identity (`senderIsOwner`,
