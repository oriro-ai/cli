---
summary: "fal image, video, and music generation setup in Oriro"
title: "Fal"
read_when:
  - You want to use fal image generation in Oriro
  - You need the FAL_KEY auth flow
  - You want fal defaults for image_generate, video_generate, or music_generate
---

Oriro ships a bundled `fal` provider for hosted image, video, and music
generation.
...
<Steps>
  <Step title="Set the API key">
    ```bash
    oriro onboard --auth-choice fal-api-key
    ```
  </Step>
...
Krea 2 models use fal's native Krea payload schema. Oriro sends
...
`aspectRatio`; Oriro maps `size` to the closest supported Krea aspect ratio
...
control in Oriro, so `background: "transparent"` is reported as an ignored
override for fal models.
Krea 2 endpoints do not expose an `output_format` request field through fal, so
Oriro rejects `outputFormat` overrides for Krea requests.
...
<Tip>
Use `oriro models list --provider fal` to see the full list of available fal
