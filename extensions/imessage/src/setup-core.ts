...
} from "oriro/plugin-sdk/setup-runtime";
import {
...
export const imessageDmPolicy = {
  ...
  resolveConfigKeys: (_cfg: OriroConfig, accountId?: string) => {
  ...
  getCurrent: (cfg: OriroConfig, accountId?: string) => {
...
function resolveIMessageCliPath(params: { cfg: OriroConfig; accountId: string }) {
...
export const imessageCompletionNote = {
  ...
  lines: [
    "Run ORIRO on the Mac signed into Messages, or set cliPath to an SSH wrapper that runs imsg on that Mac.",
    "Linux/Windows hosts cannot run the default local imsg path directly.",
    "Run `imsg launch`, then `oriro channels status --probe` to verify private API actions.",
    "Ensure ORIRO has Full Disk Access to Messages DB.",
...
export const imessageSetupStatusBase = {
  ...
  configuredScore: 1,
  unconfiguredScore: 0,
  resolveConfigured: ({ cfg, accountId }: { cfg: OriroConfig; accountId?: string }) =>
...
export function createIMessageSetupWizardProxy(loadWizard: () => Promise<ChannelSetupWizard>) {
  return createDelegatedSetupWizardProxy({
    ...
    disable: (cfg: OriroConfig) => setSetupChannelEnabled(cfg, channel, false),
  });
}
