...
describe("light background detection", () => {
  it("uses dark palette by default", async () => {
    const mod = await importThemeWithEnv({
      ORIRO_THEME: undefined,
      COLORFGBG: undefined,
    });
    expect(mod.lightMode).toBe(false);
  });

  it("selects light palette when ORIRO_THEME=light", async () => {
    const mod = await importThemeWithEnv({ ORIRO_THEME: "light" });
    expect(mod.lightMode).toBe(true);
  });

  it("selects dark palette when ORIRO_THEME=dark", async () => {
    const mod = await importThemeWithEnv({ ORIRO_THEME: "dark" });
    expect(mod.lightMode).toBe(false);
  });

  it("treats ORIRO_THEME case-insensitively", async () => {
    const mod = await importThemeWithEnv({ ORIRO_THEME: "LiGhT" });
    expect(mod.lightMode).toBe(true);
  });
  ...
  it("ORIRO_THEME overrides COLORFGBG", async () => {
    const mod = await importThemeWithEnv({
      ORIRO_THEME: "dark",
      COLORFGBG: "0;15",
    });
  ...
  it("keeps assistantText as identity in both modes", async () => {
    const lightMod = await importThemeWithEnv({ ORIRO_THEME: "light" });
    const darkMod = await importThemeWithEnv({ ORIRO_THEME: "dark" });
