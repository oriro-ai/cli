...
describe("tool search gateway e2e fetch helper", () => {
  it("rejects loose numeric env limits instead of parsing prefixes", () => {
    expect(() =>
      readToolSearchGatewayFetchLimits({
        ORIRO_TOOL_SEARCH_GATEWAY_E2E_FETCH_TIMEOUT_MS: "1e3",
      }),
    ).toThrow("invalid ORIRO_TOOL_SEARCH_GATEWAY_E2E_FETCH_TIMEOUT_MS: 1e3");
    expect(() =>
      readToolSearchGatewayFetchLimits({
        ORIRO_TOOL_SEARCH_GATEWAY_E2E_FETCH_BODY_MAX_BYTES: "1000ms",
      }),
    ).toThrow("invalid ORIRO_TOOL_SEARCH_GATEWAY_E2E_FETCH_BODY_MAX_BYTES: 1000ms");
    expect(
      readToolSearchGatewayFetchLimits({
        ORIRO_TOOL_SEARCH_GATEWAY_E2E_FETCH_BODY_MAX_BYTES: "4096",
        ORIRO_TOOL_SEARCH_GATEWAY_E2E_FETCH_TIMEOUT_MS: "120000",
      }),
    ).toEqual({
      bodyMaxBytes: 4096,
      timeoutMs: 120_000,
...
describe("tool search gateway e2e environment helpers", () => {
  it("restores mutated gateway environment values", () => {
    const env: NodeJS.ProcessEnv = {
      ORIRO_CONFIG_PATH: "/before/oriro.json",
      ORIRO_STATE_DIR: "/before/state",
    };
    const snapshot = snapshotToolSearchGatewayEnv(env);

    env.ORIRO_CONFIG_PATH = "/after/oriro.json";
    env.ORIRO_STATE_DIR = "/after/state";
    env.ORIRO_TEST_FAST = "1";
