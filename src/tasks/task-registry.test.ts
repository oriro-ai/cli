...
function configureTaskRegistryMaintenanceRuntimeForTest(params: {
  ...
  setTaskRegistryMaintenanceRuntimeForTests({
    ...
    resolveCronJobsStorePath: () => "/tmp/oriro-test-cron/jobs.json",
...
function createAcpSessionStoreEntry(params: {
  ...
  return {
    cfg: {} as never,
    storePath: "/tmp/oriro-test-sessions.json",
...
async function withTaskRegistryTempDir<T>(
  ...
  return await withTempDir({ prefix: "oriro-task-registry-" }, async (root) => {
    return await withEnvAsync({ ORIRO_STATE_DIR: root }, async () => {
...
describe("task-registry", () => {
  ...
  it("cancels childless codex-native tasks without routing through Oriro subagent sessions", async () => {
    await withTaskRegistryTempDir(async () => {
      resetTaskRegistryForTests();
      const task = createTaskRecord({
        runtime: "subagent",
