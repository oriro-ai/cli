...
function expectOriroSchemaInvalidPreservesField(params: {
  config: unknown;
  ...
  const res = OriroSchema.safeParse(params.config);
...
describe("legacy config detection", () => {
  ...
  it("rejects removed legacy provider sections", () => {
    expectOriroSchemaInvalidPreservesField({
  ...
  it("preserves claude-cli auth profile mode during validation", () => {
    ...
    const res = OriroSchema.safeParse(config);
