---
name: oriro-qa-testing
description: Run, watch, debug, extend, or explain Oriro qa-lab and qa-channel scenarios, artifacts, and live lanes.
---
...
```bash
ORIRO_LIVE_OPENAI_KEY="${OPENAI_API_KEY}" \
pnpm oriro qa suite \
...
5. If the user wants to watch the live UI, find the current `oriro-qa` listen port and report `http://127.0.0.1:<port>`.
...
`pnpm oriro qa matrix` defaults to the full `all` profile. Use explicit
profiles for faster CI/release proof:

```bash
ORIRO_QA_MATRIX_NO_REPLY_WINDOW_MS=3000 \
...
- Additional related notes/login items seen during QA credential work:
  - vault: `Private`
  - items: `ORIRO QA`, `Convex`, `Telegram`
- If a required value is missing from those notes:
  ...
  - for Telegram direct runs, `ORIRO_QA_TELEGRAM_GROUP_ID` may be stored separately from `Telegram E2E`
  - for Convex runs, the leased Telegram credential should provide the Telegram group id and bot tokens together; do not require a separate `ORIRO_QA_TELEGRAM_GROUP_ID`
  - for Convex runs, prefer `Oriro/ORIRO_QA_CONVEX_SITE_URL`; if that is stale or unclear, ask for the active pool URL before running
...
ORIRO_QA_TELEGRAM_DRIVER_BOT_TOKEN="..." \
ORIRO_QA_TELEGRAM_SUT_BOT_TOKEN="..." \
ORIRO_QA_PROVIDER_MODE="mock-openai" \
ORIRO_NPM_TELEGRAM_PACKAGE_SPEC="oriro@beta" \
...
- Live npm Telegram Docker lane note:
  - `scripts/e2e/npm-telegram-live-runner.ts` reads `ORIRO_NPM_TELEGRAM_PROVIDER_MODE`
  - do not assume `ORIRO_QA_PROVIDER_MODE` is consumed by that wrapper
  - if a 1Password note only gives `ORIRO_QA_PROVIDER_MODE`, map it explicitly to `ORIRO_NPM_TELEGRAM_PROVIDER_MODE` before running the Docker lane
- Verified live shape:
  ...
  - a real run of `pnpm test:docker:npm-telegram-live` passed with:
    ...
    - `ORIRO_NPM_TELEGRAM_PROVIDER_MODE=mock-openai`
...
- Use two dedicated WhatsApp-capable test numbers: one driver account and one SUT account. Do not use personal numbers or personal Oriro WhatsApp accounts in the shared pool.
...
- Validate with `pnpm oriro qa whatsapp --credential-source convex --credential-role maintainer --provider-mode mock-openai` and preserve artifact paths plus redacted pass/fail summaries.
...
- For isolated character/persona evals, write the persona into `SOUL.md` and blank `IDENTITY.md` in the scenario flow. Use `SOUL.md + IDENTITY.md` only when intentionally testing how the normal Oriro identity combines with the character.
...
```bash
pnpm oriro qa manual \
  --model codex-cli/<codex-model> \
  --message "Reply exactly: CODEX_OK"
```

- Treat the concrete Codex model name as user/config input; do not hardcode it in source, docs examples, or scenarios.
- Live QA preserves `CODEX_HOME` so Codex CLI auth/config works while keeping `HOME` and `ORIRO_HOME` sandboxed.
