...
import { resetDiagnosticEventsForTest } from "oriro/plugin-sdk/diagnostic-runtime";
import { clearInternalHooks, resetGlobalHookRunner } from "oriro/plugin-sdk/hook-runtime";
import { clearMemoryPluginState } from "oriro/plugin-sdk/memory-core-host-runtime-core";
import { clearPluginCommands } from "oriro/plugin-sdk/plugin-runtime";
...
export function setupRunAttemptTestHooks(): void {
  beforeEach(async () => {
    ...
    vi.stubEnv("ORIRO_TRAJECTORY", "0");
    vi.stubEnv("CODEX_API_KEY", "");
    vi.stubEnv("OPENAI_API_KEY", "");
    tempDir = await fs.mkdtemp(path.join(resolvePreferredOriroTmpDir(), "oriro-codex-run-"));
  });
  ...
  afterEach(async () => {
    ...
    testing.resetOriroCodingToolsFactoryForTests();
