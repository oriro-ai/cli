...
import { buildOauthProviderAuthResult } from "oriro/plugin-sdk/provider-auth";
import { createProviderApiKeyAuthMethod } from "oriro/plugin-sdk/provider-auth-api-key";
...
import { MINIMAX_FAST_MODE_STREAM_HOOKS } from "oriro/plugin-sdk/provider-stream-family";
import { fetchMinimaxUsage } from "oriro/plugin-sdk/provider-usage";
...
function getProviderBaseUrl(cfg: OriroConfig, providerId: string): string | undefined {
  return normalizeOptionalString(cfg.models?.providers?.[providerId]?.baseUrl);
}

function resolveMinimaxUsageBaseUrl(cfg: OriroConfig): string | undefined {
...
export function registerMinimaxProviders(api: OriroPluginApi) {
