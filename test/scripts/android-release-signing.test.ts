...
function makeTempRoot(): string {
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-android-signing-"));
    ...
    `${JSON.stringify(
      {
        ...
        gradlePropertyNames: [
          "ORIRO_ANDROID_STORE_FILE",
          "ORIRO_ANDROID_STORE_PASSWORD",
          "ORIRO_ANDROID_KEY_ALIAS",
          "ORIRO_ANDROID_KEY_PASSWORD",
        ],
      },
      null,
      2,
...
function writeSigningSources(tempRoot: string) {
  ...
  fs.writeFileSync(
    propertiesPath,
    [
      `ORIRO_ANDROID_STORE_PASSWORD=${STORE_PASSWORD}`,
      "ORIRO_ANDROID_KEY_ALIAS=oriro-upload",
      `ORIRO_ANDROID_KEY_PASSWORD=${KEY_PASSWORD}`,
      "",
    ].join("\n"),
  );
...
describe("scripts/android-release-signing.mjs", () => {
  it("documents the canonical Android release signing plan", () => {
    ...
    expect(result.stdout).toContain("Signing repo: git@github.com:oriro/apps-signing.git");
    expect(result.stdout).toContain("Signing assets: android/oriro");
  ...
  it.runIf(commandAvailable("openssl"))(
    ...
    () => {
      ...
      const encryptedProperties = fs.readFileSync(
        path.join(remoteCheck, "android", "oriro", "gradle.properties.enc"),
        "utf8",
      );
      ...
      expect(materializedProperties).toContain(
        `ORIRO_ANDROID_STORE_FILE=${path.join(materializedDir, "upload-keystore.jks")}`,
      );
      expect(materializedProperties).toContain(`ORIRO_ANDROID_STORE_PASSWORD=${STORE_PASSWORD}`);
      expect(materializedProperties).toContain("ORIRO_ANDROID_KEY_ALIAS=oriro-upload");
      expect(materializedProperties).toContain(`ORIRO_ANDROID_KEY_PASSWORD=${KEY_PASSWORD}`);
