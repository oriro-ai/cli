...
const ORIGINAL_BUILD_PRIVATE_QA = process.env.ORIRO_BUILD_PRIVATE_QA;
const ORIGINAL_QA_FORCE_RUNTIME = process.env.ORIRO_QA_FORCE_RUNTIME;
...
function restoreEnv(
  name: "ORIRO_BUILD_PRIVATE_QA" | "ORIRO_QA_FORCE_RUNTIME",
...
function makeProviderRuntimeConfig(runtime: string): OriroConfig {
  return {
    models: {
      providers: {
        openai: {
...
afterEach(() => {
  restoreEnv("ORIRO_BUILD_PRIVATE_QA", ORIGINAL_BUILD_PRIVATE_QA);
  restoreEnv("ORIRO_QA_FORCE_RUNTIME", ORIGINAL_QA_FORCE_RUNTIME);
});

describe("resolveModelRuntimePolicy", () => {
  it("ignores the QA force-runtime override when the private QA gate is unset", () => {
    deleteTestEnvValue("ORIRO_BUILD_PRIVATE_QA");
    setTestEnvValue("ORIRO_QA_FORCE_RUNTIME", "oriro");
  ...
  it("respects the QA force-runtime override when the private QA gate is set", () => {
    ...
    setTestEnvValue("ORIRO_BUILD_PRIVATE_QA", "1");
    setTestEnvValue("ORIRO_QA_FORCE_RUNTIME", "oriro");
  ...
  it("ignores invalid QA force-runtime values even when the private QA gate is set", () => {
    setTestEnvValue("ORIRO_BUILD_PRIVATE_QA", "1");
    setTestEnvValue("ORIRO_QA_FORCE_RUNTIME", "bogus");
