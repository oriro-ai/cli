...
async function persistTestSessionConfig(): Promise<void> {
  ...
  if (process.env.ORIRO_CONFIG_PATH) {
    configPaths.add(process.env.ORIRO_CONFIG_PATH);
  }
  if (process.env.ORIRO_STATE_DIR) {
    configPaths.add(path.join(process.env.ORIRO_STATE_DIR, "oriro.json"));
  }
...
async function setupGatewayTestHome() {
  ...
  tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-gateway-home-"));
...
function applyGatewaySkipEnv() {
  ...
  process.env.ORIRO_BUNDLED_PLUGINS_DIR = tempHome
    ? path.join(tempHome, "oriro-test-no-bundled-extensions")
    : "oriro-test-no-bundled-extensions";
}
...
async function resetGatewayTestState(options: { uniqueConfigRoot: boolean }) {
  ...
  if (options.uniqueConfigRoot) {
    const suiteRoot = path.join(tempHome, ".oriro-test-suite");
    ...
    tempConfigRoot = path.join(tempHome, ".oriro-test");
    await fs.rm(tempConfigRoot, {
      recursive: true,
      force: true,
      maxRetries: 20,
  ...
  tempControlUiRoot = path.join(tempHome, ".oriro-test-control-ui");
  ...
  await fs.writeFile(
    ...
    "<!doctype html><title>oriro-test-control-ui</title>\n",
    "utf-8",
  );
  setTestConfigRoot(tempConfigRoot);
  resetConfigRuntimeState();
...
const CONNECT_CHALLENGE_NONCE_KEY = "__oriroTestConnectChallengeNonce";
const CONNECT_CHALLENGE_TRACKED_KEY = "__oriroTestConnectChallengeTracked";
...
export async function startServer(token?: string, opts?: GatewayServerOptions) {
  ...
  const envSnapshot = captureEnv(["ORIRO_GATEWAY_TOKEN"]);
  const prev = process.env.ORIRO_GATEWAY_TOKEN;
  ...
  if (fallbackToken === undefined) {
    ...
    process.env.ORIRO_GATEWAY_TOKEN = fallbackToken;
  }
...
function resolveDefaultTestDeviceIdentityPath(params: {
  ...
  const suiteRoot = process.env.ORIRO_STATE_DIR ?? process.env.HOME ?? os.tmpdir();
