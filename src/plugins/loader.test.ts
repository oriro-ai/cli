...
import { ensureOriroPluginSdkAlias } from "./plugin-sdk-dist-alias.js";
...
function updatePluginManifest(plugin: Pick<TempPlugin, "dir">, patch: Record<string, unknown>) {
  const manifestPath = path.join(plugin.dir, "oriro.plugin.json");
...
function setupBundledDreamingMemoryPlugins(params?: {
  ...
  fs.writeFileSync(
    path.join(memoryCoreDir, "oriro.plugin.json"),
    JSON.stringify(
  ...
  fs.writeFileSync(
    path.join(selectedMemoryDir, "oriro.plugin.json"),
    JSON.stringify(
      {
        id: selectedId,
  ...
  process.env.ORIRO_BUNDLED_PLUGINS_DIR = bundledDir;
...
function writeBundledPlugin(params: {
  ...
  delete process.env.ORIRO_DISABLE_BUNDLED_PLUGINS;
  process.env.ORIRO_BUNDLED_PLUGINS_DIR = bundledDir;
  return { bundledDir, plugin };
}

function makeOriroDevSourceRoot() {
  const root = makeTempDir();
  fs.writeFileSync(path.join(root, "package.json"), JSON.stringify({ name: "oriro" }), "utf-8");
...
function writeWorkspacePlugin(params: {
  ...
  const workspacePluginDir = path.join(workspaceDir, ".oriro", "extensions", params.id);
...
function withStateDir<T>(run: (stateDir: string) => T) {
  ...
  return withEnv({ ORIRO_STATE_DIR: stateDir }, () => run(stateDir));
}
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  if (!options && cachedBundledMemoryDir) {
    process.env.ORIRO_BUNDLED_PLUGINS_DIR = cachedBundledMemoryDir;
    return loadOriroPlugins({
      cache: false,
      ...
      JSON.stringify(
        {
          ...
          oriro: { extensions: [`./${pluginFilename}`] },
        },
        null,
        2,
      ),
...
function setupBundledTelegramPlugin() {
  ...
  process.env.ORIRO_BUNDLED_PLUGINS_DIR = cachedBundledTelegramDir;
}

function expectTelegramLoaded(registry: ReturnType<typeof loadOriroPlugins>) {
...
function loadRegistryFromSinglePlugin(params: {
  ...
  options?: Omit<Parameters<typeof loadOriroPlugins>[0], "cache" | "workspaceDir" | "config">;
}) {
...
function loadRegistryFromAllowedPlugins(
  plugins: TempPlugin[],
  options?: Omit<Parameters<typeof loadOriroPlugins>[0], "cache" | "config">,
) {
  return loadOriroPlugins({
    cache: false,
    ...options,
...
function resolveLoadedPluginSource(
  registry: ReturnType<typeof loadOriroPlugins>,
  pluginId: string,
) {
...
function expectCachePartitionByPluginSource(params: {
  pluginId: string;
  loadFirst: () => ReturnType<typeof loadOriroPlugins>;
  loadSecond: () => ReturnType<typeof loadOriroPlugins>;
...
function expectCacheMissThenHit(params: {
  loadFirst: () => ReturnType<typeof loadOriroPlugins>;
  loadVariant: () => ReturnType<typeof loadOriroPlugins>;
}) {
  const first = params.loadFirst();
...
function createEnvResolvedPluginFixture(pluginId: string) {
  useNoBundledPlugins();
  const oriroHome = makeTempDir();
  const ignoredHome = makeTempDir();
  const stateDir = makeTempDir();
  const pluginDir = path.join(oriroHome, "plugins", pluginId);
  mkdirSafe(pluginDir);
  const plugin = writePlugin({
    id: pluginId,
    dir: pluginDir,
...
function expectEscapingEntryRejected(params: {
  ...
  const registry = loadOriroPlugins({
    cache: false,
    config: {
      plugins: {
...
describe("loadOriroPlugins", () => {
  ...
  it("can load scoped plugins from a supplied manifest registry without rereading manifests", () => {
    ...
    fs.rmSync(path.join(plugin.dir, "oriro.plugin.json"));
  ...
  it("loads installed plugin packages discovered from persisted install records", () => {
    ...
    const registry = withEnv({ ORIRO_STATE_DIR: stateDir }, () =>
      loadOriroPlugins({
        cache: false,
        config: {
          plugins: {
  ...
  it("refreshes bundled plugin-sdk aliases without deleting the shared alias directory", () => {
    ...
    const aliasDir = path.join(distRoot, "extensions", "node_modules", "oriro", "plugin-sdk");
    ...
    ensureOriroPluginSdkAlias(distRoot);
  ...
  it("keeps private local-only plugin-sdk artifacts out of package dist aliases", () => {
    ...
    const pluginSdkDir = path.join(distRoot, "plugin-sdk");
    const aliasRoot = path.join(distRoot, "extensions", "node_modules", "oriro");
  ...
  it("keeps private local-only plugin-sdk artifacts out of legacy package dist aliases", () => {
    ...
    fs.writeFileSync(
      path.join(packageRoot, "package.json"),
      JSON.stringify({ name: "oriro", version: "2026.4.22", type: "module" }, null, 2),
      "utf8",
    );
    fs.writeFileSync(path.join(pluginSdkDir, "index.js"), "export const root = true;\n", "utf8");
    fs.writeFileSync(
  ...
  it("loads bundled plugins with plugin-sdk imports from a package dist root", () => {
    ...
    fs.writeFileSync(
      path.join(packageRoot, "package.json"),
      JSON.stringify({ name: "oriro", version: "2026.4.22", type: "module" }),
      "utf-8",
    );
    fs.writeFileSync(
    ...
    ensureOriroPluginSdkAlias(path.join(packageRoot, "dist"));
    fs.writeFileSync(
    ...
    fs.writeFileSync(
      path.join(pluginRoot, "oriro.plugin.json"),
      JSON.stringify(
        {
          id: "discord",
  ...
  it.each([
    {
      ...
      assert: (registry: ReturnType<typeof loadOriroPlugins>) => {
...
module.exports = { id: "manifest-surfaces-plugin", register() { throw new Error("manifest-only snapshot should not register"); } };`,
        ...
        fs.writeFileSync(
          path.join(plugin.dir, "oriro.plugin.json"),
          JSON.stringify(
            {
              id: "manifest-surfaces-plugin",
              configSchema: EMPTY_PLUGIN_SCHEMA,
  ...
  register() {},
        ...
        fs.writeFileSync(
          path.join(memoryPlugin.dir, "oriro.plugin.json"),
          JSON.stringify(
            {
              id: "memory-demo",
              kind: "memory",
    ...
    },
    {
      ...
      run: () => {
        useNoBundledPlugins();
        const importMarker = "__oriro_loader_import_throw_marker";
...
module.exports = { id: "throws-after-import", register() {} };`,
    ...
    },
    {
      ...
      run: () => {
        useNoBundledPlugins();
        const marker = "__oriro_loader_reentry_error";
        const reenterFnMarker = "__oriro_loader_reentry_fn";
        Reflect.deleteProperty(globalThis, marker);
        Reflect.set(
          globalThis,
          reenterFnMarker,
          (options: Parameters<typeof loadOriroPlugins>[0]) => loadOriroPlugins(options),
        );
        const pluginDir = makeTempDir();
        ...
        } satisfies Parameters<typeof loadOriroPlugins>[0];
        writePlugin({
  ...
  register() {
    try {
      ...
      throw error;
        ...
        const registry = loadOriroPlugins(nestedOptions);
    ...
    },
    {
      ...
      run: () => {
        useNoBundledPlugins();
        const marker = "__oriro_runtime_registry_reentry_marker";
        const resolverMarker = "__oriro_runtime_registry_reentry_fn";
        Reflect.deleteProperty(globalThis, marker);
        Reflect.set(
          globalThis,
          resolverMarker,
  ...
  register() {
    ...
    },
    {
      ...
      run: () => {
        ...
        const full = loadOriroPlugins(options);
        const scoped = loadOriroPlugins({
          ...options,
          onlyPluginIds: ["allowed-cache-scope"],
        });
        const scopedAgain = loadOriroPlugins({
          ...options,
          onlyPluginIds: ["allowed-cache-scope"],
        });
  ...
  it("skips discovery and manifest registry loading entirely when onlyPluginIds is an explicit empty array", async () => {
    ...
    const discoverySpy = vi.spyOn(discovery, "discoverOriroPlugins");
  ...
  it("can scope bundled provider loads without hanging", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify({
        name: "@oriro/scoped-provider",
        oriro: { extensions: ["./index.cjs"] },
      }),
      "utf-8",
    );
    const plugin = writePlugin({
  ...
  it("restores cached memory capability public artifacts on cache hits", async () => {
    ...
    const first = loadOriroPlugins(options);
    ...
    const second = loadOriroPlugins(options);
  ...
  it("uses discovery registration mode for non-activating loads", () => {
    useNoBundledPlugins();
    const marker = "__oriroDiscoveryModeTest";
    const plugin = writePlugin({
    ...
    const snapshot = loadOriroPlugins({
      activate: false,
      cache: false,
      workspaceDir: plugin.dir,
      config,
  ...
  it("caches non-activating snapshots without restoring global side effects", () => {
    useNoBundledPlugins();
    clearPluginCommands();
    const marker = "__oriroSnapshotCacheRegisterCount";
    const plugin = writePlugin({
      id: "snapshot-cache",
  ...
  it("does not re-register non-bundled plugins after gateway-bindable boot loads", () => {
    useNoBundledPlugins();
    const marker = "__oriroGatewayBootRegisterCount";
    const plugin = writePlugin({
      id: "costoriro-boot-cache",
      filename: "costoriro-boot-cache.cjs",
      body: `module.exports = {
        id: "costoriro-boot-cache",
        register() {
          globalThis.${marker} = (globalThis.${marker} || 0) + 1;
    ...
    const config = {
      plugins: {
        load: { paths: [plugin.file] },
        allow: ["costoriro-boot-cache"],
        entries: {
          "costoriro-boot-cache": { enabled: true },
        },
      },
    };
  ...
  it("reuses a gateway-bindable cache entry for later default-mode loads", () => {
    useNoBundledPlugins();
    const marker = "__oriroGatewayBindableCacheRegisterCount";
    const plugin = writePlugin({
    ...
    const gatewayBindable = loadOriroPlugins({
      ...options,
      runtimeOptions: {
        allowGatewaySubagentBinding: true,
      },
    });
    const defaultMode = loadOriroPlugins(options);
  ...
  it("keeps pinned gateway hooks and later default-mode hooks dispatchable together", () => {
    ...
    const gatewayRegistry = loadOriroPlugins({
    ...
    try {
      ...
      const defaultRegistry = loadOriroPlugins({
        ...
        return {
          ...
          loadFirst: () =>
            loadOriroPlugins({
              ...
              env: {
                ...process.env,
                ORIRO_BUNDLED_PLUGINS_DIR: bundledA,
              },
            }),
          loadSecond: () =>
            loadOriroPlugins({
              ...
              env: {
                ...process.env,
                ORIRO_BUNDLED_PLUGINS_DIR: bundledB,
              },
            }),
        };
      },
          ...
          loadFirst: () =>
            loadOriroPlugins({
              ...
              env: {
                ...
                ORIRO_STATE_DIR: stateDir,
                ORIRO_BUNDLED_PLUGINS_DIR: bundledDir,
              },
            }),
          loadSecond: () =>
            loadOriroPlugins({
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const ignoredHome = makeTempDir();
        const stateDir = makeTempDir();
        const pluginDir = path.join(oriroHome, "plugins", "tracked-install-cache");
        mkdirSafe(pluginDir);
        const plugin = writePlugin({
    ...
    },
    {
      ...
      setup: () => {
        ...
        return {
          loadFirst: () => loadOriroPlugins(options),
          loadVariant: () =>
            loadOriroPlugins({
              ...options,
  ...
  it("normalizes bundled plugin env overrides against the provided env", () => {
    ...
    const registry = loadOriroPlugins({
      env: {
        ...
        HOME: homeDir,
        ORIRO_HOME: undefined,
        ORIRO_BUNDLED_PLUGINS_DIR: override,
      },
      config: {
        plugins: {
          allow: ["tilde-bundled"],
  ...
  it("prefers ORIRO_HOME over HOME for env-expanded load paths", () => {
    const ignoredHome = makeTempDir();
    const oriroHome = makeTempDir();
    const stateDir = makeTempDir();
    const bundledDir = makeTempDir();
    const plugin = writePlugin({
      id: "oriro-home-demo",
      dir: path.join(oriroHome, "plugins", "oriro-home-demo"),
      filename: "index.cjs",
      body: `module.exports = { id: "oriro-home-demo", register() {} };`,
    });
    ...
    const registry = loadOriroPlugins({
      ...
      config: {
        plugins: {
          allow: ["oriro-home-demo"],
          entries: {
            "oriro-home-demo": { enabled: true },
          },
          load: {
            paths: ["~/plugins/oriro-home-demo"],
          },
        },
      },
    });
    ...
    expect(
      fs.realpathSync(
        registry.plugins.find((entry) => entry.id === "oriro-home-demo")?.source ?? "",
      ),
    ).toBe(fs.realpathSync(plugin.file));
  });
  ...
  it("can include plugin export shape when register is missing", () => {
    ...
    const registry = withEnv({ ORIRO_PLUGIN_LOAD_DEBUG: "1" }, () =>
      loadRegistryFromSinglePlugin({
        plugin,
        pluginConfig: {
  ...
  api.registerHook("gateway:startup", () => {}, { name: "shared-hook" });
        ...
        selectCount: (registry: ReturnType<typeof loadOriroPlugins>) =>
  ...
  api.registerGatewayDiscoveryService({ id: "shared-discovery", advertise() {} });
        ...
        assertPrimaryOwner: (registry: ReturnType<typeof loadOriroPlugins>) => {
          expect(
  ...
  it("loads bundled channel entries through nested default export wrappers", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@oriro/nested-default-channel",
          oriro: {
            extensions: ["./index.cjs"],
          },
        },
  ...
  it("does not treat manifest channel ids as scoped plugin id matches", () => {
    ...
    fs.writeFileSync(
      path.join(unrelated.dir, "oriro.plugin.json"),
      JSON.stringify(
        {
          id: "unrelated-plugin",
          configSchema: EMPTY_PLUGIN_SCHEMA,
...
module.exports = {
  ...
  register(api) {
    ...
    const broadSetupRegistry = loadOriroPlugins({
      cache: false,
      config,
    ...
    const scopedSetupRegistry = loadOriroPlugins({
      cache: false,
      config,
      includeSetupOnlyChannelPlugins: true,
...
module.exports = {
  ...
  register(api) {
    ...
    fs.writeFileSync(
      path.join(workspacePluginDir, "oriro.plugin.json"),
      JSON.stringify(
        {
          id: "workspace-shadow",
          configSchema: EMPTY_PLUGIN_SCHEMA,
...
module.exports = {
  ...
  register(api) {
    api.registerChannel({
      ...
      fs.writeFileSync(
        path.join(globalDir, "oriro.plugin.json"),
        JSON.stringify(
          {
            id: "untrusted-global-channel",
            configSchema: EMPTY_PLUGIN_SCHEMA,
      ...
      fs.writeFileSync(
        ...
        JSON.stringify(
          {
            name: "@oriro/untrusted-global-channel",
            version: "0.0.0-test",
            main: "./index.cjs",
            oriro: {
              extensions: ["./index.cjs"],
  ...
  register(api) {
    api.registerChannel({
      ...
      fs.writeFileSync(
        ...
        JSON.stringify(
          {
            name: "@oriro/trusted-global-channel",
            version: "0.0.0-test",
            main: "./index.cjs",
            oriro: {
              extensions: ["./index.cjs"],
...
module.exports = {
  ...
  it.each([
    {
      ...
      fixture: {
        ...
        packageName: "@oriro/setup-entry-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@oriro/setup-only-bundled-contract-test",
        fullBlurb: "full entry should not run in setup-only mode",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@oriro/setup-runtime-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@oriro/setup-runtime-bundled-contract-test",
        fullBlurb: "full entry should not run while unconfigured",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@oriro/setup-runtime-bundled-contract-secrets-test",
        fullBlurb: "full entry should not run while unconfigured",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@oriro/setup-runtime-bundled-contract-runtime-test",
        fullBlurb: "full entry should not run while unconfigured",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@oriro/setup-runtime-bundled-route-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@oriro/setup-runtime-bundled-runtime-merge-test",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@oriro/setup-runtime-external-deferred-test",
        fullBlurb: "full entry should defer while configured",
    ...
    },
    {
      ...
      fixture: {
        ...
        packageName: "@oriro/setup-runtime-not-preferred-test",
  ...
  it("applies the bundled runtime setter before loading the merged setup-runtime plugin", () => {
    ...
    const built = createSetupEntryChannelPluginFixture({
      ...
      packageName: "@oriro/setup-runtime-order-test",
      fullBlurb: "full runtime plugin",
      setupBlurb: "setup runtime override",
      configured: false,
      useBundledFullEntryContract: true,
  ...
  it("records setup runtime setter failures without aborting the full load pass", () => {
    const built = createSetupEntryChannelPluginFixture({
      ...
      packageName: "@oriro/setup-runtime-error-test",
      fullBlurb: "full runtime plugin",
      setupBlurb: "setup runtime override",
      configured: false,
      useBundledSetupEntryContract: true,
  ...
  it("rolls back setup-runtime registrations when setup side effects fail", () => {
    const built = createSetupEntryChannelPluginFixture({
      ...
      packageName: "@oriro/setup-runtime-route-error-test",
      fullBlurb: "full runtime plugin",
      setupBlurb: "setup runtime route",
      configured: true,
      startupDeferConfiguredChannelFullLoadUntilAfterListen: true,
  ...
  it("closes setup-runtime registration APIs after synchronous registration", async () => {
    const built = createSetupEntryChannelPluginFixture({
      ...
      packageName: "@oriro/setup-runtime-late-route-test",
      fullBlurb: "full runtime plugin",
      setupBlurb: "setup runtime route",
      configured: true,
      startupDeferConfiguredChannelFullLoadUntilAfterListen: true,
  ...
  it("rejects mismatched bundled runtime entry ids before applying setup-runtime setters", () => {
    ...
    const built = createSetupEntryChannelPluginFixture({
      ...
      packageName: "@oriro/setup-runtime-mismatch-test",
      fullBlurb: "full runtime plugin",
      setupBlurb: "setup runtime override",
      configured: false,
      useBundledFullEntryContract: true,
  ...
  it("rejects mismatched bundled setup export ids before loading setup-runtime entry code", () => {
    ...
    const built = createSetupEntryChannelPluginFixture({
      ...
      packageName: "@oriro/setup-export-mismatch-test",
      fullBlurb: "full runtime plugin",
      setupBlurb: "setup runtime override",
      configured: false,
      useBundledFullEntryContract: true,
  ...
  it("isolates loadSetupPlugin errors as per-plugin diagnostics instead of crashing registry load", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@oriro/setup-entry-throws-test",
          oriro: {
            extensions: ["./index.cjs"],
            setupEntry: "./setup-entry.cjs",
          },
  ...
  it("keeps healthy sibling channel plugins loadable when a setup entry throws", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@oriro/setup-entry-throws-sibling-test",
          oriro: {
            extensions: ["./index.cjs"],
            setupEntry: "./setup-entry.cjs",
          },
    ...
    fs.writeFileSync(
      path.join(brokenDir, "oriro.plugin.json"),
      JSON.stringify(
        {
  ...
  it("records a diagnostic when registerChannel throws in the setup-entry path", () => {
    ...
    fs.writeFileSync(
      ...
      JSON.stringify(
        {
          name: "@oriro/register-channel-throws-test",
          oriro: {
            extensions: ["./index.cjs"],
            setupEntry: "./setup-entry.cjs",
          },
...
module.exports = {
  ...
  it("prefers built bundled plugin artifacts over source TS when requested", () => {
    ...
    fs.writeFileSync(
      path.join(sourceDir, "oriro.plugin.json"),
      JSON.stringify(
        {
    ...
    const registry = withEnv(
      {
        ORIRO_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "extensions"),
        ORIRO_TEST_TRUST_BUNDLED_PLUGINS_DIR: "1",
        ORIRO_DISABLE_BUNDLED_PLUGINS: undefined,
      },
      () =>
        loadOriroPlugins({
          cache: false,
    ...
    const scenarios = [
      {
        ...
        loadRegistry: () => {
          ...
          return withEnv(
            {
              ORIRO_DISABLE_BUNDLED_PLUGINS: "1",
              ORIRO_BUNDLED_PLUGINS_DIR: undefined,
            },
            () =>
              loadOriroPlugins({
                cache: false,
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          fs.writeFileSync(
            path.join(memoryADir, "oriro.plugin.json"),
            JSON.stringify(
              {
                id: "memory-a",
                kind: "memory",
          ...
          fs.writeFileSync(
            path.join(memoryBDir, "oriro.plugin.json"),
            JSON.stringify(
              {
                id: "memory-b",
                kind: "memory",
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          fs.writeFileSync(
            path.join(memoryLanceDir, "oriro.plugin.json"),
            JSON.stringify(
  ...
  it("loads dreaming sidecar metadata through a restrictive selected-memory allowlist", async () => {
    ...
    const registry = await loadOriroPluginCliRegistry({
      cache: false,
      config: {
        plugins: {
          allow: [selectedId],
  ...
  it("resolves duplicate plugin ids by source precedence", () => {
    const scenarios = [
      ...
      },
      {
        ...
        bundledFilename: "index.cjs",
        loadRegistry: () => {
          const devSourceRoot = makeOriroDevSourceRoot();
          ...
          process.env.ORIRO_BUNDLED_PLUGINS_DIR = bundledPluginsDir;
          return withEnv({ ORIRO_DEV_SOURCE_ROOT: devSourceRoot }, () =>
            withStateDir((stateDir) => {
      ...
      },
      {
        ...
        loadRegistry: () => {
          ...
          return withStateDir((stateDir) => {
            const globalDir = path.join(stateDir, "node_modules", "@oriro", "memory-lancedb");
            mkdirSafe(globalDir);
              ...
              config: {
                plugins: {
                  ...
                  installs: {
                    "memory-lancedb": {
                      source: "npm",
                      spec: "@oriro/memory-lancedb",
                      resolvedName: "@oriro/memory-lancedb",
  ...
  it("evaluates load-path provenance warnings", () => {
    ...
    const scenarios = [
      ...
      },
      {
        ...
        loadRegistry: () => {
          const stateDir = makeTempDir();
          return withEnv({ ORIRO_STATE_DIR: stateDir }, () => {
            const globalDir = path.join(stateDir, "extensions", "rogue");
            mkdirSafe(globalDir);
            writePlugin({
              id: "rogue",
  ...
  it("uses the source runtime snapshot allowlist for plugin trust checks", () => {
    useNoBundledPlugins();
    const stateDir = makeTempDir();
    withEnv({ ORIRO_STATE_DIR: stateDir }, () => {
  ...
  it("supports legacy plugins importing monolithic plugin-sdk root", () => {
    ...
    const plugin = writePlugin({
      ...
      body: `module.exports = {
  id: "legacy-root-import",
  configSchema: (require("oriro/plugin-sdk").emptyPluginConfigSchema)(),
        register() {},
      };`,
    });

    const registry = withEnv({ ORIRO_BUNDLED_PLUGINS_DIR: "/nonexistent/bundled/plugins" }, () =>
      loadOriroPlugins({
        cache: false,
        workspaceDir: plugin.dir,
        config: {
  ...
  it("supports legacy plugins subscribing to diagnostic events from the root sdk", async () => {
    useNoBundledPlugins();
    const seenKey = "__oriroLegacyRootDiagnosticSeen";
  ...
  configSchema: (require("oriro/plugin-sdk").emptyPluginConfigSchema)(),
  register() {
    const { onDiagnosticEvent } = require("oriro/plugin-sdk");
