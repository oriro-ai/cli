...
let runtimeEnvModule: typeof import("oriro/plugin-sdk/runtime-env");
...
function createConfigWithDiscordAccount(overrides: Record<string, unknown> = {}): OriroConfig {
  return {
    channels: {
      discord: {
        accounts: {
...
describe("monitorDiscordProvider", () => {
  ...
  beforeAll(async () => {
    vi.doMock("oriro/plugin-sdk/plugin-runtime", async () => {
    ...
    runtimeEnvModule = await import("oriro/plugin-sdk/runtime-env");
  ...
  it("fails closed before lifecycle when Discord bot identity has no usable id", async () => {
    const runtime = baseRuntime();
    clientFetchUserMock.mockResolvedValueOnce({ username: "Oriro" } as never);
  ...
  it("passes Oriro event queue defaults to the Discord client", async () => {
    await monitorDiscordProvider({
      config: baseConfig(),
      runtime: baseRuntime(),
    });
  ...
  it("logs Discord startup phases and early gateway debug events", async () => {
    ...
    clientFetchUserMock.mockImplementationOnce(async () => {
      ...
      return { id: "bot-1", username: "Oriro" };
    });
