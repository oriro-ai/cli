...
const WRITE_SCOPE_HEADER = { "x-oriro-scopes": "operator.write" };
...
describe("OpenAI-compatible embeddings HTTP API (e2e)", () => {
  it("embeds string and array inputs", async () => {
    ...
    const qualified = await postEmbeddings(
      ...
      { "x-oriro-model": "openai/text-embedding-3-small" },
    );
    expect(qualified.status).toBe(200);
    const qualifiedJson = (await qualified.json()) as { model?: string };
    expect(qualifiedJson.model).toBe("oriro/default");
  ...
  it("rejects explicit unknown agent ids", async () => {
    try {
      ...
      const header = await postEmbeddings(
        { model: "oriro/default", input: "hello" },
        { "x-oriro-agent-id": "missing-agent" },
      );
      await expectInvalidEmbeddingRequest(header, "Unknown agent 'missing-agent'.");

      const model = await postEmbeddings({ model: "oriro/missing-agent", input: "hello" });
  ...
  it("rejects disallowed x-oriro-model provider overrides", async () => {
    const res = await postEmbeddings(
      {
        model: "oriro/default",
        input: "hello",
      },
      { "x-oriro-model": "ollama/nomic-embed-text" },
    );
    await expectInvalidEmbeddingRequest(
      res,
  ...
  it("rejects x-oriro-model for trusted write-only callers", async () => {
