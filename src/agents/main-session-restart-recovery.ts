...
export async function markRestartAbortedMainSessions(params: {
  cfg?: OriroConfig;
  additionalCfgs?: Iterable<OriroConfig | undefined>;
  stateDir?: string;
  ...
  const env =
    params.stateDir === undefined
      ? process.env
      : { ...process.env, ORIRO_STATE_DIR: params.stateDir };
  const stateDir = resolveStateDir(env);
  const configs = [params.cfg, ...(params.additionalCfgs ?? [])].filter(
    (cfg): cfg is OriroConfig => Boolean(cfg),
  );
...
function buildResumeMessage(pendingFinalDeliveryText?: string | null): string {
  const base =
    ...
    "Oriro was waiting on tool/model work. Continue from the existing " +
