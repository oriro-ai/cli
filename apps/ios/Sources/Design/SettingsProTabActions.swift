...
extension SettingsProTab {
        ...
        ProCard(padding: 0, radius: SettingsLayout.cardRadius) {
            VStack(spacing: 0) {
                ...
                self.diagnosticCheckRow(
                    ...
                    color: self.gatewayController.gateways.isEmpty ? .secondary : OriroBrand.accent)
                Divider().padding(.leading, 60)
                self.diagnosticCheckRow(
                    icon: "waveform",
                    title: "Talk Config",
                ...
                self.diagnosticCheckRow(
                    ...
                    color: self.appModel.screenRecordActive ? OriroBrand.ok : .secondary)
                Divider().padding(.leading, 60)
                self.diagnosticCheckRow(
                    icon: "mic",
                    title: "Voice Wake",
                    ...
                    color: self.voiceWakeEnabled ? OriroBrand.ok : .secondary)
            }
        }
        .padding(.horizontal, OriroProMetric.pagePadding)
    }
    ...
    func handleLocationModeChange(_ newValue: String) {
        ...
        guard let mode = OriroLocationMode(rawValue: newValue) else { return }
    ...
    func friendlyGatewayMessage(from raw: String) -> String? {
        ...
        if lower.contains("pairing required") {
            return "Pairing required. Run /pair approve in your Oriro chat, then connect again."
        }
    ...
    var gatewayStatusColor: Color {
        if self.appModel.isAppleReviewDemoModeEnabled { return OriroBrand.accent }
        return self.gatewayConnected ? OriroBrand.ok : .secondary
    }
    ...
    var gatewayTalkConfigColor: Color {
        ...
        return self.appModel.talkMode.gatewayTalkConfigLoaded ? OriroBrand.ok : .secondary
    }
    ...
    var gatewayServer: String {
        self.appModel.gatewayServerName ?? "Oriro Gateway"
    }

    var permissionsDetail: String {
        var enabled = 0
        if self.cameraEnabled { enabled += 1 }
        if self.locationModeRaw != OriroLocationMode.off.rawValue { enabled += 1 }
    ...
    var diagnosticsRunColor: Color {
        ...
        return diagnosticsIssueCount == 0 ? OriroBrand.ok : OriroBrand.warn
    }

    var privacyDetail: String {
        let location = OriroLocationMode(rawValue: self.locationModeRaw) ?? .off
        return location == .off ? "Location off" : "Location \(self.locationLabel)"
    }

    var locationLabel: String {
        switch OriroLocationMode(rawValue: self.locationModeRaw) ?? .off {
        case .off: "Off"
