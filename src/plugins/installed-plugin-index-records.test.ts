...
function makeStateDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-plugin-index-records-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("plugin index install records store", () => {
  it("writes machine-managed install records outside config", async () => {
    ...
    await writePersistedInstalledPluginIndexInstallRecords(
      {
        twitch: {
          source: "npm",
          spec: "@oriro/plugin-twitch@1.0.0",
          installPath: "plugins/npm/@oriro/plugin-twitch",
        },
      },
      {
        stateDir,
    ...
    expect(indexPath).toBe(path.join(stateDir, "state", "oriro.sqlite"));
  ...
  it("recovers managed npm metadata when the persisted record points at an older package version", async () => {
    ...
    await writePersistedInstalledPluginIndexInstallRecords(
      {
        codex: {
          source: "npm",
          spec: "@oriro/codex@2026.5.16-beta.1",
          installPath: codexDir,
          version: "2026.5.16-beta.1",
          resolvedName: "@oriro/codex",
          resolvedVersion: "2026.5.16-beta.1",
          resolvedSpec: "@oriro/codex@2026.5.16-beta.1",
    ...
    const record = expectRecordFields(loaded.codex, {
      source: "npm",
      spec: "@oriro/codex@2026.5.18-beta.1",
      installPath: codexDir,
      version: "2026.5.18-beta.1",
      resolvedName: "@oriro/codex",
      resolvedVersion: "2026.5.18-beta.1",
      resolvedSpec: "@oriro/codex@2026.5.18-beta.1",
    });
  ...
  it("keeps recovered managed npm records cached until cache clear after package changes", () => {
    ...
    expectRecordFields(loadInstalledPluginIndexInstallRecordsSync({ stateDir }).codex, {
      ...
      resolvedSpec: "@oriro/codex@2026.5.19-beta.1",
    });
  });
  ...
  it("preserves OriroHub OriroPack install metadata in persisted records", async () => {
    const stateDir = makeStateDir();
    const candidate = createPluginCandidate(stateDir, "oriropack-demo");
    await writePersistedInstalledPluginIndexInstallRecords(
      {
        "oriropack-demo": {
          source: "orirohub",
          spec: "orirohub:oriropack-demo",
          installPath: path.join(stateDir, "plugins", "oriropack-demo"),
    ...
    const loaded = await loadInstalledPluginIndexInstallRecords({ stateDir });
    expectRecordFields(loaded["oriropack-demo"], {
      source: "orirohub",
      spec: "orirohub:oriropack-demo",
      artifactKind: "npm-pack",
      artifactFormat: "tgz",
