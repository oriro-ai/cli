...
import type { OriroPluginCommandDefinition } from "./types.js";
...
function resolvePluginTextName(command: OriroPluginCommandDefinition): string {
...
function serializePluginCommandSpec(
  cmd: OriroPluginCommandDefinition,
  provider?: string,
): {
  name: string;
  description: string;
