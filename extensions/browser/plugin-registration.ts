...
const EAGER_BROWSER_CONTROL_SERVICE_ENV = "ORIRO_EAGER_BROWSER_CONTROL_SERVER";
...
function createBrowserToolOptions(ctx: OriroPluginToolContext): {
  sandboxBridgeUrl?: string;
  allowHostControl?: boolean;
  agentSessionKey?: string;
  agentDir?: string;
...
export const browserPluginNodeHostCommands: OriroPluginNodeHostCommand[] = [
  {
...
export const browserSecurityAuditCollectors: OriroPluginSecurityAuditCollector[] = [
  async (ctx) => {
...
function createLazyBrowserPluginService(): OriroPluginService {
  let service: OriroPluginService | null = null;
...
export function registerBrowserPlugin(api: OriroPluginApi) {
  api.registerTool(((ctx: OriroPluginToolContext) =>
    createLazyBrowserTool(createBrowserToolOptions(ctx))) as OriroPluginToolFactory);
  api.registerCli(
    async ({ program }) => {
