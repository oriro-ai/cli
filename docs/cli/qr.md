---
summary: "CLI reference for `oriro qr` (generate mobile pairing QR + setup code)"
read_when:
...
```bash
oriro qr
oriro qr --setup-code-only
oriro qr --json
oriro qr --remote
oriro qr --url wss://gateway.example/ws
```
...
- With `--remote`, Oriro requires either `gateway.remote.url` or
