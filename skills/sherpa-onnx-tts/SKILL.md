...
1. Download the runtime for your OS (extracts into `$ORIRO_STATE_DIR/tools/sherpa-onnx-tts/runtime`, default `~/.oriro/tools/sherpa-onnx-tts/runtime`)
2. Download a voice model (extracts into `$ORIRO_STATE_DIR/tools/sherpa-onnx-tts/models`, default `~/.oriro/tools/sherpa-onnx-tts/models`)
...
Then write those resolved paths into the active Oriro config file (`$ORIRO_CONFIG_PATH`, default `~/.oriro/oriro.json`):
