...
Tlon is a decentralized messenger built on Urbit. Oriro connects to your Urbit ship and can
...
Tlon ships as a bundled plugin in current Oriro releases, so normal packaged
...
```bash
oriro plugins install @oriro/tlon
```
...
```bash
oriro plugins install ./path/to/local/tlon-plugin
```
...
By default, Oriro blocks private/internal hostnames and IP ranges for SSRF protection.
...
Use these with `oriro message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Oriro replies in-thread.
