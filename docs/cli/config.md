---
summary: "CLI reference for `oriro config` (get/set/patch/unset/file/schema/validate)"
read_when:
...
Config helpers for non-interactive edits in `oriro.json`: get/set/patch/unset/file/schema/validate values by path and print the active config file. Run without a subcommand to open the configure wizard (same as `oriro configure`).

<Note>
When `ORIRO_NIX_MODE=1`, Oriro treats `oriro.json` as immutable. Read-only commands such as `config get`, `config file`, `config schema`, and `config validate` still work, but config writers refuse. Agents should edit the Nix source for the install instead; for the first-party nix-oriro distribution, use [nix-oriro Quick Start](https://github.com/oriro/nix-oriro#quick-start) and set values under `programs.oriro.config` or `instances.<name>.config`.
</Note>

## Root options

<ParamField path="--section <section>" type="string">
  Repeatable guided-setup section filter when you run `oriro config` without a subcommand.
</ParamField>
...
```bash
oriro config file
oriro config --section model
oriro config --section gateway --section daemon
oriro config schema
oriro config get browser.executablePath
...
oriro config set agents.defaults.heartbeat.every "2h"
oriro config set 'agents.list[0].tools.exec.node' "node-id-or-name"
oriro config set agents.defaults.models '{"openai/gpt-5.4":{}}' --strict-json --merge
oriro config set channels.discord.token --ref-provider default --ref-source env --ref-id DISCORD_BOT_TOKEN
oriro config set secrets.providers.vaultfile --provider-source file --provider-path /etc/oriro/secrets.json --provider-mode json
oriro config patch --file ./oriro.patch.json5 --dry-run
oriro config unset plugins.entries.brave.config.webSearch.apiKey
oriro config set channels.discord.token --ref-provider default --ref-source env --ref-id DISCORD_BOT_TOKEN --dry-run
oriro config validate
oriro config validate --json
```

### `config schema`

Print the generated JSON schema for `oriro.json` to stdout as JSON.
...
```bash
oriro config schema > oriro.schema.json
```

### Paths

Paths use dot or bracket notation. Quote bracket-notation paths in shell examples so shells such as zsh do not expand `[0]` as a glob before Oriro receives the path:

```bash
oriro config get agents.defaults.workspace
oriro config get 'agents.list[0].id'
```

Use the agent list index to target a specific agent:

```bash
oriro config get agents.list
oriro config set 'agents.list[1].tools.exec.node' "node-id-or-name"
```
...
```bash
oriro config set agents.defaults.heartbeat.every "0m"
oriro config set gateway.port 19001 --strict-json
oriro config set channels.whatsapp.groups '["*"]' --strict-json
```
...
```bash
oriro config set agents.defaults.models '{"openai/gpt-5.4":{}}' --strict-json --merge
oriro config set models.providers.ollama.models '[{"id":"llama3.2","name":"Llama 3.2"}]' --strict-json --merge
```
...
`oriro config set` supports four assignment styles:

<Tabs>
  <Tab title="Value mode">
    ```bash
    oriro config set <path> <value>
    ```
  </Tab>
  <Tab title="SecretRef builder mode">
    ```bash
    oriro config set channels.discord.token \
  ...
  <Tab title="Provider builder mode">
    ...
    ```bash
    oriro config set secrets.providers.vault \
      --provider-source exec \
      --provider-command /usr/local/bin/oriro-vault \
      --provider-arg read \
  ...
  <Tab title="Batch mode">
    ...
    ```bash
    oriro config set --batch-file ./config-set.batch.json --dry-run
    ```
...
```bash
oriro config patch --file ./oriro.patch.json5 --dry-run
oriro config patch --file ./oriro.patch.json5
```

You can also pipe a patch over stdin, which is useful for remote setup scripts:

```bash
ssh oriro-host 'oriro config patch --stdin --dry-run' < ./oriro.patch.json5
ssh oriro-host 'oriro config patch --stdin' < ./oriro.patch.json5
```
...
```bash
oriro config patch --file ./discord.patch.json5 --replace-path 'channels.discord.guilds["123"].channels'
```
...
oriro config set secrets.providers.vaultfile \
  '{"source":"file","path":"/etc/oriro/secrets.json","mode":"json"}' \
  --strict-json
```
...
Use `--dry-run` to validate changes without writing `oriro.json`.
...
`oriro config set` and other Oriro-owned config writers validate the full post-change config before committing it to disk. If the new payload fails schema validation or looks like a destructive clobber, the active config is left alone and the rejected payload is saved beside it as `oriro.json.rejected.*`.

<Warning>
The active config path must be a regular file. Symlinked `oriro.json` layouts are unsupported for writes; use `ORIRO_CONFIG_PATH` to point directly at the real file instead.
</Warning>

Prefer CLI writes for small edits:

```bash
oriro config set gateway.reload.mode hybrid --dry-run
oriro config set gateway.reload.mode hybrid
oriro config validate
```
...
Direct editor writes are still allowed, but the running Gateway treats them as untrusted until they validate. Invalid direct edits fail startup or are skipped by hot reload; Gateway does not rewrite `oriro.json`. Run `oriro doctor --fix` to repair prefixed/clobbered config or restore the last-known-good copy. See [Gateway troubleshooting](/gateway/troubleshooting#gateway-rejected-invalid-config).
...
- `config file`: Print the active config file path (resolved from `ORIRO_CONFIG_PATH` or default location). The path should name a regular file, not a symlink.
...
After `oriro config validate` is passing, you can use the local TUI to have an embedded agent compare the active config against the docs while you validate each change from the same terminal:

<Note>
If validation is already failing, start with `oriro configure` or `oriro doctor --fix`. `oriro chat` does not bypass the invalid-config guard.
</Note>
...
<Steps>
  ...
  <Step title="Apply targeted edits">
    Apply targeted edits with `oriro config set` or `oriro configure`.
  </Step>
  <Step title="Re-validate">
    Rerun `oriro config validate` after each change.
  </Step>
  <Step title="Doctor for runtime issues">
    If validation passes but the runtime is still unhealthy, run `oriro doctor` or `oriro doctor --fix` for migration and repair help.
  </Step>
</Steps>
