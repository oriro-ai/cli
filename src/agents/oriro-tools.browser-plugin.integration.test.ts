// Verifies Oriro plugin tools are resolved with browser/runtime context.
...
function firstResolvePluginToolsParams(): Record<string, unknown> {
  // Captures the plugin runtime contract passed from Oriro tool resolution.
...
describe("createOriroTools browser plugin integration", () => {
  afterEach(() => {
  ...
  it("keeps the browser tool returned by plugin resolution", () => {
    ...
    const tools = resolveOriroPluginToolsForOptions({
      options: { config },
  ...
  it("does not pass a stale active snapshot as plugin runtime config for a resolved run config", () => {
    ...
    let capturedRuntimeConfig: OriroConfig | undefined;
    hoisted.resolvePluginTools.mockImplementation((params: unknown) => {
      capturedRuntimeConfig = (params as { context?: { runtimeConfig?: OriroConfig } }).context
        ?.runtimeConfig;
      return [];
    });
  ...
  it("does not let a source-less pinned config snapshot override explicit plugin tool config", () => {
    ...
    let capturedRuntimeConfig: OriroConfig | undefined;
    let getRuntimeConfig: (() => OriroConfig | undefined) | undefined;
    hoisted.resolvePluginTools.mockImplementation((params: unknown) => {
      const context = (
        params as {
          context?: {
            runtimeConfig?: OriroConfig;
            getRuntimeConfig?: () => OriroConfig | undefined;
          };
        }
      ).context;
  ...
  it("exposes a live runtime config getter to plugin tool factories", () => {
    ...
    hoisted.resolvePluginTools.mockImplementation((params: unknown) => {
      getRuntimeConfig = (
        params as { context?: { getRuntimeConfig?: () => OriroConfig | undefined } }
