...
} from "oriro/plugin-sdk/json-schema-runtime";
import type { RealtimeTranscriptionProviderPlugin } from "oriro/plugin-sdk/realtime-transcription";
import type { RealtimeVoiceProviderPlugin } from "oriro/plugin-sdk/realtime-voice";
...
vi.mock("oriro/plugin-sdk/ssrf-runtime", async (importOriginal) => {
  const actual = await importOriginal<typeof import("oriro/plugin-sdk/ssrf-runtime")>();
  return {
    ...actual,
...
describe("google-meet plugin", () => {
  ...
  afterAll(() => {
    vi.doUnmock("oriro/plugin-sdk/ssrf-runtime");
  ...
  it("defaults to chrome agent mode with safe read-only tools", () => {
    ...
    expect(config.realtime.instructions).toContain("oriro_agent_consult");
  ...
  it("declares advanced config metadata in the plugin entry and manifest", () => {
    const manifest = JSON.parse(
      readFileSync(new URL("./oriro.plugin.json", import.meta.url), "utf8"),
    ) as {
  ...
  it("uses env fallbacks for OAuth, preview, and default meeting values", () => {
    const config = resolveGoogleMeetConfigWithEnv(
      {},
      {
        ORIRO_GOOGLE_MEET_CLIENT_ID: "client-id",
        GOOGLE_MEET_CLIENT_SECRET: "client-secret",
        ORIRO_GOOGLE_MEET_REFRESH_TOKEN: "refresh-token",
        GOOGLE_MEET_ACCESS_TOKEN: "access-token",
        ORIRO_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT: "123456",
        GOOGLE_MEET_DEFAULT_MEETING: "https://meet.google.com/abc-defg-hij",
        ORIRO_GOOGLE_MEET_PREVIEW_ACK: "true",
      },
    );
  ...
  it.each(["0x10", "1e3"])("ignores non-decimal env numeric fallbacks: %s", (expiresAt) => {
    const config = resolveGoogleMeetConfigWithEnv(
      {},
      {
        ORIRO_GOOGLE_MEET_ACCESS_TOKEN: "access-token",
        ORIRO_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT: expiresAt,
      },
    );
  ...
  it("reports setup status through the tool", async () => {
    ...
    try {
      const { tools } = setup({
        chrome: {
          audioInputCommand: ["oriro-audio-bridge", "capture"],
          audioOutputCommand: ["oriro-audio-bridge", "play"],
        },
      });
      const tool = tools[0] as {
  ...
  it("writes export bundles through the tool", async () => {
    stubMeetArtifactsApi();
    const tempDir = mkdtempSync(path.join(tmpdir(), "oriro-google-meet-tool-export-"));
    const { tools } = setup();
    const tool = tools[0] as {
      execute: (
        id: string,
  ...
  it("dry-runs export bundles through the tool", async () => {
    stubMeetArtifactsApi();
    const parentDir = mkdtempSync(path.join(tmpdir(), "oriro-google-meet-tool-dry-run-"));
          ...
          if (request.path === "/act") {
            return {
              result: JSON.stringify({
                ...
                manualActionMessage: "Admit the Oriro browser participant in Google Meet.",
                title: "Meet",
                url: "https://meet.google.com/abc-defg-hij",
              }),
            };
  ...
  it("retries caption enable until the captions button is available", async () => {
    ...
    const captionsStateKey = "__oriroMeetCaptions";
    ...
    const { tools } = setup(
      ...
      },
      {
        browserActResult: {
          ...
          manualActionMessage:
            "Sign in to Google in the Oriro browser profile, then retry the Meet join.",
  ...
  it("explains when chrome-node has no capable paired node", async () => {
    ...
    expect(result.details.error).toContain("oriro node run");
  });
  ...
  it("pipes Chrome command-pair audio through the realtime provider", async () => {
    ...
    expect(progressPayload.tool).toBe("oriro_agent_consult");
    ...
    expect(callbacks.tools?.map((tool) => tool.name)).toContain("oriro_agent_consult");
    await vi.waitFor(() => {
    ...
    const transcript = [
      {
        ...
        text: "Hi Oriro, glad to have you here. Let me know if there's anything specific you'd like to cover or if you need any support during the meeting.",
      },
    ];
