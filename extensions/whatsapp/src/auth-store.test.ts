...
const hoisted = vi.hoisted(() => ({
  ...
  oauthDir: "/tmp/oriro-wa-auth-store-test-oauth",
}));
...
describe("auth-store", () => {
  ...
  it("does not restore creds from backup on ordinary reads", async () => {
    const authDir = createTempAuthDir("oriro-wa-auth-read");
  ...
  it("restores creds from a regular backup file", async () => {
    const authDir = createTempAuthDir("oriro-wa-auth-restore");
    const credsPath = path.join(authDir, "creds.json");
  ...
  it("preserves valid large creds instead of treating them as corrupt", async () => {
    const authDir = createTempAuthDir("oriro-wa-auth-large-creds");
    const credsPath = path.join(authDir, "creds.json");
  ...
  it("refuses to restore creds from a symlinked backup path", async () => {
    const authDir = createTempAuthDir("oriro-wa-auth-restore-symlink");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const authDir = createTempAuthDir("oriro-wa-auth-restore-target-symlink");
  ...
  it("reports linked auth state and snapshot from the shared read helper", async () => {
    const authDir = createTempAuthDir("oriro-wa-auth-linked");
    fsSync.writeFileSync(
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const authDir = createTempAuthDir("oriro-wa-auth-symlink-read");
      const targetPath = path.join(authDir, "target-creds.json");
      const credsPath = path.join(authDir, "creds.json");
      fsSync.writeFileSync(
        targetPath,
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const rootDir = createTempAuthDir("oriro-wa-auth-symlink-parent");
  ...
  it("reports unstable auth state when the shared barrier read times out", async () => {
    const authDir = createTempAuthDir("oriro-wa-auth-unstable-state");
    fsSync.writeFileSync(
      path.join(authDir, "creds.json"),
      JSON.stringify({ me: { id: "15551234567@s.whatsapp.net" } }),
      "utf-8",
  ...
  it("clears unreadable auth state on explicit logout", async () => {
    await withOwnedOAuthAuthDir("oriro-wa-auth-logout", async (authDir) => {
  ...
  it("does not delete the whole legacy auth root when targeted cleanup fails", async () => {
    const authDir = createTempAuthDir("oriro-wa-auth-legacy-failure");
    const previousOAuthDir = hoisted.oauthDir;
    fsSync.writeFileSync(path.join(authDir, "creds.json"), "{}", "utf-8");
  ...
  it("clears auth state even when directory enumeration fails", async () => {
    await withOwnedOAuthAuthDir("oriro-wa-auth-readdir", async (authDir) => {
      fsSync.writeFileSync(path.join(authDir, "creds.json"), "{}", "utf-8");
      const readdirSpy = vi
        .spyOn(fs, "readdir")
  ...
  it("does not delete custom auth directories outside the Oriro auth root", async () => {
    const authDir = createTempAuthDir("oriro-wa-auth-custom");
  ...
  it("does not clear auth files through a symlinked owned auth directory", async () => {
    const previousOAuthDir = hoisted.oauthDir;
    const oauthDir = createTempAuthDir("oriro-wa-auth-symlink-oauth");
    const externalDir = createTempAuthDir("oriro-wa-auth-symlink-target");
    const authDir = path.join(oauthDir, "whatsapp", "default");
    try {
  ...
  it("does not clear auth files through an intermediate symlink in the owned auth tree", async () => {
    const previousOAuthDir = hoisted.oauthDir;
    const oauthDir = createTempAuthDir("oriro-wa-auth-symlink-parent-oauth");
    const externalRoot = createTempAuthDir("oriro-wa-auth-symlink-parent-target");
  ...
  it("does not delete unrelated non-empty directories on logout", async () => {
    const authDir = createTempAuthDir("oriro-wa-auth-unrelated");
    fsSync.writeFileSync(path.join(authDir, "notes.txt"), "keep me", "utf-8");
    const runtime = {
      log: vi.fn(),
      error: vi.fn(),
  ...
  it("throws a typed unstable-auth error when channel selection times out", async () => {
    ...
    const error = await pickWebChannel("auto", "/tmp/oriro-wa-auth-unstable").catch(
