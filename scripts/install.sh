...
# Oriro Installer for macOS and Linux
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://oriro.ai/install.sh | bash
...
resolve_oriro_effective_home() {
    local oriro_home="${ORIRO_HOME:-}"
    if [[ -z "$oriro_home" ]]; then
        echo "$HOME"
        return
    fi
    if [[ "$oriro_home" == "~" ]]; then
...
GUM_VERSION="${ORIRO_GUM_VERSION:-0.17.0}"
GUM=""
GUM_STATUS="skipped"
GUM_REASON=""
...
detect_os_or_die() {
    ...
    if [[ "$OS" == "unknown" ]]; then
        ...
        echo "For Windows, use: iwr -useb https://oriro.ai/install.ps1 | iex"
        exit 1
    fi
...
show_install_plan() {
    ...
    ui_kv "Requested version" "$ORIRO_VERSION"
    if [[ "$USE_BETA" == "1" ]]; then
...
show_footer_links() {
    local faq_url="https://docs.oriro.ai/start/faq"
    if [[ -n "$GUM" ]]; then
        local content
...
cleanup_npm_oriro_paths() {
    ...
    rm -rf "$npm_root"/.oriro-* "$npm_root"/oriro 2>/dev/null || true
}

extract_oriro_conflict_path() {
    local log="$1"
    local path=""
...
cleanup_oriro_bin_conflict() {
    ...
    if [[ -n "$npm_bin" && "$bin_path" != "$npm_bin/oriro" ]]; then
        case "$bin_path" in
            "/opt/homebrew/bin/oriro"|"/usr/local/bin/oriro")
                ;;
            *)
                return 1
                ;;
    ...
    if [[ -L "$bin_path" ]]; then
        ...
        if [[ "$target" == *"/node_modules/oriro/"* ]]; then
            rm -f "$bin_path"
            ui_info "Removed stale oriro symlink at ${bin_path}"
            return 0
        fi
        return 1
    fi
    ...
    if mv "$bin_path" "$backup"; then
        ui_info "Moved existing oriro binary to ${backup}"
        return 0
    fi
    return 1
}
...
install_oriro_npm() {
    ...
    if ! run_npm_global_install "$spec" "$log"; then
        ...
        if grep -q "ENOTEMPTY: directory not empty, rename .*oriro" "$log"; then
        ...
        if grep -q "EEXIST" "$log"; then
            ...
            ui_error "npm failed because an oriro binary already exists"
            if [[ -n "$conflict" ]]; then
...
TAGLINES=()
TAGLINES+=("Your terminal just grew oriros—type something and let the bot pinch the busywork.")
...
TAGLINES+=("Automation with oriros: minimal fuss, maximal pinch.")
...
HOLIDAY_CHRISTMAS="Christmas: Ho ho ho—Santa's little oriro-sistant is here to ship joy, roll back chaos, and stash the keys safely."
...
pick_tagline() {
    ...
    if [[ -n "${ORIRO_TAGLINE_INDEX:-}" ]]; then
        if [[ "${ORIRO_TAGLINE_INDEX}" =~ ^[0-9]+$ ]]; then
            local idx=$((ORIRO_TAGLINE_INDEX % count))
            echo "${TAGLINES[$idx]}"
            return
        fi
    fi
...
NO_ONBOARD=${ORIRO_NO_ONBOARD:-0}
NO_PROMPT=${ORIRO_NO_PROMPT:-0}
DRY_RUN=${ORIRO_DRY_RUN:-0}
INSTALL_METHOD=${ORIRO_INSTALL_METHOD:-}
ORIRO_VERSION=${ORIRO_VERSION:-latest}
USE_BETA=${ORIRO_BETA:-0}
GIT_DIR_DEFAULT="$(resolve_oriro_effective_home)/oriro"
GIT_DIR=${ORIRO_GIT_DIR:-$GIT_DIR_DEFAULT}
GIT_UPDATE=${ORIRO_GIT_UPDATE:-1}
NPM_LOGLEVEL="${ORIRO_NPM_LOGLEVEL:-error}"
NPM_SILENT_FLAG="--silent"
VERBOSE="${ORIRO_VERBOSE:-0}"
VERIFY_INSTALL="${ORIRO_VERIFY_INSTALL:-0}"
ORIRO_BIN=""
PNPM_CMD=()
HELP=0
...
print_usage() {
    cat <<EOF
Oriro installer (macOS + Linux)

Usage:
  curl -fsSL --proto '=https' --tlsv1.2 https://oriro.ai/install.sh | bash -s -- [options]
...
Options:
  ...
  --git-dir, --dir <path>             Checkout directory (default: ~/oriro)
...
Environment variables:
  ORIRO_INSTALL_METHOD=git|npm
  ORIRO_VERSION=latest|next|<semver>|<spec>
  ORIRO_BETA=0|1
  ORIRO_GIT_DIR=...
  ORIRO_GIT_UPDATE=0|1
  ORIRO_NO_PROMPT=1
  ORIRO_VERIFY_INSTALL=1
  ORIRO_DRY_RUN=1
  ORIRO_NO_ONBOARD=1
  ORIRO_VERBOSE=1
  ORIRO_NPM_LOGLEVEL=error|warn|notice  Default: error (hide npm deprecation noise)
Examples:
  curl -fsSL --proto '=https' --tlsv1.2 https://oriro.ai/install.sh | bash
  curl -fsSL --proto '=https' --tlsv1.2 https://oriro.ai/install.sh | bash -s -- --no-onboard
  curl -fsSL --proto '=https' --tlsv1.2 https://oriro.ai/install.sh | bash -s -- --no-onboard --verify
  curl -fsSL --proto '=https' --tlsv1.2 https://oriro.ai/install.sh | bash -s -- --install-method git --version main
  curl -fsSL --proto '=https' --tlsv1.2 https://oriro.ai/install.sh | bash -s -- --install-method git --no-onboard
EOF
}
...
choose_install_method_interactive() {
    ...
    if [[ -n "$GUM" ]] && gum_is_tty; then
        ...
        header="Detected Oriro checkout in: ${detected_checkout}
...
${WARN}→${NC} Detected a Oriro source checkout in: ${INFO}${detected_checkout}${NC}
Choose install method:
...
detect_oriro_checkout() {
    ...
    if ! grep -q '"name"[[:space:]]*:[[:space:]]*"oriro"' "$dir/package.json" 2>/dev/null; then
        return 1
    fi
    echo "$dir"
    return 0
...
print_homebrew_admin_fix() {
    ...
    echo "Then retry:"
    echo "  curl -fsSL https://oriro.ai/install.sh | bash"
}
...
ensure_oriro_bin_link() {
    local npm_root=""
    npm_root="$(npm root -g 2>/dev/null || true)"
    if [[ -z "$npm_root" || ! -d "$npm_root/oriro" ]]; then
        return 1
    fi
    local npm_bin=""
    ...
    mkdir -p "$npm_bin"
    if [[ ! -x "${npm_bin}/oriro" ]]; then
        ln -sf "$npm_root/oriro/dist/entry.js" "${npm_bin}/oriro"
        ui_info "Created oriro bin link at ${npm_bin}/oriro"
    fi
    return 0
}
...
resolve_git_oriro_ref() {
    local requested="${ORIRO_VERSION:-latest}"
    local resolved_version=""

    case "$requested" in
        ""|latest)
            resolved_version="$(npm view "oriro" "dist-tags.${requested:-latest}" 2>/dev/null || true)"
...
collect_oriro_npm_root_candidates() {
    local root=""
    ...
    local old_ifs="$IFS"
    IFS=":"
    for extra_root in ${ORIRO_INSTALL_EXTRA_NPM_ROOTS:-}; do
...
find_oriro_global_installs() {
    local seen="|"
    local npm_root=""
    while IFS= read -r npm_root; do
        [[ -n "$npm_root" ]] || continue
        local package_dir="${npm_root%/}/oriro"
        ...
        local version=""
        version="$(oriro_package_version "$package_json")"
        printf '%s\t%s\t%s\n' "$version" "$real_package_dir" "$npm_root"
    done < <(collect_oriro_npm_root_candidates)
}

warn_duplicate_oriro_global_installs() {
    local installs=()
    local line=""
    while IFS= read -r line; do
        [[ -n "$line" ]] && installs+=("$line")
    done < <(find_oriro_global_installs)
    ...
    active_oriro="${ORIRO_BIN:-}"
    if [[ -z "$active_oriro" ]]; then
        active_oriro="$(type -P oriro 2>/dev/null || true)"
    fi
    echo -e "  Active node: ${INFO}${active_node:-none}${NC}"
    echo -e "  Active npm: ${INFO}${active_npm:-none}${NC}"
    echo -e "  Active oriro: ${INFO}${active_oriro:-none}${NC}"
    echo ""
    echo "  Found installs:"
...
warn_shell_path_missing_dir() {
    ...
    echo "  This can make oriro show as \"command not found\" in new terminals."
    echo "  Fix (zsh: ~/.zshrc, bash: ~/.bashrc):"
    echo "    export PATH=\"${dir}:\$PATH\""
}

oriro_command_for_user() {
    local oriro="${1:-}"
    if [[ -z "$oriro" ]]; then
        echo "oriro"
        return 0
    ...
    local oriro_dir="${oriro%/*}"
    if [[ "$oriro_dir" != "$oriro" ]] && path_has_dir "$ORIGINAL_PATH" "$oriro_dir"; then
        echo "oriro"
        return 0
    fi

    local quoted_oriro=""
    printf -v quoted_oriro '%q' "$oriro"
    echo "$quoted_oriro"
}
...
bounded_probe_output() {
    local label="$1"
    shift
    local timeout_seconds="${ORIRO_INSTALL_PROBE_TIMEOUT_SECONDS:-5}"
...
warn_oriro_not_found() {
    ui_warn "Installed, but oriro is not discoverable on PATH in this shell"
    echo "  Try: hash -r (bash) or rehash (zsh), then retry."
    local t=""
    t="$(type -t oriro 2>/dev/null || true)"
    if [[ "$t" == "alias" || "$t" == "function" ]]; then
        ui_warn "Found a shell ${t} named oriro; it may shadow the real binary"
    fi
    if command -v nodenv &> /dev/null; then
...
resolve_oriro_bin() {
    refresh_shell_command_cache
    local resolved=""
    resolved="$(type -P oriro 2>/dev/null || true)"
    ...
    local npm_bin=""
    npm_bin="$(npm_global_bin_dir || true)"
    if [[ -n "$npm_bin" && -x "${npm_bin}/oriro" ]]; then
        echo "${npm_bin}/oriro"
        return 0
    fi
...
install_oriro_from_git() {
    ...
    local git_ref
    git_ref="$(resolve_git_oriro_ref)"
    if [[ -z "$(git -C "$repo_dir" status --porcelain 2>/dev/null || true)" ]]; then
        ui_info "Using git ref: ${git_ref}"
        checkout_git_oriro_ref "$repo_dir" "$git_ref"
    else
    ...
    cat > "$HOME/.local/bin/oriro" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec node "${repo_dir}/dist/entry.js" "\$@"
EOF
    chmod +x "$HOME/.local/bin/oriro"
    ui_success "Oriro wrapper installed to \$HOME/.local/bin/oriro"
...
resolve_beta_version() {
    local beta=""
    beta="$(npm view oriro dist-tags.beta 2>/dev/null || true)"
...
is_oriro_source_package_install_spec() {
    local value="${1:-}"
    local normalized_value=""
    normalized_value="$(to_lowercase_ascii "$value")"
    normalized_value="${normalized_value#oriro@}"

    [[ "$normalized_value" == "main" ]] && return 0
    [[ "$normalized_value" =~ ^github:oriro/oriro($|[#/]) ]] && return 0

    normalized_value="${normalized_value#git+}"
    [[ "$normalized_value" =~ ^https?://github\.com/oriro/oriro(\.git)?($|[?#]) ]] && return 0
    [[ "$normalized_value" =~ ^ssh://git@github\.com[:/]oriro/oriro(\.git)?($|[?#]) ]] && return 0
    [[ "$normalized_value" =~ ^git://github\.com/oriro/oriro(\.git)?($|[?#]) ]] && return 0
    [[ "$normalized_value" =~ ^git@github\.com:oriro/oriro(\.git)?($|[?#]) ]] && return 0
    return 1
}
...
install_oriro() {
    local package_name="oriro"
    if [[ "$USE_BETA" == "1" ]]; then
        local beta_version=""
        beta_version="$(resolve_beta_version || true)"
        if [[ -n "$beta_version" ]]; then
            ORIRO_VERSION="$beta_version"
            ui_info "Beta tag detected (${beta_version})"
            package_name="oriro"
        else
            ORIRO_VERSION="latest"
            ui_info "No beta tag found; using latest"
        fi
    fi

    if [[ -z "${ORIRO_VERSION}" ]]; then
        ORIRO_VERSION="latest"
    fi
    ...
    local resolved_version=""
    if can_resolve_registry_package_version "${ORIRO_VERSION}"; then
    ...
    if ! install_oriro_npm "${install_spec}"; then
        ui_warn "npm install failed; retrying"
        cleanup_npm_oriro_paths
        install_oriro_npm "${install_spec}"
    fi

    if [[ "${ORIRO_VERSION}" == "latest" && "${package_name}" == "oriro" ]]; then
        if ! resolve_oriro_bin &> /dev/null; then
            ui_warn "npm install oriro@latest failed; retrying oriro@next"
            cleanup_npm_oriro_paths
            install_oriro_npm "oriro@next"
        fi
    fi
...
run_doctor() {
    ...
    local oriro="${ORIRO_BIN:-}"
    if [[ -z "$oriro" ]]; then
        oriro="$(resolve_oriro_bin || true)"
    fi
    if [[ -z "$oriro" ]]; then
        ui_info "Skipping doctor (oriro not on PATH yet)"
        warn_oriro_not_found
        return 0
    fi
    run_quiet_step "Running doctor" "$oriro" doctor --non-interactive || true
...
maybe_open_dashboard() {
    ...
    if ! "$oriro" dashboard --help >/dev/null 2>&1; then
        return 0
    fi
    "$oriro" dashboard || true
}

resolve_workspace_dir() {
    local profile="${ORIRO_PROFILE:-default}"
    local effective_home
    effective_home="$(resolve_oriro_effective_home)"
    if [[ "${profile}" != "default" ]]; then
        echo "${effective_home}/.oriro/workspace-${profile}"
    else
        echo "${effective_home}/.oriro/workspace"
    fi
}
...
run_bootstrap_onboarding_if_needed() {
    ...
    if ! is_promptable; then
        local user_oriro
        user_oriro="$(oriro_command_for_user "${ORIRO_BIN:-}")"
        ui_info "BOOTSTRAP.md found but no TTY; run ${user_oriro} onboard to finish setup"
        return
    fi
    ...
    if [[ -z "$oriro" ]]; then
        ui_info "BOOTSTRAP.md found but oriro not on PATH; skipping onboarding"
        warn_oriro_not_found
        return
    fi
    ...
    "$oriro" onboard || {
        local user_oriro
        user_oriro="$(oriro_command_for_user "$oriro")"
        ui_error "Onboarding failed; run ${user_oriro} onboard to retry"
        return
    }
}
...
if ! declare -F extract_oriro_semver >/dev/null 2>&1; then
...
resolve_oriro_version() {
    ...
    local oriro="${ORIRO_BIN:-}"
    if [[ -z "$oriro" ]] && command -v oriro &> /dev/null; then
        oriro="$(command -v oriro)"
    fi
    if [[ -n "$oriro" ]]; then
        raw_version_output=$("$oriro" --version 2>/dev/null || true)
        raw_version_output="${raw_version_output%%$'\n'*}"
        raw_version_output="${raw_version_output//$'\r'/}"
        version="$(extract_oriro_semver "$raw_version_output")"
        if [[ -z "$version" ]]; then
    ...
    if [[ -z "$version" ]]; then
        local npm_root=""
        npm_root=$(npm root -g 2>/dev/null || true)
        if [[ -n "$npm_root" && -f "$npm_root/oriro/package.json" ]]; then
            version=$(node -e "console.log(require('${npm_root}/oriro/package.json').version)" 2>/dev/null || true)
        fi
    fi
    echo "$version"
}
...
is_gateway_daemon_loaded() {
    ...
    local status_json=""
    status_json="$(bounded_probe_output "oriro daemon status --json" "$oriro" daemon status --json || true)"
...
refresh_gateway_service_if_loaded() {
    ...
    if run_quiet_step "Refreshing gateway service" "$oriro" gateway install --force; then
    ...
    if run_quiet_step "Restarting gateway service" "$oriro" gateway restart; then
    ...
    run_quiet_step "Probing gateway service" "$oriro" gateway status --deep || true
}
...
verify_installation() {
    ...
    if [[ -z "$oriro" ]]; then
        ui_error "Install verify failed: oriro not on PATH yet"
        warn_oriro_not_found
        return 1
    fi
    ...
    if is_gateway_daemon_loaded "$oriro"; then
        run_quiet_step "Checking gateway service" "$oriro" gateway status --deep || {
            ...
            ui_info "Run: oriro gateway status --deep"
            return 1
        }
    else
...
main() {
    ...
    detected_checkout="$(detect_oriro_checkout "$PWD" || true)"
        ...
        if ! is_promptable; then
            ...
            case "$selected_method" in
                ...
                *)
                    ...
                    echo "Re-run with: --install-method git|npm (or set ORIRO_INSTALL_METHOD)."
                    exit 2
                    ;;
            esac
        fi
    ...
    if [[ "$INSTALL_METHOD" == "git" ]]; then
        ...
        if npm list -g oriro &>/dev/null; then
            ui_info "Removing npm global install (switching to git)"
            npm uninstall -g oriro 2>/dev/null || true
        ...
        install_oriro_from_git "$repo_dir"
    else
        # Clean up git wrapper if switching to npm
        if [[ -x "$HOME/.local/bin/oriro" ]]; then
            ui_info "Removing git wrapper (switching to npm)"
            rm -f "$HOME/.local/bin/oriro"
    ...
    ORIRO_BIN="$(resolve_oriro_bin || true)"
    warn_duplicate_oriro_global_installs || true
    ...
    if [[ "$INSTALL_METHOD" == "git" && -n "$final_git_dir" ]]; then
        ...
        ui_kv "Wrapper" "$HOME/.local/bin/oriro"
        ui_kv "Update command" "oriro update"
        ui_kv "Switch to npm" "curl -fsSL --proto '=https' --tlsv1.2 https://oriro.ai/install.sh | bash -s -- --install-method npm"
        ...
        if has_controlling_tty || [[ "$NO_ONBOARD" == "1" || "$NO_PROMPT" == "1" ]]; then
            ...
            ui_info "Running oriro doctor"
            local doctor_ok=0
            if (( ${#doctor_args[@]} )); then
                ORIRO_UPDATE_IN_PROGRESS=1 "$oriro" doctor "${doctor_args[@]}" </dev/null && doctor_ok=1
            else
                ORIRO_UPDATE_IN_PROGRESS=1 "$oriro" doctor </dev/tty && doctor_ok=1
            fi
            if (( doctor_ok )); then
                ui_info "Updating plugins"
                ORIRO_UPDATE_IN_PROGRESS=1 "$oriro" plugins update --all || true
            else
            ...
            local user_oriro
            user_oriro="$(oriro_command_for_user "${ORIRO_BIN:-}")"
            ui_info "No TTY; run ${user_oriro} doctor and ${user_oriro} plugins update --all manually"
        fi
    else
        if [[ "$NO_ONBOARD" == "1" || "$skip_onboard" == "true" ]]; then
            local user_oriro
            user_oriro="$(oriro_command_for_user "${ORIRO_BIN:-}")"
            ui_info "Skipping onboard (requested); run ${user_oriro} onboard later"
        else
            local effective_home
            effective_home="$(resolve_oriro_effective_home)"
            local config_path="${ORIRO_CONFIG_PATH:-$effective_home/.oriro/oriro.json}"
            ...
            if is_promptable; then
                ...
                if [[ -z "$oriro" ]]; then
                    ui_info "Skipping onboarding (oriro not on PATH yet)"
                    warn_oriro_not_found
                    return 0
                fi
                exec </dev/tty
            ...
            local user_oriro
            user_oriro="$(oriro_command_for_user "${ORIRO_BIN:-}")"
            ui_info "No TTY; run ${user_oriro} onboard to finish setup"
            return 0
        fi
    fi

    if command -v oriro &> /dev/null; then
        local oriro="${ORIRO_BIN:-}"
        if [[ -z "$oriro" ]]; then
            oriro="$(resolve_oriro_bin || true)"
        fi
        if [[ -n "$oriro" ]] && is_gateway_daemon_loaded "$oriro"; then
            if [[ "$DRY_RUN" == "1" ]]; then
                ui_info "Gateway daemon detected; would restart (oriro daemon restart)"
            else
                ui_info "Gateway daemon detected; restarting"
                if ORIRO_UPDATE_IN_PROGRESS=1 "$oriro" daemon restart >/dev/null 2>&1; then
                    ui_success "Gateway restarted"
                else
                    ui_warn "Gateway restart failed; try: oriro daemon restart"
                fi
            fi
        fi
    fi
...
if [[ "${ORIRO_INSTALL_SH_NO_RUN:-0}" != "1" ]]; then
    parse_args "$@"
    configure_install_stage_total
    configure_verbose
    main
