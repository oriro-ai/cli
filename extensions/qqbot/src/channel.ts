...
import { getExecApprovalReplyMetadata } from "oriro/plugin-sdk/approval-runtime";
import {
...
import { sanitizeAssistantVisibleText } from "oriro/plugin-sdk/text-chunking";
...
import { toGatewayAccount, writeOriroConfigThroughRuntime } from "./bridge/narrowing.js";
...
export const qqbotPlugin: ChannelPlugin<ResolvedQQBotAccount> = {
  ...
  config: {
     ...
     * that wiped oriro.json mid-flight.
     */
  ...
  gateway: {
    startAccount: async (ctx) => {
      ...
      // oriro.json). We only restore when both fields are empty so a
      ...
      if (!account.appId || !account.clientSecret) {
        ...
        if (backup?.appId && backup?.clientSecret) {
          try {
            ...
            await writeOriroConfigThroughRuntime(getQQBotRuntime(), nextCfg);
            cfg = nextCfg;
      ...
      await startGateway({
        ...
        onReady: () => {
          ...
          // upgrade that might wipe oriro.json mid-flight.
    ...
    logoutAccount: async ({ accountId, cfg }) => {
      ...
      if (changed) {
        await writeOriroConfigThroughRuntime(getQQBotRuntime(), nextCfg as OriroConfig);
      }

      const resolved = resolveQQBotAccount((changed ? nextCfg : cfg) as OriroConfig, accountId);
