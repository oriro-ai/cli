...
struct InstancesStoreTests {
    ...
    func `presence event payload decodes via JSON encoder`() {
        ...
        let entry: [String: OriroProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: OriroProtocol.AnyCodable] = [
            "presence": .init([OriroProtocol.AnyCodable(entry)]),
        ]
        let payload = OriroProtocol.AnyCodable(payloadMap)
