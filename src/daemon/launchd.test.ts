...
import {
  ...
  disableOriroUpdateLaunchdJob,
  findStaleOriroUpdateLaunchdJobs,
  parseLaunchctlPrint,
  parseLaunchctlListOriroUpdateJobs,
...
describe("launchctl list detection", () => {
  it("parses stale Oriro updater jobs from launchctl list", () => {
    const jobs = parseLaunchctlListOriroUpdateJobs(
      [
        "123 0 ai.oriro.gateway",
        "- 127 ai.oriro.update.2026.5.12",
        "- 0 ai.oriro.manual-update.1717168800",
        "8142 0 ai.oriro.update.2026.5.13-beta.1",
        "- 0 ai.oriro.manual-updater.1717168800",
        "- 0 com.example.other",
      ].join("\n"),
    );
    ...
    expect(jobs).toEqual([
      ...
      {
        label: "ai.oriro.update.2026.5.13-beta.1",
        pid: 8142,
        lastExitStatus: 0,
      },
    ]);
  ...
  it.runIf(process.platform === "darwin")(
    "finds stale Oriro updater jobs via launchctl list",
    async () => {
      state.listOutput = "- 127 ai.oriro.update.2026.5.12\n";

      const jobs = await findStaleOriroUpdateLaunchdJobs();
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      state.listOutput = [
        "- 0 ai.oriro.manual-update.1717168800",
        "812 0 ai.oriro.manual-update.profile",
        "913 0 ai.oriro.manual-update.custom-label",
      ].join("\n");

      const jobs = await findStaleOriroUpdateLaunchdJobs({
        ORIRO_PROFILE: "manual-update.profile",
        ORIRO_LAUNCHD_LABEL: "ai.oriro.manual-update.custom-label",
        ORIRO_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
        ORIRO_SERVICE_KIND: GATEWAY_SERVICE_KIND,
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await expect(
        disableCurrentOriroUpdateLaunchdJob({
          LAUNCH_JOB_LABEL: "ai.oriro.update.2026.5.12",
        }),
      ).resolves.toBe(true);
      ...
      expect(state.launchctlCalls).toContainEqual([
        "disable",
        `${domain}/ai.oriro.update.2026.5.12`,
      ]);
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await expect(
        disableCurrentOriroUpdateLaunchdJob({
          LAUNCH_JOB_LABEL: "ai.oriro.manual-update.1717168800",
        }),
      ).resolves.toBe(true);
      ...
      expect(state.launchctlCalls).toContainEqual([
        "disable",
        `${domain}/ai.oriro.manual-update.1717168800`,
      ]);
      expect(launchctlCommandNames()).not.toContain("remove");
    },
  );
  ...
  it.runIf(process.platform === "darwin")(
    "disables the current legacy updater launchd job from Oriro label env",
    async () => {
      await expect(
        disableCurrentOriroUpdateLaunchdJob({
          ORIRO_LAUNCHD_LABEL: "ai.oriro.update.2026.5.12",
        }),
      ).resolves.toBe(true);
  ...
  it.runIf(process.platform === "darwin")(
    "does not let non-update launchd markers mask the Oriro update label",
    async () => {
      await expect(
        disableCurrentOriroUpdateLaunchdJob({
          XPC_SERVICE_NAME: "0",
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await expect(
        disableCurrentOriroUpdateLaunchdJob({
          LAUNCH_JOB_LABEL: "ai.oriro.update.2026.5.12",
          ORIRO_PROFILE: "update.2026.5.12",
        }),
      ).resolves.toBe(false);
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await expect(
        disableCurrentOriroUpdateLaunchdJob({
          LAUNCH_JOB_LABEL: "ai.oriro.manual-update.1717168800",
          ORIRO_PROFILE: "manual-update.1717168800",
        }),
      ).resolves.toBe(false);
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await expect(
        disableCurrentOriroUpdateLaunchdJob({
          LAUNCH_JOB_LABEL: "ai.oriro.manual-update.gateway",
        }),
      ).resolves.toBe(false);
  ...
  it.runIf(process.platform === "darwin")("disables explicit legacy updater jobs", async () => {
    await expect(disableOriroUpdateLaunchdJob("ai.oriro.update.2026.5.12")).resolves.toBe(
      true,
    );
  ...
  it.runIf(process.platform === "darwin")("disables explicit manual updater jobs", async () => {
    await expect(
      disableOriroUpdateLaunchdJob("ai.oriro.manual-update.1717168800"),
    ).resolves.toBe(true);
...
describe("launchd install", () => {
  ...
  it("writes LaunchAgent environment to an owner-only env file when provided", async () => {
    const env = createDefaultLaunchdEnv();
    const tmpDir = "/Users/test/.oriro/tmp";
    ...
    const plistPath = resolveLaunchAgentPlistPath(env);
    const envFilePath = "/Users/test/.oriro/service-env/ai.oriro.gateway.env";
    const wrapperPath = "/Users/test/.oriro/service-env/ai.oriro.gateway-env-wrapper.sh";
    ...
    expect(state.dirModes.get("/Users/test/.oriro/service-env")).toBe(0o700);
  ...
  it("repairs a mangled label-derived service-env wrapper path on restart", async () => {
    ...
    const serviceEnv = {
      ...callerEnv,
      ORIRO_STATE_DIR: "/Users/test/service-env/custom-state",
    };
    await installLaunchAgent({
      env: serviceEnv,
      stdout: new PassThrough(),
      ...
      environment: {
        ...
        ORIRO_STATE_DIR: serviceEnv.ORIRO_STATE_DIR,
      },
    });

    const plistPath = resolveLaunchAgentPlistPath(callerEnv);
    const envFilePath = "/Users/test/service-env/custom-state/service-env/ai.oriro.gateway.env";
    const wrapperPath =
      "/Users/test/service-env/custom-state/service-env/ai.oriro.gateway-env-wrapper.sh";
    const callerEnvFilePath = "/Users/test/.oriro/service-env/ai.oriro.gateway.env";
    const callerWrapperPath =
      "/Users/test/.oriro/service-env/ai.oriro.gateway-env-wrapper.sh";
    const mangledEnvFilePath =
      "/Users/test/service-env/custom-state/service-env/[ai.oriro.gateway.env](http:/ai.oriro.gateway.env)";
    const mangledWrapperPath =
      "/Users/test/service-env/custom-state/service-env/[ai.oriro.gateway-env-wrapper.sh](http:/ai.oriro.gateway-env-wrapper.sh)";
    state.files.set(
      plistPath,
    ...
    expect(command?.programArguments).toEqual(defaultProgramArguments);
    expect(command?.environment?.ORIRO_GATEWAY_PORT).toBe("18789");
    expect(command?.environment?.ORIRO_STATE_DIR).toBe(serviceEnv.ORIRO_STATE_DIR);
    expect(command?.environmentValueSources?.ORIRO_GATEWAY_PORT).toBe("file");
    ...
    expect(rewrittenEnv).toContain("export ORIRO_GATEWAY_PORT='18789'");
    expect(rewrittenEnv).toContain(
      "export ORIRO_STATE_DIR='/Users/test/service-env/custom-state'",
    );
  });
  ...
  it("writes KeepAlive=true policy with shutdown and throttle limits", async () => {
    ...
    expect(plist).toContain("<string>/Users/test/Library/Logs/oriro/gateway.log</string>");
  ...
  it("rewrites the plist before bootstrap during restart fallback", async () => {
    ...
    state.files.set(
      ...
      [
        ...
        "  <dict>",
        "    <key>Label</key>",
        "    <string>ai.oriro.gateway</string>",
  ...
  it("stops LaunchAgent via bootout by default, preserving KeepAlive for future crashes", async () => {
    ...
    const domain = typeof process.getuid === "function" ? `gui/${process.getuid()}` : "gui/501";
    const serviceId = `${domain}/ai.oriro.gateway`;
    ...
    await withProcessEnv(
      ...
      async () => {
        await expect(stopLaunchAgent({ env, stdout: new PassThrough() })).rejects.toThrow(
          "Refusing to stop LaunchAgent ai.oriro.gateway from inside the same launchd service",
        );
      },
    );
  ...
  it("allows external LaunchAgent label overrides to stop the selected target", async () => {
    const env = {
      ...
      ORIRO_LAUNCHD_LABEL: "com.example.oriro.gateway",
    };
    const stdout = new PassThrough();
    let output = "";
    stdout.on("data", (chunk: Buffer) => {
    ...
    await withProcessEnv(
      {
        ...
        ORIRO_LAUNCHD_LABEL: undefined,
        ORIRO_SERVICE_MARKER: undefined,
        ORIRO_SERVICE_KIND: undefined,
      },
      async () => {
        await stopLaunchAgent({ env, stdout });
      },
    ...
    const domain = typeof process.getuid === "function" ? `gui/${process.getuid()}` : "gui/501";
    const serviceId = `${domain}/com.example.oriro.gateway`;
    expect(state.launchctlCalls).toEqual([["bootout", serviceId]]);
    expect(output).toContain("Stopped LaunchAgent");
  });
  ...
  it("resolves the stop postcondition port from the stored LaunchAgent environment", async () => {
    ...
    await installLaunchAgent({
      ...
      stdout: new PassThrough(),
      programArguments: defaultProgramArguments,
      environment: { ORIRO_GATEWAY_PORT: "19006" },
    });
    state.launchctlCalls.length = 0;
  ...
  it("reloads launchd after rewriting an existing plist", async () => {
    ...
    state.files.set(
      ...
      [
        ...
        "    <string>/Users/test/.oriro-default/logs/gateway.log</string>",
        "  </dict>",
        "</plist>",
      ].join("\n"),
    );
  ...
  it("uses the stored LaunchAgent environment port for restart stale cleanup", async () => {
    ...
    await installLaunchAgent({
      ...
      stdout: new PassThrough(),
      programArguments: defaultProgramArguments,
      environment: { ORIRO_GATEWAY_PORT: "19007" },
    });
    state.launchctlCalls.length = 0;
  ...
  it("ignores invalid stored LaunchAgent environment ports for stale cleanup", async () => {
    ...
    await installLaunchAgent({
      ...
      stdout: new PassThrough(),
      programArguments: defaultProgramArguments,
      environment: { ORIRO_GATEWAY_PORT: "65536" },
    });
    state.launchctlCalls.length = 0;
  ...
  it("hands restart off to a detached helper when invoked from the current LaunchAgent", async () => {
    ...
    const result = await withProcessEnv({ LAUNCH_JOB_LABEL: "ai.oriro.gateway" }, async () =>
      restartLaunchAgent({
        env,
        stdout: new PassThrough(),
      }),
  ...
  it("hands plist reload off when current LaunchAgent needs rewritten paths", async () => {
    ...
    expect(state.files.get(plistPath)).toContain("/Users/test/Library/Logs/oriro/gateway.log");
    expect(state.launchctlCalls).toStrictEqual([]);
  });
  ...
  it("surfaces detached handoff failures", async () => {
    ...
    await expect(
      withProcessEnv({ LAUNCH_JOB_LABEL: "ai.oriro.gateway" }, async () =>
        restartLaunchAgent({
          env,
          stdout: new PassThrough(),
        }),
  ...
  it("shows actionable guidance when launchctl gui domain does not support bootstrap", async () => {
    ...
    expect(message).toContain("https://docs.oriro.ai/gateway");
  });
...
describe("resolveLaunchAgentPlistPath", () => {
  it.each([
    {
      name: "uses default label when ORIRO_PROFILE is unset",
      env: { HOME: "/Users/test" },
      expected: "/Users/test/Library/LaunchAgents/ai.oriro.gateway.plist",
    },
    {
      name: "uses profile-specific label when ORIRO_PROFILE is set to a custom value",
      env: { HOME: "/Users/test", ORIRO_PROFILE: "jbphoenix" },
      expected: "/Users/test/Library/LaunchAgents/ai.oriro.jbphoenix.plist",
    },
    {
      name: "prefers ORIRO_LAUNCHD_LABEL over ORIRO_PROFILE",
      env: {
        HOME: "/Users/test",
        ORIRO_PROFILE: "jbphoenix",
        ORIRO_LAUNCHD_LABEL: "com.custom.label",
      },
      expected: "/Users/test/Library/LaunchAgents/com.custom.label.plist",
    },
    {
      name: "trims whitespace from ORIRO_LAUNCHD_LABEL",
      env: {
        HOME: "/Users/test",
        ORIRO_LAUNCHD_LABEL: "  com.custom.label  ",
      },
    ...
    {
      name: "ignores empty ORIRO_LAUNCHD_LABEL and falls back to profile",
      env: {
        HOME: "/Users/test",
        ORIRO_PROFILE: "myprofile",
        ORIRO_LAUNCHD_LABEL: "   ",
      },
      expected: "/Users/test/Library/LaunchAgents/ai.oriro.myprofile.plist",
    },
  ])("$name", ({ env, expected }) => {
  ...
  it("rejects invalid launchd labels that contain path separators", () => {
    expect(() =>
      resolveLaunchAgentPlistPath({
        HOME: "/Users/test",
        ORIRO_LAUNCHD_LABEL: "../evil/label",
      }),
