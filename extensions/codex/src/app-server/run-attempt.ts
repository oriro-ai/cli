...
import { loadExecApprovals } from "oriro/plugin-sdk/exec-approvals-runtime";
import { pathExists } from "oriro/plugin-sdk/security-runtime";
import {
  resolveCodexAppServerForModelProvider,
  resolveCodexAppServerForOriroToolPolicy,
...
import {
  ...
  resolveOriroCodingToolsSessionKeys,
  resetOriroCodingToolsFactoryForTests,
  setOriroCodingToolsFactoryForTests,
...
export async function runCodexAppServerAttempt(
  ...
  let configuredAppServer = resolveCodexAppServerRuntimeOptions({
    ...
    config: params.config,
    agentDir,
    openOriroSandboxActive: sandbox?.enabled === true,
  });
  ...
  let policyAppServer = resolveCodexAppServerForOriroToolPolicy({
  ...
  if (configuredAppServer.approvalPolicy === "never" && appServer.approvalPolicy === "untrusted") {
    embeddedAgentLog.info("codex app-server approval policy promoted for ORIRO tool policy", {
      from: "never",
      to: "untrusted",
  ...
  policyAppServer = resolveCodexAppServerForOriroToolPolicy({
    appServer: configuredAppServer,
    pluginConfig,
    env: process.env,
    shouldPromote:
  ...
  // Oriro transcript is persistence/search state. Context-engine output is
  ...
  const decorateCodexTurnPromptText = (promptBuildResult: {
    ...
    const turnPromptText = prependCodexOriroPromptContext(
  ...
  const pendingOriroDynamicToolCompletionIds = new Set<string>();
  ...
  const releaseTurnAfterTerminalDynamicTool = (paramsValue: {
    ...
    if (
      !shouldReleaseTurnAfterTerminalDynamicTool({
        ...
        pendingOriroDynamicToolCompletionIdsCount: pendingOriroDynamicToolCompletionIds.size,
      })
    ) {
      return;
    }
  ...
  const emitFastModeAutoProgress = async (payload: {
    ...
    try {
      await params.onToolResult?.({
        text: summary,
        channelData: { oriroProgressKind: FAST_MODE_AUTO_PROGRESS_KIND },
      });
    } catch (error) {
  ...
  const handleNotification = async (notification: CodexServerNotification) => {
    ...
    const notificationState = applyCodexTurnNotificationState({
      ...
      activeTurnItemIds,
      activeAppServerTurnRequests,
      pendingOriroDynamicToolCompletionIds,
  ...
  const requestCleanup = client.addRequestHandler(async (request) => {
    ...
    try {
      ...
      pendingOriroDynamicToolCompletionIds.add(call.callId);
      ...
      try {
        ...
        pendingOriroDynamicToolCompletionIds.delete(call.callId);
  ...
  if (activeNativeTurnIds.length > 0) {
    ...
    // Starting an Oriro turn before that native turn completes can wedge the
  ...
  try {
    ...
    if (isCodexActiveCompactTurnError(turnStartError)) {
      ...
      // the next Oriro turn collides with that compact turn, wait for the
    ...
    if (
      ...
      // Do not try to pre-compact or summarize through Oriro here. Codex owns
      // automatic compaction; Oriro may only discard a stale projection thread
  ...
  try {
    ...
    const canUseCompletedAssistantTextAfterClientClose =
      ...
      activeTurnItemIds.size === 0 &&
      pendingOriroDynamicToolCompletionIds.size === 0;
...
function handleApprovalRequest(params: {
  ...
  execPolicy?: Pick<OriroExecPolicyForCodexAppServer, "mode">;
