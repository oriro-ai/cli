...
export function isCurrentProcessLaunchdServiceLabel(
  ...
  const configuredLabel = normalizeOptionalString(env.ORIRO_LAUNCHD_LABEL);
  if (!configuredLabel || configuredLabel !== label) {
    return false;
  }
  if (
    normalizeOptionalString(env.ORIRO_SERVICE_MARKER) === "oriro" &&
    Boolean(normalizeOptionalString(env.ORIRO_SERVICE_KIND))
  ) {
