...
enum SettingsNotificationStatus: Equatable {
    ...
    var color: Color {
        self == .allowed ? OriroBrand.ok : .secondary
    }
...
private struct SettingsGatewayStatesPreview: View {
                ...
                VStack(alignment: .leading, spacing: 16) {
                    self.stateSection("Connected") {
                        self.gatewayStatusCard(
                            ...
                            detail: "Connected to oriro-gateway.tailnet.ts.net.",
                            value: "online",
                            color: OriroBrand.ok)
                        self.gatewayFactsCard(
                            address: "100.88.41.20:18789",
                            server: "oriro-gateway",
                            discovered: "3",
                            agent: "Aiden")
                    }
    ...
    private static let pairingProblem = GatewayConnectionProblem(
        ...
        message: "Run /pair approve in your Oriro chat before this iPad can connect.",
