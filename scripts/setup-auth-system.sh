#!/bin/bash
# Setup Oriro Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'oriro-alerts')"
echo ""
...
SERVICE_TEMPLATE="$SCRIPT_DIR/systemd/oriro-auth-monitor.service"
SYSTEMD_USER_DIR="$HOME/.config/systemd/user"
SERVICE_TARGET="$SYSTEMD_USER_DIR/oriro-auth-monitor.service"
TIMER_TARGET="$SYSTEMD_USER_DIR/oriro-auth-monitor.timer"
...
SERVICE_TEMP="$(mktemp "$SYSTEMD_USER_DIR/oriro-auth-monitor.service.XXXXXX")"
SERVICE_RENDERED=""
cleanup_service_temp() {
    rm -f "$SERVICE_TEMP" "$SERVICE_RENDERED"
}
trap cleanup_service_temp EXIT
SERVICE_RENDERED="$(mktemp "$SYSTEMD_USER_DIR/oriro-auth-monitor.service.rendered.XXXXXX")"
...
RENDERED_NTFY_LINE="$(render_environment_line "NOTIFY_NTFY" "oriro-alerts" "$NTFY_TOPIC")"
...
trap - EXIT
cp "$SCRIPT_DIR/systemd/oriro-auth-monitor.timer" "$TIMER_TARGET"
systemctl --user daemon-reload
systemctl --user enable --now oriro-auth-monitor.timer
...
echo "   scp $SCRIPT_DIR/termux-quick-auth.sh phone:~/.shortcuts/OrirodAuth"
echo "   scp $SCRIPT_DIR/termux-auth-widget.sh phone:~/.shortcuts/OrirodAuth-Full"
echo ""
echo "4. Make them executable on phone:"
echo "   ssh phone 'chmod +x ~/.shortcuts/Orirod*'"
echo ""
echo "5. Add Termux:Widget to your home screen"
echo "6. Tap the widget to see your auth scripts"
echo ""
echo "The quick widget (OrirodAuth) shows status and opens auth URL if needed."
echo "The full widget (OrirodAuth-Full) provides guided re-auth flow."
echo ""
...
echo "  - Auth monitor: systemctl --user status oriro-auth-monitor.timer"
echo ""
echo "Quick commands:"
