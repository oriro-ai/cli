...
export function buildToolPluginPackageManifest(params: {
  ...
  const oriro =
    params.packageManifest.oriro &&
    typeof params.packageManifest.oriro === "object" &&
    !Array.isArray(params.packageManifest.oriro)
      ? { ...(params.packageManifest.oriro as JsonObject) }
      : {};
  const existingExtensions = Array.isArray(oriro.extensions)
    ? oriro.extensions.filter((entry): entry is string => typeof entry === "string")
    : [];
...
export function validateToolPluginProject(params: {
  ...
  if (JSON.stringify(params.manifest) !== JSON.stringify(expectedManifest)) {
    errors.push("oriro.plugin.json generated metadata is stale. Run oriro plugins build.");
  }
  if (params.manifest.id !== params.metadata.id) {
    errors.push(
      `oriro.plugin.json id (${String(params.manifest.id)}) must match entry id (${params.metadata.id})`,
    );
  }
  if (!params.manifest.configSchema || typeof params.manifest.configSchema !== "object") {
    errors.push("oriro.plugin.json must include object configSchema");
  }
  ...
  if (missing.length > 0) {
    errors.push(`oriro.plugin.json contracts.tools is missing: ${missing.join(", ")}`);
  }
  if (extra.length > 0) {
    errors.push(
      `oriro.plugin.json contracts.tools has no matching defineToolPlugin tool: ${extra.join(
        ", ",
      )}`,
    );
  }
  ...
  if (extensionResolution.status !== "ok") {
    errors.push(
      extensionResolution.status === "missing" || extensionResolution.status === "empty"
        ? "package.json must include oriro.extensions"
        : extensionResolution.error,
    );
  } else if (!extensionResolution.entries.includes(params.entry)) {
    errors.push(`package.json oriro.extensions must include ${params.entry}`);
  }
  return errors;
}
...
export async function runPluginsBuildCommand(opts: PluginsBuildOptions): Promise<void> {
  ...
  if (opts.check) {
    ...
    if (
      ...
      defaultRuntime.error("Generated plugin metadata is out of date. Run oriro plugins build.");
...
export async function runPluginsInitCommand(id: string, opts: PluginsInitOptions): Promise<void> {
  ...
  const packageManifest = {
    name: `oriro-plugin-${id}`,
    version: "0.1.0",
    type: "module",
    private: true,
    scripts: {
      build: "tsc -p tsconfig.json",
      "plugin:build": "npm run build && oriro plugins build --entry ./dist/index.js",
      "plugin:validate": "npm run build && oriro plugins validate --entry ./dist/index.js",
      test: "vitest run",
    },
...
import { getToolPluginMetadata } from "oriro/plugin-sdk/tool-plugin";
