...
Official Voice Call plugin for **Oriro**.
...
Docs: `https://docs.oriro.ai/plugins/voice-call`
Plugin system: `https://docs.oriro.ai/tools/plugin`
...
- If older configs still use `provider: "log"`, `twilio.from`, or legacy `streaming.*` OpenAI keys, run `oriro doctor --fix` to rewrite them.
- advanced webhook, streaming, and tunnel notes: `https://docs.oriro.ai/plugins/voice-call`
...
- `realtime.consultThinkingLevel` is optional. When set, it overrides the thinking level used by the model behind realtime `oriro_agent_consult` calls.
- `realtime.consultFastMode` is optional. When set, it toggles fast mode for realtime `oriro_agent_consult` calls.

## Stale call reaper

See the plugin docs for recommended ranges and production examples:
`https://docs.oriro.ai/plugins/voice-call#stale-call-reaper`

## TTS for calls

Voice Call uses the core `messages.tts` configuration for
streaming speech on calls. Override examples and provider caveats live here:
`https://docs.oriro.ai/plugins/voice-call#tts-for-calls`
