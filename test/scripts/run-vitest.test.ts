      ...
      return (
        ...
        (params.bundledPlugin &&
          (normalized === "/repo/extensions/codex/package.json" ||
            normalized === "/repo/extensions/codex/oriro.plugin.json"))
      );
    },
...
describe("scripts/run-vitest", () => {
  ...
  it("restores the workspace node_modules link from a hydrated pnpm modules directory", () => {
    ...
    expect(
      resolveVitestCliEntry({
        baseDir: "/repo",
        env: { PNPM_CONFIG_MODULES_DIR: "/runner/oriro-pnpm-node-modules" },
        fsImpl: {
          existsSync: (filePath: string) =>
            filePath.replaceAll("\\", "/") ===
            "/runner/oriro-pnpm-node-modules/vitest/package.json",
    ...
    expect(symlinks).toEqual([
      {
        target: "/runner/oriro-pnpm-node-modules",
        path: "/runner/oriro-pnpm-node-modules/node_modules",
        type: "junction",
      },
      {
        target: "/runner/oriro-pnpm-node-modules",
  ...
  it("self-links hydrated pnpm modules when pnpm lowercases the env key", () => {
    ...
    expect(
      resolveVitestCliEntry({
        baseDir: "/repo",
        env: { npm_config_modules_dir: "/runner/oriro-pnpm-node-modules" },
        fsImpl: {
          existsSync: (filePath: string) =>
            filePath.replaceAll("\\", "/") ===
            "/runner/oriro-pnpm-node-modules/vitest/package.json",
        ...
        platform: "win32",
        requireResolve: () => "/runner/oriro-pnpm-node-modules/vitest/package.json",
      }),
    ).toBe("/repo/node_modules/vitest/vitest.mjs");
    expect(symlinks).toEqual([
      {
  ...
  it("keeps tooling-excluded explicit tests on existing routing", () => {
    const argv = ["run", "test/scripts/oriro-e2e-instance.test.ts"];
    expect(resolveImplicitVitestArgs(argv)).toBe(argv);
  });
  ...
  it("parses the optional no-output timeout env", () => {
    ...
    expect(resolveVitestNoOutputTimeoutMs({ ORIRO_VITEST_NO_OUTPUT_TIMEOUT_MS: "2500" })).toBe(
      2500,
    );
    expect(
      resolveVitestNoOutputTimeoutMs({ ORIRO_VITEST_NO_OUTPUT_TIMEOUT_MS: "0" }),
    ).toBeNull();
    expect(
      resolveVitestNoOutputTimeoutMs({ ORIRO_VITEST_NO_OUTPUT_TIMEOUT_MS: "1e3" }),
    ).toBeNull();
    expect(
      resolveVitestNoOutputTimeoutMs({ ORIRO_VITEST_NO_OUTPUT_TIMEOUT_MS: "2500ms" }),
    ).toBeNull();
  });
  ...
  it("defaults direct non-watch runs to the stall watchdog", () => {
    expect(resolveRunVitestSpawnEnv({ PATH: "/usr/bin" }, ["run"])).toEqual({
      PATH: "/usr/bin",
      ORIRO_VITEST_NO_OUTPUT_HEARTBEAT_MS: "30000",
      ORIRO_VITEST_NO_OUTPUT_TIMEOUT_MS: "120000",
    });
    ...
    expect(
      resolveRunVitestSpawnEnv({ ORIRO_VITEST_NO_OUTPUT_TIMEOUT_MS: "0", PATH: "/usr/bin" }, [
        "run",
      ]),
    ).toEqual({
  ...
  it("uses a longer default stall watchdog for broad e2e and project shard configs", () => {
    ...
    for (const configArg of [
      ...
      expect(resolveRunVitestSpawnEnv({ PATH: "/usr/bin" }, ["run", configArg])).toEqual({
        PATH: "/usr/bin",
        ORIRO_VITEST_NO_OUTPUT_HEARTBEAT_MS: "30000",
        ORIRO_VITEST_NO_OUTPUT_TIMEOUT_MS: timeout,
      });
    }
    for (const configArg of [
      ...
      expect(resolveRunVitestSpawnEnv({ PATH: "/usr/bin" }, ["run", configArg])).toEqual({
        PATH: "/usr/bin",
        ORIRO_VITEST_NO_OUTPUT_HEARTBEAT_MS: "30000",
        ORIRO_VITEST_NO_OUTPUT_TIMEOUT_MS: extraLongTimeout,
      });
    }
    expect(
      resolveDefaultVitestNoOutputTimeoutMs([
  ...
  it("points symlinked source worktree Vitest runs at local bundled plugins", () => {
    expect(
      ...
      ORIRO_BUNDLED_PLUGINS_DIR: nodePath.join("/repo", "extensions"),
  ...
  it("keeps explicit bundled plugin env ahead of symlinked worktree defaults", () => {
    expect(
      resolveLinkedSourceBundledPluginsEnv(
        { ORIRO_BUNDLED_PLUGINS_DIR: "/custom/extensions", PATH: "/usr/bin", PWD: "/repo" },
        {
          baseDir: "/repo",
          fsImpl: createRunVitestFs({ nodeModulesSymlink: true, bundledPlugin: true }),
        },
  ...
  it("does not force the stall watchdog into delegated runner environments", () => {
    ...
    expect(
      resolveTestProjectsRunnerEnv({
        ORIRO_VITEST_NO_OUTPUT_TIMEOUT_MS: "2500",
        PATH: "/usr/bin",
      }),
    ).toEqual({
      ORIRO_VITEST_NO_OUTPUT_TIMEOUT_MS: "2500",
  ...
  posixIt("cleans delegated test-project children when the wrapper is signaled", async () => {
    ...
    const childPidPath = nodePath.join(
      os.tmpdir(),
      `oriro-run-vitest-delegated-child-${process.pid}-${Date.now()}.pid`,
    );
    const descendantPidPath = nodePath.join(
      os.tmpdir(),
      `oriro-run-vitest-delegated-descendant-${process.pid}-${Date.now()}.pid`,
    );
    ...
    fs.writeFileSync(
      ...
      [
        ...
        "  fs.writeFileSync(process.env.ORIRO_DELEGATED_SIGNAL_CHILD_PID!, String(process.pid));",
        "  fs.writeFileSync(process.env.ORIRO_DELEGATED_SIGNAL_DESCENDANT_PID!, String(child.pid));",
        "  await new Promise(() => {});",
        "});",
        "",
      ].join("\n"),
    ...
    const runner = spawn(
      ...
      ["scripts/run-vitest.mjs", fixturePath, "--reporter=verbose"],
      {
        env: {
          ...process.env,
          ORIRO_DELEGATED_SIGNAL_CHILD_PID: childPidPath,
          ORIRO_DELEGATED_SIGNAL_DESCENDANT_PID: descendantPidPath,
        },
        stdio: "ignore",
      },
    );
  ...
  posixIt("terminates a silent Vitest child through the watchdog", async () => {
    const watched = spawnWatchedVitestProcess({
      ...
      env: { ORIRO_VITEST_NO_OUTPUT_TIMEOUT_MS: "100" },
    });
  ...
  it("parses the optional watchdog heartbeat interval", () => {
    expect(
      resolveVitestNoOutputHeartbeatMs({ ORIRO_VITEST_NO_OUTPUT_HEARTBEAT_MS: "120000" }),
    ).toBe(120000);
    expect(
      resolveVitestNoOutputHeartbeatMs({ ORIRO_VITEST_NO_OUTPUT_HEARTBEAT_MS: "0" }),
    ).toBeNull();
  });
});
