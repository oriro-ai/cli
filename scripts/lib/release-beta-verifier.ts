// Release Beta Verifier script supports Oriro repository automation.
import { execFileSync } from "node:child_process";
import { mkdirSync, readFileSync, writeFileSync } from "node:fs";
import { dirname, resolve } from "node:path";
import { readBoundedResponseText } from "./bounded-response.ts";
import { collectOriroHubPublishablePluginPackages } from "./plugin-orirohub-release.ts";
import {
...
export type ReleaseVerifyBetaArgs = {
  ...
  skipOriroHub: boolean;
  rerunFailedOriroHub: boolean;
  workflowRuns: {
    fullReleaseValidation?: string;
    oriroNpm?: string;
    pluginNpm?: string;
    pluginOriroHub?: string;
    pluginOriroHubBootstrap?: string;
    npmTelegram?: string;
  };
};
...
export function parseReleaseVerifyBetaArgs(argv: string[]): ReleaseVerifyBetaArgs {
  ...
  if (!version || version.startsWith("-")) {
    throw new Error(
      "Usage: pnpm release:verify-beta -- <version> [--workflow-ref REF] [--orirohub-workflow-ref REF] [--full-release-validation-run ID] [--oriro-npm-run ID] [--plugin-npm-run ID] [--plugin-orirohub-run ID] [--plugin-orirohub-bootstrap-run ID] [--npm-telegram-run ID] [--skip-github-release] [--skip-orirohub]",
    );
  }
  ...
  const parsed: ReleaseVerifyBetaArgs = {
    ...
    distTag: "beta",
    repo: DEFAULT_REPO,
    registry: DEFAULT_ORIROHUB_REGISTRY,
    workflowRef: undefined,
    oriroHubWorkflowRef: undefined,
    pluginSelection: [],
    evidenceOut: undefined,
    skipPostpublish: false,
    skipGitHubRelease: false,
    skipOriroHub: false,
    rerunFailedOriroHub: false,
    workflowRuns: {},
  };
  ...
  for (let index = 0; index < values.length; index += 1) {
    ...
    switch (arg) {
      ...
      case "--workflow-ref":
        ...
        break;
      case "--orirohub-workflow-ref":
        parsed.oriroHubWorkflowRef = next();
        break;
      case "--plugins":
      ...
      case "--full-release-validation-run":
        ...
        break;
      case "--oriro-npm-run":
        parsed.workflowRuns.oriroNpm = next();
        break;
      case "--plugin-npm-run":
        parsed.workflowRuns.pluginNpm = next();
        break;
      case "--plugin-orirohub-run":
        parsed.workflowRuns.pluginOriroHub = next();
        break;
      case "--plugin-orirohub-bootstrap-run":
        parsed.workflowRuns.pluginOriroHubBootstrap = next();
        break;
      ...
      case "--skip-github-release":
        ...
        break;
      case "--skip-orirohub":
        parsed.skipOriroHub = true;
        break;
      case "--rerun-failed-orirohub":
        parsed.rerunFailedOriroHub = true;
        break;
      default:
...
async function fetchWithRetry(
  ...
  for (let attempt = 1; attempt <= attempts; attempt += 1) {
    try {
      const signal = AbortSignal.timeout(ORIROHUB_REQUEST_TIMEOUT_MS);
...
export async function readBoundedJsonResponse(
  response: Response,
  label: string,
  maxBytes = ORIROHUB_RESPONSE_BODY_MAX_BYTES,
...
function readOriroHubTags(detail: unknown): Record<string, string> {
  if (!isRecord(detail)) {
    return {};
  }
...
async function verifyOriroHubPackage(params: {
  registry: string;
  packageName: string;
  version: string;
  distTag: string;
  ...
  const tags = readOriroHubTags(detail);
  if (tags[params.distTag] !== params.version) {
    throw new Error(
      `${params.packageName}: OriroHub tag ${params.distTag} points to ${tags[params.distTag] ?? "<missing>"}, expected ${params.version}.`,
    );
  }

  const versionStatus = await fetchStatusWithRetry(versionUrl, "GET");
  if (versionStatus < 200 || versionStatus >= 300) {
    throw new Error(`${params.packageName}: OriroHub exact version returned HTTP ${versionStatus}.`);
  }

  const artifactStatus = await fetchStatusWithRetry(artifactUrl, "HEAD");
  if (artifactStatus < 200 || artifactStatus >= 400) {
    throw new Error(`${params.packageName}: OriroHub artifact returned HTTP ${artifactStatus}.`);
  }
}
...
export async function verifyBetaRelease(
  ...
  const oriroNpm = await verifyNpmPackage("oriro", args.version, args.distTag);
  lines.push(`oriro npm OK: ${args.version} (${args.distTag})`);
  ...
  if (!args.skipPostpublish) {
    ...
    lines.push("oriro postpublish verifier OK");
  }
  ...
  const oriroHubPlugins = args.skipOriroHub
    ? []
    : collectOriroHubPublishablePluginPackages(rootDir, {
        packageNames: args.pluginSelection.length > 0 ? args.pluginSelection : undefined,
      });
  if (args.skipOriroHub) {
    lines.push("OriroHub skipped");
  } else {
    assertSelectedPackagesResolved({
      label: "OriroHub plugin",
      selection: args.pluginSelection,
      packages: oriroHubPlugins,
    });
    for (const plugin of oriroHubPlugins) {
      await verifyOriroHubPackage({
    ...
    lines.push(`OriroHub OK: ${oriroHubPlugins.length}`);
  }
  ...
  if (args.workflowRuns.pluginOriroHub !== undefined) {
    const oriroHubWorkflowRef = args.oriroHubWorkflowRef ?? args.workflowRef;
    workflowRuns.push(
      verifyWorkflowRun({
        id: args.workflowRuns.pluginOriroHub,
        label: "Plugin OriroHub Release",
        repo: args.repo,
        expectedWorkflowName: "Plugin OriroHub Release",
        expectedHeadBranch: oriroHubWorkflowRef,
        rerunFailed: args.rerunFailedOriroHub,
      }),
    );
  }
  if (args.workflowRuns.pluginOriroHubBootstrap !== undefined) {
    const oriroHubWorkflowRef = args.oriroHubWorkflowRef ?? args.workflowRef;
    workflowRuns.push(
      verifyWorkflowRun({
        id: args.workflowRuns.pluginOriroHubBootstrap,
        label: "Plugin OriroHub New",
        repo: args.repo,
        expectedWorkflowName: "Plugin OriroHub New",
        expectedHeadBranch: oriroHubWorkflowRef,
        rerunFailed: false,
      }),
    );
  }
  if (args.workflowRuns.oriroNpm !== undefined) {
    workflowRuns.push(
      verifyWorkflowRun({
      ...
      `${JSON.stringify(
        {
          ...
          oriroNpmIntegrity: oriroNpm.integrity,
          oriroNpmTarball: oriroNpm.tarball,
          ...
          oriroHubPackageCount: oriroHubPlugins.length,
          workflowRuns,
        },
        null,
        2,
