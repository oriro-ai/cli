// Covers Oriro package root resolution.
...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__oriro_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "oriro-root");
...
function setPackageRoot(root: string, name = "oriro") {
  setFile(path.join(root, "package.json"), JSON.stringify({ name }));
}

function expectResolvedPackageRoot(
  syncResolver: typeof import("./oriro-root.js").resolveOriroPackageRootSync,
  asyncResolver: typeof import("./oriro-root.js").resolveOriroPackageRoot,
  opts: Parameters<typeof import("./oriro-root.js").resolveOriroPackageRootSync>[0],
...
describe("resolveOriroPackageRoot", () => {
  ...
  let resolveOriroPackageRootSync: typeof import("./oriro-root.js").resolveOriroPackageRootSync;
  let clearOriroPackageRootCaches: typeof import("./oriro-root.js").testing.clearOriroPackageRootCaches;

  beforeAll(async () => {
    ({
      resolveOriroPackageRoot,
      resolveOriroPackageRootSync,
      testing: { clearOriroPackageRootCaches },
    } = await import("./oriro-root.js"));
  });

  beforeEach(() => {
    clearOriroPackageRootCaches();
    state.entries.clear();
  ...
  it.each([
    {
      ...
      setup: () => {
        ...
        const argv1 = path.join(project, "node_modules", ".bin", "oriro");
        const pkgRoot = path.join(project, "node_modules", "oriro");
    ...
    },
    {
      ...
      setup: () => {
        ...
        const bin = path.join(project, "bin", "oriro");
        const realPkg = path.join(project, "real-pkg");
        state.realpaths.set(abs(bin), abs(path.join(realPkg, "oriro.mjs")));
    ...
    {
      name: "falls through from a non-oriro moduleUrl candidate to cwd",
      setup: () => {
        const wrongPkgRoot = fx("moduleurl-fallthrough", "wrong");
        const cwdPkgRoot = fx("moduleurl-fallthrough", "cwd");
        setPackageRoot(wrongPkgRoot, "not-oriro");
    ...
    {
      name: "returns null for non-oriro package roots",
      setup: () => {
        const pkgRoot = fx("not-oriro");
        setPackageRoot(pkgRoot, "not-oriro");
    ...
    },
    {
      ...
      setup: () => {
        ...
        state.realpaths.set(
          abs(argv1),
          abs(path.join(project, "versions", "current", "oriro.mjs")),
        );
        const pkgRoot = path.join(project, "node_modules", "oriro");
        setPackageRoot(pkgRoot);
        return { opts: { argv1 }, expected: pkgRoot };
