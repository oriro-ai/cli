---
summary: "CLI reference for `oriro models` (status/list/set/scan, aliases, fallbacks, auth)"
read_when:
...
oriro models status
oriro models list
oriro models set <model-or-alias>
oriro models scan
```

`oriro models status` shows the resolved default/fallbacks plus an auth overview.
...
when available; otherwise Oriro falls back to matching OAuth/API-key
...
the command uses `ORIRO_AGENT_DIR` if set, otherwise the
configured default agent.
Probe rows can come from auth profiles, env credentials, or `models.json`.
For OpenAI ChatGPT/Codex OAuth troubleshooting, `oriro models status`,
`oriro models auth list --provider openai`, and
`oriro config get agents.defaults.model --json` are the quickest way to
...
- If you omit the provider, Oriro resolves the input as an alias first, then
  ...
  If that provider no longer exposes the configured default model, Oriro
...
By default Oriro tries to probe tool and image support with live model calls.
...
- `--probe-concurrency <n>`
- `--probe-max-tokens <n>`
- `--agent <id>` (configured agent id; overrides `ORIRO_AGENT_DIR`)
...
- `no_model`: provider auth exists, but Oriro could not resolve a probeable
...
```bash
oriro models auth add
oriro models auth list [--provider <id>] [--json]
oriro models auth login --provider <id>
oriro models auth login --provider openai --profile-id openai:work
oriro models auth paste-api-key --provider <id>
oriro models auth setup-token --provider <id>
oriro models auth paste-token
```
...
`oriro plugins list` to see which providers are installed.
Use `oriro models auth --agent <id> <subcommand>` to write auth results to a
...
usually as a backup for Codex subscription limits. Run `oriro doctor --fix`
to migrate older legacy OpenAI Codex prefix auth/profile state to `openai`.

Examples:

```bash
oriro models auth login --provider openai --set-default
oriro models auth login --provider openai --method api-key
oriro models auth paste-api-key --provider openai
...
- `paste-api-key` accepts API keys generated elsewhere, prompts for the key
  ...
  `printf "%s\n" "$OPENAI_API_KEY" | oriro models auth paste-api-key --provider openai`.
...
- Anthropic note: Anthropic staff told us Oriro-style Claude CLI usage is allowed again, so Oriro treats Claude CLI reuse and `claude -p` usage as sanctioned for this integration unless Anthropic publishes a new policy.
- Anthropic `setup-token` / `paste-token` remain available as a supported Oriro token path, but Oriro now prefers Claude CLI reuse and `claude -p` when available.
