...
// Npm Update Smoke script supports Oriro repository automation.
...
const updateTimeoutSeconds = readPositiveIntEnv("ORIRO_PARALLELS_NPM_UPDATE_TIMEOUT_S", 1200);
...
const freshLaneTimeoutKillGraceMs = readPositiveIntEnv(
  "ORIRO_PARALLELS_NPM_UPDATE_FRESH_TIMEOUT_KILL_GRACE_MS",
  2_000,
);
...
export function freshLaneTimeoutMs(platform: Platform): number {
  ...
  return resolveSecondsTimerMs(
    readPositiveIntEnv("ORIRO_PARALLELS_NPM_UPDATE_FRESH_TIMEOUT_S", defaultSeconds),
  );
}
...
Options:
  --package-spec <npm-spec>  Baseline npm package spec. Default: oriro@latest
  --update-target <target>    Target passed to guest 'oriro update --tag'.
...
function openOriroVersionFamily(version: string): string {
  return /^(\d{4}\.\d{1,2}\.\d{1,2})(?:[-.]|$)/u.exec(version.trim())?.[1] ?? "";
}

function parseOriroPackageSpecVersion(spec: string): string {
  const value = spec.trim();
  if (!value) {
    return "";
  }
  return resolveOriroRegistryVersion(value) || "";
}
...
export class NpmUpdateSmoke {
  ...
  async run(): Promise<void> {
    ...
    this.runDir = await this.makeRunTempDir("oriro-parallels-npm-update.");
    this.tgzDir = await this.makeRunTempDir("oriro-parallels-npm-update-tgz.");
    try {
      await this.runSteps();
    } finally {
  ...
  protected async runSteps(): Promise<void> {
    ...
    this.packageSpec = this.options.packageSpec || `oriro@${this.latestVersion}`;
    ...
    say(`Run same-guest oriro update to ${this.updateTargetEffective}`);
    ...
    if (this.options.platforms.has("linux")) {
      jobs.push(
        this.spawnFresh("Linux", "linux", ["--vm", this.linuxVm], {
          ORIRO_PARALLELS_LINUX_DISABLE_BONJOUR: "1",
        }),
      );
    }
  ...
  private async prepareUpdateTarget(): Promise<void> {
    ...
    this.updateExpectedNeedle = this.isExplicitPackageTarget(this.updateTargetEffective)
      ? ""
      : resolveOriroRegistryVersion(this.updateTargetEffective) || this.updateTargetEffective;
  ...
  private resolveRegistryPackageMetadata(target: string): {
    ...
    const spec = target.startsWith("oriro@") ? target : `oriro@${target}`;
  ...
  private async guestMacos(
    ...
    const scriptPath = this.writeGuestScript(
      this.macosVm,
      script,
      "oriro-parallels-npm-update-macos",
    );
  ...
  private preflightRegistryUpdateTarget(): void {
    ...
    const baseline = resolveOriroRegistryVersion(this.packageSpec);
    const target = resolveOriroRegistryVersion(this.options.updateTarget);
    if (baseline && target && baseline === target) {
      die(
        `--update-target ${this.options.updateTarget} resolves to oriro@${target}, same as baseline ${this.packageSpec}; publish or choose a newer --update-target before running VM update coverage`,
      );
    }
  }
  ...
  private async extractLastVersion(logPath: string): Promise<string> {
    return await extractLastOriroVersionFromLog(logPath);
  }
  ...
  private async configureTargets(): Promise<void> {
    ...
    if (this.options.betaValidation) {
      const version = resolveOriroRegistryVersion(this.options.betaValidation);
      if (!version) {
        die(`could not resolve beta validation target: ${this.options.betaValidation}`);
      }
      this.options.updateTarget = version;
      this.options.freshTargetSpec = `oriro@${version}`;
      say(`Beta validation target: oriro@${version}`);
    } else if (
      this.options.updateTarget &&
      this.options.updateTarget !== "local-main" &&
      !this.isExplicitPackageTarget(this.options.updateTarget)
    ) {
      const version = resolveOriroRegistryVersion(this.options.updateTarget);
      if (version) {
        this.options.updateTarget = version;
      }
    }
    ...
    if (this.options.freshTargetSpec) {
      const version = resolveOriroRegistryVersion(this.options.freshTargetSpec);
      this.freshTargetSpec = version ? `oriro@${version}` : this.options.freshTargetSpec;
    }
  }

  private assertPublishedTargetMatchesHarnessCheckout(): void {
    if (process.env.ORIRO_PARALLELS_ALLOW_HARNESS_TARGET_MISMATCH === "1") {
      return;
    }
    const candidateVersion =
      this.targetTarballVersion ||
      (this.freshTargetSpec
        ? parseOriroPackageSpecVersion(this.freshTargetSpec)
        : parseOriroPackageSpecVersion(this.options.updateTarget));
    const targetFamily = openOriroVersionFamily(candidateVersion);
    if (!targetFamily) {
      return;
    }
    this.harnessTargetFamily = targetFamily;
    const checkoutFamily = openOriroVersionFamily(this.harnessCheckoutVersion);
    if (checkoutFamily === targetFamily) {
      return;
    }
    die(
      `refusing to run Parallels ${candidateVersion} target with harness checkout ${this.harnessCheckoutVersion || "unknown"}; checkout the matching release branch or set ORIRO_PARALLELS_ALLOW_HARNESS_TARGET_MISMATCH=1 for an intentional cross-version harness run`,
    );
  }
