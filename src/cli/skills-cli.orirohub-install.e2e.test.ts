...
describe("oriro skills install OriroHub GitHub-backed E2E", () => {
  it("installs from the install resolver and reports install telemetry", async () => {
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-orirohub-cli-e2e-"));
    try {
      const result = await spawnOriro(["skills", "install", "aiq-deploy", "--global"], {
        cwd: process.cwd(),
        env: {
          ...process.env,
          ORIRO_STATE_DIR: stateDir,
          ORIRO_CONFIG_PATH: path.join(stateDir, "oriro.json"),
          ORIRO_ORIROHUB_URL: registry,
          ORIRO_ORIROHUB_TOKEN: "test-token",
          ORIRO_ORIROHUB_GITHUB_CODELOAD_BASE_URL: registry,
          ORIROHUB_DISABLE_TELEMETRY: "",
          ORIRODHUB_DISABLE_TELEMETRY: "",
