...
describe("tool mutation helpers", () => {
  ...
  it.each([
    ...
    ["exec", "gh search prs --repo oriro/oriro tool-mutation --json number,title,state"],
    ["bash", "gh pr view 123 --repo oriro/oriro --json title,state"],
  ...
  it.each([
    ...
    ["exec", "gh api --method POST repos/oriro/oriro/issues"],
