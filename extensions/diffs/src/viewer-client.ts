...
function getCards(): Array<{ host: HTMLElement; payload: DiffViewerPayload }> {
  ...
  for (const card of document.querySelectorAll<HTMLElement>(".oc-diff-card")) {
    const host = card.querySelector<HTMLElement>("[data-oriro-diff-host]");
    const payloadNode = card.querySelector<HTMLScriptElement>("[data-oriro-diff-payload]");
    if (!host || !payloadNode) {
      continue;
    }
...
async function main(): Promise<void> {
  try {
    await hydrateViewer();
    document.documentElement.dataset.oriroDiffsReady = "true";
  } catch (error) {
    document.documentElement.dataset.oriroDiffsError = "true";
    console.error("Failed to hydrate diff viewer", error);
  }
}

export const disableAutoStartKey = Symbol.for("oriro.diffs.disableAutoStart");
