...
import {
  ...
  type LegacyOrirodBrowserProfileResidue,
...
import type { ConfigValidationIssue, OriroConfig } from "../config/types.oriro.js";
...
const BROWSER_ORIROD_PROFILE_RESIDUE_CHECK_ID = "core/doctor/browser-orirod-profile-residue";
...
export type CoreHealthCheckDeps = {
  readonly detectUnavailableSkills: (cfg: OriroConfig) => Promise<readonly SkillStatusEntry[]>;
  readonly collectSecurityWarnings: (cfg: OriroConfig) => Promise<readonly string[]>;
...
async function collectSecurityWarningsWithRuntime(cfg: OriroConfig): Promise<readonly string[]> {
...
const gatewayConfigCheck: HealthCheck = {
  ...
  kind: "core",
  description: "oriro.jsonc gateway block is set and unambiguous.",
  source: "doctor",
  async detect(ctx) {
    ...
    if (!ctx.cfg.gateway?.mode) {
      findings.push({
        ...
        path: "gateway.mode",
        fixHint:
          "Run `oriro configure` and set Gateway mode (local/remote), or `oriro config set gateway.mode local`.",
      });
    }
    if (ctx.cfg.gateway?.mode !== "remote" && hasAmbiguousGatewayAuthModeConfig(ctx.cfg)) {
      findings.push({
        ...
        fixHint:
          "Set an explicit mode: `oriro config set gateway.auth.mode token` or `... password`.",
      });
    }
    return findings;
  },
    ...
    return [
      {
        ...
        fixHint:
          "Set commands.ownerAllowFrom to your channel user id, e.g. `oriro config set commands.ownerAllowFrom '[\"telegram:123456789\"]'`.",
      },
    ];
  },
};

function resolveDoctorMode(cfg: OriroConfig): "local" | "remote" {
...
export function buildGatewayTokenSecretRefFixHint(ref: SecretRef): string {
  if (ref.source === "exec") {
    return "Run `oriro doctor --allow-exec` to verify exec SecretRefs during doctor, or `oriro secrets audit --allow-exec` to audit all exec SecretRefs.";
  }
    ...
    return [
      {
        ...
        path: "gateway.auth",
        fixHint: "Run `oriro doctor --fix --generate-gateway-token` to generate a token.",
      },
    ];
  },
};
  ...
  async detect(ctx) {
    ...
    return detected.preview.map(
      (line): HealthFinding => ({
        ...
        fixHint: "Run `oriro doctor --fix` to migrate legacy state.",
      }),
    );
  },
};
...
function unavailableSkillToFinding(skill: SkillStatusEntry): HealthFinding {
  return {
    ...
    fixHint:
      "Install/configure the missing requirement, or run `oriro doctor --fix` to disable unused unavailable skills.",
  };
}
...
function browserResidueFinding(residue: LegacyOrirodBrowserProfileResidue): HealthFinding {
  return {
    checkId: BROWSER_ORIROD_PROFILE_RESIDUE_CHECK_ID,
    severity: "warning",
    message: `Legacy managed browser profile residue was found at ${residue.legacyProfileDir}.`,
    path: residue.legacyProfileDir,
    ocPath: "oc://state/browser/orirod",
    fixHint:
      "Run `oriro doctor --fix` to archive the stale orirod profile safely instead of deleting it in place.",
  };
}

function formatWouldArchiveBrowserResidue(residue: LegacyOrirodBrowserProfileResidue): string {
  return [
    "Would archive legacy orirod managed browser profile residue.",
    `- legacy profile: ${residue.legacyProfileDir}`,
    `- canonical profile: ${residue.canonicalUserDataDir}`,
  ].join("\n");
}

const browserOrirodProfileResidueCheck: HealthCheck = {
  ...
  async repair(ctx) {
    ...
    if (!residue) {
      return {
        status: "skipped",
        reason: "legacy orirod browser profile residue no longer exists",
        changes: [],
      };
    }
    const effect = {
    ...
    const result = await maybeArchiveLegacyOrirodBrowserProfileResidue(
      ctx.cfg,
...
const finalConfigValidationCheck: HealthCheck = {
  ...
  kind: "core",
  description: "Active oriro.jsonc parses and conforms to the config schema.",
  source: "doctor",
  async detect() {
