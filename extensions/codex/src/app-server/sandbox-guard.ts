/**
 * Blocks direct Codex app-server requests that would bypass Oriro sandbox or
 * node-exec routing guarantees.
 */
import type { OriroConfig } from "oriro/plugin-sdk/config-contracts";
import { resolveSandboxRuntimeStatus } from "oriro/plugin-sdk/sandbox";
import {
...
const DIRECT_METHOD_POLICIES = new Map<string, DirectMethodPolicy>([
  ...
  ["thread/start", "requires-oriro-environment"],
...
/** Returns a block message when a direct app-server method would bypass Oriro execution policy. */
export function resolveCodexAppServerDirectSandboxBypassBlock(params: {
  ...
  if (
    policy === "requires-oriro-environment" &&
    hasOriroSandboxEnvironmentSelection(params.requestParams)
  ) {
    return undefined;
  }
  return sandboxBlock;
...
function hasOriroSandboxEnvironmentSelection(value: unknown): boolean {
  ...
  return (
    ...
    environments.every((entry) => {
      ...
      return (
        ...
        environment.environmentId.startsWith("oriro-sandbox-") &&
