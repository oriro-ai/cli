...
function hasProviderWildcardModelAllowlist(cfg: OriroConfig) {
  const modelMaps = [
...
function resolveConfiguredReplaceModeCatalog(cfg: OriroConfig) {
...
function shouldLoadFullGatewayCatalogForReplaceMode(cfg: OriroConfig) {
...
async function loadEmbeddedTuiModelCatalog(cfg: OriroConfig) {
      ...
      const result = await agentCommandFromIngress(
        {
          ...
          // bytes on the wire. See: https://github.com/oriro-ai/cli/issues/3658
