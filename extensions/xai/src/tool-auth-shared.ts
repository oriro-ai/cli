...
function readLegacyGrokFallbackAuth(cfg?: OriroConfig): XaiFallbackAuth | undefined {
...
function readLegacyGrokApiKeyResult(cfg?: OriroConfig): ConfiguredRuntimeApiKeyResolution {
  const search = cfg?.tools?.web?.search;
  if (!search || typeof search !== "object") {
    return { status: "missing" };
  }
...
export function resolveFallbackXaiAuth(cfg?: OriroConfig): XaiFallbackAuth | undefined {
