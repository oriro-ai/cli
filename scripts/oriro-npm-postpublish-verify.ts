...
// Oriro Npm Postpublish Verify script supports Oriro repository automation.
...
export type OriroNpmPostpublishVerifyArgs =
  | {
      help: false;
      version: string;
    }
...
export function parseOriroNpmPostpublishVerifyArgs(
  argv: readonly string[],
): OriroNpmPostpublishVerifyArgs {
  ...
  if (extraArg) {
    throw new Error(`Unexpected oriro npm postpublish verifier argument: ${extraArg}`);
  }
...
export function buildPublishedInstallScenarios(version: string): PublishedInstallScenario[] {
  ...
  if (parsed.channel === "stable" && parsed.correctionNumber !== undefined) {
    scenarios.push({
      ...
      installSpecs: [`oriro@${parsed.baseVersion}`, exactSpec],
      expectedVersion: version,
    });
  }
...
function resolveNpmProvenanceVerificationPolicy(
  ...
  const isTrustedRef =
    ...
    (parsedVersion.channel === "alpha" &&
      /^refs\/heads\/tideoriro\/alpha\/[0-9]{4}-[0-9]{2}-[0-9]{2}-[0-9]{4}Z$/u.test(
        workflowRef ?? "",
      ));
...
export function collectInstalledPluginSdkDeclarationErrors(packageRoot: string): string[] {
  ...
  const errors: string[] = [];
  const forbiddenPrivateWorkspaceSpecifiers = ["@oriro/llm-core"];
...
export function resolveInstalledBinaryPath(prefixDir: string, platform = process.platform): string {
  return platform === "win32"
    ? pathWin32.join(prefixDir, "oriro.cmd")
    : pathPosix.join(prefixDir, "bin", "oriro");
}
...
async function verifyPublishedRegistryProvenanceOnce(version: string): Promise<void> {
  ...
  console.log(
    `oriro-npm-postpublish-verify: registry signature and provenance attestation verified (${version})`,
  );
}
...
function verifyScenario(version: string, scenario: PublishedInstallScenario): void {
  const workingDir = mkdtempSync(join(tmpdir(), `oriro-postpublish-${scenario.name}.`));
  ...
  try {
    ...
    const packageRoot = join(globalRoot, "oriro");
    ...
    if (normalizeInstalledBinaryVersion(installedBinaryVersion) !== scenario.expectedVersion) {
      errors.push(
        `installed oriro binary version mismatch: expected ${scenario.expectedVersion}, found ${installedBinaryVersion || "<missing>"}.`,
      );
    }
    ...
    console.log(`oriro-npm-postpublish-verify: ${scenario.name} OK (${version})`);
  } finally {
...
async function main(argv = process.argv.slice(2)): Promise<void> {
  ...
  console.log(
    `oriro-npm-postpublish-verify: verified published npm install paths for ${version}.`,
  );
}
...
if (entrypoint !== null && import.meta.url === entrypoint) {
  try {
    ...
    console.error(`oriro-npm-postpublish-verify: ${formatErrorMessage(error)}`);
    process.exitCode = 1;
  }
}
