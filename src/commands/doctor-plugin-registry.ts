...
function readPluginManifestId(packageDir: string): string | undefined {
  const manifest = readJsonObject(path.join(packageDir, "oriro.plugin.json"));
  ...
  if (!params.prompter.shouldRepair) {
    note(
      [
        ...
        `Repair with ${formatCliCommand("oriro doctor --fix")} to remove stale managed npm packages and rebuild the plugin registry.`,
      ].join("\n"),
      "Plugin registry",
    );
    return false;
  ...
  if (!params.prompter.shouldRepair) {
    note(
      [
        ...
        `Repair with ${formatCliCommand("oriro doctor --fix")} to remove stale local install records and rebuild the plugin registry.`,
      ].join("\n"),
      "Plugin registry",
    );
    return [];
...
/** Relinks managed npm plugin packages to the current Oriro host packages. */
export async function maybeRepairManagedNpmOriroPeerLinks(
  params: PluginRegistryDoctorRepairParams,
): Promise<boolean> {
  const npmRoots = listManagedPluginNpmRoots(params);
  if (!params.prompter.shouldRepair) {
    const audits = await Promise.all(
      npmRoots.map((npmRoot) => auditOriroPeerDependenciesInManagedNpmRoot({ npmRoot })),
    );
...
export async function maybeRepairPluginRegistryState(
  ...
  const repairedManagedNpmOriroPeerLinks = await maybeRepairManagedNpmOriroPeerLinks(params);
    ...
    if (preflight.action === "migrate") {
      note(
        [
          ...
          `Repair with ${formatCliCommand("oriro doctor --fix")} to rebuild ${shortenHomePath(preflight.filePath)} from enabled plugins.`,
        ].join("\n"),
        "Plugin registry",
      );
    }
  ...
  if (
    ...
    repairedManagedNpmOriroPeerLinks
  ) {
