...
function configWithDefaultEnvProvider(gateway: NonNullable<OriroConfig["gateway"]>) {
  return {
    gateway,
    secrets: {
      providers: {
...
function resolveSafeProbeAuth(cfg: OriroConfig, mode: "local" | "remote" = "local") {
...
function expectUnresolvedProbeTokenWarning(cfg: OriroConfig) {
  ...
  it("resolves env SecretRef token via async secret-inputs path", async () => {
    const result = await resolveGatewayProbeAuthSafeWithSecretInputs({
      cfg: configWithDefaultEnvProvider({
        auth: tokenAuthConfig("ORIRO_GATEWAY_TOKEN"),
      }),
      mode: "local",
      env: {
        ORIRO_GATEWAY_TOKEN: "test-token-from-env",
