...
describe("sessions", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-sessions-suite-"));
  });
  ...
  const withStateDir = <T>(stateDir: string, fn: () => T): T =>
    withEnv({ ORIRO_STATE_DIR: stateDir }, fn);
  ...
  it("builds discord display name with guild+channel slugs", () => {
    expect(
      buildGroupDisplayName({
        ...
        space: "friends-of-oriro",
        id: "123",
        key: "discord:group:123",
      }),
    ).toBe("discord:friends-of-oriro#general");
  });
  ...
  it("derives session transcripts dir from ORIRO_STATE_DIR", () => {
    const dir = resolveSessionTranscriptsDir(
      { ORIRO_STATE_DIR: "/custom/state" } as NodeJS.ProcessEnv,
      () => "/home/ignored",
    );
  ...
  it("resolves cross-agent paths when ORIRO_STATE_DIR differs from stored paths", () => {
