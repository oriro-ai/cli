...
import { runPluginCommandWithTimeout } from "oriro/plugin-sdk/run-command";
import type { RuntimeEnv } from "oriro/plugin-sdk/runtime-env";
import { CONFIG_DIR, extractArchive, resolveBrewExecutable } from "oriro/plugin-sdk/setup-tools";
...
export async function installSignalCliFromRelease(
  ...
  const tmpDir = await fs.mkdtemp(path.join(resolvePreferredOriroTmpDir(), "oriro-signal-"));
