...
extension AgentProTab {
            ...
            VStack(alignment: .leading, spacing: 12) {
                ...
                if let skillMutationErrorText {
                    Text(skillMutationErrorText)
                        .font(.caption2)
                        .foregroundStyle(OriroBrand.warn)
                }
            }
        }
    ...
    var oriroHubSearchCard: some View {
        ProCard(radius: AgentLayout.cardRadius) {
            VStack(alignment: .leading, spacing: 12) {
                HStack(spacing: 10) {
                    ProIconBadge(systemName: "square.and.arrow.down", color: OriroBrand.accent)
                    VStack(alignment: .leading, spacing: 2) {
                        Text("Install Skills")
                            .font(.headline)
                        Text("Search OriroHub and install into this workspace.")
                            .font(.caption)
                            .foregroundStyle(.secondary)
                    }
                    Spacer(minLength: 8)
                    Button {
                        Task { await self.searchOriroHubSkills() }
                    } label: {
                        Image(systemName: "magnifyingglass")
                    }
                    .buttonStyle(.bordered)
                    .controlSize(.small)
                    .disabled(self.oriroHubLoading || !self.liveGatewayConnected)
                    .accessibilityLabel("Search OriroHub")
                }

                TextField("Search OriroHub", text: self.$oriroHubQuery)
                    .textInputAutocapitalization(.never)
                    .autocorrectionDisabled()
                    .font(.subheadline)
                    .submitLabel(.search)
                ...
                if self.oriroHubLoading {
                    ProgressView()
                        .controlSize(.small)
                }
                if let oriroHubErrorText {
                    Text(oriroHubErrorText)
                        .font(.caption2)
                        .foregroundStyle(OriroBrand.warn)
                }
                if !self.oriroHubResults.isEmpty {
                    VStack(spacing: 0) {
                        let results = Array(self.oriroHubResults.prefix(8))
                        ForEach(Array(results.enumerated()), id: \.element.slug) { index, result in
                            self.oriroHubResultRow(result)
    ...
    func oriroHubResultRow(_ result: OriroHubSearchResultLite) -> some View {
        let installing = self.oriroHubInstallSlug == result.slug
        return HStack(alignment: .top, spacing: 10) {
            ProIconBadge(systemName: "sparkles", color: OriroBrand.accent)
            VStack(alignment: .leading, spacing: 3) {
            ...
            Spacer(minLength: 8)
            Button {
                Task { await self.installOriroHubSkill(result) }
            } label: {
        ...
        ProCard(radius: AgentLayout.cardRadius) {
            VStack(alignment: .leading, spacing: 12) {
                ...
                if let message = self.skillConfigMessages[skill.effectiveSkillKey] {
                    Text(message.text)
                        .font(.caption2)
                        .foregroundStyle(message.kind == .success ? OriroBrand.accent : OriroBrand.warn)
                }
            }
        }
        .padding(.horizontal, OriroProMetric.pagePadding)
    ...
    @MainActor
    func installOriroHubSkill(_ result: OriroHubSearchResultLite) async {
        guard self.liveGatewayConnected else { return }
        self.oriroHubInstallSlug = result.slug
        self.oriroHubErrorText = nil
        defer { self.oriroHubInstallSlug = nil }
        do {
            let params = OriroHubInstallParams(slug: result.slug)
            _ = try await self.requestGateway(method: "skills.install", params: params, timeoutSeconds: 125)
            await self.appModel.refreshGatewayOverviewIfConnected()
            await self.refreshOverview(force: true)
        } catch {
            self.oriroHubErrorText = Self.skillMutationMessage(error)
        }
    }

    @MainActor
    func searchOriroHubSkills() async {
        guard self.liveGatewayConnected else { return }
        self.oriroHubLoading = true
        self.oriroHubErrorText = nil
        defer { self.oriroHubLoading = false }
        do {
            let query = self.oriroHubQuery.trimmingCharacters(in: .whitespacesAndNewlines)
            let params = OriroHubSearchParams(query: query.isEmpty ? nil : query, limit: 20)
            let data = try await self.requestGateway(method: "skills.search", params: params, timeoutSeconds: 20)
            self.oriroHubResults = try JSONDecoder().decode(OriroHubSearchResponseLite.self, from: data).results
        } catch {
            self.oriroHubErrorText = Self.skillMutationMessage(error)
        }
    }
    ...
    func skillStatus(_ skill: SkillStatusEntryLite) -> (text: String, color: Color) {
        ...
        if skill.hasMissingRequirements {
            return ("setup", OriroBrand.warn)
        }
        return ("enabled", OriroBrand.accent)
    }
}
