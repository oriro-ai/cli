...
type SearchProvider = NonNullable<
  NonNullable<NonNullable<NonNullable<OriroConfig["tools"]>["web"]>["search"]>["provider"]
>;
type SearchConfig = NonNullable<NonNullable<NonNullable<OriroConfig["tools"]>["web"]>["search"]>;
...
const WEB_SEARCH_DOCS_URL = "https://docs.oriro.ai/tools/web";
...
function rawKeyValue(config: OriroConfig, provider: SearchProvider): unknown {
...
export function hasExistingKey(config: OriroConfig, provider: SearchProvider): boolean {
  return hasConfiguredSecretInput(rawKeyValue(config, provider));
}

function buildSearchEnvRef(config: OriroConfig, provider: SearchProvider): SecretRef {
  const entry =
...
function preserveDisabledState(original: OriroConfig, result: OriroConfig): OriroConfig {
  if (original.tools?.web?.search?.enabled !== false) {
    return result;
  }

  const next: OriroConfig = {
    ...result,
    tools: {
      ...result.tools,
  ...
  return {
    ...next,
    plugins: nextPlugins as OriroConfig["plugins"],
  };
}
...
export async function runSearchSetupFlow(
  ...
  if (!needsCredential) {
    await prompter.note(
      [
        ...
        "Oriro will enable the plugin and use it as your web_search provider.",
        `Docs: ${entry.docsUrl ?? "https://docs.oriro.ai/tools/web"}`,
      ].join("\n"),
      "Web search",
    );
    return await finalizeSearchProviderSetup({
  ...
  if (useSecretRefMode) {
    ...
    await prompter.note(
      [
        "Secret references enabled — Oriro will store a reference instead of the API key.",
        `Env var: ${ref.id}${envAvailable ? " (detected)" : ""}.`,
        ...(envAvailable ? [] : [`Set ${ref.id} in the Gateway environment.`]),
        "Docs: https://docs.oriro.ai/tools/web",
      ].join("\n"),
      "Web search",
    );
    return await finalizeSearchProviderSetup({
