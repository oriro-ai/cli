...
const WORKSPACE_PACKAGE_NAMES = [
  "@oriro/gateway-protocol",
  "@oriro/gateway-client",
  "@oriro/sdk",
] as const;
...
function normalizeWorkspaceDependencies(
  ...
  for (const [name, spec] of Object.entries(dependencies)) {
    normalized[name] =
      name.startsWith("@oriro/") && spec === "workspace:*" ? "0.0.0-private" : spec;
  }
  return normalized;
}
...
async function startOriroRegistry(packages: PackedPackage[]): Promise<{
  registryUrl: string;
  close: () => Promise<void>;
}> {
...
describe("Oriro SDK package e2e", () => {
  afterEach(async () => {
    await Promise.all(
  ...
  it("packs and imports from an external temp consumer", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-sdk-consumer-"));
    ...
    const sdkTarball =
      packedPackages.find((pkg) => pkg.manifest.name === "@oriro/sdk")?.tarball ?? "";
    expect(sdkTarball).not.toBe("");
    const registry = await startOriroRegistry(packedPackages);

    await fs.writeFile(
      path.join(tempDir, "package.json"),
      JSON.stringify({ private: true, type: "module" }),
    );
    await fs.writeFile(path.join(tempDir, ".npmrc"), `@oriro:registry=${registry.registryUrl}`);
    try {
      await runCommand(
        "npm",
    ...
    const importScript = `
      import { GatewayClientTransport, Oriro, normalizeGatewayEvent } from "@oriro/sdk";
      if (typeof GatewayClientTransport !== "function") throw new Error("missing transport export");
      if (typeof Oriro !== "function") throw new Error("missing client export");
