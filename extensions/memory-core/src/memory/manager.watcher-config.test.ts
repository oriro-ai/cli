...
import type {
  MemorySearchConfig,
  OriroConfig,
} from "oriro/plugin-sdk/memory-core-host-engine-foundation";
...
const {
  ...
  const chokidarKey = Symbol.for("oriro.test.memoryWatchFactory");
  const nativeKey = Symbol.for("oriro.test.memoryNativeWatchFactory");
...
const CHOKIDAR_FACTORY_KEY = Symbol.for("oriro.test.memoryWatchFactory");
const NATIVE_FACTORY_KEY = Symbol.for("oriro.test.memoryNativeWatchFactory");

vi.mock("oriro/plugin-sdk/memory-core-host-engine-foundation", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("oriro/plugin-sdk/memory-core-host-engine-foundation")>();
  return {
    ...actual,
...
describe("memory watcher config", () => {
  ...
  async function setupWatcherWorkspace(seedFile: { name: string; contents: string }) {
    workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-memory-watch-"));
    vi.stubEnv("ORIRO_STATE_DIR", path.join(workspaceDir, "state"));
  ...
  function createWatcherConfig(overrides?: Partial<MemorySearchConfig>): OriroConfig {
    const defaults: NonNullable<NonNullable<OriroConfig["agents"]>["defaults"]> = {
  ...
  async function expectWatcherManager(cfg: OriroConfig) {
  ...
  it("routes Linux directories through directory-only native watchers", async () => {
    ...
    // internal/fs/recursive_watch. Oriro watches directories only so
