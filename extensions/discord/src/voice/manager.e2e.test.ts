...
import type { RealtimeVoiceAgentControlResult } from "oriro/plugin-sdk/realtime-voice";
...
vi.mock("oriro/plugin-sdk/routing", async () => {
  const actual = await vi.importActual<typeof import("oriro/plugin-sdk/routing")>(
    "oriro/plugin-sdk/routing",
  );
  return {
    ...actual,
...
vi.mock("oriro/plugin-sdk/agent-runtime", async () => {
  const actual = await vi.importActual<typeof import("oriro/plugin-sdk/agent-runtime")>(
    "oriro/plugin-sdk/agent-runtime",
  );
  return {
    ...actual,
    agentCommandFromIngress: agentCommandMock,
    getTtsProvider: vi.fn(() => "openai"),
    resolveAgentDir: vi.fn(() => "/tmp/oriro-agent"),
    resolveTtsConfig: vi.fn(() => ({
      modelOverrides: {},
      providerConfigs: {},
    })),
    resolveTtsPrefsPath: vi.fn(() => "/tmp/oriro-tts.json"),
  };
});

vi.mock("oriro/plugin-sdk/realtime-bootstrap-context", async () => {
  const actual = await vi.importActual<
    typeof import("oriro/plugin-sdk/realtime-bootstrap-context")
  >("oriro/plugin-sdk/realtime-bootstrap-context");
  return {
    ...actual,
...
vi.mock("oriro/plugin-sdk/runtime-env", async () => {
  const actual = await vi.importActual<typeof import("oriro/plugin-sdk/runtime-env")>(
    "oriro/plugin-sdk/runtime-env",
  );
  return {
    ...actual,
...
vi.mock("oriro/plugin-sdk/realtime-voice", async () => {
  const actual = await vi.importActual<typeof import("oriro/plugin-sdk/realtime-voice")>(
    "oriro/plugin-sdk/realtime-voice",
  );
  return {
    ...actual,
...
describe("DiscordVoiceManager", () => {
  ...
  it("uses agent-proxy realtime voice by default", async () => {
    ...
    expect(bridgeParams?.instructions).toContain("same Oriro agent");
    expect(bridgeParams?.instructions).toContain("short natural backchannel");
    expect(bridgeParams?.tools?.map((tool) => tool.name)).toContain("oriro_agent_consult");
    expect(bridgeParams?.tools?.map((tool) => tool.name)).toContain("oriro_agent_control");
    const player = getLastAudioPlayer();
    bridgeParams?.audioSink?.sendAudio(Buffer.alloc(24_000));
  ...
  it("routes active-run realtime transcripts to voice control before forced consults", async () => {
    ...
    await vi.waitFor(() => expectUserMessageIncludes("Cancelled the active Oriro run."));
    expect(textToSpeechMock).not.toHaveBeenCalledWith(
      expect.objectContaining({ text: "Cancelled the active Oriro run." }),
    );

    const stopCallsAfterControl = player.stop.mock.calls.length;
    bridgeParams?.onTranscript?.("assistant", "Cancelled the active Oriro run.", true);
  ...
  it("accepts Oriro as a default wake name before realtime agent-proxy consults", async () => {
    agentCommandMock.mockResolvedValueOnce({ payloads: [{ text: "oriro wake answer" }] });
    const manager = createManager(
      {
        groupPolicy: "open",
        voice: {
    ...
    ownerTurn?.sendInputAudio(Buffer.alloc(8));
    await emitFinalRealtimeUserTranscript(bridgeParams, "Oriro, how is it going");
    ...
    expect(lastAgentCommandArgs().message).toContain("how is it going");
    expect(lastAgentCommandArgs().message).not.toContain("Oriro");
    expectUserMessageIncludes("oriro wake answer");
  });
  ...
  it("ignores default agent wake names longer than two words", async () => {
    ...
    const manager = createManager(
      ...
      {
        agents: {
          list: [{ id: "agent-1", identity: { name: "Oriro Bot Helper" } }],
        },
      },
    );
    ...
    await emitFinalRealtimeUserTranscript(bridgeParams, "Oriro Bot Helper, should not wake");
    ...
    await emitFinalRealtimeUserTranscript(bridgeParams, "Oriro, fallback still wakes");
  ...
  it("accepts leading fuzzy wake names before realtime agent-proxy consults", async () => {
    ...
    expect(agentCommandArgsAt(4).message).not.toContain("Open oriro");
  ...
  it("uses configured wake names before realtime agent-proxy consults", async () => {
    ...
    const manager = createManager({
      ...
      voice: {
        ...
        realtime: {
          ...
          wakeNames: ["Oriro", "Oriro Bot", "Okay Google"],
        },
      },
    });
    ...
    await emitFinalRealtimeUserTranscript(bridgeParams, "Oriro Bot, ship it");
  ...
  it("does not accept configured realtime wake names longer than two words", async () => {
    ...
    await emitFinalRealtimeUserTranscript(bridgeParams, "Oriro Bot Helper, ship it");
    ...
    fallbackTurn?.sendInputAudio(Buffer.alloc(8));
    await emitFinalRealtimeUserTranscript(bridgeParams, "Oriro, ship it");
  ...
  it("keeps forced agent-proxy fallback diagnostics out of agent prompts", async () => {
    ...
    expect(lastAgentCommandArgs().message).not.toContain("oriro_agent_consult");
  ...
  it("starts Discord realtime voice in bidi mode with the consult tool", async () => {
    ...
    expect(bridgeParams?.autoRespondToAudio).toBe(true);
    expect(bridgeParams?.interruptResponseOnInputAudio).toBe(false);
    expect(bridgeParams?.instructions).toContain("Call oriro_agent_consult");
    expect(bridgeParams?.tools?.map((tool) => tool.name)).toContain("oriro_agent_consult");
  ...
  it("adds default bootstrap profile context to realtime voice instructions", async () => {
    ...
    resolveRealtimeBootstrapContextInstructionsMock.mockResolvedValue(
      "Oriro realtime voice profile context:\n\n### IDENTITY.md\nName: Wilfred",
    );
    const manager = createManager({
      groupPolicy: "open",
      voice: {
    ...
    const bridgeParams = lastRealtimeBridgeParams() as
      ...
      | undefined;
    expect(bridgeParams?.instructions).toContain("Oriro realtime voice profile context");
