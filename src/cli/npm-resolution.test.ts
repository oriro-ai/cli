...
const CLI_STATE_ROOT = "/tmp/oriro";
...
describe("npm-resolution helpers", () => {
  it("keeps the requested selector when pin is disabled", () => {
    const result = resolvePinnedNpmSpec({
      rawSpec: "@oriro/plugin-alpha@latest",
      pin: false,
      resolvedSpec: "@oriro/plugin-alpha@1.2.3",
    });
    expect(result).toEqual({
      recordSpec: "@oriro/plugin-alpha@latest",
    });
  });
  ...
  it("returns pinned spec notice when resolved spec is available", () => {
    ...
    expect(result).toEqual({
      recordSpec: "@oriro/plugin-alpha@1.2.3",
      pinNotice: "Pinned npm install record to @oriro/plugin-alpha@1.2.3.",
    });
  });
  ...
  it("builds common npm install record fields", () => {
    expect(
      buildNpmInstallRecordFields({
        spec: "@oriro/plugin-alpha@latest",
        installPath: ALPHA_INSTALL_PATH,
        version: "1.2.3",
        resolution: {
          name: "@oriro/plugin-alpha",
          version: "1.2.3",
          resolvedSpec: "@oriro/plugin-alpha@1.2.3",
      ...
      installPath: ALPHA_INSTALL_PATH,
      version: "1.2.3",
      resolvedName: "@oriro/plugin-alpha",
  ...
  it("resolves pinned install record and emits pin notice", () => {
    ...
    expect(logs).toEqual(["Pinned npm install record to @oriro/plugin-alpha@1.2.3."]);
  ...
  it("keeps install record selector for CLI unless --pin is requested", () => {
    ...
    expect(record.spec).toBe("@oriro/plugin-alpha");
    expect(logs).toEqual([]);
  });
});
