...
describe("web logout", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsPromises.mkdtemp(path.join(os.tmpdir(), "oriro-test-web-logout-"));
    previousOAuthDir = process.env.ORIRO_OAUTH_DIR;
    process.env.ORIRO_OAUTH_DIR = path.join(fixtureRoot, "oauth");
  ...
  it("does not delete through symlinked auth dirs inside the Oriro auth root", async () => {
  ...
  it("does not delete through intermediate symlinks inside the Oriro auth root", async () => {
