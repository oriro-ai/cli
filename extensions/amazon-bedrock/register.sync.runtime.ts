...
import { registerApiProvider, streamSimple } from "oriro/plugin-sdk/llm";
import { resolvePluginConfigObject } from "oriro/plugin-sdk/plugin-config-runtime";
import type {
  OriroPluginApi,
...
} from "oriro/plugin-sdk/provider-model-shared";
import { streamWithPayloadPatch } from "oriro/plugin-sdk/provider-stream-shared";
 ...
 * IDs that may not contain the model name). When Oriro's `isAnthropicBedrockModel`
...
function needsCachePointInjection(modelId: string): boolean {
  ...
  // Check if Oriro identifies this as an Anthropic model via the ARN heuristic.
 ...
 * same matcher Oriro uses for direct model IDs.
 */
 ...
 * the Oriro config region differs from the profile's home region.
 */
...
export function registerAmazonBedrockPlugin(api: OriroPluginApi): void {
