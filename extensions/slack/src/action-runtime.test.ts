...
describe("handleSlackAction", () => {
  function slackConfig(overrides?: Record<string, unknown>): OriroConfig {
    return {
      channels: {
        slack: {
          botToken: "tok",
  ...
  function createReplyToFirstScenario() {
    const cfg = { channels: { slack: { botToken: "tok" } } } as OriroConfig;
  ...
  function expectLastSlackSend(content: string, cfg: OriroConfig, threadTs?: string) {
  ...
  async function resolveReadToken(cfg: OriroConfig): Promise<string | undefined> {
  ...
  async function resolveSendToken(cfg: OriroConfig): Promise<string | undefined> {
    sendSlackMessage.mockClear();
  ...
  it("returns non-image downloadFile results as file metadata instead of image content", async () => {
    downloadSlackFile.mockResolvedValueOnce({
      path: "/tmp/oriro-media/report.pdf",
    ...
    expect(String(firstContent.text)).toContain("/tmp/oriro-media/report.pdf");
    ...
    expect(details.media).toEqual({
      mediaUrl: "/tmp/oriro-media/report.pdf",
      outbound: false,
      contentType: "application/pdf",
    });
  });
