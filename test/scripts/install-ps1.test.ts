...
describe("install.ps1 failure handling", () => {
  ...
  it("keeps failure termination in the top-level completion handler", () => {
    ...
    expect(completeInstallBody).toMatch(/\bthrow "Oriro installation failed with exit code/);
  ...
  it("runs Windows command shims from a Windows-local cwd", () => {
    ...
    expect(mainBody).toContain('Invoke-NpmCommand -Arguments @("uninstall", "-g", "oriro")');
  ...
  it("preserves caller-relative local tarball install specs before safe-cwd npm calls", () => {
    const resolveSpecBody = extractFunctionBody(source, "Resolve-NpmOriroInstallSpec");
  ...
  it("falls back to a user-local portable Node.js bootstrap when package managers are absent", () => {
    ...
    const depsRootBody = extractFunctionBody(source, "Get-OriroDepsRoot");
    ...
    expect(depsRootBody).toContain("Oriro\\deps");
  ...
  it("persists user-local portable Git for future git-backed updates", () => {
    ...
    expect(portableGitRootBody).toContain("Get-OriroDepsRoot");
  ...
  runIfPowerShell("selects native ARM64 MinGit when the release publishes it", () => {
    const tempDir = harness.createTempDir("oriro-install-ps1-");
  ...
  it("activates the repo-pinned pnpm version for git installs", () => {
    ...
    const ensurePnpmBody = extractFunctionBody(source, "Ensure-Pnpm");
    const gitInstallBody = extractFunctionBody(source, "Install-OriroFromGit");
    ...
    expect(mainBody).toContain("$gitInstallResults = @(Install-OriroFromGit");
    expect(mainBody).toContain("Test-BooleanSuccessResult -Results $gitInstallResults");
    expect(mainBody).toContain("$npmInstallResults = @(Install-Oriro)");
    ...
    expect(gitInstallBody).toContain('Write-Host "[!] Oriro build did not produce $entryPath"');
  ...
  it("launches interactive onboarding outside Main's captured output", () => {
    const interactiveCommandBody = extractFunctionBody(source, "Invoke-InteractiveOriroCommand");
    const mainBody = extractFunctionBody(source, "Main");
    ...
    expect(mainBody).toContain("Invoke-InteractiveOriroCommand onboard");
  });
  ...
  runIfPowerShell("fails install when interactive onboarding exits non-zero", () => {
    ...
    writeFileSync(
      ...
      [
        ...
        "function Check-ExistingOriro { return $false }",
        "function Get-NpmCommandPath { return 'npm.cmd' }",
        "function Install-Oriro { return $true }",
        "function Ensure-OriroOnPath { return $true }",
        "function Add-ToUserPath { param([string]$Path) }",
        "function Get-OriroCommandPath { return 'cmd.exe' }",
    ...
    expect(`${result.stdout}\n${result.stderr}`).toContain(
      "oriro onboard failed with exit code 17",
    );
  });
  ...
  runIfPowerShell("throws without killing the caller when run as a scriptblock", () => {
    ...
    expect(result.status).toBe(0);
    expect(result.stdout).toContain("caught=Oriro installation failed with exit code 1.");
  ...
  runIfPowerShell("treats noisy Git install false as failure", () => {
    ...
    writeFileSync(
      ...
      [
        ...
        "  return $false",
        "}",
        "function Ensure-OriroOnPath { throw 'should not continue after failed git install' }",
        "$InstallMethod = 'git'",
        "$GitDir = 'C:\\\\oriro-test'",
        "$NoOnboard = $true",
        "$result = Main",
  ...
  runIfPowerShell("keeps npm chatter out of Main's success return value", () => {
    ...
    writeFileSync(
      ...
      [
        ...
        "function Install-Oriro { Write-Output 'npm stdout'; return $true }",
        "function Ensure-OriroOnPath { return $true }",
        "function Refresh-GatewayServiceIfLoaded { }",
        "function Invoke-OriroCommand { return 'Oriro test-version' }",
        "$NoOnboard = $true",
        "$result = Main",
