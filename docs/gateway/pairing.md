...
oriro nodes rename --node <id|name|ip> --name "Living Room iPad"
```
...
changes (for example, display name or client platform hints), Oriro treats
...
Pairing state is stored under the Gateway state directory (default `~/.oriro`):

- `~/.oriro/nodes/paired.json`
- `~/.oriro/nodes/pending.json`

If you override `ORIRO_STATE_DIR`, the `nodes/` folder moves with it.
