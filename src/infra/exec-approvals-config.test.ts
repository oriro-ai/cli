...
describe("exec approvals wildcard agent", () => {
  it("merges wildcard allowlist entries with agent entries", () => {
    ...
    const prevOriroHome = process.env.ORIRO_HOME;

    try {
      process.env.ORIRO_HOME = dir;
      const approvalsPath = path.join(dir, ".oriro", "exec-approvals.json");
      ...
      if (prevOriroHome === undefined) {
        delete process.env.ORIRO_HOME;
      } else {
        process.env.ORIRO_HOME = prevOriroHome;
      }
    }
  });
});
