...
import { unlinkIfExists } from "oriro/plugin-sdk/media-runtime";
import { parseStrictFiniteNumber } from "oriro/plugin-sdk/number-runtime";
import { readResponseTextLimited } from "oriro/plugin-sdk/provider-http";
...
async function generateWaveformFromPcm(filePath: string): Promise<string> {
  const tempDir = resolvePreferredOriroTmpDir();
