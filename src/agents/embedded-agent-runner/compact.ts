...
import { createOriroCodingTools, resolveProcessToolScopeKey } from "../agent-tools.js";
...
async function compactEmbeddedAgentSessionDirectOnce(
  ...
  try {
    ...
    const toolsRaw = createOriroCodingTools({
      exec: {
    ...
    const filteredBundledTools = applyFinalEffectiveToolPolicy({
      ...
      // createOriroCodingTools(...) also omit it, so both paths resolve
    ...
    try {
      ...
      // exact Oriro-managed registrations so custom tools survive startup.
