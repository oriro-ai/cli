...
export function createAccountListHelpers(
  ...
  options?: {
    ...
    hasImplicitDefaultAccount?: (cfg: OriroConfig) => boolean;
  },
) {
  function hasImplicitDefaultAccount(cfg: OriroConfig): boolean {
  ...
  function resolveConfiguredDefaultAccountId(cfg: OriroConfig): string | undefined {
    const channel = cfg.channels?.[channelKey] as Record<string, unknown> | undefined;
  ...
  function listAccountIds(cfg: OriroConfig): string[] {
  ...
  function resolveDefaultAccountId(cfg: OriroConfig): string {
