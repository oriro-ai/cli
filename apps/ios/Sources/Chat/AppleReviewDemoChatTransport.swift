...
enum ScreenshotFixtureMode {
    static let gatewayName = "Oriro Gateway"
...
struct LocalChatFixture {
    ...
    static let appScreenshots = LocalChatFixture(
        ...
        modelID: "gpt-5.5",
        modelName: "GPT-5.5",
        responsePrefix: "Oriro is connected to your gateway.",
        seedMessages: [
            """
            Oriro is connected to your gateway. I can coordinate agents, inspect project context, and prepare \
...
struct LocalFixtureChatTransport: OriroChatTransport {
...
struct AppleReviewDemoChatTransport: OriroChatTransport {
...
private actor LocalFixtureChatStore {
    private let fixture: LocalChatFixture
    private var messages: [OriroChatMessage]
    ...
    func createSession(key: String) throws -> OriroChatCreateSessionResponse {
        try Self.decode(
            CreateSessionPayload(ok: true, key: key, sessionId: "\(self.fixture.sessionIDPrefix)-\(key)"),
            as: OriroChatCreateSessionResponse.self)
    }

    func history(sessionKey: String) throws -> OriroChatHistoryPayload {
        ...
        return try Self.decode(
            HistoryPayload(
                ...
                thinkingLevel: "auto"),
            as: OriroChatHistoryPayload.self)
    }

    func sendMessage(sessionKey _: String, message: String, runId: String) throws -> OriroChatSendResponse {
        ...
        return try Self.decode(
            ...
            as: OriroChatSendResponse.self)
    }

    func sessions() throws -> OriroChatSessionsListResponse {
        let entry = OriroChatSessionEntry(
            ...
            thinkingDefault: "auto")
        return OriroChatSessionsListResponse(
    ...
    private static var thinkingLevels: [OriroChatThinkingLevelOption] {
        [
            OriroChatThinkingLevelOption(id: "auto", label: "Auto"),
    ...
    private static func seedMessages(fixture: LocalChatFixture) -> [OriroChatMessage] {
        let now = Date().timeIntervalSince1970 * 1000
    ...
    private static func message(role: String, text: String, timestamp: Double) -> OriroChatMessage {
        OriroChatMessage(
            role: role,
            content: [
