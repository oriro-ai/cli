...
Oriro's Gateway can serve an OpenResponses-compatible `POST /v1/responses` endpoint.
...
`oriro agent`), so routing/permissions/config match your Gateway.
...
- use the matching Gateway HTTP auth path:
  ...
  - trusted-proxy local direct fallback: same-host callers with no `Forwarded`, `X-Forwarded-*`, or `X-Real-IP` headers can use `gateway.auth.password` / `ORIRO_GATEWAY_PASSWORD`
  - private-ingress open auth (`gateway.auth.mode="none"`): no auth header
- treat the endpoint as full operator access for the gateway instance
- for shared-secret auth modes (`token` and `password`), ignore narrower bearer-declared `x-oriro-scopes` values and restore the normal full operator defaults
- for trusted identity-bearing HTTP modes (for example trusted proxy auth or `gateway.auth.mode="none"`), honor `x-oriro-scopes` when present and otherwise fall back to the normal operator default scope set
- select agents with `model: "oriro"`, `model: "oriro/default"`, `model: "oriro/<agentId>"`, or `x-oriro-agent-id`
- use `x-oriro-model` when you want to override the selected agent's backend model
- use `x-oriro-session-key` for explicit session routing
- use `x-oriro-message-channel` when you want a non-default synthetic ingress channel context
...
For the canonical explanation of how agent-target models, `oriro/default`, embeddings pass-through, and backend model overrides fit together, see [OpenAI Chat Completions](/gateway/openai-http-api#agent-first-model-contract) and [Model list and agent routing](/gateway/openai-http-api#model-list-and-agent-routing).
...
- `previous_response_id`: Oriro reuses the earlier response session when the request stays within the same agent/user/requested-session scope.
...
For `tool_choice: "required"` and function-pinned `tool_choice`, the endpoint narrows the exposed client function-tool set, instructs the runtime to call a client tool before responding, and rejects the turn if it does not include a matching structured client-tool call. This contract applies to the caller-supplied HTTP `tools` list, not every internal Oriro agent tool. Non-streaming requests return `502` with an `api_error`; streaming requests emit a `response.failed` event. This matches the `/v1/chat/completions` contract.
...
`oriropdf` and its packaged PDFium WebAssembly runtime for text extraction and
page rendering.
...
Oriro normalizes common OpenAI-style aliases before those counters reach
