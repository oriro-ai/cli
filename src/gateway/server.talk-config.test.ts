...
async function createFreshOperatorDevice(scopes: string[], nonce: string) {
  const identity = loadOrCreateDeviceIdentity(
    path.join(os.tmpdir(), `oriro-talk-config-device-${process.pid}-${talkConfigDeviceSeq++}`),
  );
        ...
        async () => {
          await withTalkConfigConnection(["operator.read"], async (ws) => {
            ...
            expect(redactedApiKey).toEqual({
              id: "__ORIRO_REDACTED__",
              provider: "__ORIRO_REDACTED__",
              source: "__ORIRO_REDACTED__",
            });
