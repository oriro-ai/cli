...
const LIVE = isLiveTestEnabled(["ORIRO_LIVE_CLI_MCP_GEMINI"]);
...
async function startLocalStreamableHttpMcpServer(): Promise<{
  ...
  const mcpServer = new McpServer({ name: "oriro-gemini-live-probe", version: "1.0.0" });
  mcpServer.tool("oriro_live_probe", "Oriro Gemini MCP live probe", async () => ({
...
describeLive("Gemini CLI MCP settings smoke", () => {
  it("connects to an Oriro-configured streamable-http server", async () => {
    const geminiCommand = process.env.ORIRO_LIVE_GEMINI_COMMAND ?? "gemini";
    ...
    try {
      ...
      expect(output).toContain("oriroLiveProbe");
