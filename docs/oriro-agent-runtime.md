---
summary: "Developer workflow for Oriro agent runtime: build, test, and live validation"
title: "Oriro agent runtime workflow"
read_when:
  - Working on Oriro agent runtime code or tests
  - Running agent-runtime lint, typecheck, and live test flows
---

A sane workflow for working on the Oriro agent runtime in Oriro.
...
```bash
ORIRO_LIVE_TEST=1 pnpm test src/agents/embedded-agent-runner-extraparams.live.test.ts
```
...
- Trigger the agent directly:
  - `pnpm oriro agent --message "Hello" --thinking low`
...
State lives under the Oriro state directory. Default is `~/.oriro`. If `ORIRO_STATE_DIR` is set, use that directory instead.

To reset everything:

- `oriro.json` for config
...
- [Oriro agent runtime architecture](/agent-runtime-architecture)
