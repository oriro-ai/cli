...
/** Resolve the underlying OS home before applying Oriro overrides. */
...
/** Resolve raw home with ORIRO_HOME tilde expansion. */
function resolveRawHomeDir(
  env: NodeJS.ProcessEnv = process.env,
  homedir: () => string = os.homedir,
): string | undefined {
  const explicitHome = normalize(env.ORIRO_HOME);
  if (explicitHome) {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  return explicitHome ? { home, prefix: "$ORIRO_HOME" } : { home, prefix: "~" };
}

/** Replace the effective home path with "~" or "$ORIRO_HOME" for terminal display. */
