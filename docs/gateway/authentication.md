...
Oriro supports OAuth and API keys for model providers. For always-on gateway
...
setup, but Oriro also supports reusing a local Claude CLI login.

1. Create an API key in your provider console.
2. Put it on the **gateway host** (the machine running `oriro gateway`).
...
3. If the Gateway runs under systemd/launchd, prefer putting the key in
   `~/.oriro/.env` so the daemon can read it:
...
API keys for daemon use: `oriro onboard`.

See [Help](/help) for details on env inheritance (`env.shellEnv`,
`~/.oriro/.env`, systemd/launchd).

## Anthropic: Claude CLI and token compatibility

Anthropic setup-token auth is still available in Oriro as a supported token
path. Anthropic staff has since told us that Oriro-style Claude CLI usage is
allowed again, so Oriro treats Claude CLI reuse and `claude -p` usage as
...
2. Tell Oriro to switch Anthropic model selection to the local `claude-cli`
   backend and store the matching Oriro auth profile.
...
```bash
oriro models auth paste-token --provider openrouter
```
...
- If auth exists but Oriro cannot resolve a probeable model candidate for
...
- Anthropic staff told us this Oriro integration path is allowed again.
- Oriro therefore treats Claude CLI reuse and `claude -p` usage as sanctioned
...
- Priority order:
  - `ORIRO_LIVE_<PROVIDER>_KEY` (single override)
  - `<PROVIDER>_API_KEYS`
  - `<PROVIDER>_API_KEY`
  - `<PROVIDER>_API_KEY_*`
- Google providers also include `GOOGLE_API_KEY` as an additional fallback.
- The same key list is deduplicated before use.
- Oriro retries with the next key only for rate-limit errors (for example
...
When provider auth is removed through the Gateway control plane, Oriro deletes
...
Use `oriro models auth login --provider <id> --profile-id <profileId>` for
...
```bash
oriro models auth login --provider anthropic --force
```
...
When you debug order issues, `oriro models status --probe` shows omitted
...
Run `oriro models status` to confirm which profile is expiring. If an
