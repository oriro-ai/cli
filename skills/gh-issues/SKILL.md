...
if [ -z "${GH_TOKEN:-}" ]; then
  CONFIG_PATH="${ORIRO_CONFIG_PATH:-${ORIRO_STATE_DIR:-$HOME/.oriro}/oriro.json}"
...
```text
${ORIRO_STATE_DIR:-$HOME/.oriro}/gh-issues-<owner>-<repo>.json
```
...
Send completion/failure with oriro message send if route provided.
```
