...
function createProps(overrides: Partial<SkillsProps> = {}): SkillsProps {
  ...
  return {
    ...
    skillCardErrors: {},
    orirohubQuery: "",
    orirohubResults: null,
    orirohubSearchLoading: false,
    orirohubSearchError: null,
    orirohubDetail: null,
    ...
    onOriroHubQueryChange: () => undefined,
    onOriroHubDetailOpen: () => undefined,
    onOriroHubDetailClose: () => undefined,
    onOriroHubInstall: () => undefined,
    ...overrides,
  };
}
...
describe("renderSkills", () => {
  ...
  it("opens detail dialogs and routes OriroHub actions", async () => {
    const container = document.createElement("div");
    document.body.append(container);
    dialogRestores.push(() => container.remove());
    ...
    const onOriroHubDetailOpen = vi.fn();
    const onOriroHubInstall = vi.fn();
          ...
          orirohubResults: [
            {
              ...
              slug: "github",
              displayName: "GitHub",
              summary: "GitHub integration for Oriro",
              version: "1.2.3",
            },
          ],
          onOriroHubDetailOpen,
          onOriroHubInstall,
        }),
      ),
      container,
    ...
    expect(onOriroHubDetailOpen).toHaveBeenCalledTimes(1);
    expect(onOriroHubDetailOpen).toHaveBeenCalledWith("github");
    expect(onOriroHubInstall).toHaveBeenCalledTimes(1);
    expect(onOriroHubInstall).toHaveBeenCalledWith("github");

    onOriroHubInstall.mockClear();
    showModal.mockClear();

    render(
      renderSkills(
        createProps({
          orirohubSearchError: "rate limited",
          orirohubInstallMessage: { kind: "success", text: "Installed github" },
          orirohubDetailSlug: "github",
          orirohubDetail: {
            skill: {
              slug: "github",
              displayName: "GitHub",
    ...
    expect(normalizeText(container.querySelector(".md-preview-dialog__body")!)).toBe(
      "GitHub integration for Oriro By Oriro (@oriro) Latest: v1.2.3 Added search support Platforms: macos, linux Install GitHub",
    );
  ...
  it("renders installed OriroHub verdicts and the local Skill Card tab", async () => {
    const container = document.createElement("div");
    document.body.append(container);
    dialogRestores.push(() => container.remove());
    installDialogMethod("showModal", function (this: HTMLDialogElement) {
    ...
    const verdictKey = "https://orirohub.ai\u0000agentreceipt\u00001.2.3";
        ...
        createProps({
          ...
          orirohubVerdicts: {
            [verdictKey]: {
              ...
              version: "1.2.3",
              securityAuditUrl:
                "https://orirohub.ai/oriro/agentreceipt/security-audit?version=1.2.3",
  ...
  it("fails closed for inconsistent OriroHub verdict envelopes", async () => {
    const container = document.createElement("div");
    document.body.append(container);
    dialogRestores.push(() => container.remove());
    installDialogMethod("showModal", function (this: HTMLDialogElement) {
