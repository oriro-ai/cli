...
describe("browser client", () => {
  ...
  it("uses the expected endpoints + methods for common calls", async () => {
    ...
    expect(doctorResult.profile).toBe("oriro");
    ...
    expect(deepDoctorResult.profile).toBe("oriro");
    ...
    expect(urls.some((url) => url.endsWith("/doctor?profile=oriro&deep=true"))).toBe(true);
    ...
    const deepDoctor = calls.find((c) => c.url.endsWith("/doctor?profile=oriro&deep=true"));
