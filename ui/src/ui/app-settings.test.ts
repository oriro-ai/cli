...
const createHost = (tab: Tab): SettingsHost => ({
  ...
  theme: "oriro" as unknown as ThemeName & ThemeMode,
  themeMode: "system",
  themeResolved: "dark",
...
describe("setTabFromRoute", () => {
  ...
  it("falls back to oriro when custom is selected without a stored custom theme", () => {
    const host = createHost("chat");
    ...
    expect(host.theme).toBe("oriro");
    expect(host.settings.theme).toBe("oriro");
...
describe("applySettingsFromUrl", () => {
  ...
  it("hydrates query token params and strips them from the URL", () => {
    ...
    host.settings.gatewayUrl = "wss://control.example/oriro";
  ...
  it("hydrates native Mac app auth before the first connection", () => {
    ...
    expect(
      (
        window as unknown as {
          __ORIRO_NATIVE_CONTROL_AUTH__?: unknown;
        }
      )["__ORIRO_NATIVE_CONTROL_AUTH__"],
    ).toBeUndefined();
  });
