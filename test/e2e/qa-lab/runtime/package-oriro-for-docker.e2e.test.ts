// Package Oriro For Docker tests cover QA Lab package artifact evidence.
...
import {
  ...
  packOriroPackageForDocker,
  parseArgs,
  runCommandForTest,
} from "../../../../scripts/package-oriro-for-docker.mjs";
...
describe("package-oriro-for-docker", () => {
  it("parses package artifact output options", () => {
    expect(
      parseArgs([
        "--output-dir",
        ".artifacts/docker",
        "--output-name=oriro-current.tgz",
        "--source-dir",
        "/repo",
        "--skip-build",
      ]),
  ...
  it("uses build-all as the single bounded package artifact build step", async () => {
    ...
    const previousTimeout = process.env.ORIRO_DOCKER_PACKAGE_BUILD_TIMEOUT_MS;
    process.env.ORIRO_DOCKER_PACKAGE_BUILD_TIMEOUT_MS = "1234";
    ...
    try {
      await buildPackageArtifacts("/repo", {
        runImpl: async (
          ...
          calls.push({
            ...
            args,
            cwd,
            noPnpm: options.env?.ORIRO_BUILD_ALL_NO_PNPM,
            skipDts: options.env?.ORIRO_RUN_NODE_SKIP_DTS_BUILD,
      ...
      if (previousTimeout === undefined) {
        delete process.env.ORIRO_DOCKER_PACKAGE_BUILD_TIMEOUT_MS;
      } else {
        process.env.ORIRO_DOCKER_PACKAGE_BUILD_TIMEOUT_MS = previousTimeout;
      }
    }
  ...
  it("rejects loose package artifact timeout env values", async () => {
    ...
    try {
      for (const value of ["1e3", "123.9", "9007199254740993", "0"]) {
        process.env.ORIRO_DOCKER_PACKAGE_BUILD_TIMEOUT_MS = value;
        ...
        await expect(
          ...
          "ORIRO_DOCKER_PACKAGE_BUILD_TIMEOUT_MS must be a positive timeout in milliseconds",
        );
      }
    } finally {
      if (previousTimeout === undefined) {
  ...
  it("trims and restores the changelog around ignore-scripts package artifacts", async () => {
    const calls: string[] = [];
    const tarball = await packOriroPackageForDocker("/repo", "/out", {
      ...
      runCaptureImpl: async (
        ...
        return "oriro-2026.5.28.tgz\n";
      },
    });

    expect(tarball).toBe(path.join("/out", "oriro-2026.5.28.tgz"));
    expect(calls).toEqual([
      "prepare:/repo",
  ...
  it("rejects path-like npm pack stdout before resolving Docker package tarballs", async () => {
    for (const filename of [
      ...
      String.raw`C:\temp\oriro-2026.6.17.tgz`,
      "oriro-nested/evil.tgz",
      String.raw`oriro-nested\evil.tgz`,
      "oriro-C:evil.tgz",
    ]) {
      await expect(
        packOriroPackageForDocker("/repo", "/out", {
  ...
  it("ignores unsafe output directory tarball names when npm stdout is not usable", async () => {
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-docker-pack-"));
    try {
      fs.writeFileSync(path.join(outputDir, "oriro-C:evil.tgz"), "");
      fs.writeFileSync(path.join(outputDir, String.raw`oriro-nested\evil.tgz`), "");
      await expect(
        packOriroPackageForDocker("/repo", outputDir, {
          prepareChangelog: async () => {},
          restoreChangelog: async () => {},
          runCaptureImpl: async () => "npm notice\n",
        }),
      ).rejects.toThrow("missing packed Oriro tarball");
      ...
      await expect(
        packOriroPackageForDocker("/repo", outputDir, {
          ...
          restoreChangelog: async () => {},
          runCaptureImpl: async () => {
            fs.writeFileSync(path.join(outputDir, "oriro-2026.6.17.tgz"), "");
            return "npm notice\n";
          },
        }),
      ).resolves.toBe(path.join(outputDir, "oriro-2026.6.17.tgz"));
    } finally {
  ...
  it("ignores stale package tarballs before fallback scanning npm output", async () => {
    const outputDir = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-docker-pack-stale-"));
    try {
      fs.writeFileSync(path.join(outputDir, "oriro-9999.1.1.tgz"), "stale");
      ...
      await expect(
        packOriroPackageForDocker("/repo", outputDir, {
          ...
          restoreChangelog: async () => {},
          runCaptureImpl: async () => {
            fs.writeFileSync(path.join(outputDir, "oriro-2026.6.17.tgz"), "current");
            return "npm notice\n";
          },
        }),
      ).resolves.toBe(path.join(outputDir, "oriro-2026.6.17.tgz"));

      expect(fs.existsSync(path.join(outputDir, "oriro-9999.1.1.tgz"))).toBe(false);
      expect(fs.readFileSync(path.join(outputDir, "oriro-2026.6.17.tgz"), "utf8")).toBe(
        "current",
      );
    } finally {
      fs.rmSync(outputDir, { recursive: true, force: true });
  ...
  it("kills timed-out child process groups", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-package-timeout-"));
  ...
  it("clamps oversized kill grace before scheduling", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-package-grace-"));
  ...
  it("keeps fallback SIGKILL armed for descendants after the direct child exits", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-package-descendant-"));
    const childPidPath = path.join(tempDir, "child.pid");
    let childPid;
    try {
      const childScript = ["process.on('SIGTERM', () => {});", "setInterval(() => {}, 1000);"].join(
  ...
  it("forwards external termination to active child process groups", async () => {
    ...
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-package-signal-"));
    const childPidPath = path.join(tempDir, "child.pid");
    const scriptUrl = pathToFileURL(path.resolve("scripts/package-oriro-for-docker.mjs")).href;
    let childPid = 0;
    let runnerPid;
    try {
