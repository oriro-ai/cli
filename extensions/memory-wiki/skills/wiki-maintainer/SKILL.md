...
description: Maintain the Oriro memory wiki vault with deterministic pages, managed blocks, and source-backed updates.
---
...
- Use `oriro wiki ingest`, `oriro wiki compile`, and `oriro wiki lint` as the default maintenance loop.
- In `bridge` mode, run `oriro wiki bridge import` before relying on search results if you need the latest public memory artifacts pulled in.
- In `unsafe-local` mode, use `oriro wiki unsafe-local import` only when the user explicitly opted into private local path access.
