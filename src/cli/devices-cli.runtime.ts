...
function buildFallbackStateMismatchError(details: ConnectPairingRequiredDetails): Error {
  return new Error(
    [
      ...
      "The running gateway is probably using a different ORIRO_PROFILE or ORIRO_STATE_DIR than this CLI.",
...
function buildExplicitApproveCommand(opts: DevicesRpcOpts, requestId: string): string {
  const args = ["oriro", "devices", "approve", requestId];
...
function resolveRequiredDeviceRole(
  ...
  defaultRuntime.error(
    `--device and --role are required. Run ${formatCliCommand("oriro devices list")} to choose a paired device.`,
  );
...
export async function runDevicesRemoveCommand(
  ...
  if (!trimmed) {
    defaultRuntime.error(
      `deviceId is required. Run ${formatCliCommand("oriro devices list")} to choose a paired device.`,
    );
    defaultRuntime.exit(1);
    return;
  }
