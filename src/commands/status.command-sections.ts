...
export function buildStatusSecurityAuditLines(params: {
  ...
  lines.push(
    params.theme.muted(`Full report: ${params.formatCliCommand("oriro security audit")}`),
  );
  lines.push(
    params.theme.muted(`Deep probe: ${params.formatCliCommand("oriro security audit --deep")}`),
  );
  return lines;
}
...
export function buildStatusModelSelectionLines(params: {
  ...
  for (const sess of mismatches.slice(0, limit)) {
    ...
    lines.push(
      ...
      "  Docs: https://docs.oriro.ai/concepts/models#selection-source-and-fallback-behavior",
    );
  }
...
export function buildStatusFooterLines(params: {
  ...
  return [
    "FAQ: https://docs.oriro.ai/faq",
    "Troubleshooting: https://docs.oriro.ai/troubleshooting",
    ...(params.updateHint ? ["", params.warn(params.updateHint)] : []),
    "Next steps:",
    `  Need to share?      ${params.formatCliCommand("oriro status --all")}`,
    `  Need to debug live? ${params.formatCliCommand("oriro logs --follow")}`,
    params.nodeOnlyGateway
      ? `  Need node service?  ${params.formatCliCommand("oriro node status")}`
      : params.gatewayReachable
        ? `  Need to test channels? ${params.formatCliCommand("oriro status --deep")}`
        : `  Fix reachability first: ${params.formatCliCommand("oriro gateway probe")}`,
  ];
}
...
export function buildStatusPairingRecoveryLines(params: {
  ...
  return [
    ...
    ...(params.pairingRecovery.requestId
      ? [
          params.muted(
            `Recovery: ${params.formatCliCommand(`oriro devices approve ${params.pairingRecovery.requestId}`)}`,
          ),
        ]
      : []),
    params.muted(`Fallback: ${params.formatCliCommand("oriro devices approve --latest")}`),
    params.muted(`Inspect: ${params.formatCliCommand("oriro devices list")}`),
  ];
}
