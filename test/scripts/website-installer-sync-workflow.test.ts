...
describe("website installer sync workflow", () => {
  ...
  it("treats all website installer scripts as Oriro-owned inputs", () => {
  ...
  it("verifies installers on Linux Docker plus native macOS and Windows runners", () => {
    ...
    expect(workflow).toContain("bash /tmp/install.sh --version latest && oriro --version");
    expect(workflow).not.toContain("bash /tmp/install.sh --no-prompt --no-onboard");
    expect(workflow).toContain("bash /tmp/install-cli.sh --prefix /tmp/oriro");
    expect(workflow).toContain("macos-installer:");
    expect(workflow).toContain("runs-on: macos-15");
    expect(workflow).toContain("node-version: 24");
    expect(workflow).toContain('ORIRO_NO_ONBOARD: "1"');
    expect(workflow).toContain('ORIRO_NO_PROMPT: "1"');
    expect(workflow).toContain("bash scripts/install.sh --no-onboard --no-prompt --version latest");
    expect(workflow).toContain("oriro --version");
  ...
  it("syncs verified scripts to oriro.ai only after all installer checks pass", () => {
    ...
    expect(workflow).toContain("cp oriro/scripts/install.sh oriro.ai/public/install.sh");
    expect(workflow).toContain(
      "cp oriro/scripts/install-cli.sh oriro.ai/public/install-cli.sh",
    );
    expect(workflow).toContain("cp oriro/scripts/install.ps1 oriro.ai/public/install.ps1");
    expect(workflow).toContain("rm -f oriro.ai/public/install.cmd");
