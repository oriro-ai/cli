...
import { info, success } from "oriro/plugin-sdk/runtime-env";
import { getChildLogger } from "oriro/plugin-sdk/runtime-env";
import { defaultRuntime, type RuntimeEnv } from "oriro/plugin-sdk/runtime-env";
...
export async function pickWebChannel(
  ...
  if (!auth.exists) {
    throw new Error(
      `No WhatsApp Web session found. Run \`${formatCliCommand("oriro channels login --channel whatsapp --verbose")}\` to link.`,
    );
  }
  return choice;
}
