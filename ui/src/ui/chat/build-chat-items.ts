...
function isPendingSendMessage(message: unknown): boolean {
  return asRecord(asRecord(message)?.["__oriro"])?.kind === "pending-send";
}
...
function sourceMessageId(message: unknown): string | null {
  ...
  const oriroId = asRecord(record["__oriro"])?.id;
  if (typeof oriroId === "string" && oriroId.trim()) {
    return oriroId.trim();
  }
...
export function buildChatItems(props: BuildChatItemsProps): Array<ChatItem | MessageGroup> {
  ...
  for (let i = historyStart; i < history.length; i++) {
    ...
    const marker = raw["__oriro"] as Record<string, unknown> | undefined;
