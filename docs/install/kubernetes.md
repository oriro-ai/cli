---
summary: "Deploy Oriro Gateway to a Kubernetes cluster with Kustomize"
read_when:
  - You want to run Oriro on a Kubernetes cluster
  - You want to test Oriro in a Kubernetes environment
title: "Kubernetes"
---

A minimal starting point for running Oriro on Kubernetes — not a production-ready deployment. It covers the core resources and is meant to be adapted to your environment.

## Why not Helm?

Oriro is a single container with some config files. The interesting customization is in agent content (markdown files, skills, config overrides), not infrastructure templating. Kustomize handles overlays without the overhead of a Helm chart. If your deployment grows more complex, a Helm chart can be layered on top of these manifests.
...
kubectl port-forward svc/oriro 18789:18789 -n oriro
...
```bash
kubectl get secret oriro-secrets -n oriro -o jsonpath='{.data.ORIRO_GATEWAY_TOKEN}' | base64 -d
```
...
```
Namespace: oriro (configurable via ORIRO_NAMESPACE)
├── Deployment/oriro        # Single pod, init container + gateway
├── Service/oriro           # ClusterIP on port 18789
├── PersistentVolumeClaim      # 10Gi for agent state and config
├── ConfigMap/oriro-config  # oriro.json + AGENTS.md
└── Secret/oriro-secrets    # Gateway token + API keys
```
...
Edit `oriro.json` in `scripts/k8s/manifests/configmap.yaml`. See [Gateway configuration](/gateway/configuration) for the full reference.
...
```bash
kubectl patch secret oriro-secrets -n oriro \
  -p '{"stringData":{"<PROVIDER>_API_KEY":"..."}}'
kubectl rollout restart deployment/oriro -n oriro
```

### Custom namespace

```bash
ORIRO_NAMESPACE=my-namespace ./scripts/k8s/deploy.sh
```
...
```yaml
image: ghcr.io/oriro-ai/cli:latest # or pin to a specific version from https://github.com/oriro-ai/cli/releases
```
...
└── manifests/
    ...
    ├── configmap.yaml          # oriro.json + AGENTS.md
