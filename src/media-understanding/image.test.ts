...
const hoisted = vi.hoisted(() => ({
  completeMock: vi.fn(),
  ensureOriroModelsJsonMock: vi.fn(async () => {}),
...
vi.mock("../agents/models-config.js", async () => ({
  ...
  ensureOriroModelsJson: ensureOriroModelsJsonMock,
}));
...
describe("describeImageWithModel", () => {
  ...
  it("resolves configured image models when discovery has not registered the provider", async () => {
    ...
    expect(resolveRequest.agentDir).toBe("/tmp/oriro-agent");
    expect(
      requireRecord(
        requireRecord(
