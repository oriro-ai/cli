...
// Keep integration tests deterministic: never read a real oriro.json.
...
const suiteRootTracker = createSuiteTempRootTracker({ prefix: "oriro-pruning-integ-" });
...
describe("Integration: saveSessionStore with pruning", () => {
  ...
  beforeEach(async () => {
    ...
    savedCacheTtl = process.env.ORIRO_SESSION_CACHE_TTL_MS;
    process.env.ORIRO_SESSION_CACHE_TTL_MS = "0";
    clearSessionStoreCacheForTest();
  });
  ...
  afterEach(() => {
    ...
    if (savedCacheTtl === undefined) {
      delete process.env.ORIRO_SESSION_CACHE_TTL_MS;
    } else {
      process.env.ORIRO_SESSION_CACHE_TTL_MS = savedCacheTtl;
    }
  });
  ...
  it("removes trajectory sidecars for stale sessions pruned on write", async () => {
    ...
    await fs.writeFile(staleTranscript, '{"type":"session"}\n', "utf-8");
    await fs.writeFile(freshTranscript, '{"type":"session"}\n', "utf-8");
    await fs.writeFile(staleRuntime, '{"traceSchema":"oriro-trajectory"}\n', "utf-8");
    await fs.writeFile(freshRuntime, '{"traceSchema":"oriro-trajectory"}\n', "utf-8");
    await fs.writeFile(
      stalePointer,
      JSON.stringify({
