// Commander wiring for `oriro update`, its status/finalize subcommands, and help text.
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["oriro update repair", "Rerun post-update doctor and plugin convergence."],
          ["oriro update repair --channel beta", "Repair against the beta update channel."],
          ["oriro update repair --json", "JSON output for automation."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          ...
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.oriro.ai/cli/update")}`,
    )
...
export function registerUpdateCli(program: Command) {
  ...
  const update = program
    ...
    .addHelpText("after", () => {
      const examples = [
        ["oriro update", "Update a source checkout (git)"],
        ["oriro update --channel beta", "Switch to beta channel (git + npm)"],
        ["oriro update --channel dev", "Switch to dev channel (git + npm)"],
        ["oriro update --tag beta", "One-off update to a dist-tag or version"],
        ["oriro update --tag main", "One-off package update from GitHub main"],
        ["oriro update --dry-run", "Preview actions without changing anything"],
        ["oriro update --no-restart", "Update without restarting the service"],
        ["oriro update --json", "Output result as JSON"],
        ["oriro update --yes", "Non-interactive (accept downgrade prompts)"],
        ["oriro update repair", "Repair stranded post-update plugin state"],
        ["oriro update wizard", "Interactive update wizard"],
        ["oriro --update", "Shorthand for oriro update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run oriro update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.oriro.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["oriro update status", "Show channel + version status."],
          ["oriro update status --json", "JSON output."],
          ["oriro update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
