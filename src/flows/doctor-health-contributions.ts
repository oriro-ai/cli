...
function isUpdateDoctorRun(env: NodeJS.ProcessEnv | Record<string, string | undefined>): boolean {
  const value = env.ORIRO_UPDATE_IN_PROGRESS;
  return value === "1" || value === "true";
}

function resolveDoctorMode(cfg: OriroConfig): DoctorFlowMode {
...
export function shouldSkipLegacyUpdateDoctorConfigWrite(params: {
  ...
  if (!isTruthyEnvValue(params.env.ORIRO_UPDATE_IN_PROGRESS)) {
    return false;
  }
...
async function runGatewayConfigHealth(ctx: DoctorHealthFlowContext): Promise<void> {
  ...
  if (!ctx.cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("oriro configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("oriro config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(ctx.configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("oriro setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  if (resolveDoctorMode(ctx.cfg) === "local" && hasAmbiguousGatewayAuthModeConfig(ctx.cfg)) {
    note(
      [
        ...
        `Set token mode: ${formatCliCommand("oriro config set gateway.auth.mode token")}`,
        `Set password mode: ${formatCliCommand("oriro config set gateway.auth.mode password")}`,
      ].join("\n"),
      "Gateway auth",
    );
  }
...
async function runBrowserHealth(ctx: DoctorHealthFlowContext): Promise<void> {
  ...
  await runCoreContributionHealthRepair(ctx, ["core/doctor/browser-orirod-profile-residue"]);
...
async function runGatewayHealthChecks(ctx: DoctorHealthFlowContext): Promise<void> {
  ...
  if ((await hasActiveGatewayExecCredential(ctx)) && ctx.options.allowExec !== true) {
    note(
      "Gateway health probes skipped because gateway credentials use an exec SecretRef. Run `oriro doctor --allow-exec` to verify Gateway health with exec SecretRefs.",
      "Gateway",
    );
...
async function runWriteConfigHealth(ctx: DoctorHealthFlowContext): Promise<void> {
  ...
  if (!ctx.prompter.shouldRepair) {
    ctx.runtime.log(`Run "${formatCliCommand("oriro doctor --fix")}" to apply changes.`);
  }
}
...
export function resolveDoctorHealthContributions(): DoctorHealthContribution[] {
  return [
    ...
    createDoctorHealthContribution({
      id: "doctor:browser",
      label: "Browser",
      healthCheckIds: ["core/doctor/browser", "core/doctor/browser-orirod-profile-residue"],
