...
describeLaunchdIntegration("launchd integration", () => {
  ...
  beforeAll(async () => {
    ...
    homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `oriro-launchd-int-${testId}-`));
    env = {
      HOME: homeDir,
      ORIRO_LAUNCHD_LABEL: `ai.oriro.launchd-int-${testId}`,
      ORIRO_LOG_PREFIX: `gateway-launchd-int-${testId}`,
    };
  });
