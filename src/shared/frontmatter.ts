...
import { normalizeCsvOrLooseStringList } from "@oriro/normalization-core/string-normalization";
...
/** Parses the JSON5 Oriro manifest block embedded inside a string frontmatter field. */
export function resolveOriroManifestBlock(params: {
  frontmatter: Record<string, unknown>;
  key?: string;
...
export type OriroManifestRequires = {
...
/** Extracts normalized runtime requirement lists from an Oriro manifest block. */
export function resolveOriroManifestRequires(
  metadataObj: Record<string, unknown>,
): OriroManifestRequires | undefined {
  const requiresRaw =
...
export function resolveOriroManifestInstall<T>(
  metadataObj: Record<string, unknown>,
...
/** Extracts normalized OS allowlist entries from an Oriro manifest block. */
export function resolveOriroManifestOs(metadataObj: Record<string, unknown>): string[] {
  return normalizeStringList(metadataObj.os);
}

export type ParsedOriroManifestInstallBase = {
...
export function parseOriroManifestInstallBase(
  input: unknown,
  allowedKinds: readonly string[],
): ParsedOriroManifestInstallBase | undefined {
  ...
  const spec: ParsedOriroManifestInstallBase = {
    raw,
    kind,
  };
...
export function applyOriroManifestInstallCommonFields<
  T extends { id?: string; label?: string; bins?: string[] },
>(spec: T, parsed: Pick<ParsedOriroManifestInstallBase, "id" | "label" | "bins">): T {
  if (parsed.id) {
    spec.id = parsed.id;
  }
  if (parsed.label) {
