...
const smokeMode =
  process.env.OPENWEBUI_SMOKE_MODE ?? process.env.ORIRO_OPENWEBUI_SMOKE_MODE ?? "chat";
...
for (let attempt = 1; attempt <= modelAttempts; attempt += 1) {
  ...
  if (modelsResult?.ok) {
    ...
    targetModel =
      modelIds.find((id) => id === "oriro/default") ?? modelIds.find((id) => id === "oriro");
    if (targetModel) {
      break;
    }
    lastModelsError = `missing oriro model: ${JSON.stringify(modelIds)}`;
...
if (!targetModel) {
  throw new Error(
    `oriro model missing from Open WebUI model list after retry: ${JSON.stringify(modelIds)} (${lastModelsError})`,
  );
}
if (smokeMode === "models") {
