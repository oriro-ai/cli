...
import { normalizeOptionalString as normalizeTrimmedString } from "@oriro/normalization-core/string-coerce";
...
const PLUGIN_MANIFEST_FILENAME = "oriro.plugin.json";
...
function resolveUserPath(value: string, env: NodeJS.ProcessEnv): string {
  if (value === "~" || value.startsWith("~/")) {
    const home = env.ORIRO_HOME ?? env.HOME ?? env.USERPROFILE ?? os.homedir();
...
function resolveStateDir(env: NodeJS.ProcessEnv): string {
  const override = normalizeTrimmedString(env.ORIRO_STATE_DIR);
  if (override) {
...
export function listOriroPluginManifestMetadata(
