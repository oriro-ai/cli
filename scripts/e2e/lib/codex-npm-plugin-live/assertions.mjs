...
const allowBetaCompatDiagnostics =
  process.env.ORIRO_CODEX_NPM_PLUGIN_ALLOW_BETA_COMPAT_DIAGNOSTICS === "1";
...
const MAX_ERROR_TAIL_BYTES = readPositiveIntEnv(
  "ORIRO_CODEX_NPM_PLUGIN_ASSERT_MAX_ERROR_TAIL_BYTES",
  64 * 1024,
);
const MAX_TRANSCRIPT_FILES = readPositiveIntEnv(
  "ORIRO_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_FILES",
  64,
);
const MAX_TRANSCRIPT_WALK_ENTRIES = readPositiveIntEnv(
  "ORIRO_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_WALK_ENTRIES",
  4096,
);
const MAX_TRANSCRIPT_SCAN_BYTES = readPositiveIntEnv(
  "ORIRO_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_SCAN_BYTES",
  2 * 1024 * 1024,
);
...
function assertPlugin() {
  const spec = process.argv[3] || "npm:@oriro/codex";
  const list = readJson("/tmp/oriro-codex-plugins-list.json");
  const inspect = readJson("/tmp/oriro-codex-plugin-inspect.json");
...
function codexNpmProjectRoot() {
  return npmProjectRootForInstalledPackage(codexInstallPath(), "@oriro/codex");
}
...
function assertNpmDeps() {
  ...
  if (!fs.existsSync(pluginPackageJson)) {
    throw new Error(`missing npm-installed @oriro/codex package.json: ${pluginPackageJson}`);
  }
  ...
  if (pluginPackage.name !== "@oriro/codex") {
  ...
  if (!openAiCodexPackageJson) {
    throw new Error("missing @openai/codex dependency under .oriro/npm");
  }
...
function assertPreflight() {
  ...
  const output = readTextFileBounded("/tmp/oriro-codex-preflight.log", "Codex preflight log");
...
function assertUninstalled() {
  ...
  const list = readJson("/tmp/oriro-codex-plugins-list-after-uninstall.json");
  const plugin = (list.plugins || []).find((entry) => entry.id === "codex");
  if (plugin?.status === "loaded" || plugin?.enabled === true) {
...
function assertAgentError() {
  ...
  const stdout = fs.existsSync("/tmp/oriro-codex-agent-after-uninstall.json")
    ...
    : "";
  const stderr = fs.existsSync("/tmp/oriro-codex-agent-after-uninstall.err")
    ? readTextFileTail(
