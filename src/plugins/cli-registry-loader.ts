...
export type PluginCliCommandGroupEntry = {
  ...
  placeholders: readonly OriroPluginCliCommandDescriptor[];
  names: readonly string[];
  register: (program: OriroPluginCliContext["program"]) => Promise<void>;
};
...
export async function loadPluginCliMetadataRegistryWithContext(
  ...
  return {
    ...context,
    registry: await loadOriroPluginCliRegistry(
...
): Promise<OriroPluginCliCommandDescriptor[]> {
  try {
