...
export type JsonlReplayResult = {
  transcripts: Array<{
    ...
    cells: { oriro: RuntimeParityCell[]; codex: RuntimeParityCell[] };
...
export async function runJsonlReplay(
  ...
  for (const transcriptPath of transcriptPaths) {
    ...
    const cells: { oriro: RuntimeParityCell[]; codex: RuntimeParityCell[] } = {
      oriro: [],
      codex: [],
    };
    ...
    for (const turn of turns) {
      ...
      cells.oriro.push(parity.cells.oriro);
...
export function renderJsonlReplayMarkdownReport(report: JsonlReplayMarkdownReport): string {
  ...
  const lines = [
    `# ORIRO JSONL Replay Report - ${report.runtimePair[0]} vs ${report.runtimePair[1]}`,
    "",
