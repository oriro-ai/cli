...
import { sleepWithAbort } from "oriro/plugin-sdk/runtime-env";
import {
...
import { resolveTargetsWithOptionalToken } from "oriro/plugin-sdk/target-resolver-runtime";
import {
...
function resolveDiscordStartupDelayMs(cfg: OriroConfig, accountId: string): number {
    ...
    base: {
      ...
      agentPrompt: {
        messageToolHints: () => [
          ...
          "- Forms: add `components.modal` (title, fields). Oriro adds a trigger button and routes submissions as new messages.",
        ],
      },
      messaging: {
