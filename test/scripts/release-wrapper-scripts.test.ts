...
const UNKNOWN_PACKAGE = "@oriro/not-a-real-release-wrapper-test-package";
...
describe("release wrapper scripts", () => {
  ...
  it("loads the Oriro OriroHub plan CLI and validates required arguments before planning", () => {
    const result = runTsxScript("scripts/oriro-release-orirohub-plan.ts", [
      "--release-tag",
      "v2026.6.21-beta.1",
  ...
  it("loads the beta verifier CLI and validates required version input before remote checks", () => {
    const result = runTsxScript("scripts/release-verify-beta.ts", ["--skip-orirohub"]);
