/**
 * Standalone MCP server that exposes Oriro plugin-registered tools
...
function resolvePluginToolPolicy(config: OriroConfig): {
...
function resolveTools(config: OriroConfig): AnyAgentTool[] {
...
export function createPluginToolsMcpServer(
  ...
  return createToolsMcpServer({ name: "oriro-plugin-tools", tools });
}
