...
import { detectMime } from "oriro/plugin-sdk/media-mime";
import { isTruthyEnvValue, type RuntimeEnv } from "oriro/plugin-sdk/runtime-env";
import { resolveStateDir } from "oriro/plugin-sdk/state-paths";
import {
  lowercasePreservingWhitespace,
  normalizeOptionalString,
} from "oriro/plugin-sdk/string-coerce-runtime";
import { ensureDir, resolveUserPath } from "oriro/plugin-sdk/text-utility-runtime";
...
(() => {
  ...
  const hasIOS = () =>
    !!(
      ...
      window.webkit.messageHandlers.oriroCanvasA2UIAction
    );
  const hasAndroid = () =>
    !!(
      (window.oriroCanvasA2UIAction &&
        typeof window.oriroCanvasA2UIAction.postMessage === "function")
    );
  const hasHelper = () => typeof window.oriroSendUserAction === "function";
  ...
  window.addEventListener("oriro:a2ui-action-status", onStatus);

  function send(name, sourceComponentId) {
    if (!hasHelper()) {
      log("No action bridge found. Ensure you're viewing this on an iOS/Android ORIRO node canvas.");
      return;
    }
    const sendUserAction =
      typeof window.oriroSendUserAction === "function"
        ? window.oriroSendUserAction
        : undefined;
...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.ORIRO_SKIP_CANVAS_HOST)) {
    return true;
  }
    ...
    try {
      ...
      if (!opened) {
        if (urlPath === "/" || urlPath.endsWith("/")) {
          ...
          res.end(
            `<!doctype html><meta charset="utf-8" /><title>ORIRO Canvas</title><pre>Missing file.\nCreate ${rootDir}/index.html</pre>`,
          );
          return true;
        }
        res.statusCode = 404;
