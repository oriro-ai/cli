...
describe("command-path-policy", () => {
  ...
  it("applies exact overrides after broader channel plugin rules", () => {
    ...
    expect(
      channelsStatusPolicy.networkProxy({
        argv: ["node", "oriro", "channels", "status"],
        commandPath: ["channels", "status"],
      }),
    ).toBe("bypass");
    expect(
      channelsStatusPolicy.networkProxy({
        argv: ["node", "oriro", "channels", "status", "--probe"],
        commandPath: ["channels", "status"],
      }),
    ).toBe("default");
  ...
  it("defaults unknown command paths to network proxy routing", () => {
    expect(resolveCliNetworkProxyPolicy(["node", "oriro", "googlemeet", "login"])).toBe(
      "default",
    );
    expect(resolveCliNetworkProxyPolicy(["node", "oriro", "tool", "image_generate"])).toBe(
      "bypass",
    );
    expect(resolveCliNetworkProxyPolicy(["node", "oriro", "tools", "effective"])).toBe("bypass");
  });

  it("resolves static network proxy bypass policies from the catalog", () => {
    expect(resolveCliNetworkProxyPolicy(["node", "oriro", "status"])).toBe("bypass");
    expect(
      resolveCliNetworkProxyPolicy(["node", "oriro", "config", "get", "proxy.enabled"]),
    ).toBe("bypass");
    expect(resolveCliNetworkProxyPolicy(["node", "oriro", "proxy", "start"])).toBe("bypass");
  });

  it("resolves mixed network proxy policies from argv-sensitive catalog entries", () => {
    expect(resolveCliNetworkProxyPolicy(["node", "oriro", "gateway"])).toBe("default");
    expect(resolveCliNetworkProxyPolicy(["node", "oriro", "gateway", "run"])).toBe("default");
    expect(resolveCliNetworkProxyPolicy(["node", "oriro", "gateway", "health"])).toBe("bypass");
    expect(resolveCliNetworkProxyPolicy(["node", "oriro", "node", "run"])).toBe("default");
    expect(resolveCliNetworkProxyPolicy(["node", "oriro", "node", "status"])).toBe("bypass");
    expect(resolveCliNetworkProxyPolicy(["node", "oriro", "agent", "--local"])).toBe("default");
    expect(resolveCliNetworkProxyPolicy(["node", "oriro", "agent", "run"])).toBe("bypass");
    expect(resolveCliNetworkProxyPolicy(["node", "oriro", "channels", "status"])).toBe("bypass");
    expect(
      resolveCliNetworkProxyPolicy(["node", "oriro", "channels", "status", "--probe"]),
    ).toBe("default");
    expect(resolveCliNetworkProxyPolicy(["node", "oriro", "models", "status"])).toBe("bypass");
    expect(resolveCliNetworkProxyPolicy(["node", "oriro", "models", "status", "--probe"])).toBe(
      "default",
    );
    expect(resolveCliNetworkProxyPolicy(["node", "oriro", "skills", "info", "browser"])).toBe(
      "bypass",
    );
    expect(resolveCliNetworkProxyPolicy(["node", "oriro", "skills", "check"])).toBe("bypass");
    expect(resolveCliNetworkProxyPolicy(["node", "oriro", "skills", "list"])).toBe("bypass");
    expect(resolveCliNetworkProxyPolicy(["node", "oriro", "skills", "search", "browser"])).toBe(
      "default",
    );
  });
  ...
  it("uses the longest catalog command path for deep network proxy overrides", async () => {
    ...
    expect(
      resolveCliCatalogCommandPathLocal(["node", "oriro", "nodes", "camera", "snap"]),
    ).toEqual(["nodes", "camera", "snap"]);
    expect(resolveCliNetworkProxyPolicyLocal(["node", "oriro", "nodes", "camera", "snap"])).toBe(
      "default",
    );
    expect(resolveCliNetworkProxyPolicyLocal(["node", "oriro", "nodes", "camera", "list"])).toBe(
      "bypass",
    );
  });
  ...
  it("stops catalog command path resolution before positional arguments", () => {
    expect(
      resolveCliCatalogCommandPath(["node", "oriro", "config", "get", "proxy.enabled"]),
    ).toEqual(["config", "get"]);
    expect(
      resolveCliCatalogCommandPath(["node", "oriro", "message", "send", "--to", "demo"]),
    ).toEqual(["message"]);
  });

  it("treats bare gateway invocations with options as the gateway runtime", () => {
    const argv = ["node", "oriro", "gateway", "--port", "1234"];
  ...
  it("resolves gateway runs after root options with values", () => {
    const argv = ["node", "oriro", "--log-level", "debug", "gateway", "run"];
  ...
  it("does not let gateway run option values spoof bypass subcommands", () => {
    for (const argv of [
      ["node", "oriro", "gateway", "--token", "status"],
      ["node", "oriro", "gateway", "--token=status"],
      ["node", "oriro", "gateway", "--password", "health"],
      ["node", "oriro", "gateway", "--password-file", "status"],
      ["node", "oriro", "gateway", "--ws-log", "compact"],
    ]) {
  ...
  it("still resolves real gateway bypass subcommands after their command token", () => {
    expect(resolveCliCatalogCommandPath(["node", "oriro", "gateway", "status"])).toEqual([
      "gateway",
      "status",
    ]);
    expect(
      resolveCliCatalogCommandPath(["node", "oriro", "gateway", "status", "--token", "secret"]),
    ).toEqual(["gateway", "status"]);
    expect(resolveCliNetworkProxyPolicy(["node", "oriro", "gateway", "status"])).toBe("bypass");
  });
});
