...
import { ensureOriroCliOnPath } from "../infra/path-env.js";
...
function shouldBootstrapCliProxyBeforeFastPath(env: NodeJS.ProcessEnv = process.env): boolean {
  if (
    isTruthyEnvValue(env.ORIRO_DEBUG_PROXY_ENABLED) ||
    isTruthyEnvValue(env.ORIRO_DEBUG_PROXY_REQUIRE)
  ) {
    return true;
  }
...
function createAllowlistAgnosticCliLookupConfig(config: OriroConfig): OriroConfig {
...
async function resolveUnownedCliPrimaryMessage(params: {
  ...
  return [
    `Unknown command: oriro ${params.primary}. No built-in command or plugin CLI metadata owns "${params.primary}".`,
    suggestion,
  ]
    .filter(Boolean)
    .join("\n");
...
export async function runCli(argv: string[] = process.argv) {
  ...
  const containerTargetName =
    parsedContainer.container ?? normalizeOptionalString(process.env.ORIRO_CONTAINER) ?? null;
  ...
  let bestEffortConfigPromise: Promise<OriroConfig> | null = null;
  const isolateProxyConfigEnv = isGatewayRunInvocation;
  const readBestEffortCliConfig = async (): Promise<OriroConfig> => {
  ...
  const replaceStartedProxy = async (config: OriroConfig["proxy"]) => {
  ...
  try {
    ...
    // `oriro <typo> --help` surfaces the same Unknown command error as
    // `oriro <typo>` instead of silently showing generic top-level help.
    ...
    if (shouldRunBareRootCrestodian) {
      ...
      if (!process.stdin.isTTY || !process.stdout.isTTY) {
        console.error(
          'Crestodian needs an interactive TTY. Use `oriro crestodian --message "status"` for one command.',
        );
        process.exitCode = 1;
        return;
      }
    ...
    try {
      ...
      process.on("uncaughtException", (error) => {
        ...
        if (isBenignUncaughtExceptionError(error)) {
          console.warn(
            "[oriro] Non-fatal uncaught exception (continuing):",
