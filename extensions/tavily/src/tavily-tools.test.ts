...
function fakeApi(): OriroPluginApi {
  return {
    config: {},
  } as OriroPluginApi;
}
...
describe("tavily tools", () => {
  ...
  it("normalizes optional parameters before invoking Tavily", async () => {
    ...
    const result = await tool.execute("call-1", {
      ...
      include_answer: true,
      time_range: "week",
      include_domains: ["docs.oriro.ai", "", "oriro.ai"],
    ...
    expect(runTavilySearch).toHaveBeenCalledWith({
      ...
      includeAnswer: true,
      timeRange: "week",
      includeDomains: ["docs.oriro.ai", "oriro.ai"],
  ...
  it("late-binds dedicated tools to the resolved runtime config snapshot", async () => {
    ...
    const registeredTools: Array<Parameters<OriroPluginApi["registerTool"]>[0]> = [];
    const registeredOptions: Array<Parameters<OriroPluginApi["registerTool"]>[1]> = [];
    ...
    await searchTool.execute("search-call", { query: "oriro" });
    ...
    expect(searchParams.query).toBe("oriro");
  ...
  it("falls back to environment values and defaults", () => {
    ...
    expect(resolveTavilyBaseUrl({} as OriroConfig)).not.toBe(DEFAULT_TAVILY_BASE_URL);
