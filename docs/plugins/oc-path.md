---
summary: "Bundled `oc-path` plugin: ships the `oriro path` CLI for the `oc://` workspace-file addressing scheme"
read_when:
...
The bundled `oc-path` plugin adds the [`oriro path`](/cli/path) CLI for the
`oc://` workspace-file addressing scheme. It ships in the Oriro repo under
...
- **yaml** (`.yaml`, `.yml`, `.oriro`): map/sequence/scalar nodes through the
  YAML document API
...
- **Local automation**: shell scripts can resolve or update one workspace value
  with `oriro path … --json` instead of carrying separate markdown, JSONC,
...
oriro path resolve 'oc://config.jsonc/plugins/github/enabled' --json

# Which tool-call names appear in this session log?
oriro path find 'oc://session.jsonl/[event=tool_call]/name' --json
...
The plugin runs **in-process inside the `oriro` CLI** on the host where you
...
The plugin metadata lives in `extensions/oc-path/oriro.plugin.json`:
...
`oriro path …`, so installs that never use the verb pay no cost.
...
state. Bare `oriro path` invocations work immediately on the same host —
the CLI loads the plugin on demand.

Disable with:

```bash
oriro plugins disable oc-path
```
...
| Surface                        | Provided by                                             |
| ------------------------------ | ------------------------------------------------------- |
| `oriro path` CLI            | `extensions/oc-path/cli-registration.ts`                |
...
`__ORIRO_REDACTED__` (verbatim or as a substring) is refused at write time
...
- [`oriro path` CLI reference](/cli/path)
