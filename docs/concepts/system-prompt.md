---
summary: "What the Oriro system prompt contains and how it is assembled"
read_when:
...
Oriro builds a custom system prompt for every agent run. The prompt is **Oriro-owned** and does not use a runtime default prompt.

The prompt is assembled by Oriro and injected into each agent run.
...
the full Oriro-owned prompt. The provider runtime can:
...
- **Oriro Control**: tells the model to prefer the `gateway` tool for
  config/restart work and to avoid inventing CLI commands.
- **Oriro Self-Update**: how to inspect config safely with
...
- **Documentation**: local path to Oriro docs/source and when to read them.
...
Oriro keeps large stable content, including **Project Context**, above the
...
Oriro can render smaller system prompts for sub-agents. The runtime sets a
`promptMode` for each run (not a user-facing config):

- `full` (default): includes all sections above.
- `minimal`: used for sub-agents; omits **Memory Recall**, **Oriro
...
For channel auto-reply runs, Oriro omits the generic **Silent Replies**
...
Oriro keeps committed prompt snapshots for the Codex runtime happy path under
...
Oriro developer instructions, turn-scoped collaboration-mode instructions
when Oriro provides them, user turn input, and references to the dynamic tool
specs.
...
collaboration-mode instructions inside the Codex runtime after Oriro sends
...
On the native Codex harness, Oriro avoids repeating stable workspace files
...
Codex developer instructions. The compact Oriro skills list is also forwarded
...
Oriro prompt according to their existing gates. `HEARTBEAT.md` is omitted on
...
occurs, Oriro can inject a concise system-prompt warning notice; control this with
...
When eligible skills exist, Oriro injects a compact **available skills list**
...
points to the local Oriro docs directory (`docs/` in a Git checkout or the bundled npm
package docs). If local docs are unavailable, it falls back to
[https://docs.oriro.ai](https://docs.oriro.ai).

The same section also includes the Oriro source location. Git checkouts expose the local
...
stale. The prompt also notes the public docs mirror, community Discord, and OriroHub
([https://orirohub.ai](https://orirohub.ai)) for skills discovery. It frames docs as the
authority for Oriro self-knowledge before the model understands how Oriro works,
...
notes, and `memory_search` as instruction context or user memory rather than Oriro
design or implementation knowledge. If docs are silent or stale, the model should say so
and inspect source. The prompt also tells the model to run `oriro status` itself when
