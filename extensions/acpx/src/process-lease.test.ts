...
describe("createAcpxProcessLeaseStore", () => {
  ...
  beforeEach(async () => {
    resetPluginStateStoreForTests();
    stateDir = await mkdtemp(path.join(tmpdir(), "oriro-acpx-leases-"));
...
describe("withAcpxLeaseEnvironment", () => {
  it("adds lease environment and wrapper args on POSIX", () => {
    ...
    expect(command).toBe(
      [
        "env",
        `${ORIRO_ACPX_LEASE_ID_ENV}=lease-test`,
        `${ORIRO_GATEWAY_INSTANCE_ID_ENV}=gateway-test`,
        "node /tmp/oriro/acpx/codex-acp-wrapper.mjs",
        ORIRO_ACPX_LEASE_ID_ARG,
        "lease-test",
        ORIRO_GATEWAY_INSTANCE_ID_ARG,
        "gateway-test",
  ...
  it("keeps Windows logs keyed by lease id with wrapper args", () => {
    const command = withAcpxLeaseEnvironment({
      command: "node C:/oriro/acpx/codex-acp-wrapper.mjs",
      leaseId: "lease-test",
      gatewayInstanceId: "gateway-test",
      platform: "win32",
    });
    ...
    expect(command).toBe(
      [
        "node C:/oriro/acpx/codex-acp-wrapper.mjs",
        ORIRO_ACPX_LEASE_ID_ARG,
        "lease-test",
        ORIRO_GATEWAY_INSTANCE_ID_ARG,
        "gateway-test",
    ...
    expect(command).not.toContain(`${ORIRO_ACPX_LEASE_ID_ENV}=`);
    expect(command).not.toContain(`${ORIRO_GATEWAY_INSTANCE_ID_ENV}=`);
  });
});
