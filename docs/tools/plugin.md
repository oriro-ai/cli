---
summary: "Install, configure, and manage Oriro plugins"
read_when:
...
Plugins extend Oriro with channels, model providers, agent harnesses, tools,
...
- an Oriro checkout or installation with the `oriro` CLI available
- network access to the selected source, such as OriroHub, npm, or a git host
...
<Steps>
  <Step title="Find the plugin">
    Search [OriroHub](/orirohub) for public plugin packages:

    ```bash
    oriro plugins search "calendar"
    ```

    OriroHub is the primary discovery surface for community plugins. During the
    launch cutover, ordinary bare package specs still install from npm unless
    they match an official plugin id. Raw `@oriro/*` package specs that match
    bundled plugins use the bundled copy from the current Oriro build. Use an
  ...
  <Step title="Install the plugin">
    ...
    # From npm.
    oriro plugins install npm:<package>

    # From git.
    oriro plugins install git:github.com/<owner>/<repo>@<ref>

    # From a local development checkout.
    oriro plugins install ./my-plugin
    oriro plugins install --link ./my-plugin
    ```
  ...
  <Step title="Configure and enable it">
    ...
    ```bash
    oriro plugins enable <plugin-id>
    ```
    ...
    `oriro plugins install` adds the installed id to an existing
  ...
  <Step title="Let the Gateway reload">
    ...
    enabled, Oriro detects the changed plugin install record and restarts the
  ...
  <Step title="Verify runtime registration">
    ```bash
    oriro plugins inspect <plugin-id> --runtime --json
    ```
...
| ----------- | ------------------------------------------------------------------------------ | -------------------------------------------------------------- |
| OriroHub     | You want Oriro-native discovery, scans, version metadata, and install hints | `oriro plugins install orirohub:<package>`                   |
| npm         | You need direct npm registry or dist-tag workflows                             | `oriro plugins install npm:<package>`                       |
| git         | You need a branch, tag, or commit from a repository                            | `oriro plugins install git:github.com/<owner>/<repo>@<ref>` |
| local path  | You are developing or testing a plugin on the same machine                     | `oriro plugins install --link ./my-plugin`                  |
| marketplace | You are installing a Claude-compatible marketplace plugin                      | `oriro plugins install <plugin> --marketplace <source>`     |

Bare package specs have special compatibility behavior. If the bare name matches
a bundled plugin id, Oriro uses that bundled source. If it matches an
official external plugin id, Oriro uses the official package catalog. Other
ordinary bare package specs install through npm during the launch cutover. Raw
`@oriro/*` package specs that match bundled plugins also resolve to the
bundled copy before npm fallback. Use `npm:@oriro/<plugin>@<version>` when
you deliberately want the external npm package instead of the image-owned
bundled copy. Use `orirohub:`, `npm:`, `git:`, or `npm-pack:` when you need
deterministic source selection. See [`oriro plugins`](/cli/plugins#install)
for the full command contract.

For npm installs, unpinned package specs and `@latest` choose the newest stable
package that advertises compatibility with this Oriro build. If npm's
current latest release declares a newer `oriro.compat.pluginApi` or
`oriro.install.minHostVersion`, Oriro scans older stable package versions
...
Oriro processes where plugin hooks are loaded, so use `security.installPolicy`
...
not bypass install policy or Oriro's built-in plugin dependency denylist.
...
Run `oriro doctor` or `oriro doctor --fix` when config validation reports
stale plugin ids, allowlist/tool mismatches, or legacy bundled plugin paths.

## Understand plugin formats

Oriro recognizes two plugin formats:

| Format                 | How it loads                                                                 | Use when                                                               |
| ---------------------- | ---------------------------------------------------------------------------- | ---------------------------------------------------------------------- |
| Native Oriro plugin | `oriro.plugin.json` plus a runtime module loaded in process               | You are installing or building Oriro-specific runtime capabilities  |
| Compatible bundle      | Codex, Claude, or Cursor plugin layout mapped into Oriro plugin inventory | You are reusing compatible skills, commands, hooks, or bundle metadata |

Both formats appear in `oriro plugins list`, `oriro plugins inspect`,
`oriro plugins enable`, and `oriro plugins disable`. See
...
Plugin-managed internal hooks show up in `oriro hooks list` with
`plugin:<id>`. You cannot enable or disable them through `oriro hooks`;
enable or disable the plugin instead.

## Verify the active Gateway

`oriro plugins list` and plain `oriro plugins inspect` read cold config,
...
```bash
oriro gateway status --deep --require-rpc
oriro plugins inspect <plugin-id> --runtime --json
oriro gateway restart
```
...
sure any manual restart targets the actual `oriro gateway run` child that
...
| Symptom                                                        | Check                                                                                                                                      | Fix                                                                                                     |
| -------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------- |
| Plugin appears in `plugins list` but runtime hooks do not run  | Use `oriro plugins inspect <id> --runtime --json` and confirm the active Gateway with `gateway status --deep --require-rpc`             | Restart the live Gateway after install, update, config, or source changes                               |
| Duplicate channel or tool ownership diagnostics appear         | Run `oriro plugins list --enabled --verbose`, inspect each suspected plugin with `--runtime --json`, and compare channel/tool ownership | Disable one owner, remove stale installs, or use manifest `preferOver` for intentional replacement      |
...
| Config is invalid during install                               | Read the validation message and run `oriro doctor --fix` when it points to stale plugin state                                           | Doctor can quarantine invalid plugin config by disabling the entry and removing the invalid payload     |
| Plugin path is blocked for suspicious ownership or permissions | Inspect the diagnostic before the config error                                                                                             | Fix filesystem ownership/permissions, then run `oriro plugins registry --refresh`                    |
| `ORIRO_NIX_MODE=1` blocks lifecycle commands                | Confirm the install is managed by Nix                                                                                                      | Change plugin selection in the Nix source instead of using plugin mutator commands                      |
| Dependency import fails at runtime                             | Check whether the plugin was installed through npm/git/OriroHub or loaded from a local path                                                 | Run `oriro plugins update <id>`, reinstall the source, or install local plugin dependencies yourself |
...
other channel. Run `oriro doctor --fix` to remove stale plugin and channel
...
plugin id. If both plugins are explicitly enabled, Oriro keeps that request
...
Oriro needs at runtime. Update or reinstall after the publisher ships
...
and config validation follows with `plugin present but blocked`, Oriro found
...
Oriro as the same user that owns the state directory.

For Docker installs, the official image runs as `node` (uid `1000`), so the
host bind-mounted Oriro config and workspace directories should normally be
owned by uid `1000`:
...
If you intentionally run Oriro as root, repair the managed plugin root to
root ownership instead:

```bash
sudo chown -R root:root /path/to/oriro-config/npm
```

After fixing ownership, rerun `oriro doctor --fix` or
`oriro plugins registry --refresh` so the persisted plugin registry matches
the repaired files.
...
```bash
oriro config set logging.level trace
oriro logs --follow
```
...
Oriro caches successful plugin tool factory results for repeated resolutions
...
- [`oriro plugins`](/cli/plugins) - full CLI reference
...
- [Community plugins](/plugins/community) - OriroHub discovery and docs PR policy
