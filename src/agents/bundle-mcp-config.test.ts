/** Tests merging bundled MCP defaults with Oriro user MCP configuration. */
...
describe("loadMergedBundleMcpConfig", () => {
  it("lets Oriro mcp.servers override bundle defaults while preserving raw transport shape", () => {
  ...
  it("maps Oriro transports to downstream CLI types when requested", () => {
    expect(
  ...
  it("keeps disabled Oriro MCP servers out of embedded runtimes", () => {
    const merged = loadMergedBundleMcpConfig({
      workspaceDir: "/workspace",
      cfg: {
        mcp: {
  ...
  it("lets disabled Oriro MCP servers tombstone bundle defaults with the same name", () => {
    const merged = loadMergedBundleMcpConfig({
      workspaceDir: "/workspace",
      cfg: {
        mcp: {
