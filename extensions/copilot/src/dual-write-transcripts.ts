 ...
 * Oriro audit transcript that sits next to (but is distinct from) the
 * SDK's own session storage.
 *
 * The Oriro shell (src/agents/command/attempt-execution.ts) already
 ...
 * assistant + toolResult) into the Oriro transcript via the same
...
export function attachCopilotMirrorIdentity<T extends AgentMessage>(
  ...
  return {
    ...record,
    __oriro: { ...baseMeta, [MIRROR_IDENTITY_META_KEY]: identity },
  } as unknown as T;
}
 ...
 * the Oriro audit trail just misses the intermediate messages for
 * this turn.
 */
