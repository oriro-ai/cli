...
const SUPERVISOR_HINTS = {
  launchd: ["ORIRO_LAUNCHD_LABEL"],
  systemd: ["ORIRO_SYSTEMD_UNIT", "INVOCATION_ID", "SYSTEMD_EXEC_PID", "JOURNAL_STREAM"],
  schtasks: ["ORIRO_WINDOWS_TASK_NAME"],
} as const;
...
export interface DetectRespawnSupervisorOptions {
  includeLinuxOriroGatewayServiceMarker?: boolean;
}
...
function hasOriroGatewayServiceMarker(env: NodeJS.ProcessEnv): boolean {
  return (
    env.ORIRO_SERVICE_MARKER?.trim() === "oriro" &&
    env.ORIRO_SERVICE_KIND?.trim() === "gateway"
  );
}

function isCurrentGatewayLaunchdJob(env: NodeJS.ProcessEnv): boolean {
  const expectedLabel = resolveGatewayLaunchAgentLabel(env.ORIRO_PROFILE);
  if (
...
export function detectRespawnSupervisor(
  ...
  if (platform === "linux") {
    return hasAnyHint(env, SUPERVISOR_HINTS.systemd) ||
      (options.includeLinuxOriroGatewayServiceMarker === true &&
        hasOriroGatewayServiceMarker(env))
      ? "systemd"
      : null;
  }
  if (platform === "win32") {
    ...
    const marker = env.ORIRO_SERVICE_MARKER?.trim();
