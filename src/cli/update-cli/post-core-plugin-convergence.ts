...
import { relinkOriroPeerDependenciesInManagedNpmRoot } from "../../plugins/plugin-peer-link.js";
...
const REPAIR_GUIDANCE = "Run `oriro update repair` to retry plugin repair.";
const inspectGuidance = (pluginId: string) =>
  `Run \`oriro plugins inspect ${pluginId} --runtime --json\` for details.`;

async function repairManagedNpmOriroPeerLinks(params: {
...
export async function runPostCorePluginConvergence(params: {
  ...
  const peerLinkRepair = await repairManagedNpmOriroPeerLinks({ env });
 ...
 * resolve as a trusted-source-linked official install (npm or OriroHub)
...
export function filterRecordsToActive(params: {
  ...
  for (const [pluginId, record] of Object.entries(params.records)) {
    ...
    const officialOriroHub = resolveTrustedSourceLinkedOfficialOriroHubSpec({ pluginId, record });
    if (officialNpm || officialOriroHub) {
      filtered[pluginId] = record;
    }
  }
  return filtered;
