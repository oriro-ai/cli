...
describe("resolveMediaToolLocalRoots", () => {
  it("does not widen default local roots from media sources", () => {
    const stateDir = path.join("/tmp", "oriro-media-tool-roots-state");
    const picturesDir =
    ...
    const roots = withEnv({ ORIRO_STATE_DIR: stateDir }, () =>
  ...
  it("keeps channel inbound attachment roots separate from local roots", () => {
    ...
    const accountRoot = path.join("/tmp", "oriro-imessage-work");
    const sharedRoot = path.join("/tmp", "oriro-imessage-shared");
    const cfg = {
      channels: {
        imessage: {
