...
const DIAGNOSTICS_DOCS_URL = "https://docs.oriro.ai/gateway/diagnostics";
const GATEWAY_DIAGNOSTICS_EXPORT_JSON_LABEL = "oriro gateway diagnostics export --json";
...
function buildGatewayDiagnosticsExportJsonCommand(): string {
  return buildCurrentOriroCliCommand(["gateway", "diagnostics", "export", "--json"]);
}
...
async function requestGatewayDiagnosticsExportApproval(
  ...
  try {
    ...
    const result = await execTool.execute("chat-diagnostics-gateway-export", {
      command,
      env: buildCurrentOriroCliExecEnv(),
      security: "allowlist",
      ask: "always",
      background: true,
      timeout: timeoutSec,
