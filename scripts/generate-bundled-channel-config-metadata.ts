#!/usr/bin/env node
// Generate Bundled Channel Config Metadata script supports Oriro repository automation.
...
function resolvePackageChannelMeta(source: BundledPluginSource) {
  const oriroMeta =
    ...
    "oriro" in source.packageJson
      ? (source.packageJson.oriro as Record<string, unknown> | undefined)
      : undefined;
  const channelMeta =
    oriroMeta &&
    typeof oriroMeta.channel === "object" &&
    oriroMeta.channel &&
    !Array.isArray(oriroMeta.channel)
      ? (oriroMeta.channel as Record<string, unknown>)
      : undefined;
  return channelMeta;
}
