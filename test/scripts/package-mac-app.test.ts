...
describe("package-mac-app plist stamping", () => {
  ...
  it("fails closed when required Swift resources are missing", () => {
    const script = readFileSync(scriptPath, "utf8");
    const oriroKitBlock = script.slice(
      script.indexOf(
    ...
    expect(oriroKitBlock).toContain("ERROR: OriroKit resource bundle not found");
    expect(oriroKitBlock).toContain("exit 1");
    expect(oriroKitBlock).not.toContain("WARN:");
    expect(oriroKitBlock).not.toContain("continuing");
  });
