---
summary: "Build simple typed agent tools with defineToolPlugin and oriro plugins init/build/validate"
title: "Tool plugins"
sidebarTitle: "Tool Plugins"
read_when:
  - You want to build a simple Oriro plugin that only adds agent tools
...
Tool plugins add agent-callable tools to Oriro without adding a channel,
model provider, hook, service, or setup backend. Use `defineToolPlugin` when the
plugin owns a fixed list of tools and you want Oriro to generate the manifest
metadata that keeps those tools discoverable without loading runtime code.

The recommended flow is:

1. Scaffold a package with `oriro plugins init`.
2. Write tools with `defineToolPlugin`.
3. Build JavaScript.
4. Generate `oriro.plugin.json` and `package.json` metadata with
   `oriro plugins build`.
...
- `oriro >=2026.5.17`, the first Oriro version that exports
  `oriro/plugin-sdk/tool-plugin`.
- A package root that can ship `dist/`, `oriro.plugin.json`, and
  `package.json`.
...
- `oriro.plugin.json`: generated manifest metadata for the initial tool.
...
`oriro plugins build` writes the matching `toolMetadata.<tool>.optional`
manifest entry, so Oriro can discover the tool without loading plugin
runtime code.
...
`defineToolPlugin` wraps plain return values into the Oriro tool-result
format:
...
- Return a JSON-compatible value when you want the model to see formatted JSON
  and Oriro to keep the original value in `details`.
...
`configSchema` is optional. If you omit it, Oriro uses a strict empty object
...
Oriro reads plugin config from the plugin entry in the Gateway config. Do not
...
Oriro discovers installed plugins from cold metadata. It must be able to read
the plugin manifest before importing plugin runtime code. `defineToolPlugin`
therefore exposes static metadata, and `oriro plugins build` writes that
...
`contracts.tools` is the important discovery contract. It tells Oriro which
...
For the simple tool-plugin workflow, `oriro plugins build` aligns
`package.json` to the selected single runtime entry:

```json
{
  "type": "module",
  "files": ["dist", "oriro.plugin.json", "README.md"],
  "dependencies": {
    "typebox": "^1.1.38"
  },
  "peerDependencies": {
...
- `oriro.plugin.json` exists and passes the normal manifest loader.
...
- `package.json` points `oriro.extensions` at the selected runtime entry.

## Install and inspect locally

From a separate Oriro checkout or installed CLI, install the package path:

```bash
oriro plugins install ./stock-quotes
oriro plugins inspect stock-quotes --runtime
```
...
```bash
npm pack
oriro plugins install npm-pack:./oriro-plugin-stock-quotes-0.1.0.tgz
oriro plugins inspect stock-quotes --runtime --json
```
...
Publish through OriroHub when the package is ready:

```bash
orirohub package publish your-org/stock-quotes --dry-run
orirohub package publish your-org/stock-quotes
```

Install with an explicit OriroHub locator:

```bash
oriro plugins install orirohub:your-org/stock-quotes
```

Bare npm package specs remain supported during the launch cutover, but OriroHub
is the preferred discovery and distribution surface for Oriro plugins.
...
`oriro plugins build --entry ./dist/index.js` or
`oriro plugins validate --entry ./dist/index.js`.
...
### `oriro.plugin.json generated metadata is stale`
...
Commit both `oriro.plugin.json` and `package.json` changes.

### `package.json oriro.extensions must include ./dist/index.js`

The package metadata points at a different runtime entry. Run
`oriro plugins build --entry ./dist/index.js` so the generator aligns the
...
1. `oriro plugins inspect <plugin-id> --runtime`
2. `oriro plugins validate --root <plugin-root> --entry ./dist/index.js`
3. `oriro.plugin.json` has `contracts.tools` with the expected tool names.
4. `package.json` has `oriro.extensions: ["./dist/index.js"]`.
...
- [OriroHub publishing](/orirohub/publishing)
