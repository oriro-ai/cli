...
import { createOriroTestInstance, type OriroTestInstance } from "./oriro-test-instance.js";

export { extractFirstTextBlock };

export type GatewayInstance = OriroTestInstance;
...
export async function spawnGatewayInstance(name: string): Promise<GatewayInstance> {
  const inst = await createOriroTestInstance({ name });
  try {
