...
`,
  ...
  function expectWindowsRestartWaitOrdering(content: string, port = 18789) {
    const stateCheck = "$taskState = Get-OriroScheduledTaskState -TaskName $taskName";
    const runningGuard = 'if ($taskState -eq "Running")';
    const endCommand =
      'Invoke-OriroSchtasksWithTimeout -Arguments @("/End", "/TN", $taskName) -TimeoutSeconds 10';
    const skipEndLog = "oriro restart skipped schtasks end";
    const pollLoop = "for ($attempt = 1; $attempt -le 10; $attempt++)";
    const pollCall = `Get-OriroListenerPids -Port $port`;
    const forceKillBranch = "if ($attempt -eq 10)";
    const forceKillCommand = "Stop-Process -Id $listenerPid -Force";
    const runCommand =
      'Invoke-OriroSchtasksWithTimeout -Arguments @("/Run", "/TN", $taskName) -TimeoutSeconds 30';
  ...
  describe("prepareRestartScript", () => {
    it("creates a systemd restart script on Linux", async () => {
      ...
      expect(content).toContain("systemctl --user restart 'oriro-gateway.service'");
    ...
    it("creates restart scripts in a private temp directory with exclusive creation", async () => {
      ...
      const oldCandidatePath = path.join(os.tmpdir(), `oriro-restart-${timestamp}.sh`);
      const victimDir = await makeTempDir("oriro-restart-helper-victim-");
      ...
      try {
        ...
        expect(path.basename(scriptDir)).toMatch(/^oriro-restart-/);
    ...
    it("uses ORIRO_SYSTEMD_UNIT override for systemd scripts", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        ORIRO_PROFILE: "default",
        ORIRO_SYSTEMD_UNIT: "custom-gateway",
      });
    ...
    it("fails with sudo systemd guidance when the gateway unit is system-scoped", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const tmpDir = await makeTempDir("oriro-restart-helper-");
...
`,
      ...
      const { scriptPath } = await prepareAndReadScript({
        ...
        ORIRO_STATE_DIR: path.join(tmpDir, "state"),
      });
      const result = await executeScript(scriptPath, {
        PATH: `${fakeBinDir}:${process.env.PATH ?? ""}`,
        ORIRO_SYSTEMCTL_CALLS: callsPath,
      });
      const calls = await fs.readFile(callsPath, "utf-8");

      expect(result.code).toBe(78);
      expect(result.stderr).toContain("system-scoped oriro gateway unit detected");
      expect(result.stderr).toContain("sudo systemctl restart oriro-gateway.service");
      expect(calls).toContain("--user is-active --quiet oriro-gateway.service");
      expect(calls).toContain("is-active --quiet oriro-gateway.service");
      expect(calls).not.toContain("--user restart oriro-gateway.service");
    });
    ...
    it("creates a launchd restart script on macOS", async () => {
      ...
      expect(scriptPath.endsWith(".sh")).toBe(true);
      expect(content).toContain("#!/bin/sh");
      expect(content).toContain("launchctl kickstart -k 'gui/501/ai.oriro.gateway'");
      // Should clear disabled state and fall back to bootstrap when kickstart fails.
      expect(content).toContain("launchctl enable 'gui/501/ai.oriro.gateway'");
    ...
    it("captures macOS launchctl stderr to ~/.oriro/logs/gateway-restart.log (#68486)", async () => {
      ...
      expect(content).toContain("exec >>'/Users/testuser/.oriro/logs/gateway-restart.log' 2>&1");
    ...
    it("uses ORIRO_STATE_DIR for the macOS update restart log", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;
      ...
      expect(content).toContain(
        "if mkdir -p '/tmp/oriro-state/logs' 2>/dev/null && : >>'/tmp/oriro-state/logs/gateway-restart.log' 2>/dev/null; then",
      );
      expect(content).toContain("exec >>'/tmp/oriro-state/logs/gateway-restart.log' 2>&1");
      await cleanupScript(scriptPath);
    });
...
`,
      ...
      expect(log).toContain("oriro restart attempt source=update target=ai.oriro.gateway");
      expect(log).toContain("launchctl kickstart -k gui/501/ai.oriro.gateway");
      expect(log).toContain("oriro restart failed source=update status=42");
      expect(log).not.toContain("oriro restart done source=update");
    });
...
`,
    ...
    it("logs custom macOS launchd labels without shell expansion", async () => {
      ...
      const { scriptPath } = await prepareAndReadScript({
        ORIRO_LAUNCHD_LABEL: "ai.oriro.$(echo injected)",
        HOME: path.join(tmpDir, "home"),
        ORIRO_STATE_DIR: stateDir,
      });
      ...
      expect(result.code).toBeNull();
      expect(log).toContain("target=ai.oriro.$(echo injected)");
      expect(log).not.toContain("target=ai.oriro.injected");
    });

    it("uses ORIRO_LAUNCHD_LABEL override on macOS", async () => {
      Object.defineProperty(process, "platform", { value: "darwin" });
      process.getuid = () => 501;

      const { scriptPath, content } = await prepareAndReadScript({
        ORIRO_PROFILE: "default",
        ORIRO_LAUNCHD_LABEL: "com.custom.oriro",
      });
      expect(content).toContain("launchctl kickstart -k 'gui/501/com.custom.oriro'");
      await cleanupScript(scriptPath);
    });
    ...
    it("creates a guarded schtasks restart script on Windows", async () => {
      ...
      expect(content).toContain("$taskName = 'Oriro Gateway'");
      expect(content).toContain("function Invoke-OriroSchtasksWithTimeout");
      expect(content).toContain("function Get-OriroScheduledTaskState");
      expect(content).toContain("function Invoke-OriroStartupLauncher");
      expect(content).toContain("Get-ScheduledTask -TaskName $TaskName");
      expect(content).toContain("oriro restart skipped schtasks end");
      expect(content).toContain(
        '$launcherPath = Join-Path $env:USERPROFILE ".oriro\\gateway.cmd"',
      );
      expect(content).toContain("oriro restart launched startup fallback");
      expectWindowsRestartWaitOrdering(content);
      expect(content).toContain('del "%~f0" >nul 2>&1');
      expect(content).toContain('rmdir "%ORIRO_RESTART_SCRIPT_DIR%" >nul 2>&1');
      await cleanupScript(scriptPath);
    });

    it("uses ORIRO_WINDOWS_TASK_NAME override on Windows", async () => {
      Object.defineProperty(process, "platform", { value: "win32" });

      const { scriptPath, content } = await prepareAndReadScript({
        ORIRO_PROFILE: "default",
        ORIRO_WINDOWS_TASK_NAME: "Oriro Gateway (custom)",
      });
      expect(content).toContain("$taskName = 'Oriro Gateway (custom)'");
      expect(content).toContain("Get-OriroScheduledTaskState -TaskName $taskName");
      expect(content).toContain(
        'Invoke-OriroSchtasksWithTimeout -Arguments @("/End", "/TN", $taskName) -TimeoutSeconds 10',
      );
      expect(content).toContain("$status = Invoke-OriroStartupLauncher");
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in service names", async () => {
      Object.defineProperty(process, "platform", { value: "linux" });
      const { scriptPath, content } = await prepareAndReadScript({
        ORIRO_PROFILE: "production",
      });
      expect(content).toContain("oriro-gateway-production.service");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in macOS launchd label", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        ORIRO_PROFILE: "staging",
      });
      expect(content).toContain("gui/502/ai.oriro.staging");
      await cleanupScript(scriptPath);
    });
    ...
    it("uses custom profile in Windows task name", async () => {
      ...
      expect(content).toContain("$taskName = 'Oriro Gateway (production)'");
      expectWindowsRestartWaitOrdering(content);
      await cleanupScript(scriptPath);
    });
    ...
    it("shell-escapes the label in the plist path on macOS", async () => {
      ...
      const { scriptPath, content } = await prepareAndReadScript({
        HOME: "/Users/testuser",
        ORIRO_LAUNCHD_LABEL: "ai.oriro.it's-a-test",
      });
      // The plist path must also shell-escape the label to prevent injection
      expect(content).toContain("ai.oriro.it'\\''s-a-test.plist");
      await cleanupScript(scriptPath);
    });
