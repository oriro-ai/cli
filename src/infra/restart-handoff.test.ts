...
type GatewayRestartHandoffDatabase = Pick<OriroStateKyselyDatabase, "gateway_restart_handoff">;

function createHandoffEnv(): NodeJS.ProcessEnv {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-restart-handoff-"));
  tempDirs.push(dir);
  return {
    ...process.env,
    ORIRO_STATE_DIR: dir,
...
function legacyHandoffPath(env: NodeJS.ProcessEnv): string {
  return path.join(env.ORIRO_STATE_DIR ?? "", "gateway-supervisor-restart-handoff.json");
}
