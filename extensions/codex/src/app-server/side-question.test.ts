...
import { nativeHookRelayTesting } from "oriro/plugin-sdk/agent-harness-runtime";
import {
...
const createOriroCodingToolsMock = vi.fn();
...
vi.mock("oriro/plugin-sdk/agent-harness", () => ({
  createOriroCodingTools: (...args: unknown[]) => createOriroCodingToolsMock(...args),
}));
...
async function runSideQuestionWithManagedWebSearchCall(
  ...
  if (!options.preserveToolFactory) {
    createOriroCodingToolsMock.mockReturnValue([
      {
        name: "web_search",
...
describe("runCodexAppServerSideQuestion", () => {
  beforeEach(() => {
    ...
    createOriroCodingToolsMock.mockReset();
  ...
  it("forks an ephemeral side thread and returns the completed assistant text", async () => {
    ...
    const [toolOptions] = mockCall(createOriroCodingToolsMock);
  ...
  it("disables hosted search when side-question sender policy removes managed web_search", async () => {
    createOriroCodingToolsMock.mockImplementation((options: { senderId?: string }) =>
  ...
  it("applies native search restrictions to side forks and suppresses managed search", async () => {
    ...
    expect(toolResponse).toEqual({
      success: false,
      contentItems: [{ type: "inputText", text: "Unknown Oriro tool: web_search" }],
    });
  ...
  it("preserves managed web_search while planning hosted search for Responses side questions", async () => {
    createOriroCodingToolsMock.mockImplementation(
  ...
  it("rejects /btw before forking when the current Oriro session is sandboxed", async () => {
    await expect(
      runCodexAppServerSideQuestion(
        sideParams({
      ...
      "Codex-native /btw side-question mode is unavailable because Oriro sandboxing is active for this session.",
    );
  ...
  it("rejects /btw before forking when exec host=node is active", async () => {
    await expect(
      ...
      "Codex-native /btw side-question mode is unavailable because Oriro exec host=node is active for this session.",
    );
  ...
  it("bridges side-thread dynamic tool requests to Oriro tools", async () => {
    const client = createFakeClient();
    let toolResponse: unknown;
    client.request.mockImplementation(async (method: string) => {
      if (method === "thread/fork") {
  ...
  it("normalizes hook channel ids for side-thread dynamic tool requests", async () => {
    ...
    expect(createOriroCodingToolsMock).toHaveBeenCalledWith(
