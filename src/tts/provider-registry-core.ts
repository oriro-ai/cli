...
export type SpeechProviderRegistryResolver = {
  getProvider: (providerId: string, cfg?: OriroConfig) => SpeechProviderPlugin | undefined;
  listProviders: (cfg?: OriroConfig) => SpeechProviderPlugin[];
};
...
export function createSpeechProviderRegistry(resolver: SpeechProviderRegistryResolver) {
  const buildResolvedProviderMaps = (cfg?: OriroConfig) =>
    buildCapabilityProviderMaps(resolver.listProviders(cfg));

  const listProviders = (cfg?: OriroConfig): SpeechProviderPlugin[] => [
