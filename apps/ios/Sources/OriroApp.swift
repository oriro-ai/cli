...
@MainActor
enum OriroAppModelRegistry {
    static var appModel: NodeAppModel?
}

@MainActor
final class OriroAppDelegate: NSObject, UIApplicationDelegate, @preconcurrency UNUserNotificationCenterDelegate {
    private let logger = Logger(subsystem: "ai.orirofoundation.app", category: "Push")
    private let backgroundWakeLogger = Logger(subsystem: "ai.orirofoundation.app", category: "BackgroundWake")
    ...
    private func resolvedAppModel() -> NodeAppModel? {
        self.appModel ?? OriroAppModelRegistry.appModel
    }
...
enum WatchPromptNotificationBridge {
    static let typeKey = "oriro.type"
    static let typeValue = "watch.prompt"
    static let promptIDKey = "oriro.watch.promptId"
    static let sessionKeyKey = "oriro.watch.sessionKey"
    static let actionPrimaryIDKey = "oriro.watch.action.primary.id"
    static let actionPrimaryLabelKey = "oriro.watch.action.primary.label"
    static let actionSecondaryIDKey = "oriro.watch.action.secondary.id"
    static let actionSecondaryLabelKey = "oriro.watch.action.secondary.label"
    static let actionPrimaryIdentifier = "oriro.watch.action.primary"
    static let actionSecondaryIdentifier = "oriro.watch.action.secondary"
    static let actionIdentifierPrefix = "oriro.watch.action."
    static let actionIDKeyPrefix = "oriro.watch.action.id."
    static let actionLabelKeyPrefix = "oriro.watch.action.label."
    static let categoryPrefix = "oriro.watch.prompt.category."

    @MainActor
    static func scheduleMirroredWatchPromptNotificationIfNeeded(
        invokeID: String,
        params: OriroWatchNotifyParams,
        sendResult: WatchNotificationSendResult) async
    {
        ...
        let normalizedActions = (params.actions ?? []).compactMap { action -> OriroWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return OriroWatchAction(id: id, label: label, style: action.style)
        }
        ...
        content.title = title.isEmpty ? "Oriro" : title
        content.body = body
    ...
    private static func categoryActions(_ actions: [OriroWatchAction]) -> [UNNotificationAction] {
...
struct OriroApp: App {
    ...
    @UIApplicationDelegateAdaptor(OriroAppDelegate.self) private var appDelegate
    ...
    init() {
        ...
        #endif
        OriroAppModelRegistry.appModel = appModel
    ...
    private static var screenshotModeEnabled: Bool {
        #if DEBUG
        ProcessInfo.processInfo.arguments.contains("--oriro-screenshot-mode")
        #else
        false
        #endif
    }
    ...
    private static var screenshotNotificationGuidanceEnabled: Bool {
        #if DEBUG
        ProcessInfo.processInfo.arguments.contains("--oriro-screenshot-notification-guidance")
        #else
        false
        #endif
    }
...
extension OriroApp {
    ...
    private static func installUncaughtExceptionLogger() {
        NSLog("Oriro: installing uncaught exception handler")
