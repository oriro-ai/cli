...
describe("resolveDiffsPluginViewerBaseUrl", () => {
  ...
  it("normalizes configured viewer base URLs", () => {
    expect(
      resolveDiffsPluginViewerBaseUrl({
        viewerBaseUrl: "https://example.com/oriro/",
      }),
    ).toBe("https://example.com/oriro");
  });
});
...
describe("diffs plugin schema surfaces", () => {
  it("rejects invalid viewerBaseUrl values at manifest-validation time too", () => {
    ...
    expect(validate({ viewerBaseUrl: "https://example.com/oriro?x=1" })).toBe(false);
    expect(validate({ viewerBaseUrl: "https://example.com/oriro#frag" })).toBe(false);
    expect(validate({ viewerBaseUrl: "https://example.com/oriro/" })).toBe(true);
  });
  ...
  it("preserves defaults and security for direct safeParse callers", () => {
    ...
    expect(data.viewerBaseUrl).toBe("https://example.com/oriro");
...
describe("diffs viewer URL helpers", () => {
  ...
  it("joins viewer path under baseUrl pathname", () => {
    expect(
      buildViewerUrl({
        config: {},
        baseUrl: "https://example.com/oriro",
        viewerPath: "/plugins/diffs/view/id/token",
      }),
    ).toBe("https://example.com/oriro/plugins/diffs/view/id/token");
  });
...
describe("viewer assets", () => {
  ...
  it("serves the runtime bundle body", async () => {
    ...
    expect(String(runtime?.body)).toContain("oriroDiffsReady");
...
describe("resolveDiffsLanguagePackAvailability", () => {
  it.each(["assets", "dist/assets"])(
    ...
    (assetDir) => {
      const root = fs.mkdtempSync(join(os.tmpdir(), "oriro-diffs-language-pack-"));
      try {
        ...
        fs.writeFileSync(
          join(languagePackRoot, "oriro.plugin.json"),
