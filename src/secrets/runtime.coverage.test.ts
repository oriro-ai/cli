...
function createCoverageWebSearchProvider(params: {
  ...
  const readConfiguredCredential = (config?: OriroConfig): unknown =>
...
type SecretRegistryEntry = {
  id: string;
  configFile: "oriro.json" | "auth-profiles.json";
  pathPattern: string;
...
const DEBUG_COVERAGE_BATCHES = process.env.ORIRO_DEBUG_RUNTIME_COVERAGE === "1";
...
const PLUGIN_OWNED_ORIRO_COVERAGE_EXCLUSIONS = new Set([
...
const previousTrustBundledPluginsDir = process.env.ORIRO_TEST_TRUST_BUNDLED_PLUGINS_DIR;

process.env.ORIRO_BUNDLED_PLUGINS_DIR ??= "extensions";
process.env.ORIRO_TEST_TRUST_BUNDLED_PLUGINS_DIR ??= "1";
...
afterAll(() => {
  ...
  if (previousTrustBundledPluginsDir === undefined) {
    ...
    process.env.ORIRO_TEST_TRUST_BUNDLED_PLUGINS_DIR = previousTrustBundledPluginsDir;
  }
});
...
function addCoveragePluginLoadPath(config: OriroConfig, pluginId: string): void {
...
function collectOriroCoverageEntries(options: {
  includePluginEntries: boolean;
}): SecretRegistryEntry[] {
  return COVERAGE_REGISTRY_ENTRIES.filter(
    (entry) =>
      entry.configFile === "oriro.json" &&
      entry.id.startsWith("plugins.entries.") === options.includePluginEntries &&
      !PLUGIN_OWNED_ORIRO_COVERAGE_EXCLUSIONS.has(entry.id),
  );
}

function applyConfigForOriroTarget(
  config: OriroConfig,
  entry: SecretRegistryEntry,
  envId: string,
  wildcardToken: string,
...
async function expectOriroCoverageBatchResolved(
  label: string,
  ...
  for (const [index, entry] of batch.entries()) {
    const envId = toCoverageEnvRefId("ORIRO_SECRET_TARGET", entry.id);
    ...
    applyConfigForOriroTarget(config, entry, envId, wildcardToken);
  }
...
const ORIRO_CORE_COVERAGE_BATCHES = buildCoverageBatches(
  collectOriroCoverageEntries({ includePluginEntries: false }),
);
const ORIRO_PLUGIN_COVERAGE_BATCHES = buildCoverageBatches(
  collectOriroCoverageEntries({ includePluginEntries: true }),
);
...
describe("secrets runtime target coverage", () => {
  beforeAll(async () => {
    ...
    const googleChatBatch = ORIRO_CORE_COVERAGE_BATCHES.find((batch) =>
      batch.some((entry) => entry.id === "channels.googlechat.serviceAccount"),
    );
    if (googleChatBatch) {
      await expectOriroCoverageBatchResolved("oriro.json core", googleChatBatch);
    }
  });

  describe("oriro.json core and channel registry targets", () => {
    test.each(ORIRO_CORE_COVERAGE_BATCHES.map(toCoverageBatchCase))(
      "handles $name",
      async ({ batch }) => {
        await expectOriroCoverageBatchResolved("oriro.json core", batch);
      },
      RUNTIME_COVERAGE_TEST_TIMEOUT_MS,
    );
  });

  describe("oriro.json plugin registry targets", () => {
    test.each(ORIRO_PLUGIN_COVERAGE_BATCHES.map(toCoverageBatchCase))(
      "handles $name",
      async ({ batch }) => {
        await expectOriroCoverageBatchResolved("oriro.json plugins", batch);
      },
      RUNTIME_COVERAGE_TEST_TIMEOUT_MS,
    );
  });
  ...
  describe("auth-profiles registry targets", () => {
    test.each(AUTH_PROFILE_COVERAGE_BATCHES.map(toCoverageBatchCase))(
      ...
      async ({ batch }) => {
        ...
        for (const [index, entry] of batch.entries()) {
          const envId = toCoverageEnvRefId("ORIRO_AUTH_SECRET_TARGET", entry.id);
        ...
        const snapshot = await prepareAuthCoverageSnapshot({
          ...
          env,
          agentDirs: ["/tmp/oriro-agent-main"],
