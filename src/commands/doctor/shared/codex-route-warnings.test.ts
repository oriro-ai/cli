...
describe("collectCodexRouteWarnings", () => {
  ...
  it("warns when openai-codex primary models still use the legacy route", () => {
    ...
    expect(warnings).toStrictEqual([
      [
        ...
        "- Run `oriro doctor --fix`: it rewrites configured model refs and stale sessions to `openai/*`, moves Codex intent to provider/model runtime policy, and clears old whole-agent runtime pins.",
      ].join("\n"),
    ]);
  });
  ...
  it("ignores ORIRO_AGENT_RUNTIME when reporting legacy model refs", () => {
    const warnings = collectCodexRouteWarnings({
      cfg: {
        agents: {
          defaults: {
  ...
  it("warns when Codex app-server command includes inline arguments", () => {
    ...
    expect(warnings).toStrictEqual([
      [
        ...
        '- plugins.entries.codex.config.appServer.command: "node C:\\Users\\me\\.oriro\\npm\\node_modules\\@openai\\codex\\bin\\codex.js" starts with "node" and embeds "C:\\Users\\me\\.oriro\\npm\\node_modules\\@openai\\codex\\bin\\codex.js". The command field must be only the executable path.',
  ...
  it("warns when Codex runtime has Oriro compaction summarizer overrides", () => {
    const warnings = collectCodexRouteWarnings({
      cfg: {
        agents: {
          defaults: {
    ...
    expect(warnings).toStrictEqual([
      [
        ...
        "- Run `oriro doctor --fix`: it removes unsupported Codex compaction overrides.",
      ].join("\n"),
    ]);
  });
  ...
  it("migrates legacy Lossless compaction config to the context-engine slot", () => {
    ...
    expect(result.warnings).toStrictEqual([]);
    expect(result.cfg.plugins?.slots?.contextEngine).toBe("lossless-oriro");
    expect(result.cfg.plugins?.entries?.["lossless-oriro"]).toEqual({
      enabled: true,
      config: {
    ...
    expect(result.changes).toContain(
      'Set plugins.slots.contextEngine to "lossless-oriro" for legacy Lossless compaction config.',
    );
    expect(result.changes).toContain(
  ...
  it("does not migrate mixed Lossless provider-only and summary-model consumers", () => {
    ...
    expect(result.warnings).toStrictEqual([
      [
        ...
        "- agents.defaults.compaction.provider: lossless-oriro should become plugins.slots.contextEngine: lossless-oriro.",
        "- agents.list.fast.compaction.model: openai/gpt-5.4-mini should become plugins.entries.lossless-oriro.config.summaryModel.",
  ...
  it("preserves Codex runtime policy for migrated Lossless summary models", () => {
    ...
    expect(result.warnings).toStrictEqual([]);
    expect(result.cfg.plugins?.entries?.["lossless-oriro"]?.config).toEqual({
      summaryModel: "openai/gpt-5.4-mini",
    });
  ...
  it("does not collapse conflicting per-agent Lossless summary models", () => {
    ...
    expect(result.warnings).toStrictEqual([
      [
        "- Legacy Lossless compaction config should use the Lossless context-engine slot for Codex.",
        "- agents.list.fast.compaction.provider: lossless-oriro should become plugins.slots.contextEngine: lossless-oriro.",
        "- agents.list.fast.compaction.model: openai/gpt-5.4-mini should become plugins.entries.lossless-oriro.config.summaryModel.",
        "- agents.list.deep.compaction.provider: lossless-oriro should become plugins.slots.contextEngine: lossless-oriro.",
        "- agents.list.deep.compaction.model: openai/gpt-5.5 should become plugins.entries.lossless-oriro.config.summaryModel.",
        "- Move the Lossless config manually; doctor will not overwrite an existing non-Lossless context-engine slot or collapse conflicting per-agent summary models.",
      ].join("\n"),
    ]);
  });
  ...
  it("does not overwrite a non-Lossless context-engine slot", () => {
    ...
    expect(result.warnings).toStrictEqual([
      [
        "- Legacy Lossless compaction config should use the Lossless context-engine slot for Codex.",
        "- agents.defaults.compaction.provider: lossless-oriro should become plugins.slots.contextEngine: lossless-oriro.",
        "- agents.defaults.compaction.model: openai/gpt-5.4 should become plugins.entries.lossless-oriro.config.summaryModel.",
        "- Move the Lossless config manually; doctor will not overwrite an existing non-Lossless context-engine slot or collapse conflicting per-agent summary models.",
      ].join("\n"),
    ]);
  });
  ...
  it("preserves Codex runtime policy for blocked Lossless summary rewrites", () => {
    ...
    expect(result.warnings).toStrictEqual([
      [
        "- Legacy Lossless compaction config should use the Lossless context-engine slot for Codex.",
        "- agents.defaults.compaction.provider: lossless-oriro should become plugins.slots.contextEngine: lossless-oriro.",
        "- agents.defaults.compaction.model: openai/gpt-5.4-mini should become plugins.entries.lossless-oriro.config.summaryModel.",
        "- Move the Lossless config manually; doctor will not overwrite an existing non-Lossless context-engine slot or collapse conflicting per-agent summary models.",
      ].join("\n"),
    ]);
  });
  ...
  it("does not migrate shared Lossless summary models inherited by non-Codex agents", () => {
    ...
    expect(result.warnings).toStrictEqual([
      [
        "- Legacy Lossless compaction config should use the Lossless context-engine slot for Codex.",
        "- agents.list.codex.compaction.provider: lossless-oriro should become plugins.slots.contextEngine: lossless-oriro.",
        "- agents.defaults.compaction.model: openai/gpt-5.4 should become plugins.entries.lossless-oriro.config.summaryModel.",
        "- Move the Lossless config manually; doctor will not overwrite an existing non-Lossless context-engine slot or collapse conflicting per-agent summary models.",
      ].join("\n"),
    ]);
          ...
          list: [
            {
              ...
              model: "anthropic/claude-sonnet-4-6",
              models: {
                "anthropic/claude-sonnet-4-6": { agentRuntime: { id: "oriro" } },
              },
            },
          ],
        },
  ...
  it("keeps the Codex plugin disabled when OpenAI routes explicitly use the Oriro runtime", () => {
    const result = maybeRepairCodexRoutes({
      cfg: {
        plugins: {
          entries: {
  ...
  it("keeps the Codex plugin disabled when an auth-profiled OpenAI route explicitly uses the Oriro runtime", () => {
    const result = maybeRepairCodexRoutes({
      cfg: {
        plugins: {
          entries: {
  ...
  it("keeps repaired OpenAI refs on Codex runtime even when the OpenAI provider is otherwise Oriro/API-key routed", () => {
    const result = maybeRepairCodexRoutes({
      cfg: {
        models: {
          providers: {
  ...
  it("preserves provider-level legacy runtime pins while repairing default legacy refs", () => {
    ...
    expect(result.changes.join("\n")).toContain(
      'Set agents.defaults.models.openai/gpt-5.4.agentRuntime.id to "oriro"',
    );
    expect(
      resolveAgentHarnessPolicy({
        provider: "openai",
  ...
  it("preserves legacy provider catalog runtime pins while repairing listed-agent legacy refs", () => {
    ...
    expect(result.changes.join("\n")).toContain(
      'Set agents.list.worker.models.openai/gpt-5.4.agentRuntime.id to "oriro"',
    );
    expect(
      resolveAgentHarnessPolicy({
        provider: "openai",
  ...
  it("preserves explicit Oriro runtime pins while repairing legacy session routes", () => {
    const store: Record<string, SessionEntry> = {
      main: {
        sessionId: "s1",
        updatedAt: 1,
  ...
  it("clears stale Codex overrides while preserving explicit Oriro session pins", () => {
    const store: Record<string, SessionEntry> = {
      main: {
        sessionId: "s1",
        updatedAt: 1,
  ...
  it("preserves canonical OpenAI sessions that are explicitly pinned to Oriro", () => {
    const store: Record<string, SessionEntry> = {
      main: {
        sessionId: "s1",
        updatedAt: 1,
        ...
        agentHarnessId: "oriro",
        agentRuntimeOverride: "oriro",
        authProfileOverride: "openai:work",
      },
    };
    ...
    expect(result).toEqual({ changed: false, sessionKeys: [] });
    expect(store.main.updatedAt).toBe(1);
    expect(store.main.agentHarnessId).toBe("oriro");
    expect(store.main.agentRuntimeOverride).toBe("oriro");
  ...
  it("preserves an explicit non-default agentRuntime pin on the legacy model entry during migration (#84038)", () => {
    ...
    expect(migrated?.agentRuntime).toEqual({ id: "oriro" });
    expect(result.cfg.agents?.defaults?.models?.["openai-codex/gpt-5.4"]).toBeUndefined();
  });
