...
read_when:
  - Connecting Oriro to a ClickClack workspace
  - Testing ClickClack bot identities
title: "ClickClack"
---

ClickClack connects Oriro to a self-hosted ClickClack workspace through first-class ClickClack bot tokens.

Use this when you want an Oriro agent to appear as a ClickClack bot user. ClickClack supports independent service bots and user-owned bots; user-owned bots keep an `owner_user_id` and receive only the token scopes you grant.
...
ClickClack in channel setup or running `oriro plugins enable clickclack`
...
global `plugins.enabled: false` setting. Direct `oriro plugins install
...
When an account sets `agentId`, Oriro requires the explicit
...
```bash
oriro message send --channel clickclack --target channel:general --message "hello"
oriro message send --channel clickclack --target dm:usr_123 --message "hello"
oriro message send --channel clickclack --target thread:msg_123 --message "following up"
```
...
Oriro only needs `bot:write` for normal agent chat.
