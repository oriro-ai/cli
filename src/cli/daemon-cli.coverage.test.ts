...
const buildGatewayInstallPlan = vi.fn(
  async (params: {
    ...
    environment: {
      ORIRO_GATEWAY_PORT: String(params.port),
      ...(params.wrapperPath ? { ORIRO_WRAPPER: params.wrapperPath } : {}),
      ...(params.token ? { ORIRO_GATEWAY_TOKEN: params.token } : {}),
    },
  }),
);
...
describe("daemon-cli coverage", () => {
  ...
  beforeEach(() => {
    ...
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-daemon-cli-"));
    ...
    process.env.ORIRO_STATE_DIR = tmpDir;
    process.env.ORIRO_CONFIG_PATH = path.join(tmpDir, "oriro.json");
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        ORIRO_PROFILE: "dev",
        ORIRO_STATE_DIR: "/tmp/oriro-daemon-state",
        ORIRO_CONFIG_PATH: "/tmp/oriro-daemon-state/oriro.json",
        ORIRO_GATEWAY_PORT: "19001",
      },
      sourcePath: "/tmp/ai.oriro.gateway.plist",
    });
  ...
  it("passes the existing service environment into the install plan on forced reinstall", async () => {
    ...
    expect((installPlanParams.env as NodeJS.ProcessEnv).ORIRO_WRAPPER).toBe(
      "/usr/local/bin/oriro-doppler",
    );
  });
