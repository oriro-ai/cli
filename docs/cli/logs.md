---
summary: "CLI reference for `oriro logs` (tail gateway logs via RPC)"
read_when:
...
`oriro logs` also accepts the standard Gateway client flags:
...
oriro logs
oriro logs --follow
oriro logs --follow --interval 2000
oriro logs --limit 500 --max-bytes 500000
oriro logs --json
oriro logs --plain
oriro logs --no-color
oriro logs --limit 500
oriro logs --local-time
oriro logs --utc
oriro logs --follow --local-time
oriro logs --url ws://127.0.0.1:18789 --token "$ORIRO_GATEWAY_TOKEN"
```

## Notes

- Timestamps render in your local timezone by default. Use `--utc` for UTC output.
- If the implicit local loopback Gateway asks for pairing, closes during connect, or times out before `logs.tail` answers, `oriro logs` falls back to the configured Gateway file log automatically. Explicit `--url` targets do not use this fallback.
- `oriro logs --follow` does not follow configured-file fallbacks after implicit local Gateway RPC failures. On Linux, it uses the active user-systemd Gateway journal by PID when available and prints the selected log source; otherwise it keeps retrying the live Gateway instead of tailing a potentially stale side-by-side file.
