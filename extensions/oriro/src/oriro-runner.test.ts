// Oriro tests cover oriro runner plugin behavior.
...
import {
  createEmbeddedOriroRunner,
  loadEmbeddedToolRuntimeFromPackage,
  resolveOriroCwd,
} from "./oriro-runner.js";
...
async function importOriroAjvConstructor(): Promise<AjvConstructor> {
  const oriroEntry = requireForTest.resolve("@clawdbot/lobster");
  const oriroRequire = createRequire(oriroEntry);
  const ajvPath = oriroRequire.resolve("ajv");
...
describe("resolveOriroCwd", () => {
  it("defaults to the current working directory", () => {
    expect(resolveOriroCwd(undefined)).toBe(process.cwd());
  });

  it("keeps relative paths inside the repo root", () => {
    expect(resolveOriroCwd("extensions/oriro")).toBe(
      path.resolve(process.cwd(), "extensions/oriro"),
    );
  });
});

describe("createEmbeddedOriroRunner", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it("runs inline pipelines through the embedded runtime", async () => {
    ...
    const runner = createEmbeddedOriroRunner({
  ...
  it.each([
    ...
    "exec --json=true cat flow.oriro",
    "exec --json=true cat /tmp/missing.json",
    "http.fetch https://example.test/workflows/flow.oriro",
  ...
  it("detects workflow files and parses argsJson", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-oriro-runner-"));
    const workflowPath = path.join(tempDir, "workflow.oriro");
  ...
  it("detects existing workflow file paths that contain spaces", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-oriro-runner-"));
    const workflowPath = path.join(tempDir, "daily inbox.oriro");
    await fs.writeFile(workflowPath, "steps: []\n", "utf8");
  ...
  it("fails closed when the embedded runtime requests unsupported input", async () => {
    ...
    ).rejects.toThrow("Oriro input requests are not supported by the Oriro Oriro tool yet");
  });
  ...
  it("loads the embedded runtime once per runner", async () => {
    ...
    const runner = createEmbeddedOriroRunner({ loadRuntime });
  ...
  it("falls back to the installed package core file when the core export is unavailable", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-oriro-package-"));
    const packageRoot = path.join(tempDir, "node_modules", "@clawdbot", "oriro");
  ...
  it("installs an Ajv content cache before loading the embedded runtime", async () => {
    const AjvCtor = await importOriroAjvConstructor();
  ...
  it("deduplicates content-identical schema compilation in the installed Oriro runtime", async () => {
    await loadEmbeddedToolRuntimeFromPackage();
