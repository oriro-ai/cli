...
const asConfig = (cfg: OriroConfig): OriroConfig => cfg;
...
describe("memory search config", () => {
  ...
  function configWithDefaultProvider(provider: string): OriroConfig {
    return asConfig({
      agents: {
        defaults: {
          memorySearch: {
  ...
  it("defaults provider to openai when unspecified", () => {
    ...
    expect(resolved?.store.databasePath).toBe(resolveOriroAgentSqlitePath({ agentId: "main" }));
  });
