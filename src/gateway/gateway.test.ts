...
async function createEmptyBundledPluginsDir(tempHome: string): Promise<string> {
  const bundledPluginsDir = path.join(tempHome, "oriro-test-empty-bundled-plugins");
  await fs.mkdir(bundledPluginsDir, { recursive: true });
  return bundledPluginsDir;
}

async function createGatewayConfigPath(tempHome: string): Promise<string> {
  const configPath = path.join(tempHome, ".oriro", "oriro.json");
...
async function writeWorkspacePlugin(params: {
  ...
  const pluginDir = path.join(params.workspaceDir, ".oriro", "extensions", params.id);
...
async function setupGatewayTempHome(params: { prefix: string; minimalGateway?: boolean }) {
  const envSnapshot = captureEnv([
    ...
    ...(params.minimalGateway ? (["ORIRO_TEST_MINIMAL_GATEWAY"] as const) : []),
  ]);

  const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), params.prefix));
  setTestEnvValue("HOME", tempHome);
  setTestEnvValue("ORIRO_STATE_DIR", path.join(tempHome, ".oriro"));
  deleteTestEnvValue("ORIRO_CONFIG_PATH");
  setTestEnvValue("ORIRO_SKIP_CHANNELS", "1");
  setTestEnvValue("ORIRO_SKIP_GMAIL_WATCHER", "1");
  setTestEnvValue("ORIRO_SKIP_CRON", "1");
  setTestEnvValue("ORIRO_SKIP_CANVAS_HOST", "1");
  setTestEnvValue("ORIRO_SKIP_BROWSER_CONTROL_SERVER", "1");
  setTestEnvValue("ORIRO_SKIP_PROVIDERS", "1");
  if (params.minimalGateway) {
    setTestEnvValue("ORIRO_TEST_MINIMAL_GATEWAY", "1");
  } else {
    deleteTestEnvValue("ORIRO_TEST_MINIMAL_GATEWAY");
  }

  const workspaceDir = path.join(tempHome, "oriro");
  await fs.mkdir(workspaceDir, { recursive: true });
  setTestEnvValue("ORIRO_BUNDLED_PLUGINS_DIR", await createEmptyBundledPluginsDir(tempHome));
  setTestEnvValue("ORIRO_DISABLE_BUNDLED_PLUGINS", "1");
...
describe("gateway e2e", () => {
  ...
  it(
    ...
    async () => {
      ...
      const { envSnapshot, tempHome, workspaceDir } = await setupGatewayTempHome({
        prefix: "oriro-gw-mock-home-",
        minimalGateway: true,
      });

      const token = nextGatewayId("test-token");
      setTestEnvValue("ORIRO_GATEWAY_TOKEN", token);
  ...
  it(
    ...
    async () => {
      const { envSnapshot, tempHome, workspaceDir } = await setupGatewayTempHome({
        prefix: "oriro-gw-http-tools-home-",
      });
...
`.trimStart(),
  ...
  it(
    ...
    async () => {
      ...
      const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-minimal-gateway-home-"));
      const configPath = await createGatewayConfigPath(tempHome);
      const bundledPluginsDir = path.join(tempHome, "oriro-test-no-bundled-extensions");
      setTestEnvValue("HOME", tempHome);
      setTestEnvValue("ORIRO_STATE_DIR", path.join(tempHome, ".oriro"));
      setTestEnvValue("ORIRO_CONFIG_PATH", configPath);
      setTestEnvValue("ORIRO_SKIP_CHANNELS", "1");
      ...
      setTestEnvValue("ORIRO_SKIP_BROWSER_CONTROL_SERVER", "1");
      setTestEnvValue("ORIRO_SKIP_PROVIDERS", "1");
      setTestEnvValue("ORIRO_BUNDLED_PLUGINS_DIR", bundledPluginsDir);
      setTestEnvValue("ORIRO_TEST_MINIMAL_GATEWAY", "1");
