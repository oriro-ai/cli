---
summary: "Mantis is the visual end-to-end verification system for reproducing Oriro bugs on live transports, capturing before and after evidence, and attaching artifacts to PRs."
title: "Mantis"
read_when:
  - Building or running live visual QA for Oriro bugs
...
Mantis is the Oriro end-to-end verification system for bugs that need a real
...
Mantis lives in the Oriro QA stack.

- Oriro owns the scenario runtime, transport adapters, evidence schema, and
  local CLI under `pnpm oriro qa mantis`.
...
- OriroSweeper owns GitHub comment routing: parsing maintainer commands,
  dispatching the workflow, and posting the final PR comment.
- Oriro agents drive Mantis through Codex when a scenario needs agentic setup,
  debugging, or stuck-state reporting.

This boundary keeps transport knowledge in Oriro, machine scheduling in
Crabbox, and maintainer workflow glue in OriroSweeper.
...
```bash
pnpm oriro qa mantis discord-smoke \
...
thread, calls Oriro's `message.thread-reply` action with a repo-local
...
```bash
pnpm oriro qa mantis desktop-browser-smoke \
...
`ORIRO_MANTIS_CRABBOX_PROVIDER` when running against another Crabbox fleet.

Useful desktop smoke flags:

- `--lease-id <cbx_...>` or `ORIRO_MANTIS_CRABBOX_LEASE_ID` reuses a warmed desktop.
...
- `--browser-profile-archive-env <name>` restores a base64 `.tgz` Chrome user-data-dir archive from the named environment variable before launching the browser. Use this for logged-in witnesses such as Discord Web. The default env var is `ORIRO_MANTIS_BROWSER_PROFILE_TGZ_B64`.
...
- `--keep-lease` or `ORIRO_MANTIS_KEEP_VM=1` keeps a newly created passing lease open for VNC inspection. Failed runs keep the lease by default when one was created so an operator can reconnect.
...
REST. When `ORIRO_QA_DISCORD_CAPTURE_UI_METADATA=1` is set, the scenario also
writes a Discord Web URL artifact. When `ORIRO_QA_DISCORD_KEEP_THREADS=1` is
...
the VM, runs `pnpm oriro qa slack` inside that VM, opens Slack Web in the VNC
...
shape where the SUT Oriro gateway and the browser both live inside the same
Linux desktop VM.

With `--gateway-setup`, the command prepares a persistent disposable Oriro
home at `$HOME/.oriro-mantis/slack-oriro`, patches Slack Socket Mode
configuration for the selected channel, starts `oriro gateway run` on port
`38973`, and keeps Chrome running in the VNC session. This is the "leave me a
Linux desktop with Slack and a oriro running" mode; the bot-to-bot Slack QA lane
...
- `ORIRO_LIVE_OPENAI_KEY` for the remote model lane. If only
  `OPENAI_API_KEY` is set locally, Mantis maps it to `ORIRO_LIVE_OPENAI_KEY`
  before invoking Crabbox so Crabbox's `ORIRO_*` env forwarding can carry it
  into the VM.
...
channel id, Socket Mode app token, and bot token as the `ORIRO_MANTIS_SLACK_*`
...
- `--gateway-setup` starts a persistent Oriro Slack gateway in the VM instead of only running the bot-to-bot QA lane.
...
- `ORIRO_MANTIS_SLACK_BROWSER_PROFILE_DIR` controls the persistent Chrome profile inside the VM. The default is `$HOME/.config/oriro-mantis/slack-chrome-profile`, so a manual Slack Web login survives reruns on the same lease.
...
Crabbox CLI from
`oriro/crabbox` main so it can use the current desktop/browser lease flags
...
runs `pnpm oriro qa mantis slack-desktop-smoke --gateway-setup` against that
...
this lane when you want "a Linux desktop with Slack and a oriro running" instead
...
worktree, runs `pnpm oriro qa telegram --credential-source convex
...
`@oriro-mantis telegram desktop proof`, from the Actions UI with freeform
...
Oriro with the leased Telegram SUT bot token, starts `oriro gateway run`
...
token never logs Telegram Desktop in; it only configures Oriro. The desktop
...
- `--no-gateway-setup` installs and opens Telegram Desktop without configuring Oriro.
...
comment or creates one. The workflows publish to `oriro-crabbox-artifacts`
with public URLs under `https://artifacts.oriro.ai`. They provide bucket,
...
```text
@oriro-mantis discord status reactions
```
...
```text
@oriro-mantis discord status reactions baseline=origin/main candidate=HEAD
```
...
OriroSweeper command examples:

```text
@orirosweeper mantis discord discord-status-reactions-tool-only
@orirosweeper verify e2e discord
```
...
6. Start a child Oriro Gateway with an isolated state directory.
...
- It exercises a real Oriro Gateway, Discord bot auth, message dispatch,
...
- `pnpm oriro qa discord` already runs a live Discord lane with driver and
  SUT bots.
...
- **Headless automation**: default for CI. Chrome runs with CDP enabled, and
  Playwright or Oriro browser control captures screenshots.
...
- Node 22 and pnpm
- Oriro checkout and dependency cache
- Playwright Chromium browser cache when Playwright is used
- enough CPU and memory for one Oriro Gateway, one browser, and one model run
...
- `ORIRO_QA_DISCORD_MANTIS_BOT_TOKEN`
- `ORIRO_QA_DISCORD_DRIVER_BOT_TOKEN`
- `ORIRO_QA_DISCORD_SUT_BOT_TOKEN`
- `ORIRO_QA_DISCORD_GUILD_ID`
- `ORIRO_QA_DISCORD_CHANNEL_ID`
- `ORIRO_QA_DISCORD_NOTIFY_CHANNEL_ID`
- `ORIRO_QA_REDACT_PUBLIC_METADATA=1` for public GitHub artifact uploads
- `ORIRO_QA_CONVEX_SITE_URL`
- `ORIRO_QA_CONVEX_SECRET_CI`
- `ORIRO_QA_MANTIS_CRABBOX_COORDINATOR`
- `ORIRO_QA_MANTIS_CRABBOX_COORDINATOR_TOKEN`
...
`ORIRO_QA_REDACT_PUBLIC_METADATA=1` for this reason.
...
- When should OriroSweeper automatically recommend Mantis instead of waiting for a
  maintainer command?
