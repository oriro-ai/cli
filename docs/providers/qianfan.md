---
summary: "Use Qianfan's unified API to access many models in Oriro"
read_when:
...
```bash
oriro plugins install @oriro/qianfan-provider
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    oriro onboard --auth-choice qianfan-api-key
    ```
  </Step>
  <Step title="Verify the model is available">
    ```bash
    oriro models list --provider qianfan
    ```
  </Step>
</Steps>
