...
describe("hook policy", () => {
  describe("resolveHookEnableState", () => {
    it("keeps workspace hooks disabled by default", () => {
      const entry = makeHookEntry("workspace-hook", "oriro-workspace");
    ...
    it("keeps plugin hooks enabled without local hook toggles", () => {
      const entry = makeHookEntry("plugin-hook", "oriro-plugin");
  ...
  describe("resolveHookEntries", () => {
    it("lets managed hooks override bundled and plugin hooks", () => {
      const bundled = makeHookEntry("shared", "oriro-bundled");
      const plugin = makeHookEntry("shared", "oriro-plugin");
      const managed = makeHookEntry("shared", "oriro-managed");

      const resolved = resolveHookEntries([bundled, plugin, managed]);
      expect(resolved).toHaveLength(1);
      expect(resolved[0]?.hook.source).toBe("oriro-managed");
    });

    it("prevents workspace hooks from overriding non-workspace hooks", () => {
      const managed = makeHookEntry("shared", "oriro-managed");
      const workspace = makeHookEntry("shared", "oriro-workspace");
    ...
    it("keeps later workspace entries for the same source/name", () => {
      const first = makeHookEntry("shared", "oriro-workspace");
      const second = makeHookEntry("shared", "oriro-workspace");
      second.hook.handlerPath = "/tmp/oriro-workspace/shared/handler-2.js";
