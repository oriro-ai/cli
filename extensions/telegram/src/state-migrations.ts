...
import type { ChannelLegacyStateMigrationPlan } from "oriro/plugin-sdk/channel-contract";
import { resolveChannelAllowFromPath } from "oriro/plugin-sdk/channel-pairing";
...
} from "oriro/plugin-sdk/persistent-dedupe";
import { createPluginStateSyncKeyedStore } from "oriro/plugin-sdk/runtime-doctor";
import { statRegularFileSync } from "oriro/plugin-sdk/security-runtime";
...
function detectTelegramMessageDispatchLegacyStateMigration(params: {
  ...
  const storePath = resolveStorePath(params.cfg.session?.store, { env: params.env });
  const legacyStorePath = resolveLegacySessionStorePath(params);
  const env = params.stateDir ? { ...params.env, ORIRO_STATE_DIR: params.stateDir } : params.env;
