...
export async function startGateway({
  ...
  let child;
  const launcherArgs = resolveOriroLaunchArgs(repoRoot, sourceEntryExists);
  try {
    child = spawnImpl(
      process.execPath,
      [
      ...
      ],
      {
        ...
        env: {
          ...
          ORIRO_STATE_DIR: path.join(tempRoot, "state"),
...
async function main() {
  ...
  const repoRoot = path.resolve(args.repoRoot ?? process.env.ORIRO_REPO_ROOT ?? process.cwd());
  ...
  try {
    ...
    const requireFromOriro = createRequire(path.join(repoRoot, "package.json"));
    const WebSocket = requireFromOriro("ws");
    ...
    const connect = await client.request(
      "connect",
      {
        ...
        client: {
          id: "gateway-client",
          displayName: "oriro-rtt rpc probe",
          version: "rtt",
          platform: process.platform,
          mode: "backend",
          instanceId: `oriro-rtt-rpc-${randomUUID()}`,
        },
        locale: "en-US",
        userAgent: "oriro-rtt-rpc",
        role: "operator",
