...
export PATH="/tmp/oriro-bin:$PATH"
mkdir -p /tmp/oriro-bin
cp scripts/e2e/lib/doctor-install-switch/shims/systemctl /tmp/oriro-bin/systemctl
cp scripts/e2e/lib/doctor-install-switch/shims/loginctl /tmp/oriro-bin/loginctl
chmod +x /tmp/oriro-bin/systemctl /tmp/oriro-bin/loginctl

package_tgz="${ORIRO_CURRENT_PACKAGE_TGZ:?missing ORIRO_CURRENT_PACKAGE_TGZ}"
git_root="/tmp/oriro-git"
mkdir -p "$git_root"
...
npm_log="/tmp/oriro-doctor-switch-npm-install.log"
if ! oriro_e2e_maybe_timeout "${ORIRO_E2E_NPM_INSTALL_TIMEOUT:-600s}" npm install -g --prefix /tmp/npm-prefix --omit=optional "$package_tgz" >"$npm_log" 2>&1; then
  oriro_e2e_print_log "$npm_log"
  exit 1
fi

npm_bin="/tmp/npm-prefix/bin/oriro"
npm_root="/tmp/npm-prefix/lib/node_modules/oriro"
...
run_flow() {
  ...
  local install_log="/tmp/oriro-doctor-switch-${name}-install.log"
  local doctor_log="/tmp/oriro-doctor-switch-${name}-doctor.log"
  local command_timeout="${ORIRO_DOCKER_DOCTOR_SWITCH_COMMAND_TIMEOUT:-900s}"

  echo "== Flow: $name =="
  oriro_test_state_create "switch-${name}" empty
  export USER="testuser"

  if ! oriro_e2e_maybe_timeout "$command_timeout" bash -c "$install_cmd" >"$install_log" 2>&1; then
    oriro_e2e_print_log "$install_log"
    exit 1
  fi
  rm -f "$HOME/.zshrc" "$HOME/.bashrc" "$HOME/.bash_profile"
  rm -rf "$HOME/.config/fish" "$HOME/.config/powershell"

  unit_path="$HOME/.config/systemd/user/oriro-gateway.service"
  if [ ! -f "$unit_path" ]; then
  ...
  if ! oriro_e2e_maybe_timeout "$command_timeout" bash -c "$doctor_cmd" >"$doctor_log" 2>&1; then
    oriro_e2e_print_log "$doctor_log"
    exit 1
  fi
...
run_flow \
  ...
  "$npm_entry" \
  "ORIRO_UPDATE_IN_PROGRESS=1 node $git_cli doctor --repair --force --yes --non-interactive" \
  "$git_entry"
...
run_flow \
  ...
  "$git_entry" \
  "ORIRO_UPDATE_IN_PROGRESS=1 $npm_bin doctor --repair --force --yes --non-interactive" \
  "$npm_entry"
...
run_proxy_env_flow() {
  ...
  unit_path="$HOME/.config/systemd/user/oriro-gateway.service"
  if ! oriro_e2e_maybe_timeout "$command_timeout" env \
  ...
  if ! oriro_e2e_maybe_timeout "$command_timeout" env ORIRO_UPDATE_IN_PROGRESS=1 \
...
run_wrapper_flow() {
  ...
  local install_log="/tmp/oriro-doctor-switch-${name}-install.log"
  local reinstall_log="/tmp/oriro-doctor-switch-${name}-reinstall.log"
  local env_repair_log="/tmp/oriro-doctor-switch-${name}-env-repair.log"
  local doctor_log="/tmp/oriro-doctor-switch-${name}-doctor.log"
  local clear_log="/tmp/oriro-doctor-switch-${name}-clear.log"
  local command_timeout="${ORIRO_DOCKER_DOCTOR_SWITCH_COMMAND_TIMEOUT:-900s}"
  ...
  local wrapper="$HOME/.local/bin/oriro-wrapper"
  node scripts/e2e/lib/doctor-install-switch/write-wrapper.mjs \
    "$wrapper" \
    "$npm_bin" \
    "$HOME/oriro-wrapper-argv.log"

  local unit_path="$HOME/.config/systemd/user/oriro-gateway.service"

  if ! oriro_e2e_maybe_timeout "$command_timeout" "$npm_bin" gateway install --wrapper "$wrapper" --force >"$install_log" 2>&1; then
    oriro_e2e_print_log "$install_log"
    exit 1
  fi
  assert_exec_arg "$unit_path" 1 "$wrapper"
  assert_exec_arg "$unit_path" 2 "gateway"
  assert_env_value "$unit_path" "ORIRO_WRAPPER" "$wrapper"

  if ! oriro_e2e_maybe_timeout "$command_timeout" "$npm_bin" gateway install --force >"$reinstall_log" 2>&1; then
    oriro_e2e_print_log "$reinstall_log"
    exit 1
  fi
  assert_exec_arg "$unit_path" 1 "$wrapper"
  assert_exec_arg "$unit_path" 2 "gateway"
  ...
  sed -i "/^Environment=ORIRO_WRAPPER=/d" "$unit_path"
  if ! oriro_e2e_maybe_timeout "$command_timeout" "$npm_bin" gateway install --wrapper "$wrapper" >"$env_repair_log" 2>&1; then
    oriro_e2e_print_log "$env_repair_log"
    exit 1
  fi
  assert_exec_arg "$unit_path" 1 "$wrapper"
  assert_env_value "$unit_path" "ORIRO_WRAPPER" "$wrapper"

  sed -i "s#^Environment=ORIRO_WRAPPER=.*#Environment=ORIRO_WRAPPER=/tmp/stale-oriro-wrapper#" "$unit_path"
  if ! oriro_e2e_maybe_timeout "$command_timeout" "$npm_bin" gateway install --wrapper "$wrapper" >"$env_repair_log" 2>&1; then
    oriro_e2e_print_log "$env_repair_log"
    exit 1
  fi
  ...
  if ! oriro_e2e_maybe_timeout "$command_timeout" node "$git_cli" doctor --repair --force --yes >"$doctor_log" 2>&1; then
    oriro_e2e_print_log "$doctor_log"
    exit 1
  fi
  if ! grep -Fq "Gateway service invokes ORIRO_WRAPPER:" "$doctor_log"; then
  ...
  if ! oriro_e2e_maybe_timeout "$command_timeout" env ORIRO_WRAPPER= "$npm_bin" gateway install --force >"$clear_log" 2>&1; then
    oriro_e2e_print_log "$clear_log"
    exit 1
  fi
  assert_no_env_key "$unit_path" "ORIRO_WRAPPER"
