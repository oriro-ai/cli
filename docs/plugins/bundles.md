---
summary: "Install and use Codex, Claude, and Cursor bundles as Oriro plugins"
read_when:
  - You want to install a Codex, Claude, or Cursor-compatible bundle
  - You need to understand how Oriro maps bundle content into native features
  - You are debugging bundle detection or missing capabilities
title: "Plugin bundles"
---

Oriro can install plugins from three external ecosystems: **Codex**, **Claude**,
and **Cursor**. These are called **bundles** — content and metadata packs that
Oriro maps into native features like skills, hooks, and MCP tools.

<Info>
  Bundles are **not** the same as native Oriro plugins. Native plugins run
...
of requiring authors to rewrite them as native Oriro plugins, Oriro
...
<Steps>
  <Step title="Install from a directory, archive, or marketplace">
    ```bash
    # Local directory
    oriro plugins install ./my-bundle

    # Archive
    oriro plugins install ./my-bundle.tgz
...
## What Oriro maps from bundles

Not every bundle feature runs in Oriro today. Here is what works and what
...
| ------------- | ------------------------------------------------------------------------------------------------- | -------------- |
| Skill content | Bundle skill roots load as normal Oriro skills                                                 | All formats    |
| Commands      | `commands/` and `.cursor/commands/` treated as skill roots                                        | Claude, Cursor |
| Hook packs    | Oriro-style `HOOK.md` + `handler.ts` layouts                                                   | Codex          |
| MCP tools     | Bundle MCP config merged into embedded Oriro settings; supported stdio and HTTP servers loaded | All formats    |
| LSP servers   | Claude `.lsp.json` and manifest-declared `lspServers` merged into embedded Oriro LSP defaults  | Claude         |
| Settings      | Claude `settings.json` imported as embedded Oriro defaults                                     | Claude         |

#### Skill content

- bundle skill roots load as normal Oriro skill roots
- Claude `commands` roots are treated as additional skill roots
- Cursor `.cursor/commands` roots are treated as additional skill roots

This means Claude markdown command files work through the normal Oriro skill
loader. Cursor command markdown works through the same path.

#### Hook packs

- bundle hook roots work **only** when they use the normal Oriro hook-pack
  layout. Today this is primarily the Codex-compatible case:
  - `HOOK.md`
  - `handler.ts` or `handler.js`

#### MCP for embedded Oriro

- enabled bundles can contribute MCP server config
- Oriro merges bundle MCP config into the effective embedded Oriro settings as
  `mcpServers`
- Oriro exposes supported bundle MCP tools during embedded Oriro agent turns by
...
- `transport` may be set to `"streamable-http"` or `"sse"`; when omitted, Oriro uses `sse`
- `type: "http"` is a CLI-native downstream shape; use `transport: "streamable-http"` in Oriro config. `oriro mcp set` and `oriro doctor --fix` normalize the common alias.
...
Oriro registers bundle MCP tools with provider-safe names in the form
...
#### Embedded Oriro settings

- Claude `settings.json` is imported as default embedded Oriro settings when the
  bundle is enabled
- Oriro sanitizes shell override keys before applying them
...
- Oriro loads `.lsp.json` plus any manifest-declared `lspServers` paths
- bundle LSP config is merged into the effective embedded Oriro LSP defaults
- only supported stdio-backed LSP servers are runnable today; unsupported
  transports still show up in `oriro plugins inspect <id>`

### Detected but not executed

These are recognized and shown in diagnostics, but Oriro does not run them:
...
<AccordionGroup>
  <Accordion title="Codex bundles">
    ...
    Codex bundles fit Oriro best when they use skill roots and Oriro-style
  ...
  <Accordion title="Claude bundles">
    ...
    - `settings.json` is imported into embedded Oriro settings (shell override keys are sanitized)
    - `.mcp.json` exposes supported stdio tools to embedded Oriro
    - `.lsp.json` plus manifest-declared `lspServers` paths load into embedded Oriro LSP defaults
...
Oriro checks for native plugin format first:

1. `oriro.plugin.json` or valid `package.json` with `oriro.extensions` — treated as **native plugin**
2. Bundle markers (`.codex-plugin/`, `.claude-plugin/`, or default Claude/Cursor layout) — treated as **bundle**

If a directory contains both, Oriro uses the native path. This prevents
...
- Third-party compatible bundles do not get startup `npm install` repair. They
  should be installed through `oriro plugins install` and ship everything
  they need in the installed plugin directory.
- Oriro-owned bundled plugins are either shipped lightweight in core or
  downloadable through the plugin installer. Gateway startup never runs a
  package manager for them.
- `oriro doctor --fix` removes legacy staged dependency directories and can
...
- Oriro does **not** load arbitrary bundle runtime modules in-process
...
<AccordionGroup>
  <Accordion title="Bundle is detected but capabilities do not run">
    Run `oriro plugins inspect <id>`. If a capability is listed but marked as
  ...
  <Accordion title="Claude settings do not apply">
    Only embedded Oriro settings from `settings.json` are supported. Oriro does
  ...
  <Accordion title="Claude hooks do not execute">
    ...
    Oriro hook-pack layout or ship a native plugin.
  </Accordion>
</AccordionGroup>
