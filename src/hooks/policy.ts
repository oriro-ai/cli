...
import type { OriroConfig, HookConfig } from "../config/config.js";
...
const HOOK_SOURCE_POLICIES: Record<HookSource, HookSourcePolicy> = {
  "oriro-bundled": {
    ...
    canOverride: ["oriro-bundled"],
    canBeOverriddenBy: ["oriro-managed", "oriro-plugin"],
  },
  "oriro-plugin": {
    precedence: 20,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["oriro-bundled", "oriro-plugin"],
    canBeOverriddenBy: ["oriro-managed"],
  },
  "oriro-managed": {
    precedence: 30,
    trustedLocalCode: true,
    defaultEnableMode: "default-on",
    canOverride: ["oriro-bundled", "oriro-managed", "oriro-plugin"],
    canBeOverriddenBy: ["oriro-managed"],
  },
  "oriro-workspace": {
    precedence: 40,
    ...
    canOverride: ["oriro-workspace"],
    canBeOverriddenBy: ["oriro-workspace"],
  },
};
...
export function resolveHookEnableState(params: {
  ...
  if (entry.hook.source === "oriro-plugin") {
    return { enabled: true };
  }
