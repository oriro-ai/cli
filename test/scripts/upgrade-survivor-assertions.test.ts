...
function writeMigratedSessionState(stateDir: string): void {
  ...
  const db = new DatabaseSync(join(agentDbDir, "oriro-agent.sqlite"));
  try {
    db.exec(`
...
function assertConfiguredPluginState(params: { installPath?: string } = {}): void {
  const root = mkdtempSync(join(tmpdir(), "oriro-upgrade-survivor-"));
  try {
    ...
    writeJson(join(stateDir, "plugins", "installs.json"), {
      installRecords: {
        matrix: {
          source: "orirohub",
          spec: "orirohub:@oriro/matrix",
          installPath: matrixInstallDir,
          orirohubPackage: "@oriro/matrix",
    ...
    execFileSync(process.execPath, [ASSERTIONS_PATH, "assert-state"], {
      env: {
        ...
        ORIRO_TEST_WORKSPACE_DIR: workspace,
        ORIRO_UPGRADE_SURVIVOR_CONFIG_COVERAGE_JSON: coveragePath,
        ORIRO_UPGRADE_SURVIVOR_SCENARIO: "configured-plugin-installs",
      },
      stdio: "pipe",
    });
  } finally {
...
describe("upgrade survivor assertions", () => {
  it("accepts official OriroHub npm-pack installs for configured external plugins", () => {
    expect(() => assertConfiguredPluginState()).not.toThrow();
  });

  it("rejects OriroHub npm-pack installs outside the managed extensions root", () => {
    const root = mkdtempSync(join(tmpdir(), "oriro-upgrade-survivor-outside-"));
    try {
      expect(() =>
