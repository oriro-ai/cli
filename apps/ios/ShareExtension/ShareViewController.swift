...
final class ShareViewController: UIViewController {
    ...
    private let logger = Logger(subsystem: "ai.orirofoundation.app", category: "ShareExtension")
    ...
    private func setupUI() {
        ...
        self.sendButton.setTitle("Send to Oriro", for: .normal)
    ...
    private func sendCurrentDraft() async {
        ...
        self.showStatus("Sending to Oriro gateway…")
    ...
    private func sendMessageToGateway(_ message: String, attachments: [ShareAttachment]) async throws {
        guard let config = ShareGatewayRelaySettings.loadConfig() else {
            throw NSError(
                domain: "OriroShare",
                code: 10,
                userInfo: [NSLocalizedDescriptionKey: "Oriro is not connected to a gateway yet."])
        }
        ...
        do {
            try await gateway.connect(
                ...
                connectOptions: makeOptions("oriro-ios"),
                sessionBox: nil,
                onConnected: {},
                onDisconnected: { _ in },
                onInvoke: { req in
