...
const {
  ...
  resolveProviderHttpRequestConfigMock: vi.fn((params: Record<string, unknown>) => {
    ...
    if (params.provider === "xai") {
      const version = process.env.ORIRO_VERSION?.trim() || "unknown";
      headers.set("User-Agent", `oriro/${version}`);
      headers.set("originator", "oriro");
...
vi.mock("oriro/plugin-sdk/provider-auth-runtime", () => ({
...
vi.mock("oriro/plugin-sdk/provider-http", () => ({
...
describe("xai image generation provider", () => {
  ...
  it("builds provider with correct models, default, and capabilities", () => {
    ...
    expect(isConfigured({ agentDir: "/tmp/oriro-xai-test" })).toBe(true);
    expect(isProviderApiKeyConfiguredMock).toHaveBeenCalledWith({
      provider: "xai",
      agentDir: "/tmp/oriro-xai-test",
    });
  });
  ...
  it("forwards xAI attribution User-Agent through the SDK image request", async () => {
    ...
    const request = requirePostJsonCall();
    expect(request.headers?.get("user-agent")).toBe("oriro/2026.3.22");
    expect(request.headers?.get("originator")).toBe("oriro");
