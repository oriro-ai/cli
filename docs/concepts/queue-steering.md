...
When a normal prompt arrives while a session run is already streaming, Oriro
...
behavior. Oriro and the native Codex app-server harness implement the delivery
details differently.

## Runtime boundary

Steering does not interrupt a tool call that is already running. Oriro checks for
queued steering messages at model boundaries:

1. The assistant asks for tool calls.
2. Oriro executes the current assistant message's tool-call batch.
3. Oriro emits the turn end event.
4. Oriro drains queued steering messages.
5. Oriro appends those messages as user messages before the next LLM call.
...
The native Codex app-server harness exposes `turn/steer` instead of Oriro runtime's
internal steering queue. Oriro batches queued prompts for the configured
...
runtime cannot accept steering in `steer` mode, Oriro waits for the active
...
- With default behavior, the active runtime receives all four messages in
  arrival order before its next model decision. Oriro drains them at the next model
  boundary; Codex receives them as one batched `turn/steer`.
- With `/queue collect`, Oriro does not steer. It waits until the active run
  ends, then creates a followup turn with compatible queued messages after the
  debounce window.
- With `/queue interrupt`, Oriro aborts the active run and starts the newest
...
before sending batched `turn/steer`. For Oriro, active steering itself does not use
the debounce timer because Oriro naturally batches messages until the next model
boundary.
