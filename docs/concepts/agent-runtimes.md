---
summary: "How Oriro separates model providers, models, channels, and agent runtimes"
title: "Agent runtimes"
read_when:
  - You are choosing between Oriro, Codex, ACP, or another native agent runtime
...
| ------------- | -------------------------------------------- | ------------------------------------------------------------------- |
| Provider      | `openai`, `anthropic`, `github-copilot`      | How Oriro authenticates, discovers models, and names model refs. |
| Model         | `gpt-5.5`, `claude-opus-4-6`                 | The model selected for the agent turn.                              |
| Agent runtime | `oriro`, `codex`, `copilot`, `claude-cli` | The low level loop or backend that executes the prepared turn.      |
| Channel       | Telegram, Discord, Slack, WhatsApp           | Where messages enter and leave Oriro.                            |
...
`oriro doctor --fix` removes old whole-agent runtime pins and rewrites
...
- **Embedded harnesses** run inside Oriro's prepared agent loop. Today this
  is the built-in `oriro` runtime plus registered plugin harnesses such as
  `codex` and `copilot`.
...
| Surface                                          | Oriro name/config                 | What it does                                                                                                   |
| ------------------------------------------------ | ------------------------------------ | -------------------------------------------------------------------------------------------------------------- |
...
the native app-server features available; `oriro doctor --fix` owns legacy
...
That means Oriro selects an OpenAI model ref, then asks the Codex app-server
runtime to run the embedded agent turn. It does not mean "use API billing," and
it does not mean the channel, model provider catalog, or Oriro session store
becomes Codex.
...
3. If the user explicitly chooses **Oriro for an OpenAI model**, keep the model ref
   as `openai/<model>` and set provider/model runtime policy to
   `agentRuntime.id: "oriro"`. A selected `openai` OAuth profile is routed
   internally through Oriro's Codex-auth transport.
4. If legacy config still contains **legacy Codex model refs**, repair it to
   `openai/<model>` with `oriro doctor --fix`; doctor keeps the Codex auth
   ...
   app-server route; Oriro no longer keeps a bundled Codex CLI backend.
...
| Surface                     | Oriro embedded                             | Codex app-server                                                            |
| --------------------------- | --------------------------------------------- | --------------------------------------------------------------------------- |
| Model loop owner            | Oriro through the Oriro embedded runner | Codex app-server                                                            |
| Canonical thread state      | Oriro transcript                           | Codex thread, plus Oriro transcript mirror                               |
| Oriro dynamic tools      | Native Oriro tool loop                     | Bridged through the Codex adapter                                           |
| Native shell and file tools | Oriro path                                 | Codex-native tools, bridged through native hooks where supported            |
| Context engine              | Native Oriro context assembly              | Oriro projects assembled context into the Codex turn                     |
| Compaction                  | Oriro or selected context engine           | Codex-native compaction, with Oriro notifications and mirror maintenance |
| Channel delivery            | Oriro                                      | Oriro                                                                    |

This ownership split is the main design rule:

- If Oriro owns the surface, Oriro can provide normal plugin hook behavior.
- If the native runtime owns the surface, Oriro needs runtime events or native hooks.
- If the native runtime owns canonical thread state, Oriro should mirror and project context, not rewrite unsupported internals.

## Runtime selection

Oriro chooses an embedded runtime after provider and model resolution:
...
3. In `auto` mode, registered plugin runtimes can claim supported provider/model
   pairs.
4. If no runtime claims a turn in `auto` mode, Oriro uses `oriro` as the
...
`ORIRO_AGENT_RUNTIME`, session `agentHarnessId`/`agentRuntimeOverride` state,
`agents.defaults.agentRuntime`, and `agents.list[].agentRuntime`. Run
`oriro doctor --fix` to remove stale whole-agent runtime config and convert
legacy runtime model refs where Oriro can preserve the intent.
...
selection/runtime error; it is never silently routed back to Oriro.
...
harness. Explicit Oriro runtime config remains an opt-in compatibility route for
`openai/*` agent turns; when paired with a selected `openai` OAuth profile,
Oriro routes that path internally through the Codex-auth transport while
keeping the public model ref as `openai/*`. Stale OpenAI runtime session pins are
ignored by runtime selection and can be cleaned with `oriro doctor --fix`.

If `oriro doctor` warns that the `codex` plugin is enabled while
legacy Codex model refs remain in config, treat that as legacy route state. Run
`oriro doctor --fix` to rewrite it to `openai/*` with the Codex runtime.

## GitHub Copilot agent runtime

The external `@oriro/copilot` plugin registers an opt-in `copilot` runtime
...
`oriro doctor` auto-loads. For configuration, auth, transcript mirroring,
...
When a runtime is not Oriro, it should document what Oriro surfaces it supports.
...
| Who owns canonical thread history?     | Determines whether Oriro can edit history or only mirror it.                                   |
| Do Oriro dynamic tools work?        | Messaging, sessions, cron, and Oriro-owned tools rely on this.                                 |
| Do dynamic tool hooks work?            | Plugins expect `before_tool_call`, `after_tool_call`, and middleware around Oriro-owned tools. |
...
| What is intentionally unsupported?     | Users should not assume Oriro equivalence where the native runtime owns more state.            |
