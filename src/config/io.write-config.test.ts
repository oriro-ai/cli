...
import type { ConfigFileSnapshot, OriroConfig } from "./types.oriro.js";

const CONFIG_CLOBBER_SNAPSHOT_LIMIT = 32;
type ConfigHealthDatabase = Pick<OriroStateKyselyDatabase, "config_health_entries">;
...
describe("config io write", () => {
  const suiteRootTracker = createSuiteTempRootTracker({ prefix: "oriro-config-io-" });
  const silentLogger = {
    warn: () => {},
    error: () => {},
  };
  ...
  function readConfigHealthRow(home: string, configPath: string) {
    const { db } = openOriroStateDatabase({ env: { HOME: home } as NodeJS.ProcessEnv });
  ...
  const createFastConfigIO = (home: string) =>
    createConfigIO({
      env: { ORIRO_TEST_FAST: "1" } as NodeJS.ProcessEnv,
      homedir: () => home,
      logger: silentLogger,
    });
  ...
  it("writes health state to SQLite through public config reads", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".oriro", "oriro.json");
      const healthPath = path.join(home, ".oriro", "logs", "config-health.json");
  ...
  it("loads shipped plugin install config records without mutating config or plugin index", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".oriro", "oriro.json");
      const pluginDir = path.join(home, ".oriro", "plugins", "demo");
      const manifestPath = path.join(pluginDir, "oriro.plugin.json");
  ...
  it("retains included shipped plugin install records in write snapshots", async () => {
    await withSuiteHome(async (home) => {
      const configDir = path.join(home, ".oriro");
  ...
  it("migrates shipped plugin install config records during explicit writes even when the manifest is missing", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".oriro", "oriro.json");
      const pluginDir = path.join(home, ".oriro", "plugins", "missing");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        configPath,
        `${JSON.stringify(
  ...
  it("keeps shipped plugin install config records when index migration fails", async () => {
    ...
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".oriro", "oriro.json");
      const unwritableStatePath = path.join(home, ".oriro");
  ...
  it("keeps writes inside an ORIRO_STATE_DIR override even when the real home config exists", async () => {
    await withSuiteHome(async (home) => {
      const liveConfigPath = path.join(home, ".oriro", "oriro.json");
      ...
      const env = { ORIRO_STATE_DIR: overrideDir } as NodeJS.ProcessEnv;
      const io = createConfigIO({
        env,
        homedir: () => home,
        logger: silentLogger,
      ...
      expect(io.configPath).toBe(path.join(overrideDir, "oriro.json"));
      ...
      const overridePersisted = JSON.parse(
        await fs.readFile(path.join(overrideDir, "oriro.json"), "utf-8"),
      ) as {
  ...
  it("prints missing-meta write anomalies when anomaly logging is requested", async () => {
    await withSuiteHome(async (home) => {
      ...
      const io = createConfigIO({
        env: {
          ORIRO_CONFIG_WRITE_ANOMALY_LOG: "1",
        } as NodeJS.ProcessEnv,
        homedir: () => home,
        logger: {
          warn,
  ...
  it("suppresses overwrite audit output when skipOutputLogs is set", async () => {
    await withSuiteHome(async (home) => {
      ...
      const io = createConfigIO({
        env: {
          VITEST: "true",
          ORIRO_TEST_CONFIG_OVERWRITE_LOG: "1",
        } as NodeJS.ProcessEnv,
        homedir: () => home,
        logger: {
          warn,
  ...
  it("preserves root $schema during partial writes", async () => {
    await withSuiteHome(async (home) => {
      ...
      expect(persisted.$schema).toBe("https://oriro.ai/config.json");
  ...
  it("rejects destructive internal writes before replacing the config", async () => {
    await withSuiteHome(async (home) => {
      ...
      const original = {
        ...
        agents: { list: [{ id: "main", default: true, workspace: "/tmp/oriro-main" }] },
  ...
  it("keeps authored agent provider params during narrowed internal agent writes", async () => {
    await withSuiteHome(async (home) => {
      ...
      const persisted = JSON.parse(await fs.readFile(configPath, "utf-8")) as OriroConfig;
  ...
  it("returns the read-time environment snapshot for invalid config repairs", async () => {
    await withSuiteHome(async (home) => {
      ...
      const io = createConfigIO({
        env: {
          ORIRO_GATEWAY_TOKEN: "gateway-token-at-read",
          ORIRO_TEST_FAST: "1",
        } as NodeJS.ProcessEnv,
        homedir: () => home,
        logger: silentLogger,
      ...
      expect(result.writeOptions.envSnapshotForRestore?.ORIRO_GATEWAY_TOKEN).toBe(
  ...
  it("returns the snapshot-time hash when an included file is malformed", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".oriro", "oriro.json");
      const includePath = path.join(home, ".oriro", "plugins.json5");
  ...
  it("returns a write guard that rejects a changed active config path", async () => {
    await withSuiteHome(async (home) => {
      const firstConfigPath = path.join(home, ".oriro", "first.json");
      const secondConfigPath = path.join(home, ".oriro", "second.json");
      ...
      const env = {
        ORIRO_CONFIG_PATH: firstConfigPath,
        ORIRO_TEST_FAST: "1",
      } as NodeJS.ProcessEnv;
      const io = createConfigIO({ env, homedir: () => home, logger: silentLogger });

      const result = await io.readConfigFileSnapshotForWrite();
      env.ORIRO_CONFIG_PATH = secondConfigPath;
  ...
  it("rejects local write ownership when config env changes path selection during the read", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".oriro", "oriro.json");
      const configuredNextPath = path.join(home, ".oriro", "next.json");
      const sourceConfig = {
        env: { ORIRO_CONFIG_PATH: configuredNextPath },
        gateway: { mode: "local" },
  ...
  it("follows config env path selection before returning global write ownership", async () => {
    await withSuiteHome(async (home) => {
      ...
      const initialConfig = JSON.parse(await fs.readFile(configPath, "utf-8")) as OriroConfig;
  ...
  it("does not use expectedConfigPath as the write destination", async () => {
    await withSuiteHome(async (home) => {
      const expectedConfigPath = path.join(home, ".oriro", "expected.json");
      const activeConfigPath = path.join(home, ".oriro", "active.json");
      ...
      await withEnvAsync(
        {
          ORIRO_CONFIG_PATH: activeConfigPath,
          ORIRO_TEST_FAST: "1",
        },
        async () => {
          await writeConfigFile(
  ...
  it("rejects root-include partial writes instead of flattening the root config", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".oriro", "oriro.json");
      const includePath = path.join(home, ".oriro", "extra.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        includePath,
        `${JSON.stringify({ $schema: "https://oriro.ai/config-from-include.json" }, null, 2)}\n`,
        "utf-8",
      );
      await fs.writeFile(
        configPath,
  ...
  it("rejects invalid include-backed repairs instead of persisting substituted secrets", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".oriro", "oriro.json");
      const includePath = path.join(home, ".oriro", "gateway.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        includePath,
        `${JSON.stringify(
      ...
      const io = createConfigIO({
        env: {
          ORIRO_GATEWAY_TOKEN: "gateway-token-runtime",
          ORIRO_TEST_FAST: "1",
        } as NodeJS.ProcessEnv,
        homedir: () => home,
        logger: silentLogger,
  ...
  it("repairs invalid root-authored siblings without flattening included config", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".oriro", "oriro.json");
      const includePath = path.join(home, ".oriro", "agent-defaults.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        includePath,
  ...
  it("rejects repairs that would flatten a valid outer include with a broken nested include", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".oriro", "oriro.json");
      const pluginsPath = path.join(home, ".oriro", "plugins.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        pluginsPath,
  ...
  it("preserves escaped root literals before validating unrelated includes", async () => {
      ...
      plugins: [
        {
          ...
          skills: [],
          hooks: [],
          rootDir: "/tmp/oriro-test-literal-plugin",
          source: "/tmp/oriro-test-literal-plugin/index.ts",
          manifestPath: "/tmp/oriro-test-literal-plugin/oriro.plugin.json",
          configSchema: {
            type: "object",
            properties: {
    ...
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".oriro", "oriro.json");
      const agentsPath = path.join(home, ".oriro", "agents.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        agentsPath,
  ...
  it("repairs invalid config without flattening array-nested includes", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".oriro", "oriro.json");
      const includePath = path.join(home, ".oriro", "main-agent.json5");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
        includePath,
        `${JSON.stringify({ id: "main", workspace: "${ORIRO_AGENT_WORKSPACE}" }, null, 2)}\n`,
        "utf-8",
      );
      await fs.writeFile(
        configPath,
      ...
      const io = createConfigIO({
        env: {
          ORIRO_AGENT_WORKSPACE: "/resolved/agent-workspace",
          ORIRO_TEST_FAST: "1",
        } as NodeJS.ProcessEnv,
        homedir: () => home,
        logger: silentLogger,
      ...
      await expect(fs.readFile(includePath, "utf-8")).resolves.toContain(
        '"workspace": "${ORIRO_AGENT_WORKSPACE}"',
      );
    });
  });
      ...
      plugins: [
        {
          ...
          skills: [],
          hooks: [],
          rootDir: "/tmp/oriro-test-required-plugin",
          source: "/tmp/oriro-test-required-plugin/index.ts",
          manifestPath: "/tmp/oriro-test-required-plugin/oriro.plugin.json",
          configSchema: {
            type: "object",
            properties: {
              token: { type: "string" },
  ...
  it("writes runtime-derived edits back to source SecretRef markers", async () => {
    await withSuiteHome(async (home) => {
      ...
      await withEnvAsync({ ORIRO_CONFIG_PATH: configPath }, async () => {
        setRuntimeConfigSnapshot(
          {
            gateway: { mode: "local" },
            models: {
      ...
      try {
        await withEnvAsync(
          ...
          async () => {
            ...
            expect(persisted.gateway?.auth?.token).toBe("${ORIRO_GATEWAY_TOKEN}");
      ...
      plugins: [
        {
          ...
          skills: [],
          hooks: [],
          rootDir: "/tmp/oriro-test-demo",
          source: "/tmp/oriro-test-demo/index.ts",
          manifestPath: "/tmp/oriro-test-demo/oriro.plugin.json",
          configSchema: {
            type: "object",
            properties: {
      ...
      try {
        await withEnvAsync({ ORIRO_CONFIG_PATH: configPath }, async () => {
          ...
          const postWriteSnapshot = await createConfigIO({
            env: { ORIRO_CONFIG_PATH: configPath, VITEST: "true" } as NodeJS.ProcessEnv,
            homedir: () => home,
            logger: silentLogger,
  ...
  it("rolls back the root config when post-write runtime refresh fails", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".oriro", "oriro.json");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      const initialConfig = { gateway: { mode: "local", port: 18789 } } satisfies OriroConfig;
  ...
  it("keeps plugin install index migration when runtime refresh fails", async () => {
    await withSuiteHome(async (home) => {
      const stateDir = path.join(home, ".oriro");
      const configPath = path.join(stateDir, "oriro.json");
  ...
  it("blocks runtime preflight failures before committing root writes", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".oriro", "oriro.json");
      const initialRaw = `${JSON.stringify({ gateway: { mode: "local" } }, null, 2)}\n`;
      let observedSource: OriroConfig | undefined;
  ...
  it("restores config env vars when post-write runtime refresh rollback succeeds", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".oriro", "oriro.json");
      const envKey = "ORIRO_TEST_RUNTIME_ROLLBACK_ENV";
      const initialConfig = { gateway: { mode: "local", port: 18789 } } satisfies OriroConfig;
      const initialRaw = `${JSON.stringify(initialConfig, null, 2)}\n`;
  ...
  it("rolls back root writes when canonical reread changes config path ownership", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".oriro", "oriro.json");
      const nextConfigPath = path.join(home, ".oriro", "next.json");
      const initialConfig = { gateway: { mode: "local", port: 18789 } } satisfies OriroConfig;
      const initialRaw = `${JSON.stringify(initialConfig, null, 2)}\n`;
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(configPath, initialRaw, "utf-8");
        ...
        async () => {
          ...
          await expect(
            writeConfigFile(
              {
                gateway: { mode: "local", port: 19001 },
                env: { ORIRO_CONFIG_PATH: nextConfigPath },
              },
              {
                baseSnapshot: prepared.snapshot,
                ...prepared.writeOptions,
  ...
  it("uses injected filesystem operations when rolling back ownership loss", async () => {
    await withSuiteHome(async (home) => {
      const configPath = path.join(home, ".oriro", "oriro.json");
      const otherConfigPath = path.join(home, ".oriro", "other.json");
      const initialConfig = { gateway: { mode: "local", port: 18789 } } satisfies OriroConfig;
      const initialRaw = `${JSON.stringify(initialConfig, null, 2)}\n`;
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(configPath, initialRaw, "utf-8");
        ...
        promises: {
          ...
          rename: async (from, to) => {
            ...
            if (!committed && to === configPath) {
              committed = true;
              env.ORIRO_CONFIG_PATH = otherConfigPath;
            }
          },
        },
      } as typeof fsNode;
    ...
    await withSuiteHome(async (home) => {
        ...
        plugins: [
          {
            ...
            skills: [],
            hooks: [],
            rootDir: "/tmp/oriro-test-strict-plugin",
            source: "/tmp/oriro-test-strict-plugin/index.ts",
            manifestPath: "/tmp/oriro-test-strict-plugin/oriro.plugin.json",
            configSchema: {
              type: "object",
      ...
      try {
        ...
        await withEnvAsync({ ORIRO_CONFIG_PATH: configPath }, async () => {
          ...
          await expect(
            writeConfigFile({ agents: { list: "not-array" } } as unknown as OriroConfig, {
  ...
  it("preserves authored tilde paths when runtime-shaped writes hand back absolute paths", async () => {
    await withSuiteHome(async (home) => {
      ...
      await fs.writeFile(
        ...
        `${JSON.stringify(
          {
            logging: { file: "~/oriro-upgrade-survivor/gateway.jsonl" },
          },
          null,
          2,
        )}\n`,
      ...
      await io.writeConfigFile(
        {
          logging: {
            file: path.join(home, "oriro-upgrade-survivor", "gateway.jsonl"),
            level: "debug",
          },
        },
        { baseSnapshot: snapshot },
      ...
      const persisted = JSON.parse(await fs.readFile(configPath, "utf-8")) as OriroConfig;
      expect(persisted.logging?.file).toBe("~/oriro-upgrade-survivor/gateway.jsonl");
