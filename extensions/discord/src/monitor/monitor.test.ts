...
import type { DiscordAccountConfig, OriroConfig } from "oriro/plugin-sdk/config-contracts";
import { buildPluginBindingApprovalCustomId } from "oriro/plugin-sdk/conversation-runtime";
...
describe("discord component interactions", () => {
  ...
  beforeEach(() => {
    ...
    resolveStorePathMock.mockClear().mockReturnValue("/tmp/oriro-sessions-test.json");
  ...
  it("does not fall through to Oriro when a plugin Discord interaction already replied", async () => {
    registerDiscordComponentEntries({
      entries: [createButtonEntry({ callbackData: "codex:approve" })],
      modals: [],
    });
  ...
  it("resolves plugin binding approvals without falling through to Oriro", async () => {
    registerDiscordComponentEntries({
      entries: [
        createButtonEntry({
