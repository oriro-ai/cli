...
import { resolveMergedAccountConfig } from "oriro/plugin-sdk/account-resolution-runtime";
...
export function resolveQaChannelAccount(params: {
  ...
  const botUserId = merged.botUserId?.trim() || "oriro";
  const botDisplayName = merged.botDisplayName?.trim() || "ORIRO QA";
  return {
    accountId,
    enabled,
