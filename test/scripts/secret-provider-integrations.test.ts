...
function makeTempDir(): string {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-secret-provider-proof-"));
  tempDirs.push(root);
  return root;
}
...
function writeStallingOriro(
  ...
  const descendantScript = options.gatewayDescendantMarkerPath
    ...
    : "";
  const scriptPath = path.join(root, "fake-oriro.mjs");
  fs.writeFileSync(
    scriptPath,
    [
      "#!/usr/bin/env node",
      ...
      "  process.exit(0);",
      "}",
      "console.error(`unexpected fake oriro args: ${args.join(' ')}`);",
      "process.exit(2);",
      "",
    ].join("\n"),
    { mode: 0o755 },
...
function writeLeakingStartupOriro(root: string): string {
  const scriptPath = path.join(root, "fake-leaking-oriro.mjs");
  fs.writeFileSync(
    scriptPath,
    [
      "#!/usr/bin/env node",
...
function writeSignaledStartupOriro(root: string): string {
  const scriptPath = path.join(root, "fake-signaled-oriro.mjs");
  fs.writeFileSync(
    scriptPath,
    [
      "#!/usr/bin/env node",
...
function writeNoisySecretsConfigureOriro(root: string): string {
  const scriptPath = path.join(root, "fake-noisy-secrets-configure-oriro.mjs");
  fs.writeFileSync(
    scriptPath,
    [
      "#!/usr/bin/env node",
...
function runProofHarness(
  ...
  return spawnSync(process.execPath, [harnessPath, proofScriptPath, root, mode], {
    ...
    env: {
      ...process.env,
      ORIRO_ENTRY: fakeOriro,
      ORIRO_SECRET_PROOF_READY_MS: "60",
      ORIRO_SECRET_PROOF_RPC_MS: "1000",
      ...envOverrides,
    },
    timeout: 5_000,
  });
...
describe("secret provider integration proof harness", () => {
  it("runs pnpm-backed Oriro commands through the repo pnpm runner", async () => {
    ...
    const command = await proof.resolveOriroCommand(
      ["gateway", "status"],
      { ...process.env, ORIRO_SECRET_PROOF_SENTINEL: "1" },
      {
        nodeExecPath: "/opt/node/bin/node",
        npmExecPath: fakePnpm,
        runner: { pnpm: true, baseArgs: ["oriro"], label: "pnpm oriro" },
      },
    );

    expect(command.command).toBe("/opt/node/bin/node");
    expect(command.args).toEqual([fakePnpm, "oriro", "gateway", "status"]);
    expect(command.options.env.ORIRO_SECRET_PROOF_SENTINEL).toBe("1");
    expect(command.options.shell).toBe(false);
  });

  it("keeps stalled startup health probes inside the ready deadline", async () => {
    const root = makeTempDir();
    const fakeOriro = writeStallingOriro(root);
    const result = runProofHarness(root, fakeOriro, "start");
  ...
  it("fails fast when startup exits by signal", () => {
    const root = makeTempDir();
    const fakeOriro = writeSignaledStartupOriro(root);
    const result = runProofHarness(root, fakeOriro, "start", {
      ORIRO_SECRET_PROOF_READY_MS: "2000",
    });
  ...
  it("kills a stalled startup gateway before returning a readiness failure", async () => {
    ...
    const result = runProofHarness(root, fakeOriro, "start", {
      ORIRO_SECRET_PROOF_TEARDOWN_GRACE_MS: "100",
    });
  ...
  it("bounds captured command output", async () => {
    const previousLimit = process.env.ORIRO_SECRET_PROOF_OUTPUT_BYTES;
    process.env.ORIRO_SECRET_PROOF_OUTPUT_BYTES = "1024";
    try {
      ...
      if (previousLimit === undefined) {
        delete process.env.ORIRO_SECRET_PROOF_OUTPUT_BYTES;
      } else {
        process.env.ORIRO_SECRET_PROOF_OUTPUT_BYTES = previousLimit;
      }
    }
  });

  it("clamps oversized command timeout env values before scheduling timers", async () => {
    const previousTimeout = process.env.ORIRO_SECRET_PROOF_COMMAND_MS;
    process.env.ORIRO_SECRET_PROOF_COMMAND_MS = String(Number.MAX_SAFE_INTEGER);
    try {
      const proof = await import(
      ...
      if (previousTimeout === undefined) {
        delete process.env.ORIRO_SECRET_PROOF_COMMAND_MS;
      } else {
        process.env.ORIRO_SECRET_PROOF_COMMAND_MS = previousTimeout;
      }
    }
  });
  ...
  it("blocks skipped secret proofs unless local rehearsals explicitly allow skips", async () => {
    const previousAllowSkips = process.env.ORIRO_SECRET_PROOF_ALLOW_SKIPS;
    const proof = await import(
    ...
    try {
      delete process.env.ORIRO_SECRET_PROOF_ALLOW_SKIPS;
      expect(proof.collectBlockingProofResults(entries)).toEqual(entries);

      process.env.ORIRO_SECRET_PROOF_ALLOW_SKIPS = "1";
      expect(proof.collectBlockingProofResults(entries)).toEqual([]);
    } finally {
      if (previousAllowSkips === undefined) {
        delete process.env.ORIRO_SECRET_PROOF_ALLOW_SKIPS;
      } else {
        process.env.ORIRO_SECRET_PROOF_ALLOW_SKIPS = previousAllowSkips;
      }
    }
  });
  ...
  it.runIf(process.platform !== "win32")("bounds captured PTY configure output", async () => {
    const root = makeTempDir();
    const fakeOriro = writeNoisySecretsConfigureOriro(root);
    const previousLimit = process.env.ORIRO_SECRET_PROOF_OUTPUT_BYTES;
    const previousEntry = process.env.ORIRO_ENTRY;
    process.env.ORIRO_SECRET_PROOF_OUTPUT_BYTES = "128";
    process.env.ORIRO_ENTRY = fakeOriro;
    try {
      const proof = await import(
      ...
      if (previousEntry === undefined) {
        delete process.env.ORIRO_ENTRY;
      } else {
        process.env.ORIRO_ENTRY = previousEntry;
      }
    }
  });
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const root = makeTempDir();
      const fakeOriro = path.join(root, "fake-oriro-pty-timeout.mjs");
  ...
  it.each([
    ["ORIRO_SECRET_PROOF_COMMAND_MS", "150ms"],
    ["ORIRO_SECRET_PROOF_READY_MS", "0"],
    ["ORIRO_SECRET_PROOF_OUTPUT_BYTES", "4mb"],
    ["ORIRO_SECRET_PROOF_RESOLVER_STDIN_BYTES", "4mb"],
  ...
  it("bounds generated resolver stdin before reading the secret store", async () => {
    ...
    const previousLimit = process.env.ORIRO_SECRET_PROOF_RESOLVER_STDIN_BYTES;
    process.env.ORIRO_SECRET_PROOF_RESOLVER_STDIN_BYTES = "64";
    ...
    try {
      ...
      if (previousLimit === undefined) {
        delete process.env.ORIRO_SECRET_PROOF_RESOLVER_STDIN_BYTES;
      } else {
        process.env.ORIRO_SECRET_PROOF_RESOLVER_STDIN_BYTES = previousLimit;
      }
    }
  });
  ...
  it("fails when proof temp cleanup cannot remove the root", async () => {
    ...
    try {
      await expect(
        proof.cleanupEnv("/tmp/oriro-secret-provider-proof-stuck", {
          attempts: 3,
          retryDelayMs: 1,
        }),
  ...
  it("detects startup secret leaks after the retained output cap", () => {
    const root = makeTempDir();
    const fakeOriro = writeLeakingStartupOriro(root);
    const result = runProofHarness(root, fakeOriro, "startup-fails", {
      ORIRO_SECRET_PROOF_OUTPUT_BYTES: "128",
    });
  ...
  it("keeps stalled managed status probes inside the ready deadline", async () => {
    const root = makeTempDir();
    const fakeOriro = writeStallingOriro(root);
    const result = runProofHarness(root, fakeOriro, "status");
