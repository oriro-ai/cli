---
summary: "CLI reference for `oriro message` (send + channel actions)"
read_when:
...
```
oriro message <subcommand> [flags]
```
...
- `oriro message` resolves the selected channel to its owning plugin when `--channel` or a channel-prefixed target is present; otherwise it loads configured channel plugins for default-channel inference.
...
- On cache miss, Oriro will attempt a live directory lookup when the provider supports it.
...
- `oriro message` resolves supported channel SecretRefs before running the selected action.
- Resolution is scoped to the active action target when possible:
  ...
  - when `--account` is omitted, Oriro does not force a `default` account SecretRef scope
...
```
oriro message send --channel discord \
...
```bash
oriro message send --channel googlechat --target spaces/AAA... \
  --message "Choose:" \
...
```
oriro message poll --channel discord \
  --target channel:123 \
  --poll-question "Snack?" \
...
```
oriro message poll --channel telegram \
  --target @mychat \
  --poll-question "Lunch?" \
  --poll-option Pizza --poll-option Sushi \
...
```
oriro message poll --channel msteams \
...
```
oriro message react --channel slack \
...
```
oriro message react --channel signal \
...
```
oriro message send --channel telegram --target @mychat --message "Choose:" \
...
```
oriro message send --channel telegram --target 123456789 --message "Open app:" \
...
```bash
oriro message send --channel telegram --target @mychat \
