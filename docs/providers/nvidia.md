---
summary: "Use NVIDIA's OpenAI-compatible API in Oriro"
read_when:
  - You want to use open models in Oriro for free
...
[build.nvidia.com](https://build.nvidia.com/settings/api-keys). Oriro
...
<Steps>
  ...
  <Step title="Export the key and run onboarding">
    ...
    oriro onboard --auth-choice nvidia-api-key
    ```
  </Step>
  <Step title="Set an NVIDIA model">
    ```bash
    oriro models set nvidia/nvidia/nemotron-3-ultra-550b-a55b
    ```
  </Step>
</Steps>
...
```bash
oriro onboard --auth-choice nvidia-api-key --nvidia-api-key "nvapi-..."
```
...
When an NVIDIA API key is configured, Oriro setup and model-selection paths
...
Oriro release. When the live feed is available, the first returned model is
...
malformed, Oriro falls back to the bundled catalog and bundled default below.

## Nemotron 3 Ultra

Nemotron 3 Ultra is the default NVIDIA model in Oriro. NVIDIA's build page for
...
<AccordionGroup>
  ...
  <Accordion title="Catalog and pricing">
    Oriro prefers NVIDIA's public featured-model catalog when NVIDIA auth is
  ...
  <Accordion title="Nemotron 3 Ultra reasoning params">
    ...
    and `reasoning_budget` for reasoning output. Oriro's bundled Ultra row
