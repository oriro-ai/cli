...
An **agent** here is the full per-persona scope: workspace files, auth profiles, model registry, and session store. `agentDir` is the on-disk state directory that holds this per-agent config at `~/.oriro/agents/<agentId>/`. A **binding** maps a channel account (e.g. a Slack workspace or a WhatsApp number) to one of those agents.
...
- **Session store** (chat history + routing state) under `~/.oriro/agents/<agentId>/sessions`.
...
a local profile, but Oriro does not clone OAuth refresh tokens into the
...
Skills are loaded from each agent workspace plus shared roots such as `~/.oriro/skills`, then filtered by the effective agent skill allowlist when configured. Use `agents.defaults.skills` for a shared baseline and `agents.list[].skills` for per-agent replacement. See [Skills: per-agent vs shared](/tools/skills#per-agent-vs-shared-skills) and [Skills: agent skill allowlists](/tools/skills#agent-allowlists).
...
- State dir: `~/.oriro` (or `ORIRO_STATE_DIR`)
- Workspace: `~/.oriro/workspace` (or `~/.oriro/workspace-<agentId>`)
- Agent dir: `~/.oriro/agents/<agentId>/agent` (or `agents.list[].agentDir`)
- Sessions: `~/.oriro/agents/<agentId>/sessions`

### Single-agent mode (default)

If you do nothing, Oriro runs a single agent:

- `agentId` defaults to **`main`**.
- Sessions are keyed as `agent:main:<mainKey>`.
- Workspace defaults to `~/.oriro/workspace` (or `~/.oriro/workspace-<profile>` when `ORIRO_PROFILE` is set).
- State defaults to `~/.oriro/agents/main/agent`.
...
<Steps>
  <Step title="Create each agent workspace">
    ...
    ```bash
    oriro agents add coding
    oriro agents add social
    ```

    Each agent gets its own workspace with `SOUL.md`, `AGENTS.md`, and optional `USER.md`, plus a dedicated `agentDir` and session store under `~/.oriro/agents/<agentId>`.
...
```json5
{
  agents: {
    list: [
      { id: "alex", workspace: "~/.oriro/workspace-alex" },
      { id: "mia", workspace: "~/.oriro/workspace-mia" },
    ],
  },
  bindings: [
    {
...
<AccordionGroup>
  ...
  <Accordion title="Account-scope detail">
    ...
    - If you later add the same binding for the same agent with an explicit account id, Oriro upgrades the existing channel-only binding to account-scoped instead of duplicating it.
...
If you want a channel-wide default account when `accountId` is omitted, set `channels.<channel>.defaultAccount` (optional). When unset, Oriro falls back to `default` if present, otherwise the first configured account id (sorted).
...
<AccordionGroup>
  <Accordion title="Discord bots per agent">
    ...
    ```json5
    {
      agents: {
        list: [
          { id: "main", workspace: "~/.oriro/workspace-main" },
          { id: "coding", workspace: "~/.oriro/workspace-coding" },
        ],
      },
      bindings: [
  ...
  <Accordion title="Telegram bots per agent">
    ```json5
    {
      agents: {
        list: [
          { id: "main", workspace: "~/.oriro/workspace-main" },
          { id: "alerts", workspace: "~/.oriro/workspace-alerts" },
        ],
      },
      bindings: [
  ...
  <Accordion title="WhatsApp numbers per agent">
    ...
    ```bash
    oriro channels login --channel whatsapp --account personal
    oriro channels login --channel whatsapp --account biz
    ```

    `~/.oriro-ai/cli.json` (JSON5):
    ...
    ```js
    {
        ...
        list: [
          {
            ...
            default: true,
            name: "Home",
            workspace: "~/.oriro/workspace-home",
            agentDir: "~/.oriro/agents/home/agent",
          },
          {
            id: "work",
            name: "Work",
            workspace: "~/.oriro/workspace-work",
            agentDir: "~/.oriro/agents/work/agent",
          },
        ],
      },
      ...
      channels: {
        whatsapp: {
          accounts: {
            personal: {
              // Optional override. Default: ~/.oriro/credentials/whatsapp/personal
              // authDir: "~/.oriro/credentials/whatsapp/personal",
            },
            biz: {
              // Optional override. Default: ~/.oriro/credentials/whatsapp/biz
        ...
        list: [
          {
            id: "chat",
            name: "Everyday",
            workspace: "~/.oriro/workspace-chat",
            model: "anthropic/claude-sonnet-4-6",
          },
          {
            id: "opus",
            name: "Deep Work",
            workspace: "~/.oriro/workspace-opus",
