...
// Converts Claude project JSONL into Oriro transcript-compatible messages.
...
import { attachOriroTranscriptMeta } from "./session-transcript-readers.js";
...
function normalizeClaudeCliContent(
  ...
  for (const item of content) {
    ...
    if (type === "tool_use") {
      // Claude stores tool calls as `tool_use` with `input`; Oriro history
...
function parseClaudeCliHistoryEntry(
  ...
  if (type === "user") {
    return attachOriroTranscriptMeta(
      {
        role: "user",
        content,
