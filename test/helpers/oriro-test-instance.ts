// Oriro test instance helper spawns isolated Oriro processes.
...
import {
  ...
  type OriroTestState,
  type OriroTestStateOptions,
} from "../../src/test-utils/oriro-test-state.js";
import { sleep } from "../../src/utils.js";

export type OriroTestInstanceOptions = {
  name: string;
  cwd?: string;
  port?: number;
  gatewayToken?: string;
  ...
  state?: Omit<OriroTestStateOptions, "applyEnv" | "gateway" | "env">;
  gatewayArgs?: string[];
  startTimeoutMs?: number;
  stopTimeoutMs?: number;
};

export type OriroTestInstanceCommandResult = {
  code: number | null;
...
export type OriroTestInstance = {
  ...
  ) => Promise<OriroTestInstanceCommandResult>;
...
type OriroTestChildProcess = Pick<ChildProcessWithoutNullStreams, "kill" | "pid">;
...
async function prepareGatewayEntrypoint(cwd: string): Promise<string[]> {
  ...
  const child = spawn("node", ["scripts/run-node.mjs", "--help"], {
    ...
    detached: shouldUseOriroTestProcessGroup(),
  });
  ...
  if (completed === null) {
    signalOriroTestProcess(child, "SIGKILL");
...
export async function createOriroTestInstance(
  options: OriroTestInstanceOptions,
): Promise<OriroTestInstance> {
  ...
  const instance: OriroTestInstance = {
    name: options.name,
    port,
    ...
    stopGateway: async () => {
      ...
      if (!hasChildExited(child) && !child.killed) {
        try {
          signalOriroTestProcess(child, "SIGTERM");
        } catch {
          // ignore
        }
      }
...
function shouldUseOriroTestProcessGroup(): boolean {
  return process.platform !== "win32";
}

function signalOriroTestProcess(
  child: OriroTestChildProcess,
  signal: NodeJS.Signals,
  killProcess: (pid: number, signal: NodeJS.Signals) => boolean = process.kill,
): void {
  if (shouldUseOriroTestProcessGroup() && typeof child.pid === "number") {
    try {
