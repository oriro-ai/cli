...
import type { BrowserConfig, BrowserProfileConfig, OriroConfig } from "../config/config.js";
...
export const ORIRO_BROWSER_HEADLESS_ENV = "ORIRO_BROWSER_HEADLESS";
...
function normalizeHexColor(raw: string | undefined): string {
  ...
  if (!value) {
    return DEFAULT_ORIRO_BROWSER_COLOR;
  }
...
function isLocalManagedProfile(profile: ResolvedBrowserProfile): boolean {
  return profile.driver === "oriro" && profile.cdpIsLoopback && !profile.attachOnly;
}
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_ORIRO_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_ORIRO_BROWSER_PROFILE_NAME] = {
...
export function resolveBrowserConfig(
  ...
  const enabled = cfg?.enabled ?? DEFAULT_ORIRO_BROWSER_ENABLED;
  ...
  const defaultProfile =
    ...
    (profiles[DEFAULT_BROWSER_DEFAULT_PROFILE_NAME]
      ? DEFAULT_BROWSER_DEFAULT_PROFILE_NAME
      : profiles[DEFAULT_ORIRO_BROWSER_PROFILE_NAME]
        ? DEFAULT_ORIRO_BROWSER_PROFILE_NAME
        : "user");
...
export function resolveProfile(
  ...
  let cdpUrl;
  const driver = profile.driver === "existing-session" ? "existing-session" : "oriro";
...
export function resolveManagedBrowserHeadlessMode(
  ...
  const envHeadless = parseBooleanValue(env[ORIRO_BROWSER_HEADLESS_ENV]);
...
export function getManagedBrowserMissingDisplayError(
  ...
  return (
    ...
    `Set ${ORIRO_BROWSER_HEADLESS_ENV}=1, remove the headed override, or launch under Xvfb.`
  );
}
