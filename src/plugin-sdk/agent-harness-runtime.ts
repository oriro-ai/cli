...
export { VERSION as ORIRO_VERSION } from "../version.js";
...
export { resolveOriroAgentDir } from "./agent-dir-compat.js";
export {
  resolveAgentDir,
...
export async function detectAndLoadAgentHarnessPromptImages(params: {
  ...
  const [{ resolveImageSanitizationLimits }, { detectAndLoadPromptImages }, { MAX_IMAGE_BYTES }] =
    await Promise.all([
      ...
      import("@oriro/media-core/constants"),
    ]);
...
/**
 * Derive the same compact user-facing tool detail that embedded Oriro uses for progress logs.
 */
 ...
 * preserve the same Oriro-owned fallback signals as the built-in Oriro path
