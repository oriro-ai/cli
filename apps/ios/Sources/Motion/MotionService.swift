...
final class MotionService: MotionServicing {
    func activities(params: OriroMotionActivityParams) async throws -> OriroMotionActivityPayload {
        ...
        let mapped: [OriroMotionActivityEntry] = try await withCheckedThrowingContinuation { cont in
        ...
        return OriroMotionActivityPayload(activities: mapped)
    }
        ...
        return try await withCheckedThrowingContinuation { cont in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    ...
                    let formatter = ISO8601DateFormatter()
                    let payload = OriroPedometerPayload(
