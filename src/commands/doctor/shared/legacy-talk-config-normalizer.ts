...
function buildLegacyRealtimeTalkCompat(
  talk: Record<string, unknown>,
  normalizedTalk: NonNullable<OriroConfig["talk"]>,
): Record<string, unknown> | undefined {
  ...
  return normalizeTalkSection({ realtime: compat } as OriroConfig["talk"])?.realtime;
}
...
export function normalizeLegacyTalkConfig(cfg: OriroConfig, changes: string[]): OriroConfig {
  const rawTalk = cfg.talk;
  if (!isRecord(rawTalk)) {
    return cfg;
  }

  const normalizedTalk = normalizeTalkSection(rawTalk as OriroConfig["talk"]) ?? {};
