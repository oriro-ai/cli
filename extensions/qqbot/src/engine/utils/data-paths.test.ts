...
import { withEnv } from "oriro/plugin-sdk/test-env";
...
describe("qqbot legacy credential backup paths", () => {
  ...
  it("scopes legacy credential backup imports to the active ORIRO_STATE_DIR", () => {
  ...
  it("keeps legacy account import paths isolated across different state directories", () => {
    ...
    const gatewayAPath = withEnv({ ORIRO_STATE_DIR: stateDirA }, () =>
      getCredentialBackupFile("default"),
    );
    const gatewayBPath = withEnv({ ORIRO_STATE_DIR: stateDirB }, () =>
      getCredentialBackupFile("default"),
    );
  ...
  it("uses ORIRO_HOME for default legacy credential backup imports", () => {
    const homeDir = createTempDir("qqbot-oriro-home-");
    withEnv({ ORIRO_STATE_DIR: "", ORIRO_HOME: homeDir }, () => {
      expect(getCredentialBackupFile("default")).toBe(
        path.join(homeDir, ".oriro", "qqbot", "data", "credential-backup-default.json"),
      );
    });
  });
  ...
  it("expands tilde state-dir overrides through the canonical state resolver", () => {
    ...
    withEnv({ HOME: homeDir, ORIRO_HOME: "", ORIRO_STATE_DIR: "~/gateway-a" }, () => {
      expect(getCredentialBackupFile("default")).toBe(
