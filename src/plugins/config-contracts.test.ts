...
const mocks = vi.hoisted(() => {
  ...
  return {
    discoverOriroPlugins: vi.fn(() => ({ candidates: [], diagnostics: [] })),
...
vi.mock("./discovery.js", () => ({
  discoverOriroPlugins: mocks.discoverOriroPlugins,
}));
...
function createPluginRecord(
  ...
  return {
    ...
    hooks: [],
    source: `/tmp/${overrides.id}/oriro.plugin.json`,
...
describe("resolvePluginConfigContractsById", () => {
  beforeEach(() => {
    mocks.discoverOriroPlugins.mockReset();
    mocks.discoverOriroPlugins.mockReturnValue({ candidates: [], diagnostics: [] });
