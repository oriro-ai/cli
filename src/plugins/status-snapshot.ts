...
/** Control-plane plugin status shape used by `oriro plugins status` style surfaces. */
...
function isPluginLifecycleTraceEnabled(): boolean {
  const raw = process.env.ORIRO_PLUGIN_LIFECYCLE_TRACE?.trim().toLowerCase();
...
function buildPluginRecordFromInstalledIndex(
  ...
  const format = plugin.format ?? manifest?.format ?? "oriro";
