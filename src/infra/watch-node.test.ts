...
import { bundledPluginFile } from "oriro/plugin-sdk/test-fixtures";
...
const VOICE_CALL_MANIFEST = bundledPluginFile("voice-call", "oriro.plugin.json");
...
describe("watch-node script", () => {
  it("wires chokidar watch to run-node with watched source/config paths", async () => {
    ...
    await withTempDir({ prefix: "oriro-watch-node-" }, async (cwd) => {
      ...
      expect(spawnEnv.ORIRO_WATCH_MODE).toBe("1");
      expect(spawnEnv.ORIRO_WATCH_SESSION).toBe("1700000000000-4242");
      expect(spawnEnv.ORIRO_NO_RESPAWN).toBe("1");
      expect(spawnEnv.ORIRO_WATCH_COMMAND).toBe("gateway --force");
      expect(spawnEnv.ORIRO_TRACE_SYNC_IO).toBeUndefined();
  ...
  it("preserves explicit sync I/O trace overrides for gateway watch", async () => {
    ...
    await withTempDir({ prefix: "oriro-watch-node-" }, async (cwd) => {
      ...
      expect(spawnCall[0]).toBe("/usr/local/bin/node");
      expect(spawnCall[1]).toEqual(["scripts/run-node.mjs", "gateway", "--force"]);
      expect(requireSpawnEnv(spawn, 0).ORIRO_TRACE_SYNC_IO).toBe("0");
  ...
  it("does not run doctor after a gateway failure when auto doctor is disabled", async () => {
    ...
    const runPromise = runWatch({
      args: ["gateway", "--force"],
      createWatcher,
      env: { ORIRO_GATEWAY_WATCH_AUTO_DOCTOR: "0" },
      lockDisabled: true,
      process: fakeProcess,
      spawn,
    });
  ...
  it("forces no-respawn for watch children even when supervisor hints are inherited", async () => {
    ...
    expect(spawnCall[1]).toEqual(["scripts/run-node.mjs", "gateway", "--force"]);
    const spawnEnv = requireSpawnEnv(spawn, 0);
    expect(spawnEnv.LAUNCH_JOB_LABEL).toBe("ai.oriro.gateway");
    expect(spawnEnv.ORIRO_NO_RESPAWN).toBe("1");
  ...
  it("prints recovery guidance when chokidar fails with invalid package config", async () => {
    const error = Object.assign(
      new Error(
        'Invalid package config /tmp/oriro/.pnpm/chokidar/package.json while importing "chokidar" from /tmp/oriro/scripts/watch-node.mjs.',
      ),
    ...
    try {
      ...
      expect(errorSpy.mock.calls).toEqual([
        [""],
        [
          "[oriro] gateway:watch could not start because a dependency package config looks corrupted.",
        ],
        ["[oriro] Invalid package config: /tmp/oriro/.pnpm/chokidar/package.json"],
        ["[oriro] This usually means a file in node_modules is empty or truncated."],
        ["[oriro] Recommended recovery:"],
        ["[oriro]   rm -rf node_modules"],
        ["[oriro]   pnpm store prune"],
        ["[oriro]   pnpm install"],
        [""],
        ["[oriro] Original error:"],
        [error],
      ]);
    } finally {
  ...
  it("replaces an existing watcher lock holder before starting", async () => {
    const { child, spawn, watcher, createWatcher, fakeProcess } = createWatchHarness();
    await withTempDir({ prefix: "oriro-watch-node-lock-" }, async (cwd) => {
