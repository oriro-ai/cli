...
import { escapeRegExp, formatEnvelopeTimestamp } from "oriro/plugin-sdk/channel-test-helpers";
import type { TelegramGroupConfig } from "oriro/plugin-sdk/config-contracts";
import type { GetReplyOptions, MsgContext } from "oriro/plugin-sdk/reply-runtime";
import { withEnvAsync } from "oriro/plugin-sdk/test-env";
import { sanitizeTerminalText } from "oriro/plugin-sdk/test-fixtures";
...
const pluginStateTestRuntime = await import("oriro/plugin-sdk/plugin-state-test-runtime");
const conversationRuntime = await import("oriro/plugin-sdk/conversation-runtime");
const configMutation = await import("oriro/plugin-sdk/config-mutation");
const sessionStoreRuntime = await import("oriro/plugin-sdk/session-store-runtime");
...
function createTelegramBotTestStateDir(): string {
  const dir = mkdtempSync(path.join(tmpdir(), "oriro-telegram-bot-"));
...
describe("createTelegramBot", () => {
  ...
  beforeEach(async () => {
    previousStateDir = process.env.ORIRO_STATE_DIR;
    process.env.ORIRO_STATE_DIR = createTelegramBotTestStateDir();
    resetTelegramForumFlagCacheForTest();
  ...
  it.each(["stop", "/stop@oriro_bot"] as const)(
    ...
    try {
      ...
      await runTelegramMiddlewareChain({
        ctx: {
          ...
          message: {
            chat: { id: -1007, type: "supergroup", title: "Oriro Ops" },
            text: "first",
            date: 1736380804,
            message_id: 104,
            from: { id: 42, first_name: "Ada" },
  ...
  it("handles pairing DM flows for new and already-pending requests", async () => {
    ...
    for (const [index, testCase] of cases.entries()) {
      ...
      expect(pairingText, testCase.name).toContain("oriro pairing approve telegram");
      expectRecordFields(
  ...
  it("ignores private self-authored message updates instead of issuing a pairing challenge", async () => {
    ...
    await handler({
      message: {
        ...
        message_id: 1884,
        date: 1736380800,
        from: { id: 7, is_bot: true, first_name: "Oriro", username: "oriro_bot" },
        pinned_message: {
          message_id: 1883,
          date: 1736380799,
          chat: { id: 1234, type: "private", first_name: "Harold" },
          from: { id: 7, is_bot: true, first_name: "Oriro", username: "oriro_bot" },
          text: "Binding: Review pull request 54118 (oriro)",
        },
      },
      me: { id: 7, is_bot: true, first_name: "Oriro", username: "oriro_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });
  ...
  it("ignores group self-authored message updates instead of re-processing bot output", async () => {
    ...
    await handler({
      message: {
        chat: { id: -1001234, type: "supergroup", title: "Oriro Ops" },
        message_id: 1884,
        date: 1736380800,
        from: { id: 7, is_bot: true, first_name: "Oriro", username: "oriro_bot" },
  ...
  it("reloads topic agent overrides between messages without recreating the bot", async () => {
    ...
    const sendTopicMessage = async (chatId: number, messageId: number, text: string) => {
      await handler({
        ...
        me: { username: "oriro_bot", has_topics_enabled: true },
        getFile: async () => ({ download: async () => new Uint8Array() }),
      });
    };
  ...
  async function dispatchMessage(params: {
    ...
    await handler({
      ...
      me: params.me ?? { username: "oriro_bot" },
      getFile: async () => ({ download: async () => new Uint8Array() }),
    });
  }
  ...
  it("marks explicit Telegram bot-handle mentions in the inbound context", async () => {
    ...
    await dispatchMessage({
      message: {
        chat: { id: 7, type: "group", title: "Test Group" },
        text: "@oriro_bot status",
        entities: [{ type: "mention", offset: 0, length: "@oriro_bot".length }],
        date: 1736380800,
        message_id: 4,
        from: { id: 9, first_name: "Ada" },
      },
      me: { id: 999, username: "oriro_bot" },
    });
    ...
    expect(payload.BotUsername).toBe("oriro_bot");
  });
  ...
  it("honors routed group activation from session store", async () => {
    const storePath = "/tmp/oriro-telegram-group-activation.json";
