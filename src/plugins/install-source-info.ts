...
/** Parsed OriroHub install source metadata for a plugin package. */
export type PluginInstallOriroHubSourceInfo = {
  spec: string;
  packageName: string;
  version?: string;
  exactVersion: boolean;
...
export type PluginInstallSourceInfo = {
  ...
  orirohub?: PluginInstallOriroHubSourceInfo;
...
export function describePluginInstallSource(
  ...
  const orirohubSpec = normalizeOptionalString(install.orirohubSpec);
  ...
  let orirohub: PluginInstallOriroHubSourceInfo | undefined;
  ...
  if (orirohubSpec) {
    ...
    if (parsed) {
      if (!parsed.version) {
        warnings.push("orirohub-spec-floating");
      }
      orirohub = {
        spec: orirohubSpec,
      ...
      warnings.push("invalid-orirohub-spec");
    }
  }
  ...
  if (defaultChoice === "orirohub" && !orirohub) {
