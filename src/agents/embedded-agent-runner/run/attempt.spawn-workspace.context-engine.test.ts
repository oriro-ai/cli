...
describe("runEmbeddedAttempt context engine sessionKey forwarding", () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      toolSearchControlsCase = mockParams(
        hoisted.createOriroCodingToolsMock,
        0,
        "createOriroCodingTools options",
      );
    } finally {
  ...
  it("enables Tool Search controls for embedded Oriro runs when configured", async () => {
  ...
  it("defaults local-model lean embedded runs to Tool Search controls", async () => {
    ...
    expect(hoisted.createOriroCodingToolsMock).toHaveBeenCalledTimes(1);
    ...
    const optionsConfig = requireRecord(options.config, "createOriroCodingTools config");
    const toolsConfig = requireRecord(
      optionsConfig.tools,
      "createOriroCodingTools tools config",
    );
  ...
  it("keeps Tool Search controls off for lean message-tool-only delivery", async () => {
    hoisted.createOriroCodingToolsMock.mockReturnValueOnce([
      {
        name: "message",
        label: "Message",
  ...
  it("quarantines unsupported tool schemas before creating the model session", async () => {
    hoisted.createOriroCodingToolsMock.mockReturnValue([
      {
        name: "healthy_lookup",
  ...
  it("enforces code-mode payload surface from active-agent config during an embedded attempt", async () => {
    ...
    expect(observedOptions.at(-1)?.oriroCodeModeToolSurface).toBe(true);
  ...
  it("sends transcriptPrompt visibly and keeps runtime context out of transcript messages", async () => {
    ...
    const result = await createContextEngineAttemptRunner({
      ...
      attemptOverrides: {
        prompt: [
          "visible ask",
          "",
          "<<<BEGIN_ORIRO_INTERNAL_CONTEXT>>>",
          "secret runtime context",
          "<<<END_ORIRO_INTERNAL_CONTEXT>>>",
        ].join("\n"),
    ...
    expectFields(
      findRecord(
        ...
        (message) => message.customType === "oriro.runtime-context",
        "runtime context message",
      ),
      {
        role: "custom",
        customType: "oriro.runtime-context",
        display: false,
      },
    );
    ...
    for (const value of [
      ...
      expect(String(value)).not.toContain("ORIRO_INTERNAL_CONTEXT");
  ...
  it("rebuilds skill prompt inputs from the sandbox workspace for non-rw sandbox runs", async () => {
    const sandboxWorkspace = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-sandbox-skills-"));
    ...
    await createContextEngineAttemptRunner({
      ...
      attemptOverrides: {
        skillsSnapshot: {
          prompt:
            "<available_skills><skill><location>~/.oriro/skills/smaug/SKILL.md</location></skill></available_skills>",
          skills: [{ name: "smaug" }],
          resolvedSkills: [
            {
              name: "smaug",
              ...
              filePath: "/Users/alice/.oriro/skills/smaug/SKILL.md",
              baseDir: "/Users/alice/.oriro/skills/smaug",
              source: "oriro-workspace",
              sourceInfo: {
                path: "/Users/alice/.oriro/skills/smaug/SKILL.md",
                source: "oriro-workspace",
                scope: "project",
                origin: "top-level",
                baseDir: "/Users/alice/.oriro/skills/smaug",
  ...
  it("keeps bootstrap truncation warnings out of WebChat runtime context", async () => {
    ...
    hoisted.resolveBootstrapContextForRunMock.mockResolvedValueOnce({
      bootstrapFiles: [
        {
          name: "AGENTS.md",
          path: "/tmp/oriro-warning-workspace/AGENTS.md",
          content: "A".repeat(200),
          missing: false,
        },
      ],
      contextFiles: [
        { path: "/tmp/oriro-warning-workspace/AGENTS.md", content: "A".repeat(20) },
      ],
    });
  ...
  it("preserves bootstrap system context in the assembled system prompt", async () => {
    ...
    hoisted.createOriroCodingToolsMock.mockImplementationOnce(() => [
    ...
    hoisted.resolveBootstrapContextForRunMock.mockResolvedValueOnce({
      bootstrapFiles: [
        {
          name: "BOOTSTRAP.md",
          path: "/tmp/oriro-bootstrap-workspace/BOOTSTRAP.md",
  ...
  it("includes hook-adjusted bootstrap files preloaded before routing", async () => {
    const workspaceDir = "/tmp/oriro-hook-workspace";
  ...
  it("adds current-turn context to the current model input without exposing internal runtime context", async () => {
    ...
    expect(seenPrompt).not.toContain("ORIRO_INTERNAL_CONTEXT");
  ...
  it("submits runtime-only context through system prompt without visible prompt", async () => {
    ...
    expect(seenPrompt).toBe("Continue the Oriro runtime event.");
    expect(result.finalPromptText).toBe("Continue the Oriro runtime event.");
  ...
  it("keeps current inbound context visible on runtime-only turns", async () => {
    ...
    expect(seenPrompt).toContain("Continue the Oriro runtime event.");
    expect(result.finalPromptText).toBe(seenPrompt);
    const trajectoryEvents = (
      await fs.readFile(path.join(tempPaths[0] ?? "", "session.trajectory.jsonl"), "utf8")
    )
  ...
  it("submits suppressed room event context as the model prompt", async () => {
      ...
      attemptOverrides: {
        ...
        currentInboundContext: {
          text: [
            ...
            "Current event:\n#2003 Bob: hey oriro summarize the plan",
    ...
    expect(seenPrompt).toContain("[Oriro room event]");
    ...
    expect(seenPrompt?.trim().endsWith("[Oriro room event]")).toBe(true);
    expect(seenPrompt).not.toBe("Continue the Oriro runtime event.");
    ...
    expect(contextCompiled?.data?.prompt).toContain("[Oriro room event]");
  });
