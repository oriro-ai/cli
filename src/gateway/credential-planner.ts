...
type GatewaySecretDefaults = NonNullable<OriroConfig["secrets"]>["defaults"];
 ...
 * This prevents literal placeholder strings like `${ORIRO_GATEWAY_TOKEN}` from being
...
export function hasGatewayTokenEnvCandidate(env: NodeJS.ProcessEnv = process.env): boolean {
  return Boolean(trimToUndefined(env.ORIRO_GATEWAY_TOKEN));
}
...
export function hasGatewayPasswordEnvCandidate(env: NodeJS.ProcessEnv = process.env): boolean {
  return Boolean(trimToUndefined(env.ORIRO_GATEWAY_PASSWORD));
}
...
export function createGatewayCredentialPlan(params: {
  ...
  const envPassword = trimToUndefined(env.ORIRO_GATEWAY_PASSWORD);
