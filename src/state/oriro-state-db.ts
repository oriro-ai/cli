// Oriro state database manages shared persisted state and migrations.
...
import {
  resolveOriroStateSqliteDir,
  resolveOriroStateSqlitePath,
} from "./oriro-state-db.paths.js";
import { ORIRO_STATE_SCHEMA_SQL } from "./oriro-state-schema.generated.js";

/**
 * Shared Oriro SQLite state database lifecycle and metadata writers.
 *
 ...
 */
const ORIRO_STATE_SCHEMA_VERSION = 1;
/** Shared timeout used by state and agent SQLite handles before surfacing busy errors. */
export const ORIRO_SQLITE_BUSY_TIMEOUT_MS = 30_000;
const ORIRO_STATE_DIR_MODE = 0o700;
const ORIRO_STATE_FILE_MODE = 0o600;

/** Open shared SQLite database handle plus WAL maintenance lifecycle. */
export type OriroStateDatabase = {
  db: DatabaseSync;
  path: string;
...
export type OriroStateDatabaseOptions = {
  env?: NodeJS.ProcessEnv;
  path?: string;
};

export type OriroStateDatabaseSchemaMigration = {
  kind: "agent-databases-composite-primary-key";
  path: string;
};

const cachedDatabases = new Map<string, OriroStateDatabase>();

type OriroStateMetadataDatabase = Pick<OriroStateKyselyDatabase, "schema_meta">;
...
function ensureOriroStatePermissions(pathname: string, env: NodeJS.ProcessEnv): void {
  const dir = path.dirname(pathname);
  const defaultDir = resolveOriroStateSqliteDir(env);
  ...
  if (isDefaultStateDatabase || !dirExisted) {
    bestEffortChmodSync(dir, ORIRO_STATE_DIR_MODE);
  }
  for (const candidate of resolveSqliteDatabaseFilePaths(pathname)) {
    if (existsSync(candidate)) {
      bestEffortChmodSync(candidate, ORIRO_STATE_FILE_MODE);
    }
  }
}
...
export function detectOriroStateDatabaseSchemaMigrations(
  options: OriroStateDatabaseOptions = {},
): OriroStateDatabaseSchemaMigration[] {
...
export function repairOriroStateDatabaseSchema(options: OriroStateDatabaseOptions = {}): {
  changes: string[];
  warnings: string[];
} {
  ...
  ensureOriroStatePermissions(pathname, env);
  const sqlite = requireNodeSqlite();
  const db = new sqlite.DatabaseSync(pathname);
  db.exec(`PRAGMA busy_timeout = ${ORIRO_SQLITE_BUSY_TIMEOUT_MS};`);
  try {
...
function ensureSchema(db: DatabaseSync, pathname: string): void {
  ...
  db.exec(ORIRO_STATE_SCHEMA_SQL);
  ensureAdditiveStateColumns(db);
  db.exec(`PRAGMA user_version = ${ORIRO_STATE_SCHEMA_VERSION};`);
  const now = Date.now();
  const kysely = getNodeSqliteKysely<OriroStateMetadataDatabase>(db);
  executeSqliteQuerySync(
    db,
    kysely
      ...
      .values({
        meta_key: "primary",
        role: "global",
        schema_version: ORIRO_STATE_SCHEMA_VERSION,
        agent_id: null,
        app_version: null,
        created_at: now,
        updated_at: now,
...
function resolveDatabasePath(options: OriroStateDatabaseOptions = {}): string {
  return path.resolve(options.path ?? resolveOriroStateSqlitePath(options.env ?? process.env));
}

/** Open or return a cached shared state database after schema and migration checks. */
export function openOriroStateDatabase(
  options: OriroStateDatabaseOptions = {},
): OriroStateDatabase {
  const env = options.env ?? process.env;
  const pathname = resolveDatabasePath(options);
...
export function runOriroStateWriteTransaction<T>(
  operation: (database: OriroStateDatabase) => T,
  options: OriroStateDatabaseOptions = {},
): T {
  const database = openOriroStateDatabase(options);
  const result = runSqliteImmediateTransactionSync(database.db, () => operation(database));
  try {
    ensureOriroStatePermissions(database.path, options.env ?? process.env);
  } catch {
...
export function closeOriroStateDatabase(): void {
...
export function isOriroStateDatabaseOpen(): boolean {
  return Array.from(cachedDatabases.values()).some((database) => database.db.isOpen);
}

/** Test alias for closing shared state handles from teardown code. */
export const closeOriroStateDatabaseForTest = closeOriroStateDatabase;
