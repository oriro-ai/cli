/**
 * Builds the effective Oriro agent tool surface.
 * Assembles core, shell, channel, Oriro, plugin, and Tool Search tools, then
...
function resolveExecConfig(params: { cfg?: OriroConfig; agentId?: string }) {
...
export type OriroCodingToolConstructionPlan = {
  includeBaseCodingTools: boolean;
  includeShellTools: boolean;
  includeChannelTools: boolean;
  includeOriroTools: boolean;
  includePluginTools: boolean;
};

/** Build the runtime tool list for one agent run. */
export function createOriroCodingTools(options?: {
  agentId?: string;
  ...
  modelCompat?: ModelCompatConfig;
  /** If false, keep Oriro web_search even when a provider-native search tool is active. */
  suppressManagedWebSearch?: boolean;
  /**
  ...
  toolConstructionPlan?: OriroCodingToolConstructionPlan;
  ...
  const toolConstructionPlan = options?.toolConstructionPlan ?? {
    ...
    includeOriroTools: includeCoreTools,
  ...
  const includeOriroTools = includeCoreTools && toolConstructionPlan.includeOriroTools;
  ...
  if (includeBaseCodingTools) {
    for (const tool of createCodingTools(codingRoot) as unknown as AnyAgentTool[]) {
      if (tool.name === "read") {
        ...
        const wrapped = createOriroReadTool(freshReadTool, {
          modelContextWindowTokens: options?.modelContextWindowTokens,
          imageSanitization,
        });
        base.push(
  ...
  const pluginToolsOnly =
    includeOriroTools || !includePluginTools
      ? []
  ...
  options?.recordToolPrepStage?.("oriro-tools");
  ...
  // Always normalize tool JSON Schemas before handing them to Oriro model runtime.
