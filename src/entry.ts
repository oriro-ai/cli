#!/usr/bin/env node
// Boots the Oriro CLI entry point under Node.
// CLI process entrypoint for Oriro command execution.
...
import {
  enableOriroCompileCache,
  resolveEntryInstallRoot,
  respawnWithoutOriroCompileCacheIfNeeded,
} from "./entry.compile-cache.js";
import { buildCliRespawnPlan, runCliRespawnPlan } from "./entry.respawn.js";
import { tryHandleRootVersionFastPath } from "./entry.version-fast-path.js";
import { isTruthyEnvValue, normalizeEnv } from "./infra/env.js";
import { isMainModule } from "./infra/is-main.js";
import { ensureOriroExecMarkerOnProcess } from "./infra/oriro-exec-env.js";
import { installProcessWarningFilter } from "./infra/warning-filter.js";

const ENTRY_WRAPPER_PAIRS = [
  { wrapperBasename: "oriro.mjs", entryBasename: "entry.js" },
  { wrapperBasename: "oriro.js", entryBasename: "entry.js" },
] as const;
...
if (
  ...
  const waitingForCompileCacheRespawn = respawnWithoutOriroCompileCacheIfNeeded({
  ...
  if (!waitingForCompileCacheRespawn) {
    ...
    if (shouldForceReadOnlyAuthStore(process.argv)) {
      process.env.ORIRO_AUTH_STORE_READONLY = "1";
    }
    ...
    if (!ensureCliRespawnReady()) {
      ...
      if (!parsedContainer.ok) {
        console.error(`[oriro] ${parsedContainer.error}`);
        process.exit(2);
      }
      ...
      if (!parsed.ok) {
        ...
        console.error(`[oriro] ${parsed.error}`);
        process.exit(2);
      }

      const containerTargetName = resolveCliContainerTarget(process.argv);
      if (containerTargetName && parsed.profile) {
        console.error("[oriro] --container cannot be combined with --profile/--dev");
        process.exit(2);
      }
...
export async function tryHandlePrecomputedCommandHelpFastPath(
  ...
  if (env.ORIRO_DISABLE_CLI_STARTUP_HELP_FAST_PATH === "1") {
    return false;
  }
