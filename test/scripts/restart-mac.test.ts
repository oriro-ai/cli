...
function runGatewayPortCheck(fakeLsof: string) {
  const root = mkdtempSync(join(tmpdir(), "oriro-restart-mac-test-"));
  tempRoots.push(root);
...
function runCleanupFunction(fakePs: string) {
  ...
  writeFileSync(
    ...
    [
      ...
      'DEBUG_PROCESS_PATTERN="/worktree/apps/macos/.build/debug/Oriro"',
      'LOCAL_PROCESS_PATTERN="/worktree/apps/macos/.build-local/debug/Oriro"',
      'RELEASE_PROCESS_PATTERN="/worktree/apps/macos/.build/release/Oriro"',
      "kill() {",
      '  printf "%s\\n" "$*" >> "$ORIRO_TEST_KILL_CALLS"',
      "  return 0",
      "}",
      "kill_all_oriro",
    ].join("\n"),
  ...
  const result = spawnSync("bash", [harnessPath], {
    ...
    env: {
      ...process.env,
      ORIRO_TEST_KILL_CALLS: killCallsPath,
      PATH: `${binDir}:${process.env.PATH ?? ""}`,
    },
  });
...
describe("scripts/restart-mac.sh", () => {
  ...
  it("keeps the default restart log scoped to the current worktree lock", () => {
    ...
    expect(script).not.toContain('LOG_PATH="${ORIRO_RESTART_LOG:-/tmp/oriro-restart.log}"');
  });
  ...
  it("prefers the freshly packaged app unless an explicit app bundle is set", () => {
    ...
    expect(script).toContain('fail "ORIRO_APP_BUNDLE does not exist: ${APP_BUNDLE}"');
    expect(chooseBlock).toContain("canonicalize_app_bundle");
    expect(chooseBlock.indexOf("${ROOT_DIR}/dist/Oriro.app")).toBeGreaterThan(-1);
    expect(chooseBlock.indexOf("/Applications/Oriro.app")).toBeGreaterThan(-1);
    expect(chooseBlock.indexOf("${ROOT_DIR}/dist/Oriro.app")).toBeLessThan(
      chooseBlock.indexOf("/Applications/Oriro.app"),
    );
  });

  it("keeps restart cleanup scoped to known Oriro app and build paths", () => {
    const script = readFileSync(restartScriptPath, "utf8");
    ...
    expect(cleanupBlock).toContain('"/Applications/Oriro.app/${APP_EXECUTABLE_RELATIVE_PATH}"');
  ...
  it("normalizes custom app bundle paths before process matching", () => {
    const root = mkdtempSync(join(tmpdir(), "oriro-restart-mac-test-"));
    tempRoots.push(root);
    const appBundle = join(root, "dist", "Oriro.app");
    mkdirSync(appBundle, { recursive: true });

    const { result } = runCanonicalizeAppBundle(`${appBundle}/../Oriro.app/`);
  ...
  it("fails restart cleanup when scoped processes survive every kill attempt", () => {
    const { killCalls, result } = runCleanupFunction(
      [
        "#!/usr/bin/env bash",
        "printf '%s\\n' '  321 /worktree/dist/Oriro.app/Contents/MacOS/Oriro --attach-only'",
      ].join("\n"),
    );
  ...
  it("passes restart cleanup when the final kill attempt clears the process", () => {
    const { killCalls, result } = runCleanupFunction(
      [
        "#!/usr/bin/env bash",
        'kill_count="$(wc -l < "$ORIRO_TEST_KILL_CALLS" 2>/dev/null || echo 0)"',
  ...
  it("does not kill unrelated Oriro app bundles", () => {
    const { killCalls, result } = runCleanupFunction(
      [
        "#!/usr/bin/env bash",
        "printf '%s\\n' '  654 /tmp/Other/Oriro.app/Contents/MacOS/Oriro'",
      ].join("\n"),
    );
