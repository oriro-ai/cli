...
function createSnapshot(config: OriroConfig): PreparedSecretsRuntimeSnapshot {
  return {
...
function activateSnapshot(config: OriroConfig) {
  activateSecretsRuntimeSnapshot(createSnapshot(config));
}

function mockResolvedSecrets(config: OriroConfig) {
...
// set ORIRO_SKIP_CHANNELS / ORIRO_SKIP_PROVIDERS at module load. When a
