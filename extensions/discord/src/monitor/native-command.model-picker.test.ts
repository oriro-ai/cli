...
import * as commandRegistryModule from "oriro/plugin-sdk/command-auth-native";
import type {
  ChatCommandDefinition,
  CommandArgsParsing,
} from "oriro/plugin-sdk/command-auth-native";
import type { ModelsProviderData } from "oriro/plugin-sdk/command-auth-native";
import type { OriroConfig } from "oriro/plugin-sdk/config-contracts";
import * as globalsModule from "oriro/plugin-sdk/runtime-env";
import {
  loadSessionStore,
  resolveStorePath,
  upsertSessionEntry,
} from "oriro/plugin-sdk/session-store-runtime";
import * as commandTextModule from "oriro/plugin-sdk/text-utility-runtime";
...
describe("Discord model picker interactions", () => {
  beforeEach(async () => {
    tempDir = await mkdtemp(path.join(os.tmpdir(), "oriro-discord-model-picker-"));
    vi.useRealTimers();
    vi.restoreAllMocks();
  });
    ...
    pickerData.runtimeChoicesByProvider = new Map([
      [
        ...
        [
          ...
          { id: "oriro", label: "Oriro Default", description: "Use Oriro." },
        ],
      ],
    ]);
    const modelCommand = createModelCommandDefinition();
