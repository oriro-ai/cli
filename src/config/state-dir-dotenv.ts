...
export function isUnresolvedShellReference(value: string): boolean {
  ...
  //   $VAR_NAME          (simple reference, Oriro env-var style)
...
type ParsedStateDirDotEnv = {
   ...
   * reference ($VAR, ${VAR}, or $(cmd)). These are still Oriro-managed keys:
 ...
 * literal references for keys Oriro previously managed.
 */
...
/**
 * Read and parse `~/.oriro/.env` (or `$ORIRO_STATE_DIR/.env`), returning
