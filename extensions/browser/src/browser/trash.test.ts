...
const resolvePreferredOriroTmpDirMock = vi.hoisted(() => vi.fn(() => "/tmp/oriro"));
...
describe("browser trash", () => {
  ...
  beforeEach(() => {
    vi.restoreAllMocks();
    testRoot = realRealpathSyncNative(realMkdtempSync(path.join(os.tmpdir(), "oriro-browser-")));
    ...
    resolvePreferredOriroTmpDirMock.mockReturnValue(tmpDir);
  ...
  it("refuses to trash paths outside allowed roots", async () => {
    ...
    const outsidePath = path.join(outsideDir, "oriro-demo");
