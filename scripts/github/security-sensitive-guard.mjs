...
export const securitySensitiveGuardMarker = "<!-- oriro:security-sensitive-guard -->";
...
const securityTeamSlug = process.env.ORIRO_SECURITY_TEAM_SLUG ?? "oriro-secops";
...
export function renderSecuritySensitiveAwarenessComment(changes) {
  return [
    ...
    "- If this PR intentionally needs the change, a repository admin or member of `@oriro-ai/cli-secops` must approve the exact head SHA.",
  ].join("\n");
}
...
export function renderAuthorizedSecuritySensitiveComment(override) {
  const lines = [
    ...
    "",
    "This PR includes security-sensitive file changes. A repository admin or member of `@oriro-ai/cli-secops` authorized this exact head SHA with `/allow-security-sensitive-change`.",
    "",
...
export function renderTrustedSecuritySensitiveComment({ actor, headSha, changes }) {
  return [
    ...
    "",
    "This PR includes security-sensitive file changes. The guard is informational because the PR author is a repository admin or a member of `@oriro-ai/cli-secops`.",
    "",
...
export function renderBlockedSecuritySensitiveComment({ headSha, changes }) {
  return [
    ...
    "",
    "Oriro does not accept security-sensitive file changes through PRs unless a repository admin or security explicitly authorizes the current head SHA.",
    "",
    "Detected security-sensitive changes:",
    ...renderChangedFileLines(changes),
    "",
    "If this PR intentionally needs these changes, ask a repository admin or member of `@oriro-ai/cli-secops` to comment:",
    "",
    "```text",
...
export function githubApi(token, options = {}) {
  return createGitHubApi(token, {
    ...options,
    userAgent: "oriro-security-sensitive-guard",
  });
}
...
async function main() {
  ...
  const explicitSecurityApprovers = securityApproverSet(process.env.ORIRO_SECURITY_APPROVERS);
  const trustedCommentAuthors = securitySensitiveGuardCommentAuthors(
    process.env.ORIRO_SECURITY_SENSITIVE_GUARD_COMMENT_BOTS,
  );
  ...
  const mode = process.env.ORIRO_SECURITY_SENSITIVE_GUARD_MODE ?? "enforce";
