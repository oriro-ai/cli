...
struct TalkProTab: View {
    ...
    private var header: some View {
        HStack(alignment: .center, spacing: 11) {
            ...
            OriroProMark(size: 31, shadowRadius: 9)
...
struct TalkProState: Equatable {
    ...
    var title: String {
        ...
        if self.normalizedStatus.contains("thinking") { return "Asking Oriro" }
    ...
    var color: Color {
        ...
        case .requestFailed, .loadFailed:
            return OriroBrand.danger
        case .missingScope, .requestingUpgrade, .upgradeRequested, .apiKeyMissing:
            return OriroBrand.warn
        default:
            if !self.isConfigLoaded { return OriroBrand.warn }
            return self.isEnabled ? OriroBrand.ok : OriroBrand.accentHot
        }
    }
    ...
    var primaryButtonFill: AnyShapeStyle {
        switch self.primaryAction {
        case .stop:
            AnyShapeStyle(OriroBrand.danger)
        case .waiting:
            AnyShapeStyle(OriroBrand.warn.opacity(0.72))
        default:
            AnyShapeStyle(LinearGradient(
                colors: [self.color.opacity(0.95), OriroBrand.accent],
