...
export async function resolveApnsAuthConfigFromEnv(
  ...
  const teamId = normalizeNonEmptyString(env.ORIRO_APNS_TEAM_ID);
  const keyId = normalizeNonEmptyString(env.ORIRO_APNS_KEY_ID);
  if (!teamId || !keyId) {
    return {
      ok: false,
      error: "APNs auth missing: set ORIRO_APNS_TEAM_ID and ORIRO_APNS_KEY_ID",
    };
  }

  const inlineKeyRaw =
    normalizeNonEmptyString(env.ORIRO_APNS_PRIVATE_KEY_P8) ??
    normalizeNonEmptyString(env.ORIRO_APNS_PRIVATE_KEY);
  if (inlineKeyRaw) {
    return {
      ok: true,
      value: {
  ...
  const keyPath = normalizeNonEmptyString(env.ORIRO_APNS_PRIVATE_KEY_PATH);
  if (!keyPath) {
    return {
      ok: false,
      error:
        "APNs private key missing: set ORIRO_APNS_PRIVATE_KEY_P8 or ORIRO_APNS_PRIVATE_KEY_PATH",
    };
  }
  try {
    ...
    return {
      ok: false,
      error: `failed reading ORIRO_APNS_PRIVATE_KEY_PATH (${keyPath}): ${message}`,
    };
  }
}
