...
const pluginManifestRegistryMocks = vi.hoisted(() => ({
  ...
  listOriroPluginManifestMetadata: vi.fn(),
}));
...
vi.mock("../plugins/manifest-metadata-scan.js", async (importOriginal) => {
  ...
  return {
    ...
    listOriroPluginManifestMetadata: (
      params?: Parameters<typeof actual.listOriroPluginManifestMetadata>[0],
    ) => {
      pluginManifestRegistryMocks.listOriroPluginManifestMetadata(params);
      return actual.listOriroPluginManifestMetadata(params);
    },
  };
});
...
describe("model-pricing-cache", () => {
  beforeEach(() => {
    ...
    pluginManifestRegistryMocks.listOriroPluginManifestMetadata.mockClear();
  ...
  it("collects manifest-owned web search plugin model refs without a hardcoded plugin list", () => {
    ...
    } as OriroConfig).map((ref) => modelKey(ref.provider, ref.model));
  ...
  it("does not load plugin manifests for pricing when plugins are globally disabled", async () => {
    ...
    expect(pluginManifestRegistryMocks.listOriroPluginManifestMetadata).not.toHaveBeenCalled();
...
function createManifestRecord(overrides: Partial<PluginManifestRecord>): PluginManifestRecord {
  return {
    ...
    manifestPath: "/tmp/plugin/oriro.plugin.json",
    ...overrides,
  };
}
