// Zai Fallback Repro script supports Oriro repository automation.
...
export async function runZaiFallbackRepro(deps: RunZaiFallbackReproDeps = {}): Promise<number> {
  ...
  const baseDir = await mkdtemp(path.join(os.tmpdir(), "oriro-zai-fallback-"));
  const stateDir = path.join(baseDir, "state");
  const configPath = path.join(baseDir, "oriro.json");
  try {
    ...
    const sessionId = env.ORIRO_ZAI_FALLBACK_SESSION_ID ?? createUuid();
    ...
    const run1 = await run(
      "run1",
      ["oriro", "agent", "--local", "--session-id", sessionId, "--message", toolPrompt],
      envValidAnthropic,
    );
    if (run1.code !== 0) {
      return run1.code ?? 1;
    ...
    const run2 = await run(
      "run2",
      ["oriro", "agent", "--local", "--session-id", sessionId, "--message", followupPrompt],
      envInvalidAnthropic,
    );
