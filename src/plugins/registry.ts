...
import { uniqueValues } from "@oriro/normalization-core/string-normalization";
import {
  normalizeStringEntries,
  normalizeUniqueStringEntries,
} from "@oriro/normalization-core/string-normalization";
...
import type {
  ...
  OriroPluginApi,
  OriroPluginChannelRegistration,
  OriroPluginCliCommandDescriptor,
  OriroPluginCliRegistrar,
  OriroPluginCommandDefinition,
  PluginConversationBindingResolvedEvent,
  OriroPluginGatewayRuntimeScopeSurface,
  OriroGatewayDiscoveryService,
  OriroPluginHostedMediaResolver,
  OriroPluginHttpRouteParams,
  OriroPluginHookOptions,
  OriroPluginNodeHostCommand,
  OriroPluginNodeInvokePolicy,
  OriroPluginReloadRegistration,
  OriroPluginSecurityAuditCollector,
...
function isOfficialCodexPluginRecord(
  ...
  if (record.packageName === "@oriro/codex") {
    return true;
  }
  const sourcePath = path
    ...
    .split(path.sep)
    .join("/");
  return sourcePath.includes("/node_modules/@oriro/codex");
}
...
const ACTIVE_PLUGIN_HOOK_REGISTRATIONS_KEY = Symbol.for("oriro.activePluginHookRegistrations");
...
export function createPluginRegistry(registryParams: PluginRegistryParams) {
  ...
  const registerCodexAppServerExtensionFactory = (
    record: PluginRecord,
    factory: Parameters<OriroPluginApi["registerCodexAppServerExtensionFactory"]>[0],
  ) => {
  ...
  const registerAgentToolResultMiddleware = (
    record: PluginRecord,
    handler: Parameters<OriroPluginApi["registerAgentToolResultMiddleware"]>[0],
    options: Parameters<OriroPluginApi["registerAgentToolResultMiddleware"]>[1],
  ) => {
  ...
  const registerTool = (
    ...
    const factory: OriroPluginToolFactory =
      typeof tool === "function" ? tool : (_ctx: OriroPluginToolContext) => tool;
  ...
  const registerHook = (
    ...
    opts: OriroPluginHookOptions | undefined,
    config: OriroPluginApi["config"],
  ...
  const registerHttpRoute = (record: PluginRecord, params: OriroPluginHttpRouteParams) => {
  ...
  const registerHostedMediaResolver = (
    record: PluginRecord,
    resolver: OriroPluginHostedMediaResolver,
  ) => {
  ...
  const registerChannel = (
    record: PluginRecord,
    registration: OriroPluginChannelRegistration | ChannelPlugin,
    ...
    const normalized =
      typeof (registration as OriroPluginChannelRegistration).plugin === "object"
        ? (registration as OriroPluginChannelRegistration)
  ...
  const registerCli = (
    ...
    const descriptors = (opts?.descriptors ?? [])
      ...
      .filter(
        (descriptor): descriptor is OriroPluginCliCommandDescriptor => descriptor !== null,
      );
    const commands = [
      ...(opts?.commands ?? []),
  ...
  const registerReload = (record: PluginRecord, registration: OriroPluginReloadRegistration) => {
    const normalized: OriroPluginReloadRegistration = {
  ...
  const registerNodeHostCommand = (
    record: PluginRecord,
    nodeCommand: OriroPluginNodeHostCommand,
  ) => {
  ...
  const registerNodeInvokePolicy = (
    record: PluginRecord,
    policy: OriroPluginNodeInvokePolicy,
  ...
  const registerSecurityAuditCollector = (
    record: PluginRecord,
    collector: OriroPluginSecurityAuditCollector,
  ) => {
  ...
  const registerService = (record: PluginRecord, service: OriroPluginService) => {
  ...
  const registerGatewayDiscoveryService = (
    record: PluginRecord,
    service: OriroGatewayDiscoveryService,
  ) => {
    const id = service.id.trim();
    if (!id) {
      return;
  ...
  const registerCommand = (record: PluginRecord, command: OriroPluginCommandDefinition) => {
      ...
      handlers: {
        ...(registrationCapabilities.capabilityHandlers
          ? {
              ...
              registerCompactionProvider: (
                provider: Parameters<OriroPluginApi["registerCompactionProvider"]>[0],
              ) => {
                const id = normalizeOptionalString(
                  (
                    provider as Partial<
                      Parameters<OriroPluginApi["registerCompactionProvider"]>[0]
                    > | null
                  )?.id,
                );
                if (!id) {
              ...
              enqueueNextTurnInjection: (injection) => {
                ...
                return enqueuePluginNextTurnInjection({
                  cfg: registryParams.runtime.config.current() as OriroConfig,
                  pluginId: record.id,
                  pluginName: record.name,
                  injection,
                });
              ...
              sendSessionAttachment: async (attachment) => {
                ...
                try {
                  ...
                  const runtimeConfig =
                    (registryParams.runtime.config?.current?.() as OriroConfig | undefined) ??
                    params.config;
