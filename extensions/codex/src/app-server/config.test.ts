...
import { MAX_TIMER_TIMEOUT_MS } from "oriro/plugin-sdk/number-runtime";
...
import {
  ...
  resolveOriroExecModeForCodexAppServer,
  resolveOriroExecModeFromConfig,
  resolveOriroExecPolicyForCodexAppServer,
...
describe("Codex app-server config", () => {
  it("only auto-approves app-server approvals for full yolo runtime policy", () => {
    ...
    expect(
      shouldAutoApproveCodexAppServerApprovals({
        ...
        sandbox: "danger-full-access",
        networkProxy: {
          profileName: "oriro-network",
          configFingerprint: "network-proxy-v1",
          configPatch: {
            "features.network_proxy.enabled": true,
            default_permissions: "oriro-network",
            permissions: {},
          },
        },
      }),
  ...
  it("parses typed plugin config before falling back to environment knobs", () => {
    const runtime = resolveRuntimeForTest({
      ...
      env: {
        ORIRO_CODEX_APP_SERVER_APPROVAL_POLICY: "never",
        ORIRO_CODEX_APP_SERVER_SANDBOX: "read-only",
      },
  ...
  it("uses read-only filesystem rules for read-only network proxy profiles", () => {
    ...
    expect(profileName).toMatch(/^oriro-network-[a-f0-9]{16}$/u);
    ...
    expect(
      readCodexPluginConfig({
        appServer: {
          ...
          remoteWorkspace: {
            localRoot: "/Users/kevinlin/code/oriro",
            remoteRoot: "/home/oai/oriro-workspaces",
          },
        },
      }),
    ).toStrictEqual({});
    ...
    const runtime = resolveRuntimeForTest({
      pluginConfig: {
        appServer: {
          ...
          url: "wss://codex-app-server.example.internal/ws",
          authToken: "capability-token",
          remoteWorkspaceRoot: " /home/oai/oriro-workspaces ",
        },
      },
    });
  ...
  it("treats only explicit OpenAI model context as safe for Codex-backed auto-review", () => {
    ...
    for (const openAIProvider of [
      ...
      {
        baseUrl: "https://api.openai.com/v1",
        headers: { "x-oriro-reviewer-proxy": "local" },
        models: [],
      },
      {
        baseUrl: "https://api.openai.com/v1",
...
`,
  ...
  it("keeps explicit yolo mode when requirements disallow full access", () => {
    ...
    expectRuntimePolicy(
      resolveRuntimeForTest({
        pluginConfig: {},
        env: { ORIRO_CODEX_APP_SERVER_MODE: "yolo" },
        requirementsToml,
      }),
      {
        approvalPolicy: "never",
  ...
  it("rejects the retired dynamic tool profile key", () => {
    expect(
      readCodexPluginConfig({
        codexDynamicToolsProfile: "oriro-compat",
        codexDynamicToolsLoading: "direct",
      }),
    ).toEqual({});
  });
  ...
  it("treats configured and environment commands as explicit overrides", () => {
    expectFields(
      resolveRuntimeForTest({
        ...
        env: { ORIRO_CODEX_APP_SERVER_BIN: "/usr/local/bin/codex" },
      }).start,
      "configured start",
      {
  ...
  it("rejects Codex app-server command overrides that include inline arguments", () => {
    ...
    ).toThrow("ORIRO_CODEX_APP_SERVER_BIN must be only the Codex app-server executable path");
  });
  ...
  it("resolves Computer Use setup from plugin config and environment fallbacks", () => {
    expect(
      resolveCodexComputerUseConfig({
        ...
        env: {
          ORIRO_CODEX_COMPUTER_USE_PLUGIN_NAME: "env-fallback-plugin",
        },
      }),
    ).toEqual({
      enabled: true,
    ...
    expectFields(
      resolveCodexComputerUseConfig({
        pluginConfig: {},
        env: {
          ORIRO_CODEX_COMPUTER_USE: "1",
          ORIRO_CODEX_COMPUTER_USE_MARKETPLACE_SOURCE: "github:example/plugins",
          ORIRO_CODEX_COMPUTER_USE_AUTO_INSTALL: "true",
          ORIRO_CODEX_COMPUTER_USE_MARKETPLACE_DISCOVERY_TIMEOUT_MS: "30000",
        },
      }),
      "computer use config",
      {
    ...
    for (const value of ["0x10", "1e3"]) {
      expectFields(
        resolveCodexComputerUseConfig({
          ...
          env: {
            ORIRO_CODEX_COMPUTER_USE: "1",
            ORIRO_CODEX_COMPUTER_USE_MARKETPLACE_DISCOVERY_TIMEOUT_MS: value,
          },
        }),
        "computer use config",
        {
  ...
  it("allows environment mode fallback to opt in to guardian-reviewed local execution", () => {
    const runtime = resolveRuntimeForTest({
      pluginConfig: {},
      modelProvider: "openai",
      env: { ORIRO_CODEX_APP_SERVER_MODE: "guardian" },
    });
  ...
  it("maps normalized Oriro auto exec mode to guardian-reviewed local execution", () => {
    const runtime = resolveRuntimeForTest({
      pluginConfig: {},
      execMode: "auto",
      modelProvider: "openai",
  ...
  it("forces guarded app-server policy fields for auto mode", () => {
    const runtime = resolveRuntimeForTest({
      ...
      env: {
        ORIRO_CODEX_APP_SERVER_APPROVAL_POLICY: "never",
        ORIRO_CODEX_APP_SERVER_SANDBOX: "danger-full-access",
      },
      execMode: "auto",
      modelProvider: "openai",
    });
  ...
  it("preserves explicit read-only app-server sandbox for auto mode", () => {
    ...
    const envRuntime = resolveRuntimeForTest({
      ...
      modelProvider: "openai",
      env: {
        ORIRO_CODEX_APP_SERVER_MODE: "yolo",
        ORIRO_CODEX_APP_SERVER_APPROVAL_POLICY: "never",
        ORIRO_CODEX_APP_SERVER_SANDBOX: "read-only",
      },
    });
  ...
  it.each(["deny", "allowlist"] as const)(
    "blocks Codex app-server local execution for normalized Oriro %s exec mode",
    (execMode) => {
      expect(() =>
        resolveRuntimeForTest({
          pluginConfig: {},
  ...
  it("maps normalized Oriro ask exec mode away from Codex yolo", () => {
    const runtime = resolveRuntimeForTest({
      pluginConfig: {},
      execMode: "ask",
    });
  ...
  it("fails closed when normalized Oriro ask mode cannot use user approvals", () => {
    expect(() =>
      resolveRuntimeForTest({
        pluginConfig: {},
        execMode: "ask",
  ...
  it.each([
    ...
    "fails closed when normalized Oriro $execMode mode can only use $policies approvals",
    ({ execMode, policies }) => {
      expect(() =>
        resolveRuntimeForTest({
          pluginConfig: {},
  ...
  it("keeps normalized Oriro full exec mode on default Codex yolo", () => {
    const runtime = resolveRuntimeForTest({
      pluginConfig: {},
      execMode: "full",
    });
  ...
  it("fails closed when normalized Oriro auto mode can only use on-failure approvals", () => {
    expect(() =>
      resolveRuntimeForTest({
        pluginConfig: {},
        execMode: "auto",
  ...
  it("fails closed when normalized Oriro auto mode cannot force prompting over yolo", () => {
    expect(() =>
      resolveRuntimeForTest({
        pluginConfig: {},
        execMode: "auto",
  ...
  it("uses user approvals when normalized Oriro auto mode cannot use Codex auto-review", () => {
    const runtime = resolveRuntimeForTest({
      pluginConfig: {},
      execMode: "auto",
      requirementsToml:
  ...
  it("keeps normalized Oriro auto mode when legacy app-server yolo was schema-defaulted", () => {
    const runtime = resolveRuntimeForTest({
      pluginConfig: {
        appServer: {
          command: "codex",
  ...
  it("forces guarded policy fields for normalized Oriro auto mode", () => {
    const runtime = resolveRuntimeForTest({
      pluginConfig: {
        appServer: {
          approvalPolicy: "never",
  ...
  it("resolves agent-scoped normalized Oriro exec mode for Codex app-server mapping", () => {
    const config = {
      tools: {
        exec: {
          mode: "ask",
    ...
    expect(resolveOriroExecModeFromConfig({ config, agentId: "codex-agent" })).toBe("auto");
    expect(resolveOriroExecModeFromConfig({ config, agentId: "other-agent" })).toBe("ask");
  });

  it("keeps legacy exec security overrides ahead of normalized Oriro exec mode", () => {
    expect(
      resolveOriroExecModeFromConfig({
        config: {
          tools: {
  ...
  it.each(["always"] as const)(
    ...
    (ask) => {
      ...
      const execPolicy = resolveOriroExecPolicyForCodexAppServer({ config });

      expect(resolveOriroExecModeForCodexAppServer({ config })).toBe("ask");
      expectRuntimePolicy(
        resolveRuntimeForTest({
          pluginConfig: {
            appServer: {
  ...
  it("keeps legacy full exec security with ask=on-miss on default Codex yolo", () => {
    ...
    expect(resolveOriroExecModeForCodexAppServer({ config })).toBe("full");
  ...
  it("clamps legacy full exec with ask when an Oriro sandbox is active", () => {
    const config = {
      tools: {
        exec: {
          security: "full",
  ...
  it("applies host exec approval security floors before starting Codex app-server", () => {
    const execPolicy = resolveOriroExecPolicyForCodexAppServer({
      config: {
        tools: {
          exec: {
            mode: "full",
  ...
  it("treats ask-only legacy overrides as normalized mode overrides", () => {
    ...
    expect(resolveOriroExecModeFromConfig({ config, agentId: "codex-agent" })).toBe("allowlist");
    const execMode = resolveOriroExecModeForCodexAppServer({
      config,
      agentId: "main",
      execOverrides: {
        ask: "always",
  ...
  it("ignores removed ORIRO_CODEX_APP_SERVER_GUARDIAN fallback", () => {
    const runtime = resolveRuntimeForTest({
      pluginConfig: {},
      env: { ORIRO_CODEX_APP_SERVER_GUARDIAN: "1" },
    });
  ...
  it("keeps runtime config keys aligned with manifest schema and UI hints", async () => {
    const manifest = JSON.parse(
      await fs.readFile(new URL("../../oriro.plugin.json", import.meta.url), "utf8"),
    ) as {
      configSchema: {
        properties: {
