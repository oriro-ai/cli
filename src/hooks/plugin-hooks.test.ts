...
describe("bundle plugin hooks", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fsp.mkdtemp(path.join(os.tmpdir(), "oriro-plugin-hooks-"));
  });
  ...
  beforeEach(async () => {
    ...
    previousBundledHooksDir = process.env.ORIRO_BUNDLED_HOOKS_DIR;
    process.env.ORIRO_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
  ...
  afterEach(() => {
    ...
    if (previousBundledHooksDir === undefined) {
      delete process.env.ORIRO_BUNDLED_HOOKS_DIR;
    } else {
      process.env.ORIRO_BUNDLED_HOOKS_DIR = previousBundledHooksDir;
    }
  });
  ...
  async function writeBundleHookFixture(): Promise<string> {
    const bundleRoot = path.join(workspaceDir, ".oriro", "extensions", "sample-bundle");
  ...
  function createConfig(enabled: boolean): OriroConfig {
    return {
      hooks: {
        internal: {
          enabled: true,
  ...
  it("exposes enabled bundle hook dirs as plugin-managed hook entries", async () => {
    ...
    expect(entry.hook.source).toBe("oriro-plugin");
  ...
  it("does not treat Claude hooks.json bundles as Oriro hook packs", async () => {
    const bundleRoot = path.join(workspaceDir, ".oriro", "extensions", "claude-bundle");
