...
beforeAll(async () => {
  sandboxFixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-sandbox-context-"));
});
...
describe("resolveSandboxContext", () => {
  ...
  it("materializes skills into a hidden read-only workspace for writable sandboxes", async () => {
      ...
      agents: {
        defaults: {
          sandbox: {
            ...
            scope: "session",
            workspaceAccess: "rw",
            workspaceRoot: path.join(workspaceDir, ".oriro", "sandboxes"),
          },
        },
      },
    };
    ...
    expect(syncOptions?.targetWorkspaceDir).toContain(
      path.join(".oriro", "sandbox", "skills-workspaces"),
    );
    expect(syncOptions?.targetWorkspaceDir).toMatch(
      /[\\/]agent-main-main-[a-f0-9]{8}[\\/]\.oriro[\\/]sandbox-skills$/,
    );
    expect(syncOptions?.targetWorkspaceDir).not.toBe(
      path.join(workspaceDir, ".oriro", "sandbox-skills"),
    );
    expect(syncOptions?.targetWorkspaceDir?.startsWith(path.join(workspaceDir, ".oriro"))).toBe(
      false,
    );
    expect(syncOptions?.config).toBe(cfg);
    expect(syncOptions?.agentId).toBe("main");
  ...
  it("uses the SSH backend remote workspace for sandbox workspace info", async () => {
    ...
    expect(result?.containerWorkdir).toMatch(
      /^\/remote\/oriro\/oriro-ssh-agent-main-main-[a-f0-9]{8}\/workspace$/,
    );
  ...
  it("materializes skills for shared writable sandboxes even when roots match", async () => {
    ...
    expect(syncOptions?.targetWorkspaceDir).toMatch(
      /[\\/]shared-[a-f0-9]{8}[\\/]\.oriro[\\/]sandbox-skills$/,
    );
    expect(syncOptions?.targetWorkspaceDir).not.toBe(
      path.join(workspaceDir, ".oriro", "sandbox-skills"),
    );
