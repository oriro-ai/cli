...
function resolveOriroVersion(): string {
  try {
 ...
 * and appends the Oriro version.
 *
 * Format: "teams.ts[apps]/<sdk-version> Oriro/<oriro-version>"
 * Example: "teams.ts[apps]/2.0.5 Oriro/2026.3.22"
 *
...
export function buildUserAgent(): string {
  ...
  cachedUserAgent = `teams.ts[apps]/${resolveTeamsSdkVersion()} Oriro/${resolveOriroVersion()}`;
  return cachedUserAgent;
}
 ...
 * only contribute the Oriro piece — passing the full `buildUserAgent()`
 * would double-print the SDK token.
 *
 * Format: "Oriro/<oriro-version>"
 */
export function buildOriroUserAgentFragment(): string {
  return `Oriro/${resolveOriroVersion()}`;
}
