...
function makeTempDir() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-plugin-loader-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("createPluginModuleLoader", () => {
  it("loads bundled JavaScript without creating a module loader", async () => {
    ...
    const { loadOriroPlugins } = await importFreshModule<typeof import("./loader.js")>(
      import.meta.url,
      "./loader.js?scope=native-module-loader",
    );

    const pluginRoot = writeBundledPluginFixture("demo");
    process.env.ORIRO_BUNDLED_PLUGINS_DIR = pluginRoot;
  ...
  it("loads packaged JavaScript without creating a module loader", async () => {
    ...
    process.env.ORIRO_BUNDLED_PLUGINS_DIR = makeTempDir();
