// Loads, validates, migrates, snapshots, and writes Oriro config files.
import crypto from "node:crypto";
import fs from "node:fs";
import os from "node:os";
import path from "node:path";
import { collectManifestModelIdNormalizationPolicies } from "@oriro/model-catalog-core/provider-model-id-normalization";
...
import type { OriroConfig, ConfigFileSnapshot, LegacyConfigIssue } from "./types.js";
import {
...
export type ConfigWriteResult = { persistedHash: string; persistedConfig: OriroConfig };
...
export type ConfigWriteOptions = {
   ...
   */
  explicitSetValueSource?: OriroConfig;
  /**
   ...
   */
  preCommitRuntimePreflight?: (sourceConfig: OriroConfig) => Promise<unknown>;
...
function coerceConfig(value: unknown): OriroConfig {
  if (!value || typeof value !== "object" || Array.isArray(value)) {
    return {};
  }
  return value as OriroConfig;
}
...
function stampConfigVersion(cfg: OriroConfig, version?: string): OriroConfig {
  return stampConfigWriteMetadata(cfg, new Date().toISOString(), version);
}

function warnIfConfigFromFuture(cfg: OriroConfig, logger: Pick<typeof console, "warn">): void {
  ...
  if (shouldWarnOnTouchedVersion(VERSION, touched)) {
    ...
    logger.warn(
      [
        `Your Oriro config was written by version ${touched}, but this command is running ${VERSION}.`,
        "Check: `oriro --version`, `which oriro`, and `oriro gateway status --deep`.",
        "If unexpected, update PATH so `oriro` points to the version you want, or reinstall the Gateway service from that same Oriro install.",
      ].join("\n"),
    );
  }
}
...
function shouldSuppressFutureVersionWarningForEnv(env: NodeJS.ProcessEnv): boolean {
  return (
    isTruthyEnvValue(env.ORIRO_UPDATE_IN_PROGRESS) ||
    isTruthyEnvValue(env.ORIRO_UPDATE_POST_CORE)
  );
}
...
export function parseConfigJson5(
  ...
  try {
    ...
    // on the JSON files Oriro writes itself.
  }
  try {
...
function resolveConfigForRead(
  ...
  if (resolvedIncludes && typeof resolvedIncludes === "object" && "env" in resolvedIncludes) {
    applyConfigEnvVars(resolvedIncludes as OriroConfig, env, { lowerPrecedenceEnv });
  }
...
export function createConfigIO(
  ...
  function finalizeLoadedRuntimeConfig(cfg: OriroConfig): OriroConfig {
  ...
  function createValidationPluginMetadataSnapshotLoader(params: {
    ...
    load: (config: OriroConfig) => PluginMetadataSnapshot;
  ...
  function resolveRuntimePreflightSourceConfig(candidate: OriroConfig): OriroConfig {
  ...
  function ensureShippedPluginInstallConfigRecordsMigratedForWrite(
    ...
    try {
      ...
      throw new Error(
        `Config write blocked: shipped plugins.installs records in ${configPath} could not be migrated into the plugin index. Fix state directory permissions or run oriro plugins registry --refresh, then retry. ${formatErrorMessage(
          err,
        )}`,
        { cause: err },
      );
  ...
  function resolveSuspiciousRecoveryBackupCandidate(parsed: unknown): OriroConfig | null {
    try {
  ...
  function loadConfigLocal(options: { skipSuspiciousRecovery?: boolean } = {}): OriroConfig {
    try {
      ...
      const preValidationDuplicates = findDuplicateAgentDirs(
        validationConfigRaw as OriroConfig,
        {
          env: deps.env,
          homedir: deps.homedir,
        },
  ...
  async function readConfigFileSnapshotInternal(
    ...
    let fallbackSourceConfig: OriroConfig = {};
    ...
    try {
      ...
      if (
        ...
        let recoveryCandidate: OriroConfig | null = null;
  ...
  async function readBestEffortConfigLocal(): Promise<OriroConfig> {
    return (await readBestEffortConfigSnapshotLocal()).config;
  }

  async function readSourceConfigBestEffortLocal(): Promise<OriroConfig> {
    maybeLoadDotEnvForConfig(deps.env);
    const exists = deps.fs.existsSync(configPath);
    if (!exists) {
      return {};
  ...
  async function writeConfigFileLocal(
    ...
    persistCandidate = applyUnsetPathsForWrite(persistCandidate as OriroConfig, unsetPaths);
    ...
    const validationCandidate = containsConfigIncludeDirective(validationSourceCandidate)
      ? resolveRuntimePreflightSourceConfig(validationSourceCandidate as OriroConfig)
    ...
    let cfgToWrite = persistCandidate as OriroConfig;
    try {
      if (deps.fs.existsSync(configPath)) {
    ...
    const logConfigOverwrite = () => {
      ...
      const shouldLogInVitest = deps.env.ORIRO_TEST_CONFIG_OVERWRITE_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
      if (!isVerbose() && deps.env.ORIRO_CONFIG_OVERWRITE_LOG !== "1" && !shouldLogInVitest) {
        return;
      }
      deps.logger.warn(
        formatConfigOverwriteLogMessage({
    ...
    const logConfigWriteAnomalies = () => {
      ...
      const isVitest = deps.env.VITEST === "true";
      const shouldLogInVitest = deps.env.ORIRO_TEST_CONFIG_WRITE_ANOMALY_LOG === "1";
      if (isVitest && !shouldLogInVitest) {
        return;
      }
      const shouldLogBenignMissingMeta =
        isVerbose() || deps.env.ORIRO_CONFIG_WRITE_ANOMALY_LOG === "1" || shouldLogInVitest;
    ...
    const preCommitRuntimePreflight =
      ...
      (async (sourceConfig: OriroConfig) => {
...
// module scope. `ORIRO_CONFIG_PATH` (and friends) are expected to work even
...
export function loadConfig(options?: {
  ...
  // reparsing oriro.json on hot code paths.
...
export async function readSourceConfigBestEffort(): Promise<OriroConfig> {
...
export async function writeConfigFile(
  ...
  let canonicalSourceConfig: OriroConfig = nextCfg;
