...
import { createUnionActionGate } from "oriro/plugin-sdk/channel-actions";
import type {
  ChannelMessageActionAdapter,
  ChannelMessageActionName,
  ChannelMessageToolDiscovery,
} from "oriro/plugin-sdk/channel-contract";
import type { DiscordActionConfig, OriroConfig } from "oriro/plugin-sdk/config-contracts";
...
function listDiscoverableDiscordAccounts(cfg: OriroConfig) {
  return listDiscordAccountIds(cfg)
    .map((accountId) => inspectDiscordAccount({ cfg, accountId }))
    .filter((account) => account.enabled && account.configured);
}

function resolveDiscordActionDiscovery(cfg: OriroConfig) {
