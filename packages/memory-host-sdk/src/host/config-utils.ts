...
export { splitShellArgs } from "./oriro-runtime-io.js";

// Shared Oriro config helpers used by memory host, QMD, and agent context code.
...
/** Narrow Oriro config shape consumed by memory host utilities. */
...
/** Resolve the underlying OS home before applying Oriro-specific overrides. */
...
/** Resolve ORIRO_HOME or the OS home, falling back to cwd for hermetic tests. */
function resolveRequiredHomeDir(
  env: NodeJS.ProcessEnv = process.env,
  homedir: () => string = os.homedir,
): string {
  const explicitHome = normalizeHomeValue(env.ORIRO_HOME);
...
/** Resolve absolute user paths, including "~" against the effective Oriro home. */
...
export function resolveStateDir(
  ...
  const override = env.ORIRO_STATE_DIR?.trim();
  if (override) {
    return resolveUserPath(override, env, homedir);
  }
  const effectiveHome = () => resolveRequiredHomeDir(env, homedir);
  const nextDir = path.join(effectiveHome(), NEW_STATE_DIRNAME);
  if (env.ORIRO_TEST_FAST === "1" || fs.existsSync(nextDir)) {
    return nextDir;
  }
  // Existing legacy state remains authoritative until an explicit migration creates .oriro.
...
/** Resolve the default agent workspace, partitioned by ORIRO_PROFILE when set. */
function resolveDefaultAgentWorkspaceDir(env: NodeJS.ProcessEnv = process.env): string {
  const home = resolveRequiredHomeDir(env, os.homedir);
  const profile = env.ORIRO_PROFILE?.trim();
  if (profile && normalizeLowercaseStringOrEmpty(profile) !== "default") {
    return path.join(home, ".oriro", `workspace-${profile}`);
  }
  return path.join(home, ".oriro", "workspace");
}

/** Return configured agent entries after dropping nullish placeholders. */
function listAgentEntries(cfg: OriroConfig): AgentConfig[] {
...
function resolveDefaultAgentId(cfg: OriroConfig): string {
...
function resolveAgentConfig(cfg: OriroConfig, agentId: string): AgentConfig | undefined {
