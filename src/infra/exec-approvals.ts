...
const DEFAULT_EXEC_APPROVALS_STATE_DIR = "~/.oriro";
...
export function resolveExecApprovalsTranscriptPath(): string {
  return process.env.ORIRO_STATE_DIR?.trim()
    ? `$ORIRO_STATE_DIR/${EXEC_APPROVALS_FILE}`
...
function hasUnmigratedLegacyExecApprovals(filePath: string): boolean {
  if (!process.env.ORIRO_STATE_DIR?.trim()) {
    return false;
  }
...
function isReadOnlySecurityAuditSuppressionInspection(argv: string[]): boolean {
  ...
  let offset = command === "pnpm" && argv[1] === "oriro" ? 1 : 0;
  if (normalizeCommandName(argv[offset]) !== "oriro") {
    return false;
  }
  offset += 1;
