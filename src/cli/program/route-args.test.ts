...
describe("route-args", () => {
  it("parses health and status route args", () => {
    expect(
      parseHealthRouteArgs(["node", "oriro", "health", "--json", "--timeout", "5000"]),
    ).toEqual({
      json: true,
      verbose: false,
      timeoutMs: 5000,
    ...
    expect(parseStatusRouteArgs(["node", "oriro", "status", "--timeout"])).toBeNull();
  });
  ...
  it("defers status/health --timeout with a present-but-invalid value to Commander", () => {
    ...
    for (const bad of ["0", "-5", "nope", "5s"]) {
      expect(parseStatusRouteArgs(["node", "oriro", "status", "--timeout", bad])).toBeNull();
      expect(parseHealthRouteArgs(["node", "oriro", "health", "--timeout", bad])).toBeNull();
    }
    expect(
      parseStatusRouteArgs([
        "node",
    ...
    expect(parseStatusRouteArgs(["node", "oriro", "status", "--timeout", "5000"])).toMatchObject(
      { timeoutMs: 5000 },
    );
    // No --timeout flag at all still uses the fast path (undefined timeout).
    expect(parseStatusRouteArgs(["node", "oriro", "status"])).toMatchObject({
      timeoutMs: undefined,
    });
  });
  ...
  it("parses gateway status route args and rejects probe-only ssh flags", () => {
    ...
    expect(
      parseGatewayStatusRouteArgs(["node", "oriro", "gateway", "status", "--ssh", "host"]),
    ).toBeNull();
    expect(
      parseGatewayStatusRouteArgs(["node", "oriro", "gateway", "status", "--ssh-auto"]),
    ).toBeNull();
  });
  ...
  it("parses sessions and agents list route args", () => {
    ...
    expect(parseSessionsRouteArgs(["node", "oriro", "sessions", "--agent"])).toBeNull();
    expect(parseSessionsRouteArgs(["node", "oriro", "sessions", "--limit"])).toBeNull();
    expect(
      parseAgentsListRouteArgs(["node", "oriro", "agents", "list", "--json", "--bindings"]),
    ).toEqual({
      json: true,
      bindings: true,
    });
    expect(parseAgentsListRouteArgs(["node", "oriro", "agents"])).toEqual({
      json: false,
      bindings: false,
    });
  });
  ...
  it("parses config routes", () => {
    ...
    expect(parseConfigGetRouteArgs(["node", "oriro", "config", "get", "--json"])).toBeNull();
  });
  ...
  it("parses models list and models status route args", () => {
    ...
    expect(
      parseModelsStatusRouteArgs(["node", "oriro", "models", "status", "--probe-profile"]),
    ).toBeNull();
  });
});
