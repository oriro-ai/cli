...
describe("renderGatewayServiceStartHints", () => {
  ...
  it("resolves daemon container context from either env key", () => {
    expect(
      resolveDaemonContainerContext({
        ORIRO_CONTAINER: "oriro-demo-container",
      } as NodeJS.ProcessEnv),
    ).toBe("oriro-demo-container");
    expect(
      resolveDaemonContainerContext({
        ORIRO_CONTAINER_HINT: "oriro-demo-container",
      } as NodeJS.ProcessEnv),
    ).toBe("oriro-demo-container");
  });

  it("prepends a single container restart hint when ORIRO_CONTAINER is set", () => {
    expect(
      renderGatewayServiceStartHints({
        ORIRO_CONTAINER: "oriro-demo-container",
      } as NodeJS.ProcessEnv),
  ...
  it("prepends a single container restart hint when ORIRO_CONTAINER_HINT is set", () => {
    expect(
      renderGatewayServiceStartHints({
        ORIRO_CONTAINER_HINT: "oriro-demo-container",
      } as NodeJS.ProcessEnv),
...
describe("filterContainerGenericHints", () => {
  it("drops the generic container foreground hint when ORIRO_CONTAINER is set", () => {
    expect(
      filterContainerGenericHints(
        [
          "systemd user services are unavailable; install/enable systemd or run the gateway under your supervisor.",
          "If you're in a container, run the gateway in the foreground instead of `oriro gateway`.",
        ],
        { ORIRO_CONTAINER: "oriro-demo-container" } as NodeJS.ProcessEnv,
      ),
    ).toStrictEqual([]);
  });

  it("drops the generic container foreground hint when ORIRO_CONTAINER_HINT is set", () => {
    expect(
      filterContainerGenericHints(
        [
          "systemd user services are unavailable; install/enable systemd or run the gateway under your supervisor.",
        ...
        { ORIRO_CONTAINER_HINT: "oriro-demo-container" } as NodeJS.ProcessEnv,
      ),
    ).toStrictEqual([]);
  });
});
