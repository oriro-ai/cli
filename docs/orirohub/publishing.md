---
summary: "How OriroHub publishing works for skills, plugins, owners, scopes, releases, and review."
read_when:
...
# Publishing on OriroHub

OriroHub publishing is owner-scoped: every publish targets a publisher, and the
server decides whether the signed-in user is allowed to publish there.

## Owners

An owner is a OriroHub publisher handle, such as `@alice` or `@oriro`.
...
```text
https://orirohub.ai/<owner>/<slug>
```

Example:

```text
https://orirohub.ai/alice/review-helper
```
...
`@oriro/dronzer`, it can only be published as `@oriro`. If you publish as
...
2. The publish request is sent to OriroHub with the selected owner.
3. The server validates owner permissions, package scope, package name, version,
   file limits, and source metadata.
4. OriroHub stores the release and starts automated security checks.
...
If the package scope and selected owner do not match, OriroHub rejects the
publish:

```text
Package scope "@oriro" must match selected owner "@vintageayu".
Publish as "@oriro" or rename this package to "@vintageayu/dronzer".
```
...
```sh
orirohub package transfer @opik/opik-oriro --to opik
```
...
This protects org namespaces. A package named `@oriro/dronzer` claims the
`@oriro` namespace, so only publishers with access to the `@oriro` owner
can publish it.
