// Temp home test helpers create isolated Oriro home directories for plugin tests.
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked ORIRO_HOME.
  delete process.env.ORIRO_HOME;
  process.env.ORIRO_STATE_DIR = path.join(base, ".oriro");
...
export async function withTempHome<T>(
  ...
  const prefix = opts.prefix ?? "oriro-test-home-";
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".oriro", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
