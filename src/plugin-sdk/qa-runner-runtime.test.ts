...
const resolveOriroPackageRootSync = vi.hoisted(() => vi.fn());
...
describe("plugin-sdk qa-runner-runtime", () => {
  ...
  const originalPrivateQaCli = process.env.ORIRO_ENABLE_PRIVATE_QA_CLI;
  ...
  beforeEach(() => {
    ...
    resolveOriroPackageRootSync.mockReset().mockReturnValue(null);
  ...
  it("loads bundled plugin test APIs with the private QA source tree override", async () => {
    const sourceRoot = makePrivateQaSourceRoot(tempDirs, "oriro-qa-test-api-root-");
    resolveOriroPackageRootSync.mockReturnValue(sourceRoot);
    ...
    expect(testApiCall?.env?.ORIRO_ENABLE_PRIVATE_QA_CLI).toBe("1");
    expect(testApiCall?.env?.ORIRO_BUNDLED_PLUGINS_DIR).toBe(
  ...
  it("prefers the source bundled tree for private qa discovery in repo checkouts", async () => {
    const sourceRoot = makePrivateQaSourceRoot(tempDirs, "oriro-qa-runner-root-");
    resolveOriroPackageRootSync.mockReturnValue(sourceRoot);
    ...
    expect(manifestCall?.env?.ORIRO_ENABLE_PRIVATE_QA_CLI).toBe("1");
    expect(manifestCall?.env?.ORIRO_BUNDLED_PLUGINS_DIR).toBe(
      path.join(sourceRoot, "extensions"),
    );
    ...
    expect(publicSurfaceCall?.env?.ORIRO_ENABLE_PRIVATE_QA_CLI).toBe("1");
    expect(publicSurfaceCall?.env?.ORIRO_BUNDLED_PLUGINS_DIR).toBe(
      path.join(sourceRoot, "extensions"),
    );
  });
  ...
  it("fails when runtime registrations include an undeclared command", async () => {
    ...
    expect(() => module.listQaRunnerCliContributions()).toThrow(
      'QA runner plugin "qa-matrix" exported "extra" from runtime-api.js but did not declare it in oriro.plugin.json',
    );
  });
});
