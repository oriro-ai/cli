...
describe("runCapability video provider wiring", () => {
  it("merges video baseUrl and headers with entry precedence", async () => {
    ...
    await withTempDir({ prefix: "oriro-video-auth-" }, async (isolatedAgentDir) => {
      await withVideoFixture("oriro-video-merge", async ({ ctx, media, cache }) => {
        const cfg = {
          models: {
            providers: {
              moonshot: {
  ...
  it("auto-selects moonshot for video when google is unavailable", async () => {
    await withTempDir({ prefix: "oriro-video-agent-" }, async (isolatedAgentDir) => {
      await withEnvAsync(
        {
        ...
        async () => {
          await withVideoFixture("oriro-video-auto-moonshot", async ({ ctx, media, cache }) => {
            const cfg = {
              models: {
                providers: {
                  moonshot: {
  ...
  it("does not use provider api config as video auth modelApi", async () => {
    ...
    await withTempDir({ prefix: "oriro-video-provider-api-" }, async (isolatedAgentDir) => {
      await withVideoFixture("oriro-video-provider-api", async ({ ctx, media, cache }) => {
