...
private final class MockWatchMessagingService: @preconcurrency WatchMessagingServicing, @unchecked Sendable {
    ...
    var sendError: Error?
    var lastSent: (id: String, params: OriroWatchNotifyParams)?
    var lastSentExecApprovalPrompt: OriroWatchExecApprovalPromptMessage?
    var lastSentExecApprovalResolved: OriroWatchExecApprovalResolvedMessage?
    var lastSentExecApprovalExpired: OriroWatchExecApprovalExpiredMessage?
    var lastSentExecApprovalSnapshot: OriroWatchExecApprovalSnapshotMessage?
    var lastSentAppSnapshot: OriroWatchAppSnapshotMessage?
    ...
    func sendNotification(id: String, params: OriroWatchNotifyParams) async throws -> WatchNotificationSendResult {
...
@Suite(.serialized) struct NodeAppModelInvokeTests {
    @Test @MainActor func decodeParamsFailsWithoutJSON() {
        #expect(throws: Error.self) {
            _ = try NodeAppModel._test_decodeParams(OriroCanvasNavigateParams.self, from: nil)
        }
    }
    ...
    @Test @MainActor func sessionKeyExtractsCanonicalAgentID() {
        #expect(SessionKey.agentId(from: "agent:rust-oriro:mattermost:channel:w6g") == "rust-oriro")
    ...
    @Test @MainActor func chatAgentNameUsesFocusedCanonicalSessionAgent() {
        ...
        appModel.openChat(sessionKey: "agent:rust-oriro:mattermost:channel:w6gjp6iz3fyp3fo15q4fwfpnno")

        #expect(appModel.selectedAgentId == "main")
        #expect(appModel.activeAgentName == "Joshtimus Prime")
        #expect(appModel.chatAgentId == "rust-oriro")
        #expect(appModel.chatAgentName == "Rust Oriro")
    }
    ...
    @Test @MainActor func chatAgentNameFallsBackToSelectedAgentForUnscopedSession() {
        ...
        appModel.setSelectedAgentId("rust-oriro")
    ...
    @Test @MainActor func selectingAgentClearsExplicitChatFocus() {
        let appModel = NodeAppModel()
        appModel.gatewayDefaultAgentId = "main"
        let rustSessionKey = SessionKey.makeAgentSessionKey(agentId: "rust-oriro", baseKey: "main")
    ...
    @Test @MainActor func watchAppSnapshotUsesConfiguredAgentAvatar() async throws {
        ...
        appModel.gatewayAgents = [
            AgentSummary(
                ...
                name: "Main",
                identity: [
                    "avatarUrl": AnyCodable("https://example.com/oriro.png"),
                    "emoji": AnyCodable("OC"),
                ],
                workspace: nil,
                model: nil,
        ...
        let snapshot = try #require(watchService.lastSentAppSnapshot)
        #expect(snapshot.agentAvatarURL == "https://example.com/oriro.png")
    ...
    @Test @MainActor func handleInvokeRejectsBackgroundCommands() async {
        ...
        let req = BridgeInvokeRequest(id: "bg", command: OriroCanvasCommand.present.rawValue)
    ...
    @Test @MainActor func handleInvokeRejectsCameraWhenDisabled() async {
        let appModel = NodeAppModel()
        let req = BridgeInvokeRequest(id: "cam", command: OriroCameraCommand.snap.rawValue)
    ...
    @Test @MainActor func systemNotifyReturnsUnavailableWhenNotificationsOff() async throws {
        ...
        center.status = .notDetermined
        let appModel = NodeAppModel(notificationCenter: center)
        let params = OriroSystemNotifyParams(title: "Approval", body: "Review request")
        let paramsData = try JSONEncoder().encode(params)
        let req = BridgeInvokeRequest(
            id: "notify-off",
            command: OriroSystemCommand.notify.rawValue,
    ...
    @Test @MainActor func chatPushWithoutSpeechReturnsUnavailableWhenNotificationsOff() async throws {
        ...
        center.status = .notDetermined
        let appModel = NodeAppModel(notificationCenter: center)
        let params = OriroChatPushParams(text: "Build finished", speak: false)
        let paramsData = try JSONEncoder().encode(params)
        let req = BridgeInvokeRequest(
            id: "chat-push-off",
            command: OriroChatCommand.push.rawValue,
            paramsJSON: String(decoding: paramsData, as: UTF8.self))
    ...
    @Test @MainActor func handleInvokeRejectsInvalidScreenFormat() async {
        let appModel = NodeAppModel()
        let params = OriroScreenRecordParams(format: "gif")
        let data = try? JSONEncoder().encode(params)
        let json = data.flatMap { String(data: $0, encoding: .utf8) }

        let req = BridgeInvokeRequest(
            id: "screen",
            command: OriroScreenCommand.record.rawValue,
            paramsJSON: json)
    ...
    @Test @MainActor func handleInvokeCanvasCommandsUpdateScreen() async throws {
        ...
        let present = BridgeInvokeRequest(id: "present", command: OriroCanvasCommand.present.rawValue)
        ...
        let navigateParams = OriroCanvasNavigateParams(url: "http://example.com/")
        let navData = try JSONEncoder().encode(navigateParams)
        let navJSON = String(decoding: navData, as: UTF8.self)
        let navigate = BridgeInvokeRequest(
            id: "nav",
            command: OriroCanvasCommand.navigate.rawValue,
            paramsJSON: navJSON)
        let navRes = await appModel._test_handleInvoke(navigate)
        #expect(navRes.ok == true)
        #expect(appModel.screen.urlString == "http://example.com/")

        let evalParams = OriroCanvasEvalParams(javaScript: "1+1")
        ...
        let eval = BridgeInvokeRequest(
            id: "eval",
            command: OriroCanvasCommand.evalJS.rawValue,
            paramsJSON: evalJSON)
    ...
    @Test @MainActor func handleInvokeA2UICommandsFailWhenLocalHostUnavailable() async throws {
        ...
        let reset = BridgeInvokeRequest(id: "reset", command: OriroCanvasA2UICommand.reset.rawValue)
        ...
        let pushParams = OriroCanvasA2UIPushJSONLParams(jsonl: jsonl)
        ...
        let push = BridgeInvokeRequest(
            id: "push",
            command: OriroCanvasA2UICommand.pushJSONL.rawValue,
            paramsJSON: pushJSON)
    ...
    @Test @MainActor func handleInvokeWatchStatusReturnsServiceSnapshot() async throws {
        ...
        let appModel = NodeAppModel(watchMessagingService: watchService)
        let req = BridgeInvokeRequest(id: "watch-status", command: OriroWatchCommand.status.rawValue)

        let res = await appModel._test_handleInvoke(req)
        #expect(res.ok == true)

        let payloadData = try #require(res.payloadJSON?.data(using: .utf8))
        let payload = try JSONDecoder().decode(OriroWatchStatusPayload.self, from: payloadData)
    ...
    @Test @MainActor func handleInvokeWatchNotifyRoutesToWatchService() async throws {
        ...
        watchService.nextSendResult = WatchNotificationSendResult(
            ...
            transport: "transferUserInfo")
        let appModel = NodeAppModel(watchMessagingService: watchService)
        let params = OriroWatchNotifyParams(
            title: "Oriro",
        ...
        let req = BridgeInvokeRequest(
            id: "watch-notify",
            command: OriroWatchCommand.notify.rawValue,
            paramsJSON: paramsJSON)

        let res = await appModel._test_handleInvoke(req)
        #expect(res.ok == true)
        #expect(watchService.lastSent?.params.title == "Oriro")
        ...
        let payloadData = try #require(res.payloadJSON?.data(using: .utf8))
        let payload = try JSONDecoder().decode(OriroWatchNotifyPayload.self, from: payloadData)
    ...
    @Test @MainActor func handleInvokeWatchNotifyRejectsEmptyMessage() async throws {
        let watchService = MockWatchMessagingService()
        let appModel = NodeAppModel(watchMessagingService: watchService)
        let params = OriroWatchNotifyParams(title: "   ", body: "\n")
        let paramsData = try JSONEncoder().encode(params)
        let paramsJSON = String(decoding: paramsData, as: UTF8.self)
        let req = BridgeInvokeRequest(
    ...
    @Test @MainActor func handleInvokeWatchNotifyDerivesPriorityFromRiskAndCapsActions() async throws {
        ...
        let params = OriroWatchNotifyParams(
            ...
            risk: .high,
            actions: [
                OriroWatchAction(id: "a1", label: "A1"),
                OriroWatchAction(id: "a2", label: "A2"),
                OriroWatchAction(id: "a3", label: "A3"),
                OriroWatchAction(id: "a4", label: "A4"),
                OriroWatchAction(id: "a5", label: "A5"),
            ])
        let paramsData = try JSONEncoder().encode(params)
        let paramsJSON = String(decoding: paramsData, as: UTF8.self)
        let req = BridgeInvokeRequest(
    ...
    @Test @MainActor func handleInvokeWatchNotifyReturnsUnavailableOnDeliveryFailure() async throws {
        ...
        let appModel = NodeAppModel(watchMessagingService: watchService)
        let params = OriroWatchNotifyParams(title: "Oriro", body: "Delivery check")
        let paramsData = try JSONEncoder().encode(params)
        let paramsJSON = String(decoding: paramsData, as: UTF8.self)
        let req = BridgeInvokeRequest(
    ...
    @Test @MainActor func handleDeepLinkSetsErrorWhenNotConnected() async throws {
        let appModel = NodeAppModel()
        let url = try #require(URL(string: "oriro://agent?message=hello"))
    ...
    @Test @MainActor func handleDeepLinkRejectsOversizedMessage() async throws {
        ...
        let url = try #require(URL(string: "oriro://agent?message=\(msg)"))
        await appModel.handleDeepLink(url: url)
    ...
    @Test @MainActor func operatorAdminScopeCacheRefreshesFromStoredToken() throws {
        ...
        let previousStateDir = ProcessInfo.processInfo.environment["ORIRO_STATE_DIR"]
        setenv("ORIRO_STATE_DIR", tempDir.path, 1)
        defer {
            if let previousStateDir {
                setenv("ORIRO_STATE_DIR", previousStateDir, 1)
            } else {
                unsetenv("ORIRO_STATE_DIR")
            }
