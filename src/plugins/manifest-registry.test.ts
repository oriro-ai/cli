...
import type { OriroPackageManifest } from "./manifest.js";
...
function makeTempDir() {
  return makeTrackedTempDir("oriro-manifest-registry", tempDirs);
}
...
function writeManifest(dir: string, manifest: Record<string, unknown>) {
  fs.writeFileSync(path.join(dir, "oriro.plugin.json"), JSON.stringify(manifest), "utf-8");
}
...
function createPluginCandidate(params: {
  ...
  packageManifest?: OriroPackageManifest;
  packageDir?: string;
...
function prepareLinkedManifestFixture(params: { id: string; mode: "symlink" | "hardlink" }): {
  ...
  const outsideManifest = path.join(outsideDir, "oriro.plugin.json");
  const linkedManifest = path.join(rootDir, "oriro.plugin.json");
...
describe("loadPluginManifestRegistry", () => {
  ...
  it("prefers dev source bundled plugins over installed globals with the same id", () => {
    ...
    const registry = loadPluginManifestRegistry({
      env: hermeticEnv({ ORIRO_DEV_SOURCE_ROOT: devSourceRoot }),
      installRecords: {
        codex: {
          source: "npm",
          installPath: globalDir,
    ...
    const registry = loadPluginManifestRegistry({
      installRecords: {
        "diagnostics-prometheus": {
          source: "npm",
          installPath: dir,
          resolvedName: "@oriro/diagnostics-prometheus",
  ...
  it("marks official diagnostics-otel config paths trusted when the install record matches", () => {
    ...
    const registry = loadPluginManifestRegistry({
      installRecords: {
        "diagnostics-otel": {
          source: "npm",
          spec: "@oriro/diagnostics-otel",
          installPath: dir,
          resolvedName: "@oriro/diagnostics-otel",
          resolvedVersion: "2026.5.18",
          resolvedSpec: "@oriro/diagnostics-otel@2026.5.18",
        },
      },
      candidates: [
        createPluginCandidate({
  ...
  it("reports non-bundled providerAuthEnvVars as deprecated compat metadata", () => {
    ...
    expectDiagnosticFields(registry, {
      ...
      source: path.join(dir, "oriro.plugin.json"),
  ...
  it("ignores provider catalog entries outside the plugin root", () => {
    ...
    expectDiagnosticFields(registry, {
      ...
      source: path.join(pluginDir, "oriro.plugin.json"),
  ...
  it("preserves host-trusted plugin contracts from catalog overlays", () => {
    const contracts = manifestRegistryTesting.mergeManifestContracts(
      {
        agentToolResultMiddleware: ["oriro"],
      },
      {
  ...
  it("hydrates bundled channel config metadata onto manifest records", () => {
    ...
    const registry = loadRegistry([
      createPluginCandidate({
        ...
        rootDir: dir,
        origin: "bundled",
        bundledManifestPath: path.join(dir, "oriro.plugin.json"),
        bundledManifest: {
          id: "telegram",
          configSchema: { type: "object" },
          channels: ["telegram"],
  ...
  it.each([
    {
      ...
      env: { ORIRO_VERSION: "2026.3.21" } as NodeJS.ProcessEnv,
      expectedMessage: "plugin requires Oriro >=2026.3.22, but this host is 2026.3.21",
      expectWarn: true,
    },
    {
      name: "skips plugins whose beta minHostVersion is newer than the current host",
      minHostVersion: ">=2026.5.1-beta.1",
      env: { ORIRO_VERSION: "2026.4.30" } as NodeJS.ProcessEnv,
      expectedMessage: "plugin requires Oriro >=2026.5.1-beta.1, but this host is 2026.4.30",
      expectWarn: true,
    },
    {
      name: "rejects invalid minHostVersion metadata",
      minHostVersion: "2026.3.22",
      expectedMessage: "plugin manifest invalid | oriro.install.minHostVersion must use",
      expectWarn: false,
    },
    {
      name: "warns distinctly when host version cannot be determined",
      minHostVersion: ">=2026.3.22",
      env: { ORIRO_VERSION: "unknown" } as NodeJS.ProcessEnv,
  ...
  it("accepts legacy bare minHostVersion metadata for recorded installed globals", () => {
    ...
    expectNoRegistryDiagnosticContains(registry, "oriro.install.minHostVersion must use");
  });
  ...
  it("does not runtime-gate bundled source plugins by install minHostVersion", () => {
    ...
    expectNoRegistryDiagnosticContains(registry, "requires Oriro");
  });
  ...
  it("skips installed plugins whose package plugin API range is newer than the current host", () => {
    ...
    const registry = loadRegistryForPluginApiCase({
      rootDir: dir,
      pluginApi: ">=2026.5.27",
      env: { ORIRO_VERSION: "2026.5.10-beta.1" } as NodeJS.ProcessEnv,
    });
  ...
  it("skips installed plugins whose package plugin API metadata is malformed", () => {
    ...
    const registry = loadRegistryForPluginApiCase({
      rootDir: dir,
      pluginApi: 20260527,
      env: { ORIRO_VERSION: "2026.5.27" } as NodeJS.ProcessEnv,
    });

    expect(registry.plugins).toStrictEqual([]);
    expectRegistryDiagnosticContains(
      registry,
      "plugin manifest invalid | package.json oriro.compat.pluginApi must be a string",
    );
  ...
  it("loads installed plugins when a beta host is on the package plugin API floor", () => {
    ...
    const registry = loadRegistryForPluginApiCase({
      rootDir: dir,
      pluginApi: ">=2026.5.27",
      env: { ORIRO_VERSION: "2026.5.27-beta.1" } as NodeJS.ProcessEnv,
    });
  ...
  it("suppresses duplicate warning when global candidates come from the same package artifact", () => {
    ...
    const manifest = { id: "opik-oriro", configSchema: { type: "object" } };
  ...
  it("still rejects config manifest hardlinks outside the Nix store in Nix mode", () => {
    ...
    const registry = loadPluginManifestRegistry({
      env: hermeticEnv({ ORIRO_NIX_MODE: "1" }),
      candidates: [
        createPluginCandidate({
  ...
  it("resolves load-path manifests from the current env home", () => {
    ...
    const first = loadPluginManifestRegistry({
      ...
      env: hermeticEnv({
        ...
        ORIRO_STATE_DIR: path.join(homeA, ".state"),
      }),
    });
    const second = loadPluginManifestRegistry({
      ...
      env: hermeticEnv({
        HOME: homeB,
        ORIRO_HOME: undefined,
        ORIRO_STATE_DIR: path.join(homeB, ".state"),
      }),
    });
  ...
  it("resolves manifests against the current host version", () => {
    ...
    const olderHost = loadPluginManifestRegistry({
      candidates,
      env: hermeticEnv({
        ORIRO_VERSION: "2026.3.21",
      }),
    });
    const newerHost = loadPluginManifestRegistry({
      candidates,
      env: hermeticEnv({
        ORIRO_VERSION: "2026.3.22",
      }),
    });
