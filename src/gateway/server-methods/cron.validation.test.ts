...
const getRuntimeConfig = vi.hoisted(() =>
  vi.fn<() => OriroConfig>(() => ({}) as OriroConfig),
);
...
function setRuntimeConfig(config: OriroConfig): void {
  getRuntimeConfig.mockReturnValue(config);
}

function pluginEntries(...ids: string[]): OriroConfig["plugins"] {
  return {
    entries: Object.fromEntries(ids.map((id) => [id, { enabled: true }])),
  };
}

function telegramConfig(): OriroConfig {
  return {
    channels: {
      telegram: {
...
function telegramSlackConfig(params: { includeMainSession?: boolean } = {}): OriroConfig {
  return {
...
function msteamsConfig(): OriroConfig {
  return {
    channels: {
      msteams: {
...
function slackSynologyConfig(): OriroConfig {
  return {
    channels: {
      slack: {
        botToken: "xoxb-slack-token",
...
function slackConfig(params: { includeMainSession?: boolean } = {}): OriroConfig {
  return {
    ...(params.includeMainSession ? { session: { mainKey: "main" } } : {}),
    channels: {
      slack: {
...
describe("cron method validation", () => {
  beforeEach(() => {
    getRuntimeConfig.mockReset().mockReturnValue({} as OriroConfig);
