// Oriro SDK helper module supports normalize behavior.
import type { GatewayEvent, JsonObject, OriroEvent, OriroEventType } from "./types.js";
...
function normalizeLifecycleEndEventType(data: JsonObject): OriroEventType {
  const status = readLowerString(data.status);
  const stopReason = readLowerString(data.stopReason);
...
function normalizeAgentEventType(payload: JsonObject): OriroEventType {
...
function normalizeNamedEventType(event: GatewayEvent): OriroEventType {
...
export function normalizeGatewayEvent(event: GatewayEvent): OriroEvent {
  const payload = asRecord(event.payload);
