...
async function runConfigureChannelAccess<TResolved>(params: {
  ...
  label?: string;
  placeholder?: string;
  setPolicy: (cfg: OriroConfig, policy: ChannelAccessPolicy) => OriroConfig;
  resolveAllowlist: (params: { cfg: OriroConfig; entries: string[] }) => Promise<TResolved>;
  applyAllowlist: (params: { cfg: OriroConfig; resolved: TResolved }) => OriroConfig;
}) {
...
describe("configureChannelAccessWithAllowlist", () => {
  it("returns input config when user skips access configuration", async () => {
    ...
    const setPolicy = vi.fn((next: OriroConfig) => next);
    const resolveAllowlist = vi.fn(async () => [] as string[]);
    const applyAllowlist = vi.fn((params: { cfg: OriroConfig }) => params.cfg);
  ...
  it("applies non-allowlist policy directly", async () => {
    ...
    const setPolicy = vi.fn(
      (next: OriroConfig, policy: ChannelAccessPolicy): OriroConfig => ({
        ...next,
  ...
  it("resolves allowlist entries and applies them after forcing allowlist policy", async () => {
    ...
    const calls: string[] = [];
    const setPolicy = vi.fn((next: OriroConfig, policy: ChannelAccessPolicy): OriroConfig => {
    ...
    const resolveAllowlist = vi.fn(async (params: { cfg: OriroConfig; entries: string[] }) => {
      calls.push("resolve");
      expect(params.cfg).toBe(cfg);
      expect(params.entries).toEqual(["#general", "#support"]);
      return ["C1", "C2"];
    });
    const applyAllowlist = vi.fn((params: { cfg: OriroConfig; resolved: string[] }) => {
      calls.push("apply");
