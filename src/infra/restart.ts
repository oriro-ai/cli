...
export function emitGatewayRestart(
  ...
  try {
    if (process.listenerCount("SIGUSR1") > 0) {
      ...
      // triggerOriroRestart() uses schtasks to restart the gateway.
...
export function triggerOriroRestart(): RestartAttempt {
  ...
  if (process.platform === "linux") {
    const unit = normalizeSystemdUnit(
      process.env.ORIRO_SYSTEMD_UNIT,
      process.env.ORIRO_PROFILE,
    );
  ...
  const label =
    process.env.ORIRO_LAUNCHD_LABEL ||
    resolveGatewayLaunchAgentLabel(process.env.ORIRO_PROFILE);
