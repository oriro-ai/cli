...
| Local             | `local`             | `@oriro/llama-cpp-provider`      |
| Mistral           | `mistral`           |                                     |
...
Oriro indexes `MEMORY.md` and `memory/*.md` into chunks (~400 tokens with
...
- **Reindex on demand:** `oriro memory index --force`
...
**Memory search disabled?** Check `oriro memory status`. If no provider is
...
**Stale results?** Run `oriro memory index --force` to rebuild. The watcher
may miss changes in rare edge cases.

**sqlite-vec not loading?** Oriro falls back to in-process cosine similarity
automatically. `oriro memory status --deep` reports the local vector store
