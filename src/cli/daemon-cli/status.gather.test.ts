...
import type { StaleOriroUpdateLaunchdJob } from "../../daemon/launchd.js";
...
const findStaleOriroUpdateLaunchdJobs = vi.fn<
  (env?: NodeJS.ProcessEnv) => Promise<StaleOriroUpdateLaunchdJob[]>
>(async () => []);
...
>(async (_env?: NodeJS.ProcessEnv) => ({
  ...
  environment: {
    ...
    ORIRO_CONFIG_PATH: "/tmp/oriro-daemon/oriro.json",
  },
}));
...
const resolveStateDir = vi.fn(
  (env: NodeJS.ProcessEnv) => env.ORIRO_STATE_DIR ?? "/tmp/oriro-cli",
);
const resolveConfigPath = vi.fn((env: NodeJS.ProcessEnv, stateDir: string) => {
  return env.ORIRO_CONFIG_PATH ?? `${stateDir}/oriro.json`;
});
...
vi.mock("../../config/config.js", () => ({
  createConfigIO: ({
    ...
    const isDaemon = configPath.includes("/oriro-daemon/");
...
vi.mock("../../daemon/launchd.js", () => ({
  findStaleOriroUpdateLaunchdJobs: (env?: NodeJS.ProcessEnv) =>
    findStaleOriroUpdateLaunchdJobs(env),
}));
...
describe("gatherDaemonStatus", () => {
  ...
  beforeEach(() => {
    ...
    setTestEnvValue("ORIRO_STATE_DIR", "/tmp/oriro-cli");
    setTestEnvValue("ORIRO_CONFIG_PATH", "/tmp/oriro-cli/oriro.json");
    ...
    findStaleOriroUpdateLaunchdJobs.mockReset();
    findStaleOriroUpdateLaunchdJobs.mockResolvedValue([]);
  ...
  it("forwards requireRpc and configPath to the daemon probe", async () => {
    ...
    expect(probeInput.configPath).toBe("/tmp/oriro-daemon/oriro.json");
  });
  ...
  it("reuses the shared CLI config snapshot when the daemon uses the same config path", async () => {
    ...
    expect(readConfigFileSnapshotCalls).toHaveBeenCalledWith("/tmp/oriro-cli/oriro.json");
  ...
  it("reuses command environment when reading runtime status", async () => {
    ...
    serviceReadRuntime.mockImplementationOnce(async (env?: NodeJS.ProcessEnv) => ({
      status: env?.ORIRO_GATEWAY_PORT === "19001" ? "running" : "unknown",
      detail: env?.ORIRO_GATEWAY_PORT ?? "missing-port",
    }));
    ...
    expect(
      serviceReadRuntime.mock.calls.some(([env]) => env?.ORIRO_GATEWAY_PORT === "19001"),
    ).toBe(true);
  ...
  it("surfaces recent service restart handoffs only during deep status", async () => {
    ...
    expect(handoffInput.ORIRO_STATE_DIR).toBe("/tmp/oriro-daemon");
    expect(handoffInput.ORIRO_CONFIG_PATH).toBe("/tmp/oriro-daemon/oriro.json");
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      ...
      findStaleOriroUpdateLaunchdJobs.mockResolvedValueOnce([
        {
      ...
      const staleScanEnv = findStaleOriroUpdateLaunchdJobs.mock.calls[0]?.[0];
      expect(staleScanEnv?.ORIRO_STATE_DIR).toBe("/tmp/oriro-daemon");
      expect(staleScanEnv?.ORIRO_CONFIG_PATH).toBe("/tmp/oriro-daemon/oriro.json");
      expect(staleScanEnv?.ORIRO_LAUNCHD_LABEL).toBe("ai.oriro.manual-update.gateway");
  ...
  it("does not read restart handoffs during normal status", async () => {
    ...
    expect(findStaleOriroUpdateLaunchdJobs).not.toHaveBeenCalled();
      ...
      connections: [
        {
          ...
          ppid: 1,
          command: "node",
          commandLine: "node /tmp/newer-oriro/dist/index.js logs --follow",
  ...
  it("uses the fast config path for plain same-file status reads", async () => {
    const tmp = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-status-config-"));
    const configPath = path.join(tmp, "oriro.json");
    await fs.writeFile(
      configPath,
      JSON.stringify({
        gateway: {
    ...
    setTestEnvValue("ORIRO_STATE_DIR", tmp);
    setTestEnvValue("ORIRO_CONFIG_PATH", configPath);
    serviceReadCommand.mockResolvedValueOnce({
      programArguments: ["/bin/node", "cli", "gateway", "--port", "19001"],
      environment: {
        ORIRO_STATE_DIR: tmp,
  ...
  it("keeps remote probe auth strict when remote token is missing", async () => {
    ...
    setTestEnvValue("ORIRO_GATEWAY_TOKEN", "env-token");
    setTestEnvValue("ORIRO_GATEWAY_PASSWORD", "env-password"); // pragma: allowlist secret
  ...
  it("surfaces stale gateway listener pids from restart health inspection when probe fails", async () => {
    ...
    inspectGatewayRestart.mockResolvedValueOnce({
      ...
      portUsage: {
        port: 19001,
        status: "busy",
        listeners: [{ pid: 9000, ppid: 8999, commandLine: "oriro-gateway" }],
        hints: [],
      },
      healthy: false,
  ...
  it("reads install records from the merged daemon service environment, not the CLI process env", async () => {
    ...
    // The mock daemon service command sets ORIRO_STATE_DIR=/tmp/oriro-daemon,
    // distinct from the CLI process ORIRO_STATE_DIR=/tmp/oriro-cli. Drift
