...
export const QA_LIVE_ANTHROPIC_SETUP_TOKEN_ENV = "ORIRO_QA_LIVE_ANTHROPIC_SETUP_TOKEN";
export const QA_LIVE_SETUP_TOKEN_VALUE_ENV = "ORIRO_LIVE_SETUP_TOKEN_VALUE";
const QA_LIVE_ANTHROPIC_SETUP_TOKEN_PROFILE_ENV = "ORIRO_QA_LIVE_ANTHROPIC_SETUP_TOKEN_PROFILE";
...
const QA_LIVE_API_KEY_ALIASES: Readonly<Record<string, readonly string[]>> = Object.freeze({
  anthropic: ["ORIRO_LIVE_ANTHROPIC_KEY"],
  gemini: ["ORIRO_LIVE_GEMINI_KEY"],
  openai: [
    "CODEX_API_KEY",
    "ORIRO_LIVE_CODEX_API_KEY",
...
function qaLiveOpenAiUsesCodexByDefault(cfg: OriroConfig): boolean {
...
function resolveQaLiveProviderConfig(params: { cfg: OriroConfig; providerId: string }) {
...
function hasQaLiveStagedApiKeyProfile(params: { cfg: OriroConfig; providerId: string }) {
...
function qaLiveRequiresCodexAuth(params: {
  ...
  const forcedRuntime = params.env.ORIRO_QA_FORCE_RUNTIME?.trim().toLowerCase();
  if (forcedRuntime === "oriro") {
    return false;
  }
