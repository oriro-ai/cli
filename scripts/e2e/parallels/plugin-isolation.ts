// Plugin Isolation script supports Oriro repository automation.
...
export function posixCodexPlatformPackageRepairFunction(): string {
  ...
  state_home="\${ORIRO_PARALLELS_HOME:-\${HOME:-}}"
  codex_manifest=""
  for candidate in "$state_home"/.oriro/npm/projects/*/node_modules/@oriro/codex/package.json; do
  ...
  project_root="\${codex_manifest%/node_modules/@oriro/codex/package.json}"
  cache_dir="$(mktemp -d "\${TMPDIR:-/tmp}/oriro-npm-cache.XXXXXX")"
...
export function windowsCodexPlatformPackageRepairFunction(): string {
  ...
  $projectsRoot = Join-Path $env:USERPROFILE '.oriro\npm\projects'
  $codexManifest = Get-ChildItem -Path $projectsRoot -Filter package.json -File -Recurse -ErrorAction SilentlyContinue |
    Where-Object { $_.FullName -match 'node_modules[\\/]@oriro[\\/]codex[\\/]package\.json$' } |
  ...
  $cacheDir = Join-Path ([System.IO.Path]::GetTempPath()) ('oriro-npm-cache-' + [guid]::NewGuid().ToString('N'))
...
export function posixProviderOnlyPluginIsolationScript(options: PluginIsolationOptions): string {
  ...
  const homeEnv = options.homeFallback
    ? `ORIRO_PARALLELS_HOME=${shellQuote(options.homeFallback)} `
    : "";
...
export function windowsProviderOnlyPluginIsolationScript(options: PluginIsolationOptions): string {
  ...
  return `$env:ORIRO_PARALLELS_PLUGIN_ISOLATION = @'
${payloadJson}
'@
$isolationScriptPath = Join-Path ([System.IO.Path]::GetTempPath()) 'oriro-parallels-plugin-isolation.cjs'
@'
...
Remove-Item Env:ORIRO_PARALLELS_PLUGIN_ISOLATION -Force -ErrorAction SilentlyContinue`;
}
...
function providerOnlyPluginIsolationNodeScript(options: PluginIsolationOptions): string {
  ...
  return `process.env.ORIRO_PARALLELS_PLUGIN_ISOLATION = ${JSON.stringify(payloadJson)};
${providerOnlyPluginIsolationNodeSource()}`;
}
...
const payload = JSON.parse(process.env.ORIRO_PARALLELS_PLUGIN_ISOLATION || "{}");
const home =
  process.env.ORIRO_PARALLELS_HOME ||
...
fs.rmSync(path.join(stateDir, "npm", "node_modules", "@oriro", "codex"), {
  recursive: true,
  force: true,
});
