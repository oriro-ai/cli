...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? OriroChatTheme.userText : OriroChatTheme.assistantText
    ...
    private var inlineAttachments: [OriroChatMessageContent] {
    ...
    private var toolCalls: [OriroChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [OriroChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? OriroChatTheme.userBubble
        }
        if self.style == .onboarding {
            return OriroChatTheme.onboardingAssistantBubble
        }
        return OriroChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return OriroChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: OriroChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? OriroChatTheme.userText : OriroChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: OriroChatMessageContent
    let isUser: Bool
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [OriroChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: OriroChatTheme.assistantText)
            }
        }
    }
}
