...
const pluginRecordSnapshot = () => {
  const config = readJson(oriroPath("oriro.json"));
  const records = readPluginInstallRecords({ fallbackRecords: config.plugins?.installs ?? {} });
  const record = records["lossless-oriro"] ?? records["@example/lossless-oriro"];
  if (!record) {
...
function oriroPath(...parts) {
  return path.join(home, ".oriro", ...parts);
}
...
function seedInstallState() {
  writeJson(oriroPath("extensions", "lossless-oriro", "package.json"), {
    name: "@example/lossless-oriro",
    version: "0.9.0",
  });
  writeJson(process.env.ORIRO_CONFIG_PATH, { plugins: {} });
  writePluginInstallIndexForE2E({
    version: 1,
    warning: "DO NOT EDIT. This file is generated by Oriro plugin registry commands.",
    ...
    installRecords: {
      "lossless-oriro": {
        source: "npm",
        spec: "@example/lossless-oriro@0.9.0",
        installPath: "~/.oriro/extensions/lossless-oriro",
        resolvedName: "@example/lossless-oriro",
        resolvedVersion: "0.9.0",
        resolvedSpec: "@example/lossless-oriro@0.9.0",
...
function registryHealthy() {
  return new Promise((resolve) => {
    const req = http.get("http://127.0.0.1:4873/@example%2flossless-oriro", (res) => {
...
async function readOutputEvidence(logPath) {
  ...
  for await (const chunk of fs.createReadStream(logPath)) {
    ...
    sawDownload ||= searchable.includes("Downloading @example/lossless-oriro");
    sawUpToDate ||= searchable.includes("lossless-oriro is up to date (0.9.0).");
...
function isCorruptPluginDisabledAfterUpdate(evidence, pluginId) {
  ...
  return (
    ...
    message.includes("Oriro will continue without it")
  );
}
...
function assertCorruptPluginDetails(plugins, pluginId) {
  ...
  const expectedFragments = disabledAfterFailure
    ? [
        ...
        "Oriro will continue without it",
