...
import { asNullableObjectRecord } from "@oriro/normalization-core/record-coerce";
...
import { resolveOriroAgentDir } from "../plugin-sdk/agent-dir-compat.js";
...
function listOrphanAgentDirs(cfg: OriroConfig, stateDir: string): OrphanAgentDir[] {
  ...
  const liveCompatibilityAgentDir = resolveOriroAgentDir();
  try {
...
function findOtherStateDirs(stateDir: string): string[] {
  ...
  for (const root of roots) {
    ...
    for (const entry of entries) {
      ...
      const candidates = [".oriro"].map((dir) => path.resolve(root, entry.name, dir));
...
export function formatLinuxSdBackedStateDirWarning(
  ...
  return [
    ...
    "- For better startup and state durability, prefer SSD/NVMe (or USB SSD on Raspberry Pi) for ORIRO_STATE_DIR.",
  ].join("\n");
}
...
export function formatLinuxVolatileStateDirWarning(
  ...
  return [
    ...
    "- Move ORIRO_STATE_DIR to a persistent filesystem to avoid data loss.",
  ].join("\n");
}
...
export function detectMacCloudSyncedStateDir(
  ...
  // ORIRO_HOME can relocate app data defaults, but iCloud/CloudStorage remain under the OS home.
...
function shouldRequireOAuthDir(cfg: OriroConfig, env: NodeJS.ProcessEnv): boolean {
  if (env.ORIRO_OAUTH_DIR?.trim()) {
    return true;
  }
...
function shouldSuppressOrphanTranscriptWarning(cfg: OriroConfig, agentId: string): boolean {
  ...
  if (cloudSyncedStateDir) {
    warnings.push(
      [
        ...
        "- Prefer a local non-synced state dir (for example: ~/.oriro).",
        `  Set locally: ORIRO_STATE_DIR=~/.oriro ${formatCliCommand("oriro doctor")}`,
      ].join("\n"),
    );
  }
    ...
    if (missing.length > 0) {
      warnings.push(
        [
          ...
          `  Verify sessions in store: ${formatCliCommand(`oriro sessions --store "${absoluteStorePath}"`)}`,
          `  Preview cleanup impact: ${formatCliCommand(`oriro sessions cleanup --store "${absoluteStorePath}" --dry-run`)}`,
          `  Prune missing entries: ${formatCliCommand(`oriro sessions cleanup --store "${absoluteStorePath}" --enforce --fix-missing`)}`,
        ].join("\n"),
      );
    }
    ...
    if (wedgedSubagentSessions.length > 0) {
      ...
      warnings.push(
        [
          ...
          "  Oriro will not auto-resume these child sessions on restart; reconcile their task records instead.",
          `  Examples: ${wedgedSubagentSessions
            .slice(0, 3)
            .map(([key]) => key)
            .join(", ")}`,
          `  Fix: ${formatCliCommand("oriro tasks maintenance --apply")}`,
        ].join("\n"),
      );
...
export function collectWorkspaceBackupTip(workspaceDir: string): string | null {
  ...
  return [
    ...
    "- Keep ~/.oriro out of git; it contains credentials and session history.",
