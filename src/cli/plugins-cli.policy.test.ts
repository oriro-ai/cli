...
describe("plugins cli policy mutations", () => {
  ...
  function requireFirstWrittenConfig(): OriroConfig {
  ...
  function requirePluginEntries(
    config: OriroConfig,
  ): NonNullable<NonNullable<OriroConfig["plugins"]>["entries"]> {
  ...
  it("refreshes the persisted plugin registry after enabling a plugin", async () => {
    const sourceConfig = {} as OriroConfig;
    const enabledConfig = {
      plugins: {
        entries: {
          alpha: { enabled: true },
  ...
  it.each(["enable", "disable"] as const)(
    ...
    async (command) => {
      ...
      expect(runtimeErrors).toContain(
        "Plugin not found: missing-plugin. Run `oriro plugins list` to see installed plugins, or `oriro plugins search missing-plugin` to look for installable plugins.",
      );
      expect(enablePluginInConfig).not.toHaveBeenCalled();
      expect(writeConfigFile).not.toHaveBeenCalled();
