// Phone Control plugin entrypoint registers its Oriro integration.
import {
  asDateTimestampMs,
...
function openArmStateStore(api: OriroPluginApi) {
...
async function readArmState(api: OriroPluginApi): Promise<ArmStateFile | null> {
  return (await openArmStateStore(api).lookup(ARM_STATE_KEY)) ?? null;
}

async function writeArmState(api: OriroPluginApi, state: ArmStateFile | null): Promise<void> {
...
export default definePluginEntry({
  ...
  register(api: OriroPluginApi) {
    ...
    const timerService: OriroPluginService = {
