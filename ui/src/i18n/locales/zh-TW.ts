...
export const zh_TW: TranslationMap = {
  ...
  overview: {
    ...
    pairing: {
      ...
      mobileHint:
        "在手機上？從桌面執行 oriro dashboard --no-open 複製完整 URL（包括 #token=...）。",
    ...
    cards: {
      ...
      modelAuthAttentionExpiredDesc: "{providers} — 使用 oriro models auth 重新驗證",
  ...
  login: {
    ...
    failure: {
      ...
      authRequired: {
        ...
        stepPaste: "貼上 oriro dashboard --no-open 提供的權杖，或輸入已設定的密碼。",
        stepGenerate:
          "如果尚未設定權杖，請在 Gateway 主機上執行 oriro doctor --generate-gateway-token。",
        stepConnect: "更新憑證後再次按一下 Connect。",
      },
      authFailed: {
        title: "驗證不相符",
        summary: "提供的憑證遭到拒絕。最常見原因是權杖過期，或權杖複製自另一個 Gateway URL。",
        stepDashboard: "執行 oriro dashboard --no-open 並開啟新的 URL，或貼上其中的權杖。",
      ...
      pairing: {
        ...
        stepList: "在 Gateway 主機上執行 oriro devices list。",
        stepApproveId: "核准此要求：oriro devices approve {requestId}。",
      ...
      network: {
        ...
        stepGateway: "使用 oriro status 或 oriro gateway run 確認 Gateway 正在執行。",
        stepUrl: "檢查 WebSocket URL；當 Gateway 位於 HTTPS/Tailscale Serve 後方時使用 wss://。",
        stepDashboard:
          "使用 oriro dashboard --no-open 重新開啟 dashboard，以重新複製目前 URL 與驗證詳細資料。",
      },
    },
  },
  chat: {
