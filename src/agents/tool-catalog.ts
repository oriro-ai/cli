...
function buildCoreToolGroupMap() {
  ...
  const oriroTools = CORE_TOOL_DEFINITIONS.filter((tool) => tool.includeInOriroGroup).map(
    (tool) => tool.id,
  );
  return {
    "group:oriro": oriroTools,
