...
function resolveSnapshotRuntimeConfig(snapshot: ConfigFileSnapshot | null): OriroConfig | null {
...
function coerceStatusConfig(value: unknown): OriroConfig {
...
export type DaemonStatus = {
  ...
  service: {
    ...
    staleUpdateLaunchdJobs?: StaleOriroUpdateLaunchdJob[];
  };
  config?: {
    cli: ConfigSummary;
   ...
   * can happen after `npm install -g oriro@<v>` updates the gateway binary
   * without a corresponding `oriro plugins update`.
   */
  ...
  const staleUpdateLaunchdJobs =
    opts.deep && process.platform === "darwin"
      ? await loadLaunchdModule()
          .then(({ findStaleOriroUpdateLaunchdJobs }) =>
            findStaleOriroUpdateLaunchdJobs(serviceEnv),
          )
          .catch(() => [])
      : [];
