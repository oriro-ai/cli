...
export async function startControlUiE2eServer(): Promise<ControlUiE2eServer> {
  ...
  const server = await createServer({
    ...
    configFile: false,
    define: {
      ORIRO_CONTROL_UI_BUILD_ID: JSON.stringify("e2e"),
    },
    logLevel: "error",
    optimizeDeps: {
      include: [
...
function installControlUiMockGateway(input: {
  ...
  type WindowWithGateway = Window & {
    oriroControlUiE2eGateway?: ExposedGateway;
  };
  ...
  (window as WindowWithGateway).oriroControlUiE2eGateway = exposed;
