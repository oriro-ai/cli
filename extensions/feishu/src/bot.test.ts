...
import type * as ConversationRuntime from "oriro/plugin-sdk/conversation-runtime";
...
import { resolveGroupSessionKey } from "oriro/plugin-sdk/session-store-runtime";
...
let currentRuntimeConfig = {} as OriroConfig;
...
describe("createFeishuMessageReceiveHandler media dedupe", () => {
  it("preserves the original dispatch dedupe key when debounce merges text content", async () => {
    ...
    const handler = createFeishuMessageReceiveHandler({
      cfg: { channels: { feishu: { dmPolicy: "open" } } } as OriroConfig,
