...
export function registerOnboardCommand(program: Command): void {
  const command = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/onboard", "docs.oriro.ai/cli/onboard")}\n`,
    )
    .option("--workspace <dir>", "Agent workspace directory (default: ~/.oriro/workspace)")
    .option(
      "--reset",
  ...
  command
    ...
    .option(
      ...
      "Gateway token SecretRef env var name (token auth; e.g. ORIRO_GATEWAY_TOKEN)",
    )
