...
describe("persistPluginInstall", () => {
  ...
  it("adds installed plugins to restrictive allowlists before enabling", async () => {
    ...
    enablePluginInConfig.mockImplementation((...args: unknown[]) => {
      const [cfg, pluginId] = args as [OriroConfig, string];
  ...
  it("removes a replaced managed install directory before refreshing the registry", async () => {
    ...
    setInstalledPluginIndexInstallRecords({
      codex: {
        source: "orirohub",
        spec: "orirohub:@oriro/codex",
        installPath: "/tmp/oriro/extensions/codex",
      },
    });
    planPluginUninstall.mockReturnValueOnce({
      ...
      directoryRemoval: {
        target: "/tmp/oriro/extensions/codex",
      },
    });
    ...
    await persistPluginInstall({
      ...
      install: {
        source: "npm",
        spec: "@oriro/codex",
        installPath: "/tmp/oriro/npm/node_modules/@oriro/codex",
      },
    });
    ...
    expect(runtimeLogs.join("\n")).toContain(
      "Removed previous plugin install directory: /tmp/oriro/extensions/codex",
    );
  });
  ...
  it("warns when an installed npm plugin remains shadowed by a config-selected source", async () => {
    ...
    const next = await persistPluginInstall({
      ...
      install: {
        source: "npm",
        spec: "@oriro/discord",
        installPath: "/tmp/oriro/npm/node_modules/@oriro/discord/index.ts",
      },
    });
    ...
    expect(runtimeLogs.join("\n")).toContain(
      "active config source: /tmp/oriro-upstream/extensions/discord/index.ts",
    );
    expect(runtimeLogs.join("\n")).toContain(
      "installed npm source: /tmp/oriro/npm/node_modules/@oriro/discord/index.ts",
    );
    expect(runtimeLogs.join("\n")).toContain("oriro plugins doctor");
  });
  ...
  it("does not warn when the config-selected source is inside the npm install path", async () => {
      ...
      plugins: [
        {
          id: "discord",
          origin: "config",
          source: "/tmp/oriro/npm/node_modules/@oriro/discord/dist/index.js",
          status: "loaded",
        },
      ],
      diagnostics: [],
    ...
    await persistPluginInstall({
      ...
      install: {
        source: "npm",
        spec: "@oriro/discord",
        installPath: "/tmp/oriro/npm/node_modules/@oriro/discord",
      },
    });
