...
export const DISABLE_PLUGIN_REGISTRY_MIGRATION_ENV = "ORIRO_DISABLE_PLUGIN_REGISTRY_MIGRATION";
export const FORCE_PLUGIN_REGISTRY_MIGRATION_ENV = "ORIRO_FORCE_PLUGIN_REGISTRY_MIGRATION";
...
export type PluginRegistryInstallMigrationParams = LoadInstalledPluginIndexParams &
  InstalledPluginIndexStoreOptions & {
    ...
    readConfig?: () => Promise<OriroConfig> | OriroConfig;
  };
...
function listConfiguredChannelIds(config: OriroConfig): Set<string> {
...
function listConfiguredModelProviderIds(config: OriroConfig): Set<string> {
...
export async function migratePluginRegistryForInstall(
  ...
  const config = stripShippedPluginInstallConfigRecords(rawConfig) as OriroConfig;
