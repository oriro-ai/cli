...
describe("Codex app-server policy", () => {
  it("keeps implicit Codex yolo approval policy when untrusted approvals are disallowed", () => {
    ...
    const resolved = resolveCodexAppServerForOriroToolPolicy({
      appServer,
  ...
  it("promotes implicit yolo approval policy when Oriro tool policy requires review", () => {
    const appServer = resolveCodexAppServerRuntimeOptions({ env: {}, requirementsToml: null });
  ...
  it("preserves explicit operator app-server policy", () => {
    ...
    const explicitConfig = resolveCodexAppServerForOriroToolPolicy({
      appServer,
    ...
    const explicitEnv = resolveCodexAppServerForOriroToolPolicy({
      appServer,
      pluginConfig: readCodexPluginConfig({}),
      env: { ORIRO_CODEX_APP_SERVER_APPROVAL_POLICY: "never" },
      shouldPromote: true,
      canUseUntrustedApprovalPolicy: true,
    });
    const explicitRequirements = resolveCodexAppServerForOriroToolPolicy({
