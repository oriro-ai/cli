// Resolve Oriro Package Candidate tests cover resolve oriro package candidate script behavior.
...
import {
  ...
  cleanPackedOriroTarballsForTest,
  downloadUrl,
...
describe("resolve-oriro-package-candidate", () => {
  it("accepts only Oriro release package specs for npm candidates", () => {
    for (const spec of [
      "oriro@beta",
      "oriro@alpha",
      "oriro@latest",
      "oriro@2026.4.27",
      "oriro@2026.4.27-1",
      "oriro@2026.4.27-beta.2",
      "oriro@2026.4.27-alpha.2",
    ]) {
      expect(validateOriroPackageSpec(spec), spec).toBeUndefined();
    }

    expect(() => validateOriroPackageSpec("@evil/oriro@1.0.0")).toThrow(
      "package_spec must be oriro@alpha",
    );
    expect(() => validateOriroPackageSpec("oriro@canary")).toThrow(
      "package_spec must be oriro@alpha",
    );
    expect(() => validateOriroPackageSpec("oriro@2026.04.27")).toThrow(
      "package_spec must be oriro@alpha",
    );
    expect(() => validateOriroPackageSpec("oriro@npm:other-package")).toThrow(
      "package_spec must be oriro@alpha",
    );
    expect(() => validateOriroPackageSpec("oriro@file:../other-package.tgz")).toThrow(
      "package_spec must be oriro@alpha",
    );
  });
  ...
  it("parses optional empty workflow inputs without rejecting the command line", () => {
    expect(
      ...
      outputName: "oriro-current.tgz",
      packageSha256: "",
      packageRef: "release/2026.4.27",
      packageSpec: "oriro@beta",
      packageUrl: "",
      source: "npm",
      trustedSourceId: "",
  ...
  it("rejects package candidate output names that escape the output directory", () => {
    ...
    expect(parseArgs(["--output-name", "oriro-current.tar.gz"]).outputName).toBe(
      "oriro-current.tar.gz",
    );
  });
  ...
  it("resolves npm package candidates through the Windows npm.cmd toolchain shim", () => {
    ...
    const runner = resolveNpmPackageCandidatePackRunner(
      "oriro@2026.5.26-beta.1",
      "C:\\oriro\\.artifacts\\docker-e2e-package",
      {
    ...
    expect(runner).toEqual({
      ...
      args: [
        ...
        "/s",
        "/c",
        `${npmCmdPath} pack oriro@2026.5.26-beta.1 --ignore-scripts --json --pack-destination C:\\oriro\\.artifacts\\docker-e2e-package`,
      ],
      shell: false,
  ...
  it("keeps npm pack filenames inside the package candidate output directory", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "oriro-package-npm-pack-"));
    tempDirs.push(dir);
    await writeFile(path.join(dir, "oriro-2026.6.17.tgz"), "package");

    await expect(
      moveNewestPackedTarballForTest(
        dir,
        JSON.stringify([{ filename: "oriro-2026.6.17.tgz" }]),
        "oriro-current.tgz",
      ),
    ).resolves.toBe(path.join(dir, "oriro-current.tgz"));
    await expect(readFile(path.join(dir, "oriro-current.tgz"), "utf8")).resolves.toBe("package");
  });
  ...
  it("rejects path-like npm pack filenames instead of renaming outside the output directory", async () => {
    ...
    const unsafeFilenames = [
      "../oriro-2026.6.17.tgz",
      "nested/oriro-2026.6.17.tgz",
      "nested\\oriro-2026.6.17.tgz",
      "/tmp/oriro-2026.6.17.tgz",
      "C:\\temp\\oriro-2026.6.17.tgz",
      "oriro-2026.6.17.tar.gz",
    ];

    for (const filename of unsafeFilenames) {
      await expect(
        moveNewestPackedTarballForTest(dir, JSON.stringify([{ filename }]), "oriro-current.tgz"),
      ).rejects.toThrow("npm pack reported unsafe Oriro tarball filename");
    }
  });
  ...
  it("rejects unsafe text npm pack filenames instead of using loose stdout fallback", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "oriro-package-npm-pack-"));
    tempDirs.push(dir);
    await writeFile(path.join(dir, "oriro-2026.6.17.tgz"), "safe fallback");

    for (const filename of ["../oriro-2026.6.17.tgz", "C:oriro-2026.6.17.tgz"]) {
      await expect(
        moveNewestPackedTarballForTest(
          dir,
  ...
  it("cleans stale package tarballs before npm fallback scanning", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "oriro-package-npm-pack-stale-"));
    tempDirs.push(dir);
    await writeFile(path.join(dir, "oriro-9999.1.1.tgz"), "stale");
    await writeFile(path.join(dir, "oriro-C:evil.tgz"), "unsafe");

    await cleanPackedOriroTarballsForTest(dir);
    await writeFile(path.join(dir, "oriro-2026.6.17.tgz"), "current");

    await expect(
      moveNewestPackedTarballForTest(dir, "npm notice\n", "oriro-current.tgz"),
    ).resolves.toBe(path.join(dir, "oriro-current.tgz"));
    await expect(missing(path.join(dir, "oriro-9999.1.1.tgz"))).resolves.toBe(true);
    await expect(readFile(path.join(dir, "oriro-C:evil.tgz"), "utf8")).resolves.toBe("unsafe");
    await expect(readFile(path.join(dir, "oriro-current.tgz"), "utf8")).resolves.toBe("current");
  });
  ...
  it("kills timed-out package runner process groups", async () => {
    ...
    const dir = await mkdtemp(path.join(tmpdir(), "oriro-package-runner-timeout-"));
    tempDirs.push(dir);
  ...
  it("rejects timed-out package runner commands when descendants exit cleanly", async () => {
    ...
    const dir = await mkdtemp(path.join(tmpdir(), "oriro-package-runner-timeout-clean-"));
    tempDirs.push(dir);
    const childPidPath = path.join(dir, "child.pid");
    ...
    const childScript = [
      "const fs = require('node:fs');",
      "process.on('SIGTERM', () => {",
      "  fs.writeFileSync(process.env.ORIRO_TEST_CHILD_CLEANUP, 'clean');",
      "  setTimeout(() => process.exit(0), 75);",
      "});",
      "fs.writeFileSync(process.env.ORIRO_TEST_CHILD_READY, 'ready');",
      "setInterval(() => {}, 1000);",
    ].join("");
    const parentScript = [
      "const { spawn } = require('node:child_process');",
      ...
      "  env: {",
      "    ...process.env,",
      "    ORIRO_TEST_CHILD_CLEANUP: process.env.ORIRO_TEST_CHILD_CLEANUP,",
      "    ORIRO_TEST_CHILD_READY: process.env.ORIRO_TEST_CHILD_READY,",
      "  },",
      "});",
      "fs.writeFileSync(process.env.ORIRO_TEST_CHILD_PID, String(child.pid));",
    ...
    const timeoutAssertion = expect(
      runCommandForTest(process.execPath, ["-e", parentScript], {
        env: {
          ...process.env,
          ORIRO_TEST_CHILD_CLEANUP: cleanupPath,
          ORIRO_TEST_CHILD_PID: childPidPath,
          ORIRO_TEST_CHILD_READY: readyPath,
        },
        killAfterMs: 1000,
        timeoutMs: 1000,
      }),
  ...
  it("forwards external termination to package runner process groups", async () => {
    ...
    const dir = await mkdtemp(path.join(tmpdir(), "oriro-package-runner-signal-"));
    tempDirs.push(dir);
    const childPidPath = path.join(dir, "child.pid");
    const scriptUrl = pathToFileURL(
      path.resolve("scripts/resolve-oriro-package-candidate.mjs"),
    ).href;
    let childPid: number | undefined;
  ...
  it("fails successful ref candidates when package source worktree cleanup fails", async () => {
    await expect(
      cleanupPackageSourceWorktreeForTest("/tmp/oriro-package-source-stuck", {
        runImpl: async () => {
  ...
  it("preserves original ref candidate failures when worktree cleanup also fails", async () => {
    ...
    expect(warnings).toEqual([
      "warning: failed to remove temporary package source worktree /tmp/oriro-package-source-stuck: worktree remove denied",
    ]);
  });

  it("loads named trusted package URL source policies", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "oriro-trusted-package-source-"));
    tempDirs.push(dir);
  ...
  it("does not forward trusted package auth headers to redirect hosts", async () => {
    ...
    tempDirs.push(dir);
    const target = path.join(dir, "oriro.tgz");
    const previousToken = process.env.ORIRO_TRUSTED_PACKAGE_TOKEN;
    process.env.ORIRO_TRUSTED_PACKAGE_TOKEN = "token-123";
    const trustedSource = {
      allowPrivateNetwork: true,
      auth: { type: "bearer" },
      hosts: ["packages.internal"],
    ...
    try {
      ...
      if (previousToken === undefined) {
        ...
        process.env.ORIRO_TRUSTED_PACKAGE_TOKEN = previousToken;
      }
    }
  ...
  it("validates redirects for package_url downloads", async () => {
    ...
    expect(requestedUrls).toEqual(["https://packages.example/oriro.tgz"]);
  });
  ...
  it("times out stalled package_url response bodies", async () => {
    ...
    await expect(
      ...
      "package_url download timed out after 25ms: https://packages.example/oriro.tgz",
    );
  ...
  it("reads package source metadata from package artifacts", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "oriro-package-candidate-"));
    tempDirs.push(dir);
    await writeFile(
  ...
  it("accepts uppercase package artifact SHA-256 metadata", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "oriro-package-sha-"));
    tempDirs.push(dir);
    const file = path.join(dir, "oriro.tgz");
    await writeFile(file, "oriro package bytes");
  ...
  it("rejects source artifact scans that exceed the filesystem entry limit", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "oriro-package-artifact-scan-"));
    tempDirs.push(dir);
  ...
  it("rejects source artifact directories with multiple tarballs", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "oriro-package-artifact-duplicates-"));
    tempDirs.push(dir);

    await writeFile(path.join(dir, "oriro-a.tgz"), "a");
    ...
    expect(message).toContain("oriro-a.tgz");
    expect(message).not.toContain(path.join(dir, "nested.tar.gz"));
    expect(message).not.toContain(path.join(dir, "oriro-a.tgz"));
  });

  it("reads the source SHA from packed npm build metadata", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "oriro-package-build-info-"));
    tempDirs.push(dir);
    const root = path.join(dir, "package");
    await mkdir(path.join(root, "dist"), { recursive: true });
    await writeFile(path.join(root, "package.json"), JSON.stringify({ name: "oriro" }));
    await writeFile(
      path.join(root, "dist", "build-info.json"),
      JSON.stringify({ commit: "66CE632B9B7C5C7FDD3E66C739687D51638AD6E2" }),
    );
    const tarball = path.join(dir, "oriro.tgz");
