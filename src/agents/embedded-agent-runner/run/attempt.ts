...
import {
  ...
  ORIRO_EMBEDDED_CONTEXT_ENGINE_HOST,
...
import { isTranscriptOnlyOriroAssistantMessage } from "../../../shared/transcript-only-oriro-assistant.js";
...
import { resolveOriroReferencePaths } from "../../docs-path.js";
import {
    ...
    params.sessionManager.removeTrailingEntries(
      (entry) => entry.type === "message" && isMidTurnPrecheckAssistantError(entry.message),
      {
        preserveTrailing: (entry) =>
          ...
          (entry.type === "message" && isTranscriptOnlyOriroAssistantMessage(entry.message)),
      },
    ) > 0;
  ...
  try {
    ...
    if (activeContextEngine && activeContextEngine.info.id !== "legacy") {
      assertContextEngineHostSupport({
        ...
        operation: "agent-run",
        host: ORIRO_EMBEDDED_CONTEXT_ENGINE_HOST,
      });
    }
    ...
    const toolsRaw = !shouldConstructTools
      ...
      : (() => {
          ...
          corePluginToolStages.mark("attempt:create-oriro-coding-tools");
    ...
    try {
      ...
      await withOwnedSessionWriteLock(async () => {
        await runAttemptContextEngineBootstrap({
          ...
          contextEngineHostSupport: ORIRO_EMBEDDED_CONTEXT_ENGINE_HOST,
      ...
      // DefaultResourceLoader.reload() rehydrates settings from disk and can drop Oriro
      // compaction overrides applied in createPreparedEmbeddedAgentSettingsManager — same
      // rehydration also restores Oriro runtime's auto-compaction (oriro#75799), so re-apply
      // both guards.
      applyAgentCompactionSettingsFromConfig({
        settingsManager,
        cfg: params.config,
      ...
      // exact Oriro-managed registrations so custom tools survive startup and
      ...
      if (isRawModelRun) {
        ...
        // normal Oriro agent/tool prompt when `session.prompt()` starts.
      ...
      if (activeContextEngine?.info.ownsCompaction === true) {
        ...
        const contextEngineLoopRuntimeSettings = buildContextEngineRuntimeSettings({
          contextEngineHost: ORIRO_EMBEDDED_CONTEXT_ENGINE_HOST,
          provider: params.provider,
      ...
      try {
        if (isRawModelRun) {
          ...
          if (params.sessionKey && params.config && !isRawModelRun) {
            // Capability guidance must include deferred Oriro tools without
      ...
      toolSearchCatalogExecutor = async (toolParams) => {
        try {
          if (toolParams.source === "oriro" && toolParams.sourceName === "core") {
      ...
      try {
        ...
        await withOwnedSessionWriteLock(async () => {
          ...
          // double-compaction. See: https://github.com/oriro-ai/cli/issues/9282
          ...
          if (promptError && promptErrorSource === "prompt" && !compactionOccurredThisAttempt) {
            try {
              activeSessionManager.appendCustomEntry("oriro:prompt-error", {
                timestamp: Date.now(),
                runId: params.runId,
                sessionId: params.sessionId,
                provider: params.provider,
      ...
      // See: https://github.com/oriro-ai/cli/issues/8643
