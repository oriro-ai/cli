---
summary: "Use GMI Cloud's OpenAI-compatible API with Oriro"
read_when:
  - You want to run Oriro with GMI Cloud models
  - You need the GMI provider id, key, or endpoint
title: "GMI Cloud"
---

GMI Cloud is a hosted inference platform for frontier and open-weight models
behind an OpenAI-compatible API. In Oriro it is an official external provider
...
This provider uses OpenAI-compatible chat semantics. Oriro owns the provider
...
```bash
oriro plugins install @oriro/gmi-provider
...
```bash
oriro onboard --auth-choice gmi-api-key
```
...
all times. Use Oriro's model listing command to see what the configured
provider reports in your environment:

```bash
oriro models list --provider gmi
```

## Troubleshooting

- `401` or `403`: check that `GMI_API_KEY` is set for the process running
  Oriro, or re-run onboarding to store the key in the provider auth profile.
- Unknown model errors: confirm the model exists in your GMI account and use the
  full `gmi/<route-id>` ref shown by `oriro models list --provider gmi`.
