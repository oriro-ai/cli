...
vi.mock("oriro/plugin-sdk/setup", async () => {
  const actual = await vi.importActual<object>("oriro/plugin-sdk/setup");
  return {
    ...actual,
...
vi.mock("oriro/plugin-sdk/channel-config-schema", async () => {
  const actual = await vi.importActual<object>("oriro/plugin-sdk/channel-config-schema");
  return {
    ...actual,
...
vi.mock("oriro/plugin-sdk/webhook-ingress", async () => {
  const actual = await vi.importActual<object>("oriro/plugin-sdk/webhook-ingress");
  return {
    ...actual,
...
vi.mock("./runtime.js", () => ({
  getSynologyRuntime: vi.fn(() => ({
    ...
    channel: {
      ...
      session: {
        resolveStorePath: vi.fn(() => "/tmp/oriro/synology-chat-sessions.json"),
