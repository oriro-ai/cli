...
extension GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [
            ...
            OriroCapability.screen.rawValue,
        ]
        ...
        if cameraEnabled { caps.append(OriroCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(OriroCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = OriroLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(OriroCapability.location.rawValue) }

        caps.append(OriroCapability.device.rawValue)
        caps.append(OriroCapability.talk.rawValue)
        if WatchMessagingService.isSupportedOnDevice() {
            caps.append(OriroCapability.watch.rawValue)
        }
        caps.append(OriroCapability.photos.rawValue)
        caps.append(OriroCapability.contacts.rawValue)
        caps.append(OriroCapability.calendar.rawValue)
        caps.append(OriroCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(OriroCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            OriroScreenCommand.record.rawValue,
            OriroSystemCommand.notify.rawValue,
            OriroChatCommand.push.rawValue,
        ...
        if caps.contains(OriroCapability.camera.rawValue) {
            commands.append(OriroCameraCommand.list.rawValue)
            commands.append(OriroCameraCommand.snap.rawValue)
            commands.append(OriroCameraCommand.clip.rawValue)
        }
        if caps.contains(OriroCapability.location.rawValue) {
            commands.append(OriroLocationCommand.get.rawValue)
        }
        if caps.contains(OriroCapability.device.rawValue) {
            commands.append(OriroDeviceCommand.status.rawValue)
            commands.append(OriroDeviceCommand.info.rawValue)
        }
        if caps.contains(OriroCapability.watch.rawValue) {
            commands.append(OriroWatchCommand.status.rawValue)
            commands.append(OriroWatchCommand.notify.rawValue)
        }
        if caps.contains(OriroCapability.photos.rawValue) {
            commands.append(OriroPhotosCommand.latest.rawValue)
        }
        if caps.contains(OriroCapability.contacts.rawValue) {
            commands.append(OriroContactsCommand.search.rawValue)
            commands.append(OriroContactsCommand.add.rawValue)
        }
        if caps.contains(OriroCapability.calendar.rawValue) {
            commands.append(OriroCalendarCommand.events.rawValue)
            commands.append(OriroCalendarCommand.add.rawValue)
        }
        if caps.contains(OriroCapability.reminders.rawValue) {
            commands.append(OriroRemindersCommand.list.rawValue)
            commands.append(OriroRemindersCommand.add.rawValue)
        }
        if caps.contains(OriroCapability.motion.rawValue) {
            commands.append(OriroMotionCommand.activity.rawValue)
            commands.append(OriroMotionCommand.pedometer.rawValue)
        }
