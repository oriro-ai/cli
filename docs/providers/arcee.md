...
title: "Arcee AI"
read_when:
  - You want to use Arcee AI with Oriro
...
```bash
oriro plugins install @oriro/arcee-provider
...
<Tabs>
  <Tab title="Direct (Arcee platform)">
    <Steps>
      ...
      <Step title="Run onboarding">
        ```bash
        oriro onboard --auth-choice arceeai-api-key
        ```
      </Step>
  ...
  <Tab title="Via OpenRouter">
    <Steps>
      ...
      <Step title="Run onboarding">
        ```bash
        oriro onboard --auth-choice arceeai-openrouter
        ```
      </Step>
      <Step title="Set a default model">
        ```json5
...
Oriro currently ships this Arcee static catalog:
...
<AccordionGroup>
  ...
  <Accordion title="OpenRouter routing">
    ...
    Oriro handles routing transparently based on your auth choice. See the
