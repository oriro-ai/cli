...
const {
  ...
  prepareMirroredDocsDir: (
    sourceDir?: string,
    options?: {
      resolveOriroHubRepoPathImpl?: (value?: string, options?: { required?: boolean }) => string;
      syncOriroHubDocsTreeImpl?: (
...
describe("docs-link-audit", () => {
  ...
  it("cleans anchor audit docs copies when docs.json is invalid", () => {
    ...
    const before = tempEntries("oriro-docs-anchor-audit-");
    try {
      expect(() => prepareAnchorAuditDocsDir(docsRoot)).toThrow();
      const after = tempEntries("oriro-docs-anchor-audit-");
  ...
  it("cleans mirrored docs copies when OriroHub sync fails", () => {
    const before = tempEntries("oriro-docs-link-audit-");

    expect(() =>
      prepareMirroredDocsDir(undefined, {
        resolveOriroHubRepoPathImpl() {
          return path.join(os.tmpdir(), "orirohub-docs");
        },
        syncOriroHubDocsTreeImpl() {
    ...
    expect(() =>
      runDocsLinkAuditCli({
        ...
        prepareMirroredDocsDirImpl: () => ({
          ...
          dir: path.join(os.tmpdir(), "oriro-docs-mirrored"),
  ...
  it("uses Mintlify through pnpm dlx for anchor validation", () => {
    ...
    const exitCode = runDocsLinkAuditCli({
      args: ["--anchors"],
      env: { ...process.env, ORIRO_DOCS_LINK_SENTINEL: "1" },
    ...
    expect(invocation).toEqual({
      ...
      options: expect.objectContaining({
        cwd: anchorDocsDir,
        env: expect.objectContaining({ ORIRO_DOCS_LINK_SENTINEL: "1" }),
        shell: false,
        stdio: "inherit",
      }),
    });
