...
import { parseExecApprovalCommandText } from "oriro/plugin-sdk/approval-reply-runtime";
import { resolveChannelConfigWrites } from "oriro/plugin-sdk/channel-config-helpers";
import {
  buildMentionRegexes,
...
} from "oriro/plugin-sdk/channel-inbound-debounce";
import { resolveStoredModelOverride } from "oriro/plugin-sdk/command-auth-native";
import { hasControlCommand } from "oriro/plugin-sdk/command-detection";
import { isAbortRequestText } from "oriro/plugin-sdk/command-primitives-runtime";
import { buildCommandsMessagePaginated } from "oriro/plugin-sdk/command-status";
import type { DmPolicy, OriroConfig } from "oriro/plugin-sdk/config-contracts";
import type {
  TelegramGroupConfig,
  TelegramTopicConfig,
} from "oriro/plugin-sdk/config-contracts";
import { mutateConfigFile } from "oriro/plugin-sdk/config-mutation";
import { resolveChannelContextVisibilityMode } from "oriro/plugin-sdk/context-visibility-runtime";
import {
...
import { applyModelOverrideToSessionEntry } from "oriro/plugin-sdk/model-session-runtime";
import { formatModelsAvailableHeader } from "oriro/plugin-sdk/models-provider-runtime";
...
import { resolveThreadSessionKeys } from "oriro/plugin-sdk/routing";
import { danger, logVerbose, warn } from "oriro/plugin-sdk/runtime-env";
import { evaluateSupplementalContextVisibility } from "oriro/plugin-sdk/security-runtime";
import {
  getSessionEntry,
  listSessionEntries,
  patchSessionEntry,
        ...
        if (modelCallback.type === "select") {
          ...
          try {
            ...
            const scopeText = isDefaultSelection
              ...
              : `Session-only model selection. Runtime unchanged. Use /model ${escapeHtml(selection.provider)}/${escapeHtml(selection.model)} --runtime &lt;runtime&gt; to switch harnesses. The agent default in oriro.json is unchanged; /reset or a new session may return to that default.`;
            await editMessageWithButtons(
              `✅ Model ${actionText}\n\n${scopeText}`,
