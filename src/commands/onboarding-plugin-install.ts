 ...
 *
 * It selects local, OriroHub, npm, or override install sources; records durable
...
import { isOriroOrgNpmSpec, parseRegistryNpmSpec } from "../infra/npm-registry-spec.js";
import { normalizeUpdateChannel, resolveRegistryUpdateChannel } from "../infra/update-channels.js";
import {
  findBundledPluginSourceInMap,
  resolveBundledPluginSources,
} from "../plugins/bundled-sources.js";
import { ORIROHUB_INSTALL_ERROR_CODE } from "../plugins/orirohub-error-codes.js";
...
type InstallChoice = "orirohub" | "npm" | "local" | "skip";
type InstallPluginFromOriroHubResult = Awaited<
  ReturnType<(typeof import("../plugins/orirohub.js"))["installPluginFromOriroHub"]>
>;
...
function shouldFallbackOriroHubToNpm(params: {
  ...
  // Only official Oriro npm packages are safe fallback targets for OriroHub
...
function addPluginLoadPath(cfg: OriroConfig, pluginPath: string): OriroConfig {
...
function resolveOriroHubSpecForOnboarding(install: PluginPackageInstall): string | null {
  const orirohubSpec = install.orirohubSpec?.trim();
  if (!orirohubSpec) {
    return null;
  }
  const parsed = parseOriroHubPluginSpec(orirohubSpec);
  return parsed ? orirohubSpec : null;
}
...
function resolveInstallDefaultChoice(params: {
  ...
  hasOriroHubSpec: boolean;
  hasNpmSpec: boolean;
}): InstallChoice {
  const { cfg, entry, localPath, bundledLocalPath, hasOriroHubSpec, hasNpmSpec } = params;
  const hasRemoteSpec = hasOriroHubSpec || hasNpmSpec;
  const entryDefault = entry.install.defaultChoice;
  const remoteDefault = (): InstallChoice => {
    if (entryDefault === "orirohub" && hasOriroHubSpec) {
      return "orirohub";
    }
    if (entryDefault === "npm" && hasNpmSpec) {
      return "npm";
    }
    return hasNpmSpec ? "npm" : "orirohub";
  };
  if (!hasRemoteSpec) {
...
async function promptInstallChoice(params: {
  ...
  effectiveOriroHubSpec?: string | null;
}): Promise<InstallChoice> {
  const rawOriroHubSpec = resolveOriroHubSpecForOnboarding(params.entry.install);
  ...
  const orirohubSpec = params.bundledLocalPath
    ? null
    : (params.effectiveOriroHubSpec ?? rawOriroHubSpec);
  const npmSpec = params.bundledLocalPath ? null : (params.effectiveNpmSpec ?? rawNpmSpec);
  const safeLabel = sanitizeTerminalText(params.entry.label);
  const safeOriroHubSpec = orirohubSpec ? sanitizeTerminalText(orirohubSpec) : null;
  ...
  if (safeOriroHubSpec) {
    options.push({
      value: "orirohub",
      label: t("wizard.plugins.downloadFromOriroHub", { spec: safeOriroHubSpec }),
    });
  }
  if (safeNpmSpec) {
    options.push({
    ...
    params.defaultChoice === "local" && !params.localPath
      ? orirohubSpec
        ...
        : npmSpec
          ? "npm"
          : "skip"
      : params.defaultChoice === "orirohub" && !orirohubSpec
        ? npmSpec
          ? "npm"
          : params.localPath
            ? "local"
 ...
 * OriroHub…`, `Extracting /tmp/…/wecom-…-2026.4.23.tgz…`, `Installing to
...
async function installPluginFromOriroHubSpecWithProgress(params: {
  cfg: OriroConfig;
  entry: OnboardingPluginInstallEntry;
  orirohubSpec: string;
  prompter: WizardPrompter;
  ...
  | {
      status: "completed";
      result: InstallPluginFromOriroHubResult;
    }
> {
  const safeLabel = sanitizeTerminalText(params.entry.label);
  const progress = params.prompter.progress(formatPluginInstallProgress(safeLabel));
  ...
  try {
    const { installPluginFromOriroHub } = await import("../plugins/orirohub.js");
    const result = await withTimeout(
      installPluginFromOriroHub({
        spec: params.orirohubSpec,
        timeoutMs: ONBOARDING_PLUGIN_INSTALL_TIMEOUT_MS,
        config: params.cfg,
        extensionsDir: resolveDefaultPluginExtensionsDir(),
        expectedPluginId: params.entry.pluginId,
...
export async function ensureOnboardingPluginInstalled(params: {
  ...
  const orirohubSpec = resolveOriroHubSpecForOnboarding(entry.install);
  ...
  const orirohubSpecs = orirohubSpec
    ? resolveOriroHubInstallSpecsForUpdateChannel({
        spec: orirohubSpec,
        updateChannel,
      })
  ...
  const npmSpecs = npmSpec
    ...
    : null;
  const orirohubInstallSpec = orirohubSpecs?.installSpec ?? orirohubSpec;
  ...
  const defaultChoice = resolveInstallDefaultChoice({
    ...
    localPath,
    bundledLocalPath,
    hasOriroHubSpec: Boolean(orirohubSpec),
  ...
  const choice =
    params.promptInstall === false
      ...
      : await promptInstallChoice({
          ...
          effectiveOriroHubSpec: orirohubInstallSpec,
  ...
  if (choice === "orirohub" && orirohubInstallSpec) {
    const installOutcome = await installPluginFromOriroHubSpecWithProgress({
      cfg: next,
      entry,
      orirohubSpec: orirohubInstallSpec,
      prompter,
      runtime,
    });
    ...
    if (installOutcome.status === "timed_out") {
      await prompter.note(
        formatPluginInstallTimedOutNote(sanitizeTerminalText(orirohubInstallSpec)),
        t("wizard.plugins.installTitle"),
      );
      runtime.error?.(
        `Plugin install timed out after ${ONBOARDING_PLUGIN_INSTALL_TIMEOUT_MS}ms: ${sanitizeTerminalText(orirohubInstallSpec)}`,
      );
      return {
        cfg: next,
        installed: false,
    ...
    if (result.ok) {
      ...
      next = recordPluginInstall(next, {
        pluginId: result.pluginId,
        ...buildOriroHubPluginInstallRecordFields(result.orirohub),
    ...
    await prompter.note(
      [
        t("wizard.plugins.installFailed", {
          spec: sanitizeTerminalText(orirohubInstallSpec),
          error: summarizeInstallError(result.error),
        }),
        t("wizard.plugins.returningToSelection"),
      ].join("\n"),
    ...
    if (!npmInstallSpec || !shouldFallbackOriroHubToNpm({ result, npmSpec: npmInstallSpec })) {
      runtime.error?.(`Plugin install failed: ${sanitizeTerminalText(result.error)}`);
      return {
        cfg: next,
        installed: false,
    ...
    // OriroHub package/version misses for official packages can recover through
