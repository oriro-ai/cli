...
case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/oriro/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|ORIRO_EXPIRING)
