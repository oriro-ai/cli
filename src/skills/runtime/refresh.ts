...
function resolveWatchTargets(workspaceDir: string, config?: OriroConfig): WatchTarget[] {
  const baseRoots: Array<{ path: string; source: string }> = [];
  if (workspaceDir.trim()) {
    baseRoots.push({ path: path.join(workspaceDir, "skills"), source: "oriro-workspace" });
    baseRoots.push({
      path: path.join(workspaceDir, ".agents", "skills"),
      source: "agents-skills-project",
    });
  }
  baseRoots.push({ path: path.join(CONFIG_DIR, "skills"), source: "oriro-managed" });
  baseRoots.push({
...
function isTrustedSymlinkSkillTarget(
  ...
  if (source === "oriro-managed" || source === "agents-skills-personal") {
    return true;
  }
  return (
...
function resolveWatchDebounceMs(config?: OriroConfig): number {
...
export function ensureSkillsWatcher(params: { workspaceDir: string; config?: OriroConfig }) {
