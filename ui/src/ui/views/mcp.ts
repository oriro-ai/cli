...
function renderServerRow(props: McpViewProps, server: McpServerRow) {
  ...
  const probeCommand = `oriro mcp probe ${quotedName}`;
  const loginCommand = `oriro mcp login ${quotedName}`;
  return html`
    ...
    <section class="mcp-page">
      ...
      <section class="card mcp-command-card">
        ...
        <div class="mcp-command-card__grid">
          <code>oriro mcp status --verbose</code>
          <code>oriro mcp doctor --probe</code>
          <code>oriro mcp login &lt;name&gt;</code>
          <code>oriro mcp reload</code>
        </div>
      </section>
