...
Run the [Agent Client Protocol (ACP)](https://agentclientprotocol.com/) bridge that talks to an Oriro Gateway.

This command speaks ACP over stdio for IDEs and forwards prompts to the Gateway
over WebSocket. It keeps ACP sessions mapped to Gateway session keys.

`oriro acp` is a Gateway-backed ACP bridge, not a full ACP-native editor
runtime. It focuses on session routing, prompt delivery, and basic streaming
updates.

If you want an external MCP client to talk directly to Oriro channel
conversations instead of hosting an ACP harness session, use
[`oriro mcp serve`](/cli/mcp) instead.
...
- Oriro acts as an ACP server
- an IDE or ACP client connects to Oriro
- Oriro forwards that work into a Gateway session

This is different from [ACP Agents](/tools/acp-agents), where Oriro runs an
external harness such as Codex or Claude Code through `acpx`.

Quick rule:

- editor/client wants to talk ACP to Oriro: use `oriro acp`
- Oriro should launch Codex/Claude/Gemini as an ACP harness: use `/acp spawn` and [ACP Agents](/tools/acp-agents)
...
| Session lineage metadata                                              | Implemented | Session listings and session info snapshots include Oriro parent and child lineage in `_meta` so ACP clients can render subagent graphs without private Gateway side channels.                                                                |
...
| Per-session MCP servers (`mcpServers`)                                | Unsupported | Bridge mode rejects per-session MCP server requests. Configure MCP on the Oriro gateway or agent instead.                                                                                                                                     |
...
# Remote Gateway
oriro acp --url wss://gateway-host:18789 --token <token>

# Remote Gateway (token from file)
oriro acp --url wss://gateway-host:18789 --token-file ~/.oriro/gateway.token

# Attach to an existing session key
oriro acp --session agent:main:main

# Attach by label (must already exist)
oriro acp --session-label "support inbox"

# Reset the session key before the first prompt
oriro acp --session agent:main:main --reset-session
```
...
oriro acp client --server-args --url wss://gateway-host:18789 --token-file ~/.oriro/gateway.token

# Override the server command (default: oriro)
oriro acp client --server "node" --server-args oriro.mjs acp --url ws://127.0.0.1:19001
```
...
- This ACP bridge policy is separate from ACPX harness permissions. If you run Oriro through the `acpx` backend, `plugins.entries.acpx.config.permissionMode=approve-all` is the break-glass "yolo" switch for that harness session.

## Protocol smoke testing

For protocol-level debugging, start a Gateway with isolated state and drive
`oriro acp` over stdio with an ACP JSON-RPC client. Cover `initialize`,
...
Avoid using `oriro gateway call sessions.list` as the only ACP proof. That
...
it to drive an Oriro Gateway session.

1. Ensure the Gateway is running (local or remote).
2. Configure the Gateway target (config or flags).
3. Point your IDE to run `oriro acp` over stdio.

Example config (persisted):

```bash
oriro config set gateway.remote.url wss://gateway-host:18789
oriro config set gateway.remote.token <token>
```
...
```bash
oriro acp --session agent:main:main
oriro acp --session agent:design:main
oriro acp --session agent:qa:bug-123
```
...
If you want ACPX-backed sessions to see Oriro plugin tools or selected
...
[Oriro tools MCP bridge](/tools/acp-agents-setup#oriro-tools-mcp-bridge).

## Use from `acpx` (Codex, Claude, other ACP clients)

If you want a coding agent such as Codex or Claude Code to talk to your
Oriro bot over ACP, use `acpx` with its built-in `oriro` target.

Typical flow:

1. Run the Gateway and make sure the ACP bridge can reach it.
2. Point `acpx oriro` at `oriro acp`.
3. Target the Oriro session key you want the coding agent to use.

Examples:

```bash
# One-shot request into your default Oriro ACP session
acpx oriro exec "Summarize the active Oriro session state."

# Persistent named session for follow-up turns
acpx oriro sessions ensure --name codex-bridge
acpx oriro -s codex-bridge --cwd /path/to/repo \
  "Ask my Oriro work agent for recent context relevant to this repo."
```

If you want `acpx oriro` to target a specific Gateway and session key every
time, override the `oriro` agent command in `~/.acpx/config.json`:

```json
{
  "agents": {
    "oriro": {
      "command": "env ORIRO_HIDE_BANNER=1 ORIRO_SUPPRESS_NOTES=1 oriro acp --url ws://127.0.0.1:18789 --token-file ~/.oriro/gateway.token --session agent:main:main"
    }
  }
}
```

For a repo-local Oriro checkout, use the direct CLI entrypoint instead of the
dev runner so the ACP stream stays clean. For example:

```bash
env ORIRO_HIDE_BANNER=1 ORIRO_SUPPRESS_NOTES=1 node oriro.mjs acp ...
```

This is the easiest way to let Codex, Claude Code, or another ACP-aware client
pull contextual information from an Oriro agent without scraping a terminal.
...
In Zed, open the Agent panel and select "Oriro ACP" to start a thread.
...
- Prefer `--token-file`/`--password-file` or environment variables (`ORIRO_GATEWAY_TOKEN`, `ORIRO_GATEWAY_PASSWORD`).
- Gateway auth resolution follows the shared contract used by other Gateway clients:
  - local mode: env (`ORIRO_GATEWAY_*`) -> `gateway.auth.*` -> `gateway.remote.*` fallback only when `gateway.auth.*` is unset (configured-but-unresolved local SecretRefs fail closed)
...
- ACP runtime backend child processes receive `ORIRO_SHELL=acp`, which can be used for context-specific shell/profile rules.
- `oriro acp client` sets `ORIRO_SHELL=acp-client` on the spawned bridge process.

### `acp client` options

- `--cwd <dir>`: working directory for the ACP session.
- `--server <command>`: ACP server command (default: `oriro`).
