...
import { resolveAccountEntry } from "oriro/plugin-sdk/account-core";
import { CHANNEL_APPROVAL_NATIVE_RUNTIME_CONTEXT_CAPABILITY } from "oriro/plugin-sdk/approval-handler-runtime";
import { resolveInboundDebounceMs } from "oriro/plugin-sdk/channel-inbound-debounce";
import { registerChannelRuntimeContext } from "oriro/plugin-sdk/channel-runtime-context";
import { formatCliCommand } from "oriro/plugin-sdk/cli-runtime";
import { isControlCommandMessage } from "oriro/plugin-sdk/command-detection";
import { drainPendingDeliveries } from "oriro/plugin-sdk/delivery-queue-runtime";
import { DEFAULT_GROUP_HISTORY_LIMIT } from "oriro/plugin-sdk/reply-history";
import { resolveAgentRoute } from "oriro/plugin-sdk/routing";
import { logVerbose } from "oriro/plugin-sdk/runtime-env";
import { registerUnhandledRejectionHandler } from "oriro/plugin-sdk/runtime-env";
import { getChildLogger } from "oriro/plugin-sdk/runtime-env";
import {
  defaultRuntime,
  formatDurationPrecise,
  warn,
  type RuntimeEnv,
} from "oriro/plugin-sdk/runtime-env";
import { enqueueSystemEvent } from "oriro/plugin-sdk/system-event-runtime";
    ...
    while (true) {
        ...
        if (setupDecision) {
          ...
          if (setupDecision.action === "stop") {
            ...
            if (setupDecision.healthState === "logged-out") {
              runtime.error(
                `WhatsApp session logged out during setup. Run \`${formatCliCommand("oriro channels login --channel whatsapp")}\` to relink.`,
              );
            } else if (setupDecision.healthState === "conflict") {
              runtime.error(
                `WhatsApp Web connection closed during setup (status ${setupDecision.normalized.statusLabel}: session conflict). Resolve conflicting WhatsApp Web sessions, then restart the channel. To force a fresh QR, run \`${formatCliCommand("oriro channels logout --channel whatsapp")}\` before \`${formatCliCommand("oriro channels login --channel whatsapp")}\`. Stopping web monitoring.`,
              );
            } else {
              runtime.error(
                `WhatsApp Web connection closed during setup (status ${setupDecision.normalized.statusLabel}) after ${setupDecision.reconnectAttempts}/${reconnectPolicy.maxAttempts} attempts. Relink with \`${formatCliCommand("oriro channels login --channel whatsapp")}\` if the issue persists.`,
              );
            }
            await controller.shutdown();
            break;
      ...
      if (decision.action === "stop") {
        ...
        if (decision.healthState === "logged-out") {
          runtime.error(
            `WhatsApp session logged out. Run \`${formatCliCommand("oriro channels login --channel whatsapp")}\` to relink.`,
          );
