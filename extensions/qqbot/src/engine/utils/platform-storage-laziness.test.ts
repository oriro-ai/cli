...
describe("qqbot storage laziness", () => {
  ...
  it("does not create ~/.oriro/qqbot from module imports or read-only probes", async () => {
    ...
    const qqbotRoot = path.join(homeDir, ".oriro", "qqbot");
  ...
  it("creates storage when qqbot persists runtime state", async () => {
    ...
    const qqbotRoot = path.join(homeDir, ".oriro", "qqbot");
    const sqlitePath = path.join(stateDir, "state", "oriro.sqlite");
