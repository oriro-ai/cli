// Update Orirotributors tests cover update orirotributors script behavior.
...
function mockOrirotributorsFixture() {
  const readme = [
    "# Fixture",
    "",
  ...
  vi.doMock("node:fs", () => ({
    readFileSync: vi.fn((path: string) => {
      if (path.endsWith("scripts/orirotributors-map.json")) {
        return "{}\n";
      }
  ...
  const execSync = vi.fn((cmd: string) => {
    if (cmd === 'gh api "repos/oriro-ai/cli/contributors?per_page=100&anon=1" --paginate') {
    ...
    if (
      cmd ===
      "gh pr list -R oriro-ai/cli --state merged --limit 5000 --json author --jq '.[].author.login'"
    ) {
      return "";
    }
...
async function importUpdateOrirotributors() {
  const scriptUrl = pathToFileURL(resolve(originalCwd, "scripts/update-orirotributors.ts")).href;
  await import(`${scriptUrl}?case=${Date.now()}`);
}

describe("update-orirotributors", () => {
  it("rejects unsafe avatar probe content lengths before reading the body", async () => {
    const fixture = mockOrirotributorsFixture();
    ...
    await importUpdateOrirotributors();
  ...
  it("cancels stalled avatar probe body reads at the probe timeout", async () => {
    ...
    vi.useFakeTimers();
    const imported = importUpdateOrirotributors();
