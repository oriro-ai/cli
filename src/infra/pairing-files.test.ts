...
describe("pairing file helpers", () => {
  it("resolves pairing file paths from explicit base dirs", () => {
    expect(resolvePairingPaths("/tmp/oriro-state", "devices")).toEqual({
      dir: path.join("/tmp/oriro-state", "devices"),
      pendingPath: path.join("/tmp/oriro-state", "devices", "pending.json"),
      pairedPath: path.join("/tmp/oriro-state", "devices", "paired.json"),
    });
  });
