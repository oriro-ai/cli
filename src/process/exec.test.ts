...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with base env and drops undefined values", () => {
    ...
    expect(resolved.ORIRO_BASE_ENV).toBe("base");
    expect(resolved.ORIRO_TEST_ENV).toBe("ok");
    expect(resolved.ORIRO_TO_REMOVE).toBeUndefined();
    expect(resolved.ORIRO_CLI).toBe(ORIRO_CLI_ENV_VALUE);
  });
