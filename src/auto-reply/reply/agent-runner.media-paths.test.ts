...
describe("runReplyAgent media path normalization", () => {
  ...
  it("reuses the provided media context inside runAgentTurnWithFallback", async () => {
    // Regression test for oriro-ai/cli#68056.
  ...
  it("passes current inbound media paths as native Oriro images", async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), "oriro-native-agent-media-"));
  ...
  it("does not pass recent history images as unlabeled native Oriro images", async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), "oriro-native-agent-history-"));
    cleanupPaths.push(tmpDir);
  ...
  it("falls back to prompt refs instead of forwarding partial current media", async () => {
    const tmpDir = await mkdtemp(path.join(os.tmpdir(), "oriro-native-agent-partial-"));
    cleanupPaths.push(tmpDir);
