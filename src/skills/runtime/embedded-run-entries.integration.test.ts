...
async function setupBundledDiffsPlugin() {
  const bundledPluginsDir = await createTempDir("oriro-bundled-");
  const workspaceDir = await createTempDir("oriro-workspace-");
...
async function resolveBundledDiffsSkillEntries(config?: OriroConfig) {
