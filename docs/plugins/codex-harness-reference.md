...
By default, Oriro starts the managed Codex binary shipped with the bundled
plugin:
...
| `clearEnv`                                    | `[]`                                                   | Extra environment variable names removed from the spawned stdio app-server process after Oriro builds its inherited environment.                                                                                                                                                                                                                                                             |
...
| `sandbox`                                     | `"danger-full-access"` or an allowed guardian sandbox  | Native Codex sandbox mode sent to thread start and resume. Active Oriro sandboxes narrow `danger-full-access` turns to Codex `workspace-write`; the turn network flag follows Oriro sandbox egress.                                                                                                                                                                                       |
...
Oriro treats non-loopback WebSocket app-server URLs as remote and requires
...
shorthand before Oriro builds app-server start options, and unresolved
structured SecretRefs fail before any token or header is sent. When native Codex
plugins are configured, Oriro uses the connected app-server's plugin control
plane to install or refresh those plugins and then refreshes app inventory so
plugin-owned apps are visible to the Codex thread. Only connect Oriro to
remote app-servers that are trusted to accept Oriro-managed plugin installs
...
unattended Oriro turns and heartbeats make progress without native approval
prompts that nobody is around to answer.

If Codex's local system requirements file disallows implicit YOLO approval,
reviewer, or sandbox values, Oriro treats the implicit default as guardian
...
When an Oriro sandbox is active, the local Codex app-server process still
runs on the Gateway host. Oriro therefore disables Codex native Code Mode,
user MCP servers, and app-backed plugin execution for that turn instead of
treating Codex host-side sandboxing as equivalent to the Oriro sandbox
backend. Shell access is exposed through Oriro sandbox-backed dynamic tools
...
`workspace-write` without active Oriro sandboxing. If you see
`bwrap: setting up uid map: Permission denied` or
`bwrap: loopback: Failed RTM_NEWADDR: Operation not permitted`, run
`oriro doctor` and fix the reported host namespace policy for the Oriro
...
The stable default is fail-closed: active Oriro sandboxing disables native
...
try Codex's remote environment support with Oriro's sandbox backend. This
...
When the flag is on and the current Oriro session is sandboxed, Oriro
...
Oriro-owned environment. If the app-server cannot register the environment,
...
loopback exec-server unless it is running on the same host, so Oriro rejects
that combination.
...
1. An explicit Oriro Codex auth profile for the agent.
...
Stdio app-server launches inherit Oriro's process environment by default.
Oriro owns the Codex app-server account bridge and sets `CODEX_HOME` to a
per-agent directory under that agent's Oriro state. That keeps Codex config,
accounts, plugin cache/data, and thread state scoped to the Oriro agent
instead of leaking in from the operator's personal `~/.codex` home.

Oriro does not rewrite `HOME` for normal local app-server launches. Codex-run
subprocesses such as `oriro`, `gh`, `git`, cloud CLIs, and shell commands see
...
Oriro plugins and Oriro skill snapshots still flow through Oriro's own
...
part of an Oriro agent, inventory them explicitly:

```bash
oriro migrate codex --dry-run
oriro migrate apply codex --yes
```
...
Most remaining Oriro integration tools, such as messaging, media, cron,
browser, nodes, gateway, `heartbeat_respond`, and `web_search`, are available
through Codex tool search under the `oriro` namespace. This keeps the initial
...
surface, while explicit Oriro or ACP delegation is still available through
the `oriro` dynamic tool namespace.
...
Dynamic tool budgets are capped at 600000 ms. On timeout, Oriro aborts the
...
post-tool idle budget: after Oriro returns an `item/tool/call` response, after
...
`turn/completed`, Oriro best-effort interrupts the native turn and releases
...
availability is owned by Codex app-server, so the list can change when Oriro
...
If discovery fails or times out, Oriro uses a bundled fallback catalog for:
...
Codex handles `AGENTS.md` itself through native project-doc discovery. Oriro
...
For Oriro workspace parity, the Codex harness resolves the other bootstrap
files. `SOUL.md`, `IDENTITY.md`, `TOOLS.md`, and `USER.md` are forwarded as
Oriro Codex developer instructions because they define the active agent,
available workspace guidance, and user profile. The compact Oriro skills
...
`BOOTSTRAP.md` when present is forwarded as Oriro turn input reference
context.
