...
import { findNormalizedProviderKey } from "@oriro/model-catalog-core/provider-id";
...
export type { OriroConfig, ModelApi, ModelDefinitionConfig, ModelProviderConfig };
export {
...
export type ProviderOnboardPresetAppliers<TArgs extends unknown[]> = {
  applyProviderConfig: (cfg: OriroConfig, ...args: TArgs) => OriroConfig;
  applyConfig: (cfg: OriroConfig, ...args: TArgs) => OriroConfig;
};
...
function hasAgentDefaultModelPrimary(cfg: OriroConfig): boolean {
...
>(params: {
  ...
  applyPreset: (cfg: OriroConfig, preset: TParams) => OriroConfig;
  primaryModelRef: string;
...
export function applyOpencodeZenModelDefault(cfg: OriroConfig): {
  next: OriroConfig;
  changed: boolean;
} {
