...
read_when:
  - You want to connect Oriro to SMS through Twilio
  - You need SMS webhook or allowlist setup
title: "SMS"
---

Oriro can receive and send SMS through a Twilio phone number or Messaging Service. The Gateway registers an inbound webhook route, validates Twilio request signatures by default, and sends replies back through Twilio's Messages API.
...
- A public HTTPS URL that reaches your Oriro Gateway.
...
```bash
oriro config patch --file ./sms.patch.json5 --dry-run
oriro config patch --file ./sms.patch.json5
```
...
```bash
oriro pairing list sms
oriro pairing approve sms <CODE>
```
...
`authToken` can be a SecretRef. Use this when the Gateway should resolve the Twilio Auth Token from the Oriro secrets runtime instead of storing plaintext config:
...
```bash
oriro message send --channel sms --target sms:+15551234567 --message "hello"
```
...
```bash
oriro message send --target twilio-sms:+15551234567 --message "hello"
```
...
SMS output is plain text. Oriro strips markdown, flattens fenced code blocks, preserves readable links, and chunks long replies before sending them through Twilio.
...
```bash
oriro channels capabilities --channel sms
oriro channels status --channel sms --probe --json
```

3. Send an SMS to the Twilio number from your phone.
4. Run `oriro pairing list sms`.
5. Approve the pairing code with `oriro pairing approve sms <CODE>`.
6. Send another SMS and confirm the agent replies.

For outbound-only testing, use:

```bash
oriro message send --channel sms --target sms:+15557654321 --message "Oriro SMS test"
```
...
```bash
imsg send --to "+15551234567" --service sms --text "Oriro SMS E2E $(date -u +%Y%m%dT%H%M%SZ)" --json
oriro pairing list sms
oriro pairing approve sms <CODE>
...
By default, Oriro validates `X-Twilio-Signature` using `publicWebhookUrl` and `authToken`. Keep `publicWebhookUrl` byte-for-byte aligned with the URL configured in Twilio, including scheme, host, path, and query string.
...
### Twilio returns 403 or Oriro rejects the webhook
