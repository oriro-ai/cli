...
function formatTelegramAccountConfigPath(cfg: OriroConfig, accountId: string): string {
  const telegram = asObjectRecord((cfg.channels as Record<string, unknown> | undefined)?.telegram);
  const accounts = asObjectRecord(telegram?.accounts);
...
export function maybeRepairTelegramApiRoots(cfg: OriroConfig): {
  config: OriroConfig;
  changes: string[];
} {
...
async function repairTelegramConfig(params: { cfg: OriroConfig }): Promise<{
  config: OriroConfig;
  changes: string[];
}> {
...
export async function maybeRepairTelegramAllowFromUsernames(cfg: OriroConfig): Promise<{
  config: OriroConfig;
  changes: string[];
}> {
  ...
  const { getChannelsCommandSecretTargetIds, resolveCommandSecretRefsViaGateway } =
    await import("oriro/plugin-sdk/runtime");
