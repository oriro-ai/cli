...
pnpm test:parallels:macos \
  --discord-token-env ORIRO_PARALLELS_DISCORD_TOKEN \
...
- Use the `oriro` wrapper for guest `message send/read`; `node oriro.mjs message ...` does not expose the lazy message subcommands the same way.
...
- Per-phase logs: `/tmp/oriro-parallels-smoke.*`
...
- host inbound nonce appears in `oriro message read` output
