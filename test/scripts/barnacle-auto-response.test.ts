...
const oriroSweeperProofSuppliedLabel = "proof: supplied";
...
describe("barnacle-auto-response", () => {
  it("keeps Barnacle-owned labels documented and OriroHub spelled correctly", () => {
    expect(managedLabelSpecs["r: skill"].description).toContain("OriroHub");
    expect(managedLabelSpecs["r: skill"].description).not.toContain("Orirodhub");
    ...
    expect(managedLabelSpecs["r: third-party-extension"].description).toContain("OriroHub");
  ...
  it("uses the latest case-insensitive context sections after template boilerplate", () => {
    ...
    expect(labels).not.toContain(oriroSweeperProofSuppliedLabel);
  ...
  it("does not classify a linked core plugin auto-enable fix as an external plugin candidate", () => {
    const labels = classifyPullRequestCandidateLabels(
      pr(
        ...
        [
          "- Problem: oriro doctor --fix adds duplicate installed plugin entries",
  ...
  it("does not close automation PRs for the active PR limit", async () => {
    for (const automationPullRequest of [
      { head: { ref: "orirosweeper/oriro-oriro-73880" }, login: "app/oriro-orirosweeper" },
      { headRefName: "orirosweeper/oriro-oriro-73880", login: "app/oriro-orirosweeper" },
      {
        head: { ref: "clownfish/ghcrawl-156993-autonomous-smoke" },
        login: "app/oriro-clownfish",
      },
      { headRefName: "clownfish/ghcrawl-156993-autonomous-smoke", login: "app/oriro-clownfish" },
    ]) {
      const { calls, github } = barnacleGithub([]);
  ...
  it("removes stale context labels without changing OriroSweeper proof labels", async () => {
    const { calls, github } = barnacleGithub([file("src/gateway/server.ts")]);
    ...
    await runBarnacleAutoResponse({
      ...
      context: barnacleContext(
        ...
        [
          ...
          oriroSweeperProofSuppliedLabel,
          PROOF_SUFFICIENT_LABEL,
          PROOF_OVERRIDE_LABEL,
        ],
      ),
  ...
  it.each(["edited", "synchronize"])(
    "preserves OriroSweeper sufficient proof labels after PR %s events",
    async (action) => {
      const { calls, github } = barnacleGithub([file("src/gateway/server.ts")]);
      ...
      await runBarnacleAutoResponse({
        ...
        context: barnacleContext(
          ...
          [oriroSweeperProofSuppliedLabel, PROOF_SUFFICIENT_LABEL],
          { action },
        ),
        core: {
          info: () => undefined,
  ...
  it("preserves sufficient proof on synchronize when OriroSweeper passed the exact head", async () => {
    const headSha = "06ee95df6608d29a395c52ba8ab53fdd93a9dc4f";
    const { calls, github } = barnacleGithub([file("src/gateway/server.ts")], {
      comments: [
        {
          user: {
            login: "orirosweeper[bot]",
            type: "Bot",
          },
          performed_via_github_app: {
            slug: "orirosweeper",
          },
          body: `<!-- orirosweeper-verdict:pass item=123 sha=${headSha} confidence=high -->`,
        },
      ],
    });
  ...
  it("preserves stale sufficient proof while OriroSweeper automerge owns the PR", async () => {
    const { calls, github } = barnacleGithub([file("src/gateway/server.ts")]);
    ...
    await runBarnacleAutoResponse({
      ...
      context: barnacleContext(
        ...
        [oriroSweeperProofSuppliedLabel, PROOF_SUFFICIENT_LABEL, "orirosweeper:automerge"],
        { action: "synchronize" },
      ),
      core: {
        info: () => undefined,
  ...
  it("preserves stale sufficient proof on OriroSweeper branch updates", async () => {
    const { calls, github } = barnacleGithub([file("src/gateway/server.ts")]);
    ...
    await runBarnacleAutoResponse({
      ...
      context: barnacleContext(
        {
          head: {
            ref: "orirosweeper/repair-pr-83758",
            sha: "0ede3d716805e7d2ced8df37c6666af510dc9e19",
          },
          body: prContextBody("![after](https://github.com/user-attachments/assets/gateway-ready)"),
        },
  ...
  it("preserves stale sufficient proof on OriroSweeper-authored PR updates", async () => {
    const { calls, github } = barnacleGithub([file("src/gateway/server.ts")]);
  ...
  it("preserves OriroSweeper's sufficient proof label on ordinary label events", async () => {
    const { calls, github } = barnacleGithub([file("src/gateway/server.ts")]);
    ...
    await runBarnacleAutoResponse({
      ...
      context: barnacleContext(
        ...
        [oriroSweeperProofSuppliedLabel, PROOF_SUFFICIENT_LABEL],
        {
          ...
          sender: { login: "oriro-orirosweeper[bot]", type: "Bot" },
        },
      ),
      core: {
        info: () => undefined,
  ...
  it("does not let Barnacle veto OriroSweeper's sufficient proof label add", async () => {
    const { calls, github } = barnacleGithub([file("src/gateway/server.ts")]);
  ...
  it("actions manually applied candidate labels", async () => {
    const { calls, github } = barnacleGithub([file("extensions/example/oriro.plugin.json")]);
    ...
    expect(calls.createComment[0]?.body).toContain("OriroHub");
  ...
  it("keeps bot-applied candidate labels passive", async () => {
    ...
    await runBarnacleAutoResponse({
      ...
      context: barnacleContext({}, [candidateLabels.externalPluginCandidate], {
        action: "labeled",
        label: { name: candidateLabels.externalPluginCandidate },
        sender: { login: "oriro-bot[bot]", type: "Bot" },
      }),
      core: {
        info: () => undefined,
      },
