...
type GatewayConnectionDetailResolvers = {
  ...
  resolveGatewayPort?: (cfg?: OriroConfig, env?: NodeJS.ProcessEnv) => number;
};
...
export function buildGatewayConnectionDetailsWithResolvers(
  ...
  const envUrlOverride =
    cliUrlOverride || options.ignoreEnvUrlOverride
      ? undefined
      : normalizeOptionalString(process.env.ORIRO_GATEWAY_URL);
  ...
  const allowPrivateWs = process.env.ORIRO_ALLOW_INSECURE_PRIVATE_WS === "1";
