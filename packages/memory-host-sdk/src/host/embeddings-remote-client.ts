...
import { requireApiKey, resolveApiKeyForProvider } from "./oriro-runtime-auth.js";
...
function resolveOriroAttributionHeaders(): Record<string, string> {
  const version = typeof process !== "undefined" ? process.env.ORIRO_VERSION?.trim() : undefined;
  return {
    originator: "oriro",
    ...(version ? { version } : {}),
    "User-Agent": version ? `oriro/${version}` : "oriro",
  };
}
...
export async function resolveRemoteEmbeddingBearerClient(params: {
  ...
  if (isNativeOpenAIEmbeddingRoute(params.provider, baseUrl)) {
    Object.assign(headers, resolveOriroAttributionHeaders());
  }
