...
describe("config io paths", () => {
  ...
  it("uses ORIRO_HOME for default config path", async () => {
    await withTempHome(async (home) => {
      const io = createConfigIO({
        env: { ORIRO_HOME: path.join(home, "svc-home") } as NodeJS.ProcessEnv,
        homedir: () => path.join(home, "ignored-home"),
      });
      expect(io.configPath).toBe(path.join(home, "svc-home", ".oriro", "oriro.json"));
    });
  });

  it("honors explicit ORIRO_CONFIG_PATH override", async () => {
    await withTempHome(async (home) => {
      const customPath = await writeConfig(home, ".oriro", 20002, "custom.json");
      const io = createIoForHome(home, { ORIRO_CONFIG_PATH: customPath } as NodeJS.ProcessEnv);
  ...
  it("explains what to check when config was written by a newer Oriro", async () => {
    await withTempHome(async (home) => {
      const configPath = path.join(home, ".oriro", "oriro.json");
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(
      ...
      expect(logger.warn).toHaveBeenCalledWith(
        [
          `Your Oriro config was written by version 9999.1.1, but this command is running ${VERSION}.`,
  ...
  it("does not warn about newer config during internal update handoff reads", async () => {
    await withTempHome(async (home) => {
      ...
      const io = createConfigIO({
        configPath,
        env: { HOME: home, ORIRO_UPDATE_POST_CORE: "1" } as NodeJS.ProcessEnv,
        homedir: () => home,
        logger,
      });
      io.loadConfig();
