...
metadata:
  oriro:
    homepage: https://github.com/oriro/discrawl
    requires:
      bins:
        - discrawl
    install:
      - kind: go
        module: github.com/oriro/discrawl/cmd/discrawl@latest
        bins:
          - discrawl
---
...
- Repo: `oriro/discrawl`; use `~/GIT/_Perso/discrawl` only after verifying
  its remote targets `oriro/discrawl`, otherwise use a fresh checkout
...
`oriro/discrawl` checkout before concluding the feature is missing.
...
## OriroSweeper Sandbox

Use the sandbox reader only:

```bash
discrawl-sandbox search --limit 20 "query"
discrawl-sandbox messages --channel orirotributors --days 7 --all
discrawl-sandbox status --json
```

This reader imports `https://github.com/oriro/discord-store.git` into
`/root/orirosweeper-sandbox-workspace/.discrawl/discrawl.db` with
