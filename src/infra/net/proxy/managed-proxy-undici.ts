// Bridges Oriro-managed proxy TLS trust into Undici EnvHttpProxyAgent and
// explicit ProxyAgent options without changing unrelated operator proxies.
import { isRecord as isProxyTlsRecord } from "@oriro/normalization-core/record-coerce";
...
/** Resolves managed proxy TLS trust only when the target proxy is Oriro's active proxy. */
export function resolveActiveManagedProxyTlsOptions(
  ...
  const proxyCaFile = resolveManagedProxyCaFileForUrl({
    ...
    caFileOverride: env["ORIRO_PROXY_CA_FILE"],
  });
  try {
