...
import { describeAccountSnapshot } from "oriro/plugin-sdk/account-helpers";
import { formatAllowFromLowercase } from "oriro/plugin-sdk/allow-from";
import {
...
import { createChatChannelPlugin } from "oriro/plugin-sdk/channel-core";
import {
...
import { createPairingPrefixStripper } from "oriro/plugin-sdk/channel-pairing";
import {
  createAllowlistProviderGroupPolicyWarningCollector,
  projectConfigAccountIdWarningCollector,
} from "oriro/plugin-sdk/channel-policy";
import { getSessionBindingService } from "oriro/plugin-sdk/conversation-runtime";
import {
  createChannelDirectoryAdapter,
  createRuntimeDirectoryLiveAdapter,
} from "oriro/plugin-sdk/directory-runtime";
import { normalizeMessagePresentation } from "oriro/plugin-sdk/interactive-runtime";
import { createLazyRuntimeNamedExport } from "oriro/plugin-sdk/lazy-runtime";
import { parseStrictPositiveInteger } from "oriro/plugin-sdk/number-runtime";
import { createComputedAccountStatusAdapter } from "oriro/plugin-sdk/status-helpers";
...
function areAnyFeishuReactionActionsEnabled(cfg: OriroConfig): boolean {
...
export const feishuPlugin: ChannelPlugin<ResolvedFeishuAccount, FeishuProbeResult> =
  createChatChannelPlugin({
    base: {
      ...
      auth: {
        login: async ({ cfg }) => {
          const { createClackPrompter } = await import("oriro/plugin-sdk/setup-runtime");
          const { replaceConfigFile } = await import("oriro/plugin-sdk/config-mutation");
