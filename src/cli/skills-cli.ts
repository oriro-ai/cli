...
>;
type ResolvedOriroHubSkillVerificationTarget = Extract<
  Awaited<ReturnType<typeof resolveOriroHubSkillVerificationTarget>>,
  { ok: true }
>;
...
function resolveOriroHubTargetWorkspaceDir(
  command: Command | undefined,
...
function shouldFailSkillVerification(result: OriroHubSkillVerificationResponse): boolean {
...
function buildSkillVerificationOutput(
  result: OriroHubSkillVerificationResponse,
  target: ResolvedOriroHubSkillVerificationTarget,
): Record<string, unknown> {
  const verifiedSourceUrl = readVerifiedOriroHubSkillSourceUrl(result.provenance);
  return {
    ...result,
    oriro: {
      resolution: {
...
function readVerifiedSkillCardUrl(
  ...
  if (!result.card || typeof result.card !== "object" || Array.isArray(result.card)) {
    return { ok: false, error: "OriroHub verification response did not include a Skill Card URL." };
  }
...
export function registerSkillsCli(program: Command) {
  const skills = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/skills", "docs.oriro.ai/cli/skills")}\n`,
    );

  skills
    .command("search")
    .description("Search OriroHub skills")
    ...
    .action(async (queryParts: string[], opts: { limit?: number; json?: boolean }) => {
      try {
        const results = await searchSkillsFromOriroHub({
        ...
        if (results.length === 0) {
          defaultRuntime.log("No OriroHub skills found.");
          return;
        }
  ...
  skills
    .command("install")
    .description("Install a skill from OriroHub, git, or a local directory")
    .argument(
      "<skill-ref>",
      "OriroHub skill ref (@owner/slug), git:<repo>, or local skill directory",
    )
    .option("--version <version>", "Install a specific version")
    .option("--force", "Overwrite an existing workspace skill", false)
    .option(
      "--force-install",
      "Install a pending GitHub-backed skill before OriroHub scan completes",
      false,
    )
    ...
    .addHelpText("after", "\nExamples:\n  oriro skills install @owner/weather\n")
    .action(
      async (
        slug: string,
        opts: {
        ...
        try {
          ...
          if (isSkillSourceInstallSpec(slug)) {
            if (opts.version) {
              defaultRuntime.error("--version is only supported for OriroHub skill installs.");
              defaultRuntime.exit(1);
              return;
            }
  ...
  skills
    .command("update")
    .description("Update OriroHub-installed skills in the active or shared managed directory")
    .argument("[slug]", "Single skill slug")
    .option("--all", "Update all tracked OriroHub skills", false)
    .option(
      "--force-install",
      "Install a pending GitHub-backed skill before OriroHub scan completes",
      false,
    ...
    .action(
      async (
        ...
        try {
          ...
          const tracked = await readTrackedOriroHubSkillSlugs(workspaceDir);
          if (opts.all && tracked.length === 0) {
            defaultRuntime.log("No tracked OriroHub skills to update.");
            return;
          }
  ...
  skills
    .command("verify")
    .description("Verify a OriroHub skill with OriroHub")
    .argument("<slug>", "OriroHub skill slug")
    ...
    .action(
      async (
        ...
        try {
          ...
          const target = await resolveOriroHubSkillVerificationTarget({
            workspaceDir,
            slug,
            version: opts.version,
            tag: opts.tag,
          ...
          if (!target.ok) {
            ...
            if (opts.card) {
              ...
              if (!cardUrl.ok) {
                ...
                const card = await fetchOriroHubSkillCard({
                  url: cardUrl.url,
                  baseUrl: target.baseUrl,
                });
