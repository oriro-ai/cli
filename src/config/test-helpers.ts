...
import { withTempHome as withTempHomeBase } from "oriro/plugin-sdk/test-env";
import { resetPluginLoaderTestStateForTest } from "../plugins/loader.test-fixtures.js";
import { clearPluginSetupRegistryCache } from "../plugins/setup-registry.js";
import { resetConfigRuntimeState, type OriroConfig } from "./config.js";
  ...
  try {
    return await withTempHomeBase(fn, {
      ...
      env: {
        ...
        ORIRO_MPM_CATALOG_PATHS: undefined,
...
export async function writeOriroConfig(home: string, config: unknown): Promise<string> {
...
export async function writeStateDirDotEnv(
  ...
  const stateDir = params?.stateDir ?? params?.env?.ORIRO_STATE_DIR?.trim();
  if (!stateDir) {
    throw new Error("Expected ORIRO_STATE_DIR or explicit stateDir for .env test setup");
  }
...
export async function withTempHomeConfig<T>(
  ...
  return withTempHome(async (home) => {
    const configPath = await writeOriroConfig(home, config);
