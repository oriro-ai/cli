---
summary: "Host Oriro on Upstash Box with keep-alive and SSH tunnel access"
read_when:
  - Deploying Oriro to Upstash Box
  - You want a managed Linux environment for Oriro with SSH-tunneled dashboard access
title: "Upstash Box"
---

Run a persistent Oriro Gateway on Upstash Box, a managed Linux environment
...
Upstash maintains its current Oriro Box walkthrough at
[Oriro Setup](https://upstash.com/docs/box/guides/oriro-setup).

## Connect with an SSH tunnel

Forward the Oriro dashboard port to your local machine. Use your Box API key
...
```bash
oriro config set gateway.bind lan
nohup oriro gateway > gateway.log 2>&1 &
```
...
- [Updating Oriro](/install/updating)
