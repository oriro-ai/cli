...
import { AGENT_MODEL_CONFIG_KEYS } from "@oriro/model-catalog-core/configured-model-refs";
import { normalizeProviderId } from "@oriro/model-catalog-core/provider-id";
import { asOptionalRecord as asMutableRecord } from "@oriro/normalization-core/record-coerce";
import { normalizeOptionalLowercaseString as normalizeString } from "@oriro/normalization-core/string-coerce";
...
const LOSSLESS_CONTEXT_ENGINE_ID = "lossless-oriro";
...
function resolveImplicitDefaultAgentModelRef(cfg: OriroConfig): string {
...
function collectConfigModelRefs(cfg: OriroConfig): CodexRouteHit[] {
...
function codexPluginAllowlistIsRestrictive(cfg: OriroConfig): boolean {
  const allow = cfg.plugins?.allow;
  return Array.isArray(allow) && allow.length > 0 && !pluginIdListIncludes(allow, "codex");
}

function isCodexPluginUnavailableByConfig(cfg: OriroConfig): boolean {
...
function codexPluginIsBlockedOutsideEntry(cfg: OriroConfig): boolean {
...
function collectDisabledCodexPluginRouteHits(cfg: OriroConfig): DisabledCodexPluginRouteHit[] {
...
}): { cfg: OriroConfig; changes: string[] } {
...
function clearConfigLegacyAgentRuntimePolicies(cfg: OriroConfig): string[] {
  const changes: string[] = [];
...
function configRepairWouldClearLegacyRuntimePins(params: { cfg: OriroConfig }): boolean {
...
function rewriteConfigModelRefs(params: { cfg: OriroConfig }): ConfigRouteRepairResult {
...
function formatLegacyLosslessCompactionWarning(params: {
  ...
  return [
    ...
    ...configLines,
    params.canAutoFix
      ? "- Run `oriro doctor --fix`: it migrates legacy Lossless compaction config to the Lossless context-engine slot."
...
function collectCodexAppServerCommandWarnings(cfg: OriroConfig): string[] {
  const plugins = asMutableRecord(cfg.plugins);
  const entries = asMutableRecord(plugins?.entries);
...
}): { cfg: OriroConfig; warnings: string[]; changes: string[] } {
  const hits = collectConfigModelRefs(params.cfg);
  const disabledCodexPluginHits = collectDisabledCodexPluginRouteHits(params.cfg);
  const ignoreLegacyAgentRuntimePins = configRepairWouldClearLegacyRuntimePins(params);
  const unsupportedCompactionOverrides = collectUnsupportedCodexCompactionOverrides({
...
function clearStaleSessionRuntimePins(entry: SessionEntry): boolean {
  ...
  let changed = false;
  if (entry.agentHarnessId !== undefined && harnessRuntime !== "oriro") {
    delete entry.agentHarnessId;
    changed = true;
  }
  if (entry.agentRuntimeOverride !== undefined && overrideRuntime !== "oriro") {
        ...
        stale.length > 0
          ? [
              [
                ...
                "- Run `oriro doctor --fix` to rewrite stale session model/provider pins across all agent session stores.",
              ].join("\n"),
            ]
          : [],
      changes: [],
