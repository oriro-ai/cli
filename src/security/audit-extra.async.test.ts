...
`,
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-security-audit-async-"));
  ...
  it("ignores install backup and debris dirs when scanning installed plugin roots", async () => {
    ...
    try {
      ...
      expect(findings.map((f) => f.title).join("\n")).not.toContain(".oriro-install-backups");
    } finally {
      scanSpy.mockRestore();
    }
  });
  ...
  it("audits canonical auth profile SQLite store permissions", async () => {
    ...
    const databasePath = path.join(agentDir, "oriro-agent.sqlite");
    ...
    expect(readableAuthTargets).toEqual(
      expect.arrayContaining([
        expect.stringContaining("oriro-agent.sqlite"),
        expect.stringContaining("oriro-agent.sqlite-wal"),
        expect.stringContaining("oriro-agent.sqlite-shm"),
        expect.stringContaining("oriro-agent.sqlite-journal"),
      ]),
    );
  });
});
