...
import type { MemoryEmbeddingProbeResult } from "oriro/plugin-sdk/memory-core-host-engine-storage";
import {
  resolveMemoryDreamingConfig,
  resolveMemoryRemDreamingConfig,
} from "oriro/plugin-sdk/memory-core-host-status";
import { buildAgentSessionKey } from "oriro/plugin-sdk/routing";
...
function formatMemoryIndexIdentityWarning(
  ...
  return {
    reason,
    fix: `Run: oriro memory status --index --agent ${agentId}`,
  };
}
...
function resolveMemoryPluginConfig(cfg: OriroConfig): Record<string, unknown> {
...
async function createHistoricalRemHarnessWorkspace(params: {
  ...
  const workspaceDir = await fs.mkdtemp(
    path.join(resolvePreferredOriroTmpDir(), "oriro-rem-harness-"),
  );
...
function formatDreamingSummary(cfg: OriroConfig): string {
...
function resolveAgent(cfg: OriroConfig, agent?: string) {
...
function resolveAgentIds(cfg: OriroConfig, agent?: string): string[] {
  const trimmed = agent?.trim();
  if (trimmed) {
    return [trimmed];
  }
...
export async function runMemoryStatus(opts: MemoryCommandOptions) {
  ...
  for (const result of allResults) {
    ...
    if (audit?.issues.length) {
      ...
      if (!opts.fix) {
        lines.push(`  ${muted(`Fix: oriro memory status --fix --agent ${agentId}`)}`);
      }
    }
  ...
  await withMemoryManagerForAgent({
    ...
    run: async (manager) => {
      ...
      const scratchDir = await fs.mkdtemp(
        path.join(resolvePreferredOriroTmpDir(), "oriro-rem-backfill-"),
      );
      try {
