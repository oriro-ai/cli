...
function auditGatewayToken(
  ...
  issues.push({
    ...
    message: "Gateway service embeds ORIRO_GATEWAY_TOKEN and should be reinstalled.",
    detail: "Run `oriro gateway install --force` to remove embedded service token.",
    level: "recommended",
  });
  ...
  issues.push({
    ...
    message:
      "Gateway service ORIRO_GATEWAY_TOKEN does not match gateway.auth.token in oriro.json",
...
export function readEmbeddedGatewayToken(command: GatewayServiceCommand): string | undefined {
  ...
  if (isEnvironmentFileOnlySource(command.environmentValueSources?.ORIRO_GATEWAY_TOKEN)) {
    return undefined;
  }
  return normalizeOptionalString(command.environment?.ORIRO_GATEWAY_TOKEN);
}
...
function auditGatewayServiceVersion(command: GatewayServiceCommand, issues: ServiceConfigIssue[]) {
  const serviceVersion = command?.environment?.ORIRO_SERVICE_VERSION?.trim();
