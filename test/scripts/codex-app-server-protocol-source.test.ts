...
const originalOriroCodexRepo = process.env.ORIRO_CODEX_REPO;

afterEach(() => {
  if (originalOriroCodexRepo === undefined) {
    delete process.env.ORIRO_CODEX_REPO;
  } else {
    process.env.ORIRO_CODEX_REPO = originalOriroCodexRepo;
  }
});
...
describe("codex app-server protocol source resolver", () => {
  ...
  it("allows an explicit local disk headroom override", () => {
    expect(resolveCodexProtocolMinFreeBytes({ ORIRO_CODEX_PROTOCOL_MIN_FREE_BYTES: "0" })).toBe(
      0,
    );
    expect(() =>
      validateCodexProtocolGenerationHeadroom({
  ...
  it("rejects malformed local disk headroom overrides", () => {
    expect(() =>
      resolveCodexProtocolMinFreeBytes({ ORIRO_CODEX_PROTOCOL_MIN_FREE_BYTES: "nope" }),
  ...
  it("uses ORIRO_CODEX_REPO when provided", async () => {
    const root = createTempDir("oriro-protocol-source-root-");
    const codexRepo = createTempDir("oriro-protocol-source-codex-");
    createProtocolSchema(codexRepo);
    process.env.ORIRO_CODEX_REPO = codexRepo;
  ...
  it("finds the primary checkout sibling from a git worktree", async () => {
    const parentDir = createTempDir("oriro-protocol-source-parent-");
    const primaryOriro = path.join(parentDir, "oriro");
    const codexRepo = path.join(parentDir, "codex");
    const worktreeRoot = createTempDir("oriro-protocol-source-worktree-");
    fs.mkdirSync(path.join(primaryOriro, ".git", "worktrees", "codex-harness"), {
      recursive: true,
    });
    fs.mkdirSync(worktreeRoot, { recursive: true });
    fs.writeFileSync(
      path.join(worktreeRoot, ".git"),
      `gitdir: ${path.join(primaryOriro, ".git", "worktrees", "codex-harness")}\n`,
    );
    createProtocolSchema(codexRepo);
    delete process.env.ORIRO_CODEX_REPO;
