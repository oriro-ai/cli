...
export const ORIRO_MODEL_ID = "oriro";
/** Default OpenAI-compatible model alias that targets the default Oriro agent. */
export const ORIRO_DEFAULT_MODEL_ID = "oriro/default";
...
export class GatewaySessionKeyOverrideError extends Error {
  constructor() {
    super("`x-oriro-session-key` cannot use reserved internal session namespaces.");
...
function resolveAgentIdFromHeader(req: IncomingMessage): string | undefined {
  const raw =
    normalizeOptionalString(getHeader(req, "x-oriro-agent-id")) ||
    normalizeOptionalString(getHeader(req, "x-oriro-agent")) ||
    "";
  if (!raw) {
    return undefined;
  }
...
export function resolveAgentIdFromModel(
  ...
  if (lowered === ORIRO_MODEL_ID || lowered === ORIRO_DEFAULT_MODEL_ID) {
    return resolveDefaultAgentId(cfg);
  }

  const m =
    raw.match(/^oriro[:/](?<agentId>[a-z0-9][a-z0-9_-]{0,63})$/i) ??
...
/** Validates and resolves the `x-oriro-model` override for OpenAI-compatible requests. */
export async function resolveOpenAiCompatModelOverride(params: {
  ...
  if (requestModel && !resolveAgentIdFromModel(requestModel)) {
    return {
      errorMessage: "Invalid `model`. Use `oriro` or `oriro/<agentId>`.",
    };
  }

  const raw = getHeader(params.req, "x-oriro-model")?.trim();
  if (!raw) {
    return {};
  }
  ...
  if (!parsed) {
    return { errorMessage: "Invalid `x-oriro-model`." };
  }
...
function resolveSessionKey(params: {
  ...
  const explicit = getHeader(params.req, "x-oriro-session-key")?.trim();
  if (explicit) {
...
export function resolveGatewayRequestContext(params: {
  ...
  const messageChannel = params.useMessageChannelHeader
    ? (normalizeMessageChannel(getHeader(params.req, "x-oriro-message-channel")) ??
