...
`@oriro/llama-cpp-provider` and set `provider: "local"`. Source checkouts
...
Oriro runs two retrieval paths in parallel and merges the results:
...
**No results?** Run `oriro memory status` to check the index. If empty, run
