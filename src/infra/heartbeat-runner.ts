...
function resolveHeartbeatTimeoutOverrideSeconds(cfg: OriroConfig, heartbeat?: HeartbeatConfig) {
...
export type HeartbeatRunner = {
  stop: () => void;
  updateConfig: (cfg: OriroConfig) => void;
};
...
function hasExplicitHeartbeatAgents(cfg: OriroConfig) {
...
function resolveHeartbeatAgents(cfg: OriroConfig): HeartbeatAgent[] {
  const list = cfg.agents?.list ?? [];
...
function resolveHeartbeatPromptRaw(cfg: OriroConfig, heartbeat?: HeartbeatConfig) {
  return heartbeat?.prompt ?? cfg.agents?.defaults?.heartbeat?.prompt;
}

export function resolveHeartbeatPrompt(cfg: OriroConfig, heartbeat?: HeartbeatConfig) {
  return resolveHeartbeatPromptText(resolveHeartbeatPromptRaw(cfg, heartbeat));
}

function resolveHeartbeatResponseToolPrompt(cfg: OriroConfig, heartbeat?: HeartbeatConfig) {
...
function resolveHeartbeatAckMaxChars(cfg: OriroConfig, heartbeat?: HeartbeatConfig) {
  return Math.max(
    0,
...
function isHeartbeatTypingEnabled(params: { cfg: OriroConfig; hasChatDelivery: boolean }) {
...
function resolveHeartbeatTypingIntervalSeconds(cfg: OriroConfig) {
...
export function startHeartbeatRunner(opts: {
  ...
  const updateConfig = (cfg: OriroConfig) => {
    if (state.stopped) {
      return;
    }
    const now = Date.now();
