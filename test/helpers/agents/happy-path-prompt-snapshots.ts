...
import { createOriroCodingTools } from "../../../src/plugin-sdk/agent-harness.js";
...
const WORKSPACE_DIR = "/tmp/oriro-happy-path/workspace";
const AGENT_DIR = "/tmp/oriro-happy-path/agent";
const SESSION_FILE = "/tmp/oriro-happy-path/session.jsonl";
...
const dynamicToolsConfig: OriroConfig = {
  ...baseConfig,
  plugins: {
    enabled: true,
    slots: {
  ...
  return [
    {
      ...
      notes: [
        ...
        "A quiet turn is represented by not calling `message(action=send)`; the normal final assistant text is private to Oriro/Codex.",
      ],
      trigger: "user",
      ctx: telegramDirectCtx,
      prompt: createPrompt(
...
function prependCodexOriroRuntimeContext(prompt: string): string {
  return [buildCodexOriroRuntimeContext(), "", "Current user request:", prompt].join("\n");
}
...
function renderScenarioSnapshot(
  ...
  const codexTurnPromptText = prependCodexOriroRuntimeContext(scenario.prompt);
