...
const DEFAULT_TRIGGERS = ["oriro", "claude", "computer"];
const VOICEWAKE_CONFIG_KEY = "default";

type VoiceWakeDatabase = Pick<OriroStateKyselyDatabase, "voicewake_triggers">;
...
export async function setVoiceWakeTriggers(
  ...
  runOriroStateWriteTransaction(
    ...
    baseDir ? { env: { ...process.env, ORIRO_STATE_DIR: baseDir } } : {},
  );
  return {
    triggers: sanitized,
    updatedAtMs,
