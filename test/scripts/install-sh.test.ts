...
function runInstallShell(script: string, env: NodeJS.ProcessEnv = {}) {
  const home = mkdtempSync(join(tmpdir(), "oriro-install-home-"));
  try {
...
describe("install.sh", () => {
  ...
  it("runs installer snippets without inherited shell startup files", () => {
    const tmp = mkdtempSync(join(tmpdir(), "oriro-install-shell-env-"));
    const bashEnvPath = join(tmp, "bash_env");
    writeFileSync(bashEnvPath, "export ORIRO_BASH_ENV_LEAKED=1\n");

    try {
      const result = runInstallShell('printf "leaked=%s\\n" "${ORIRO_BASH_ENV_LEAKED:-0}"', {
  ...
  it("installs Git with apk on Alpine", () => {
    const tmp = mkdtempSync(join(tmpdir(), "oriro-install-git-apk-"));
  ...
  it("does not select apk Git on non-Alpine hosts", () => {
    const tmp = mkdtempSync(join(tmpdir(), "oriro-install-git-native-"));
    const bin = join(tmp, "bin");
    const apkLog = join(tmp, "apk-args.txt");
    mkdirSync(bin, { recursive: true });
    const fakeApk = join(bin, "apk");
  ...
  it("does not emit --before when raw user npmrc config contains min-release-age", () => {
    const tmp = mkdtempSync(join(tmpdir(), "oriro-install-npmrc-"));
    const bin = join(tmp, "bin");
    ...
    try {
      const result = runInstallShell(
        [
          ...
          `run_npm_global_install oriro@latest ${JSON.stringify(join(tmp, "install.log"))}`,
  ...
  it("does not emit --before when default global npmrc config contains min-release-age", () => {
    const tmp = mkdtempSync(join(tmpdir(), "oriro-install-global-npmrc-"));
    const bin = join(tmp, "bin");
    const home = join(tmp, "home");
  ...
  it("does not emit --before when builtin npmrc config contains min-release-age", () => {
    const tmp = mkdtempSync(join(tmpdir(), "oriro-install-builtin-npmrc-"));
    const bin = join(tmp, "bin");
    const home = join(tmp, "home");
  ...
  it("uses ORIRO_HOME for git and onboarding defaults", () => {
    const tmp = mkdtempSync(join(tmpdir(), "oriro-install-home-"));
    const osHome = join(tmp, "os-home");
    const oriroHome = join(tmp, "oriro-home");
    mkdirSync(osHome, { recursive: true });
    mkdirSync(oriroHome, { recursive: true });
    ...
    expect(output).toContain(`git=${join(oriroHome, "oriro")}`);
    expect(output).toContain(`workspace=${join(oriroHome, ".oriro", "workspace")}`);
    expect(output).toContain(
      `workspaceProfile=${join(oriroHome, ".oriro", "workspace-work")}`,
    );
  ...
  it("skips bootstrap onboarding when legacy HOME config exists with ORIRO_HOME", () => {
    const tmp = mkdtempSync(join(tmpdir(), "oriro-install-legacy-config-"));
    const osHome = join(tmp, "os-home");
    const oriroHome = join(tmp, "oriro-home");
    const legacyConfigDir = join(osHome, ".oriro");
    const bootstrapDir = join(oriroHome, ".oriro", "workspace");
    mkdirSync(legacyConfigDir, { recursive: true });
    mkdirSync(bootstrapDir, { recursive: true });
    writeFileSync(join(legacyConfigDir, "oriro.json"), "{}\n");
  ...
  it("rejects Oriro GitHub source targets for npm installs", () => {
    const result = runInstallShell(`
      set -euo pipefail
      source "${SCRIPT_PATH}"
      set +e
    ...
    expect(result.stdout).toContain("npm installs do not support Oriro GitHub source targets");
  ...
  it("does not emit before args when npmrc min-release-age computes a before cutoff", () => {
    const tmp = mkdtempSync(join(tmpdir(), "oriro-install-npm-freshness-"));
    const bin = join(tmp, "bin");
    const home = join(tmp, "home");
  ...
  it("ignores project npmrc when choosing global install freshness args", () => {
    const tmp = mkdtempSync(join(tmpdir(), "oriro-install-global-freshness-"));
    const bin = join(tmp, "bin");
    const home = join(tmp, "home");
  ...
  it("bounds installer npm prefix probes during finalization helpers", () => {
    const tmp = mkdtempSync(join(tmpdir(), "oriro-install-npm-probe-"));
    ...
    try {
      const result = runInstallShell(
        ...
        {
          ORIRO_INSTALL_PROBE_TIMEOUT_SECONDS: "1",
          PATH: `${tmp}:${process.env.PATH ?? ""}`,
        },
      );

      expect(result.status).toBe(0);
      expect(result.stdout.trim()).toBe("/tmp/oriro-npm/bin");
  ...
  it("bounds daemon status probes during finalization helpers", () => {
    const tmp = mkdtempSync(join(tmpdir(), "oriro-install-probe-"));
    const oriro = join(tmp, "oriro");
    writeFileSync(
      oriro,
      [
        "#!/usr/bin/env bash",
    ...
    chmodSync(oriro, 0o755);
    try {
      const result = runInstallShell(
        [
          `source ${JSON.stringify(SCRIPT_PATH)}`,
          `if is_gateway_daemon_loaded ${JSON.stringify(oriro)}; then`,
          '  printf "loaded\\n"',
          "else",
          '  printf "not-loaded\\n"',
          "fi",
        ].join("\n"),
        { ORIRO_INSTALL_PROBE_TIMEOUT_SECONDS: "0.1" },
      );

      expect(result.status).toBe(0);
      expect(result.stdout.trim()).toBe("not-loaded");
      expect(result.stderr).toContain(
        "timed out during installer finalization probe: oriro daemon status --json",
      );
    } finally {
      rmSync(tmp, { force: true, recursive: true });
    }
  ...
  it("loads nvm before checking Node.js so stale system Node does not win", () => {
    ...
    const tmp = mkdtempSync(join(tmpdir(), "oriro-install-nvm-"));
    const home = join(tmp, "home");
  ...
  it("promotes a supported Linux Node binary over stale PATH entries", () => {
    const tmp = mkdtempSync(join(tmpdir(), "oriro-install-node-promote-"));
  ...
  it("uses the package engine floor when accepting existing Node runtimes", () => {
    ...
    const tmp = mkdtempSync(join(tmpdir(), "oriro-install-node-floor-"));
    const bin = join(tmp, "bin");
    mkdirSync(bin, { recursive: true });
  ...
  it("persists a supported Linux Node path before noninteractive shell guards", () => {
    const tmp = mkdtempSync(join(tmpdir(), "oriro-install-linux-node-path-"));
    const home = join(tmp, "home");
  ...
  it("warns before redirecting an unwritable npm prefix", () => {
    const tmp = mkdtempSync(join(tmpdir(), "oriro-install-npm-prefix-"));
    const home = join(tmp, "home");
    ...
    expect(result?.stdout).toContain("npm i -g oriro@latest");
  ...
  it("persists npm prefix PATH before noninteractive shell guards", () => {
    const tmp = mkdtempSync(join(tmpdir(), "oriro-install-npm-prefix-shell-"));
    const home = join(tmp, "home");
    mkdirSync(home, { recursive: true });
    writeFileSync(
      join(home, ".bashrc"),
  ...
  it("uses a quoted absolute oriro path in follow-up commands when npm bin is not on the original PATH", () => {
    const tmp = mkdtempSync(join(tmpdir(), "oriro-install-command-"));
    const npmBin = join(tmp, "npm bin");
    const visibleBin = join(tmp, "visible-bin");
    mkdirSync(npmBin, { recursive: true });
    mkdirSync(visibleBin, { recursive: true });
    const oriroBin = join(npmBin, "oriro");
    writeFileSync(oriroBin, "#!/bin/sh\nexit 0\n");
    chmodSync(oriroBin, 0o755);
    ...
    try {
      result = runInstallShell(`
        ...
        printf 'missing=%s\\n' "$(oriro_command_for_user "${oriroBin}")"
    ...
    expect(result?.status).toBe(0);
    expect(result?.stdout).toContain(`missing=${oriroBin.replace(/ /g, "\\ ")}`);
    expect(result?.stdout).toContain("present=oriro");
  });
  ...
  it("resolves requested git install versions to checkout refs", () => {
    const result = runInstallShell(`
      ...
      source "${SCRIPT_PATH}"
      npm() {
        if [[ "$1" == "view" && "$2" == "oriro" && "$3" == "dist-tags.beta" ]]; then
          printf '2026.5.12-beta.3\\n'
          return 0
        fi
        return 1
      }
      ORIRO_VERSION=v2026.5.12-beta.3
      printf 'tag=%s\\n' "$(resolve_git_oriro_ref)"
      ORIRO_VERSION=2026.5.12-beta.3
      printf 'semver=%s\\n' "$(resolve_git_oriro_ref)"
      ORIRO_VERSION=beta
      printf 'beta=%s\\n' "$(resolve_git_oriro_ref)"
      ORIRO_VERSION=main
  ...
  it("uses the repo Corepack pnpm when a global pnpm version is already present", () => {
    const tmp = mkdtempSync(join(tmpdir(), "oriro-install-pnpm-version-"));
    const bin = join(tmp, "bin");
...
describe("install.sh macOS Homebrew Node behavior", () => {
  ...
  it("reruns spinner-wrapped commands when gum reports ioctl failure", () => {
    const dir = mkdtempSync(join(tmpdir(), "oriro-install-sh-gum-"));
    try {
...
describe("install.sh duplicate Oriro install detection", () => {
  it("warns with concrete package paths and versions for duplicate npm roots", () => {
    const result = runInstallShell(`
      ...
      mkdir -p "$root/brew/oriro" "$root/fnm/oriro"
      printf '{"version":"2026.3.7"}\\n' > "$root/brew/oriro/package.json"
      printf '{"version":"2026.3.1"}\\n' > "$root/fnm/oriro/package.json"
      collect_oriro_npm_root_candidates() { printf '%s\\n' "$root/brew" "$root/fnm"; }
      ORIRO_BIN="$root/fnm/.bin/oriro"
      ui_warn() { echo "WARN: $*"; }
      warn_duplicate_oriro_global_installs
    `);

    expect(result.status).toBe(0);
    expect(result.stdout).toContain("Multiple Oriro global installs detected");
    expect(result.stdout).toContain("2026.3.7");
    expect(result.stdout).toContain("2026.3.1");
    expect(result.stdout).toContain("/brew/oriro");
    expect(result.stdout).toContain("/fnm/oriro");
    expect(result.stdout).toContain("Active oriro:");
    expect(result.stdout).toContain("npm uninstall -g oriro");
  });

  it("stays quiet when only one Oriro npm root exists", () => {
    const result = runInstallShell(`
      set -euo pipefail
      source "${SCRIPT_PATH}"
      root="$(mktemp -d)"
      trap 'rm -rf "$root"' EXIT
      mkdir -p "$root/only/oriro"
      printf '{"version":"2026.3.7"}\\n' > "$root/only/oriro/package.json"
      collect_oriro_npm_root_candidates() { printf '%s\\n' "$root/only"; }
      ui_warn() { echo "WARN: $*"; }
      warn_duplicate_oriro_global_installs
    `);
    ...
    expect(result.status).toBe(0);
    expect(result.stdout).not.toContain("Multiple Oriro global installs detected");
  });
});
