...
Plugin hooks are in-process extension points for Oriro plugins. Use them
...
current plugin options; Oriro injects it per handler without mutating the
...
- `subagent_spawned` includes `resolvedModel` and `resolvedProvider` when Oriro has resolved the child session's native model before launch.
...
use `oriro sessions` or the Gateway session/status surfaces. When debugging
...
- Oriro strips `toolResult.details` before provider replay and compaction
...
When a run is blocked, Oriro stores only the replacement text in
...
`before_agent_start` and `agent_end` include `event.runId` when Oriro can
...
identifier when Oriro can derive one from the session key or delivery
metadata.
...
forever. A timeout is logged and Oriro continues; it does not cancel
...
`durationMs`/`outcome`, and `upstreamRequestIdHash` when Oriro can derive a
...
Codex native `Stop` hooks are relayed into this hook as Oriro
...
reach the next model turn exactly once. Oriro drains queued injections before
...
policy runs from Oriro config, covers CLI install and update paths, and fails
closed when enabled but unavailable.

`before_install` is a plugin-runtime lifecycle hook. It runs after
`security.installPolicy` only in the Oriro process where plugin hooks have
...
field remains in the event payload for compatibility, but Oriro no longer
...
context when syncing external wake schedulers, and keep Oriro as the
