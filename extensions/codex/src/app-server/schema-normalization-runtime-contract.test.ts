...
describe("Codex app-server dynamic tool schema boundary contract", () => {
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-codex-schema-contract-"));
  });
  ...
  it("passes prepared executable dynamic tool schemas through legacy thread start specs", async () => {
    ...
    expect(startPayload?.serviceName).toBe("Oriro");
    ...
    expect(startPayload?.developerInstructions).toContain("Oriro");
  });
