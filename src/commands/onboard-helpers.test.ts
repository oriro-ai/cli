...
describe("handleReset", () => {
  it("uses active profile paths for destructive reset targets", async () => {
    const homeDir = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-reset-profile-"));
    const profileStateDir = path.join(homeDir, ".oriro-work");
    const defaultStateDir = path.join(homeDir, ".oriro");
    const profileConfigPath = path.join(profileStateDir, "oriro.json");
      ...
      await withEnvAsync(
        {
          ...
          ORIRO_HOME: homeDir,
          ORIRO_PROFILE: "work",
          ORIRO_STATE_DIR: profileStateDir,
          ORIRO_CONFIG_PATH: profileConfigPath,
        },
...
describe("moveToTrash", () => {
  it("uses fs-safe trash instead of resolving a PATH trash command", async () => {
    const testRoot = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-trash-helper-"));
  ...
  it("allows fs-safe trash to move a symlink whose target resolves outside the parent", async () => {
    const testRoot = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-trash-symlink-"));
    const targetPath = path.join(testRoot, "target-link");
    const outsideTarget = path.join(os.tmpdir(), "oriro-trash-symlink-target");
  ...
  it("canonicalizes a symlinked parent before calling fs-safe trash", async () => {
    const testRoot = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-trash-parent-link-"));
    const lexicalParent = path.join(testRoot, "state-link");
    const realParent = path.join(testRoot, "state-real");
    const targetPath = path.join(lexicalParent, "oriro.json");
    const sourcePath = path.join(realParent, "oriro.json");
