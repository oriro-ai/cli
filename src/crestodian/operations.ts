...
const SETUP_RE =
  /^(?:setup(?!\s+agent\b)|set\s+me\s+up|set\s+up\s+oriro|onboard|onboard\s+me|bootstrap|first\s+run)(?:\b|$)/i;
const PLUGIN_LIST_RE = /^(?:plugins?|orirohub)\s+list$|^list\s+plugins?$/i;
const PLUGIN_SEARCH_RE =
  /^(?:(?:plugins?|orirohub)\s+search|search\s+plugins?(?:\s+for)?)\s+(?<query>.+)$/i;
const PLUGIN_INSTALL_RE =
  /^(?:(?:plugins?)\s+install|install\s+(?:(?<source>npm|orirohub)\s+)?plugins?)\s+(?<spec>\S+)$/i;
...
export function parseCrestodianOperation(input: string): CrestodianOperation {
  ...
  if (!trimmed) {
    return {
      kind: "none",
      message: "Tiny oriro tap: say status, doctor, models, agents, or talk to agent.",
    };
  }
...
function normalizePluginInstallSpec(spec: string, source: string | undefined): string {
  ...
  if (normalizedSource === "orirohub" && !trimmed.toLowerCase().startsWith("orirohub:")) {
    return `orirohub:${trimmed}`;
  }
  return trimmed;
}
...
function validateCrestodianPluginInstallSpec(spec: string): string | null {
  ...
  if (/\s/.test(trimmed)) {
    return "Crestodian plugin install accepts one npm or OriroHub package spec.";
  }
  if (/^(?:\.{1,2}\/|\/|~\/|file:|git(?:\+ssh|\+https)?:|https?:)/i.test(trimmed)) {
    ...
    return "Crestodian plugin install accepts npm or OriroHub package specs only.";
  }
  return null;
}
...
function formatSetupPlanDescription(
  ...
  return `bootstrap ORIRO setup for workspace ${workspace}${model}`;
}
