...
type SlackRouteBinding = NonNullable<OriroConfig["bindings"]>[number];
type SlackRouteBindingPeer = NonNullable<SlackRouteBinding["match"]["peer"]>;

const slackRouteBindingConfigCache = new WeakMap<
  OriroConfig,
  { bindingsRef: OriroConfig["bindings"]; normalizedCfg: OriroConfig }
>();
...
function normalizeSlackRouteBindingConfig(cfg: OriroConfig): OriroConfig {
  ...
  const normalizedCfg = changed
    ? ({ ...cfg, bindings: normalizedBindings } as OriroConfig)
    : cfg;
