...
import { SessionManager } from "oriro/plugin-sdk/agent-sessions";
import {
  onInternalDiagnosticEvent,
  waitForDiagnosticEventsDrained,
  type DiagnosticEventPayload,
} from "oriro/plugin-sdk/diagnostic-runtime";
import { initializeGlobalHookRunner, registerInternalHook } from "oriro/plugin-sdk/hook-runtime";
import { registerMemoryCapability } from "oriro/plugin-sdk/memory-core-host-runtime-core";
import { MESSAGE_TOOL_DELIVERY_HINTS } from "oriro/plugin-sdk/message-tool-delivery-hints";
import { registerPluginCommand } from "oriro/plugin-sdk/plugin-runtime";
import { createMockPluginRegistry } from "oriro/plugin-sdk/plugin-test-runtime";
import { describe, expect, it, vi } from "vitest";
import WebSocket from "ws";
import { CODEX_GPT5_BEHAVIOR_CONTRACT } from "../../prompt-overlay.js";
import { defaultCodexAppInventoryCache } from "./app-inventory-cache.js";
import {
  buildCodexOriroPromptContext,
  buildCodexSystemPromptReport,
  buildCodexWorkspaceBootstrapContext,
  getCodexWorkspaceMemoryToolNames,
  prependCodexOriroPromptContext,
...
import {
  CODEX_ORIRO_DYNAMIC_TOOL_NAMESPACE,
...
async function buildCodexTurnContextForTest(
  ...
  const threadDeveloperInstructions = [
    ...
    .join("\n\n");
  const openOriroPromptContext = buildCodexOriroPromptContext({
    params,
    workspacePromptContext: workspaceBootstrapContext.promptContext,
  });
  const codexTurnPromptText = prependCodexOriroPromptContext(
    params.prompt,
    openOriroPromptContext,
  );
...
describe("runCodexAppServerAttempt", () => {
  ...
  it("starts active Oriro sandbox threads with Codex native execution disabled", async () => {
    testing.setOriroCodingToolsFactoryForTests(() => [
  ...
  it("routes native Codex execution through an Oriro sandbox exec-server when opted in", async () => {
    const appServer = {
    ...
    try {
      ...
      expect(environmentAddParams?.environmentId).toMatch(/^oriro-sandbox-/);
  ...
  it("starts Codex threads without duplicate Oriro workspace tools by default", async () => {
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
  ...
  it("includes Codex app-server scoped plugin command guidance in developer instructions", () => {
    registerPluginCommand("demo-plugin", {
      ...
      agentPromptGuidance: [
        ...
        {
          text: "Oriro main command guidance.",
          surfaces: ["oriro_main"],
        },
      ],
    ...
    expect(instructions).not.toContain("Oriro main command guidance.");
  });

  it("passes Oriro skills as turn collaboration developer instructions", async () => {
    const llmInput = vi.fn();
    initializeGlobalHookRunner(
      createMockPluginRegistry([{ hookName: "llm_input", handler: llmInput }]),
    );
    vi.stubEnv("ORIRO_TRAJECTORY", "1");
    vi.stubEnv("ORIRO_TRAJECTORY_DIR", path.join(tempDir, "trajectory"));
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
    ...
    const collaborationInstructions =
      turnStartParams.collaborationMode?.settings?.developer_instructions ?? "";
    expect(collaborationInstructions).toContain("## Oriro Skills");
    expect(collaborationInstructions).toContain("<available_skills>");
    const inputText = turnStartParams.input?.[0]?.text ?? "";
    expect(inputText).not.toContain("## Oriro Skills");
    ...
    expect(compiledContext?.data?.systemPrompt).toContain("## Oriro Skills");
  ...
  it("emits TUI-compatible tool events for Codex dynamic tool calls", async () => {
    ...
    await expect(
      ...
      success: false,
      contentItems: [{ type: "inputText", text: "Unknown Oriro tool: python" }],
    });
    await harness.completeTurn({ threadId: "thread-1", turnId: "turn-1" });
    await run;
    ...
    expect(onRunAgentEvent).toHaveBeenCalledWith({
      ...
      data: {
        ...
        isError: true,
        result: {
          content: [{ type: "text", text: "Unknown Oriro tool: python" }],
        },
      },
    });
  ...
  it("keeps leading delivery hints out of the Codex current user request", async () => {
    for (const [index, deliveryHint] of MESSAGE_TOOL_DELIVERY_HINTS.entries()) {
      ...
      const inputText = turnStartParams.input?.[0]?.text ?? "";
      expect(inputText).toContain("Oriro delivery metadata:");
  ...
  it("keeps searchable Oriro dynamic tools when code-mode-only is enabled", () => {
    const tools = [
      createRuntimeDynamicTool("message"),
    ...
    expect(webSearch?.namespace).toBe(CODEX_ORIRO_DYNAMIC_TOOL_NAMESPACE);
    expect(webSearch?.deferLoading).toBe(true);
    expect(heartbeat?.namespace).toBe(CODEX_ORIRO_DYNAMIC_TOOL_NAMESPACE);
    expect(heartbeat?.deferLoading).toBe(true);
    expect(sessionsSpawn?.namespace).toBe(CODEX_ORIRO_DYNAMIC_TOOL_NAMESPACE);
  ...
  it("registers heartbeat response durably without advertising it on normal turns", async () => {
    testing.setOriroCodingToolsFactoryForTests((options) => [
      createRuntimeDynamicTool("message"),
    ...
    expect(normalInstructions).toContain(
      "Deferred searchable Oriro dynamic tools available: message.",
    );
    expect(normalInstructions).not.toContain(
      "Deferred searchable Oriro dynamic tools available: heartbeat_respond",
    );
  ...
  it("keeps message in the registered schema when disabled for an internal turn", async () => {
    ...
    await expect(
      ...
      contentItems: [
        {
          type: "inputText",
          text: "Oriro tool is not available for this turn: message",
        },
      ],
    });
  });
  ...
  it("fails closed for Codex app defaults when restricted native tools have no plugin config", async () => {
    testing.setOriroCodingToolsFactoryForTests(() => [createRuntimeDynamicTool("message")]);
    const params = createParams(
      path.join(tempDir, "session.jsonl"),
      path.join(tempDir, "workspace"),
    );
  ...
  it("projects bounded continuity when starting Codex without a native thread binding", async () => {
    ...
    const harness = createStartedThreadHarness();
    const params = createParams(sessionFile, workspaceDir);
    params.prompt = "make the default webpage oriro";
    ...
    expect(inputText).toContain("Oriro assembled context for this turn:");
    ...
    expect(inputText).toContain("Current user request:");
    expect(inputText).toContain("make the default webpage oriro");
  });
  ...
  it("does not replay mirrored history already covered by an existing Codex binding", async () => {
    ...
    expect(inputText).not.toContain("Oriro assembled context for this turn:");
  ...
  it("projects only newer visible history when a resumed Codex binding is stale", async () => {
    ...
    const copilotMirrorMessage = {
      ...
      __oriro: { mirrorIdentity: "copilot:assistant-1" },
    } as ReturnType<typeof assistantMessage> & { __oriro: { mirrorIdentity: string } };
  ...
  it("does not project Codex mirrored transcript echoes as stale binding continuity", async () => {
    ...
    const codexMirrorAssistantMessage = {
      ...
      __oriro: { mirrorIdentity: "codex-app-server:assistant-1" },
    } as ReturnType<typeof assistantMessage> & { __oriro: { mirrorIdentity: string } };
  ...
  it("does not project mirrored messages on consecutive resumes", async () => {
    ...
    const firstInputText =
      (firstTurnStart?.params as { input?: Array<{ text?: string }> } | undefined)?.input?.[0]
        ?.text ?? "";
    expect(firstInputText).toContain("Oriro assembled context for this turn:");
    ...
    const secondInputText =
      (secondTurnStart?.params as { input?: Array<{ text?: string }> } | undefined)?.input?.[0]
        ?.text ?? "";
    expect(secondInputText).not.toContain("Oriro assembled context for this turn:");
  ...
  it("passes stable workspace files as Codex developer instructions and routes MEMORY.md through tools", async () => {
    ...
    expect(threadDeveloperInstructions).toContain("Oriro Workspace Instructions");
    ...
    expect(collaborationInstructions).toContain("Oriro Agent Soul");
    ...
    expect(collaborationInstructions).toContain("Oriro Workspace Memory");
    ...
    expect(inputText).not.toContain("Oriro runtime context for this turn:");
    ...
    expect(inputText).not.toContain("Oriro Workspace Memory");
  ...
  it("adds memory recall guidance when dated memory notes exist without root MEMORY.md", async () => {
    ...
    expect(collaborationInstructions).toContain("memory_search");
    expect(collaborationInstructions).toContain("memory_get");
    expect(collaborationInstructions).not.toContain("Oriro Workspace Memory");
  ...
  it("sends workspace bootstrap instructions through Codex app-server payloads", async () => {
    ...
    expect(threadStartParams.developerInstructions).toContain("Oriro Workspace Instructions");
  ...
  it("routes MEMORY.md through memory_get when search is unavailable", async () => {
    ...
    await fs.writeFile(path.join(workspaceDir, "MEMORY.md"), memorySummary);
    registerMemoryPromptForTest();
    testing.setOriroCodingToolsFactoryForTests(() => [createRuntimeDynamicTool("memory_get")]);
    const params = createParams(sessionFile, workspaceDir);
    params.disableTools = false;
    params.runtimePlan = createCodexRuntimePlanFixture();
    setAgentWorkspaceForTest(params, workspaceDir);
  ...
  it("omits heartbeat Codex workspace pointers for empty HEARTBEAT.md files", async () => {
    ...
    expect(collaborationInstructions).toContain("This is an Oriro heartbeat turn");
    expect(collaborationInstructions).not.toContain("HEARTBEAT.md exists");
  });

  it("keeps lightweight cron Codex turns out of Oriro bootstrap context", async () => {
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
    const exactCommand =
      "cd /Users/phaedrus/Projects/oriro && /Users/phaedrus/orirod/scripts/orirosweeper-related-scan.py";
    await fs.mkdir(workspaceDir, { recursive: true });
  ...
  it("keeps lightweight cron delivery hints byte-for-byte without Oriro prompt context", async () => {
  ...
  it("promotes implicit Codex yolo approval policy when Oriro tool policy exists", async () => {
    initializeGlobalHookRunner(
    ...
    expect(info).toHaveBeenCalledWith(
      "codex app-server approval policy promoted for Oriro tool policy",
      {
        from: "never",
        to: "untrusted",
  ...
  it("keeps explicit Codex yolo mode unpromoted when Oriro tool policy exists", async () => {
    initializeGlobalHookRunner(
      createMockPluginRegistry([{ hookName: "before_tool_call", handler: vi.fn() }]),
    );
    const sessionFile = path.join(tempDir, "session.jsonl");
  ...
  it("keeps normalized full exec mode unpromoted when Oriro tool policy exists", async () => {
    initializeGlobalHookRunner(
      createMockPluginRegistry([{ hookName: "before_tool_call", handler: vi.fn() }]),
    );
    const sessionFile = path.join(tempDir, "session.jsonl");
  ...
  it("ignores invalid Codex app-server env overrides when promoting tool policy approval", async () => {
    ...
    vi.stubEnv("ORIRO_CODEX_APP_SERVER_MODE", " ");
    vi.stubEnv("ORIRO_CODEX_APP_SERVER_APPROVAL_POLICY", "always");
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
    const harness = createStartedThreadHarness();
  ...
  it("keeps managed web_search for provider-qualified Codex model overrides", async () => {
    testing.setOriroCodingToolsFactoryForTests(() => [createRuntimeDynamicTool("web_search")]);
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
    const { requests, waitForMethod, completeTurn } = createStartedThreadHarness(async (method) => {
  ...
  it("announces Codex app-server fast auto progress after the crossing tool result", async () => {
    ...
    expect(payloads[offIndex]?.channelData).toEqual({
      oriroProgressKind: "fast-mode-auto",
    });
