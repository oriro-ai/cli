...
import {
  ...
  isPendingOriroDynamicToolCompletionNotification,
...
export function applyCodexTurnNotificationState(params: {
  ...
  pendingOriroDynamicToolCompletionIds: Set<string>;
  ...
  const trackedDynamicToolCompletion = isPendingOriroDynamicToolCompletionNotification(
    notification,
    params.pendingOriroDynamicToolCompletionIds,
  );
  ...
  if (
    ...
    // accepts a turn or after Oriro hands a turn-scoped request result
    // back to Codex. Bookkeeping that closes the just-served Oriro
  ...
  if (trackedDynamicToolCompletion) {
    ...
    if (itemId) {
      params.pendingOriroDynamicToolCompletionIds.delete(itemId);
      params.onScheduleTerminalDynamicToolReleaseCheck();
    }
  }
