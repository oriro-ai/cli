...
import { runTasksWithConcurrency } from "oriro/plugin-sdk/concurrency-runtime";
import { retryTransientMemoryRead } from "oriro/plugin-sdk/memory-core-host-engine-storage";
import {
  replaceManagedMarkdownBlock,
  withTrailingNewline,
} from "oriro/plugin-sdk/memory-host-markdown";
...
const AGENT_DIGEST_PATH = ".oriro-wiki/cache/agent-digest.json";
...
async function writeDashboardPage(params: {
  ...
  const updatedBody = replaceManagedMarkdownBlock({
    ...
    heading: "## Generated",
    startMarker: `<!-- oriro:wiki:${path.basename(params.definition.relativePath, ".md")}:start -->`,
    endMarker: `<!-- oriro:wiki:${path.basename(params.definition.relativePath, ".md")}:end -->`,
...
export async function compileMemoryWikiVault(
  ...
  if (
    await writeManagedMarkdownFile({
      ...
      title: "Wiki Index",
      startMarker: "<!-- oriro:wiki:index:start -->",
      endMarker: "<!-- oriro:wiki:index:end -->",
  ...
  for (const group of COMPILE_PAGE_GROUPS) {
    ...
    if (
      await writeManagedMarkdownFile({
        ...
        relativePath,
        title: group.heading,
        startMarker: `<!-- oriro:wiki:${group.dir}:index:start -->`,
        endMarker: `<!-- oriro:wiki:${group.dir}:index:end -->`,
