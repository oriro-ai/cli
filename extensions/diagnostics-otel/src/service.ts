...
import { waitForDiagnosticEventsDrained } from "oriro/plugin-sdk/diagnostic-runtime";
import { registerUnhandledRejectionHandler } from "oriro/plugin-sdk/runtime-env";
import type {
...
const DEFAULT_SERVICE_NAME = "oriro";
...
const PRELOADED_OTEL_SDK_ENV = "ORIRO_OTEL_PRELOADED";
...
function assignModelCallSizeTimingAttrs(
  ...
  assignPositiveNumberAttr(attrs, "oriro.model_call.request_bytes", evt.requestPayloadBytes);
  assignPositiveNumberAttr(attrs, "oriro.model_call.response_bytes", evt.responseStreamBytes);
  assignPositiveNumberAttr(
    attrs,
    "oriro.model_call.time_to_first_byte_ms",
...
function addUpstreamRequestIdSpanEvent(
  ...
  span.addEvent?.("oriro.provider.request", {
    "oriro.upstreamRequestIdHash": boundedHash,
  });
}
...
function assignOtelModelContentAttributes(
  ...
  if (policy.toolDefinitions) {
    assignOtelContentAttribute(
      attributes,
      "oriro.content.tool_definitions",
  ...
  if (policy.outputMessages) {
    assignOtelContentAttribute(
      attributes,
      "oriro.content.output_messages",
      content?.outputMessages,
    );
  }
  if (policy.systemPrompt) {
    assignOtelContentAttribute(attributes, "oriro.content.system_prompt", content?.systemPrompt);
  }
}
...
function assignOtelToolContentAttributes(
  ...
  if (policy.toolInputs) {
    assignOtelContentAttribute(attributes, "oriro.content.tool_input", content?.toolInput);
  }
  if (policy.toolOutputs) {
    assignOtelContentAttribute(attributes, "oriro.content.tool_output", content?.toolOutput);
  }
}
...
function assignOtelLogEventAttributes(
  ...
  for (const rawKey in eventAttributes) {
    ...
    assignOtelLogAttribute(attributes, `oriro.${key}`, eventAttributes[rawKey]);
  }
}
...
function assignOtelSecurityAttributes(
  ...
  assignOtelLogAttribute(attributes, "oriro.security.event_id", evt.eventId);
  assignOtelLogAttribute(attributes, "oriro.security.category", evt.category);
  assignOtelLogAttribute(attributes, "oriro.security.action", lowCardinalityAttr(evt.action));
  assignOtelLogAttribute(attributes, "oriro.security.outcome", evt.outcome);
  assignOtelLogAttribute(attributes, "oriro.security.severity", evt.severity);
  if (evt.reason) {
    assignOtelLogAttribute(attributes, "oriro.security.reason", lowCardinalityAttr(evt.reason));
  }
  if (evt.actor) {
    assignOtelLogAttribute(attributes, "oriro.security.actor.kind", evt.actor.kind);
    if (evt.actor.idHash) {
      assignOtelLogAttribute(
        attributes,
        "oriro.security.actor.id_hash",
    ...
    if (evt.actor.deviceIdHash) {
      assignOtelLogAttribute(
        attributes,
        "oriro.security.actor.device_id_hash",
    ...
    if (evt.actor.scopes?.length) {
      assignOtelLogAttribute(
        attributes,
        "oriro.security.actor.scopes",
        evt.actor.scopes.map((scope) => lowCardinalityAttr(scope)).join(","),
      );
    }
  }
  if (evt.target) {
    assignOtelLogAttribute(attributes, "oriro.security.target.kind", evt.target.kind);
    if (evt.target.idHash) {
      assignOtelLogAttribute(
        attributes,
        "oriro.security.target.id_hash",
  ...
  if (evt.policy) {
    if (evt.policy.id) {
      assignOtelLogAttribute(
        attributes,
        "oriro.security.policy.id",
        lowCardinalityAttr(evt.policy.id),
      );
    }
    if (evt.policy.decision) {
      assignOtelLogAttribute(attributes, "oriro.security.policy.decision", evt.policy.decision);
    }
    if (evt.policy.reason) {
      assignOtelLogAttribute(
        attributes,
        "oriro.security.policy.reason",
  ...
  if (evt.control) {
    if (evt.control.id) {
      assignOtelLogAttribute(
        attributes,
        "oriro.security.control.id",
        lowCardinalityAttr(evt.control.id),
      );
    }
    if (evt.control.family) {
      assignOtelLogAttribute(attributes, "oriro.security.control.family", evt.control.family);
    }
  }
...
function addTraceAttributes(
  ...
  attributes["oriro.traceId"] = normalized.traceId;
  if (normalized.spanId) {
    attributes["oriro.spanId"] = normalized.spanId;
  }
  if (normalized.parentSpanId) {
    attributes["oriro.parentSpanId"] = normalized.parentSpanId;
  }
  if (normalized.traceFlags) {
    attributes["oriro.traceFlags"] = normalized.traceFlags;
  }
}

export function createDiagnosticsOtelService(): OriroPluginService {
  ...
  return {
    ...
    async start(ctx) {
      ...
      const meter = metrics.getMeter("oriro");
      const tracer = trace.getTracer("oriro");
      ...
      const tokensCounter = meter.createCounter("oriro.tokens", {
        unit: "1",
      ...
      const costCounter = meter.createCounter("oriro.cost.usd", {
        unit: "1",
        description: "Estimated model cost (USD)",
      });
      const durationHistogram = meter.createHistogram("oriro.run.duration_ms", {
        unit: "ms",
        description: "Agent run duration",
      });
      const harnessDurationHistogram = meter.createHistogram("oriro.harness.duration_ms", {
        unit: "ms",
        description: "Agent harness lifecycle duration",
      });
      const contextHistogram = meter.createHistogram("oriro.context.tokens", {
        unit: "1",
        description: "Context window size and usage",
      });
      const webhookReceivedCounter = meter.createCounter("oriro.webhook.received", {
        unit: "1",
        description: "Webhook requests received",
      });
      const webhookErrorCounter = meter.createCounter("oriro.webhook.error", {
        unit: "1",
        description: "Webhook processing errors",
      });
      const webhookDurationHistogram = meter.createHistogram("oriro.webhook.duration_ms", {
        unit: "ms",
        description: "Webhook processing duration",
      });
      const messageQueuedCounter = meter.createCounter("oriro.message.queued", {
        unit: "1",
        description: "Messages queued for processing",
      });
      const messageReceivedCounter = meter.createCounter("oriro.message.received", {
        unit: "1",
        description: "Inbound messages received",
      });
      const messageDispatchStartedCounter = meter.createCounter(
        "oriro.message.dispatch.started",
        {
          unit: "1",
      ...
      const messageDispatchCompletedCounter = meter.createCounter(
        "oriro.message.dispatch.completed",
        {
          unit: "1",
      ...
      const messageDispatchDurationHistogram = meter.createHistogram(
        "oriro.message.dispatch.duration_ms",
        {
          unit: "ms",
          description: "Inbound message dispatch duration",
        },
      );
      const messageProcessedCounter = meter.createCounter("oriro.message.processed", {
        unit: "1",
        description: "Messages processed by outcome",
      });
      const messageDurationHistogram = meter.createHistogram("oriro.message.duration_ms", {
        unit: "ms",
        description: "Message processing duration",
      });
      const messageDeliveryStartedCounter = meter.createCounter(
        "oriro.message.delivery.started",
        {
          unit: "1",
      ...
      const queueDepthHistogram = meter.createHistogram("oriro.queue.depth", {
        unit: "1",
        description: "Queue depth on enqueue/dequeue",
      });
      const queueWaitHistogram = meter.createHistogram("oriro.queue.wait_ms", {
        unit: "ms",
        description: "Queue wait time before execution",
      });
      const laneEnqueueCounter = meter.createCounter("oriro.queue.lane.enqueue", {
        unit: "1",
        description: "Command queue lane enqueue events",
      });
      const laneDequeueCounter = meter.createCounter("oriro.queue.lane.dequeue", {
        unit: "1",
        description: "Command queue lane dequeue events",
      });
      const sessionStateCounter = meter.createCounter("oriro.session.state", {
        unit: "1",
        description: "Session state transitions",
      });
      const sessionTurnCreatedCounter = meter.createCounter("oriro.session.turn.created", {
        unit: "1",
        description: "Agent session turns created",
      });
      const sessionStuckCounter = meter.createCounter("oriro.session.stuck", {
        unit: "1",
        description: "Sessions stuck in processing",
      });
      const sessionStuckAgeHistogram = meter.createHistogram("oriro.session.stuck_age_ms", {
        unit: "ms",
        description: "Age of stuck sessions",
      });
      const sessionRecoveryRequestedCounter = meter.createCounter(
        "oriro.session.recovery.requested",
        {
          unit: "1",
      ...
      const sessionRecoveryCompletedCounter = meter.createCounter(
        "oriro.session.recovery.completed",
        {
          unit: "1",
      ...
      const sessionRecoveryAgeHistogram = meter.createHistogram(
        "oriro.session.recovery.age_ms",
        {
          unit: "ms",
          description: "Age of sessions selected for recovery",
        },
      );
      const talkEventCounter = meter.createCounter("oriro.talk.event", {
        unit: "1",
        description: "Talk events emitted by type",
      });
      const talkEventDurationHistogram = meter.createHistogram("oriro.talk.event.duration_ms", {
        unit: "ms",
        description: "Talk event duration when reported",
      });
      const talkAudioBytesHistogram = meter.createHistogram("oriro.talk.audio.bytes", {
        unit: "By",
        description: "Talk audio frame byte lengths",
      });
      const runAttemptCounter = meter.createCounter("oriro.run.attempt", {
        unit: "1",
        description: "Run attempts",
      });
      const toolLoopCounter = meter.createCounter("oriro.tool.loop", {
        unit: "1",
        description: "Detected repetitive tool-call loop events",
      });
      const skillUsedCounter = meter.createCounter("oriro.skill.used", {
        unit: "1",
        description: "Skills used by agent runs",
      });
      const modelCallDurationHistogram = meter.createHistogram("oriro.model_call.duration_ms", {
        unit: "ms",
        description: "Model call duration",
      });
      const modelCallRequestBytesHistogram = meter.createHistogram(
        "oriro.model_call.request_bytes",
        {
          unit: "By",
      ...
      const modelCallResponseBytesHistogram = meter.createHistogram(
        "oriro.model_call.response_bytes",
        {
          unit: "By",
      ...
      const modelFailoverCounter = meter.createCounter("oriro.model.failover", {
        unit: "1",
      ...
      const toolExecutionDurationHistogram = meter.createHistogram(
        "oriro.tool.execution.duration_ms",
        {
          unit: "ms",
          description: "Tool execution duration",
        },
      );
      const toolExecutionBlockedCounter = meter.createCounter("oriro.tool.execution.blocked", {
        unit: "1",
        description: "Tool executions blocked by policy or sandbox diagnostics",
      });
      const execProcessDurationHistogram = meter.createHistogram("oriro.exec.duration_ms", {
        unit: "ms",
        description: "Exec process duration",
      });
      const memoryRssHistogram = meter.createHistogram("oriro.memory.rss_bytes", {
        unit: "By",
        description: "Resident set size reported by diagnostic memory samples",
      });
      const memoryHeapUsedHistogram = meter.createHistogram("oriro.memory.heap_used_bytes", {
        unit: "By",
        description: "Heap used bytes reported by diagnostic memory samples",
      });
      const memoryHeapTotalHistogram = meter.createHistogram("oriro.memory.heap_total_bytes", {
        unit: "By",
        description: "Heap total bytes reported by diagnostic memory samples",
      });
      const memoryExternalHistogram = meter.createHistogram("oriro.memory.external_bytes", {
        unit: "By",
      ...
      const livenessEventLoopDelayMaxHistogram = meter.createHistogram(
        "oriro.liveness.event_loop_delay_max_ms",
        {
          unit: "ms",
      ...
      const livenessEventLoopUtilizationHistogram = meter.createHistogram(
        "oriro.liveness.event_loop_utilization",
        {
          unit: "1",
      ...
      const livenessCpuCoreRatioHistogram = meter.createHistogram(
        "oriro.liveness.cpu_core_ratio",
        {
          unit: "1",
          description: "CPU core ratio reported by diagnostic liveness warnings",
        },
      );
      const telemetryExporterCounter = meter.createCounter("oriro.telemetry.exporter.events", {
        unit: "1",
      ...
      if (logsEnabled) {
        ...
        if (logsToOtlp) {
          ...
          otelLogger = logProvider.getLogger("oriro");
        }
        ...
        const buildDiagnosticLogRecord = (
          ...
          assignOtelLogAttribute(attributes, "oriro.log.level", logLevelName);
          if (evt.loggerName) {
            assignOtelLogAttribute(attributes, "oriro.logger", evt.loggerName);
          }
          if (evt.loggerParents?.length) {
            assignOtelLogAttribute(
              attributes,
              "oriro.logger.parents",
      ...
      const addRunAttrs = (
        ...
        if (evt.provider) {
          spanAttrs["oriro.provider"] = evt.provider;
        }
        if (evt.model) {
          spanAttrs["oriro.model"] = evt.model;
        }
        if (evt.channel) {
          spanAttrs["oriro.channel"] = evt.channel;
        }
        if (evt.trigger) {
          spanAttrs["oriro.trigger"] = evt.trigger;
        }
      };
      ...
      const paramsSummaryAttrs = (
        ...
        return {
          "oriro.tool.params.kind": summary.kind,
          ...("length" in summary ? { "oriro.tool.params.length": summary.length } : {}),
        };
      };
      ...
      const recordModelUsage = (
        ...
        const attrs = {
          "oriro.channel": evt.channel ?? "unknown",
          "oriro.agent": lowCardinalityAttr(evt.agentId),
          "oriro.provider": evt.provider ?? "unknown",
          "oriro.model": evt.model ?? "unknown",
        };
        ...
        if (usage.input) {
          tokensCounter.add(usage.input, { ...attrs, "oriro.token": "input" });
        ...
        if (usage.output) {
          tokensCounter.add(usage.output, { ...attrs, "oriro.token": "output" });
        ...
        if (usage.cacheRead) {
          tokensCounter.add(usage.cacheRead, { ...attrs, "oriro.token": "cache_read" });
        }
        if (usage.cacheWrite) {
          tokensCounter.add(usage.cacheWrite, { ...attrs, "oriro.token": "cache_write" });
        }
        if (usage.promptTokens) {
          tokensCounter.add(usage.promptTokens, { ...attrs, "oriro.token": "prompt" });
        }
        if (usage.total) {
          tokensCounter.add(usage.total, { ...attrs, "oriro.token": "total" });
        }
        ...
        const spanAttrs: Record<string, string | number> = {
          ...attrs,
          "oriro.tokens.input": usage.input ?? 0,
          "oriro.tokens.output": usage.output ?? 0,
          "oriro.tokens.cache_read": usage.cacheRead ?? 0,
          "oriro.tokens.cache_write": usage.cacheWrite ?? 0,
          "oriro.tokens.total": usage.total ?? 0,
        };
        ...
        const span = spanWithDuration("oriro.model.usage", spanAttrs, evt.durationMs, {
      ...
      const recordWebhookReceived = (
        ...
        const attrs = {
          "oriro.channel": evt.channel ?? "unknown",
          "oriro.webhook": evt.updateType ?? "unknown",
        };
      ...
      const recordWebhookProcessed = (
        ...
        const attrs = {
          "oriro.channel": lowCardinalityAttr(evt.channel),
          "oriro.webhook": lowCardinalityAttr(evt.updateType),
        };
        ...
        const span = spanWithDuration("oriro.webhook.processed", spanAttrs, evt.durationMs);
        span.end();
      };
      ...
      const recordWebhookError = (
        ...
        const spanAttrs: Record<string, string | number> = {
          ...attrs,
          "oriro.error": redactedError,
        };
        const span = tracer.startSpan("oriro.webhook.error", {
      ...
      const recordMessageQueued = (
        ...
        const attrs = {
          "oriro.channel": lowCardinalityAttr(evt.channel),
          "oriro.source": lowCardinalityAttr(evt.source),
        };
      ...
      const recordMessageDispatchStarted = (
        ...
        trackInternalOrTrustedSpan(
          evt,
          metadata,
          spanWithDuration("oriro.message.processed", attrs, undefined, {
      ...
      const recordMessageDispatchCompleted = (
        ...
        const attrs = {
          "oriro.channel": lowCardinalityAttr(evt.channel),
          "oriro.outcome": evt.outcome,
          "oriro.reason": lowCardinalityAttr(evt.reason, "none"),
          "oriro.source": lowCardinalityAttr(evt.source),
        };
      ...
      const recordMessageProcessed = (
        ...
        const attrs = {
          "oriro.channel": lowCardinalityAttr(evt.channel),
          "oriro.outcome": evt.outcome ?? "unknown",
        };
        ...
        const spanAttrs: Record<string, string | number> = { ...attrs };
        if (evt.reason) {
          spanAttrs["oriro.reason"] = lowCardinalityAttr(evt.reason, "unknown");
        }
        const trackedSpan = getTrackedInternalOrTrustedSpan(evt, metadata);
        const span =
          trackedSpan ??
          spanWithDuration("oriro.message.processed", spanAttrs, evt.durationMs, {
            parentContext: internalOrTrustedExplicitParentContext(evt, metadata),
            endTimeMs: evt.ts,
          });
      ...
      const messageDeliveryAttrs = (
        ...
        "oriro.channel": lowCardinalityAttr(evt.channel),
        "oriro.delivery.kind": lowCardinalityAttr(evt.deliveryKind, "other"),
      });
      ...
      const recordMessageDeliveryCompleted = (
        ...
        const span = spanWithDuration(
          ...
          {
            ...attrs,
            "oriro.delivery.result_count": evt.resultCount,
          },
          evt.durationMs,
      ...
      const recordMessageDeliveryError = (
        ...
        const attrs = {
          ...messageDeliveryAttrs(evt),
          "oriro.outcome": "error",
          "oriro.errorCategory": lowCardinalityAttr(evt.errorCategory, "other"),
        };
        messageDeliveryDurationHistogram.record(evt.durationMs, attrs);
        if (!tracesEnabled) {
          return;
        }
        const span = spanWithDuration("oriro.message.delivery", attrs, evt.durationMs, {
      ...
      const recordRunStarted = (
        ...
        const span = trackTrustedSpan(
          evt,
          metadata,
          spanWithDuration("oriro.run", spanAttrs, undefined, {
            parentContext: activeTrustedParentContext(evt, metadata),
            startTimeMs: evt.ts,
          }),
        );
      ...
      const recordLaneEnqueue = (
        ...
        const attrs = { "oriro.lane": lowCardinalityQueueLaneAttr(evt.lane) };
      ...
      const recordSessionState = (
        ...
        const attrs: Record<string, string> = { "oriro.state": evt.state };
        if (evt.reason) {
          attrs["oriro.reason"] = redactSensitiveText(evt.reason);
        }
      ...
      const recordSessionTurnCreated = (
        ...
        sessionTurnCreatedCounter.add(1, {
          "oriro.agent": lowCardinalityAttr(evt.agentId, "unknown"),
          "oriro.channel": lowCardinalityAttr(evt.channel, "unknown"),
          "oriro.trigger": evt.trigger,
        });
      };
      ...
      const recordSessionStuck = (
        ...
        const spanAttrs: Record<string, string | number> = { ...attrs };
        spanAttrs["oriro.queueDepth"] = evt.queueDepth ?? 0;
        spanAttrs["oriro.ageMs"] = evt.ageMs;
        const span = tracer.startSpan("oriro.session.stuck", { attributes: spanAttrs });
      ...
      const sessionRecoveryAttrs = (evt: SessionRecoveryDiagnosticEvent) => {
        ...
        if (evt.activeWorkKind) {
          attrs["oriro.active_work_kind"] = evt.activeWorkKind;
        }
        return attrs;
      };
      ...
      const recordSessionRecoveryRequested = (
        ...
        attrs["oriro.action"] = evt.allowActiveAbort ? "abort" : "recover";
      ...
      const recordSessionRecoveryCompleted = (
        ...
        const attrs = sessionRecoveryAttrs(evt);
        attrs["oriro.status"] = evt.status;
        attrs["oriro.action"] = lowCardinalityAttr(evt.action, "unknown");
        if (evt.outcomeReason) {
          attrs["oriro.reason"] = redactSensitiveText(evt.outcomeReason);
        }
      ...
      const talkEventAttrs = (evt: TalkDiagnosticEvent): Record<string, string> => ({
        "oriro.talk.brain": lowCardinalityAttr(evt.brain),
        "oriro.talk.event_type": lowCardinalityAttr(evt.talkEventType),
        "oriro.talk.mode": lowCardinalityAttr(evt.mode),
        "oriro.talk.provider": lowCardinalityAttr(evt.provider),
        "oriro.talk.transport": lowCardinalityAttr(evt.transport),
      });
      ...
      const recordRunAttempt = (evt: Extract<DiagnosticEventPayload, { type: "run.attempt" }>) => {
        runAttemptCounter.add(1, { "oriro.attempt": evt.attempt });
      };

      const toolLoopAttrs = (
        evt: Extract<DiagnosticEventPayload, { type: "tool.loop" }>,
      ): Record<string, string | number> => ({
        "oriro.toolName": lowCardinalityAttr(evt.toolName, "tool"),
        "oriro.loop.level": evt.level,
        "oriro.loop.action": evt.action,
        "oriro.loop.detector": evt.detector,
        "oriro.loop.count": evt.count,
        ...(evt.pairedToolName
          ? { "oriro.loop.paired_tool": lowCardinalityAttr(evt.pairedToolName, "tool") }
          : {}),
      });
      ...
      const recordToolLoop = (evt: Extract<DiagnosticEventPayload, { type: "tool.loop" }>) => {
        ...
        const span = spanWithDuration("oriro.tool.loop", attrs, 0, { endTimeMs: evt.ts });
      ...
      const recordMemoryPressure = (
        ...
        const attrs = {
          "oriro.memory.level": evt.level,
          "oriro.memory.reason": evt.reason,
        };
        ...
        const spanAttrs: Record<string, string | number | boolean> = {
          ...attrs,
          "oriro.memory.rss_bytes": evt.memory.rssBytes,
          "oriro.memory.heap_used_bytes": evt.memory.heapUsedBytes,
          "oriro.memory.heap_total_bytes": evt.memory.heapTotalBytes,
          "oriro.memory.external_bytes": evt.memory.externalBytes,
          "oriro.memory.array_buffers_bytes": evt.memory.arrayBuffersBytes,
          ...(evt.thresholdBytes !== undefined
            ? { "oriro.memory.threshold_bytes": evt.thresholdBytes }
            : {}),
          ...(evt.rssGrowthBytes !== undefined
            ? { "oriro.memory.rss_growth_bytes": evt.rssGrowthBytes }
            : {}),
          ...(evt.windowMs !== undefined ? { "oriro.memory.window_ms": evt.windowMs } : {}),
        };
        const span = spanWithDuration("oriro.memory.pressure", spanAttrs, 0, {
          endTimeMs: evt.ts,
        });
      ...
      const recordRunCompleted = (
        ...
        if (evt.channel) {
          attrs["oriro.channel"] = evt.channel;
        }
        if (evt.blockedBy) {
          attrs["oriro.blocked_by"] = lowCardinalityAttr(evt.blockedBy, "unknown");
        }
        durationHistogram.record(evt.durationMs, attrs);
        if (!tracesEnabled) {
          return;
        ...
        addRunAttrs(spanAttrs, evt);
        if (evt.blockedBy) {
          spanAttrs["oriro.blocked_by"] = lowCardinalityAttr(evt.blockedBy, "unknown");
        }
        if (evt.errorCategory) {
          spanAttrs["oriro.errorCategory"] = lowCardinalityAttr(evt.errorCategory, "other");
        }
        ...
        const span =
          trackedSpan ??
          spanWithDuration("oriro.run", spanAttrs, evt.durationMs, {
            parentContext: activeTrustedParentContext(evt, metadata),
            endTimeMs: evt.ts,
          });
        setSpanAttrs(span, spanAttrs);
      ...
      const harnessRunMetricAttrs = (evt: HarnessRunDiagnosticEvent) => ({
        "oriro.harness.id": lowCardinalityAttr(evt.harnessId, "unknown"),
        "oriro.harness.plugin": lowCardinalityAttr(evt.pluginId),
        ...(evt.type === "harness.run.started"
          ? {}
          : {
              "oriro.outcome": evt.type === "harness.run.error" ? "error" : evt.outcome,
            }),
        "oriro.provider": lowCardinalityAttr(evt.provider, "unknown"),
        "oriro.model": lowCardinalityAttr(evt.model, "unknown"),
        ...(evt.channel ? { "oriro.channel": lowCardinalityAttr(evt.channel) } : {}),
      });
      ...
      const recordHarnessRunStarted = (
        ...
        trackTrustedSpan(
          evt,
          metadata,
          spanWithDuration("oriro.harness.run", harnessRunMetricAttrs(evt), undefined, {
            parentContext: activeTrustedParentContext(evt, metadata),
            startTimeMs: evt.ts,
          }),
        );
      ...
      const recordHarnessRunCompleted = (
        ...
        if (evt.resultClassification) {
          spanAttrs["oriro.harness.result_classification"] = lowCardinalityAttr(
            evt.resultClassification,
          );
        }
        if (typeof evt.yieldDetected === "boolean") {
          spanAttrs["oriro.harness.yield_detected"] = evt.yieldDetected;
        }
        if (evt.itemLifecycle) {
          spanAttrs["oriro.harness.items.started"] = evt.itemLifecycle.startedCount;
          spanAttrs["oriro.harness.items.completed"] = evt.itemLifecycle.completedCount;
          spanAttrs["oriro.harness.items.active"] = evt.itemLifecycle.activeCount;
        }
        const trustedTrace = trustedTraceContext(evt, metadata);
        const trackedSpan = trustedTrace?.spanId
          ? activeTrustedSpans.get(trustedTrace.spanId)
        ...
        const span =
          trackedSpan ??
          spanWithDuration("oriro.harness.run", spanAttrs, evt.durationMs, {
            parentContext: activeTrustedParentContext(evt, metadata),
            endTimeMs: evt.ts,
          });
        setSpanAttrs(span, spanAttrs);
      ...
      const recordHarnessRunError = (
        ...
        const attrs = {
          ...harnessRunMetricAttrs(evt),
          "oriro.harness.phase": evt.phase,
          "oriro.errorCategory": errorType,
        };
        ...
        const spanAttrs: Record<string, string | number | boolean> = {
          ...
          ...(evt.cleanupFailed ? { "oriro.harness.cleanup_failed": true } : {}),
        };
        const span =
      ...
      const recordContextAssembled = (
        ...
        const spanAttrs: Record<string, string | number | boolean> = {
          "oriro.context.message_count": evt.messageCount,
          "oriro.context.history_text_chars": evt.historyTextChars,
          "oriro.context.history_image_blocks": evt.historyImageBlocks,
          "oriro.context.max_message_text_chars": evt.maxMessageTextChars,
          "oriro.context.system_prompt_chars": evt.systemPromptChars,
          "oriro.context.prompt_chars": evt.promptChars,
          "oriro.context.prompt_images": evt.promptImages,
        };
        addRunAttrs(spanAttrs, evt);
        if (evt.contextTokenBudget !== undefined) {
          spanAttrs["oriro.context.token_budget"] = evt.contextTokenBudget;
        }
        if (evt.reserveTokens !== undefined) {
          spanAttrs["oriro.context.reserve_tokens"] = evt.reserveTokens;
        }
        const span = spanWithDuration("oriro.context.assembled", spanAttrs, 0, {
          parentContext: activeTrustedParentContext(evt, metadata),
          endTimeMs: evt.ts,
        });
        span.end(evt.ts);
      ...
      const recordModelFailover = (
        ...
        const metricAttrs: Record<string, string> = {
          "oriro.failover.reason": lowCardinalityAttr(evt.reason, "unknown"),
          "oriro.failover.suspended":
            evt.suspended === undefined ? "unknown" : String(evt.suspended),
          "oriro.lane": lowCardinalityQueueLaneAttr(evt.lane, "unknown"),
          "oriro.model": lowCardinalityAttr(evt.fromModel),
          "oriro.provider": lowCardinalityAttr(evt.fromProvider),
          "oriro.failover.to_model": lowCardinalityAttr(evt.toModel),
          "oriro.failover.to_provider": lowCardinalityAttr(evt.toProvider),
        };
        ...
        if (evt.fromProvider) {
          spanAttrs["oriro.provider"] = evt.fromProvider;
        }
        if (evt.fromModel) {
          spanAttrs["oriro.model"] = evt.fromModel;
        }
        if (evt.toProvider) {
          spanAttrs["oriro.failover.to_provider"] = evt.toProvider;
        }
        if (evt.toModel) {
          spanAttrs["oriro.failover.to_model"] = evt.toModel;
        }
        if (evt.lane) {
          spanAttrs["oriro.lane"] = lowCardinalityQueueLaneAttr(evt.lane, "unknown");
        }
        if (evt.suspended !== undefined) {
          spanAttrs["oriro.failover.suspended"] = evt.suspended;
        }
        if (evt.cascadeDepth !== undefined) {
          spanAttrs["oriro.failover.cascade_depth"] = evt.cascadeDepth;
        }
        const span = spanWithDuration("oriro.model.failover", spanAttrs, 0, {
          parentContext: activeTrustedParentContext(evt, metadata),
          endTimeMs: evt.ts,
        });
        span.end(evt.ts);
      ...
      const modelCallMetricAttrs = (evt: ModelCallLifecycleDiagnosticEvent) => ({
        "oriro.provider": evt.provider,
        "oriro.model": evt.model,
        "oriro.api": lowCardinalityAttr(evt.api),
        "oriro.transport": lowCardinalityAttr(evt.transport),
      });
      ...
      const recordModelCallStarted = (
        ...
        if (evt.api) {
          spanAttrs["oriro.api"] = evt.api;
        }
        if (evt.transport) {
          spanAttrs["oriro.transport"] = evt.transport;
        }
        trackTrustedSpan(
          evt,
          metadata,
      ...
      const recordModelCallError = (
        ...
        const metricAttrs = {
          ...
          "oriro.errorCategory": errorType,
          ...(evt.failureKind
            ? { "oriro.failureKind": lowCardinalityAttr(evt.failureKind, "other") }
            : {}),
        };
        modelCallDurationHistogram.record(evt.durationMs, metricAttrs);
        recordModelCallSizeTimingMetrics(evt, metricAttrs);
        ...
        if (evt.failureKind) {
          spanAttrs["oriro.failureKind"] = lowCardinalityAttr(evt.failureKind, "other");
        }
        assignGenAiModelCallAttrs(spanAttrs, evt);
        if (evt.api) {
          spanAttrs["oriro.api"] = evt.api;
      ...
      const toolExecutionBaseAttrs = (
        ...
        "oriro.toolName": evt.toolName,
        "oriro.tool.source": lowCardinalityAttr(evt.toolSource, "core"),
        "gen_ai.tool.name": evt.toolName,
        ...(evt.toolOwner ? { "oriro.tool.owner": lowCardinalityAttr(evt.toolOwner) } : {}),
      ...
      const skillUsedAttrs = (
        ...
        "oriro.skill.name": lowCardinalityAttr(evt.skillName, "skill"),
        "oriro.skill.source": lowCardinalityAttr(evt.skillSource),
        "oriro.skill.activation": lowCardinalityAttr(evt.activation),
        ...(evt.agentId ? { "oriro.agent": lowCardinalityAttr(evt.agentId) } : {}),
        ...(evt.toolName ? { "oriro.toolName": lowCardinalityAttr(evt.toolName, "tool") } : {}),
      });
      ...
      const recordSkillUsed = (
        ...
        addRunAttrs(spanAttrs, evt);
        const span = spanWithDuration("oriro.skill.used", spanAttrs, 0, {
          parentContext: activeTrustedParentContext(evt, metadata),
          endTimeMs: evt.ts,
        });
        setSpanAttrs(span, spanAttrs);
      ...
      const recordToolExecutionStarted = (
        ...
        trackTrustedSpan(
          evt,
          metadata,
          spanWithDuration("oriro.tool.execution", toolExecutionBaseAttrs(evt), undefined, {
            parentContext: activeTrustedParentContext(evt, metadata),
            startTimeMs: evt.ts,
          }),
        );
      ...
      const recordToolExecutionCompleted = (
        ...
        const span =
          takeTrackedTrustedSpan(evt, metadata) ??
          spanWithDuration("oriro.tool.execution", spanAttrs, evt.durationMs, {
            parentContext: activeTrustedParentContext(evt, metadata),
            endTimeMs: evt.ts,
          });
        setSpanAttrs(span, spanAttrs);
      ...
      const recordToolExecutionError = (
        ...
        addRunAttrs(spanAttrs, evt);
        if (evt.errorCode) {
          spanAttrs["oriro.errorCode"] = lowCardinalityAttr(evt.errorCode, "other");
        }
        assignOtelToolContentAttributes(spanAttrs, toolContent, contentCapturePolicy);
        const span =
          takeTrackedTrustedSpan(evt, metadata) ??
      ...
      const recordToolExecutionBlocked = (
        ...
        toolExecutionBlockedCounter.add(1, {
          ...toolExecutionBaseAttrs(evt),
          "oriro.deniedReason": lowCardinalityAttr(evt.deniedReason, "other"),
        });
        if (!tracesEnabled) {
          return;
        }
        const spanAttrs: Record<string, string | number | boolean> = {
          ...toolExecutionBaseAttrs(evt),
          "oriro.outcome": "blocked",
          "oriro.deniedReason": lowCardinalityAttr(evt.deniedReason, "other"),
        };
        addRunAttrs(spanAttrs, evt);
        const span = spanWithDuration("oriro.tool.execution", spanAttrs, 0, {
          parentContext: activeTrustedParentContext(evt, metadata),
          endTimeMs: evt.ts,
        });
        setSpanAttrs(span, spanAttrs);
      ...
      const recordPayloadLarge = (
        ...
        const attrs = {
          "oriro.payload.action": evt.action,
          "oriro.payload.surface": lowCardinalityAttr(evt.surface, "unknown"),
          "oriro.channel": lowCardinalityAttr(evt.channel, "none"),
          "oriro.plugin": lowCardinalityAttr(evt.pluginId, "none"),
          "oriro.reason": lowCardinalityAttr(evt.reason, "none"),
        };
      ...
      const recordExecProcessCompleted = (
        ...
        const attrs: Record<string, string | number> = {
          "oriro.exec.target": evt.target,
          "oriro.exec.mode": evt.mode,
          "oriro.outcome": evt.outcome,
        };
        if (evt.failureKind) {
          attrs["oriro.failureKind"] = evt.failureKind;
        }
        ...
        const spanAttrs: Record<string, string | number | boolean> = {
          ...attrs,
          "oriro.exec.command_length": evt.commandLength,
        };
        if (typeof evt.exitCode === "number") {
          spanAttrs["oriro.exec.exit_code"] = evt.exitCode;
        }
        if (evt.exitSignal) {
          spanAttrs["oriro.exec.exit_signal"] = lowCardinalityAttr(evt.exitSignal, "other");
        }
        if (evt.timedOut !== undefined) {
          spanAttrs["oriro.exec.timed_out"] = evt.timedOut;
        }

        const span = spanWithDuration("oriro.exec", spanAttrs, evt.durationMs, {
          endTimeMs: evt.ts,
        });
      ...
      const recordHeartbeat = (
        ...
        queueDepthHistogram.record(evt.queued, { "oriro.channel": "heartbeat" });
      };
      ...
      const recordLivenessWarning = (
        ...
        const attrs = {
          "oriro.liveness.reason": lowCardinalityAttr(reason, "unknown"),
        };
        livenessWarningCounter.add(1, attrs);
        queueDepthHistogram.record(evt.queued, { "oriro.channel": "liveness" });
        ...
        const spanAttrs: Record<string, string | number> = {
          ...attrs,
          "oriro.liveness.active": evt.active,
          "oriro.liveness.waiting": evt.waiting,
          "oriro.liveness.queued": evt.queued,
          "oriro.liveness.interval_ms": evt.intervalMs,
          ...(evt.eventLoopDelayP99Ms !== undefined
            ? { "oriro.liveness.event_loop_delay_p99_ms": evt.eventLoopDelayP99Ms }
            : {}),
          ...(evt.eventLoopDelayMaxMs !== undefined
            ? { "oriro.liveness.event_loop_delay_max_ms": evt.eventLoopDelayMaxMs }
            : {}),
          ...(evt.eventLoopUtilization !== undefined
            ? { "oriro.liveness.event_loop_utilization": evt.eventLoopUtilization }
            : {}),
          ...(evt.cpuUserMs !== undefined
            ? { "oriro.liveness.cpu_user_ms": evt.cpuUserMs }
            : {}),
          ...(evt.cpuSystemMs !== undefined
            ? { "oriro.liveness.cpu_system_ms": evt.cpuSystemMs }
            : {}),
          ...(evt.cpuTotalMs !== undefined
            ? { "oriro.liveness.cpu_total_ms": evt.cpuTotalMs }
            : {}),
          ...(evt.cpuCoreRatio !== undefined
            ? { "oriro.liveness.cpu_core_ratio": evt.cpuCoreRatio }
            : {}),
        };
        const span = spanWithDuration("oriro.liveness.warning", spanAttrs, 0, {
          endTimeMs: evt.ts,
        });
        span.setStatus({
          code: SpanStatusCode.ERROR,
      ...
      const recordDiagnosticPhaseCompleted = (
        ...
        const spanAttrs: Record<string, string | number> = {
          "oriro.phase": lowCardinalityAttr(evt.name, "unknown"),
          ...(evt.cpuUserMs !== undefined ? { "oriro.phase.cpu_user_ms": evt.cpuUserMs } : {}),
          ...(evt.cpuSystemMs !== undefined
            ? { "oriro.phase.cpu_system_ms": evt.cpuSystemMs }
            : {}),
          ...(evt.cpuTotalMs !== undefined
            ? { "oriro.phase.cpu_total_ms": evt.cpuTotalMs }
            : {}),
          ...(evt.cpuCoreRatio !== undefined
            ? { "oriro.phase.cpu_core_ratio": evt.cpuCoreRatio }
            : {}),
        };
        for (const [key, value] of Object.entries(evt.details ?? {})) {
          spanAttrs[`oriro.phase.detail.${key}`] =
            typeof value === "boolean" ? String(value) : value;
        }
        const span = spanWithDuration("oriro.diagnostic.phase", spanAttrs, evt.durationMs, {
          endTimeMs: evt.ts,
        });
        span.end(evt.ts);
      };
      ...
      const recordTelemetryExporter = (
        ...
        telemetryExporterCounter.add(1, {
          "oriro.exporter": lowCardinalityAttr(evt.exporter, "unknown"),
          "oriro.signal": evt.signal,
          "oriro.status": evt.status,
          ...(evt.reason ? { "oriro.reason": evt.reason } : {}),
          ...(evt.errorCategory
            ? { "oriro.errorCategory": lowCardinalityAttr(evt.errorCategory, "other") }
            : {}),
        });
