...
describe("noteSessionLockHealth", () => {
  let state: OriroTestState;

  beforeEach(async () => {
    note.mockClear();
    state = await createOriroTestState({
      layout: "state-only",
      prefix: "oriro-doctor-locks-",
    });
  });
  ...
  it("uses configured stale threshold without removing live Oriro lock files", async () => {
    const sessionsDir = state.sessionsDir();
    await fs.mkdir(sessionsDir, { recursive: true });
  ...
  it("removes fresh live locks when the owner is not an Oriro process", async () => {
    const sessionsDir = state.sessionsDir();
    await fs.mkdir(sessionsDir, { recursive: true });
    ...
    expect(note).toHaveBeenCalledTimes(1);
    const [message] = firstNoteCall();
    expect(message).toContain("stale=yes (non-oriro-owner)");
    expect(message).toContain("[removed]");
    expect(message).toContain("Removed 1 stale session lock file");
