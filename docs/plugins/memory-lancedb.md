...
The plugin is published to npm and is not bundled into the Oriro runtime image.
...
Set `dimensions` for non-standard embedding models. Oriro knows the
...
`memory_forget` query path, and `oriro ltm search`. Auto-recall prefers the
...
```bash
oriro ltm list
oriro ltm search "project preferences"
oriro ltm stats
```
...
```bash
oriro ltm query --cols id,text,createdAt --limit 20
oriro ltm query --filter "category = 'preference'" --order-by createdAt:desc
```
...
By default, LanceDB data lives under `~/.oriro/memory/lancedb`. Override the
path with `dbPath`:
...
Oriro treats that package as part of the plugin package. Gateway startup
...
`@lancedb/lancedb` error during plugin load, upgrade Oriro and restart the
Gateway.
...
```bash
oriro ltm stats
oriro ltm search "recent preference"
```
