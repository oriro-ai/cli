---
summary: "CLI reference for `oriro nodes` (status, pairing, invoke, camera/canvas/screen)"
read_when:
...
```bash
oriro nodes list
oriro nodes list --connected
oriro nodes list --last-connected 24h
oriro nodes pending
oriro nodes approve <requestId>
oriro nodes reject <requestId>
oriro nodes remove --node <id|name|ip>
oriro nodes rename --node <id|name|ip> --name <displayName>
oriro nodes status
oriro nodes status --connected
oriro nodes status --last-connected 24h
```
...
- `oriro nodes pending` only needs pairing scope.
...
- `oriro nodes approve <requestId>` inherits extra scope requirements from the
  pending request:
...
```bash
oriro nodes invoke --node <id|name|ip> --command <command> --params <json>
```
...
For shell execution on a node, use the `exec` tool with `host=node` instead of `oriro nodes run`.
The `nodes` CLI is now capability-focused: direct RPC via `nodes invoke`, plus pairing, camera,
screen, location, Canvas, and notifications. Canvas commands are implemented by the bundled experimental Canvas plugin; core keeps a compatibility hook so they remain under `oriro nodes canvas`.
