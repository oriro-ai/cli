...
export function buildQaRuntimeEnv(params: {
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    ...(provider?.appliesLiveEnvAliases
      ...
      : {}),
    ORIRO_HOME: params.homeDir,
    ORIRO_CONFIG_PATH: params.configPath,
    ORIRO_STATE_DIR: params.stateDir,
    ORIRO_OAUTH_DIR: path.join(params.stateDir, "credentials"),
    ...
    ORIRO_NO_RESPAWN: "1",
    ORIRO_TEST_FAST: "1",
    ORIRO_EMBEDDED_ABORT_SETTLE_TIMEOUT_MS: "2000",
    ORIRO_QA_PARENT_PID: String(process.pid),
    ORIRO_QA_TEMP_ROOT: params.tempRoot,
    ...(params.stagedBundledPluginsRoot
      ? { ORIRO_QA_STAGED_RUNTIME_ROOT: params.stagedBundledPluginsRoot }
      : {}),
    ORIRO_QA_ALLOW_LOCAL_IMAGE_PROVIDER: "1",
    ...
    ORIRO_ALLOW_SLOW_REPLY_TESTS: "1",
    XDG_CONFIG_HOME: params.xdgConfigHome,
    XDG_DATA_HOME: params.xdgDataHome,
    XDG_CACHE_HOME: params.xdgCacheHome,
    ...(params.bundledPluginsDir ? { ORIRO_BUNDLED_PLUGINS_DIR: params.bundledPluginsDir } : {}),
    ...(params.compatibilityHostVersion
      ? { ORIRO_COMPATIBILITY_HOST_VERSION: params.compatibilityHostVersion }
      : {}),
  };
...
export async function startQaGatewayChild(params: {
  ...
  forwardHostHome?: boolean;
  mutateConfig?: (cfg: OriroConfig) => OriroConfig;
  runtimeEnvPatch?: NodeJS.ProcessEnv;
}) {
  const tempRoot = await fs.mkdtemp(
    path.join(resolvePreferredOriroTmpDir(), "oriro-qa-suite-"),
  );
  ...
  const keepTemp = process.env.ORIRO_QA_KEEP_TEMP === "1";
  let gatewayPort = 0;
  let baseUrl = "";
  let wsUrl = "";
