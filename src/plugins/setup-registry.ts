...
function collectConfiguredPluginEntryIds(config: OriroConfig): string[] {
...
function resolveRegister(mod: OriroPluginModule): {
...
function resolveSetupRegistration(record: PluginManifestRecord): {
  ...
  let mod: OriroPluginModule;
  try {
    mod = getModuleLoader(setupSource)(setupSource) as OriroPluginModule;
  } catch {
    return null;
  }

  const resolved = resolveRegister((mod as { default?: OriroPluginModule }).default ?? mod);
...
function pushDescriptorRuntimeDisabledDiagnostic(params: {
  ...
  params.diagnostics.push({
    ...
    message:
      "setup.requiresRuntime is false, so Oriro ignored the plugin setup runtime entry. Remove setup-api/oriro.setupEntry or set requiresRuntime true if setup lookup still needs plugin code.",
  });
}
