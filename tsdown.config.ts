...
const RUN_NODE_SKIP_DTS_BUILD = process.env.ORIRO_RUN_NODE_SKIP_DTS_BUILD === "1";
...
function buildInputOptions(options: InputOptionsArg): InputOptionsReturn {
  if (process.env.ORIRO_BUILD_VERBOSE === "1") {
    return undefined;
  }
...
const shouldBuildPrivateQaEntries = process.env.ORIRO_BUILD_PRIVATE_QA === "1";
...
function shouldAlwaysBundleDependency(id: string): boolean {
  return (
    id === "@oriro/fs-safe" ||
    id.startsWith("@oriro/fs-safe/") ||
    id === "@oriro/normalization-core" ||
    id.startsWith("@oriro/normalization-core/") ||
    id === "@oriro/media-core" ||
    id.startsWith("@oriro/media-core/") ||
    id === "@oriro/acp-core" ||
    id.startsWith("@oriro/acp-core/") ||
    id === "zod" ||
    id.startsWith("zod/")
  );
}
...
function shouldExternalizeGatewayClientDependency(id: string): boolean {
  return (
    id === "ws" ||
    id.startsWith("ws/") ||
    id === "@oriro/gateway-protocol" ||
    id.startsWith("@oriro/gateway-protocol/")
  );
}
...
function shouldExternalizeSpeechCoreDependency(id: string): boolean {
  return id === "oriro" || id.startsWith("oriro/");
}
...
function shouldExternalizeLlmRuntimeDependency(id: string): boolean {
  return id === "@oriro/llm-core" || id.startsWith("@oriro/llm-core/");
}
