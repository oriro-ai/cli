---
summary: "Schema-accurate configuration examples for common Oriro setups"
read_when:
  - Learning how to configure Oriro
...
Save to `~/.oriro-ai/cli.json` and you can DM the bot from that number.
...
```json5
{
  ...
  session: {
    ...
    store: "~/.oriro/agents/default/sessions/sessions.json",
    maintenance: {
      mode: "warn",
      pruneAfter: "30d",
      maxEntries: 500,
  ...
  gateway: {
    ...
    port: 18789,
    bind: "loopback",
    controlUi: { enabled: true, basePath: "/oriro" },
    auth: {
      mode: "token",
      token: "gateway-token",
      allowTailscale: true,
...
```json5
{
  agents: {
    ...
    list: [
      ...
      { id: "docs", workspace: "~/.oriro/workspace-docs", skills: ["docs-search"] },
    ],
  },
}
```
