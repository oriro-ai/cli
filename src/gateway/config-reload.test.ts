...
describe("buildGatewayReloadPlan", () => {
  ...
  it("treats plugin install timestamp-only changes as no-op", () => {
    const plan = buildGatewayReloadPlan([
      "plugins.installs.lossless-oriro.resolvedAt",
      "plugins.installs.lossless-oriro.installedAt",
    ]);
    expect(plan.restartGateway).toBe(false);
    ...
    expect(resolveConfigReloadMetadata("plugins.installs.lossless-oriro.resolvedAt").kind).toBe(
      "none",
    );
    expect(resolveConfigReloadMetadata("plugins.installs.lossless-oriro.installedAt").kind).toBe(
      "none",
    );
  });
  ...
  it("hot-reloads plugin entry config changes", () => {
    const plan = buildGatewayReloadPlan(["plugins.entries.lossless-oriro.config.mode"]);
    expect(plan.restartGateway).toBe(false);
    expect(plan.reloadPlugins).toBe(true);
    expect(plan.disposeMcpRuntimes).toBe(true);
    expect(plan.hotReasons).toContain("plugins.entries.lossless-oriro.config.mode");
  });
...
function createReloaderHarness(
  ...
  options: {
    initialCompareConfig?: OriroConfig;
  ...
  const onHotReload = vi.fn(async (_plan: GatewayReloadPlan, _nextConfig: OriroConfig) => {});
  const onRestart = vi.fn((_plan: GatewayReloadPlan, _nextConfig: OriroConfig) => {});
...
function getOnlyRestartCall(harness: ReloaderHarness): [GatewayReloadPlan, OriroConfig] {
...
function getOnlyHotReloadCall(harness: ReloaderHarness): [GatewayReloadPlan, OriroConfig] {
...
describe("startGatewayConfigReloader", () => {
  ...
  it("skips plugin-local invalid reloads without degraded mode", async () => {
    const activeConfig: OriroConfig = {
      gateway: { reload: { debounceMs: 0 } },
    ...
    const invalidSnapshot = makeSnapshot({
      ...
      issues: [
        {
          path: "plugins.entries.lossless-oriro.config.cacheAwareCompaction",
    ...
    const promoteSnapshot = vi.fn(async (_snapshot: ConfigFileSnapshot, _reason: string) => true);
    const previousConfig: OriroConfig = {
      ...activeConfig,
      plugins: {
        entries: {
          "lossless-oriro": {
    ...
    expect(
      log.warn.mock.calls.some(([message]) =>
        message.includes(
          "config reload skipped (invalid config): plugins.entries.lossless-oriro.config.cacheAwareCompaction:",
        ),
      ),
    ).toBe(true);
  ...
  it("does not suppress functional install changes that collide with timestamp paths", async () => {
    ...
    const nextSourceConfig: OriroConfig = {
      gateway: { reload: { debounceMs: 0 } },
      plugins: {
        installs: {
          lossless: {
  ...
  it("queues restart when an external plugin source write only changes the managed index", async () => {
    ...
    const readPluginInstallRecords = vi.fn().mockResolvedValueOnce({
      "lossless-oriro": {
        source: "npm",
        spec: "@martian-engineering/lossless-oriro",
        installPath: "/tmp/oriro/plugins/lossless-oriro",
        installedAt: "2026-04-22T00:00:00.000Z",
      },
    ...
    expect(harness.onHotReload).not.toHaveBeenCalled();
    const [plan, nextConfig] = getOnlyRestartCall(harness);
    expect(plan.changedPaths).toEqual(["plugins.installs.lossless-oriro"]);
    expect(plan.restartGateway).toBe(true);
    expect(plan.restartReasons).toEqual(["plugins.installs.lossless-oriro"]);
  ...
  it("keeps external plugin policy-only writes on the hot reload path", async () => {
    ...
    const installRecords = {
      telegram: {
        source: "npm",
        spec: "@oriro/telegram",
        installPath: "/tmp/oriro/plugins/telegram",
      },
    } satisfies Record<string, PluginInstallRecord>;
    const readSnapshot = vi.fn<() => Promise<ConfigFileSnapshot>>().mockResolvedValueOnce(
      makeSnapshot({
  ...
  it("queues restart when an external plugin source write also changes plugin config", async () => {
    ...
    expect(plan.changedPaths).toEqual(["plugins.entries", "plugins.installs.lossless-oriro"]);
    expect(plan.restartGateway).toBe(true);
    expect(plan.restartReasons).toEqual(["plugins.installs.lossless-oriro"]);
