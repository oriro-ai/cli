...
export OPENAI_API_KEY="sk-oriro-release-media-memory"
export ORIRO_QA_ALLOW_LOCAL_IMAGE_PROVIDER=1

PORT="18789"
MOCK_PORT="44200"
SUCCESS_MARKER="ORIRO_E2E_OK_MEDIA_MEMORY"
MEMORY_MARKER="release-media-memory-saffron-$(date +%s)"
media_root="$(mktemp -d /tmp/oriro-release-media-memory.XXXXXX)"
...
entry="$(oriro_e2e_package_entrypoint "$package_root")"
{
  ...
  printf 'HOME=%s\n' "$HOME"
  printf 'ORIRO_HOME=%s\n' "$ORIRO_HOME"
  printf 'ORIRO_STATE_DIR=%s\n' "$ORIRO_STATE_DIR"
  printf 'ORIRO_CONFIG_PATH=%s\n' "$ORIRO_CONFIG_PATH"
} >"$ENV_LOG"
...
mock_pid="$(oriro_e2e_start_mock_openai "$MOCK_PORT" "$MOCK_OPENAI_LOG")"
...
cp "$ORIRO_CONFIG_PATH" "$CONFIG_JSON"
oriro plugins list --json >"$PLUGINS_JSON" 2>"$PLUGINS_STDERR_LOG"
...
mkdir -p "$ORIRO_STATE_DIR/workspace/memory"
...
cat >"$ORIRO_STATE_DIR/workspace/MEMORY.md" <<EOF
# Long-term memory

- The release media memory marker is $MEMORY_MARKER.
EOF

oriro memory index --force >"$INDEX_LOG" 2>&1
oriro memory search "$MEMORY_MARKER" --json >"$SEARCH_BEFORE_JSON" 2>"$SEARCH_BEFORE_STDERR_LOG"
...
oriro memory search "$MEMORY_MARKER" --json >"$SEARCH_AFTER_JSON" 2>"$SEARCH_AFTER_STDERR_LOG"
