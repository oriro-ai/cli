...
read_when:
  - You are building an Oriro plugin
  - You need to ship a plugin config schema or debug plugin validation errors
title: "Plugin manifest"
---

This page is for the **native Oriro plugin manifest** only.
...
Oriro auto-detects those bundle layouts too, but they are not validated
against the `oriro.plugin.json` schema described here.

For compatible bundles, Oriro currently reads bundle metadata plus declared
...
Oriro runtime expectations.

Every native Oriro plugin **must** ship a `oriro.plugin.json` file in the
**plugin root**. Oriro uses this manifest to validate configuration
...
`oriro.plugin.json` is the metadata Oriro reads **before it loads your
...
- QA runner metadata the shared `oriro qa` host can inspect
...
| `providerAuthEnvVars`                | No       | `Record<string, string[]>`       | Deprecated compatibility env metadata for provider auth/status lookup. Prefer `setup.providers[].envVars` for new plugins; Oriro still reads this during the deprecation window.                                                             |
| `providerAuthAliases`                | No       | `Record<string, string>`         | Provider ids that should reuse another provider id for auth lookup, for example a coding provider that shares the base provider API key and auth profiles.                                                                                      |
| `channelEnvVars`                     | No       | `Record<string, string[]>`       | Cheap channel env metadata that Oriro can inspect without loading plugin code. Use this for env-driven channel setup or auth surfaces that generic startup/config helpers should see.                                                        |
...
| `qaRunners`                          | No       | `object[]`                       | Cheap QA runner descriptors used by the shared `oriro qa` host before plugin runtime loads.                                                                                                                                                  |
...
Oriro reads these fields before provider runtime loads so core tools can
...
ownership. `toolMetadata` declares cheap availability evidence so Oriro can
...
If a tool has no `toolMetadata`, Oriro preserves the existing behavior and
...
Oriro reads this before provider runtime loads.
...
| `choiceLabel`         | No       | `string`                                                              | User-facing label. If omitted, Oriro falls back to `choiceId`.                                        |
...
mistakenly put in `plugins.allow` or try to run as a root CLI command. Oriro
...
the shared `oriro qa` root. Keep this metadata cheap and static; the plugin
...
| Field         | Required | Type     | What it means                                                      |
| ------------- | -------- | -------- | ------------------------------------------------------------------ |
| `commandName` | Yes      | `string` | Subcommand mounted beneath `oriro qa`, for example `matrix`.    |
...
Oriro also includes `setup.providers[].envVars` in generic provider auth and
...
Oriro can also derive simple setup choices from `setup.providers[].authMethods`
...
setup surface. Oriro treats explicit `false` as a descriptor-only contract
and will not execute `setup-api` or `oriro.setupEntry` for setup lookup. If
a descriptor-only plugin still ships one of those setup runtime entries,
Oriro reports an additive diagnostic and continues ignoring it. Omitted
...
Use `contracts` only for static capability ownership metadata that Oriro can
read without importing the plugin runtime.

```json
{
  "contracts": {
    "agentToolResultMiddleware": ["oriro", "codex"],
...
| `migrationProviders`             | `string[]` | Import provider ids this plugin owns for `oriro migrate`.                                                                         |
...
Oriro reads manifest metadata to decide which plugin owns that configured
...
`channelConfigs` entries. Without them, Oriro can still load the plugin, but
...
the same defaults through `package.json#oriro.channel.commands` alongside
...
When `channels.chat` is configured, Oriro considers both the channel id and
the preferred plugin id. If the lower-priority plugin was only selected because
it is bundled or enabled by default, Oriro disables it in the effective
runtime config so one plugin owns the channel and its tools. Explicit user
selection still wins: if the user explicitly enables both plugins, Oriro
...
Use `modelSupport` when Oriro should infer your provider plugin from
...
Use `modelCatalog` when Oriro should know provider model metadata before
...
provider-filtered list uses an alias, Oriro can read the owning manifest and
...
| `compat`        | `object`                                                       | Optional compatibility flags matching Oriro model config compatibility.  |
...
refreshable rows are not authoritative by themselves. Use `runtime` when Oriro
...
exec provider preset. Oriro reads this metadata before plugin runtime loads,
...
from the managed plugin install roots, such as git and OriroHub installs.
...
The map key is the integration id. If `providerAlias` is omitted, Oriro uses
...
When an operator selects the preset, Oriro writes a provider reference like:
...
At startup/reload, Oriro resolves that provider by loading current plugin
...
Oriro materializes it at startup/reload to the current Node executable and
...
Oriro derives `trustedDirs` for manifest presets from the plugin root and,
...
| Field                      | Type               | What it means                                                                                                        |
| -------------------------- | ------------------ | -------------------------------------------------------------------------------------------------------------------- |
| `provider`                 | `string`           | External catalog provider id when it differs from the Oriro provider id, for example `z-ai` for a `zai` provider. |
...
### Oriro Provider Index

The Oriro Provider Index is Oriro-owned preview metadata for providers
...
4. Oriro Provider Index preview rows.
...
Legacy top-level capability keys are deprecated. Use `oriro doctor --fix` to
...
| File                   | Use it for                                                                                                                       |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `oriro.plugin.json` | Discovery, config validation, auth-choice metadata, and UI hints that must exist before plugin code runs                         |
| `package.json`         | npm metadata, dependency installation, and the `oriro` block used for entrypoints, install gating, setup, or catalog metadata |

If you are unsure where a piece of metadata belongs, use this rule:

- if Oriro must know it before loading plugin code, put it in `oriro.plugin.json`
...
`oriro` block instead of `oriro.plugin.json`.
`oriro.bundle` and `oriro.bundle.json` are not Oriro plugin contracts;
native plugins must use `oriro.plugin.json` plus the supported
`package.json#oriro` fields below.

Important examples:

| Field                                                                                      | What it means                                                                                                                                                                        |
| ------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `oriro.extensions`                                                                      | Declares native plugin entrypoints. Must stay inside the plugin package directory.                                                                                                   |
| `oriro.runtimeExtensions`                                                               | Declares built JavaScript runtime entrypoints for installed packages. Must stay inside the plugin package directory.                                                                 |
| `oriro.setupEntry`                                                                      | Lightweight setup-only entrypoint used during onboarding, deferred channel startup, and read-only channel status/SecretRef discovery. Must stay inside the plugin package directory. |
| `oriro.runtimeSetupEntry`                                                               | Declares the built JavaScript setup entrypoint for installed packages. Requires `setupEntry`, must exist, and must stay inside the plugin package directory.                         |
| `oriro.channel`                                                                         | Cheap channel catalog metadata like labels, docs paths, aliases, and selection copy.                                                                                                 |
| `oriro.channel.commands`                                                                | Static native command and native skill auto-default metadata used by config, audit, and command-list surfaces before channel runtime loads.                                          |
| `oriro.channel.configuredState`                                                         | Lightweight configured-state checker metadata that can answer "does env-only setup already exist?" without loading the full channel runtime.                                         |
| `oriro.channel.persistedAuthState`                                                      | Lightweight persisted-auth checker metadata that can answer "is anything already signed in?" without loading the full channel runtime.                                               |
| `oriro.install.orirohubSpec` / `oriro.install.npmSpec` / `oriro.install.localPath` | Install/update hints for bundled and externally published plugins.                                                                                                                   |
| `oriro.install.defaultChoice`                                                           | Preferred install path when multiple install sources are available.                                                                                                                  |
| `oriro.install.minHostVersion`                                                          | Minimum supported Oriro host version, using a semver floor like `>=2026.3.22` or `>=2026.5.1-beta.1`.                                                                             |
| `oriro.compat.pluginApi`                                                                | Minimum Oriro plugin API range required by this package, using a semver floor like `>=2026.5.27`.                                                                                 |
| `oriro.install.expectedIntegrity`                                                       | Expected npm dist integrity string such as `sha512-...`; install and update flows verify the fetched artifact against it.                                                            |
| `oriro.install.allowInvalidConfigRecovery`                                              | Allows a narrow bundled-plugin reinstall recovery path when config is invalid.                                                                                                       |
| `oriro.install.requiredPlatformPackages`                                                | npm package aliases that must materialize when their lockfile platform constraints match the current host.                                                                           |
| `oriro.startup.deferConfiguredChannelFullLoadUntilAfterListen`                          | Lets setup-runtime channel surfaces load before listen, then defers the full configured channel plugin until post-listen activation.                                                 |

Manifest metadata decides which provider/channel/setup choices appear in
onboarding before runtime loads. `package.json#oriro.install` tells
onboarding how to fetch or enable that plugin when the user picks one of those
choices. Do not move install hints into `oriro.plugin.json`.

`oriro.install.minHostVersion` is enforced during install and manifest
...
`oriro.install.requiredPlatformPackages` is for npm packages that expose
...
Oriro verifies only the declared alias whose lockfile constraints match the
current host. If npm reports success but omits that alias, Oriro retries once
with a fresh cache and rolls back the install if the alias is still missing.

`oriro.compat.pluginApi` is enforced during package install for non-bundled
plugin sources. Use it for the Oriro plugin SDK/runtime API floor that the
...
flows. Official Oriro release sync bumps existing official plugin API floors
to the Oriro release version by default, but plugin-only releases can keep a
lower floor when the package intentionally supports older hosts. Do not use the
package version alone as the compatibility contract. `peerDependencies.oriro`
remains npm package metadata; Oriro uses the `oriro.compat.pluginApi`
contract for install compatibility decisions.

Official install-on-demand metadata should use `orirohubSpec` when the plugin is
published on OriroHub; onboarding treats that as the preferred remote source and
records OriroHub artifact facts after install. `npmSpec` remains the compatibility
fallback for packages that have not moved to OriroHub yet.

Exact npm version pinning already lives in `npmSpec`, for example
`"npmSpec": "@wecom/wecom-oriro-plugin@1.2.3"`. Official external catalog
...
Channel plugins should provide `oriro.setupEntry` when status, channel list,
...
entrypoint fields. For example, `oriro.runtimeExtensions` cannot make an
escaping `oriro.extensions` path loadable.

`oriro.install.allowInvalidConfigRecovery` is intentionally narrow. It does
...
`oriro.channel.persistedAuthState` is package metadata for a tiny checker
module:
...
`oriro.channel.configuredState` follows the same shape for cheap env-only
configured checks:
...
Oriro discovers plugins from several roots. For the raw filesystem scan
...
2. **Bundled** — plugins shipped with Oriro
3. **Global install** — plugins installed into the global Oriro plugin root
...
- When extending or forking a bundled plugin with new config keys, update that plugin's `oriro.plugin.json` `configSchema` at the same time. Bundled plugin schemas are strict, so adding `plugins.entries.<id>.config.myNewKey` in user config without adding `myNewKey` to `configSchema.properties` will be rejected before the plugin runtime loads.
...
- The manifest is **required for native Oriro plugins**, including local filesystem loads. Runtime still loads the plugin module separately; the manifest is only for discovery + validation.
...
- Declare exclusive plugin kind in this manifest. Runtime-entry `OriroPluginDefinition.kind` is deprecated and remains only as a compatibility fallback for older plugins.
