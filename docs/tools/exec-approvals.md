...
| ---------------------------------------------------------------- | -------------------------------------------------------------------------------------- |
| `oriro approvals get` / `--gateway` / `--node <id\|name\|ip>` | Requested policy, host policy sources, and the effective result.                       |
| `oriro exec-policy show`                                      | Local-machine merged view.                                                             |
| `oriro exec-policy set` / `preset`                            | Synchronize the local requested policy with the local host approvals file in one step. |
...
- **Gateway host** → `oriro` process on the gateway machine.
...
- For shell scripts and direct interpreter/runtime file invocations, Oriro also tries to bind one concrete local file operand. If that bound file changes after approval but before execution, the run is denied instead of executing drifted content.
...
`ORIRO_STATE_DIR` is set, the file follows that state directory;
otherwise it uses the default Oriro state directory:

```text
$ORIRO_STATE_DIR/exec-approvals.json
# otherwise
~/.oriro/exec-approvals.json
```

The default approval socket follows the same root:
`$ORIRO_STATE_DIR/exec-approvals.sock`, or
`~/.oriro/exec-approvals.sock` when the variable is unset.
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.oriro/exec-approvals.sock",
...
- `auto` - use allowlist policy, run deterministic matches directly, and send approval misses through Oriro's native auto reviewer before falling back to a human approval route.
...
<ParamField path="askFallback" type='"deny" | "allowlist" | "full"'>
  ...
  field is omitted, Oriro defaults to `deny`.
...
<ParamField path="strictInlineEval" type="boolean">
  When `true`, Oriro treats inline code-eval forms as approval-only
...
<ParamField path="commandHighlighting" type="boolean" default="false">
  ...
  Oriro may attach parser-derived command spans so Web approval
...
**both** policy layers - requested exec policy in Oriro config
(`tools.exec.*`) **and** host-local approvals policy in
the execution host approvals file.

Oriro defaults omitted `askFallback` to `deny`. Set host
...
- In YOLO mode, Oriro does **not** add a separate heuristic command-obfuscation approval gate or script-preflight rejection layer on top of the configured host exec policy.
...
`--permission-mode bypassPermissions` when Oriro's effective exec
policy is YOLO. For Oriro-managed Claude live sessions, Oriro's
...
If you want a more conservative setup, tighten Oriro exec policy back to
...
approvals remotely, use `oriro approvals set --gateway` or
`oriro approvals set --node <id|name|ip>`.
...
- `oriro exec-policy` does not synchronize node approvals.
- `oriro exec-policy set --host node` is rejected.
- Node exec approvals are fetched from the node at runtime, so node-targeted updates must use `oriro approvals --node ...`.
...
specific argument shape. Oriro evaluates the regular expression
...
entries instead of hand-editing the encoded value. If Oriro cannot
...
CLI: `oriro approvals` supports gateway or node editing - see
...
Oriro posts the denial back into that session as an internal followup so the
agent can stop waiting on the async command and avoid a missing-result repair.
If there is no session or the session cannot be resumed, Oriro can still
...
When an async exec approval is denied, Oriro treats the host command as
...
session delivery is unavailable, Oriro falls back to a concise operator or
