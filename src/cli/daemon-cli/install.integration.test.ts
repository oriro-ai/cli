...
describe("runDaemonInstall integration", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await makeTempWorkspace("oriro-daemon-install-int-");
    configPath = path.join(tempHome, "oriro.json");
    process.env.HOME = tempHome;
    process.env.ORIRO_STATE_DIR = tempHome;
  ...
  it("refuses service install when config was written by a newer Oriro", async () => {
    await fs.writeFile(
      configPath,
      JSON.stringify(
        {
  ...
  it("auto-mints token when no source exists without embedding it into service env", async () => {
    ...
    expect(installEnv?.ORIRO_GATEWAY_TOKEN).toBeUndefined();
  });
});
