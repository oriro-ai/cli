...
import { resolveStateDir, type OriroConfig } from "../config/config.js";
...
/** Stable finding codes emitted by `oriro secrets audit`. */
...
export async function runSecretsAudit(
  ...
  const config = snapshot.valid ? snapshot.config : ({} as OriroConfig);
  let resolution = {
    refsChecked: 0,
    skippedExecRefs: 0,
