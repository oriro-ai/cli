...
import { callGatewayFromCli } from "oriro/plugin-sdk/gateway-runtime";
...
import type { RuntimeLogger } from "oriro/plugin-sdk/plugin-runtime";
...
function meetStatusScript(params: {
  ...
  const captionState = (() => {
    ...
    const w = window;
    if (!inCall && !w.__oriroMeetCaptions) return undefined;
    if (!w.__oriroMeetCaptions) {
      w.__oriroMeetCaptions = {
