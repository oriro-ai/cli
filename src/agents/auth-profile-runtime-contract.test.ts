...
function providerRuntimeConfig(provider: string, runtime: string): OriroConfig {
  return {
    models: {
      providers: {
        [provider]: {
          baseUrl: "https://api.oriro.test/v1",
...
async function runAuthContractAttempt(params: {
  ...
  const cfg = params.cfg ?? ({} as OriroConfig);
...
describe("Auth profile runtime contract - embedded Oriro and CLI adapter", () => {
  let tmpDir: string;
  let storePath: string;

  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-auth-contract-"));
  ...
  it("forwards a legacy OpenAI Codex auth profile through the embedded Oriro path", async () => {
    await runAuthContractAttempt({
      tmpDir,
      storePath,
  ...
  it("forwards an OpenAI auth profile through the explicit embedded OpenAI Oriro path", async () => {
    await runAuthContractAttempt({
      tmpDir,
      storePath,
      ...
      authProfileProvider: AUTH_PROFILE_RUNTIME_CONTRACT.openAiProvider,
      authProfileOverride: AUTH_PROFILE_RUNTIME_CONTRACT.openAiProfileId,
      cfg: providerRuntimeConfig(AUTH_PROFILE_RUNTIME_CONTRACT.openAiProvider, "oriro"),
    });
  ...
  it("routes explicit OpenAI Oriro runs with legacy Codex OAuth through OpenAI transport", async () => {
    await runAuthContractAttempt({
      tmpDir,
      storePath,
      providerOverride: AUTH_PROFILE_RUNTIME_CONTRACT.openAiProvider,
