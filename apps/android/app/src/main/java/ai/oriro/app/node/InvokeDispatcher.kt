...
class InvokeDispatcher(
  ...
  suspend fun handleInvoke(
    ...
    // Command strings come from OriroProtocolConstants; the registry above owns advertised availability.
    return when (command) {
      // Canvas commands
      ...
      OriroCanvasCommand.Hide.rawValue -> GatewaySession.InvokeResult.ok(null)
      OriroCanvasCommand.Navigate.rawValue -> {
        val url = CanvasController.parseNavigateUrl(paramsJson)
        canvas.navigate(url)
        GatewaySession.InvokeResult.ok(null)
      }
      OriroCanvasCommand.Eval.rawValue -> {
        val js =
      ...
      OriroCanvasA2UICommand.Push.rawValue, OriroCanvasA2UICommand.PushJSONL.rawValue -> {
        val messages =
          try {
      ...
      // Camera commands
      OriroCameraCommand.List.rawValue -> cameraHandler.handleList(paramsJson)
      OriroCameraCommand.Snap.rawValue -> cameraHandler.handleSnap(paramsJson)
      OriroCameraCommand.Clip.rawValue -> cameraHandler.handleClip(paramsJson)

      // Location command
      OriroLocationCommand.Get.rawValue -> locationHandler.handleLocationGet(paramsJson)

      // Device commands
      OriroDeviceCommand.Status.rawValue -> deviceHandler.handleDeviceStatus(paramsJson)
      OriroDeviceCommand.Info.rawValue -> deviceHandler.handleDeviceInfo(paramsJson)
      OriroDeviceCommand.Permissions.rawValue -> deviceHandler.handleDevicePermissions(paramsJson)
      OriroDeviceCommand.Health.rawValue -> deviceHandler.handleDeviceHealth(paramsJson)
      OriroDeviceCommand.Apps.rawValue -> deviceHandler.handleDeviceApps(paramsJson)

      // Notifications command
      OriroNotificationsCommand.List.rawValue -> notificationsHandler.handleNotificationsList(paramsJson)
      OriroNotificationsCommand.Actions.rawValue -> notificationsHandler.handleNotificationsActions(paramsJson)

      // System command
      OriroSystemCommand.Notify.rawValue -> systemHandler.handleSystemNotify(paramsJson)

      // Talk commands
      OriroTalkCommand.PttStart.rawValue -> talkHandler.handlePttStart(paramsJson)
      OriroTalkCommand.PttStop.rawValue -> talkHandler.handlePttStop(paramsJson)
      OriroTalkCommand.PttCancel.rawValue -> talkHandler.handlePttCancel(paramsJson)
      OriroTalkCommand.PttOnce.rawValue -> talkHandler.handlePttOnce(paramsJson)

      // Photos command
      ai.oriro.app.protocol.OriroPhotosCommand.Latest.rawValue ->
        photosHandler.handlePhotosLatest(
          paramsJson,
        )

      // Contacts command
      OriroContactsCommand.Search.rawValue -> contactsHandler.handleContactsSearch(paramsJson)
      OriroContactsCommand.Add.rawValue -> contactsHandler.handleContactsAdd(paramsJson)

      // Calendar command
      OriroCalendarCommand.Events.rawValue -> calendarHandler.handleCalendarEvents(paramsJson)
      OriroCalendarCommand.Add.rawValue -> calendarHandler.handleCalendarAdd(paramsJson)

      // Motion command
      OriroMotionCommand.Activity.rawValue -> motionHandler.handleMotionActivity(paramsJson)
      OriroMotionCommand.Pedometer.rawValue -> motionHandler.handleMotionPedometer(paramsJson)

      // SMS command
      OriroSmsCommand.Send.rawValue -> smsHandler.handleSmsSend(paramsJson)
      OriroSmsCommand.Search.rawValue -> smsHandler.handleSmsSearch(paramsJson)

      // CallLog command
      OriroCallLogCommand.Search.rawValue -> callLogHandler.handleCallLogSearch(paramsJson)
