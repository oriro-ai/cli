...
This is handy when you need to toggle obscure settings without editing `oriro.json`.

Examples:

```
/debug show
/debug set messages.responsePrefix="[oriro]"
...
Use `ORIRO_PLUGIN_LIFECYCLE_TRACE=1` when plugin lifecycle commands feel slow
...
```bash
ORIRO_PLUGIN_LIFECYCLE_TRACE=1 oriro plugins install tokenjuice --force
```
...
runtime with `node dist/entry.js ...` after `pnpm build`; `pnpm oriro ...`
...
`ORIRO_RUN_NODE_CPU_PROF_DIR`:

```bash
ORIRO_RUN_NODE_CPU_PROF_DIR=.artifacts/cli-cpu pnpm oriro status
```
...
```bash
ORIRO_TRACE_SYNC_IO=1 pnpm oriro gateway --force
```

`pnpm gateway:watch` leaves this flag disabled by default for the watched
Gateway child. Set `ORIRO_TRACE_SYNC_IO=1` when you explicitly want Node
...
`oriro-gateway-watch-main` (or a profile/port-specific variant such as
`oriro-gateway-watch-dev-19001`) and auto-attaches from interactive terminals.
...
```bash
tmux attach -t oriro-gateway-watch-main
```
...
# or
ORIRO_GATEWAY_WATCH_TMUX=0 pnpm gateway:watch
```

Disable auto-attach while keeping tmux management:

```bash
ORIRO_GATEWAY_WATCH_ATTACH=0 pnpm gateway:watch
```
...
`ORIRO_TRACE_SYNC_IO=1` with `--benchmark` when you explicitly want both CPU
...
`ORIRO_PROFILE`, `ORIRO_CONFIG_PATH`, `ORIRO_STATE_DIR`,
`ORIRO_GATEWAY_PORT`, and `ORIRO_SKIP_CHANNELS` into the pane. Put
...
`oriro doctor --fix --non-interactive` once and restarts the Gateway child.
Use `ORIRO_GATEWAY_WATCH_AUTO_DOCTOR=0` when you want the original startup
...
extension `package.json` and `oriro.plugin.json` metadata, `tsconfig.json`,
...
- **Global `--dev` (profile):** isolates state under `~/.oriro-dev` and
...
```bash
pnpm gateway:dev
ORIRO_PROFILE=dev oriro tui
```

If you don't have a global install yet, run the CLI via `pnpm oriro ...`.

What this does:

1. **Profile isolation** (global `--dev`)
   - `ORIRO_PROFILE=dev`
   - `ORIRO_STATE_DIR=~/.oriro-dev`
   - `ORIRO_CONFIG_PATH=~/.oriro-dev/oriro.json`
   - `ORIRO_GATEWAY_PORT=19001` (browser/canvas shift accordingly)
...
2. **Dev bootstrap** (`gateway --dev`)
   ...
   - Skips channel providers in dev mode (`ORIRO_SKIP_CHANNELS=1`).
...
```bash
ORIRO_PROFILE=dev oriro gateway --dev --reset
```
...
## Raw stream logging (Oriro)

Oriro can log the **raw assistant stream** before any filtering/formatting.
...
```bash
pnpm gateway:watch --raw-stream --raw-stream-path ~/.oriro/logs/raw-stream.jsonl
```
...
```bash
ORIRO_RAW_STREAM=1
ORIRO_RAW_STREAM_PATH=~/.oriro/logs/raw-stream.jsonl
```

Default file:

`~/.oriro/logs/raw-stream.jsonl`
...
```bash
ORIRO_RAW_STREAM_PATH=~/.oriro/logs/raw-openai-completions.jsonl
```

Default file:

`~/.oriro/logs/raw-openai-completions.jsonl`
...
- If you need to use the built Oriro CLI for other tasks (i.e. `dashboard --no-open` if your debug session spawns a new auth token), you can execute it in another terminal as `node ./oriro.mjs` or create a shell alias like `alias oriro-build="node $(pwd)/oriro.mjs"`
