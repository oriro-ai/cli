...
private func historyPayload(
    ...
    messages: [AnyCodable] = []) -> OriroChatHistoryPayload
{
    OriroChatHistoryPayload(
...
private func sessionEntry(key: String, updatedAt: Double) -> OriroChatSessionEntry {
...
private func thinkingOption(_ id: String, label: String? = nil) -> OriroChatThinkingLevelOption {
    OriroChatThinkingLevelOption(id: id, label: label ?? id)
}
...
private func sessionEntry(
    ...
    updatedAt: Double,
    model: String?,
    modelProvider: String? = nil) -> OriroChatSessionEntry
{
    OriroChatSessionEntry(
        key: key,
        kind: nil,
...
private func modelChoice(id: String, name: String, provider: String = "anthropic") -> OriroChatModelChoice {
    OriroChatModelChoice(modelID: id, name: name, provider: provider, contextWindow: nil)
}

private func makeViewModel(
    sessionKey: String = "main",
    historyResponses: [OriroChatHistoryPayload],
    sessionsResponses: [OriroChatSessionsListResponse] = [],
    modelResponses: [[OriroChatModelChoice]] = [],
    ...
    -> (TestChatTransport, OriroChatViewModel)
{
...
private func sendUserMessage(_ vm: OriroChatViewModel, text: String = "hi") async {
    await MainActor.run {
        vm.input = text
        vm.send()
    }
...
private final class TestChatTransport: @unchecked Sendable, OriroChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [OriroChatHistoryPayload]
    private let sessionsResponses: [OriroChatSessionsListResponse]
    private let modelResponses: [[OriroChatModelChoice]]
    ...
    private let stream: AsyncStream<OriroChatTransportEvent>
    private let continuation: AsyncStream<OriroChatTransportEvent>.Continuation
        ...
        healthResponses: [Bool] = [true])
    {
        ...
        var cont: AsyncStream<OriroChatTransportEvent>.Continuation!
        ...
        parentSessionKey: String?) async throws -> OriroChatCreateSessionResponse
    {
        ...
        return OriroChatCreateSessionResponse(ok: true, key: key, sessionId: "created-\(key)")
    }
    ...
    func requestHistory(sessionKey: String) async throws -> OriroChatHistoryPayload {
        ...
        return self.historyResponses.last ?? OriroChatHistoryPayload(
            sessionKey: sessionKey,
            sessionId: nil,
            messages: [],
            thinkingLevel: "off")
        ...
        attachments _: [OriroChatAttachmentPayload]) async throws -> OriroChatSendResponse
    {
        ...
        return OriroChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> OriroChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? OriroChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: OriroChatTransportEvent) {
...
struct ChatViewModelTests {
    @Test func `displays error message fallback only for assistant error turns`() throws {
        func decodeMessage(role: String, stopReason: String, contentText: String? = nil) throws -> OriroChatMessage {
        ...
        #expect(
            OriroChatMessage.errorDisplayText(
    ...
    @Test @MainActor func `bounded repeated same text reply invalidates older stale refresh`() async throws {
        ...
        transport.emit(OriroChatTransportEvent.seqGap)
    ...
    @Test func `appends external session user message for active session`() async throws {
        ...
        transport.emit(
            .sessionMessage(
                OriroSessionMessageEventPayload(
                    sessionKey: "agent:aiden:main",
                    message: OriroChatMessage(
                        role: "user",
                        content: [
    ...
    @Test func `session choices prefer main and recent`() async throws {
        ...
        let sessions = OriroChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func `bootstraps model selection from session and defaults`() async throws {
        ...
        let sessions = OriroChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: OriroChatSessionsDefaults(model: "openai/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func `selecting default model patches nil and updates selection`() async throws {
        ...
        await MainActor.run { vm.selectModel(OriroChatViewModel.defaultModelSelectionID) }
        ...
        #expect(await MainActor.run { vm.modelSelectionID } == OriroChatViewModel.defaultModelSelectionID)
    }
    ...
    @Test func `selecting provider qualified model disambiguates duplicate model I ds`() async throws {
        ...
        let sessions = OriroChatSessionsListResponse(
            ...
            path: nil,
            count: 1,
            defaults: OriroChatSessionsDefaults(model: "openrouter/gpt-4.1-mini", contextTokens: nil),
            sessions: [
    ...
    @Test func `late model completion does not replay current session selection into previous session`() async throws {
        let now = Date().timeIntervalSince1970 * 1000
        let initialSessions = OriroChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
        ...
        let sessionsAfterOtherSelection = OriroChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 2,
            defaults: nil,
    ...
    @Test func `explicit thinking level wins over history and persists changes`() async throws {
        let history = OriroChatHistoryPayload(
            sessionKey: "main",
            sessionId: "sess-main",
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func `decodes gateway thinking metadata from session list`() throws {
        ...
        let decoded = try JSONDecoder().decode(
            OriroChatSessionsListResponse.self,
            from: Data(json.utf8))
    ...
    @Test func `strips inbound metadata from history messages`() async throws {
        ...
        let vm = await MainActor.run { OriroChatViewModel(sessionKey: "main", transport: transport) }
