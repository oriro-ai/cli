---
summary: "Delegate architecture: running Oriro as a named agent on behalf of an organization"
title: Delegate architecture
read_when: "You want an agent with its own identity that acts on behalf of humans in an organization."
status: active
---

Goal: run Oriro as a **named delegate** - an agent with its own identity that acts "on behalf of" people in an organization. The agent never impersonates a human. It sends, reads, and schedules under its own account with explicit delegation permissions.

This extends [Multi-Agent Routing](/concepts/multi-agent) from personal use into organizational deployments.

## What is a delegate?

A **delegate** is an Oriro agent that:
...
Oriro's default mode is a **personal assistant** - one human, one agent. Delegates extend this to organizations:
...
2. **Scope control**: the identity provider enforces what the delegate can access, independent of Oriro's own tool policy.
...
{
  id: "delegate",
  workspace: "~/.oriro/workspace-delegate",
  tools: {
...
- Cron run history: Oriro shared SQLite state database
- Session transcripts: `~/.oriro/agents/delegate/sessions`
- Identity provider audit logs (Exchange, Google Workspace)

All delegate actions flow through Oriro's session store. For compliance, ensure these logs are retained and reviewed.
...
```bash
oriro agents add delegate
```

This creates:

- Workspace: `~/.oriro/workspace-delegate`
- State: `~/.oriro/agents/delegate/agent`
- Sessions: `~/.oriro/agents/delegate/sessions`
...
```json5
{
  agents: {
    list: [
      { id: "main", workspace: "~/.oriro/workspace" },
      {
        id: "delegate",
        workspace: "~/.oriro/workspace-delegate",
        tools: {
...
~/.oriro/agents/delegate/agent/auth-profiles.json
```
...
```json5
{
  agents: {
    list: [
      { id: "main", default: true, workspace: "~/.oriro/workspace" },
      {
        id: "org-assistant",
        name: "[Organization] Assistant",
        workspace: "~/.oriro/workspace-org",
        agentDir: "~/.oriro/agents/org-assistant/agent",
...
recall view. Oriro redacts credential/token-like text, truncates long
