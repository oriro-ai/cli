...
parallels_macos_desktop_user_exec_with_secret_file() {
  ...
  local secret_path
  secret_path="/tmp/oriro-secret-${api_key_env:-env}-$RANDOM-$RANDOM"
  ...
  local wrapper
  local wrapper_path
  wrapper_path="/tmp/oriro-secret-env-wrapper-$RANDOM-$RANDOM.sh"
  wrapper='#!/bin/bash
set -e
cleanup() {
  rm -f "${ORIRO_WRAPPER_FILE:-}"
}
trap cleanup EXIT
if [ -n "${ORIRO_SECRET_ENV_NAME:-}" ] && [ -n "${ORIRO_SECRET_FILE:-}" ] && [ -f "$ORIRO_SECRET_FILE" ]; then
  secret_value="$(cat "$ORIRO_SECRET_FILE")"
  rm -f "$ORIRO_SECRET_FILE"
  export "${ORIRO_SECRET_ENV_NAME}=${secret_value}"
fi
"$@"
'
  ...
  if [[ "$user_flag" == "current-user" ]]; then
    prlctl exec "$vm_name" --current-user /usr/bin/env \
      "PATH=$path_value" \
      "ORIRO_SECRET_ENV_NAME=$api_key_env" \
      "ORIRO_SECRET_FILE=$secret_path" \
      "ORIRO_WRAPPER_FILE=$wrapper_path" \
