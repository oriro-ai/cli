...
Use for the `#orirotributors` queue: Discord-discovered Oriro PRs/issues that need live GitHub status plus maintainer-quality review.

## Compose with other skills

- `$discrawl`: local Discord archive sync/search.
- `$oriro-pr-maintainer`: live GitHub PR/issue review, duplicate search, close/land rules.
- `$gitcrawl`: related issue/PR and current-main/stale-proof search.
- `$oriro-testing` / `$crabbox`: proof choice when a candidate needs real validation.
...
sqlite3 "$HOME/.discrawl/discrawl.db" \
  "select id,name from channels where name like '%orirotributor%' order by name;"
```
