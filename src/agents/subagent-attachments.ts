...
function resolveAttachmentLimits(config: OriroConfig): AttachmentLimits {
...
export async function materializeSubagentAttachments(params: {
  ...
  const absRootDir = path.join(childWorkspaceDir, ".oriro", "attachments");
  const relDir = path.posix.join(".oriro", "attachments", attachmentId);
