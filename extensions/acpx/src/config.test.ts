...
describe("embedded acpx plugin config", () => {
  it("resolves workspace stateDir and cwd by default", () => {
    const workspaceDir = path.resolve("/tmp/oriro-acpx");
  ...
  it("injects the built-in plugin-tools MCP server only when explicitly enabled", () => {
    ...
    const server = resolved.mcpServers["oriro-plugin-tools"];
  ...
  it("injects the built-in Oriro tools MCP server only when explicitly enabled", () => {
    ...
    const server = resolved.mcpServers["oriro-tools"];
    expect(server).toEqual({
      command: process.execPath,
      args: expectedMcpServerArgs({
        sourceEntry: "src/mcp/oriro-tools-serve.ts",
        distEntry: "dist/mcp/oriro-tools-serve.js",
      }),
    });
  });
  ...
  it("keeps the runtime json schema in sync with the manifest config schema", () => {
    ...
    const manifest = JSON.parse(
      fs.readFileSync(path.join(pluginRoot, "oriro.plugin.json"), "utf8"),
