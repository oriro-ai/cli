---
summary: "Pre-flight and rollback checklist before exposing an Oriro Gateway beyond loopback"
...
- Backup location for `~/.oriro-ai/cli.json` and credentials.
...
```bash
oriro gateway probe --url ws://127.0.0.1:18789 --token "$ORIRO_GATEWAY_TOKEN"
```
...
Run `oriro security audit --deep` after proxy changes. Trusted-proxy findings
...
1. Re-run `oriro security audit --deep`.
...
5. Re-run `oriro security audit --deep`.
