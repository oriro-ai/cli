// Ios Version script supports Oriro repository automation.
...
function main(argv = process.argv.slice(2)): number {
  ...
  if (options.format === "shell") {
    process.stdout.write(
      [
        `ORIRO_IOS_VERSION=${version.canonicalVersion}`,
        `ORIRO_MARKETING_VERSION=${version.marketingVersion}`,
        `ORIRO_BUILD_VERSION=${version.buildVersion}`,
      ].join("\n") + "\n",
    );
  } else {
