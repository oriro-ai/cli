 ...
 *
 * Registers provider-specific stream functions and rewrites models that need Oriro-managed transport semantics.
 */
...
const PROVIDER_SIMPLE_COMPLETION_API_PREFIX = "oriro-provider-simple:";
...
function resolveAnthropicVertexSimpleApi(baseUrl?: string): Api {
  ...
  return `oriro-anthropic-vertex-simple:${suffix}`;
}
...
function applyProviderSimpleCompletionWrapper(model: Model, cfg?: OriroConfig): Model {
...
function prepareCodexSimpleTransportModel<TApi extends Api>(
  ...
  // completion path must use Oriro's transport so resolved request auth is applied.
  const transportModel = {
    ...model,
    baseUrl: normalizeCodexResponsesBaseUrlForOpenAISdk(model.baseUrl),
  } as Model;
  const api = resolveTransportAwareSimpleApi(model.api);
  const streamFn = createOriroTransportStreamFnForModel(transportModel, { cfg });
  if (!api || !streamFn) {
    return undefined;
  }
