...
function makePlist(): string {
  const dir = mkdtempSync(path.join(tmpdir(), "oriro-plistbuddy-"));
  tempDirs.push(dir);
...
function runStopPackagedAppHarness(killZeroStatus: 0 | 1) {
  const root = mkdtempSync(path.join(tmpdir(), "oriro-package-stop-root-"));
  const toolsDir = mkdtempSync(path.join(tmpdir(), "oriro-package-stop-tools-"));
  tempDirs.push(root, toolsDir);

  const appRoot = path.join(root, "dist", "Oriro.app");
  const appBinary = path.join(appRoot, "Contents", "MacOS", "Oriro");
...
function runSwiftCompatibilityHarness(buildConfig: "debug" | "release") {
  const root = mkdtempSync(path.join(tmpdir(), "oriro-package-swift-root-"));
  const toolsDir = mkdtempSync(path.join(tmpdir(), "oriro-package-swift-tools-"));
  const developerDir = path.join(root, "Xcode.app", "Contents", "Developer");
  const appRoot = path.join(root, "Oriro.app");
...
describe("package-mac-app plist stamping", () => {
  ...
  it("falls back to corepack pnpm when the pnpm shim is absent", () => {
    ...
    const tempRoot = mkdtempSync(path.join(tmpdir(), "oriro-package-pnpm-root-"));
    const toolsDir = mkdtempSync(path.join(tmpdir(), "oriro-package-pnpm-tools-"));
    ...
    const result = runHelper(`
      ...
      ORIRO_TEST_LOG=${JSON.stringify(logPath)}
      export ORIRO_TEST_LOG
      PATH=${JSON.stringify(`${toolsDir}:/usr/bin:/bin`)}
      ${helperBlock}
  ...
  it("prefers repo Corepack pnpm over a global pnpm shim", () => {
    const helperBlock = getPackageManagerHelperBlock();
    const tempRoot = mkdtempSync(path.join(tmpdir(), "oriro-package-pnpm-root-"));
    const outerRoot = mkdtempSync(path.join(tmpdir(), "oriro-package-pnpm-outer-"));
    const toolsDir = mkdtempSync(path.join(tmpdir(), "oriro-package-pnpm-tools-"));
  ...
  it("runs Sparkle build metadata derivation from the repository root", () => {
    ...
    const tempRoot = mkdtempSync(path.join(tmpdir(), "oriro-package-sparkle-root-"));
    const toolsDir = mkdtempSync(path.join(tmpdir(), "oriro-package-sparkle-tools-"));
    tempDirs.push(tempRoot, toolsDir);
    ...
    const result = runHelper(`
      set -euo pipefail
      ROOT_DIR=${JSON.stringify(tempRoot)}
      ORIRO_ROOT=${JSON.stringify(tempRoot)}
      PATH=${JSON.stringify(`${toolsDir}:/usr/bin:/bin`)}
      export ORIRO_ROOT PATH
      cd /tmp
      ${helperBlock}
  ...
  it("does not kill unrelated Oriro processes during packaging", () => {
    const script = readFileSync(scriptPath, "utf8");
    ...
    expect(script).not.toContain("killall -q Oriro");
    expect(stopBlock).toContain('local app_binary="$APP_ROOT/Contents/MacOS/Oriro"');
  ...
  it("fails when the packaged app survives forced shutdown", () => {
    ...
    expect(result.status).toBe(1);
    expect(result.stderr).toContain("ERROR: Packaged Oriro bundle did not exit: 123");
  });
  ...
  it("fails closed when required Swift resources are missing", () => {
    const script = readFileSync(scriptPath, "utf8");
    const openOriroKitBlock = script.slice(
      script.indexOf(
        'ORIROKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/OriroKit_OriroKit.bundle"',
      ),
      script.indexOf('echo "📦 Copying Textual resources"'),
    );

    expect(openOriroKitBlock).toContain("ERROR: OriroKit resource bundle not found");
    expect(openOriroKitBlock).toContain("exit 1");
    expect(openOriroKitBlock).not.toContain("WARN:");
    expect(openOriroKitBlock).not.toContain("continuing");
  });
  ...
  it.runIf(process.platform === "darwin")(
    ...
    () => {
      ...
      const result = runHelper(`
        ...
        plist_set_string_required ${JSON.stringify(plist)} CFBundleIdentifier 'ai.oriro.test'
      ...
      expect(result.status).toBe(1);
      expect(result.stdout).toContain("ai.oriro.test");
