...
alongside Oriro. It combines BM25, vector search, and reranking in a single
...
- **Automatic fallback** -- if QMD is unavailable, Oriro falls back to the
...
Oriro creates a self-contained QMD home under
`~/.oriro/agents/<agentId>/qmd/` and manages the sidecar lifecycle
...
- Oriro creates collections from your workspace memory files and any
...
- If QMD fails entirely, Oriro falls back to the builtin SQLite engine.
  ...
  `oriro memory status` and one-shot CLI probes still recheck QMD directly.
...
Oriro keeps the QMD search path compatible with both current and older QMD
installs.

On startup, Oriro checks the installed QMD help text once per manager. If the
binary advertises support for multiple collection filters, Oriro searches all
...
Older QMD builds only accept one collection filter. When Oriro detects one
...
process, so you can tune QMD globally without adding new Oriro config:
...
collection under `~/.oriro/agents/<id>/qmd/sessions/`.
...
When scope denies a search, Oriro logs a warning with the derived channel and
...
**QMD not found?** Ensure the binary is on the gateway's `PATH`. If Oriro
runs as a service, create a symlink:
`sudo ln -s ~/.bun/bin/qmd /usr/local/bin/qmd`.

If `qmd --version` works in your shell but Oriro still reports
...
with `oriro memory status --deep`.

**First search very slow?** QMD downloads GGUF models on first use. Pre-warm
with `qmd query "test"` using the same XDG dirs Oriro uses.

**Many QMD subprocesses during search?** Update QMD if possible. Oriro uses
...
`memory.qmd.searchMode = "search"`. Oriro treats that mode as lexical-only,
...
the next startup reconciliation. Oriro recreates stale managed collections
...
Oriro's builtin symlink rules. Keep temporary monorepo checkouts under
