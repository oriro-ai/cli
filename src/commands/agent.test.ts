...
vi.mock("../config/sessions/transcript-resolve.runtime.js", () => {
  ...
  const resolveSessionFile = (sessionId: string, agentId: string, sessionsDir?: string): string =>
    joinPath(sessionsDir ?? ".oriro", "agents", agentId, "sessions", `${sessionId}.jsonl`);
...
function mockConfig(
  home: string,
  storePath: string,
  agentOverrides?: Partial<NonNullable<NonNullable<OriroConfig["agents"]>["defaults"]>>,
  telegramOverrides?: Partial<NonNullable<NonNullable<OriroConfig["channels"]>["telegram"]>>,
  agentsList?: NonNullable<NonNullable<OriroConfig["agents"]>["list"]>,
) {
  const cfg = {
    agents: {
      defaults: {
...
beforeEach(() => {
  ...
  configIoMocks.readConfigFileSnapshotForWrite.mockResolvedValue({
    snapshot: { valid: false, resolved: {} as OriroConfig },
    writeOptions: {},
  });
});
...
describe("agentCommand", () => {
  it("enables Codex, provider owner, and memory slot plugins for one-shot OpenAI model overrides", async () => {
    await withTempHome(async (home) => {
      ...
      for (const [registryLoad] of pluginRegistryMocks.ensurePluginRegistryLoaded.mock.calls) {
        ...
        expect(registryLoad?.workspaceDir).toBe(path.join(home, "oriro"));
  ...
  it("does not enable Codex for one-shot OpenAI overrides when the provider forces Oriro", async () => {
    await withTempHome(async (home) => {
      const storePath = path.join(home, "sessions.json");
  ...
  it("uses no-tools plain prompt mode for one-shot model runs", async () => {
    await withTempHome(async (home) => {
      ...
      await agentCommand(
        {
          message: "Reply with exactly ORIRO-MODEL-OK",
          agentId: "main",
          model: "openrouter/auto",
          modelRun: true,
          promptMode: "none",
  ...
  it("bypasses ACP sessions for one-shot model runs", async () => {
    await withTempHome(async (home) => {
      ...
      expect(callArgs?.provider).toBe("openrouter");
      expect(callArgs?.model).toBe("openrouter/auto");
      expect(callArgs?.prompt).toBe("Reply with exactly ORIRO-MODEL-OK");
      expect(callArgs?.modelRun).toBe(true);
      expect(callArgs?.promptMode).toBe("none");
      expect(callArgs?.disableTools).toBe(true);
    });
