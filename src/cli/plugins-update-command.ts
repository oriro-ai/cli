// `oriro plugins update` command implementation for tracked npm plugins and hook packs.
...
function mayMutatePluginInstallRecord(
  ...
  if (record?.source === "orirohub") {
    return Boolean(record.orirohubPackage);
  }
...
function projectUpdaterResultOntoSourceConfig(params: {
  runtimeBase: OriroConfig;
  sourceBase: OriroConfig;
  updatedConfig: OriroConfig;
}): OriroConfig {
  const updatePatch = createMergePatch(params.runtimeBase, params.updatedConfig);
  return applyMergePatch(params.sourceBase, updatePatch) as OriroConfig;
}
