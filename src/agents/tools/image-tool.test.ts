...
import { isInboundPathAllowed } from "@oriro/media-core/inbound-path-policy";
...
type CreateOriroCodingToolsArgs = Parameters<typeof createOriroCodingTools>[0];
type MockOriroToolsOptions = {
  config?: OriroConfig;
  agentDir?: string;
  workspaceDir?: string;
  sandboxRoot?: string;
...
vi.mock("../auth-profiles.js", () => ({
  ...
  ensureAuthProfileStore: (agentDir?: string) => {
    ...
    if (process.env.ORIRO_TEST_CODEX_CLI_OAUTH === "1") {
...
vi.mock("../auth-profiles/external-cli-sync.js", () => ({
  resolveExternalCliAuthProfiles: (
    ...
    if (
      process.env.ORIRO_TEST_CODEX_CLI_OAUTH !== "1" ||
...
vi.mock("../model-auth.js", () => ({
  ...
  hasRuntimeAvailableProviderAuth: (params: {
    ...
    if (params.provider === "codex") {
      return process.env.ORIRO_TEST_CODEX_ROUTE === "1";
    }
  ...
  hasUsableCustomProviderApiKey: (cfg?: OriroConfig, provider?: string) => {
...
vi.mock("../oriro-tools.js", async () => {
  const { createImageTool: createImageToolLocal } = await import("./image-tool.js");
  return {
    createOriroTools: vi.fn((options?: MockOriroToolsOptions) => {
...
async function createOriroCodingToolsWithFreshModules(options?: CreateOriroCodingToolsArgs) {
  ...
  return createOriroCodingTools(options);
}

async function withTempAgentDir<T>(run: (agentDir: string) => Promise<T>): Promise<T> {
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-image-"));
  try {
...
async function withTempWorkspacePng(
  ...
  const workspaceParent = await fs.mkdtemp(path.join(parentDir, "oriro-workspace-image-"));
  try {
...
function createMinimaxImageConfig(): OriroConfig {
  return {
    agents: {
      defaults: {
...
async function withTempSandboxState(
  ...
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-image-sandbox-"));
...
describe("image tool implicit imageModel config", () => {
  ...
  registerImageToolEnvReset(priorFetch, [
    ...
    "ZAI_API_KEY",
    "Z_AI_API_KEY",
    "ORIRO_TEST_CODEX_CLI_OAUTH",
    "ORIRO_TEST_CODEX_ROUTE",
  ...
  it.each(implicitImageRoutingCases)(
    ...
    async ({ cfg, profiles, codexRoute, openAiApiKey, expected }) => {
      if (codexRoute) {
        vi.stubEnv("ORIRO_TEST_CODEX_ROUTE", "1");
      }
      if (openAiApiKey) {
  ...
  it("lets external CLI Codex OAuth survive the candidate auth filter", async () => {
    await withTempAgentDir(async (agentDir) => {
      vi.stubEnv("ORIRO_TEST_CODEX_CLI_OAUTH", "1");
      vi.stubEnv("ORIRO_TEST_CODEX_ROUTE", "1");
  ...
  it("respects fsPolicy.workspaceOnly for non-sandbox image paths", async () => {
    await withTempWorkspacePng(async ({ workspaceDir, imagePath }) => {
      ...
      await withTempAgentDir(async (agentDir) => {
        ...
        const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-outside-"));
  ...
  it("still rejects non-workspace local image paths when workspaceOnly is disabled", async () => {
    ...
    await withTempAgentDir(async (agentDir) => {
      const cfg = createMinimaxImageConfig();
      const outsideDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-image-outside-"));
      const outsideImage = path.join(outsideDir, "secret.png");
      await fs.writeFile(outsideImage, Buffer.from(ONE_PIXEL_PNG_B64, "base64"));
      try {
        const tool = createRequiredImageTool({
  ...
  it("allows image paths from the current iMessage account attachment roots", async () => {
    await withTempAgentDir(async (agentDir) => {
      ...
      const attachmentRoot = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-imessage-root-"));
  ...
  it("allows image paths from current iMessage wildcard attachment roots", async () => {
    await withTempAgentDir(async (agentDir) => {
      ...
      const attachmentRootParent = await fs.mkdtemp(
        path.join(os.tmpdir(), "oriro-imessage-wildcard-root-"),
      );
  ...
  it("allows workspace images via createOriroCodingTools when workspace root is explicit", async () => {
    await withTempWorkspacePng(async ({ workspaceDir, imagePath }) => {
      const fetch = stubMinimaxOkFetch();
      await withTempAgentDir(async (agentDir) => {
        const cfg = createMinimaxImageConfig();

        const tools = await createOriroCodingToolsWithFreshModules({
          config: cfg,
          agentDir,
          workspaceDir,
        });
...
describe("image tool MiniMax VLM routing", () => {
  ...
  async function createMinimaxVlmFixture(baseResp: { status_code: number; status_msg: string }) {
    ...
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-minimax-vlm-"));
    vi.stubEnv("MINIMAX_API_KEY", "minimax-test");
    const cfg = createMinimaxImageConfig();
    const tool = createRequiredImageTool({ config: cfg, agentDir });
    return { fetch, tool };
...
describe("image tool managed inbound media", () => {
  ...
  async function withManagedInboundPng(
    ...
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-image-managed-inbound-"));
