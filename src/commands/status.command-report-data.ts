// Builds the data model for the standard `oriro status` text report.
...
export async function buildStatusCommandReportData(
  ...
  const securityAuditLines = params.securityAudit
    ...
    : [
        params.theme.muted(
          `Skipped in fast status. Full report: ${params.formatCliCommand("oriro security audit")}`,
        ),
        params.theme.muted(`Deep probe: ${params.formatCliCommand("oriro status --deep")}`),
      ];
  ...
  return {
    ...
    taskMaintenanceHint: `Task maintenance: ${params.formatCliCommand("oriro tasks maintenance --apply")}`,
