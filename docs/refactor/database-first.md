...
read_when:
  - Moving Oriro runtime data, cache, transcripts, task state, or scratch files into SQLite
...
- Global database: `~/.oriro/state/oriro.sqlite`
...
- Configuration stays file-backed: `oriro.json` remains outside the
  database. Runtime auth profiles move to SQLite; external provider or CLI
  credential files remain owner-managed outside Oriro's database.
...
- Raw stream logging uses `ORIRO_RAW_STREAM=1` plus SQLite diagnostics rows.
  The old pi-mono `PI_RAW_STREAM`, `PI_RAW_STREAM_PATH`, and
  `raw-openai-completions.jsonl` file logger contract is not part of Oriro
  runtime or tests.
...
- One per-agent SQLite database owns data-plane state:
  `agents/<agentId>/agent/oriro-agent.sqlite`.
- Config remains file-backed. `oriro.json` is not part of this database
  refactor.
...
- Task registry: `clean`. Task and Task Flow runtime rows live in
  `state/oriro.sqlite`; unshipped sidecar SQLite importers are deleted.
...
- [x] Keep Kysely generated types aligned after any schema change.
      Files: `src/state/oriro-state-schema.sql`,
      `src/state/oriro-agent-schema.sql`,
      `src/state/*generated*`.
...
- `oriro doctor --fix` owns the legacy file-to-database migration step.
  Runtime startup and `oriro migrate` should not carry legacy Oriro
...
- The old runtime-owned JSONL transcript streaming helper was deleted. Doctor
  ...
  SQLite rows.
- Codex app-server bindings use the Oriro `sessionId` as the canonical
...
former stores now write to `state/oriro.sqlite` or the owning
`oriro-agent.sqlite` database.
...
- Plugin migrations are data migrations over plugin-owned namespaces, not host
  ...
  `oriro-state-schema.sql` unless the host itself is taking ownership of a
  new cross-plugin contract.
- `src/state/oriro-agent-db.ts` opens
  `agents/<agentId>/agent/oriro-agent.sqlite`, registers the database in the
...
- Per-agent external conversation identity is relational too:
  ...
  `session_conversations` links one Oriro session to one or more external
  ...
  Oriro session can represent multiple external DM peers without demoting
...
- Secret target metadata now talks about stores instead of pretending every
  credential target is a config file. `oriro.json` remains the config store;
...
- Workspace bootstrap completion markers now live in typed shared
  ...
  `.oriro/workspace-state.json`; runtime no longer reads or rewrites the
  legacy workspace marker, and helper APIs no longer pass around a fake
  `.oriro/setup-state` path just to derive storage identity.
- Exec approvals now live in the typed shared SQLite `exec_approvals_config`
  singleton row. Doctor imports legacy `~/.oriro/exec-approvals.json`;
  ...
  `state/oriro.sqlite` table row; it keeps only the Unix prompt socket on disk
...
- GitHub Copilot compaction no longer writes `oriro-compaction-*.json`
  workspace sidecars. The harness calls the SDK history compaction RPC for the
  tracked SDK session, and Oriro keeps durable session/transcript state in
  SQLite instead of compatibility marker files.
- The shared Swift runtime (`OriroKit`) uses the same
  `state/oriro.sqlite` rows for device identity and device auth. macOS app
...
- Android device identity uses the same TypeScript-compatible key material
  stored in typed `state/oriro.sqlite#table/device_identities` rows. It never
  reads or writes `oriro/identity/device.json`; a leftover legacy file blocks
  startup until doctor imports it into SQLite.
- Android cached device auth tokens also use typed
  `state/oriro.sqlite#table/device_auth_tokens` rows and share the same
...
- Non-doctor commands do not auto-run legacy config repair. For example,
  `oriro update --channel` now fails on invalid legacy config and asks the
...
- Plugin state now uses the shared `state/oriro.sqlite` database. The old
...
- Task and Task Flow runtime tables now live in the shared
  `state/oriro.sqlite` database instead of `tasks/runs.sqlite` and
...
- Session target resolution now exposes per-agent database targets, not legacy
  ...
  `oriro sessions` enumerate `agent_databases` plus configured agents.
...
- Cache trace, Anthropic payload, raw stream, and diagnostics timeline records
  ...
  `diagnostics.cacheTrace.filePath`, `ORIRO_CACHE_TRACE_FILE`,
  `ORIRO_ANTHROPIC_PAYLOAD_LOG_FILE`, and
  `ORIRO_DIAGNOSTICS_TIMELINE_PATH` JSONL override paths are removed, and
...
- Memory Wiki activity logs now use SQLite plugin state instead of
  `.oriro-wiki/log.jsonl`. The Memory Wiki migration provider imports old
  JSONL logs; wiki markdown and user vault content stay file-backed as
  workspace content.
- Memory Wiki no longer creates `.oriro-wiki/state.json` or the unused
  `.oriro-wiki/locks` directory. The migration provider removes those retired
...
- The macOS companion no longer writes app-local `logs/config-audit.jsonl` or
  `logs/config-health.json` sidecars while editing `oriro.json`. The config
...
- Codex app-server conversation bindings now key SQLite plugin state by
  Oriro session key or explicit `{agentId, sessionId}` scope. They must not
...
- Zalo hosted outbound media now uses shared SQLite `plugin_blob_entries`
  instead of `oriro-zalo-outbound-media` JSON/bin temp sidecars.
...
- ACPX generated wrapper scripts and the isolated Codex home are temporary
  materialization under the Oriro temp root, not durable Oriro state. The
...
- Gateway media attachments now use the shared `media_blobs` SQLite table as
  ...
  `$ORIRO_STATE_DIR/media` or config-dir `media` roots; those directories are
  doctor import sources only.
- Shell completion no longer writes `$ORIRO_STATE_DIR/completions/*` cache
...
- Subagent inline attachments no longer materialize under workspace
  `.oriro/attachments/*`. The spawn path prepares SQLite VFS seed entries,
...
- CLI image hydration no longer maintains stable `oriro-cli-images` cache
...
- `oriro secrets apply` scrubs the config file, env file, and SQLite
...
- User-facing auth docs now describe
  `state/oriro.sqlite#table/auth_profile_stores/<agentDir>` instead of
...
- Memory Wiki imported-source sync ledgers now store one SQLite plugin-state row
  per vault/source key instead of rewriting `.oriro-wiki/source-sync.json`;
...
- Memory Wiki ChatGPT import-run records now store one SQLite plugin-state row
  per vault/run id instead of writing `.oriro-wiki/import-runs/*.json`.
...
- Memory Wiki compiled digests now store SQLite plugin blob rows instead of
  writing `.oriro-wiki/cache/agent-digest.json` and
  `.oriro-wiki/cache/claims.jsonl`. The migration provider imports old cache
  files and removes the cache directory when it becomes empty.
- OriroHub skill install tracking now stores one SQLite plugin-state row per
  workspace/skill instead of writing or reading `.orirohub/lock.json` and
  `.orirohub/origin.json` sidecars at runtime. Runtime code uses tracked-install
...
- ACPX wrapper scripts and the isolated Codex home are generated in the
  Oriro temp root. They are recreated as needed and are not backup or
  migration inputs.
...
- Backup stages the state directory before archiving, copies non-database files,
  ...
  completed backup runs in SQLite with the archive manifest. `oriro backup
create` validates the written archive by default; `--no-verify` is the
  explicit fast path.
- `oriro backup restore` validates the archive before extraction, reuses the
...
- `oriro reset --scope config+creds+sessions` removes per-agent
  `oriro-agent.sqlite` databases plus WAL/SHM sidecars, not only legacy
...
- Generated model catalog config is stored in typed global SQLite
  ...
  `ensureOriroModelCatalog`; there is no `models.json` compatibility API in
...
- Memory-core runtime imports SQLite transcript indexing helpers from
  `oriro/plugin-sdk/memory-core-host-engine-session-transcripts`, not the
...
- QMD's own `index.sqlite` is now a temp runtime materialization backed by the
  ...
  `~/.oriro/agents/<agentId>/qmd` sidecar.
- The optional `memory-lancedb` plugin no longer creates
  `~/.oriro/memory/lancedb` as an implicit Oriro-managed store. It is an
...
`oriro doctor --fix` invokes the state migration implementation after
ordinary config preflight and creates a verified backup before import. Runtime
startup and `oriro migrate` must not import legacy Oriro state files.
...
- Add a single doctor-owned state migration service for file-to-database import.
  Done: `oriro doctor --fix` uses the legacy-state migration implementation.
...
Keep shared coordination state in `state/oriro.sqlite`:
...
```text
~/.oriro/state/oriro.sqlite
~/.oriro/agents/<agentId>/agent/oriro-agent.sqlite
```
...
- Verify by opening every DB snapshot and running `PRAGMA integrity_check`.
  `oriro backup create` does this archive verification by default;
...
5. Verify the archive by opening every included SQLite snapshot and running
   ...
   `oriro backup create` does this by default; `--no-verify` is only for
...
- Use `{ agentId, sessionKey }` row operations.
  ...
  and the `oriro sessions` listing command now read per-agent rows directly
...
8. Backup integration.
   ...
   - Add restore from verified archive snapshots. Done: `oriro backup
...
- Remove tests that assert runtime creation of `sessions.json` or transcript
  ...
  Oriro test-state helper no longer exposes a legacy `auth-profiles.json`
...
- ad-hoc `oriro-state.sqlite` runtime sidecars
- `tasks/runs.sqlite`
...
- Memory Wiki `.oriro-wiki/log.jsonl`
- Memory Wiki `.oriro-wiki/state.json`
- Memory Wiki `.oriro-wiki/locks/`
- Memory Wiki `.oriro-wiki/source-sync.json`
- Memory Wiki `.oriro-wiki/import-runs/*.json`
- Memory Wiki `.oriro-wiki/cache/agent-digest.json`
- Memory Wiki `.oriro-wiki/cache/claims.jsonl`
- OriroHub `.orirohub/lock.json`
- OriroHub `.orirohub/origin.json`
- Browser profile decoration `.oriro-profile-decorated`
