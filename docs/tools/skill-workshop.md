...
Skill Workshop is Oriro's governed path for creating and updating workspace
skills.
...
plugin, OriroHub, extra-root, managed, personal-agent, or system skills.
...
```bash
oriro skills workshop propose-update trip-planning --proposal ./PROPOSAL.md
```
...
- `autonomous.enabled`: allows Oriro to create pending proposals from durable
...
```text
<ORIRO_STATE_DIR>/skill-workshop/
  proposals.json
...
| Proposal does not show in list                 | Check the selected `--agent` workspace and `ORIRO_STATE_DIR`.                                                                                                                                            |
...
- [Skills CLI](/cli/skills) for `oriro skills` commands
