...
import type { ChannelMessageActionAdapter } from "oriro/plugin-sdk/channel-contract";
import {
...
import { createAccountStatusSink } from "oriro/plugin-sdk/channel-outbound";
...
import type { RoutePeer } from "oriro/plugin-sdk/routing";
import {
...
const telegramMessageAdapter = createChannelMessageAdapterFromOutbound<OriroConfig>({
  id: "telegram",
  live: {
    capabilities: {
      draftPreview: true,
...
function resolveTelegramNativeTopicThreadId(
  ...
  // Keep the chat-scoped canonical id inside Oriro state; translate it back
  ...
  base: {
    ...
    gateway: {
      ...
      logoutAccount: async ({ accountId, cfg }) => {
        ...
        const nextCfg = { ...cfg } as OriroConfig;
