...
function createTempHome(): string {
  return makeTempDir(tempDirs, "oriro-test-env-real-home-");
}
...
describe("installTestEnv", () => {
  it("keeps live tests on a temp HOME while copying config and auth state", () => {
    ...
    writeFile(
      path.join(realHome, "custom-oriro.json5"),
      `{
        // Preserve provider config, strip host-bound paths.
        agents: {
          defaults: {
            workspace: "/Users/peter/Projects",
            agentDir: "/Users/peter/.oriro/agents/main/agent",
          },
          list: [
            {
              id: "dev",
              workspace: "/Users/peter/dev-workspace",
              agentDir: "/Users/peter/.oriro/agents/dev/agent",
            },
          ],
        },
        models: {
    ...
    writeFile(path.join(realHome, ".oriro", "credentials", "token.txt"), "secret\n");
    writeFile(
      path.join(realHome, ".oriro", "external-plugins", "glueoriro", "oriro.plugin.json"),
      '{"id":"glueoriro"}\n',
    );
    writeFile(
      path.join(realHome, ".oriro", "agents", "main", "agent", "auth-profiles.json"),
    ...
    process.env.ORIRO_LIVE_TEST_QUIET = "1";
    process.env.ORIRO_CONFIG_PATH = "~/custom-oriro.json5";
    process.env.ORIRO_TEST_HOME = priorIsolatedHome;
    process.env.ORIRO_STATE_DIR = path.join(priorIsolatedHome, ".oriro");
    ...
    expect(process.env.ORIRO_TEST_HOME).toBe(testEnv.tempHome);
    expect(process.env.TEST_PROFILE_ONLY).toBe("from-profile");

    const copiedConfigPath = path.join(testEnv.tempHome, ".oriro", "oriro.json");
    ...
    expect(
      fs.existsSync(path.join(testEnv.tempHome, ".oriro", "credentials", "token.txt")),
    ).toBe(true);
    expect(
      fs.existsSync(
        path.join(
    ...
    expect(
      fs.existsSync(
        path.join(testEnv.tempHome, ".oriro", "agents", "main", "agent", "auth-profiles.json"),
      ),
    ).toBe(true);
  ...
  it("allows explicit live runs against the real HOME", () => {
    ...
    process.env.USERPROFILE = realHome;
    process.env.ORIRO_LIVE_TEST = "1";
    process.env.ORIRO_LIVE_USE_REAL_HOME = "1";
    process.env.ORIRO_LIVE_TEST_QUIET = "1";
  ...
  it("does not load ~/.profile for normal isolated test runs", () => {
    ...
    delete process.env.ORIRO_LIVE_TEST;
    delete process.env.ORIRO_LIVE_GATEWAY;
    delete process.env.ORIRO_LIVE_USE_REAL_HOME;
    delete process.env.ORIRO_LIVE_TEST_QUIET;
