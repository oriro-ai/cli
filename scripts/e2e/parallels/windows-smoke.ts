...
// Windows Smoke script supports Oriro repository automation.
...
const defaultOptions = (): WindowsOptions => ({
  ...
  installUrl: "https://oriro.ai/install.ps1",
  installVersion: "",
  json: false,
  keepServer: false,
  latestVersion: "",
  ...
  snapshotHint: "pre-oriro-native-e2e-2026-03-12",
  targetPackageSpec: "",
...
Options:
  ...
  --snapshot-hint <name>     Snapshot name substring/fuzzy match.
                             Default: "pre-oriro-native-e2e-2026-03-12"
  --mode <fresh|upgrade|both>
  --provider <openai|anthropic|minimax>
  --model <provider/model>    Override the model used for the agent-turn smoke.
  --api-key-env <var>        Host env var name for provider API key.
  --openai-api-key-env <var> Alias for --api-key-env (backward compatible)
  --install-url <url>        Installer URL for latest release. Default: https://oriro.ai/install.ps1
  ...
  --upgrade-from-packed-main
                             ...
                             then run oriro update --channel dev.
...
class WindowsSmoke extends SmokeRunController<WindowsOptions> {
  ...
  private gatewayRecoveryAfterMs =
    readPositiveIntEnv("ORIRO_PARALLELS_WINDOWS_GATEWAY_RECOVERY_AFTER_S", 180) * 1000;
  ...
  async run(): Promise<void> {
    this.runDir = await makeTempDir("oriro-parallels-windows.");
    this.phases = new PhaseRunner(this.runDir);
    this.guest = new WindowsGuest(this.options.vmName, this.phases);
    this.tgzDir = await makeTempDir("oriro-parallels-windows-tgz.");
    try {
  ...
  protected async runFreshLane(): Promise<void> {
    ...
    await this.phase("fresh.install-main", 420, () => this.installMain("oriro-main-fresh.tgz"));
  ...
  private guestPowerShell(
    ...
    return this.guest.powershell(`${windowsOriroResolver}\n${script}`, options);
  }
  ...
  private logGuestPreflight(cleanOriro: boolean): void {
    const cleanScript = cleanOriro
      ? "npm.cmd uninstall -g oriro --no-fund --no-audit --loglevel=error 2>$null; $global:LASTEXITCODE = 0"
      : "";
...
if ($LASTEXITCODE -ne 0) { throw "oriro --version failed with exit code $LASTEXITCODE" }`,
  ...
  private verifyVersionContains(needle: string): void {
    const version = this.guestPowerShell("Invoke-Oriro --version");
...
Invoke-Oriro onboard --non-interactive --mode local --auth-choice ${psSingleQuote(this.auth.authChoice)} --secret-input-mode ref --gateway-port 18789 --gateway-bind loopback --install-daemon --skip-skills --skip-health --accept-risk --json
if ($LASTEXITCODE -ne 0) { throw "oriro onboard failed with exit code $LASTEXITCODE" }
...
${windowsScopedEnvFunction}
$script:OriroUpdateExit = 0
Invoke-WithScopedEnv @{ ORIRO_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS = '1'; ORIRO_DISABLE_BUNDLED_PLUGINS = '1' } {
  Invoke-Oriro update --channel dev --yes --json
  $script:OriroUpdateExit = $LASTEXITCODE
}
if ($script:OriroUpdateExit -ne 0) { throw "oriro update failed with exit code $script:OriroUpdateExit" }
Invoke-Oriro --version
Invoke-Oriro update status --json`,
...
if ($LASTEXITCODE -ne 0) { throw "gateway ${action} failed with exit code $LASTEXITCODE" }`,
  ...
  private verifyGatewayReachable(): void {
    ...
    while (Date.now() < deadline) {
      const probe = this.guestPowerShell(
        "Invoke-Oriro gateway probe --url ws://127.0.0.1:18789 --timeout 30000 --json",
      ...
      if (!recoveryTried && Date.now() - start >= this.gatewayRecoveryAfterMs) {
        ...
        this.guestPowerShell("Invoke-Oriro gateway start", {
          check: false,
          timeoutMs: 120_000,
        });
        recoveryTried = true;
  ...
  private showGatewayStatusCompat(): void {
    const help = this.guestPowerShell("Invoke-Oriro gateway status --help", {
      check: false,
    });
    const suffix = help.includes("--require-rpc") ? "--deep --require-rpc" : "--deep";
    this.guestPowerShell(`Invoke-Oriro gateway status ${suffix}`);
  }
...
if (-not $agentOk) { throw 'oriro agent finished without OK response' }`,
  ...
  private async extractLastVersion(phaseName: string): Promise<string> {
    return await extractLastOriroVersion(this.runDir, phaseName, /Oriro\s+([0-9][^\s]*)/gi);
  }
