/** Updates installed plugins across npm, OriroHub, marketplace, Git, and bundled bridge sources. */
...
import { satisfiesPluginApiRange } from "../infra/orirohub.js";
...
import {
  ...
  installedPackageNeedsOriroPeerLinkRepair,
...
import { buildOriroHubPluginInstallRecordFields } from "./orirohub-install-records.js";
import { ORIROHUB_INSTALL_ERROR_CODE, installPluginFromOriroHub } from "./orirohub.js";
import { normalizePluginsConfig, resolveEffectiveEnableState } from "./config-state.js";
import {
  getExternalizedBundledPluginLegacyPathSuffix,
  getExternalizedBundledPluginOriroHubSpec,
...
import {
  resolveTrustedSourceLinkedOfficialOriroHubSpec,
...
import { linkOriroPeerDependencies } from "./plugin-peer-link.js";
...
function formatOriroHubInstallFailure(params: {
  pluginId: string;
  spec: string;
  phase: "check" | "update";
  error: string;
}): string {
  return `Failed to ${params.phase} ${params.pluginId}: ${params.error} (OriroHub ${params.spec}).`;
}
...
function compareNpmSemverForUpdate(left: string, right: string): number {
  const releaseCmp = compareOriroReleaseVersions(left, right);
  if (releaseCmp !== null) {
    return releaseCmp;
  }
...
> {
  ...
  if (
    !parsedSpec ||
    !parsedSpec.name.startsWith("@oriro/") ||
...
function isNpmMetadataCompatibleWithCurrentHost(metadata: NpmSpecResolution): boolean {
  ...
  const installMetadata = metadata.packageOriro?.install;
  ...
  const pluginApiRangeCheck = resolvePackagePluginApiRange(metadata.packageOriro);
...
function isBundledVersionNewer(bundledVersion: string, installedVersion: string): boolean {
  const releaseCmp = compareOriroReleaseVersions(bundledVersion, installedVersion);
  if (releaseCmp !== null) {
    return releaseCmp > 0;
  }
...
function shouldFallbackOriroHubBridgeToNpm(params: {
  result: { ok: false; code?: string };
  npmSpec?: string;
}): boolean {
  if (!isOriroOrgNpmSpec(params.npmSpec)) {
    return false;
  }
  return (
    params.result.code === ORIROHUB_INSTALL_ERROR_CODE.PACKAGE_NOT_FOUND ||
    params.result.code === ORIROHUB_INSTALL_ERROR_CODE.VERSION_NOT_FOUND ||
    params.result.code === ORIROHUB_INSTALL_ERROR_CODE.ARTIFACT_DOWNLOAD_UNAVAILABLE ||
    params.result.code === ORIROHUB_INSTALL_ERROR_CODE.ARTIFACT_UNAVAILABLE
  );
}

function shouldFallbackOriroHubToDefault(result: { ok: false; code?: string }): boolean {
  return (
    result.code === ORIROHUB_INSTALL_ERROR_CODE.PACKAGE_NOT_FOUND ||
    result.code === ORIROHUB_INSTALL_ERROR_CODE.VERSION_NOT_FOUND
  );
}

function shouldFallbackBetaOriroHubUpdate(result: { ok: false; code?: string }): boolean {
  return shouldFallbackOriroHubToDefault(result);
}
...
function resolveTrustedSourceLinkedOfficialNpmFallbackForOriroHubUpdate(params: {
  pluginId: string;
  record: PluginInstallRecord;
  effectiveOriroHubSpec?: string;
  recordOriroHubSpec?: string;
  ...
  if (!isOfficialOriroHubInstallRecord(params.record)) {
    return null;
  }
  ...
  const recordedPackageNames = [
    params.record.orirohubPackage,
    resolveOriroHubSpecPackageName(params.record.spec),
    resolveOriroHubSpecPackageName(params.effectiveOriroHubSpec),
  ].filter((value): value is string => Boolean(value));
  if (!recordedPackageNames.includes(officialPackageName)) {
    return null;
  }

  const effectiveOriroHubVersion = params.effectiveOriroHubSpec
    ? parseOriroHubPluginSpec(params.effectiveOriroHubSpec)?.version
    : undefined;
  const recordOriroHubVersion = params.recordOriroHubSpec
    ? parseOriroHubPluginSpec(params.recordOriroHubSpec)?.version
    : undefined;
  if (effectiveOriroHubVersion && effectiveOriroHubVersion.toLowerCase() !== "latest") {
    return {
      installSpec: `${officialPackageName}@${effectiveOriroHubVersion}`,
      recordSpec:
        recordOriroHubVersion && recordOriroHubVersion.toLowerCase() !== "latest"
          ? `${officialPackageName}@${recordOriroHubVersion}`
          : officialSpec,
      ...(params.updateChannel === "beta" && effectiveOriroHubVersion.toLowerCase() === "beta"
...
function isBridgeOriroHubInstall(params: {
  bridge: ExternalizedBundledPluginBridge;
  record: PluginInstallRecord;
}): boolean {
  ...
  const orirohubSpec = getExternalizedBundledPluginOriroHubSpec(params.bridge);
  const bridgeOriroHubPackage = orirohubSpec ? parseOriroHubPluginSpec(orirohubSpec)?.name : undefined;
  const recordOriroHubPackage =
    params.record.orirohubPackage ?? parseOriroHubPluginSpec(params.record.spec ?? "")?.name;
  return Boolean(bridgeOriroHubPackage && recordOriroHubPackage === bridgeOriroHubPackage);
}
...
function resolveOriroHubUpdateSpecs(params: {
  record: PluginInstallRecord;
  officialSpecOverride?: string;
  updateChannel?: UpdateChannel;
}): {
  ...
  if (!params.officialSpecOverride && !params.record.orirohubPackage) {
    return {};
  }
  const recordSpec =
    params.officialSpecOverride ?? params.record.spec ?? `orirohub:${params.record.orirohubPackage}`;
...
function isBridgeAlreadyInstalledFromPreferredSource(params: {
  ...
  return preferredSource === "orirohub"
    ? isBridgeOriroHubInstall(params)
...
function migratePluginConfigId(cfg: OriroConfig, fromId: string, toId: string): OriroConfig {
...
function withoutPluginInstallRecord(cfg: OriroConfig, pluginId: string): OriroConfig {
...
function resetDisabledPluginSlots(
  slots: NonNullable<OriroConfig["plugins"]>["slots"] | undefined,
  pluginId: string,
): NonNullable<OriroConfig["plugins"]>["slots"] | undefined {
  if (!slots) {
    return slots;
  }
  let next = slots;
...
function disablePluginConfigEntry(config: OriroConfig, pluginId: string): OriroConfig {
...
async function repairOriroPeerLinksForNpmInstalls(params: {
  config: OriroConfig;
  logger: PluginUpdateLogger;
}): Promise<boolean> {
  let repaired = false;
  for (const [pluginId, record] of Object.entries(params.config.plugins?.installs ?? {})) {
    ...
    try {
      ...
      params.logger.warn?.(
        `Could not repair oriro peer link for "${pluginId}" due to invalid install path: ${String(err)}`,
      );
      continue;
    }

    if (!installedPackageNeedsOriroPeerLinkRepair(installPath)) {
      continue;
    }
    ...
    try {
      ...
      const peerLinkRepair = await linkOriroPeerDependencies({
      ...
      if (peerLinkRepair.skipped > 0) {
        params.logger.warn?.(
          `Could not repair oriro peer link for "${pluginId}" at ${installPath}: ${warnings.join("; ") || "peer link repair was skipped"}`,
        );
        continue;
      }
      repaired = !installedPackageNeedsOriroPeerLinkRepair(installPath) || repaired;
    } catch (err) {
      params.logger.warn?.(
        `Could not repair oriro peer link for "${pluginId}" at ${installPath}: ${String(err)}`,
      );
    }
  }
  return repaired;
...
export async function updateNpmInstalledPlugins(params: {
  ...
  for (const pluginId of targets) {
    ...
    const officialNpmSpec = params.syncOfficialPluginInstalls
      ...
      : undefined;
    const officialOriroHubSpec = params.syncOfficialPluginInstalls
      ? resolveTrustedSourceLinkedOfficialOriroHubSpec({ pluginId, record })
      : undefined;
    ...
    if (normalizedPluginConfig) {
      ...
      if (!enableState.enabled && !officialNpmSpec && !officialOriroHubSpec) {
        outcomes.push({
          pluginId,
          status: "skipped",
    ...
    const orirohubSpecs =
      record.source === "orirohub"
        ? resolveOriroHubUpdateSpecs({
            record,
            officialSpecOverride: officialOriroHubSpec,
            updateChannel: params.updateChannel,
          })
        : undefined;
    const effectiveSpec =
      record.source === "npm"
        ...
        : record.source === "orirohub"
          ? orirohubSpecs?.installSpec
          : record.spec;
    const recordSpec =
      record.source === "npm"
        ? npmSpecs?.recordSpec
        : record.source === "orirohub"
          ? orirohubSpecs?.recordSpec
          : record.spec;
    const officialNpmFallbackSpecs =
      record.source === "orirohub"
        ? resolveTrustedSourceLinkedOfficialNpmFallbackForOriroHubUpdate({
            pluginId,
            record,
            effectiveOriroHubSpec: effectiveSpec,
            recordOriroHubSpec: recordSpec,
            updateChannel: params.updateChannel,
          })
        : null;
    ...
    let activeOriroHubInstallSpec = effectiveSpec;
    ...
    if (record.source === "orirohub" && !record.orirohubPackage && !officialOriroHubSpec) {
      outcomes.push({
        pluginId,
        status: "skipped",
        message: `Skipping "${pluginId}" (missing OriroHub package metadata).`,
      });
      continue;
    }

    if (record.source === "orirohub" || record.source === "marketplace") {
    ...
    if (
      ...
      if (metadataResult.ok) {
        ...
        if (
          ...
          !installedPackageNeedsOriroPeerLinkRepair(installPath) &&
    ...
    if (params.dryRun) {
          ...
          record.source === "npm"
            ...
            : record.source === "orirohub"
              ? await installPluginFromOriroHub({
                  spec: effectiveSpec ?? `orirohub:${record.orirohubPackage!}`,
                  config: params.config,
                  baseUrl: record.orirohubUrl,
                  mode: "update",
                  extensionsDir,
                  timeoutMs: params.timeoutMs,
                  dryRun: true,
      ...
      if (
        !probe.ok &&
        record.source === "orirohub" &&
        orirohubSpecs?.fallbackSpec &&
        shouldFallbackBetaOriroHubUpdate(probe)
      ) {
        channelFallbackSuffix = formatBetaChannelFallbackOutcomeSuffix({
          fallbackLabel: orirohubSpecs.fallbackLabel ?? effectiveSpec,
          fallbackSpec: orirohubSpecs.fallbackSpec,
          verb: "would use",
        });
        logger.warn?.(
          `Plugin "${pluginId}" has no beta OriroHub release for ${orirohubSpecs.fallbackLabel ?? effectiveSpec}; using ${orirohubSpecs.fallbackSpec} instead. Core update can still complete.`,
        );
        probe = await installPluginFromOriroHub({
          spec: orirohubSpecs.fallbackSpec,
          config: params.config,
          baseUrl: record.orirohubUrl,
          mode: "update",
          extensionsDir,
        ...
        activeOriroHubInstallSpec = orirohubSpecs.fallbackSpec;
      ...
      if (
        ...
        shouldFallbackOriroHubBridgeToNpm({
          result: probe,
          npmSpec: officialNpmFallbackInstallSpec,
        })
      ) {
        channelFallbackSuffix = ` (warning: official OriroHub artifact fallback would use ${officialNpmFallbackInstallSpec}).`;
        logger.warn?.(
          `Plugin "${pluginId}" could not download official OriroHub artifact for ${activeOriroHubInstallSpec ?? `orirohub:${record.orirohubPackage!}`}; using npm ${officialNpmFallbackInstallSpec} instead. Core update can still complete.`,
        );
        usedNpmFallback = true;
      ...
      if (!probe.ok) {
        recordFailure(
          ...
          record.source === "npm" || usedOfficialNpmFallback
            ...
            : record.source === "orirohub"
              ? formatOriroHubInstallFailure({
                  pluginId,
                  spec: activeOriroHubInstallSpec ?? `orirohub:${record.orirohubPackage!}`,
                  phase: "check",
                  error: probe.error,
                })
              : record.source === "git"
    ...
    activeOriroHubInstallSpec = effectiveSpec;
    let npmChannelFallback: PluginUpdateChannelFallback | undefined;
    ...
    if (
      ...
      record.source === "orirohub" &&
      orirohubSpecs?.fallbackSpec &&
      shouldFallbackBetaOriroHubUpdate(result)
    ) {
      channelFallbackSuffix = formatBetaChannelFallbackOutcomeSuffix({
        fallbackLabel: orirohubSpecs.fallbackLabel ?? effectiveSpec,
        fallbackSpec: orirohubSpecs.fallbackSpec,
      ...
      result = await installPluginFromOriroHub({
        spec: orirohubSpecs.fallbackSpec,
        config: params.config,
        baseUrl: record.orirohubUrl,
        mode: "update",
    ...
    if (
      ...
      usedOfficialNpmFallback = true;
      resultSource = "npm";
      channelFallbackSuffix = ` (warning: official OriroHub artifact fallback used ${officialNpmFallbackInstallSpec}).`;
      result = await installNpmSpecForUpdate({
        spec: officialNpmFallbackInstallSpec,
        config: params.config,
        mode: "update",
    ...
    } else if (resultSource === "orirohub") {
      const orirohubResult = result as Extract<
        Awaited<ReturnType<typeof installPluginFromOriroHub>>,
        { ok: true }
      >;
      next = recordPluginInstall(next, {
        pluginId: resolvedPluginId,
        ...buildOriroHubPluginInstallRecordFields(orirohubResult.orirohub),
        spec: recordSpec ?? record.spec ?? `orirohub:${record.orirohubPackage!}`,
        installPath: result.targetDir,
        version: nextVersion,
      });
  ...
  if (ranNpmInstaller) {
    changed =
      (await repairOriroPeerLinksForNpmInstalls({
        config: next,
        logger,
      })) || changed;
  }
...
export async function syncPluginsForUpdateChannel(params: {
  ...
  if (params.channel === "dev") {
    ...
    for (const bridge of bridges) {
      ...
      const orirohubSpec = getExternalizedBundledPluginOriroHubSpec(bridge);
      const trustedSourceLinkedOfficialInstall = isTrustedSourceLinkedOfficialBridgeNpmInstall({
        targetPluginId,
        npmSpec,
      });
      let installSource = preferredSource;
      let installSpec = preferredSource === "orirohub" ? orirohubSpec : npmSpec;
      let result:
        | Awaited<ReturnType<typeof installPluginFromNpmSpec>>
        | Awaited<ReturnType<typeof installPluginFromOriroHub>>;
      ...
      if (preferredSource === "orirohub") {
        result = await installPluginFromOriroHub({
          spec: orirohubSpec,
          config: params.config,
          ...(bridge.orirohubUrl ? { baseUrl: bridge.orirohubUrl } : {}),
          mode: "update",
          expectedPluginId: targetPluginId,
          logger,
        });
        if (!result.ok && npmSpec && shouldFallbackOriroHubBridgeToNpm({ result, npmSpec })) {
          const warning = `OriroHub ${orirohubSpec} unavailable for ${targetPluginId}; falling back to npm ${npmSpec}.`;
      ...
      removeBridgeBundledLoadPaths({ bridge, loadPaths: loadHelpers, env });
      if (installSource === "orirohub") {
        summary.switchedToOriroHub.push(resolvedPluginId);
      } else {
