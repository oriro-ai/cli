...
private const val NOTIFICATION_CHANNEL_BASE_ID = "oriro.system.notify"
  ...
  private fun ensureChannel(priority: String?): String {
    ...
    val (suffix, importance, name) =
      when (normalizedPriority) {
        "passive" -> Triple("passive", NotificationManager.IMPORTANCE_LOW, "Oriro Passive")
        "timesensitive" -> Triple("timesensitive", NotificationManager.IMPORTANCE_HIGH, "Oriro Time Sensitive")
        else -> Triple("active", NotificationManager.IMPORTANCE_DEFAULT, "Oriro Active")
      }
