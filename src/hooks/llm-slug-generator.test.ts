...
vi.mock("../agents/agent-scope.js", () => ({
  ...
  resolveAgentWorkspaceDir: vi.fn(() => "/tmp/oriro-agent"),
  resolveAgentDir: vi.fn(() => "/tmp/oriro-agent/.oriro-agent"),
  resolveAgentEffectiveModelPrimary: vi.fn((cfg: OriroConfig) => {
...
function requireFirstRunOptions(): Record<string, unknown> {
  ...
  if (!call) {
    throw new Error("expected embedded Oriro agent run");
  }
  const [options] = call;
  if (!options || typeof options !== "object") {
    throw new Error("expected embedded Oriro agent run options");
  }
