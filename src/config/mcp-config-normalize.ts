...
type OriroMcpHttpTransport = "sse" | "streamable-http";

const CLI_MCP_TYPE_TO_ORIRO_TRANSPORT: Record<string, OriroMcpHttpTransport | "stdio"> = {
...
/** Maps CLI-native MCP type aliases to Oriro HTTP transport names. */
export function resolveOriroMcpTransportAlias(
  value: unknown,
): OriroMcpHttpTransport | undefined {
  const mapped = CLI_MCP_TYPE_TO_ORIRO_TRANSPORT[normalizeMcpString(value)];
  return mapped === "sse" || mapped === "streamable-http" ? mapped : undefined;
}

/** Checks whether a raw MCP `type` value is a legacy CLI alias Oriro can rewrite. */
export function isKnownCliMcpTypeAlias(value: unknown): boolean {
  return Object.hasOwn(CLI_MCP_TYPE_TO_ORIRO_TRANSPORT, normalizeMcpString(value));
}
...
export function canonicalizeConfiguredMcpServer(
  ...
  const next = { ...server };
  const transportAlias = resolveOriroMcpTransportAlias(next.type);
  // `transport` is Oriro's canonical field; legacy `type` only fills a gap.
