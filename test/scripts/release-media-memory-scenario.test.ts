...
describe("release media memory scenario", () => {
  it("fails when packaged plugin listing is broken or omits memory-core", () => {
    ...
    const listIndex = script.indexOf('oriro plugins list --json >"$PLUGINS_JSON"');
  ...
  it("uses a per-run temp root for generated media artifacts", () => {
    ...
    expect(script).toContain('media_root="$(mktemp -d /tmp/oriro-release-media-memory.XXXXXX)"');
    ...
    expect(script).not.toContain("/tmp/oriro-release-media-memory-plugins.json");
    expect(script).not.toContain(
      'mkdir -p "$ORIRO_STATE_DIR/workspace/memory" /tmp/oriro-release-media-memory',
    );
  });
});
