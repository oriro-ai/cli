---
summary: "CLI reference for `oriro onboard` (interactive onboarding)"
read_when:
...
Full guided onboarding for local or remote Gateway setup. Use this when you want Oriro to walk through model auth, workspace, gateway, channels, skills, and health in one flow.
...
<CardGroup cols={2}>
  ...
  <Card title="Onboarding overview" href="/start/onboarding-overview" icon="map">
    How Oriro onboarding fits together.
  </Card>
...
oriro onboard --flow quickstart
oriro onboard --flow manual
oriro onboard --flow import
oriro onboard --import-from hermes --import-source ~/.hermes
oriro onboard --skip-bootstrap
oriro onboard --mode remote --remote-url wss://gateway-host:18789
```

`--flow import` uses plugin-owned migration providers such as Hermes. It only runs against a fresh Oriro setup; if existing config, credentials, sessions, or workspace memory/identity files are present, reset or choose a fresh setup before importing.

`--modern` starts the Crestodian conversational onboarding preview. Without
`--modern`, `oriro onboard` keeps the classic onboarding flow.

On a fresh install where the active config file is missing or has no authored
settings (empty or metadata-only), bare `oriro` also starts the classic
onboarding flow. Once a config file has authored settings, bare `oriro`
...
`ORIRO_ALLOW_INSECURE_PRIVATE_WS=1` in the onboarding process environment.
...
```bash
ORIRO_LOCALE=zh-CN oriro onboard
```
...
Oriro marks common vision model IDs as image-capable automatically. Pass `--custom-image-input` for unknown custom vision IDs, or `--custom-text-input` to force text-only metadata.
...
```bash
export ORIRO_GATEWAY_TOKEN="your-token"
oriro onboard --non-interactive \
  --mode local \
  --auth-choice skip \
  --gateway-auth token \
...
- `--install-daemon` starts the managed gateway install path first. Without it, you must already have a local gateway running, for example `oriro gateway run`.
...
<AccordionGroup>
  ...
  <Accordion title="Other behaviors">
    ...
    - Fastest first chat: `oriro dashboard` (Control UI, no channel setup).
...
oriro channels add
oriro configure
oriro agents add <name>
```

Use `oriro setup` instead when you only need the baseline config/workspace. Use `oriro configure` later for targeted changes and `oriro channels add` for channel-only setup.
