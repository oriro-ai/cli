...
vi.mock("../daemon/program-args.js", () => ({
  ORIRO_WRAPPER_ENV_KEY: "ORIRO_WRAPPER",
  resolveGatewayProgramArguments: mocks.resolveGatewayProgramArguments,
  resolveOriroWrapperPath: mocks.resolveOriroWrapperPath,
}));
...
function mockNodeGatewayPlanFixture(
  ...
  const {
    ...
    supported = true,
    warning,
    serviceEnvironment = { ORIRO_PORT: "3000" },
  } = params;
  const workingDirectory = Object.hasOwn(params, "workingDirectory")
    ? params.workingDirectory
    : "/Users/me";
  mocks.resolvePreferredNodePath.mockResolvedValue("/opt/node");
  mocks.resolveOriroWrapperPath.mockImplementation(async (value: string | undefined) =>
...
describe("buildGatewayInstallPlan", () => {
  // Prevent tests from reading the developer's real ~/.oriro/.env when
  ...
  it("uses provided nodePath and returns plan", async () => {
    ...
    expect(plan.environment).toEqual({ ORIRO_PORT: "3000" });
  ...
  it("adds the active oriro command bin directory to the managed service PATH", async () => {
    mockNodeGatewayPlanFixture();
    const originalArgv = process.argv;
    const oriroBinPath = path.join(isolatedHome, ".npm-global", "bin", "oriro");
    process.argv = ["node", oriroBinPath, "gateway", "install"];
    ...
    ).toStrictEqual(["/opt/homebrew/opt/node/bin", path.dirname(oriroBinPath)]);
  });
  ...
  it("uses the state dir as the default macOS launchd working directory", async () => {
    ...
    expect(plan.workingDirectory).toBe(path.join(isolatedHome, ".oriro"));
    expect(mocks.buildServiceEnvironment).toHaveBeenCalledOnce();
  ...
  it("passes ORIRO_WRAPPER through program args and managed service env", async () => {
    const wrapperPath = path.resolve("/usr/local/bin/oriro-doppler");
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
        ORIRO_PORT: "3000",
        ORIRO_WRAPPER: wrapperPath,
      },
    });
    ...
    expect(mocks.buildServiceEnvironment).toHaveBeenCalledOnce();
    expect(
      firstMockArg(mocks.buildServiceEnvironment, "buildServiceEnvironment").env?.ORIRO_WRAPPER,
    ).toBe(wrapperPath);
    expect(plan.environment.ORIRO_WRAPPER).toBe(wrapperPath);
  });

  it("clears a Windows wrapper env that points at the generated gateway.cmd script", async () => {
    const selfWrapperPath = path.join(isolatedHome, ".oriro", "gateway.cmd");
    const warn = vi.fn();
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
        ORIRO_PORT: "3000",
    ...
    expect(plan.environment.ORIRO_WRAPPER).toBeUndefined();
    expect(warn).toHaveBeenCalledWith(
      expect.stringContaining(
        "Ignoring ORIRO_WRAPPER because it points to the Windows task script",
      ),
    );
  });
  ...
  it("tracks safe config env keys without embedding literal values", async () => {
    ...
    expect(plan.environment.ORIRO_PORT).toBe("3000");
    expect(plan.environment.ORIRO_SERVICE_MANAGED_ENV_KEYS).toBe(
  ...
  it("includes env SecretRef values from config into the service environment", async () => {
    ...
    expect(plan.environment.ORIRO_SERVICE_MANAGED_ENV_KEYS).toBeUndefined();
  });
  ...
  it("allows safe inherited passEnv names while blocking dangerous exec SecretRef env", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      env: isolatedPlanEnv({
        BASH_ENV: "/tmp/oriro-test-bashenv",
        XDG_CONFIG_HOME: "/tmp/oriro-test-xdg-home",
  ...
  it("does not embed gateway auth SecretRef values into the service environment", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      env: isolatedPlanEnv({
        ORIRO_GATEWAY_TOKEN: "gateway-test-token",
      }),
      port: 3000,
      runtime: "node",
      config: {
        gateway: {
          auth: {
            token: { source: "env", provider: "default", id: "ORIRO_GATEWAY_TOKEN" },
          },
        },
      },
    });

    expect(plan.environment.ORIRO_GATEWAY_TOKEN).toBeUndefined();
    expect(plan.environment.ORIRO_SERVICE_MANAGED_ENV_KEYS).toBeUndefined();
  });
  ...
  it("does not inline config env SecretRef values already backed by state-dir dotenv", async () => {
    await writeStateDirDotEnv("DISCORD_BOT_TOKEN=discord-dotenv-token\n", {
      stateDir: path.join(isolatedHome, ".oriro"),
    });
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
        ORIRO_PORT: "3000",
    ...
    expect(plan.environment.ORIRO_SERVICE_MANAGED_ENV_KEYS).toBe("DISCORD_BOT_TOKEN");
  });
...
describe("buildGatewayInstallPlan — dotenv merge", () => {
  ...
  it("tracks .env vars with config while preserving service precedence", async () => {
    await writeStateDirDotEnv(
      ...
      {
        stateDir: path.join(tmpDir, ".oriro"),
      },
    );
    mockNodeGatewayPlanFixture({
      serviceEnvironment: {
  ...
  it("retains .env values for macOS LaunchAgent env SecretRefs", async () => {
    ...
    expect(plan.environment.ORIRO_SERVICE_MANAGED_ENV_KEYS).toBe("MINIMAX_API_KEY");
  });
  ...
  it("works when .env file does not exist", async () => {
    mockNodeGatewayPlanFixture({ serviceEnvironment: { ORIRO_PORT: "3000" } });
  ...
  it("preserves safe custom vars from an existing service env and merges PATH", async () => {
    ...
    expect(plan.environment.ORIRO_SERVICE_MARKER).toBeUndefined();
  });
  ...
  it("drops keys that were previously tracked as managed service env", async () => {
    ...
    const plan = await buildGatewayInstallPlan({
      ...
      existingEnvironment: {
        ...
        BLOGWATCHER_HOME: "/Users/test/.blogwatcher",
        GOPATH: "/Users/test/.local/gopath",
        ORIRO_SERVICE_MANAGED_ENV_KEYS: "GOBIN,GOPATH",
      },
    });
  ...
  it("drops legacy inline env values when the key is now managed by .env", async () => {
    ...
    expect(plan.environment.ORIRO_SERVICE_MANAGED_ENV_KEYS).toBe("TAVILY_API_KEY");
  ...
  it("keeps source metadata for EnvironmentFile-backed preserved vars", async () => {
    ...
    expect(plan.environment.ORIRO_GATEWAY_TOKEN).toBeUndefined();
    expect(plan.environmentValueSources?.ORIRO_GATEWAY_TOKEN).toBeUndefined();
  });
  ...
  it("does not embed auth-profile env refs when the key is already durable", async () => {
    ...
    expect(plan.environment.ORIRO_SERVICE_MANAGED_ENV_KEYS).toBe("OPENAI_API_KEY");
  });
});
...
describe("gatewayInstallErrorHint", () => {
  it("returns platform-specific hints", () => {
    ...
    expect(gatewayInstallErrorHint("linux")).toMatch(
      /(?:oriro|oriro)( --profile isolated)? gateway install/,
    );
  });
});
...
describe("collectPreservedExistingServiceEnvVars — operator opt-in allowlist", () => {
  ...
  it("continues to drop stale ORIRO_ALLOW_ROOT", () => {
    const result = collectPreservedExistingServiceEnvVars(
      { ORIRO_ALLOW_ROOT: "1" },
      managedKeys,
    );
    expect(result.ORIRO_ALLOW_ROOT).toBeUndefined();
  });

  it("preserves ORIRO_CLI_CONTAINER_BYPASS and ORIRO_CONTAINER_HINT", () => {
    const result = collectPreservedExistingServiceEnvVars(
      {
        ORIRO_CLI_CONTAINER_BYPASS: "1",
        ORIRO_CONTAINER_HINT: "ci",
      },
      managedKeys,
    );
    expect(result.ORIRO_CLI_CONTAINER_BYPASS).toBe("1");
    expect(result.ORIRO_CONTAINER_HINT).toBe("ci");
  });

  it("still drops arbitrary ORIRO_FOO", () => {
    const result = collectPreservedExistingServiceEnvVars({ ORIRO_FOO: "bar" }, managedKeys);
    expect(result.ORIRO_FOO).toBeUndefined();
  });

  it("preserves container opt-ins while dropping unrelated ORIRO_* keys", () => {
    const result = collectPreservedExistingServiceEnvVars(
      {
        ORIRO_CLI_CONTAINER_BYPASS: "1",
        ORIRO_CONTAINER_HINT: "ci",
    ...
    expect(result.ORIRO_CLI_CONTAINER_BYPASS).toBe("1");
    expect(result.ORIRO_CONTAINER_HINT).toBe("ci");
    expect(result.ORIRO_BAZ).toBeUndefined();
  });
});
