...
import {
  ...
  type OriroProviderIndexProvider,
...
type ProviderInstallCatalogParams = {
  config?: import("../config/types.oriro.js").OriroConfig;
  workspaceDir?: string;
...
function normalizeDefaultChoice(value: unknown): PluginPackageInstall["defaultChoice"] | undefined {
  return value === "orirohub" || value === "npm" || value === "local" ? value : undefined;
}
...
function resolveInstallInfoFromInstallRecord(
  ...
  if (record.source === "orirohub" && record.spec?.trim()) {
    return {
      orirohubSpec: record.spec.trim(),
...
function resolveInstallInfoFromPackageSource(params: {
  ...
  const orirohub = isRecord(source?.orirohub) ? source.orirohub : undefined;
  ...
  const orirohubSpec =
    params.origin === "bundled" || params.origin === "config"
      ? normalizeOptionalString(orirohub?.spec)
      : undefined;
  const localPath = normalizeOptionalString(local?.path);
  if (!orirohubSpec && !npmSpec && !localPath) {
    return null;
  }
  ...
  return {
    ...
    ...(defaultChoice
      ? { defaultChoice }
      : orirohubSpec
        ? { defaultChoice: "orirohub" as const }
        : npmSpec
...
function resolveInstallInfoFromProviderIndex(
  provider: OriroProviderIndexProvider,
): PluginPackageInstall | null {
  ...
  if (!orirohubSpec && !npmSpec) {
    return null;
  }
  const defaultChoice =
    normalizeDefaultChoice(install.defaultChoice) ?? (orirohubSpec ? "orirohub" : "npm");
  return {
    ...(orirohubSpec ? { orirohubSpec } : {}),
    ...(npmSpec ? { npmSpec } : {}),
    defaultChoice,
