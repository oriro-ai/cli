...
Oriro also includes `x_search` for X (formerly Twitter) posts and
...
<Steps>
  ...
  <Step title="Use it">
    ...
    ```javascript
    await web_search({ query: "Oriro plugin SDK" });
    ```
...
search and Oriro's managed `web_search` dynamic tool are mutually exclusive,
so managed search cannot bypass native domain restrictions. Oriro uses the
managed tool when hosted search is unavailable, explicitly disabled, or
replaced by a selected managed provider. Oriro keeps Codex's standalone
...
- Set `tools.web.search.provider` to a managed provider such as `brave` to use
  Oriro's managed `web_search` instead
...
use the managed `web_search` fallback through Oriro's dynamic tool namespace.
Use an explicit managed provider when you need Oriro's provider-specific
...
Codex app-server first with `oriro models auth login --provider openai`.
The parent agent can use any model or runtime; only the bounded search worker
runs through Codex.

## Network safety

Managed HTTP `web_search` provider calls use Oriro's guarded fetch path. For
trusted provider API hosts, Oriro allows Surge, Clash, and sing-box fake-IP
...
If no `provider` is set, Oriro checks providers in this order and uses the
...
`oriro configure --section web`. Oriro does not send managed
...
<Note>
  ...
  selected through auto-detect. In auto-detect mode, Oriro resolves only the
...
Grok can also reuse an xAI OAuth auth profile from `oriro models auth login
...
Oriro keeps startup resilient and reports a warning so you can reinstall the
plugin or run `oriro doctor --fix` to clean up the stale config.

`web_fetch` fallback provider selection is separate:

- choose it with `tools.web.fetch.provider`
- or omit that field and let Oriro auto-detect the first ready web-fetch
...
When you choose **Kimi** during `oriro onboard` or
`oriro configure --section web`, Oriro can also ask for:
...
Legacy `tools.web.x_search.*` config is auto-migrated by `oriro doctor --fix`.
When you choose Grok during `oriro onboard` or `oriro configure --section web`,
Oriro can also offer optional `x_search` setup with the same credential.
This is a separate follow-up step inside the Grok path, not a separate top-level
web-search provider choice. If you pick another provider, Oriro does not
...
<Tabs>
  <Tab title="Config file">
    Run `oriro configure --section web` or set the key directly:
  ...
  <Tab title="Environment variable">
    ...
    For a gateway install, put it in `~/.oriro/.env`.
...
optional structured filters. Oriro only enables the built-in xAI `x_search`
