---
summary: "Use OpenShell as a managed sandbox backend for Oriro agents"
title: OpenShell
read_when:
...
OpenShell is a managed sandbox backend for Oriro. Instead of running Docker
containers locally, Oriro delegates sandbox lifecycle to the `openshell` CLI,
...
2. Restart the Gateway. On the next agent turn, Oriro creates an OpenShell
...
- You edit files locally outside Oriro and want those changes visible in the
...
- When the sandbox is first created, Oriro seeds the remote workspace from
...
- Oriro does **not** sync remote changes back into the local workspace.
...
<Warning>
If you edit files on the host outside Oriro after the initial seed, the remote sandbox does **not** see those changes. Use `oriro sandbox recreate` to re-seed.
</Warning>
...
| `from`                    | `string`                 | `"oriro"`  | Sandbox source for first-time create                  |
...
1. Oriro calls `openshell sandbox create` (with `--from`, `--gateway`,
   `--policy`, `--providers`, `--gpu` flags as configured).
2. Oriro calls `openshell sandbox ssh-config <name>` to get SSH connection
...
- [Sandbox CLI](/cli/sandbox) -- `oriro sandbox` commands
