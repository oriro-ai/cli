---
summary: "Quick examples for listing, installing, updating, inspecting, and uninstalling Oriro plugins"
read_when:
...
[`oriro plugins`](/cli/plugins).

Most install workflows are:

1. find a package
2. install it from OriroHub, npm, git, or a local path
...
`plugins list` is a cold inventory check. It shows what Oriro can discover
...
install hints such as `oriro plugins install orirohub:<package>`.

## Install plugins

```bash
# Search OriroHub for plugin packages.
oriro plugins search "calendar"

# Install from OriroHub.
oriro plugins install orirohub:<package>
oriro plugins install orirohub:<package>@1.2.3
oriro plugins install orirohub:<package>@beta

# Install from npm.
oriro plugins install npm:<package>
oriro plugins install npm:@scope/oriro-plugin@1.2.3
oriro plugins install npm:@oriro/codex

# Install from a local npm pack artifact.
oriro plugins install npm-pack:<path.tgz>

# Install from git or a local development checkout.
oriro plugins install git:github.com/acme/oriro-plugin@v1.0.0
oriro plugins install ./my-plugin
oriro plugins install --link ./my-plugin
```

Bare package specs install from npm during the launch cutover. Use `orirohub:`,
`npm:`, `git:`, or `npm-pack:` when you need deterministic source selection.
If the bare name matches an official plugin id, Oriro can install the
catalog entry directly.

Use `--force` only when you intentionally want to overwrite an existing install
target. For routine upgrades of tracked npm, OriroHub, or hook-pack installs, use
...
```bash
oriro plugins update <plugin-id>
oriro plugins update <npm-package-or-spec>
oriro plugins update --all
oriro plugins update <plugin-id> --dry-run
```

When you pass a plugin id, Oriro reuses the tracked install spec. Stored
...
```bash
oriro plugins update @scope/oriro-plugin@beta
oriro plugins update @scope/oriro-plugin
```
...
When `oriro update` runs on the beta channel, plugin records can prefer
matching `@beta` releases. For the exact fallback and pinning rules, see
[`oriro plugins`](/cli/plugins#update).

## Uninstall plugins

```bash
oriro plugins uninstall <plugin-id> --dry-run
oriro plugins uninstall <plugin-id>
oriro plugins uninstall <plugin-id> --keep-files
```
...
In Nix mode (`ORIRO_NIX_MODE=1`), plugin install, update, uninstall, enable,
...
| ----------- | --------------------------------------------------------------------------- | -------------------------------------------------------------- |
| OriroHub     | You want Oriro-native discovery, scan summaries, versions, and hints     | `oriro plugins install orirohub:<package>`                   |
| npmjs.com   | You already ship JavaScript packages or need npm dist-tags/private registry | `oriro plugins install npm:@acme/oriro-plugin`           |
| git         | You want a branch, tag, or commit from a repository                         | `oriro plugins install git:github.com/<owner>/<repo>@<ref>` |
| local path  | You are developing or testing a plugin on the same machine                  | `oriro plugins install --link ./my-plugin`                  |
| npm pack    | You are proving a local package artifact through npm install semantics      | `oriro plugins install npm-pack:<path.tgz>`                 |
...
OriroHub is the primary public discovery surface for Oriro plugins. Publish
...
oriro plugins install npm:@acme/oriro-plugin
oriro plugins install npm:@acme/oriro-plugin@beta
oriro plugins install npm:@acme/oriro-plugin@1.0.0
```
...
If the same package is available on both OriroHub and npm, use the explicit
`orirohub:` or `npm:` prefix when you need to force one source.

## Related

- [Plugins](/tools/plugin) - install, configure, restart, and troubleshoot
- [`oriro plugins`](/cli/plugins) - full CLI reference
- [Community plugins](/plugins/community) - public discovery and OriroHub publishing
- [OriroHub](/orirohub/cli) - registry CLI operations
