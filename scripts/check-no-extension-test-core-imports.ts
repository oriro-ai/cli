// Check No Extension Test Core Imports script supports Oriro repository automation.
...
const FORBIDDEN_PATTERNS: Array<{ pattern: RegExp; hint: string }> = [
  {
    pattern: /["']oriro\/plugin-sdk["']/,
    hint: "Use oriro/plugin-sdk/<subpath> instead of the monolithic root entry.",
  },
  {
    pattern: /["']oriro\/plugin-sdk\/test-utils["']/,
    hint: "Use a focused plugin-sdk test subpath for the public extension test surface.",
  },
  {
    pattern: /["']oriro\/plugin-sdk\/testing["']/,
    hint: "Use a focused plugin-sdk test subpath instead of the broad compatibility testing barrel.",
  },
  {
    pattern: /["']oriro\/plugin-sdk\/compat["']/,
    hint: "Use a focused public plugin-sdk subpath instead of compat.",
  },
  {
    pattern: /["'](?:\.\.\/)+(?:test-utils\/)[^"']+["']/,
    hint: "Use a documented oriro/plugin-sdk test subpath for bundled extension test helpers.",
  },
  {
    pattern: /["'](?:\.\.\/)+(?:test\/helpers\/plugins\/)[^"']+["']/,
    hint: "Use a documented oriro/plugin-sdk test subpath instead of repo-only plugin helper bridges.",
  },
  {
    pattern: /["'](?:\.\.\/)+(?:test\/helpers\/channels\/)[^"']+["']/,
    hint: "Use oriro/plugin-sdk/channel-test-helpers or another focused SDK test subpath instead of repo-only channel helper bridges.",
  },
  {
    pattern: /["'](?:\.\.\/)+(?:test\/helpers\/media-generation\/)[^"']+["']/,
    hint: "Use oriro/plugin-sdk/provider-test-contracts or oriro/plugin-sdk/provider-http-test-mocks instead of repo-only media provider helper bridges.",
  },
  {
    pattern:
      /["'](?:\.\.\/)+(?:test\/helpers\/(?:bundled-channel-entry|envelope-timestamp|pairing-reply)\.(?:js|ts))["']/,
    hint: "Use oriro/plugin-sdk/channel-test-helpers instead of repo-only channel test helper bridges.",
  },
  {
    pattern:
      /["'](?:\.\.\/)+(?:test\/helpers\/(?:http-test-server|mock-incoming-request|temp-home)\.(?:js|ts))["']/,
    hint: "Use oriro/plugin-sdk/test-env instead of repo-only environment/network test helper bridges.",
  },
  {
    pattern:
      /["'](?:\.\.\/)+(?:test\/helpers\/(?:bundled-plugin-paths|import-fresh|node-builtin-mocks)\.(?:js|ts))["']/,
    hint: "Use oriro/plugin-sdk/test-fixtures instead of repo-only generic test helper bridges.",
  },
  {
    pattern:
      /["'](?:\.\.\/)+(?:test\/helpers\/(?:provider-replay-policy|stt-live-audio)\.(?:js|ts))["']/,
    hint: "Use oriro/plugin-sdk/provider-test-contracts instead of repo-only provider test helper bridges.",
  },
  {
    pattern: /["'](?:\.\.\/)+(?:test\/helpers\/)[^"']+["']/,
    hint: "Use a documented oriro/plugin-sdk test subpath instead of repo-only test helper bridges.",
  },
  {
    pattern: /["'](?:\.\.\/)+(?:src\/channels\/plugins\/contracts\/test-helpers\/)[^"']+["']/,
    hint: "Use oriro/plugin-sdk/channel-test-helpers or another focused SDK test subpath instead of core-only channel contract helpers.",
  },
  {
    pattern: /["'](?:\.\.\/)+(?:src\/test-utils\/)[^"']+["']/,
    hint: "Use a documented oriro/plugin-sdk test subpath for public surfaces.",
  },
  {
  ...
  },
  {
    ...
    hint: "Use oriro/plugin-sdk/channel-contract-testing for channel contract test helpers.",
  },
];
...
const RETIRED_EXTENSION_TEST_HELPER_BRIDGE_FILES = [
  ...
  "test/helpers/agents/oriro-owned-tool-runtime-contract.ts",
...
function main() {
  ...
  for (const file of retiredChannelHelperFiles) {
    offenders.push({
      file,
      hint: "Keep core channel contract helpers under src/channels/plugins/contracts/test-helpers and public plugin helpers under focused oriro/plugin-sdk test subpaths.",
    });
  }
  ...
  for (const file of RETIRED_EXTENSION_TEST_HELPER_BRIDGE_FILES) {
    ...
    offenders.push({
      file: filePath,
      hint: "Import the helper directly from a documented oriro/plugin-sdk testing subpath instead of recreating this bridge.",
    });
  }
