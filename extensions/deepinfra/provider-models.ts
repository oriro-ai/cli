...
import { isProviderApiKeyConfigured } from "oriro/plugin-sdk/provider-auth";
import {
...
import { asPositiveSafeInteger } from "oriro/plugin-sdk/string-coerce-runtime";
...
export const DEEPINFRA_MODELS_URL = `${DEEPINFRA_BASE_URL}/models?sort_by=oriro&filter=with_meta`;
...
// Static fallback. Chat rows live in oriro.plugin.json (manifest-validated);
...
// `/v1/openai/models?sort_by=oriro&filter=with_meta` projection is the
...
// `isProviderApiKeyConfigured` helper (covers SecretRef / `ORIRO_LIVE_*`
// indirection too).
