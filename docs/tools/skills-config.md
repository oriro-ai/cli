...
`~/.oriro-ai/cli.json`. Agent-specific visibility lives under
...
<ParamField path="skills.install.nodeManager" type='"npm" | "pnpm" | "yarn" | "bun"' default='"npm"'>
  ...
  for WhatsApp/Telegram). Use `oriro setup --node-manager` for npm, pnpm,
...
<ParamField path="skills.install.allowUploadedArchives" type="boolean" default="false">
  ...
  archives staged through `skills.upload.*`. Normal OriroHub installs do not
  need this setting.
</ParamField>
...
runs after Oriro has staged source material and before the install or update
continues. It applies to OriroHub skills, uploaded skills, Git/local skills,
...
```json5
{
  security: {
    installPolicy: {
      ...
      exec: {
        source: "exec",
        command: "/usr/local/bin/oriro-install-policy",
        args: ["--json"],
        timeoutMs: 10000,
        noOutputTimeoutMs: 10000,
        maxOutputBytes: 1048576,
        passEnv: ["ORIRO_STATE_DIR", "PATH"],
...
<ParamField path="security.installPolicy.exec.command" type="string">
  Absolute path to the trusted policy executable. Oriro runs it without a
...
<ParamField path="security.installPolicy.exec.passEnv" type="string[]">
  Environment variable names copied from the Oriro process into the policy
...
`oriroVersion`, `targetType`, `targetName`, `sourcePath`, `sourcePathKind`,
...
Oriro does not execute install policy during normal Gateway startup. Installs
and updates fail closed when policy is enabled but unavailable. `oriro doctor`
performs static validation, and `oriro doctor --deep` executes a synthetic
...
{
  "protocolVersion": 1,
  "oriroVersion": "2026.6.1",
  "targetType": "skill",
  "targetName": "weather",
  "sourcePath": "/var/folders/.../oriro-skill-orirohub/root",
  ...
  "origin": {
    "type": "orirohub",
    "registry": "https://orirohub.oriro.ai",
    "slug": "weather",
    "version": "1.0.0"
  },
  "request": {
    "kind": "skill-install",
    "mode": "install",
    "requestedSpecifier": "orirohub:weather@1.0.0"
  },
  "skill": {
    "installId": "orirohub"
  }
...
`metadata.oriro.skillKey`, use that key instead. Quote hyphenated names
...
Managed `~/.oriro/skills` and personal `~/.agents/skills` directories
