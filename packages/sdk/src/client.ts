// Oriro SDK module implements client behavior.
...
// High-level Oriro SDK client. Namespaces below translate friendly SDK calls
...
/** Connection and transport options for the Oriro SDK client. */
export type OriroOptions = {
  gateway?: "auto" | (string & {});
  url?: string;
  token?: string;
  password?: string;
  requestTimeoutMs?: number;
  transport?: OriroTransport;
};

function resolveGatewayUrl(options: OriroOptions): string | undefined {
  if (options.url) {
    return options.url;
  }
...
function readChatProjection(event: OriroEvent): ChatProjection | undefined {
  const raw = event.raw;
...
function isAssistantRunEvent(event: OriroEvent): boolean {
  return event.type === "assistant.delta" || event.type === "assistant.message";
}

function isTerminalRunEvent(event: OriroEvent): boolean {
  return (
...
export class Oriro {
  ...
  private readonly transport: OriroTransport;
  private readonly normalizedEvents = new EventHub<OriroEvent>({
    replayLimit: MAX_NORMALIZED_REPLAY_EVENTS,
  });
  private readonly replayByRunId = new Map<string, OriroEvent[]>();
  ...
  constructor(options: OriroOptions = {}) {
    this.transport =
      options.transport ??
  ...
  events(filter?: (event: OriroEvent) => boolean): AsyncIterable<OriroEvent> {
    return this.iterateEvents(filter);
  }

  runEvents(
    runId: string,
    filter?: (event: OriroEvent) => boolean,
  ): AsyncIterable<OriroEvent> {
  ...
  private async *iterateRunEvents(
    ...
    const toRunStreamEvent = (event: OriroEvent): OriroEvent | undefined => {
    ...
    const matches = (event: OriroEvent) => event.runId === runId;
  ...
  private recordReplayEvent(event: OriroEvent): void {
    if (!event.runId) {
      return;
    }
  ...
  private replaySnapshot(runId: string): OriroEvent[] {
...
export class RunsNamespace {
  ...
  events(runId: string): AsyncIterable<OriroEvent> {
...
class RpcNamespace {
  constructor(
    protected readonly client: Oriro,
