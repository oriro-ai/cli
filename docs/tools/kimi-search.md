...
Oriro supports Kimi as a `web_search` provider, using Moonshot web search
...
`https://api.moonshot.cn/v1`), Oriro reuses that same host for Kimi
...
Gateway environment. For a gateway install, put it in `~/.oriro/.env`.

If you omit `baseUrl`, Oriro defaults to `https://api.moonshot.ai/v1`.
If you omit `model`, Oriro defaults to `kimi-k2.6`.
...
Oriro treats Kimi `web_search` as successful only after Moonshot returns
...
Oriro returns a structured `kimi_web_search_ungrounded` error instead of
