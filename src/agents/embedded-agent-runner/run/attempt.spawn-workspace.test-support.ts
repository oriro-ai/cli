...
type AttemptSpawnWorkspaceHoisted = {
  ...
  createOriroCodingToolsMock: UnknownMock;
...
vi.mock("../../docs-path.js", () => ({
  resolveOriroReferencePaths: async () => ({ docsPath: undefined, sourcePath: undefined }),
}));
...
vi.mock("../../agent-tools.js", () => ({
  createOriroCodingTools: (options?: { workspaceDir?: string; spawnWorkspaceDir?: string }) =>
    hoisted.createOriroCodingToolsMock(options),
...
vi.mock("../cache-ttl.js", () => ({
  appendCacheTtlTimestamp: (
    ...
    data: unknown,
  ) => sessionManager.appendCustomEntry?.("oriro.cache-ttl", data),
...
export function resetEmbeddedAttemptHarness(
  ...
  hoisted.createOriroCodingToolsMock.mockReset().mockImplementation((...args: unknown[]) => {
...
export async function createContextEngineAttemptRunner(params: {
  ...
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-ctx-engine-workspace-"));
  const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-ctx-engine-agent-"));
  ...
  const previousTrajectoryEnv = process.env.ORIRO_TRAJECTORY;
  if (params.trajectory !== true) {
    process.env.ORIRO_TRAJECTORY = "0";
  }
  try {
    return await (
      await loadRunEmbeddedAttempt()
    ...
    if (previousTrajectoryEnv === undefined) {
      delete process.env.ORIRO_TRAJECTORY;
    } else {
      process.env.ORIRO_TRAJECTORY = previousTrajectoryEnv;
    }
  }
}
