...
describe("plugins cli list", () => {
  ...
  it("reports stale plugin config in doctor output without claiming full plugin health", async () => {
    ...
    expect(output).toContain('plugins.allow: stale plugin reference "lossless-oriro" was found.');
    expect(output).toContain(
      'plugins.entries.lossless-oriro: stale plugin reference "lossless-oriro" was found.',
    );
    expect(output).toContain(
      'plugins.slots.contextEngine: slot references missing plugin "lossless-oriro".',
    );
    expect(output).toContain(
      'Run "oriro doctor --fix" to remove stale plugin ids and dangling channel references.',
    );
    expect(output).toContain(
  ...
  it("reports missing configured Codex runtime plugin in doctor output", async () => {
    ...
    expect(output).toContain("oriro doctor --fix");
    expect(output).toContain("oriro plugins install @oriro/codex");
    expect(output).toContain(
      "No plugin install-tree issues detected; configuration warnings remain.",
    );
    expect(output).not.toContain("No plugin issues detected.");
  ...
  it("reports missing configured ACPX runtime plugin in doctor output", async () => {
    ...
    expect(output).toContain("oriro doctor --fix");
    expect(output).toContain("oriro plugins install @oriro/acpx");
    expect(output).not.toContain("No plugin issues detected.");
  });
  ...
  it("reports blocked configured ACPX runtime with ACP-specific guidance", async () => {
    ...
    expect(output).not.toContain('runtime policy to "oriro"');
    expect(output).not.toContain("oriro plugins install @oriro/acpx");
    expect(output).not.toContain("No plugin issues detected.");
  });
  ...
  it("reports configured Codex runtime when the plugin record is disabled", async () => {
    ...
    expect(output).not.toContain("oriro plugins install @oriro/codex");
    expect(output).not.toContain("No plugin issues detected.");
  });
  ...
  it("reports blocked configured Codex runtime without install advice", async () => {
    ...
    expect(output).not.toContain('Run "oriro doctor --fix" to install');
    expect(output).not.toContain("oriro plugins install @oriro/codex");
    expect(output).not.toContain("No plugin issues detected.");
  });
  ...
  it("reports config-selected plugin source shadowing in doctor output", async () => {
    buildPluginDiagnosticsReport.mockReturnValue({
      plugins: [
        createPluginRecord({
          id: "discord",
          origin: "config",
          source: "/tmp/oriro-upstream/extensions/discord/index.ts",
          status: "error",
      ...
      diagnostics: [
        {
          level: "warn",
          pluginId: "discord",
          source: "/tmp/oriro/npm/node_modules/@oriro/discord/index.ts",
          message:
            "duplicate plugin id resolved by explicit config-selected plugin; global plugin will be overridden by config plugin (/tmp/oriro-upstream/extensions/discord/index.ts)",
        },
      ],
    });
    ...
    expect(output).toContain("active: /tmp/oriro-upstream/extensions/discord/index.ts");
    expect(output).toContain("shadowed: /tmp/oriro/npm/node_modules/@oriro/discord/index.ts");
    expect(output).toContain("oriro plugins registry --refresh");
  });
  ...
  it("reports persisted plugin registry state without refreshing", async () => {
    ...
    expect(runtimeLogs.join("\n")).toContain("oriro plugins registry --refresh");
  });
  ...
  it("keeps inspect on the static snapshot by default", async () => {
    setInstalledPluginIndexInstallRecords({
      "oriro-mem0": {
        source: "orirohub",
        spec: "orirohub:oriro-mem0",
        installPath: "/plugins/oriro-mem0",
        version: "2026.5.1",
        orirohubPackage: "oriro-mem0",
        orirohubChannel: "official",
        artifactKind: "npm-pack",
        artifactFormat: "tgz",
        npmIntegrity: "sha512-oriropack",
        npmShasum: "1".repeat(40),
        npmTarballName: "oriro-mem0-2026.5.1.tgz",
    ...
    buildPluginSnapshotReport.mockReturnValue({
      plugins: [createPluginRecord({ id: "oriro-mem0", name: "Mem0" })],
      diagnostics: [],
    });
    buildPluginInspectReport.mockReturnValue({
      workspaceDir: "/workspace",
      plugin: createPluginRecord({ id: "oriro-mem0", name: "Mem0" }),
      shape: "hook-only",
    ...
    await runPluginsCommand(["plugins", "inspect", "oriro-mem0"]);

    expect(buildPluginDiagnosticsReport).not.toHaveBeenCalled();
    expect(runtimeLogs.join("\n")).toContain("Policy");
    expect(runtimeLogs.join("\n")).toContain("allowConversationAccess: true");
    expect(runtimeLogs.join("\n")).toContain("OriroHub package: oriro-mem0");
    expect(runtimeLogs.join("\n")).toContain("Artifact kind: npm-pack");
    expect(runtimeLogs.join("\n")).toContain("Npm integrity: sha512-oriropack");
    expect(runtimeLogs.join("\n")).toContain(
      "OriroPack sha256: aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa",
    );
    expect(runtimeLogs.join("\n")).toContain("OriroPack spec: 1");
    expect(runtimeLogs.join("\n")).toContain("OriroPack size: 4096 bytes");
  });
  ...
  it("runtime-inspects without repairing deps", async () => {
    ...
    await runPluginsCommand(["plugins", "inspect", "oriro-mem0", "--runtime"]);

    expect(buildPluginDiagnosticsReport).toHaveBeenCalledWith({
      config: {},
      onlyPluginIds: ["oriro-mem0"],
    });
  });
