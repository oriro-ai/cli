...
} & PluginActivationConfigSourceLike<OriroConfig>;
...
const hasExplicitMemorySlot = (plugins?: OriroConfig["plugins"]) =>
  Boolean(plugins?.slots && Object.hasOwn(plugins.slots, "memory"));

const hasExplicitMemoryEntry = (plugins?: OriroConfig["plugins"]) =>
  Boolean(plugins?.entries && Object.hasOwn(plugins.entries, defaultSlotIdForKey("memory")));

export const hasExplicitPluginConfig = (plugins?: OriroConfig["plugins"]) =>
