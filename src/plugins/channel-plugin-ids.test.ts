...
function withManifestLoadPaths<T extends { id: string }>(
  ...
  return {
    ...
    manifestPath: `/tmp/plugins/${plugin.id}/oriro.plugin.json`,
    skills: [],
    hooks: [],
    ...plugin,
  };
...
describe("resolveGatewayStartupPluginIds", () => {
  beforeEach(() => {
    listPotentialConfiguredChannelIds.mockReset().mockImplementation((config: OriroConfig) => {
    ...
    listPotentialConfiguredChannelPresenceSignals
      .mockReset()
      .mockImplementation((config: OriroConfig) => {
        return listPotentialConfiguredChannelIds(config).map((channelId: string) => ({
          channelId,
          source: "config",
        }));
      });
    hasPotentialConfiguredChannels.mockReset().mockImplementation((config: OriroConfig) => {
      if (Object.hasOwn(config, "channels")) {
  ...
  it("does not treat persisted auth alone as gateway startup intent", () => {
    ...
    expectStartupPluginIdsCase({
      ...
      env: createPluginPlanningTestEnv({
        ORIRO_STATE_DIR: "/tmp/oriro-with-persisted-demo-channel",
      }),
      expected: ["browser", "memory-core"],
    });
  });
  ...
  it("includes the selected context-engine slot plugin in startup scope even without activation.onStartup (#76576)", () => {
    expectStartupPluginIdsCase({
      config: createStartupConfig({
        enabledPluginIds: ["lossless-oriro"],
        contextEngine: "lossless-oriro",
      }),
      expected: ["demo-channel", "browser", "memory-core", "lossless-oriro"],
    });
  });
  ...
  it("does not include Codex when an OpenAI model is manually pinned to Oriro", () => {
    expectStartupPluginIdsCase({
      config: {
        agents: {
          defaults: {
