 ...
 */
/** Opening delimiter for protected Oriro runtime context blocks. */
export const INTERNAL_RUNTIME_CONTEXT_BEGIN = "<<<BEGIN_ORIRO_INTERNAL_CONTEXT>>>";
/** Closing delimiter for protected Oriro runtime context blocks. */
export const INTERNAL_RUNTIME_CONTEXT_END = "<<<END_ORIRO_INTERNAL_CONTEXT>>>";

const ESCAPED_INTERNAL_RUNTIME_CONTEXT_BEGIN = "[[ORIRO_INTERNAL_CONTEXT_BEGIN]]";
const ESCAPED_INTERNAL_RUNTIME_CONTEXT_END = "[[ORIRO_INTERNAL_CONTEXT_END]]";

/** Notice inserted into runtime-generated context blocks. */
export const ORIRO_RUNTIME_CONTEXT_NOTICE =
...
function isRuntimeContextPromptHeader(line: string): boolean {
  return (
    line === ORIRO_NEXT_TURN_RUNTIME_CONTEXT_HEADER || line === ORIRO_RUNTIME_EVENT_HEADER
  );
}
...
function stripRuntimeContextPromptPreface(text: string): string {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    if (
      ...
      nextLine.trim() === ORIRO_RUNTIME_CONTEXT_NOTICE
    ) {
      changed = true;
      index += 1;
...
export function hasInternalRuntimeContext(text: string): boolean {
  ...
  return (
    ...
    text.includes(
      `${ORIRO_NEXT_TURN_RUNTIME_CONTEXT_HEADER}\n${ORIRO_RUNTIME_CONTEXT_NOTICE}`,
    ) ||
    text.includes(`${ORIRO_RUNTIME_EVENT_HEADER}\n${ORIRO_RUNTIME_CONTEXT_NOTICE}`)
  );
}

function isOriroRuntimeContextCustomMessage(message: unknown): boolean {
  if (!message || typeof message !== "object") {
    return false;
  }
  const candidate = message as { role?: unknown; customType?: unknown };
  return (
    candidate.role === "custom" && candidate.customType === ORIRO_RUNTIME_CONTEXT_CUSTOM_TYPE
  );
}

/** Remove all structured runtime-context custom messages. */
export function stripRuntimeContextCustomMessages<T>(messages: T[]): T[] {
  if (!messages.some(isOriroRuntimeContextCustomMessage)) {
    return messages;
  }
  return messages.filter((message) => !isOriroRuntimeContextCustomMessage(message));
}
...
export function stripHistoricalRuntimeContextCustomMessages<T>(messages: T[]): T[] {
  ...
  for (let index = lastUserIndex - 1; index >= 0; index -= 1) {
    if (!isOriroRuntimeContextCustomMessage(messages[index])) {
      break;
    }
    currentRuntimeContextIndexes.add(index);
  }
  return messages.filter((message, index) => {
    if (!isOriroRuntimeContextCustomMessage(message)) {
      return true;
    }
