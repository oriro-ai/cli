...
import type { OriroPluginNodeInvokePolicy } from "oriro/plugin-sdk/plugin-entry";
import { FILE_TRANSFER_NODE_INVOKE_COMMANDS } from "./node-invoke-policy-commands.js";

type LoadFileTransferNodeInvokePolicy = () => Promise<OriroPluginNodeInvokePolicy>;
...
export function createLazyFileTransferNodeInvokePolicy(
  ...
  let policyPromise: Promise<OriroPluginNodeInvokePolicy> | undefined;

  return {
    commands: [...FILE_TRANSFER_NODE_INVOKE_COMMANDS],
    async handle(ctx) {
      let policy: OriroPluginNodeInvokePolicy;
      try {
