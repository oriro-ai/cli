---
summary: "CLI reference for `oriro agent` (send one agent turn via the Gateway)"
read_when:
...
```bash
oriro agent --to +15555550123 --message "status update" --deliver
oriro agent --agent ops --message "Summarize logs"
oriro agent --agent ops --model openai/gpt-5.4 --message "Summarize logs"
oriro agent --session-key agent:ops:incident-42 --message "Summarize status"
oriro agent --agent ops --session-key incident-42 --message "Summarize status"
oriro agent --session-id 1234 --message "Summarize inbox" --thinking medium
oriro agent --to +15555550123 --message "Trace logs" --verbose on --json
oriro agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"
oriro agent --agent ops --message "Run locally" --local
```
