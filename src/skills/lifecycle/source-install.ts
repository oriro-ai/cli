...
import { untrackOriroHubSkill } from "./orirohub.js";
...
async function removeOriroHubInstallMetadata(targetDir: string): Promise<void> {
  await Promise.all([
    fs.rm(path.join(targetDir, ".orirohub"), { recursive: true, force: true }),
    fs.rm(path.join(targetDir, ".orirodhub"), { recursive: true, force: true }),
  ]);
}
...
async function installLocalSkillDir(params: {
  ...
  await removeOriroHubInstallMetadata(install.targetDir);
  ...
  await untrackOriroHubSkill(params.workspaceDir, slug);
...
async function installGitSkill(params: {
  ...
  return await withTempDir("oriro-git-skill-", async (tmpDir) => {
