...
type CrestodianLocalPlannerBackend = {
  ...
  provider: string;
  model: string;
  buildConfig: (workspaceDir: string) => OriroConfig;
};
...
function buildCliPlannerConfig(workspaceDir: string, modelRef: string): OriroConfig {
  return {
    agents: {
      defaults: {
...
function buildCodexAppServerPlannerConfig(workspaceDir: string): OriroConfig {
  return {
    agents: {
      defaults: {
        workspace: workspaceDir,
