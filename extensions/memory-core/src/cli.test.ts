...
vi.mock("./cli.host.runtime.js", async () => {
  const [runtimeCli, runtimeCore, runtimeFiles] = await Promise.all([
    import("oriro/plugin-sdk/memory-core-host-runtime-cli"),
    import("oriro/plugin-sdk/memory-core-host-runtime-core"),
    import("oriro/plugin-sdk/memory-core-host-runtime-files"),
  ]);
  return {
...
let defaultRuntime: typeof import("oriro/plugin-sdk/memory-core-host-runtime-cli").defaultRuntime;
let isVerbose: typeof import("oriro/plugin-sdk/memory-core-host-runtime-cli").isVerbose;
let setVerbose: typeof import("oriro/plugin-sdk/memory-core-host-runtime-cli").setVerbose;
let fixtureRoot = "";
...
beforeAll(async () => {
  ...
  ({ defaultRuntime, isVerbose, setVerbose } =
    await import("oriro/plugin-sdk/memory-core-host-runtime-cli"));
...
describe("memory cli", () => {
  ...
  it("prints index identity mismatch reasons", async () => {
    ...
    expectLogged(log, "Fix: Run: oriro memory status --index --agent main");
    expect(close).toHaveBeenCalled();
  });
  ...
  it("shows the fix hint only before --fix has been run", async () => {
    await withTempWorkspace(async (workspaceDir) => {
      ...
      const log = spyRuntimeLogs(defaultRuntime);
      await runMemoryCli(["status"]);
      expectLogged(log, "Fix: oriro memory status --fix --agent main");
      ...
      await runMemoryCli(["status", "--fix"]);
      expectNotLogged(log, "Fix: oriro memory status --fix --agent main");
    });
  });
  ...
  it("writes grounded rem backfill entries into DREAMS.md", async () => {
    await withTempWorkspace(async (workspaceDir) => {
      ...
      expect(dreams).toContain("oriro:dreaming:backfill-entry");
  ...
  it("prefers persistence-relevant evidence over narrated operational logs in grounded what happened", async () => {
    await withTempWorkspace(async (workspaceDir) => {
      ...
      await fs.writeFile(
        historyPath,
        [
          "## Oriro / runtime / workflow preferences and corrections",
      ...
      expect(rendered).not.toContain("jporirohq updated and running");
      expect(close).toHaveBeenCalled();
    });
  });
    ...
    await withTempWorkspace(async (workspaceDir) => {
      ...
      await fs.writeFile(
        ...
        [
          ...
          "*January 1, 2025*",
          "",
          "<!-- oriro:dreaming:backfill-entry day=2025-01-01 source=memory/2025-01-01.md -->",
          "",
          "What Happened",
          "1. Remove this entry.",
          "",
          "<!-- oriro:dreaming:diary:end -->",
          "",
        ].join("\n"),
        "utf-8",
      );
  ...
  it("applies top promote candidates into MEMORY.md", async () => {
    await withTempWorkspace(async (workspaceDir) => {
      ...
      expect(memoryText).toContain("oriro-memory-promotion:");
