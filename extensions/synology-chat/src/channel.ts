/**
 * Synology Chat Channel Plugin for Oriro.
 *
...
import { createEmptyChannelDirectoryAdapter } from "oriro/plugin-sdk/directory-runtime";
import {
...
const resolveSynologyChatDmPolicy = createScopedDmSecurityResolver<ResolvedSynologyChatAccount>({
  ...
  approveHint: "oriro pairing approve synology-chat <code>",
...
> & {
  pairing: {
    ...
    notifyApproval: (params: { cfg: OriroConfig; id: string }) => Promise<void>;
  };
  security: {
    resolveDmPolicy: (params: { cfg: OriroConfig; account: ResolvedSynologyChatAccount }) => {
