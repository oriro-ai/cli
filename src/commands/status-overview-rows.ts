// Builds overview table rows for `oriro status` and `oriro status --all`.
...
/** Builds the default `oriro status` overview rows from scan, health, memory, and session inputs. */
  ...
  return buildStatusOverviewRowsFromSurface({
    ...
    middleRows: [
      ...(params.updateRestartValue
        ? [{ Item: "Update restart", Value: params.updateRestartValue }]
        : []),
      { Item: "Security", Value: `Run: ${formatCliCommand("oriro security audit --deep")}` },
    ],
