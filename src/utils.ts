...
/** Resolves `~` and Oriro home-relative paths with injectable env/home sources. */
...
/** Resolves the Oriro config directory from state/config env overrides or home. */
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".oriro");
  try {
...
/** Resolves the effective Oriro home directory, if one can be determined. */
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.ORIRO_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$ORIRO_HOME" };
  }
  return { home, prefix: "~" };
}

/** Replaces the leading home directory in a path with `~` or `$ORIRO_HOME`. */
