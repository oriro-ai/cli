...
const gatewayAlreadyRunningHint = `Gateway already running locally. Stop it (${formatCliCommand("oriro gateway stop")}) or use a different port.`;
...
describe("ports-format", () => {
  it.each([
    ...
    [{ commandLine: "node /Users/me/Projects/oriro/dist/entry.js gateway" }, "gateway"],
  ...
  it("builds ordered hints for mixed listener kinds and multiplicity", () => {
    expect(
      buildPortHints(
        [
          { commandLine: "node dist/index.js oriro gateway" },
  ...
  it("treats single-process loopback dual-stack gateway listeners as benign", () => {
    const listeners = [
      { pid: 4242, commandLine: "oriro-gateway", address: "127.0.0.1:18789" },
      { pid: 4242, commandLine: "oriro-gateway", address: "[::1]:18789" },
    ];
  ...
  it.each([
    ...
    const listeners = [{ pid: 4242, commandLine: "oriro-gateway", address }];
  ...
  it("keeps Gateway conflict hints for ambiguous Gateway listeners", () => {
    expect(
      buildPortHints(
        [
          { pid: 4242, commandLine: "oriro-gateway", address: "0.0.0.0:18789" },
          { pid: 4243, commandLine: "oriro-gateway", address: "127.0.0.1:18789" },
        ],
        18789,
      ),
