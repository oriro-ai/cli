// Plugin state SQLite helpers persist plugin state in the Oriro state database.
...
type PluginStateEntriesTable = OriroStateKyselyDatabase["plugin_state_entries"];
type PluginStateStoreDatabase = Pick<OriroStateKyselyDatabase, "plugin_state_entries">;
...
function wrapPluginStateError(
  ...
  message: string,
  pathname = resolveOriroStateSqlitePath(process.env),
): PluginStateStoreError {
...
function parseStoredJson(raw: string, operation: PluginStateStoreOperation): unknown {
  try {
    ...
    throw createPluginStateError({
      ...
      path: resolveOriroStateSqlitePath(process.env),
      cause: error,
    });
  }
}
...
function openPluginStateDatabase(
  ...
  const pathname = resolveOriroStateSqlitePath(env);
...
function envOptions(env?: NodeJS.ProcessEnv): OriroStateDatabaseOptions {
  return env ? { env } : {};
}
...
function runWriteTransaction<T>(
  ...
  return runOriroStateWriteTransaction(() => {
...
export function probePluginStateStore(): PluginStateStoreProbeResult {
  const databasePath = resolveOriroStateSqlitePath(process.env);
  const steps: PluginStateStoreProbeStep[] = [];
  const wasOpen = cachedDatabase !== null;
  const stateWasOpen = isOriroStateDatabaseOpen();
  ...
  try {
    ...
    openOriroStateDatabase().walMaintenance.checkpoint();
    pushOk("checkpoint");
  } catch (error) {
