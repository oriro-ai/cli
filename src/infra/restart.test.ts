...
const { triggerOriroRestart } = await import("./restart.js");
...
describe.runIf(process.platform !== "win32")("findGatewayPidsOnPortSync", () => {
  it("parses lsof output and filters non-oriro/current processes", () => {
  ...
  it("kills stale gateway pids discovered on the gateway port", () => {
    ...
    spawnSyncMock
      .mockReturnValueOnce({
        error: undefined,
        status: 0,
        stdout: [`p${stalePidA}`, "coriro", `p${stalePidB}`, "coriro-gateway"].join("\n"),
      })
      .mockReturnValue({
        error: undefined,
        status: 1,
...
describe("triggerOriroRestart", () => {
  it("does not kickstart after bootstrap registers an unloaded LaunchAgent", () => {
    setPlatform("darwin");
    withEnv(
      { VITEST: undefined, NODE_ENV: undefined, HOME: "/Users/test", ORIRO_PROFILE: "default" },
      () => {
        ...
        const result = triggerOriroRestart();
        ...
        expect(result).toEqual({
          ...
          tried: [
            ...
            `launchctl bootstrap gui/${uid} /Users/test/Library/LaunchAgents/ai.oriro.gateway.plist`,
          ],
        });
      },
    );
      ...
      () => {
        ...
        expect(result).toEqual({
          ...
          tried: [
            `launchctl kickstart -k gui/${uid}/ai.oriro.gateway`,
            `launchctl bootstrap gui/${uid} /Users/test/Library/LaunchAgents/ai.oriro.gateway.plist`,
            `launchctl kickstart gui/${uid}/ai.oriro.gateway`,
          ],
        });
      },
    );
