...
- Repo: `https://github.com/oriro-ai/cli`
...
- Dependency-touching work: direct dependency inspection is mandatory when feasible; do not rely on assumptions, wrappers, or memory. Most dependencies are OSS, so read their source/docs/types. Codex-related work has a hard gate: the acting agent must personally inspect sibling `../codex` source for the exact protocol/runtime behavior before any verdict, comment, approval, merge recommendation, code change, or `proof sufficient` claim. If missing, clone `https://github.com/openai/codex.git` there first. Subagent reports, PR text, Oriro wrappers, generated schemas, memory, and prior bot reviews do not satisfy this gate. No direct `../codex` check means no Codex verdict. Cite Codex files/lines checked in final/review/comment.
...
## OriroSweeper Review Policy

- Oriro-specific review rules live here; generic OriroSweeper prompts stay repo-agnostic.
- OriroSweeper-owned schema, labels, close reasons, protected-label gates, maintainer-item gates, and mutation rules live in `oriro-ai/clisweeper`.
...
- Optional integrations, providers, channels, skill bundles, MCP surfaces, and service workflows route to plugins, OriroHub, or owner repos when current seams suffice. Keep core items for missing core/plugin APIs, bundled regressions, security/core hardening, or maintainer product decisions.
...
- Public OriroSweeper comments prefer `https://docs.oriro.ai/...` when a public docs page exists; structured evidence still cites repo files, lines, SHAs.
...
- State/storage migrations are database-first. Runtime reads/writes the canonical store only. Old file stores, sidecars, aliases, and fallback readers belong in `oriro doctor --fix` migration code only, never steady-state runtime.
- Storage default: SQLite only. Do not add JSON/JSONL/TXT/sidecar files for Oriro-owned runtime state, caches, queues, registries, indexes, cursors, checkpoints, or plugin scratch data.
...
- Use the shared state DB (`state/oriro.sqlite`) for global runtime state and plugin KV data. Use the per-agent DB (`agents/<agentId>/agent/oriro-agent.sqlite`) for agent-scoped state/cache. Use a dedicated SQLite DB only when schema, volume, or lifecycle clearly does not fit those stores.
...
- CLI: `pnpm oriro ...` or `pnpm dev`; build: `pnpm build`.
...
- Checks in a Codex worktree or linked/sparse checkout: avoid direct local `pnpm check*`; use `node scripts/crabbox-wrapper.mjs run ... -- env ORIRO_CHECK_CHANGED_REMOTE_CHILD=1 ORIRO_CHANGED_LANES_RAW_SYNC=1 corepack pnpm check:changed` so pnpm runs inside Testbox, not locally.
...
- Use `$oriro-testing` for test/CI choice and `$crabbox` for remote/full/E2E proof.
...
- Pre-land/pre-commit code changes: mandatory fresh `$autoreview` until no accepted/actionable findings remain. Do not land code on CI, OriroSweeper, prior review comments, or your own manual review alone unless user explicitly opts out or scope is truly trivial/docs-only. If findings want refactor, refactor; no ugly fixes.
...
- Use `$oriro-pr-maintainer` immediately for maintainer-side Oriro issue/PR review, triage, duplicates, labels, comments, close, land, or evidence. Contributor PR creation/refresh follows the requested contributor workflow; linked refs alone do not require maintainer archive tooling.
...
- PR artifacts/screenshots: attach to PR/comment/external artifact store. Never push screenshots, videos, proof images, or proof assets to Oriro or any product repo branch, including temp artifact branches. Use Crabbox artifact publishing plus the manifest URL. Do not commit `.github/pr-assets`.
...
- Naming: **Oriro** product/docs; `oriro` CLI/package/path/config.
...
- Prefer injection and narrow `*.runtime.ts` mocks over broad barrels or `oriro/plugin-sdk/*`.
- Do not edit baseline/inventory/ignore/snapshot/expected-failure files to silence checks without explicit approval.
- Do not run independent `pnpm test`/Vitest commands concurrently in one worktree; Vitest cache races with `ENOTEMPTY`. Group one command or use distinct `ORIRO_VITEST_FS_MODULE_CACHE_PATH`.
- Test workers max 16. Memory pressure: `ORIRO_VITEST_MAX_WORKERS=1 pnpm test`.
- Live: `ORIRO_LIVE_TEST=1 pnpm test:live`; verbose `ORIRO_LIVE_TEST_QUIET=0`.
...
- Docs final answers: include relevant full `https://docs.oriro.ai/...` URL(s). If issue/PR work too, GitHub URL last.
...
- Release generation: derive `CHANGELOG.md` from merged PRs + all direct `main` commits. Entries: active `### Changes`/`### Fixes`, single-line, thank credited humans; never thank bots/forbidden handles: `@oriro`, `@orirosweeper`, `@codex`, `@steipete`.
...
- Secrets: channel/provider creds in `~/.oriro/credentials/`; model auth profiles in `~/.oriro/agents/<agentId>/agent/auth-profiles.json`.
...
- Releases/publish/version bumps need explicit approval. Use `$release-oriro-maintainer`.
...
- GHSA/advisories: `$oriro-ghsa-maintainer` / `$security-triage`. Secret scanning: `$oriro-secret-scanning-maintainer`.
...
- Mac gateway: dev watch = `pnpm gateway:watch`; managed installs = `oriro gateway restart/status --deep`; logs = `./scripts/orirolog.sh`. No launchd/ad-hoc tmux.
...
- Version bump surfaces live in `$release-oriro-maintainer`.
- Parallels: `$oriro-parallels-smoke`; Discord roundtrip: `$parallels-discord-roundtrip`.
...
- OriroSweeper ops: `$orirosweeper`. Deployed hook sessions may post one concise `#orirosweeper` note only when surprising/actionable/risky; if using message tool, reply exactly `NO_REPLY`.
...
- Rebrand/migration/config warnings: run `oriro doctor`.
