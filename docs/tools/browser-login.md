...
When a site requires login, **sign in manually** in the **host** browser profile (the oriro browser).
...
Oriro controls a **dedicated Chrome profile** (named `oriro`, orange-tinted UI). This is separate from your daily browser profile.

For agent browser tool calls:

- Default choice: the agent should use its isolated `oriro` browser.
...
```bash
oriro browser start
oriro browser open https://x.com
```

If you have multiple profiles, pass `--browser-profile <name>` (the default is `oriro`).
...
```bash
oriro browser open https://x.com --browser-profile oriro
```
