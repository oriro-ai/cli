/**
 * Oriro ACPX runtime adapter. It wraps the upstream acpx runtime with
 * Oriro session metadata, lease tracking, model scoping, and cleanup policy.
 */
...
import { parseStrictPositiveInteger } from "oriro/plugin-sdk/number-runtime";
import { redactSensitiveText } from "oriro/plugin-sdk/security-runtime";
import { normalizeStringEntries } from "oriro/plugin-sdk/string-coerce-runtime";
...
import {
  cleanupOriroOwnedAcpxProcessTree,
  isOriroLeaseAwareAcpxProcessCommand,
...
type OriroAcpxRuntimeOptions = AcpRuntimeOptions & {
  oriroWrapperRoot?: string;
  oriroGatewayInstanceId?: string;
  oriroProcessLeaseStore?: AcpxProcessLeaseStore;
};
type AcpxRuntimeTestOptions = Record<string, unknown> & {
  oriroProcessCleanup?: AcpxProcessCleanupDeps;
};
type OriroRuntimeTurnInput = Parameters<NonNullable<AcpRuntime["startTurn"]>>[0];
type OriroRuntimeEnsureInput = Parameters<AcpRuntime["ensureSession"]>[0];
...
type OriroLeaseSessionMetadata = {
  oriroLeaseId: string;
  oriroGatewayInstanceId: string;
};

function withOriroManagedTurnTimeout<T extends object>(input: T): T & { timeoutMs: 0 } {
  // Oriro owns ACP turn deadlines. acpx treats timeout after partial agent
...
function withOriroLeaseSessionMetadata<T extends object>(
  record: T,
  metadata: OriroLeaseSessionMetadata,
): T & OriroLeaseSessionMetadata {
  return {
    ...record,
    oriroLeaseId: metadata.oriroLeaseId,
    oriroGatewayInstanceId: metadata.oriroGatewayInstanceId,
  };
}
...
function readOriroLeaseIdFromRecord(record: unknown): string | undefined {
  if (typeof record !== "object" || record === null) {
    return undefined;
  }
  const { oriroLeaseId } = record as { oriroLeaseId?: unknown };
  return typeof oriroLeaseId === "string" ? oriroLeaseId.trim() || undefined : undefined;
}

function readOriroGatewayInstanceIdFromRecord(record: unknown): string | undefined {
  if (typeof record !== "object" || record === null) {
    return undefined;
  }
  const { oriroGatewayInstanceId } = record as { oriroGatewayInstanceId?: unknown };
  return typeof oriroGatewayInstanceId === "string"
    ? oriroGatewayInstanceId.trim() || undefined
    : undefined;
}
...
function createResetAwareSessionStore(
  ...
  return {
    async load(sessionId: string): Promise<AcpLoadedSessionRecord> {
      ...
      return withOriroLeaseSessionMetadata(record, {
        oriroLeaseId: lease.leaseId,
        oriroGatewayInstanceId: lease.gatewayInstanceId,
      });
    },
    async save(record: AcpSessionRecord): Promise<void> {
      ...
      if (
        ...
        recordToSave = withOriroLeaseSessionMetadata(
          {
            ...record,
          ...
          {
            oriroLeaseId: launch.leaseId,
            oriroGatewayInstanceId: launch.gatewayInstanceId,
          },
        );
      }
...
const ORIRO_BRIDGE_EXECUTABLE = "oriro";
const ORIRO_BRIDGE_SUBCOMMAND = "acp";
const CODEX_ACP_AGENT_ID = "codex";
const CODEX_ACP_ORIRO_PREFIX = "openai/";
const CLAUDE_ACP_ORIRO_PREFIX = "anthropic/";
...
function isOriroBridgeCommand(command: string | undefined): boolean {
  if (!command) {
    return false;
  }
  const parts = unwrapEnvCommand(splitCommandParts(command.trim()));
  if (basename(parts[0] ?? "") === ORIRO_BRIDGE_EXECUTABLE) {
    return parts[1] === ORIRO_BRIDGE_SUBCOMMAND;
  }
  if (basename(parts[0] ?? "") !== "node") {
    return false;
  }
  const scriptName = basename(parts[1] ?? "");
  return /^oriro(?:\.[cm]?js)?$/i.test(scriptName) && parts[2] === ORIRO_BRIDGE_SUBCOMMAND;
}
...
function normalizeCodexAcpModelOverride(
  ...
  let value = raw;
  if (value.toLowerCase().startsWith(CODEX_ACP_ORIRO_PREFIX)) {
    value = value.slice(CODEX_ACP_ORIRO_PREFIX.length);
  }
...
function normalizeClaudeAcpModelOverride(rawModel: string | undefined): string | undefined {
  ...
  if (!raw.toLowerCase().startsWith(CLAUDE_ACP_ORIRO_PREFIX)) {
    return raw;
  }
  return raw.slice(CLAUDE_ACP_ORIRO_PREFIX.length).trim() || undefined;
}

function withAcpxSessionOptions(input: OriroRuntimeEnsureInput): AcpxDelegateEnsureInput {
...
function shouldUseBridgeSafeDelegateForCommand(command: string | undefined): boolean {
  return isOriroBridgeCommand(command);
}
...
/** Oriro-managed ACP runtime implementation backed by the upstream acpx runtime. */
export class AcpxRuntime implements AcpRuntime {
  ...
  constructor(options: OriroAcpxRuntimeOptions, testOptions?: AcpxRuntimeTestOptions) {
    const { oriroProcessCleanup, ...delegateTestOptions } = testOptions ?? {};
    this.processCleanupDeps = oriroProcessCleanup;
    this.wrapperRoot = options.oriroWrapperRoot;
    this.gatewayInstanceId = options.oriroGatewayInstanceId;
    this.processLeaseStore = options.oriroProcessLeaseStore;
  ...
  private async readCodexTurnFailureStderr(params: { handle: AcpRuntimeHandle }): Promise<string> {
    ...
    return readCodexWrapperStderrTail({
      wrapperRoot: this.wrapperRoot,
      leaseId: readOriroLeaseIdFromRecord(record),
    });
  }
  ...
  private async cleanupProcessTreeForRecord(
    ...
    const leaseId = readOriroLeaseIdFromRecord(record);
    ...
    if (lease && lease.gatewayInstanceId === this.gatewayInstanceId && lease.rootPid > 0) {
      ...
      const result = await cleanupOriroOwnedAcpxProcessTree({
    ...
    const expectedGatewayInstanceId = readOriroGatewayInstanceIdFromRecord(record);
    await cleanupOriroOwnedAcpxProcessTree({
      rootPid,
      rootCommand,
  ...
  async *runTurn(input: Parameters<AcpRuntime["runTurn"]>[0]): AsyncIterable<AcpRuntimeEvent> {
    ...
    try {
      for await (const event of delegate.runTurn(withOriroManagedTurnTimeout(input))) {
        if (
          event.type !== "error" ||
          !isCodexAcpCommand(command) ||
  ...
  startTurn(input: OriroRuntimeTurnInput): AcpRuntimeTurn {
    ...
    const turnPromise = Promise.all([
      ...
      try {
        return {
          command,
          turn: delegate.startTurn(withOriroManagedTurnTimeout(input)),
        };
      } catch (error) {
        if (!isCodexAcpCommand(command) || !isGenericInternalAcpError(error)) {
          throw error;
