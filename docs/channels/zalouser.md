...
read_when:
  - Setting up Zalo Personal for Oriro
  - Debugging Zalo Personal login or message flow
title: "Zalo personal"
---

Status: experimental. This integration automates a **personal Zalo account** via native `zca-js` inside Oriro.
...
Zalo Personal ships as a bundled plugin in current Oriro releases, so normal
packaged builds do not need a separate install.

If you are on an older build or a custom install that excludes Zalo Personal,
install the npm package directly:

- Install via CLI: `oriro plugins install @oriro/zalouser`
- Pinned version: `oriro plugins install @oriro/zalouser@2026.5.2`
- Or from a source checkout: `oriro plugins install ./path/to/local/zalouser-plugin`
...
- `oriro pairing list zalouser`
- `oriro pairing approve zalouser <code>`
...
- On startup, Oriro resolves group/user names in allowlists to IDs and logs the mapping only when `channels.zalouser.dangerouslyAllowNameMatching: true` is enabled.
...
- When a group message is skipped because mention is required, Oriro stores it as pending group history and includes it on the next processed group message.
...
Accounts map to `zalouser` profiles in Oriro state. Example:
...
Profile names select the saved Zalo login credentials in Oriro state. Resolution order is:
...
- Oriro sends a typing event before dispatching a reply (best-effort).
...
- For inbound messages that include event metadata, Oriro sends delivered + seen acknowledgements (best-effort).
...
- Re-login: `oriro channels logout --channel zalouser && oriro channels login --channel zalouser`
...
- The channel now runs fully in Oriro without external CLI binaries.
