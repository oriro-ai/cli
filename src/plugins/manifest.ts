/** Loads and normalizes Oriro plugin manifests, including contracts and config schemas. */
...
export const PLUGIN_MANIFEST_FILENAME = "oriro.plugin.json";
...
export type PluginManifestQaRunner = {
  /** Subcommand mounted beneath `oriro qa`, for example `matrix`. */
  commandName: string;
...
// package.json "oriro" metadata (used for setup/catalog)
...
export type PluginPackageInstall = {
  ...
  npmSpec?: string;
  localPath?: string;
  defaultChoice?: "orirohub" | "npm" | "local";
...
export type OriroPackageStartup = {
  /**
...
export type OriroPackageSetupFeatures = {
  configPromotion?: boolean;
  legacyStateMigrations?: boolean;
  legacySessionSurfaces?: boolean;
};

export type OriroPackageCompat = {
  pluginApi?: string;
};

export type OriroPackageManifest = {
  extensions?: string[];
  runtimeExtensions?: string[];
  setupEntry?: string;
  runtimeSetupEntry?: string;
  setupFeatures?: OriroPackageSetupFeatures;
  plugin?: {
    id?: string;
    label?: string;
  };
  channel?: PluginPackageChannel;
  compat?: OriroPackageCompat;
  install?: PluginPackageInstall;
  startup?: OriroPackageStartup;
};
...
} & Partial<Record<ManifestKey, OriroPackageManifest>>;

export function getPackageManifestMetadata(
  manifest: PackageManifest | undefined,
): OriroPackageManifest | undefined {
  if (!manifest) {
    return undefined;
  }
...
export function resolvePackageExtensionEntries(
  ...
  const rawOriro = manifest?.[MANIFEST_KEY] as unknown;
  if (rawOriro === undefined || rawOriro === null) {
    return { status: "missing", entries: [] };
  }
  if (!isRecord(rawOriro)) {
    return {
      status: "invalid",
      entries: [],
      error: "package.json oriro must be an object",
    };
  }
  const raw = rawOriro.extensions;
  ...
  if (!Array.isArray(raw)) {
    return {
      status: "invalid",
      entries: [],
      error: "package.json oriro.extensions must be an array",
    };
  }
  ...
  for (const [index, entry] of raw.entries()) {
    ...
    if (!normalized) {
      return {
        status: "invalid",
        entries: [],
        error: `package.json oriro.extensions[${index}] must be a non-empty string`,
      };
    }
