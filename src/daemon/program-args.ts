...
export const ORIRO_WRAPPER_ENV_KEY = "ORIRO_WRAPPER";
...
async function resolveCliEntrypointPathForService(): Promise<string> {
  ...
  if (looksLikeDist) {
    ...
    // since symlinks like node_modules/oriro -> .pnpm/oriro@X.Y.Z/...
...
function appendNodeModulesBinCandidates(
  ...
  // oriro from node_modules/.bin points at the package root sibling.
...
export async function resolveOriroWrapperPath(
  ...
  try {
    ...
    throw new Error(
      `${ORIRO_WRAPPER_ENV_KEY} must point to an executable file: ${resolved}${detail}`,
      { cause: error },
    );
  }
  return resolved;
...
async function resolveCliProgramArguments(params: {
  ...
  const wrapperPath = await resolveOriroWrapperPath(params.wrapperPath);
  if (wrapperPath) {
