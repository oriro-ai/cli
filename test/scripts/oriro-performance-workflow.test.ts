// Oriro Performance Workflow tests cover oriro performance workflow script behavior.
import { readFileSync } from "node:fs";
import { describe, expect, it } from "vitest";
import { parse } from "yaml";

const WORKFLOW = ".github/workflows/oriro-performance.yml";
...
describe("Oriro performance workflow", () => {
  it("uses an optional dispatch identifier to name parent-owned runs", () => {
    const workflow = readFileSync(WORKFLOW, "utf8");

    expect(workflow).toContain(
      "run-name: ${{ inputs.dispatch_id != '' && format('Oriro Performance {0}', inputs.dispatch_id) || 'Oriro Performance' }}",
    );
  ...
  it("uses the orirogrit reports token for every report repo push path", () => {
    const prepare = findStep("Prepare orirogrit reports checkout");
    const publish = findStep("Publish to orirogrit reports");
  ...
  it("keeps optional orirogrit report publishing bounded", () => {
    const prepare = findStep("Prepare orirogrit reports checkout");
    const publish = findStep("Publish to orirogrit reports");
    ...
    expect(publish.if).toContain("steps.orirogrit_reports.outputs.ready == 'true'");
