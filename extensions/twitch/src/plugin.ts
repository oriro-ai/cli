/**
 * Twitch channel plugin for Oriro.
 *
...
} from "oriro/plugin-sdk/channel-pairing";
...
} from "oriro/plugin-sdk/extension-shared";
import {
 ...
 * for Oriro. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<ResolvedTwitchAccount> =
  createChatChannelPlugin<ResolvedTwitchAccount>({
    ...
    base: {
      ...
      config: {
        listAccountIds: (cfg: OriroConfig): string[] => listAccountIds(cfg),
        resolveAccount: (cfg: OriroConfig, accountId?: string | null): ResolvedTwitchAccount => {
        ...
        defaultAccountId: (cfg: OriroConfig): string => resolveDefaultTwitchAccountId(cfg),
        isConfigured: (_account: unknown, cfg: OriroConfig): boolean =>
      ...
      resolver: {
        resolveTargets: async ({
          ...
          runtime: import("oriro/plugin-sdk/runtime-env").RuntimeEnv;
