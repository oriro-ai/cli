...
import type { ConfigFileSnapshot, ModelDefinitionConfig, OriroConfig } from "../config/types.js";
import type { PluginMetadataSnapshot } from "../plugins/plugin-metadata-snapshot.js";
import { buildTestConfigSnapshot } from "./test-helpers.config-snapshots.js";

const applyPluginAutoEnable = vi.hoisted(() =>
  vi.fn((params: { config: OriroConfig }) => ({
...
vi.mock("../config/runtime-overrides.js", () => ({
  applyConfigOverrides: vi.fn((config: OriroConfig) => config),
}));
...
vi.mock("../config/plugin-auto-enable.js", () => ({
  applyPluginAutoEnable: (params: { config: OriroConfig }) => applyPluginAutoEnable(params),
}));
...
const configPath = "/tmp/oriro-startup-recovery.json";
...
function buildRuntimeSnapshot(
  ...
  runtimeConfig: OriroConfig = sourceConfig,
): ConfigFileSnapshot {
  return {
...
function expectPluginAutoEnableFor(config: OriroConfig) {
...
function mockRuntimeAutoEnable(config: OriroConfig) {
...
function buildInvalidConfigSnapshot(params: {
  ...
  return buildTestConfigSnapshot({
    ...
    valid: false,
    config: params.config ?? (params.rawConfig as OriroConfig),
...
describe("gateway startup config validation", () => {
  ...
  it("does not suggest doctor repair for plugin packaging compiled-output failures", async () => {
    ...
    const invalidSnapshot = buildInvalidConfigSnapshot({
      rawConfig,
      config: rawConfig as OriroConfig,
      issues: [
        {
    ...
    await start.catch((error: unknown) => {
      expect(String(error)).not.toContain("oriro doctor --fix");
    });
  });
  ...
  it("keeps doctor repair guidance for mixed plugin packaging and core invalidity", async () => {
    ...
    const invalidSnapshot = buildInvalidConfigSnapshot({
      rawConfig,
      config: rawConfig as unknown as OriroConfig,
      issues: [
        {
          path: "plugins.slots.memory",
          message: "plugin not found: source-only-pack",
    ...
    await expectStartupRejects('Run "oriro doctor --fix" to repair, then retry.');
  });
      ...
      issues: [
        {
          ...
          message:
            "plugin feishu: plugin requires Oriro >=2026.4.23, but this host is 2026.4.22; skipping load",
        },
      ],
    });
    vi.mocked(configIo.readConfigFileSnapshot).mockResolvedValueOnce(invalidSnapshot);
