// `oriro plugins list`: builds registry reports and defers terminal-only formatting modules.
...
export async function runPluginsListCommand(
  ...
  if (list.length === 0) {
    runtime.log(
      theme.muted(
        `No plugins found. Run ${formatCliCommand("oriro plugins install <plugin>")} to add one, or ${formatCliCommand("oriro plugins list --json")} to inspect raw discovery state.`,
      ),
    );
    return;
  }
