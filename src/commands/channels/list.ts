// Implements `oriro channels list` across runtime accounts, local config, and catalog-only entries.
...
export async function channelsListCommand(
  ...
  // there are no configured channels even though oriro.json has one.
    ...
    lines.push(
      theme.muted(
        showAll
          ...
          : "- no configured chat channels (run `oriro channels list --all` to see installable channels)",
      ),
    );
  } else {
  ...
  runtime.log(
    theme.muted(
      "Model provider usage moved out of `channels list` — see `oriro status` or `oriro models list`.",
    ),
  );
