...
describe("auth profile sqlite store", () => {
  ...
  it("persists auth profiles and runtime scheduling state in the agent sqlite database", async () => {
    await withAgentDirEnv("oriro-auth-sqlite-", (agentDir) => {
      saveAuthProfileStore(
        {
          ...apiKeyStore("sk-test"),
      ...
      expect(fs.existsSync(path.join(agentDir, "oriro-agent.sqlite"))).toBe(true);
    });
  });

  it("does not read legacy auth-profiles.json at runtime", async () => {
    await withAgentDirEnv("oriro-auth-no-json-fallback-", (agentDir) => {
      fs.writeFileSync(
  ...
  it("does not create sqlite files for missing-store reads", async () => {
    await withAgentDirEnv("oriro-auth-sqlite-no-create-", (agentDir) => {
      expect(loadPersistedAuthProfileStore(agentDir)).toBeNull();
      expect(fs.existsSync(path.join(agentDir, "oriro-agent.sqlite"))).toBe(false);
    });
  });

  it("reads existing sqlite auth stores without registering shared state", async () => {
    await withAgentDirEnv("oriro-auth-sqlite-readonly-", (agentDir) => {
      saveAuthProfileStore(apiKeyStore("sk-test"), agentDir);
      closeOriroAgentDatabasesForTest();
      closeOriroStateDatabaseForTest();
      const stateDbPath = resolveOriroStateSqlitePath();
  ...
  it("waits for brief rollback-journal contention before reading persisted auth", async () => {
    await withAgentDirEnv("oriro-auth-sqlite-contention-", async (agentDir) => {
      saveAuthProfileStore(apiKeyStore("sk-test"), agentDir);
      closeOriroAgentDatabasesForTest();
  ...
  it("uses the configured agent id for custom agentDir databases", async () => {
    await withAgentDirEnv("oriro-auth-sqlite-custom-agent-", (envAgentDir) => {
  ...
  it("keeps SecretRef-backed credentials from persisting duplicate plaintext", async () => {
    await withAgentDirEnv("oriro-auth-sqlite-secret-ref-", (agentDir) => {
      saveAuthProfileStore(
        {
          version: 1,
          profiles: {
  ...
  it("recomputes runtime-only external auth overlays from the sqlite base store", async () => {
    await withAgentDirEnv("oriro-auth-sqlite-overlay-", (agentDir) => {
      saveAuthProfileStore(apiKeyStore("sk-test"), agentDir);
