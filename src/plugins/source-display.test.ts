...
const PLUGIN_SOURCE_ROOTS = {
  stock: path.resolve(path.sep, "opt", "homebrew", "lib", "node_modules", "oriro", "extensions"),
  global: path.resolve(path.sep, "Users", "x", ".oriro", "extensions"),
  workspace: path.resolve(path.sep, "Users", "x", "ws", ".oriro", "extensions"),
};
...
describe("formatPluginSourceForTable", () => {
  ...
  it("ignores untrusted explicit env override for the stock source root", () => {
    const homeDir = path.resolve(path.sep, "tmp", "oriro-home");
    const rawEnv = {
    ...
    expectResolvedSourceRoots({
      ...
      expected: {
        ...
        workspace: path.join(homeDir, "ws", ".oriro", "extensions"),
      },
    });
  });
});
