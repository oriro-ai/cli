...
async function withStateFixture(
  ...
  await withTempDir({ prefix: "orphan-keys-test-" }, async (tmpDir) => {
    const stateDir = path.join(tmpDir, ".oriro");
...
function sharedMainOpsConfig(sharedStorePath: string): OriroConfig {
  return {
...
async function migrateFixtureState(stateDir: string, cfg: OriroConfig = OPS_WORK_CONFIG) {
