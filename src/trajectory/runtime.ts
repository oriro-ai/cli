...
export function createTrajectoryRuntimeRecorder(
  ...
  // override so operators can still disable recording with ORIRO_TRAJECTORY=0.
  const enabled = parseBooleanValue(env.ORIRO_TRAJECTORY) ?? true;
  if (!enabled) {
    return null;
  }
