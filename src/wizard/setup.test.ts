...
import type { ProviderPlugin } from "oriro/plugin-sdk/provider-model-shared";
...
const resolveGatewayPort = vi.hoisted(() =>
  vi.fn((_cfg?: unknown, env?: NodeJS.ProcessEnv) => {
    const raw = env?.ORIRO_GATEWAY_PORT ?? process.env.ORIRO_GATEWAY_PORT;
...
describe("runSetupWizard", () => {
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-onboard-suite-"));
  });
  ...
  it("allows size-drop writes for pending plugin install record migration", async () => {
    ...
    readConfigFileSnapshot.mockResolvedValueOnce({
      ...
      config: {
        plugins: {
          installs: {
            demo: { source: "npm", spec: "@oriro/demo-plugin" },
          },
        },
      },
      issues: [],
  ...
  it("shows the resolved gateway port in quickstart for fresh envs", async () => {
    const previousPort = process.env.ORIRO_GATEWAY_PORT;
    process.env.ORIRO_GATEWAY_PORT = "18791";
    const note: WizardPrompter["note"] = vi.fn(async () => {});
    const prompter = buildWizardPrompter({ note });
    const runtime = createRuntime();
    ...
    try {
      ...
      if (previousPort === undefined) {
        ...
        process.env.ORIRO_GATEWAY_PORT = previousPort;
      }
    }
