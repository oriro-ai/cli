...
describe("argv-invocation", () => {
  it("resolves root help and empty command path", () => {
    expect(resolveCliArgvInvocation(["node", "oriro", "--help"])).toEqual({
  ...
  it("resolves command path and primary with root options", () => {
    expect(
      resolveCliArgvInvocation(["node", "oriro", "--profile", "work", "gateway", "status"]),
    ).toEqual({
      argv: ["node", "oriro", "--profile", "work", "gateway", "status"],
