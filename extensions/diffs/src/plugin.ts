...
export function registerDiffsPlugin(api: OriroPluginApi): void {
  const store = new DiffArtifactStore({
    rootDir: path.join(resolvePreferredOriroTmpDir(), "oriro-diffs"),
    logger: api.logger,
  });
...
export function resolveDiffsLanguagePackAvailability(api: OriroPluginApi): boolean {
  const currentConfig = (api.runtime.config?.current?.() ?? api.config) as OriroConfig;
...
function hasSiblingLanguagePackRuntime(rootDir: string | undefined): boolean {
  ...
  return (
    fs.existsSync(path.join(languagePackRoot, "oriro.plugin.json")) &&
