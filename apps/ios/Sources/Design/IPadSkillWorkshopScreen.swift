...
struct IPadSkillWorkshopScreen: View {
    ...
    init(headerLeadingAction: OriroSidebarHeaderAction? = nil, openSettings: @escaping () -> Void = {}) {
...
private struct IPadSkillProposalKanbanCard: View {
    ...
    var body: some View {
                    ...
                    HStack(alignment: .top, spacing: 10) {
                        ...
                        VStack(alignment: .leading, spacing: 4) {
                            Text(self.proposal.title)
                                .font(.subheadline.weight(.semibold))
                                .foregroundStyle(self.isSelected ? OriroBrand.accent : .primary)
                                .lineLimit(2)
        ...
        .padding(12)
        .background(
            self.isSelected ? OriroBrand.accent.opacity(0.08) : Color.clear,
...
struct IPadSkillProposal: Identifiable {
    ...
    var statusColor: Color {
        switch self.status {
        case "pending": OriroBrand.warn
        case "applied": OriroBrand.ok
        case "rejected": .secondary
        case "quarantined", "stale": OriroBrand.warn
        default: OriroBrand.accent
        }
    }
