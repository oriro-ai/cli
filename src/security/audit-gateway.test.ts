...
describe("security audit gateway config findings", () => {
  it("evaluates gateway auth presence and rate-limit guardrails", async () => {
    await Promise.all([
      ...
      (async () => {
        ...
        const resolvedConfig: OriroConfig = {
          gateway: {
            bind: "lan",
            auth: {},
          },
  ...
  it("warns when ORIRO_GATEWAY_TOKEN shadows a different configured token source", () => {
    const cfg: OriroConfig = {
  ...
  it("does not warn when gateway.auth.token resolves from ORIRO_GATEWAY_TOKEN", () => {
    const cfg: OriroConfig = {
