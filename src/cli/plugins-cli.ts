...
export function registerPluginsCli(program: Command) {
  const plugins = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/plugins", "docs.oriro.ai/cli/plugins")}\n`,
    );
  ...
  plugins
    .command("search")
    .description("Search OriroHub plugin packages")
  ...
  plugins
    .command("install")
    .description(
      "Install a plugin or hook pack (path, archive, npm spec, git repo, orirohub:package, or marketplace entry)",
    )
    .argument(
