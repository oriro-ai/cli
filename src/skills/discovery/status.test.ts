...
import { readLocalSkillCardContentSync } from "../lifecycle/orirohub.js";
...
describe("buildWorkspaceSkillStatus", () => {
  it("surfaces valid OriroHub linkage and local Skill Card metadata", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-skill-status-"));
    try {
      ...
      expect(report.skills[0]?.orirohub).toEqual({
        status: "linked",
        valid: true,
        registry: "https://orirohub.ai",
        slug: "agentreceipt",
  ...
  it("uses OriroHub origin metadata for linkage when the skill name is a display name", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-skill-status-"));
    try {
      const skillDir = path.join(workspaceDir, "skills", "agentreceipt");
      await writeOriroHubStatusFixture({
        workspaceDir,
        skillDir,
        slug: "agentreceipt",
      });
      ...
      expect(report.skills[0]?.orirohub).toMatchObject({
        status: "linked",
        valid: true,
        registry: "https://orirohub.ai",
        slug: "agentreceipt",
  ...
  it("does not link OriroHub origin metadata from the wrong install directory", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-skill-status-"));
    try {
      ...
      expect(report.skills[0]?.orirohub).toMatchObject({
        ...
        valid: false,
        slug: "agentreceipt",
        reason: expect.stringContaining("expected OriroHub install directory"),
      });
    } finally {
      await fs.rm(workspaceDir, { recursive: true, force: true });
    }
  ...
  it("does not link OriroHub origin metadata when the lockfile registry disagrees", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-skill-status-"));
    try {
      const skillDir = path.join(workspaceDir, "skills", "agentreceipt");
      await writeOriroHubStatusFixture({
        ...
        skillDir,
        slug: "agentreceipt",
        originRegistry: "https://orirohub.ai",
  ...
  it("surfaces malformed or mismatched OriroHub linkage without trusting it", async () => {
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-skill-status-"));
    try {
      ...
      await fs.mkdir(path.join(malformedDir, ".orirohub"), { recursive: true });
      await fs.writeFile(path.join(malformedDir, ".orirohub", "origin.json"), "{not json", "utf8");
      ...
      expect(requireSkillStatus(byName, "malformed").orirohub).toMatchObject({
        status: "invalid",
        valid: false,
        reason: expect.stringContaining("Malformed OriroHub origin metadata"),
      });
      expect(requireSkillStatus(byName, "missing-lock").orirohub).toMatchObject({
        status: "invalid",
        valid: false,
        reason: expect.stringContaining("not tracked by the workspace OriroHub lockfile"),
      });
      expect(requireSkillStatus(byName, "mismatch").orirohub).toMatchObject({
        status: "invalid",
        valid: false,
        reason: expect.stringContaining("does not match the workspace OriroHub lockfile"),
      });
      expect(requireSkillStatus(byName, "local-only").orirohub).toBeUndefined();
  ...
  it("classifies a mixed broken skill pack without flattening visibility reasons", () => {
    const missingBin = "oriro-test-definitely-missing-skill-bin";
    const report = buildWorkspaceSkillStatus("/tmp/ws", {
      agentId: "specialist",
      config: {
        agents: {
      ...
      entries: [
        ...
        createEntry("needs-bin", {
          metadata: {
            ...
            install: [
              {
                kind: "node",
                package: "@oriro/missing-skill-bin",
                bins: [missingBin],
              },
            ],
          },
        ...
        createEntry("needs-env", {
          metadata: {
            primaryEnv: "ORIRO_TEST_MISSING_SKILL_KEY",
            requires: { env: ["ORIRO_TEST_MISSING_SKILL_KEY"] },
          },
        }),
        ...
        createEntry("bundled-blocked", { source: "oriro-bundled" }),
      ],
    });
    ...
    expect(needsBin.install).toStrictEqual([
      {
        kind: "node",
        id: "node-0",
        label: "Install @oriro/missing-skill-bin (pnpm)",
        bins: [missingBin],
      },
    ]);
    const needsEnv = requireSkillStatus(byName, "needs-env");
    expect(needsEnv.eligible).toBe(false);
    expect(needsEnv.primaryEnv).toBe("ORIRO_TEST_MISSING_SKILL_KEY");
    expect(needsEnv.missing).toStrictEqual({
      anyBins: [],
      bins: [],
      config: [],
      env: ["ORIRO_TEST_MISSING_SKILL_KEY"],
      os: [],
    });
...
async function writeOriroHubStatusFixture(params: {
  workspaceDir: string;
  skillDir: string;
  slug: string;
  ...
  const originPath = path.join(params.skillDir, ".orirohub", "origin.json");
  await fs.mkdir(path.dirname(originPath), { recursive: true });
  await fs.writeFile(
    originPath,
    `${JSON.stringify(
      {
        version: 1,
        registry: params.originRegistry ?? "https://orirohub.ai",
        slug: params.slug,
        installedVersion,
        installedAt,
      },
  ...
  const lockPath = path.join(params.workspaceDir, ".orirohub", "lock.json");
  await fs.mkdir(path.dirname(lockPath), { recursive: true });
  await fs.writeFile(
    lockPath,
    `${JSON.stringify(
      {
        ...
        skills: {
          [params.slug]: {
            ...
            installedAt,
            registry: params.lockRegistry ?? "https://orirohub.ai",
          },
        },
      },
      null,
