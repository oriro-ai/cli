...
metadata: { "oriro": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Oriro `sessions_spawn`
- **File I/O** == Oriro `read`/`write`
- **Remote fetch** == Oriro `web_fetch` (or `exec` with curl when POST is required)
