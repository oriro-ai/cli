...
ACP_AGENT_LIST_RAW="${ORIRO_LIVE_ACP_BIND_AGENTS:-${ORIRO_LIVE_ACP_BIND_AGENT:-claude,codex,gemini}}"
...
oriro_live_acp_bind_append_build_extension() {
...
oriro_live_acp_bind_resolve_auth_provider() {
  case "${1:-}" in
    claude) printf '%s\n' "claude-cli" ;;
    codex) printf '%s\n' "codex-cli" ;;
    droid) printf '%s\n' "droid" ;;
    ...
    opencode) printf '%s\n' "opencode" ;;
    *)
      echo "Unsupported ORIRO_LIVE_ACP_BIND agent: ${1:-} (expected claude, codex, droid, gemini, or opencode)" >&2
      return 1
      ;;
  esac
}

oriro_live_acp_bind_resolve_agent_command() {
  case "${1:-}" in
    claude) printf '%s' "${ORIRO_LIVE_ACP_BIND_AGENT_COMMAND_CLAUDE:-${ORIRO_LIVE_ACP_BIND_AGENT_COMMAND:-}}" ;;
    codex) printf '%s' "${ORIRO_LIVE_ACP_BIND_AGENT_COMMAND_CODEX:-${ORIRO_LIVE_ACP_BIND_AGENT_COMMAND:-}}" ;;
    droid) printf '%s' "${ORIRO_LIVE_ACP_BIND_AGENT_COMMAND_DROID:-${ORIRO_LIVE_ACP_BIND_AGENT_COMMAND:-}}" ;;
    gemini) printf '%s' "${ORIRO_LIVE_ACP_BIND_AGENT_COMMAND_GEMINI:-${ORIRO_LIVE_ACP_BIND_AGENT_COMMAND:-}}" ;;
    opencode) printf '%s' "${ORIRO_LIVE_ACP_BIND_AGENT_COMMAND_OPENCODE:-${ORIRO_LIVE_ACP_BIND_AGENT_COMMAND:-}}" ;;
    *) return 1 ;;
  esac
}
...
oriro_live_acp_bind_load_factory_api_key_from_profile() {
...
run_setup_command() {
  local timeout_value="${ORIRO_LIVE_ACP_BIND_SETUP_TIMEOUT_SECONDS:?missing live ACP bind setup timeout seconds}s"
  local timeout_bin=""
...
agent="${ORIRO_LIVE_ACP_BIND_AGENT:-claude}"
case "$agent" in
  claude)
...
if [ -n "\${ORIRO_LIVE_ACP_BIND_ANTHROPIC_API_KEY:-}" ]; then
  export ANTHROPIC_API_KEY="\${ORIRO_LIVE_ACP_BIND_ANTHROPIC_API_KEY}"
fi
if [ -n "\${ORIRO_LIVE_ACP_BIND_ANTHROPIC_API_KEY_OLD:-}" ]; then
  export ANTHROPIC_API_KEY_OLD="\${ORIRO_LIVE_ACP_BIND_ANTHROPIC_API_KEY_OLD}"
fi
...
NODE
  ...
  opencode)
    ...
    export OPENCODE_CONFIG_CONTENT="$(
      node -e 'process.stdout.write(JSON.stringify({model: process.env.ORIRO_LIVE_ACP_BIND_OPENCODE_MODEL || "opencode/kimi-k2.6"}))'
    )"
    ;;
  *)
    echo "Unsupported ORIRO_LIVE_ACP_BIND_AGENT: $agent" >&2
    exit 1
    ;;
esac
tmp_dir="$(mktemp -d)"
...
cd "$tmp_dir"
export ORIRO_LIVE_ACP_BIND_AGENT_COMMAND="${ORIRO_LIVE_ACP_BIND_AGENT_COMMAND:-}"
node scripts/test-live.mjs -- ${ORIRO_LIVE_ACP_BIND_TEST_FILES:-src/gateway/gateway-acp-bind.live.test.ts}
EOF

oriro_live_acp_bind_append_build_extension acpx
...
for token in "${ACP_AGENT_TOKENS[@]}"; do
  agent="$(oriro_live_trim "$token")"
  [[ -n "$agent" ]] || continue
  oriro_live_acp_bind_resolve_auth_provider "$agent" >/dev/null
  ACP_AGENTS+=("$agent")
done

if ((${#ACP_AGENTS[@]} == 0)); then
  echo "No ACP bind agents selected. Use ORIRO_LIVE_ACP_BIND_AGENTS=claude,codex,droid,gemini,opencode." >&2
  exit 1
fi

for ACP_AGENT in "${ACP_AGENTS[@]}"; do
  AUTH_PROVIDER="$(oriro_live_acp_bind_resolve_auth_provider "$ACP_AGENT")"
  AGENT_COMMAND="$(oriro_live_acp_bind_resolve_agent_command "$ACP_AGENT")"
  ...
  if [[ -n "${ORIRO_DOCKER_AUTH_DIRS:-}" ]]; then
    ...
    while IFS= read -r auth_dir; do
      [[ -n "$auth_dir" ]] || continue
      AUTH_DIRS+=("$auth_dir")
    done < <(oriro_live_collect_auth_dirs_from_csv "$AUTH_PROVIDER")
    while IFS= read -r auth_file; do
      [[ -n "$auth_file" ]] || continue
      AUTH_FILES+=("$auth_file")
    done < <(oriro_live_collect_auth_files_from_csv "$AUTH_PROVIDER")
  fi
  ...
  echo "==> Run ACP bind live test in Docker"
  echo "==> Agent: $ACP_AGENT"
  echo "==> Test files: ${ORIRO_LIVE_ACP_BIND_TEST_FILES:-src/gateway/gateway-acp-bind.live.test.ts}"
  echo "==> Profile file: $PROFILE_STATUS"
  echo "==> Auth dirs: ${AUTH_DIRS_CSV:-none}"
  echo "==> Auth files: ${AUTH_FILES_CSV:-none}"
  if oriro_live_uses_managed_bind_dirs; then
  ...
  DOCKER_RUN_ARGS=()
  oriro_live_init_docker_run_args DOCKER_RUN_ARGS "${ORIRO_LIVE_ACP_BIND_DOCKER_RUN_TIMEOUT:-2700s}"
  DOCKER_RUN_ARGS+=(--rm -t \
    -u "$DOCKER_USER" \
    --entrypoint bash \
    -e ANTHROPIC_API_KEY \
    -e ANTHROPIC_API_KEY_OLD \
    -e ORIRO_LIVE_ACP_BIND_ANTHROPIC_API_KEY="${ANTHROPIC_API_KEY:-}" \
    -e ORIRO_LIVE_ACP_BIND_ANTHROPIC_API_KEY_OLD="${ANTHROPIC_API_KEY_OLD:-}" \
    -e GEMINI_API_KEY \
    -e GOOGLE_API_KEY \
    -e FACTORY_API_KEY \
    -e OPENAI_API_KEY \
    ...
    -e ORIRO_LIVE_TEST=1 \
    -e ORIRO_LIVE_ACP_BIND=1 \
    -e ORIRO_LIVE_ACP_BIND_AGENT="$ACP_AGENT" \
    -e ORIRO_LIVE_ACP_BIND_TEST_FILES="${ORIRO_LIVE_ACP_BIND_TEST_FILES:-}" \
    -e ORIRO_LIVE_ACP_BIND_CODEX_MODEL="${ORIRO_LIVE_ACP_BIND_CODEX_MODEL:-}" \
    -e ORIRO_LIVE_ACP_BIND_SETUP_TIMEOUT_SECONDS="$ACP_SETUP_TIMEOUT_SECONDS" \
    -e ORIRO_LIVE_ACP_BIND_OPENCODE_MODEL="${ORIRO_LIVE_ACP_BIND_OPENCODE_MODEL:-opencode/kimi-k2.6}" \
    -e ORIRO_LIVE_ACP_SPAWN_DEFAULTS="${ORIRO_LIVE_ACP_SPAWN_DEFAULTS:-}" \
    -e ORIRO_LIVE_ACP_SPAWN_DEFAULTS_AGENT="${ORIRO_LIVE_ACP_SPAWN_DEFAULTS_AGENT:-}" \
    -e ORIRO_LIVE_ACP_SPAWN_DEFAULTS_CONNECT_TIMEOUT_MS="${ORIRO_LIVE_ACP_SPAWN_DEFAULTS_CONNECT_TIMEOUT_MS:-}" \
    -e ORIRO_LIVE_ACP_SPAWN_DEFAULTS_MODEL="${ORIRO_LIVE_ACP_SPAWN_DEFAULTS_MODEL:-}" \
    -e ORIRO_LIVE_ACP_SPAWN_DEFAULTS_THINKING="${ORIRO_LIVE_ACP_SPAWN_DEFAULTS_THINKING:-}" \
    -e ORIRO_LIVE_ACP_SPAWN_DEFAULTS_TIMEOUT_MS="${ORIRO_LIVE_ACP_SPAWN_DEFAULTS_TIMEOUT_MS:-}" \
    -e ORIRO_LIVE_ACP_BIND_AGENT_COMMAND="$AGENT_COMMAND")
