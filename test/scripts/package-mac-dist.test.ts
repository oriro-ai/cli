...
function makePlist(): string {
  const dir = mkdtempSync(path.join(tmpdir(), "oriro-dist-plist-"));
  tempDirs.push(dir);
...
describe("package-mac-dist plist validation", () => {
  it("fails closed for required Info.plist reads", () => {
    ...
    const readBlock = script.slice(
      ...
      script.indexOf('ZIP="$ROOT_DIR/dist/Oriro-$VERSION.zip"'),
    );
  ...
  it("prefers repo Corepack pnpm over a global pnpm shim", () => {
    ...
    const tempRoot = mkdtempSync(path.join(tmpdir(), "oriro-dist-pnpm-root-"));
    const outerRoot = mkdtempSync(path.join(tmpdir(), "oriro-dist-pnpm-outer-"));
    const toolsDir = mkdtempSync(path.join(tmpdir(), "oriro-dist-pnpm-tools-"));
  ...
  it("keeps dependency bootstrap output out of captured Sparkle build values", () => {
    ...
    const dir = mkdtempSync(path.join(tmpdir(), "oriro-dist-sparkle-"));
    tempDirs.push(dir);
    ...
    const result = runHelper(`
      ...
      ORIRO_ROOT=${JSON.stringify(process.cwd())}
      ORIRO_MARKER=${JSON.stringify(marker)}
      PATH=${JSON.stringify(tools)}:/usr/bin:/bin
      export ORIRO_MARKER ORIRO_ROOT PATH
      ${helpers}
