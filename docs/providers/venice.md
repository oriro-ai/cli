---
summary: "Use Venice AI privacy-focused models in Oriro"
read_when:
  - You want privacy-focused inference in Oriro
...
<Steps>
  ...
  <Step title="Configure Oriro">
    ...
    <Tabs>
      <Tab title="Interactive (recommended)">
        ```bash
        oriro onboard --auth-choice venice-api-key
        ```
  ...
  <Step title="Verify setup">
    ```bash
    oriro agent --model venice/kimi-k2-5 --message "Hello, are you working?"
    ```
  </Step>
</Steps>
...
After setup, Oriro shows all available Venice models. Pick based on your needs:
...
```bash
oriro models set venice/kimi-k2-5
oriro models set venice/claude-opus-4-6
```

List all available models:

```bash
oriro models list --all --provider venice
```

You can also run `oriro configure`, select **Model/auth**, and choose **Venice AI**.
...
`venice/deepseek-v4-flash`, Oriro fills the required DeepSeek V4
...
Oriro keeps that provider-specific replay fix separate from the native
...
Oriro ships a manifest-backed Venice seed catalog for read-only model listing. Runtime refresh can still discover models from the Venice API, and falls back to the manifest catalog if the API is unreachable.
...
oriro agent --model venice/kimi-k2-5 --message "Quick health check"

# Use Claude Opus via Venice (anonymized)
oriro agent --model venice/claude-opus-4-6 --message "Summarize this task"

# Use uncensored model
oriro agent --model venice/venice-uncensored --message "Draft options"

# Use vision model with image
oriro agent --model venice/qwen3-vl-235b-a22b --message "Review attached image"

# Use coding model
oriro agent --model venice/qwen3-coder-480b-a35b-instruct --message "Refactor this function"
```
...
<AccordionGroup>
  ...
  <Accordion title="Model not available">
    The Venice model catalog updates dynamically. Run `oriro models list` to see currently available models. Some models may be temporarily offline.
  </Accordion>
