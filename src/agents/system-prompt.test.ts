...
describe("buildAgentSystemPrompt", () => {
  it("resolves helper session keys to scoped prompt surfaces", () => {
    ...
    expect(resolveAgentPromptSurfaceForSessionKey("agent:main")).toBe("oriro_main");
    expect(resolveAgentPromptSurfaceForSessionKey(undefined)).toBe("oriro_main");
  });
  ...
  it("includes an Oriro control section", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/oriro",
    });

    expect(prompt).toContain("## Oriro Control");
    expect(prompt).toContain("prefer `gateway` tool");
    expect(prompt).toContain("CLI lifecycle only on explicit user request");
    expect(prompt).toContain("oriro gateway status|restart|start|stop");
  ...
  it("includes embed guidance only for webchat sessions", () => {
    ...
    expect(prompt).toContain(
      '[embed url="/__oriro__/canvas/documents/cv_123/index.html" title="Status" height="320" /]',
    );
    expect(prompt).toContain(
  ...
  it("keeps the Oriro empty-tool fallback on the main prompt surface", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/oriro",
      toolNames: [],
    });

    expect(prompt).toContain("Oriro lists the standard tools above");
  ...
  it("omits ACP harness guidance when ACP is disabled", () => {
    ...
    expect(prompt).toContain("- sessions_spawn: Spawn an isolated sub-agent session");
    expect(prompt).toContain("- agents_list: List Oriro agent ids allowed for sessions_spawn");
  });
  ...
  it("preserves tool casing in the prompt", () => {
    ...
    expect(prompt).toContain("Docs: /tmp/oriro/docs");
    expect(prompt).toContain(
      "Docs are authoritative for Oriro self-knowledge: before understanding how Oriro works (memory/daily notes, sessions, tools, Gateway, config, commands, project context), use `Read` or search local docs first; treat AGENTS.md/project context, workspace/profile/memory notes, and `memory_search` as instruction context or user memory, not Oriro design/implementation knowledge.",
    );
  });
  ...
  it("includes docs guidance when docsPath is provided", () => {
    ...
    expect(prompt).toContain("Docs: /tmp/oriro/docs");
    expect(prompt).toContain("Source: /tmp/oriro");
    expect(prompt).toContain(
      "Docs are authoritative for Oriro self-knowledge: before understanding how Oriro works (memory/daily notes, sessions, tools, Gateway, config, commands, project context), use `read` or search local docs first; treat AGENTS.md/project context, workspace/profile/memory notes, and `memory_search` as instruction context or user memory, not Oriro design/implementation knowledge.",
    );
  ...
  it("falls back to public docs and GitHub source guidance when local docs are unavailable", () => {
    ...
    expect(prompt).toContain("Docs: https://docs.oriro.ai");
    expect(prompt).toContain("Source: https://github.com/oriro-ai/cli");
    expect(prompt).toContain(
  ...
  it("includes bootstrap instructions in system prompt when bootstrap is pending", () => {
    const prompt = buildAgentSystemPrompt({
      workspaceDir: "/tmp/oriro",
      bootstrapMode: "full",
      contextFiles: [{ path: "/tmp/oriro/BOOTSTRAP.md", content: "Ask who I am." }],
    });
    ...
    expect(prompt).toContain("## /tmp/oriro/BOOTSTRAP.md");
  ...
  // See: https://github.com/oriro-ai/cli/commit/66eec295b894bce8333886cfbca3b960c57c4946
  // Agents should use session_status or message timestamps to determine the date/time.
  // Related: https://github.com/oriro-ai/cli/issues/1897
  //          https://github.com/oriro-ai/cli/issues/3658
  it("does NOT include a date or time in the system prompt (cache stability)", () => {
    ...
    // https://github.com/oriro-ai/cli/issues/3658 for the preferred approach:
  ...
  it("adds ClaudeBot self-update guidance when gateway tool is available", () => {
    ...
    expect(prompt).toContain("## Oriro Self-Update");
  ...
  it("describes Telegram rich text only for rich Telegram runtimes", () => {
    ...
    expect(telegramPrompt).toContain("Oriro renders Telegram-safe rich messages");
  ...
  it("describes sandboxed runtime and elevated when allowed", () => {
    ...
    expect(prompt).toContain(
      "For read/write/edit/apply_patch, file paths resolve against host workspace: /tmp/oriro. For bash/exec commands, use sandbox container paths under /workspace (or relative paths from that workdir), not host paths.",
    );
...
describe("buildAgentBootstrapSystemPromptSections", () => {
  it("can render bootstrap guidance without duplicating Project Context", () => {
    ...
    expect(sections).not.toContain("## /tmp/oriro/BOOTSTRAP.md");
...
describe("buildSubagentSystemPrompt", () => {
  it("renders depth-1 orchestrator guidance, labels, and recovery notes", () => {
    ...
    expect(prompt).toContain("Do not use `exec` (`oriro ...`, `acpx ...`)");
    expect(prompt).toContain("Use `subagents` only for Oriro subagents");
