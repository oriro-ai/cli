...
async function createBundleMcpFixture(params: {
  ...
  setTestEnvValue("ORIRO_DISABLE_PERSISTED_PLUGIN_REGISTRY", "1");
  ...
  const fakeClaudePidPath = params.liveSession
    ...
    : undefined;
  const pluginRoot = path.join(tempHome, ".oriro", "extensions", "bundle-probe");
...
describe("CLI bundle MCP e2e", () => {
  it(
    ...
    async () => {
      ...
      const fixture = await createBundleMcpFixture({
        tempPrefix: "oriro-cli-bundle-mcp-",
      });
  ...
  it(
    ...
    async () => {
      ...
      const fixture = await createBundleMcpFixture({
        liveSession: true,
        tempPrefix: "oriro-cli-live-cleanup-",
      });
      const context = await prepareBundleMcpExecutionContext({
