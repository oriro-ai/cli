...
const ensureOriroModelsJsonMock = vi.fn(async () => ({ wrote: false }));
const loggerWarnMock = vi.fn();
let refreshRuntimeAuthOnFirstPromptError = false;
let clearRuntimeConfigSnapshot: typeof import("../config/config.js").clearRuntimeConfigSnapshot;
let setRuntimeConfigSnapshot: typeof import("../config/config.js").setRuntimeConfigSnapshot;

vi.mock("oriro/plugin-sdk/llm", async () => {
  const actual =
    await vi.importActual<typeof import("oriro/plugin-sdk/llm")>("oriro/plugin-sdk/llm");
...
const installRunEmbeddedMocks = () => {
  ...
  vi.doMock("./models-config.js", async () => {
    ...
    return {
      ...mod,
      ensureOriroModelsJson: (...args: Parameters<typeof ensureOriroModelsJsonMock>) =>
        ensureOriroModelsJsonMock(...args),
    };
  });
};

let runEmbeddedAgent: typeof import("./embedded-agent-runner/run.js").runEmbeddedAgent;
let SessionManager: typeof import("oriro/plugin-sdk/agent-sessions").SessionManager;
...
beforeAll(async () => {
  ...
  ({ SessionManager } = await import("oriro/plugin-sdk/agent-sessions"));
  e2eWorkspace = await createEmbeddedAgentRunnerTestWorkspace("oriro-embedded-agent-");
...
beforeEach(() => {
  ...
  ensureOriroModelsJsonMock.mockReset();
  ensureOriroModelsJsonMock.mockResolvedValue({ wrote: false });
...
describe("runEmbeddedAgent", () => {
  ...
  it("resolves explicit OpenAI Oriro runs through Codex when auth order starts with Codex OAuth", async () => {
    const sessionFile = nextSessionFile();
    ...
    await runEmbeddedAgent({
      ...
      timeoutMs: 5_000,
      agentDir,
      runId: nextRunId("codex-first-oriro"),
      enqueue: immediateEnqueue,
    });
