    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          [
            "oriro node run --host 127.0.0.1 --port 18789",
            "Run the node host in the foreground.",
          ],
          ["oriro node status", "Check node host service status."],
          ["oriro node install", "Install the node host service."],
          ["oriro node start", "Start the installed node host service."],
          ["oriro node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.oriro.ai/cli/node")}\n`,
    );
