...
async function writeSessionFile(params: { sessionFile: string; sessionId: string }) {
  // The lifecycle compacts canonical Oriro session JSONL, so tests write the
...
describe("runCliTurnCompactionLifecycle", () => {
  ...
  beforeEach(async () => {
    tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-cli-compaction-"));
  ...
  it("compacts over-budget CLI transcripts and clears external CLI resume state", async () => {
    ...
    // Once Oriro rewrites the transcript, external CLI resume ids are stale
  ...
  it("ignores stale native harness ids when the active provider no longer matches", async () => {
    const sessionKey = "agent:main:oriro-after-codex";
    const sessionId = "session-oriro-after-codex";
    const sessionFile = path.join(tmpDir, "session-oriro-after-codex.jsonl");
    const storePath = path.join(tmpDir, "sessions-oriro-after-codex.json");
    await writeSessionFile({ sessionFile, sessionId });
  ...
  it("bounds a hung CLI context-engine compaction and leaves resume state intact", async () => {
    ...
    const pending = runCliTurnCompactionLifecycle({
      cfg: { agents: { defaults: { compaction: { timeoutSeconds: 1 } } } } as OriroConfig,
      sessionId,
      sessionKey,
      sessionEntry,
      sessionStore,
