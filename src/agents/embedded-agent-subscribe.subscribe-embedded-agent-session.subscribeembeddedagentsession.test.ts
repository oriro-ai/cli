...
describe("subscribeEmbeddedAgentSession", () => {
  ...
  async function captureToolLifecycleLogSubsystems(messageChannel?: string): Promise<string[]> {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-tool-log-attribution-"));
    const logFile = path.join(tempDir, "oriro.log");
    try {
      setLoggerOverride({
        level: "debug",
        consoleLevel: "silent",
      ...
      internalEvents: [
        {
          ...
          taskLabel: "oriro boss theme",
          status: "ok",
          statusLabel: "completed successfully",
          result: "Generated 1 track.\nMEDIA:/tmp/oriro-boss.mp3",
          mediaUrls: ["/tmp/oriro-boss.mp3"],
