...
vi.mock("oriro/plugin-sdk/channel-pairing", async () => {
  const actual = await vi.importActual<typeof import("oriro/plugin-sdk/channel-pairing")>(
...
vi.mock("oriro/plugin-sdk/media-store", async () => {
  const actual = await vi.importActual<typeof import("oriro/plugin-sdk/media-store")>(
    "oriro/plugin-sdk/media-store",
  );
  return {
    ...actual,
...
vi.mock("./runtime.js", async () => {
  const { createChannelIngressQueueForTests: createChannelIngressQueue } = await Promise.resolve(
    vi.importActual<typeof import("oriro/plugin-sdk/plugin-state-test-runtime")>(
      "oriro/plugin-sdk/plugin-state-test-runtime",
    ),
  );
  const stateDir = `/tmp/oriro-whatsapp-inbound-media-${Date.now()}-${Math.random()}`;
  return {
...
const HOME = path.join(os.tmpdir(), `oriro-inbound-media-${crypto.randomUUID()}`);
