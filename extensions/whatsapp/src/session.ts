...
import { VERSION } from "oriro/plugin-sdk/cli-runtime";
import {
  createHttp1EnvHttpProxyAgent,
  createHttp1ProxyAgent,
  createNodeProxyAgent,
} from "oriro/plugin-sdk/fetch-runtime";
import { danger, success } from "oriro/plugin-sdk/runtime-env";
import { getChildLogger, toPinoLikeLogger } from "oriro/plugin-sdk/runtime-env";
      ...
      try {
        ...
        if (connection === "close") {
          ...
          if (status === LOGGED_OUT_STATUS) {
            console.error(
              danger(
                `WhatsApp session logged out. Run: ${formatCliCommand("oriro channels login")}`,
              ),
            );
          }
        }
