...
function shouldLogMcpLoopbackHttp(): boolean {
  return (
    isTruthyEnvValue(process.env.ORIRO_CLI_BACKEND_LOG_OUTPUT) ||
    isTruthyEnvValue(process.env.ORIRO_LIVE_CLI_BACKEND_DEBUG)
  );
}
...
function resolveScopedSessionKey(cfg: OriroConfig, rawSessionKey: string | undefined): string {
...
export function resolveMcpHttpBodyTimeoutMs(): number {
  return readPositiveIntEnv("ORIRO_MCP_LOOPBACK_BODY_TIMEOUT_MS", DEFAULT_MCP_BODY_TIMEOUT_MS);
}

export function resolveMcpCliCaptureKey(req: IncomingMessage): string | undefined {
  return normalizeOptionalString(getHeader(req, "x-oriro-cli-capture-key"));
}
...
export function resolveMcpRequestContext(
  ...
  return {
    ...
    sessionId: normalizeOptionalString(getHeader(req, "x-oriro-session-id")),
    messageProvider:
      normalizeMessageChannel(getHeader(req, "x-oriro-message-channel")) ?? undefined,
    currentChannelId: normalizeOptionalString(getHeader(req, "x-oriro-current-channel-id")),
    currentThreadTs: normalizeOptionalString(getHeader(req, "x-oriro-current-thread-ts")),
    currentMessageId: normalizeOptionalString(getHeader(req, "x-oriro-current-message-id")),
    currentInboundAudio: normalizeMcpBooleanHeader(
      getHeader(req, "x-oriro-current-inbound-audio"),
    ),
    accountId: normalizeOptionalString(getHeader(req, "x-oriro-account-id")),
    inboundEventKind: normalizeMcpInboundEventKind(getHeader(req, "x-oriro-inbound-event-kind")),
    sourceReplyDeliveryMode: normalizeMcpSourceReplyDeliveryMode(
      getHeader(req, "x-oriro-source-reply-delivery-mode"),
    ),
    requireExplicitMessageTarget: normalizeMcpBooleanHeader(
      getHeader(req, "x-oriro-require-explicit-message-target"),
    ),
