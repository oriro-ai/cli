...
const PUBLISHED_INSTALLER_BASE_URL = "https://oriro.ai";
...
export const CROSS_OS_AGENT_TURN_TIMEOUT_SECONDS = parsePositiveIntegerEnv(
  "ORIRO_CROSS_OS_AGENT_TURN_TIMEOUT_SECONDS",
  600,
);
...
const CROSS_OS_PROCESS_TREE_KILL_AFTER_MS = parsePositiveIntegerEnv(
  "ORIRO_CROSS_OS_PROCESS_TREE_KILL_AFTER_MS",
  15_000,
);
...
export function resolveProviderConfig(provider, env = process.env) {
  ...
  const providerEnvKey = `ORIRO_CROSS_OS_${provider.toUpperCase().replace(/[^A-Z0-9]+/gu, "_")}_MODEL`;
  const model = env[providerEnvKey]?.trim() || env.ORIRO_CROSS_OS_MODEL?.trim() || config.model;
  return { ...config, model };
}
...
function buildReleaseProviderConfigOverride(providerMeta) {
  ...
  return {
    ...
    ...(providerMeta.extensionId === "openai" ? { agentRuntime: { id: "oriro" } } : {}),
    models: [],
...
const INSTALL_STAGE_DEBRIS_DIR_PATTERN = /^\.oriro-install-stage(?:-[^/]+)?$/iu;
...
export const CROSS_OS_DISCORD_FETCH_TIMEOUT_MS = parsePositiveIntegerEnv(
  "ORIRO_CROSS_OS_DISCORD_FETCH_TIMEOUT_MS",
  10_000,
);
...
export const CROSS_OS_COMMAND_HEARTBEAT_SECONDS = parsePositiveIntegerEnv(
  "ORIRO_CROSS_OS_COMMAND_HEARTBEAT_SECONDS",
  60,
);
...
export function resolveCrossOsAgentTurnOptional(env = process.env) {
  return parseBooleanEnv("ORIRO_CROSS_OS_AGENT_TURN_OPTIONAL", false, env);
}
...
export function readRunnerOverrideEnv(env = process.env) {
  ...
  return {
    varUbuntuRunner: preferNonEmptyEnv(
      ...
      env.ORIRO_RELEASE_CHECKS_UBUNTU_RUNNER,
    ),
    varWindowsRunner: preferNonEmptyEnv(
      env.VAR_WINDOWS_RUNNER,
      env.ORIRO_RELEASE_CHECKS_WINDOWS_RUNNER,
    ),
    varMacosRunner: preferNonEmptyEnv(
      env.VAR_MACOS_RUNNER,
      env.ORIRO_RELEASE_CHECKS_MACOS_RUNNER,
    ),
  };
}
...
async function main(argv) {
  ...
  const baselineSpec =
    ...
    (previousVersion ? `oriro@${previousVersion}` : "oriro@latest");
  ...
  const summary = {
    ...
    runnerOs: process.env.ORIRO_RELEASE_CHECK_OS ?? "",
    runnerLabel: process.env.ORIRO_RELEASE_CHECK_RUNNER ?? "",
    provider,
    mode,
    suite,
    ref: inputRef || null,
  ...
  try {
    ...
    await runInstalledCli({
      ...
      env: {
        ...
        ORIRO_UPDATE_DEV_TARGET_REF: verificationRef,
      },
      cwd: lane.homeDir,
...
function createLaneState(name) {
  const rootDir = mkdtempSync(join(tmpdir(), `oriro-${name}-`));
  const prefixDir = join(rootDir, "prefix");
  const homeDir = join(rootDir, "home");
  const stateDir = join(homeDir, ".oriro");
...
function buildLaneEnv(lane, providerMeta, providerSecretValue) {
  ...
  return {
    ...
    ORIRO_HOME: lane.homeDir,
    ORIRO_STATE_DIR: lane.stateDir,
    ORIRO_CONFIG_PATH: join(lane.stateDir, "oriro.json"),
    ORIRO_DISABLE_BONJOUR: "1",
...
export function buildRealUpdateEnv(env) {
  ...
  delete updateEnv.ORIRO_DISABLE_BUNDLED_PLUGIN_POSTINSTALL;
...
export function isRecoverableWindowsPackagedUpgradeTimeoutError(
  ...
  return (
    ...
    /[/\\]oriro\.mjs update --tag http:\/\/127\.0\.0\.1:\d+\/oriro[^/\s]*\.tgz --yes --json(?: --no-restart)? --timeout \d+/u.test(
      message,
    )
  );
}
...
export function resolveExplicitBaselineVersion(baselineSpec) {
  ...
  if (!trimmed || trimmed === "oriro@latest") {
    return "";
  }
  if (trimmed.startsWith("oriro@")) {
    return trimmed.slice("oriro@".length);
  }
  return trimmed;
}
...
async function resolveInstallerTargetVersion(params) {
  ...
  const latestResult = await runCommand(npmCommand(), ["view", "oriro@latest", "version"], {
    logPath: join(params.logsDir, `${params.suiteName}-latest-version.log`),
    timeoutMs: 2 * 60 * 1000,
  });
  const latestVersion = latestResult.stdout.trim();
  if (!latestVersion) {
    throw new Error("npm view oriro@latest version did not return a version.");
  }
...
if ($null -ne $npmCommand) {
  ...
  if (-not [string]::IsNullOrWhiteSpace($npmPrefix)) {
    ...
    foreach ($candidate in @(
      (Join-Path $npmPrefix 'oriro.cmd'),
      (Join-Path $npmPrefix 'oriro.ps1')
    )) {
...
if ([string]::IsNullOrWhiteSpace($commandPath)) {
  $cmd = Get-Command oriro -ErrorAction Stop
...
Write-Output "__ORIRO_PATH__=$commandPath"
...
async function verifyFreshShellCommand(params) {
  if (process.platform === "win32") {
    ...
    const cliPath = normalizeWindowsInstalledCliPath(
      parseMarkerLine(result.stdout, "__ORIRO_PATH__="),
    );
    if (!cliPath) {
      throw new Error("Failed to resolve installed oriro path from fresh Windows shell.");
    }
    return {
      cliPath,
  ...
  const script = [
    ...
    "command -v oriro >/dev/null 2>&1",
    'printf "__ORIRO_PATH__=%s\\n" "$(command -v oriro)"',
    "oriro --version",
  ].join("\n");
  ...
  const cliPath = parseMarkerLine(result.stdout, "__ORIRO_PATH__=");
  const versionOutput = `${result.stdout}\n${result.stderr}`.trim();
  if (!cliPath) {
    throw new Error("Failed to resolve installed oriro path from fresh POSIX shell.");
  }
...
async function ensureDevUpdateGitInstall(params) {
  ...
  // The dev-update lane must prove that `oriro update --channel dev` landed on
...
async function configureDiscordSmoke(params) {
  ...
  if (!shouldUseManagedGatewayService()) {
    ...
    const gateway = await startManualGatewayFromInstalledCli({
      ...
      cliPath: params.cliPath,
      env: gatewayEnv,
      logPath: join(params.cwd, `.oriro/logs/${params.lane.name}-discord-gateway.log`),
    });
...
export function dashboardHtmlMarkerStatus(html: string): {
  ...
  const title = html.includes("<title>Oriro Control</title>");
  const app = html.includes("<oriro-app></oriro-app>");
...
async function maybeRunDiscordRoundtrip(params) {
  const token =
    process.env.ORIRO_DISCORD_SMOKE_BOT_TOKEN?.trim() ||
    process.env.DISCORD_BOT_TOKEN?.trim() ||
    "";
  const guildId = process.env.ORIRO_DISCORD_SMOKE_GUILD_ID?.trim() || "";
  const channelId = process.env.ORIRO_DISCORD_SMOKE_CHANNEL_ID?.trim() || "";
...
async function runBundledPluginPostinstall(params) {
  ...
  delete installEnv.ORIRO_DISABLE_BUNDLED_PLUGIN_POSTINSTALL;
...
export function buildInstalledBrowserOverrideImportProbeScript(
  runtimeModuleSpecifier = "oriro/plugin-sdk/plugin-runtime",
) {
  return `
import { existsSync } from "node:fs";
import { startLazyPluginServiceModule } from ${JSON.stringify(runtimeModuleSpecifier)};

const startedPath = process.env.ORIRO_BROWSER_OVERRIDE_STARTED_PATH;
const stoppedPath = process.env.ORIRO_BROWSER_OVERRIDE_STOPPED_PATH;

if (!process.env.ORIRO_BROWSER_CONTROL_MODULE) {
  throw new Error("Missing ORIRO_BROWSER_CONTROL_MODULE.");
}
...
const handle = await startLazyPluginServiceModule({
  overrideEnvVar: "ORIRO_BROWSER_CONTROL_MODULE",
...
export async function startBrowserControlService() {
  writeFileSync(process.env.ORIRO_BROWSER_OVERRIDE_STARTED_PATH, "started\\n", "utf8");
}

export async function stopBrowserControlService() {
  writeFileSync(process.env.ORIRO_BROWSER_OVERRIDE_STOPPED_PATH, "stopped\\n", "utf8");
}
`.trim();
}
...
async function runInstalledBrowserOverrideImportSmoke(params) {
  ...
  await runCommand(process.execPath, [probePath], {
    ...
    env: {
      ...
      ORIRO_BROWSER_OVERRIDE_STARTED_PATH: startedPath,
      ORIRO_BROWSER_OVERRIDE_STOPPED_PATH: stoppedPath,
    },
    logPath: params.logPath,
    timeoutMs: 60_000,
  });
...
async function runOriro(params) {
...
function installedEntryPath(prefixDir) {
  return join(installedPackageRoot(prefixDir), "oriro.mjs");
}
