...
import {
  ...
  evaluateOriroSweeperExactHeadProof,
  evaluatePullRequestContext,
  hasOriroSweeperExactHeadProof,
...
describe("real-behavior-proof-policy", () => {
    ...
    const evaluation = evaluatePullRequestContext({
      pullRequest: externalPr(
        [
          ...
          "",
          "- Behavior addressed: The gateway dropped the configured Discord channel during startup.",
          "- Real environment tested: macOS 15.4, Node 24, local Oriro gateway.",
          "- Exact steps or command run after this patch: pnpm oriro gateway restart",
  ...
  it("accepts out-of-scope follow-ups as not-tested proof detail", () => {
    const body = [
      ...
      "  - `lowSupported: true`",
      "  - `fallbackFromLow: low`",
      "  - `local command version: Oriro 2026.5.21`",
      "",
  ...
  it("accepts source PR proof when explicit gaps live in out-of-scope follow-ups", () => {
    const body = [
      ...
      "## Evidence",
      "",
      "- Real environment tested: Local macOS source checkout, Node v24.8.0, Oriro 2026.5.21 (c8a35c4), local `oriro` shim pointed at the freshly built checkout. No channel credentials or provider API keys were used.",
      "- Exact steps or command run after this patch:",
      "  1. Built the local checkout with `node scripts/build-all.mjs`.",
      "  2. Updated `/Users/example/.local/bin/oriro` to run this checkout's `oriro.mjs` and verified `/Users/example/.local/bin/oriro --version`.",
      ...
      "  - `lowSupported: true`",
      "  - `fallbackFromLow: low`",
      "  - `local command version: Oriro 2026.5.21 (c8a35c4)`",
      "",
      "## Out-of-scope Follow-ups",
  ...
  it("accepts OriroSweeper pass verdict comments only for the exact PR head", () => {
    const pullRequest = {
      number: 83581,
      head: {
    ...
    const comments = [
      {
        ...
        body: [
          "Codex review: passed.",
          "<!-- orirosweeper-verdict:pass item=83581 sha=06ee95df6608d29a395c52ba8ab53fdd93a9dc4f confidence=high -->",
        ].join("\n"),
      },
    ];

    expect(hasOriroSweeperExactHeadProof({ pullRequest, comments })).toBe(true);
    expect(evaluateOriroSweeperExactHeadProof({ pullRequest, comments }).passed).toBe(true);
    expect(
      hasOriroSweeperExactHeadProof({
        pullRequest: {
          ...pullRequest,
  ...
  it("rejects forged OriroSweeper pass verdict markers from contributor comments", () => {
    const pullRequest = {
      number: 83581,
      head: {
        sha: "06ee95df6608d29a395c52ba8ab53fdd93a9dc4f",
    ...
    const comments = [
      {
        ...
        body: "<!-- orirosweeper-verdict:pass item=83581 sha=06ee95df6608d29a395c52ba8ab53fdd93a9dc4f confidence=high -->",
      },
    ];

    expect(hasOriroSweeperExactHeadProof({ pullRequest, comments })).toBe(false);
    expect(evaluateOriroSweeperExactHeadProof({ pullRequest, comments }).passed).toBe(false);
  });

  it("accepts exact OriroSweeper bot pass verdict markers when GitHub omits the app source", () => {
    const pullRequest = {
      number: 83581,
      head: {
        sha: "06ee95df6608d29a395c52ba8ab53fdd93a9dc4f",
  ...
  it("accepts exact Oriro OriroSweeper bot pass verdict markers when GitHub omits the app source", () => {
    const pullRequest = {
      number: 83581,
      head: {
        sha: "06ee95df6608d29a395c52ba8ab53fdd93a9dc4f",
...
describe("isMaintainerTeamMember", () => {
  ...
  it("returns true for active members", async () => {
    ...
    expect(fetch).toHaveBeenCalledWith(
      "https://api.github.com/orgs/oriro/teams/maintainer/memberships/private-maint",
