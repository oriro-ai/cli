...
import type { PluginCommandContext } from "oriro/plugin-sdk/core";
...
function resolveStoredDreaming(config: OriroConfig): Record<string, unknown> {
  const entry = asRecord(config.plugins?.entries?.["memory-core"]);
  const pluginConfig = asRecord(entry?.config);
  return asRecord(pluginConfig?.dreaming) ?? {};
}

function createHarness(initialConfig: OriroConfig = {}) {
  let runtimeConfig: OriroConfig = initialConfig;

  const runtime = {
    config: {
      current: vi.fn(() => runtimeConfig),
      loadConfig: vi.fn(() => runtimeConfig),
      mutateConfigFile: vi.fn(async ({ mutate }: { mutate: (draft: OriroConfig) => void }) => {
      ...
      writeConfigFile: vi.fn(async (nextConfig: OriroConfig) => {
        runtimeConfig = nextConfig;
      }),
    },
