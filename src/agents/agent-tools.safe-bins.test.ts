...
let createOriroCodingTools: typeof import("./agent-tools.js").createOriroCodingTools;
...
beforeAll(async () => {
  await withEnvAsync(
    {
      ORIRO_BUNDLED_PLUGINS_DIR: path.join(os.tmpdir(), "oriro-test-no-bundled-extensions"),
    },
    async () => {
      ({ createOriroCodingTools } = await import("./agent-tools.js"));
    },
  );
});
...
vi.mock("oriro/plugin-sdk/agent-sessions", () => ({
  AuthStorage: vi.fn(),
...
describe("createOriroCodingTools safeBins", () => {
  it("threads tools.exec.safeBins into exec allowlist checks", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "oriro-safe-bins-",
        safeBins: ["echo"],
        safeBinProfiles: {
  ...
  it("rejects unprofiled custom safe-bin entries", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "oriro-safe-bins-unprofiled-",
        safeBins: ["echo"],
      },
      async ({ tmpDir, execTool }) => {
        await expect(
  ...
  it("does not allow env var expansion to smuggle file args via safeBins", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "oriro-safe-bins-expand-",
        safeBins: ["head", "wc"],
  ...
  it("blocks sort output/compress bypass attempts in safeBins mode", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "oriro-safe-bins-sort-",
        safeBins: ["sort"],
  ...
  it("blocks shell redirection metacharacters in safeBins mode", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "oriro-safe-bins-redirect-",
        safeBins: ["head"],
  ...
  it("blocks grep recursive flags from reading cwd via safeBins", async () => {
    await withSafeBinsExecTool(
      {
        tmpPrefix: "oriro-safe-bins-grep-",
        safeBins: ["grep"],
