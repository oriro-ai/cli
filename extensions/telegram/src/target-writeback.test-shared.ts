...
vi.mock("oriro/plugin-sdk/config-mutation", async () => {
  const actual = await vi.importActual<typeof import("oriro/plugin-sdk/config-mutation")>(
    "oriro/plugin-sdk/config-mutation",
  );
  return {
    ...actual,
...
vi.mock("oriro/plugin-sdk/cron-store-runtime", async () => {
  const actual = await vi.importActual<typeof import("oriro/plugin-sdk/cron-store-runtime")>(
    "oriro/plugin-sdk/cron-store-runtime",
  );
  return {
    ...actual,
    loadCronStore,
...
export function installMaybePersistResolvedTelegramTargetTests(params?: {
  ...
  describe("maybePersistResolvedTelegramTarget", () => {
    ...
    it("writes back matching config and cron targets", async () => {
      ...
      expect(writeOptions.expectedConfigPath).toBe("/tmp/oriro.json");
      expect(saveCronStore).toHaveBeenCalledTimes(1);
