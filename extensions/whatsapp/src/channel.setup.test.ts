...
import { createQueuedWizardPrompter } from "oriro/plugin-sdk/plugin-test-runtime";
...
vi.mock("oriro/plugin-sdk/setup", async () => {
  ...
  return {
    ...
    setSetupChannelEnabled: (cfg: OriroConfig, channel: string, enabled: boolean) => ({
      ...cfg,
      channels: {
        ...cfg.channels,
        [channel]: {
          ...(cfg.channels?.[channel as keyof NonNullable<OriroConfig["channels"]>] as object),
          enabled,
        },
      },
    }),
