...
read_when:
  ...
  - You need the numbers behind the Oriro performance and dependency blog post
  - You are changing release gates, package shrinkwrap, or plugin dependency boundaries
title: "Release performance sweep"
---

This page captures the evidence behind the May 2026 Oriro performance,
...
- **Release performance sweep:** GitHub Releases from `v2026.5.28` back through
  stable `v2026.4.23`, using the `Oriro Performance` workflow,
  ...
  artifacts.
- **Earlier April context:** published `orirogrit-reports` mock-provider
...
- **npm package size sweep:** `npm pack oriro@<version> --dry-run --json`
...
medians from `orirogrit-reports`. Treat the numbers as trend evidence and
...
<CardGroup cols={2}>
  ...
  <Card title="Latest stable install" icon="hard-drive">
    **361.7MiB fresh install**
    ...
    `v2026.5.28` cuts the nested Oriro dependency tree sharply, but a
...
<CardGroup cols={2}>
  ...
  <Card title="Shrinkwrap introduced" icon="lock">
    **1,020.6MB install**
    ...
    911.8MB landed under nested `oriro/node_modules`.
  ...
  <Card title="Latest stable" icon="tag">
    **361.7MiB install**
    ...
    installs a 259.7MiB nested Oriro tree.
...
<CardGroup cols={2}>
  ...
  <Card title="Nested tree" icon="unplug">
    Nested `oriro/node_modules` fell from **656.1MiB** to **259.7MiB** in
...
`orirogrit-reports` mock-provider run. That run used repeat 3 and failed only
...
| Nested `oriro/node_modules` from `2026.5.22` |   911.8MB |     259.7MiB | 71.5% lower |
| Nested `oriro/node_modules` from `2026.5.27` |  656.1MiB |     259.7MiB | 60.4% lower |
...
| Point              | Installed deps | Fresh install | Oriro package | Nested `oriro/node_modules` | Root shrinkwrap | Canvas install behavior                   |
| ------------------ | -------------: | ------------: | ---------------: | -----------------------------: | --------------- | ----------------------------------------- |
...
<CardGroup cols={2}>
  <Card title="Before shrinkwrap" icon="unlock">
    `2026.5.20` has no root shrinkwrap and no large nested Oriro dependency
    tree.
  </Card>
...
that made npm materialize a large nested Oriro dependency tree and all 12
