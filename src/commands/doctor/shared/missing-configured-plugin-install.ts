...
import { normalizeOptionalLowercaseString } from "@oriro/normalization-core/string-coerce";
import {
...
import { buildOriroHubPluginInstallRecordFields } from "../../../plugins/orirohub-install-records.js";
import { ORIROHUB_INSTALL_ERROR_CODE, installPluginFromOriroHub } from "../../../plugins/orirohub.js";
...
const ORIRO_BETA_COMPANION_VERSION_RE = /^(\d{4}\.[1-9]\d?\.[1-9]\d?)-beta\.[1-9]\d*$/;
const ORIRO_STABLE_OR_BETA_COMPANION_VERSION_RE =
  /^(\d{4}\.[1-9]\d?\.[1-9]\d?)(?:-beta\.[1-9]\d*)?$/;

function shouldFallbackOriroHubToNpm(params: {
...
function resolveCandidateOriroHubSpec(install: PluginPackageInstall): string | undefined {
  const explicit = install.orirohubSpec?.trim();
  if (explicit) {
    return explicit;
  }
  return undefined;
...
function addConfiguredAgentRuntimePluginIds(ids: Set<string>, cfg: OriroConfig): void {
...
function addConfiguredSpeechProviderPluginIds(ids: Set<string>, cfg: OriroConfig): void {
  for (const pluginId of resolveOfficialExternalProviderContractPluginIds({
...
function addConfiguredWebFetchProviderPluginIds(ids: Set<string>, cfg: OriroConfig): void {
...
function collectConfiguredPluginIds(cfg: OriroConfig, env?: NodeJS.ProcessEnv): Set<string> {
...
function collectBlockedPluginIds(cfg: OriroConfig): Set<string> {
  const ids = new Set<string>();
...
function collectConfiguredChannelIds(cfg: OriroConfig, env?: NodeJS.ProcessEnv): Set<string> {
  const ids = new Set<string>();
...
function collectDownloadableInstallCandidates(params: {
  ...
  for (const entry of listRawChannelPluginCatalogEntries({
    ...
    const orirohubSpec = resolveCandidateOriroHubSpec(entry.install);
    if (!npmSpec && !orirohubSpec) {
      continue;
    }
  ...
  for (const entry of listOfficialExternalPluginCatalogEntries()) {
    ...
    const orirohubSpec = resolveCandidateOriroHubSpec(install);
    if (!npmSpec && !orirohubSpec) {
      continue;
    }
    candidates.set(pluginId, {
...
function installedRuntimePackageVersionIsStale(params: {
  ...
  const comparison = compareOriroReleaseVersions(params.installedVersion, params.currentVersion);
...
function betaCompanionMatchesCurrentStableVersion(params: {
  ...
  const installedBase = ORIRO_BETA_COMPANION_VERSION_RE.exec(params.installedVersion)?.[1];
  const currentBase = ORIRO_STABLE_OR_BETA_COMPANION_VERSION_RE.exec(params.currentVersion)?.[1];
...
function collectCandidateOfficialPackageNames(
  ...
  const orirohubName = candidate.orirohubSpec
    ? parseOriroHubPluginSpec(candidate.orirohubSpec)?.name
    : undefined;
  if (npmName) {
    names.add(npmName);
  }
  if (orirohubName) {
    names.add(orirohubName);
  }
  return names;
}
...
function collectInstalledRecordPackageNames(record: PluginInstallRecord): Set<string> {
  ...
  if (record.source === "orirohub") {
    const specName = record.spec ? parseOriroHubPluginSpec(record.spec)?.name : undefined;
    for (const value of [record.orirohubPackage, specName]) {
      if (value) {
        names.add(value);
      }
    }
...
function isTrustedOfficialInstallRecordForCandidate(params: {
  ...
  if (record.source !== "npm" && record.source !== "orirohub") {
    return false;
  }
...
function recordMatchesBundledPackage(
  ...
  if (record.source === "orirohub") {
    return [record.orirohubPackage, record.spec].some(
      (value) => recordOriroHubPackageName(value) === packageName,
    );
  }
  return false;
}
...
function recordOriroHubPackageName(value: string | undefined): string | undefined {
  const trimmed = value?.trim();
  if (!trimmed) {
    return undefined;
  }
  return parseOriroHubPluginSpec(trimmed)?.name ?? trimmed;
}
...
async function installCandidate(params: {
  ...
  const orirohubSpecs = candidate.orirohubSpec
    ? resolveOriroHubInstallSpecsForUpdateChannel({
        spec: candidate.orirohubSpec,
        updateChannel: params.updateChannel,
      })
    : null;
  const npmSpecs = candidate.npmSpec
    ...
    : null;
  const orirohubInstallSpec = orirohubSpecs?.installSpec ?? candidate.orirohubSpec;
  const npmInstallSpec = npmSpecs?.installSpec ?? candidate.npmSpec;
  const npmDir = resolveDefaultPluginNpmDir(params.env);
  const existingOriroHubPackagePath = orirohubInstallSpec
    ? resolveExistingCandidateOriroHubPackagePath({
        candidate,
        extensionsDir,
      })
    : null;
  ...
  const shouldTryOriroHub =
    orirohubInstallSpec &&
    !existingNpmPackagePath &&
    !(params.preferNpm && npmInstallSpec) &&
    candidate.defaultChoice !== "npm";
  if (shouldTryOriroHub) {
    const orirohubResult = await installPluginFromOriroHub({
      spec: orirohubInstallSpec,
      extensionsDir,
      env: params.env,
      expectedPluginId: candidate.pluginId,
      mode: params.mode === "update" || existingOriroHubPackagePath ? "update" : "install",
    });
    if (orirohubResult.ok) {
      const pluginId = orirohubResult.pluginId;
      return {
        records: {
          ...params.records,
          [pluginId]: {
            ...buildOriroHubPluginInstallRecordFields(orirohubResult.orirohub),
            spec: orirohubSpecs?.recordSpec ?? orirohubInstallSpec,
            installPath: orirohubResult.targetDir,
        ...
        changes: [
          formatInstalledConfiguredPluginChange({
            pluginId,
            installSpec: orirohubInstallSpec,
    ...
    if (
      !npmInstallSpec ||
      !shouldFallbackOriroHubToNpm({ result: orirohubResult, npmSpec: npmInstallSpec })
    ) {
      return {
        records: params.records,
        changes: [],
        warnings: [
          `Failed to install missing configured plugin "${candidate.pluginId}" from ${orirohubInstallSpec}: ${orirohubResult.error}`,
        ],
        failedPluginId: candidate.pluginId,
      };
    }
    changes.push(
      `OriroHub ${orirohubInstallSpec} unavailable for "${candidate.pluginId}"; falling back to npm ${npmInstallSpec}.`,
    );
  }
  if (!npmInstallSpec) {
    return {
...
function resolveExistingCandidateOriroHubPackagePath(params: {
  candidate: DownloadableInstallCandidate;
...
/** Repair missing installs inferred from the current Oriro config. */
...
async function repairMissingPluginInstalls(params: {
  ...
  if (shouldDeferConfiguredPluginInstallRepair(env)) {
    ...
    for (const pluginId of updateDeferredPluginIds) {
      ...
      const detail = `Skipped package-manager repair for configured plugin "${pluginId}" during package update; rerun "oriro doctor --fix" after the update completes.`;
      changes.push(detail);
