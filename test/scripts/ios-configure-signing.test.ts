...
function runConfigureSigning(teamId: string, user = "localuser"): string {
  return execFileSync(BASH_BIN, bashArgs(fixtureScript), {
    env: {
      ...
      ORIRO_IOS_APP_BUNDLE_ID: "",
      ORIRO_IOS_BUNDLE_ID_BASE: "",
      ORIRO_IOS_BUNDLE_SUFFIX: "",
      ORIRO_IOS_APP_GROUP_ID: "",
      ORIRO_IOS_SHARE_BUNDLE_ID: "",
      ORIRO_IOS_ACTIVITY_WIDGET_BUNDLE_ID: "",
      ORIRO_IOS_WATCH_APP_BUNDLE_ID: "",
      USER: user,
    },
    encoding: "utf8",
...
describe.sequential("scripts/ios-configure-signing.sh", () => {
  beforeAll(() => {
    const fixtureRoot = makeTempDir(tempDirs, "oriro-ios-configure-signing-");
  ...
  it("uses the canonical app bundle ID for the canonical Oriro team", () => {
    const stdout = runConfigureSigning("FWJYW4S8P8");
    const generated = readGeneratedSigning();

    expect(stdout).toContain("team=FWJYW4S8P8 app=ai.orirofoundation.app");
    expect(generated).toContain("ORIRO_DEVELOPMENT_TEAM = FWJYW4S8P8");
    expect(generated).toContain("ORIRO_APP_BUNDLE_ID = ai.orirofoundation.app");
    expect(generated).toContain("ORIRO_SHARE_BUNDLE_ID = ai.orirofoundation.app.share");
    expect(generated).toContain("ORIRO_APP_GROUP_ID = group.ai.orirofoundation.app.shared");
    expect(generated).toContain("ORIRO_ACTIVITY_WIDGET_PROFILE = ");
  });
  ...
  it("keeps unique local bundle IDs for non-canonical fallback teams", () => {
    ...
    expect(stdout).toContain(
      "canonical_team=FWJYW4S8P8 local_team=Y3YUZP442G app=ai.orirofoundation.app.test.localuser-y3yuzp442g",
    );
    expect(generated).toContain("ORIRO_DEVELOPMENT_TEAM = Y3YUZP442G");
    expect(generated).toContain(
      "ORIRO_APP_BUNDLE_ID = ai.orirofoundation.app.test.localuser-y3yuzp442g",
    );
    expect(generated).toContain(
      "ORIRO_APP_GROUP_ID = group.ai.orirofoundation.app.test.localuser-y3yuzp442g.shared",
    );
  });
});
