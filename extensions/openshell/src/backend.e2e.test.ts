...
import { createSandboxTestContext } from "oriro/plugin-sdk/test-fixtures";
import {
...
const ORIRO_OPENSHELL_E2E = process.env.ORIRO_E2E_OPENSHELL === "1";
const ORIRO_OPENSHELL_E2E_TIMEOUT_MS = 12 * 60_000;
const ORIRO_OPENSHELL_COMMAND =
  process.env.ORIRO_E2E_OPENSHELL_COMMAND?.trim() || "openshell";
...
RUN echo "oriro-openshell-e2e" > /opt/openshell-e2e-marker.txt
...
describe("openshell sandbox backend e2e", () => {
  it.runIf(process.platform !== "win32" && ORIRO_OPENSHELL_E2E)(
    "creates a remote-canonical sandbox through OpenShell and executes over SSH",
    { timeout: ORIRO_OPENSHELL_E2E_TIMEOUT_MS },
    async () => {
      if (!(await dockerReady())) {
        return;
      }
      if (!(await commandAvailable(ORIRO_OPENSHELL_COMMAND))) {
        return;
      }
      if (!(await openshellGatewayAvailable(ORIRO_OPENSHELL_COMMAND))) {
        return;
      }

      const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-openshell-e2e-"));
      ...
      const gatewayName = `oriro-e2e-${scopeSuffix}`;
      const scopeKey = `session:openshell-e2e-deny:${scopeSuffix}`;
      const allowSandboxName = `oriro-policy-allow-${scopeSuffix}`;
      ...
      const pluginConfig = resolveOpenShellPluginConfig({
        command: ORIRO_OPENSHELL_COMMAND,
        gateway: gatewayName,
        from: dockerfilePath,
        mode: "remote",
        autoProviders: false,
      ...
      try {
        ...
        expect(stdout).toContain("oriro-openshell-e2e");
