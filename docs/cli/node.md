---
summary: "CLI reference for `oriro node` (headless node host)"
read_when:
...
`oriro node run` and `oriro node install` resolve gateway auth from config/env (no `--token`/`--password` flags on node commands):

- `ORIRO_GATEWAY_TOKEN` / `ORIRO_GATEWAY_PASSWORD` are checked first.
...
trusted private-DNS names, set `ORIRO_ALLOW_INSECURE_PRIVATE_WS=1`; without
it, node startup fails closed and asks you to use `wss://`, an SSH tunnel, or
Tailscale. This is a process-environment opt-in, not an `oriro.json` config
key.
`oriro node install` persists it into the supervised node service when it is
...
```bash
oriro node install --host <gateway-host> --port 18789
```
...
oriro node stop
oriro node restart
oriro node uninstall
```

Use `oriro node run` for a foreground node host (no service).
...
- `oriro approvals --node <id|name|ip>` (edit from the Gateway)

For approved async node exec, Oriro prepares a canonical `systemRunPlan`
