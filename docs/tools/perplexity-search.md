...
Oriro supports Perplexity Search API as a `web_search` provider.
It returns structured results with `title`, `url`, and `snippet` fields.

For compatibility, Oriro also supports legacy Perplexity Sonar/OpenRouter setups.
...
**Via config:** run `oriro configure --section web`. It stores the key in
`~/.oriro-ai/cli.json` under `plugins.entries.perplexity.config.webSearch.apiKey`.
...
`~/.oriro/.env` (or your service environment). See [Env vars](/help/faq#env-vars-and-env-loading).
