...
function readServiceProxyEnvironment(
  ...
  // Service env intentionally preserves only the canonical Oriro proxy knob;
  // generic shell proxy vars are audited but not frozen into services.
  const proxyUrl = normalizeOptionalString(env.ORIRO_PROXY_URL);
  return proxyUrl ? { ORIRO_PROXY_URL: proxyUrl } : {};
}
...
export function buildServiceEnvironment(params: {
  ...
  const profile = env.ORIRO_PROFILE;
  const wrapperPath = normalizeOptionalString(env.ORIRO_WRAPPER);
  const resolvedLaunchdLabel =
    launchdLabel || (platform === "darwin" ? resolveGatewayLaunchAgentLabel(profile) : undefined);
  const systemdUnit = resolveGatewaySystemdUnitEnv(env);
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    ORIRO_PROFILE: profile,
    ORIRO_WRAPPER: wrapperPath,
    ORIRO_GATEWAY_PORT: String(port),
    ORIRO_LAUNCHD_LABEL: resolvedLaunchdLabel,
    ORIRO_SYSTEMD_UNIT: systemdUnit,
    ORIRO_WINDOWS_TASK_NAME: resolveGatewayWindowsTaskName(profile),
    ORIRO_SERVICE_MARKER: GATEWAY_SERVICE_MARKER,
    ORIRO_SERVICE_KIND: GATEWAY_SERVICE_KIND,
    ORIRO_SERVICE_VERSION: VERSION,
  };
}
...
export function buildNodeServiceEnvironment(params: {
  ...
  const gatewayToken = normalizeOptionalString(env.ORIRO_GATEWAY_TOKEN);
  const allowInsecurePrivateWs = normalizeOptionalString(env.ORIRO_ALLOW_INSECURE_PRIVATE_WS);
  return {
    ...buildCommonServiceEnvironment(env, sharedEnv),
    ORIRO_GATEWAY_TOKEN: gatewayToken,
    ORIRO_ALLOW_INSECURE_PRIVATE_WS: allowInsecurePrivateWs,
    ORIRO_LAUNCHD_LABEL: resolveNodeLaunchAgentLabel(),
    ORIRO_SYSTEMD_UNIT: resolveNodeSystemdServiceName(),
    ORIRO_WINDOWS_TASK_NAME: resolveNodeWindowsTaskName(),
    ORIRO_WINDOWS_TASK_HIDDEN_LAUNCHER: "1",
    ORIRO_TASK_SCRIPT_NAME: NODE_WINDOWS_TASK_SCRIPT_NAME,
    ORIRO_LOG_PREFIX: "node",
    ORIRO_SERVICE_MARKER: NODE_SERVICE_MARKER,
    ORIRO_SERVICE_KIND: NODE_SERVICE_KIND,
    ORIRO_SERVICE_VERSION: VERSION,
  };
}
...
function buildCommonServiceEnvironment(
  ...
  const serviceEnv: Record<string, string | undefined> = {
    ...
    ORIRO_STATE_DIR: sharedEnv.stateDir,
    ORIRO_CONFIG_PATH: sharedEnv.configPath,
    ...sharedEnv.proxyEnv,
  };
...
function resolveSharedServiceEnvironmentFields(
  ...
  const stateDir = env.ORIRO_STATE_DIR;
