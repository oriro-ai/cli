...
describe("resolveMissingOfficialExternalChannelPluginRepairHint", () => {
  ...
  it("prefers the OriroHub install hint for externalized WhatsApp", () => {
    mocks.resolveConfiguredChannelPresencePolicy.mockReturnValue([
      {
        channelId: "whatsapp",
        sources: ["explicit-config"],
    ...
    expect(
      ...
      channelId: "whatsapp",
      label: "WhatsApp",
      installSpec: "orirohub:@oriro/whatsapp",
      installCommand: "oriro plugins install orirohub:@oriro/whatsapp",
    });
  });
