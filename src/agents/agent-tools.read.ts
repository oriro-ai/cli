...
function resolveAdaptiveReadMaxBytes(options?: OriroReadToolOptions): number {
...
function withWorkspaceSafeTempHint(error: unknown): unknown {
  ...
  const message = error.message.includes(".oriro/tmp/")
    ? error.message
    : `${error.message}. Use a relative path under \`.oriro/tmp/\` inside the workspace for scratch/temp/meta files that file tools need to read or write later.`;
...
/** Create a sandbox-backed read tool with Oriro result normalization. */
export function createSandboxedReadTool(params: SandboxToolParams) {
  const base = createReadTool(params.root, {
    operations: createSandboxReadOperations(params),
  }) as unknown as AnyAgentTool;
  return createOriroReadTool(base, {
...
/** Wrap the base read tool with Oriro paging, MIME, and image handling. */
export function createOriroReadTool(
  base: AnyAgentTool,
  options?: OriroReadToolOptions,
): AnyAgentTool {
  return {
    ...base,
