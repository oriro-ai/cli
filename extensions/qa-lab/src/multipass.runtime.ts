...
import { sleep } from "oriro/plugin-sdk/runtime-env";
...
const MULTIPASS_MOUNTED_REPO_PATH = "/workspace/oriro-host";
const MULTIPASS_GUEST_REPO_PATH = "/workspace/oriro";
const MULTIPASS_GUEST_CODEX_HOME_PATH = "/workspace/oriro-codex-home";
...
export function createQaMultipassPlan(params: {
  ...
  const hostLiveProviderConfigPath =
    liveProviderConfig && fs.existsSync(liveProviderConfig.path)
      ...
      : undefined;
  const vmName = `oriro-qa-${createVmSuffix()}`;
...
export function renderQaMultipassGuestScript(
  ...
  const qaCommand = [
    ...Object.entries(plan.forwardedEnv)
      .filter(
        ([key]) =>
          ...
          key !== "ORIRO_QA_LIVE_PROVIDER_CONFIG_PATH",
      )
    ...
    ...(plan.guestLiveProviderConfigPath
      ? [
          `ORIRO_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
          `ORIRO_QA_LIVE_PROVIDER_CONFIG_PATH=${shellQuote(plan.guestLiveProviderConfigPath)}`,
        ]
      : []),
...
export async function runQaMultipass(params: {
  ...
  try {
    ...
    throw new Error(
      `Multipass is not installed on this host. Install it with '${resolveMultipassInstallHint()}', then rerun 'pnpm oriro qa suite --runner multipass'.`,
      { cause: error },
    );
  }

  const hostTransferDirPath = await fs.promises.mkdtemp(
    path.join(resolvePreferredOriroTmpDir(), `${plan.vmName}-qa-suite-`),
  );
