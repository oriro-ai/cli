...
type CollectDangerousConfigFlags = (cfg: OriroConfig) => string[];
...
export function collectGatewayConfigFindings(
  ...
  const envTokenConfigured = hasNonEmptyString(env.ORIRO_GATEWAY_TOKEN);
  const envPasswordConfigured = hasNonEmptyString(env.ORIRO_GATEWAY_PASSWORD);
