#!/usr/bin/env node
/** ACP stdio server that bridges Agent Client Protocol clients to the Oriro Gateway. */
...
function printHelp(): void {
  console.log(`Usage: oriro acp [options]
...
if (isMainModule({ currentFile: fileURLToPath(import.meta.url) })) {
  ...
  if (argv.includes("--token") || argv.includes("--gateway-token")) {
    console.error(
      "Warning: --token can be exposed via process listings. Prefer --token-file or ORIRO_GATEWAY_TOKEN.",
    );
  }
