...
describe("default wiki prompt section", () => {
  ...
  it("can append a compact compiled digest snapshot when enabled", async () => {
    ...
    await fs.mkdir(path.join(rootDir, ".oriro-wiki", "cache"), { recursive: true });
    await fs.writeFile(
      path.join(rootDir, ".oriro-wiki", "cache", "agent-digest.json"),
      JSON.stringify(
        {
          claimCount: 8,
  ...
  it("stabilizes digest prompt ordering for prompt-cache-friendly output", async () => {
    ...
    const digestPath = path.join(rootDir, ".oriro-wiki", "cache", "agent-digest.json");
