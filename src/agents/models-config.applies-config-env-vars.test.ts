...
async function resolveProvidersForConfigEnvTest(params: {
  ...
  return await resolveProvidersForModelsJsonWithDeps(
    {
      cfg: params.cfg,
      agentDir: "/tmp/oriro-models-config-env-vars-test",
      env,
    },
    {
...
function createConfigEnvVarsConfig(): OriroConfig {
  return {
    models: { providers: {} },
    env: {
      vars: {
...
async function resolveProvidersAndCaptureDiscoveryEnv(cfg: OriroConfig) {
...
let unauthenticatedProviderWritePlan: Awaited<ReturnType<typeof planOriroModelsJsonWithDeps>>;
...
beforeAll(async () => {
  ...
  unauthenticatedProviderWritePlan = await planOriroModelsJsonWithDeps(
    {
...
describe("models-config", () => {
  it("keeps the implicit provider catalog when explicit baseUrl is blank", async () => {
    let observedConfig: OriroConfig | undefined;
  ...
  it("threads workspace scope into implicit provider discovery", async () => {
    ...
    expect(observedWorkspaceDir).toBe("/tmp/oriro-workspace");
  });
  ...
  it("threads plugin metadata snapshots through models.json planning", async () => {
    ...
    await planOriroModelsJsonWithDeps(
      {
        cfg: { models: { providers: {} } },
        agentDir: "/tmp/oriro-models-config-env-vars-test",
        env: {},
  ...
  it("keeps google-vertex static catalog rows when an auth profile supplies the API key", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-google-vertex-models-"));
    try {
  ...
  it("keeps google-vertex static catalog rows when ADC auth evidence supplies the marker", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-google-vertex-adc-models-"));
    ...
    try {
      const plan = await withEnvAsync(
        {
          ORIRO_BUNDLED_PLUGINS_DIR: BUNDLED_PLUGINS_DIR,
