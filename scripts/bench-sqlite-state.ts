// SQLite state benchmark seeds Oriro DBs and reports hot-query proof lines.
...
} from "../src/state/oriro-agent-db.js";
import {
...
function printUsage(): void {
  console.log(`Oriro SQLite state benchmark
...
function main(): void {
  ...
  const stateDir =
    options.stateDir ?? fs.mkdtempSync(path.join(os.tmpdir(), "oriro-sqlite-perf-"));
  const env = { ORIRO_STATE_DIR: stateDir };
  const started = nowMs();
  try {
    const stateDatabase = openOriroStateDatabase({ env });
    const agentDatabases = Array.from({ length: config.agentCount }, (_, index) =>
      openOriroAgentDatabase({ agentId: `perf-agent-${index}`, env }),
    );
