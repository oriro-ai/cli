...
describe("normalizeProviders", () => {
  const createModel = (
    overrides: Partial<
      NonNullable<NonNullable<OriroConfig["models"]>["providers"]>[string]["models"][number]
    > = {},
  ) => ({
  ...
  it("trims provider keys so image models remain discoverable for custom providers", async () => {
    const agentDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-agent-"));
    try {
      const providers: NonNullable<NonNullable<OriroConfig["models"]>["providers"]> = {
        " dashscope-vision ": {
  ...
  it("normalizes retired Google Gemini model ids before emitting provider config", async () => {
    ...
    try {
      const providers: NonNullable<NonNullable<OriroConfig["models"]>["providers"]> = {
        ...
        "google-gemini-cli": {
          baseUrl: "oriro://google-gemini-cli",
          models: [
            createModel({
              id: "gemini-3-pro-preview",
  ...
  it("replaces resolved env var value with env var name to prevent plaintext persistence", async () => {
    ...
    const env = {
      ...
      ORIRO_TEST_MINIMAL_GATEWAY: undefined,
    };
  ...
  it("ignores non-object provider entries during source-managed enforcement", () => {
    ...
    } as unknown as NonNullable<NonNullable<OriroConfig["models"]>["providers"]>;

    const sourceProviders: NonNullable<NonNullable<OriroConfig["models"]>["providers"]> = {
      openai: {
        baseUrl: "https://api.openai.com/v1",
        api: "openai-completions",
