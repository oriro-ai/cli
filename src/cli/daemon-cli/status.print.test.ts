...
describe("printDaemonStatus", () => {
  ...
  it("prints stale gateway pid guidance when runtime does not own the listener", () => {
    ...
    expectMockLineContains(runtime.error, formatCliCommand("oriro gateway restart"));
  });
  ...
  it("prints established gateway client guidance gathered by deep status", () => {
          ...
          established: [
            {
              ...
              ppid: 1,
              command: "node",
              commandLine: "/tmp/newer-oriro/bin/oriro logs --follow",
    ...
    expectMockLineContains(runtime.log, "newer-oriro");
  ...
  it("prints stale updater launchd job guidance", () => {
    ...
    expectMockLineContains(runtime.error, "Stale Oriro updater launchd job(s) detected.");
    expectMockLineContains(runtime.error, "ai.oriro.update.2026.5.12");
    expectMockLineContains(runtime.error, "ai.oriro.manual-update.1717168800");
  ...
  it("prints macOS launchd stdout and suppressed stderr when gateway is not listening", () => {
    ...
    expectMockLineContains(runtime.error, "/Users/test/Library/Logs/oriro/gateway.log");
  ...
  it("prints CLI and gateway versions with readable guidance when they differ", () => {
    printDaemonStatus(
      {
        cli: {
          version: "2026.4.23",
          entrypoint: "/usr/local/bin/oriro",
        },
        service: {
          label: "LaunchAgent",
          loaded: true,
    ...
    expectMockLineContains(runtime.log, "CLI version: 2026.4.23 (/usr/local/bin/oriro)");
    expectMockLineContains(runtime.log, "Gateway version: 2026.5.6");
    expectMockLineContains(runtime.error, "this Oriro command is version 2026.4.23");
    expectMockLineContains(
      runtime.error,
      "if this mismatch is unexpected, update PATH so `oriro` points to the version you want",
    );
  });
    ...
    printDaemonStatus(
      {
        ...
        config: {
          cli: {
            path: "/tmp/oriro-cli/oriro.json",
            exists: true,
            valid: true,
          },
          daemon: {
            path: "/tmp/oriro-daemon/oriro.json",
            exists: true,
            valid: true,
    ...
    printDaemonStatus(
      {
        ...
        extraServices: [{ label: "ai.oriro.gateway.rescue", scope: "user", detail: "loaded" }],
      },
      { json: false },
    );

    expectMockLineContains(runtime.log, "Other gateway-like services detected");
    expectMockLineContains(runtime.log, "ai.oriro.gateway.rescue");
  ...
  it("prints a terse plugin drift warning outside deep mode", () => {
    ...
    expectMockLineContains(runtime.log, "oriro gateway status --deep");
  ...
  it("prints detailed plugin drift entries in deep mode", () => {
    ...
    expectMockLineContains(runtime.log, "- whatsapp: 2026.5.3 (orirohub)");
    expectMockLineContains(runtime.log, "oriro plugins update <plugin-id>");
    expectMockLineContains(runtime.log, "oriro gateway restart");
  });
