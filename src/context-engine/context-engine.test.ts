...
function configWithSlot(engineId: string): OriroConfig {
...
function requireRegistryState() {
  const registryState = (globalThis as Record<symbol, unknown>)[
    Symbol.for("oriro.contextEngineRegistryState")
...
describe("Registry tests", () => {
  ...
  it("exposes the trusted plugin owner for a resolved registered engine", async () => {
    ...
    registerContextEngineForOwner(engineId, () => new MockContextEngine(), "plugin:lossless-oriro", {
      allowSameOwnerRefresh: true,
    });

    const engine = await resolveContextEngine(configWithSlot(engineId));

    expect(resolveContextEngineOwnerPluginId(engine)).toBe("lossless-oriro");
  });
...
// lossless-oriro registered successfully but resolution could not find it.
// ═══════════════════════════════════════════════════════════════════════════
