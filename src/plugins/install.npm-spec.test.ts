...
const resolveOriroPackageRootSyncMock = vi.fn();
...
vi.mock("../infra/oriro-root.js", () => ({
  resolveOriroPackageRootSync: (...args: unknown[]) =>
    resolveOriroPackageRootSyncMock(...args),
}));
...
const suiteTempRootTracker = createSuiteTempRootTracker("oriro-plugin-install-npm-spec");
...
function writeInstalledNpmPlugin(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      oriro: params.oriro ?? { extensions: ["./dist/index.js"] },
      ...(params.dependency
...
function writeNpmRootPackageLock(params: {
  ...
  for (const pkg of params.packages) {
    ...
    if (pkg.materializesRootOriro) {
      lockPackages["node_modules/oriro"] = {
        peer: true,
        version: "2026.5.3",
      };
    }
...
function prunePluginLocalOriroPeerLinks(npmRoot: string) {
  ...
  for (const entry of fs.readdirSync(nodeModulesDir, { withFileTypes: true })) {
    ...
    for (const packageDir of packageDirs) {
      ...
      fs.rmSync(path.join(packageNodeModulesDir, "oriro"), {
        recursive: true,
        force: true,
      });
    }
  ...
  runCommandWithTimeoutMock.mockImplementation(
    async (argv: string[], options?: { cwd?: string }) => {
      ...
      if (viewPackage) {
        return successfulSpawn(
          JSON.stringify({
            ...
            ...(viewPackage.oriro ? { oriro: viewPackage.oriro } : {}),
          }),
        );
      }
      ...
      if (isManagedNpmInstallCommand(argv)) {
        ...
        prunePluginLocalOriroPeerLinks(npmRoot);
        for (const packageName of Object.keys(manifest.dependencies ?? {})) {
          if (packageName === "oriro") {
            const oriroRoot = path.join(npmRoot, "node_modules", "oriro");
            fs.mkdirSync(oriroRoot, { recursive: true });
            fs.writeFileSync(
              path.join(oriroRoot, "package.json"),
              JSON.stringify({ name: "oriro", version: "0.0.0-test" }),
              "utf8",
            );
            continue;
          }
          ...
          if (pkg.materializesRootOriro) {
            ...
            fs.writeFileSync(
              path.join(oriroRoot, "package.json"),
              JSON.stringify({ name: "oriro", version: "2026.5.3" }),
              "utf8",
            );
          }
      ...
      if (argv[0] === "npm" && argv[1] === "uninstall") {
        ...
        if (packageName === "oriro") {
          ...
          fs.rmSync(path.join(npmRoot, "node_modules", "oriro"), {
            recursive: true,
            force: true,
          });
          return successfulSpawn();
...
beforeEach(() => {
  ...
  resolveOriroPackageRootSyncMock.mockReset();
  const hostRoot = suiteTempRootTracker.makeTempDir();
  fs.writeFileSync(
    path.join(hostRoot, "package.json"),
    `${JSON.stringify({ name: "oriro", version: "0.0.0-test" }, null, 2)}\n`,
    "utf8",
  );
  resolveOriroPackageRootSyncMock.mockReturnValue(hostRoot);
  vi.unstubAllEnvs();
  process.env.NPM_CONFIG_GLOBALCONFIG = npmGlobalConfigPath;
});
...
beforeAll(async () => {
  ...
  const archivePath = path.join(stateDir, "oriro-pack-demo-1.2.3.tgz");
  fs.writeFileSync(archivePath, "fixture pack contents", "utf8");

  mockNpmViewAndInstallMany([
    {
      packageName: "@oriro/pack-demo",
      version: "1.2.3",
      pluginId: "pack-demo",
      npmRoot,
    ...
    {
      spec: "@oriro/voice-call@0.0.1",
      packageName: "@oriro/voice-call",
      version: "0.0.1",
      pluginId: "voice-call",
      npmRoot,
    },
  ...
  const dependencySpec = managedManifest.dependencies?.["@oriro/pack-demo"];
...
describe("installPluginFromNpmSpec", () => {
  it("installs npm pack archives through the managed npm root", async () => {
    ...
    expect(result.targetDir).toBe(resolveTestPluginPackageDir(npmRoot, "@oriro/pack-demo"));
    expect(result.npmResolution?.resolvedSpec).toBe("@oriro/pack-demo@1.2.3");
    expect(result.npmResolution?.integrity).toBe("sha512-pack-demo");
    expect(result.npmTarballName).toBe("oriro-pack-demo-1.2.3.tgz");
    expectNpmInstallIntoProject({
      calls,
      npmRoot,
      packageName: "@oriro/pack-demo",
    });
    expect(dependencySpec).toMatch(/^file:\.\/_oriro-pack-archives\/.+\.tgz$/);
  ...
  it("rejects npm pack archive metadata with traversal package names", async () => {
    ...
    expect(fs.existsSync(path.join(npmRoot, "_oriro-pack-archives"))).toBe(false);
  ...
  it("updates staged npm pack archives when dangerous-looking code is present", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const packageName = "@oriro/pack-demo";
    const archiveV1Path = path.join(stateDir, "oriro-pack-demo-1.0.0.tgz");
    const archiveV2Path = path.join(stateDir, "oriro-pack-demo-2.0.0.tgz");
  ...
  it("installs staged npm pack archives with dangerous-looking code", async () => {
    ...
    const npmRoot = path.join(stateDir, "npm");
    const packageName = "@oriro/pack-demo";
    const archivePath = path.join(stateDir, "oriro-pack-demo-2.0.0.tgz");
  ...
  it("installs npm plugins into .oriro/npm", async () => {
    ...
    expect(result.targetDir).toBe(resolveTestPluginPackageDir(npmRoot, "@oriro/voice-call"));
    expect(result.npmResolution?.resolvedSpec).toBe("@oriro/voice-call@0.0.1");
  ...
  it("repairs omitted current-platform packages with a fresh npm cache", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const packageName = "@oriro/codex-fixture";
    ...
    expect(repairCacheDir).toContain("oriro-npm-cache-");
  ...
  it("quarantines and rebuilds a corrupt managed npm project after npm from-argument failures", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const packageName = "@oriro/voice-call";
    const warnings: string[] = [];
    const npmProjectRoot = resolvePluginNpmProjectDir({ npmDir: npmRoot, packageName });
    ...
    const quarantineParent = path.join(npmProjectRoot, "_oriro-quarantined-npm-projects");
  ...
  it.runIf(process.platform !== "win32")(
    "does not let managed oriro peer links poison later npm installs",
    async () => {
      const stateDir = suiteTempRootTracker.makeTempDir();
      const npmRoot = path.join(stateDir, "npm");
      ...
      mockNpmViewAndInstallMany([
        {
          ...
          npmRoot,
          peerDependencies: { oriro: "^2026.0.0" },
        },
        {
      ...
      expect(
        fs.lstatSync(path.join(peerPluginDir, "node_modules", "oriro")).isSymbolicLink(),
      ).toBe(true);
      ...
      if (!second.ok) {
        expect(second.error).not.toContain("peer-plugin/node_modules/oriro");
      }
      expect(
        fs.lstatSync(path.join(peerPluginDir, "node_modules", "oriro")).isSymbolicLink(),
      ).toBe(true);
  ...
  it("rejects managed npm plugins when their oriro peer link cannot be repaired", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const warnings: string[] = [];

    resolveOriroPackageRootSyncMock.mockReturnValue(null);
    mockNpmViewAndInstall({
      spec: "@oriro/codex@2026.5.7",
      packageName: "@oriro/codex",
      version: "2026.5.7",
      pluginId: "@oriro/codex",
      npmRoot,
      peerDependencies: { oriro: ">=2026.5.7" },
    });
    ...
    expect(result.error).toContain("@oriro/codex");
    expect(result.error).toContain("plugin-local node_modules/oriro link");
    expect(
      warnings.some((warning) => warning.includes("Could not locate oriro package root")),
    ).toBe(true);
    expect(fs.existsSync(resolveTestPluginPackageDir(npmRoot, "@oriro/codex"))).toBe(false);
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
      packageName: "@oriro/codex",
    });
  ...
  it("rejects exact npm plugins whose package compatibility requires a newer host", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    vi.stubEnv("ORIRO_COMPATIBILITY_HOST_VERSION", "2026.5.10-beta.1");

    mockNpmViewAndInstall({
      spec: "@oriro/whatsapp@2026.5.27",
      packageName: "@oriro/whatsapp",
      version: "2026.5.27",
      pluginId: "whatsapp",
      npmRoot,
      peerDependencies: { oriro: ">=2026.5.27" },
      oriro: {
        extensions: ["./dist/index.js"],
    ...
    expect(fs.existsSync(path.join(npmRoot, "node_modules", "@oriro", "whatsapp"))).toBe(false);
    expect(fs.existsSync(path.join(npmRoot, "package.json"))).toBe(false);
    expect(
  ...
  it("installs the newest compatible npm version for unpinned plugins", async () => {
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@oriro/whatsapp@2026.5.26");
    expect(result.npmResolution?.version).toBe("2026.5.26");
    expect(warnings.join("\n")).toContain("using newest compatible @oriro/whatsapp@2026.5.26");
    expect(
      JSON.parse(
        fs.readFileSync(
          path.join(resolveTestPluginPackageDir(npmRoot, "@oriro/whatsapp"), "package.json"),
          "utf8",
        ),
      ).version,
    ).toBe("2026.5.26");
  ...
  it("preserves an existing npm plugin by resolving update metadata to a compatible version", async () => {
    ...
    expect(managedManifest.dependencies?.["@oriro/whatsapp"]).toBe("2026.5.26");
    expect(
      runCommandWithTimeoutMock.mock.calls.some(([argv]) => isManagedNpmInstallCommand(argv)),
    ).toBe(true);
  });
  ...
  it("resolves incompatible prerelease tags to a compatible prerelease version", async () => {
    ...
    mockNpmViewAndInstallMany([
      {
        spec: "@oriro/msteams@beta",
        packageName: "@oriro/msteams",
      ...
      {
        spec: "@oriro/msteams@2026.5.28-beta.3",
        packageName: "@oriro/msteams",
    ...
    expect(result.npmResolution?.resolvedSpec).toBe("@oriro/msteams@2026.5.28-beta.3");
    expect(result.npmResolution?.version).toBe("2026.5.28-beta.3");
    expect(warnings.join("\n")).toContain(
      "using newest compatible @oriro/msteams@2026.5.28-beta.3",
    );
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
      packageName: "@oriro/msteams",
    ...
    expect(managedManifest.dependencies?.["@oriro/msteams"]).toBe("2026.5.28-beta.3");
  });
  ...
  it.runIf(process.platform !== "win32")(
    "repairs root oriro materialized by npm peer handling",
    async () => {
      const stateDir = suiteTempRootTracker.makeTempDir();
      const npmRoot = path.join(stateDir, "npm");
      ...
      expect(fs.existsSync(path.join(npmProjectRoot, "node_modules", "oriro"))).toBe(false);
      const lockfile = JSON.parse(
        fs.readFileSync(path.join(npmProjectRoot, "package-lock.json"), "utf8"),
      ) as {
        packages?: Record<string, unknown>;
      };
      expect(lockfile.packages?.["node_modules/oriro"]).toBeUndefined();
      expect(
        fs.lstatSync(path.join(requiredPeerPluginDir, "node_modules", "oriro")).isSymbolicLink(),
      ).toBe(true);
    },
  );

  it("repairs stale managed oriro root packages before npm plugin installs", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
      packageName: "@oriro/discord",
    });
    fs.mkdirSync(path.join(npmProjectRoot, "node_modules", "oriro"), { recursive: true });
    fs.writeFileSync(
      path.join(npmProjectRoot, "package.json"),
      JSON.stringify(
        {
      ...
      `${JSON.stringify(
        {
          ...
          packages: {
            ...
            "node_modules/oriro": {
              version: "2026.5.4",
              resolved: "https://registry.npmjs.org/oriro/-/oriro-2026.5.4.tgz",
            },
          },
          dependencies: {
            oriro: {
    ...
    fs.writeFileSync(
      path.join(npmProjectRoot, "node_modules", "oriro", "package.json"),
      JSON.stringify({
        name: "oriro",
        version: "2026.5.4",
      }),
    ...
    mockNpmViewAndInstall({
      spec: "@oriro/discord@beta",
      packageName: "@oriro/discord",
      version: "2026.5.5-beta.1",
      pluginId: "discord",
      npmRoot,
      peerDependencies: { oriro: ">=2026.5.5-beta.1" },
    ...
    expect(manifest.dependencies).not.toHaveProperty("oriro");
    expect(manifest.dependencies?.["@oriro/discord"]).toBe("2026.5.5-beta.1");
    const lockfile = JSON.parse(
      fs.readFileSync(path.join(npmProjectRoot, "package-lock.json"), "utf8"),
    ) as {
      packages?: Record<string, unknown>;
    ...
    expect(lockfile.packages?.["node_modules/oriro"]).toBeUndefined();
    expect(lockfile.dependencies?.oriro).toBeUndefined();
  });

  it("preserves the active host oriro runtime package during npm plugin installs", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const hostPackageRoot = path.join(npmRoot, "node_modules", "oriro");
    ...
    resolveOriroPackageRootSyncMock.mockReturnValue(hostPackageRoot);
    mockNpmViewAndInstall({
      spec: "@xdarkicex/oriro-memory-libravdb@1.4.69",
      packageName: "@xdarkicex/oriro-memory-libravdb",
      version: "1.4.69",
    ...
    expect(baseManifest.dependencies?.oriro).toBe("2026.5.12-beta.6");
    expect(baseManifest.dependencies?.["@xdarkicex/oriro-memory-libravdb"]).toBeUndefined();
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
      packageName: "@xdarkicex/oriro-memory-libravdb",
    });
    ...
    expect(projectManifest.dependencies?.["@xdarkicex/oriro-memory-libravdb"]).toBe("1.4.69");
    expect(fs.existsSync(hostPackageRoot)).toBe(true);
    expect(result.targetDir).toBe(
      resolveTestPluginPackageDir(npmRoot, "@xdarkicex/oriro-memory-libravdb"),
    );
    expect(
  ...
  it("rolls back the managed npm root when npm install fails", async () => {
    ...
    runCommandWithTimeoutMock.mockImplementation(
      async (argv: string[], options?: { cwd?: string }) => {
        if (JSON.stringify(argv) === JSON.stringify(npmViewArgv("@oriro/voice-call@0.0.1"))) {
          return successfulSpawn(
            JSON.stringify({
        ...
        if (argv[0] === "npm" && argv[1] === "uninstall") {
          if (!(argv as string[]).includes("--legacy-peer-deps")) {
            fs.mkdirSync(path.join(options?.cwd ?? npmRoot, "node_modules", "oriro"), {
              recursive: true,
            });
          }
          return successfulSpawn("");
    ...
    await expect(
      fs.promises.access(path.join(npmProjectRoot, "node_modules", "oriro")),
    ).rejects.toHaveProperty("code", "ENOENT");
  });

  it("does not fail rollback snapshots on plugin-local oriro peer symlinks", async () => {
    const npmRoot = path.join(suiteTempRootTracker.makeTempDir(), "npm");
    const npmProjectRoot = resolvePluginNpmProjectDir({
      npmDir: npmRoot,
      packageName: "@oriro/codex",
    ...
    writeNpmRootPackageLock({
      npmRoot: npmProjectRoot,
      dependencies: { "@oriro/codex": "0.0.1" },
      packages: [
        {
          packageName: "@oriro/codex",
          version: "0.0.1",
    ...
    const installedDir = writeInstalledNpmPlugin({
      ...
      packageName: "@oriro/codex",
      version: "0.0.1",
      peerDependencies: { oriro: "*" },
    });
    const peerLink = path.join(installedDir, "node_modules", "oriro");
    ...
    runCommandWithTimeoutMock.mockImplementation(
      async (argv: string[], options?: { cwd?: string }) => {
        if (JSON.stringify(argv) === JSON.stringify(npmViewArgv("@oriro/codex@0.0.2"))) {
          return successfulSpawn(
            JSON.stringify({
              name: "@oriro/codex",
              version: "0.0.2",
    ...
    runCommandWithTimeoutMock.mockImplementation(
      async (argv: string[], options?: { cwd?: string }) => {
        if (isManagedNpmInstallCommand(argv)) {
          ...
          const manifest = JSON.parse(
            fs.readFileSync(path.join(npmProjectRoot, "package.json"), "utf8"),
          ) as { overrides?: Record<string, unknown>; oriro?: { managedOverrides?: string[] } };
          if (installAttempts === 1) {
            expect(manifest.overrides?.["node-domexception"]).toBe(
              "npm:@nolyfill/domexception@1.0.28",
            );
            expect(manifest.oriro?.managedOverrides).toEqual([
              "axios",
              "nested",
              "node-domexception",
            ]);
          ...
          expect(manifest.oriro?.managedOverrides).toEqual(["axios", "nested"]);
        }
    ...
    async ({ spec, pluginId, indexJs }) => {
      ...
      expect(
        warnings.some((warning) =>
          warning.includes("allowed because it is an official Oriro package"),
        ),
      ).toBe(false);
    },
  );
  ...
  it("rejects duplicate npm installs unless update mode is requested", async () => {
    const stateDir = suiteTempRootTracker.makeTempDir();
    const npmRoot = path.join(stateDir, "npm");
    const installRoot = resolveTestPluginPackageDir(npmRoot, "@oriro/voice-call");
  ...
  it("preserves previously installed sibling plugins during npm install", async () => {
    ...
    expect(fs.existsSync(resolveTestPluginPackageDir(npmRoot, "@oriro/voice-call"))).toBe(true);
    expect(fs.existsSync(resolveTestPluginPackageDir(npmRoot, "@oriro/whatsapp"))).toBe(true);
  });
  ...
  it("rejects implicit prerelease npm specs with beta guidance", async () => {
    ...
    if (!rejected.ok) {
      ...
      expect(rejected.error).toContain('"@oriro/voice-call@beta"');
    }
  });
  ...
  it("falls back to the latest stable version for official prerelease packages", async () => {
    ...
    expect(officialFallback.npmResolution?.resolvedSpec).toBe("@oriro/voice-call@0.0.1");
    expect(warnings.join("\n")).toContain("falling back to stable @oriro/voice-call@0.0.1");
  });
  ...
  it("keeps stable correction versions when resolving official npm packages", async () => {
    ...
    expect(stableCorrection.npmResolution?.resolvedSpec).toBe("@oriro/voice-call@2026.5.3-1");
  ...
  it("uses the newest prerelease when an official package has no stable versions", async () => {
    ...
    expect(prereleaseOnly.npmResolution?.resolvedSpec).toBe("@oriro/voice-call@0.0.2-beta.1");
    expect(prereleaseOnlyWarnings.join("\n")).toContain("has no stable npm versions yet");
    expect(prereleaseOnlyWarnings.join("\n")).toContain(
      "using newest prerelease @oriro/voice-call@0.0.2-beta.1",
    );
  });
  ...
  it("accepts explicit prerelease npm dist-tags", async () => {
    ...
    expect(accepted.npmResolution?.resolvedSpec).toBe("@oriro/voice-call@0.0.2-beta.1");
    expectNpmInstallIntoProject({
      calls: runCommandWithTimeoutMock.mock.calls,
      npmRoot,
      packageName: "@oriro/voice-call",
