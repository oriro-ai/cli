...
@MainActor
public struct OriroChatView: View {
    public enum Style {
        case standard
        case onboarding
    }
    ...
    @State private var viewModel: OriroChatViewModel
    ...
    private let talkControl: OriroChatTalkControl?
    ...
    public init(
        ...
        talkControl: OriroChatTalkControl? = nil)
    {
    ...
    private var visibleMessages: [OriroChatMessage] {
        let base: [OriroChatMessage]
    ...
    private func mergeToolResults(in messages: [OriroChatMessage]) -> [OriroChatMessage] {
    ...
    private func isToolResultMessage(_ message: OriroChatMessage) -> Bool {
        let role = message.role.lowercased()
        return role == "toolresult" || role == "tool_result"
    }

    private func shouldDisplayMessage(_ message: OriroChatMessage) -> Bool {
    ...
    private func primaryText(in message: OriroChatMessage) -> String {
        let parts = message.content.compactMap { content -> String? in
            let kind = (content.type ?? "text").lowercased()
            guard kind == "text" || kind.isEmpty else { return nil }
            return content.text
        }
        return OriroChatMessage.displayText(
    ...
    private func hasInlineAttachments(in message: OriroChatMessage) -> Bool {
    ...
    private func toolCalls(in message: OriroChatMessage) -> [OriroChatMessageContent] {
    ...
    private func inlineToolResults(in message: OriroChatMessage) -> [OriroChatMessageContent] {
        message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private func toolCallIds(in message: OriroChatMessage) -> Set<String> {
        var ids = Set<String>()
    ...
    private func toolResultText(from message: OriroChatMessage) -> String {
...
private struct ChatAssistantIntroCard: View {
    ...
    var body: some View {
        Text(self.text)
            .font(.system(size: 15))
            .lineSpacing(4)
            .foregroundStyle(OriroChatTheme.assistantText)
            .multilineTextAlignment(.leading)
            .padding(.vertical, 12)
            .padding(.horizontal, 14)
            .background(
                RoundedRectangle(cornerRadius: 16, style: .continuous)
                    .fill(OriroChatTheme.assistantBubble)
                    .overlay(
                        RoundedRectangle(cornerRadius: 16, style: .continuous)
