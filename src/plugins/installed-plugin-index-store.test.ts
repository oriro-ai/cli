...
function makeTempDir() {
  return makeTrackedTempDir("oriro-installed-plugin-index-store", tempDirs);
}
...
function createIndex(overrides: Partial<InstalledPluginIndex> = {}): InstalledPluginIndex {
  return {
    ...
    plugins: [
      {
        pluginId: "demo",
        manifestPath: "/plugins/demo/oriro.plugin.json",
...
describe("installed plugin index persistence", () => {
  ...
  it("preserves startup config paths across persisted index roundtrips", async () => {
    ...
    const index = createIndex({
      plugins: [
        {
          pluginId: "browser",
          manifestPath: "/plugins/browser/oriro.plugin.json",
      ...
      plugins: [
        {
          ...
          manifestPath: "/plugins/provider-owner/oriro.plugin.json",
  ...
  it("preserves OriroHub OriroPack source facts when refreshing the manifest cache", async () => {
    const stateDir = makeTempDir();
    const installPath = path.join(stateDir, "plugins", "oriropack-demo");
    await writePersistedInstalledPluginIndex(
      createIndex({
        installRecords: {
          "oriropack-demo": {
    ...
    expectInstallRecord(index, "oriropack-demo", expectedRecord);
    expectPluginIds(index, []);
    await expectPersistedIndex(stateDir, {
      pluginIds: [],
      installRecords: {
        "oriropack-demo": expectedRecord,
      },
    });
  });
});
