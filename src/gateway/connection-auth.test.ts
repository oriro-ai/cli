...
type ConnectionAuthCase = {
  name: string;
  cfgLocal: OriroConfig;
  env: NodeJS.ProcessEnv;
  options?: Partial<Omit<GatewayConnectionAuthOptions, "config" | "env">>;
  expected: ResolvedAuth;
};

function cfg(input: Partial<OriroConfig>): OriroConfig {
  return input as OriroConfig;
}
...
const DEFAULT_ENV = {
  ...
  ORIRO_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
...
describe("resolveGatewayConnectionAuth", () => {
  ...
  it("resolves local SecretRef token when ORIRO env is absent", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first token SecretRef even when ORIRO env token exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
  ...
  it("resolves config-first password SecretRef even when ORIRO env password exists", async () => {
    const config = cfg({
      gateway: {
        mode: "local",
        auth: {
