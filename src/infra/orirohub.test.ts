// Covers OriroHub metadata and artifact fetch helpers.
...
describe("orirohub helpers", () => {
  const originalEnv = captureEnv(["HOME", "XDG_CONFIG_HOME"]);

  afterEach(() => {
    delete process.env.ORIRO_ORIROHUB_URL;
    delete process.env.ORIRO_ORIROHUB_TOKEN;
    delete process.env.ORIROHUB_TOKEN;
    delete process.env.ORIROHUB_AUTH_TOKEN;
    delete process.env.ORIRO_ORIROHUB_CONFIG_PATH;
    delete process.env.ORIROHUB_CONFIG_PATH;
    delete process.env.ORIRODHUB_CONFIG_PATH;
    originalEnv.restore();
  });

  it("parses explicit OriroHub package specs", () => {
    expect(parseOriroHubPluginSpec("orirohub:demo")).toEqual({
      name: "demo",
    });
    expect(parseOriroHubPluginSpec("orirohub:demo@1.2.3")).toEqual({
      name: "demo",
      version: "1.2.3",
    });
    expect(parseOriroHubPluginSpec("orirohub:@scope/pkg")).toEqual({
      name: "@scope/pkg",
    });
    expect(parseOriroHubPluginSpec("orirohub:@scope/pkg@1.2.3")).toEqual({
      name: "@scope/pkg",
      version: "1.2.3",
    });
    expect(parseOriroHubPluginSpec("orirohub:demo@")).toBeNull();
    expect(parseOriroHubPluginSpec("orirohub:@scope/pkg@")).toBeNull();
    expect(parseOriroHubPluginSpec("@scope/pkg")).toBeNull();
  });
  ...
  it("treats Oriro release correction versions as stable plugin API hosts", () => {
  ...
  it("checks min gateway versions with loose host labels", () => {
    ...
    expect(satisfiesGatewayMinimum("Oriro 2026.3.22", "2026.3.0")).toBe(true);
    expect(satisfiesGatewayMinimum("2026.2.9", "2026.3.0")).toBe(false);
    expect(satisfiesGatewayMinimum("unknown", "2026.3.0")).toBe(false);
  });

  it("normalizes raw OriroHub SHA-256 hashes into integrity strings", () => {
    ...
    expect(normalizeOriroHubSha256Integrity(hex)).toBe(integrity);
    expect(normalizeOriroHubSha256Integrity(`sha256:${hex}`)).toBe(integrity);
    expect(normalizeOriroHubSha256Integrity(integrity)).toBe(integrity);
    expect(normalizeOriroHubSha256Integrity(unpaddedIntegrity)).toBe(integrity);
    expect(normalizeOriroHubSha256Integrity(`sha256=${hex}`)).toBeNull();
    expect(normalizeOriroHubSha256Integrity("sha256-a=")).toBeNull();
    expect(normalizeOriroHubSha256Integrity("not-a-hash")).toBeNull();
  });

  it("normalizes OriroHub SHA-256 hex values", () => {
    expect(normalizeOriroHubSha256Hex("AA".repeat(32))).toBe("aa".repeat(32));
    expect(normalizeOriroHubSha256Hex("not-a-hash")).toBeNull();
  });

  it("resolves OriroHub auth token from config.json", async () => {
    await withTempDir({ prefix: "oriro-orirohub-config-" }, async (configRoot) => {
      const configPath = path.join(configRoot, "orirohub", "config.json");
      process.env.ORIRO_ORIROHUB_CONFIG_PATH = configPath;
      await fs.mkdir(path.dirname(configPath), { recursive: true });
      await fs.writeFile(configPath, JSON.stringify({ auth: { token: "cfg-token-123" } }), "utf8");

      await expect(resolveOriroHubAuthToken()).resolves.toBe("cfg-token-123");
    });
  });

  it("resolves OriroHub auth token from the legacy config path override", async () => {
    await withTempDir({ prefix: "oriro-orirodhub-config-" }, async (configRoot) => {
      const configPath = path.join(configRoot, "config.json");
      process.env.ORIRODHUB_CONFIG_PATH = configPath;
      await fs.writeFile(configPath, JSON.stringify({ token: "legacy-token-123" }), "utf8");

      await expect(resolveOriroHubAuthToken()).resolves.toBe("legacy-token-123");
    });
  });

  it.runIf(process.platform === "darwin")(
    "resolves OriroHub auth token from the macOS Application Support path",
    async () => {
      await withTempDir({ prefix: "oriro-orirohub-home-" }, async (fakeHome) => {
        ...
        try {
          ...
          await expect(resolveOriroHubAuthToken()).resolves.toBe("macos-token-123");
        } finally {
  ...
  it.runIf(process.platform === "darwin")(
    ...
    async () => {
      await withTempDir({ prefix: "oriro-orirohub-home-" }, async (fakeHome) => {
        await withTempDir({ prefix: "oriro-orirohub-xdg-" }, async (xdgRoot) => {
          const configPath = path.join(xdgRoot, "orirohub", "config.json");
          const homedirSpy = vi.spyOn(os, "homedir").mockReturnValue(fakeHome);
          ...
          try {
            ...
            await expect(resolveOriroHubAuthToken()).resolves.toBe("xdg-token-123");
          } finally {
            homedirSpy.mockRestore();
          }
        });
  ...
  it("injects resolved auth token into OriroHub requests", async () => {
    process.env.ORIRO_ORIROHUB_TOKEN = "env-token-123";
    ...
    await expect(searchOriroHubSkills({ query: "calendar", fetchImpl })).resolves.toStrictEqual([]);
  });

  it("preserves the configured OriroHub base URL path prefix", async () => {
    process.env.ORIRO_ORIROHUB_URL = "https://internal.example.com/orirohub";
    let requestedUrl = "";
    ...
    expect(url.pathname).toBe("/orirohub/api/v1/search");
        ...
        fetchImpl: async (input) => {
          ...
          return new Response(
            JSON.stringify({
              ...
              archive: {
                version: "1.0.0",
                downloadUrl: "https://orirohub.ai/api/v1/download?slug=weather&version=1.0.0",
              },
            }),
            { headers: { "content-type": "application/json" } },
          );
  ...
  it("fetches skill verification reports and lets version take precedence over tag", async () => {
    ...
    const envelope = {
      ...
      card: {
        available: true,
        url: "https://orirohub.ai/api/v1/skills/agentreceipt/card?version=1.2.3",
      },
      artifact: {
  ...
  it("sends owner-qualified skill verification lookups as slug plus ownerHandle", async () => {
    ...
    ).resolves.toMatchObject({ schema: "orirohub.skill.verify.v1" });
  ...
  it("clamps oversized OriroHub request timeouts before scheduling", async () => {
  ...
  it("wraps non-200 skill card responses", async () => {
    ...
    ).rejects.toThrow("OriroHub /api/v1/skills/agentreceipt/card failed (404): card missing");
  });
  ...
  it("rejects oversized generated Skill Card markdown", async () => {
    await expect(
      ...
      "OriroHub skill card for agentreceipt exceeded 262144 bytes (262145 bytes received)",
    );
  });
  ...
  it("wraps non-200 skill verification responses", async () => {
    ...
    ).rejects.toThrow("OriroHub /api/v1/skills/agentreceipt/verify failed (404): not found");
  });
  ...
  it("fetches typed package artifact resolver reports", async () => {
          ...
          return new Response(
            JSON.stringify({
              artifact: {
                ...
                version: "2026.3.22",
                downloadUrl: "https://orirohub.ai/api/v1/oriropacks/abc",
    ...
    expect(new URL(requestedUrl).pathname).toBe(
      "/api/v1/packages/%40oriro%2Fdiagnostics-otel/versions/2026.3.22/artifact",
    );
  });

  it("downloads package archives to sanitized temp paths and cleans them up", async () => {
    const archive = await downloadOriroHubPackageArchive({
  ...
  it("downloads OriroPack package artifacts from the version route and verifies response headers", async () => {
      ...
      fetchImpl: async (input) => {
        ...
        return new Response(bytes, {
          ...
          headers: {
            ...
            "X-OriroHub-Artifact-Sha256": sha256Hex,
          },
        });
      },
    });
    ...
    try {
      ...
      expect(archive.artifact).toBe("oriropack");
      expect(archive.sha256Hex).toBe(sha256Hex);
      expect(archive.oriropackHeaderSha256).toBe(sha256Hex);
  ...
  it("rejects OriroPack package artifacts when the declared digest does not match the bytes", async () => {
    await expect(
      downloadOriroHubPackageArchive({
        name: "demo",
        version: "1.2.3",
        ...
        fetchImpl: async () =>
          new Response(new Uint8Array([7, 8, 9]), {
            ...
            headers: {
              "content-type": "application/octet-stream",
              "X-OriroHub-Artifact-Sha256": "0".repeat(64),
            },
          }),
      }),
  ...
  it("annotates 429 errors with the reset hint and a sign-in hint when unauthenticated", async () => {
    process.env.ORIRO_ORIROHUB_CONFIG_PATH = path.join(os.tmpdir(), "oriro-no-orirohub-config");
    await expect(
      searchOriroHubSkills({
        query: "calendar",
        fetchImpl: async () =>
  ...
  it("wraps malformed successful OriroHub JSON responses", async () => {
    await expect(
      searchOriroHubSkills({
        query: "calendar",
        fetchImpl: async () =>
    ...
    ).rejects.toThrow("OriroHub /api/v1/search returned malformed JSON");
  });
  ...
  it("times out and cancels stalled OriroPack artifact body reads", async () => {
    const stalled = createStalledBodyResponse({
    ...
    ).rejects.toThrow(/OriroPack download for demo@1.2.3 body stalled after 5ms/i);
    expect(stalled.cancel).toHaveBeenCalledTimes(1);
    expect(stalled.cancel.mock.calls[0]?.[0]).toBeInstanceOf(Error);
  });

  it("downloads skill archives to sanitized temp paths and cleans them up", async () => {
    const archive = await downloadOriroHubSkillArchive({
      slug: "agentreceipt",
      version: "1.0.0",
      fetchImpl: async () =>
  ...
  it("does not send ambient OriroHub auth tokens to off-registry resolver archive URLs", async () => {
    process.env.ORIRO_ORIROHUB_TOKEN = "env-token-123";
    let requestedUrl = "";
    let requestedInit: RequestInit | undefined;

    const archive = await downloadOriroHubSkillArchiveUrl({
      baseUrl: "https://orirohub.ai",
