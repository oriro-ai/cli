...
import { readPluginPackageVersion } from "oriro/plugin-sdk/extension-shared";
...
const USER_AGENT = `oriro-parallel/${PLUGIN_VERSION} (${process.platform})`;
...
export async function executeParallelWebSearchProviderTool(
  ...
  // Generic `query` arg fallback: oriro's operator-facing CLI
  // (`oriro capability web.search ...`) always passes the shared
  ...
  // Always pass max_results so Parallel matches the oriro web_search default
