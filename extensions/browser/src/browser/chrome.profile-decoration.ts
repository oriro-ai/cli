/**
 * Oriro-managed Chrome profile decoration.
 *
...
import { loadJsonFile, saveJsonFile } from "oriro/plugin-sdk/json-store";
import {
...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".oriro-profile-decorated");
}
...
/**
 * Best-effort profile decoration (name + oriro-orange). Chrome preference keys
 * vary by version; we keep this conservative and idempotent.
 */
export function decorateOriroProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string; downloadDir?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_ORIRO_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_ORIRO_BROWSER_COLOR).toUpperCase();
