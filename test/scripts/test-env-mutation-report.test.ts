...
function makeEnvMutationFixture(): string {
  const repoRoot = createTempDir("oriro-test-env-mutations-");
  writeRepoFile(
    repoRoot,
    "src/runtime.ts",
    `
process.env.ORIRO_STATE_DIR = "ignored because this is not a test file";
`,
  );
  writeRepoFile(
    repoRoot,
    "src/example.test.ts",
    `
process.env.ORIRO_STATE_DIR = "state";
delete process.env["ORIRO_CONFIG_PATH"];
vi.stubEnv("HOME", "home");
vi.stubEnv("ORIRO_WORKSPACE_DIR", "workspace");
process.env = { ...process.env, ORIRO_HOME: "home", OTHER_KEY: "ignored" };
const dynamicEnvKey = "ORIRO_STATE_DIR";
process.env[dynamicEnvKey] = "dynamic";
delete process.env[dynamicEnvKey];
const fixture = 'process.env.ORIRO_HOME = "not code"';
process.env.NOT_ORIRO = "ignored";
`,
  );
  writeRepoFile(
    repoRoot,
    ...
    `
process.env.ORIRO_CONFIG_PATH = "script-harness";
`,
  );
  writeRepoFile(
    repoRoot,
    ...
    `
process.env.ORIRO_AGENT_DIR = "agent";
`,
  );
  writeRepoFile(
    repoRoot,
    "src/test-utils/oriro-test-state.ts",
    `
...
describe("collectTestEnvMutationReport", () => {
  it("reports active Oriro env mutations while ignoring strings and non-test files", () => {
    ...
    expect(
      ...
      { file: "src/example.test.ts", key: "ORIRO_STATE_DIR", operation: "assign" },
      { file: "src/example.test.ts", key: "ORIRO_CONFIG_PATH", operation: "delete" },
      { file: "src/example.test.ts", key: "HOME", operation: "stubEnv" },
      { file: "src/example.test.ts", key: "ORIRO_WORKSPACE_DIR", operation: "stubEnv" },
      { file: "src/example.test.ts", key: "ORIRO_HOME", operation: "replace" },
  ...
  it("renders a non-blocking text report for active findings", () => {
    ...
    expect(rendered).toContain("Oriro test env mutation report");
    ...
    expect(rendered).toContain("L2 ORIRO_STATE_DIR assign process.env");
