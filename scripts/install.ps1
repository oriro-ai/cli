# Oriro Installer for Windows
# Usage: powershell -c "irm https://oriro.ai/install.ps1 | iex"
#        powershell -c "& ([scriptblock]::Create((irm https://oriro.ai/install.ps1))) -Tag beta -NoOnboard -DryRun"
...
if (-not $PSBoundParameters.ContainsKey("InstallMethod")) {
    if (-not [string]::IsNullOrWhiteSpace($env:ORIRO_INSTALL_METHOD)) {
        $InstallMethod = $env:ORIRO_INSTALL_METHOD
    }
}
if (-not $PSBoundParameters.ContainsKey("GitDir")) {
    if (-not [string]::IsNullOrWhiteSpace($env:ORIRO_GIT_DIR)) {
        $GitDir = $env:ORIRO_GIT_DIR
    }
}
if (-not $PSBoundParameters.ContainsKey("NoOnboard")) {
    if ($env:ORIRO_NO_ONBOARD -eq "1") {
        $NoOnboard = $true
    }
}
if (-not $PSBoundParameters.ContainsKey("NoGitUpdate")) {
    if ($env:ORIRO_GIT_UPDATE -eq "0") {
        $NoGitUpdate = $true
    }
}
if (-not $PSBoundParameters.ContainsKey("DryRun")) {
    if ($env:ORIRO_DRY_RUN -eq "1") {
        $DryRun = $true
    }
}
...
if ([string]::IsNullOrWhiteSpace($GitDir)) {
    ...
    $GitDir = (Join-Path $userHome "oriro")
}
...
function Get-PortableNodeRoot {
    return (Join-Path (Get-OriroDepsRoot) "portable-node")
}
...
function Ensure-PortableNodeOnUserPath {
    ...
    if (Add-ToUserPath $nodeDir) {
        Write-Host "[!] Added $nodeDir to user PATH (restart terminal if node or oriro is not found)" -ForegroundColor Yellow
    }
}
...
function Get-PortableGitRoot {
    return (Join-Path (Get-OriroDepsRoot) "portable-git")
}
...
function Install-PortableGit {
    ...
    $portableParent = Split-Path -Parent $portableRoot
    $tmpZip = Join-Path $env:TEMP $download.Name
    $tmpExtract = Join-Path $env:TEMP ("oriro-portable-git-" + [guid]::NewGuid().ToString("N"))
...
function Get-OriroCommandPath {
    $oriroCmd = Get-Command oriro.cmd -ErrorAction SilentlyContinue
    if ($oriroCmd -and $oriroCmd.Source) {
        return $oriroCmd.Source
    }

    $oriro = Get-Command oriro -ErrorAction SilentlyContinue
    if ($oriro -and $oriro.Source) {
        return $oriro.Source
    }
...
function Invoke-OriroCommand {
    param(
        [Parameter(ValueFromRemainingArguments = $true)]
        [string[]]$Arguments
    )

    $commandPath = Get-OriroCommandPath
    if (-not $commandPath) {
        throw "oriro command not found on PATH."
    }

    & $commandPath @Arguments
}

function Invoke-InteractiveOriroCommand {
    param(
        [Parameter(ValueFromRemainingArguments = $true)]
        [string[]]$Arguments
    )
    ...
    if ($process.ExitCode -ne 0) {
        throw "oriro $($Arguments -join ' ') failed with exit code $($process.ExitCode)."
    }
}
...
function Ensure-OriroOnPath {
    ...
    foreach ($npmBin in $npmBins) {
        if (-not (Test-Path (Join-Path $npmBin "oriro.cmd"))) {
            continue
        }
    ...
    Write-Host "[!] oriro is not on PATH yet." -ForegroundColor Yellow
...
function Resolve-NpmOriroInstallSpec {
    param(
        [string]$PackageName,
        [string]$RequestedTag
    )
...
function Test-OriroSourcePackageInstallSpec {
    ...
    if ($normalizedTag.StartsWith("oriro@")) {
        $normalizedTag = $normalizedTag.Substring("oriro@".Length)
    }

    if ($normalizedTag -eq "main") {
        return $true
    }
    if ($normalizedTag -match '^github:oriro/oriro($|[#/])') {
        return $true
    }
    ...
    return (
        $normalizedTag -match '^https?://github\.com/oriro/oriro(\.git)?($|[?#])' -or
        $normalizedTag -match '^ssh://git@github\.com[:/]oriro/oriro(\.git)?($|[?#])' -or
        $normalizedTag -match '^git://github\.com/oriro/oriro(\.git)?($|[?#])' -or
        $normalizedTag -match '^git@github\.com:oriro/oriro(\.git)?($|[?#])'
    )
}
...
function Install-Oriro {
    ...
    # Use oriro package for beta, oriro for stable
    $packageName = "oriro"
    ...
    try {
        ...
        if ($LASTEXITCODE -ne 0) {
            ...
            if ($npmOutput -match "spawn git" -or $npmOutput -match "ENOENT.*git") {
                ...
                Write-Host '  powershell -c "irm https://oriro.ai/install.ps1 | iex"' -ForegroundColor Cyan
            }
...
# Install Oriro from GitHub
function Install-OriroFromGit {
    param(
        [string]$RepoDir,
        [switch]$SkipUpdate
    )
    ...
    $cmdPath = Join-Path $binDir "oriro.cmd"
...
function Run-Doctor {
    ...
    try {
        Invoke-OriroCommand doctor --non-interactive
    } catch {
...
function Test-GatewayServiceLoaded {
    try {
        $statusJson = (Invoke-OriroCommand daemon status --json 2>$null)
...
function Refresh-GatewayServiceIfLoaded {
    ...
    try {
        Invoke-OriroCommand gateway install --force | Out-Null
    } catch {
    ...
    try {
        Invoke-OriroCommand gateway restart | Out-Null
        Invoke-OriroCommand gateway status --json | Out-Null
...
function Get-LegacyRepoDir {
    if (-not [string]::IsNullOrWhiteSpace($env:ORIRO_GIT_DIR)) {
        return $env:ORIRO_GIT_DIR
    }
    $userHome = [Environment]::GetFolderPath("UserProfile")
    return (Join-Path $userHome "oriro")
}
...
function Main {
    ...
    if ($InstallMethod -eq "git") {
        try {
            ...
            if ($npmCommand) {
                Invoke-NpmCommand -Arguments @("uninstall", "-g", "oriro") 2>$null | Out-Null
                Write-Host "[OK] Removed npm global install if present" -ForegroundColor Green
            }
        } catch { }
        $finalGitDir = $GitDir
        $gitInstallResults = @(Install-OriroFromGit -RepoDir $GitDir -SkipUpdate:$NoGitUpdate)
        if (-not (Test-BooleanSuccessResult -Results $gitInstallResults)) {
            return (Fail-Install)
        }
    } else {
        $gitWrapper = Join-Path (Join-Path $env:USERPROFILE ".local\\bin") "oriro.cmd"
        ...
        $npmInstallResults = @(Install-Oriro)
    ...
    try {
        $installedVersion = (Invoke-OriroCommand --version 2>$null).Trim()
    } catch {
        $installedVersion = $null
    }
    if (-not $installedVersion) {
        try {
            ...
            if ($npmList -and $npmList.dependencies -and $npmList.dependencies.oriro -and $npmList.dependencies.oriro.version) {
                $installedVersion = $npmList.dependencies.oriro.version
            }
        } catch {
            $installedVersion = $null
        }
    ...
    if ($InstallMethod -eq "git") {
        ...
        Write-Host "Wrapper: $env:USERPROFILE\\.local\\bin\\oriro.cmd" -ForegroundColor Cyan
        Write-Host ""
    }

    if ($isUpgrade) {
        Write-Host "Upgrade complete. Run " -NoNewline
        Write-Host "oriro doctor" -ForegroundColor Cyan -NoNewline
        Write-Host " to check for additional migrations."
    } else {
        if ($NoOnboard) {
            Write-Host "Skipping onboard (requested). Run " -NoNewline
            Write-Host "oriro onboard" -ForegroundColor Cyan -NoNewline
            Write-Host " later."
        } else {
            Write-Host "Starting setup..." -ForegroundColor Cyan
            Write-Host ""
            Invoke-InteractiveOriroCommand onboard
        }
    }
