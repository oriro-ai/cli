...
describe("openai tts", () => {
  ...
  describe("openaiTTS diagnostics", () => {
    it("adds Oriro attribution headers to native OpenAI speech requests", async () => {
      ...
      expect(headers?.originator).toBe("oriro");
      expect(headers?.version).toBe("2026.3.22");
      expect(headers?.["User-Agent"]).toBe("oriro/2026.3.22");
    });
    ...
    it("records TTS exchanges in debug proxy capture mode", async () => {
      ...
      process.env.ORIRO_DEBUG_PROXY_SESSION_ID = "tts-session";
    ...
    it("does not double-capture TTS exchanges when the global fetch patch is installed", async () => {
      ...
      process.env.ORIRO_DEBUG_PROXY_ENABLED = "1";
      process.env.ORIRO_STATE_DIR = tempDir;
      process.env.ORIRO_DEBUG_PROXY_SESSION_ID = "tts-patched-session";
