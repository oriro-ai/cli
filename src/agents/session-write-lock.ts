...
const CLEANUP_STATE_KEY = Symbol.for("oriro.sessionWriteLockCleanupState");
const WATCHDOG_STATE_KEY = Symbol.for("oriro.sessionWriteLockWatchdogState");
...
const SESSION_LOCKS = createFileLockManager("oriro.session-write-lock");
...
const SESSION_WRITE_LOCK_ENV: Record<SessionWriteLockMsKey, string> = {
  acquireTimeoutMs: "ORIRO_SESSION_WRITE_LOCK_ACQUIRE_TIMEOUT_MS",
  staleMs: "ORIRO_SESSION_WRITE_LOCK_STALE_MS",
  maxHoldMs: "ORIRO_SESSION_WRITE_LOCK_MAX_HOLD_MS",
};
...
function shouldStartBackgroundWatchdog(): boolean {
  return process.env.VITEST !== "true" || process.env.ORIRO_TEST_SESSION_LOCK_WATCHDOG === "1";
}
...
function isOriroSessionOwnerArgv(args: string[]): boolean {
  const normalized = args.map(normalizeOwnerProcessArg).filter(Boolean);
  if (normalized.length === 0) {
    return false;
  }
  const exe = (normalized[0] ?? "").replace(/\.(bat|cmd|exe)$/i, "");
  if (exe === "oriro" || exe.endsWith("/oriro") || exe.endsWith("/oriro-gateway")) {
    return true;
  }
  if (
    normalized.some(
  ...
  const hasOriroCommandToken = normalized.some((arg) => arg === "gateway" || arg === "agent");
  return normalized.some(
    (arg) => entryCandidates.some((entry) => arg.endsWith(entry)) && hasOriroCommandToken,
  );
}
...
function shouldTreatAsNonOriroOwner(params: {
  payload: LockFilePayload | null;
  ...
  return !isOriroSessionOwnerArgv(args);
}
...
function inspectLockPayloadForSession(params: {
  ...
  if (
    ...
    return {
      ...inspected,
      stale: true,
      staleReasons: [...inspected.staleReasons, "non-oriro-owner"],
    };
  }
