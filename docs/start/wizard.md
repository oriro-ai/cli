...
CLI onboarding is the **recommended** terminal setup path for Oriro on
...
`ORIRO_LOCALE`, then `LC_ALL`, then `LC_MESSAGES`, then `LANG`, and falls
...
Names and stable identifiers stay literal: `Oriro`, `Gateway`, `Tailscale`,
commands, config keys, URLs, provider IDs, model IDs, and plugin/channel labels
are not translated.

<Info>
Fastest first chat: open the Control UI (no channel setup needed). Run
`oriro dashboard` and chat in the browser. Docs: [Dashboard](/web/dashboard).
</Info>
...
`oriro configure --section web`. Docs: [Web tools](/tools/web).
</Tip>
...
2. **Workspace** — Location for agent files (default `~/.oriro/workspace`). Seeds bootstrap files.
...
If the config is invalid or contains legacy keys, onboarding asks you to run `oriro doctor` first.
</Note>
...
Use `oriro agents add <name>` to create a separate agent with its own workspace,
...
- Default workspaces follow `~/.oriro/workspace-<agentId>`.
...
- CLI command reference: [`oriro onboard`](/cli/onboard)
