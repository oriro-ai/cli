---
summary: "Use Mistral models and Voxtral transcription with Oriro"
read_when:
  - You want to use Mistral models in Oriro
  - You want Voxtral realtime transcription for Voice Call
  - You need Mistral API key onboarding and model refs
title: "Mistral"
---

Oriro includes a bundled Mistral plugin that registers four contracts: chat completions, media understanding (Voxtral batch transcription), realtime STT for Voice Call (Voxtral Realtime), and memory embeddings (`mistral-embed`).
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    oriro onboard --auth-choice mistral-api-key
    ```

    Or pass the key directly:

    ```bash
    oriro onboard --mistral-api-key "$MISTRAL_API_KEY"
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    oriro models list --provider mistral
    ```
  </Step>
</Steps>
...
Oriro currently ships this bundled Mistral catalog:
...
```bash
oriro models list --all --provider mistral --plain
```
...
<Note>
Oriro defaults Mistral realtime STT to `pcm_mulaw` at 8 kHz so Voice Call
...
<AccordionGroup>
  <Accordion title="Adjustable reasoning">
    ...
    Oriro maps the session **thinking** level to Mistral's API:

    | Oriro thinking level                          | Mistral `reasoning_effort` |
    | ------------------------------------------------ | -------------------------- |
    | **off** / **minimal**                            | `none`                     |
    ...
    answers, turn thinking off/minimal so Oriro sends
