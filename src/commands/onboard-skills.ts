...
export async function setupSkills(
  ...
  let next: OriroConfig = cfg;
  if (installable.length === 0 && missing.length === 0) {
    await prompter.note(
      [
        "No missing skill dependencies to install.",
        `To inspect available skills, run: ${formatCliCommand("oriro skills list --verbose")}`,
        `To check skill status, run: ${formatCliCommand("oriro skills check")}`,
      ].join("\n"),
  ...
  if (installable.length > 0) {
    ...
    for (const name of selected) {
      ...
      // visible through `oriro skills list --verbose`.
      ...
      runtime.log(
        `Tip: run \`${formatCliCommand("oriro doctor")}\` to review skills + requirements.`,
      );
