// CommonJS fixture server for OriroHub package/install E2E scenarios.
...
const packageName = "@oriro/kitchen-sink";
...
async function buildNpmPackArtifact(fixture) {
  const packRoot = await fs.promises.mkdtemp(path.join(os.tmpdir(), "oriro-orirohub-fixture-"));
  try {
    ...
    return {
      archive,
      oriropackSha256: crypto.createHash("sha256").update(archive).digest("hex"),
      oriropackSize: archive.length,
    ...
    api.registerChannel({
      plugin: {
        ...
        meta: {
          ...
          blurb: "Kitchen sink OriroHub fixture channel",
        },
...
`,
    ...
    packageDetail(artifact) {
      const oriropack = buildOriroPackSummary(artifact);
      ...
      const packageDetail = {
        package: {
          ...
          verification: {
            tier: "source-linked",
            sourceRepo: "https://github.com/oriro/kitchen-sink",
            hasProvenance: false,
            scanStatus: "passed",
          },
...
if (!fixture || !portFile) {
  console.error("usage: orirohub-fixture-server.cjs <kitchen-sink-plugin|plugins> <port-file>");
  process.exit(1);
}
...
async function main() {
  ...
  zip.file("package/oriro.plugin.json", manifestJson, { date: new Date(0) });

  const archive = await zip.generateAsync({ type: "nodebuffer", compression: "DEFLATE" });
  const sha256hash = crypto.createHash("sha256").update(archive).digest("hex");
  const oriropack = await buildNpmPackArtifact(fixture);
  ...
  const artifactResolverDetail = {
    ...
    artifact: {
      ...
      artifactSha256: oriropack.oriropackSha256,
      npmIntegrity: oriropack.npmIntegrity,
      npmShasum: oriropack.npmShasum,
    },
  };
  ...
  const server = http.createServer((request, response) => {
    ...
    if (
      ...
      response.writeHead(200, {
        ...
        "content-length": String(oriropack.archive.length),
        "X-OriroHub-Artifact-Type": "npm-pack-tarball",
        "X-OriroHub-Artifact-Sha256": oriropack.oriropackSha256,
        "X-OriroHub-Npm-Integrity": oriropack.npmIntegrity,
        "X-OriroHub-Npm-Shasum": oriropack.npmShasum,
      });
      response.end(oriropack.archive);
      return;
    }
