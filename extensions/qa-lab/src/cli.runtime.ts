...
const QA_CREDENTIAL_PAYLOAD_MAX_BYTES_ENV = "ORIRO_QA_CREDENTIAL_PAYLOAD_MAX_BYTES";
...
function parseQaRuntimePair(value: string | undefined): [RuntimeId, RuntimeId] | undefined {
  ...
  if (runtimes.length !== 2) {
    throw new Error('--runtime-pair must use exactly two runtimes, e.g. "oriro,codex".');
  }
  const [left, right] = runtimes;
  if (!left || !right) {
    throw new Error('--runtime-pair only supports "oriro" and "codex".');
  }
  if (left === right) {
...
async function withTemporaryQaProfileEnv<T>(profile: string, run: () => Promise<T>): Promise<T> {
  ...
  process.env.ORIRO_QA_PROFILE = profile;
  try {
    return await run();
  } finally {
...
export async function runQaJsonlReplayCommand(opts: {
  ...
  const repoRoot = path.resolve(opts.repoRoot ?? process.cwd());
  const runtimePair = parseQaRuntimePair(opts.runtimePair) ?? ["oriro", "codex"];
  if (runtimePair[0] !== "oriro" || runtimePair[1] !== "codex") {
    throw new Error('--runtime-pair for jsonl-replay must be "oriro,codex".');
  }
...
export async function runQaLabUiCommand(opts: {
  ...
  const server = await startQaLabServer({
    ...
    controlUiProxyToken: process.env.ORIRO_QA_CONTROL_UI_PROXY_TOKEN,
