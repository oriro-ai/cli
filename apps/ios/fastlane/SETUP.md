...
APP_STORE_CONNECT_KEYCHAIN_SERVICE=oriro-app-store-connect-key
...
```bash
APP_STORE_CONNECT_APP_IDENTIFIER=ai.orirofoundation.app
# or
...
Tip: run `scripts/ios-team-id.sh --require-canonical` from repo root to verify the canonical Oriro iOS team (`FWJYW4S8P8`) is available locally. Fastlane uses the same canonical-only path when `IOS_DEVELOPMENT_TEAM` is missing, and rejects non-canonical teams for release archives.
...
For local/manual iOS builds that stay on direct APNs, configure the gateway host separately with `ORIRO_APNS_TEAM_ID`, `ORIRO_APNS_KEY_ID`, and either `ORIRO_APNS_PRIVATE_KEY_P8` or `ORIRO_APNS_PRIVATE_KEY_PATH`. Those gateway runtime env vars are separate from Fastlane's `.env`.
...
The screenshot lane runs the app with `--oriro-screenshot-mode`, which enters the built-in connected screenshot fixture instead of pairing with a live gateway. By default it captures the tab set on `iPhone 16 Pro Max` and `iPad Pro 13-inch (M4)`; override devices with a comma-separated `ORIRO_SNAPSHOT_DEVICES` value when the requested simulators exist locally.
...
- confirm `apps/ios/build/app-store/Oriro-<version>.ipa` exists
...
- The release flow regenerates `apps/ios/Oriro.xcodeproj` from `apps/ios/project.yml` before archiving
