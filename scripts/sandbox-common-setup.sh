...
BASE_IMAGE="${BASE_IMAGE:-oriro-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-oriro-sandbox-common:bookworm-slim}"
...
ORIRO_DOCKER_BUILD_USE_BUILDX="${ORIRO_DOCKER_BUILD_USE_BUILDX:-0}"
ORIRO_DOCKER_BUILD_CACHE_FROM="${ORIRO_DOCKER_BUILD_CACHE_FROM:-}"
ORIRO_DOCKER_BUILD_CACHE_TO="${ORIRO_DOCKER_BUILD_CACHE_TO:-}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=oriro.sandbox=1)
NOTE
