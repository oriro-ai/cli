...
type CreateOriroTools = typeof import("../oriro-tools.js").createOriroTools;
...
let createOriroTools: CreateOriroTools;
...
beforeAll(async () => {
  ...
  ({ createOriroTools } = await import("../oriro-tools.js"));
});
...
describe("message tool secret scoping", () => {
  ...
  it("forwards source reply delivery mode through createOriroTools", () => {
...
describe("message tool agent routing", () => {
  ...
  it("forwards agentThreadId through createOriroTools to the message tool", async () => {
    mockSendResult({ channel: "slack", to: "channel:C123" });
    const plugin = createChannelPlugin({
      id: "slack",
      label: "Slack",
  ...
  it("forwards the routable target through createOriroTools to the message tool", async () => {
    mockSendResult({ channel: "slack", to: "user:U123" });
    const plugin = createChannelPlugin({
      id: "slack",
      label: "Slack",
...
describe("message tool boot-echo guard", () => {
  ...
  it("suppresses text-only sends that echo a substantial chunk of the registered boot prompt without preserving the wrapper markers (#53732)", async () => {
    ...
    // sentence verbatim — exactly the leak vector orirosweeper called out
