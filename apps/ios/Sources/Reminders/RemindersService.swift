...
final class RemindersService: RemindersServicing {
    func list(params: OriroRemindersListParams) async throws -> OriroRemindersListPayload {
        ...
        let payload: [OriroReminderPayload] = try await withCheckedThrowingContinuation { cont in
        ...
        return OriroRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: OriroRemindersAddParams) async throws -> OriroRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = OriroReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return OriroRemindersAddPayload(reminder: payload)
    }
