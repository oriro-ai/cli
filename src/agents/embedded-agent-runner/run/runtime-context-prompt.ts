...
import {
  ...
  ORIRO_RUNTIME_CONTEXT_CUSTOM_TYPE,
  ORIRO_RUNTIME_CONTEXT_NOTICE,
  ORIRO_RUNTIME_EVENT_HEADER,
...
export function resolveRuntimeContextPromptParts(params: {
  ...
  if (!prompt.trim()) {
    return runtimeContext
      ? {
          prompt: ORIRO_RUNTIME_EVENT_USER_PROMPT,
          ...(modelPromptText.trim() && modelPromptText !== ORIRO_RUNTIME_EVENT_USER_PROMPT
            ? { modelPrompt: modelPromptText }
            : {}),
          runtimeContext,
          runtimeOnly: true,
