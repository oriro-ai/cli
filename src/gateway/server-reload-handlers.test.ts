...
const hoisted = vi.hoisted(() => ({
  ...
  runtimeConfig: { value: { session: { store: "/tmp/active-sessions.json" } } as OriroConfig },
  reloadEvents: [] as string[],
  loadModelCatalog: vi.fn(async (_params: { config: OriroConfig }) => []),
  resetModelCatalogCache: vi.fn(() => {}),
  refreshContextWindowCache: vi.fn(async (_cfg: OriroConfig) => {}),
  clearCurrentProviderAuthState: vi.fn(() => {}),
  warmCurrentProviderAuthStateOffMainThread: vi.fn(async (_cfg: OriroConfig) => {}),
...
vi.mock("../agents/model-catalog.js", () => ({
  loadModelCatalog: (params: { config: OriroConfig }) => {
...
vi.mock("../agents/context.js", () => ({
  refreshContextWindowCache: async (cfg: OriroConfig) => {
...
vi.mock("../agents/model-provider-auth.js", () => ({
  ...
  warmCurrentProviderAuthStateOffMainThread: async (cfg: OriroConfig) => {
...
describe("gateway hot reload model state", () => {
  it("resets prepared model runtime state for every hot reload and rewarms after plugin reload", async () => {
    ...
    const nextConfig = { plugins: { enabled: true } } as OriroConfig;
  ...
  it("disposes cached MCP runtimes on MCP config hot reloads", async () => {
    ...
    const nextConfig = { mcp: { servers: {} } } as OriroConfig;
  ...
  it.each([
    ...
    const { applyHotReload } = createReloadHandlersForTest();
    const previousConfig = testCase.previousConfig as OriroConfig;
    const nextConfig = testCase.nextConfig as OriroConfig;
...
describe("gateway Gmail hot reload handlers", () => {
  ...
  function createGmailConfig(account: string): OriroConfig {
    return {
  ...
  it("aborts an in-flight managed Gmail restart when the reloader stops", async () => {
    ...
    const reloader = startManagedGatewayConfigReloader({
      ...
      activateRuntimeSecrets: vi.fn(async (config: OriroConfig) => ({
        sourceConfig: config,
        config,
        authStores: [],
        warnings: [],
