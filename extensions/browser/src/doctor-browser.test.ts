...
describe("browser doctor readiness", () => {
  ...
  it("warns when managed browser profiles have no local executable", async () => {
    ...
    expect(noteFn).toHaveBeenCalledWith(
      [
        "- Oriro-managed browser profile(s) are configured: oriro.",
  ...
  it("warns about legacy orirod managed browser profile residue", async () => {
    const noteFn = vi.fn();
    const configDir = "/tmp/oriro-home";
    ...
    await noteChromeMcpBrowserReadiness(
      ...
      },
      {
        ...
        getUid: () => 1000,
        configDir,
        pathExists: (targetPath) => targetPath.endsWith("/browser/orirod/user-data"),
        resolveManagedExecutable: () => ({ kind: "chrome", path: "/usr/bin/google-chrome" }),
      },
    );
    ...
    expect(note).toContain("/tmp/oriro-home/browser/orirod");
    expect(note).toContain("/tmp/oriro-home/browser/oriro/user-data");
    expect(note).toContain("oriro doctor --fix");
  });

  it("does not warn when orirod is still configured as a browser profile", async () => {
    const noteFn = vi.fn();
...
describe("legacy orirod browser profile cleanup", () => {
  it("archives stale orirod residue with the safe trash mover", async () => {
    const movePathToTrash = vi.fn(async () => "/tmp/oriro-home/browser/.trash/orirod");
    ...
    expect(movePathToTrash).toHaveBeenCalledWith("/tmp/oriro-home/browser/orirod");
    expect(result.warnings).toStrictEqual([]);
    expect(result.changes.join("\n")).toContain(
      "Archived legacy orirod managed browser profile residue.",
    );
    expect(result.changes.join("\n")).toContain("/tmp/oriro-home/browser/oriro/user-data");
  });

  it("does not archive a configured orirod browser profile", async () => {
    const movePathToTrash = vi.fn(async () => "/tmp/unused");

    const result = await maybeArchiveLegacyOrirodBrowserProfileResidue(
      {
        browser: {
          defaultProfile: "orirod",
          profiles: {
            orirod: { color: "#FF4500" },
          },
        },
