...
*No output — tool failed.*`);
  ...
  it("extracts canvas handle payloads into canvas previews", () => {
    const [card] = extractToolCards(
      {
        ...
        content: JSON.stringify({
          ...
          view: {
            backend: "canvas",
            id: "cv_inline",
            url: "/__oriro__/canvas/documents/cv_inline/index.html",
          },
          presentation: {
    ...
    expect(card?.preview?.url).toBe("/__oriro__/canvas/documents/cv_inline/index.html");
    ...
    const [card] = extractToolCards(
      {
        ...
        __oriro: { id: "msg-tool-history-1", seq: 7 },
      },
      "msg:history",
    );
    ...
    const cases = [
      {
        ...
        content: JSON.stringify({
          ...
          view: {
            backend: "canvas",
            id: "cv_tool_card",
            url: "/__oriro__/canvas/documents/cv_tool_card/index.html",
          },
          presentation: {
            target: "tool_card",
