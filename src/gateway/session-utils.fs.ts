...
/** Attach Oriro metadata to a transcript message without dropping existing metadata. */
export function attachOriroTranscriptMeta(
  message: unknown,
  ...
  const existing =
    record["__oriro"] &&
    typeof record["__oriro"] === "object" &&
    !Array.isArray(record["__oriro"])
      ? (record["__oriro"] as Record<string, unknown>)
      : {};
  return {
    ...record,
    __oriro: {
...
function buildOversizedTranscriptRecord(line: string): TailTranscriptRecord {
  ...
  const record: Record<string, unknown> = {
    ...
    message: {
      ...
      __oriro: { truncated: true, reason: "oversized" },
    },
  };
  return { record };
}
...
export function readRecentSessionMessagesWithStats(
  ...
  const messagesWithSeq = messages.map((message, index) =>
    attachOriroTranscriptMeta(message, { seq: firstSeq + index }),
  );
    ...
    const recordTimestampMs =
      typeof entry.timestamp === "string"
        ...
        : typeof entry.timestamp === "number"
          ? entry.timestamp
          : Number.NaN;
    return attachOriroTranscriptMeta(entry.message, {
...
function extractTranscriptTokenEstimateFromLine(line: string): {
  ...
  try {
    ...
    const isDeliveryMirror =
      role === "assistant" && modelProvider === "oriro" && model === "delivery-mirror";
    if (isDeliveryMirror) {
      return null;
    }
    ...
    const model =
      typeof message.model === "string"
        ...
        : typeof parsed.model === "string"
          ? parsed.model.trim()
          : undefined;
    const isDeliveryMirror = modelProvider === "oriro" && model === "delivery-mirror";
