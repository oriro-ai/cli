...
function resolveDefaultLogDir(): string {
  return canUseNodeFs() ? resolvePreferredOriroTmpDir() : POSIX_ORIRO_TMP_DIR;
}

function resolveDefaultLogFile(defaultLogDir: string): string {
  return canUseNodeFs()
    ? path.join(defaultLogDir, "oriro.log")
    : `${POSIX_ORIRO_TMP_DIR}/oriro.log`;
}
...
type LoggerConfigLoader = () => OriroConfig["logging"] | undefined;
...
function canUseSilentVitestFileLogFastPath(envLevel: LogLevel | undefined): boolean {
  return (
    ...
    process.env.ORIRO_TEST_FILE_LOG !== "1" &&
    !envLevel &&
...
function resolveDefaultActiveLogFile(): string {
  if (process.env.VITEST === "true" && process.env.ORIRO_TEST_FILE_LOG === "1") {
    return path.join(
      process.cwd(),
      ".artifacts",
      "test-logs",
...
function resolveSettings(): ResolvedSettings {
  ...
  const cfg: OriroConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? loadLoggerConfig();
  const defaultLevel =
    process.env.VITEST === "true" && process.env.ORIRO_TEST_FILE_LOG !== "1" ? "silent" : "info";
      ...
      if (currentFileBytes > 0 && nextBytes > settings.maxFileBytes) {
        if (rotateLogFile(activeFile)) {
          ...
          process.stderr.write(
            `[oriro] log file rotation failed; continuing writes file=${activeFile} maxFileBytes=${settings.maxFileBytes}\n`,
          );
        }
      }
