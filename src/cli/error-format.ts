...
export function formatUnknownChannelMessage(params: {
  ...
  const listCommand = params.listCommand ?? "oriro channels list --all";
...
export function formatUnsupportedChannelActionMessage(params: {
  ...
  const inspectCommand =
    params.inspectCommand ?? `oriro channels capabilities --channel ${params.channel}`;
...
export function formatStrictJsonParseFailure(params: { value: string; cause: unknown }): string {
  ...
  return [
    ...
    `Use valid JSON, for example ${formatInlineCliCommand(
      "oriro config set gateway.port 18789 --strict-json",
    )}.`,
...
export function formatGatewayCommandFailure(params: {
  ...
  const message = raw
    .replace(/\s*Run [`"]?oriro doctor[`"]? for diagnostics\.?/gi, "")
    .replace(/\s+Gateway target:\s+.*$/isu, "")
    .replace(/\s+/g, " ")
    .trim()
    .replace(/[.。]+$/u, "");
  const inspectCommand = params.inspectCommand ?? "oriro gateway status --deep";
...
/** Format a plugin lookup miss with optional OriroHub search guidance. */
export function formatMissingPluginMessage(params: {
  id: string;
  listCommand?: string;
  includeSearch?: boolean;
}): string {
  const listCommand = params.listCommand ?? "oriro plugins list";
  const searchHint = params.includeSearch
    ? `, or ${formatInlineCliCommand("oriro plugins search " + params.id)} to look for installable plugins`
    : "";
