...
private struct WatchControlSurfaceView: View {
    ...
    var body: some View {
        ...
        .background(WatchOriroStyle.background.ignoresSafeArea())
        .navigationTitle("")
    }
    ...
    private var nowFace: some View {
        WatchFaceScroll {
            ...
            NavigationLink {
                ...
                WatchPrimaryLabel(title: "Talk to Oriro")
            }
            .buttonStyle(.plain)
                ...
                if record.isResolving {
                    ...
                    HStack(spacing: 8) {
                        ...
                        if record.approval.allowedDecisions.contains(.deny) {
                            WatchDecisionButton(title: "Deny", color: WatchOriroStyle.accent) {
...
private struct WatchOriroAvatar: View {
    var size: CGFloat
    ...
    var body: some View {
        ...
        .overlay {
            Circle()
                .strokeBorder(WatchOriroStyle.accent.opacity(0.32), lineWidth: 1)
        }
        .shadow(color: WatchOriroStyle.accent.opacity(0.30), radius: 5, y: 2)
        ...
        HStack(alignment: .center, spacing: 7) {
            ...
            VStack(alignment: .leading, spacing: 1) {
                Text(self.section)
                    ...
                    .foregroundStyle(WatchOriroStyle.accent)
                    .lineLimit(1)
                Text(self.title)
    ...
    var body: some View {
        ...
        .background {
            RoundedRectangle(cornerRadius: 17, style: .continuous)
                .fill(WatchOriroStyle.raised)
                .overlay {
                    RoundedRectangle(cornerRadius: 17, style: .continuous)
                        .strokeBorder(WatchOriroStyle.border, lineWidth: 1)
                }
        }
    }
}
...
private struct WatchPrimaryLabel: View {
    ...
    var body: some View {
        ...
        .background {
            Capsule(style: .continuous)
                .fill(WatchOriroStyle.hotGradient)
        }
    }
}
    ...
    var body: some View {
        HStack(spacing: 5) {
            ForEach(0..<max(self.pageCount, 1), id: \.self) { index in
                Capsule(style: .continuous)
                    .fill(index == self.selectedIndex ? WatchOriroStyle.accent : Color.white.opacity(0.20))
...
private struct WatchStackCard: View {
    ...
    var body: some View {
        ...
        .background {
            RoundedRectangle(cornerRadius: 17, style: .continuous)
                .fill(self.isProminent ? WatchOriroStyle.raised : WatchOriroStyle.surface)
                .overlay {
                    RoundedRectangle(cornerRadius: 17, style: .continuous)
                        .strokeBorder(WatchOriroStyle.border, lineWidth: 1)
                }
        ...
        HStack(alignment: .bottom, spacing: 6) {
            ...
            VStack(alignment: self.isUser ? .trailing : .leading, spacing: 3) {
                Text(self.roleTitle)
                    ...
                    .foregroundStyle(self.isUser ? .secondary : WatchOriroStyle.accent)
                Text(self.item.text)
                    .font(.system(size: 13))
                    .fixedSize(horizontal: false, vertical: true)
            }
            ...
            .background {
                RoundedRectangle(cornerRadius: 15, style: .continuous)
                    .fill(self.isUser ? WatchOriroStyle.accent.opacity(0.88) : WatchOriroStyle.surface)
            }
