...
function expectFailure() {
  ...
  const expected =
    source === "orirohub"
      ? /Version not found on OriroHub|OriroHub .* failed \(404\)|version.*not found/iu
...
function shouldScanLogFile(entry) {
  if (!(/\.(?:log|jsonl)$/u.test(entry) || /oriro-kitchen-sink-/u.test(path.basename(entry)))) {
    return false;
  }
...
function scanLogs() {
  ...
  const roots = [scratchRoot, path.join(process.env.HOME, ".oriro")];
  const deny = [
  ...
  if (scannedFiles === 0) {
    throw new Error(
      "kitchen-sink log scan found no files under the isolated scratch root or Oriro home",
    );
  }
...
function assertOriroHubExternalInstallContract(installPath) {
  ...
  if (!fs.existsSync(oriroPeerPath)) {
    throw new Error(`missing kitchen-sink oriro peer symlink: ${oriroPeerPath}`);
  }
  if (!fs.lstatSync(oriroPeerPath).isSymbolicLink()) {
    throw new Error(`kitchen-sink oriro peer is not a symlink: ${oriroPeerPath}`);
  }
  const hostRoot = fs.realpathSync(process.cwd());
  const linkedHostRoot = fs.realpathSync(oriroPeerPath);
  if (linkedHostRoot !== hostRoot) {
    throw new Error(`expected kitchen-sink oriro peer ${linkedHostRoot} to target ${hostRoot}`);
  }
...
function assertOriroHubArtifactMetadata(record) {
  ...
  if (record.artifactKind !== "npm-pack" || record.artifactFormat !== "tgz") {
    throw new Error(`missing kitchen-sink OriroHub artifact metadata: ${JSON.stringify(record)}`);
  }
  if (!record.oriropackSha256 || typeof record.oriropackSize !== "number") {
    throw new Error(`missing kitchen-sink OriroPack metadata: ${JSON.stringify(record)}`);
  }
...
function assertInstalled() {
  ...
  } else if (source === "orirohub") {
    const value = spec.slice("orirohub:".length).trim();
    const slashIndex = value.lastIndexOf("/");
    const atIndex = value.lastIndexOf("@");
    const packageName = atIndex > 0 && atIndex > slashIndex ? value.slice(0, atIndex) : value;
    if (record.spec !== spec) {
      throw new Error(`expected kitchen-sink OriroHub spec ${spec}, got ${record.spec}`);
    }
    if (record.orirohubPackage !== packageName) {
      throw new Error(`expected OriroHub package ${packageName}, got ${record.orirohubPackage}`);
    }
    if (record.orirohubFamily !== "code-plugin" && record.orirohubFamily !== "bundle-plugin") {
      throw new Error(`unexpected OriroHub family: ${record.orirohubFamily}`);
    }
    if (!record.version || !record.integrity || !record.resolvedAt) {
      throw new Error(`missing OriroHub resolution metadata: ${JSON.stringify(record)}`);
    }
    assertOriroHubArtifactMetadata(record);
  }
  ...
  if (source === "orirohub") {
    ...
    assertRealPathInside(extensionsRoot, installPath, "kitchen-sink OriroHub install path");
  }
  if (source === "orirohub" && record.artifactKind === "npm-pack") {
