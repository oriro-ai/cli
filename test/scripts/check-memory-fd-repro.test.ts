...
describe("check-memory-fd-repro", () => {
  ...
  it("rejects loose numeric environment limits before generating files", () => {
    expect(
      withEnv(
        {
          ORIRO_MEMORY_FD_REPRO_FILES: "17",
          ORIRO_MEMORY_FD_REPRO_MAX_WORKSPACE_REG_FDS: "0",
          ORIRO_MEMORY_FD_REPRO_SAMPLE_DELAY_MS: "0",
          ORIRO_MEMORY_FD_REPRO_SETTLE_DELAY_MS: String(MAX_TIMER_TIMEOUT_MS + 1),
          ORIRO_MEMORY_FD_REPRO_TIMEOUT_MS: String(MAX_TIMER_TIMEOUT_MS + 1),
        },
        () => parseArgs([]),
      ),
    ).toMatchObject({
    ...
    expect(() =>
      withEnv({ ORIRO_MEMORY_FD_REPRO_FILES: "17files" }, () => parseArgs([])),
    ).toThrow("ORIRO_MEMORY_FD_REPRO_FILES must be a non-negative integer");
    expect(() =>
      withEnv({ ORIRO_MEMORY_FD_REPRO_TIMEOUT_MS: "1e3" }, () => parseArgs([])),
    ).toThrow("ORIRO_MEMORY_FD_REPRO_TIMEOUT_MS must be a non-negative integer");
  });
  ...
  it("lets explicit CLI numeric flags override malformed inherited env defaults", () => {
    expect(
      withEnv(
        {
          ORIRO_MEMORY_FD_REPRO_FILES: "17files",
          ORIRO_MEMORY_FD_REPRO_MAX_WORKSPACE_REG_FDS: "4fds",
          ORIRO_MEMORY_FD_REPRO_TIMEOUT_MS: "1e3",
          ORIRO_MEMORY_FD_REPRO_SAMPLE_DELAY_MS: "soon",
          ORIRO_MEMORY_FD_REPRO_SETTLE_DELAY_MS: "later",
        },
        () =>
          parseArgs([
            "--files",
  ...
  it("stops parsing options after the argument terminator", () => {
    ...
    expect(
      withEnv({ ORIRO_MEMORY_FD_REPRO_FILES: "17" }, () => parseArgs(["--", "--unknown"])),
    ).toMatchObject({
      fileCount: 17,
    });
  });
  ...
  it("writes an offline FTS-only memory search config for repro indexing", () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-memory-fd-config-"));
    try {
      ...
      expect(memorySearch).toMatchObject({
        ...
        model: "",
        store: {
          path: path.join(homeDir, ".oriro", "memory", "main.sqlite"),
