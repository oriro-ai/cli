...
describe("discord doctor", () => {
  ...
  it("removes unsupported Discord realtime wake names", () => {
      ...
      cfg: {
        channels: {
          discord: {
            voice: {
              realtime: {
                wakeNames: ["Oriro", "Oriro Bot Helper", "Open Oriro"],
              },
            },
            accounts: {
              work: {
    ...
    expect(result.changes).toEqual([
      ...
      "Removed empty channels.discord.accounts.empty.voice.realtime.wakeNames; unset wake names use the default agent/Oriro fallback.",
  ...
  it("repairs safe numeric ids into strings and warns for unsafe lists", () => {
    ...
    const result = maybeRepairDiscordNumericIds(cfg, "oriro doctor --fix");
  ...
  it("formats repair guidance for unsafe numeric ids", () => {
    ...
    expect(warnings[1]).toContain("oriro doctor --fix");
  });
