...
import type { MockFn } from "oriro/plugin-sdk/plugin-test-runtime";
...
const signalToolResultSessionStorePath = vi.hoisted(
  () => `/tmp/oriro-signal-tool-result-sessions-${process.pid}.json`,
);
...
vi.mock("oriro/plugin-sdk/runtime-config-snapshot", async () => {
  const actual = await vi.importActual<
    typeof import("oriro/plugin-sdk/runtime-config-snapshot")
  >("oriro/plugin-sdk/runtime-config-snapshot");
  return {
    ...actual,
...
vi.mock("oriro/plugin-sdk/reply-runtime", async () => {
  const actual = await vi.importActual<typeof import("oriro/plugin-sdk/reply-runtime")>(
...
vi.mock("oriro/plugin-sdk/security-runtime", async () => {
  const actual = await vi.importActual<typeof import("oriro/plugin-sdk/security-runtime")>(
    "oriro/plugin-sdk/security-runtime",
  );
  return {
    ...actual,
...
vi.mock("oriro/plugin-sdk/system-event-runtime", async () => {
  const actual = await vi.importActual<typeof import("oriro/plugin-sdk/system-event-runtime")>(
    "oriro/plugin-sdk/system-event-runtime",
  );
  return {
    ...actual,
...
export function installSignalToolResultTestHooks() {
  beforeEach(async () => {
    const [{ resetInboundDedupe }, { resetSystemEventsForTest }] = await Promise.all([
      import("oriro/plugin-sdk/reply-runtime"),
      import("oriro/plugin-sdk/system-event-runtime"),
    ]);
    resetInboundDedupe();
    config = {
