...
describe("resolveGatewayService", () => {
  ...
  it("guards mutating service adapters when config was written by a newer Oriro", async () => {
    const tempHome = await makeTempWorkspace("oriro-service-future-config-");
    const stateDir = path.join(tempHome, ".oriro");
    const configPath = path.join(stateDir, "oriro.json");
    const envSnapshot = captureEnv(["HOME", "ORIRO_STATE_DIR", "ORIRO_CONFIG_PATH"]);
    try {
...
describe("readGatewayServiceState", () => {
  it("tracks installed, loaded, and running separately", async () => {
    ...
    const state = await readGatewayServiceState(service, {
      env: { ORIRO_GATEWAY_PORT: "1" },
    });

    expect(state.installed).toBe(true);
    expect(state.loaded).toBe(true);
    expect(state.running).toBe(true);
    expect(state.env.ORIRO_GATEWAY_PORT).toBe("18789");
  });
  ...
  it("keeps the caller-selected service identity when merging persisted env", async () => {
    ...
    const state = await readGatewayServiceState(service, {
      env: { ORIRO_SYSTEMD_UNIT: "oriro-gateway-maintenance.service" },
    });

    expect(state.env.ORIRO_SYSTEMD_UNIT).toBe("oriro-gateway-maintenance.service");
...
describe("startGatewayService", () => {
  ...
  it("requests repair before start when the loaded service version is stale", async () => {
    const service = createService({
      readCommand: vi.fn(async () => ({
        programArguments: ["oriro", "gateway", "run"],
        environment: { ORIRO_SERVICE_VERSION: "2026.4.24" },
      })),
      isLoaded: vi.fn(async () => true),
    ...
    if (result.outcome === "repair-required") {
      expect(formatGatewayServiceStartRepairIssues(result.issues)).toContain(
        "service was installed by Oriro 2026.4.24",
      );
    }
    expect(service.restart).not.toHaveBeenCalled();
  });
  ...
  it("requests repair before start when the loaded service points at temporary install paths", async () => {
    const service = createService({
      readCommand: vi.fn(async () => ({
        programArguments: [
          "/private/tmp/oriro-ai-install-cli-pr118/tools/node/bin/node",
          "/tmp/oriro-ai-install-cli-pr118/lib/node_modules/oriro/dist/index.js",
          "gateway",
        ],
        environment: {},
      })),
