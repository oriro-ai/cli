// Check No Monolithic Plugin Sdk Entry Imports script supports Oriro repository automation.
import fs from "node:fs";
import path from "node:path";
import { discoverOriroPlugins } from "../src/plugins/discovery.js";
...
const ROOT_IMPORT_PATTERN = /["']oriro\/plugin-sdk["']/;
const LEGACY_COMPAT_IMPORT_PATTERN = /["']oriro\/plugin-sdk\/compat["']/;
const LEGACY_BROAD_SUBPATH_PATTERNS = [
  {
    pattern: /["']oriro\/plugin-sdk\/channel-runtime["']/,
    label: "oriro/plugin-sdk/channel-runtime",
  },
  {
    pattern: /["']oriro\/plugin-sdk\/config-runtime["']/,
    label: "oriro/plugin-sdk/config-runtime",
  },
  {
    pattern: /["']oriro\/plugin-sdk\/infra-runtime["']/,
    label: "oriro/plugin-sdk/infra-runtime",
  },
] as const;
...
function main() {
  ...
  if (
    ...
    if (monolithicOffenders.length > 0) {
      console.error("Bundled plugin source files must not import monolithic oriro/plugin-sdk.");
    ...
    if (legacyCompatOffenders.length > 0) {
      console.error(
        "Bundled plugin source files must not import legacy oriro/plugin-sdk/compat.",
      );
    ...
    if (
      ...
      console.error(
        "Use focused oriro/plugin-sdk/<domain> subpaths for bundled plugins; root, compat, and broad runtime barrels are legacy surfaces only.",
      );
    }
    process.exit(1);
  }
