...
import { fetchWithSsrFGuard } from "oriro/plugin-sdk/ssrf-runtime";
import {
...
export type RuntimeId = "oriro" | "codex";
...
export function isRuntimeParityResultPass(result: RuntimeParityResult) {
  return (
    ...
    isRuntimeParityCellPassable(result.cells.oriro) &&
...
const HEARTBEAT_TRANSCRIPT_PROMPT = "[ORIRO heartbeat poll]";
...
function classifyRuntimeParityCells(params: {
  oriro: RuntimeParityCell;
  codex: RuntimeParityCell;
  oriroScenarioStatus: "pass" | "fail";
  codexScenarioStatus: "pass" | "fail";
}): Pick<RuntimeParityResult, "drift" | "driftDetails"> {
  if (
    isHardFailureRuntimeError(params.oriro.runtimeErrorClass) ||
    isHardFailureRuntimeError(params.codex.runtimeErrorClass) ||
    params.oriro.transportErrorClass ||
    params.codex.transportErrorClass
  ) {
    return {
      drift: "failure-mode",
      driftDetails:
        params.oriro.transportErrorClass || params.codex.transportErrorClass
  ...
  if (
    hasMissingToolResult(params.oriro.toolCalls) ||
  ...
  if (
    params.oriroScenarioStatus === "fail" ||
    params.codexScenarioStatus === "fail" ||
    !isRuntimeParityCellPassable(params.oriro) ||
    !isRuntimeParityCellPassable(params.codex)
  ) {
    return {
      drift: "failure-mode",
      driftDetails:
        params.oriroScenarioStatus === params.codexScenarioStatus
          ? "at least one runtime failed"
          : `scenario status differs (${params.oriroScenarioStatus} vs ${params.codexScenarioStatus})`,
    };
  }

  const toolCallShapeDetails = compareToolCallShape(
    params.oriro.toolCalls,
  ...
  const oriroTranscriptLines = params.oriro.transcriptBytes.trim().length
    ? params.oriro.transcriptBytes.trim().split(/\r?\n/u).length
    : 0;
  const codexTranscriptLines = params.codex.transcriptBytes.trim().length
    ? params.codex.transcriptBytes.trim().split(/\r?\n/u).length
    : 0;
  if (
    oriroTranscriptLines !== codexTranscriptLines ||
    (!params.oriro.finalText && Boolean(params.codex.finalText)) ||
    (Boolean(params.oriro.finalText) && !params.codex.finalText)
  ) {
    return {
      drift: "structural",
      driftDetails: `transcript/final-text structure differs (${oriroTranscriptLines} lines vs ${codexTranscriptLines})`,
    };
  }

  if (
    normalizeTextForParity(params.oriro.finalText) ===
...
export async function runRuntimeParityScenario(params: {
  ...
  const oriro = await params.runCell("oriro");
  const codex = await params.runCell("codex");
  const drift = classifyRuntimeParityCells({
    oriro: oriro.cell,
    codex: codex.cell,
    oriroScenarioStatus: oriro.scenarioStatus,
