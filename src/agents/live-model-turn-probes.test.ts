...
describe("live model turn probes", () => {
  it("defaults probes on and accepts common opt-out values", () => {
    expect(isLiveModelProbeEnabled({}, "ORIRO_LIVE_MODEL_IMAGE_PROBE")).toBe(true);
    expect(
      isLiveModelProbeEnabled(
        { ORIRO_LIVE_MODEL_IMAGE_PROBE: "false" },
        "ORIRO_LIVE_MODEL_IMAGE_PROBE",
      ),
    ).toBe(false);
    expect(
      isLiveModelProbeEnabled(
        { ORIRO_LIVE_MODEL_IMAGE_PROBE: "1" },
        "ORIRO_LIVE_MODEL_IMAGE_PROBE",
      ),
    ).toBe(true);
  });
