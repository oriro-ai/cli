...
import type { NativeCommandSpec } from "oriro/plugin-sdk/command-auth-native";
import { resolveDirectStatusReplyForSession } from "oriro/plugin-sdk/command-status-runtime";
...
import { dispatchReplyWithDispatcher } from "oriro/plugin-sdk/reply-dispatch-runtime";
import {
  clearRuntimeConfigSnapshot,
  setRuntimeConfigSnapshot,
} from "oriro/plugin-sdk/runtime-config-snapshot";
import { getSessionEntry } from "oriro/plugin-sdk/session-store-runtime";
...
function createConfig(): OriroConfig {
  return {
    channels: {
      discord: {
...
async function createNativeCommand(cfg: OriroConfig, commandSpec: NativeCommandSpec) {
...
async function createPluginCommand(params: { cfg: OriroConfig; name: string }) {
  return createDiscordNativeCommand({
    command: {
      name: params.name,
      description: "Pair",
...
async function createStatusCommand(cfg: OriroConfig) {
...
describe("Discord native plugin command dispatch", () => {
  ...
  beforeEach(() => {
    ...
    discordNativeCommandTesting.setMatchPluginCommand(
      runtimeModuleMocks.matchPluginCommand as typeof import("oriro/plugin-sdk/plugin-runtime").matchPluginCommand,
    );
    discordNativeCommandTesting.setExecutePluginCommand(
      runtimeModuleMocks.executePluginCommand as typeof import("oriro/plugin-sdk/plugin-runtime").executePluginCommand,
    );
    ...
    discordNativeCommandTesting.setGetSessionEntry(
      runtimeModuleMocks.getSessionEntry as typeof import("oriro/plugin-sdk/session-store-runtime").getSessionEntry,
    );
  });
  ...
  it("refreshes native command routing config between invocations", async () => {
    ...
    const resolveRouteState = vi.fn(async (params: { cfg: OriroConfig }) =>
      createUnboundRouteState({
        sessionKey:
  ...
  it("passes the configured binding agent to plugin-owned Discord command sessions", async () => {
    const cfg = createConfig();
    const interaction = createInteraction();
    const pluginSessionKey = "plugin-binding:oriro-codex-app-server:dm";
