...
async function withTempOriroStateDir<T>(test: (stateDir: string) => Promise<T>): Promise<T> {
...
describe("runMessageAction media behavior", () => {
  ...
  it("materializes buffer-only send attachments into outbound media paths", async () => {
    ...
    await withTempOriroStateDir(async () => {
  ...
  it("does not stage buffer-only send attachments before target validation passes", async () => {
    ...
    await withTempOriroStateDir(async (stateDir) => {
      await expect(
        runMessageAction({
          cfg: workspaceConfig,
          action: "send",
  ...
  describe("reply hydration", () => {
    ...
    // params as send. Before oriro#79864 the runner only hydrated
  ...
  describe("sandboxed media validation", () => {
    ...
    it("allows media paths under preferred Oriro tmp root", async () => {
      ...
      try {
        ...
        const hostTmpOutsideOriro = path.join(os.tmpdir(), "outside-oriro", "test-media.png");
        await expect(
          runMessageAction({
            cfg: workspaceConfig,
            action: "send",
