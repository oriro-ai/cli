...
import type { OriroConfig } from "../../../src/config/types.oriro.js";
...
const embeddedAgentMocks = getSharedMocks("oriro.trigger-handling.embedded-agent-mocks", () => ({
...
const modelCatalogMocks = getSharedMocks("oriro.trigger-handling.model-catalog-mocks", () => ({
...
const modelFallbackMocks = getSharedMocks("oriro.trigger-handling.model-fallback-mocks", () => ({
...
const webSessionMocks = getSharedMocks("oriro.trigger-handling.web-session-mocks", () => ({
...
function snapshotTempHomeEnv(): TempHomeEnvSnapshot {
  return {
    ...
    oriroHome: process.env.ORIRO_HOME,
...
function restoreTempHomeEnv(snapshot: TempHomeEnvSnapshot): void {
  ...
  restoreKey("ORIRO_HOME", snapshot.oriroHome);
  restoreKey("ORIRO_STATE_DIR", snapshot.stateDir);
}
...
function setTempHomeEnv(home: string): void {
  ...
  process.env.ORIRO_STATE_DIR = join(home, ".oriro");
...
beforeAll(async () => {
  suiteTempHomeRoot = await fs.mkdtemp(join(os.tmpdir(), "oriro-triggers-suite-"));
});
...
export async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  ...
  await fs.mkdir(join(home, ".oriro", "agents", "main", "sessions"), { recursive: true });
  setTempHomeEnv(home);
...
export function makeCfg(home: string): OriroConfig {
  return withFastReplyConfig({
    agents: {
      defaults: {
        model: { primary: "anthropic/claude-opus-4-7" },
        workspace: join(home, "oriro"),
