...
describe("formatInstallHint", () => {
  it("describes dual-source npm-default installs as npm first", () => {
    ...
    ).toBe("npm, with OriroHub fallback");
  });

  it("keeps dual-source orirohub-default installs OriroHub first", () => {
    expect(
      testing.formatInstallHint({
        orirohubSpec: "orirohub:@oriro/diagnostics-otel",
        npmSpec: "@oriro/diagnostics-otel",
    ...
    ).toBe("OriroHub, with npm fallback");
  });
});
  ...
  it("installs selected optional official plugins through the shared onboarding installer", async () => {
      ...
      expect.arrayContaining([
        {
          value: "acpx",
          label: "ACPX Runtime",
          hint: "Oriro ACP runtime backend",
        },
        {
          value: "diagnostics-otel",
          label: "Diagnostics OpenTelemetry",
          hint: "Oriro diagnostics OpenTelemetry exporter",
        },
        {
          value: "diagnostics-prometheus",
          label: "Diagnostics Prometheus",
          hint: "Oriro diagnostics Prometheus exporter",
        },
        {
          value: "tokenjuice",
          label: "Tokenjuice",
          hint: "Oriro tokenjuice exec output compaction plugin",
        },
      ]),
    );
    expect(ensureOnboardingPluginInstalled).toHaveBeenCalledExactlyOnceWith({
      ...
      entry: {
        ...
        label: "Diagnostics OpenTelemetry",
        description: "Oriro diagnostics OpenTelemetry exporter",
        install: {
          orirohubSpec: "orirohub:@oriro/diagnostics-otel",
          npmSpec: "@oriro/diagnostics-otel",
          defaultChoice: "npm",
