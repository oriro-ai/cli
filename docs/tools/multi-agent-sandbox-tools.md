...
<Note>
Legacy `agent.*` configs are migrated by `oriro doctor`; prefer `agents.defaults` + `agents.list` going forward.
</Note>
...
<Steps>
  ...
  <Step title="Verify sandbox containers">
    ```bash
    docker ps --filter "name=oriro-sbx-"
    ```
  </Step>
  ...
  <Step title="Monitor logs">
    ```bash
    tail -f "${ORIRO_STATE_DIR:-$HOME/.oriro}/logs/gateway.log" | grep -E "routing|sandbox|tools"
    ```
  </Step>
</Steps>
