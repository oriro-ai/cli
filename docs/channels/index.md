---
summary: "Messaging platforms Oriro can connect to"
read_when:
  - You want to choose a chat channel for Oriro
  - You need a quick overview of supported messaging platforms
title: "Chat channels"
---

Oriro can talk to you on any chat app you already use. Each channel connects via the Gateway.
...
- WhatsApp setup is install-on-demand: onboarding can show the setup flow before
  ...
  OriroHub/npm plugin only when the channel is actually active.
...
- [iMessage](/channels/imessage) - Native macOS integration via the `imsg` bridge on a signed-in Mac (or SSH wrapper when the Gateway runs elsewhere), including private API actions for replies, tapbacks, effects, attachments, and group management. Preferred for new Oriro iMessage setups when host permissions and Messages access fit.
...
- [Zalo OriroBot](/channels/zaloorirobot) - Personal Zalo assistant via QR login; owner-bound (external plugin).
- [Zalo Personal](/channels/zalouser) - Zalo personal account via QR login (bundled plugin).

## Notes

- Channels can run simultaneously; configure multiple and Oriro will route per chat.
