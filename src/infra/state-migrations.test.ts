...
vi.mock("../channels/plugins/bundled.js", () => {
  ...
  function resolveChatAppAccountId(cfg: OriroConfig): string {
  ...
  return {
    ...
    listBundledChannelLegacyStateMigrationDetectors: vi.fn(() => [
      ...
      ({ cfg, env }: { cfg: OriroConfig; env: NodeJS.ProcessEnv }) => {
        const root = env.ORIRO_STATE_DIR;
        if (!root) {
          return [];
        }
...
type PluginBindingApprovalsDatabase = Pick<OriroStateKyselyDatabase, "plugin_binding_approvals">;
...
const createTempDir = () => tempDirs.make("oriro-state-migrations-test-");
...
describe("state migrations", () => {
  ...
  it("migrates legacy plugin binding approvals from the home state dir when using a custom state dir", async () => {
    ...
    const env = createEnv(stateDir);
    const cfg = createConfig();
    const sourcePath = path.join(root, ".oriro", "plugin-binding-approvals.json");
