...
describe("pickTagline", () => {
  ...
  it("keeps ORIRO_TAGLINE_INDEX behavior in random mode", () => {
    const value = pickTagline({
      mode: "random",
      env: { ORIRO_TAGLINE_INDEX: "0" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(
  ...
  it("ignores partial ORIRO_TAGLINE_INDEX values", () => {
    expect(
      pickTagline({
        mode: "random",
        env: { ORIRO_TAGLINE_INDEX: "1abc" } as NodeJS.ProcessEnv,
        random: () => 0,
      }),
    ).toBe("Your terminal just grew oriros\u2014type something and let the bot pinch the busywork.");
  });
});
