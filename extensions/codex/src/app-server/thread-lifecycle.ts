...
import { buildCodexUserMcpServersThreadConfigPatch } from "oriro/plugin-sdk/codex-mcp-projection";
import { listRegisteredPluginAgentPromptGuidance } from "oriro/plugin-sdk/plugin-runtime";
...
function toCodexThreadStartDynamicTools(
  ...
  // Oriro namespaces internally, but omit `type` on the wire so Codex does
...
function buildDefaultCollaborationInstructions(): string {
  ...
  // is null. Oriro adds per-turn workspace instructions here, so preserve that
...
export function resolveCodexAppServerModelProvider(params: {
  ...
  if (!normalized || normalizedLower === "codex") {
    // `codex` is Oriro's virtual provider; let Codex app-server keep its
  ...
  if (isCodexAppServerNativeAuthProfile(params) && normalizedLower === "openai") {
    // When Oriro is forwarding ChatGPT/Codex OAuth, `openai` is Codex's
