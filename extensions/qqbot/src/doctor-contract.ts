...
import type { GroupToolPolicyConfig } from "oriro/plugin-sdk/channel-policy";
import type { OriroConfig } from "oriro/plugin-sdk/config-contracts";
import { asObjectRecord } from "oriro/plugin-sdk/runtime-doctor";
...
export const legacyConfigRules: ChannelDoctorLegacyConfigRule[] = [
  {
    ...
    message:
      'channels.qqbot.groups.<id>.toolPolicy is legacy and was ignored by QQBot group tool enforcement; use channels.qqbot.groups.<id>.tools instead. Run "oriro doctor --fix".',
    match: hasLegacyGroupToolPolicy,
  },
  {
    path: ["channels", "qqbot", "accounts"],
    message:
      'channels.qqbot.accounts.<id>.groups.<groupId>.toolPolicy is legacy and was ignored by QQBot group tool enforcement; use channels.qqbot.accounts.<id>.groups.<groupId>.tools instead. Run "oriro doctor --fix".',
...
export function normalizeCompatibilityConfig({
  ...
  return {
    config: {
      ...
      channels: {
        ...cfg.channels,
        qqbot: updated as unknown as NonNullable<OriroConfig["channels"]>["qqbot"],
