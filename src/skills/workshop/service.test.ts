...
beforeEach(async () => {
  testState = await createOriroTestState({
    layout: "state-only",
    prefix: "oriro-skill-workshop-state-",
  });
...
async function makeWorkspace(): Promise<string> {
  return await tempDirs.make("oriro-skill-workshop-");
}
...
describe("skill workshop proposals", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const workspaceDir = await makeWorkspace();
      const targetSkillsDir = await tempDirs.make("oriro-skill-workshop-target-skills-");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const workspaceDir = await makeWorkspace();
      const targetSkillsDir = await tempDirs.make("oriro-skill-workshop-readonly-skills-");
      await fs.symlink(targetSkillsDir, path.join(workspaceDir, "skills"), "dir");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const workspaceDir = await makeWorkspace();
      const targetSkillsDir = await tempDirs.make("oriro-skill-workshop-support-trusted-");
      const untrustedSkillsDir = await tempDirs.make("oriro-skill-workshop-support-untrusted-");
      await fs.symlink(targetSkillsDir, path.join(workspaceDir, "skills"), "dir");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const workspaceDir = await makeWorkspace();
      const targetSkillsDir = await tempDirs.make("oriro-skill-workshop-untrusted-skills-");
      await fs.symlink(targetSkillsDir, path.join(workspaceDir, "skills"), "dir");
      const proposal = await proposeCreateSkill({
        workspaceDir,
