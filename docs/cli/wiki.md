---
summary: "CLI reference for `oriro wiki` (memory-wiki vault status, search, compile, lint, apply, bridge, and Obsidian helpers)"
read_when:
  - You want to use the memory-wiki CLI
  - You are documenting or changing `oriro wiki`
title: "Wiki"
---
...
Use `oriro wiki` when you want a compiled knowledge vault with:
...
oriro wiki doctor
oriro wiki init
oriro wiki ingest ./notes/alpha.md
oriro wiki okf import ./knowledge-catalog/okf/bundles/ga4
oriro wiki compile
oriro wiki lint
oriro wiki search "alpha"
oriro wiki search "who should I ask about Teams?" --mode route-question
oriro wiki get entity.alpha --from 1 --lines 80

oriro wiki apply synthesis "Alpha Summary" \
  --body "Short synthesis body" \
  --source-id source.alpha

oriro wiki apply metadata entity.alpha \
  --source-id source.alpha \
  --status review \
  --question "Still active?"

oriro wiki bridge import
oriro wiki unsafe-local import

oriro wiki obsidian status
oriro wiki obsidian search "alpha"
oriro wiki obsidian open syntheses/alpha-summary.md
oriro wiki obsidian command workspace:quick-switcher
oriro wiki obsidian daily
```
...
```bash
oriro wiki okf import ./bundles/ga4
oriro wiki okf import ./bundles/ga4 --json
oriro wiki search "BigQuery Table" --mode source-evidence --json
oriro wiki get <path-from-json-result>
```
...
- `.oriro-wiki/cache/agent-digest.json`
- `.oriro-wiki/cache/claims.jsonl`
...
For one broad shared recall pass, prefer `oriro memory search` when the
...
```bash
oriro wiki search "bgroux" --mode find-person
oriro wiki search "who knows Teams rollout?" --mode route-question
oriro wiki search "maintainer-whois" --mode source-evidence
oriro wiki search "strong route Teams" --mode raw-claim --json
```
...
```bash
oriro wiki get entity.alpha
oriro wiki get syntheses/alpha-summary.md --from 1 --lines 80
```
...
`oriro wiki` behavior is shaped by:
