// Tool Search Gateway E2E script supports Oriro repository automation.
...
export function readToolSearchGatewayFetchLimits(
  ...
  return {
    bodyMaxBytes: readPositiveIntEnv(
      "ORIRO_TOOL_SEARCH_GATEWAY_E2E_FETCH_BODY_MAX_BYTES",
      1024 * 1024,
      env,
    ),
    timeoutMs: readPositiveIntEnv(
      "ORIRO_TOOL_SEARCH_GATEWAY_E2E_FETCH_TIMEOUT_MS",
      180_000,
      env,
    ),
  };
...
export function snapshotToolSearchGatewayEnv(
  ...
  return {
    configPath: env.ORIRO_CONFIG_PATH,
    stateDir: env.ORIRO_STATE_DIR,
    testFast: env.ORIRO_TEST_FAST,
  };
}
...
function restoreEnvValue(
  ...
  key: keyof Pick<
    NodeJS.ProcessEnv,
    "ORIRO_CONFIG_PATH" | "ORIRO_STATE_DIR" | "ORIRO_TEST_FAST"
  >,
...
export function restoreToolSearchGatewayEnv(
  ...
  restoreEnvValue(env, "ORIRO_CONFIG_PATH", snapshot.configPath);
  restoreEnvValue(env, "ORIRO_STATE_DIR", snapshot.stateDir);
  restoreEnvValue(env, "ORIRO_TEST_FAST", snapshot.testFast);
}
...
async function writeFakePlugin(params: {
  ...
  await fs.writeFile(
    ...
    `${JSON.stringify(
      {
        name: "@oriro/tool-search-e2e-fixture",
        version: "0.0.0",
        type: "module",
        oriro: {
          extensions: ["./index.js"],
...
export async function main() {
  ...
  const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-tool-search-"));
