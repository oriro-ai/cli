...
export async function handleIrcInbound(params: {
  ...
  const allowTextCommands = core.channel.commands.shouldHandleTextCommands({
    cfg: config as OriroConfig,
    surface: CHANNEL_ID,
  });
  const hasControlCommand = core.channel.text.hasControlCommand(rawBody, config as OriroConfig);
  const mentionRegexes = core.channel.mentions.buildMentionRegexes(config as OriroConfig);
