...
import { generateSecureUuid } from "oriro/plugin-sdk/core";
import { redactIdentifier } from "oriro/plugin-sdk/logging-core";
import {
  convertMarkdownTables,
  resolveMarkdownTableMode,
} from "oriro/plugin-sdk/markdown-table-runtime";
import { requireRuntimeConfig } from "oriro/plugin-sdk/plugin-config-runtime";
import { normalizePollInput, type PollInput } from "oriro/plugin-sdk/poll-runtime";
import { createSubsystemLogger, getChildLogger } from "oriro/plugin-sdk/runtime-env";
import {
...
function requireOutboundActiveWebListener(params: { cfg: OriroConfig; accountId?: string }): {
  accountId: string;
  ...
  if (!listener) {
    throw new Error(
      `No active WhatsApp Web listener (account: ${resolvedAccountId}). Start the gateway, then link WhatsApp with: ${formatCliCommand(`oriro channels login --channel whatsapp --account ${resolvedAccountId}`)}.`,
    );
  }
...
export async function sendPollWhatsApp(
  to: string,
  poll: PollInput,
  options: { verbose: boolean; accountId?: string; cfg: OriroConfig },
): Promise<{ messageId: string; toJid: string }> {
  const correlationId = generateSecureUuid();
  const startedAt = Date.now();
