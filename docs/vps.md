---
summary: "Run Oriro on a Linux server or cloud VPS — provider picker, architecture, and tuning"
read_when:
  - You want to run the Gateway on a Linux server or cloud VPS
  - You need a quick map of hosting guides
  - You want generic Linux server tuning for Oriro
title: "Linux server"
sidebarTitle: "Linux Server"
---

Run the Oriro Gateway on any Linux server or cloud VPS. This page helps you
...
Before you install Oriro on a public VPS, decide how you want to administer
the box itself.
...
- This is separate from Gateway access. You can still keep Oriro bound to
...
```bash
grep -q 'NODE_COMPILE_CACHE=/var/tmp/oriro-compile-cache' ~/.bashrc || cat >> ~/.bashrc <<'EOF'
...
- Add service env for a stable startup path:
  - `ORIRO_NO_RESPAWN=1`
  - `NODE_COMPILE_CACHE=/var/tmp/oriro-compile-cache`
...
For the standard `oriro onboard --install-daemon` path, edit the user unit:
...
`oriro-gateway.service` via `sudo systemctl edit oriro-gateway.service`.
