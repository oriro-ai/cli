    ...
    [
      {
        id: "logging-file",
        intent: "logging",
        argv: ["config", "set", "logging.file", "~/oriro-upgrade-survivor/gateway.jsonl"],
      },
    ],
  ],
  [
...
function selectedScenario() {
  return process.env.ORIRO_UPGRADE_SURVIVOR_SCENARIO || "base";
}
...
export function resolveUpgradeSurvivorOriroCommand(argv, params = {}) {
  const platform = params.platform ?? process.platform;
  if (platform === "win32") {
    const comSpec = params.comSpec ?? process.env.ComSpec ?? "cmd.exe";
    return {
      command: comSpec,
      args: ["/d", "/s", "/c", buildCmdExeCommandLine("oriro.cmd", argv)],
      commandLabel: ["oriro", ...argv].join(" "),
      shell: false,
...
export function runUpgradeSurvivorOriroStep(step, params = {}) {
  const invocation = resolveUpgradeSurvivorOriroCommand(step.argv);
...
function applyRecipe() {
  ...
  for (const step of [...recipe.slice(0, -1), ...scenarioSteps, recipe.at(-1)]) {
    ...
    const outcome = runUpgradeSurvivorOriroStep(adaptedStep);
