// Voice Call plugin entrypoint registers its Oriro integration.
import { formatErrorMessage } from "oriro/plugin-sdk/error-runtime";
import { ErrorCodes, errorShape } from "oriro/plugin-sdk/gateway-runtime";
import { timestampMsToIsoString } from "oriro/plugin-sdk/number-runtime";
...
const voiceCallConfigSchema = {
  ...
  uiHints: {
    ...
    "realtime.toolPolicy": {
      ...
      help: "Controls the shared oriro_agent_consult tool.",
      advanced: true,
    },
    "realtime.consultPolicy": {
      label: "Realtime Consult Policy",
      help: "Guides when the realtime voice model should call oriro_agent_consult.",
      advanced: true,
    },
...
function isCliOnlyProcess(): boolean {
  return process.env.ORIRO_CLI === "1" && !process.argv.slice(2).includes("gateway");
}

const VOICE_CALL_RUNTIME_KEY = Symbol.for("oriro.voice-call.runtime");
const VOICE_CALL_RUNTIME_PROMISE_KEY = Symbol.for("oriro.voice-call.runtimePromise");
const VOICE_CALL_RUNTIME_STOP_PROMISE_KEY = Symbol.for("oriro.voice-call.runtimeStopPromise");
