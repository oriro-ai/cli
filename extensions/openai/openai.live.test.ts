...
import type { ResolvedTtsConfig } from "oriro/plugin-sdk/agent-runtime";
import { AuthStorage, ModelRegistry } from "oriro/plugin-sdk/agent-sessions";
import type { OriroConfig } from "oriro/plugin-sdk/config-contracts";
import { encodePngRgba, fillPixel } from "oriro/plugin-sdk/media-runtime";
import {
  registerProviderPlugin,
  requireRegisteredProvider,
} from "oriro/plugin-sdk/plugin-test-runtime";
import { runRealtimeSttLiveTest } from "oriro/plugin-sdk/provider-test-contracts";
...
const LIVE_MODEL_ID = process.env.ORIRO_LIVE_OPENAI_PLUGIN_MODEL?.trim() || "gpt-5.5";
const LIVE_IMAGE_MODEL = process.env.ORIRO_LIVE_OPENAI_IMAGE_MODEL?.trim() || "gpt-image-2";
const LIVE_VISION_MODEL = process.env.ORIRO_LIVE_OPENAI_VISION_MODEL?.trim() || "gpt-5.4-mini";
const liveEnabled = OPENAI_API_KEY.trim().length > 0 && process.env.ORIRO_LIVE_TEST === "1";
...
function createLiveConfig(): OriroConfig {
...
describeLive("openai plugin live", () => {
  ...
  it("lists voices and synthesizes audio through the registered speech provider", async () => {
    ...
    const audioFile = await speechProvider.synthesize({
      text: "Oriro integration test OK.",
      cfg,
  ...
  it("streams realtime STT through the registered transcription provider", async () => {
    ...
    const cfg = createLiveConfig();
    const ttsConfig = createLiveTtsConfig();
    const phrase = "Testing Oriro OpenAI realtime transcription integration test OK.";
