...
import { parseStrictInteger, timestampMsToIsoString } from "oriro/plugin-sdk/number-runtime";
import type { ChannelSetupInput } from "oriro/plugin-sdk/setup";
...
import { isOriroManagedMatrixDevice } from "./matrix/device-health.js";
...
function formatMatrixCliCommandParts(parts: string[], accountId?: string): string {
  const normalizedAccountId = normalizeAccountId(accountId);
  const command = ["oriro", "matrix", ...parts];
...
async function addMatrixAccount(params: {
  ...
  try {
    ...
    deviceHealth = {
      ...
      staleOriroDeviceIds: addedDevices
        .filter((device) => !device.current && isOriroManagedMatrixDevice(device.displayName))
...
export function registerMatrixCli(params: { program: Command }): void {
  const root = params.program
    ...
    .addHelpText("after", () => "\nDocs: https://docs.oriro.ai/channels/matrix\n");
      ...
      async (options: {
        ...
        await runMatrixCliCommand({
          ...
          onText: (result) => {
            ...
            } else if (result.deviceHealth.staleOriroDeviceIds.length > 0) {
              const staleDeviceIds = result.deviceHealth.staleOriroDeviceIds
            ...
            const bindHint = `oriro agents bind --agent <id> --bind matrix:${result.accountId}`;
