...
function writeExecApprovalsFixture(root: string, file: Record<string, unknown>): void {
  const stateDir = process.env.ORIRO_STATE_DIR ?? path.join(root, "state");
...
describe("exec security floor", () => {
  ...
  beforeEach(() => {
    ...
    tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-exec-security-floor-"));
    setTestEnvValue("HOME", tempRoot);
    setTestEnvValue("USERPROFILE", tempRoot);
    setTestEnvValue("ORIRO_HOME", tempRoot);
    setTestEnvValue("ORIRO_STATE_DIR", path.join(tempRoot, "state"));
