...
extension NodeAppModel {
    static func normalizeWatchNotifyParams(_ params: OriroWatchNotifyParams) -> OriroWatchNotifyParams {
    ...
    static func normalizeWatchActions(
        _ actions: [OriroWatchAction]?,
        kind: String?,
        promptId: String?) -> [OriroWatchAction]
    {
        let provided = (actions ?? []).compactMap { action -> OriroWatchAction? in
            let id = action.id.trimmingCharacters(in: .whitespacesAndNewlines)
            let label = action.label.trimmingCharacters(in: .whitespacesAndNewlines)
            guard !id.isEmpty, !label.isEmpty else { return nil }
            return OriroWatchAction(
                id: id,
                label: label,
        ...
        if normalizedKind.contains("approval") || normalizedKind.contains("approve") {
            return [
                OriroWatchAction(id: "approve", label: "Approve"),
                OriroWatchAction(id: "decline", label: "Decline", style: "destructive"),
                OriroWatchAction(id: "open_phone", label: "Open iPhone"),
                OriroWatchAction(id: "escalate", label: "Escalate"),
            ]
        }

        return [
            OriroWatchAction(id: "done", label: "Done"),
            OriroWatchAction(id: "snooze_10m", label: "Snooze 10m"),
            OriroWatchAction(id: "open_phone", label: "Open iPhone"),
            OriroWatchAction(id: "escalate", label: "Escalate"),
        ]
    }
    ...
    static func normalizedWatchRisk(
        _ risk: OriroWatchRisk?,
        priority: OriroNotificationPriority?) -> OriroWatchRisk?
    {
    ...
    static func normalizedWatchPriority(
        _ priority: OriroNotificationPriority?,
        risk: OriroWatchRisk?) -> OriroNotificationPriority?
    {
