...
} from "@oriro/model-catalog-core/model-catalog-types";
...
import type {
  OriroPluginHookOptions,
  OriroPluginToolFactory,
  OriroPluginToolOptions,
} from "./tool-types.js";
 ...
 */
export type OriroPluginConfigSchema = {
...
export type ProviderAuthResult = {
   ...
   */
  configPatch?: Partial<OriroConfig>;
  defaultModel?: string;
  notes?: string[];
  /**
...
export type ProviderAuthMethod = {
   ...
   * key + OAuth, or region-specific login flows). Oriro uses this to expose
 ...
 *
 * Runs after Oriro resolves an explicit/discovered/dynamic model and before
 ...
 *
 * The helper methods cover the common Oriro auth resolution paths:
 *
 ...
 *
 * Called when OAuth refresh fails and Oriro wants a provider-specific repair
...
/**
 * Provider-owned extra-param normalization before Oriro builds its generic
 ...
 * `buildReplayPolicy`, `normalizeToolSchemas`, and `wrapStreamFn`. Oriro no
...
/**
 * Provider-owned stream wrapper hook after Oriro applies its generic
 ...
 *
 * Return `true` or `false` to override Oriro's built-in provider cache TTL
 ...
 *
 * Runs only after Oriro exhausts normal env/profile/config auth resolution
 ...
 * hint suffix that Oriro should append to the generic `Unknown model`
 * error.
 */
 ...
 *
 * Runs after Oriro loads the discovered model catalog and merges configured
...
export type ProviderOAuthProfileIdRepair = {
   ...
   *
   * When omitted, Oriro falls back to `<provider>:default`.
   */
...
export type ProviderPlugin = {
   ...
   *
   * Oriro calls this only from async model resolution paths. After it
   ...
   *
   * Runs after Oriro performs generic transcript cleanup. Use this for
   ...
   *
   * Use this for transport-family schema cleanup before Oriro registers
  ...
  /**
   * Provider-owned stream wrapper applied after generic Oriro wrappers.
   *
   ...
   *
   * Called after Oriro resolves the raw configured credential but before the
   ...
   * hint than Oriro's generic auth-store guidance.
   */
   ...
   * Oriro's generic `Unknown model` error after catalog/runtime lookup
   * fails.
   */
   ...
   * `errorMessage` when Oriro should surface a provider-specific hint for
   ...
   * merging. Oriro deduplicates by `provider/id`, so plugins only need to
   ...
   * or model exposes a custom set of thinking levels. Oriro stores the
   ...
   * without replacing the full Oriro-owned system prompt.
   */
   ...
   *
   * Runs after Oriro's built-in overlay is resolved and before the
   ...
   *
   * Return undefined/null to keep Oriro's default fallback policy.
   */
   ...
   * rewrites after Oriro has assembled the complete prompt. Return
   ...
   * `output` applies to assistant text deltas/final text before Oriro handles
   ...
   *
   * Oriro uses this when a stored auth profile is already valid and needs to
  ...
  /**
   * Legacy auth-profile ids that should be retired by `oriro doctor`.
   *
  ...
  /**
   * Legacy OAuth profile-id migrations that `oriro doctor` should offer.
   *
   ...
   *
   * Oriro calls this before falling back to the shared `shared model runtime` OAuth
   ...
   * path. Return nothing to keep Oriro's generic error text.
   */
  buildAuthDoctorHint?: (
   ...
   * Oriro falls back to that stored profile.
   */
...
export type OriroPluginGatewayMethod = {
  method: string;
...
export type PluginCommandDiagnosticsSession = {
  ...
  sessionKey?: string;
  /** Ephemeral Oriro session id when available. */
  sessionId?: string;
  /** Transcript file for this Oriro session when available. */
  sessionFile?: string;
...
export type PluginCommandContext = {
  ...
  sessionId?: string;
  /** Transcript file for the active Oriro session when available. */
  sessionFile?: string;
  /** Raw command arguments after the command name */
  args?: string;
  /** The full normalized command body */
  commandBody: string;
  /** Current Oriro configuration */
  config: OriroConfig;
...
export type OriroPluginCommandDefinition = {
...
export type OriroPluginHttpRouteAuth = "gateway" | "plugin";
export type OriroPluginHttpRouteMatch = "exact" | "prefix";
export type OriroPluginGatewayRuntimeScopeSurface = "write-default" | "trusted-operator";

export type OriroPluginHttpRouteHandler = (
  req: IncomingMessage,
  res: ServerResponse,
) => Promise<boolean | void> | boolean | void;

export type OriroPluginHttpRouteUpgradeHandler = (
  req: IncomingMessage,
  socket: Duplex,
  head: Buffer,
) => Promise<boolean | void> | boolean | void;

export type OriroPluginHttpRouteParams = {
  path: string;
  handler: OriroPluginHttpRouteHandler;
  handleUpgrade?: OriroPluginHttpRouteUpgradeHandler;
  auth: OriroPluginHttpRouteAuth;
  match?: OriroPluginHttpRouteMatch;
  gatewayRuntimeScopeSurface?: OriroPluginGatewayRuntimeScopeSurface;
  nodeCapability?: {
    surface: string;
    ttlMs?: number;
  };
...
export type OriroPluginHostedMediaResolver = (
  mediaUrl: string,
) => string | null | undefined | Promise<string | null | undefined>;

export type OriroPluginCliContext = {
  /**
   * Command object where this plugin should register its commands.
   *
   * For root CLI registrations this is the root `oriro` program. For nested
...
export type OriroPluginCliRegistrar = (ctx: OriroPluginCliContext) => void | Promise<void>;

/**
 * Top-level CLI metadata for plugin-owned commands.
 *
 * Descriptors are the parse-time contract for lazy plugin CLI registration.
 * If you want Oriro to keep a plugin command lazy-loaded while still
 ...
 */
export type OriroPluginCliCommandDescriptor = {
  name: string;
  description: string;
  hasSubcommands: boolean;
};

export type OriroPluginNodeCliFeatureOptions = {
  /** Explicit node feature command names owned under `oriro nodes`. */
  commands?: string[];
  /**
   * Parse-time command descriptors for lazy node feature CLI registration.
   *
   * Descriptors are registered under `oriro nodes`, so a descriptor named
   * `"camera"` exposes `oriro nodes camera`.
   */
  descriptors?: OriroPluginCliCommandDescriptor[];
};

export type OriroPluginReloadRegistration = {
  restartPrefixes?: string[];
  hotPrefixes?: string[];
  noopPrefixes?: string[];
};

export type OriroPluginNodeHostCommand = {
  command: string;
  cap?: string;
  dangerous?: boolean;
...
export type OriroPluginNodeInvokeTransportResult =
  | {
      ok: true;
      payload?: unknown;
...
export type OriroPluginNodeInvokeApprovalDecision = "allow-once" | "allow-always" | "deny";

export type OriroPluginNodeInvokePolicyApprovalRuntime = {
  request: (input: {
    title: string;
    description: string;
    ...
    id?: string;
    decision?: OriroPluginNodeInvokeApprovalDecision | null;
  }>;
};

export type OriroPluginNodeInvokePolicyContext = {
  nodeId: string;
  command: string;
  params: unknown;
  timeoutMs?: number;
  ...
  approvals?: OriroPluginNodeInvokePolicyApprovalRuntime;
  invokeNode: (input?: {
    params?: unknown;
    timeoutMs?: number;
    idempotencyKey?: string;
  }) => Promise<OriroPluginNodeInvokeTransportResult>;
};

export type OriroPluginNodeInvokePolicyResult =
  | {
      ok: true;
      payload?: unknown;
      payloadJSON?: string | null;
...
export type OriroPluginNodeInvokePolicy = {
  commands: string[];
  /**
  ...
  handle: (
    ctx: OriroPluginNodeInvokePolicyContext,
  ) => Promise<OriroPluginNodeInvokePolicyResult> | OriroPluginNodeInvokePolicyResult;
};

export type OriroPluginSecurityAuditContext = {
  config: OriroConfig;
...
export type OriroPluginSecurityAuditCollector = (
  ctx: OriroPluginSecurityAuditContext,
) => SecurityAuditFinding[] | Promise<SecurityAuditFinding[]>;

export type OriroGatewayDiscoveryAdvertiseContext = {
...
export type OriroGatewayDiscoveryService = {
  id: string;
  advertise: (
    ctx: OriroGatewayDiscoveryAdvertiseContext,
  ) => void | Promise<void | { stop?: () => void | Promise<void> }>;
};

/** Context passed to long-lived plugin services. */
export type OriroPluginServiceContext = {
  config: OriroConfig;
  workspaceDir?: string;
  stateDir: string;
  logger: PluginLogger;
...
export type OriroPluginService = {
  id: string;
  start: (ctx: OriroPluginServiceContext) => void | Promise<void>;
  stop?: (ctx: OriroPluginServiceContext) => void | Promise<void>;
};

export type OriroPluginChannelRegistration = {
  plugin: ChannelPlugin;
};

/** Module-level plugin definition loaded from a native plugin entry file. */
export type OriroPluginDefinition = {
  id?: string;
  name?: string;
  description?: string;
  version?: string;
  /**
   * @deprecated Declare exclusive plugin kind in `oriro.plugin.json` via
  ...
  configSchema?: OriroPluginConfigSchema;
  reload?: OriroPluginReloadRegistration;
  nodeHostCommands?: OriroPluginNodeHostCommand[];
  securityAuditCollectors?: OriroPluginSecurityAuditCollector[];
  register?: (api: OriroPluginApi) => void;
  activate?: (api: OriroPluginApi) => void;
};

export type OriroPluginModule = OriroPluginDefinition | ((api: OriroPluginApi) => void);
...
export type PluginConfigMigration = (config: OriroConfig) =>
  | {
      config: OriroConfig;
      changes: string[];
    }
...
/** Migration source implemented by a plugin and orchestrated by `oriro migrate`. */
...
export type OriroPluginSessionStateApi = {
...
export type OriroPluginSessionWorkflowApi = {
...
export type OriroPluginSessionControlsApi = {
...
export type OriroPluginSessionApi = {
  state: OriroPluginSessionStateApi;
  workflow: OriroPluginSessionWorkflowApi;
  controls: OriroPluginSessionControlsApi;
};

export type OriroPluginAgentEventsApi = {
...
export type OriroPluginAgentApi = {
  events: OriroPluginAgentEventsApi;
};

export type OriroPluginRunContextApi = {
...
export type OriroPluginLifecycleApi = {
...
export type OriroPluginApi = {
   ...
   */
  session: OriroPluginSessionApi;
  /** Grouped facade for agent-event workflow seams. */
  agent: OriroPluginAgentApi;
  /** Grouped facade for run-scoped plugin scratch state. */
  runContext: OriroPluginRunContextApi;
  /** Grouped facade for plugin-owned lifecycle cleanup hooks. */
  lifecycle: OriroPluginLifecycleApi;
  registerTool: (
    tool: AnyAgentTool | OriroPluginToolFactory,
    opts?: OriroPluginToolOptions,
  ) => void;
  registerHook: (
    events: string | string[],
    handler: InternalHookHandler,
    opts?: OriroPluginHookOptions,
  ) => void;
  registerHttpRoute: (params: OriroPluginHttpRouteParams) => void;
  /** Register a plugin-owned resolver for browser-style hosted media URLs. */
  registerHostedMediaResolver: (resolver: OriroPluginHostedMediaResolver) => void;
  /** Register a native messaging channel plugin (channel capability). */
  registerChannel: (registration: OriroPluginChannelRegistration | ChannelPlugin) => void;
  /**
  ...
  registerCli: (
    registrar: OriroPluginCliRegistrar,
    opts?: {
       ...
       *
       * When descriptors cover every command exposed at `parentPath`, Oriro
  ...
  /**
   * Register a plugin-owned node feature command group under `oriro nodes`.
   *
  ...
  registerNodeCliFeature: (
    registrar: OriroPluginCliRegistrar,
    opts?: OriroPluginNodeCliFeatureOptions,
  ) => void;
  registerReload: (registration: OriroPluginReloadRegistration) => void;
  registerNodeHostCommand: (command: OriroPluginNodeHostCommand) => void;
  registerNodeInvokePolicy: (policy: OriroPluginNodeInvokePolicy) => void;
  registerSecurityAuditCollector: (collector: OriroPluginSecurityAuditCollector) => void;
  registerService: (service: OriroPluginService) => void;
  /** Register a local gateway discovery advertiser such as mDNS/Bonjour. */
  registerGatewayDiscoveryService: (service: OriroGatewayDiscoveryService) => void;
  ...
  /** Register an importer for `oriro migrate` (migration capability). */
   ...
   */
  registerCommand: (command: OriroPluginCommandDefinition) => void;
