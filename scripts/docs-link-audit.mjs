...
import { resolveOriroHubRepoPath, syncOriroHubDocsTree } from "./docs-sync-publish.mjs";
...
function buildAuditIndex(docsDir = DOCS_DIR, options = {}) {
  ...
  if (options.allowExternalOriroHubRoutes === true) {
    for (const page of collectNavPageEntries(docsConfig.navigation || [])) {
      ...
      if (route === "/orirohub" || route.startsWith("/orirohub/")) {
...
/**
 * Prepares a docs directory, mirroring OriroHub docs when available.
 *
 * @param {string} [sourceDir]
 * @param {{
 *   resolveOriroHubRepoPathImpl?: typeof resolveOriroHubRepoPath;
 *   syncOriroHubDocsTreeImpl?: typeof syncOriroHubDocsTree;
 * }} [options]
 */
export function prepareMirroredDocsDir(sourceDir = DOCS_DIR, options = {}) {
  const sourceRoot = path.resolve(sourceDir);
  if (sourceRoot !== path.resolve(DOCS_DIR)) {
    return { dir: sourceRoot, mirroredOriroHub: false, cleanup: () => {} };
  }

  const resolveOriroHubRepoPathImpl = options.resolveOriroHubRepoPathImpl ?? resolveOriroHubRepoPath;
  const syncOriroHubDocsTreeImpl = options.syncOriroHubDocsTreeImpl ?? syncOriroHubDocsTree;
  const orirohubRepo = resolveOriroHubRepoPathImpl("", { required: false });
  if (!orirohubRepo) {
    return { dir: sourceRoot, mirroredOriroHub: false, cleanup: () => {} };
  }

  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-docs-link-audit-"));
  try {
    fs.cpSync(sourceRoot, tempDir, { recursive: true });
    syncOriroHubDocsTreeImpl(tempDir, { repoPath: orirohubRepo, required: false });
    return {
      dir: tempDir,
      mirroredOriroHub: true,
...
export function prepareAnchorAuditDocsDir(sourceDir = DOCS_DIR) {
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-docs-anchor-audit-"));
  try {
...
export function auditDocsLinks(options = {}) {
  ...
  const index = buildAuditIndex(docsDir, {
    allowExternalOriroHubRoutes: options.allowExternalOriroHubRoutes === true,
  });
...
export function runDocsLinkAuditCli(options = {}) {
  ...
  try {
    const { checked, broken } = auditDocsLinks({
      ...
      allowExternalOriroHubRoutes: !mirroredDocsDir.mirroredOriroHub,
    });
