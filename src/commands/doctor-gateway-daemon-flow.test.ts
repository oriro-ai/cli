...
vi.mock("../daemon/constants.js", () => ({
  resolveGatewayLaunchAgentLabel: vi.fn(() => "ai.oriro.gateway"),
  resolveNodeLaunchAgentLabel: vi.fn(() => "ai.oriro.node"),
}));
...
describe("maybeRepairGatewayDaemon", () => {
  ...
  it("reports recent restart handoffs during deep doctor", async () => {
    ...
    service.readCommand.mockResolvedValueOnce({
      ...
      environment: {
        ORIRO_STATE_DIR: "/tmp/oriro-service",
        ORIRO_CONFIG_PATH: "/tmp/oriro-service/oriro.json",
      },
    });
    ...
    const [handoffEnv] = readGatewayRestartHandoffSync.mock.calls[0] as unknown as [
      { ORIRO_STATE_DIR?: string; ORIRO_CONFIG_PATH?: string },
    ];
    expect(handoffEnv?.ORIRO_STATE_DIR).toBe("/tmp/oriro-service");
    expect(handoffEnv?.ORIRO_CONFIG_PATH).toBe("/tmp/oriro-service/oriro.json");
    expect(note).toHaveBeenCalledWith(
  ...
  it("suppresses busy-port note for expected Gateway listeners", async () => {
    setPlatform("linux");
    const listeners = [{ pid: 5001, commandLine: "oriro-gateway", address: "0.0.0.0:18789" }];
    inspectPortUsage.mockResolvedValue({
      port: 18789,
      status: "busy",
      listeners,
  ...
  it("keeps busy-port note for unexpected Gateway listeners", async () => {
    ...
    inspectPortUsage.mockResolvedValue({
      ...
      status: "busy",
      listeners: [
        { pid: 5001, commandLine: "oriro-gateway", address: "0.0.0.0:18789" },
        { pid: 5002, commandLine: "oriro-gateway", address: "127.0.0.1:18789" },
      ],
  ...
  it("skips gateway install during non-interactive doctor repairs", async () => {
    ...
    expect(service.restart).not.toHaveBeenCalled();
    expect(note).toHaveBeenCalledWith(
      `Run ${formatCliCommand("oriro gateway install")} when you want to install the gateway service.`,
      "Gateway",
    );
  });
  ...
  it("skips gateway service install when a system Oriro gateway service exists", async () => {
    setPlatform("linux");
    service.isLoaded.mockResolvedValue(false);
    findSystemGatewayServices.mockResolvedValue([
      {
        ...
        detail: "unit: /etc/systemd/system/oriro-gateway.service",
        scope: "system",
        marker: "oriro",
        legacy: false,
      },
    ...
    expect(note).toHaveBeenCalledWith(
      [
        ...
        "- oriro-gateway.service (unit: /etc/systemd/system/oriro-gateway.service)",
