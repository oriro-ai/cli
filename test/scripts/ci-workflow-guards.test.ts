...
describe("ci workflow guards", () => {
  it("makes the hosted release-gate fallback explicit and exact-SHA only", () => {
    ...
    expect(readFileSync(".github/workflows/ci.yml", "utf8")).toContain(
      "ORIRO_CI_RUN_ANDROID: ${{ github.event_name == 'workflow_dispatch' && (inputs.release_gate || inputs.include_android) && 'true' || steps.changed_scope.outputs.run_android || 'false' }}",
    );
  });
  ...
  it("kills timed manual checkout fetches after the grace period", () => {
    ...
    for (const [workflowPath, timeoutSeconds] of workflowPaths) {
      ...
      const fetchTimeouts = workflow.match(
        new RegExp(
          `timeout --signal=TERM[^\\n]* ${timeoutSeconds} git(?: -C "(?:\\$workdir|\\$GITHUB_WORKSPACE|orirohub-source)")?`,
          "g",
        ),
      );
  ...
  it("fails and retries quiet Node test shard stalls quickly", () => {
    ...
    expect(runStep.env.ORIRO_VITEST_NO_OUTPUT_TIMEOUT_MS).toBe("300000");
    expect(runStep.env.ORIRO_VITEST_NO_OUTPUT_RETRY).toBe("1");
    expect(runStep.env.ORIRO_TEST_PROJECTS_PARALLEL).toBe("2");
  });
