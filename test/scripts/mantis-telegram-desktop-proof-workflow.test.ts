...
describe("Mantis Telegram Desktop proof workflow", () => {
  ...
  it("releases Telegram Desktop proof leases left by interrupted agents", () => {
    ...
    expect(codexStep.env?.ORIRO_QA_CREDENTIAL_OWNER_ID).toContain(
      "mantis-telegram-desktop-${{ github.run_id }}-${{ github.run_attempt }}",
    );
    expect(workflowStep("Prepare Codex user").run).toContain("ORIRO_QA_CREDENTIAL_OWNER_ID");
    ...
    expect(cleanupStep.env?.ORIRO_QA_CONVEX_SECRET_CI).toContain(
      "secrets.ORIRO_QA_CONVEX_SECRET_CI",
    );
    expect(cleanupStep.env?.ORIRO_QA_CONVEX_SITE_URL).toContain(
      "secrets.ORIRO_QA_CONVEX_SITE_URL",
    );
  ...
  it("uses the Oriro Mantis mention as the comment trigger", () => {
    const workflow = readFileSync(WORKFLOW, "utf8");
    const liveWorkflow = readFileSync(LIVE_WORKFLOW, "utf8");
    expect(workflow).toContain("@oriro-mantis");
    expect(workflow).toContain("/oriro-mantis");
  ...
  it("runs when OriroSweeper applies the Telegram proof label", () => {
    const workflow = parse(readFileSync(WORKFLOW, "utf8")) as Workflow;
    ...
    expect(workflowText).toContain("allow-bot-users: orirosweeper[bot]");
  });
  ...
  it("installs local proof tools before the Codex agent runs", () => {
    ...
    expect(install.run).toContain("/usr/local/bin/oriro-telegram-user-crabbox-proof");
    ...
    expect(agent.env?.ORIRO_TELEGRAM_USER_DRIVER_SCRIPT).toBe(
      "${{ github.workspace }}/scripts/e2e/telegram-user-driver.py",
    );
    expect(agent.env?.ORIRO_TELEGRAM_USER_PROOF_CMD).toBe(
      "/usr/local/bin/oriro-telegram-user-crabbox-proof",
    );
    expect(agent.env?.ORIRO_TELEGRAM_USER_CRABBOX_BIN).toBe("/usr/local/bin/crabbox");
    expect(agent.env?.CRABBOX_COORDINATOR).toContain(
      "secrets.CRABBOX_COORDINATOR || secrets.ORIRO_QA_MANTIS_CRABBOX_COORDINATOR",
    );
    expect(agent.env?.CRABBOX_COORDINATOR_TOKEN).toContain(
      "secrets.CRABBOX_COORDINATOR_TOKEN || secrets.ORIRO_QA_MANTIS_CRABBOX_COORDINATOR_TOKEN",
    );

    const prepare = workflowStep("Prepare Codex user");
    expect(prepare.run).toContain(
      "ORIRO_TELEGRAM_USER_CRABBOX_BIN ORIRO_TELEGRAM_USER_CRABBOX_PROVIDER ORIRO_TELEGRAM_USER_DRIVER_SCRIPT ORIRO_TELEGRAM_USER_PROOF_CMD",
    );
    expect(prepare.run).toContain("MANTIS_CANDIDATE_TRUST");

    const prompt = readFileSync(PROMPT, "utf8");
    expect(prompt).toContain("$ORIRO_TELEGRAM_USER_PROOF_CMD");
