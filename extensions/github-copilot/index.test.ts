...
import { MAX_DATE_TIMESTAMP_MS, MAX_TIMER_TIMEOUT_MS } from "oriro/plugin-sdk/number-runtime";
import type {
  OriroConfig,
  OriroPluginApi,
  ProviderAuthResult,
...
type RegisteredMemoryEmbeddingProvider = Parameters<
  OriroPluginApi["registerMemoryEmbeddingProvider"]
>[0];
type RegisteredProvider = Parameters<OriroPluginApi["registerProvider"]>[0];
type GithubCopilotTestProvider = RegisteredProvider & {
  auth: Array<{
    run: (ctx: unknown) => Promise<ProviderAuthResult | null>;
    runNonInteractive: (ctx: unknown) => Promise<OriroConfig | null>;
  }>;
  catalog: {
...
async function createAgentDir() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-github-copilot-test-"));
  tempDirs.push(dir);
  return dir;
}
...
function registerProviderAndCatalogWithPluginConfig(pluginConfig: Record<string, unknown>) {
  const registerProviderMock = vi.fn<OriroPluginApi["registerProvider"]>();
  const registerModelCatalogProviderMock =
    vi.fn<OriroPluginApi["registerModelCatalogProvider"]>();
...
describe("github-copilot plugin", () => {
  ...
  it("registers embedding provider", () => {
    const registerMemoryEmbeddingProviderMock =
      vi.fn<OriroPluginApi["registerMemoryEmbeddingProvider"]>();
