...
function createClaudeCliOverrideConfig(config: CliBackendConfig): OriroConfig {
  return {
    agents: {
      defaults: {
        cliBackends: {
...
describe("resolveCliBackendConfig claude-cli defaults", () => {
  it("derives bypassPermissions from Oriro's default YOLO exec policy", () => {
    ...
    expect(resolved?.config.args).toContain("mcp__oriro__*");
    ...
    expect(resolved?.config.resumeArgs).toContain("mcp__oriro__*");
  ...
  it("keeps Claude permission mode unset when Oriro exec policy is not YOLO", () => {
    const resolved = requireCliBackendConfig("claude-cli", {
