...
describe("provider transport stream contracts", () => {
  it("covers the supported transport api alias matrix", () => {
    // Supported APIs can be projected to Oriro transport aliases when needed.
    const cases = [
      {
        ...
        alias: "oriro-openai-responses-transport",
      },
      {
      ...
      },
      {
        ...
        alias: "oriro-openai-completions-transport",
      },
      {
        ...
        alias: "oriro-azure-openai-responses-transport",
      },
      {
        ...
        alias: "oriro-anthropic-messages-transport",
      },
      {
        ...
        alias: "oriro-google-generative-ai-transport",
  ...
  it("keeps OpenAI API-key default streams on Oriro transport", () => {
    const cases = [
    ...
    for (const model of cases) {
      expect(createBoundaryAwareStreamFnForModel(model)).toBeTypeOf("function");
      expect(createOriroTransportStreamFnForModel(model)).toBeTypeOf("function");
      expect(createTransportAwareStreamFnForModel(model)).toBeUndefined();
      expect(buildTransportAwareSimpleStreamFn(model)).toBeUndefined();
      expect(prepareTransportAwareSimpleModel(model)).toBe(model);
    }
  ...
  it("routes localService models through the Oriro simple-completion transport", () => {
    ...
    expect(buildTransportAwareSimpleStreamFn(model)).toBeTypeOf("function");
    const preparedModel = prepareTransportAwareSimpleModel(model);
    expect(preparedModel.api).toBe("oriro-openai-completions-transport");
    expect(preparedModel.provider).toBe("inferrs");
    expect(preparedModel.id).toBe("google/gemma-4-E2B-it");
  });

  it("keeps Codex defaults on the Oriro transport until Oriro preserves attribution", () => {
