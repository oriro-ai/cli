---
summary: "CLI reference for `oriro policy` conformance checks"
read_when:
  - You want to check Oriro settings against an authored policy.jsonc
...
`oriro policy` is provided by the bundled Policy plugin. Policy is an
enterprise conformance layer over existing Oriro settings. It does not add a
second configuration system. `policy.jsonc` defines authored requirements,
Oriro observes the active workspace as evidence, and policy health checks
...
data-handling posture, Oriro config secret provider/auth profile posture, and governed tool
...
posture, require Oriro config SecretRefs to use managed providers, require
...
repeatable way to prove that Oriro still conforms to that statement. Use
...
exposure, agent workspace posture, configured sandbox runtime posture, Oriro
...
when a concrete rule is present. Oriro reads current `channels.*` settings
...
posture, Oriro config agent sandbox workspace access and tool deny posture,
...
`edit`, and `apply_patch`; Oriro config `group:fs` covers file mutation tools
...
present in `policy.jsonc`. The observed state is existing Oriro config or
...
present in `agents.list[]`, Oriro evaluates the scoped rule against inherited
...
Container posture policy is evaluated only against evidence Oriro can
...
artifact. By default this is `~/.oriro/exec-approvals.json`; when
`ORIRO_STATE_DIR` is set, Policy reads
`$ORIRO_STATE_DIR/exec-approvals.json`. Actual posture rules such as
...
```bash
oriro policy check
oriro policy check --json
oriro policy check --severity-min error
```

`policy check` runs only the policy check set and emits evidence, findings, and
attestation hashes. The same findings also appear in `oriro doctor --lint`
when the Policy plugin is enabled.

Compare an operator policy file to an authored baseline policy file:

```bash
oriro policy compare --baseline official.policy.jsonc
oriro policy compare --baseline official.policy.jsonc --policy policy.jsonc --json
```

`policy compare` compares policy file syntax to policy file syntax. It does not
inspect Oriro runtime state, evidence, credentials, or secrets. The command
...
```json
{
  ...
  "evidence": {
    "channels": [
      {
        "id": "telegram",
        "provider": "telegram",
        "source": "oc://oriro.config/channels/telegram",
        "enabled": false
      }
    ],
    "mcpServers": [
      {
        "id": "docs",
        "transport": "stdio",
        "source": "oc://oriro.config/mcp/servers/docs",
        "command": "npx"
      }
    ],
    "modelProviders": [
      {
        "id": "openai",
        "source": "oc://oriro.config/models/providers/openai"
      }
    ],
    "modelRefs": [
      {
        ...
        "provider": "openai",
        "model": "gpt-5.5",
        "source": "oc://oriro.config/agents/defaults/model"
      }
    ],
    "network": [
      {
        "id": "browser-private-network",
        "source": "oc://oriro.config/browser/ssrfPolicy/dangerouslyAllowPrivateNetwork",
        "value": false
      }
    ],
    "gatewayExposure": [
      {
        "id": "gateway-bind",
        "kind": "bind",
        "source": "oc://oriro.config/gateway/bind",
        "value": "loopback",
        "nonLoopback": false,
        "explicit": true
      }
    ...
    "agentWorkspace": [
      {
        ...
        "value": "ro",
        "sandboxMode": "all",
        "sandboxModeSource": "oc://oriro.config/agents/defaults/sandbox/mode",
        "sandboxEnabled": true,
        "explicit": true
      },
      {
    ...
    "secrets": [
      {
        "id": "vault",
        "kind": "provider",
        "source": "oc://oriro.config/secrets/providers/vault",
        "providerSource": "env"
      },
      {
        "id": "oc://oriro.config/models/providers/openai/apiKey",
        "kind": "input",
    ...
    "authProfiles": [
      {
        "id": "github",
        "source": "oc://oriro.config/auth/profiles/github",
        "validMetadata": true,
        "provider": "github",
        "mode": "token"
      }
...
records the observed Oriro state used by the policy checks. The
...
5. Re-run `oriro doctor --lint` in CI or release gates.
...
`oriro policy watch` runs the same check repeatedly and reports when the
current evidence no longer matches `expectedAttestationHash`:

```bash
oriro policy watch --json
```
...
```json
{
  ...
  "target": "oc://oriro.config/mcp/servers/remote",
...
```json
{
  ...
  "target": "oc://oriro.config/agents/defaults/model/fallbacks/#0",
...
```json
{
  ...
  "target": "oc://oriro.config/browser/ssrfPolicy/dangerouslyAllowPrivateNetwork",
...
```json
{
  ...
  "target": "oc://oriro.config/gateway/bind",
...
```json
{
  ...
  "target": "oc://oriro.config/agents/defaults/sandbox/workspaceAccess",
...
In this version, repair can disable channels that are enabled in Oriro config
