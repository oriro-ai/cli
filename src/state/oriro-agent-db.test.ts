// Oriro agent database tests cover agent-scoped DB storage and migrations.
...
describe("oriro agent database", () => {
  it("resolves under the per-agent state directory", () => {
    ...
    ).toBe(path.join(stateDir, "agents", "worker-1", "agent", "oriro-agent.sqlite"));
  });
  ...
  it("creates the per-agent schema and registers it globally", () => {
    const stateDir = createTempStateDir();
    const database = openOriroAgentDatabase({
      agentId: "worker-1",
      env: { ORIRO_STATE_DIR: stateDir },
    });

    expect(collectSqliteSchemaShape(database.db)).toEqual(
      createSqliteSchemaShapeFromSql(new URL("./oriro-agent-schema.sql", import.meta.url)),
    );
  ...
  it("keeps multiple registered paths for the same agent", () => {
    ...
    const relocated = openOriroAgentDatabase({
      agentId: "worker-1",
      env,
      path: relocatedPath,
    });
    const defaultDatabase = openOriroAgentDatabase({
      agentId: "worker-1",
      env,
    });
  ...
  it("rejects the legacy agent registry primary key with a doctor repair hint", () => {
    ...
    ).toThrow(/run oriro doctor --fix/);
  });

  it("keys explicit relative paths by resolved database pathname", () => {
    const agentModuleUrl = new URL("./oriro-agent-db.ts", import.meta.url).href;
    const stateModuleUrl = new URL("./oriro-state-db.ts", import.meta.url).href;
    const output = execFileSync(
      ...
      [
        ...
        `
          ...
          const env = { ORIRO_STATE_DIR: stateDir };
          const root = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-agent-db-relative-"));
          ...
          try {
            ...
            const first = openOriroAgentDatabase({
              agentId: "worker-1",
              env,
              path: "agent.sqlite",
            });
            ...
            const second = openOriroAgentDatabase({
              agentId: "worker-1",
              env,
              path: "agent.sqlite",
            });
  ...
  it("rejects explicit paths that point at the global state database", () => {
    ...
    const stateDatabase = openOriroStateDatabase({
      env,
      path: databasePath,
    });
    closeOriroStateDatabaseForTest();
    ...
    const reopenedStateDatabase = openOriroStateDatabase({
      env,
      path: databasePath,
    });
  ...
  it("does not chmod shared parent directories for explicit database paths", () => {
    ...
    const env = { ORIRO_STATE_DIR: parentDir };
