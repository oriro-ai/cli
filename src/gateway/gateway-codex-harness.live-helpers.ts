...
export function isExpectedCodexStatusCommandText(text: string): boolean {
  ...
  const mentionsOriroStatus =
    normalized.includes("oriro is running on") ||
    /oriro\s+\S+\s+is running on/u.test(normalized) ||
    normalized.includes("oriro status:") ||
  ...
  return (
    ...
    isReadyStatus ||
    isOnlineIdleStatus ||
    (mentionsOriroStatus && mentionsHarnessSession && mentionsModel)
  );
}
...
export function isExpectedCodexModelsCommandText(text: string): boolean {
  ...
  const mentionsSessionModel =
    ...
    normalized.includes("current session model from oriro status") ||
  ...
  const mentionsConfigSummary =
    ...
    normalized.includes("live oriro config shows") ||
