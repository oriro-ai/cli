...
function makeTempDir(): string {
  const root = mkdtempSync(path.join(tmpdir(), "oriro-parallels-npm-update-"));
  tempDirs.push(root);
  return root;
}
...
function extractWindowsBackgroundControlMarkers(decoded: string): {
  ...
  return {
    done: marker("__ORIRO_BACKGROUND_DONE__", false),
    exitPrefix: marker("__ORIRO_BACKGROUND_EXIT__", true),
    lengthPrefix: marker("__ORIRO_LOG_LENGTH__", true),
    offsetPrefix: marker("__ORIRO_LOG_OFFSET__", true),
  };
}
...
describe("parallels npm update smoke", () => {
  it("accepts one prepared tarball target for update and fresh install", () => {
    expect(parseArgs(["--target-tarball", "/tmp/oriro-candidate.tgz"])).toMatchObject({
      targetTarball: "/tmp/oriro-candidate.tgz",
      updateTarget: "",
      freshTargetSpec: undefined,
    });
    expect(() =>
      parseArgs(["--target-tarball", "/tmp/oriro-candidate.tgz", "--update-target", "beta"]),
...
printf '%s\\n' "$*" >>"$log_path"
args=" $* "
if [[ "$args" == *" /usr/bin/tee /tmp/oriro-parallels-npm-update-linux-"* ]]; then
  cat >/dev/null
  exit 0
fi
if [[ "$args" == *" /bin/chmod 755 /tmp/oriro-parallels-npm-update-linux-"* ]]; then
  echo "chmod denied" >&2
  exit 7
fi
if [[ "$args" == *" /bin/rm -f /tmp/oriro-parallels-npm-update-linux-"* ]]; then
  printf 'cleanup\\n' >>"$log_path"
  exit 0
fi
exit 1
`,
      ...
      () => {
        ...
        expect(() =>
          writeGuestScript.call(
            ...
            "Linux VM",
            "echo update",
            "oriro-parallels-npm-update-linux",
          ),
    ...
    expect(log).toContain("/bin/chmod 755 /tmp/oriro-parallels-npm-update-linux-");
    expect(log).toContain("/bin/rm -f /tmp/oriro-parallels-npm-update-linux-");
  ...
  it("has a one-command beta validation mode with fresh target coverage", () => {
    ...
    expect(script).toContain("resolveOriroRegistryVersion");
    expect(script).toContain("this.options.updateTarget = version");
    expect(script).toContain("this.options.freshTargetSpec = `oriro@${version}`");
  ...
  it("accepts keyed and nested npm metadata for published update targets", () => {
    ...
    expect(
      parseRegistryPackageMetadata(
        JSON.stringify({
          ...
          "dist.tarball": "https://registry.example/oriro-keyed.tgz",
          gitHead: "abcdef0123456789",
        }),
      ),
    ).toEqual({
      version: "2026.5.20-beta.1",
      tarball: "https://registry.example/oriro-keyed.tgz",
      gitHead: "abcdef0123456789",
    });
    ...
    expect(
      parseRegistryPackageMetadata(
        JSON.stringify({
          version: "2026.5.20-beta.1",
          dist: { tarball: "https://registry.example/oriro-nested.tgz" },
        }),
      ),
    ).toEqual({
      version: "2026.5.20-beta.1",
      tarball: "https://registry.example/oriro-nested.tgz",
      gitHead: "",
    });
  });
  ...
  it("bounds POSIX guest failure logs", () => {
    ...
    expect(scripts).toContain("ORIRO_PARALLELS_NPM_UPDATE_LOG_TAIL_BYTES");
    expect(scripts).toContain('print_log_tail "$output_file"');
    expect(scripts).toContain("print_log_tail /tmp/oriro-parallels-macos-gateway.log >&2");
    expect(scripts).toContain("print_log_tail /tmp/oriro-parallels-linux-gateway.log >&2");
    expect(scripts).not.toContain('cat "$output_file"');
    expect(scripts).not.toContain("cat /tmp/oriro-parallels-");
  });
  ...
  it("passes platform model timeouts to POSIX update agent turns", () => {
    ...
    withEnv(
      {
        ORIRO_PARALLELS_LINUX_MODEL_TIMEOUT_S: undefined,
        ORIRO_PARALLELS_MACOS_MODEL_TIMEOUT_S: undefined,
        ORIRO_PARALLELS_MODEL_TIMEOUT_S: undefined,
      },
      () => {
    ...
    withEnv(
      {
        ORIRO_PARALLELS_LINUX_MODEL_TIMEOUT_S: "321",
        ORIRO_PARALLELS_MACOS_MODEL_TIMEOUT_S: "654",
      },
      () => {
  ...
  it("sets platform-aware fresh lane timeouts", () => {
    withEnv({ ORIRO_PARALLELS_NPM_UPDATE_FRESH_TIMEOUT_S: undefined }, () => {
  ...
  it("clears update stream timers when spawning the guest command fails", async () => {
    ...
    await expect(
      runStreamingToJobLog.call(smoke, "oriro-definitely-missing-command", [], 60 * 60 * 1000, {
        append: () => undefined,
        logPath: "",
  ...
  it("cleans timed-out Windows background work and reads bounded log chunks", async () => {
    ...
    expect(commands).toContain("function Write-OriroUtf8File");
    expect(commands).toContain("[System.Text.UTF8Encoding]::new($false)");
    expect(payloads).toContain("Write-OriroUtf8File $exitPath '0'");
    expect(payloads).toContain("Write-OriroUtf8File $donePath 'done'");
    expect(commands).toContain("Write-OriroUtf8File $pidPath ([string]$process.Id)");
    ...
    expect(commands).toContain("Stop-OriroBackgroundProcessTree ([int]$backgroundPid)");
  ...
  it("does not treat Windows background log text as completion control", async () => {
    ...
    const fakeRun: typeof hostCommandRun = (_command, args) => {
      ...
      if (decoded.includes("__ORIRO_LOG_LENGTH__")) {
        ...
        return {
          ...
          stdout: [
            ...
            "__ORIRO_BACKGROUND_EXIT__:0",
            "__ORIRO_BACKGROUND_DONE__",
            "",
          ].join("\n"),
        };
      }
    ...
    expect(decodedCommands.join("\n")).toContain(
      "Stop-OriroBackgroundProcessTree ([int]$backgroundPid)",
    );
  });
  ...
  it("drains completed Windows background logs before cleanup", async () => {
    ...
    expect(decodedCommands.join("\n")).not.toContain("Stop-OriroBackgroundProcessTree");
    expect(decodedCommands.join("\n")).toContain(
      "Remove-Item -Path $scriptPath, $logPath, $donePath, $exitPath, $pidPath",
    );
  });
...
if [[ "$args" == *" /usr/bin/dscl . -list /Users NFSHomeDirectory"* ]]; then
  printf '%s\\n' '_daemon /var/root'
  printf '%s\\n' 'orirouser /Volumes/Macintosh HD/Users/orirouser'
  exit 0
fi
exit 7
`,
    ...
    withEnv(
      ...
      () => {
        ...
        expect(resolveMacosDesktopUser.call(smoke)).toBe("orirouser");
      },
    );
  });
...
set -euo pipefail
args=" $* "
if [[ "$args" == *" /usr/bin/dscl . -read /Users/orirouser NFSHomeDirectory"* ]]; then
  printf '%s\\n' 'NFSHomeDirectory: /Volumes/Macintosh HD/Users/orirouser'
  exit 0
fi
exit 7
`,
    ...
    withEnv(
      ...
      () => {
        ...
        expect(resolveMacosDesktopHome.call(smoke, "orirouser")).toBe(
          "/Volumes/Macintosh HD/Users/orirouser",
        );
      },
    );
  });
  ...
  it("scrubs future plugin entries before invoking old same-guest updaters", () => {
    ...
    expect(script).toContain("Invoke-WithScopedEnv @{ ORIRO_DISABLE_BUNDLED_PLUGINS = '1'");
    expect(macosScript).toContain('ORIRO_BIN="$(resolve_required_command oriro)"');
    expect(macosScript).toContain("/usr/local/bin:/usr/local/sbin");
    expect(macosScript).toContain(
      'ORIRO_DISABLE_BUNDLED_PLUGINS=1 "$ORIRO_BIN" update --tag',
    );
    expect(macosScript).not.toContain("/opt/homebrew/bin/oriro");
    expect(script).toContain("ORIRO_DISABLE_BUNDLED_PLUGINS=1 oriro update --tag");
    expect(macosScript).toContain(
      'ORIRO_DISABLE_BUNDLED_PLUGINS=1 "$ORIRO_BIN" gateway stop',
    );
    expect(script).toContain(
      "ORIRO_DISABLE_BUNDLED_PLUGINS=1 ORIRO_ALLOW_ROOT=1 oriro gateway stop",
    );
  });
  ...
  it("reenables bundled plugins before Windows post-update verification", () => {
    ...
    const updateIndex = script.indexOf("Invoke-Oriro update --tag");
    const scopedIndex = script.indexOf("Invoke-WithScopedEnv @{ ORIRO_DISABLE_BUNDLED_PLUGINS");
    const versionIndex = script.indexOf("Invoke-Oriro --version", scopedIndex);
    const restartIndex = script.indexOf("Invoke-Oriro gateway restart");
    const agentIndex = script.indexOf("Invoke-Oriro agent --local");
    ...
    expect(script).not.toContain("$env:ORIRO_DISABLE_BUNDLED_PLUGINS = '1'");
  });
  ...
  it("generates a .NET-safe Windows stale import regex in the update-failure guard", () => {
    ...
    expect(staleImportPattern).toBe(
      String.raw`node_modules\\oriro\\dist\\[^\\]+-[A-Za-z0-9_-]+\.js`,
    );
    expect(staleImportPattern).not.toContain("node_modules\\oriro\\dist\\");
    expect(staleImportPattern.match(/\\\\/g)).toHaveLength(4);
    const representativeUpdateFailure = String.raw`Error [ERR_MODULE_NOT_FOUND]: Cannot find module 'C:\Users\runner\AppData\Roaming\npm\node_modules\oriro\dist\main-a1_B2.js' imported from C:\Users\runner\AppData\Roaming\npm\node_modules\oriro\dist\cli.js`;
    const generatedRegex = new RegExp(staleImportPattern);
    expect(generatedRegex.test(representativeUpdateFailure)).toBe(true);
    expect(generatedRegex.test(String.raw`node_modules\oriro\dist\main.js`)).toBe(false);
  });
});
