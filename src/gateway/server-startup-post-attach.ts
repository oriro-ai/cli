...
import type { loadOriroPlugins } from "../plugins/loader.js";
...
const SKIP_STARTUP_MODEL_PREWARM_ENV = "ORIRO_SKIP_STARTUP_MODEL_PREWARM";
...
function resolveGatewayMemoryStartupPolicy(cfg: OriroConfig): GatewayMemoryStartupPolicy {
...
async function resolveRestartSentinelPathFast(
  ...
  const resolveHome = () => {
    const explicitHome = normalizePathEnv(env.ORIRO_HOME);
    if (explicitHome) {
  ...
  const override = normalizePathEnv(env.ORIRO_STATE_DIR);
  if (override) {
    return path.join(resolveUserPath(override), RESTART_SENTINEL_FILENAME);
  }
  const home = resolveHome();
  const newStateDir = path.join(home, ".oriro");
  if (env.ORIRO_TEST_FAST === "1" || (await pathExists(newStateDir))) {
...
function hasGatewayStartHooks(pluginRegistry: ReturnType<typeof loadOriroPlugins>): boolean {
...
async function prewarmConfiguredPrimaryModel(params: {
  ...
  const { normalizeProviderId } = await import("@oriro/model-catalog-core/provider-id");
  if (
  ...
  const { ensureOriroModelsJson } = await import("../agents/models-config.js");
  const agentDir = resolveDefaultAgentDir(params.cfg);
  const workspaceDir =
    params.workspaceDir ?? resolveAgentWorkspaceDir(params.cfg, resolveDefaultAgentId(params.cfg));
  try {
    await ensureOriroModelsJson(params.cfg, agentDir, {
      workspaceDir,
...
export async function startGatewaySidecars(params: {
  cfg: OriroConfig;
  pluginRegistry: ReturnType<typeof loadOriroPlugins>;
    ...
    if (!skipChannels) {
      ...
      await measureStartup(params.startupTrace, "sidecars.channel-skip", () =>
        params.logChannels.info(
          "skipping channel start (ORIRO_SKIP_CHANNELS=1 or ORIRO_SKIP_PROVIDERS=1)",
        ),
      );
    }
  });
      ...
      await measureStartup(params.startupTrace, "sidecars.acp.identity-reconcile", async () => {
        const [{ getAcpSessionManager }, { ACP_SESSION_IDENTITY_RENDERER_VERSION }] =
          await Promise.all([
            ...
            import("@oriro/acp-core/runtime/session-identifiers"),
          ]);
...
export async function startGatewayPostAttachRuntime(
  params: {
    ...
    gatewayPluginConfigAtStart: OriroConfig;
    pluginRegistry: ReturnType<typeof loadOriroPlugins>;
    defaultWorkspaceDir: string;
    deps: CliDeps;
    startChannels: () => Promise<void>;
