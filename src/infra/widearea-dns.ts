...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.ORIRO_WIDE_AREA_DOMAIN ?? null;
  try {
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*oriro-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "oriro";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "oriro");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "oriro-gw");
  ...
  records.push(`_oriro-gw._tcp IN PTR ${instanceLabel}._oriro-gw._tcp`);
  records.push(`${instanceLabel}._oriro-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._oriro-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; oriro-content-hash: ${contentHash}\n${contentBody}`;
}
