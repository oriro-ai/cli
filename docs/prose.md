...
summary: "OpenProse is a markdown-first workflow format for multi-agent AI sessions. In Oriro it ships as a plugin with a /prose slash command and a skill pack."
read_when:
  - You want to run or write .prose workflow files
  - You want to enable the OpenProse plugin
  - You need to understand how OpenProse maps to Oriro primitives
---

OpenProse is a portable, markdown-first workflow format for orchestrating AI
sessions. In Oriro it ships as a plugin that installs an OpenProse skill
...
<Steps>
  <Step title="Enable the plugin">
    ...
    ```bash
    oriro plugins enable open-prose
    ```
...
For a local checkout: `oriro plugins install ./path/to/local/open-prose-plugin`
...
## Oriro runtime mapping

OpenProse programs map to Oriro primitives:

| OpenProse concept         | Oriro tool    |
| ------------------------- | ---------------- |
...
Treat `.prose` files like code. Review them before running. Use Oriro tool
allowlists and approval gates to control side effects. For deterministic,
approval-gated workflows, compare with [Oriro](/tools/oriro).
...
<CardGroup cols={2}>
  ...
  <Card title="Subagents" href="/tools/subagents" icon="users">
    Oriro's native multi-agent coordination layer.
  </Card>
