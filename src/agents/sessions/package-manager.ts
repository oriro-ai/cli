...
type SkillDiscoveryMode = "oriro" | "agents";
...
function readResourceManifestFile(packageJsonPath: string): ResourceManifest | null {
  try {
    ...
    const pkg = JSON.parse(content) as { oriro?: ResourceManifest };
    return pkg.oriro ?? null;
  } catch {
    return null;
  }
}
...
function collectResourceFiles(dir: string, resourceType: ResourceType): string[] {
  if (resourceType === "skills") {
    return collectSkillEntries(dir, "oriro");
  }
...
export class DefaultPackageManager implements PackageManager {
  ...
  private getTemporaryDir(prefix: string, suffix?: string): string {
    const hash = createHash("sha256")
      ...
      .digest("hex")
      .slice(0, 8);
    return join(tmpdir(), "oriro-resources", prefix, hash, suffix ?? "");
  }
  ...
  private addAutoDiscoveredResources(
    ...
    addResources(
      "skills",
      collectAutoSkillEntries(projectDirs.skills, "oriro"),
      projectMetadata,
    ...
    // User extensions from ~/.oriro/agent/
    addResources(
      "extensions",
    ...
    // User skills from ~/.oriro/agent/
    addResources(
      "skills",
      collectAutoSkillEntries(userDirs.skills, "oriro"),
      userMetadata,
