...
export function createConfigWriteAuditRecordBase(params: {
  ...
  return {
    ...
    watchMode: params.env.ORIRO_WATCH_MODE === "1",
    watchSession: normalizeAuditLabel(params.env.ORIRO_WATCH_SESSION),
    watchCommand: normalizeAuditLabel(params.env.ORIRO_WATCH_COMMAND),
...
type ConfigAuditScrubResult = {
  ...
  // to swap the file. Caller should re-run `oriro doctor --fix` once the
