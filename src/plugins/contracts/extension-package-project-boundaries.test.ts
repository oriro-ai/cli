...
const MEMORY_HOST_SDK_ALLOWED_CORE_BRIDGE_FILES = [
  "packages/memory-host-sdk/src/host/oriro-runtime-auth.ts",
  "packages/memory-host-sdk/src/host/oriro-runtime-network.ts",
  "packages/memory-host-sdk/src/host/oriro-runtime.ts",
] as const;
const MEMORY_HOST_SDK_RUNTIME_ADAPTER_FILES = [
  "packages/memory-host-sdk/src/host/oriro-runtime-agent.ts",
  "packages/memory-host-sdk/src/host/oriro-runtime-cli.ts",
  "packages/memory-host-sdk/src/host/oriro-runtime-config.ts",
  "packages/memory-host-sdk/src/host/oriro-runtime-io.ts",
  "packages/memory-host-sdk/src/host/oriro-runtime-memory.ts",
  "packages/memory-host-sdk/src/host/oriro-runtime-session.ts",
] as const;
...
function collectOriroRuntimeDirectImportFiles(relativeDir: string): string[] {
  return collectCodeFiles(relativeDir).filter((file) => {
    const source = fs.readFileSync(resolve(REPO_ROOT, file), "utf8");
    return source.includes('"./oriro-runtime.js"');
  });
}
...
describe("opt-in extension package boundaries", () => {
  ...
  it("keeps every opt-in extension rooted inside its package and on the package sdk", () => {
    ...
    for (const extensionName of optInExtensions) {
      ...
      expect(packageJson.devDependencies?.["@oriro/plugin-sdk"]).toBe("workspace:*");
    }
  });
  ...
  it("keeps plugin-sdk package types generated from the package build, not a hand-maintained types bridge", () => {
    ...
    expect(packageJson.name).toBe("@oriro/plugin-sdk");
  ...
  it("keeps memory-host-sdk as a private package-owned contract surface", () => {
    ...
    expect(packageJson.name).toBe("@oriro/memory-host-sdk");
    ...
    expect(collectOriroRuntimeDirectImportFiles("packages/memory-host-sdk/src")).toEqual([
