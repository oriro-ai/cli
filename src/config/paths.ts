...
/**
 * Nix mode detection: When ORIRO_NIX_MODE=1, the gateway is running under Nix.
 * In this mode:
...
/** Build a homedir thunk that respects ORIRO_HOME for the given env. */
 ...
 * Can be overridden via ORIRO_STATE_DIR.
 * Default: ~/.oriro
 */
...
export function normalizeStateDirEnv(env: NodeJS.ProcessEnv = process.env): void {
  ...
  const oriroOverride = env.ORIRO_STATE_DIR?.trim();
  if (oriroOverride) {
    env.ORIRO_STATE_DIR = resolveUserPath(oriroOverride, env, effectiveHomedir);
  }
}
 ...
 * outside the config directory. Set via `ORIRO_INCLUDE_ROOTS` as a
 ...
 * directory containing `oriro.json`.
 */
export function resolveIncludeRoots(
  env: NodeJS.ProcessEnv = process.env,
  homedir: () => string = envHomedir(env),
): string[] {
  const raw = env.ORIRO_INCLUDE_ROOTS?.trim();
  if (!raw) {
    return [];
  }
  const effectiveHomedir = () => resolveRequiredHomeDir(env, homedir);
...
export function resolveCanonicalConfigPath(
  ...
  const override = env.ORIRO_CONFIG_PATH?.trim();
  if (override) {
...
export function resolveDefaultConfigCandidates(
  ...
  const effectiveHomedir = () => resolveRequiredHomeDir(env, homedir);
  const explicit = env.ORIRO_CONFIG_PATH?.trim();
  if (explicit) {
    return [resolveUserPath(explicit, env, effectiveHomedir)];
  }

  const candidates: string[] = [];
  const oriroStateDir = env.ORIRO_STATE_DIR?.trim();
  if (oriroStateDir) {
    const resolved = resolveUserPath(oriroStateDir, env, effectiveHomedir);
 ...
 * Default: os.tmpdir()/oriro-<uid> (uid suffix when available).
 */
export function resolveGatewayLockDir(tmpdir: () => string = os.tmpdir): string {
  const base = tmpdir();
  const uid = typeof process.getuid === "function" ? process.getuid() : undefined;
  const suffix = uid != null ? `oriro-${uid}` : "oriro";
 ...
 *
 * Precedence:
 * - `ORIRO_OAUTH_DIR` (explicit override)
...
export function resolveOAuthDir(
  ...
  const override = env.ORIRO_OAUTH_DIR?.trim();
  if (override) {
    return resolveUserPath(override, env, envHomedir(env));
  }
...
export function resolveGatewayPort(
  ...
  const envRaw = env.ORIRO_GATEWAY_PORT?.trim();
