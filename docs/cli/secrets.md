---
summary: "CLI reference for `oriro secrets` (reload, audit, configure, apply)"
read_when:
...
Use `oriro secrets` to manage SecretRefs and keep the active runtime snapshot healthy.
...
```bash
oriro secrets reload
oriro secrets reload --json
oriro secrets reload --url ws://127.0.0.1:18789 --token <token>
```
...
Scan Oriro state for:

- plaintext secret storage
- unresolved refs
- precedence drift (`auth-profiles.json` credentials shadowing `oriro.json` refs)
...
```bash
oriro secrets configure
oriro secrets configure --plan-out /tmp/oriro-secrets-plan.json
oriro secrets configure --apply --yes
oriro secrets configure --providers-only
oriro secrets configure --skip-provider-setup
oriro secrets configure --agent ops
oriro secrets configure --json
```
...
- `configure` targets secret-bearing fields in `oriro.json` plus `auth-profiles.json` for the selected agent scope.
...
- On Windows, if ACL verification is unavailable for a provider path, Oriro fails closed. For trusted paths only, set `allowInsecurePath: true` on that provider to bypass path security checks.
...
oriro secrets apply --from /tmp/oriro-secrets-plan.json --json
```
...
- `oriro.json` (SecretRef targets + provider upserts/deletes)
- `auth-profiles.json` (provider-target scrubbing)
- legacy `auth.json` residues
- `~/.oriro/.env` known secret keys whose values were migrated
