...
function hasConfiguredProvider(params: { cfg?: OriroConfig; provider: string }): boolean {
  const normalizedProvider = normalizeProviderId(params.provider);
...
function isCliAgentRuntime(runtime: string | undefined, cfg: OriroConfig | undefined): boolean {
...
async function resolveModelFallbackCandidateHarnessAuthPrecheck(
  ...
  if (isCliAgentRuntime(agentRuntime, params.cfg)) {
    // CLI runtimes own their transport/auth, so stale Oriro provider
    // profile state must not block the candidate before the CLI starts.
    return { skipsProviderAuthCooldown: true };
  }
  if (agentRuntime === "oriro") {
    return { skipsProviderAuthCooldown: false };
  }
  ...
  // Explicit non-Codex plugin harnesses own transport/auth; stale Oriro
...
export function resolveImageFallbackDefaultProvider(cfg: OriroConfig | undefined): string {
...
function resolveFallbackCandidateModelProviderCacheParts(cfg: OriroConfig | undefined): unknown {
