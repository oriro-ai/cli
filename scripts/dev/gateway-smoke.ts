// Gateway Smoke script supports Oriro repository automation.
...
function parseGatewaySmokeCli(
  ...
  return {
    help: has("--help") || has("-h"),
    token: getArg("--token") ?? env.ORIRO_GATEWAY_TOKEN,
    urlRaw: getArg("--url") ?? env.ORIRO_GATEWAY_URL,
  };
}
...
export async function runGatewaySmoke(
  ...
  try {
    ...
    const connectRes = await request("connect", {
      ...
      client: {
        id: "oriro-ios",
        displayName: "oriro gateway smoke test",
        version: "dev",
        platform: "dev",
        mode: "ui",
        instanceId: "oriro-dev-smoke",
      },
      locale: "en-US",
