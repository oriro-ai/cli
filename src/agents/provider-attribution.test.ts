...
vi.mock("../plugins/manifest-metadata-scan.js", () => ({
  listOriroPluginManifestMetadata: () =>
...
describe("provider attribution", () => {
  it("resolves the canonical Oriro product and runtime version", () => {
  ...
  it("returns a hidden-spec OpenAI attribution policy", () => {
    expect(resolveProviderAttributionPolicy("openai", { ORIRO_VERSION: "2026.3.22" })).toEqual({
      provider: "openai",
      enabledByDefault: true,
    ...
    expect(
      resolveProviderAttributionPolicy("openai", { ORIRO_VERSION: "2026.3.22" })?.headers,
    ).toEqual({
      originator: "oriro",
      version: "2026.3.22",
      "User-Agent": "oriro/2026.3.22",
  ...
  it("returns a hidden-spec xAI attribution policy", () => {
    expect(resolveProviderAttributionPolicy("xai", { ORIRO_VERSION: "2026.3.22" })).toEqual({
      provider: "xai",
      enabledByDefault: true,
      verification: "vendor-hidden-api-spec",
      hook: "request-headers",
      reviewNote:
        "xAI api.x.ai accepts a standard oriro User-Agent. Companion originator/version headers mirror the OpenAI attribution shape for consistency; they are not validated against an xAI-specific spec and are expected to be ignored by xAI's OpenAI-compatible surface.",
      product: "Oriro",
      version: "2026.3.22",
      headers: {
        originator: "oriro",
    ...
    expect(
      resolveProviderAttributionPolicy("xai", { ORIRO_VERSION: "2026.3.22" })?.headers,
    ).toEqual({
      originator: "oriro",
      version: "2026.3.22",
      "User-Agent": "oriro/2026.3.22",
  ...
  it("lists the current attribution support matrix", () => {
    ...
    expect(
      listProviderAttributionPolicies({ ORIRO_VERSION: "2026.3.22" }).map((policy) => [
        policy.provider,
