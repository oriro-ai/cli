...
import { canResolveEnvSecretRefInReadOnlyPath } from "oriro/plugin-sdk/extension-shared";
import { resolvePositiveTimeoutSeconds } from "oriro/plugin-sdk/provider-web-fetch";
import { resolveSecretInputString, normalizeSecretInput } from "oriro/plugin-sdk/secret-input";
...
type WebSearchConfig = NonNullable<OriroConfig["tools"]>["web"] extends infer Web
  ? Web extends { search?: infer Search }
    ? Search
    : undefined
  : undefined;

type WebFetchConfig = NonNullable<OriroConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: OriroConfig): WebSearchConfig {
...
function resolveFetchConfig(cfg?: OriroConfig): WebFetchConfig {
...
export function resolveFirecrawlSearchConfig(cfg?: OriroConfig): FirecrawlSearchConfig {
...
function resolveFirecrawlFetchConfig(cfg?: OriroConfig): FirecrawlFetchConfig {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlApiKey(cfg?: OriroConfig): string | undefined {
  const pluginConfig = cfg?.plugins?.entries?.firecrawl?.config as PluginEntryConfig;
...
export function resolveFirecrawlBaseUrl(cfg?: OriroConfig): string {
  const search = resolveFirecrawlSearchConfig(cfg);
  const fetch = resolveFirecrawlFetchConfig(cfg);
  const configured =
...
export function resolveFirecrawlOnlyMainContent(cfg?: OriroConfig, override?: boolean): boolean {
...
export function resolveFirecrawlMaxAgeMs(cfg?: OriroConfig, override?: number): number {
