import { resolvePreferredOriroTmpDir } from "oriro/plugin-sdk/sandbox";
import { resolveLocalPathFromRootsSync } from "oriro/plugin-sdk/security-runtime";
...
function trustedOriroTmpRoot(): string | null {
  if (cachedTrustedTmpRoot === undefined) {
    try {
      cachedTrustedTmpRoot = resolvePreferredOriroTmpDir();
    } catch {
      return null;
    }
  }
 ...
 * roots, this also trusts Oriro's permission-hardened temp root, where
...
export function resolveTrustedOutboundMediaPath(
  ...
  const tmpRoot = trustedOriroTmpRoot();
  if (!tmpRoot) {
    return null;
  }
  return (
