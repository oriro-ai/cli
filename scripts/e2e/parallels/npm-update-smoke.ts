...
export function freshLaneTimeoutMs(platform: Platform): number {
  ...
  return readPositiveIntEnv("ORIRO_PARALLELS_NPM_UPDATE_FRESH_TIMEOUT_S", defaultSeconds) * 1000;
}
...
function oriroVersionFamily(version: string): string {
...
export class NpmUpdateSmoke {
  ...
  private assertPublishedTargetMatchesHarnessCheckout(): void {
    ...
    const targetFamily = oriroVersionFamily(candidateVersion);
    if (!targetFamily) {
      return;
    }
    this.harnessTargetFamily = targetFamily;
    const checkoutFamily = oriroVersionFamily(this.harnessCheckoutVersion);
