...
const pluginRuntimeMocks = vi.hoisted(() => {
  ...
  let stateDir = `/tmp/oriro-whatsapp-ingress-${Date.now()}-${Math.random()}`;
  ...
  return {
    ...
    reset: () => {
      ...
      stateDir = `/tmp/oriro-whatsapp-ingress-${Date.now()}-${Math.random()}`;
    },
  };
});
...
vi.mock("oriro/plugin-sdk/channel-activity-runtime", async () => {
  const actual = await vi.importActual<
    typeof import("oriro/plugin-sdk/channel-activity-runtime")
  >("oriro/plugin-sdk/channel-activity-runtime");
  return {
    ...actual,
...
function expectInboxPairingReplyText(
  ...
  expect(text).toContain("Oriro: access not configured.");
...
export function installWebMonitorInboxUnitTestHooks(opts?: { authDir?: boolean }) {
  ...
  beforeEach(async () => {
    ...
    resetWebInboundDedupe();
    if (createAuthDir) {
      authDir = fsSync.mkdtempSync(path.join(os.tmpdir(), "oriro-auth-"));
    } else {
      authDir = undefined;
    }
  });
