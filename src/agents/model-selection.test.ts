...
function resolveAnthropicOpusThinking(cfg: OriroConfig) {
...
function resolveAnthropicOpus47Thinking(cfg: OriroConfig) {
  return resolveThinkingDefault({
    cfg,
    provider: "anthropic",
...
function resolveAnthropicOpus48Thinking(cfg: OriroConfig) {
  return resolveThinkingDefault({
    cfg,
    provider: "anthropic",
...
function resolveAnthropicVertexOpus48Thinking(cfg: OriroConfig) {
  return resolveThinkingDefault({
    cfg,
    provider: "anthropic-vertex",
    model: "claude-opus-4-8",
...
function resolveClaudeCliOpus48Thinking(cfg: OriroConfig) {
  return resolveThinkingDefault({
    cfg,
    provider: "claude-cli",
    model: "claude-opus-4-8",
...
function resolveConfiguredRefForTest(cfg: Partial<OriroConfig>) {
  return resolveConfiguredModelRef({
    cfg: cfg as OriroConfig,
...
describe("model-selection", () => {
  ...
  describe("resolveConfiguredModelRef", () => {
    ...
    it("should fall back to the configured default provider and warn if provider is missing for non-alias", async () => {
      const warnLogs = createWarnLogCapture("oriro-model-selection-test");
      try {
        const cfg: Partial<OriroConfig> = {
          agents: {
            defaults: {
