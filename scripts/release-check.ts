...
// Release Check script supports Oriro repository automation.
...
} from "./oriro-npm-postpublish-verify.ts";
...
export const CRITICAL_PLUGIN_SDK_SIZE_CHECK_SPECIFIERS = [
  "oriro/plugin-sdk/core",
  "oriro/plugin-sdk/provider-entry",
  "oriro/plugin-sdk/runtime",
] as const;
export const CRITICAL_PLUGIN_SDK_IMPORT_SMOKE_SPECIFIERS = ["oriro/plugin-sdk/core"] as const;
...
export function runReleaseCheckCommand(
  ...
  const output = execFileSync(invocation.command, invocation.args, {
    ...
    maxBuffer:
      options.maxBuffer ??
      positiveEnvInt(
        "ORIRO_RELEASE_CHECK_COMMAND_MAX_BUFFER_BYTES",
    ...
    timeout:
      options.timeoutMs ??
      positiveEnvInt(
        "ORIRO_RELEASE_CHECK_COMMAND_TIMEOUT_MS",
...
export function collectPackedInstalledPackageVerificationErrors(params: {
  ...
  if (
    ...
    errors.push(
      `installed oriro binary version mismatch: expected ${params.expectedVersion}, found ${params.installedBinaryVersion || "<missing>"}.`,
    );
  }
...
export function createPackedPluginSdkTypescriptSmokeProject(params: {
  ...
  writeFileSync(
    ...
    `${JSON.stringify(
      {
        name: "oriro-plugin-sdk-type-smoke",
        private: true,
        type: "module",
        dependencies: {
          oriro: params.packageSpec,
        },
      },
      null,
      2,
...
function runPackedPluginSdkTypescriptSmoke(tarballPath: string, tmpRoot: string): void {
  ...
  const installedOriroRoot = join(consumerDir, "node_modules", "oriro");
  const tscPath = [
    join(consumerDir, "node_modules", "typescript", "bin", "tsc"),
    join(installedOriroRoot, "node_modules", "typescript", "bin", "tsc"),
...
export function writePackedBundledPluginActivationConfig(homeDir: string): void {
  const configPath = join(homeDir, ".oriro", "oriro.json");
  mkdirSync(join(homeDir, ".oriro"), { recursive: true });
  writeFileSync(
    configPath,
    `${JSON.stringify(
      {
...
function runPackedBundledPluginActivationSmoke(packageRoot: string, tmpRoot: string): void {
  ...
  const env = createPackedCliSmokeEnv(process.env, {
    HOME: homeDir,
    OPENAI_API_KEY: "sk-oriro-release-check",
  });
  ...
  runReleaseCheckCommand(
    {
      command: process.execPath,
      args: [join(packageRoot, "oriro.mjs"), ...PACKED_BUNDLED_RUNTIME_DEPS_REPAIR_ARGS],
    },
    {
      cwd: packageRoot,
      stdio: "inherit",
  ...
  runReleaseCheckCommand(
    { command: process.execPath, args: [join(packageRoot, "oriro.mjs"), "plugins", "doctor"] },
    {
      cwd: packageRoot,
      stdio: "inherit",
      env,
...
function runPackedBundledChannelEntrySmoke(): void {
  const tmpRoot = mkdtempSync(join(tmpdir(), "oriro-release-pack-smoke-"));
  try {
    ...
    const packageRoot = join(resolveGlobalRoot(prefixDir, tmpRoot), "oriro");
    ...
    runReleaseCheckCommand(
      {
        command: process.execPath,
        args: [join(packageRoot, "oriro.mjs"), ...PACKED_COMPLETION_SMOKE_ARGS],
      },
      {
        cwd: packageRoot,
        stdio: "inherit",
  ...
  return [...paths]
    .filter(
      (path) =>
        ...
        /(^|\/)\.oriro-runtime-deps-[^/]+(\/|$)/u.test(path) ||
        path.endsWith("/.oriro-runtime-deps-stamp.json") ||
...
// Critical functions that channel extension plugins import from oriro/plugin-sdk.
...
export function collectCriticalPluginSdkEntrypointSizeErrors(rootDir = process.cwd()): string[] {
  ...
  for (const specifier of CRITICAL_PLUGIN_SDK_SIZE_CHECK_SPECIFIERS) {
    const subpath = specifier.slice("oriro/plugin-sdk/".length);
