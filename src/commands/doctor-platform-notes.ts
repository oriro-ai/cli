...
import { findStaleOriroUpdateLaunchdJobs } from "../daemon/launchd.js";
...
export function collectMacLaunchAgentOverrideWarning(deps?: {
  ...
  const markerCandidates = [path.join(home, ".oriro", "disable-launchagent")];
...
/** Returns a warning for stale Oriro updater launchd jobs left after interrupted updates. */
export async function collectMacStaleOriroUpdateLaunchdJobsWarning(deps?: {
  platform?: NodeJS.Platform;
  findJobs?: typeof findStaleOriroUpdateLaunchdJobs;
  ...
  const jobs = await (deps?.findJobs ?? findStaleOriroUpdateLaunchdJobs)(scanEnv).catch(
    () => [],
  );
  if (jobs.length === 0) {
    return null;
  ...
  return [
    ...
    `  ${formatCliCommand("oriro gateway restart")}`,
  ].join("\n");
}

/** Emits stale updater launchd job notes using the gateway service environment when available. */
export async function noteMacStaleOriroUpdateLaunchdJobs(deps?: {
  platform?: NodeJS.Platform;
  findJobs?: typeof findStaleOriroUpdateLaunchdJobs;
  env?: NodeJS.ProcessEnv;
...
function hasConfigGatewayCreds(cfg: OriroConfig): boolean {
...
export async function collectMacLaunchctlGatewayEnvOverrideWarning(
  ...
  const tokenEntries = [
    ["ORIRO_GATEWAY_TOKEN", await getenv("ORIRO_GATEWAY_TOKEN")],
  ] as const;
  const passwordEntries = [
    ["ORIRO_GATEWAY_PASSWORD", await getenv("ORIRO_GATEWAY_PASSWORD")],
  ] as const;
  ...
  return [
    ...
    envToken && envTokenKey
      ...
      : undefined,
    envPassword
      ? `- \`${envPasswordKey ?? "ORIRO_GATEWAY_PASSWORD"}\` is set; it can make local clients use a different password than gateway.auth.password.`
      : undefined,
...
export async function collectMacGatewayPlatformWarnings(
  ...
  const serviceEnv =
    platform === "darwin" ? await resolveGatewayServiceEnvForPlatformNotes(deps) : deps?.env;
  const staleUpdateWarning = await collectMacStaleOriroUpdateLaunchdJobsWarning({
    env: serviceEnv,
    findJobs: deps?.findJobs,
    platform,
  });
...
export function noteStartupOptimizationHints(
  ...
  const noRespawn = normalizeOptionalString(env.ORIRO_NO_RESPAWN) ?? "";
  const lines: string[] = [];
  ...
  if (noRespawn !== "1") {
    lines.push(
      "- ORIRO_NO_RESPAWN is not set to 1; set it when you want routine gateway restarts to stay in-process instead of handing off to a managed supervisor.",
    );
  }
