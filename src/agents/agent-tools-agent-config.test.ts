...
describe("Agent-specific tool filtering", () => {
  ...
  async function withApplyPatchEscapeCase(
    ...
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-agent-tools-"));
...
*** End Patch`;
  ...
  function createMainSessionTools(cfg: OriroConfig) {
  ...
  function createMainAgentConfig(params: {
    tools: NonNullable<OriroConfig["tools"]>;
    agentTools?: NonNullable<NonNullable<OriroConfig["agents"]>["list"]>[number]["tools"];
  }): OriroConfig {
    return {
      tools: params.tools,
      agents: {
