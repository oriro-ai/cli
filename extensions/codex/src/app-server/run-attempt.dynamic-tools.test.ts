...
import { initializeGlobalHookRunner } from "oriro/plugin-sdk/hook-runtime";
...
describe("runCodexAppServerAttempt dynamic tools", () => {
  it("passes the live run session key to Codex dynamic tools when sandbox policy uses another key", () => {
    ...
    expect(
      testing.resolveOriroCodingToolsSessionKeys(
        params,
    ...
    expect(testing.resolveOriroCodingToolsSessionKeys(params, "agent:main:main")).toEqual({
  ...
  it("emits normalized tool progress around app-server dynamic tool requests", async () => {
    ...
    expect(toolResult.contentItems?.[0]?.text).toMatch(/^Unknown Oriro tool: lookup$/u);
    ...
    expect(resultEvent?.data?.result?.content?.[0]?.text).toBe("Unknown Oriro tool: lookup");
      ...
      emitDynamicToolTerminalDiagnostic({
        ...
        response: {
          ...
          contentItems: [
            {
              type: "inputText",
              text: "Oriro dynamic tool call timed out after 1ms while running tool echo.",
            },
          ],
        },
      });
