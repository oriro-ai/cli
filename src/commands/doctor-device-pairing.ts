...
import { normalizeUniqueSingleOrTrimmedStringList } from "@oriro/normalization-core/string-normalization";
...
function resolvePendingPairingIssue(
  ...
  const approveCommand = formatCliArgs(["oriro", "devices", "approve", pending.requestId]);
  const inspectCommand = formatCliArgs(["oriro", "devices", "list"]);
  if (!paired) {
    return {
      kind: "first-time",
      pending,
  ...
  if (paired.publicKey !== pending.publicKey) {
    return {
      ...
      approveCommand,
      inspectCommand,
      removeCommand: formatCliArgs(["oriro", "devices", "remove", pending.deviceId]),
    };
  }
