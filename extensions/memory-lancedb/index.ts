...
import type { AgentToolResult } from "oriro/plugin-sdk/agent-core";
import {
  optionalFiniteNumberSchema,
  optionalPositiveIntegerSchema,
} from "oriro/plugin-sdk/channel-actions";
import { BUNDLED_CHAT_CHANNEL_ENVELOPE_PREFIXES } from "oriro/plugin-sdk/chat-channel-ids";
import type { OriroConfig } from "oriro/plugin-sdk/config-contracts";
import type { MemoryEmbeddingProvider } from "oriro/plugin-sdk/memory-core-host-engine-embeddings";
...
import { readFiniteNumberParam, readPositiveIntegerParam } from "oriro/plugin-sdk/param-readers";
import { resolveLivePluginConfigObject } from "oriro/plugin-sdk/plugin-config-runtime";
import { ensureGlobalUndiciEnvProxyDispatcher } from "oriro/plugin-sdk/runtime-env";
import {
...
import { definePluginEntry, type OriroPluginApi } from "./api.js";
import {
...
let memoryEmbeddingProviderModulePromise:
  | Promise<typeof import("oriro/plugin-sdk/memory-core-host-engine-embeddings")>
  | undefined;
function loadMemoryEmbeddingProviderModule(): Promise<
  typeof import("oriro/plugin-sdk/memory-core-host-engine-embeddings")
> {
  memoryEmbeddingProviderModulePromise ??=
    import("oriro/plugin-sdk/memory-core-host-engine-embeddings");
  return memoryEmbeddingProviderModulePromise;
}

let memoryHostCoreModulePromise:
  | Promise<typeof import("oriro/plugin-sdk/memory-host-core")>
  | undefined;
function loadMemoryHostCoreModule(): Promise<
  typeof import("oriro/plugin-sdk/memory-host-core")
> {
  memoryHostCoreModulePromise ??= import("oriro/plugin-sdk/memory-host-core");
...
class ProviderAdapterEmbeddings implements Embeddings {
  ...
  private async createProvider(): Promise<MemoryEmbeddingProvider> {
    const cfg = (this.api.runtime.config?.current?.() ?? this.api.config) as OriroConfig;
...
function createEmbeddings(api: OriroPluginApi, cfg: MemoryConfig): Embeddings {
...
/**
 * Returns true if `text` looks like it contains Oriro-injected envelope or
...
/**
 * Strips Oriro-injected envelope metadata from a user message so that only
    ...
    const resolveCurrentHookConfig = () => {
      const runtimePluginConfig = resolveLivePluginConfigObject(
        api.runtime.config?.current
          ? () => api.runtime.config.current() as OriroConfig
          : undefined,
        "memory-lancedb",
