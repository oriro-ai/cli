...
export async function writeGeminiSystemSettings(
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-gemini-mcp-"));
...
export async function writeGeminiMcpCaptureSettings(params: {
  ...
  const oriro = isRecord(mcpServers.oriro) ? mcpServers.oriro : {};
  const headers = normalizeStringRecord(oriro.headers) ?? {};
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-gemini-mcp-attempt-"));
  const settingsPath = path.join(tempDir, "settings.json");
  await writeJson(
    settingsPath,
    {
