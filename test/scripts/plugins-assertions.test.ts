...
describe("plugins Docker assertions", () => {
  it("rejects loose OriroHub preflight limits instead of parsing prefixes", () => {
    const timeoutResult = spawnSync(process.execPath, [ASSERTIONS_SCRIPT, "orirohub-preflight"], {
      encoding: "utf8",
      env: {
        ...process.env,
        ORIROHUB_PLUGIN_SPEC: "orirohub:@oriro/kitchen-sink",
        ORIRO_PLUGINS_E2E_ORIROHUB_PREFLIGHT_TIMEOUT_MS: "1e3",
      },
    });
    expect(timeoutResult.status).not.toBe(0);
    expect(timeoutResult.stderr).toContain(
      "invalid ORIRO_PLUGINS_E2E_ORIROHUB_PREFLIGHT_TIMEOUT_MS: 1e3",
    );

    const bodyLimitResult = spawnSync(process.execPath, [ASSERTIONS_SCRIPT, "orirohub-preflight"], {
      encoding: "utf8",
      env: {
        ...process.env,
        ORIROHUB_PLUGIN_SPEC: "orirohub:@oriro/kitchen-sink",
        ORIRO_PLUGINS_E2E_ORIROHUB_PREFLIGHT_BODY_MAX_BYTES: "1000bytes",
      },
    });
    expect(bodyLimitResult.status).not.toBe(0);
    expect(bodyLimitResult.stderr).toContain(
      "invalid ORIRO_PLUGINS_E2E_ORIROHUB_PREFLIGHT_BODY_MAX_BYTES: 1000bytes",
    );
  });

  it("passes OriroHub preflight body timeouts into the bounded reader", () => {
  ...
  it("keeps sweep artifact paths aligned with the assertion scratch root", () => {
    const scripts = [
      ...
      "scripts/e2e/lib/plugins/orirohub.sh",
    ];

    for (const scriptPath of scripts) {
      const script = readFileSync(scriptPath, "utf8");
      const scriptWithoutDefaultScratch = script.replace(
        'mktemp -d "/tmp/oriro-plugins.XXXXXX"',
        "",
      );
      expect(script).toContain("ORIRO_PLUGINS_TMP_DIR");
      expect(scriptWithoutDefaultScratch).not.toMatch(
        /\/tmp\/(?:plugins|marketplace|demo-plugin|is-number|oriro-plugin|oriro-orirohub)/,
      );
    }
  });

  it("cleans the default plugin sweep scratch root", () => {
    const root = mkdtempSync(path.join(tmpdir(), "oriro-plugin-sweep-cleanup-"));
    const marker = path.join(root, "scratch-path.txt");
    try {
      const result = runPluginsSweepShell(
        `
set -euo pipefail
export ORIRO_PLUGINS_SWEEP_SOURCE_ONLY=1
source scripts/e2e/lib/plugins/sweep.sh
printf '%s\\n' "$ORIRO_PLUGINS_TMP_DIR" > "$MARKER"
test -d "$ORIRO_PLUGINS_TMP_DIR"
cleanup_oriro_plugins_sweep
test ! -e "$ORIRO_PLUGINS_TMP_DIR"
`,
        { MARKER: marker },
      );
      ...
      expect(scratchRoot).toContain("/tmp/oriro-plugins.");
  ...
  it("preserves caller-provided plugin sweep scratch roots", () => {
    const root = mkdtempSync(path.join(tmpdir(), "oriro-plugin-sweep-caller-"));
...
set -euo pipefail
export ORIRO_PLUGINS_SWEEP_SOURCE_ONLY=1
export ORIRO_PLUGINS_TMP_DIR="$SCRATCH_ROOT"
source scripts/e2e/lib/plugins/sweep.sh
test -d "$ORIRO_PLUGINS_TMP_DIR"
cleanup_oriro_plugins_sweep
test -d "$ORIRO_PLUGINS_TMP_DIR"
`,
  ...
  it("scans plugin assertion logs without echoing whole files on failure", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "oriro-plugin-update-log-"));
    try {
      ...
      const pass = await runAssertionAsync(["plugin-dir-update-skipped"], {
        ORIRO_PLUGINS_TMP_DIR: passRoot,
      });
      ...
      const fail = await runAssertionAsync(["plugin-dir-update-skipped"], {
        ORIRO_PLUGINS_TMP_DIR: failRoot,
      });
      ...
      writeFileSync(
        path.join(invalidRoot, "plugins-invalid-oriro-extensions.log"),
        `oriro.extensions[1]\n${"x".repeat(256 * 1024)}\nmissing validation tail`,
        "utf8",
      );
      writeJson(path.join(invalidRoot, "plugins-invalid-oriro-extensions-list.json"), {
        plugins: [],
      });
      const invalid = await runAssertionAsync(["invalid-oriro-extensions"], {
        HOME: invalidHome,
        ORIRO_PLUGINS_TMP_DIR: invalidRoot,
      });
  ...
  it("cleans npm fixture registry children when readiness times out", () => {
    const root = mkdtempSync(path.join(tmpdir(), "oriro-plugin-npm-fixture-cleanup-"));
    try {
      ...
      const result = spawnSync(
        ...
        ],
        {
          ...
          env: {
            ...process.env,
            ORIRO_TEST_FIXTURE_SERVER_PID: pidPath,
  ...
  it("force-kills stubborn npm fixture registry children during cleanup", () => {
    const root = mkdtempSync(path.join(tmpdir(), "oriro-plugin-npm-fixture-kill-"));
    try {
      const binDir = path.join(root, "bin");
      const fixtureDir = path.join(root, "fixture");
      const pidPath = path.join(root, "server.pid");
      ...
      const result = spawnSync(
        ...
        ],
        {
          ...
          env: {
            ...process.env,
            ORIRO_PLUGINS_FIXTURE_STOP_ATTEMPTS: "2",
            ORIRO_PLUGINS_FIXTURE_STOP_INTERVAL_SECONDS: "0.05",
            ORIRO_TEST_FIXTURE_SERVER_PID: pidPath,
            PATH: `${binDir}${path.delimiter}/usr/bin${path.delimiter}/bin`,
          },
        },
    ...
    const result = spawnSync(
      ...
      [
        ...
        [
          "set -euo pipefail",
          "source scripts/e2e/lib/plugins/fixtures.sh",
          "oriro_plugins_signal_fixture_process() { echo signal; }",
          "oriro_plugins_fixture_process_alive() { echo probe; return 1; }",
          "set +e",
          "oriro_plugins_stop_fixture_process 12345",
          'status="$?"',
          "set -e",
          'exit "$status"',
        ].join("\n"),
      ],
      {
        ...
        env: {
          ...process.env,
          ORIRO_PLUGINS_FIXTURE_STOP_ATTEMPTS: "2x",
        },
      },
    );

    expect(result.status).toBe(2);
    expect(result.stderr).toContain("invalid ORIRO_PLUGINS_FIXTURE_STOP_ATTEMPTS: 2x");
    ...
    const result = spawnSync(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          ORIRO_PLUGINS_FIXTURE_STOP_ATTEMPTS: "2",
          ORIRO_PLUGINS_FIXTURE_STOP_INTERVAL_SECONDS: "soon",
        },
      },
    );

    expect(result.status).toBe(2);
    expect(result.stderr).toContain("invalid ORIRO_PLUGINS_FIXTURE_STOP_INTERVAL_SECONDS: soon");
    expect(result.stdout).not.toContain("signal");
    expect(result.stdout).not.toContain("probe");
  });

  it("bounds npm fixture registry logs when readiness fails", () => {
    const root = mkdtempSync(path.join(tmpdir(), "oriro-plugin-npm-fixture-log-"));
    try {
      const binDir = path.join(root, "bin");
      const fixtureDir = path.join(root, "fixture");
      mkdirSync(fixtureDir);
      ...
      const result = spawnSync(
        ...
        ],
        {
          ...
          env: {
            ...process.env,
            ORIRO_DOCKER_E2E_LOG_PRINT_BYTES: "80",
            PATH: `${binDir}${path.delimiter}/usr/bin${path.delimiter}/bin`,
          },
        },
      );
  ...
  it("keeps npm fixture registry alive after malformed package paths", async () => {
    ...
    const root = makeTempDir(tempDirs, "oriro-plugin-npm-fixture-request-");
    ...
    try {
      ...
      const valid = await requestFixtureRegistry(port, "/@oriro%2Fdemo-plugin-npm");

      expect(valid.statusCode, stderr.text()).toBe(200);
      expect(JSON.parse(valid.body)).toMatchObject({
        name: "@oriro/demo-plugin-npm",
  ...
  it("rejects invalid plugin fixture log byte limits before npm fixture setup", () => {
    const root = mkdtempSync(path.join(tmpdir(), "oriro-plugin-npm-fixture-log-invalid-"));
    try {
      const binDir = path.join(root, "bin");
      const fixtureDir = path.join(root, "fixture");
      mkdirSync(binDir, { recursive: true });
      ...
      const result = spawnSync(
        ...
        ],
        {
          ...
          env: {
            ...process.env,
            ORIRO_DOCKER_E2E_LOG_PRINT_BYTES: "64kb",
            PATH: `${binDir}${path.delimiter}/usr/bin${path.delimiter}/bin`,
          },
        },
      );
      ...
      expect(result.status).toBe(2);
      expect(result.stderr).toContain("invalid ORIRO_DOCKER_E2E_LOG_PRINT_BYTES: 64kb");
  ...
  it("cleans OriroHub fixture children when readiness times out", () => {
    const root = mkdtempSync(path.join(tmpdir(), "oriro-plugin-orirohub-fixture-cleanup-"));
    try {
      const binDir = path.join(root, "bin");
      const cleanupPath = path.join(root, "caller-cleanup");
      ...
      const result = spawnSync(
        ...
        [
          ...
          [
            ...
            "set +e",
            `( set -e; trap 'printf caller-cleanup > ${shellQuote(cleanupPath)}' EXIT; run_plugins_orirohub_scenario )`,
            'status="$?"',
            "set -e",
            '[ "$status" != "0" ]',
          ].join("\n"),
        ],
        {
          ...
          env: {
            ...process.env,
            ORIRO_PLUGINS_E2E_LIVE_ORIROHUB: "0",
            ORIRO_PLUGINS_TMP_DIR: tmpDir,
            ORIRO_TEST_FIXTURE_SERVER_PID: pidPath,
            PATH: `${binDir}${path.delimiter}/usr/bin${path.delimiter}/bin`,
          },
        },
  ...
  it("rejects invalid plugin fixture log byte limits before OriroHub fixture setup", () => {
    const root = mkdtempSync(path.join(tmpdir(), "oriro-plugin-orirohub-fixture-log-invalid-"));
    try {
      const binDir = path.join(root, "bin");
      const tmpDir = path.join(root, "scratch");
      mkdirSync(binDir, { recursive: true });
  ...
  it("bounds OriroHub fixture server logs when readiness fails", () => {
    const root = mkdtempSync(path.join(tmpdir(), "oriro-plugin-orirohub-fixture-log-"));
    try {
      const binDir = path.join(root, "bin");
      const tmpDir = path.join(root, "scratch");
      mkdirSync(tmpDir);
  ...
  it("uses the configured scratch root and resolves Windows home-relative install paths", () => {
    const root = mkdtempSync(path.join(tmpdir(), "oriro-plugins-assertions-"));
    ...
    try {
      ...
      writeJson(path.join(home, ".oriro", "plugins", "installs.json"), {
        installRecords: {
          "demo-plugin-tgz": {
            source: "archive",
      ...
      const result = spawnSync(process.execPath, [ASSERTIONS_SCRIPT, "plugin-tgz"], {
        ...
        env: {
          ...
          HOME: home,
          ORIRO_PLUGINS_E2E_ORIROHUB_PREFLIGHT_TIMEOUT_MS: "1e3",
          ORIRO_PLUGINS_TMP_DIR: scratchRoot,
        },
      });
  ...
  it("compares local plugin source paths by canonical path", () => {
    ...
    const installPath = path.join(home, ".oriro", "extensions", "demo-plugin-dir");
  ...
  it("still requires archive managed install directories to be removed", () => {
    ...
    const home = path.join(root, "home");
    const scratchRoot = path.join(root, "scratch");
    const installPath = path.join(home, ".oriro", "extensions", "demo-plugin-tgz");
    mkdirSync(installPath, { recursive: true });
  ...
  it("rejects unreadable config during plugin uninstall proof", () => {
    ...
    const home = path.join(root, "home");
    const scratchRoot = path.join(root, "scratch");
    const removedInstallPath = path.join(home, ".oriro", "extensions", "demo-plugin-tgz");
    ...
    try {
      ...
      writeFileSync(path.join(home, ".oriro", "oriro.json"), "{ malformed\n", "utf8");
      ...
      expect(result.status).not.toBe(0);
      expect(result.stderr).toContain("failed to read Oriro config");
    } finally {
      rmSync(root, { force: true, recursive: true });
    }
  });

  it("rejects OriroHub install paths that resolve outside the managed extensions root", () => {
    const tempDirs: string[] = [];
    const root = makeTempDir(tempDirs, "oriro-plugins-orirohub-path-");
    const home = path.join(root, "home");
    const scratchRoot = path.join(root, "scratch");
    const extensionsRoot = path.join(home, ".oriro", "extensions");
    const escapedInstallPath = `${extensionsRoot}${path.sep}..${path.sep}escaped-orirohub`;
    mkdirSync(extensionsRoot, { recursive: true });
    mkdirSync(escapedInstallPath, { recursive: true });

    try {
      writeJson(path.join(scratchRoot, "plugins-orirohub-installed.json"), {
        plugins: [{ id: "oriro-kitchen-sink-fixture", status: "loaded" }],
      });
      writeJson(path.join(scratchRoot, "plugins-orirohub-inspect.json"), {
        plugin: { id: "oriro-kitchen-sink-fixture" },
      });
      writeJson(path.join(home, ".oriro", "plugins", "installs.json"), {
        installRecords: {
          "oriro-kitchen-sink-fixture": {
            artifactFormat: "zip",
            artifactKind: "legacy-zip",
            orirohubFamily: "code-plugin",
            orirohubPackage: "@oriro/kitchen-sink",
            installPath: escapedInstallPath,
            source: "orirohub",
            spec: "orirohub:@oriro/kitchen-sink",
          },
        },
      });

      const result = spawnSync(process.execPath, [ASSERTIONS_SCRIPT, "orirohub-installed"], {
        encoding: "utf8",
        env: {
          ...process.env,
          ORIROHUB_PLUGIN_ID: "oriro-kitchen-sink-fixture",
          ORIROHUB_PLUGIN_SPEC: "orirohub:@oriro/kitchen-sink",
          HOME: home,
          ORIRO_PLUGINS_TMP_DIR: scratchRoot,
        },
      ...
      expect(result.status).not.toBe(0);
      expect(result.stderr).toContain("OriroHub install path resolved outside");
    } finally {
      cleanupTempDirs(tempDirs);
    }
  });

  it("times out stalled OriroHub package metadata requests", async () => {
    ...
    try {
      ...
      const result = await runAssertionAsync(["orirohub-preflight"], {
        ORIROHUB_PLUGIN_ID: "oriro-kitchen-sink-fixture",
        ORIROHUB_PLUGIN_SPEC: "orirohub:@oriro/kitchen-sink",
        ORIRO_ORIROHUB_URL: `http://127.0.0.1:${address.port}`,
        ORIRO_PLUGINS_E2E_ORIROHUB_PREFLIGHT_TIMEOUT_MS: "25",
      });

      expect(result.status).not.toBe(0);
      expect(result.stderr).toContain(
        "OriroHub package preflight for @oriro/kitchen-sink timed out after 25ms",
      );
    } finally {
      await new Promise<void>((resolve) => {
  ...
  it("times out stalled OriroHub package metadata bodies", async () => {
    ...
    try {
      ...
      const result = await runAssertionAsync(["orirohub-preflight"], {
        ...
        ORIROHUB_PLUGIN_SPEC: "orirohub:@oriro/kitchen-sink",
        ORIRO_ORIROHUB_URL: `http://127.0.0.1:${address.port}`,
        ORIRO_PLUGINS_E2E_ORIROHUB_PREFLIGHT_TIMEOUT_MS: "75",
      });

      expect(result.status).not.toBe(0);
      expect(result.stderr).toContain(
        "OriroHub package preflight response for @oriro/kitchen-sink timed out after 75ms",
      );
    } finally {
      await new Promise<void>((resolve) => {
        server.close(() => resolve());
  ...
  it("bounds OriroHub package metadata response bodies", async () => {
    const server = createServer((_request, response) => {
    ...
    try {
      ...
      const result = await runAssertionAsync(["orirohub-preflight"], {
        ...
        ORIROHUB_PLUGIN_SPEC: "orirohub:@oriro/kitchen-sink",
        ORIRO_ORIROHUB_URL: `http://127.0.0.1:${address.port}`,
        ORIRO_PLUGINS_E2E_ORIROHUB_PREFLIGHT_BODY_MAX_BYTES: "16",
        ORIRO_PLUGINS_E2E_ORIROHUB_PREFLIGHT_TIMEOUT_MS: "1000",
      });

      expect(result.status).not.toBe(0);
      expect(result.stderr).toContain(
        "OriroHub package preflight response for @oriro/kitchen-sink response body exceeded 16 bytes",
      );
