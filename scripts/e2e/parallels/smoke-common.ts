// Smoke Common helper supports Oriro script workflows.
import { readFile, rm } from "node:fs/promises";
import path from "node:path";
import { extractLastOriroVersionFromLog } from "./filesystem.ts";
...
export async function packAndServeSmokeArtifact(
  ...
  const artifact = await packOriro({
    destination: tgzDir,
    packageSpec,
    requireControlUi,
  });
...
export async function extractLastOriroVersion(
  runDir: string,
  phaseName: string,
  pattern: RegExp,
): Promise<string> {
  return await extractLastOriroVersionFromLog(path.join(runDir, `${phaseName}.log`), pattern);
}
