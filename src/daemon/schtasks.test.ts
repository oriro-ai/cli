...
describe("resolveTaskScriptPath", () => {
  it.each([
    {
      name: "uses default path when ORIRO_PROFILE is unset",
      env: { USERPROFILE: "C:\\Users\\test" },
      expected: path.join("C:\\Users\\test", ".oriro", "gateway.cmd"),
    },
    {
      name: "uses profile-specific path when ORIRO_PROFILE is set to a custom value",
      env: { USERPROFILE: "C:\\Users\\test", ORIRO_PROFILE: "jbphoenix" },
      expected: path.join("C:\\Users\\test", ".oriro-jbphoenix", "gateway.cmd"),
    },
    {
      name: "prefers ORIRO_STATE_DIR over profile-derived defaults",
      env: {
        USERPROFILE: "C:\\Users\\test",
        ORIRO_PROFILE: "rescue",
        ORIRO_STATE_DIR: "C:\\State\\oriro",
      },
      expected: path.join("C:\\State\\oriro", "gateway.cmd"),
    },
    {
      name: "falls back to HOME when USERPROFILE is not set",
      env: { HOME: "/home/test", ORIRO_PROFILE: "default" },
      expected: path.join("/home/test", ".oriro", "gateway.cmd"),
    },
    {
      name: "uses a custom task script file name inside the state directory",
      env: {
        USERPROFILE: "C:\\Users\\test",
        ORIRO_TASK_SCRIPT_NAME: "gateway-node.cmd",
      },
      expected: path.join("C:\\Users\\test", ".oriro", "gateway-node.cmd"),
    },
  ])("$name", ({ env, expected }) => {
  ...
  it.each([
    ...
    expect(() =>
      resolveTaskScriptPath({
        USERPROFILE: "C:\\Users\\test",
        ORIRO_TASK_SCRIPT_NAME: scriptName,
      }),
    ).toThrow("ORIRO_TASK_SCRIPT_NAME must be a file name only");
  });
});
...
describe("readScheduledTaskCommand", () => {
  async function withScheduledTaskScript(
    ...
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-schtasks-test-"));
    try {
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          "rem Oriro Gateway",
          "cd /d C:\\Projects\\oriro",
          "set NODE_ENV=production",
          "set ORIRO_PORT=18789",
      ...
      async (env) => {
        ...
        expect(result).toEqual({
          ...
          workingDirectory: "C:\\Projects\\oriro",
          environment: {
            NODE_ENV: "production",
            ORIRO_PORT: "18789",
          },
    ...
    await withScheduledTaskScript(
      {
        scriptLines: [
          "@echo off",
          '"\\\\fileserver\\Oriro Share\\node.exe" "\\\\fileserver\\Oriro Share\\dist\\index.js" gateway --port 18789',
        ],
      },
      async (env) => {
        const result = await readScheduledTaskCommand(env);
        expect(result).toEqual({
          programArguments: [
            "\\\\fileserver\\Oriro Share\\node.exe",
            "\\\\fileserver\\Oriro Share\\dist\\index.js",
            "gateway",
            "--port",
            "18789",
          ],
  ...
  it("reads script from ORIRO_STATE_DIR override", async () => {
    await withScheduledTaskScript(
      {
        env: (tmpDir) => ({ ORIRO_STATE_DIR: path.join(tmpDir, "custom-state") }),
