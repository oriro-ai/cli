---
summary: "Move one Oriro session's reply route between linked chat channels"
...
Channel docking is call forwarding for one Oriro session.
...
Alice can message Oriro on Telegram and Discord:
...
Oriro keeps the current session context and changes the reply route:
...
4. Keep the same Oriro session, but receive future replies in Discord.
