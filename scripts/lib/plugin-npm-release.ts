// Plugin Npm Release script supports Oriro repository automation.
...
import { parseReleaseVersion } from "../oriro-npm-release-check.ts";
...
export function collectPublishablePluginPackageErrors(
  ...
  const installNpmSpec = normalizeOptionalString(packageJson.oriro?.install?.npmSpec);
  const repositoryUrl =
    typeof packageJson.repository === "string"
      ? packageJson.repository.trim()
      : (packageJson.repository?.url?.trim() ?? "");
  const extensions = packageJson.oriro?.extensions ?? [];

  if (!packageName.startsWith("@oriro/")) {
    errors.push(
      `package name must start with "@oriro/"; found "${packageName || "<missing>"}".`,
    );
  }
  ...
  if (repositoryUrl !== ORIRO_PLUGIN_NPM_REPOSITORY_URL) {
    errors.push(
      `package.json repository.url must be "${ORIRO_PLUGIN_NPM_REPOSITORY_URL}" so npm provenance can validate GitHub trusted publishing; found "${repositoryUrl || "<missing>"}".`,
    );
  }
  if (!packageVersion) {
  ...
  if (!Array.isArray(extensions) || extensions.length === 0) {
    errors.push("oriro.extensions must contain at least one entry.");
  }
  if (extensions.some((entry) => typeof entry !== "string" || !entry.trim())) {
    errors.push("oriro.extensions must contain only non-empty strings.");
  }
  if (!installNpmSpec) {
    errors.push("oriro.install.npmSpec must be a non-empty string for publishable plugins.");
  }
  errors.push(
...
export function collectPublishablePluginPackages(
  ...
  for (const candidate of collectExtensionPackageJsonCandidates(rootDir)) {
    ...
    publishable.push({
      ...
      installNpmSpec: normalizeOptionalString(packageJson.oriro?.install?.npmSpec),
    });
  }
...
function isPluginVersionPublished(packageName: string, version: string): boolean {
  const tempDir = mkdtempSync(join(tmpdir(), "oriro-plugin-npm-view-"));
