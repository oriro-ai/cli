...
describe("command-analysis risks", () => {
  ...
  it("builds executable payload candidates through carriers and shell wrappers", () => {
    ...
    expect(buildCommandPayloadCandidates(["sudo", "ORIRO_ENV=1", "/approve", "abc"])).toEqual([
      "/approve abc",
    ]);
    ...
    expect(buildCommandPayloadCandidates(["exec", "-a", "oriro", "/approve", "abc"])).toEqual([
      "/approve abc",
    ]);
    ...
    ).toContain("oriro channels login --channel whatsapp");
  });
