...
const LIVE =
  isLiveTestEnabled() &&
  isTruthyEnvValue(process.env.ORIRO_LIVE_INFER_CLI_TEST) &&
...
describeLive("image generation infer CLI live", () => {
  it("generates an image through oriro infer", () => {
    const outputBase = path.join(os.tmpdir(), `oriro-infer-image-${process.pid}.png`);
