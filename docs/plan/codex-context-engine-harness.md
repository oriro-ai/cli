...
summary: "Specification for making the bundled Codex app-server harness honor Oriro context-engine plugins"
read_when:
  - You are wiring context-engine lifecycle behavior into the Codex harness
  - You need lossless-oriro or another context-engine plugin to work with codex/* embedded harness sessions
  - You are comparing embedded Oriro and Codex app-server context behavior
---
...
Make the bundled Codex app-server harness honor the same Oriro context-engine
lifecycle contract that embedded Oriro turns already honor.
...
`lossless-oriro`, control context assembly, post-turn ingest, maintenance, and
Oriro-level compaction policy as far as the Codex app-server boundary allows.

## Non-goals

- Do not reimplement Codex app-server internals.
- Do not make Codex native thread compaction produce a lossless-oriro summary.
...
as built-in Oriro attempts:

- `extensions/codex/src/app-server/run-attempt.ts`

That means the required hook point is in Oriro-controlled code. The external
boundary is the Codex app-server protocol itself: Oriro can control what it
...
Built-in Oriro attempts call the context-engine lifecycle directly:
...
For Codex harness turns, Oriro should preserve this lifecycle:

1. Read the mirrored Oriro session transcript.
...
8. Mirror the Codex result back into the Oriro transcript.
...
11. Preserve Codex native compaction signals and Oriro compaction hooks.

## Design constraints

### Codex app-server remains canonical for native thread state

Codex owns its native thread and any internal extended history. Oriro should
not try to mutate the app-server's internal history except through supported
protocol calls.

Oriro's transcript mirror remains the source for Oriro features:
...
The context-engine interface returns Oriro `AgentMessage[]`, not a Codex
...
For engines like lossless-oriro, the assembled context should be deterministic
...
- Label it clearly as Oriro assembled context.
...
inside Oriro and preserves context-engine semantics.
...
we need strict Oriro parity, run context-engine assembly before hook
...
1. Oriro context-engine `compact()`
2. Codex app-server native `thread/compact/start`

Do not silently conflate them.

#### `/compact` and explicit Oriro compaction

When the selected context engine has `info.ownsCompaction === true`, explicit
Oriro compaction should prefer the context engine's `compact()` result for
the Oriro transcript mirror and plugin state.
...
- If Codex native compaction fails after context-engine compaction succeeds,
  do not fail the whole Oriro compaction when the context engine is primary.
...
Avoid requiring lossless-oriro in Oriro core tests. Use a small in-repo fake
...
- Context-engine-owned compaction remains primary for Oriro/plugin state.
