// Creates temporary Oriro directories for runtime scratch work.
import fs from "node:fs";
import { tmpdir as getOsTmpDir } from "node:os";
import path from "node:path";

/** Preferred shared Oriro temp root on POSIX systems when ownership and permissions are safe. */
export const POSIX_ORIRO_TMP_DIR = "/tmp/oriro";
...
export type ResolvePreferredOriroTmpDirOptions = {
...
/** Resolves a safe Oriro temp root, falling back to user-scoped os.tmpdir paths when needed. */
export function resolvePreferredOriroTmpDir(
  options: ResolvePreferredOriroTmpDirOptions = {},
): string {
  ...
  const fallback = (): string => {
    const suffix = uid === undefined ? "oriro" : `oriro-${uid}`;
  ...
  const tryRepairWritableBits = (candidatePath: string): boolean => {
    try {
      ...
      warn(`[oriro] tightened permissions on temp dir: ${candidatePath}`);
      return resolveDirState(candidatePath) === "available";
    } catch {
      return false;
    }
  ...
  const preferredDir = POSIX_ORIRO_TMP_DIR;
