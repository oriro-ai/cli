---
summary: "Host Oriro on a DigitalOcean Droplet"
read_when:
  - Setting up Oriro on DigitalOcean
  - Looking for a simple paid VPS for Oriro
title: "DigitalOcean"
---

Run a persistent Oriro Gateway on a DigitalOcean Droplet (~$6/month for the 1 GB Basic plan).
...
<Steps>
  ...
  <Step title="Connect and install">
    ...
    # Create the non-root user that will own Oriro state and services.
    adduser oriro
    usermod -aG sudo oriro
    loginctl enable-linger oriro

    su - oriro
    oriro --version
    ```

    Use the root shell only for system bootstrap. Run Oriro commands as the non-root `oriro` user so state lives under `/home/oriro/.oriro/` and the Gateway installs as that user's systemd service.
  ...
  <Step title="Access the Control UI">
    ...
    **Option C: Tailnet bind (no Serve)**
    ...
    ```bash
    oriro config set gateway.bind tailnet
    oriro gateway restart
    ```
...
DigitalOcean snapshots back the whole Droplet up; `oriro backup create` is portable across hosts.
