...
import { readJsonFileWithFallback } from "oriro/plugin-sdk/json-store";
import {
...
import {
  ORIRO_ACPX_LEASE_ID_ARG,
  ORIRO_ACPX_LEASE_ID_ENV,
  ORIRO_GATEWAY_INSTANCE_ID_ARG,
...
const RUN_CONFIGURED_COMMAND_SENTINEL = "--oriro-run-configured";
...
function readManifestDependencyVersion(packageName: string): string {
  ...
  if (typeof version !== "string" || version.trim() === "") {
    throw new Error(`Missing ${packageName} dependency version in @oriro/acpx manifest`);
  }
  return version;
}
...
async function resolveInstalledCodexAcpBinPath(): Promise<string | undefined> {
  // Keep Oriro's isolated CODEX_HOME wrapper, but launch the plugin-local
...
function readOriroWrapperArg(args, name) {
...
function resolveStderrLogPath(args) {
  ...
  const leaseId =
    process.env[${JSON.stringify(ORIRO_ACPX_LEASE_ID_ENV)}] ||
    readOriroWrapperArg(args, ${quoteCommandPart(ORIRO_ACPX_LEASE_ID_ARG)}) ||
    "pid-" + process.pid;
...
function stripOriroWrapperArgs(args) {
  const stripped = [];
...
const configuredArgs = stripOriroWrapperArgs(rawConfiguredArgs);
...
if (!command) {
  console.error("[oriro] missing configured ${params.displayName} ACP command");
  process.exit(1);
}
...
child.on("error", (error) => {
  console.error(\`[oriro] failed to launch ${params.displayName} ACP wrapper: \${error.message}\`);
  process.exit(1);
});
...
function buildClaudeAcpWrapperScript(installedBinPath?: string): string {
  return buildAdapterWrapperScript({
    displayName: "Claude",
    // This package is patched in Oriro; fallback must not float to an unpatched newer release.
