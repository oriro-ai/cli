...
const CODEX_BIND_LIVE = isTruthyEnvValue(process.env.ORIRO_LIVE_CODEX_BIND);
const describeLive = LIVE && CODEX_BIND_LIVE ? describe : describe.skip;
const CODEX_BIND_TIMEOUT_MS = resolveLiveTimeoutMs(
  process.env.ORIRO_LIVE_CODEX_BIND_TIMEOUT_MS,
  900_000,
);
const CODEX_BIND_REQUEST_TIMEOUT_MS = resolveLiveTimeoutMs(
  process.env.ORIRO_LIVE_CODEX_BIND_REQUEST_TIMEOUT_MS,
  300_000,
);
...
async function writePluginBindingApproval(params: {
  ...
  const oriroDir = path.join(params.homeDir, ".oriro");
  await fs.mkdir(oriroDir, { recursive: true });
  await fs.writeFile(
    path.join(oriroDir, "plugin-binding-approvals.json"),
    `${JSON.stringify(
      {
        version: 1,
        approvals: [
...
function resolveCodexBindModelProvider(): string | undefined {
  const configured = process.env.ORIRO_LIVE_CODEX_BIND_PROVIDER?.trim();
  if (configured) {
    return configured;
  }
  return process.env.ORIRO_LIVE_CODEX_HARNESS_AUTH === "api-key" ? "openai" : undefined;
}
  ...
  it(
    ...
    async () => {
      const previous = {
        ...
        gatewayToken: process.env.ORIRO_GATEWAY_TOKEN,
      ...
      const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-live-codex-bind-"));
      ...
      const bindModel =
        process.env.ORIRO_LIVE_CODEX_BIND_MODEL?.trim() || DEFAULT_CODEX_BIND_MODEL;
      ...
      try {
        ...
        try {
          ...
          restoreEnvVar("ORIRO_CONFIG_PATH", previous.configPath);
          restoreEnvVar("ORIRO_GATEWAY_TOKEN", previous.gatewayToken);
          restoreEnvVar("HOME", previous.home);
          restoreEnvVar("ORIRO_SKIP_CANVAS_HOST", previous.skipCanvas);
          restoreEnvVar("ORIRO_SKIP_CHANNELS", previous.skipChannels);
          restoreEnvVar("ORIRO_SKIP_CRON", previous.skipCron);
          restoreEnvVar("ORIRO_SKIP_GMAIL_WATCHER", previous.skipGmail);
          restoreEnvVar("ORIRO_STATE_DIR", previous.stateDir);
        }
      }
    },
