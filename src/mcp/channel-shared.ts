...
/** Resolve the visible message id, including Oriro metadata attached to raw entries. */
export function resolveMessageId(entry: Record<string, unknown>): string | undefined {
  return (
    toText(entry.id) ??
    (entry["__oriro"] && typeof entry["__oriro"] === "object"
      ? toText((entry["__oriro"] as { id?: unknown }).id)
      : undefined)
  );
}
