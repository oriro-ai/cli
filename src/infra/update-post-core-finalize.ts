...
// update.
//
// `runGatewayUpdate` (git mode) runs `oriro doctor --fix` with
// `ORIRO_UPDATE_PARENT_SUPPORTS_DOCTOR_CONFIG_WRITE=1`, which makes the doctor
// pass DEFER configured-plugin repair to a later convergence step (see
// `shouldDeferConfiguredPluginInstallRepair`). The `oriro update` CLI resumes
...
// binary's hidden `oriro update finalize` entrypoint — the designed
...
function buildFinalizeEnv(
  ...
  delete env.ORIRO_SERVICE_MARKER;
  delete env.ORIRO_SERVICE_KIND;
  delete env[GATEWAY_SERVICE_RUNTIME_PID_ENV];
  env[UPDATE_EFFECTIVE_CHANNEL_ENV] = effectiveChannel;
  if (compatHostVersion) {
    env.ORIRO_COMPATIBILITY_HOST_VERSION = compatHostVersion;
  }
  if (sourceConfigPath) {
...
// re-enters the full `oriro update` CLI. Re-run convergence on no-op retries:
...
export async function runPostCoreFinalizeAfterGatewayUpdate(params: {
  ...
  try {
    ...
    if (params.preUpdateConfig) {
      sourceConfigDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-update-post-core-"));
