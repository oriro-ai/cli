...
describe("resolveGatewayInstallEntrypointCandidates", () => {
  it("prefers index.js before legacy entry.js", () => {
    expect(resolveGatewayInstallEntrypointCandidates("/tmp/oriro-root")).toEqual([
      path.join("/tmp/oriro-root", "dist", "index.js"),
      path.join("/tmp/oriro-root", "dist", "index.mjs"),
      path.join("/tmp/oriro-root", "dist", "entry.js"),
      path.join("/tmp/oriro-root", "dist", "entry.mjs"),
    ]);
  });
});
  ...
  it("uses the managed service port ahead of the caller environment", () => {
    expect(
      resolveUpdatedGatewayRestartPort({
        ...
        processEnv: { ORIRO_GATEWAY_PORT: "19001" },
        serviceEnv: { ORIRO_GATEWAY_PORT: "19002" },
      }),
    ).toBe(19002);
  });
...
describe("resolvePostUpdateServiceStateReadEnv", () => {
  it("keeps package restart preparation anchored to the pre-update service env", () => {
    const processEnv = {
      ORIRO_STATE_DIR: "/source/state",
      ORIRO_CONFIG_PATH: "/source/oriro.json",
    } as NodeJS.ProcessEnv;
    const prePackageServiceEnv = {
      ORIRO_STATE_DIR: "/managed/state",
      ORIRO_CONFIG_PATH: "/managed/oriro.json",
    } as NodeJS.ProcessEnv;
  ...
  it("keeps git updates tied to the caller environment", () => {
    const processEnv = { ORIRO_STATE_DIR: "/source/state" } as NodeJS.ProcessEnv;
    const prePackageServiceEnv = { ORIRO_STATE_DIR: "/managed/state" } as NodeJS.ProcessEnv;
  ...
  it("uses the managed service environment for git updates stopped by this updater", () => {
    const processEnv = { ORIRO_STATE_DIR: "/source/state" } as NodeJS.ProcessEnv;
    const preManagedServiceEnv = { ORIRO_STATE_DIR: "/managed/state" } as NodeJS.ProcessEnv;
...
describe("resolvePostInstallDoctorEnv", () => {
  it("uses the managed service profile paths for post-install doctor", () => {
    const env = resolvePostInstallDoctorEnv({
      invocationCwd: "/srv/oriro",
      baseEnv: {
        PATH: "/bin",
        ORIRO_STATE_DIR: "/wrong/state",
        ORIRO_CONFIG_PATH: "/wrong/oriro.json",
        ORIRO_PROFILE: "wrong",
      },
      serviceEnv: {
        ORIRO_STATE_DIR: "daemon-state",
        ORIRO_CONFIG_PATH: "daemon-state/oriro.json",
        ORIRO_PROFILE: "work",
      },
    });
    ...
    expect(env.ORIRO_STATE_DIR).toBe(path.join("/srv/oriro", "daemon-state"));
    expect(env.ORIRO_CONFIG_PATH).toBe(
      path.join("/srv/oriro", "daemon-state", "oriro.json"),
    );
    expect(env.ORIRO_PROFILE).toBe("work");
  });
  ...
  it("keeps the caller env when no managed service env is available", () => {
    const env = resolvePostInstallDoctorEnv({
      baseEnv: {
        PATH: "/bin",
        ORIRO_STATE_DIR: "/caller/state",
        ORIRO_PROFILE: "caller",
      },
    });

    expect(env.PATH).toBe("/bin");
    expect(env.NODE_DISABLE_COMPILE_CACHE).toBe("1");
    expect(env.ORIRO_STATE_DIR).toBe("/caller/state");
    expect(env.ORIRO_PROFILE).toBe("caller");
  });
});

describe("collectMissingPluginInstallPayloads", () => {
  it("reports tracked npm install records whose package payload is absent", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-update-plugin-payload-"));
    const presentDir = path.join(tmpDir, "state", "npm", "node_modules", "@oriro", "present");
    const missingDir = path.join(tmpDir, "state", "npm", "node_modules", "@oriro", "missing");
    ...
    try {
      ...
      await fs.writeFile(path.join(presentDir, "package.json"), '{"name":"@oriro/present"}\n');
      ...
      await expect(
        collectMissingPluginInstallPayloads({
          ...
          records: {
            present: {
              source: "npm",
              spec: "@oriro/present@beta",
              installPath: presentDir,
            },
            missing: {
              source: "npm",
              spec: "@oriro/missing@beta",
              installPath: missingDir,
            },
            "no-package-json": {
              source: "npm",
              spec: "@oriro/no-package-json@beta",
              installPath: noPackageJsonDir,
            },
            "missing-install-path": {
              source: "npm",
              spec: "@oriro/missing-install-path@beta",
            },
            local: {
              source: "path",
  ...
  it("keeps disabled trusted official npm records eligible for payload repair when requested", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-update-plugin-payload-"));
    const missingDir = path.join(tmpDir, "state", "npm", "node_modules", "@oriro", "codex");
    try {
      await expect(
        collectMissingPluginInstallPayloads({
          env: { HOME: tmpDir } as NodeJS.ProcessEnv,
  ...
  it("keeps disabled trusted official OriroHub records eligible for payload repair when requested", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-update-plugin-payload-"));
    const missingDir = path.join(tmpDir, "state", "orirohub", "diagnostics-otel");
    try {
      await expect(
        collectMissingPluginInstallPayloads({
          env: { HOME: tmpDir } as NodeJS.ProcessEnv,
...
describe("formatPostUpdateGatewayRecoveryInstructions", () => {
  ...
  it("uses systemd wording on Linux instead of macOS LaunchAgent instructions", () => {
    ...
    expect(line).toContain("oriro gateway restart");
    expect(line).toContain("oriro gateway install --force");
    expect(line).toContain("oriro gateway status --deep");
...
describe("recoverInstalledLaunchAgentAfterUpdate", () => {
  it("re-bootstraps an installed-but-not-loaded macOS LaunchAgent after update", async () => {
    ...
    const serviceEnv = { ORIRO_PROFILE: "stomme" } as NodeJS.ProcessEnv;
    const recoveredEnv = { ...serviceEnv, ORIRO_PORT: "18790" } as NodeJS.ProcessEnv;
  ...
  it("does not recover a loaded LaunchAgent", async () => {
    const readState = vi.fn(async () => ({
      ...
      loaded: true,
      running: true,
      env: { ORIRO_PROFILE: "stomme" } as NodeJS.ProcessEnv,
      command: null,
  ...
  it("does not report recovered update health until the gateway passes the post-recovery wait", async () => {
    ...
    await expect(
      recoverLaunchAgentAndRecheckGatewayHealth({
        ...
        env: { ORIRO_PROFILE: "stomme", ORIRO_PORT: "18790" },
...
describe("resolvePostCoreUpdateChildStdio", () => {
  it('returns "pipe" on Windows so the child never inherits the parent console handles', () => {
    ...
    // causing the terminal to hang after `oriro update` completes (#78445).
