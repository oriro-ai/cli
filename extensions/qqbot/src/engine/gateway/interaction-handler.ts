 ...
 *
 * 1. **Config query**  (type=2001) — reads config, ACKs with `oriro_cfg`.
...
import { isImplicitSameChatApprovalAuthorization } from "oriro/plugin-sdk/approval-auth-runtime";
...
/**
 * Build the canonical `oriro_cfg` snapshot returned in interaction ACKs.
 *
 * Pure function — all resolution helpers live in engine/config/.
 */
function buildOriroCfgSnapshot(
...
/** Apply a config-update interaction and return the updated oriro_cfg. */
async function applyConfigUpdate(
  ...
  const oriroCfgUpdate = resolved?.oriro_cfg as Record<string, unknown> | undefined;
  const groupOpenid = event.group_openid ?? "";

  const currentCfg = structuredClone(configApi.current());
  let changed = false;

  if (oriroCfgUpdate?.require_mention !== undefined && groupOpenid) {
    applyRequireMentionUpdate(currentCfg, accountId, groupOpenid, oriroCfgUpdate);
    changed = true;
  }
  ...
  if (changed) {
    ...
    log?.info(
      `Config updated via interaction ${event.id}: require_mention=${String(oriroCfgUpdate?.require_mention)}, group=${groupOpenid}`,
    );
  }

  const latestCfg = changed ? configApi.current() : currentCfg;
  return buildOriroCfgSnapshot(latestCfg, accountId, groupOpenid, runtime);
}
  ...
  return (event) => {
    ...
    if (type === InteractionType.CONFIG_QUERY && runtime.config) {
      void handleWithAck(creds, event, log, "CONFIG_QUERY", () => {
        ...
        return buildOriroCfgSnapshot(cfg, account.accountId, event.group_openid ?? "", runtime);
      });
      return;
    }
...
async function handleApprovalButtonInteraction(params: {
  ...
  event: InteractionEvent;
  getActiveCfg?: () => OriroConfig | Record<string, unknown>;
  log?: EngineLogger;
  ...
  let cfg: OriroConfig;
  try {
    cfg = params.getActiveCfg() as OriroConfig;
  } catch (err) {
    await acknowledgeApprovalInteraction(params.creds, params.event, params.log, {
      content: "Approval is unavailable.",
    });
...
async function handleWithAck(
  ...
  try {
    const oriroCfg = await handler();
    await acknowledgeInteraction(creds, event.id, 0, { oriro_cfg: oriroCfg });
