...
// Test Force script supports Oriro repository automation.
...
function runTests() {
  const isolatedLock =
    process.env.ORIRO_GATEWAY_LOCK ??
    path.join(os.tmpdir(), `oriro-gateway.lock.test.${Date.now()}`);
  const result = spawnSync(process.execPath, ["scripts/test-projects.mjs"], {
    stdio: "inherit",
    env: {
      ...process.env,
      ORIRO_GATEWAY_LOCK: isolatedLock,
    },
  });
  if (result.error) {
