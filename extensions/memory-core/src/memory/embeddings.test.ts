...
import type { EmbeddingProviderAdapter } from "oriro/plugin-sdk/embedding-providers";
import type { MemoryEmbeddingProviderAdapter } from "oriro/plugin-sdk/memory-core-host-engine-embeddings";
...
const mockEmbeddingRegistry = vi.hoisted(() => ({
  ...
  genericLookupConfigs: [] as Array<OriroConfig | undefined>,
}));

vi.mock("oriro/plugin-sdk/embedding-providers", () => ({
  getEmbeddingProvider: (id: string, config?: OriroConfig) => {
...
vi.mock("oriro/plugin-sdk/memory-core-host-engine-embeddings", () => ({
