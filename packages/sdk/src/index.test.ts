...
class FakeTransport implements OriroTransport {
...
class EventsOnlyTransport implements OriroTransport {
...
describe("Oriro SDK", () => {
  ...
  it("rejects normalized event streams when the event pump fails before yielding", async () => {
    ...
    let futureIterator: AsyncIterator<OriroEvent> | undefined;
  ...
  it("uses cumulative text for the first replayed chat projection", async () => {
    ...
    let text = "";
    let iterator: AsyncIterator<OriroEvent> | undefined;
