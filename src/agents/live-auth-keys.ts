...
const GOOGLE_LIVE_SINGLE_KEY = "ORIRO_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "ORIRO_LIVE_ANTHROPIC_KEY",
    listVar: "ORIRO_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "ORIRO_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `ORIRO_LIVE_${base}_KEY`;
...
export function collectProviderApiKeys(
  ...
  const forcedSingle = config.liveSingle
    ...
    : undefined;
  if (forcedSingle) {
    // ORIRO_LIVE_*_KEY pins a single key so retries do not rotate fixtures.
    return [forcedSingle];
  }
