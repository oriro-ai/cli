...
export function createPackageManagerWarningMessage(packageManager) {
  ...
  return [
    `[oriro] warning: detected ${packageManager} for install lifecycle.`,
    "[oriro] this repo works best with pnpm; npm-compatible installs are slower and much larger here.",
    "[oriro] prefer: corepack pnpm install",
  ].join("\n");
}
