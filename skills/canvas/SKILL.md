...
name: canvas
description: "Present HTML on connected ORIRO node canvases, navigate/eval/snapshot, and debug canvas host URLs."
metadata: { "oriro": { "emoji": "🖼️" } }
---
...
Active config: `$ORIRO_CONFIG_PATH` or `~/.oriro/oriro.json`.
...
4. Present the hosted URL: `/__oriro__/canvas/<file>.html`.
5. Use `snapshot` when the user needs proof.

## URL shape

```text
http://<gateway-host>:<gateway.port>/__oriro__/canvas/index.html
http://<gateway-host>:<gateway.port>/__oriro__/canvas/games/snake.html
```

Path mapping:

- `/__oriro__/canvas/index.html` -> `<canvas host root>/index.html`
- `/__oriro__/canvas/games/snake.html` -> `<canvas host root>/games/snake.html`
