...
import { asNullableObjectRecord as readRecord } from "@oriro/normalization-core/record-coerce";
import {
...
export function getTestTailscaleBinaryOverride(
  ...
  const forcedBinary = env.ORIRO_TEST_TAILSCALE_BINARY?.trim();
  if (!forcedBinary) {
    return null;
  }
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for Oriro. You can keep running the web gateway without it: \`${formatCliCommand("oriro gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
