...
/** Return true when an installed package needs an oriro peer link repair. */
export function installedPackageNeedsOriroPeerLinkRepair(dir: string): boolean {
  ...
  try {
    fsSync.statSync(path.join(dir, "node_modules", "oriro"));
    return false;
  } catch (error) {
