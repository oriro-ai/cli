...
function isBlockedConfigEnvVar(key: string): boolean {
  return (
    ...
    key.toUpperCase() === "ORIRO_INCLUDE_ROOTS" ||
...
function collectConfigEnvVarsByTarget(cfg?: OriroConfig): Record<string, string> {
...
export function collectConfigRuntimeEnvVars(cfg?: OriroConfig): Record<string, string> {
  return collectConfigEnvVarsByTarget(cfg);
}

/** Collects config env vars safe to persist into managed service environments. */
export function collectConfigServiceEnvVars(cfg?: OriroConfig): Record<string, string> {
...
export function applyConfigEnvVars(
  ...
  for (const [key, value] of Object.entries(entries)) {
    ...
    // (e.g. process.env.ORIRO_GATEWAY_TOKEN = "${VAULT_TOKEN}") which downstream auth
