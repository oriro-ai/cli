...
Optional custom relay:
  ORIRO_PUSH_RELAY_BASE_URL=https://relay.example.com \
    scripts/ios-release-prepare.sh --build-number 7 [--team-id TEAMID]
...
- regenerates apps/ios/Oriro.xcodeproj via xcodegen
EOF
}
...
DEFAULT_IOS_PUSH_RELAY_BASE_URL="https://ios-push-relay.oriro.ai"
PUSH_RELAY_BASE_URL="${ORIRO_PUSH_RELAY_BASE_URL:-${IOS_PUSH_RELAY_BASE_URL:-${DEFAULT_IOS_PUSH_RELAY_BASE_URL}}}"
...
validate_push_relay_base_url() {
  ...
  if [[ "${value}" =~ [[:space:]] ]]; then
    echo "Invalid ORIRO_PUSH_RELAY_BASE_URL: whitespace is not allowed." >&2
    exit 1
  fi

  if [[ "${value}" == *'$'* || "${value}" == *'('* || "${value}" == *')'* || "${value}" == *'='* ]]; then
    echo "Invalid ORIRO_PUSH_RELAY_BASE_URL: contains forbidden xcconfig characters." >&2
    exit 1
  fi

  if [[ ! "${value}" =~ ^https://[A-Za-z0-9.-]+(:([0-9]{1,5}))?(/[A-Za-z0-9._~!&*+,;:@%/-]*)?$ ]]; then
    echo "Invalid ORIRO_PUSH_RELAY_BASE_URL: expected https://host[:port][/path]." >&2
    exit 1
  fi

  local port="${BASH_REMATCH[2]:-}"
  if [[ -n "${port}" ]] && (( 10#${port} > 65535 )); then
    echo "Invalid ORIRO_PUSH_RELAY_BASE_URL: port must be between 1 and 65535." >&2
    exit 1
  fi
}
...
if [[ "${TEAM_ID}" != "${CANONICAL_TEAM_ID}" ]]; then
  echo "iOS App Store release must use canonical ORIRO Team ID ${CANONICAL_TEAM_ID}; got ${TEAM_ID}." >&2
  exit 1
fi
...
PUSH_RELAY_BASE_URL_XCCONFIG="$(
  printf '%s' "${PUSH_RELAY_BASE_URL}" \
    | sed 's#//#$(ORIRO_URL_SLASH)$(ORIRO_URL_SLASH)#g'
)"
...
${RELEASE_SIGNING_XCCONFIG}
ORIRO_DEVELOPMENT_TEAM = ${TEAM_ID}
ORIRO_IOS_SELECTED_TEAM = ${TEAM_ID}
ORIRO_APP_BUNDLE_ID = ai.orirofoundation.app
ORIRO_SHARE_BUNDLE_ID = ai.orirofoundation.app.share
ORIRO_ACTIVITY_WIDGET_BUNDLE_ID = ai.orirofoundation.app.activitywidget
ORIRO_WATCH_APP_BUNDLE_ID = ai.orirofoundation.app.watchkitapp
ORIRO_APNS_ENTITLEMENT_ENVIRONMENT = production
ORIRO_PUSH_TRANSPORT = relay
ORIRO_PUSH_DISTRIBUTION = official
ORIRO_URL_SLASH = /
ORIRO_PUSH_RELAY_BASE_URL = ${PUSH_RELAY_BASE_URL_XCCONFIG}
ORIRO_PUSH_APNS_ENVIRONMENT = production
EOF
