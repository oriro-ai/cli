...
import { isTranscriptOnlyOriroAssistantModel } from "../../shared/transcript-only-oriro-assistant.js";
...
function parseAssistantTranscriptText(
  line: string,
  options?: { excludeTranscriptOnlyOriroAssistant?: boolean },
  ...
  if (
    options?.excludeTranscriptOnlyOriroAssistant &&
    isTranscriptOnlyOriroAssistantMessage(message)
  ) {
    return undefined;
  }
...
function isTranscriptOnlyOriroAssistantMessage(message: {
  provider?: unknown;
  model?: unknown;
}): boolean {
  return isTranscriptOnlyOriroAssistantModel(message.provider, message.model);
}
...
function parseRecentConversationText(line: string): SessionRecentConversationText | undefined {
  ...
  if (message.role === "assistant" && isTranscriptOnlyOriroAssistantMessage(message)) {
    return undefined;
  }
  const text =
  ...
  for await (const line of streamSessionTranscriptLinesReverse(sessionFile)) {
    try {
      const assistantText = parseAssistantTranscriptText(line, {
        excludeTranscriptOnlyOriroAssistant: true,
      });
      if (assistantText) {
...
export async function readTailAssistantTextFromSessionTranscript(
  ...
  for await (const line of streamSessionTranscriptLinesReverse(sessionFile)) {
    try {
      ...
      // Skip non-message entries (e.g. `oriro.cache-ttl` custom events) so
...
export async function appendAssistantMessageToSessionTranscript(params: {
  ...
  return appendExactAssistantMessageToSessionTranscript({
    ...
    message: {
      ...
      timestamp: Date.now(),
      ...(params.deliveryMirror ? { oriroDeliveryMirror: params.deliveryMirror } : {}),
...
function isRedundantDeliveryMirror(message: SessionTranscriptAssistantMessage): boolean {
  return message.provider === "oriro" && message.model === "delivery-mirror";
}

function isChannelFinalDeliveryMirror(message: SessionTranscriptAssistantMessage): boolean {
  const marker = (message as { oriroDeliveryMirror?: SessionTranscriptDeliveryMirror })
    .oriroDeliveryMirror;
