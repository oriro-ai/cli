...
Explicit copy flows, such as `oriro agents add`, use this portability policy:
...
has such a marker, `oriro doctor --fix` moves it to `auth.profiles` and
...
- Probe targets can come from auth profiles, environment credentials, or
  `models.json`.
- If a provider has credentials but Oriro cannot resolve a probeable model
