...
import { buildOriroUserAgentFragment } from "./user-agent.js";
...
type MSTeamsAppOn = {
  ...
  // delegates back into the SDK handlers after Oriro sender policy passes.
...
export async function createMSTeamsApp(
  ...
  // backend can identify Oriro traffic for usage telemetry. Teams SDK
  ...
  const appOptions: Record<string, unknown> = {
    client: options?.httpClient ?? {
      headers: { "User-Agent": buildOriroUserAgentFragment() },
    },
