...
const DEFAULT_ORIROHUB_REPO_CANDIDATES = [
  path.resolve(ROOT, "..", "orirohub-docs-orirohub"),
  path.resolve(ROOT, "..", "orirohub"),
];
const SYNC_SUPPORT_FILES = [
  {
    source: path.join(ROOT, "scripts", "check-docs-mdx.mjs"),
    target: path.join(".oriro-sync", "check-docs-mdx.mjs"),
  },
  {
    source: path.join(ROOT, "scripts", "lib", "mintlify-accordion.mjs"),
    target: path.join(".oriro-sync", "lib", "mintlify-accordion.mjs"),
  },
  {
    source: path.join(ROOT, ".github", "codex", "prompts", "docs-mdx-repair.md"),
    target: path.join(".oriro-sync", "docs-mdx-repair.md"),
  },
];
...
export function parseArgs(argv) {
  const args = {
    ...
    sourceRepo: "",
    sourceSha: "",
    orirohubRepo: process.env[ORIROHUB_REPO_ENV] || "",
    orirohubSourceRepo:
      process.env.ORIRO_DOCS_SYNC_ORIROHUB_SOURCE_REPO || DEFAULT_ORIROHUB_SOURCE_REPO,
    orirohubSourceSha: process.env.ORIRO_DOCS_SYNC_ORIROHUB_SOURCE_SHA || "",
  };
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    switch (part) {
      ...
      case "--source-sha":
        ...
        break;
      case "--orirohub-repo":
        args.orirohubRepo = readOptionValue(argv, index, part);
        index += 1;
        break;
      case "--orirohub-source-repo":
        args.orirohubSourceRepo = readOptionValue(argv, index, part);
        index += 1;
        break;
      case "--orirohub-source-sha":
        args.orirohubSourceSha = readOptionValue(argv, index, part);
        index += 1;
        break;
      default:
...
/**
 * Resolves the local OriroHub repository path used for docs mirroring.
 */
export function resolveOriroHubRepoPath(value = "", options = {}) {
  const required = options.required !== false;
  const candidates = [
    value,
    process.env[ORIROHUB_REPO_ENV] || "",
    ...DEFAULT_ORIROHUB_REPO_CANDIDATES,
  ...
  if (required) {
    throw new Error(`missing OriroHub docs source; pass --orirohub-repo or set ${ORIROHUB_REPO_ENV}`);
  }
  return "";
}
...
function shouldExcludeOriroHubDocsPath(relativePath) {
...
function toOriroHubTargetRelativePath(relativePath) {
  const normalized = normalizeSlashes(relativePath);
  if (normalized === "README.md") {
    return "";
  }
  if (normalized === "orirohub.md") {
    return "index.md";
  }
  return normalized.replace(/\/README\.md$/iu, "/index.md");
}

function toOriroHubDocsRoute(relativePath) {
  const targetRelativePath = toOriroHubTargetRelativePath(relativePath);
  ...
  if (normalized === "index") {
    return `/${ORIROHUB_DOCS_TARGET_DIR}`;
  }
  if (normalized.endsWith("/index")) {
    return `/${ORIROHUB_DOCS_TARGET_DIR}/${normalized.slice(0, -"/index".length)}`;
  }
  return `/${ORIROHUB_DOCS_TARGET_DIR}/${normalized}`;
}
...
function rewriteOriroHubMarkdownLinkTarget(rawTarget, relativeSourceDir, source) {
  ...
  const route = toOriroHubDocsRoute(normalizedRelative);
  return route ? `${route}${rest}${suffix}` : rawTarget;
}

function rewriteOriroHubMarkdownLinks(raw, relativeSourcePath, source) {
  ...
  return raw.replace(/(!?\[[^\]]*\]\()([^)]+)(\))/gu, (_match, prefix, target, suffix) => {
    return `${prefix}${rewriteOriroHubMarkdownLinkTarget(target, baseDir, source)}${suffix}`;
  });
}

/**
 * Mirrors OriroHub docs into the target docs tree.
 */
export function syncOriroHubDocsTree(targetDocsDir, options = {}) {
  const repoPath = resolveOriroHubRepoPath(options.repoPath || "", {
    required: options.required !== false,
  });
  if (!repoPath) {
    return {
      repository: options.sourceRepo || DEFAULT_ORIROHUB_SOURCE_REPO,
  ...
  const targetDir = path.join(targetDocsDir, ORIROHUB_DOCS_TARGET_DIR);
  const source = {
    repository: options.sourceRepo || DEFAULT_ORIROHUB_SOURCE_REPO,
  ...
  for (const sourcePath of walkFiles(sourceDocsDir)) {
    ...
    if (shouldExcludeOriroHubDocsPath(relativeSourcePath)) {
      continue;
    }

    const targetRelativePath = toOriroHubTargetRelativePath(relativeSourcePath);
    if (!targetRelativePath) {
      continue;
    }
    ...
    if (/\.mdx?$/iu.test(sourcePath)) {
      ...
      fs.writeFileSync(
        targetPath,
        rewriteOriroHubMarkdownLinks(raw, relativeSourcePath, source),
        "utf8",
      );
    } else {
  ...
  console.log(`Synced ${copied} OriroHub doc asset(s) from ${repoPath}.`);
  return {
    ...source,
    path: repoPath,
    files: copied,
...
function syncDocsTree(targetRoot, options = {}) {
  ...
  const orirohubSource = syncOriroHubDocsTree(targetDocsDir, {
    repoPath: options.orirohubRepo,
    sourceRepo: options.orirohubSourceRepo,
    sourceSha: options.orirohubSourceSha,
  });
  ...
  return { orirohub: orirohubSource };
}
...
function writeSyncMetadata(targetRoot, args, sources) {
  const metadata = {
    ...
    sources: {
      ...
      orirohub: {
        repository:
          sources.orirohub.repository || args.orirohubSourceRepo || DEFAULT_ORIROHUB_SOURCE_REPO,
        sha: sources.orirohub.sha || args.orirohubSourceSha || "",
      },
    },
    syncedAt: new Date().toISOString(),
  };
  writeJson(path.join(targetRoot, ".oriro-sync", "source.json"), metadata);
}
...
function main() {
  ...
  const orirohubRepo = resolveOriroHubRepoPath(args.orirohubRepo);
  const sources = syncDocsTree(targetRoot, {
    orirohubRepo,
    orirohubSourceRepo: args.orirohubSourceRepo,
    orirohubSourceSha: args.orirohubSourceSha,
  });
