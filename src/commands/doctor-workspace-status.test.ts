...
async function runNoteWorkspaceStatusForTest(
  ...
  const cfg: OriroConfig = opts?.cfg ?? {};
...
describe("noteWorkspaceStatus", () => {
  ...
  it("surfaces active official managed plugin version drift", async () => {
    ...
    try {
      ...
      expect(body).toContain("1 active official plugin not on Oriro 2026.6.1");
      expect(body).toContain("codex: 2026.5.30-beta.1 (npm) -> expected 2026.6.1");
      expect(body).toContain("oriro plugins update codex");
      expect(body).toContain("oriro gateway restart");
    } finally {
      noteSpy.mockRestore();
    }
  });
  ...
  it("adds TaskFlow recovery hints for broken blocked flows", async () => {
    ...
    try {
      ...
      expect(body).toContain("oriro tasks flow show <flow-id>");
    } finally {
      noteSpy.mockRestore();
    }
  });
