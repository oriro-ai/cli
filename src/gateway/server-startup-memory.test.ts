...
async function startMemoryBackendForTest(cfg: OriroConfig) {
  const log = createGatewayLogMock();
  await startGatewayMemoryBackend({ cfg, log });
  return log;
}

async function startQmdBackendWithManager(cfg: OriroConfig) {
...
function expectQmdManagerRequests(cfg: OriroConfig, agentIds: string[]) {
