...
function asPluginInstallConfig(records: PluginInstallRecords): OriroConfig {
  return {
    plugins: {
      installs: records,
    },
...
export function startGatewayConfigReloader(opts: {
  ...
  onHotReload: (plan: GatewayReloadPlan, nextConfig: OriroConfig) => Promise<void>;
  onRestart: (plan: GatewayReloadPlan, nextConfig: OriroConfig) => void | Promise<void>;
  ...
  const queueRestart = (plan: GatewayReloadPlan, nextConfig: OriroConfig) => {
    if (restartQueued) {
      return;
    }
    restartQueued = true;
