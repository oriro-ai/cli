/** Builds the interactive `oriro secrets configure` target list and apply plan. */
...
/** Credential target shown by `oriro secrets configure` before a SecretRef is selected. */
...
function getSecretProviders(config: OriroConfig): Record<string, SecretProviderConfig> {
...
/** Builds configure candidates for the current Oriro config only. */
export function buildConfigureCandidates(config: OriroConfig): ConfigureCandidate[] {
...
/** Builds configure candidates for Oriro config plus an optional auth-profile scope. */
export function buildConfigureCandidatesForScope(params: {
  config: OriroConfig;
  authoredOriroConfig?: OriroConfig;
  authProfiles?: {
    agentId: string;
    store: AuthProfileStore;
  };
}): ConfigureCandidate[] {
  const authoredConfig = params.authoredOriroConfig ?? params.config;

  const hasPathInAuthoredConfig = (pathSegments: string[]): boolean =>
    hasPath(authoredConfig, pathSegments);

  const oriroCandidates = discoverConfigSecretTargets(params.config)
      ...
      return Object.assign(
        {
          ...
          label: entry.path,
          configFile: `oriro.json` as const,
  ...
  return [...oriroCandidates, ...authCandidates].toSorted((a, b) =>
...
export function buildSecretsConfigurePlan(params: {
  ...
  return {
    ...
    generatedBy: "oriro secrets configure",
