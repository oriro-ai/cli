...
import { OriroChannelBridge } from "./channel-bridge.js";
import { ClaudePermissionRequestSchema, type ClaudeChannelMode } from "./channel-shared.js";
import { getChannelMcpCapabilities, registerChannelMcpTools } from "./channel-tools.js";

/**
 * MCP stdio server assembly for Oriro channel conversations.
 *
 ...
 */
export { OriroChannelBridge } from "./channel-bridge.js";

/** Options accepted by the channel MCP server factory and stdio entry point. */
type OriroMcpServeOptions = {
  gatewayUrl?: string;
  gatewayToken?: string;
...
async function resolveMcpConfig(config: OriroConfig | undefined): Promise<OriroConfig> {
  if (config) {
    return config;
  }
...
export async function createOriroChannelMcpServer(opts: OriroMcpServeOptions = {}): Promise<{
  server: McpServer;
  bridge: OriroChannelBridge;
  start: () => Promise<void>;
  close: () => Promise<void>;
}> {
  ...
  const server = new McpServer(
    { name: "oriro", version: VERSION },
    capabilities ? { capabilities } : undefined,
  );
  const bridge = new OriroChannelBridge(cfg, {
...
export async function serveOriroChannelMcp(opts: OriroMcpServeOptions = {}): Promise<void> {
  const { server, start, close } = await createOriroChannelMcpServer(opts);
