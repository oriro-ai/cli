...
function resolveLegacyDebugProxyCapturePaths(
  ...
  return {
    sourcePath: env.ORIRO_DEBUG_PROXY_DB_PATH?.trim() || path.join(rootDir, "capture.sqlite"),
    blobDir: env.ORIRO_DEBUG_PROXY_BLOB_DIR?.trim() || path.join(rootDir, "blobs"),
  };
}
...
export function detectLegacyDebugProxyCaptureSidecar(
  ...
  if (
    ...
    path.resolve(resolveOriroStateSqlitePath({ ...env, ORIRO_STATE_DIR: stateDir }))
  ) {
