...
describe("doctor browser facade", () => {
  ...
  it("delegates legacy orirod browser profile detection to the browser facade surface", async () => {
    const residue = {
    ...
    loadBundledPluginPublicSurfaceModuleSync.mockReturnValue({
      ...
      detectLegacyOrirodBrowserProfileResidue: detect,
    });
    const cfg: OriroConfig = {
      browser: {
        profiles: {
    ...
    const deps = {
      ...
      pathExists: (targetPath: string) => targetPath === "/tmp/oriro-home/browser/orirod",
    };

    await expect(detectLegacyOrirodBrowserProfileResidue(cfg, deps)).resolves.toEqual(residue);
    expect(loadBundledPluginPublicSurfaceModuleSync).toHaveBeenCalledWith({
      dirName: "browser",
      artifactBasename: "browser-doctor.js",
    });
  ...
  it("delegates legacy orirod browser profile cleanup to the browser facade surface", async () => {
    const cleanup = vi.fn().mockResolvedValue({ changes: ["archived"], warnings: [] });
    loadBundledPluginPublicSurfaceModuleSync.mockReturnValue({
      noteChromeMcpBrowserReadiness: vi.fn(),
      maybeArchiveLegacyOrirodBrowserProfileResidue: cleanup,
    });
    ...
    await expect(maybeArchiveLegacyOrirodBrowserProfileResidue(cfg, deps)).resolves.toEqual({
      changes: ["archived"],
      warnings: [],
    });
    expect(loadBundledPluginPublicSurfaceModuleSync).toHaveBeenCalledWith({
