...
describe("Docker E2E client scripts", () => {
  ...
  it("keeps session runtime-context checks wired to packaged transcript behavior", () => {
    ...
    expect(source).toContain("oriro/plugin-sdk/agent-sessions");
    ...
    expect(source).toContain("<<<BEGIN_ORIRO_INTERNAL_CONTEXT>>>");
    expect(source).toContain("oriro.runtime-context");
