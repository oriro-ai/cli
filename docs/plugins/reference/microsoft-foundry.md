---
summary: "Adds Microsoft Foundry model provider support to Oriro."
read_when:
...
Adds Microsoft Foundry model provider support to Oriro.

## Distribution

- Package: `@oriro/microsoft-foundry`
- Install route: included in Oriro

## Surface

providers: microsoft-foundry; contracts: imageGenerationProviders

<!-- oriro-plugin-reference:manual-start -->
...
- For Entra ID auth, install the Azure CLI and run `az login` before
  onboarding. Oriro refreshes Microsoft Foundry runtime tokens through
...
Oriro uses the Foundry `/openai/v1` endpoint for supported OpenAI-compatible
chat APIs:
...
Claude model ID, set `params.canonicalModelId` on the model entry so Oriro
...
deployment through onboarding or include model metadata so Oriro can verify
...
<!-- oriro-plugin-reference:manual-end -->
