...
const { clearPluginCommands, getPluginCommandSpecs, loadOriroPlugins, matchPluginCommand } =
  await import(pathToFileURL(smokeEntryPath).href);

assert.equal(typeof loadOriroPlugins, "function", "built loader export missing");
...
const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-build-smoke-"));
...
fs.writeFileSync(
  path.join(distPluginDir, "oriro.plugin.json"),
  JSON.stringify(
    {
      id: pluginId,
      configSchema: {
...
fs.writeFileSync(
  ...
  [
    "import sdk from 'oriro/plugin-sdk';",
...
const registry = loadOriroPlugins({
  ...
  env: {
    ...process.env,
    ORIRO_BUNDLED_PLUGINS_DIR: path.join(repoRoot, "dist-runtime", "extensions"),
  },
  config: {
    plugins: {
      enabled: true,
