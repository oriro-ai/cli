...
import { resolveAuthStorePathForDisplay } from "oriro/plugin-sdk/agent-runtime";
...
vi.mock("oriro/plugin-sdk/provider-auth", async (importOriginal) => ({
  ...(await importOriginal<typeof import("oriro/plugin-sdk/provider-auth")>()),
...
async function makeTempRoot() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-hermes-secret-failure-"));
  tempRoots.add(root);
  return root;
}
