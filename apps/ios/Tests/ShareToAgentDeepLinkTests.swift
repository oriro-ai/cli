...
@Suite struct ShareToAgentDeepLinkTests {
    @Test func appGroupIdentifierUsesCanonicalOriroGroup() {
        #expect(OriroAppGroup.canonicalIdentifier == "group.ai.orirofoundation.app.shared")
    }
    ...
    @Test func buildURLEncodesAgentRoute() {
        ...
        guard case let .agent(agent)? = parsed else {
            Issue.record("Expected oriro://agent deep link")
            return
        }
