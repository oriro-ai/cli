...
/** Supported package managers for Oriro global install and update flows. */
...
const PRIMARY_PACKAGE_NAME = "oriro";
...
function isPnpmOriroSourceInstallSpec(spec: string): boolean {
...
/**
 * Extracts a pinned installed version from package specs like `oriro@1.2.3`.
...
/**
 * Verifies that a global package root looks like a packaged Oriro install
  ...
  await Promise.all(
    BUNDLED_RUNTIME_SIDECAR_PATHS.map(async (relativePath) => {
      ...
      if (
        ...
        (await pathExists(path.join(packageRoot, pluginRoot, "oriro.plugin.json")))
      ) {
...
export function resolveGlobalInstallSpec(params: {
  ...
  const override =
    params.env?.ORIRO_UPDATE_PACKAGE_SPEC?.trim() ||
    process.env.ORIRO_UPDATE_PACKAGE_SPEC?.trim();
  if (override) {
    return override;
  }
  const target = normalizePackageTarget(params.tag);
  if (isMainPackageTarget(target)) {
    return ORIRO_MAIN_PACKAGE_SPEC;
  }
 ...
 * Used for staged installs where Oriro creates the prefix itself.
 */
...
/**
 * Detects an installed global Oriro package by probing package-manager roots
...
/**
 * Builds the primary package-manager argv for a global Oriro install.
...
export function globalInstallArgs(
  ...
  if (resolved.manager === "pnpm") {
    return [
      ...
      ...(isPnpmOriroSourceInstallSpec(spec) ? [PNPM_ORIRO_BUILD_ALLOWLIST_FLAG] : []),
      spec,
    ];
  }
  if (resolved.manager === "bun") {
