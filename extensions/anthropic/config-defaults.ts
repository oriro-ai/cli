 ...
 */
import type { OriroConfig } from "oriro/plugin-sdk/plugin-entry";
import {
  isRecord,
...
function usesClaudeCliModelSelection(config: OriroConfig): boolean {
...
function usesSelectedClaudeCliAuthProfile(config: OriroConfig): boolean {
  const profiles = config.auth?.profiles ?? {};
...
function collectClaudeCliRuntimeRefsFromConfig(config: OriroConfig): string[] {
  const refs = new Set<string>(
...
/** Apply Anthropic and Claude CLI defaults to an Oriro config object. */
