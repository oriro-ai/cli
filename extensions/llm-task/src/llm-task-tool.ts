...
import { buildModelAliasIndex, resolveModelRefFromString } from "oriro/plugin-sdk/agent-runtime";
import {
...
import { resolvePreferredOriroTmpDir, withTempWorkspace } from "../api.js";
...
type ThinkingPolicy = ReturnType<OriroPluginApi["runtime"]["agent"]["resolveThinkingPolicy"]>;
...
function supportsThinkingPolicyLevel(
  ...
  level: ReturnType<OriroPluginApi["runtime"]["agent"]["normalizeThinkingLevel"]>,
): boolean {
  return Boolean(level) && policy.levels.some((entry) => entry.id === level);
}

export function createLlmTaskTool(api: OriroPluginApi) {
  return {
    ...
    async execute(_id: string, params: LlmTaskParams) {
      ...
      let thinkLevel: ReturnType<OriroPluginApi["runtime"]["agent"]["normalizeThinkingLevel"]> =
        undefined;
      if (thinkingRaw) {
      ...
      return await withTempWorkspace(
        { rootDir: resolvePreferredOriroTmpDir(), prefix: "oriro-llm-task-" },
        async ({ dir: tmpDir }) => {
