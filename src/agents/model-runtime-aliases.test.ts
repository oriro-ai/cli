...
function createAnthropicAuthConfig(params: {
  order?: string[];
  models?: NonNullable<NonNullable<OriroConfig["agents"]>["defaults"]>["models"];
}): OriroConfig {
...
describe("resolveCliRuntimeExecutionProvider", () => {
  ...
  it("does not override an explicit Oriro model-runtime policy with CLI auth", () => {
    // Runtime policy is more explicit than profile order, so CLI auth cannot
    // force a model onto the CLI harness when config says Oriro.
    expect(
      resolveCliRuntimeExecutionProvider({
        cfg: createAnthropicAuthConfig({
          order: ["anthropic:claude-cli"],
          models: {
            "anthropic/opus-4.7": { agentRuntime: { id: "oriro" } },
          },
        }),
        provider: "anthropic",
        modelId: "opus-4.7",
