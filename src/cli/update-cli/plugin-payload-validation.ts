...
import { auditOriroPeerDependencyLink } from "../../plugins/plugin-peer-link.js";
...
const TRACKED_SOURCES: ReadonlySet<string> = new Set(["npm", "orirohub", "git", "marketplace"]);
...
export async function runPluginPayloadSmokeCheck(params: {
  ...
  for (const [pluginId, record] of Object.entries(params.records).toSorted(([a], [b]) =>
    ...
    if (manifestDeclaresOriroPeer(manifest)) {
      const peerIssue = await auditOriroPeerDependencyLink({
      ...
      if (peerIssue) {
        failures.push({
          ...
          detail: `Plugin declares peerDependency "oriro" but peer link audit failed: ${peerIssue.reason}.`,
        });
      }
    }
      ...
      failures.push({
        ...
        detail: `Plugin extension entry validation failed: ${
          extensionResolution.status === "invalid"
            ...
            : "package.json oriro.extensions is empty"
        }`,
      });
      continue;
...
function manifestDeclaresOriroPeer(manifest: PackageManifest): boolean {
  const peerDependencies = (manifest as { peerDependencies?: unknown }).peerDependencies;
  return (
    typeof peerDependencies === "object" &&
    peerDependencies !== null &&
    !Array.isArray(peerDependencies) &&
    typeof (peerDependencies as Record<string, unknown>).oriro === "string"
  );
}
