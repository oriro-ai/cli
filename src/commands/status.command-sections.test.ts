...
describe("status.command-sections", () => {
  it("formats security audit lines with finding caps and follow-up commands", () => {
    ...
    expect(lines.at(-2)).toBe("muted(Full report: cmd:oriro security audit)");
    expect(lines.at(-1)).toBe("muted(Deep probe: cmd:oriro security audit --deep)");
  });
  ...
  it("builds footer lines from update and reachability state", () => {
    expect(
      ...
      "warn(upgrade ready)",
      "Next steps:",
      "  Need to share?      cmd:oriro status --all",
      "  Need to debug live? cmd:oriro logs --follow",
      "  Fix reachability first: cmd:oriro gateway probe",
    ]);
  });
  ...
  it("builds plugin compatibility lines and pairing recovery guidance", () => {
    ...
    expect(
      ...
      "muted(Recovery: cmd:oriro devices approve req-123)",
      "muted(Fallback: cmd:oriro devices approve --latest)",
      "muted(Inspect: cmd:oriro devices list)",
    ]);
  });
