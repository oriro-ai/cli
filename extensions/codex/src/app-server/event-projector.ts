...
import { emitTrustedDiagnosticEvent } from "oriro/plugin-sdk/diagnostic-runtime";
import { generatedImageAssetFromBase64 } from "oriro/plugin-sdk/image-generation";
import type { AssistantMessage, Usage } from "oriro/plugin-sdk/llm";
import { saveMediaBuffer } from "oriro/plugin-sdk/media-store";
import { asDateTimestampMs } from "oriro/plugin-sdk/number-runtime";
...
const MISSING_TOOL_RESULT_ERROR =
  "ORIRO recorded a native Codex tool.call without a matching tool.result before the turn completed.";
const GENERATED_IMAGE_MEDIA_SUBDIR = "tool-image-generation";
const BYTES_PER_MB = 1024 * 1024;
// Match Oriro's default image media cap for generated image tool outputs.
...
export class CodexAppServerEventProjector {
  ...
  buildResult(
    ...
    // context for Oriro history, search, and future harness switching.
    if (reasoningText) {
...
function shouldSuppressChannelProgressForItem(item: CodexThreadItem): boolean {
  ...
  // Dynamic Oriro tool requests are emitted at the item/tool/call request
