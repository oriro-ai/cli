...
title: "Cohere"
read_when:
  - You want to use Cohere with Oriro
  - You need the Cohere API key env var or CLI auth choice
---

[Cohere](https://cohere.com) provides OpenAI-compatible inference through its Compatibility API. Oriro ships the Cohere provider during its externalization transition and also publishes it as an official external plugin with the Command A model catalog.
...
1. Cohere is included in current Oriro packages. If it is unavailable, install the external package and restart the Gateway:

```bash
oriro plugins install @oriro/cohere-provider
...
```bash
oriro models list --provider cohere
```
