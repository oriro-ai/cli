...
function isCodexRuntimeConfigured(cfg: OriroConfig, _env: NodeJS.ProcessEnv): boolean {
  return collectConfiguredAgentHarnessRuntimes(cfg).includes("codex");
}

function isCodexPluginConfigured(cfg: OriroConfig): boolean {
...
function shouldScanCodexNativeAssets(cfg: OriroConfig, env: NodeJS.ProcessEnv): boolean {
