...
import { openOriroStateDatabase } from "../state/oriro-state-db.js";
...
beforeAll(async () => {
  testState = await createOriroTestState({ label: "plugin-state-store" });
  rmSync(path.dirname(resolveOriroStateSqlitePath()), { recursive: true, force: true });
});
...
describe("plugin state keyed store", () => {
      ...
      async (stateA) => {
        await withOriroTestState(
          ...
          async (stateB) => {
            ...
            expect(resolveOriroStateSqlitePath(stateA.env)).not.toBe(
              resolveOriroStateSqlitePath(stateB.env),
            );
          },
        );
      },
  ...
  it("reopens after the shared state DB cache closes its handle", async () => {
    await withPluginStateTestState(async () => {
      ...
      const secondary = await createOriroTestState({
        label: "plugin-state-cache-secondary",
        applyEnv: false,
      });
      try {
        openOriroStateDatabase({ env: secondary.env });
        testState?.applyEnv();
        await expect(store.lookup("k")).resolves.toEqual({ ok: true });
      } finally {
  ...
  it.runIf(process.platform !== "win32")("hardens DB directory and file permissions", async () => {
    await withPluginStateTestState(async () => {
      ...
      const databasePath = resolveOriroStateSqlitePath();
