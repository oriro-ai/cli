// Mcp Code Mode Gateway E2E script supports Oriro repository automation.
...
export async function main() {
  const rootDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-mcp-code-mode-"));
  const keep = process.env.ORIRO_MCP_CODE_MODE_GATEWAY_E2E_KEEP === "1";
  const previousEnv = {
  ...
  try {
    ...
    await provider?.stop();
    resetConfigRuntimeState();
    restoreEnvValue("ORIRO_STATE_DIR", previousEnv.stateDir);
    restoreEnvValue("ORIRO_CONFIG_PATH", previousEnv.configPath);
    restoreEnvValue("ORIRO_TEST_FAST", previousEnv.testFast);
    if (!keep) {
