...
import type { AuthProfileStore } from "oriro/plugin-sdk/agent-runtime";
...
const ORIRO_CODEX_APP_SERVER_ARGS_ENV_VAR = "ORIRO_CODEX_APP_SERVER_ARGS";
...
function buildPrivateCodexAppServerStartOptions(
  ...
  const privateEnv = Object.fromEntries(
    Object.entries(start.env ?? {}).filter(
      ([name]) => name.trim().toUpperCase() !== ORIRO_CODEX_APP_SERVER_ARGS_ENV_VAR,
    ),
  );
  const clearEnv = (start.clearEnv ?? []).filter((name) => {
    const normalized = name.trim().toUpperCase();
    return normalized !== "CODEX_HOME" && normalized !== ORIRO_CODEX_APP_SERVER_ARGS_ENV_VAR;
  });
  return {
    ...start,
    ...
    clearEnv: [...clearEnv, ORIRO_CODEX_APP_SERVER_ARGS_ENV_VAR],
  };
}
