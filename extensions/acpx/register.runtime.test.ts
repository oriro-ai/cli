...
vi.mock("oriro/plugin-sdk/acp-runtime-backend", () => ({
...
const previousSkipRuntime = process.env.ORIRO_SKIP_ACPX_RUNTIME;

function restoreEnv(): void {
  if (previousSkipRuntime === undefined) {
    delete process.env.ORIRO_SKIP_ACPX_RUNTIME;
  } else {
    process.env.ORIRO_SKIP_ACPX_RUNTIME = previousSkipRuntime;
  }
}

function createServiceContext() {
  return {
    workspaceDir: "/tmp/oriro-acpx-register-test",
    stateDir: "/tmp/oriro-acpx-register-test/state",
    config: {},
    logger: {
      info: vi.fn(),
      warn: vi.fn(),
