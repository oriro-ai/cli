...
export const ja_JP: TranslationMap = {
  ...
  overview: {
    ...
    pairing: {
      ...
      mobileHint:
        "モバイルを使用していますか？ デスクトップで oriro dashboard --no-open から完全な URL（#token=... を含む）をコピーしてください。",
    ...
    cards: {
      ...
      modelAuthAttentionExpiredDesc: "{providers} — oriro models auth で再認証してください",
  ...
  login: {
    ...
    failure: {
      ...
      authRequired: {
        ...
        stepPaste:
          "oriro dashboard --no-open のトークンを貼り付けるか、構成済みのパスワードを入力します。",
        stepGenerate:
          "トークンが構成されていない場合は、Gateway ホストで oriro doctor --generate-gateway-token を実行します。",
      ...
      authFailed: {
        ...
        stepDashboard:
          "oriro dashboard --no-open を実行し、新しい URL を開くか、そのトークンを貼り付けます。",
        stepReplace:
      ...
      pairing: {
        ...
        stepList: "Gateway ホストで oriro devices list を実行します。",
        stepApproveId: "このリクエストを承認します: oriro devices approve {requestId}.",
      ...
      network: {
        ...
        stepGateway:
          "oriro status または oriro gateway run で Gateway が実行中であることを確認します。",
        stepUrl:
          "WebSocket URL を確認し、Gateway が HTTPS/Tailscale Serve の背後にある場合は wss:// を使用します。",
        stepDashboard:
          "oriro dashboard --no-open で dashboard を開き直し、現在の URL と認証詳細を再コピーします。",
      },
    },
  },
  chat: {
