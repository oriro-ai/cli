...
const {
  ...
  const baseDir = mkdtempSync(nodePath.join(tmpdir(), "oriro-sandbox-registry-"));
  const previousStateDir = process.env.ORIRO_STATE_DIR;
  Reflect.set(process.env, "ORIRO_STATE_DIR", baseDir);

  return {
    TEST_STATE_DIR: baseDir,
    PREVIOUS_ORIRO_STATE_DIR: previousStateDir,
...
afterAll(async () => {
  ...
  if (PREVIOUS_ORIRO_STATE_DIR === undefined) {
    deleteTestEnvValue("ORIRO_STATE_DIR");
  } else {
    setTestEnvValue("ORIRO_STATE_DIR", PREVIOUS_ORIRO_STATE_DIR);
  }
});
