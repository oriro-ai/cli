...
import { sortUniqueStrings } from "@oriro/normalization-core/string-normalization";
...
type LoadOriroPlugins = typeof import("./loader.js").loadOriroPlugins;
...
const loadOriroPluginsMock = vi.fn<LoadOriroPlugins>();
...
function createManifestProviderPlugin(params: {
  ...
  return {
    ...
    manifestPath: `/tmp/${params.id}/oriro.plugin.json`,
  };
}
...
function expectLastSetupRegistryCall(params: {
  ...
  const call = getLastMockCallArg(loadOriroPluginsMock, "Oriro plugin setup loader");
...
function getLastSetupLoadedPluginConfig() {
  const call = expectRecordFields(
    getLastMockCallArg(loadOriroPluginsMock, "Oriro plugin setup loader"),
    {},
  );
...
function createAutoEnabledProviderConfig() {
  const rawConfig: OriroConfig = {
    plugins: {},
  };
  const autoEnabledConfig: OriroConfig = {
    ...rawConfig,
    plugins: {
      entries: {
        google: { enabled: true },
...
describe("resolvePluginProviders", () => {
  beforeAll(async () => {
    ...
    vi.doMock("./loader.js", () => ({
      loadOriroPlugins: (...args: Parameters<LoadOriroPlugins>) =>
        loadOriroPluginsMock(...args),
