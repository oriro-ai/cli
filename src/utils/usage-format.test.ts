...
describe("usage-format", () => {
  ...
  beforeEach(async () => {
    envSnapshot = captureEnv(["ORIRO_AGENT_DIR", "ORIRO_STATE_DIR"]);
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-usage-format-"));
  ...
  it("prefers models.json pricing over oriro config and cached pricing", async () => {
    const config = {
      models: {
        providers: {
          "demo-preferred": {
  ...
  it("falls back to oriro config pricing when models.json is absent", () => {
    const config = {
      models: {
        providers: {
