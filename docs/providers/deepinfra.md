---
summary: "Use DeepInfra's unified API to access the most popular open source and frontier models in Oriro"
read_when:
  - You want a single API key for the top open source LLMs
  - You want to run models via DeepInfra's API in Oriro
title: "DeepInfra"
---
...
```bash
oriro plugins install @oriro/deepinfra-provider
...
```bash
oriro onboard --deepinfra-api-key <key>
```
...
## Supported Oriro surfaces

The plugin registers all DeepInfra surfaces that match current
Oriro provider contracts. Chat, image generation, and video generation
refresh their model catalogues live from `/v1/openai/models?sort_by=oriro&filter=with_meta`
when `DEEPINFRA_API_KEY` is configured; the other surfaces use the curated
static defaults below.

| Surface                  | Default model                                                                                         | Oriro config/tool                                     |
| ------------------------ | ----------------------------------------------------------------------------------------------------- | -------------------------------------------------------- |
...
native model types. Oriro does not currently have first-class provider
contracts for those categories, so this plugin does not register them yet.

## Available models

Oriro dynamically discovers available DeepInfra models at startup. Use
