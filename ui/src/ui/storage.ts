...
const SETTINGS_KEY_PREFIX = "oriro.control.settings.v1:";
const LEGACY_SETTINGS_KEY = "oriro.control.settings.v1";
const LOCAL_USER_IDENTITY_KEY = "oriro.control.user.v1";
const LOCAL_ASSISTANT_IDENTITY_KEY = "oriro.control.assistant.v1";
const LEGACY_TOKEN_SESSION_KEY = "oriro.control.token.v1";
const TOKEN_SESSION_KEY_PREFIX = "oriro.control.token.v1:";
...
function deriveDefaultGatewayUrl(): { pageUrl: string; effectiveUrl: string } {
  ...
  const configured =
    ...
    normalizeOptionalString(window["__ORIRO_CONTROL_UI_BASE_PATH__"]);
...
export function loadSettings(): UiSettings {
  ...
  try {
    ...
    const settings = {
      ...
      theme: theme === "custom" && !customTheme ? "oriro" : theme,
      themeMode: mode,
      chatShowThinking:
...
function persistSettings(next: UiSettings) {
  ...
  try {
    ...
    const raw =
      storage?.getItem(scopedKey) ??
      storage?.getItem(SETTINGS_KEY_PREFIX + "default") ??
      storage?.getItem("oriro.control.settings.v1");
    if (raw) {
      const parsed = JSON.parse(raw) as PersistedUiSettings;
