...
const CAPTURE_MARKER = "__ORIRO_SHELL_SNAPSHOT_CAPTURE__";
const ENV_MARKER = "__ORIRO_SHELL_SNAPSHOT_ENV__";
...
async function captureShellSnapshot(opts: ShellSnapshotWrapOptions): Promise<string | null> {
  ...
  const captureOutputDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-shell-snapshot-"));
...
function buildTrustedSnapshotCaptureEnv(
  ...
  // ORIRO_SHELL is injected by the exec runtime, so startup files can keep
  // their documented exec-specific branches without trusting model input.
  if (runtimeEnv.ORIRO_SHELL === "exec") {
    env.ORIRO_SHELL = "exec";
  }
  return env;
}
