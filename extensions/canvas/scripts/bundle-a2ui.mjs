...
async function main() {
  ...
  if (!hasA2uiPackage || !hasAppDir) {
    ...
    if (process.env.ORIRO_SPARSE_PROFILE || process.env.ORIRO_A2UI_SKIP_MISSING === "1") {
      console.error(
        "A2UI package missing; skipping bundle because ORIRO_A2UI_SKIP_MISSING=1 or ORIRO_SPARSE_PROFILE is set.",
      );
      return;
    }
