// Issue 78851 Model Resolution script supports Oriro repository automation.
...
function printUsage(): void {
  process.stdout.write(`ORIRO issue #78851 model-resolution profiler
...
function buildConfig(options: Options, workspaceDir: string): OriroConfig {
...
async function measurePhase(params: {
  ...
  const ensureResult = await ensureOriroModelsJson(params.config, params.agentDir, {
...
async function main(): Promise<void> {
  ...
  const tempRoot = await mkdtemp(path.join(tmpdir(), "oriro-issue-78851-"));
