...
async function loadRawVitestConfig(configPath: string): Promise<VitestConfig> {
  ...
  const previousIncludeFile = process.env.ORIRO_VITEST_INCLUDE_FILE;
  process.argv = [previousArgv[0] ?? "node", previousArgv[1] ?? "vitest"];
  delete process.env.ORIRO_VITEST_INCLUDE_FILE;
  try {
    ...
    if (previousIncludeFile === undefined) {
      ...
      process.env.ORIRO_VITEST_INCLUDE_FILE = previousIncludeFile;
    }
  }
}
...
function withTinyGitRepo(files: Record<string, string>, test: (cwd: string) => void): void {
  const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-test-projects-"));
  try {
...
function commitTinyGitRepo(cwd: string): void {
  const commit = spawnSync("git", ["commit", "-m", "initial"], {
    ...
    env: {
      ...
      GIT_AUTHOR_NAME: "Oriro Test",
      GIT_COMMITTER_EMAIL: "test@example.com",
      GIT_COMMITTER_NAME: "Oriro Test",
    },
    stdio: "ignore",
  });
...
describe("scripts/test-projects changed-target routing", () => {
  ...
  it("keeps the broad changed run available for Vitest wiring edits", () => {
    expect(
      resolveChangedTargetArgs(
        ...
        { env: { ORIRO_TEST_CHANGED_BROAD: "1" } },
      ),
    ).toBeNull();
  });
  ...
  it("routes nested scripts through conventional owner tests", () => {
    ...
    expect(resolveChangedTestTargetPlan(["scripts/github/resolve-oriro-ref.sh"])).toEqual({
      mode: "targets",
      targets: ["test/scripts/resolve-oriro-ref.test.ts"],
    });
  });
  ...
  it("routes nested e2e shell helpers through their sourced owner tests", () => {
    const expectedTargets = new Map([
      ...
      [
        "scripts/e2e/lib/orirohub-fixture-server.cjs",
        [
          "test/scripts/orirohub-fixture-server.test.ts",
          "test/scripts/plugin-prerelease-test-plan.test.ts",
        ],
      ],
      [
      ...
      [
        ...
        ["test/scripts/e2e-shell-tempfiles.test.ts", "test/scripts/oriro-test-state.test.ts"],
      ],
      [
      ...
      [
        ...
        [
          ...
          "test/scripts/upgrade-survivor-assertions.test.ts",
          "test/scripts/oriro-test-state.test.ts",
        ],
      ],
      [
      ...
      ["scripts/e2e/lib/plugins/orirohub.sh", ["test/scripts/plugins-assertions.test.ts"]],
      ...
      [
        "scripts/e2e/lib/skills/orirohub-install-proof.sh",
        ["test/scripts/e2e-shell-tempfiles.test.ts"],
      ],
      [
  ...
  it("keeps release-check workflow edits on release workflow regression tests", () => {
    expect(resolveChangedTestTargetPlan([".github/workflows/oriro-release-checks.yml"])).toEqual(
      {
        mode: "targets",
        targets: [
          "test/scripts/package-acceptance-workflow.test.ts",
          "test/scripts/oriro-cross-os-release-checks.test.ts",
          "test/scripts/plugin-prerelease-test-plan.test.ts",
          "test/scripts/test-install-sh-docker.test.ts",
          "test/scripts/ci-workflow-guards.test.ts",
        ],
  ...
  it("keeps CI, dependency, and docs tooling edits on owner tests", () => {
    ...
    expect(
      resolveChangedTestTargetPlan(["scripts/github/run-oriro-cross-os-release-checks.sh"]),
    ).toEqual({
      mode: "targets",
      targets: ["test/scripts/oriro-cross-os-release-workflow.test.ts"],
    });
    ...
    expect(resolveChangedTestTargetPlan(["scripts/orirotributors-map.json"])).toEqual({
      mode: "targets",
      targets: ["test/scripts/update-orirotributors.test.ts"],
    });
  ...
  it("keeps package, release, and install tooling edits on owner tests", () => {
    const expectedTargets = new Map([
      ...
      ["scripts/podman/oriro.container.in", ["test/scripts/test-install-sh-docker.test.ts"]],
      [
        "scripts/package-oriro-for-docker.mjs",
        ["test/e2e/qa-lab/runtime/package-oriro-for-docker.e2e.test.ts"],
      ],
      ...
      ["scripts/oriro-prepack.ts", ["test/oriro-prepack.test.ts"]],
      ["scripts/oriro-npm-release-check.ts", ["test/oriro-npm-release-check.test.ts"]],
      [
        "scripts/oriro-npm-postpublish-verify.ts",
        ["test/oriro-npm-postpublish-verify.test.ts"],
      ],
  ...
  it("keeps auth monitoring helper edits on owner tests", () => {
    const expectedTargets = new Map([
      ...
      ["scripts/systemd/oriro-auth-monitor.service", ["test/scripts/auth-monitor.test.ts"]],
      ["scripts/systemd/oriro-auth-monitor.timer", ["test/scripts/auth-monitor.test.ts"]],
    ...
    const expectedTargets = new Map([
      ...
      [
        ...
        [
          ...
          "test/release-check.test.ts",
          "test/oriro-npm-release-check.test.ts",
          "test/scripts/check-gateway-watch-regression.test.ts",
          "test/scripts/check-oriro-package-tarball.test.ts",
          "test/scripts/oriro-cross-os-release-checks.test.ts",
        ],
      ],
      [
      ...
      [
        ".github/workflows/oriro-live-and-e2e-checks-reusable.yml",
        [
          "test/scripts/package-acceptance-workflow.test.ts",
          "test/scripts/ci-workflow-guards.test.ts",
      ...
      [
        ...
        [
          ...
          "test/plugin-orirohub-release.test.ts",
      ...
      [
        ...
        ["test/release-check.test.ts", "test/oriro-npm-release-check.test.ts"],
      ],
      [
        "scripts/oriro-release-orirohub-runtime-state.ts",
        ["test/scripts/oriro-release-orirohub-runtime-state.test.ts"],
      ],
      [
        "scripts/oriro-release-orirohub-plan.ts",
        ["test/scripts/release-wrapper-scripts.test.ts"],
      ],
      ["scripts/lib/oriro-release-orirohub-plan.ts", ["test/plugin-orirohub-release.test.ts"]],
      [
        "scripts/lib/plugin-orirohub-release.ts",
        ["test/plugin-orirohub-release.test.ts", "test/plugin-npm-release.test.ts"],
      ],
      [
        "scripts/lib/plugin-npm-release.ts",
        ["test/plugin-npm-release.test.ts", "test/plugin-orirohub-release.test.ts"],
      ],
      [
  ...
  it("routes the shell helper test to the isolated tooling shard", () => {
    expect(
      buildVitestRunPlans(["--changed", "origin/main"], process.cwd(), () => [
        "test/scripts/oriro-e2e-instance.test.ts",
      ]),
    ).toEqual([
      {
        config: "test/vitest/vitest.tooling-isolated.config.ts",
        forwardedArgs: [],
        includePatterns: ["test/scripts/oriro-e2e-instance.test.ts"],
        watchMode: false,
      },
    ]);
  });
  ...
  it("chunks the broad shell helper tooling shard after isolated targets", () => {
    ...
    expect(toolingTargets).not.toContain("test/scripts/oriro-e2e-instance.test.ts");
  ...
  it("rejects explicit test-support helper files with no importing tests", () => {
    const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-test-targets-"));
    try {
  ...
  it("explains changed paths that need explicit broad fallback before skipping", () => {
    withTinyGitRepo({ "package.json": '{"scripts":{}}\n' }, (cwd) => {
      ...
      expect(result.stderr).toContain(
        "[test] run `ORIRO_TEST_CHANGED_BROAD=1 pnpm test:changed` for broad coverage.",
      );
    });
  });
  ...
  it("skips app-only changes because app tests are separate from Vitest lanes", () => {
    expect(
      buildVitestRunPlans(["--changed", "origin/main"], process.cwd(), () => [
        "apps/macos/Oriro/AppDelegate.swift",
      ]),
    ).toStrictEqual([]);
  });
  ...
  it("uses collision-resistant include-file names for scoped Vitest specs", () => {
    const tempDir = path.join("tmp", "oriro-vitest-specs");
    ...
    expect(path.basename(spec?.includeFilePath ?? "")).toMatch(
      /^oriro-vitest-include-[0-9a-f-]{36}-0\.json$/u,
    );
...
describe("scripts/test-projects local heavy-check lock", () => {
  const localCheckEnv = () => ({
    ...process.env,
    ORIRO_TEST_HEAVY_CHECK_LOCK_HELD: undefined,
    ORIRO_TEST_PROJECTS_FORCE_LOCK: undefined,
  });
  ...
  it("allows forcing the lock back on", () => {
    expect(
      shouldAcquireLocalHeavyCheckLock(
        ...
        {
          ...localCheckEnv(),
          ORIRO_TEST_PROJECTS_FORCE_LOCK: "1",
        },
      ),
    ).toBe(true);
  });
...
describe("scripts/test-projects full-suite sharding", () => {
  ...
  beforeAll(async () => {
    ...
    const previous = process.env.ORIRO_TEST_PROJECTS_LEAF_SHARDS;
    const gatewayServerConfig = "test/vitest/vitest.gateway-server.config.ts";
    process.env.ORIRO_TEST_PROJECTS_LEAF_SHARDS = "1";
    try {
      ...
      if (previous === undefined) {
        delete process.env.ORIRO_TEST_PROJECTS_LEAF_SHARDS;
      } else {
        process.env.ORIRO_TEST_PROJECTS_LEAF_SHARDS = previous;
      }
    }
  });
  ...
  it("keeps explicit parallel overrides ahead of the host-aware profile", () => {
    expect(
      resolveParallelFullSuiteConcurrency(
        61,
        {
          ORIRO_TEST_PROJECTS_PARALLEL: "3",
        },
        {
          cpuCount: 14,
          loadAverage1m: 0,
  ...
  it("rejects malformed parallel full-suite overrides", () => {
    expect(() =>
      resolveParallelFullSuiteConcurrency(
        61,
        {
          ORIRO_TEST_PROJECTS_PARALLEL: "3x",
        },
        {
          cpuCount: 14,
          loadAverage1m: 0,
    ...
    ).toThrow("ORIRO_TEST_PROJECTS_PARALLEL must be a positive integer; got: 3x");

    expect(() =>
      resolveParallelFullSuiteConcurrency(
        61,
        {
          ORIRO_TEST_PROJECTS_PARALLEL: "0",
        },
        {
          cpuCount: 14,
          loadAverage1m: 0,
    ...
    ).toThrow("ORIRO_TEST_PROJECTS_PARALLEL must be a positive integer; got: 0");
  });
  ...
  it("rejects malformed conservative worker budget values", () => {
    ...
    ).toThrow("ORIRO_VITEST_MAX_WORKERS must be a positive integer; got: 1e0");

    expect(() =>
      resolveParallelFullSuiteConcurrency(
        61,
        {
          ORIRO_TEST_WORKERS: "1 worker",
        },
        {
          cpuCount: 14,
          loadAverage1m: 0,
    ...
    ).toThrow("ORIRO_TEST_WORKERS must be a positive integer; got: 1 worker");
  });

  it("keeps serial untargeted local runs on leaf project configs", () => {
    const previousParallel = process.env.ORIRO_TEST_PROJECTS_PARALLEL;
    const previousSerial = process.env.ORIRO_TEST_PROJECTS_SERIAL;
    const previousCi = process.env.CI;
    const previousActions = process.env.GITHUB_ACTIONS;
    delete process.env.ORIRO_TEST_PROJECTS_LEAF_SHARDS;
    delete process.env.ORIRO_TEST_SKIP_FULL_EXTENSIONS_SHARD;
    delete process.env.ORIRO_TEST_PROJECTS_PARALLEL;
    ...
    try {
      ...
      if (previousParallel === undefined) {
        ...
        process.env.ORIRO_TEST_PROJECTS_PARALLEL = previousParallel;
      }
      if (previousSerial === undefined) {
        delete process.env.ORIRO_TEST_PROJECTS_SERIAL;
      } else {
        process.env.ORIRO_TEST_PROJECTS_SERIAL = previousSerial;
      }
  ...
  it("expands untargeted local runs to leaf project configs by default", () => {
    const previousLeafShards = process.env.ORIRO_TEST_PROJECTS_LEAF_SHARDS;
    const previousParallel = process.env.ORIRO_TEST_PROJECTS_PARALLEL;
    const previousSerial = process.env.ORIRO_TEST_PROJECTS_SERIAL;
    const previousCi = process.env.CI;
    const previousActions = process.env.GITHUB_ACTIONS;
    const previousVitestMaxWorkers = process.env.ORIRO_VITEST_MAX_WORKERS;
    const previousTestWorkers = process.env.ORIRO_TEST_WORKERS;
    delete process.env.ORIRO_TEST_PROJECTS_LEAF_SHARDS;
    delete process.env.ORIRO_TEST_PROJECTS_PARALLEL;
    delete process.env.ORIRO_TEST_PROJECTS_SERIAL;
    delete process.env.CI;
    delete process.env.GITHUB_ACTIONS;
    delete process.env.ORIRO_VITEST_MAX_WORKERS;
    delete process.env.ORIRO_TEST_WORKERS;
    try {
      const configs = buildFullSuiteVitestRunPlans([], process.cwd()).map((plan) => plan.config);
      ...
      if (previousLeafShards === undefined) {
        ...
        process.env.ORIRO_TEST_PROJECTS_LEAF_SHARDS = previousLeafShards;
      }
      if (previousParallel === undefined) {
        delete process.env.ORIRO_TEST_PROJECTS_PARALLEL;
      } else {
      ...
      if (previousVitestMaxWorkers === undefined) {
        ...
        process.env.ORIRO_VITEST_MAX_WORKERS = previousVitestMaxWorkers;
      }
      if (previousTestWorkers === undefined) {
        delete process.env.ORIRO_TEST_WORKERS;
      } else {
        process.env.ORIRO_TEST_WORKERS = previousTestWorkers;
      }
    }
  });
  ...
  it("can skip the aggregate extension shard when CI runs dedicated extension shards", () => {
    const previous = process.env.ORIRO_TEST_SKIP_FULL_EXTENSIONS_SHARD;
    const previousParallel = process.env.ORIRO_TEST_PROJECTS_PARALLEL;
    const previousSerial = process.env.ORIRO_TEST_PROJECTS_SERIAL;
    const previousCi = process.env.CI;
    delete process.env.ORIRO_TEST_PROJECTS_PARALLEL;
    process.env.ORIRO_TEST_PROJECTS_SERIAL = "1";
    process.env.CI = "true";
    process.env.ORIRO_TEST_SKIP_FULL_EXTENSIONS_SHARD = "1";
    try {
      const configs = buildFullSuiteVitestRunPlans([], process.cwd()).map((plan) => plan.config);
      ...
      if (previous === undefined) {
        ...
        process.env.ORIRO_TEST_SKIP_FULL_EXTENSIONS_SHARD = previous;
      }
      if (previousParallel === undefined) {
        delete process.env.ORIRO_TEST_PROJECTS_PARALLEL;
      } else {
  ...
  it("skips extension project configs when leaf sharding and the aggregate extension shard is disabled", () => {
    const previousLeafShards = process.env.ORIRO_TEST_PROJECTS_LEAF_SHARDS;
    const previousSkipExtensions = process.env.ORIRO_TEST_SKIP_FULL_EXTENSIONS_SHARD;
    process.env.ORIRO_TEST_PROJECTS_LEAF_SHARDS = "1";
    process.env.ORIRO_TEST_SKIP_FULL_EXTENSIONS_SHARD = "1";
    try {
      const configs = buildFullSuiteVitestRunPlans([], process.cwd()).map((plan) => plan.config);
      ...
      if (previousSkipExtensions === undefined) {
        ...
        process.env.ORIRO_TEST_SKIP_FULL_EXTENSIONS_SHARD = previousSkipExtensions;
      }
    }
  });
  ...
  it("expands full-suite shards before running them in parallel", () => {
    ...
    const previousParallel = process.env.ORIRO_TEST_PROJECTS_PARALLEL;
    delete process.env.ORIRO_TEST_PROJECTS_LEAF_SHARDS;
    process.env.ORIRO_TEST_PROJECTS_PARALLEL = "6";
    try {
      const configs = buildFullSuiteVitestRunPlans([], process.cwd()).map((plan) => plan.config);
  ...
  it("rejects malformed full-suite expansion parallel overrides", () => {
    ...
    const previousParallel = process.env.ORIRO_TEST_PROJECTS_PARALLEL;
    delete process.env.ORIRO_TEST_PROJECTS_LEAF_SHARDS;
    process.env.ORIRO_TEST_PROJECTS_PARALLEL = "6x";
    try {
      expect(() => buildFullSuiteVitestRunPlans([], process.cwd())).toThrow(
        "ORIRO_TEST_PROJECTS_PARALLEL must be a positive integer; got: 6x",
      );
    } finally {
      if (previousLeafShards === undefined) {
        delete process.env.ORIRO_TEST_PROJECTS_LEAF_SHARDS;
...
describe("scripts/test-projects parallel cache paths", () => {
  it("assigns isolated Vitest fs-module cache paths per parallel shard", () => {
    ...
    expect(specs.map((spec) => spec.env)).toEqual([
      {
        ORIRO_VITEST_FS_MODULE_CACHE_PATH: path.join(
          "/repo",
          "node_modules",
  ...
  it("keeps an explicit global cache path", () => {
    const [spec] = applyParallelVitestCachePaths(
      [{ config: "test/vitest/vitest.gateway.config.ts", env: {}, pnpmArgs: [] }],
      { cwd: "/repo", env: { ORIRO_VITEST_FS_MODULE_CACHE_PATH: "/tmp/cache" } },
    );

    expect(spec?.env.ORIRO_VITEST_FS_MODULE_CACHE_PATH).toBeUndefined();
  });
});
...
describe("scripts/test-projects Vitest stall watchdog", () => {
  it("adds default no-output watchdog settings to non-watch specs", () => {
    ...
    expect(spec?.env.ORIRO_VITEST_NO_OUTPUT_TIMEOUT_MS).toBe(
      DEFAULT_TEST_PROJECTS_VITEST_NO_OUTPUT_TIMEOUT_MS,
    );
    expect(spec?.env.ORIRO_VITEST_NO_OUTPUT_HEARTBEAT_MS).toBe(
      DEFAULT_TEST_PROJECTS_VITEST_NO_OUTPUT_HEARTBEAT_MS,
    );
  });
  ...
  it("extends the no-output watchdog for slow silent full-suite configs", () => {
    ...
    expect(specs[0]?.env.ORIRO_VITEST_NO_OUTPUT_TIMEOUT_MS).toBe("2400000");
    expect(specs[1]?.env.ORIRO_VITEST_NO_OUTPUT_TIMEOUT_MS).toBe("2400000");
    expect(specs[2]?.env.ORIRO_VITEST_NO_OUTPUT_TIMEOUT_MS).toBe("2400000");
    expect(specs[3]?.env.ORIRO_VITEST_NO_OUTPUT_TIMEOUT_MS).toBe(
      DEFAULT_TEST_PROJECTS_VITEST_NO_OUTPUT_TIMEOUT_MS,
    );
  });
  ...
  it("keeps explicit watchdog settings and watch mode untouched", () => {
    const specs = applyDefaultVitestNoOutputTimeout(
      [
        ...
        },
        {
          ...
          env: {
            ORIRO_VITEST_NO_OUTPUT_HEARTBEAT_MS: "25000",
            ORIRO_VITEST_NO_OUTPUT_TIMEOUT_MS: "0",
            PATH: "/usr/bin",
          },
          includeFilePath: null,
          includePatterns: null,
    ...
    expect(specs[0]?.env.ORIRO_VITEST_NO_OUTPUT_TIMEOUT_MS).toBeUndefined();
    expect(specs[0]?.env.ORIRO_VITEST_NO_OUTPUT_HEARTBEAT_MS).toBeUndefined();
    expect(specs[1]?.env.ORIRO_VITEST_NO_OUTPUT_TIMEOUT_MS).toBe("0");
    expect(specs[1]?.env.ORIRO_VITEST_NO_OUTPUT_HEARTBEAT_MS).toBe("25000");
  });
  ...
  it("allows changed checks to disable automatic silent-run retries", () => {
    ...
    expect(shouldRetryVitestNoOutputTimeout({ ORIRO_VITEST_NO_OUTPUT_RETRY: "1" })).toBe(true);
    expect(shouldRetryVitestNoOutputTimeout({ ORIRO_VITEST_NO_OUTPUT_RETRY: "0" })).toBe(false);
    expect(shouldRetryVitestNoOutputTimeout({ ORIRO_VITEST_NO_OUTPUT_RETRY: "false" })).toBe(
      false,
    );
  });
});
...
describe("scripts/test-projects Vitest cache isolation", () => {
  it("assigns isolated fs-module caches to multi-spec non-watch runs", () => {
    ...
    expect(specs.map((spec) => spec.env.ORIRO_VITEST_FS_MODULE_CACHE_PATH)).toEqual([
      path.join(
        "/repo",
        "node_modules",
        ".experimental-vitest-cache",
