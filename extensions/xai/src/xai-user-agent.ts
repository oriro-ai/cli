// Shared User-Agent for xAI sidecar HTTP/WS requests; mirrors `formatOriroUserAgent`.

import { ORIRO_VERSION as PACKAGE_VERSION } from "oriro/plugin-sdk/agent-harness-runtime";

const ORIGINATOR = "oriro";
...
function resolveXaiUserAgentVersion(): string {
  ...
  const envVersion = trimToUndefined(process.env.ORIRO_VERSION);
  if (envVersion) {
    return envVersion;
  }
  ...
  return (
    trimToUndefined(process.env.ORIRO_SERVICE_VERSION) ??
...
// an empty record so the oriro identity is not forwarded to the proxy.
