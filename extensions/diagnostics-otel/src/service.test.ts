...
vi.mock("oriro/plugin-sdk/runtime-env", () => ({
...
import type { OriroPluginServiceContext } from "../api.js";
import { emitDiagnosticEvent } from "../api.js";
import { createDiagnosticsOtelService } from "./service.js";

const OTEL_TEST_STATE_DIR = "/tmp/oriro-diagnostics-otel-test";
...
const ORIGINAL_ORIRO_OTEL_PRELOADED = process.env.ORIRO_OTEL_PRELOADED;
...
type OtelContextFlags = {
  ...
  protocol?: NonNullable<
    NonNullable<OriroPluginServiceContext["config"]["diagnostics"]>["otel"]
  >["protocol"];
...
function createTraceOnlyContext(endpoint: string): OriroPluginServiceContext {
...
describe("diagnostics-otel service", () => {
  beforeEach(() => {
    resetDiagnosticEventsForTest();
    delete process.env.ORIRO_OTEL_PRELOADED;
  ...
  afterEach(() => {
    resetDiagnosticEventsForTest();
    if (ORIGINAL_ORIRO_OTEL_PRELOADED === undefined) {
      delete process.env.ORIRO_OTEL_PRELOADED;
    } else {
      process.env.ORIRO_OTEL_PRELOADED = ORIGINAL_ORIRO_OTEL_PRELOADED;
    }
  ...
  test("drops camelCase and snake_case diagnostic id log attributes before export", async () => {
    ...
    expect(emitCall.attributes?.["oriro.provider"]).toBe("openai");
    for (const key of [
      "oriro.callId",
      "oriro.call_id",
      "oriro.chatId",
  ...
  test("records message-flow metrics and spans", async () => {
    ...
    expect(telemetryState.counters.get("oriro.webhook.received")?.add).toHaveBeenCalledWith(1, {
      "oriro.channel": "telegram",
      "oriro.webhook": "telegram-post",
    });
    expect(
      telemetryState.histograms.get("oriro.webhook.duration_ms")?.record,
    ).toHaveBeenCalledWith(120, {
      "oriro.channel": "telegram",
      "oriro.webhook": "telegram-post",
    });
    expect(telemetryState.counters.get("oriro.message.queued")?.add).toHaveBeenCalledWith(1, {
      "oriro.channel": "telegram",
      "oriro.source": "telegram",
    });
    expect(telemetryState.histograms.get("oriro.queue.depth")?.record).toHaveBeenCalledTimes(2);
    expect(telemetryState.histograms.get("oriro.queue.depth")?.record).toHaveBeenCalledWith(2, {
      "oriro.channel": "telegram",
      "oriro.source": "telegram",
    });
    expect(telemetryState.histograms.get("oriro.queue.depth")?.record).toHaveBeenCalledWith(3, {
      "oriro.lane": "main",
    });
    expect(telemetryState.counters.get("oriro.message.processed")?.add).toHaveBeenCalledWith(1, {
      "oriro.channel": "telegram",
      "oriro.outcome": "completed",
    });
    expect(telemetryState.counters.get("oriro.message.received")?.add).toHaveBeenCalledWith(1, {
      "oriro.channel": "telegram",
      "oriro.source": "webhook",
    });
    expect(telemetryState.counters.get("oriro.message.received")?.add).toHaveBeenCalledWith(1, {
      "oriro.channel": "unknown",
      "oriro.source": "unknown",
    });
    expect(
      telemetryState.counters.get("oriro.message.dispatch.started")?.add,
    ...
    expect(
      telemetryState.counters.get("oriro.message.dispatch.completed")?.add,
    ...
    expect(
      telemetryState.histograms.get("oriro.message.dispatch.duration_ms")?.record,
    ).toHaveBeenCalledWith(
      25,
      expect.objectContaining({
        "oriro.channel": "telegram",
    ...
    expect(
      telemetryState.histograms.get("oriro.message.duration_ms")?.record,
    ).toHaveBeenCalledWith(55, {
      "oriro.channel": "telegram",
      "oriro.outcome": "completed",
    });
    expect(telemetryState.histograms.get("oriro.queue.wait_ms")?.record).toHaveBeenCalledWith(
      10,
      {
        "oriro.lane": "main",
      },
    );
    expect(telemetryState.counters.get("oriro.session.stuck")?.add).toHaveBeenCalledTimes(1);
    expect(telemetryState.counters.get("oriro.session.stuck")?.add).toHaveBeenCalledWith(1, {
      "oriro.state": "processing",
    });
    expect(
      telemetryState.histograms.get("oriro.session.stuck_age_ms")?.record,
    ).toHaveBeenCalledWith(125_000, {
      "oriro.state": "processing",
    });
    expect(telemetryState.counters.get("oriro.run.attempt")?.add).toHaveBeenCalledWith(1, {
      "oriro.attempt": 2,
    });
    ...
    expect(telemetryState.counters.get("oriro.session.turn.created")?.add).toHaveBeenCalledWith(
      1,
      {
        "oriro.agent": "agent.default",
        "oriro.channel": "telegram",
        "oriro.trigger": "user",
      },
    );
    ...
    expect(spanNames).toContain("oriro.webhook.processed");
    expect(spanNames).toContain("oriro.message.processed");
    expect(spanNames).toContain("oriro.session.stuck");
    const webhookSpanOptions = startedSpanOptions("oriro.webhook.processed");
    expect(webhookSpanOptions?.attributes).not.toHaveProperty("oriro.chatId");
    expect(webhookSpanOptions?.startTime).toBeTypeOf("number");
    const messageSpanOptions = startedSpanOptions("oriro.message.processed");
    expect(messageSpanOptions?.attributes?.["oriro.channel"]).toBe("telegram");
    expect(messageSpanOptions?.attributes?.["oriro.outcome"]).toBe("completed");
    expect(messageSpanOptions?.attributes?.["oriro.reason"]).toBe("unknown");
    expect(messageSpanOptions?.attributes).not.toHaveProperty("oriro.chatId");
    expect(messageSpanOptions?.attributes).not.toHaveProperty("oriro.messageId");
  ...
  test("uses a preloaded OpenTelemetry SDK without dropping diagnostic listeners", async () => {
    process.env.ORIRO_OTEL_PRELOADED = "1";
    const service = createDiagnosticsOtelService();
    const ctx = createOtelContext(OTEL_TEST_ENDPOINT, { traces: true, metrics: true, logs: true });
    await service.start(ctx);
    ...
    const runDurationRecordCall = lastHistogramRecord("oriro.run.duration_ms");
    expect(runDurationRecordCall?.[0]).toBe(100);
    const runDurationAttributes = runDurationRecordCall?.[1];
    expect(runDurationAttributes?.["oriro.provider"]).toBe("openai");
    expect(runDurationAttributes?.["oriro.model"]).toBe("gpt-5.4");
    const runSpanOptions = startedSpanOptions("oriro.run");
    expect(runSpanOptions?.attributes?.["oriro.outcome"]).toBe("completed");
    expect(logEmit).toHaveBeenCalled();
  ...
  test("emits and records bounded telemetry exporter health events", async () => {
    ...
    expect(
      telemetryState.counters.get("oriro.telemetry.exporter.events")?.add,
    ).toHaveBeenCalledWith(1, {
      "oriro.exporter": "diagnostics-otel",
      "oriro.signal": "logs",
      "oriro.status": "started",
      "oriro.reason": "configured",
    });
  ...
  test("exports trusted security events as bounded OTLP logs", async () => {
    ...
    expect(emitCall.body).toBe("oriro.security.event");
    expect(emitCall.severityText).toBe("WARN");
    expect(emitCall.severityNumber).toBe(13);
    expect(emitCall.attributes).toMatchObject({
      "oriro.security.event_id": "security-event-1",
      "oriro.security.category": "tool",
      "oriro.security.action": "tool.execution.blocked",
      "oriro.security.outcome": "denied",
      "oriro.security.severity": "medium",
      "oriro.security.reason": "tools.deny",
      "oriro.security.actor.kind": "agent",
      "oriro.security.actor.id_hash": "agent-hash-1",
      "oriro.security.actor.role": "operator",
      "oriro.security.actor.scopes": "operator.read,operator.approvals",
      "oriro.security.target.kind": "plugin",
      "oriro.security.target.name": "@acme/security-event-plugin",
      "oriro.security.target.owner": "plugin-installer",
      "oriro.security.policy.id": "tools.exec",
      "oriro.security.policy.decision": "deny",
      "oriro.security.policy.reason": "allowlist.miss",
      "oriro.security.control.id": "exec-approval",
      "oriro.security.control.family": "approval",
      "oriro.security.attribute.params_kind": "object",
      "oriro.security.attribute.secretish": "unknown",
    });
    ...
    expect(Object.hasOwn(emitCall.attributes ?? {}, "oriro.security.attribute.__proto__")).toBe(
      false,
    );
  ...
  test("exports trusted security events as stdout JSONL logs", async () => {
    ...
    try {
      ...
      expect(record.body).toBe("oriro.security.event");
      expect(record.severityText).toBe("WARN");
      expect(record.severityNumber).toBe(13);
      expect(record.attributes).toMatchObject({
        "oriro.security.event_id": "security-event-stdout",
        "oriro.security.category": "tool",
        "oriro.security.action": "tool.execution.blocked",
        "oriro.security.outcome": "denied",
        "oriro.security.severity": "medium",
      ...
      expect(Object.hasOwn(record.attributes ?? {}, "oriro.security.attribute.__proto__")).toBe(
        false,
      );
  ...
  test("records liveness warning diagnostics", async () => {
    ...
    expect(telemetryState.counters.get("oriro.liveness.warning")?.add).toHaveBeenCalledWith(1, {
      "oriro.liveness.reason": "event_loop_delay:cpu",
    });
    expect(
      telemetryState.histograms.get("oriro.liveness.event_loop_delay_p99_ms")?.record,
    ).toHaveBeenCalledWith(250, {
      "oriro.liveness.reason": "event_loop_delay:cpu",
    });
    expect(
      telemetryState.histograms.get("oriro.liveness.cpu_core_ratio")?.record,
    ).toHaveBeenCalledWith(1.4, {
      "oriro.liveness.reason": "event_loop_delay:cpu",
    });
    const livenessSpanOptions = startedSpanOptions("oriro.liveness.warning");
    expect(livenessSpanOptions?.attributes?.["oriro.liveness.reason"]).toBe(
      "event_loop_delay:cpu",
    );
    expect(livenessSpanOptions?.attributes?.["oriro.liveness.active"]).toBe(2);
    expect(livenessSpanOptions?.attributes?.["oriro.liveness.queued"]).toBe(4);
    const span = telemetryState.spans.find((item) => item.name === "oriro.liveness.warning");
  ...
  test("records oversized payload metrics without raw identifiers", async () => {
    ...
    expect(telemetryState.counters.get("oriro.payload.large")?.add).toHaveBeenCalledWith(1, {
      "oriro.payload.action": "rejected",
      "oriro.payload.surface": "gateway.frame",
      "oriro.channel": "web",
      "oriro.plugin": "none",
      "oriro.reason": "body-too-large",
    });
    expect(
      telemetryState.histograms.get("oriro.payload.large_bytes")?.record,
  ...
  test("reports log exporter emit failures without exporting raw error text", async () => {
    ...
    expect(
      ...
      "oriro.exporter": "diagnostics-otel",
      "oriro.signal": "logs",
      "oriro.status": "failure",
      "oriro.reason": "emit_failed",
      "oriro.errorCategory": "TypeError",
    });
  ...
  test("ignores untrusted telemetry exporter events for OTEL metrics", async () => {
    ...
    await service.start(ctx);
    telemetryState.counters.get("oriro.telemetry.exporter.events")?.add.mockClear();
    emitDiagnosticEvent({
  ...
  test("records hook-blocked run metrics with safe blocker originator", async () => {
    ...
    const runDurationRecordCall = lastHistogramRecord("oriro.run.duration_ms");
    expect(runDurationRecordCall?.[0]).toBe(100);
    expect(runDurationRecordCall?.[1]?.["oriro.outcome"]).toBe("blocked");
    expect(runDurationRecordCall?.[1]?.["oriro.blocked_by"]).toBe("policy-plugin");
  ...
  test("honors disabled traces when an OpenTelemetry SDK is preloaded", async () => {
    ...
    const runDurationRecordCall = lastHistogramRecord("oriro.run.duration_ms");
    expect(runDurationRecordCall?.[0]).toBe(100);
    expect(runDurationRecordCall?.[1]?.["oriro.provider"]).toBe("openai");
    expect(telemetryState.tracer.startSpan).not.toHaveBeenCalled();
  ...
  test("treats omitted diagnostics enabled flag as enabled", async () => {
    ...
    const modelCall = telemetryState.tracer.startSpan.mock.calls.find(
      (call) => call[0] === "oriro.model.call",
    );
    const attrs = (modelCall?.[1] as { attributes?: Record<string, unknown> } | undefined)
      ?.attributes;
    expect(attrs?.["oriro.content.input_messages"]).toBe("user prompt");
  ...
  test("exports diagnostic logs as stdout JSONL without constructing the OTLP log exporter", async () => {
    ...
    ctx.config.diagnostics!.otel!.serviceName = "rovooriro-oriro";
    const stdout = captureStdoutWrites();
    ...
    try {
      ...
      expect(record.signal).toBe("oriro.diagnostic.log");
      expect(record["service.name"]).toBe("rovooriro-oriro");
      expect(record.severityText).toBe("WARN");
      expect(record.severityNumber).toBe(13);
      expect(String(record.body)).not.toContain("sk-1234567890abcdef1234567890abcdef");
      expect(String(record.body)).toContain("sk-123");
      expect(record.attributes).toMatchObject({
        "oriro.log.level": "WARN",
        "oriro.subsystem": "diagnostic",
      });
      const tokenAttr = record.attributes?.["oriro.token"];
  ...
  test("redacts sensitive data from log attributes before export", async () => {
    ...
    const tokenAttr = emitCall?.attributes?.["oriro.token"];
    expect(tokenAttr).not.toBe("ghp_abcdefghijklmnopqrstuvwxyz123456"); // pragma: allowlist secret
  ...
  test("does not attach untrusted diagnostic trace context to exported logs", async () => {
    ...
    expect(Object.hasOwn(emitCall?.attributes ?? {}, "oriro.traceId")).toBe(false);
    expect(Object.hasOwn(emitCall?.attributes ?? {}, "oriro.spanId")).toBe(false);
    expect(Object.hasOwn(emitCall?.attributes ?? {}, "oriro.traceFlags")).toBe(false);
  ...
  test("bounds plugin-emitted log attributes and omits source paths", async () => {
    ...
    emitDiagnosticEvent({
      ...
      attributes,
      code: {
        filepath: "/Users/alice/oriro/src/private.ts",
        line: 42,
        functionName: "handler",
        location: "/Users/alice/oriro/src/private.ts:42",
      },
    ...
    expect(String(emitCall.attributes["oriro.good"])).toMatch(/^y+/);
    expect(emitCall.attributes["code.lineno"]).toBe(42);
    expect(emitCall.attributes["code.function"]).toBe("handler");
    expect(String(emitCall.attributes["oriro.good"]).length).toBeLessThanOrEqual(4200);
    expect(Object.hasOwn(emitCall.attributes, `oriro.${PROTO_KEY}`)).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "oriro.constructor")).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "oriro.prototype")).toBe(false);
    expect(
      Object.hasOwn(
        emitCall.attributes,
        "oriro.sk-1234567890abcdef1234567890abcdef", // pragma: allowlist secret
      ),
    ).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "oriro.bad key")).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "code.filepath")).toBe(false);
    expect(Object.hasOwn(emitCall.attributes, "oriro.code.location")).toBe(false);
    await service.stop?.(ctx);
  });
  ...
  test("exports GenAI client token usage histogram for input and output only", async () => {
    ...
    const tokens = telemetryState.counters.get("oriro.tokens");
    expect(tokens?.add).toHaveBeenCalledWith(12, {
      "oriro.channel": "webchat",
      "oriro.agent": "ops",
      "oriro.provider": "openai",
      "oriro.model": "gpt-5.4",
      "oriro.token": "input",
    });
  ...
  test("bounds agent identifiers on model usage metric attributes", async () => {
    ...
    expect(telemetryState.counters.get("oriro.tokens")?.add).toHaveBeenCalledWith(2, {
      "oriro.channel": "unknown",
      "oriro.agent": "unknown",
      "oriro.provider": "openai",
      "oriro.model": "gpt-5.4",
    ...
    expect(
      JSON.stringify(telemetryState.counters.get("oriro.tokens")?.add.mock.calls),
  ...
  test("drops session-shaped queue lane metric attributes", async () => {
    ...
    expect(telemetryState.counters.get("oriro.queue.lane.enqueue")?.add).toHaveBeenCalledWith(
      1,
      {
        "oriro.lane": "session",
      },
    ...
    expect(
      JSON.stringify(telemetryState.counters.get("oriro.queue.lane.enqueue")?.add.mock.calls),
    ).not.toContain("Agent:qa:otel-trace-smoke");
    await service.stop?.(ctx);
  });
  ...
  test("exports GenAI usage attributes on model usage spans without diagnostic identifiers", async () => {
    ...
    const modelUsageOptions = startedSpanOptions("oriro.model.usage");
    ...
    expect(Object.hasOwn(modelUsageOptions?.attributes ?? {}, "oriro.sessionKey")).toBe(false);
    expect(Object.hasOwn(modelUsageOptions?.attributes ?? {}, "oriro.sessionId")).toBe(false);
  ...
  test("exports skill usage counter and span without raw identifiers", async () => {
    ...
    const expectedAttrs = {
      "oriro.agent": "main",
      "oriro.skill.activation": "read",
      "oriro.skill.name": "tiny-llm-brainstorm",
      "oriro.skill.source": "workspace",
      "oriro.toolName": "read",
    };
    expect(telemetryState.counters.get("oriro.skill.used")?.add).toHaveBeenCalledWith(
      1,
      expectedAttrs,
    );
    const skillSpanCall = telemetryState.tracer.startSpan.mock.calls.find(
      (call) => call[0] === "oriro.skill.used",
    );
  ...
  test("exports run, model call, and tool execution lifecycle spans", async () => {
    ...
    const spanNames = telemetryState.tracer.startSpan.mock.calls.map((call) => call[0]);
    expect(spanNames).toContain("oriro.run");
    expect(spanNames).toContain("oriro.model.call");
    expect(spanNames).toContain("oriro.harness.run");
    expect(spanNames).toContain("oriro.tool.execution");

    const runOptions = startedSpanOptions("oriro.run");
    expect(runOptions?.attributes?.["oriro.outcome"]).toBe("completed");
    expect(runOptions?.attributes?.["oriro.provider"]).toBe("openai");
    expect(runOptions?.attributes?.["oriro.model"]).toBe("gpt-5.4");
    expect(runOptions?.attributes?.["oriro.channel"]).toBe("webchat");
    ...
    expect(Object.hasOwn(runOptions?.attributes ?? {}, "oriro.runId")).toBe(false);
    expect(Object.hasOwn(runOptions?.attributes ?? {}, "oriro.sessionKey")).toBe(false);
    expect(Object.hasOwn(runOptions?.attributes ?? {}, "oriro.traceId")).toBe(false);
    expect(runOptions?.startTime).toBeTypeOf("number");

    const modelCall = startedSpanCall("oriro.model.call");
    ...
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "oriro.callId")).toBe(false);
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "oriro.runId")).toBe(false);
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "oriro.sessionKey")).toBe(false);
    ...
    const harnessCall = startedSpanCall("oriro.harness.run");
    const harnessOptions = harnessCall?.[1];
    expect(harnessOptions?.attributes?.["oriro.harness.id"]).toBe("codex");
    expect(harnessOptions?.attributes?.["oriro.harness.plugin"]).toBe("codex-plugin");
    expect(harnessOptions?.attributes?.["oriro.outcome"]).toBe("completed");
    expect(harnessOptions?.attributes?.["oriro.provider"]).toBe("codex");
    expect(harnessOptions?.attributes?.["oriro.model"]).toBe("gpt-5.4");
    expect(harnessOptions?.attributes?.["oriro.channel"]).toBe("qa");
    expect(harnessOptions?.attributes?.["oriro.harness.result_classification"]).toBe(
      "reasoning-only",
    );
    expect(harnessOptions?.attributes?.["oriro.harness.yield_detected"]).toBe(true);
    expect(harnessOptions?.attributes?.["oriro.harness.items.started"]).toBe(3);
    expect(harnessOptions?.attributes?.["oriro.harness.items.completed"]).toBe(2);
    expect(harnessOptions?.attributes?.["oriro.harness.items.active"]).toBe(1);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "oriro.runId")).toBe(false);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "oriro.sessionId")).toBe(false);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "oriro.sessionKey")).toBe(false);
    expect(Object.hasOwn(harnessOptions?.attributes ?? {}, "oriro.traceId")).toBe(false);
    expect(harnessOptions?.startTime).toBeTypeOf("number");
    expect(harnessCall?.[2]).toBeUndefined();

    const toolCall = startedSpanCall("oriro.tool.execution");
    const toolOptions = toolCall?.[1];
    expect(toolOptions?.attributes?.["oriro.toolName"]).toBe("read");
    expect(toolOptions?.attributes?.["oriro.tool.source"]).toBe("core");
    expect(toolOptions?.attributes?.["oriro.errorCategory"]).toBe("TypeError");
    expect(toolOptions?.attributes?.["oriro.errorCode"]).toBe("429");
    expect(toolOptions?.attributes?.["oriro.tool.params.kind"]).toBe("object");
    expect(toolOptions?.attributes?.["gen_ai.tool.name"]).toBe("read");
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "oriro.toolCallId")).toBe(false);
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "oriro.runId")).toBe(false);
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "oriro.sessionKey")).toBe(false);
    expect(toolOptions?.startTime).toBeTypeOf("number");
    expect(toolCall?.[2]).toBeUndefined();

    const modelCallDuration = lastHistogramRecord("oriro.model_call.duration_ms");
    expect(modelCallDuration?.[0]).toBe(80);
    expect(modelCallDuration?.[1]?.["oriro.provider"]).toBe("openai");
    expect(modelCallDuration?.[1]?.["oriro.model"]).toBe("gpt-5.4");
    const requestBytes = lastHistogramRecord("oriro.model_call.request_bytes");
    expect(requestBytes?.[0]).toBe(1234);
    expect(requestBytes?.[1]?.["oriro.provider"]).toBe("openai");
    expect(requestBytes?.[1]?.["oriro.model"]).toBe("gpt-5.4");
    const responseBytes = lastHistogramRecord("oriro.model_call.response_bytes");
    expect(responseBytes?.[0]).toBe(567);
    expect(responseBytes?.[1]?.["oriro.provider"]).toBe("openai");
    expect(responseBytes?.[1]?.["oriro.model"]).toBe("gpt-5.4");
    const timeToFirstByte = lastHistogramRecord("oriro.model_call.time_to_first_byte_ms");
    expect(timeToFirstByte?.[0]).toBe(45);
    expect(timeToFirstByte?.[1]?.["oriro.provider"]).toBe("openai");
    expect(timeToFirstByte?.[1]?.["oriro.model"]).toBe("gpt-5.4");
    const modelSpanAttributes = firstSpanAttributes("oriro.model.call");
    expect(modelSpanAttributes["oriro.model_call.request_bytes"]).toBe(1234);
    expect(modelSpanAttributes["oriro.model_call.response_bytes"]).toBe(567);
    expect(modelSpanAttributes["oriro.model_call.time_to_first_byte_ms"]).toBe(45);
    const runDuration = lastHistogramRecord("oriro.run.duration_ms");
    expect(runDuration?.[0]).toBe(100);
    expect(Object.hasOwn(runDuration?.[1] ?? {}, "oriro.runId")).toBe(false);
    const harnessDuration = lastHistogramRecord("oriro.harness.duration_ms");
    expect(harnessDuration?.[0]).toBe(90);
    expect(harnessDuration?.[1]?.["oriro.harness.id"]).toBe("codex");
    expect(harnessDuration?.[1]?.["oriro.harness.plugin"]).toBe("codex-plugin");
    expect(harnessDuration?.[1]?.["oriro.outcome"]).toBe("completed");
    expect(Object.hasOwn(harnessDuration?.[1] ?? {}, "oriro.runId")).toBe(false);
    expect(Object.hasOwn(harnessDuration?.[1] ?? {}, "oriro.sessionKey")).toBe(false);
    const toolDuration = lastHistogramRecord("oriro.tool.execution.duration_ms");
    expect(toolDuration?.[0]).toBe(20);
    expect(toolDuration?.[1]?.["oriro.tool.source"]).toBe("core");
    expect(Object.hasOwn(toolDuration?.[1] ?? {}, "oriro.errorCode")).toBe(false);
    expect(Object.hasOwn(toolDuration?.[1] ?? {}, "oriro.runId")).toBe(false);

    const toolSpan = spanByName("oriro.tool.execution");
    expect(toolSpan?.setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "TypeError",
    });
    expect(firstSpanEndTime("oriro.tool.execution")).toBeTypeOf("number");
    expect(telemetryState.tracer.setSpanContext).not.toHaveBeenCalled();
    await service.stop?.(ctx);
  });
  ...
  test("exports model failover spans", async () => {
    ...
    const failoverOptions = startedSpanOptions("oriro.model.failover");
    expect(failoverOptions?.attributes?.["oriro.provider"]).toBe("anthropic");
    expect(failoverOptions?.attributes?.["oriro.model"]).toBe("claude-opus-4-6");
    expect(failoverOptions?.attributes?.["oriro.failover.to_provider"]).toBe("openai");
    expect(failoverOptions?.attributes?.["oriro.failover.to_model"]).toBe("gpt-5.4");
    expect(failoverOptions?.attributes?.["oriro.failover.reason"]).toBe("overloaded");
    expect(failoverOptions?.attributes?.["oriro.failover.suspended"]).toBe(true);
    expect(failoverOptions?.attributes?.["oriro.failover.cascade_depth"]).toBe(1);
    expect(failoverOptions?.attributes?.["oriro.lane"]).toBe("main");
    expect(Object.hasOwn(failoverOptions?.attributes ?? {}, "oriro.sessionId")).toBe(false);
    expect(Object.hasOwn(failoverOptions?.attributes ?? {}, "oriro.sessionKey")).toBe(false);
    expect(failoverOptions?.startTime).toBeTypeOf("number");
    expect(firstSpanEndTime("oriro.model.failover")).toBeTypeOf("number");
    expect(firstCounterAddCall("oriro.model.failover")).toStrictEqual([
      1,
      {
        "oriro.failover.reason": "overloaded",
        "oriro.failover.suspended": "true",
        "oriro.lane": "main",
        "oriro.model": "claude-opus-4-6",
        "oriro.provider": "anthropic",
        "oriro.failover.to_model": "gpt-5.4",
        "oriro.failover.to_provider": "openai",
      },
    ]);
    await service.stop?.(ctx);
  });
  ...
  test("records blocked tool metrics even when traces are disabled", async () => {
    ...
    expect(firstCounterAddCall("oriro.tool.execution.blocked")).toStrictEqual([
      1,
      {
        "oriro.toolName": "browser",
        "oriro.tool.source": "mcp",
        "gen_ai.tool.name": "browser",
        "oriro.tool.owner": "browser-tools",
        "oriro.tool.params.kind": "object",
        "oriro.deniedReason": "tools.deny",
      },
    ]);
  ...
  test("drops session-shaped queue lanes from model failover spans", async () => {
    ...
    const failoverOptions = startedSpanOptions("oriro.model.failover");
    expect(failoverOptions?.attributes?.["oriro.lane"]).toBe("session");
  ...
  test("maps model call APIs to GenAI operation names and error type", async () => {
    ...
    const modelCallAttrs = telemetryState.tracer.startSpan.mock.calls
      .filter((call) => call[0] === "oriro.model.call")
  ...
  test("uses latest GenAI inference span shape only when semconv opt-in is set", async () => {
    ...
    expect(startedSpanOptions("oriro.model.call")).toBeUndefined();
  ...
  test("records upstream request id hashes as model call span events only", async () => {
    ...
    const modelCallOptions = startedSpanOptions("oriro.model.call");
    expect(modelCallOptions?.attributes?.["oriro.failureKind"]).toBe("terminated");
    expect(
      Object.hasOwn(modelCallOptions?.attributes ?? {}, "oriro.upstreamRequestIdHash"),
    ).toBe(false);
    expect(modelCallOptions?.startTime).toBeTypeOf("number");
    const span = telemetryState.spans.find((candidate) => candidate.name === "oriro.model.call");
    expect(span?.addEvent).toHaveBeenCalledWith("oriro.provider.request", {
      "oriro.upstreamRequestIdHash": "sha256:123456abcdef",
    });
    const modelCallDuration = lastHistogramRecord("oriro.model_call.duration_ms");
    expect(modelCallDuration?.[0]).toBe(40);
    expect(modelCallDuration?.[1]?.["oriro.failureKind"]).toBe("terminated");
    expect(Object.hasOwn(modelCallDuration?.[1] ?? {}, "oriro.upstreamRequestIdHash")).toBe(
      false,
    );
    await service.stop?.(ctx);
  });
  ...
  test("exports trusted context assembly spans without prompt content", async () => {
    ...
    const contextCall = startedSpanCall("oriro.context.assembled");
    const contextOptions = contextCall?.[1];
    const runSpan = telemetryState.spans.find((span) => span.name === "oriro.run");
    const runSpanId = runSpan?.spanContext.mock.results[0]?.value?.spanId;
    expect(contextOptions?.attributes?.["oriro.provider"]).toBe("openai");
    expect(contextOptions?.attributes?.["oriro.model"]).toBe("gpt-5.4");
    expect(contextOptions?.attributes?.["oriro.channel"]).toBe("webchat");
    expect(contextOptions?.attributes?.["oriro.trigger"]).toBe("message");
    expect(contextOptions?.attributes?.["oriro.context.message_count"]).toBe(12);
    expect(contextOptions?.attributes?.["oriro.context.history_text_chars"]).toBe(1234);
    expect(contextOptions?.attributes?.["oriro.context.history_image_blocks"]).toBe(2);
    expect(contextOptions?.attributes?.["oriro.context.max_message_text_chars"]).toBe(456);
    expect(contextOptions?.attributes?.["oriro.context.system_prompt_chars"]).toBe(789);
    expect(contextOptions?.attributes?.["oriro.context.prompt_chars"]).toBe(42);
    expect(contextOptions?.attributes?.["oriro.context.prompt_images"]).toBe(1);
    expect(contextOptions?.attributes?.["oriro.context.token_budget"]).toBe(128_000);
    expect(contextOptions?.attributes?.["oriro.context.reserve_tokens"]).toBe(4096);
  ...
  test("exports tool loop diagnostics without loop messages or session identifiers", async () => {
    ...
    expect(telemetryState.counters.get("oriro.tool.loop")?.add).toHaveBeenCalledWith(1, {
      "oriro.toolName": "process",
      "oriro.loop.level": "critical",
      "oriro.loop.action": "block",
      "oriro.loop.detector": "known_poll_no_progress",
      "oriro.loop.count": 20,
      "oriro.loop.paired_tool": "read",
    });
    const loopSpanCall = startedSpanCall("oriro.tool.loop");
    const loopOptions = loopSpanCall?.[1];
    expect(loopOptions?.attributes?.["oriro.toolName"]).toBe("process");
    expect(loopOptions?.attributes?.["oriro.loop.level"]).toBe("critical");
    expect(loopOptions?.attributes?.["oriro.loop.action"]).toBe("block");
    expect(loopOptions?.attributes?.["oriro.loop.detector"]).toBe("known_poll_no_progress");
    expect(loopOptions?.attributes?.["oriro.loop.count"]).toBe(20);
    expect(loopOptions?.attributes?.["oriro.loop.paired_tool"]).toBe("read");
    const loopSpan = telemetryState.spans.find((span) => span.name === "oriro.tool.loop");
  ...
  test("exports diagnostic memory samples and pressure without session identifiers", async () => {
    ...
    expect(telemetryState.histograms.get("oriro.memory.rss_bytes")?.record).toHaveBeenCalledWith(
      100,
      {},
    );
    expect(telemetryState.histograms.get("oriro.memory.rss_bytes")?.record).toHaveBeenCalledWith(
      200,
      {
        "oriro.memory.level": "critical",
        "oriro.memory.reason": "rss_growth",
      },
    );
    expect(telemetryState.counters.get("oriro.memory.pressure")?.add).toHaveBeenCalledWith(1, {
      "oriro.memory.level": "critical",
      "oriro.memory.reason": "rss_growth",
    });
    const pressureCall = startedSpanCall("oriro.memory.pressure");
    const pressureOptions = pressureCall?.[1];
    expect(pressureOptions?.attributes?.["oriro.memory.level"]).toBe("critical");
    expect(pressureOptions?.attributes?.["oriro.memory.reason"]).toBe("rss_growth");
    expect(pressureOptions?.attributes?.["oriro.memory.rss_bytes"]).toBe(200);
    expect(pressureOptions?.attributes?.["oriro.memory.heap_used_bytes"]).toBe(50);
    expect(pressureOptions?.attributes?.["oriro.memory.heap_total_bytes"]).toBe(90);
    expect(pressureOptions?.attributes?.["oriro.memory.external_bytes"]).toBe(20);
    expect(pressureOptions?.attributes?.["oriro.memory.array_buffers_bytes"]).toBe(6);
    expect(pressureOptions?.attributes?.["oriro.memory.threshold_bytes"]).toBe(512);
    expect(pressureOptions?.attributes?.["oriro.memory.rss_growth_bytes"]).toBe(256);
    expect(pressureOptions?.attributes?.["oriro.memory.window_ms"]).toBe(60_000);
    const pressureSpan = telemetryState.spans.find(
      (span) => span.name === "oriro.memory.pressure",
    );
  ...
  test("records async diagnostic queue drop summaries", async () => {
    ...
    const counter = telemetryState.counters.get("oriro.diagnostic.async_queue.dropped");
    expect(counter?.add).toHaveBeenCalledWith(4, {
      "oriro.diagnostic.async_queue.drop_class": "total",
    });
    expect(counter?.add).toHaveBeenCalledWith(1, {
      "oriro.diagnostic.async_queue.drop_class": "trusted",
    });
    expect(counter?.add).toHaveBeenCalledWith(2, {
      "oriro.diagnostic.async_queue.drop_class": "untrusted",
    });
    expect(counter?.add).toHaveBeenCalledWith(1, {
      "oriro.diagnostic.async_queue.drop_class": "priority",
    });
  ...
  test("parents trusted diagnostic lifecycle spans from active started spans", async () => {
    ...
    const runSpan = telemetryState.spans.find((span) => span.name === "oriro.run");
    const modelSpan = telemetryState.spans.find((span) => span.name === "oriro.model.call");
    const toolSpan = telemetryState.spans.find((span) => span.name === "oriro.tool.execution");
    const runSpanId = runSpan?.spanContext.mock.results[0]?.value?.spanId;
    ...
    expect(parentBySpanName["oriro.run"]).toBeUndefined();
    expect(parentBySpanName["oriro.model.call"]).toBe(runSpanId);
    expect(parentBySpanName["oriro.tool.execution"]).toBe(modelSpanId);
    expect(toolSpan?.setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "TypeError",
    });
  ...
  test("correlates one channel message waterfall across message, harness, usage, and model spans", async () => {
    ...
    const messageSpan = spanByName("oriro.message.processed");
    const harnessSpan = spanByName("oriro.harness.run");
    const runSpan = spanByName("oriro.run");
    const usageSpan = spanByName("oriro.model.usage");
    const modelCallSpan = spanByName("oriro.model.call");
    ...
    expect(parentBySpanName["oriro.message.processed"]?.spanId).toBe(SPAN_ID);
    expect(parentBySpanName["oriro.harness.run"]?.spanId).toBe(messageSpanContext.spanId);
    expect(parentBySpanName["oriro.run"]?.spanId).toBe(harnessSpanContext.spanId);
    expect(parentBySpanName["oriro.model.usage"]?.spanId).toBe(harnessSpanContext.spanId);
    expect(parentBySpanName["oriro.model.call"]?.spanId).toBe(runSpanContext.spanId);
    await service.stop?.(ctx);
  });
  ...
  test("does not force a remote parent for root message lifecycle helpers", async () => {
    ...
    expect(spanByName("oriro.message.processed").spanContext().traceId).toBe(TRACE_ID);
    expect(startedSpanParentContexts("oriro.message.processed")[0]).toBeUndefined();
    await service.stop?.(ctx);
  });
  ...
  test("parents outbound delivery spans under the active message lifecycle span", async () => {
    ...
    const messageSpanContext = spanByName("oriro.message.processed").spanContext();
    const deliveryParentContexts = startedSpanParentContexts("oriro.message.delivery");
  ...
  test("correlates skipped duplicate message lifecycle helpers to the active inbound trace", async () => {
    ...
    const messageSpan = spanByName("oriro.message.processed");
    const messageSpanContext = messageSpan.spanContext();
    const parentContext = startedSpanParentContexts("oriro.message.processed")[0];

    expect(messageSpanContext.traceId).toBe(TRACE_ID);
    expect(parentContext?.traceId).toBe(TRACE_ID);
    expect(parentContext?.spanId).toBe(SPAN_ID);
    expect(firstSpanAttributes("oriro.message.processed")["oriro.reason"]).toBe("duplicate");
    expect(messageSpan.end).toHaveBeenCalledTimes(1);
    await service.stop?.(ctx);
  });
  ...
  test("does not retain fallback message processed spans as active parents", async () => {
    ...
    expect(spanByName("oriro.message.processed").end).toHaveBeenCalledTimes(1);
    ...
    expect(telemetryState.tracer.setSpanContext).not.toHaveBeenCalled();
    expect(startedSpanCall("oriro.harness.run")?.[2]).toBeUndefined();
    await service.stop?.(ctx);
  });
  ...
  test("retains trusted run context long enough for exact post-completion usage parenting", async () => {
    ...
    expect(firstSpanEndTime("oriro.run")).toBeTypeOf("number");
    await service.stop?.(ctx);
  });
  ...
  test("does not parent sibling active runs through shared upstream aliases", async () => {
    ...
    const runContexts = startedSpanParentContextsByName("oriro.run");
  ...
  test("parents retained upstream alias events only when the owner matches", async () => {
    ...
    const runSpanContext = spanByName("oriro.run").spanContext();
    const modelParentContext = startedSpanParentContexts("oriro.model.call")[0];
  ...
  test("parents multi-batch late model spans from the retained run context", async () => {
    ...
    const modelParentContexts = startedSpanParentContexts("oriro.model.call");
  ...
  test("removes retained run contexts after queued diagnostics drain", async () => {
    ...
    expect(telemetryState.tracer.setSpanContext).not.toHaveBeenCalled();
    expect(startedSpanCall("oriro.model.usage")?.[2]).toBeUndefined();
    await service.stop?.(ctx);
  });
  ...
  test("does not force remote parents for completed-only trusted lifecycle spans", async () => {
    ...
    expect(parentBySpanName["oriro.run"]).toBeUndefined();
    expect(parentBySpanName["oriro.model.call"]).toBeUndefined();
    await service.stop?.(ctx);
  });
  ...
  test("does not parent untrusted diagnostic lifecycle spans from injected trace ids", async () => {
    ...
    expect(parentBySpanName["oriro.run"]).toBeUndefined();
    expect(parentBySpanName["oriro.model.call"]).toBeUndefined();
    expect(parentBySpanName["oriro.tool.execution"]).toBeUndefined();
    await service.stop?.(ctx);
  });
  ...
  test("does not create live started spans for untrusted lifecycle diagnostics", async () => {
    ...
    expect(
      telemetryState.tracer.startSpan.mock.calls.filter((call) => call[0] === "oriro.run"),
    ).toHaveLength(1);
    expect(
    ...
    expect(
      telemetryState.tracer.startSpan.mock.calls.filter(
        (call) => call[0] === "oriro.tool.execution",
      ),
    ).toHaveLength(1);
    expect(
      telemetryState.tracer.startSpan.mock.calls.filter(
        (call) => call[0] === "oriro.harness.run",
      ),
    ).toHaveLength(1);
    await service.stop?.(ctx);
  });
  ...
  test("exports exec process spans without command text", async () => {
    ...
    const execDuration = lastHistogramRecord("oriro.exec.duration_ms");
    expect(execDuration?.[0]).toBe(30);
    expect(execDuration?.[1]?.["oriro.exec.target"]).toBe("host");
    expect(execDuration?.[1]?.["oriro.exec.mode"]).toBe("child");
    expect(execDuration?.[1]?.["oriro.outcome"]).toBe("failed");
    expect(execDuration?.[1]?.["oriro.failureKind"]).toBe("runtime-error");

    const execCall = startedSpanCall("oriro.exec");
    const execOptions = execCall?.[1];
    expect(execOptions?.attributes?.["oriro.exec.target"]).toBe("host");
    expect(execOptions?.attributes?.["oriro.exec.mode"]).toBe("child");
    expect(execOptions?.attributes?.["oriro.outcome"]).toBe("failed");
    expect(execOptions?.attributes?.["oriro.exec.command_length"]).toBe(42);
    expect(execOptions?.attributes?.["oriro.exec.exit_code"]).toBe(1);
    expect(execOptions?.attributes?.["oriro.exec.timed_out"]).toBe(false);
    expect(execOptions?.attributes?.["oriro.failureKind"]).toBe("runtime-error");
    expect(Object.hasOwn(execOptions?.attributes ?? {}, "oriro.exec.command")).toBe(false);
    expect(Object.hasOwn(execOptions?.attributes ?? {}, "oriro.exec.workdir")).toBe(false);
    expect(Object.hasOwn(execOptions?.attributes ?? {}, "oriro.sessionKey")).toBe(false);
    expect(execOptions?.startTime).toBeTypeOf("number");

    const execSpan = spanByName("oriro.exec");
    expect(execSpan?.setStatus).toHaveBeenCalledWith({
      code: 2,
      message: "runtime-error",
    });
    expect(firstSpanEndTime("oriro.exec")).toBeTypeOf("number");
    await service.stop?.(ctx);
  });
  ...
  test("exports message delivery spans and metrics with low-cardinality attributes", async () => {
    ...
    expect(
      telemetryState.counters.get("oriro.message.delivery.started")?.add,
    ).toHaveBeenCalledWith(1, {
      "oriro.channel": "matrix",
      "oriro.delivery.kind": "text",
    });
    const deliveryDurationRecords = telemetryState.histograms.get(
      "oriro.message.delivery.duration_ms",
    )?.record.mock.calls as Array<[unknown, Record<string, unknown>]>;
    expect(deliveryDurationRecords[0]?.[0]).toBe(25);
    expect(deliveryDurationRecords[0]?.[1]["oriro.channel"]).toBe("matrix");
    expect(deliveryDurationRecords[0]?.[1]["oriro.delivery.kind"]).toBe("text");
    expect(deliveryDurationRecords[0]?.[1]["oriro.outcome"]).toBe("completed");
    expect(deliveryDurationRecords[1]?.[0]).toBe(40);
    expect(deliveryDurationRecords[1]?.[1]["oriro.channel"]).toBe("discord");
    expect(deliveryDurationRecords[1]?.[1]["oriro.delivery.kind"]).toBe("media");
    expect(deliveryDurationRecords[1]?.[1]["oriro.outcome"]).toBe("error");
    expect(deliveryDurationRecords[1]?.[1]["oriro.errorCategory"]).toBe("TypeError");

    const deliverySpanCalls = telemetryState.tracer.startSpan.mock.calls.filter(
      (call) => call[0] === "oriro.message.delivery",
    );
    ...
    const firstDeliveryOptions = deliverySpanCalls[0]?.[1] as
      ...
      | undefined;
    expect(firstDeliveryOptions?.attributes?.["oriro.channel"]).toBe("matrix");
    expect(firstDeliveryOptions?.attributes?.["oriro.delivery.kind"]).toBe("text");
    expect(firstDeliveryOptions?.attributes?.["oriro.outcome"]).toBe("completed");
    expect(firstDeliveryOptions?.attributes?.["oriro.delivery.result_count"]).toBe(1);
    expect(firstDeliveryOptions?.startTime).toBeTypeOf("number");
    const secondDeliveryOptions = deliverySpanCalls[1]?.[1] as
      | { attributes?: Record<string, unknown>; startTime?: unknown }
      | undefined;
    expect(secondDeliveryOptions?.attributes?.["oriro.channel"]).toBe("discord");
    expect(secondDeliveryOptions?.attributes?.["oriro.delivery.kind"]).toBe("media");
    expect(secondDeliveryOptions?.attributes?.["oriro.outcome"]).toBe("error");
    expect(secondDeliveryOptions?.attributes?.["oriro.errorCategory"]).toBe("TypeError");
    ...
    for (const call of deliverySpanCalls) {
      ...
      expect(Object.hasOwn(options.attributes ?? {}, "oriro.chatId")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "oriro.sessionKey")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "oriro.messageId")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "oriro.conversationId")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "oriro.content")).toBe(false);
      expect(Object.hasOwn(options.attributes ?? {}, "oriro.to")).toBe(false);
      expect(options.startTime).toBeTypeOf("number");
    }
    const errorSpan = telemetryState.spans.find(
      (span) => span.name === "oriro.message.delivery" && span.setStatus.mock.calls.length > 0,
    );
  ...
  test("bounds unsafe message delivery attributes before export", async () => {
    ...
    const deliveryDuration = lastHistogramRecord("oriro.message.delivery.duration_ms");
    expect(deliveryDuration?.[0]).toBe(20);
    expect(deliveryDuration?.[1]?.["oriro.channel"]).toBe("unknown");
    expect(deliveryDuration?.[1]?.["oriro.delivery.kind"]).toBe("other");
    expect(deliveryDuration?.[1]?.["oriro.outcome"]).toBe("completed");
    const deliverySpanCall = startedSpanCall("oriro.message.delivery");
    const deliveryOptions = deliverySpanCall?.[1];
    expect(deliveryOptions?.attributes?.["oriro.channel"]).toBe("unknown");
    expect(deliveryOptions?.attributes?.["oriro.delivery.kind"]).toBe("other");
    expect(deliveryOptions?.attributes?.["oriro.outcome"]).toBe("completed");
    expect(deliveryOptions?.attributes?.["oriro.delivery.result_count"]).toBe(1);
  ...
  test("exports session recovery and talk metrics with bounded attributes", async () => {
    ...
    const recoveryRequestedCall = firstCounterAddCall("oriro.session.recovery.requested");
    expect(recoveryRequestedCall[0]).toBe(1);
    expect(recoveryRequestedCall[1]?.["oriro.state"]).toBe("processing");
    expect(recoveryRequestedCall[1]?.["oriro.action"]).toBe("abort");
    expect(recoveryRequestedCall[1]?.["oriro.active_work_kind"]).toBe("tool_call");
    const recoveryCompletedCall = firstCounterAddCall("oriro.session.recovery.completed");
    expect(recoveryCompletedCall[0]).toBe(1);
    expect(recoveryCompletedCall[1]?.["oriro.state"]).toBe("processing");
    expect(recoveryCompletedCall[1]?.["oriro.status"]).toBe("released");
    expect(recoveryCompletedCall[1]?.["oriro.action"]).toBe("abort-active-run");
    const recoveryAgeRecord = lastHistogramRecord("oriro.session.recovery.age_ms");
    expect(recoveryAgeRecord?.[0]).toBe(13_000);
    expect(recoveryAgeRecord?.[1]?.["oriro.status"]).toBe("released");
    expect(telemetryState.counters.get("oriro.talk.event")?.add).toHaveBeenCalledWith(1, {
      "oriro.talk.brain": "agent-consult",
      "oriro.talk.event_type": "input.audio.delta",
      "oriro.talk.mode": "realtime",
      "oriro.talk.provider": "openai",
      "oriro.talk.transport": "gateway-relay",
    });
    expect(telemetryState.histograms.get("oriro.talk.audio.bytes")?.record).toHaveBeenCalledWith(
      320,
      {
        "oriro.talk.brain": "agent-consult",
        "oriro.talk.event_type": "input.audio.delta",
    ...
    expect(
      telemetryState.histograms.get("oriro.talk.event.duration_ms")?.record,
    ).toHaveBeenCalledWith(45, {
      "oriro.talk.brain": "agent-consult",
      "oriro.talk.event_type": "latency.metrics",
      "oriro.talk.mode": "realtime",
      "oriro.talk.provider": "openai",
      "oriro.talk.transport": "gateway-relay",
    });
    ...
    const talkCounterCalls = JSON.stringify(
      telemetryState.counters.get("oriro.talk.event")?.add.mock.calls,
    );
  ...
  test("does not export model or tool content unless capture is explicitly enabled", async () => {
    ...
    const modelOptions = startedSpanOptions("oriro.model.call");
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "oriro.content.input_messages")).toBe(
      false,
    );
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "oriro.content.output_messages")).toBe(
      false,
    );
    expect(Object.hasOwn(modelOptions?.attributes ?? {}, "oriro.content.system_prompt")).toBe(
      false,
    );
    expect(modelOptions?.startTime).toBeTypeOf("number");
    const toolOptions = startedSpanOptions("oriro.tool.execution");
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "oriro.content.tool_input")).toBe(false);
    expect(Object.hasOwn(toolOptions?.attributes ?? {}, "oriro.content.tool_output")).toBe(
      false,
    );
    expect(toolOptions?.startTime).toBeTypeOf("number");
    await service.stop?.(ctx);
  ...
  test("exports bounded redacted content when capture fields are opted in", async () => {
    ...
    expect(modelAttrs?.["oriro.content.output_messages"]).toBe("model reply");
    expect(modelAttrs?.["oriro.content.system_prompt"]).toBe("system prompt");
    expect(String(modelAttrs?.["oriro.content.input_messages"])).not.toContain(
      "sk-1234567890abcdef1234567890abcdef", // pragma: allowlist secret
    );
    expect(toolAttrs?.["oriro.content.tool_input"]).toBe("tool input");
    expect(String(toolAttrs?.["oriro.content.tool_output"]).length).toBeLessThanOrEqual(
      MAX_TEST_OTEL_CONTENT_ATTRIBUTE_CHARS + OTEL_TRUNCATED_SUFFIX_MAX_CHARS,
    );
    expect(String(toolAttrs?.["oriro.content.tool_output"])).not.toContain("a".repeat(11));
    await service.stop?.(ctx);
  });
  ...
  test("omits absent model content fields when capture fields are opted in", async () => {
    ...
    expect(attrs["oriro.content.input_messages"]).toBe("user prompt");
    expect(Object.hasOwn(attrs, "oriro.content.output_messages")).toBe(false);
    expect(Object.hasOwn(attrs, "oriro.content.system_prompt")).toBe(false);
    expect(Object.hasOwn(attrs, "oriro.content.tool_definitions")).toBe(false);
  ...
  test("exports tool definitions without requiring input message capture", async () => {
    ...
    expect(Object.hasOwn(attrs ?? {}, "oriro.content.input_messages")).toBe(false);
    expect(JSON.parse(stringAttribute(attrs, "gen_ai.tool.definitions"))).toEqual([
      {
        type: "function",
        name: "lookup",
    ...
    expect(JSON.parse(String(attrs?.["oriro.content.tool_definitions"]))).toEqual([
      {
        name: "lookup",
        description: "Lookup data",
        parameters: { type: "object" },
  ...
  test("redacts sensitive reason in session.state metric attributes", async () => {
    ...
    const sessionStateCall = firstCounterAddCall("oriro.session.state");
    const attrs = sessionStateCall[1];
    expect(sessionStateCall[0]).toBe(1);
    expect(String(attrs?.["oriro.reason"])).toContain("…");
    expect(typeof attrs?.["oriro.reason"]).toBe("string");
    expect(String(attrs?.["oriro.reason"])).not.toContain(
