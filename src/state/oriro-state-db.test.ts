// Oriro state database tests cover state DB migrations and persistence.
...
import { resolveOriroStateSqlitePath } from "./oriro-state-db.paths.js";
import {
  collectSqliteSchemaShape,
  createSqliteSchemaShapeFromSql,
} from "./sqlite-schema-shape.test-support.js";

type StateDbTestDatabase = Pick<OriroStateKyselyDatabase, "diagnostic_events" | "schema_meta">;

function createTempStateDir(): string {
  return fs.mkdtempSync(path.join(os.tmpdir(), "oriro-state-db-"));
}
...
describe("oriro state database", () => {
  it("resolves under the shared state database directory", () => {
    const stateDir = createTempStateDir();

    expect(resolveOriroStateSqlitePath({ ORIRO_STATE_DIR: stateDir })).toBe(
      path.join(stateDir, "state", "oriro.sqlite"),
    );
  });
  ...
  it("keeps test default state under a worker-sharded temp directory", () => {
    expect(
      ...
      path.join(os.tmpdir(), "oriro-test-state", `${process.pid}-7`, "state", "oriro.sqlite"),
    );
  });

  it("creates the shared state schema from the committed SQL shape", () => {
    const stateDir = createTempStateDir();
    const database = openOriroStateDatabase({
      env: { ORIRO_STATE_DIR: stateDir },
    });

    expect(collectSqliteSchemaShape(database.db)).toEqual(
      createSqliteSchemaShapeFromSql(new URL("./oriro-state-schema.sql", import.meta.url)),
    );
    expect(database.path).toBe(path.join(stateDir, "state", "oriro.sqlite"));
  });

  it.runIf(process.platform === "linux")("closes the database when initialization fails", () => {
    const databasePath = path.join(createTempStateDir(), "oriro.sqlite");
    fs.writeFileSync(databasePath, "not a sqlite database");

    expect(() => openOriroStateDatabase({ path: databasePath })).toThrow(
  ...
  it("migrates requester and executor attribution for existing cross-agent tasks", () => {
    ...
    const reopened = openOriroStateDatabase({
      env: { ORIRO_STATE_DIR: stateDir },
    });
    ...
    const currentReopened = openOriroStateDatabase({
      env: { ORIRO_STATE_DIR: stateDir },
    });
    expect(
      currentReopened.db
  ...
  it("opens databases with early cron tables before creating cron indexes", () => {
    const stateDir = createTempStateDir();
    const databasePath = path.join(stateDir, "state", "oriro.sqlite");
  ...
  it("opens databases with early cron run-log tables before creating cron indexes", () => {
    ...
    try {
      ...
      if (previousStateDir === undefined) {
        ...
        process.env["ORIRO_STATE_DIR"] = previousStateDir;
      }
    }
  });
  ...
  it("does not chmod shared parent directories for explicit database paths", () => {
    const databasePath = path.join(
      os.tmpdir(),
      `oriro-explicit-state-${process.pid}-${Date.now()}.sqlite`,
    );

    expect(() => openOriroStateDatabase({ path: databasePath })).not.toThrow();
  ...
  it("keeps cached handles open when another state path is opened", () => {
    ...
    const first = openOriroStateDatabase({ path: firstPath });
    const second = openOriroStateDatabase({ path: secondPath });

    expect(first.db.isOpen).toBe(true);
    expect(second.db.isOpen).toBe(true);
    expect(openOriroStateDatabase({ path: firstPath })).toBe(first);
    expect(readSqliteNumberPragma(first.db, "user_version")).toBe(1);
  });

  it("keys explicit relative paths by resolved database pathname", () => {
    const moduleUrl = new URL("./oriro-state-db.ts", import.meta.url).href;
    const output = execFileSync(
      ...
      [
        ...
        `
          ...
          const root = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-state-db-relative-"));
          ...
          try {
            ...
            const first = openOriroStateDatabase({ path: "state.sqlite" });
            first.db
            ...
            const second = openOriroStateDatabase({ path: "state.sqlite" });
            second.db
              .prepare("INSERT INTO diagnostic_events (scope, event_key, payload_json, created_at) VALUES (?, ?, ?, ?)")
  ...
  it("uses savepoints for nested write transaction rollback", () => {
    const stateDir = createTempStateDir();
    const options = { env: { ORIRO_STATE_DIR: stateDir } };

    runOriroStateWriteTransaction((database) => {
      const stateDb = getNodeSqliteKysely<StateDbTestDatabase>(database.db);
      executeSqliteQuerySync(
        database.db,
      ...
      expect(() =>
        runOriroStateWriteTransaction((inner) => {
  ...
  it("rejects Promise-returning write transactions", () => {
    ...
    expect(() =>
      runOriroStateWriteTransaction(async () => {
        return "not sync";
      }, options),
