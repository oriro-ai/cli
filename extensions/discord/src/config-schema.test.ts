...
describe("discord config schema", () => {
  ...
  it("loads guild map and dm group settings", () => {
    ...
    expect(cfg.dm?.groupChannels).toEqual(["oriro-dm"]);
    ...
    expect(cfg.guilds?.["123"]?.slug).toBe("friends-of-oriro");
  ...
  it("rejects invalid Discord realtime voice modes", () => {
    for (const voice of [
      ...
      { mode: "agent-proxy", realtime: { wakeNames: [""] } },
      { mode: "agent-proxy", realtime: { wakeNames: ["Oriro Bot Helper"] } },
  ...
  it.each([
    ...
    },
    {
      ...
      config: { activity: "Live", activityUrl: "https://twitch.tv/oriro" },
    },
    {
