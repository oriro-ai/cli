...
function createConfig(port: number): OriroConfig {
  return {
    channels: {
      msteams: {
        enabled: true,
...
function updateMSTeamsConfig(
  cfg: OriroConfig,
  patch: NonNullable<NonNullable<OriroConfig["channels"]>["msteams"]>,
): void {
...
function requireRegisteredMSTeamsConfig(): OriroConfig {
  ...
  it("gates poll card votes before recording them", async () => {
    ...
    const response = await cardActionHandler({
      activity: {
        ...
        value: { action: { data: { oriroPollId: "poll-1", choices: "0" } } },
      },
    });
