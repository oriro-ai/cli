...
import { CUSTOM_LOCAL_AUTH_MARKER } from "oriro/plugin-sdk/provider-auth";
import type { OriroConfig } from "oriro/plugin-sdk/provider-auth";
import type { ModelDefinitionConfig } from "oriro/plugin-sdk/provider-model-shared";
import { resolveAgentModelPrimaryValue } from "oriro/plugin-sdk/provider-onboard";
import {
  SELF_HOSTED_DEFAULT_CONTEXT_WINDOW,
  type ProviderAuthMethodNonInteractiveContext,
  type ProviderCatalogContext,
} from "oriro/plugin-sdk/provider-setup";
import type { WizardPrompter } from "oriro/plugin-sdk/setup";
...
vi.mock("oriro/plugin-sdk/provider-auth", async (importOriginal) => {
  const actual = await importOriginal<typeof import("oriro/plugin-sdk/provider-auth")>();
  return {
    ...actual,
...
vi.mock("oriro/plugin-sdk/provider-setup", async (importOriginal) => {
  const actual = await importOriginal<typeof import("oriro/plugin-sdk/provider-setup")>();
  return {
    ...actual,
...
afterAll(() => {
  ...
  vi.doUnmock("oriro/plugin-sdk/provider-auth");
  vi.doUnmock("oriro/plugin-sdk/provider-setup");
  vi.resetModules();
});
...
function buildConfig(): OriroConfig {
  return {
    models: {
      providers: {
        lmstudio: {
...
describe("lmstudio setup", () => {
  ...
  it("non-interactive setup fails when requested model is missing", async () => {
    ...
    const dockerSetup = ["1", "true", "yes", "on"].includes(
      process.env.ORIRO_DOCKER_SETUP?.trim().toLowerCase() ?? "",
    );
  ...
  it("interactive Docker setup defaults to the host LM Studio endpoint", async () => {
    vi.stubEnv("ORIRO_DOCKER_SETUP", "1");
    const { prompter, text } = createQueuedWizardPrompterHarness([
