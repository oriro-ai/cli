...
function resolveMemoryRuntimePluginIds(config: OriroConfig): string[] {
...
function resolveMemoryRuntimeWorkspaceDir(cfg: OriroConfig): string | undefined {
...
function ensureMemoryRuntime(cfg?: OriroConfig) {
...
export function resolveActiveMemoryBackendConfig(params: { cfg: OriroConfig; agentId: string }) {
  return ensureMemoryRuntime(params.cfg)?.resolveMemoryBackendConfig(params) ?? null;
}

/** Closes all active plugin-backed memory search managers. */
export async function closeActiveMemorySearchManagers(cfg?: OriroConfig): Promise<void> {
  void cfg;
