...
import { resolveOriroDevSourceRoot } from "./dev-source-root.js";
...
function hasTrustedOriroRootIndicator(params: {
  packageRoot: string;
  ...
  const hasOriroBin =
    (typeof params.packageJson.bin === "string" &&
      normalizeLowercaseStringOrEmpty(params.packageJson.bin).includes("oriro")) ||
    (typeof params.packageJson.bin === "object" &&
      params.packageJson.bin !== null &&
      typeof params.packageJson.bin.oriro === "string");
  const hasOriroEntrypoint = fs.existsSync(path.join(params.packageRoot, "oriro.mjs"));
  return hasCliEntryExport || hasOriroBin || hasOriroEntrypoint;
}
...
function readPluginSdkSubpathsFromPackageRoot(packageRoot: string): string[] | null {
  ...
  if (!hasTrustedOriroRootIndicator({ packageRoot, packageJson: pkg })) {
    return null;
  }
...
function resolveTrustedOriroRootFromArgvHint(params: {
  argv1?: string;
  cwd: string;
}): string | null {
  if (!params.argv1) {
  ...
  return hasTrustedOriroRootIndicator({ packageRoot, packageJson }) ? packageRoot : null;
}
...
function resolveDevSourceRootParam(params: { devSourceRoot?: string | null }): string | null {
  return params.devSourceRoot !== undefined
    ...
    : resolveOriroDevSourceRoot(process.env);
}
...
function resolveLoaderPluginSdkPackageRoot(
  ...
  const cwd = params.cwd ?? path.dirname(params.modulePath);
  const fromCwd = resolveOriroPackageRootSync({ cwd });
  const fromExplicitHints =
    resolveTrustedOriroRootFromArgvHint({ cwd, argv1: params.argv1 }) ??
    (params.moduleUrl
...
const PLUGIN_SDK_PACKAGE_NAMES = ["oriro/plugin-sdk", "@oriro/plugin-sdk"] as const;
...
const PRIVATE_PLUGIN_SDK_SUBPATH_OWNERS: readonly PrivatePluginSdkSubpathOwner[] = [
  {
    ...
    officialInstalledPackageName: "@oriro/codex",
...
const WORKSPACE_PACKAGE_ALIAS_ENTRIES: WorkspacePackageAliasEntry[] = [
  {
    packageName: "@oriro/gateway-client",
  ...
  {
    packageName: "@oriro/gateway-protocol",
  ...
  {
    packageName: "@oriro/markdown-core",
  ...
  {
    packageName: "@oriro/media-generation-core",
  ...
  {
    packageName: "@oriro/normalization-core",
  ...
  {
    packageName: "@oriro/terminal-core",
  ...
  {
    packageName: "@oriro/model-catalog-core",
...
export function listWorkspacePackageExportAliasEntries(params: {
  ...
  const fallbackPackageRoot = resolveOriroPackageRootSync({ cwd: process.cwd() });
  const packageJson =
...
function readBundledPluginPackageName(packageJsonPath: string): string | null {
  ...
  return name.startsWith("@oriro/") ? name : null;
}
...
function shouldIncludePrivateLocalOnlyPluginSdkSubpaths() {
  return process.env.ORIRO_ENABLE_PRIVATE_QA_CLI === "1";
}
...
export function resolvePluginSdkScopedAliasMap(
  ...
  for (const subpath of listPluginSdkExportedSubpaths({
    ...
    for (const kind of orderedKinds) {
      ...
      if (Object.hasOwn(aliasMap, `oriro/plugin-sdk/${subpath}`)) {
        break;
      }
    }
  }
...
export function buildPluginLoaderAliasMap(
  ...
  const result: Record<string, string> = {
    ...(extensionApiAlias
      ? { "oriro/extension-api": normalizeJitiAliasTargetPath(extensionApiAlias) }
      : {}),
