// Thread Ownership plugin entrypoint registers its Oriro integration.
...
type AgentEntry = NonNullable<NonNullable<OriroConfig["agents"]>["list"]>[number];
...
function resolveOwnershipAgent(config: OriroConfig): { id: string; name: string } {
...
export default definePluginEntry({
  ...
  register(api: OriroPluginApi) {
    const resolveCurrentState = () => {
      const currentConfig = (api.runtime.config?.current?.() ?? api.config) as OriroConfig;
