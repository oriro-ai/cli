...
export function resolveAcpInstallCommandHint(cfg: OriroConfig): string {
  ...
  if (backendId === "acpx") {
    ...
    if (existsSync(workspaceLocalPath)) {
      return `oriro plugins install ${workspaceLocalPath}`;
    }
    ...
    if (bundledInstallHint) {
      const localPath = bundledInstallHint.replace(/^oriro plugins install /u, "");
    ...
    return "oriro plugins install acpx";
  }
