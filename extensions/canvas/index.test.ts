...
import type { AnyAgentTool, OriroPluginApi } from "oriro/plugin-sdk/plugin-entry";
...
function registerCanvas() {
  const routes: Array<Parameters<OriroPluginApi["registerHttpRoute"]>[0]> = [];
  const services: Array<Parameters<OriroPluginApi["registerService"]>[0]> = [];
  const resolvers: Array<Parameters<OriroPluginApi["registerHostedMediaResolver"]>[0]> = [];
  const tools: Array<Parameters<OriroPluginApi["registerTool"]>[0]> = [];
  const cliFeatures: Array<{
    registrar: Parameters<OriroPluginApi["registerNodeCliFeature"]>[0];
    opts: Parameters<OriroPluginApi["registerNodeCliFeature"]>[1];
  }> = [];
...
describe("Canvas plugin entry", () => {
  ...
  it("defers Canvas host implementation until a registered route is used", async () => {
    ...
    await routes[0]?.handler({ url: "/__oriro__/canvas" } as never, {} as never);
  ...
  it("defers Canvas resolver, CLI, and tool implementations until use", async () => {
    ...
    await expect(resolvers[0]?.("/__oriro__/canvas/documents/id/index.html")).resolves.toBe(
      "/tmp/canvas-asset",
    );
