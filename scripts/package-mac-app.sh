...
# Build and bundle Oriro into a minimal .app we can open.
# Outputs to dist/Oriro.app
...
BUNDLE_ID="${BUNDLE_ID:-ai.oriro.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/oriro/oriro/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Oriro/Resources/Info.plist"
...
plist_set_string_required "$APP_ROOT/Contents/Info.plist" OriroBuildTimestamp "$BUILD_TS"
plist_set_string_required "$APP_ROOT/Contents/Info.plist" OriroGitCommit "$GIT_COMMIT"
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Oriro"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Oriro"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Oriro"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Oriro" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Oriro/Resources/Oriro.icns" "$APP_ROOT/Contents/Resources/Oriro.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Oriro/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying OriroKit resources"
ORIROKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/OriroKit_OriroKit.bundle"
if [ -d "$ORIROKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/OriroKit_OriroKit.bundle"
  cp -R "$ORIROKIT_BUNDLE" "$APP_ROOT/Contents/Resources/OriroKit_OriroKit.bundle"
else
  echo "ERROR: OriroKit resource bundle not found at $ORIROKIT_BUNDLE" >&2
  exit 1
fi
...
running_packaged_app_pids() {
  ...
  local app_binary="$APP_ROOT/Contents/MacOS/Oriro"
  local pid
