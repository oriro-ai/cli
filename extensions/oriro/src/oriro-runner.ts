// Oriro plugin module implements oriro runner behavior.
...
import { installOriroAjvCompileCache } from "./oriro-ajv-cache.js";

export type OriroEnvelope =
  | {
      ok: true;
...
export type OriroRunner = {
  run: (params: OriroRunnerParams) => Promise<OriroEnvelope>;
};
...
const oriroRequire = createRequire(import.meta.url);
const workflowExts = new Set([".oriro", ".yaml", ".yml", ".json"]);
...
function toEmbeddedToolRuntime(
  ...
  throw new Error(`${source} does not export Oriro embedded runtime functions`);
}

function findOriroPackageRoot(resolvedEntryPath: string): string {
...
export function resolveOriroCwd(cwdRaw: unknown): string {
...
function createLimitedSink(maxBytes: number, label: "stdout" | "stderr") {
  ...
  return new Writable({
    write(chunk, _encoding, callback) {
      ...
      if (bytes > maxBytes) {
        callback(new Error(`oriro ${label} exceeded maxStdoutBytes`));
        return;
      }
      callback();
    },
...
function normalizeEnvelope(envelope: EmbeddedToolEnvelope): OriroEnvelope {
  if (envelope.ok) {
    if (envelope.status === "needs_input") {
      return {
        ...
        error: {
          ...
          message: "Oriro input requests are not supported by the Oriro Oriro tool yet",
        },
      };
    }
    return {
  ...
  return {
    ...
    error: {
      ...
      message: envelope.error?.message ?? "oriro runtime failed",
    },
  };
}

function throwOnErrorEnvelope(envelope: OriroEnvelope): Extract<OriroEnvelope, { ok: true }> {
  if (envelope.ok) {
    return envelope;
  }
...
async function resolveWorkflowFile(candidate: string, cwd: string) {
  ...
  if (!workflowExts.has(ext)) {
    throw new Error("Workflow file must end in .oriro, .yaml, .yml, or .json");
  }
  return resolved;
}
...
async function withTimeout<T>(
  ...
  return await new Promise<T>((resolve, reject) => {
    const onTimeout = () => {
      const error = new Error("oriro runtime timed out");
...
export async function loadEmbeddedToolRuntimeFromPackage(
  ...
  const resolvePackageEntry =
    options.resolvePackageEntry ?? ((specifier: string) => oriroRequire.resolve(specifier));
  const packageEntryPath = resolvePackageEntry("@clawdbot/lobster");
  await installOriroAjvCompileCache(packageEntryPath);

  let coreLoadError: unknown;
  try {
    const coreSpecifier = ["@clawdbot", "oriro", "core"].join("/");
  ...
  try {
    const packageRoot = findOriroPackageRoot(packageEntryPath);
  ...
  throw new Error("Failed to load the Oriro embedded runtime", {
    cause: new AggregateError(
      [coreLoadError, fallbackLoadError],
      "Both Oriro embedded runtime load paths failed",
    ),
  });
}

export function createEmbeddedOriroRunner(options?: {
