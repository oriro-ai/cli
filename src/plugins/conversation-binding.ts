...
const LEGACY_CODEX_PLUGIN_SESSION_PREFIXES = [
  "oriro-app-server:thread:",
  "oriro-codex-app-server:thread:",
] as const;
...
const PLUGIN_BINDING_PENDING_REQUESTS_KEY = Symbol.for("oriro.pluginBindingPendingRequests");
...
const pluginBindingGlobalStateKey = Symbol.for("oriro.plugins.binding.global-state");
...
export function buildPluginBindingUnavailableText(binding: PluginConversationBinding): string {
  return `The bound plugin ${resolvePluginBindingDisplayName(binding)} is not currently loaded. Routing this message to ORIRO instead. If this started after an update, run "oriro doctor --fix"; otherwise reinstall or enable the plugin.${buildDetachHintSuffix(binding.detachHint)}`;
}
