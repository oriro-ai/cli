...
import { FILE_TYPE_SNIFF_MAX_BYTES } from "@oriro/media-core/mime";
import {
...
async function withSessionStore(
  ...
  const stateDir = await fs.mkdtemp(
    path.join(resolvePreferredOriroTmpDir(), "oriro-session-attachments-"),
  );
...
describe("plugin session attachments", () => {
  afterEach(() => {
    ...
    delete (globalThis as { proofAttachmentApi?: OriroPluginApi }).proofAttachmentApi;
