...
describe("initSessionState BodyStripped", () => {
  it("prefers BodyForAgent over Body for group chats", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-sender-meta-"));
  ...
  it("prefers BodyForAgent over Body for direct chats", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-sender-meta-direct-"));
    const storePath = path.join(root, "sessions.json");
    const cfg = { session: { store: storePath } } as OriroConfig;
...
describe("mention helpers", () => {
  it("builds regexes and skips invalid or unsafe patterns", () => {
    const regexes = buildMentionRegexes({
      messages: {
        groupChat: { mentionPatterns: ["\\boriro\\b", "(invalid", "(a+)+$"] },
      },
    });
    expect(regexes).toHaveLength(1);
    expect(regexes[0]?.test("oriro")).toBe(true);
  });

  it("normalizes zero-width characters", () => {
    expect(normalizeMentionText("open\u200boriro")).toBe("oriro");
  });

  it("matches patterns case-insensitively", () => {
    const regexes = buildMentionRegexes({
      messages: { groupChat: { mentionPatterns: ["\\boriro\\b"] } },
    });
    expect(matchesMentionPatterns("ORIRO: hi", regexes)).toBe(true);
  });
  ...
  it("scopes configured mention patterns by provider conversation policy", () => {
    ...
    expect(matchesMentionPatterns("oriro: hi", allowed)).toBe(true);
    expect(matchesMentionPatterns("oriro: hi", denied)).toBe(false);
  });
  ...
  it("preserves mention patterns for callers without scoped policy facts", () => {
    ...
    expect(matchesMentionPatterns("oriro", regexes)).toBe(true);
  });
  ...
  it("strips safe mention patterns and ignores unsafe ones", () => {
    const stripped = stripMentions("oriro " + "a".repeat(28) + "!", {} as MsgContext, {
      messages: {
        groupChat: { mentionPatterns: ["\\boriro\\b", "(a+)+$"] },
      },
    });
