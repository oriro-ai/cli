...
import { normalizeOptionalString as normalizeString } from "@oriro/normalization-core/string-coerce";
...
function getConfigPath(config: OriroConfig | undefined, path: string): unknown {
...
function setConfigPath(target: OriroConfig, path: string, value: unknown): void {
...
function buildProviderEntry(params: {
  ...
  return {
    ...
    getConfiguredCredentialValue: (config?: OriroConfig) =>
      getConfigPath(config, credentialPath),
    setConfiguredCredentialValue: (configTarget: OriroConfig, value: unknown) => {
      setConfigPath(configTarget, credentialPath, value);
    },
    applySelectionConfig: (config: OriroConfig) =>
