...
function countRuntimeToolCalls(
  ...
  const cell = runtime === "oriro" ? result.cells.oriro : result.cells.codex;
...
function buildRow(params: {
  ...
  return {
    ...
    oriro: result ? cellStatus(result.cells.oriro) : "not-run",
    codex: result ? cellStatus(result.cells.codex) : "not-run",
    drift: result?.drift ?? "not-run",
    oriroToolCalls: countRuntimeToolCalls(result, "oriro", runtimeToolName),
...
function coverageFailureForRow(row: QaToolCoverageRow): string | undefined {
  ...
  if (row.oriro !== "pass" || row.codex !== "pass") {
    return `${row.tool} status oriro=${row.oriro} codex=${row.codex}`;
  }
  if (row.drift === "failure-mode") {
    return `${row.tool} drift=failure-mode${row.details ? ` (${row.details})` : ""}`;
  }
  if (row.runtimeToolName && row.oriroToolCalls === 0) {
    return `${row.tool} missing oriro tool call ${row.runtimeToolName}`;
  }
...
export function buildQaToolCoverageReport(params: {
  ...
  return {
    ...
    dynamicIntegrationTools: rows.filter((row) => row.bucket === "oriro-dynamic-integration")
      .length,
    searchableDynamicTools: rows.filter(
      (row) => row.capabilityLayer === "oriro-dynamic-searchable",
    ).length,
...
export function renderQaToolCoverageMarkdownReport(report: QaToolCoverageReport): string {
  const lines = [
    `# Oriro Runtime Tool Coverage — ${report.runtimePair[0]} vs ${report.runtimePair[1]}`,
    "",
    ...
    `- Oriro dynamic integration tools: ${report.dynamicIntegrationTools}`,
    ...
    "",
    "| Tool | Bucket | Expected layer | Capability layer | Required | Fixtures | Oriro | Codex | Drift | Codex default impact | QA impact | Action | Tracking |",
    "| --- | --- | --- | --- | --- | ---: | --- | --- | --- | --- | --- | --- | --- |",
  ];

  for (const row of report.rows) {
    lines.push(
      `| ${row.tool} | ${row.bucket} | ${row.expectedLayer} | ${row.capabilityLayer} | ${row.required ? "yes" : "no"} | ${row.fixtureCount} | ${row.oriro} | ${row.codex} | ${row.drift} | ${row.codexDefaultImpact ?? ""} | ${row.qaImpact ?? ""} | ${row.action ?? ""} | ${row.tracking ?? ""} |`,
    );
  }
