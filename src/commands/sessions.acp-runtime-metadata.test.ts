 ...
 *
 *   - RED: ACP-keyed session resolves to `id: "oriro"`, `source: "implicit"`.
...
/**
 * Build a minimal `OriroConfig` that mirrors the deployed scenario:
 ...
 *
 * Result: the old metadata resolver fell through to the implicit "oriro"
 * branch — which is the bug under test.
 */
function buildConfigWithoutAgentRuntimePolicy(): OriroConfig {
  return {
    agents: {
      list: [
        {
...
describe("sessions --json agentRuntime classifier (catalog #18)", () => {
  it("RED→GREEN: ACP session key is no longer misclassified (overlay applies)", () => {
    ...
    // resolved metadata said id="oriro", source="implicit".
    ...
    expect(
      agentRuntime.id,
      `ACP session ${ACP_SESSION_KEY} should no longer be misclassified as "auto" or "oriro". ` +
  ...
  it("FIX-SHAPE expectation: ACP session should resolve to 'acpx'", () => {
    ...
    // from "oriro" and reflects the actual runtime driving the session.
