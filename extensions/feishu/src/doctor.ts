...
function collectConfiguredAgentIds(cfg: OriroConfig): string[] {
...
function resolveConfiguredDefaultAgentId(cfg: OriroConfig): string {
...
function formatPreviewWarning(inspection: FeishuDoctorInspection): string {
  ...
  return [
    ...
    '- Run "oriro doctor --fix" to rebuild Feishu local state.',
  ].join("\n");
}
...
function hasConfiguredFeishuChannel(cfg: OriroConfig): boolean {
