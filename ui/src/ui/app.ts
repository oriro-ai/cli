...
declare global {
  interface Window {
    __ORIRO_CONTROL_UI_BASE_PATH__?: string;
  }
}
...
export class OriroApp extends LitElement {
  ...
  @state() theme: ThemeName = this.settings.theme ?? "oriro";
  ...
  @state() orirohubSearchQuery = "";
  @state() orirohubSearchResults: OriroHubSearchResult[] | null = null;
  @state() orirohubSearchLoading = false;
  @state() orirohubSearchError: string | null = null;
  @state() orirohubDetail: OriroHubSkillDetail | null = null;
  @state() orirohubDetailSlug: string | null = null;
  @state() orirohubDetailLoading = false;
  @state() orirohubDetailError: string | null = null;
  @state() orirohubInstallSlug: string | null = null;
  @state() orirohubInstallMessage: { kind: "success" | "error"; text: string } | null = null;
  @state() orirohubVerdicts: Record<string, OriroHubSkillSecurityVerdict> = {};
  @state() orirohubVerdictsLoading = false;
  @state() orirohubVerdictsError: string | null = null;
  ...
  clearCustomTheme() {
    const nextTheme = this.theme === "custom" ? "oriro" : this.theme;
    this.customThemeImportExpanded = true;
    this.customThemeImportSelectOnSuccess = false;
    applySettingsInternal(this as unknown as Parameters<typeof applySettingsInternal>[0], {
      ...this.settings,
...
if (!customElements.get("oriro-app")) {
  customElements.define("oriro-app", OriroApp);
}
