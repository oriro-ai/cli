/**
 * Oriro-managed Chrome lifecycle and CDP helpers.
 *
...
import { prepareOomScoreAdjustedSpawn } from "oriro/plugin-sdk/process-runtime";
...
async function ensureManagedChromePortAvailable(
  ...
  // Chromium tries IPv4 loopback first, while Oriro polls the configured endpoint.
...
function chromeLaunchHints(params: {
  ...
  if (CHROME_MISSING_DISPLAY_PATTERN.test(params.stderrOutput) && !headlessMode.headless) {
    hints.push(
      "No DISPLAY/X server was detected. Set ORIRO_BROWSER_HEADLESS=1, remove the headed override, start Xvfb, or run the Gateway in a desktop session.",
    );
  }
  if (CHROME_SINGLETON_IN_USE_PATTERN.test(params.stderrOutput)) {
    hints.push(
      `The Chromium profile "${params.profile.name}" is locked. Stop the existing browser or remove stale Singleton* lock files under ~/.oriro/browser/${params.profile.name}/user-data.`,
    );
  }
...
export type RunningChrome = {
   ...
   * through stopOriroChrome without type churn.
   */
...
/** Resolve the user-data-dir path for a managed Oriro Chrome profile. */
export function resolveOriroUserDataDir(profileName = DEFAULT_ORIRO_BROWSER_PROFILE_NAME) {
...
/** Build Chrome launch arguments for the managed Oriro browser. */
export function buildOriroChromeLaunchArgs(params: {
  resolved: ResolvedBrowserConfig;
  profile: ResolvedBrowserProfile;
  userDataDir: string;
...
/** Launch or attach to the managed Oriro Chrome profile. */
export async function launchOriroChrome(
  resolved: ResolvedBrowserConfig,
  profile: ResolvedBrowserProfile,
  launchOptions: ManagedBrowserHeadlessOptions = {},
  ...
  const needsDecorate = !isProfileDecorated(
    userDataDir,
    profile.name,
    (profile.color ?? DEFAULT_ORIRO_BROWSER_COLOR).toUpperCase(),
  ...
  const spawnOnce = () => {
    ...
    if (process.platform === "linux") {
      const chromiumStateDir = path.join(resolvePreferredOriroTmpDir(), ".chromium");
  ...
  try {
    ...
    log.warn(`oriro browser clean-exit prefs failed: ${String(err)}`);
  }
...
export async function stopOriroChrome(
