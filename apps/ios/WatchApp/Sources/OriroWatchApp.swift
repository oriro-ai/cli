...
@main
struct OriroWatchApp: App {
    @Environment(\.scenePhase) private var scenePhase
    @State private var inboxStore = WatchInboxStore(
        requestNotificationAuthorization: !OriroWatchApp.isScreenshotMode)
    @State private var receiver: WatchConnectivityReceiver?
    @State private var execApprovalRefreshTask: Task<Void, Never>?

    private static let screenshotModeDefaultsKey = "oriro.watch.screenshotMode"
    private static let isScreenshotMode = ProcessInfo.processInfo.arguments.contains(
        "--oriro-watch-screenshot-mode")
        || ProcessInfo.processInfo.environment["ORIRO_WATCH_SCREENSHOT_MODE"] == "1"
        || UserDefaults.standard.bool(forKey: OriroWatchApp.screenshotModeDefaultsKey)
    ...
    var body: some Scene {
        WindowGroup {
            WatchInboxView(
                ...
                .task {
                    if OriroWatchApp.isScreenshotMode {
