...
export function parseArgs(argv) {
  ...
  const options = {
    ...
    pluginIds: [],
    shardTotal: readOptionalPositiveIntEnv("ORIRO_PLUGIN_GATEWAY_GAUNTLET_TOTAL") ?? 1,
    shardIndex: readOptionalNonNegativeIntEnv("ORIRO_PLUGIN_GATEWAY_GAUNTLET_INDEX") ?? 0,
    limit: undefined,
    skipPrebuild: false,
    skipLifecycle: false,
    skipQa: false,
    ...
    qaTimeoutMs: 900_000,
    allowEmpty: false,
    failOnObservation: process.env.ORIRO_PLUGIN_GATEWAY_GAUNTLET_FAIL_ON_OBSERVATION === "1",
    keepRunRoot: process.env.ORIRO_PLUGIN_GATEWAY_GAUNTLET_KEEP_RUN_ROOT === "1",
  };
  const envIds = normalizeCsv(process.env.ORIRO_PLUGIN_GATEWAY_GAUNTLET_IDS);
...
Environment:
  ORIRO_PLUGIN_GATEWAY_GAUNTLET_IDS   Comma-separated plugin ids to include
  ORIRO_PLUGIN_GATEWAY_GAUNTLET_TOTAL Total plugin shards
  ORIRO_PLUGIN_GATEWAY_GAUNTLET_INDEX Zero-based shard index
  ORIRO_PLUGIN_GATEWAY_GAUNTLET_FAIL_ON_OBSERVATION=1
  ORIRO_PLUGIN_GATEWAY_GAUNTLET_KEEP_RUN_ROOT=1
  ORIRO_PLUGIN_GATEWAY_GAUNTLET_QA_SUMMARY_MAX_BYTES  QA summary read ceiling
`);
}
...
function oriroCommand(repoRoot, args) {
  return {
    command: process.execPath,
...
function sourceOriroCommand(repoRoot, args) {
  return {
    command: process.execPath,
...
function createIsolatedEnv(repoRoot, runRoot) {
  ...
  return {
    ...
    ORIRO_LOG_DIR: path.join(runRoot, "logs"),
    ORIRO_QA_SUITE_PROGRESS: process.env.ORIRO_QA_SUITE_PROGRESS ?? "1",
...
function buildSlashHelpProbe(params) {
  ...
  return {
    ...
    ...oriroCommand(params.repoRoot, [command, "--help"]),
...
async function runPluginLifecycleCommand(params) {
  ...
  params.rows.push(
    await runMeasuredCommand({
      ...
      ...oriroCommand(params.repoRoot, ["plugins", ...params.args]),
      label: params.label,
...
async function runQaChunks(params) {
  ...
  for (let index = 0; index < chunks.length; index += 1) {
    ...
    const row = await runMeasuredCommand({
      ...
      ...sourceOriroCommand(params.repoRoot, [
        "qa",
        "suite",
        "--provider-mode",
        "mock-openai",
...
async function main() {
  ...
  const runRoot = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-plugin-gauntlet-"));
