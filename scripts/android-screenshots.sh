...
boot_emulator() {
  ...
  EMULATOR_LOG="$(mktemp "${TMPDIR:-/tmp}/oriro-android-screenshot-emulator.XXXXXX.log")"
...
for scene in "${SCENES[@]}"; do
  output_path="${OUTPUT_DIR}/oriro-${scene}.png"
  "$ADB_BIN" -s "$ADB_SERIAL" shell am force-stop ai.oriro.app >/dev/null
  "$ADB_BIN" -s "$ADB_SERIAL" shell am start -W \
    -n ai.oriro.app/.MainActivity \
    --ez oriro.screenshotMode true \
    --es oriro.screenshotScene "$scene" >/dev/null
