...
frontmatter and a markdown body. Oriro loads bundled skills plus any local
...
Oriro loads from these sources, **highest precedence first**. When the same
...
| 4           | Managed / local skills | `~/.oriro/skills`                    |
| 5           | Bundled skills         | shipped with the install                |
| 6 — lowest  | Extra directories      | `skills.load.extraDirs` + plugin skills |

Skill roots support grouped layouts. Oriro discovers a skill whenever
...
<Note>
  Codex CLI's native `$CODEX_HOME/skills` directory is **not** an Oriro
  skill root. Use `oriro migrate plan codex` to inventory those skills, then
  `oriro migrate codex` to copy them into your Oriro workspace.
</Note>
...
| Shared managed | `~/.oriro/skills`         | All agents on this machine  |
...
`oriro.plugin.json` (paths relative to the plugin root). Plugin skills load
...
skill overrides them. Gate them via `metadata.oriro.requires.config` on the
...
## Installing from OriroHub

[OriroHub](https://orirohub.ai) is the public skills registry. Use
`oriro skills` commands for install and update, or the `orirohub` CLI for
publish and sync.

| Action                             | Command                                                |
| ---------------------------------- | ------------------------------------------------------ |
| Install a skill into the workspace | `oriro skills install <slug>`                       |
| Install from a Git repository      | `oriro skills install git:owner/repo@ref`           |
| Install a local skill directory    | `oriro skills install ./path/to/skill --as my-tool` |
| Install for all local agents       | `oriro skills install <slug> --global`              |
| Update all workspace skills        | `oriro skills update --all`                         |
| Update a shared managed skill      | `oriro skills update <slug> --global`               |
| Update all shared managed skills   | `oriro skills update --all --global`                |
| Verify a skill's trust envelope    | `oriro skills verify <slug>`                        |
| Print the generated Skill Card     | `oriro skills verify <slug> --card`                 |
| Publish / sync via OriroHub CLI     | `orirohub sync --all`                                   |

<AccordionGroup>
  <Accordion title="Install details">
    `oriro skills install` installs into the active workspace `skills/`
    directory by default. Add `--global` to install into the shared
    `~/.oriro/skills` directory, visible to all local agents unless agent
    ...
    `oriro skills update` tracks OriroHub installs only — reinstall Git or
    local sources to refresh them.

  </Accordion>
  <Accordion title="Verification and security scanning">
    `oriro skills verify <slug>` asks OriroHub for the skill's
    `orirohub.skill.verify.v1` trust envelope. Installed OriroHub skills verify
    against the version and registry recorded in `.orirohub/origin.json`.

    OriroHub skill pages expose the latest security scan state before install,
    with detail pages for VirusTotal, OriroScan, and static analysis. The
    command exits non-zero when OriroHub marks verification as failed. Publishers
    recover false positives through the OriroHub dashboard or
    `orirohub skill rescan <slug>`.

  </Accordion>
  <Accordion title="Private archive installs">
    Gateway clients that need non-OriroHub delivery can stage a zip skill archive
    ...
    `oriro.json`. Normal OriroHub installs never need that setting.
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  <Accordion title="Path containment">
    ...
    Managed `~/.oriro/skills` and personal `~/.agents/skills` may contain
  ...
  <Accordion title="Operator install policy">
    ...
    source path, applies to OriroHub, uploaded, Git, local, update, and
...
<Note>
  Oriro follows the [AgentSkills](https://agentskills.io) spec. The
...
<ParamField path="disable-model-invocation" type="boolean" default="false">
  When `true`, Oriro keeps the skill's instructions out of the agent's normal
...
Oriro filters skills at load time using `metadata.oriro` (single-line
JSON in the frontmatter). A skill with no `metadata.oriro` block is always
...
<ParamField path="requires.config" type="string[]">
  Each `oriro.json` path must be truthy.
</ParamField>
...
<Note>
  ...
  `metadata.oriro` is absent, so older installed skills keep their
...
<AccordionGroup>
  <Accordion title="Installer selection rules">
    ...
    - If all installers are `download`, Oriro lists each entry so you can
      see all available artifacts.
    - Specs can include `os: ["darwin"|"linux"|"win32"]` to filter by platform.
    - Node installs honor `skills.install.nodeManager` in `oriro.json`
  ...
  <Accordion title="Per-installer details">
    - **Homebrew:** Oriro does not auto-install Homebrew or translate brew
    ...
    - **Download:** `url` (required), `archive` (`tar.gz` | `tar.bz2` | `zip`),
      ...
      `targetDir` (default: `~/.oriro/tools/<skillKey>`).
  </Accordion>
...
<ParamField path="apiKey" type='string | { source, provider, id }'>
  Convenience field for skills that declare `metadata.oriro.primaryEnv`.
...
<Note>
  ...
  `metadata.oriro.skillKey`, use that key under `skills.entries`. Quote
  hyphenated names: JSON5 allows quoted keys.
</Note>

## Environment injection

When an agent run starts, Oriro:

<Steps>
  <Step title="Reads skill metadata">
    Oriro resolves the effective skill list for the agent, applying gating
...
For the bundled `claude-cli` backend, Oriro also materializes the same
...
Oriro snapshots eligible skills **when a session starts** and reuses that
...
allowlist changes, Oriro refreshes the snapshot to keep visible skills
aligned.

<AccordionGroup>
  <Accordion title="Skills watcher">
    By default, Oriro watches skill folders and bumps the snapshot when
  ...
  <Accordion title="Remote macOS nodes (Linux gateway)">
    ...
    `system.run` allowed, Oriro can treat macOS-only skills as eligible when
    ...
    answering bin probes, Oriro clears its cached bin matches.
...
When skills are eligible, Oriro injects a compact XML block into the system
