...
import { resolveIntegerOption } from "@oriro/acp-core/numeric-options";
...
/** Resolves the legacy file-backed ACP ledger path under the Oriro state directory. */
...
export async function migrateFileAcpEventLedgerToSqlite(
  params: { filePath: string; archiveSource?: boolean } & OriroStateDatabaseOptions,
...
export function createSqliteAcpEventLedger(
  params: OriroStateDatabaseOptions & LedgerOptions = {},
): AcpEventLedger {
  ...
  const mutate = (fn: (db: DatabaseSync) => void) =>
    runOriroStateWriteTransaction((database) => fn(database.db), dbOptions);
  const read = <T>(fn: (db: DatabaseSync) => T): T => fn(openOriroStateDatabase(dbOptions).db);
