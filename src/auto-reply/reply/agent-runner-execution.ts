...
function buildMissingApiKeyFailureText(input: { message: string; error?: unknown }): string | null {
  ...
  if (provider === "openai") {
    return '⚠️ Missing API key for provider "openai". Run `oriro doctor --fix` to repair stale OpenAI model/session routes, restart the gateway if doctor asks, then try again. If doctor has nothing to repair or the error persists, re-auth with `oriro models auth login --provider openai` or run `oriro configure`.';
  }
        ...
        runWithModelFallback<EmbeddedAgentRunResult>({
          ...
          run: async (provider, model, runOptions) => {
            ...
            const embeddedRunHarnessOverride =
              ...
              (agentHarnessPolicy.runtime === "oriro" && embeddedRunProvider !== provider
                ? "oriro"
                : undefined);
            return (async () => {
