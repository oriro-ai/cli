---
summary: "Inworld streaming text-to-speech for Oriro replies"
read_when:
...
Inworld is a streaming text-to-speech (TTS) provider. In Oriro it
...
Oriro posts to Inworld's streaming TTS endpoint, concatenates the
...
```bash
oriro plugins install @oriro/inworld-speech
...
<Steps>
  ...
  <Step title="Send a message">
    Send a reply through any connected channel. Oriro synthesizes the
...
<AccordionGroup>
  ...
  <Accordion title="Audio outputs">
    ...
    Oriro asks Inworld for `OGG_OPUS` so the audio plays as a native
