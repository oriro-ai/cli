...
describe("skill_workshop tool", () => {
  it("is exposed in the Oriro tool set", async () => {
    const workspaceDir = await tempDirs.make("oriro-skill-workshop-tool-");
  ...
  it("is not exposed from sandboxed Oriro tool sets", async () => {
    const workspaceDir = await tempDirs.make("oriro-skill-workshop-tool-");
    const tools = createOriroTools({
      workspaceDir,
      config: {},
  ...
  it("scopes proposal discovery to the tool workspace", async () => {
    const firstWorkspaceDir = await tempDirs.make("oriro-skill-workshop-tool-first-");
    const secondWorkspaceDir = await tempDirs.make("oriro-skill-workshop-tool-second-");
