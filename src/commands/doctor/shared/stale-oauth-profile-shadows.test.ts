...
describe("stale OAuth profile shadow doctor repair", () => {
  const envSnapshot = captureEnv(["ORIRO_STATE_DIR", "ORIRO_HOME"]);
  let tempRoot = "";
  let stateDir = "";

  beforeEach(async () => {
    clearRuntimeAuthProfileStoreSnapshots();
    tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-stale-oauth-shadow-"));
    stateDir = path.join(tempRoot, "state");
    process.env.ORIRO_STATE_DIR = stateDir;
    process.env.ORIRO_HOME = stateDir;
  });
  ...
  it("warns about stale local OAuth shadows without modifying the child store", async () => {
    ...
    expect(warnings[0]).toContain("oriro doctor --fix");
  ...
  it("uses the injected env for the main auth store", async () => {
    ...
    const injectedEnv = {
      ...process.env,
      ORIRO_STATE_DIR: injectedStateDir,
      ORIRO_HOME: injectedStateDir,
    };
    saveAuthProfileStore(
      storeWith(
        profileId,
  ...
  it("removes stale child OAuth shadows and local cooldown state", async () => {
    ...
    const result = await repairStaleOAuthProfileShadows({
      cfg: { agents: { list: [{ id: "telegram" }] } } satisfies OriroConfig,
      now,
    });
