...
const logLevel = process.env.ORIRO_BUILD_VERBOSE ? "info" : "warn";
const INEFFECTIVE_DYNAMIC_IMPORT_MARKER = "[INEFFECTIVE_DYNAMIC_IMPORT]";
const UNRESOLVED_IMPORT_RE = /\[UNRESOLVED_IMPORT\]/;
const ANSI_ESCAPE_RE = new RegExp(String.raw`\u001B\[[0-9;]*m`, "g");
const DEPENDENCY_PATH_MARKERS = ["node_modules/", "oriro-pnpm-node-modules/"];
...
const PRESERVE_CLI_STARTUP_METADATA_ENV = "ORIRO_PRESERVE_CLI_STARTUP_METADATA";
...
export function resolveTsdownBuildInvocation(params = {}) {
  ...
  if (env.ORIRO_BUILD_ALL_NO_PNPM === "1") {
    return {
...
export async function runTsdownBuildInvocation(invocation, params = {}) {
  ...
  const timeoutMs = parsePositiveIntegerEnv(
    env.ORIRO_TSDOWN_TIMEOUT_MS,
    "ORIRO_TSDOWN_TIMEOUT_MS",
  );
  const heartbeatMs =
    parseNonNegativeIntegerEnv(env.ORIRO_TSDOWN_HEARTBEAT_MS, "ORIRO_TSDOWN_HEARTBEAT_MS") ??
