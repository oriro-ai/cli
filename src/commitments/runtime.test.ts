...
function requireFirstEmbeddedAgentRequest(): {
  ...
  if (!call) {
    throw new Error("expected embedded Oriro agent extraction request");
  }
  const [request] = call;
...
describe("commitment extraction runtime", () => {
  ...
  async function createConfig(): Promise<OriroConfig> {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-commitment-runtime-"));
    tmpDirs.push(tmpDir);
    stateDirEnvSnapshot ??= captureEnv(["ORIRO_STATE_DIR"]);
    setTestEnvValue("ORIRO_STATE_DIR", tmpDir);
    return {
      commitments: {
        enabled: true,
      },
