---
summary: "How Oriro remembers things across sessions"
...
Oriro remembers things by writing **plain Markdown files** in your agent's
...
These files live in the agent workspace (default `~/.oriro/workspace`).
...
If `MEMORY.md` grows past the bootstrap file budget, Oriro keeps the file on
...
`oriro doctor` to see raw vs injected sizes and truncation status.
...
Memory can preserve approval context, but it does not enforce policy. Use Oriro approval settings, sandboxing, and scheduled tasks for hard operational controls.
...
for that case. Oriro infers them in a hidden background pass, scopes them to
...
<Info>
Oriro uses OpenAI embeddings by default. Set
...
Before [compaction](/concepts/compaction) summarizes your conversation, Oriro
...
```bash
oriro memory rem-backfill --rollback
oriro memory rem-backfill --rollback-short-term
```

## CLI

```bash
oriro memory status          # Check index status and provider
oriro memory search "query"  # Search from the command line
oriro memory index --force   # Rebuild the index
```
