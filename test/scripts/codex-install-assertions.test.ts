...
const tmpFixtureFiles = ["/tmp/oriro-codex-inspect.json", "/tmp/oriro-plugins-list.json"];
...
function writeAuthProfileStoreSqlite(agentDir: string) {
  ...
  const db = new DatabaseSync(path.join(agentDir, "oriro-agent.sqlite"));
  try {
    db.exec(`
...
function runCodexOnDemandAssertions(root: string) {
  return spawnSync(process.execPath, [ASSERTIONS_SCRIPT], {
    ...
    env: {
      ...
      ORIRO_CONFIG_PATH: path.join(root, "state", "oriro.json"),
...
function createCodexInstallFixture(root: string) {
  ...
  const installPath = path.join(npmRoot, "projects", "codex", "node_modules", "@oriro", "codex");
  const projectRoot = npmProjectRootForInstalledPackage(installPath, "@oriro/codex");
  writeJson(path.join(installPath, "package.json"), { name: "@oriro/codex" });
  ...
  writeJson(path.join(stateDir, "oriro.json"), {
    ...
    plugins: {
      installs: {
        codex: {
          installPath,
          source: "npm",
          spec: "npm:@oriro/codex",
        },
      },
    },
  });
  writeJson("/tmp/oriro-codex-inspect.json", {
    plugin: { id: "codex", status: "loaded", agentHarnessIds: ["codex"] },
  });
  writeJson("/tmp/oriro-plugins-list.json", {
...
describe("Codex install helpers", () => {
  it("resolves package roots and package manifests inside managed npm installs", () => {
    const root = makeTempDir(tempDirs, "oriro-codex-install-utils-");
    ...
    const projectRoot = npmProjectRootForInstalledPackage(packageRoot, "@oriro/codex");
  ...
  it("accepts a complete on-demand Codex npm install fixture", () => {
    const root = makeTempDir(tempDirs, "oriro-codex-on-demand-");
  ...
  it("rejects on-demand fixtures missing the managed @openai/codex dependency", () => {
    const root = makeTempDir(tempDirs, "oriro-codex-on-demand-missing-");
    createCodexInstallFixture(root);
  ...
  it("rejects on-demand fixtures missing the managed Codex executable", () => {
    const root = makeTempDir(tempDirs, "oriro-codex-on-demand-missing-bin-");
    createCodexInstallFixture(root);
    rmSync(
      path.join(
        root,
