...
import { createTestWizardPrompter } from "oriro/plugin-sdk/plugin-test-runtime";
import { NON_ENV_SECRETREF_MARKER } from "oriro/plugin-sdk/provider-auth-runtime";
import { createNonExitingRuntime } from "oriro/plugin-sdk/runtime-env";
import { withEnv, withEnvAsync, withFetchPreconnect } from "oriro/plugin-sdk/test-env";
...
vi.mock("oriro/plugin-sdk/provider-auth", async (importOriginal) => {
  const original = await importOriginal<typeof import("oriro/plugin-sdk/provider-auth")>();
  return {
    ...original,
...
vi.mock("oriro/plugin-sdk/provider-auth-runtime", async (importOriginal) => {
  const original =
    await importOriginal<typeof import("oriro/plugin-sdk/provider-auth-runtime")>();
  return {
    ...original,
...
vi.mock("oriro/plugin-sdk/provider-web-search", async (importOriginal) => {
  const original = await importOriginal<typeof import("oriro/plugin-sdk/provider-web-search")>();
  return {
    ...original,
...
describe("xai web search config resolution", () => {
  ...
  it("treats unresolved non-env SecretRefs as missing credentials instead of using env fallback", async () => {
    await withEnvAsync({ XAI_API_KEY: "ambient-xai-test-key" }, async () => {
      ...
      const result = await maybeTool.execute({ query: "Oriro" });
  ...
  it("uses xAI OAuth auth before API-key fallback for web search", async () => {
    ...
    const tool = provider.createTool({
      config: {
        agents: {
          list: [{ id: "main", default: true, agentDir: "/tmp/oriro-xai-main-agent" }],
        },
        plugins: {
          entries: {
            xai: {
    ...
    await tool.execute({ query: "Oriro Grok OAuth web search" });

    expect(providerAuthRuntimeMocks.resolveApiKeyForProvider).toHaveBeenCalledWith(
      expect.objectContaining({
        provider: "xai",
        agentDir: "/tmp/oriro-xai-main-agent",
      }),
    );
  ...
  it("uses the active agentDir for xAI OAuth web search auth", async () => {
    ...
    const provider = createXaiWebSearchProvider();
    const tool = provider.createTool({
      agentDir: "/tmp/oriro-xai-active-agent",
      config: {
        agents: {
          list: [
            { id: "main", default: true, agentDir: "/tmp/oriro-xai-main-agent" },
            { id: "side", agentDir: "/tmp/oriro-xai-active-agent" },
          ],
        },
      },
    });
    ...
    await tool.execute({ query: "Oriro Grok active agent OAuth web search" });
  ...
  it("refreshes xAI OAuth auth and retries web search after a 401", async () => {
    ...
    const result = await tool.execute({ query: "Oriro Grok OAuth refresh test" });
  ...
  it("falls back to xAI API-key auth when OAuth refresh cannot recover", async () => {
    ...
    const result = await tool.execute({ query: "Oriro Grok API fallback test" });
  ...
  it("falls back to an xAI API-key auth profile when stale OAuth remains first", async () => {
    ...
    const result = await tool.execute({ query: "Oriro Grok profile fallback test" });
  ...
  it("falls back to env auth after a stale xAI API-key auth profile returns unauthorized", async () => {
    ...
    const result = await tool.execute({ query: "Oriro Grok API-key fallback test" });
  ...
  it("routes Grok web search through plugin webSearch.baseUrl", async () => {
    ...
    await tool.execute({ query: "Oriro Grok proxy test" });
  ...
  it("reports malformed xAI web search JSON as a provider error", async () => {
    ...
    await expect(tool.execute({ query: "Oriro" })).rejects.toThrow(
...
describe("xai provider models", () => {
  ...
  it("publishes the remaining Grok 3 family in the Oriro catalog", () => {
