...
read_when:
  - Looking for an overview of Oriro's media capabilities
...
Oriro generates images, videos, and music, understands inbound media
...
For async tools, Oriro submits the request to the provider, returns a task
...
Oriro wakes the agent with the generated media paths so it can tell the
...
Oriro sends an idempotent direct fallback with only the missing media. Media
...
<AccordionGroup>
  ...
  <Accordion title="DeepInfra">
    ...
    registered until Oriro has dedicated provider contracts for those
    categories.
  </Accordion>
  <Accordion title="xAI">
    ...
    not registered in Oriro until the shared realtime-voice contract can
    represent it.
  </Accordion>
</AccordionGroup>
