...
function buildAgentToolResultMiddlewareFactory(
  ...
  const runner = createAgentToolResultMiddlewareRunner({ runtime: "oriro" });
  return (agent) => {
    agent.on("tool_result", async (rawEvent: unknown, ctx: { cwd?: string }) => {
      ...
      const eventToolCallId =
        typeof event.toolCallId === "string" && event.toolCallId.trim()
          ? event.toolCallId
          : undefined;
      const toolCallId = eventToolCallId ?? `oriro-${randomUUID()}`;
...
export function buildEmbeddedExtensionFactories(params: {
  ...
  // ORIRO Guardian V3 — always-on security gate, registered FIRST so it intercepts
