...
import { formatPluginConfigIssue } from "oriro/plugin-sdk/extension-shared";
...
const ACPX_PLUGIN_TOOLS_MCP_SERVER_NAME = "oriro-plugin-tools";
const ACPX_ORIRO_TOOLS_MCP_SERVER_NAME = "oriro-tools";
const requireFromHere = createRequire(import.meta.url);

function isAcpxPluginRoot(dir: string): boolean {
  return (
    fs.existsSync(path.join(dir, "oriro.plugin.json")) &&
...
function resolveAcpxPluginRootFromOriroLayout(moduleUrl: string): string | null {
  let cursor = path.dirname(fileURLToPath(moduleUrl));
  for (let i = 0; i < 5; i += 1) {
    const candidates = [
...
export function resolveAcpxPluginRoot(moduleUrl: string = import.meta.url): string {
  ...
  return (
    ...
    // Scan common Oriro layouts before falling back to the nearest path guess.
    resolveAcpxPluginRootFromOriroLayout(moduleUrl) ??
    resolvedRoot
  );
}
...
function resolveOriroRoot(currentRoot: string): string {
  if (
    path.basename(currentRoot) === "acpx" &&
    path.basename(path.dirname(currentRoot)) === "extensions"
  ) {
