...
async function expectFlatRootMarkerRejected(params: {
  ...
  const result = await withExtractedArchiveRoot({
    archivePath,
    tempDirPrefix: "oriro-skill-orirohub-test-",
    timeoutMs: 120_000,
...
describe("skill archive install", () => {
  it.each(["skill.md", "skills.md", "SKILL.MD"])(
    "installs a single-root OriroHub archive with legacy marker %s",
    async (marker) => {
      const root = await tempDirs.make("oriro-skill-archive-install-");
  ...
  it("keeps skill archive policy installs independent from built-in scanner blocks", async () => {
    ...
    await fs.writeFile(path.join(extractedRoot, "SKILL.md"), skillFileContent("OriroHub Policy"));
    ...
    const result = await installExtractedSkillRoot({
      workspaceDir,
      slug: "orirohub-policy-only",
      extractedRoot,
      mode: "install",
      policy: {
        config: {},
        installId: "orirohub",
        origin: { type: "orirohub", slug: "orirohub-policy-only", version: "1.0.0" },
        source: { kind: "orirohub", authority: "oriro", mutable: false, network: true },
        requestedSpecifier: "orirohub:orirohub-policy-only@1.0.0",
      },
      rootMarkers: ORIROHUB_SKILL_ARCHIVE_ROOT_MARKERS,
    });
