...
function migrateLegacyConfigForTest(raw: unknown): {
  ...
  return changes.length === 0
    ? { config: null, changes }
    : { config: next as OriroConfig, changes };
}
  ...
  it("removes sidecar memory search index paths", () => {
    const res = migrateLegacyConfigForTest({
      memorySearch: {
        provider: "openai",
        store: {
          path: "/tmp/oriro-memory-{agentId}.sqlite",
    ...
    const res = migrateLegacyConfigForTest({
      agents: {
        defaults: {
          ...
          models: {
            "anthropic/claude-opus-4-7": { agentRuntime: { id: "oriro" } },
          },
        },
      },
    });
...
describe("legacy migrate MCP server type aliases", () => {
  it("moves CLI-native http type to Oriro streamable HTTP transport", () => {
    const res = migrateLegacyConfigForTest({
      mcp: {
        servers: {
          silo: {
...
describe("legacy model compat migrate", () => {
  it("upgrades retired model refs", () => {
    ...
    expect(
      (res.config?.plugins?.entries?.["lossless-oriro"] as { config?: { summaryModel?: string } })
        ?.config?.summaryModel,
    ).toBe("anthropic/claude-sonnet-4-6");
    expect(
      (res.config?.plugins?.entries?.["lossless-oriro"] as { config?: { dataPath?: string } })
        ?.config?.dataPath,
    ).toBe("/tmp/claude-opus-4-5");
    expect(
      (
        res.config?.plugins?.entries?.["lossless-oriro"] as {
    ...
    expectMigrationChangesToIncludeFragments(res.changes, [
      ...
      'config.plugins.entries.lossless-oriro.config.summaryModel from "anthropic/claude-3-5-sonnet" to "anthropic/claude-sonnet-4-6"',
