...
title: "Tokenjuice"
read_when:
  - You want shorter `exec` or `bash` tool results in Oriro
...
Today this applies to Oriro embedded runs and Oriro dynamic tools in the Codex
app-server harness. Tokenjuice hooks Oriro's tool-result middleware and
...
```bash
oriro plugins install orirohub:@oriro/tokenjuice
```
...
```bash
oriro config set plugins.entries.tokenjuice.enabled false
```

Or:

```bash
oriro plugins disable tokenjuice
```
