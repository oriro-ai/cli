...
declare const __ORIRO_VERSION__: string | undefined;
const CORE_PACKAGE_NAME = "oriro";
...
function readInjectedVersion(): string | undefined {
  return typeof __ORIRO_VERSION__ === "string" ? __ORIRO_VERSION__ : undefined;
}
...
function resolveVersionFromRuntimeSources(params: {
  ...
  const preferredCandidates =
    params.preference === "env-first"
      ? [params.env["ORIRO_VERSION"], params.runtimeVersion]
      : [params.runtimeVersion, params.env["ORIRO_VERSION"]];
  return (
    firstNonEmpty(
      ...preferredCandidates,
      params.env["ORIRO_SERVICE_VERSION"],
...
export function resolveCompatibilityHostVersion(
  ...
  const explicitCompatibilityVersion = firstNonEmpty(env.ORIRO_COMPATIBILITY_HOST_VERSION);
...
// Single source of truth for the current Oriro version.
// - Embedded/bundled builds: injected define or env var.
// - Dev/npm builds: package.json.
export const VERSION = resolveBinaryVersion({
  moduleUrl: import.meta.url,
  injectedVersion: readInjectedVersion(),
  bundledVersion: process.env.ORIRO_BUNDLED_VERSION,
});
