...
async function tryWriteSingleTopLevelIncludeMutation(params: {
  ...
  writeOptions?: ConfigWriteOptions;
  io?: ConfigMutationIO;
}): Promise<{ persistedHash: string | null; persistedConfig: OriroConfig } | null> {
...
function resolveConfigWriteResult(
  ...
  fallbackConfig: OriroConfig,
): { persistedHash: string | null; persistedConfig: OriroConfig } {
  if (result) {
    return {
...
export async function mutateConfigFile<T = void>(params: {
  ...
  writeOptions?: ConfigWriteOptions;
  io?: ConfigMutationIO;
  mutate: (draft: OriroConfig, context: ConfigMutationContext) => Promise<T | void> | T | void;
}): Promise<ConfigMutationResult<T>> {
