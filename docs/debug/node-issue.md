...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Oriro
...
Running Oriro via Node with `tsx` fails at startup with:

```
[oriro] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `oriro status` and `gateway:watch` worked.
