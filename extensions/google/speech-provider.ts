...
import { transcodeAudioBufferToOpus } from "oriro/plugin-sdk/media-runtime";
import {
...
import { asObject, trimToUndefined } from "oriro/plugin-sdk/speech-core";
...
function resolveGoogleTtsModelProviderApiKey(cfg?: OriroConfig): string | undefined {
...
function isOriroGoogleAudioProfilePrompt(text: string): boolean {
  return (
...
export function buildGoogleSpeechProvider(): SpeechProviderPlugin {
  return {
    ...
    prepareSynthesis: (ctx) => {
      ...
      if (!shouldWrap || isOriroGoogleAudioProfilePrompt(ctx.text)) {
        return undefined;
      }
      return {
