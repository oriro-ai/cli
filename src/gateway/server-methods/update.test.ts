...
import type { ConfigFileSnapshot, OriroConfig } from "../../config/types.oriro.js";
...
const startManagedServiceUpdateHandoffMock = vi.fn(async () => ({
  ...
  pid: 12345,
  command: "oriro update --yes --timeout 1800",
  logPath: "/tmp/oriro-update-run-handoff/handoff.log",
}));
...
vi.mock("../../infra/oriro-root.js", async () => {
  const actual = await vi.importActual<typeof import("../../infra/oriro-root.js")>(
    "../../infra/oriro-root.js",
  );
  return {
    ...actual,
    resolveOriroPackageRoot: async () => "/tmp/oriro",
  };
});
...
vi.mock("../../infra/update-managed-service-handoff.js", () => ({
  ...
  formatManagedServiceUpdateCommand: (params?: {
    ...
    params?.timeoutMs
      ? `oriro update --yes --timeout ${Math.ceil(params.timeoutMs / 1000)}`
      : "oriro update --yes",
  buildManagedServiceHandoffUnavailableMessage: (command: string) =>
    [
      "Oriro updates cannot safely run inside the live gateway process without a managed-service handoff.",
...
beforeEach(() => {
  ...
  readConfigFileSnapshotMock.mockResolvedValue({
    ...
    raw: "{}",
    parsed: {},
    resolved: {} as OriroConfig,
    sourceConfig: {} as OriroConfig,
    valid: true,
    config: {} as OriroConfig,
    runtimeConfig: {} as OriroConfig,
    issues: [],
    warnings: [],
    legacyIssues: [],
  });
...
function mockGlobalInstallSurface() {
  resolveUpdateInstallSurfaceMock.mockResolvedValueOnce({
    kind: "global",
    mode: "npm",
    root: "/tmp/oriro-global",
    packageRoot: "/tmp/oriro-global",
  });
}
...
describe("update.run restart scheduling", () => {
  ...
  it("hands managed package updates to the CLI path instead of running them in-process", async () => {
    ...
    const payload = await withProcessEnv({ ORIRO_LAUNCHD_LABEL: "ai.oriro.gateway" }, () =>
  ...
  it("keeps a startup grace before restarting after systemd handoff spawn", async () => {
    ...
    await withProcessEnv({ ORIRO_SYSTEMD_UNIT: "oriro-gateway.service" }, () =>
  ...
  it("starts managed package handoff when the gateway cwd is unavailable", async () => {
    ...
    try {
      await withProcessEnv({ ORIRO_LAUNCHD_LABEL: "ai.oriro.gateway" }, () =>
        invokeUpdateRun({}),
      );
    } finally {
      cwdSpy.mockRestore();
  ...
  it("hands supervised git/dev updates to the CLI path instead of rebuilding live dist in-process", async () => {
    detectRespawnSupervisorMock.mockReturnValueOnce("launchd");
    mockGitInstallSurface("/tmp/oriro-git");
    const payload = await withProcessEnv({ ORIRO_LAUNCHD_LABEL: "ai.oriro.gateway" }, () =>
      captureUpdateRunPayload(),
    );
  ...
  it("returns a safe command when package updates cannot be handed off", async () => {
    ...
    expect(payload?.handoff).toEqual({
      ...
      message:
        "Oriro updates cannot safely run inside the live gateway process without a managed-service handoff.\n" +
        "Run `oriro update --yes --timeout 1800` from a shell outside the gateway service, or restart/update from the host UI.",
    });
  });
...
describe("update.run post-core plugin finalize", () => {
  ...
  it("resumes official plugin convergence after a git/source core update", async () => {
    runPostCoreFinalizeAfterGatewayUpdateMock.mockResolvedValueOnce({
      status: "ok",
      entrypoint: "/tmp/oriro-git/dist/index.mjs",
    });
    mockGitOkUpdate("/tmp/oriro-git");
  ...
  it("carries the pre-doctor source config into the git finalizer", async () => {
    ...
    const [finalizeParams] = firstMockCall(
      runPostCoreFinalizeAfterGatewayUpdateMock,
      "post-core finalize",
    ) as [{ preUpdateConfig?: { sourceConfig?: OriroConfig; authoredConfig?: OriroConfig } }];
