---
summary: "Configure migrated native Codex plugins for Codex-mode Oriro agents"
title: "Native Codex plugins"
read_when:
  - You want Codex-mode Oriro agents to use native Codex plugins
  - You are migrating source-installed openai-curated Codex plugins
  - You are troubleshooting codexPlugins, app inventory, destructive actions, or plugin app diagnostics
---

Native Codex plugin support lets a Codex-mode Oriro agent use Codex
app-server's own app and plugin capabilities inside the same Codex thread that
handles the Oriro turn.

Oriro does not translate Codex plugins into synthetic `codex_plugin_*`
Oriro dynamic tools. Plugin calls stay in the native Codex transcript, and
...
- The selected Oriro agent runtime must be the native Codex harness.
...
`codexPlugins` has no effect on Oriro runs, normal OpenAI provider runs, ACP
...
```bash
oriro migrate codex --dry-run --verify-plugin-apps
```
...
`enable` and `disable` write only to Oriro config at
`~/.oriro-ai/cli.json`; they do not edit `~/.codex/config.toml` or install
...
- Enabled: Oriro config is willing to make the plugin available to Codex
  harness turns.
...
Migration is the durable install/eligibility step. During planning, Oriro
...
Oriro reads Codex app inventory through app-server `app/list`, caches it for
one hour, and refreshes stale or missing entries asynchronously. The cache is
in memory only; restarting the CLI or gateway drops it, and Oriro rebuilds it
...
A plugin app is exposed only when Oriro can map it back to the migrated
...
Oriro injects a restrictive `config.apps` patch for the Codex thread:
`_default` is disabled and only apps owned by enabled migrated plugins are
enabled.

Oriro sets app-level `destructive_enabled` from the effective global or
...
plugin apps are emitted with `open_world_enabled: true`; Oriro does not
...
- Per-plugin `allow_destructive_actions` overrides the global policy for that
  plugin.
- When policy is `false`, Oriro returns a deterministic decline.
- When policy is `true`, Oriro auto-accepts only safe schemas it can map to
  an approval response, such as a boolean approve field.
- When policy is `"auto"`, Oriro exposes destructive plugin actions to
  Codex but turns ownership-proven MCP approval elicitations into Oriro
...
empty or stale cache. Oriro schedules an async refresh and excludes plugin
...
Codex thread bindings keep the app config they started with until Oriro
