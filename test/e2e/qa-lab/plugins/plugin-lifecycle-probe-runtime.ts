...
type ProbeEnv = Pick<NodeJS.ProcessEnv, "HOME" | "ORIRO_CONFIG_PATH" | "ORIRO_STATE_DIR">;
...
function stateDir(env: ProbeEnv = process.env) {
  return env.ORIRO_STATE_DIR || path.join(env.HOME ?? os.homedir(), ".oriro");
}

function configPath(env: ProbeEnv = process.env) {
  return env.ORIRO_CONFIG_PATH || path.join(stateDir(env), "oriro.json");
}
...
function createMatrixStateEnv(resourceDir: string): MatrixEnv {
  ...
  const configFile = path.join(stateDir, "oriro.json");
  ...
  return {
    ...
    ORIRO_CONFIG_PATH: configFile,
    ORIRO_TEST_WORKSPACE_DIR: workspaceDir,
    ORIRO_AUTH_PROFILE_SECRET_KEY: randomBytes(32).toString("hex"),
  };
}
...
async function installOriroPackage(prefix: string, env: MatrixEnv) {
  ...
  await runCommand(
    ...
    ["install", "-g", "--prefix", prefix, packageTgz, "--no-fund", "--no-audit"],
    {
      ...
      timeoutMs: parseDurationMs(env.ORIRO_E2E_NPM_INSTALL_TIMEOUT, "600s"),
    },
  );
}
...
export async function runPluginLifecycleMatrix() {
  const pluginId = "lifecycle-oriro";
  const packageName = "@oriro/lifecycle-oriro";
  const resourceDir = tempDirs.make("oriro-plugin-lifecycle-matrix-");
  const npmPrefix = "/tmp/npm-prefix";
  const env = createMatrixStateEnv(resourceDir);
  const tarballV1 = path.join(resourceDir, "lifecycle-oriro-1.0.0.tgz");
  const tarballV2 = path.join(resourceDir, "lifecycle-oriro-2.0.0.tgz");
  ...
  try {
    await installOriroPackage(npmPrefix, env);
