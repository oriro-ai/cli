...
function runStopExistingLocalApp(params: { fakeLsof?: string; fakePgrep: string }) {
  const root = mkdtempSync(join(tmpdir(), "oriro-build-run-mac-test-"));
  tempRoots.push(root);
  ...
  writeFileSync(
    ...
    [
      "#!/usr/bin/env bash",
      "set -euo pipefail",
      'BIN_ABS="/worktree/apps/macos/.build-local/debug/Oriro"',
      'BIN=".build-local/debug/Oriro"',
      'APP_CWD="/worktree/apps/macos"',
      "kill() {",
      '  printf "%s\\n" "$*" >> "$ORIRO_TEST_KILL_CALLS"',
      '  touch "$ORIRO_TEST_KILLED_MARKER"',
      "  return 0",
      "}",
      stopFunction,
  ...
  const result = spawnSync("bash", [harnessPath], {
    ...
    env: {
      ...process.env,
      ORIRO_TEST_KILLED_MARKER: join(root, "killed"),
      ORIRO_TEST_KILL_CALLS: killCallsPath,
      ORIRO_TEST_PGREP_CALLS: pgrepCallsPath,
      ORIRO_TEST_PGREP_COUNT: join(root, "pgrep-count.txt"),
...
describe("scripts/build-and-run-mac.sh", () => {
  ...
  it("stops only the local debug app binary before relaunching", () => {
    ...
    const { killCalls, pgrepCalls, result } = runStopExistingLocalApp({
      fakePgrep: [
        "#!/usr/bin/env bash",
        `printf '%s\\n' "$*" >> "$ORIRO_TEST_PGREP_CALLS"`,
        'count="$(cat "$ORIRO_TEST_PGREP_COUNT" 2>/dev/null || echo 0)"',
        'next="$((count + 1))"',
        'printf "%s\\n" "$next" > "$ORIRO_TEST_PGREP_COUNT"',
        'if [[ "$2" == "/worktree/apps/macos/.build-local/debug/Oriro" ]]; then exit 1; fi',
        'if [[ "$2" == ".build-local/debug/Oriro" && "$count" == "1" ]]; then echo 321; exit 0; fi',
        "exit 1",
      ].join("\n"),
    });
    ...
    expect(pgrepCalls).toContain("-f /worktree/apps/macos/.build-local/debug/Oriro");
    expect(pgrepCalls).toContain("-f .build-local/debug/Oriro");
  ...
  it("fails when the scoped local app process survives cleanup", () => {
    const { result } = runStopExistingLocalApp({
      fakePgrep: [
        "#!/usr/bin/env bash",
        'if [[ "$2" == ".build-local/debug/Oriro" ]]; then echo 321; exit 0; fi',
        "exit 1",
      ].join("\n"),
    });
