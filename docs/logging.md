...
read_when:
  - You need a beginner-friendly overview of Oriro logging
  - You want to configure log levels, formats, or redaction
  - You are troubleshooting and need to find logs quickly
title: "Logging"
---

Oriro has two main log surfaces:
...
Oriro keeps up to five numbered archives beside the active file, such as
`oriro-YYYY-MM-DD.1.log`, and keeps writing to a fresh active log instead of
suppressing diagnostics.

You can override this in `~/.oriro-ai/cli.json`:

```json
{
  "logging": {
    "file": "/path/to/oriro.log"
  }
}
```
...
or times out before `logs.tail` answers, `oriro logs` falls back to the
configured Gateway file log automatically. Explicit `--url` targets do not use
this fallback. `oriro logs --follow` is stricter: on Linux it uses the active
...
```bash
oriro channels logs --channel whatsapp
```
...
Oriro preserves the original structured log arguments alongside these fields
...
`oriro gateway` also has WebSocket protocol logging for RPC traffic:
...
```bash
oriro gateway
oriro gateway --verbose --ws-log compact
oriro gateway --verbose --ws-log full
```

## Configuring logging

All logging configuration lives under `logging` in `~/.oriro-ai/cli.json`.

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/oriro-ai/cli-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
You can override both via the **`ORIRO_LOG_LEVEL`** environment variable (e.g. `ORIRO_LOG_LEVEL=debug`). The env var takes precedence over the config file, so you can raise verbosity for a single run without editing `oriro.json`. You can also pass the global CLI option **`--log-level <level>`** (for example, `oriro --log-level debug gateway run`), which overrides the environment variable for that command.
...
```bash
ORIRO_DEBUG_MODEL_TRANSPORT=1 oriro gateway
ORIRO_DEBUG_MODEL_PAYLOAD=tools ORIRO_DEBUG_SSE=events oriro gateway
```

Available flags:

- `ORIRO_DEBUG_MODEL_TRANSPORT=1`: emit request start, fetch response, SDK
  headers, first streaming event, stream completion, and transport errors at
  `info` level.
- `ORIRO_DEBUG_MODEL_PAYLOAD=summary`: include a bounded request payload
  summary in model request logs.
- `ORIRO_DEBUG_MODEL_PAYLOAD=tools`: include all model-facing tool names in
  the payload summary.
- `ORIRO_DEBUG_MODEL_PAYLOAD=full-redacted`: include a redacted, capped JSON
  payload snapshot. Use only while debugging; secrets are redacted but prompts
  and message text may still be present.
- `ORIRO_DEBUG_SSE=events`: emit first-event and stream-completion timing.
- `ORIRO_DEBUG_SSE=peek`: also emit the first five redacted SSE event
  payloads, capped per event.
- `ORIRO_DEBUG_CODE_MODE=1`: emit code-mode model-surface diagnostics,
  including when native provider tools are hidden because code mode owns the
  tool surface.

These flags log through normal Oriro logging, so `oriro logs --follow`
...
Oriro writes the trace fields as top-level JSON keys (`traceId`, `spanId`,
...
Oriro can redact sensitive tokens before they hit console output, file logs,
...
policy. Oriro still redacts safety-boundary payloads that can be shown to UI
...
- **Diagnostics flags** — targeted debug-log flags that route extra logs to
  ...
  or via the `ORIRO_DIAGNOSTICS=...` env override. Full guide:
...
- **Gateway not reachable?** Run `oriro doctor` first.
