...
vi.mock("../../agents/oriro-tools.runtime.js", () => ({
  createOriroTools: (...args: unknown[]) => createOriroToolsMock(...args),
}));
...
function mockToolDispatchedSkillCommand() {
  ...
  createOriroToolsMock.mockReturnValue([
    {
      name: "send_status",
      execute: toolExecute,
    },
...
describe("handleInlineActions", () => {
  beforeEach(() => {
    ...
    createOriroToolsMock.mockReset();
  ...
  it("passes requesterAgentIdOverride into inline tool runtimes", async () => {
    ...
    expect(result).toEqual({ kind: "reply", reply: { text: "✅ Done." } });
    expect(
      mockObjectArg(createOriroToolsMock, "createOriroTools").requesterAgentIdOverride,
    ).toBe("named-worker");
  ...
  it("passes sender identity into inline tool runtimes", async () => {
    ...
    expect(result).toEqual({ kind: "reply", reply: { text: "✅ Done." } });
    const toolsArgs = mockObjectArg(createOriroToolsMock, "createOriroTools");
  ...
  it("applies subagent policy to ACP envelope inline dispatch sessions", async () => {
    const tmpDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-inline-acp-policy-"));
    try {
  ...
  it("passes sandboxed runtime state into inline tool construction", async () => {
    ...
    expect(createOriroToolsMock).toHaveBeenCalledWith(
