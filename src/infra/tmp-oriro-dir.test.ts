// Covers preferred Oriro temp directory resolution.
import { constants as fsConstants } from "node:fs";
import path from "node:path";
import { describe, expect, it, vi } from "vitest";
import { POSIX_ORIRO_TMP_DIR, resolvePreferredOriroTmpDir } from "./tmp-oriro-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredOriroTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `oriro-${uid}`);
}
...
function resolveWithReadOnlyTmpFallback(params: {
  ...
  return resolvePreferredOriroTmpDir({
    accessSync: readOnlyTmpAccessSync(),
    lstatSync: vi.fn((target: string) => {
      if (target === POSIX_ORIRO_TMP_DIR) {
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredOriroTmpDir({
    accessSync,
    chmodSync,
...
describe("resolvePreferredOriroTmpDir", () => {
  it("prefers /tmp/oriro when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_ORIRO_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/oriro when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_ORIRO_TMP_DIR, {
      recursive: true,
      mode: 0o700,
    });
    expect(tmpdir).not.toHaveBeenCalled();
  ...
  it.each([
    {
      name: "falls back to os.tmpdir()/oriro when /tmp/oriro is not a directory",
      lstatSync: vi.fn(() => makeDirStat({ isDirectory: false, mode: 0o100644 })),
    },
    {
      name: "falls back to os.tmpdir()/oriro when /tmp is not writable",
      lstatSync: vi.fn(() => {
        throw nodeErrorWithCode("ENOENT");
      }),
    ...
    {
      name: "falls back when /tmp/oriro exists but is not writable",
    ...
    {
      name: "falls back when /tmp/oriro is a symlink",
      lstatSync: symlinkTmpDirLstat(),
    },
    {
      name: "falls back when /tmp/oriro is not owned by the current user",
      lstatSync: vi.fn(() => makeDirStat({ uid: 0 })),
    },
    {
      name: "falls back when /tmp/oriro is group/other writable",
  ...
  it("repairs existing /tmp/oriro permissions when they are too broad", () => {
    let preferredMode = 0o40777;
    const chmodSync = vi.fn((target: string, mode: number) => {
      if (target === POSIX_ORIRO_TMP_DIR && mode === 0o700) {
    ...
    expect(resolved).toBe(POSIX_ORIRO_TMP_DIR);
    expect(chmodSync).toHaveBeenCalledWith(POSIX_ORIRO_TMP_DIR, 0o700);
    expect(warn).toHaveBeenCalledWith(
      `[oriro] tightened permissions on temp dir: ${POSIX_ORIRO_TMP_DIR}`,
    );
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("repairs /tmp/oriro after create when the initial mode stays too broad", () => {
    ...
    const chmodSync = vi.fn((target: string, mode: number) => {
      chmodCalls += 1;
      if (target === POSIX_ORIRO_TMP_DIR && mode === 0o700 && chmodCalls > 1) {
        preferredMode = 0o40700;
      }
    });
    const warn = vi.fn();
  ...
  it("throws when fallback path is a symlink", () => {
    ...
    ).toThrow(/Unsafe fallback Oriro temp dir/);
  });
  ...
  it("uses an unscoped fallback suffix when process uid is unavailable", () => {
    ...
    const fallbackPath = path.join(tmpdirPath, "oriro");
  ...
  it("repairs existing fallback directory when permissions are too broad", () => {
    ...
    expect(chmodSync).toHaveBeenCalledWith(fallbackPath, 0o700);
    expect(warn).toHaveBeenCalledWith(
      `[oriro] tightened permissions on temp dir: ${fallbackPath}`,
    );
  });

  it("uses /tmp/oriro when another process tightened permissions before repair", () => {
  ...
  it("uses /tmp/oriro when chmod loses a concurrent repair race", () => {
    const chmodSync = vi.fn((target: string, mode: number) => {
      if (target === POSIX_ORIRO_TMP_DIR && mode === 0o700) {
  ...
  it("throws when the fallback directory cannot be created", () => {
    expect(() =>
      resolvePreferredOriroTmpDir({
        accessSync: readOnlyTmpAccessSync(),
        lstatSync: vi.fn((target: string) => {
          if (target === POSIX_ORIRO_TMP_DIR || target === fallbackTmp()) {
            throw nodeErrorWithCode("ENOENT");
          }
          return secureDirStat();
        }),
    ...
    ).toThrow(/Unable to create fallback Oriro temp dir/);
  });
  ...
  it("skips the POSIX preferred path on Windows even when /tmp is accessible (#60713)", () => {
    ...
    // create it), the previous code path returned `/tmp/oriro` and routed
    // log files / TTS temp files there instead of `%TEMP%\oriro`. The
    // platform: "win32" branch must skip the POSIX path entirely.
    const winFallback = path.win32.join("C:\\Users\\u\\AppData\\Local\\Temp", "oriro-501");
    const accessSync = vi.fn();
    const lstatSync = vi.fn((target: string) => {
      if (target === POSIX_ORIRO_TMP_DIR || target === winFallback) {
        return secureDirStat();
      }
      throw nodeErrorWithCode("ENOENT");
    });
    ...
    const result = resolvePreferredOriroTmpDir({
      platform: "win32",
      accessSync,
      lstatSync,
      mkdirSync,
    ...
    expect(result).not.toBe(POSIX_ORIRO_TMP_DIR);
    expect(tmpdir).toHaveBeenCalled();
  });
  ...
  it("still uses the POSIX preferred path on non-Windows platforms when available", () => {
    ...
    expect(result).toBe(POSIX_ORIRO_TMP_DIR);
  });
});
