...
final class MacNodeModeCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.oriro", category: "mac-node")
    ...
    private func run() async {
        ...
        while !Task.isCancelled {
            ...
            let browserControlEnabled = OriroConfigFile.browserControlEnabled()
                    ...
                    onInvoke: { [weak self] req in
                        guard let self else {
                            return BridgeInvokeResponse(
                                id: req.id,
                                ok: false,
                                error: OriroNodeError(code: .unavailable, message: "UNAVAILABLE: node not ready"))
                        }
    ...
    nonisolated static func resolvedCaps(
        ...
        cameraEnabled: Bool,
        locationMode: OriroLocationMode,
        connectionMode: AppState.ConnectionMode) -> [String]
    {
        ...
        if browserControlEnabled, connectionMode == .local {
            caps.append(OriroCapability.browser.rawValue)
        }
        if cameraEnabled {
            caps.append(OriroCapability.camera.rawValue)
        }
        if locationMode != .off {
            caps.append(OriroCapability.location.rawValue)
        }
        return caps
    }
    ...
    private func currentCaps() -> [String] {
        ...
        return Self.resolvedCaps(
            browserControlEnabled: OriroConfigFile.browserControlEnabled(),
            cameraEnabled: UserDefaults.standard.object(forKey: cameraEnabledKey) as? Bool ?? false,
            locationMode: OriroLocationMode(rawValue: rawLocationMode) ?? .off,
            connectionMode: AppStateStore.shared.connectionMode)
    }
    ...
    nonisolated static func resolvedCommands(caps: [String]) -> [String] {
        var commands: [String] = [
            ...
            OriroSystemCommand.which.rawValue,
            OriroSystemCommand.run.rawValue,
            OriroSystemCommand.execApprovalsGet.rawValue,
            OriroSystemCommand.execApprovalsSet.rawValue,
        ]

        let capsSet = Set(caps)
        if capsSet.contains(OriroCapability.browser.rawValue) {
            commands.append(OriroBrowserCommand.proxy.rawValue)
        }
        if capsSet.contains(OriroCapability.camera.rawValue) {
        ...
        if capsSet.contains(OriroCapability.location.rawValue) {
