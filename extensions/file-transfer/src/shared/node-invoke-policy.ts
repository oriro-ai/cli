...
import { readPositiveIntegerParam } from "oriro/plugin-sdk/param-readers";
import type {
  OriroPluginNodeInvokePolicy,
  OriroPluginNodeInvokePolicyContext,
  OriroPluginNodeInvokePolicyResult,
...
}): Promise<OriroPluginNodeInvokePolicyResult | null> {
  const nodeDisplayName = input.ctx.node?.displayName;
  const missingCode =
...
type PreflightResult =
  ...
  | {
      ok: false;
      result: OriroPluginNodeInvokePolicyResult;
    };
...
async function handleFileTransferInvoke(
  ctx: OriroPluginNodeInvokePolicyContext,
): Promise<OriroPluginNodeInvokePolicyResult> {
...
export function createFileTransferNodeInvokePolicy(): OriroPluginNodeInvokePolicy {
  return {
