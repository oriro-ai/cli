...
import type { OriroCodingToolConstructionPlan } from "../../agent-tools.js";
...
// Names here must be emitted directly by createOriroTools(). Catalog entries
...
const ORIRO_TOOL_FACTORY_NAMES = new Set([
  "agents_list",
  "canvas",
  "cron",
  "gateway",
...
const ALL_CODING_TOOL_CONSTRUCTION_PLAN: OriroCodingToolConstructionPlan = {
...
const NO_CODING_TOOL_CONSTRUCTION_PLAN: OriroCodingToolConstructionPlan = {
...
function cloneCodingToolConstructionPlan(
  plan: OriroCodingToolConstructionPlan,
): OriroCodingToolConstructionPlan {
  return { ...plan };
}
...
function isKnownLocalCodingToolName(normalized: string): boolean {
  ...
  return (
    ...
    ORIRO_TOOL_FACTORY_NAMES.has(normalized)
  );
}
...
function resolveCodingToolConstructionPlanForAllowlist(
  ...
  const includeOriroTools = normalized.some((name) => ORIRO_TOOL_FACTORY_NAMES.has(name));
...
export function resolveEmbeddedAttemptToolConstructionPlan(params: {
  ...
  codingToolConstructionPlan: OriroCodingToolConstructionPlan;
} {
  ...
  const includeCoreTools =
    ...
    codingToolConstructionPlan.includeOriroTools;
