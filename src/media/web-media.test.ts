...
const CANVAS_HOST_PATH = "/__oriro__/canvas";
...
beforeAll(async () => {
  ...
  fixtureRoot = await fs.mkdtemp(path.join(resolvePreferredOriroTmpDir(), "web-media-core-"));
...
describe("loadWebMedia", () => {
  ...
  it("resolves home-relative local media paths through allowed local roots", async () => {
    await withEnvAsync({ ORIRO_HOME: fixtureRoot }, async () => {
  ...
  it("allows trusted generated host-read HTML reports under Oriro temp root", async () => {
  ...
  it("rejects host-read HTML files outside the trusted Oriro temp root", async () => {
  ...
  it("rejects trusted host-read HTML symlinks that resolve outside Oriro temp root", async () => {
    const outsideRoot = await fs.mkdtemp(path.join(os.tmpdir(), "web-media-host-html-"));
  ...
  it("rejects trusted host-read HTML hardlinks to files outside Oriro temp root", async () => {
    const outsideRoot = await fs.mkdtemp(
      path.join(path.dirname(resolvePreferredOriroTmpDir()), "web-media-host-html-"),
    );
    const outsideHtml = path.join(outsideRoot, "report.html");
