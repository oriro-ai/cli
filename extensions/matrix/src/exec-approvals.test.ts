...
function createTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-matrix-exec-approvals-"));
  tempDirs.push(dir);
  return dir;
}

function buildConfig(
  execApprovals?: NonNullable<NonNullable<OriroConfig["channels"]>["matrix"]>["execApprovals"],
  channelOverrides?: Partial<NonNullable<NonNullable<OriroConfig["channels"]>["matrix"]>>,
): OriroConfig {
  return {
    channels: {
      matrix: {
