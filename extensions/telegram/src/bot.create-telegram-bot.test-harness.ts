...
import { buildChannelInboundEventContext } from "oriro/plugin-sdk/channel-inbound";
...
type GetRuntimeConfigFn =
  typeof import("oriro/plugin-sdk/runtime-config-snapshot").getRuntimeConfig;
type LoadSessionStoreFn =
  typeof import("oriro/plugin-sdk/session-store-runtime").loadSessionStore;
type ResolveStorePathFn =
  typeof import("oriro/plugin-sdk/session-store-runtime").resolveStorePath;
type ReadSessionUpdatedAtFn =
  typeof import("oriro/plugin-sdk/session-store-runtime").readSessionUpdatedAt;
...
const { sessionStorePath } = vi.hoisted(() => {
  ...
  return {
    sessionStorePath: `${tempRoot.replace(/[\\/]+$/u, "")}${separator}oriro-telegram-${
      process.pid
...
vi.mock("oriro/plugin-sdk/web-media", () => ({
  loadWebMedia,
}));
...
function resolveDefaultModelForAgentForTest(params: { cfg: OriroConfig }): {
  provider: string;
  model: string;
} {
...
function createModelsProviderDataFromConfig(cfg: OriroConfig): {
...
const DEFAULT_TELEGRAM_TEST_CONFIG: OriroConfig = {
  agents: {
    defaults: {
...
beforeEach(() => {
  ...
  buildModelsProviderData.mockImplementation(async (cfg: OriroConfig) => {
