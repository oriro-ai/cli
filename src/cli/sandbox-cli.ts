...
const SANDBOX_EXAMPLES = {
  main: [
    ["oriro sandbox list", "List all sandbox containers."],
    ["oriro sandbox list --browser", "List only browser containers."],
    ["oriro sandbox recreate --all", "Recreate all containers."],
    ["oriro sandbox recreate --session main", "Recreate a specific session."],
    ["oriro sandbox recreate --agent mybot", "Recreate agent containers."],
    ["oriro sandbox explain", "Explain effective sandbox config."],
  ],
  list: [
    ["oriro sandbox list", "List all sandbox containers."],
    ["oriro sandbox list --browser", "List only browser containers."],
  ...
  recreate: [
    ["oriro sandbox recreate --all", "Recreate all containers."],
    ["oriro sandbox recreate --session main", "Recreate a specific session."],
    ["oriro sandbox recreate --agent mybot", "Recreate a specific agent (includes sub-agents)."],
    ["oriro sandbox recreate --browser --all", "Recreate only browser containers."],
    ["oriro sandbox recreate --all --force", "Skip confirmation."],
  ],
  explain: [
    ["oriro sandbox explain", "Show effective sandbox config."],
    ["oriro sandbox explain --session agent:main:main", "Explain a specific session."],
    ["oriro sandbox explain --agent work", "Explain an agent sandbox."],
    ["oriro sandbox explain --json", "JSON output."],
  ],
} as const;
...
export function registerSandboxCli(program: Command) {
  const sandbox = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/sandbox", "docs.oriro.ai/cli/sandbox")}\n`,
    )
    .action(() => {
