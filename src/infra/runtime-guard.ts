// Validates the current runtime against Oriro's Node engine floor.
...
/** Returns whether a detected runtime meets Oriro's minimum runtime contract. */
...
/** Checks a Node version label against Oriro's current minimum Node version. */
...
export function assertSupportedRuntime(
  ...
  runtime.error(
    [
      "oriro requires Node >=22.19.0.",
      `Detected: ${runtimeLabel} (exec: ${execLabel}).`,
      `PATH searched: ${details.pathEnv}`,
      "Install Node: https://nodejs.org/en/download",
      "Upgrade Node and re-run oriro.",
    ].join("\n"),
  );
  runtime.exit(1);
}
