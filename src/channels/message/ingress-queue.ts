 ...
 *
 * Stores, claims, completes, and tombstones inbound channel events in Oriro state.
 */
...
import type {
  ...
  DB as OriroStateKyselyDatabase,
} from "../../state/oriro-state-db.generated.js";
import {
...
type ChannelIngressDatabase = Pick<OriroStateKyselyDatabase, "channel_ingress_events">;
...
function openStateDatabase(stateDir?: string) {
  return openOriroStateDatabase({
    env: stateDir ? { ...process.env, ORIRO_STATE_DIR: stateDir } : process.env,
  });
}
...
/** Creates a durable channel/account-scoped ingress queue backed by the Oriro state database. */
