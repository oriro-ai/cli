...
export function formatUnavailableSkillDoctorLines(skills: SkillStatusEntry[]): string[] {
  ...
  lines.push(`Disable unused skills: ${formatCliCommand("oriro doctor --fix")}`);
  lines.push(
    `Inspect details: ${formatCliCommand("oriro skills check --agent <id>")} or ${formatCliCommand("oriro skills info <name> --agent <id>")}`,
  );
  return lines;
}
