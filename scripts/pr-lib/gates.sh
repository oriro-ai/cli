...
prepare_gates() {
  ...
  if [ "${ORIRO_TESTBOX:-}" != "1" ] && [ "$docs_only" = "true" ] && [ -n "$previous_last_verified_head" ] && git merge-base --is-ancestor "$previous_last_verified_head" HEAD 2>/dev/null; then
  ...
  if [ "${ORIRO_TESTBOX:-}" = "1" ]; then
    ...
    if [ "$docs_only" = "true" ]; then
      ...
      gates_mode="full"
      if [ -n "${ORIRO_VITEST_MAX_WORKERS:-}" ]; then
        echo "Running pnpm test with ORIRO_VITEST_MAX_WORKERS=$ORIRO_VITEST_MAX_WORKERS."
        run_quiet_logged \
          "pnpm test" \
          ".local/gates-test.log" \
          env ORIRO_VITEST_MAX_WORKERS="$ORIRO_VITEST_MAX_WORKERS" pnpm test
      else
