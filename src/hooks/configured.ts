...
export function hasConfiguredInternalHooks(config: OriroConfig): boolean {
...
export function resolveConfiguredInternalHookNames(config: OriroConfig): Set<string> | null {
  const internal = config.hooks?.internal;
  if (!internal || internal.enabled === false) {
    return new Set();
  }
