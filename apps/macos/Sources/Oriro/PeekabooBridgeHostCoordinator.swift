...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.oriro", category: "PeekabooBridge")
    ...
    private static var oriroSocketPath: String {
        ...
        return Self.makeSocketPath(for: "Oriro", in: base)
    }
    ...
    private func startIfNeeded() async {
        ...
        let services = OriroPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.oriroSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.oriroSocketPath, privacy: .public)")
    }
        ...
        do {
            ...
            if linkValues?.isSymbolicLink == true {
                ...
                let destinationURL = URL(fileURLWithPath: destination, relativeTo: linkURL.deletingLastPathComponent())
                    .standardizedFileURL
                if destinationURL.path == URL(fileURLWithPath: Self.oriroSocketPath).standardizedFileURL.path {
                    return
                }
            ...
            try fileManager.createSymbolicLink(atPath: legacyPath, withDestinationPath: Self.oriroSocketPath)
        } catch {
...
@MainActor
private final class OriroPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.oriro.peekaboo")
