...
import { closeOriroAgentDatabasesForTest } from "../state/oriro-agent-db.js";
import {
  closeOriroStateDatabase,
  openOriroStateDatabase,
} from "../state/oriro-state-db.js";
import { withOriroTestState } from "../test-utils/oriro-test-state.js";
import {
...
function makeResult(overrides: Partial<BackupCreateResult> = {}): BackupCreateResult {
  return {
    ...
    archiveRoot: "oriro-backup-2026-01-01",
    archivePath: "/tmp/oriro-backup.tar.gz",
    dryRun: false,
...
describe("formatBackupCreateSummary", () => {
  const backupArchiveLine = "Backup archive: /tmp/oriro-backup.tar.gz";
  ...
  it.each([
    {
      ...
      expected: [
        ...
        "- state: ~/.oriro",
        "Skipped 1 path:",
        "- workspace: ~/Projects/oriro (covered by ~/.oriro)",
        "Created /tmp/oriro-backup.tar.gz",
        ...
        assets: [
          {
            ...
            displayPath: "~/.oriro/config.json",
          },
          {
            kind: "credentials",
            sourcePath: "/oauth",
            archivePath: "archive/oauth",
            displayPath: "~/.oriro/oauth",
          },
        ],
      }),
      expected: [
        backupArchiveLine,
        "Included 2 paths:",
        "- config: ~/.oriro/config.json",
        "- credentials: ~/.oriro/oauth",
  ...
  it("surfaces the volatile skip count in the summary", () => {
    expect(
      ...
      "Backup archive: /tmp/oriro-backup.tar.gz",
      "Included 1 path:",
      "- state: ~/.oriro",
      "Created /tmp/oriro-backup.tar.gz",
...
describe("buildExtensionsNodeModulesFilter", () => {
  it("excludes dependency trees only under state extensions", () => {
    ...
    expect(filter("/state/extensions/demo/oriro.plugin.json")).toBe(true);
  ...
  it("normalizes Windows path separators", () => {
    const filter = buildExtensionsNodeModulesFilter("C:\\Users\\me\\.oriro\\");

    expect(filter(String.raw`C:\Users\me\.oriro\extensions\demo\index.js`)).toBe(true);
    expect(
      filter(String.raw`C:\Users\me\.oriro\extensions\demo\node_modules\dep\index.js`),
    ).toBe(false);
  });
});
...
describe("createBackupArchive", () => {
  it("falls back when injected nowMs is outside Date range", async () => {
    await withOriroTestState(
      {
        layout: "state-only",
        prefix: "oriro-backup-invalid-now-",
        scenario: "minimal",
      },
      async (state) => {
        ...
        try {
          ...
          expect(path.basename(result.archivePath)).toContain("oriro-backup.tar.gz");
  ...
  it("falls back to epoch when injected nowMs and Date.now are outside Date range", async () => {
    await withOriroTestState(
      {
        layout: "state-only",
        prefix: "oriro-backup-invalid-fallback-now-",
        scenario: "minimal",
      },
      async (state) => {
        const outputDir = state.path("backups");
  ...
  it("scrubs transient SQLite delivery queue rows from archive snapshots", async () => {
    await withOriroTestState(
      {
        layout: "state-only",
        prefix: "oriro-backup-sqlite-queue-",
        scenario: "minimal",
      },
      async (state) => {
        const outputDir = state.path("backups");
        ...
        const { db } = openOriroStateDatabase({ env: state.env });
        db.prepare(
          `
        ...
        try {
          ...
          const archivedDbEntry = entries.find((entry) =>
            entry.endsWith("/state/state/oriro.sqlite"),
          );
          expect(archivedDbEntry).toBeDefined();
          expect(entries.some((entry) => entry.endsWith("/state/state/oriro.sqlite-wal"))).toBe(
            false,
          );
  ...
  it("snapshots per-agent SQLite auth stores without deleted secret pages", async () => {
    await withOriroTestState(
      {
        layout: "state-only",
        prefix: "oriro-backup-agent-sqlite-",
        scenario: "minimal",
      },
      async (state) => {
        const outputDir = state.path("backups");
        ...
        const sqlite = requireNodeSqlite();
        const liveDbPath = path.join(state.agentDir(), "oriro-agent.sqlite");
        const deletedSecretMarker = "ORIRO_DELETED_SECRET_PAGE_MARKER";
        ...
        const entries = await listArchiveEntries(result.archivePath);
        const archivedDbEntry = entries.find((entry) =>
          entry.endsWith("/state/agents/main/agent/oriro-agent.sqlite"),
        );
        expect(archivedDbEntry).toBeDefined();
        expect(
          entries.some((entry) =>
            entry.endsWith("/state/agents/main/agent/oriro-agent.sqlite-wal"),
          ),
        ).toBe(false);
  ...
  it("snapshots nested live SQLite databases with transaction continuity", async () => {
    await withOriroTestState(
      {
        layout: "state-only",
        prefix: "oriro-backup-nested-sqlite-",
        scenario: "minimal",
      },
      async (state) => {
        const outputDir = state.path("backups");
  ...
  it("fails instead of raw-copying malformed nested SQLite databases", async () => {
    await withOriroTestState(
      {
        layout: "state-only",
        prefix: "oriro-backup-malformed-sqlite-",
        scenario: "minimal",
      },
      async (state) => {
        const outputDir = state.path("backups");
  ...
  it.each(["late.sqlite", "late.sqlite-wal"])(
    ...
    async (lateName) => {
      await withOriroTestState(
        {
          layout: "state-only",
          prefix: "oriro-backup-late-sqlite-",
          scenario: "minimal",
        },
        async (state) => {
          const outputDir = state.path("backups");
  ...
  it("omits pre-existing orphan SQLite sidecars without failing backup", async () => {
    await withOriroTestState(
      {
        layout: "state-only",
        prefix: "oriro-backup-orphan-sqlite-sidecars-",
        scenario: "minimal",
      },
      async (state) => {
        const outputDir = state.path("backups");
  ...
  it("omits transient memory reindex databases and sidecars", async () => {
    await withOriroTestState(
      {
        layout: "state-only",
        prefix: "oriro-backup-memory-reindex-lock-",
        scenario: "minimal",
      },
      async (state) => {
        const outputDir = state.path("backups");
  ...
  it("preserves noncanonical symlinked SQLite paths without dereferencing them", async () => {
    ...
    await withOriroTestState(
      {
        layout: "state-only",
        prefix: "oriro-backup-symlinked-sqlite-",
        scenario: "minimal",
      },
      async (state) => {
        const outputDir = state.path("backups");
  ...
  it("snapshots the canonical global SQLite symlink as a complete regular file", async () => {
    ...
    await withOriroTestState(
      {
        layout: "state-only",
        prefix: "oriro-backup-global-sqlite-symlink-",
        scenario: "minimal",
      },
      async (state) => {
        const outputDir = state.path("backups");
        ...
        const linkedDbPath = state.statePath("state", "oriro.sqlite");
        await state.writeConfig({
          agents: {
            list: [{ id: "main", default: true, workspace: state.workspaceDir }],
          },
        ...
        try {
          ...
          const entries = await listArchiveEntryDetails(result.archivePath);
          const archivedDbEntries = entries.filter((entry) =>
            entry.path.endsWith("/state/state/oriro.sqlite"),
          );
  ...
  it("fails when the canonical global SQLite path is not a file", async () => {
    await withOriroTestState(
      {
        layout: "state-only",
        prefix: "oriro-backup-global-sqlite-directory-",
        scenario: "minimal",
      },
      async (state) => {
        const outputDir = state.path("backups");
        const globalDbPath = state.statePath("state", "oriro.sqlite");
  ...
  it("omits installed plugin node_modules from the real archive while keeping plugin files", async () => {
    await withOriroTestState(
      {
        layout: "state-only",
        prefix: "oriro-backup-plugin-deps-",
        scenario: "minimal",
      },
      async (state) => {
        ...
        await fs.writeFile(
          path.join(stateDir, "extensions", "demo", "oriro.plugin.json"),
          '{"id":"demo"}\n',
          "utf8",
        );
        await fs.writeFile(
        ...
        expect(entrySuffixes).toContain("/state/extensions/demo/oriro.plugin.json");
  ...
  it("dereferences hardlinks instead of emitting restore-hostile Link entries", async () => {
    await withOriroTestState(
      {
        layout: "state-only",
        prefix: "oriro-backup-hardlink-",
        scenario: "minimal",
      },
      async (state) => {
        const stateDir = state.stateDir;
        const outputDir = state.path("backups");
        const sourcePath = path.join(stateDir, "workspace-adx", "oriro-src", "node_modules");
  ...
  it("does not duplicate the root manifest when the system tempdir lives inside the state dir", async () => {
    await withOriroTestState(
      {
        layout: "state-only",
        prefix: "oriro-backup-tmp-overlap-",
        scenario: "minimal",
      },
      async (state) => {
        const stateDir = state.stateDir;
  ...
  it("does not duplicate the root manifest when the system tempdir is the state dir itself", async () => {
    await withOriroTestState(
      {
        layout: "state-only",
        prefix: "oriro-backup-tmp-equals-state-",
        scenario: "minimal",
      },
      async (state) => {
        const outputDir = state.path("backups");
        ...
        try {
          ...
          expect(entries.some((entry) => entry.includes("/oriro-state-db-"))).toBe(false);
