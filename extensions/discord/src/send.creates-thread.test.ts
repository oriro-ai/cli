...
vi.mock("oriro/plugin-sdk/web-media", async () => {
...
afterAll(() => {
  vi.doUnmock("oriro/plugin-sdk/web-media");
});
...
describe("uploadStickerDiscord", () => {
  ...
  it("uploads sticker assets", async () => {
    ...
    expect(stickerBody.name).toBe("oriro_wave");
    expect(stickerBody.description).toBe("Oriro waving");
