...
Oriro has three public release lanes:
...
- Every stable Oriro release ships the npm package, macOS app, and signed
...
6. Run `Oriro NPM Release` with `preflight_only=true`. Before a tag exists,
...
9. For a tagged beta candidate, run
   ...
   proof, records plugin npm and OriroHub plans, and prints the exact
   `Oriro Release Publish` command only after the evidence bundle is green.
   `Oriro Release Publish` dispatches the selected or all-publishable plugin
   packages to npm and the same set to OriroHub in parallel, and then promotes the
   prepared Oriro npm preflight artifact with the matching dist-tag as soon as
   plugin npm publish succeeds.
   After the Oriro npm publish child succeeds, it creates or updates the
   ...
   evidence as soon as Oriro npm publish succeeds, waits for OriroHub whenever
   Oriro npm is being published, then runs `pnpm release:verify-beta` and
   uploads postpublish evidence for the GitHub release, npm package, selected
   plugin npm packages, selected OriroHub packages, child workflow run IDs, and
   optional NPM Telegram run ID. The OriroHub path retries transient CLI
   ...
   `oriro@YYYY.M.PATCH-beta.N` or
   `oriro@beta` package. If a pushed or published prerelease needs a fix,
...
10. For stable, continue only after the vetted beta or release candidate has the
    ...
    `Oriro Release Publish`, reusing the successful preflight artifact via
    ...
    readiness requires the signed `OriroCompanion-Setup-x64.exe`,
    `OriroCompanion-Setup-arm64.exe`, and
    `OriroCompanion-SHA256SUMS.txt` assets on the Oriro GitHub release.
    Pass the exact signed `oriro-ai/cli-windows-node` release tag as
    `windows_node_tag` and its candidate-approved installer digest map as
    `windows_node_installer_digests`; `Oriro Release Publish` keeps the
...
5. Run `pnpm release:generated:check`, `pnpm deps:shrinkwrap:check`, and
   `ORIRO_TESTBOX=1 pnpm check:changed`. Push, then verify `origin/main`
...
6. Keep the repository variables `RELEASE_ROLLBACK_DRILL_ID` and
   ...
   `Oriro Stable Main Closeout` starts from the `main` push that carries the
...
- Plugin version sync updates official plugin package versions and existing
  `oriro.compat.pluginApi` floors to the Oriro release version by
  ...
  compatible with older Oriro hosts, keep the floor at the oldest supported
...
- Run the manual `Full Release Validation` workflow before release approval to
  ...
  `Oriro Release Checks` for install smoke, package acceptance, cross-OS
...
- Run the manual `Package Acceptance` workflow when you want side-channel proof
  ...
  `oriro@beta`, `oriro@latest`, or an exact release version; `source=ref`
  ...
  Example: `gh workflow run package-acceptance.yml --ref main -f workflow_ref=main -f source=npm -f package_spec=oriro@beta -f suite_profile=product -f published_upgrade_survivor_baseline=oriro@2026.4.26 -f telegram_mode=mock-openai`
  Common profiles:
  - `smoke`: install/channel/agent, gateway network, and config reload lanes
  - `package`: artifact-native package/update/restart/plugin lanes without OpenWebUI or live OriroHub
...
- `Oriro NPM Release` preflight generates dependency release evidence before
...
- The preflight uploads dependency evidence as
  `oriro-release-dependency-evidence-<tag>` and also embeds it under
  ...
  to the GitHub release as `oriro-<version>-dependency-evidence.zip`.
- Run `Oriro Release Publish` for the mutating publish sequence after the
  tag exists. Dispatch it from `release/YYYY.M.PATCH` (or `main` when publishing a
  main-reachable tag), pass the release tag, successful Oriro npm
  ...
  OriroHub publish, and Oriro npm publish so the core package is not published
  before its externalized plugins.
- Stable `Oriro Release Publish` requires an exact `windows_node_tag` after
  the matching non-prerelease `oriro-ai/cli-windows-node` release exists.
  ...
  while the Oriro release is still a draft, carrying the pinned installer
  ...
  signatures use the expected Oriro Foundation signer on a Windows runner,
  writes a SHA-256 manifest, and uploads the installers plus manifest onto the
  canonical Oriro GitHub release, then re-downloads the promoted assets and
  ...
  rejects unexpected `OriroCompanion-*` asset names before replacing the
  ...
  approved source release. Website download links should target exact Oriro
...
- Release checks now run in a separate manual workflow:
  `Oriro Release Checks`
- `Oriro Release Checks` also runs the QA Lab mock parity lane plus the fast
...
- Cross-OS install and upgrade runtime validation is part of public
  `Oriro Release Checks` and `Full Release Validation`, which call the
  reusable workflow
  `.github/workflows/oriro-cross-os-release-checks-reusable.yml` directly
...
- `Oriro Release Checks` accepts a branch, tag, or full commit SHA as long
  as the resolved commit is reachable from an Oriro branch or release tag
- `Oriro NPM Release` validation-only preflight also accepts the current
...
- That workflow runs
  `ORIRO_LIVE_TEST=1 ORIRO_LIVE_CACHE_TEST=1 pnpm test:live:cache`
...
- Before tagging a release candidate locally, run
  ...
  runs the fast release guardrails, plugin npm/OriroHub release checks, build,
  UI build, and `release:oriro:npm:check` in the order that catches common
  approval-blocking mistakes before the GitHub publish workflow starts.
- Run `RELEASE_TAG=vYYYY.M.PATCH node --import tsx scripts/oriro-npm-release-check.ts`
  (or the matching beta/correction tag) before approval
- After npm publish, run
  `node --import tsx scripts/oriro-npm-postpublish-verify.ts YYYY.M.PATCH`
  (or the matching beta/correction version) to verify the published registry
  install path in a fresh temp prefix
- After a beta publish, run `ORIRO_NPM_TELEGRAM_PACKAGE_SPEC=oriro@YYYY.M.PATCH-beta.N ORIRO_NPM_TELEGRAM_CREDENTIAL_SOURCE=convex ORIRO_NPM_TELEGRAM_CREDENTIAL_ROLE=ci pnpm test:docker:npm-telegram-live`
  ...
  `ORIRO_QA_TELEGRAM_*` env credentials directly.
...
- Maintainer release automation now uses preflight-then-promote:
  ...
  - token-based npm dist-tag mutation now lives in
    `oriro/releases/.github/workflows/oriro-npm-dist-tags.yml` because
...
gh workflow run full-release-validation.yml \
  ...
  -f evidence_package_spec=oriro@YYYY.M.PATCH-beta.N
```

The workflow resolves the target ref, dispatches manual `CI` with
`target_ref=<release-ref>`, dispatches `Oriro Release Checks`, prepares a
...
`npm_telegram_package_spec` is set. `Oriro Release
...
`Oriro Release Checks` uses the trusted workflow ref to resolve the target
...
After a beta is already on npm, set `release_package_spec=oriro@YYYY.M.PATCH-beta.N`
...
The cross-OS OpenAI install smoke uses `ORIRO_CROSS_OS_OPENAI_MODEL` when the
...
gh workflow run full-release-validation.yml \
  ...
  -f release_package_spec=oriro@YYYY.M.PATCH-beta.N \
  -f evidence_package_spec=oriro@YYYY.M.PATCH-beta.N \
...
validation, including required Oriro dynamic tool drift in the standard tier.
Tideoriro alpha runs may still treat non-package-safety release-check lanes as
advisory. When `live_suite_filter` explicitly requests a gated QA live lane such
as Discord, WhatsApp, or Slack, the matching
`ORIRO_RELEASE_QA_*_LIVE_CI_ENABLED` repo variable must be enabled; otherwise
...
The Docker box lives in `Oriro Release Checks` through
`oriro-live-and-e2e-checks-reusable.yml`, plus the release-mode
...
The QA Lab box is also part of `Oriro Release Checks`. It is the agentic
...
`scripts/resolve-oriro-package-candidate.mjs`. The resolver normalizes a
...
- `source=npm`: `oriro@beta`, `oriro@latest`, or an exact Oriro release
  version
...
`Oriro Release Checks` runs Package Acceptance with `source=artifact`, the
...
configured-auth update restart, live OriroHub skill install, stale plugin dependency cleanup, offline plugin
...
gh workflow run package-acceptance.yml \
  ...
  -f published_upgrade_survivor_baseline=oriro@2026.4.26
```
...
- `smoke`: quick package install/channel/agent, gateway network, and config
  reload lanes
- `package`: install/update/restart/plugin package contracts plus live OriroHub
...
`Oriro Release Publish` is the normal mutating publish entrypoint. It
...
4. Dispatch `Plugin NPM Release` with `publish_scope=all-publishable` and
   `ref=<release-sha>`.
5. Dispatch `Plugin OriroHub Release` with the same scope and SHA.
6. Dispatch `Oriro NPM Release` with the release tag, npm dist-tag, and
...
gh workflow run oriro-release-publish.yml \
  ...
  -f windows_node_installer_digests='{"OriroCompanion-Setup-x64.exe":"sha256:<approved-x64-sha256>","OriroCompanion-Setup-arm64.exe":"sha256:<approved-arm64-sha256>"}' \
  -f preflight_run_id=<successful-oriro-npm-preflight-run-id> \
  -f full_release_validation_run_id=<successful-full-release-validation-run-id> \
  -f npm_dist_tag=beta
```
...
Use the lower-level `Plugin NPM Release` and `Plugin OriroHub Release` workflows
only for focused repair or republish work. `Oriro Release Publish` rejects
`plugin_publish_scope=selected` when `publish_oriro_npm=true` so the core
package cannot ship without every publishable official plugin, including
`@oriro/diffs-language-pack`. For a selected plugin repair, set
`publish_oriro_npm=false` with `plugin_publish_scope=selected` and
`plugins=@oriro/name`, or dispatch the child workflow directly.

## NPM workflow inputs

`Oriro NPM Release` accepts these operator-controlled inputs:
...
`Oriro Release Publish` accepts these operator-controlled inputs:

- `tag`: required release tag; must already exist
- `preflight_run_id`: successful `Oriro NPM Release` preflight run id;
  required when `publish_oriro_npm=true`
- `full_release_validation_run_id`: successful `Full Release Validation` run
  id; required when `publish_oriro_npm=true`
- `windows_node_tag`: exact non-prerelease `oriro-ai/cli-windows-node`
  release tag; required for stable Oriro publish
- `windows_node_installer_digests`: candidate-approved compact JSON map of the
  ...
  for stable Oriro publish
- `npm_dist_tag`: npm target tag for the Oriro package
- `plugin_publish_scope`: defaults to `all-publishable`; use `selected` only
  for focused plugin-only repair work with `publish_oriro_npm=false`
- `plugins`: comma-separated `@oriro/*` package names when
  `plugin_publish_scope=selected`
- `publish_oriro_npm`: defaults to `true`; set `false` only when using the
  workflow as a plugin-only repair orchestrator
- `wait_for_orirohub`: defaults to `false` so npm availability is not blocked by
  the OriroHub sidecar; set `true` only when workflow completion must include
  OriroHub completion

`Oriro Release Checks` accepts these operator-controlled inputs:

- `ref`: branch, tag, or full commit SHA to validate. Secret-bearing checks
  require the resolved commit to be reachable from an Oriro branch or
  release tag.
...
- For `Oriro NPM Release`, full commit SHA input is allowed only when
  `preflight_only=true`
- `Oriro Release Checks` and `Full Release Validation` are always
  validation-only
...
1. Run `Oriro NPM Release` with `preflight_only=true`
...
5. Select the exact non-prerelease `oriro-ai/cli-windows-node` release tag
...
7. Run `Oriro Release Publish` with the same `tag`, the same `npm_dist_tag`,
   ...
   it publishes externalized plugins to npm and OriroHub before promoting the
   Oriro npm package
8. If the release landed on `beta`, use the
   `oriro/releases/.github/workflows/oriro-npm-dist-tags.yml`
...
- [`.github/workflows/full-release-validation.yml`](https://github.com/oriro-ai/cli/blob/main/.github/workflows/full-release-validation.yml)
- [`.github/workflows/package-acceptance.yml`](https://github.com/oriro-ai/cli/blob/main/.github/workflows/package-acceptance.yml)
- [`.github/workflows/oriro-npm-release.yml`](https://github.com/oriro-ai/cli/blob/main/.github/workflows/oriro-npm-release.yml)
- [`.github/workflows/oriro-release-checks.yml`](https://github.com/oriro-ai/cli/blob/main/.github/workflows/oriro-release-checks.yml)
- [`.github/workflows/oriro-cross-os-release-checks-reusable.yml`](https://github.com/oriro-ai/cli/blob/main/.github/workflows/oriro-cross-os-release-checks-reusable.yml)
- [`scripts/resolve-oriro-package-candidate.mjs`](https://github.com/oriro-ai/cli/blob/main/scripts/resolve-oriro-package-candidate.mjs)
- [`scripts/oriro-npm-release-check.ts`](https://github.com/oriro-ai/cli/blob/main/scripts/oriro-npm-release-check.ts)
- [`scripts/package-mac-dist.sh`](https://github.com/oriro-ai/cli/blob/main/scripts/package-mac-dist.sh)
- [`scripts/make_appcast.sh`](https://github.com/oriro-ai/cli/blob/main/scripts/make_appcast.sh)

Maintainers use the private release docs in
[`oriro/maintainers/release/README.md`](https://github.com/oriro/maintainers/blob/main/release/README.md)
