...
Oriro includes syntax highlighting for common source, config, and documentation languages:
  ...
  plugins: {
    entries: {
      diffs: {
        enabled: true,
        config: {
          viewerBaseUrl: "https://gateway.example.com/oriro",
        },
      },
    },
  },
...
- Artifacts are stored under the temp subfolder: `$TMPDIR/oriro-diffs`.
...
<Steps>
  <Step title="Config">
    `browser.executablePath` in Oriro config.
  </Step>
