...
struct OriroMacCLI {
    static func main() async {
        ...
        case let .unknown(exitCode):
            fputs("oriro-mac: unknown command\n", stderr)
            printUsage()
            exit(exitCode)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      oriro-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
      ...
      oriro-mac discover [--timeout <ms>] [--json] [--include-local]
      oriro-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      oriro-mac connect
      oriro-mac configure-remote --ssh-target user@gateway.local --remote-port 18789
      oriro-mac connect --url ws://127.0.0.1:18789 --json
      oriro-mac discover --timeout 3000 --json
      oriro-mac wizard --mode local
    """)
}
