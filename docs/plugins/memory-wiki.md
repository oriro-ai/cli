...
If the active memory plugin exposes shared recall artifacts, Oriro can search
...
currently exposing public bridge inputs yet. Run `oriro wiki doctor` first,
...
`oriro wiki status`, `oriro wiki doctor`, and `oriro wiki bridge
...
artifact, then let `memory-wiki` turn it into Oriro-native concept pages and
compiled digests.
...
oriro wiki apply synthesis "Alpha Summary" --body "..." --source-id source.alpha
