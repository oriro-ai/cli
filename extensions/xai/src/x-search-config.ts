...
function resolveLegacyXSearchConfig(config?: OriroConfig): JsonRecord | undefined {
...
function resolvePluginXSearchConfig(config?: OriroConfig): JsonRecord | undefined {
...
function resolveLegacyGrokWebSearchConfig(config?: OriroConfig): JsonRecord | undefined {
  const web = config?.tools?.web as Record<string, unknown> | undefined;
...
function resolvePluginWebSearchConfig(config?: OriroConfig): JsonRecord | undefined {
  const pluginConfig = config?.plugins?.entries?.xai?.config;
...
export function resolveEffectiveXSearchConfig(config?: OriroConfig): JsonRecord | undefined {
