...
struct MacNodeModeCoordinatorTests {
    @Test func `remote mode does not advertise browser proxy`() {
        ...
        #expect(!caps.contains(OriroCapability.browser.rawValue))
        #expect(!commands.contains(OriroBrowserCommand.proxy.rawValue))
        #expect(commands.contains(OriroCanvasCommand.present.rawValue))
