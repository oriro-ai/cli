...
import { resolveChannelStreamingBlockEnabled } from "oriro/plugin-sdk/channel-outbound";
...
import type { ChannelGroupPolicy } from "oriro/plugin-sdk/config-contracts";
import type {
  ReplyToMode,
...
import { resolveMarkdownTableMode } from "oriro/plugin-sdk/markdown-table-runtime";
import { resolveSendableOutboundReplyParts } from "oriro/plugin-sdk/reply-payload";
import { resolveAgentRoute } from "oriro/plugin-sdk/routing";
import { getRuntimeConfigSnapshot } from "oriro/plugin-sdk/runtime-config-snapshot";
import { danger, logVerbose } from "oriro/plugin-sdk/runtime-env";
...
type TelegramChunkMode = ReturnType<
  typeof import("oriro/plugin-sdk/reply-dispatch-runtime").resolveChunkMode
>;
type TelegramNativeReplyPayload = import("oriro/plugin-sdk/reply-dispatch-runtime").ReplyPayload;
...
export const registerTelegramNativeCommands = ({
  ...
  const loadFreshRuntimeConfig = (): OriroConfig => telegramDeps.getRuntimeConfig();
  const resolveFreshTelegramConfig = (runtimeCfg: OriroConfig): TelegramAccountConfig => {
    try {
