...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/oriro-status-corrupt-plugin-deps.XXXXXX")"
cleanup() {
  rm -rf "$TMP_DIR"
}
trap cleanup EXIT
...
const stageDir = process.env.ORIRO_PLUGIN_STAGE_DIR || "";
const renameResidue = path.join(stageDir, "node_modules", "ansi-escapes", ".oriro-rename-tmp");
...
run_oriro() {
  HOME="$HOME_DIR" \
  ORIRO_HOME="$STATE_DIR" \
  ORIRO_STATE_DIR="$STATE_DIR" \
  ORIRO_CONFIG_PATH="$CONFIG_PATH" \
  ORIRO_PLUGIN_STAGE_DIR="$STAGE_DIR" \
...
run_oriro status --all --timeout 1 > "$BEFORE"
grep -F "e2e-corrupt-chat" "$BEFORE" >/dev/null
grep -F "plugin load failed: dependency tree corrupted; run oriro doctor --fix" "$BEFORE" >/dev/null

run_oriro doctor --fix --non-interactive --yes > "$DOCTOR"
if [[ -e "$STAGE_DIR" ]]; then
  echo "doctor --fix did not remove corrupt plugin stage dir: $STAGE_DIR" >&2
  exit 1
fi

run_oriro status --all --timeout 1 > "$AFTER"
