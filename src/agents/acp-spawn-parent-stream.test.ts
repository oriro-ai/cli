...
function progressModeConfig(acp?: OriroConfig["acp"]): OriroConfig {
  return {
    ...(acp ? { acp } : {}),
    channels: {
      forum: {
...
describe("startAcpSpawnParentStreamRelay", () => {
  ...
  it("resolves ACP spawn stream log path from session metadata", () => {
    readAcpSessionEntryMock.mockReturnValue({
      storePath: "/tmp/oriro/agents/codex/sessions/sessions.json",
      entry: {
        sessionId: "sess-123",
        sessionFile: "/tmp/oriro/agents/codex/sessions/sess-123.jsonl",
      },
    });
    resolveSessionFilePathMock.mockReturnValue(
      "/tmp/oriro/agents/codex/sessions/sess-123.jsonl",
    );
    ...
    expect(resolved).toBe("/tmp/oriro/agents/codex/sessions/sess-123.acp-stream.jsonl");
    ...
    expect(options.storePath).toBe("/tmp/oriro/agents/codex/sessions/sessions.json");
  });
});
