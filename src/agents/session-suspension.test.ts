...
vi.mock("./command/session.js", () => ({
  resolveStoredSessionKeyForSessionId: () => ({
    ...
    storePath: "/tmp/oriro-session-suspension-test/sessions.json",
  }),
}));

async function suspendLane(ttlMs: number, cfg: OriroConfig, laneId: CommandLane) {
...
describe("session suspension", () => {
  ...
  it("auto-resumes cron lanes to the cron concurrency default", async () => {
    ...
    await suspendLane(100, {} as OriroConfig, CommandLane.CronNested);
  ...
  it("auto-resumes cron lanes to configured and clamped cron concurrency", async () => {
    ...
    await suspendLane(100, { cron: { maxConcurrentRuns: 3 } } as OriroConfig, CommandLane.Cron);
    await vi.advanceTimersByTimeAsync(100);
    ...
    await suspendLane(100, { cron: { maxConcurrentRuns: 0 } } as OriroConfig, CommandLane.Cron);
    await vi.advanceTimersByTimeAsync(100);
  ...
  it("clamps oversized suspension TTLs for timers and persisted resume time", async () => {
    ...
    await suspendLane(Number.MAX_SAFE_INTEGER, {} as OriroConfig, CommandLane.Main);
