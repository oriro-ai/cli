...
import { resolveOriroStateSqliteDir } from "./oriro-state-db.paths.js";
...
export type OriroAgentSqlitePathOptions = {
  agentId: string;
...
export function resolveOriroAgentSqlitePath(options: OriroAgentSqlitePathOptions): string {
  const agentId = normalizeAgentId(options.agentId);
  return path.resolve(
    options.path ??
      path.join(
        path.dirname(resolveOriroStateSqliteDir(options.env ?? process.env)),
        "agents",
        agentId,
        "agent",
        "oriro-agent.sqlite",
