...
function useTempHome(): string {
  const home = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-approval-runtime-"));
  tempHomes.push(home);
  setTestEnvValue("HOME", home);
  setTestEnvValue("ORIRO_HOME", home);
  return home;
}

function execApprovalsPath(home: string): string {
  return path.join(home, ".oriro", "exec-approvals.json");
}

function writeExecApprovalsToken(home: string, token: string): void {
  fs.mkdirSync(path.join(home, ".oriro"), { recursive: true });
  fs.writeFileSync(
