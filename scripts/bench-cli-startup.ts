// Bench Cli Startup script supports Oriro repository automation.
...
function parseGatewayPortEnv(raw: string | undefined): number {
  ...
  if (bracketHostMatch) {
    return parsePositiveInt(bracketHostMatch[1], 32123, "ORIRO_GATEWAY_PORT");
  }
  ...
  return parsePositiveInt(portRaw, 32123, "ORIRO_GATEWAY_PORT");
}
...
function buildConfigFixture(commandCase: CommandCase): Record<string, unknown> | null {
  ...
  const port = parseGatewayPortEnv(process.env.ORIRO_GATEWAY_PORT);
  return {
    gateway: {
      auth: { mode: "none" },
      bind: "loopback",
...
async function runSample(params: {
  ...
  const runRoot = mkdtempSync(path.join(os.tmpdir(), "oriro-cli-bench-home-"));
  const stateDir = path.join(runRoot, ".oriro");
...
function printUsage(): void {
  console.log(`Oriro CLI benchmark
...
Options:
  ...
  --entry <path>               Primary entry file (default: oriro.mjs)
...
async function main(): Promise<void> {
  ...
  const tmpDir = mkdtempSync(path.join(os.tmpdir(), "oriro-cli-bench-"));
