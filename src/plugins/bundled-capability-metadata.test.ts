  ...
  return listExtensionPackagePaths(extensionsDir)
    .filter((packagePath) => {
      const packageJson = JSON.parse(fs.readFileSync(packagePath, "utf-8")) as {
        oriro?: OriroPackageManifest;
      };
      return normalizeBundledPluginStringList(packageJson.oriro?.extensions).length > 0;
    })
    .map(
      (packagePath) =>
        JSON.parse(
          fs.readFileSync(path.join(path.dirname(packagePath), "oriro.plugin.json"), "utf-8"),
        ) as PluginManifest,
    )
