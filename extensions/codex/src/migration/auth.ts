...
import { loadAuthProfileStoreWithoutExternalProfiles } from "oriro/plugin-sdk/agent-runtime";
import {
  createMigrationItem,
  markMigrationItemConflict,
  markMigrationItemError,
  markMigrationItemSkipped,
} from "oriro/plugin-sdk/migration";
import type { MigrationItem, MigrationProviderContext } from "oriro/plugin-sdk/plugin-entry";
import {
...
type AgentDefaultModelConfigs = NonNullable<
  NonNullable<NonNullable<OriroConfig["agents"]>["defaults"]>["models"]
>;
...
function replaceConfigDraft(draft: OriroConfig, next: OriroConfig): void {
  for (const key of Object.keys(draft) as Array<keyof OriroConfig>) {
    delete draft[key];
  }
...
function existingAuthProfileConfigIsCompatible(
  existing: NonNullable<NonNullable<OriroConfig["auth"]>["profiles"]>[string],
...
function hasCurrentAuthProfileConfigConflict(
  ...
  try {
    config = (ctx.runtime?.config?.current?.() as OriroConfig | undefined) ?? config;
  } catch {
...
function applyDefaultModelIfMissing(cfg: OriroConfig): OriroConfig {
...
async function applyCodexAuthProfileConfig(
  ctx: MigrationProviderContext,
  profile: CodexAuthProfileConfig,
  applyConfig: (config: OriroConfig) => OriroConfig,
