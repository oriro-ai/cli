...
vi.mock("../loader.js", () => ({
  loadOriroPlugins: (...args: unknown[]) => loadOriroPluginsMock(...args),
}));
...
function getOnlyLoadOriroPluginsOptions(): PluginLoadOptions {
  expect(loadOriroPluginsMock).toHaveBeenCalledTimes(1);
  const options = loadOriroPluginsMock.mock.calls[0]?.[0];
  if (!options || typeof options !== "object") {
    throw new Error("expected loadOriroPlugins to receive plugin load options");
  }
...
describe("loadPluginMetadataRegistrySnapshot", () => {
  ...
  beforeEach(() => {
    ...
    loadOriroPluginsMock.mockReturnValue({ plugins: [], diagnostics: [] });
  });
  ...
  it("defaults to a non-activating validate snapshot", () => {
    loadPluginMetadataRegistrySnapshot({
      ...
      env: { HOME: "/tmp/oriro-home" } as NodeJS.ProcessEnv,
      workspaceDir: "/workspace",
      onlyPluginIds: ["demo"],
    });

    const loadOptions = getOnlyLoadOriroPluginsOptions();
    expect(loadOptions).toMatchObject({
      config: { plugins: {} },
      activationSourceConfig: { plugins: { allow: ["demo"] } },
      autoEnabledReasons: {},
      workspaceDir: "/workspace",
      env: { HOME: "/tmp/oriro-home" },
  ...
  it("forwards an explicit logger through metadata snapshots", () => {
    ...
    expect(getOnlyLoadOriroPluginsOptions()).toMatchObject({
      config: { plugins: {} },
      activationSourceConfig: { plugins: {} },
      autoEnabledReasons: {},
      workspaceDir: "/workspace",
  ...
  it("honors explicit load options when reusing a resolved runtime context", () => {
    ...
    expect(getOnlyLoadOriroPluginsOptions()).toEqual({
      config: { plugins: { allow: ["compat-provider"] } },
      activationSourceConfig: { plugins: { allow: ["raw-plugin"] } },
      autoEnabledReasons: {},
      workspaceDir: "/compat-workspace",
