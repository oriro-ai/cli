...
describe("compactSkillPaths", () => {
  ...
  it("replaces home directory prefix with ~ in skill locations", () => {
    ...
    const skillDir = path.join(home, ".oriro-test-skills", "test-skill");
  ...
  it("does not compact explicit state-root managed skill paths to OS-home tilde paths", () => {
    ...
    const stateDir = path.join(osHome, ".oriro");
    const skillDir = path.join(stateDir, "skills", "world-cup-soccer-oriro-skill");
    ...
    const prompt = withEnv(
      ...
      () =>
        buildPromptForFixtureSkill({
          ...
          skillDir,
          name: "world-cup-soccer-oriro-skill",
          description: "World Cup standings lookup",
        }),
    );

    expect(prompt).toContain(`<location>${skillFile}</location>`);
    expect(prompt).not.toContain("~/.oriro/skills/world-cup-soccer-oriro-skill/SKILL.md");
  });
  ...
  it("does not compact explicit state-root plugin skill paths to OS-home tilde paths", () => {
    ...
    expect(prompt).toContain(`<location>${skillFile}</location>`);
    expect(prompt).not.toContain("~/.oriro/plugin-skills/calendar-plugin-skill/SKILL.md");
  });
  ...
  it("compacts managed skill paths when OS-home tilde reaches the same path", () => {
    ...
    expect(prompt).toContain("<location>~/.oriro/skills/home-managed-skill/SKILL.md</location>");
  ...
  it("normalizes compacted Windows skill locations to forward slashes", () => {
    ...
    const skillPath = path.win32.join(home, ".oriro-test-skills", "win-skill", "SKILL.md");

    const compactedPath = workspaceSkillsTesting.compactHomePath(skillPath, [home]);

    expect(compactedPath).toBe("~/.oriro-test-skills/win-skill/SKILL.md");
  });

  it("preserves POSIX literal backslashes after home compaction", () => {
    const home = os.homedir();
    const skillDir = path.join(home, ".oriro-test-skills\\literal-skill");
  ...
  it("preserves paths outside home directory", () => {
    const outsideHome = path.join(path.parse(os.homedir()).root, "oriro-external-skills");
