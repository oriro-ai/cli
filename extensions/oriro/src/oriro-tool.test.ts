// Oriro tests cover oriro tool plugin behavior.
import { createTestPluginApi } from "oriro/plugin-sdk/plugin-test-api";
import { describe, expect, it, vi } from "vitest";
import type { OriroPluginApi, OriroPluginToolContext } from "../runtime-api.js";
import { createOriroTool } from "./oriro-tool.js";
import { createFakeTaskFlow } from "./taskflow-test-helpers.js";

function fakeApi(overrides: Partial<OriroPluginApi> = {}): OriroPluginApi {
...
function fakeCtx(overrides: Partial<OriroPluginToolContext> = {}): OriroPluginToolContext {
  return {
    config: {},
    workspaceDir: "/tmp",
    agentDir: "/tmp",
...
describe("oriro plugin tool", () => {
  it("returns the Oriro envelope in details", async () => {
    const runner = {
    ...
    const tool = createOriroTool(fakeApi(), { runner });
    ...
    const details = requireRecord(res.details, "oriro tool details");
  ...
  it("supports approval envelopes without changing the tool contract", async () => {
    ...
    const details = requireRecord(res.details, "approval oriro tool details");
    expect(details.ok).toBe(true);
  ...
  it("throws when the runner returns an error envelope", async () => {
    const tool = createOriroTool(fakeApi(), {
      runner: {
        run: vi.fn().mockResolvedValue({
          ok: false,
          error: {
  ...
  it("can run through managed TaskFlow mode", async () => {
    ...
    const tool = createOriroTool(fakeApi(), { runner, taskFlow });
    const res = await tool.execute("call-managed-run", {
      action: "run",
      pipeline: "noop",
      flowControllerId: "tests/oriro",
      flowGoal: "Run Oriro workflow",
      flowStateJson: '{"lane":"email"}',
      flowCurrentStep: "run_oriro",
    ...
    const details = requireRecord(res.details, "managed run oriro tool details");
    expect(details.ok).toBe(true);
    expect(details.status).toBe("needs_approval");
  ...
  it("can resume managed TaskFlow mode with only approvalId", async () => {
    ...
    const details = requireRecord(res.details, "managed resume oriro tool details");
    expect(details.ok).toBe(true);
    expect(details.status).toBe("ok");
  ...
  it("can be gated off in sandboxed contexts", () => {
    const api = fakeApi();
    const factoryTool = (ctx: OriroPluginToolContext) => {
      if (ctx.sandboxed) {
        return null;
      }
      return createOriroTool(api, {
        runner: { run: vi.fn() },
      });
    };

    expect(factoryTool(fakeCtx({ sandboxed: true }))).toBeNull();
    expect(factoryTool(fakeCtx({ sandboxed: false }))?.name).toBe("oriro");
  });
});
