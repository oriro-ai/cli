...
import { createOriroAgentHarness } from "./builtin-oriro.js";
...
type AgentHarnessSelectionDecision = {
  ...
  selectedReason:
    | "forced_oriro"
    | "forced_plugin"
    // Implicit Codex preference found no registered Codex harness, so Oriro handled the run.
    | "implicit_plugin_unavailable_oriro"
    // Provider-owned CLI runtime aliases have no agent harness plugin counterpart.
    | "cli_runtime_passthrough_oriro"
    // Auto mode chose a registered plugin harness that supports the provider/model.
    | "auto_plugin"
    // Auto mode found no supporting plugin harness, so Oriro handled the run.
    | "auto_oriro";
  ...
  if (runtime !== "auto") {
    ...
    if (forced) {
      ...
      if (isCliRuntimeAliasForProvider({ runtime, provider: params.provider })) {
        return buildSelectionDecision({
          ...
          selectedReason: "cli_runtime_passthrough_oriro",
          candidates: listHarnessCandidates(pluginHarnesses),
        });
      }
      throw new Error(
    ...
    if (runtime === "codex" && policy.runtimeSource === "implicit") {
      return buildSelectionDecision({
        ...
        selectedReason: "implicit_plugin_unavailable_oriro",
        candidates: listHarnessCandidates(pluginHarnesses),
      });
    }
    if (
...
export async function runAgentHarnessAttempt(
  ...
  const attemptParams =
    harness.id === "oriro" ? params : applyPluginHarnessDenyAllToolPolicy(params);
