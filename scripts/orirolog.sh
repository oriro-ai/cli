...
handle_sudo_error() {
    ...
    echo -e "${RED}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}\n"
    echo -e "orirolog needs to use sudo to show complete log data (Apple hides sensitive info by default)."
...
show_usage() {
    cat << EOF
orirolog - Oriro Logging Utility

USAGE:
    orirolog [OPTIONS]

DESCRIPTION:
    View Oriro logs with full details (bypasses Apple's privacy redaction).
    Requires sudo access configured for /usr/bin/log command.

LOG FLOW ARCHITECTURE:
    Oriro logs flow through the macOS unified log (subsystem: ai.oriro).
...
QUICK START:
    orirolog -n 100             Show last 100 lines from all components
    orirolog -f                 Follow logs in real-time
    orirolog -e                 Show only errors
    orirolog -c ServerManager   Show logs from ServerManager only
...
EXAMPLES:
    orirolog                   Show last 50 lines from past 5 minutes (default)
    orirolog -f                Stream logs continuously
    orirolog -n 100            Show last 100 lines
    orirolog -e                Show only recent errors
    orirolog -l 30m -n 200     Show last 200 lines from past 30 minutes
    orirolog -c ServerManager  Show recent ServerManager logs
    orirolog -s "fail"         Search for "fail" in recent logs
    orirolog --server -e       Show recent server errors
    orirolog -f -d             Stream debug logs continuously
