...
import { installOriroPluginSdkNativeResolver } from "./plugin-sdk-native-resolver.js";
...
export type PluginLoadOptions = {
  config?: OriroConfig;
  activationSourceConfig?: OriroConfig;
...
function createGuardedPluginRegistrationApi(api: OriroPluginApi): {
  api: OriroPluginApi;
  close: () => void;
} {
  let closed = false;
...
function runPluginRegisterSync(
  ...
  api: Parameters<NonNullable<OriroPluginDefinition["register"]>>[0],
): void {
...
function resolvePluginLoadCacheContext(options: PluginLoadOptions = {}) {
  ...
  const devSourceRoot = resolveOriroDevSourceRoot(env);
...
export function resolveRuntimePluginRegistry(
  ...
  // plugin registration is still in flight. Let direct loadOriroPlugins(...)
  // callers surface the hard error instead.
  if (isPluginRegistryLoadInFlight(options)) {
    return undefined;
  }
  return loadOriroPlugins(options);
}
...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: OriroPluginDefinition;
  register?: OriroPluginDefinition["register"];
} {
  ...
  for (let index = 0; index < candidates.length && index < 12; index += 1) {
    ...
    if (typeof resolved === "function") {
      return {
        register: resolved as OriroPluginDefinition["register"],
      };
    }
    if (resolved && typeof resolved === "object") {
      const def = resolved as OriroPluginDefinition;
...
export function loadOriroPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  try {
    ...
    for (const candidate of orderedCandidates) {
      ...
      if (record.format === "bundle") {
        ...
        for (const capability of unsupportedCapabilities) {
          registry.diagnostics.push({
            ...
            pluginId: record.id,
            source: record.source,
            message: `bundle capability detected but not wired into Oriro yet: ${capability}`,
          });
        }
        if (
          enableState.enabled &&
      ...
      let mod: OriroPluginModule | null = null;
      let moduleLoadMs = 0;
      ...
      try {
        ...
        mod = withProfile(
          ...
          registrationMode,
          () => loadPluginModule(safeSource) as OriroPluginModule,
        );
      } catch (err) {
        recordPluginError({
          logger,
      ...
      if (registrationPlan.loadSetupEntry && manifestRecord.setupSource) {
        ...
        if (setupRegistration.plugin) {
          ...
          if (
            ...
            let runtimeMod: OriroPluginModule | null = null;
            try {
              runtimeMod = withProfile(
                { pluginId: record.id, source: safeRuntimeSource },
                "load-setup-runtime-entry",
                () => loadPluginModule(safeRuntimeSource) as OriroPluginModule,
              );
            } catch (err) {
              recordPluginError({
                logger,
      ...
      if (failedPlugins.length > 0) {
        logger.warn(
          `[plugins] ${failedPlugins.length} plugin(s) failed to initialize (${formatPluginFailureSummary(
            failedPlugins,
          )}). Run 'oriro plugins inspect <id> --runtime --json' for runtime diagnostics, 'oriro plugins list' for registry state, and restart the Gateway after plugin code or load-path changes.`,
        );
      }
    }
...
export async function loadOriroPluginCliRegistry(
  options: PluginLoadOptions = {},
  ...
  for (const candidate of orderedCandidates) {
    ...
    let mod: OriroPluginModule | null;
    try {
      mod = withProfile(
        { pluginId: record.id, source: safeSource },
        "cli-metadata",
