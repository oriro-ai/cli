...
export const ORIRO_LIVE_TRANSCRIPT_MARKER_RE = /open(?:oriro|cl|flaw|clar|core)/;

export function expectOriroLiveTranscriptMarker(value: string): void {
  expect(normalizeTranscriptForMatch(value)).toMatch(ORIRO_LIVE_TRANSCRIPT_MARKER_RE);
}
...
export async function runRealtimeSttLiveTest(params: {
  ...
  const errors: Error[] = [];
  const expected = params.expectedNormalizedText ?? ORIRO_LIVE_TRANSCRIPT_MARKER_RE;
