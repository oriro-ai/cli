...
Autonomous maintainer workflow for producing five landable Oriro bugfix PR URLs.
...
Use `$gitcrawl` for discovery/clustering, `$oriro-pr-maintainer` for live GitHub mutation rules, `$github-author-context` when contributor trust matters, `$oriro-testing` for proof choice, `$autoreview` before publishing/landing, and `$crabbox` for broad/E2E/live proof.
