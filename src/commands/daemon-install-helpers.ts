...
import {
  ORIRO_WRAPPER_ENV_KEY,
  resolveGatewayProgramArguments,
  resolveOriroWrapperPath,
...
// they share the ORIRO_ prefix that is otherwise stripped from preserved
...
const PRESERVED_ORIRO_OPERATOR_OPT_IN_ENV_KEYS = new Set([
  "ORIRO_CLI_CONTAINER_BYPASS",
  "ORIRO_CONTAINER_HINT",
]);
...
export function collectPreservedExistingServiceEnvVars(
  ...
  for (const [rawKey, rawValue] of Object.entries(existingEnvironment)) {
    ...
    if (
      ...
      upper === "PATH" ||
      upper === "TMPDIR" ||
      (upper.startsWith("ORIRO_") && !PRESERVED_ORIRO_OPERATOR_OPT_IN_ENV_KEYS.has(upper))
    ) {
      continue;
    }
...
export async function buildGatewayInstallPlan(params: {
  ...
  const wrapperInput = params.wrapperPath ?? params.env[ORIRO_WRAPPER_ENV_KEY];
  ...
  const wrapperPath = wrapperPointsAtWindowsTaskScript
    ? undefined
    : await resolveOriroWrapperPath(wrapperInput);
  const serviceInputEnv: Record<string, string | undefined> = wrapperPath
    ? { ...params.env, [ORIRO_WRAPPER_ENV_KEY]: wrapperPath }
    : wrapperPointsAtWindowsTaskScript
      ? omitEnvKey(params.env, ORIRO_WRAPPER_ENV_KEY)
      : params.env;
  ...
  const serviceEnvironment = buildServiceEnvironment({
    ...
    launchdLabel:
      platform === "darwin"
        ? resolveGatewayLaunchAgentLabel(serviceInputEnv.ORIRO_PROFILE)
        : undefined,
    platform,
...
export function gatewayInstallErrorHint(platform = process.platform): string {
  return platform === "win32"
    ...
    : `Tip: rerun \`${formatCliCommand("oriro gateway install")}\` after fixing the error.`;
}
