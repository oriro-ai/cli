// Bench Gateway Restart script supports Oriro repository automation.
...
function printUsage(): void {
  console.log(`Oriro Gateway restart benchmark
...
function writeConfig(root: string, benchCase: GatewayBenchCase): string {
  ...
  const configPath = path.join(root, "oriro.json");
...
function sanitizedEnv(
  ...
  return {
    ...
    LOGNAME: process.env.LOGNAME ?? "oriro-bench",
    NO_COLOR: "1",
    PATH: process.env.PATH,
    SHELL: process.env.SHELL,
    TMPDIR: process.env.TMPDIR,
    USER: process.env.USER ?? "oriro-bench",
    npm_config_update_notifier: "false",
    ORIRO_CONFIG: configPath,
    ...
    ORIRO_TEST_DISABLE_UPDATE_CHECK: "1",
    ...benchCase.env,
  };
}
...
async function runGatewaySample(options: {
  ...
  const root = mkdtempSync(path.join(tmpdir(), "oriro-gateway-restart-bench-"));
