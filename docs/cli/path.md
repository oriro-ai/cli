---
summary: "CLI reference for `oriro path` (inspect and edit workspace files via the `oc://` addressing scheme)"
read_when:
...
files (markdown, jsonc, jsonl, yaml/yml/oriro). Self-hosters, plugin
...
```bash
oriro plugins enable oc-path
```

## Why use it

Oriro state is spread across human-edited markdown, commented JSONC config,
...
`oriro path` gives those callers a stable address instead of a one-off grep,
...
You probably do not need `oriro path` for ordinary whole-file edits, rich
...
```bash
oriro path resolve 'oc://config.jsonc/plugins/github/enabled'
```

Preview a write without touching disk:

```bash
oriro path set 'oc://config.jsonc/plugins/github/enabled' 'true' --dry-run
```

Find matching records in an append-only JSONL log:

```bash
oriro path find 'oc://session.jsonl/[event=tool_call]/name'
```
...
```bash
oriro path resolve 'oc://AGENTS.md/runtime-safety/oriro-gateway'
```

Validate a path in CI or a preflight script before the script reads or writes:

```bash
oriro path validate 'oc://AGENTS.md/tools/$last/risk'
```
...
2. Chooses the file-kind adapter from the target extension (`.md`, `.jsonc`,
   `.jsonl`, `.yaml`, `.yml`, `.oriro`, and related aliases).
...
| YAML/YML/.oriro | Map keys and sequence indexes; comments and flow style are handled by the YAML document API.        |
...
# Read a leaf
oriro path resolve 'oc://gateway.jsonc/version'

# Wildcard search
oriro path find 'oc://session.jsonl/*/event' --file ./logs/session.jsonl

# Dry-run a write
oriro path set 'oc://gateway.jsonc/version' '2.0' --dry-run

# Dry-run a write as a unified diff
oriro path set 'oc://gateway.jsonc/version' '2.0' --dry-run --diff

# Apply the write
oriro path set 'oc://gateway.jsonc/version' '2.0'

# Byte-fidelity round-trip (diagnostic)
oriro path emit ./AGENTS.md
```
...
oriro path resolve 'oc://config.jsonc/agents.defaults.models/"anthropic/claude-opus-4-7"/alias'

# Deep JSON/JSONC paths can use slash segments; they normalize to dotted subsegments
oriro path set 'oc://oriro.json/agents/list/0/tools/exec/security' 'allowlist' --dry-run

# Replace a JSONC leaf with a parsed object
oriro path set 'oc://oriro.json/gateway/auth/token' '{"source":"file","provider":"secrets","id":"/test"}' --value-json --dry-run

# Predicate search over JSONC children
oriro path find 'oc://config.jsonc/plugins/[enabled=true]/id'

# Insert into a JSONC array
oriro path set 'oc://config.jsonc/items/+1' '{"id":"new","enabled":true}' --dry-run

# Insert a JSONC object key
oriro path set 'oc://config.jsonc/plugins/+github' '{"enabled":true}' --dry-run

# Append a JSONL event
oriro path set 'oc://session.jsonl/+' '{"event":"checkpoint","ok":true}' --file ./logs/session.jsonl

# Resolve the last JSONL value line
oriro path resolve 'oc://session.jsonl/$last/event' --file ./logs/session.jsonl

# Resolve a YAML workflow step
oriro path resolve 'oc://workflow.yaml/steps/0/id'

# Update a YAML scalar
oriro path set 'oc://workflow.yaml/steps/$last/id' 'classify-renamed' --dry-run

# Address markdown frontmatter
oriro path resolve 'oc://AGENTS.md/[frontmatter]/name'

# Insert markdown frontmatter
oriro path set 'oc://AGENTS.md/[frontmatter]/+description' 'Agent instructions' --dry-run

# Find markdown item fields
oriro path find 'oc://SKILL.md/Tools/*/send_email'

# Validate a session-scoped path
oriro path validate 'oc://AGENTS.md/Tools/$last/risk?session=cron-daily'
```
...
```bash
$ oriro path resolve 'oc://x.md/[frontmatter]/tier' --file frontmatter.md --human
leaf @ L4: "core" (string)

$ oriro path resolve 'oc://x.md/tools/gh/gh' --file frontmatter.md --human
leaf @ L9: "GitHub CLI" (string)

$ oriro path find 'oc://x.md/tools/*' --file frontmatter.md --human
...
```bash
$ oriro path resolve 'oc://config.jsonc/plugins/github/enabled' --file config.jsonc --human
leaf @ L4: "true" (boolean)

$ oriro path set 'oc://config.jsonc/plugins/slack/enabled' 'true' --file config.jsonc --dry-run
...
```bash
$ oriro path find 'oc://session.jsonl/[event=action]/userId' --file session.jsonl --human
1 match for oc://session.jsonl/[event=action]/userId:
  oc://session.jsonl/L2/userId  →  leaf @ L2: "u1" (string)

$ oriro path resolve 'oc://session.jsonl/L2/ts' --file session.jsonl --human
leaf @ L2: "2" (number)
```
...
steps:
  ...
  - id: classify
    command: oriro.invoke
```

```bash
$ oriro path resolve 'oc://workflow.yaml/steps/0/id' --file workflow.yaml --human
leaf @ L3: "fetch" (string)

$ oriro path set 'oc://workflow.yaml/steps/$last/id' 'classify-renamed' --file workflow.yaml --dry-run
...
adapter handles `.yaml`, `.yml`, and `.oriro` files.
...
```bash
oriro path resolve 'oc://AGENTS.md/tools/gh/risk' --human
oriro path resolve 'oc://gateway.jsonc/server/port' --json
```
...
```bash
oriro path find 'oc://AGENTS.md/tools/**/risk'
oriro path find 'oc://session.jsonl/[event=action]/userId'
oriro path find 'oc://config.jsonc/plugins/{github,slack}/enabled'
```
...
oriro path set 'oc://gateway.jsonc/version' '2.0' --dry-run --diff
oriro path set 'oc://gateway.jsonc/version' '2.0'
oriro path set 'oc://AGENTS.md/Tools/+gh/risk' 'low'
```
...
```bash
$ oriro path validate 'oc://AGENTS.md/tools/gh' --human
...
```bash
oriro path emit ./AGENTS.md
oriro path emit ./gateway.jsonc --json
```
...
`oriro path` is TTY-aware: human-readable output on a terminal, JSON when
...
- `set` writes bytes through the substrate's emit path, which applies the
  ...
  `__ORIRO_REDACTED__` (verbatim or as a substring) is refused at write
  time.
