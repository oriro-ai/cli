...
export type WideAreaDiscoveryConfig = {
  ...
  enabled?: boolean;
  /** Optional unicast DNS-SD domain (e.g. "oriro.internal"). */
  domain?: string;
};
...
export type GatewayControlUiConfig = {
  ...
  enabled?: boolean;
  /** Optional base path prefix for the Control UI (e.g. "/oriro"). */
  basePath?: string;
   ...
   * Default off; prefer hosted /__oriro__/canvas or /__oriro__/a2ui content.
   */
 ...
 * Used when Oriro runs behind an identity-aware proxy (Pomerium, Caddy + OAuth, etc.)
...
export type GatewayTailscaleConfig = {
  ...
  resetOnExit?: boolean;
  /** Optional Tailscale Service name, such as `svc:oriro`, for Serve mode. */
  serviceName?: string;
  /**
   ...
   * startup. Lets operators manage Funnel exposure outside Oriro without
...
export type GatewayConfig = {
   ...
   * Env var ORIRO_HANDSHAKE_TIMEOUT_MS takes precedence. Default: 15000.
   */
