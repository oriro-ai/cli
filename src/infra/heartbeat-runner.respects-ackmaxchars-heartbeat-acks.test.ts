...
describe("runHeartbeatOnce ack handling", () => {
  ...
  function expectTelegramMessageSend(
    ...
    params: { to: string; text: string; cfg: OriroConfig },
  ) {
  ...
  it("respects ackMaxChars for heartbeat acks", async () => {
    await withTempHeartbeatSandbox(async ({ tmpDir, storePath, replySpy }) => {
      ...
      replySpy.mockResolvedValue({ text: "HEARTBEAT_OK ORIRO" });
  ...
  it.each([
    ...
    },
    {
      ...
      replyText: "[oriro] HEARTBEAT_OK all good",
      messages: { responsePrefix: "[oriro]" },
      expectedCalls: 0,
    },
    {
