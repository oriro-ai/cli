...
vi.mock("../../agents/agent-scope.js", () => ({
  resolveAgentDir: (_cfg: OriroConfig, agentId: string) => `/tmp/oriro/agents/${agentId}`,
...
vi.mock("../../agents/auth-profiles.js", () => ({
  ...
  resolveAuthStatePathForDisplay: (agentDir: string) => `${agentDir}/oriro-agent.sqlite`,
}));
...
describe("modelsAuthListCommand", () => {
  beforeEach(() => {
    mocks.loadModelsConfig.mockReset().mockResolvedValue({} as OriroConfig);
    ...
    expect(runtime.jsonPayloads).toStrictEqual([
      {
        ...
        agentId: "coder",
        authStatePath: "/tmp/oriro/agents/coder/oriro-agent.sqlite",
        profiles: [
          {
    ...
    expect(runtime.jsonPayloads).toStrictEqual([
      {
        ...
        agentId: "main",
        authStatePath: "/tmp/oriro/agents/main/oriro-agent.sqlite",
        profiles: [
          {
  ...
  it("prints an empty profile list without failing", async () => {
    ...
    expect(runtime.logs).toEqual([
      "Agent: main",
      "Auth state store: /tmp/oriro/agents/main/oriro-agent.sqlite",
      "Profiles: (none)",
    ]);
  });
