...
import { listOriroPluginManifestMetadata } from "./manifest-metadata-scan.js";
...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-model-id-normalization-"));
  tempDirs.push(dir);
  return dir;
}
  ...
  writePersistedInstalledPluginIndexSync(
    {
      ...
      plugins: [
        {
          pluginId: "normalizer",
          manifestPath: path.join(params.pluginDir, "oriro.plugin.json"),
...
function createCurrentSnapshot(params: {
  ...
  const index: InstalledPluginIndex = {
    ...
    plugins: [
      {
        pluginId: "normalizer",
        manifestPath: `/tmp/normalizer-${params.manifestHash}/oriro.plugin.json`,
...
describe("manifest model id normalization", () => {
  ...
  it("reuses current metadata when callers omit config", () => {
    const config: OriroConfig = { plugins: { allow: ["normalizer"] } };
    setCurrentPluginMetadataSnapshot(
      createCurrentSnapshot({
        manifestHash: "alpha",
        prefix: "alpha",
  ...
  it("keeps process metadata stable across manifest edits and reflects state-dir changes", () => {
    ...
    setTestEnvValue("ORIRO_STATE_DIR", stateDirA);
    deleteTestEnvValue("ORIRO_HOME");
    setTestEnvValue("ORIRO_DISABLE_BUNDLED_PLUGINS", "1");
    deleteTestEnvValue("ORIRO_BUNDLED_PLUGINS_DIR");
    ...
    setTestEnvValue("ORIRO_STATE_DIR", stateDirB);
    clearPluginMetadataLifecycleCaches();
  ...
  it("reuses manifest metadata while file fingerprints are unchanged", () => {
    ...
    expect(listOriroPluginManifestMetadata(process.env)).toHaveLength(1);
    expect(listOriroPluginManifestMetadata(process.env)).toHaveLength(1);
