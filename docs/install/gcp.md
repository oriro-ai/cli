---
summary: "Run Oriro Gateway 24/7 on a GCP Compute Engine VM (Docker) with durable state"
read_when:
  - You want Oriro running 24/7 on GCP
  - You want a production-grade, always-on Gateway on your own VM
  - You want full control over persistence, binaries, and restart behavior
title: "GCP"
---

Run a persistent Oriro Gateway on a GCP Compute Engine VM using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "Oriro 24/7 for ~$5-12/mo", this is a reliable setup on Google Cloud.
...
- Start the Oriro Gateway in Docker
- Persist `~/.oriro` + `~/.oriro/workspace` on the host (survives restarts/rebuilds)
- Access the Control UI from your laptop via an SSH tunnel

That mounted `~/.oriro` state includes `oriro.json`, per-agent
...
<Steps>
  ...
  <Step title="Create a GCP project">
    **CLI:**
    ...
    ```bash
    gcloud projects create my-oriro-project --name="Oriro Gateway"
    gcloud config set project my-oriro-project
    ```
  ...
  <Step title="Create the VM">
    ...
    **CLI:**
    ...
    ```bash
    gcloud compute instances create oriro-gateway \
      --zone=us-central1-a \
  ...
  <Step title="SSH into the VM">
    **CLI:**
    ...
    ```bash
    gcloud compute ssh oriro-gateway --zone=us-central1-a
    ```
  ...
  <Step title="Clone the Oriro repository">
    ```bash
  ...
  <Step title="Configure environment variables">
    ...
    ```bash
    ORIRO_IMAGE=oriro:latest
    ORIRO_GATEWAY_TOKEN=
    ORIRO_GATEWAY_BIND=lan
    ORIRO_GATEWAY_PORT=18789

    ORIRO_CONFIG_DIR=/home/$USER/.oriro
    ORIRO_WORKSPACE_DIR=/home/$USER/.oriro/workspace

    GOG_KEYRING_PASSWORD=
    XDG_CONFIG_HOME=/home/node/.oriro
    ```

    Set `ORIRO_GATEWAY_TOKEN` when you want to manage the stable gateway
    token through `.env`; otherwise configure `gateway.auth.token` before
    relying on clients across restarts. If neither source exists, Oriro uses
    ...
    **Do not commit this file.**
    ...
    This `.env` file is for container/runtime env such as `ORIRO_GATEWAY_TOKEN`.
    ...
    services:
      oriro-gateway:
        ...
        environment:
          ...
          - NODE_ENV=production
          - TERM=xterm-256color
          - ORIRO_GATEWAY_BIND=${ORIRO_GATEWAY_BIND}
          - ORIRO_GATEWAY_PORT=${ORIRO_GATEWAY_PORT}
        ...
        ports:
          ...
          - "127.0.0.1:${ORIRO_GATEWAY_PORT}:18789"
        command:
          [
            "node",
            "dist/index.js",
  ...
  <Step title="GCP-specific launch notes">
    ...
    When binding to LAN (`ORIRO_GATEWAY_BIND=lan`), configure a trusted browser origin before continuing:

    ```bash
    docker compose run --rm oriro-cli config set gateway.controlUi.allowedOrigins '["http://127.0.0.1:18789"]' --strict-json
    ```
  ...
  <Step title="Access from your laptop">
    ...
    ```bash
    gcloud compute ssh oriro-gateway --zone=us-central1-a -- -L 18789:127.0.0.1:18789
    ```
...
```bash
# Stop the VM first
gcloud compute instances stop oriro-gateway --zone=us-central1-a

# Change machine type
gcloud compute instances set-machine-type oriro-gateway \
  --zone=us-central1-a \
  --machine-type=e2-small

# Start the VM
gcloud compute instances start oriro-gateway --zone=us-central1-a
```
...
1. Create a service account:
   ...
   ```bash
   gcloud iam service-accounts create oriro-deploy \
     --display-name="Oriro Deployment"
   ```

2. Grant Compute Instance Admin role (or narrower custom role):

   ```bash
   gcloud projects add-iam-policy-binding my-oriro-project \
     --member="serviceAccount:oriro-deploy@my-oriro-project.iam.gserviceaccount.com" \
