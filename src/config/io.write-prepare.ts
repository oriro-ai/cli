...
import { normalizeConfiguredProviderCatalogModelId } from "@oriro/model-catalog-core/provider-model-id-normalization";
...
export function formatConfigValidationFailure(pathLabel: string, issueMessage: string): string {
  ...
  return [
    ...
    "",
    "Fix with:",
    `  oriro config set ${allowPath} '["*"]'`,
    "",
    "Or switch policy:",
    `  oriro config set ${policyPath} "pairing"`,
  ].join("\n");
}
...
export function unsetPathForWrite(
  root: OriroConfig,
  pathSegments: string[],
): { changed: boolean; next: OriroConfig } {
