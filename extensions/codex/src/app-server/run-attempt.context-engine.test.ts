...
import type { EmbeddedRunAttemptParams } from "oriro/plugin-sdk/agent-harness";
import {
  embeddedAgentLog,
...
} from "oriro/plugin-sdk/hook-runtime";
...
import { registerSandboxBackend } from "oriro/plugin-sdk/sandbox";
...
function threadStartResult(threadId = "thread-1") {
  return {
    thread: {
      ...
      status: { type: "idle" },
      path: null,
      cwd: tempDir || "/tmp/oriro-codex-test",
      cliVersion: "0.125.0",
      source: "unknown",
      agentNickname: null,
      agentRole: null,
...
describe("runCodexAppServerAttempt context-engine lifecycle", () => {
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-codex-context-engine-"));
  });
  ...
  it("bootstraps and assembles non-legacy context before the Codex turn starts", async () => {
    ...
    expectRequestInputTextContains(harness, "Oriro assembled context for this turn:");
  ...
  it("projects thread-bootstrap context only once for a matching context-engine epoch", async () => {
    ...
    expectRequestInputTextContains(firstHarness, "Oriro assembled context for this turn:");
  ...
  it("resumes a matching thread-bootstrap binding even when the bootstrap turn exceeded the opt-in native byte guard", async () => {
    ...
    await writeCodexAppServerBinding(sessionFile, {
      ...
      contextEngine: {
        ...
        engineId: "lossless-oriro",
        policyFingerprint:
          '{"schemaVersion":1,"engineId":"lossless-oriro","ownsCompaction":true,"projectionMaxChars":24000}',
        projection: {
          schemaVersion: 1,
          mode: "thread_bootstrap",
          epoch: "epoch-1",
  ...
  it("retries a resumed context-engine thread on a fresh Codex thread without plugin compaction", async () => {
    ...
    await writeCodexAppServerBinding(sessionFile, {
      ...
      contextEngine: {
        ...
        engineId: "lossless-oriro",
        policyFingerprint:
          '{"schemaVersion":1,"engineId":"lossless-oriro","ownsCompaction":true,"contextTokenBudget":400000,"projectionMaxChars":1000000}',
        projection: {
          schemaVersion: 1,
          mode: "thread_bootstrap",
          epoch: "epoch-before",
    ...
    expect(savedBinding?.contextEngine?.engineId).toBe("lossless-oriro");
    expect(savedBinding?.contextEngine?.projection).toBeUndefined();
  });
  ...
  it("fails first-turn Codex context overflow instead of falling back to Oriro compaction", async () => {
    const sessionFile = path.join(tempDir, "session.jsonl");
    const workspaceDir = path.join(tempDir, "workspace");
    const compact = vi.fn<ContextEngine["compact"]>(async () => ({
      ok: true,
  ...
  it("keeps current inbound context at the front of the Codex context-engine prompt", async () => {
    ...
    params.currentInboundContext = {
      text: [
        ...
        "#6474 Sun 2026-05-10 22:22 GMT+5:30 [reply target] Oriro: anchor REPLYCTX this is the old message",
        "#6498 Sun 2026-05-10 22:22 GMT+5:30 Oriro: filler REPLYCTX 23",
      ].join("\n"),
    };
    ...
    expect(inputText).toContain("Oriro assembled context for this turn:");
    expect(inputText).toContain("Current user request:\nhello");
    expect(inputText).toContain("[reply target] Oriro: anchor REPLYCTX");
