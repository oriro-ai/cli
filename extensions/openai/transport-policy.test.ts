...
import type { ProviderRuntimeModel } from "oriro/plugin-sdk/plugin-entry";
...
describe("openai transport policy", () => {
  ...
  it("builds native turn state for direct OpenAI routes", () => {
    ...
    expect(state?.headers?.["x-oriro-session-id"]).toBe("session-123");
    expect(state?.headers?.["x-oriro-turn-id"]).toBe("turn-123");
    expect(state?.headers?.["x-oriro-turn-attempt"]).toBe("2");
    expect(state?.metadata?.oriro_session_id).toBe("session-123");
    expect(state?.metadata?.oriro_turn_id).toBe("turn-123");
    expect(state?.metadata?.oriro_turn_attempt).toBe("2");
    expect(state?.metadata?.oriro_transport).toBe("websocket");
  });
  ...
  it("returns websocket session headers and cooldown for native routes", () => {
    ...
    expect(policy?.headers?.["x-oriro-session-id"]).toBe("session-123");
