...
const resolveOriroPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  ...
  loadOriroPlugins: () => createEmptyPluginRegistry(),
...
vi.mock("oriro/plugin-sdk/provider-auth", () => ({
  isNonSecretApiKeyMarker: () => false,
}));

vi.mock("oriro/plugin-sdk/provider-model-shared", () => ({
  DEFAULT_CONTEXT_TOKENS: 32768,
  normalizeProviderId: (value: string) => normalizeLowercaseStringOrEmpty(value),
}));

vi.mock("oriro/plugin-sdk/provider-stream-shared", () => ({
...
vi.mock("../infra/oriro-root.js", () => ({
  ...
  resolveOriroPackageRootSync: vi.fn(() => "/tmp/oriro"),
}));
...
beforeEach(() => {
  ...
  resolveOriroPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.ORIRO_STATE_DIR;
  originalUpdateInProgress = process.env.ORIRO_UPDATE_IN_PROGRESS;
  process.env.ORIRO_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-doctor-state-"));
