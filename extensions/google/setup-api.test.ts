...
import type { CliBackendPlugin } from "oriro/plugin-sdk/cli-backend";
...
describe("google gemini cli backend auth bridge", () => {
  it("materializes selected Oriro OAuth credentials into a persistent profile-scoped Gemini CLI home", async () => {
    const backend = buildGoogleGeminiCliBackend();
    const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-test-workspace-"));
    ...
    try {
      ...
      await fs.writeFile(
        ...
        `${JSON.stringify({
          ...
          mcp: { allowed: ["oriro"] },
          mcpServers: { oriro: { url: "http://127.0.0.1:23119/mcp" } },
        })}\n`,
        "utf8",
      );
      ...
      expect(
        path.relative(resolvePreferredOriroTmpDir(), path.dirname(systemSettingsPath ?? "")),
      ).toMatch(/^oriro-gemini-cli-/);
