...
DEFAULT_PROVIDER="${ORIRO_DOCKER_CLI_BACKEND_PROVIDER:-claude-cli}"
CLI_MODEL="${ORIRO_LIVE_CLI_BACKEND_MODEL:-}"
CLI_PROVIDER="${CLI_MODEL%%/*}"
CLI_DISABLE_MCP_CONFIG="${ORIRO_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG:-}"
CLI_AUTH_MODE="${ORIRO_LIVE_CLI_BACKEND_AUTH:-auto}"
...
case "$CLI_AUTH_MODE" in
  auto | api-key | subscription)
    ;;
  *)
    echo "ERROR: ORIRO_LIVE_CLI_BACKEND_AUTH must be one of: auto, api-key, subscription." >&2
    exit 1
    ;;
esac

if [[ "$CLI_AUTH_MODE" == "subscription" && "$CLI_PROVIDER" != "claude-cli" ]]; then
  echo "ERROR: ORIRO_LIVE_CLI_BACKEND_AUTH=subscription is only supported for claude-cli." >&2
  exit 1
fi
...
export ORIRO_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE="${ORIRO_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE:-0}"
export ORIRO_LIVE_CLI_BACKEND_IMAGE_PROBE="${ORIRO_LIVE_CLI_BACKEND_IMAGE_PROBE:-0}"
export ORIRO_LIVE_CLI_BACKEND_MCP_PROBE="${ORIRO_LIVE_CLI_BACKEND_MCP_PROBE:-0}"
...
if [[ "$CLI_PROVIDER" == "claude-cli" && "$CLI_AUTH_MODE" == "subscription" ]]; then
  ...
  if [[ -z "${ORIRO_LIVE_CLI_BACKEND_PRESERVE_ENV:-}" ]]; then
    if [[ "$CLAUDE_SUBSCRIPTION_AUTH_SOURCE" == "env-token" ]]; then
      export ORIRO_LIVE_CLI_BACKEND_PRESERVE_ENV='["CLAUDE_CODE_OAUTH_TOKEN"]'
    else
      export ORIRO_LIVE_CLI_BACKEND_PRESERVE_ENV="[]"
    fi
  fi
  if [[ "$ORIRO_LIVE_CLI_BACKEND_PRESERVE_ENV" == *ANTHROPIC_API_KEY* ]]; then
    echo "ERROR: subscription auth smoke must not preserve Anthropic API-key env vars." >&2
    exit 1
  fi
  if [[ "$CLAUDE_SUBSCRIPTION_AUTH_SOURCE" == "env-token" && "$ORIRO_LIVE_CLI_BACKEND_PRESERVE_ENV" != *CLAUDE_CODE_OAUTH_TOKEN* ]]; then
  ...
  export ORIRO_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE="${ORIRO_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE:-0}"
  export ORIRO_LIVE_CLI_BACKEND_RESUME_PROBE="${ORIRO_LIVE_CLI_BACKEND_RESUME_PROBE:-1}"
  export ORIRO_LIVE_CLI_BACKEND_IMAGE_PROBE="${ORIRO_LIVE_CLI_BACKEND_IMAGE_PROBE:-0}"
  export ORIRO_LIVE_CLI_BACKEND_MCP_PROBE="${ORIRO_LIVE_CLI_BACKEND_MCP_PROBE:-0}"
fi
...
if [[ -n "${ORIRO_DOCKER_AUTH_DIRS:-}" ]]; then
  ...
  while IFS= read -r auth_dir; do
    [[ -n "$auth_dir" ]] || continue
    AUTH_DIRS+=("$auth_dir")
  done < <(oriro_live_collect_auth_dirs_from_csv "$CLI_PROVIDER")
  while IFS= read -r auth_file; do
    [[ -n "$auth_file" ]] || continue
    AUTH_FILES+=("$auth_file")
  done < <(oriro_live_collect_auth_files_from_csv "$CLI_PROVIDER")
fi
if [[ "${CLAUDE_SUBSCRIPTION_AUTH_SOURCE:-}" == "env-token" ]]; then
  retained_auth_files=()
...
run_setup_command() {
  local timeout_value="${ORIRO_LIVE_CLI_BACKEND_SETUP_TIMEOUT_SECONDS:?missing live CLI backend setup timeout seconds}s"
  local timeout_bin=""
...
provider="${ORIRO_DOCKER_CLI_BACKEND_PROVIDER:-claude-cli}"
default_command="${ORIRO_DOCKER_CLI_BACKEND_COMMAND_DEFAULT:-}"
docker_package="${ORIRO_DOCKER_CLI_BACKEND_NPM_PACKAGE:-}"
binary_name="${ORIRO_DOCKER_CLI_BACKEND_BINARY_NAME:-}"
if [ -z "$binary_name" ] && [ -n "$default_command" ]; then
  binary_name="$(basename "$default_command")"
fi
if [ -z "${ORIRO_LIVE_CLI_BACKEND_COMMAND:-}" ] && [ -n "$binary_name" ]; then
  export ORIRO_LIVE_CLI_BACKEND_COMMAND="$NPM_CONFIG_PREFIX/bin/$binary_name"
fi
...
if [ -n "${ORIRO_LIVE_CLI_BACKEND_COMMAND:-}" ] && [ ! -x "${ORIRO_LIVE_CLI_BACKEND_COMMAND}" ] && [ -n "$docker_package" ]; then
  run_setup_command npm install -g "$docker_package"
elif [ -n "$docker_package" ] && package_has_explicit_version "$docker_package"; then
  run_setup_command npm install -g "$docker_package"
fi
if [ -n "${ORIRO_LIVE_CLI_BACKEND_COMMAND:-}" ] && [ -x "${ORIRO_LIVE_CLI_BACKEND_COMMAND}" ]; then
  echo "==> CLI backend binary: ${ORIRO_LIVE_CLI_BACKEND_COMMAND}"
  "${ORIRO_LIVE_CLI_BACKEND_COMMAND}" -V || "${ORIRO_LIVE_CLI_BACKEND_COMMAND}" --version || true
fi
if [ "$provider" = "claude-cli" ]; then
  auth_mode="${ORIRO_LIVE_CLI_BACKEND_AUTH:-auto}"
...
if [ -n "\${ORIRO_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY:-}" ]; then
  export ANTHROPIC_API_KEY="\${ORIRO_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY}"
fi
if [ -n "\${ORIRO_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY_OLD:-}" ]; then
  export ANTHROPIC_API_KEY_OLD="\${ORIRO_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY_OLD}"
fi
...
WRAP
  ...
  if [ -z "${ORIRO_LIVE_CLI_BACKEND_PRESERVE_ENV:-}" ]; then
    export ORIRO_LIVE_CLI_BACKEND_PRESERVE_ENV='["ANTHROPIC_API_KEY","ANTHROPIC_API_KEY_OLD"]'
  fi
  if [ "$auth_mode" = "subscription" ]; then
    claude --version
    direct_token="ORIRO-CLAUDE-SUBSCRIPTION-DIRECT"
...
DOCKER_AUTH_ENV=(
  -e ORIRO_LIVE_CLI_BACKEND_AUTH="$CLI_AUTH_MODE"
)
if [[ "$CLI_PROVIDER" == "claude-cli" && "$CLI_AUTH_MODE" == "subscription" ]]; then
  DOCKER_AUTH_ENV+=(
    -e CLAUDE_CODE_OAUTH_TOKEN="${CLAUDE_CODE_OAUTH_TOKEN:-}"
    -e ORIRO_LIVE_CLI_BACKEND_PRESERVE_ENV="$ORIRO_LIVE_CLI_BACKEND_PRESERVE_ENV"
  )
else
  DOCKER_AUTH_ENV+=(
    -e ANTHROPIC_API_KEY
    -e ANTHROPIC_API_KEY_OLD
    -e ORIRO_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY="${ANTHROPIC_API_KEY:-}"
    -e ORIRO_LIVE_CLI_BACKEND_ANTHROPIC_API_KEY_OLD="${ANTHROPIC_API_KEY_OLD:-}"
    -e ORIRO_LIVE_CLI_BACKEND_PRESERVE_ENV="${ORIRO_LIVE_CLI_BACKEND_PRESERVE_ENV:-}"
  )
fi

DOCKER_RUN_ARGS=()
oriro_live_init_docker_run_args DOCKER_RUN_ARGS "${ORIRO_LIVE_CLI_BACKEND_DOCKER_RUN_TIMEOUT:-2700s}"
DOCKER_RUN_ARGS+=(--rm -t \
  ...
  -e ORIRO_LIVE_CLI_BACKEND_SETUP_TIMEOUT_SECONDS="$CLI_SETUP_TIMEOUT_SECONDS" \
  -e ORIRO_DOCKER_CLI_BACKEND_PROVIDER="$CLI_PROVIDER" \
  -e ORIRO_DOCKER_CLI_BACKEND_COMMAND_DEFAULT="$CLI_DEFAULT_COMMAND" \
  -e ORIRO_DOCKER_CLI_BACKEND_NPM_PACKAGE="$CLI_DOCKER_NPM_PACKAGE" \
  -e ORIRO_DOCKER_CLI_BACKEND_BINARY_NAME="$CLI_DOCKER_BINARY_NAME" \
  -e ORIRO_LIVE_TEST=1 \
  -e ORIRO_LIVE_CLI_BACKEND=1 \
  -e ORIRO_LIVE_CLI_BACKEND_DEBUG="${ORIRO_LIVE_CLI_BACKEND_DEBUG:-}" \
  -e ORIRO_LIVE_CLI_BACKEND_ADVISORY="${ORIRO_LIVE_CLI_BACKEND_ADVISORY:-}" \
  -e ORIRO_LIVE_CLI_BACKEND_ALLOW_PROVIDER_SKIP="${ORIRO_LIVE_CLI_BACKEND_ALLOW_PROVIDER_SKIP:-}" \
  -e ORIRO_CLI_BACKEND_LOG_OUTPUT="${ORIRO_CLI_BACKEND_LOG_OUTPUT:-}" \
  -e ORIRO_TEST_CONSOLE="${ORIRO_TEST_CONSOLE:-}" \
  -e ORIRO_LIVE_CLI_BACKEND_MODEL="$CLI_MODEL" \
  -e ORIRO_LIVE_CLI_BACKEND_COMMAND="${ORIRO_LIVE_CLI_BACKEND_COMMAND:-}" \
  -e ORIRO_LIVE_CLI_BACKEND_ARGS="${ORIRO_LIVE_CLI_BACKEND_ARGS:-}" \
  -e ORIRO_LIVE_CLI_BACKEND_RESUME_ARGS="${ORIRO_LIVE_CLI_BACKEND_RESUME_ARGS:-}" \
  -e ORIRO_LIVE_CLI_BACKEND_CLEAR_ENV="${ORIRO_LIVE_CLI_BACKEND_CLEAR_ENV:-}" \
  -e ORIRO_LIVE_CLI_BACKEND_DISABLE_MCP_CONFIG="$CLI_DISABLE_MCP_CONFIG" \
  -e ORIRO_LIVE_CLI_BACKEND_RESUME_PROBE="${ORIRO_LIVE_CLI_BACKEND_RESUME_PROBE:-}" \
  -e ORIRO_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE="${ORIRO_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE:-}" \
  -e ORIRO_LIVE_CLI_BACKEND_IMAGE_PROBE="${ORIRO_LIVE_CLI_BACKEND_IMAGE_PROBE:-}" \
  -e ORIRO_LIVE_CLI_BACKEND_MCP_PROBE="${ORIRO_LIVE_CLI_BACKEND_MCP_PROBE:-}" \
  -e ORIRO_LIVE_CLI_BACKEND_MCP_SCHEMA_PROBE="${ORIRO_LIVE_CLI_BACKEND_MCP_SCHEMA_PROBE:-}" \
  -e ORIRO_LIVE_CLI_BACKEND_IMAGE_ARG="${ORIRO_LIVE_CLI_BACKEND_IMAGE_ARG:-}" \
  -e ORIRO_LIVE_CLI_BACKEND_IMAGE_MODE="${ORIRO_LIVE_CLI_BACKEND_IMAGE_MODE:-}")
