...
} from "oriro/plugin-sdk/migration-runtime";
import type {
...
const HERMES_STATE_DB_SNAPSHOT_PREFIX = "oriro-migrate-hermes-state-";
...
async function archiveHermesItem(item: MigrationItem, reportDir: string): Promise<MigrationItem> {
  ...
  try {
    ...
    return await withTempWorkspace(
      { rootDir: resolvePreferredOriroTmpDir(), prefix: HERMES_STATE_DB_SNAPSHOT_PREFIX },
      async ({ dir: tempDir }) => {
