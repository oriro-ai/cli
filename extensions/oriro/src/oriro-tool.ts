// Oriro plugin module implements oriro tool behavior.
import {
...
} from "oriro/plugin-sdk/param-readers";
...
import {
  ...
  resolveOriroCwd,
  type OriroRunner,
  type OriroRunnerParams,
} from "./oriro-runner.js";
import {
  type ManagedOriroFlowResult,
  resumeManagedOriroFlow,
  runManagedOriroFlow,
} from "./oriro-taskflow.js";
...
function resolveManagedFlowToolResult(result: ManagedOriroFlowResult) {
  if (!result.ok) {
    throw result.error;
  }
...
export function createOriroTool(api: OriroPluginApi, options?: OriroToolOptions) {
  const runner = options?.runner ?? createEmbeddedOriroRunner();
  return {
    name: "oriro",
    label: "Oriro Workflow",
    description:
      "Run Oriro pipelines as a local-first workflow runtime (typed JSON envelope + resumable approvals).",
    ...
    async execute(_id: string, params: Record<string, unknown>) {
      ...
      const cwd = resolveOriroCwd(params.cwd);
      ...
      if (api.runtime?.version && api.logger?.debug) {
        api.logger.debug(`oriro plugin runtime=${api.runtime.version}`);
      }

      const runnerParams: OriroRunnerParams = {
        action,
      ...
      if (action === "run") {
        ...
        if (flowParams) {
          return resolveManagedFlowToolResult(
            await resumeManagedOriroFlow({
              taskFlow: requireTaskFlowRuntime(taskFlow, "resume"),
              runner,
              runnerParams: runnerParams as OriroRunnerParams & {
                action: "resume";
                approve: boolean;
