...
The plugin is included with Oriro, but it is off by default. When disabled, the route is not registered. When enabled, it adds:
...
Use the WebSocket RPC path for Oriro clients and interactive tools that can keep a Gateway WebSocket connection open.
...
<Tabs>
  <Tab title="CLI">
    ```bash
    oriro plugins enable admin-http-rpc
...
```bash
oriro plugins disable admin-http-rpc
oriro gateway restart
```
...
- For `token` and `password` auth, narrower `x-oriro-scopes` headers are ignored and the normal full operator defaults are restored.
- Trusted identity-bearing HTTP modes honor `x-oriro-scopes` when present.
...
The normal Gateway WebSocket RPC path remains the preferred control-plane API for Oriro clients. Use admin HTTP RPC only for host tooling that needs a request/response HTTP surface.
