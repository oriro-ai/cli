...
import { createProviderUsageFetch, makeResponse } from "oriro/plugin-sdk/test-env";
import { beforeEach, describe, expect, it, vi } from "vitest";
import { deriveCopilotApiBaseUrlFromToken, resolveCopilotApiToken } from "./token.js";
import { fetchCopilotUsage } from "./usage.js";

vi.mock("oriro/plugin-sdk/provider-model-shared", async (importOriginal) => ({
  ...(await importOriginal<typeof import("oriro/plugin-sdk/provider-model-shared")>()),
...
vi.mock("oriro/plugin-sdk/json-store", () => ({
  loadJsonFile: jsonStoreMocks.loadJsonFile,
  saveJsonFile: jsonStoreMocks.saveJsonFile,
}));

vi.mock("oriro/plugin-sdk/state-paths", () => ({
  resolveStateDir: () => "/tmp/oriro-state",
}));

import type { ProviderResolveDynamicModelContext } from "oriro/plugin-sdk/core";
...
describe("github-copilot token", () => {
  const cachePath = "/tmp/oriro-state/credentials/github-copilot.token.json";
