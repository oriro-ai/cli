...
export function resolveWindowsExtensionChunkSize(env = process.env) {
  return resolvePositiveEnvIntWithFallback(
    env,
    "ORIRO_OXLINT_WINDOWS_EXTENSION_CHUNK_SIZE",
...
export function shouldRunOxlintShardsSerial({
  ...
  const explicitMode = env.ORIRO_OXLINT_SHARDS_SERIAL?.trim();
  if (explicitMode === "1") {
    return true;
  }
  if (platform === "win32") {
  ...
  const localCheckMode = env.ORIRO_LOCAL_CHECK_MODE?.trim().toLowerCase();
...
function isRemoteChangedGateEnv(env) {
  return (
    env.ORIRO_CHECK_CHANGED_REMOTE_CHILD === "1" || env.ORIRO_CHANGED_LANES_RAW_SYNC === "1"
  );
}
...
export function resolveOxlintShardConcurrency({
  ...
  const explicitConcurrency = resolvePositiveEnvInt(env, "ORIRO_OXLINT_SHARD_CONCURRENCY");
...
export function resolveShardHeartbeatMs(env) {
  return resolveNonNegativeEnvInt(
    env,
    "ORIRO_OXLINT_SHARD_HEARTBEAT_MS",
...
export function resolveShardKillGraceMs(env) {
  return resolveNonNegativeEnvInt(
    env,
    "ORIRO_OXLINT_SHARD_KILL_GRACE_MS",
