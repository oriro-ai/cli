...
const logPathTracker = createSuiteLogPathTracker("oriro-test-env-log-level-");

describe("ORIRO_LOG_LEVEL", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["ORIRO_LOG_LEVEL"]);
  ...
  it("applies a valid env override to both file and console levels", () => {
    ...
    process.env.ORIRO_LOG_LEVEL = "debug";
  ...
  it("warns once and ignores invalid env values", () => {
    ...
    process.env.ORIRO_LOG_LEVEL = "nope";
    ...
    const warnings = stderrSpy.mock.calls
      ...
      .filter((line) => line.includes("ORIRO_LOG_LEVEL"));
    expect(warnings).toHaveLength(1);
    expect(warnings[0]).toContain('Ignoring invalid ORIRO_LOG_LEVEL="nope"');
  });
});
