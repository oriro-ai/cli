---
summary: "Deploy Oriro on Render with Infrastructure-as-Code"
read_when:
  - Deploying Oriro to Render
...
Deploy Oriro on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/oriro/oriro)
...
repository configures everything needed to run Oriro:
...
services:
  - type: web
    ...
    envVars:
      - key: ORIRO_GATEWAY_PORT
        value: "8080"
      - key: ORIRO_STATE_DIR
        value: /data/.oriro
      - key: ORIRO_WORKSPACE_DIR
        value: /data/workspace
      - key: ORIRO_GATEWAY_TOKEN
...
your fork's `render.yaml` (but note: no persistent disk means Oriro state
...
`ORIRO_GATEWAY_TOKEN` (find it in **Dashboard → your service →
...
If you use the original Oriro repository, Render will not auto-deploy your Oriro. To update it, run a manual Blueprint sync from the dashboard.
...
For Oriro, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This creates a portable backup archive with Oriro state plus any configured
...
- Missing `ORIRO_GATEWAY_TOKEN` — verify it is set in **Dashboard → Environment**
- Port mismatch — ensure `ORIRO_GATEWAY_PORT=8080` is set so the gateway binds to the port Render expects
...
regularly export a full backup via `oriro backup create` in the Render shell.
