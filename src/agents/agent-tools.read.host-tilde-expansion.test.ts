...
describe("host tool tilde expansion (non-workspace mode)", () => {
  ...
  it("edit readFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("oriro-tilde-test-edit-");
  ...
  it("write writeFile expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("oriro-tilde-test-write-");
  ...
  it("write mkdir expands ~ to the OS home directory", async () => {
    const dir = await createTempDir("oriro-tilde-test-mkdir-");
  ...
  it("ignores ORIRO_HOME for write operations", async () => {
    const oriroHome = await createTempDir("oriro-home-override-", os.tmpdir());
    const dir = await createTempDir("oriro-tilde-test-write-");
    const testFile = path.join(dir, "os-home-write.txt");

    await withEnvAsync({ ORIRO_HOME: oriroHome }, async () => {
      createHostWorkspaceWriteTool(oriroHome, { workspaceOnly: false });
      ...
      await expectMissingPath(fs.access(path.join(oriroHome, path.basename(testFile))));
    });
  });

  it("ignores ORIRO_HOME for mkdir operations", async () => {
    const oriroHome = await createTempDir("oriro-home-override-", os.tmpdir());
    const dir = await createTempDir("oriro-tilde-test-mkdir-");
    ...
    await withEnvAsync({ ORIRO_HOME: oriroHome }, async () => {
      ...
      expect((await fs.stat(newDir)).isDirectory()).toBe(true);
      await expectMissingPath(fs.access(path.join(oriroHome, path.basename(newDir))));
    });
  });

  it("ignores ORIRO_HOME for readFile operations", async () => {
    const oriroHome = await createTempDir("oriro-home-override-", os.tmpdir());
    const dir = await createTempDir("oriro-tilde-test-edit-");
    ...
    await withEnvAsync({ ORIRO_HOME: oriroHome }, async () => {
      createHostWorkspaceEditTool(oriroHome, { workspaceOnly: false });
      const content = await readEditOps().readFile(toTildePath(testFile));
  ...
  it("ignores ORIRO_HOME for access operations", async () => {
    const oriroHome = await createTempDir("oriro-home-override-", os.tmpdir());
    const dir = await createTempDir("oriro-tilde-test-edit-");
