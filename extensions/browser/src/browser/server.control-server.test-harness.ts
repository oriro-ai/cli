...
function restoreGatewayPortEnv(prevGatewayPort: string | undefined): void {
  ...
  process.env.ORIRO_GATEWAY_PORT = prevGatewayPort;
}
...
export function setBrowserControlServerProfiles(
  ...
  defaultProfile = Object.keys(profiles)[0] ?? "oriro",
): void {
...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/oriro" }));
...
function defaultProfilesForState(testPort: number): HarnessState["cfgProfiles"] {
  return {
    oriro: { cdpPort: defaultBrowserCdpPortForState(testPort), color: "#FF4500" },
  };
}
...
vi.mock("../config/config.js", async () => {
  ...
  const mutateConfigFile = vi.fn(
    async (params: {
      ...
      const result = await params.mutate(draft, { snapshot: { path: "/tmp/oriro.json" } });
      await writeConfigFile(draft);
      return {
        path: "/tmp/oriro.json",
        previousHash: "test-hash",
        persistedHash: "test-hash",
        snapshot: { path: "/tmp/oriro.json" },
        nextConfig: draft,
        result,
        attempts: 1,
...
vi.mock("./chrome.js", () => ({
  ...
  launchOriroChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
  ...
  resolveOriroUserDataDir: vi.fn(() => chromeUserDataDir.dir),
  stopOriroChrome: vi.fn(async () => {
...
export async function resetBrowserControlServerTestContext(): Promise<void> {
  ...
  state.cfgProfiles = defaultProfilesForState(state.testPort);
  state.prevGatewayPort = process.env.ORIRO_GATEWAY_PORT;
  process.env.ORIRO_GATEWAY_PORT = String(state.testPort - 2);
  ...
  state.prevGatewayToken = process.env.ORIRO_GATEWAY_TOKEN;
  state.prevGatewayPassword = process.env.ORIRO_GATEWAY_PASSWORD;
...
function restoreGatewayAuthEnv(
  ...
  if (prevGatewayToken === undefined) {
    ...
    process.env.ORIRO_GATEWAY_TOKEN = prevGatewayToken;
  }
  if (prevGatewayPassword === undefined) {
    delete process.env.ORIRO_GATEWAY_PASSWORD;
  } else {
    process.env.ORIRO_GATEWAY_PASSWORD = prevGatewayPassword;
  }
}
