...
struct IOSGatewayChatTransport: OriroChatTransport {
    static let logger = Logger(subsystem: "ai.orirofoundation.app", category: "ios.chat.transport")
    ...
    private struct ChatSendParams: Codable {
        ...
        var message: String
        var thinking: String
        var attachments: [OriroChatAttachmentPayload]?
        var timeoutMs: Int
    ...
    static func makeChatSendParamsJSON(
        ...
        attachments: [OriroChatAttachmentPayload]) throws -> String
    {
        ...
        parentSessionKey: String?) async throws -> OriroChatCreateSessionResponse
    {
        ...
        return try JSONDecoder().decode(OriroChatCreateSessionResponse.self, from: res)
    }
    ...
    func listSessions(limit: Int?) async throws -> OriroChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(OriroChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> OriroChatHistoryPayload {
        ...
        return try JSONDecoder().decode(OriroChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [OriroChatAttachmentPayload]) async throws -> OriroChatSendResponse
    {
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(OriroChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(OriroGatewayHealthOK.self, from: res))?.ok ?? true
    }
    ...
    static func mapEventFrame(_ evt: EventFrame) -> OriroChatTransportEvent? {
        switch evt.event {
        case "tick":
            return .tick
        case "seqGap":
        ...
        case "health":
            ...
            let ok = (try? GatewayPayloadDecoding.decode(
                payload,
                as: OriroGatewayHealthOK.self))?.ok ?? true
            return .health(ok: ok)
        case "chat":
            guard let payload = evt.payload else { return nil }
        ...
        case "session.message":
            ...
            guard let message = try? GatewayPayloadDecoding.decode(
                payload,
                as: OriroSessionMessageEventPayload.self)
            else {
                return nil
            }
        ...
        case "agent":
            ...
            guard let agentPayload = try? GatewayPayloadDecoding.decode(
                payload,
                as: OriroAgentEventPayload.self)
            else {
                return nil
            }
