...
describe("configureGatewayForSetup", () => {
  ...
  it("prefers ORIRO_GATEWAY_TOKEN during quickstart token setup", async () => {
    const prevToken = process.env.ORIRO_GATEWAY_TOKEN;
    process.env.ORIRO_GATEWAY_TOKEN = "token-from-env";
    mocks.randomToken.mockReturnValue("generated-token");
  ...
  it("keeps ORIRO_GATEWAY_TOKEN in advanced flow when user confirms keeping existing", async () => {
    const prevToken = process.env.ORIRO_GATEWAY_TOKEN;
    process.env.ORIRO_GATEWAY_TOKEN = "advanced-env-token";
  ...
  it("honors secretInputMode=ref for gateway password prompts", async () => {
    const previous = process.env.ORIRO_GATEWAY_PASSWORD;
    process.env.ORIRO_GATEWAY_PASSWORD = "gateway-secret"; // pragma: allowlist secret
    try {
      const prompter = createPrompter({
        selectQueue: ["loopback", "password", "off", "env"],
        textQueue: ["18789", "ORIRO_GATEWAY_PASSWORD"],
      });
      const runtime = createRuntime();
      ...
      if (previous === undefined) {
        ...
        process.env.ORIRO_GATEWAY_PASSWORD = previous;
      }
    }
  });

  it("stores gateway token as SecretRef when secretInputMode=ref", async () => {
    const previous = process.env.ORIRO_GATEWAY_TOKEN;
    process.env.ORIRO_GATEWAY_TOKEN = "token-from-env";
    try {
      const prompter = createPrompter({
        selectQueue: ["loopback", "token", "off", "env"],
        textQueue: ["18789", "ORIRO_GATEWAY_TOKEN"],
      });
      const runtime = createRuntime();
      ...
      if (previous === undefined) {
        ...
        process.env.ORIRO_GATEWAY_TOKEN = previous;
      }
    }
  });
