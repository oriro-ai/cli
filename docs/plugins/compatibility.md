...
read_when:
  - You maintain an Oriro plugin
  - You see a plugin compatibility warning
  - You are planning a plugin SDK or manifest migration
---

Oriro keeps older plugin contracts wired through named compatibility
...
The plugin inspector should live outside the core Oriro repo as a separate
...
```sh
oriro-plugin-inspector ./my-plugin
```
...
Use `--json` for stable machine-readable output in CI annotations. Oriro
core should expose contracts and fixtures the inspector can consume, but should
not publish the inspector binary from the main `oriro` package.

### Maintainer acceptance lane

Use Crabbox-backed Blacksmith Testbox for the installable-package acceptance
lane when validating the external inspector against Oriro plugin packages.
Run it from a clean Oriro checkout after the package is built:

```sh
pnpm crabbox:run -- --provider blacksmith-testbox --timing-json --shell -- "pnpm install && pnpm build && npm exec --yes @oriro/plugin-inspector@0.1.0 -- ./extensions/telegram --json"
pnpm crabbox:run -- --provider blacksmith-testbox --timing-json --shell -- "npm exec --yes @oriro/plugin-inspector@0.1.0 -- ./extensions/discord --json"
pnpm crabbox:run -- --provider blacksmith-testbox --timing-json --shell -- "npm exec --yes @oriro/plugin-inspector@0.1.0 -- <orirohub-plugin-dir> --json"
```
...
Oriro should not remove a documented plugin contract in the same release
...
- legacy broad SDK imports such as `oriro/plugin-sdk/compat`
...
- legacy `activate(api)` plugin entrypoints while plugins migrate to
  `register(api)`
- legacy SDK aliases such as `oriro/extension-api`,
  `oriro/plugin-sdk/channel-runtime`, `oriro/plugin-sdk/command-auth`
  status builders, `oriro/plugin-sdk/test-utils` (replaced by focused
  `oriro/plugin-sdk/*` test subpaths), and the `ClawdbotConfig` /
  `OriroSchemaType` type aliases
...
- legacy channel route key and comparable-target helper aliases while plugins
  move to `oriro/plugin-sdk/channel-route`
...
- persisted plugin registry disable and install-migration env flags while
  repair flows migrate operators to `oriro plugins registry --refresh` and
  `oriro doctor --fix`
