// Verifies state-dir migrations preserve existing Oriro runtime data.
...
async function withStateDirFixture(run: (root: string) => Promise<void>): Promise<void> {
  try {
    await withTempDir({ prefix: "oriro-state-dir-" }, async (root) => {
      await run(root);
    });
  } finally {
    resetAutoMigrateLegacyStateDirForTest();
...
describe("legacy state dir auto-migration", () => {
  ...
  it("skips state-dir migration when ORIRO_STATE_DIR is explicitly set", async () => {
    await withStateDirFixture(async (root) => {
      ...
      const result = await autoMigrateLegacyStateDir({
        env: { ORIRO_STATE_DIR: path.join(root, "custom-state") } as NodeJS.ProcessEnv,
        homedir: () => root,
      });
