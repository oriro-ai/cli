...
async function makeTestState(seed = "legacy-oauth-seed"): Promise<OriroTestState> {
  const state = await createOriroTestState({
    layout: "state-only",
    prefix: "oriro-doctor-oauth-sidecar-",
    env: {
      ORIRO_AGENT_DIR: undefined,
      ORIRO_AUTH_PROFILE_SECRET_KEY: seed,
    },
  });
  states.push(state);
  return state;
...
function encryptLegacySidecarMaterial(params: {
  ref: { source: "oriro-credentials"; provider: "openai-codex"; id: string };
  profileId: string;
  provider: string;
  seed: string;
...
describe("maybeRepairLegacyOAuthSidecarProfiles", () => {
  ...
  it("scans ORIRO_AGENT_DIR before treating sidecars as unreferenced", async () => {
    const state = await makeTestState();
    const previousAgentDir = process.env.ORIRO_AGENT_DIR;
    ...
    try {
      ...
      if (previousAgentDir === undefined) {
        ...
        process.env.ORIRO_AGENT_DIR = previousAgentDir;
      }
    }
  });
