...
describe("qa scenario catalog", () => {
  ...
  it("loads the YAML pack as the canonical source of truth", () => {
    ...
    expect(pack.kickoffTask).toContain("Oriro Invaders");
  ...
  it("loads the opt-in update.run package self-upgrade sentinel", () => {
    ...
    expect(config?.allowEnv).toBe("ORIRO_QA_ALLOW_UPDATE_RUN_SELF");
  ...
  it("includes the Kitchen Sink live OpenAI plugin gauntlet", () => {
    ...
    expect(config?.requiredProviderMode).toBe("live-frontier");
    expect(config?.requiredProvider).toBe("openai");
    expect(config?.pluginSpec).toBe("npm:@oriro/kitchen-sink@latest");
    expect(config?.pluginId).toBe("oriro-kitchen-sink-fixture");
