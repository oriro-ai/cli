 ...
 * silk-wasm availability checks — all without importing `oriro/plugin-sdk`.
 ...
 * `ORIRO_HOME`. QQ Bot still checks this tree for legacy state imports and
...
/**
 * Resolve the effective Oriro home directory.
 *
 * Mirrors the contract from core (`src/infra/home-dir.ts::resolveEffectiveHomeDir`)
 * so QQ Bot media roots live under the same tree the rest of Oriro treats as
 * `~`. The extension cannot import the core helper directly (it is a separate
 * package with `oriro` as a peer dependency), so this re-implements the
 * minimal contract:
 *
 * 1. `ORIRO_HOME` when set (with `~` / `~/...` expanded against the OS home).
 ...
 */
function resolveOriroHome(): string {
  const raw = process.env.ORIRO_HOME?.trim();
...
/**
 * Return a legacy path under `~/.oriro/qqbot` without creating it.
 *
...
export function getQQBotDataPath(...subPaths: string[]): string {
  return path.join(getHomeDir(), ".oriro", "qqbot", ...subPaths);
}

/** Return a path under `~/.oriro/qqbot`, creating it on demand. */
...
/**
 * Return a path under `<oriro-home>/.oriro/media/qqbot` without creating it.
 *
 * Unlike `getQQBotDataPath`, this lives under Oriro's core media allowlist
 * so downloaded images and audio can be accessed by framework media tooling.
 * The base honors `ORIRO_HOME` (when set) so files written by agents into
 * the Oriro-managed media tree are reachable by this plugin even when
 * `HOME` and `ORIRO_HOME` differ (Docker, multi-user hosts). Fixes #83562.
 */
export function getQQBotMediaPath(...subPaths: string[]): string {
  return path.join(resolveOriroHome(), ".oriro", "media", "qqbot", ...subPaths);
}

/** Return a path under `<oriro-home>/.oriro/media/qqbot`, creating it on demand. */
...
/**
 * Return `<oriro-home>/.oriro/media`, Oriro's shared media root.
 *
 * This mirrors the directory that core's `buildMediaLocalRoots` exposes as an
 * allowlisted location (see `oriro/src/media/local-roots.ts`). Using it as a
 ...
 * {@link getQQBotMediaPath}, the base honors `ORIRO_HOME`.
 */
function getOriroMediaDir(): string {
  return path.join(resolveOriroHome(), ".oriro", "media");
}
...
export function resolveQQBotLocalMediaPath(p: string): string {
  ...
  const oriroHomeDir = resolveOriroHome();
  const mediaRoot = getQQBotMediaPath();
  const dataRoot = getQQBotDataPath();
  // When ORIRO_HOME differs from HOME we have to consider workspace roots
  // under both trees: agents may be configured with `~`-relative paths (HOME)
  // or with the Oriro-managed home tree. Deduplicate when they match.
  const workspaceRoots = Array.from(
    new Set([
      path.join(osHomeDir, ".oriro", "workspace", "qqbot"),
      path.join(oriroHomeDir, ".oriro", "workspace", "qqbot"),
    ]),
  );
...
export function resolveQQBotPayloadLocalFilePath(p: string): string | null {
  ...
  // Trust both the QQ Bot-owned subdirectory and Oriro's shared `~/.oriro/media`
  ...
  const allowedRoots = [getOriroMediaDir(), getQQBotMediaPath()];
