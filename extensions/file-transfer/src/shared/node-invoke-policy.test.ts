...
import type { OriroPluginNodeInvokePolicyContext } from "oriro/plugin-sdk/plugin-entry";
...
function createCtx(overrides: {
  ...
  approvals?: OriroPluginNodeInvokePolicyContext["approvals"];
}) {
  const invokeNode = vi.fn<OriroPluginNodeInvokePolicyContext["invokeNode"]>(
    async ({
      params,
    }: Parameters<OriroPluginNodeInvokePolicyContext["invokeNode"]>[0] = {}) => ({
      ok: true,
      payload: {
        ok: true,
        path:
...
function requireInvokeParams(
  invokeNode: ReturnType<typeof vi.fn<OriroPluginNodeInvokePolicyContext["invokeNode"]>>,
  callIndex: number,
) {
