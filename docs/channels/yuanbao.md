...
Tencent Yuanbao is Tencent's AI assistant platform. The Oriro channel plugin
connects Yuanbao bots to Oriro over WebSocket so they can interact with users
...
> **Requires Oriro 2026.4.10 or above.** Run `oriro --version` to check. Upgrade with `oriro update`.

<Steps>
  <Step title="Add the Yuanbao channel with your credentials">
  ```bash
  oriro channels add --channel yuanbao --token "appKey:appSecret"
  ```
...
```bash
oriro channels login --channel yuanbao
```
...
```bash
oriro pairing list yuanbao
oriro pairing approve yuanbao <CODE>
```
