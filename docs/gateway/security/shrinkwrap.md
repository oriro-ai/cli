---
summary: "Plain-English and technical explanation of npm shrinkwrap in Oriro releases"
read_when:
  - You want to know what npm shrinkwrap means in an Oriro release
...
Oriro source checkouts use `pnpm-lock.yaml`. Published Oriro npm
...
it does not replace host isolation, `oriro security audit`, package
...
| --------------------- | ------------------------ | --------------------------------- |
| `pnpm-lock.yaml`      | Oriro source checkout | Maintainer dependency graph       |
| `npm-shrinkwrap.json` | Published npm package    | npm install graph for users       |
| `package-lock.json`   | Local npm apps           | Not the Oriro publish contract |

## Why Oriro uses it

Oriro is a gateway, plugin host, model router, and agent runtime. A default
...
The root `oriro` npm package and Oriro-owned npm plugin packages include
`npm-shrinkwrap.json` when they publish. Suitable Oriro-owned plugin
...
Oriro package validators require shrinkwrap in new root package tarballs.
...
validators reject `package-lock.json` for published Oriro packages.

To inspect a published root package:

```bash
npm pack oriro@<version> --json --pack-destination /tmp/oriro-pack
tar -tf /tmp/oriro-pack/oriro-<version>.tgz | grep '^package/npm-shrinkwrap.json$'
```

To inspect an Oriro-owned plugin package:

```bash
npm pack @oriro/discord@<version> --json --pack-destination /tmp/oriro-plugin-pack
tar -tf /tmp/oriro-plugin-pack/oriro-discord-<version>.tgz | grep '^package/npm-shrinkwrap.json$'
tar -tf /tmp/oriro-plugin-pack/oriro-discord-<version>.tgz | grep '^package/node_modules/'
```
