...
import type {
  OriroHubSkillSecurityVerdict,
  OriroHubSearchResult,
  OriroHubSkillDetail,
  SkillMessageMap,
} from "../controllers/skills.ts";
import { orirohubVerdictKey } from "../controllers/skills.ts";
...
export type SkillsProps = {
  ...
  orirohubVerdicts: Record<string, OriroHubSkillSecurityVerdict>;
  orirohubVerdictsLoading: boolean;
  orirohubVerdictsError: string | null;
  skillCardContents: Record<string, string>;
  skillCardLoadingKey: string | null;
  skillCardErrors: Record<string, string>;
  orirohubQuery: string;
  orirohubResults: OriroHubSearchResult[] | null;
  orirohubSearchLoading: boolean;
  orirohubSearchError: string | null;
  orirohubDetail: OriroHubSkillDetail | null;
  orirohubDetailSlug: string | null;
  orirohubDetailLoading: boolean;
  orirohubDetailError: string | null;
  orirohubInstallSlug: string | null;
  orirohubInstallMessage: { kind: "success" | "error"; text: string } | null;
  ...
  onOriroHubQueryChange: (query: string) => void;
  onOriroHubDetailOpen: (slug: string) => void;
  onOriroHubDetailClose: () => void;
  onOriroHubInstall: (slug: string) => void;
};
...
function verdictForSkill(skill: SkillStatusEntry, verdicts: SkillsProps["orirohubVerdicts"]) {
...
function verdictLabel(verdict: OriroHubSkillSecurityVerdict | null | undefined): string {
  if (!verdict) {
    return "Unavailable";
  }
...
function verdictChipClass(verdict: OriroHubSkillSecurityVerdict | null | undefined): string {
  if (!verdict) {
    return "chip-warn";
  }
  if (verdict.ok && verdict.decision === "pass") {
...
export function renderSkills(props: SkillsProps) {
  ...
  return html`
    <section class="card">
      ...
      <div style="margin-top: 16px; border-top: 1px solid var(--border); padding-top: 16px;">
        <div style="display: flex; align-items: center; gap: 12px; margin-bottom: 12px;">
          <div style="font-weight: 600;">OriroHub</div>
        ...
        <div style="display: flex; align-items: center; gap: 12px; flex-wrap: wrap;">
          <label class="field" style="flex: 1; min-width: 180px;">
            <input
              .value=${props.orirohubQuery}
              @input=${(e: Event) =>
                props.onOriroHubQueryChange((e.target as HTMLInputElement).value)}
              placeholder="Search OriroHub skills…"
              autocomplete="off"
              name="orirohub-search"
            />
          </label>
          ${props.orirohubSearchLoading ? html`<span class="muted">Searching…</span>` : nothing}
        </div>
        ${props.orirohubSearchError
          ? html`<div class="callout danger" style="margin-top: 8px;">
              ${props.orirohubSearchError}
            </div>`
          : nothing}
        ${props.orirohubInstallMessage
          ? html`<div
              class="callout ${props.orirohubInstallMessage.kind === "error" ? "danger" : "success"}"
              style="margin-top: 8px;"
            >
              ${props.orirohubInstallMessage.text}
            </div>`
          : nothing}
        ${renderOriroHubResults(props)}
      </div>
    ...
    ${props.orirohubDetailSlug ? renderOriroHubDetailDialog(props) : nothing}
  `;
}

function renderOriroHubResults(props: SkillsProps) {
  const results = props.orirohubResults;
  if (!results) {
    return nothing;
  }
  if (results.length === 0) {
    return html`<div class="muted" style="margin-top: 8px;">No skills found on OriroHub.</div>`;
  }
  return html`
      ...
      ${results.map(
        (r) => html`
          <div
            ...
            @click=${() => props.onOriroHubDetailOpen(r.slug)}
          >
            <div class="list-main">
            ...
            <div class="list-meta" style="display: flex; align-items: center; gap: 8px;">
              ...
              <button
                class="btn btn--sm"
                ?disabled=${props.orirohubInstallSlug !== null}
                @click=${(e: Event) => {
                  e.stopPropagation();
                  props.onOriroHubInstall(r.slug);
                }}
              >
                ${props.orirohubInstallSlug === r.slug ? "Installing\u2026" : "Install"}
              </button>
            </div>
          </div>
        `,
...
function renderOriroHubDetailDialog(props: SkillsProps) {
  const detail = props.orirohubDetail;
  ...
  return html`
    <dialog
      ...
      @close=${props.onOriroHubDetailClose}
    >
      <div class="md-preview-dialog__panel">
        <div class="md-preview-dialog__header">
          <div class="md-preview-dialog__title">
            ${detail?.skill?.displayName ?? props.orirohubDetailSlug}
          </div>
          <button
            class="btn btn--sm"
            @click=${(e: Event) => {
        ...
        <div class="md-preview-dialog__body" style="display: grid; gap: 16px;">
          ${props.orirohubDetailLoading
            ? html`<div class="muted">${t("common.loading")}</div>`
            : props.orirohubDetailError
              ? html`<div class="callout danger">${props.orirohubDetailError}</div>`
              : detail?.skill
                ? html`
                    ...
                    <button
                      ...
                      ?disabled=${props.orirohubInstallSlug !== null}
                      @click=${() => {
                        if (props.orirohubDetailSlug) {
                          props.onOriroHubInstall(props.orirohubDetailSlug);
                        }
                      }}
                    >
                      ${props.orirohubInstallSlug === props.orirohubDetailSlug
                        ? "Installing\u2026"
...
function renderSkill(skill: SkillStatusEntry, props: SkillsProps) {
  ...
  const verdict = verdictForSkill(skill, props.orirohubVerdicts);
  ...
  return html`
    <div class="list-item list-item-clickable" @click=${() => props.onDetailOpen(skill.skillKey)}>
      ...
      >
        ${skill.orirohub?.status === "linked"
          ? html`<span class="chip ${verdictChipClass(verdict)}">${verdictLabel(verdict)}</span>`
          : skill.orirohub?.status === "invalid"
            ? html`<span class="chip chip-warn">OriroHub link invalid</span>`
            : nothing}
...
function renderSkillDetail(skill: SkillStatusEntry, props: SkillsProps) {
  ...
  const showBundledBadge = Boolean(skill.bundled && skill.source !== "oriro-bundled");
    ...
    >
      <div class="md-preview-dialog__panel">
        ...
        <div class="md-preview-dialog__body" style="display: grid; gap: 16px;">
          ...
          ${skill.orirohub || skill.skillCard?.present
            ? html`
            ...
            : nothing}
          ${detailTab === "overview"
            ? renderInstalledOriroHubOverview(skill, props, verdict)
...
function renderInstalledOriroHubOverview(
  skill: SkillStatusEntry,
  props: SkillsProps,
  verdict: OriroHubSkillSecurityVerdict | null,
) {
  const link = skill.orirohub;
  if (!link) {
    return nothing;
  ...
  if (link.status === "invalid") {
    return html`<div class="callout danger">
      <div style="font-weight: 600; margin-bottom: 4px;">OriroHub link invalid</div>
      <div>${link.reason}</div>
    </div>`;
  }
  ...
  return html`
    ...
    >
      <div style="display: flex; align-items: center; gap: 8px; flex-wrap: wrap;">
        ...
        ${props.orirohubVerdictsLoading ? html`<span class="muted">Refreshing…</span>` : nothing}
      </div>
      ${props.orirohubVerdictsError
        ? html`<div class="muted" style="font-size: 13px;">${props.orirohubVerdictsError}</div>`
        : reasonText
