...
import { isRecord as isPlainObject } from "@oriro/normalization-core/record-coerce";
import {
...
function assertGatewayConfigMutationAllowed(params: {
  ...
  // Uses the same flag enumeration as `oriro security audit`.
  const currentFlags = new Set(
    collectEnabledInsecureOrDangerousFlags(params.currentConfig as OriroConfig),
  );
  const nextFlags = collectEnabledInsecureOrDangerousFlags(nextConfig as OriroConfig);
