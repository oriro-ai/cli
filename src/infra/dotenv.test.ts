...
const CREDENTIAL_AND_GATEWAY_ENV_KEYS = [
  ...
  "ORIRO_LIVE_ANTHROPIC_KEYS",
  "ORIRO_LIVE_GEMINI_KEY",
  "ORIRO_LIVE_OPENAI_KEY",
  "ORIRO_GATEWAY_TOKEN",
  "ORIRO_GATEWAY_PASSWORD",
  "ORIRO_GATEWAY_SECRET",
] as const;

const BUNDLED_TRUST_ROOT_ENV_LINES = [
  "ORIRO_BROWSER_CONTROL_MODULE=data:text/javascript,boom",
  "ORIRO_BUNDLED_HOOKS_DIR=./attacker-hooks",
  "ORIRO_BUNDLED_PLUGINS_DIR=./attacker-plugins",
  "ORIRO_BUNDLED_SKILLS_DIR=./attacker-skills",
...
async function withDotEnvFixture(run: (fixture: DotEnvFixture) => Promise<void>) {
  const base = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-dotenv-test-"));
...
describe("loadDotEnv", () => {
  it("loads ~/.oriro/.env as fallback without overriding CWD .env", async () => {
  ...
  it("loads the Ubuntu gateway.env compatibility fallback after ~/.oriro/.env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        setTestEnvValue("HOME", base);
        const defaultStateDir = path.join(base, ".oriro");
        setTestEnvValue("ORIRO_STATE_DIR", defaultStateDir);
        await writeEnvFile(path.join(defaultStateDir, ".env"), "FOO=from-global\n");
        await writeEnvFile(
          path.join(base, ".config", "oriro", "gateway.env"),
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "ORIRO_STATE_DIR=./evil-state",
            "ORIRO_CONFIG_PATH=./evil-config.json",
        ...
        expect(process.env.ORIRO_STATE_DIR).toBe(stateDir);
      ...
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "ORIRO_LIVE_ANTHROPIC_KEY=sk-ant-live",
            "ORIRO_LIVE_ANTHROPIC_KEYS=sk-ant-live-a,sk-ant-live-b",
            "ORIRO_LIVE_GEMINI_KEY=sk-gemini-live",
            "ORIRO_LIVE_OPENAI_KEY=sk-openai-live",
            "ORIRO_GATEWAY_TOKEN=attacker-token",
            "ORIRO_GATEWAY_PASSWORD=attacker-password",
            "ORIRO_GATEWAY_SECRET=attacker-secret",
          ].join("\n"),
        );
  ...
  it("blocks path-override vars from workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        ...
        expect(process.env.ORIRO_BUNDLED_PLUGINS_DIR).toBeUndefined();
        expect(process.env.ORIRO_OAUTH_DIR).toBeUndefined();
        expect(process.env.PI_CODING_AGENT_DIR).toBeUndefined();
      });
    });
  });

  it("blocks ORIRO_TEST_TAILSCALE_BINARY from workspace .env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          "ORIRO_TEST_TAILSCALE_BINARY=/tmp/attacker-tailscale\n",
        );

        delete process.env.ORIRO_TEST_TAILSCALE_BINARY;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.ORIRO_TEST_TAILSCALE_BINARY).toBeUndefined();
      });
    });
  });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "ORIRO_ALLOW_PLUGIN_INSTALL_OVERRIDES=1",
            'ORIRO_PLUGIN_INSTALL_OVERRIDES={"codex":"npm-pack:/tmp/codex.tgz"}',
          ].join("\n"),
        );
        ...
        expect(process.env.ORIRO_ALLOW_PLUGIN_INSTALL_OVERRIDES).toBeUndefined();
        expect(process.env.ORIRO_PLUGIN_INSTALL_OVERRIDES).toBeUndefined();
      });
    });
  });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(
          path.join(cwdDir, ".env"),
          [
            "ORIRO_PINNED_PYTHON=./attacker-python",
            "ORIRO_PINNED_WRITE_PYTHON=./attacker-write-python",
          ].join("\n"),
        );

        delete process.env.ORIRO_PINNED_PYTHON;
        delete process.env.ORIRO_PINNED_WRITE_PYTHON;

        loadWorkspaceDotEnvFile(path.join(cwdDir, ".env"), { quiet: true });

        expect(process.env.ORIRO_PINNED_PYTHON).toBeUndefined();
        expect(process.env.ORIRO_PINNED_WRITE_PYTHON).toBeUndefined();
      });
    });
  });
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "ORIRO_PINNED_PYTHON=/trusted/python",
            "ORIRO_PINNED_WRITE_PYTHON=/trusted/write-python",
          ].join("\n"),
        );
        vi.spyOn(process, "cwd").mockReturnValue(cwdDir);
        delete process.env.ANTHROPIC_BASE_URL;
        ...
        expect(process.env.ORIRO_PINNED_PYTHON).toBe("/trusted/python");
        expect(process.env.ORIRO_PINNED_WRITE_PYTHON).toBe("/trusted/write-python");
      });
    });
  });
      ...
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        await writeEnvFile(
          ...
          [
            ...
            "ORIRO_LIVE_GEMINI_KEY=sk-gemini-live",
            "ORIRO_LIVE_OPENAI_KEY=sk-openai-live",
            "ORIRO_GATEWAY_TOKEN=trusted-token",
            "ORIRO_GATEWAY_PASSWORD=trusted-password",
            "ORIRO_GATEWAY_SECRET=trusted-secret",
          ].join("\n"),
        );
        vi.spyOn(process, "cwd").mockReturnValue(cwdDir);
        clearEnv(CREDENTIAL_AND_GATEWAY_ENV_KEYS);
        ...
        expect(process.env.ORIRO_LIVE_ANTHROPIC_KEY).toBe("sk-ant-live");
        expect(process.env.ORIRO_LIVE_ANTHROPIC_KEYS).toBe("sk-ant-live-a,sk-ant-live-b");
        expect(process.env.ORIRO_LIVE_GEMINI_KEY).toBe("sk-gemini-live");
        expect(process.env.ORIRO_LIVE_OPENAI_KEY).toBe("sk-openai-live");
        expect(process.env.ORIRO_GATEWAY_TOKEN).toBe("trusted-token");
        expect(process.env.ORIRO_GATEWAY_PASSWORD).toBe("trusted-password");
        expect(process.env.ORIRO_GATEWAY_SECRET).toBe("trusted-secret");
      });
    });
  });
  ...
  it("does not let CWD .env redirect which global .env is loaded", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir, stateDir }) => {
        ...
        await writeEnvFile(path.join(cwdDir, ".env"), "ORIRO_STATE_DIR=./evil-state\n");
...
describe("loadCliDotEnv", () => {
  it("blocks ORIRO_STATE_DIR from workspace .env even when unset in process env", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        await writeEnvFile(path.join(cwdDir, ".env"), "ORIRO_STATE_DIR=./evil-state\n");
  ...
  it("does not load gateway.env when ORIRO_STATE_DIR is explicitly set", async () => {
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ base, cwdDir }) => {
        const customStateDir = path.join(base, "custom-state");
        setTestEnvValue("HOME", base);
        setTestEnvValue("ORIRO_STATE_DIR", customStateDir);
        await writeEnvFile(
          path.join(base, ".config", "oriro", "gateway.env"),
          "FOO=from-gateway\n",
        );
        ...
        expect(process.env.FOO).toBeUndefined();
        expect(process.env.ORIRO_STATE_DIR).toBe(customStateDir);
        expect(process.env.BAR).toBeUndefined();
      });
    });
  });
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir, stateDir }) => {
        const plugin: PluginManifestRecord = {
          ...
          manifestPath: "/plugins/runtime-cloud/oriro.plugin.json",
          providerAuthEnvVars: {
    ...
    await withIsolatedEnvAndCwd(async () => {
      await withDotEnvFixture(async ({ cwdDir }) => {
        const runtimeControlKeys = [
          ...
          "ORIRO_WORKSPACE_DIR",
          "ORIRO_MDNS_HOSTNAME",
          "ORIRO_SESSION_CACHE_TTL_MS",
          "ORIRO_UPDATE_PACKAGE_SPEC",
          "ORIRO_GATEWAY_PORT",
          "ORIRO_GATEWAY_URL",
          "ORIRO_ORIROHUB_URL",
          "ORIROHUB_URL",
          ...
          "ORIROHUB_TOKEN",
          "ORIROHUB_AUTH_TOKEN",
          "ORIROHUB_CONFIG_PATH",
          "ORIRO_DISABLE_BUNDLED_PLUGINS",
          "ORIRO_ALLOW_INSECURE_PRIVATE_WS",
          "ORIRO_BROWSER_EXECUTABLE_PATH",
          "EXAMPLE_API_HOST",
          "HOMEBREW_BREW_FILE",
          "HOMEBREW_PREFIX",
          "IRC_HOST",
          ...
          "ORIRO_SKIP_CHANNELS",
          "ORIRO_SKIP_PROVIDERS",
          "ORIRO_SKIP_CRON",
          "ORIRO_RAW_STREAM",
          "ORIRO_RAW_STREAM_PATH",
          "ORIRO_CACHE_TRACE",
          "ORIRO_CACHE_TRACE_FILE",
          "ORIRO_CACHE_TRACE_MESSAGES",
          "ORIRO_CACHE_TRACE_PROMPT",
          "ORIRO_CACHE_TRACE_SYSTEM",
          "ORIRO_SHOW_SECRETS",
          "ORIRO_PLUGIN_CATALOG_PATHS",
          "ORIRO_MPM_CATALOG_PATHS",
          "ORIRO_NODE_EXEC_HOST",
          "ORIRO_NODE_EXEC_FALLBACK",
          "ORIRO_ALLOW_PROJECT_LOCAL_BIN",
          "PATH",
          "HOMEBREW_BREW_FILE",
          "HOMEBREW_PREFIX",
          "SystemRoot",
