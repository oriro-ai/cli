...
import "./test-helpers/fast-oriro-tools-sessions.js";
...
function createOriroTools(options?: {
...
describe("sessions tools", () => {
  ...
  it("sessions_list forwards mailbox filters and includes messages", async () => {
    ...
    const tool = createOriroTools().find((candidate) => candidate.name === "sessions_list");
    if (!tool) {
  ...
  it("derives mailbox previews only after agent visibility filtering", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-sessions-list-preview-"));
  ...
  it("sessions_history filters tool messages by default", async () => {
    ...
    const tool = createOriroTools().find((candidate) => candidate.name === "sessions_history");
    if (!tool) {
    ...
    expect(details.messages).toContainEqual(
      expect.objectContaining({ provider: "oriro", model: "gateway-injected" }),
    );
    expect(details.messages).toContainEqual(
  ...
  it("sessions_history caps oversized payloads and strips heavy fields", async () => {
    ...
    expect(thinkingBlock?.oriroReasoningReplay).toBeUndefined();
  });
