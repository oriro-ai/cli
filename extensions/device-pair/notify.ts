...
import type { OriroPluginService } from "oriro/plugin-sdk/core";
import { listDevicePairing } from "oriro/plugin-sdk/device-bootstrap";
...
import type { PluginStateKeyedStore } from "oriro/plugin-sdk/plugin-state-runtime";
...
async function readNotifyState(api: OriroPluginApi): Promise<NotifyStateFile> {
...
async function writeNotifyState(api: OriroPluginApi, state: NotifyStateFile): Promise<void> {
  const subscriberStore = openNotifySubscriberStore(api);
...
async function notifyPendingPairingRequests(params: { api: OriroPluginApi }): Promise<void> {
...
export function createPairingNotifierService(api: OriroPluginApi): OriroPluginService {
