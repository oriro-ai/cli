  ...
  return new Promise((resolve) => {
    const child = spawn(process.execPath, [clientPath], {
      env: {
        ...
        ORIRO_OPENAI_CHAT_TOOLS_TIMEOUT_SECONDS: "1",
        PORT: String(port),
        ...env,
      },
...
function runWriteConfig(root: string, env: Record<string, string> = {}) {
  return spawnSync(process.execPath, [writeConfigPath], {
    ...
    env: {
      ...process.env,
      ORIRO_CONFIG_PATH: path.join(root, "oriro.json"),
      ORIRO_GATEWAY_TOKEN: "test-token",
      ORIRO_OPENAI_CHAT_TOOLS_MODEL: "openai/gpt-5.5",
      ORIRO_STATE_DIR: path.join(root, "state"),
      ORIRO_TEST_WORKSPACE_DIR: path.join(root, "workspace"),
      PORT: "18789",
      ...env,
    },
  });
...
function runDockerRunnerAuthPreflight(root: string, env: Record<string, string> = {}) {
  return spawnSync("bash", [dockerRunnerPath], {
    ...
    env: {
      ...
      OPENAI_API_KEY: "",
      OPENAI_BASE_URL: "",
      ORIRO_OPENAI_CHAT_TOOLS_PROFILE_FILE: path.join(root, "missing.profile"),
      ...env,
    },
  });
}
...
describe("scripts/e2e/lib/openai-chat-tools/client.mjs", () => {
  ...
  it("fails auth preflight before Docker build work starts", () => {
    const root = mkdtempSync(path.join(tmpdir(), "oriro-openai-chat-tools-"));
    try {
  ...
  it("treats placeholder profile auth as missing before Docker build work starts", () => {
    ...
    try {
      ...
      const result = runDockerRunnerAuthPreflight(root, {
        ORIRO_OPENAI_CHAT_TOOLS_PROFILE_FILE: profile,
      });
      const output = `${result.stdout}\n${result.stderr}`;
  ...
  it.each([
    ["timeout", "ORIRO_OPENAI_CHAT_TOOLS_TIMEOUT_SECONDS", "1e3"],
    ["body limit", "ORIRO_OPENAI_CHAT_TOOLS_MAX_BODY_BYTES", "64bytes"],
  ])(
    "rejects invalid Docker runner %s before auth or Docker build work starts",
    (_label, envName, value) => {
      const tempDirs: string[] = [];
      const root = makeTempDir(tempDirs, "oriro-openai-chat-tools-");
      try {
  ...
  it("passes normalized timeout and body limits into the Docker runner", () => {
    ...
    expect(runner).toContain(
      "docker_e2e_read_positive_int_env ORIRO_OPENAI_CHAT_TOOLS_TIMEOUT_SECONDS 180",
    );
    expect(runner).toContain(
      "docker_e2e_read_positive_int_env ORIRO_OPENAI_CHAT_TOOLS_MAX_BODY_BYTES 1048576",
    );
    expect(runner).toContain('-e "ORIRO_OPENAI_CHAT_TOOLS_TIMEOUT_SECONDS=$TIMEOUT_SECONDS"');
    expect(runner).toContain('-e "ORIRO_OPENAI_CHAT_TOOLS_MAX_BODY_BYTES=$MAX_BODY_BYTES"');
  });

  it("rejects loose timeout env values instead of parsing numeric prefixes", async () => {
    const result = await runClient(1, {
      ORIRO_OPENAI_CHAT_TOOLS_TIMEOUT_SECONDS: "1e3",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("invalid ORIRO_OPENAI_CHAT_TOOLS_TIMEOUT_SECONDS: 1e3");
  });

  it("rejects loose body limit env values instead of parsing numeric prefixes", async () => {
    const result = await runClient(1, {
      ORIRO_OPENAI_CHAT_TOOLS_MAX_BODY_BYTES: "64bytes",
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("invalid ORIRO_OPENAI_CHAT_TOOLS_MAX_BODY_BYTES: 64bytes");
  });
  ...
  it("writes strict positive timeout and port values into generated config", () => {
    ...
    try {
      const result = runWriteConfig(root, {
        ORIRO_OPENAI_CHAT_TOOLS_TIMEOUT_SECONDS: "240",
        PORT: "19001",
      });

      expect(result.status).toBe(0);
      const config = JSON.parse(readFileSync(path.join(root, "oriro.json"), "utf8"));
  ...
  it("accepts a matching chat completions tool call response", async () => {
    const server = createServer((request, response) => {
      ...
      expect(request.headers["x-oriro-model"]).toBe("openai/gpt-5.4-mini");
      response.writeHead(200, { "content-type": "application/json" });
  ...
  it("caps chat completion response bodies before JSON parsing", async () => {
    ...
    const port = await listen(server);
    try {
      const result = await runClient(port, { ORIRO_OPENAI_CHAT_TOOLS_MAX_BODY_BYTES: "64" });
