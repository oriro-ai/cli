...
const {
  ...
  startManagedServiceUpdateHandoffMock: vi.fn(async () => ({
    ...
    pid: 12345,
    command: "oriro update --yes --channel beta --timeout 2700",
    logPath: "/tmp/oriro-handoff.log",
  })),
}));

vi.mock("./oriro-root.js", async () => {
  const actual = await vi.importActual<typeof import("./oriro-root.js")>("./oriro-root.js");
  return {
    ...actual,
...
type UpdateCheckStateDatabase = Pick<OriroStateKyselyDatabase, "update_check_state">;
...
describe("update-startup", () => {
  let tempDir: string;
  let testState: OriroTestState;

  let resolveOriroPackageRoot: (typeof import("./oriro-root.js"))["resolveOriroPackageRoot"];
  ...
  function readPersistedUpdateCheckState(): PersistedUpdateCheckState | null {
    const { db } = openOriroStateDatabase();
  ...
  function writePersistedUpdateCheckState(state: PersistedUpdateCheckState): void {
    runOriroStateWriteTransaction(({ db }) => {
      const stateDb = getNodeSqliteKysely<UpdateCheckStateDatabase>(db);
      executeSqliteQuerySync(
        db,
  ...
  beforeEach(async () => {
    ...
    testState = await createOriroTestState({
      layout: "state-only",
      prefix: "oriro-update-check-suite-",
      env: {
        ORIRO_NO_AUTO_UPDATE: undefined,
        ORIRO_SERVICE_KIND: undefined,
        ORIRO_SERVICE_MARKER: undefined,
        ORIRO_GATEWAY_SERVICE_PID: undefined,
        ORIRO_LAUNCHD_LABEL: undefined,
        ORIRO_SYSTEMD_UNIT: undefined,
        ORIRO_WINDOWS_TASK_NAME: undefined,
    ...
    if (!loaded) {
      ({ resolveOriroPackageRoot } = await import("./oriro-root.js"));
    ...
    vi.mocked(resolveOriroPackageRoot).mockClear();
  ...
  function mockPackageInstallStatus() {
    vi.mocked(resolveOriroPackageRoot).mockResolvedValue("/opt/oriro");
  ...
  it.each([
    ...
    expect(log.info).toHaveBeenCalledWith(
      `update available (latest): v2.0.0 (current v1.0.0). Run: ${formatCliCommand("oriro update")}`,
    );
  ...
  it("honors ORIRO_NO_AUTO_UPDATE for configured auto-updates", async () => {
    mockPackageUpdateStatus("beta", "2.0.0-beta.1");
    process.env.ORIRO_NO_AUTO_UPDATE = "1";
    const log = { info: vi.fn() };
    const runAutoUpdate = createAutoUpdateSuccessMock();
    ...
    const disabledLogCall = log.info.mock.calls.find(
      ([message]) => message === "auto-update disabled by ORIRO_NO_AUTO_UPDATE",
    );
    expect(disabledLogCall).toEqual([
      "auto-update disabled by ORIRO_NO_AUTO_UPDATE",
      {
  ...
  it("uses current runtime + entrypoint for default auto-update command execution", async () => {
    ...
    process.argv = [process.execPath, "/opt/oriro/dist/entry.js"];
    try {
      await runAutoUpdateCheckWithDefaults({
        cfg: createBetaAutoUpdateConfig(),
      });
    ...
    expect(options.env).toEqual({ ORIRO_AUTO_UPDATE: "1" });
  });
