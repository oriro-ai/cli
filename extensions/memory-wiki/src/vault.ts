...
import { FsSafeError, pathExists, root as fsRoot } from "oriro/plugin-sdk/security-runtime";
...
- Use \`.oriro-wiki/cache/agent-digest.json\` and \`claims.jsonl\` for machine reads; markdown pages are the human view.
`);
}
...
- \`.oriro-wiki/cache/agent-digest.json\` is the agent-facing compiled digest.
