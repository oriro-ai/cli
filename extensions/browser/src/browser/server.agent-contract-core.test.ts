...
describe("profile CRUD endpoints", () => {
  ...
  it("validates profile create/delete endpoints", async () => {
    ...
    const createDuplicate = await realFetch(`${base}/profiles/create`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: "oriro" }),
    });
    ...
    const createOrirod = await realFetch(`${base}/profiles/create`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ name: "legacyorirod", driver: "orirod" }),
    });
    expect(createOrirod.status).toBe(200);
    const createOrirodBody = (await createOrirod.json()) as {
      profile?: string;
      transport?: string;
      cdpPort?: number | null;
      userDataDir?: string | null;
    };
    expect(createOrirodBody.profile).toBe("legacyorirod");
    expect(createOrirodBody.transport).toBe("cdp");
    expect(createOrirodBody.cdpPort).toBeTypeOf("number");
    expect(createOrirodBody.userDataDir).toBeNull();

    const explicitUserDataDir = "/tmp/oriro-brave-profile";
    ...
    const deleteDefault = await realFetch(`${base}/profiles/oriro`, {
      method: "DELETE",
    });
