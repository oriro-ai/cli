...
read_when:
  - You want to connect Oriro to LINE
  - You need LINE webhook + credential setup
  - You want LINE-specific message options
title: LINE
---

LINE connects to Oriro via the LINE Messaging API. The plugin runs as a webhook
...
```bash
oriro plugins install @oriro/line
```

Local checkout (when running from a git repo):

```bash
oriro plugins install ./path/to/local/line-plugin
```
...
- LINE signature verification is body-dependent (HMAC over the raw body), so Oriro applies strict pre-auth body limits and timeout before verification.
- Oriro processes webhook events from the verified raw request bytes. Upstream middleware-transformed `req.body` values are ignored for signature-integrity safety.
...
```bash
oriro pairing list line
oriro pairing approve line <CODE>
```
...
- Inbound media is saved under `~/.oriro/media/inbound/` before it is passed
...
Outbound media URLs must be public HTTPS URLs. Oriro validates the target hostname before handing the URL to LINE and rejects loopback, link-local, and private-network targets.
