...
// Oriro npm postpublish tests validate postpublish verification behavior.
...
} from "../scripts/oriro-npm-postpublish-verify.ts";

const INSTALLED_ROOT_DIST_JS_FILE_SCAN_LIMIT = 10_000;

describe("parseOriroNpmPostpublishVerifyArgs", () => {
  it("supports help and package-manager separators", () => {
    expect(parseOriroNpmPostpublishVerifyArgs(["--help"])).toEqual({
      help: true,
      version: "",
    });
    expect(parseOriroNpmPostpublishVerifyArgs(["--", "2026.3.23"])).toEqual({
      help: false,
      version: "2026.3.23",
    });
  });
  ...
  it("rejects missing, option-like, and extra arguments before verification", () => {
    ...
    expect(() => parseOriroNpmPostpublishVerifyArgs(["2026.3.23", "extra"])).toThrow(
      "Unexpected oriro npm postpublish verifier argument: extra",
    );
  });
});
...
describe("buildPublishedInstallScenarios", () => {
  ...
  it("adds a stable-to-correction upgrade scenario for correction releases", () => {
    expect(buildPublishedInstallScenarios("2026.3.23-2")).toEqual([
      {
        name: "fresh-exact",
        installSpecs: ["oriro@2026.3.23-2"],
        expectedVersion: "2026.3.23-2",
      },
      {
        name: "upgrade-from-base-stable",
        installSpecs: ["oriro@2026.3.23", "oriro@2026.3.23-2"],
        expectedVersion: "2026.3.23-2",
      },
    ]);
  });
...
describe("npm registry provenance verification", () => {
  ...
  it("fetches npm registry JSON with bounded response handling", async () => {
    ...
    await expect(
      fetchRegistryJson("https://registry.example/oriro", {
        fetchImpl,
        timeoutMs: 1234,
      }),
  ...
  it("bounds oversized npm registry response bodies", async () => {
    ...
    await expect(
      ...
      "npm registry https://registry.example/oriro response body exceeded 64 bytes",
    );
  });
  ...
  it("keeps npm registry timeouts active while reading response bodies", async () => {
    ...
    await expect(
      ...
      "npm registry request timed out after 5ms: https://registry.example/oriro",
    );
  });
            ...
            bundle: {
              dsseEnvelope: {
                payload: Buffer.from(
                  JSON.stringify({
                    ...provenancePayload,
                    subject: [{ name: "pkg:npm/oriro@2026.3.24", digest: {} }],
                  }),
                  "utf8",
                ).toString("base64"),
              },
  ...
  it("rejects matching provenance from an untrusted source before Sigstore verification", async () => {
    ...
    ).rejects.toThrow("does not bind 2026.3.23 to the trusted Oriro GitHub release workflow");
    ...
    await expect(
      retryNpmRegistryProvenanceRead(
        async () => {
          ...
          if (attempts < 3) {
            throw new Error(
              "npm registry provenance metadata is incomplete for oriro@2026.3.23.",
            );
          }
          return "verified";
        },
...
describe("buildPublishedInstallCommandArgs", () => {
  it("runs lifecycle scripts for published install verification", () => {
    const args = buildPublishedInstallCommandArgs("/tmp/oriro-prefix", "oriro@2026.4.10");
...
describe("collectInstalledPackageErrors", () => {
  function makeInstalledPackageRoot(): string {
    return mkdtempSync(join(tmpdir(), "oriro-postpublish-package-"));
  }
...
describe("collectInstalledContextEngineRuntimeErrors", () => {
  function makeInstalledPackageRoot(): string {
    return mkdtempSync(join(tmpdir(), "oriro-postpublish-context-engine-"));
  }
...
describe("collectInstalledPluginSdkZodArtifactErrors", () => {
  function withInstalledPackageRoot(run: (packageRoot: string) => void): void {
    const packageRoot = mkdtempSync(join(tmpdir(), "oriro-postpublish-zod-sdk-"));
    try {
      run(packageRoot);
    } finally {
      rmSync(packageRoot, { recursive: true, force: true });
...
describe("normalizeInstalledBinaryVersion", () => {
  it("accepts decorated CLI version output", () => {
    expect(normalizeInstalledBinaryVersion("Oriro 2026.4.8 (9ece252)")).toBe("2026.4.8");
    expect(normalizeInstalledBinaryVersion("Oriro 2026.4.8-beta.1 (9ece252)")).toBe(
      "2026.4.8-beta.1",
    );
    expect(normalizeInstalledBinaryVersion("Oriro 2026.4.8-alpha.1 (9ece252)")).toBe(
      "2026.4.8-alpha.1",
    );
  });
});
...
describe("resolveInstalledBinaryPath", () => {
  it("uses the Unix global bin path on non-Windows platforms", () => {
    expect(resolveInstalledBinaryPath("/tmp/oriro-prefix", "darwin")).toBe(
      "/tmp/oriro-prefix/bin/oriro",
    );
  });

  it("uses the Windows npm shim path on win32", () => {
    expect(resolveInstalledBinaryPath("C:/oriro-prefix", "win32")).toBe(
      "C:\\oriro-prefix\\oriro.cmd",
    );
  });
});
...
describe("resolveInstalledBinaryCommandInvocation", () => {
  it("runs the Unix installed binary directly", () => {
    expect(
      resolveInstalledBinaryCommandInvocation("/tmp/oriro-prefix", ["--version"], {
        platform: "linux",
      }),
    ).toEqual({
      command: "/tmp/oriro-prefix/bin/oriro",
      args: ["--version"],
    });
  });
  ...
  it("wraps the Windows installed npm shim without Node shell argv", () => {
    expect(
      ...
      args: [
        ...
        "/s",
        "/c",
        '""C:\\oriro prefix\\oriro.cmd" agent --message "hello world""',
      ],
...
describe("collectInstalledRootDependencyManifestErrors", () => {
  function makeInstalledPackageRoot(): string {
    return mkdtempSync(join(tmpdir(), "oriro-postpublish-root-deps-"));
  }
