...
describe("maybeRepairSandboxImages", () => {
  ...
  function createSandboxConfig(mode: "off" | "all" | "non-main"): OriroConfig {
    return {
      agents: {
        defaults: {
          sandbox: {
  ...
  function createSandboxConfigWithDockerNetwork(network: string): OriroConfig {
    return {
      agents: {
        defaults: {
          sandbox: {
...
describe("maybeRepairSandboxRegistryFiles", () => {
  ...
  it("warns about legacy registry files without migrating outside doctor --fix", async () => {
    inspectLegacySandboxRegistryFiles.mockResolvedValue([
      {
        kind: "containers",
        registryPath: "/tmp/oriro/sandbox/containers.json",
        shardedDir: "/tmp/oriro/sandbox/containers",
        source: "monolithic",
        exists: true,
        valid: true,
        entries: 2,
    ...
    expect(note).toHaveBeenCalledWith(
      [
        ...
        "- containers monolithic: /tmp/oriro/sandbox/containers.json (2 entries)",
        "Run oriro doctor --fix to migrate them to SQLite.",
      ].join("\n"),
      "Sandbox",
    );
  });
