#!/usr/bin/env node
// Builds the Oriro package artifact used by Docker E2E.
...
function resolvePackedOriroFileName(value) {
  const filename = value.trim();
  if (
    !filename.endsWith(".tgz") ||
    (!filename.startsWith("oriro-") &&
      !filename.includes(":") &&
      !filename.includes("/") &&
      !filename.includes("\\"))
  ) {
  ...
  for (const step of PACKAGE_ARTIFACT_BUILD_STEPS) {
    ...
    await runImpl(step.command, step.args, sourceDir, {
      ...
      timeoutMs: resolveTimeoutMs(
        "ORIRO_DOCKER_PACKAGE_BUILD_TIMEOUT_MS",
...
async function newestOriroTarball(outputDir, packOutput) {
  let fromOutput = "";
  for (const line of packOutput.split(/\r?\n/u)) {
    const filename = resolvePackedOriroFileName(line);
    if (filename) {
      fromOutput = filename;
    }
  }
  ...
  const packed = entries
    .filter((entry) => {
      try {
        return resolvePackedOriroFileName(entry) === entry;
      } catch {
        return false;
      }
    })
...
export async function packOriroPackageForDocker(sourceDir, outputDir, options = {}) {
  ...
  try {
    ...
    packOutput = await runCaptureImpl(
      ...
      sourceDir,
      {
        ...
        timeoutMs: resolveTimeoutMs(
          "ORIRO_DOCKER_PACKAGE_PACK_TIMEOUT_MS",
  ...
  return await newestOriroTarball(outputDir, packOutput);
}
...
async function main() {
  ...
  await run(
    ...
    {
      timeoutMs: resolveTimeoutMs(
        "ORIRO_DOCKER_PACKAGE_INVENTORY_TIMEOUT_MS",
        DEFAULT_PACKAGE_INVENTORY_TIMEOUT_MS,
      ),
    },
  );

  let tarball = await packOriroPackageForDocker(sourceDir, outputDir);
  ...
  await run(
    "node",
    [path.join(ROOT_DIR, "scripts/check-oriro-package-tarball.mjs"), tarball],
    sourceDir,
    {
      timeoutMs: resolveTimeoutMs(
        "ORIRO_DOCKER_PACKAGE_TARBALL_CHECK_TIMEOUT_MS",
