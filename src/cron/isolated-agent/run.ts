...
function hasConfiguredAuthProfiles(cfg: OriroConfig): boolean {
  return (
...
type PreparedCronRunContext = {
  ...
  cfgWithAgentDefaults: OriroConfig;
  agentId: string;
...
function resolveCronActiveRuntimeConfig(cfg: OriroConfig): OriroConfig {
...
async function prepareCronRunContext(params: {
  ...
  const cfgWithAgentDefaults: OriroConfig = {
    ...runtimeCfg,
...
export async function runCronIsolatedAgentTurn(params: {
  ...
  const isFastTestEnv = process.env.ORIRO_TEST_FAST === "1";
