...
Oriro can use Bonjour (mDNS / DNS-SD) to discover an active Gateway (WebSocket endpoint).
...
2. Publish DNS-SD records for `_oriro-gw._tcp` under a dedicated zone
   (example: `oriro.internal.`).
3. Configure Tailscale **split DNS** so your chosen domain resolves via that
   DNS server for clients (including iOS).

Oriro supports any discovery domain; `oriro.internal.` is just an example.
...
```bash
oriro dns setup --apply
```

This installs CoreDNS and configures it to:

- listen on port 53 only on the gateway's Tailscale interfaces
- serve your chosen domain (example: `oriro.internal.`) from `~/.oriro/dns/<domain>.db`

Validate from a tailnet-connected machine:

```bash
dns-sd -B _oriro-gw._tcp oriro.internal.
dig @<TAILNET_IPV4> -p 53 _oriro-gw._tcp.oriro.internal PTR +short
```
...
`_oriro-gw._tcp` in your discovery domain without multicast.
...
- Set `gateway.bind: "tailnet"` in `~/.oriro-ai/cli.json`.
- Restart the Gateway (or restart the macOS menubar app).

## What advertises

Only the Gateway advertises `_oriro-gw._tcp`. LAN multicast advertising is
provided by the bundled `bonjour` plugin when the plugin is enabled; wide-area
DNS-SD publishing remains Gateway-owned.

## Service types

- `_oriro-gw._tcp` - gateway transport beacon (used by macOS/iOS/Android nodes).
...
- Browse instances:
  ...
  ```bash
  dns-sd -B _oriro-gw._tcp local.
  ```

- Resolve one instance (replace `<instance>`):

  ```bash
  dns-sd -L "<instance>" _oriro-gw._tcp local.
  ```
...
in-progress states. If the service never reaches `announced`, Oriro eventually
...
invalid DNS-label character, Oriro falls back to `oriro.local`. Set
`ORIRO_MDNS_HOSTNAME=<name>` before starting the Gateway when you need an
explicit host label.

## Debugging on iOS node

The iOS node uses `NWBrowser` to discover `_oriro-gw._tcp`.
...
discovery plugin for that Oriro config:

```bash
oriro plugins disable bonjour
```

## Docker gotchas

The bundled Bonjour plugin auto-disables LAN multicast advertising in detected
containers when `ORIRO_DISABLE_BONJOUR` is unset. Docker bridge networks
...
- Disabling Bonjour does not change `gateway.bind`; Docker still defaults to
  `ORIRO_GATEWAY_BIND=lan` so the published host port can work.
...
- Reusing the same `ORIRO_CONFIG_DIR` outside Docker does not persist the
  container auto-disable policy.
- Set `ORIRO_DISABLE_BONJOUR=0` only for host networking, macvlan, or another
...
1. Confirm whether the Gateway is running in auto, forced-on, or forced-off mode:
   ...
   ```bash
   docker compose config | grep ORIRO_DISABLE_BONJOUR
   ```
...
4. If you deliberately enabled the Bonjour plugin in Docker and forced advertising
   with `ORIRO_DISABLE_BONJOUR=0`, test multicast from the host:
   ...
   cancellations, restore `ORIRO_DISABLE_BONJOUR=1` and use a direct or
   Tailnet route.
...
- **Advertiser stuck in probing/announcing**: hosts with blocked multicast,
  ...
  non-announced state. Oriro retries a few times and then disables Bonjour
...
- **Docker bridge networking**: Bonjour auto-disables in detected containers.
  Set `ORIRO_DISABLE_BONJOUR=0` only for host, macvlan, or another
...
- `oriro plugins enable bonjour` enables the bundled LAN discovery plugin on hosts where it is not default-enabled.
- `oriro plugins disable bonjour` disables LAN multicast advertising by disabling the bundled plugin.
- `ORIRO_DISABLE_BONJOUR=1` disables LAN multicast advertising without changing plugin config; accepted truthy values are `1`, `true`, `yes`, and `on` (legacy: `ORIRO_DISABLE_BONJOUR`).
- `ORIRO_DISABLE_BONJOUR=0` forces LAN multicast advertising on, including inside detected containers; accepted falsy values are `0`, `false`, `no`, and `off`.
- When the Bonjour plugin is enabled and `ORIRO_DISABLE_BONJOUR` is unset, Bonjour advertises on normal hosts and auto-disables inside detected containers.
- `gateway.bind` in `~/.oriro-ai/cli.json` controls the Gateway bind mode.
- `ORIRO_SSH_PORT` overrides the SSH port when `sshPort` is advertised (legacy: `ORIRO_SSH_PORT`).
- `ORIRO_TAILNET_DNS` publishes a MagicDNS hint in TXT when mDNS full mode is enabled (legacy: `ORIRO_TAILNET_DNS`).
- `ORIRO_CLI_PATH` overrides the advertised CLI path (legacy: `ORIRO_CLI_PATH`).
