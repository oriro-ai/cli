...
export function resolveHooksConfig(cfg: OriroConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: OriroConfig, defaultAgentId: string): Set<string> {
...
/** Extract the hook bearer token from Authorization or x-oriro-token headers. */
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken = normalizeOptionalString(req.headers["x-oriro-token"]) ?? "";
  if (headerToken) {
    return headerToken;
  }
  return undefined;
...
export function resolveHookIdempotencyKey(params: {
  ...
  return (
    ...
    resolveOptionalHookIdempotencyKey(params.headers?.["x-oriro-idempotency-key"]) ||
