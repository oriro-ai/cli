...
function writeStartupMetadataSourceSignatureFixture(rootDir: string): void {
  const fixtures = new Map<string, string>([
    ...
    ["src/cli/banner.ts", "export const banner = 'oriro';\n"],
    [
...
describe("write-cli-startup-metadata", () => {
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempRoot = createTempDir("oriro-startup-metadata-timeout-");
  ...
  it.runIf(process.platform !== "win32")(
    ...
    async () => {
      const tempRoot = createTempDir("oriro-startup-metadata-signal-");
  ...
  it("writes startup metadata with populated root help text when dist falls back to source rendering", async () => {
    const tempRoot = createTempDir("oriro-startup-metadata-");
    ...
    await writeCliStartupMetadata({
      ...
      renderSourceRootHelpText: () => "Usage: oriro\n",
      renderSourceBrowserHelpText: () => "Usage: oriro browser\n",
      renderSourceSecretsHelpText: () => "Usage: oriro secrets\n",
      renderSourceNodesHelpText: () => "Usage: oriro nodes\n",
      renderSourceSubcommandHelpTextRecord: () => ({
        doctor: "Usage: oriro doctor\n",
        gateway: "Usage: oriro gateway\n",
        models: "Usage: oriro models\n",
        plugins: "Usage: oriro plugins\n",
      }),
    });
    ...
    expect(written.browserHelpText).toContain("oriro browser");
    expect(written.secretsHelpText).toContain("Usage:");
    expect(written.secretsHelpText).toContain("oriro secrets");
    expect(written.nodesHelpText).toContain("Usage:");
    expect(written.nodesHelpText).toContain("oriro nodes");
    expect(written.rootHelpText).toContain("Usage:");
    expect(written.rootHelpText).toContain("oriro");
    expect(written.subcommandHelpText.doctor).toContain("oriro doctor");
    expect(written.subcommandHelpText.gateway).toContain("oriro gateway");
    expect(written.subcommandHelpText.models).toContain("oriro models");
    expect(written.subcommandHelpText.plugins).toContain("oriro plugins");
  });

  it("renders independent startup help snapshots concurrently", async () => {
    const tempRoot = createTempDir("oriro-startup-metadata-concurrency-");
    const distDir = path.join(tempRoot, "dist");
    const extensionsDir = path.join(tempRoot, "extensions");
    const outputPath = path.join(distDir, "cli-startup-metadata.json");
    ...
    const writePromise = writeCliStartupMetadata({
      ...
      renderBundledRootHelpText: async () => "Usage: oriro\n",
      renderSourceBrowserHelpText: renderAfterUnblock("browser", "Usage: oriro browser\n"),
      renderSourceSecretsHelpText: renderAfterUnblock("secrets", "Usage: oriro secrets\n"),
      renderSourceNodesHelpText: renderAfterUnblock("nodes", "Usage: oriro nodes\n"),
  ...
  it("regenerates nodes help when bundled canvas CLI help sources change", async () => {
    const tempRoot = createTempDir("oriro-startup-metadata-signature-");
    const distDir = path.join(tempRoot, "dist");
    const extensionsDir = path.join(tempRoot, "extensions");
    const outputPath = path.join(distDir, "cli-startup-metadata.json");
    ...
    const writeMetadata = async (): Promise<void> => {
      await writeCliStartupMetadata({
        ...
        renderSourceNodesHelpText: () => {
          nodesRenderCount += 1;
          return `Usage: oriro nodes ${nodesRenderCount}\n`;
        },
        renderSourceSubcommandHelpTextRecord: () => ({
          doctor: "Usage: oriro doctor\n",
          gateway: "Usage: oriro gateway\n",
    ...
    expect(written.nodesHelpText).toContain("oriro nodes 3");
  });
});
