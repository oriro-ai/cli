...
function formatCapturedWatchLog(text, truncated) {
  return truncated
    ? `[oriro] log truncated to last ${WATCH_LOG_CAPTURE_MAX_CHARS} chars\n${text}`
    : text;
}
...
function buildTimedWatchCommand(pidFilePath, timeFilePath, isolatedHomeDir, port) {
  const isolatedStateDir = path.join(isolatedHomeDir, ".oriro");
  const isolatedConfigPath = path.join(isolatedStateDir, "oriro.json");
  const shellSource = [
    'echo "$$" > "$ORIRO_WATCH_PID_FILE"',
    'mkdir -p "$ORIRO_STATE_DIR"',
    `printf '%s\n' '{"gateway":{"controlUi":{"enabled":false}},"plugins":{"enabled":false}}' > "$ORIRO_CONFIG_PATH"`,
    `exec node scripts/watch-node.mjs gateway --force --allow-unconfigured --port ${String(port)} --token watch-regression-token`,
  ].join("\n");
  const env = {
    ORIRO_WATCH_PID_FILE: pidFilePath,
    HOME: isolatedHomeDir,
    ORIRO_HOME: isolatedHomeDir,
    ORIRO_CONFIG_PATH: isolatedConfigPath,
    ORIRO_STATE_DIR: isolatedStateDir,
...
export async function runTimedWatch(options, outputDir, deps = {}) {
  ...
  const isolatedHomeDir = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-gateway-watch-"));
