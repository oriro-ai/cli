...
const diagnosticEventsStateKey = Symbol.for("oriro.diagnosticEvents.state.v1");
...
function loadRootAliasWithStubs(options?: {
  ...
  const localRequire = ((id: string) => {
    ...
    if (id === "node:os") {
      return {
        tmpdir: () =>
          context.process.env.TMPDIR ?? options?.defaultTmpDir ?? "/tmp/oriro-root-alias-test",
      };
    }
    if (id === "jiti") {
      return {
...
function ensureDiagnosticEventsStateFixture(
  ...
  const state = vm.runInNewContext(
    `({
      marker: Symbol.for("oriro.diagnosticEvents.state.v1"),
      enabled: true,
      seq: 0,
      listeners: new Set(),
      dispatchDepth: 0,
...
describe("plugin-sdk root alias", () => {
  ...
  it("loads legacy root exports on demand and preserves reflection", () => {
    ...
    expect(lazyModule.createJitiOptions.at(-1)?.fsCache).toBe(
      path.join("/tmp/oriro-root-alias-test", "jiti", "oriro", "3.4.5", "12345-678"),
    );
  ...
  it("preserves jiti's tmpdir guard when root-alias TMPDIR resolves to cwd", () => {
    const lazyModule = loadRootAliasWithStubs({
      cwd: "/tmp/oriro-root-alias-cwd",
      defaultTmpDir: "/tmp/oriro-root-alias-fallback",
      env: { TMPDIR: "/tmp/oriro-root-alias-cwd" },
      packageVersion: "3.4.5",
    });

    expect("slowHelper" in lazyModule.moduleExports).toBe(true);
    expect(lazyModule.createJitiOptions.at(-1)?.fsCache).toBe(
      path.join("/tmp/oriro-root-alias-fallback", "jiti", "oriro", "3.4.5", "12345-678"),
    );
  });
  ...
  it("builds scoped and unscoped plugin-sdk aliases for jiti loads", () => {
    ...
    expect(aliasMap["oriro/plugin-sdk"]).toBe(rootAliasPath);
    expect(aliasMap["@oriro/plugin-sdk"]).toBe(rootAliasPath);
    expect(aliasMap["oriro/plugin-sdk/group-access"]).toContain(
  ...
  it("keeps agent-core package imports on the source graph when llm-core dist is missing", () => {
    ...
    expect((lazyModule.moduleExports.slowHelper as () => string)()).toBe("loaded");
    const aliasMap = (lazyModule.createJitiOptions.at(-1)?.alias ?? {}) as Record<string, string>;
    expect(aliasMap["@oriro/llm-core"]).toBe(sourceLlmCorePath);
  });
  ...
  it("keeps bootstrap plugin-sdk aliases deterministic and ignores unsafe subpaths", () => {
    ...
    expect(aliasKeys).toEqual([
      "oriro/plugin-sdk/alpha",
      "@oriro/plugin-sdk/alpha",
      "oriro/plugin-sdk/group-access",
      "@oriro/plugin-sdk/group-access",
      "oriro/plugin-sdk/zeta",
      "@oriro/plugin-sdk/zeta",
      "@oriro/llm-core",
      "@oriro/llm-core/diagnostics",
      "@oriro/llm-core/event-stream",
      "@oriro/llm-core/types",
      "@oriro/llm-core/validation",
      "oriro/plugin-sdk",
      "@oriro/plugin-sdk",
    ]);
  });
  ...
  it("ignores unsafe private local-only plugin-sdk subpaths in the CJS root alias", () => {
    ...
    const lazyModule = loadRootAliasWithStubs({
      env: { ORIRO_ENABLE_PRIVATE_QA_CLI: "1" },
    ...
    expect((lazyModule.moduleExports.slowHelper as () => string)()).toBe("loaded");
    const aliasMap = (lazyModule.createJitiOptions.at(-1)?.alias ?? {}) as Record<string, string>;
    expect(aliasMap["oriro/plugin-sdk/qa-lab"]).toBe(qaLabPath);
    expect(aliasMap["@oriro/plugin-sdk/qa-lab"]).toBe(qaLabPath);
    expect(aliasMap).not.toHaveProperty("oriro/plugin-sdk/../escape");
    expect(aliasMap).not.toHaveProperty("oriro/plugin-sdk/nested/path");
    expect(aliasMap).not.toHaveProperty("oriro/plugin-sdk/ssrf-runtime-internal");
    expect(aliasMap).not.toHaveProperty("@oriro/plugin-sdk/ssrf-runtime-internal");
  });
  ...
  it("keeps non-QA private local-only plugin-sdk subpaths out of the CJS root alias", () => {
    ...
    expect((lazyModule.moduleExports.slowHelper as () => string)()).toBe("loaded");
    const aliasMap = (lazyModule.createJitiOptions.at(-1)?.alias ?? {}) as Record<string, string>;
    expect(aliasMap).not.toHaveProperty("oriro/plugin-sdk/codex-mcp-projection");
    expect(aliasMap).not.toHaveProperty("@oriro/plugin-sdk/codex-mcp-projection");
    expect(aliasMap).not.toHaveProperty("oriro/plugin-sdk/qa-runtime");
  });
  ...
  it("builds source plugin-sdk subpath aliases through the wider source extension family", () => {
    ...
    expect((lazyModule.moduleExports.slowHelper as () => string)()).toBe("loaded");
    const aliasMap = (lazyModule.createJitiOptions.at(-1)?.alias ?? {}) as Record<string, string>;
    expect(aliasMap["oriro/plugin-sdk/channel-runtime"]).toBe(
  ...
  it("bridges diagnostic listeners through shared process state when the lazy module is isolated", () => {
    ...
    const state = lazyModule.globalContext[Symbol.for("oriro.diagnosticEvents.state.v1")] as {
      listeners: Set<(event: { type: string }, metadata: { trusted: boolean }) => void>;
    };
