...
import { AcpRuntimeError } from "@oriro/acp-core/runtime/errors";
...
export function isAcpEnabledByPolicy(cfg: OriroConfig): boolean {
  return cfg.acp?.enabled !== false;
}

/** Resolves the effective dispatch policy state for inbound ACP routing. */
function resolveAcpDispatchPolicyState(cfg: OriroConfig): AcpDispatchPolicyState {
...
export function resolveAcpDispatchPolicyMessage(cfg: OriroConfig): string | null {
...
export function resolveAcpDispatchPolicyError(cfg: OriroConfig): AcpRuntimeError | null {
...
export function resolveAcpExplicitTurnPolicyError(cfg: OriroConfig): AcpRuntimeError | null {
  if (isAcpEnabledByPolicy(cfg)) {
    return null;
  }
...
function isAcpAgentAllowedByPolicy(cfg: OriroConfig, agentId: string): boolean {
