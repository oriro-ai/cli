...
const REMOTE_ROOT = "/tmp/oriro-telegram-user-crabbox";
...
export function readTelegramUserProofLogTailBytes(env: NodeJS.ProcessEnv = process.env): number {
  return readPositiveIntEnv("ORIRO_TELEGRAM_USER_PROOF_LOG_TAIL_BYTES", 256 * 1024, env);
}
...
export function parseArgs(argvInput: string[]): Options {
  ...
  const opts: Options = {
    ...
    ttl: "120m",
    userDriverScript:
      trimToValue(process.env.ORIRO_TELEGRAM_USER_DRIVER_SCRIPT) ?? DEFAULT_USER_DRIVER,
  };
...
function childProcessBaseEnv() {
  const keys = [
    ...
    "LC_ALL",
    "NODE_OPTIONS",
    "ORIRO_BUILD_PRIVATE_QA",
    "ORIRO_ENABLE_PRIVATE_QA_CLI",
    "PATH",
    "PNPM_HOME",
    "SHELL",
    "TEMP",
...
function gatewayEnv(params: { configPath: string; stateDir: string; sutToken: string }) {
  return {
    ...childProcessBaseEnv(),
    OPENAI_API_KEY: "sk-oriro-e2e-mock",
    ORIRO_CONFIG_PATH: params.configPath,
    ORIRO_STATE_DIR: params.stateDir,
    TELEGRAM_BOT_TOKEN: params.sutToken,
  };
}

export function createOriroGatewaySpawnSpec(params: {
  ...
  const spec = createPnpmRunnerSpawnSpec({
    ...
    pnpmArgs: ["oriro", "gateway", "--port", String(params.gatewayPort)],
  });
  return {
    args: spec.args,
...
function writeSutConfig(params: {
  ...
  const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-tg-crabbox-sut-"));
  const stateDir = path.join(tempRoot, "state");
  const workspace = path.join(tempRoot, "workspace");
  fs.mkdirSync(stateDir, { recursive: true });
  fs.mkdirSync(workspace, { recursive: true });
  const configPath = path.join(tempRoot, "oriro.json");
  const config = {
    agents: {
      defaults: {
...
type StartLocalSutDeps = {
  createGatewaySpawnSpec?: typeof createOriroGatewaySpawnSpec;
...
export async function startLocalSut(
  ...
  const createGatewaySpawnSpec = deps.createGatewaySpawnSpec ?? createOriroGatewaySpawnSpec;
...
async function startLocalSutDaemon(params: {
  ...
  try {
    ...
    const gatewaySpec = createOriroGatewaySpawnSpec({
      env: gatewayEnvVars,
      gatewayPort: params.gatewayPort,
      repoRoot: params.repoRoot,
    });
...
tdlib_url=${tdlibUrl}
setup_step_timeout_kill_after="\${ORIRO_TELEGRAM_USER_SETUP_KILL_AFTER_SECONDS:-30}s"
apt_timeout="\${ORIRO_TELEGRAM_USER_APT_TIMEOUT_SECONDS:-900}s"
download_timeout="\${ORIRO_TELEGRAM_USER_DOWNLOAD_TIMEOUT_SECONDS:-600}"
download_connect_timeout="\${ORIRO_TELEGRAM_USER_DOWNLOAD_CONNECT_TIMEOUT_SECONDS:-15}"
download_retries="\${ORIRO_TELEGRAM_USER_DOWNLOAD_RETRIES:-3}"
download_retry_delay="\${ORIRO_TELEGRAM_USER_DOWNLOAD_RETRY_DELAY_SECONDS:-5}"
tdlib_clone_timeout="\${ORIRO_TELEGRAM_USER_TDLIB_CLONE_TIMEOUT_SECONDS:-600}s"
tdlib_build_timeout="\${ORIRO_TELEGRAM_USER_TDLIB_BUILD_TIMEOUT_SECONDS:-1800}s"
run_setup_step() {
  local label="$1"
...
async function startSession(root: string, opts: Options, outputDir: string) {
  ...
  const hasConvexEnv =
    trimToValue(process.env.ORIRO_QA_CONVEX_SITE_URL) &&
    trimToValue(process.env.ORIRO_QA_CONVEX_SECRET_CI);
  ...
  try {
    ...
    return {
      ...
      commands: {
        send: `oriro-telegram-user-crabbox-proof send --session ${path.relative(root, pathname)} --text '/status'`,
        view: `oriro-telegram-user-crabbox-proof view --session ${path.relative(root, pathname)} --message-id <message-id>`,
        run: `oriro-telegram-user-crabbox-proof run --session ${path.relative(root, pathname)} -- bash -lc 'source ${REMOTE_ROOT}/env.sh && python3 ${REMOTE_ROOT}/user-driver.py transcript --limit 20 --json'`,
        finish: `oriro-telegram-user-crabbox-proof finish --session ${path.relative(root, pathname)} --preview-crop telegram-window`,
      },
    };
  } catch (error) {
...
async function main() {
  ...
  const localRoot = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-telegram-crabbox-"));
  const summary: JsonObject = {
    artifacts: {},
