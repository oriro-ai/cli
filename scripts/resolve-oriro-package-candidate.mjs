...
const DEFAULT_OUTPUT_NAME = "oriro-current.tgz";
...
const TRUSTED_PACKAGE_SOURCE_TOKEN_ENV = "ORIRO_TRUSTED_PACKAGE_TOKEN";
...
export const ORIRO_PACKAGE_SPEC_RE =
  /^oriro@(alpha|beta|latest|[0-9]{4}\.[1-9][0-9]*\.[1-9][0-9]*(-[1-9][0-9]*|-(alpha|beta)\.[1-9][0-9]*)?)$/u;

function usage() {
  return `Usage: node scripts/resolve-oriro-package-candidate.mjs --source <ref|npm|url|trusted-url|artifact> --output-dir <dir> [options]
...
function resolvePackedOriroTarballFilename(value) {
  const filename = typeof value === "string" ? value.trim() : "";
  if (
    !/^oriro-[A-Za-z0-9._-]+\.tgz$/u.test(filename) ||
    filename.includes("\0") ||
...
export function resolveNpmPackageCandidatePackRunner(packageSpec, outputDir, params = {}) {
  validateOriroPackageSpec(packageSpec);
...
async function preparePackageSourceWorktree(ref) {
  ...
  const sourceDir = path.join(
    ...
    `oriro-package-source-${process.pid}`,
  );
...
async function moveNewestPackedTarball(outputDir, packOutput, outputName) {
  ...
  if (Array.isArray(parsed)) {
    ...
    if (packedFilename) {
      filename = resolvePackedOriroTarballFilename(packedFilename);
    }
  }
  if (!filename) {
    for (const line of packOutput.split(/\r?\n/u)) {
      ...
      if (
        ...
        (trimmed.startsWith("oriro-") ||
          trimmed.includes(":") ||
          trimmed.includes("/") ||
          trimmed.includes("\\"))
      ) {
        filename = resolvePackedOriroTarballFilename(trimmed);
      }
    }
  }
  if (!filename) {
    ...
    filename = entries
      .filter((entry) => {
        try {
          return resolvePackedOriroTarballFilename(entry) === entry;
        } catch {
          return false;
        }
      })
...
async function cleanPackedOriroTarballs(outputDir) {
  let entries;
  try {
...
export const cleanPackedOriroTarballsForTest = cleanPackedOriroTarballs;
...
async function resolveCandidate(options) {
  ...
  try {
    if (options.source === "ref") {
      ...
      await cleanPackedOriroTarballs(outputDir);
  ...
  if (pkg.name !== "oriro") {
    throw new Error(`package candidate must be named "oriro"; got: ${pkg.name || "<missing>"}`);
  }
  if (!pkg.version) {
