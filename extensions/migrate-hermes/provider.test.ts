...
import { createCapturedPluginRegistration } from "oriro/plugin-sdk/plugin-test-runtime";
...
describe("Hermes migration provider", () => {
  ...
  it("resolves tilde source paths against the OS home when ORIRO_HOME is set", () => {
    const previous = process.env.ORIRO_HOME;
    process.env.ORIRO_HOME = path.join(path.sep, "tmp", "oriro-home");
    try {
      ...
      if (previous === undefined) {
        ...
        process.env.ORIRO_HOME = previous;
      }
    }
  });
