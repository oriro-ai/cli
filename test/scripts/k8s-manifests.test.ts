...
describe("k8s manifests", () => {
  ...
  it("keeps gateway service selectors and ports aligned with deployment labels", () => {
    ...
    expect(matchLabels).toEqual({ app: "oriro" });
  ...
  it("keeps deployment mounts, secrets, and security posture deployable", () => {
    ...
    expect(findNamed(env, "ORIRO_CONFIG_DIR")).toMatchObject({ value: "/home/node/.oriro" });
    expect(findNamed(env, "ORIRO_GATEWAY_TOKEN")).toMatchObject({
      valueFrom: { secretKeyRef: { key: "ORIRO_GATEWAY_TOKEN", name: "oriro-secrets" } },
    });
    expect(findNamed(volumes, "oriro-home")).toMatchObject({
      persistentVolumeClaim: { claimName: "oriro-home-pvc" },
    });
    expect(findNamed(volumes, "config")).toMatchObject({ configMap: { name: "oriro-config" } });
  ...
  it("keeps config and persistence manifests aligned with the gateway", () => {
    ...
    const config = JSON.parse(String(data["oriro.json"])) as Record<string, unknown>;
    const gateway = asRecord(config.gateway, "oriro config gateway");
    const auth = asRecord(gateway.auth, "oriro config auth");
    const agents = asRecord(config.agents, "oriro config agents");
    const defaults = asRecord(agents.defaults, "oriro config agent defaults");
    ...
    expect(configMap).toMatchObject({
      apiVersion: "v1",
      kind: "ConfigMap",
      metadata: { name: "oriro-config" },
    });
    expect(gateway).toMatchObject({ mode: "local", port: 18789 });
    expect(auth).toMatchObject({ mode: "token" });
    expect(defaults).toMatchObject({ workspace: "~/.oriro/workspace" });
    expect(data["AGENTS.md"]).toContain("Oriro Assistant");
    expect(pvc).toMatchObject({
      apiVersion: "v1",
      kind: "PersistentVolumeClaim",
      metadata: { name: "oriro-home-pvc" },
    });
