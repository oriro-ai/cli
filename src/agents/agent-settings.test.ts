...
describe("isSilentOverflowProneModel", () => {
  ...
  // oriro#75799 reporter's setup: an OpenAI-compatible in-house gateway
  ...
  // string. False positives only disable Oriro runtime's secondary compaction path;
  // Oriro's preemptive compaction continues to handle real overflow.
...
describe("applyAgentAutoCompactionGuard", () => {
  // Direct repro of oriro#75799: shared model runtime's silent-overflow detection misfires
  // on a successful turn against z.ai-style providers, triggering Oriro runtime's
  ...
  // state.messages intact; Oriro's preemptive compaction continues to
  ...
  // Default-mode runs against ordinary providers must keep Oriro runtime's auto-compaction
  // enabled. Disabling it across the board would silently remove Oriro runtime's
