...
import type { OpenKeyedStoreOptions } from "oriro/plugin-sdk/plugin-state-runtime";
import {
...
const { cleanupOriroOwnedAcpxProcessTreeMock } = vi.hoisted(() => ({
  cleanupOriroOwnedAcpxProcessTreeMock: vi.fn(
    async (): Promise<{
...
const { reapStaleOriroOwnedAcpxOrphansMock } = vi.hoisted(() => ({
  reapStaleOriroOwnedAcpxOrphansMock: vi.fn(
    async (): Promise<{
      inspectedPids: number[];
      terminatedPids: number[];
      skippedReason?: string;
...
vi.mock("./process-reaper.js", () => ({
  cleanupOriroOwnedAcpxProcessTree: cleanupOriroOwnedAcpxProcessTreeMock,
  reapStaleOriroOwnedAcpxOrphans: reapStaleOriroOwnedAcpxOrphansMock,
}));

import { getAcpRuntimeBackend } from "../runtime-api.js";
import type { OriroPluginServiceContext } from "../runtime-api.js";
...
const previousEnv = {
  ORIRO_ACPX_RUNTIME_STARTUP_PROBE: process.env.ORIRO_ACPX_RUNTIME_STARTUP_PROBE,
  ORIRO_SKIP_ACPX_RUNTIME: process.env.ORIRO_SKIP_ACPX_RUNTIME,
  ORIRO_SKIP_ACPX_RUNTIME_PROBE: process.env.ORIRO_SKIP_ACPX_RUNTIME_PROBE,
};
...
async function makeTempDir(): Promise<string> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-acpx-service-"));
  tempDirs.push(dir);
  return dir;
}
...
afterEach(async () => {
  ...
  cleanupOriroOwnedAcpxProcessTreeMock.mockClear();
  reapStaleOriroOwnedAcpxOrphansMock.mockClear();
  acpxRuntimeConstructorMock.mockClear();
  createAgentRegistryMock.mockClear();
  createFileSessionStoreMock.mockClear();
  restoreEnv("ORIRO_ACPX_RUNTIME_STARTUP_PROBE");
  restoreEnv("ORIRO_SKIP_ACPX_RUNTIME");
  restoreEnv("ORIRO_SKIP_ACPX_RUNTIME_PROBE");
  for (const dir of tempDirs.splice(0)) {
    await fs.rm(dir, { recursive: true, force: true });
  }
});

function createServiceContext(workspaceDir: string): OriroPluginServiceContext {
  return {
    workspaceDir,
    stateDir: path.join(workspaceDir, ".oriro-plugin-state"),
    config: {},
    logger: {
      info: vi.fn(),
      warn: vi.fn(),
...
function createOpenKeyedStore(ctx: OriroPluginServiceContext) {
  const env = { ...process.env, ORIRO_STATE_DIR: ctx.stateDir };
...
function openGatewayInstanceStore(ctx: OriroPluginServiceContext) {
...
function openProcessLeaseStore(ctx: OriroPluginServiceContext) {
...
describe("createAcpxRuntimeService", () => {
  ...
  it("skips the startup probe and does not advertise backend health when explicitly disabled", async () => {
    process.env.ORIRO_ACPX_RUNTIME_STARTUP_PROBE = "0";
    delete process.env.ORIRO_SKIP_ACPX_RUNTIME_PROBE;
    const workspaceDir = await makeTempDir();
  ...
  it("waits for the embedded runtime startup probe before resolving by default", async () => {
    delete process.env.ORIRO_ACPX_RUNTIME_STARTUP_PROBE;
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
  ...
  it("reaps stale ACPX process leases from the generated wrapper root at startup", async () => {
    ...
    cleanupOriroOwnedAcpxProcessTreeMock.mockResolvedValueOnce({
    ...
    expect(cleanupOriroOwnedAcpxProcessTreeMock).toHaveBeenCalledWith({
      rootPid: 101,
    ...
    expect(ctx.logger.info).toHaveBeenCalledWith("reaped 2 stale Oriro-owned ACPX processes");
  ...
  it("runs wrapper-root orphan cleanup before dropping pending ACPX leases", async () => {
    ...
    await openProcessLeaseStore(ctx).register(lease.leaseId, lease);
    reapStaleOriroOwnedAcpxOrphansMock.mockResolvedValueOnce({
    ...
    expect(cleanupOriroOwnedAcpxProcessTreeMock).not.toHaveBeenCalled();
    expect(reapStaleOriroOwnedAcpxOrphansMock).toHaveBeenCalledWith({
      wrapperRoot,
      deps: processCleanupDeps,
    });
    expect(ctx.logger.info).toHaveBeenCalledWith("reaped 2 stale Oriro-owned ACPX processes");
  ...
  it("runs the embedded runtime probe at startup when explicitly enabled and reports health", async () => {
    process.env.ORIRO_ACPX_RUNTIME_STARTUP_PROBE = "1";
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
    const probeAvailability = vi.fn(async () => {});
    const runtime = createMockRuntime({
  ...
  it("lets the skip env override the opt-in embedded runtime startup probe without advertising health", async () => {
    process.env.ORIRO_ACPX_RUNTIME_STARTUP_PROBE = "1";
    process.env.ORIRO_SKIP_ACPX_RUNTIME_PROBE = "1";
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
    const probeAvailability = vi.fn(async () => {});
    const runtime = createMockRuntime({
  ...
  it("can skip the embedded runtime backend via env", async () => {
    process.env.ORIRO_SKIP_ACPX_RUNTIME = "1";
    const workspaceDir = await makeTempDir();
    const ctx = createServiceContext(workspaceDir);
    ...
    expect(ctx.logger.info).toHaveBeenCalledWith(
      "skipping embedded acpx runtime backend (ORIRO_SKIP_ACPX_RUNTIME=1)",
    );
  });
});
