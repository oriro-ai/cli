...
import type { OriroPluginApi } from "oriro/plugin-sdk/plugin-entry";
...
vi.mock("oriro/plugin-sdk/memory-host-search", () => ({
...
describe("active-memory plugin", () => {
  ...
  const api: any = {
    ...
    runtime: {
      agent: {
        runEmbeddedAgent,
        session: {
          resolveStorePath: vi.fn(() => "/tmp/oriro-session-store.json"),
  ...
  beforeEach(async () => {
    ...
    stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-active-memory-test-"));
    configFile = {
      plugins: {
        entries: {
          "active-memory": {
    ...
    plugin.register(api as unknown as OriroPluginApi);
  });
  ...
  it("keeps subagent transcripts off disk by default by using a temp session file", async () => {
    ...
    expect(sessionFile).toMatch(/oriro-active-memory-.*\/session\.jsonl$/);
