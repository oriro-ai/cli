 ...
 * ORIRO_FALLBACK_SKIP_TTL_MS.
 */
const DEFAULT_FALLBACK_SKIP_TTL_MS = 0;
const FALLBACK_SKIP_TTL_ENV = "ORIRO_FALLBACK_SKIP_TTL_MS";
...
function getState(): SkipCacheState {
  const globalStore = globalThis as typeof globalThis & {
    oriroFallbackSkipCache?: SkipBySession;
    oriroFallbackSkipCacheState?: SkipCacheState;
  };
  if (!globalStore.oriroFallbackSkipCacheState) {
    ...
    const buckets = globalStore.oriroFallbackSkipCache ?? new Map();
    globalStore.oriroFallbackSkipCacheState = {
      buckets,
      lastGlobalPruneAtMs: 0,
    };
    globalStore.oriroFallbackSkipCache = buckets;
  }
  return globalStore.oriroFallbackSkipCacheState;
}
