...
describe("createAgentToolsSandboxContext", () => {
  it("provides stable defaults for agent-tools sandbox tests", () => {
    ...
    expect(sandbox.containerName).toBe("oriro-sbx-test");
    expect(sandbox.containerWorkdir).toBe("/workspace");
    expect(sandbox.docker.image).toBe("oriro-sandbox:bookworm-slim");
    expect(sandbox.docker.containerPrefix).toBe("oriro-sbx-");
