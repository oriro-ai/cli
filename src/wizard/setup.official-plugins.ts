...
function isInstalledOrConfigured(config: OriroConfig, pluginId: string): boolean {
...
function formatInstallHint(install: PluginPackageInstall): string {
  if (install.orirohubSpec && install.npmSpec) {
    return install.defaultChoice === "orirohub"
      ? "OriroHub, with npm fallback"
      : "npm, with OriroHub fallback";
  }
  if (install.orirohubSpec) {
    return "OriroHub";
  }
  if (install.npmSpec) {
    return "npm";
