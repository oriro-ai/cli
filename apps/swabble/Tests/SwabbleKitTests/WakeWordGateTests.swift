...
final class WakeWordGateTests: XCTestCase {
    func testMatchRequiresGapAfterTrigger() {
        let transcript = "hey orirod do thing"
        ...
        let config = WakeWordGateConfig(triggers: ["orirod"], minPostTriggerGap: 0.3)
    ...
    func testMatchHandlesMultiWordTriggers() {
        let transcript = "hey orirod do it"
        let segments = makeSegments(
            transcript: transcript,
            words: [
                ("hey", 0.0, 0.1),
        ...
        let config = WakeWordGateConfig(triggers: ["hey orirod"], minPostTriggerGap: 0.3)
        let match = WakeWordGate.match(transcript: transcript, segments: segments, config: config)
    ...
    func testMatchPrefersMostSpecificTriggerWhenOverlapping() {
        ...
        let config = WakeWordGateConfig(triggers: ["orirod", "hey orirod"], minPostTriggerGap: 0.3)
        let match = WakeWordGate.match(transcript: transcript, segments: segments, config: config)
        XCTAssertEqual(match?.trigger, "hey orirod")
    }
    ...
    func testCommandTextHandlesForeignRangeIndices() {
        ...
        let segments = [
            ...
            WakeWordSegment(text: "orirod", start: 0.2, duration: 0.1, range: transcript.range(of: "orirod")),
