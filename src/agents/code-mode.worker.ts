...
(() => {
  ...
  const catalog = Array.isArray(globalThis.__oriroCatalog) ? globalThis.__oriroCatalog : [];
  const apiFiles = Array.isArray(globalThis.__oriroApiFiles) ? globalThis.__oriroApiFiles : [];
  const namespaceDescriptors = Array.isArray(globalThis.__oriroNamespaces) ? globalThis.__oriroNamespaces : [];
  ...
  function request(method, args) {
    const id = String(globalThis.__oriroHostRequest(String(method), JSON.stringify(safe(args ?? []))));
  ...
  Object.defineProperties(globalThis, {
    ...
    __oriroSettleBridge: { value: settle },
    __oriroTakeOutput: { value: () => output.splice(0) },
  });
})();
`;

function buildUserSource(code: string): string {
  return `globalThis.__oriroResult = (async () => {\n${code}\n})()`;
}
...
async function createVm(params: {
  ...
  try {
    vm.setProp(vm.global, "__oriroCatalog", catalogHandle);
  } finally {
    catalogHandle.dispose();
  }
  const namespacesHandle = vm.hostToHandle(params.namespaces);
  try {
    vm.setProp(vm.global, "__oriroNamespaces", namespacesHandle);
  } finally {
    namespacesHandle.dispose();
  }
  const apiFilesHandle = vm.hostToHandle(params.apiFiles);
  try {
    vm.setProp(vm.global, "__oriroApiFiles", apiFilesHandle);
  } finally {
  ...
  try {
    vm.setProp(vm.global, "__oriroHostRequest", hostRequest);
  } finally {
    hostRequest.dispose();
  }
  vm.evalCode(CONTROLLER_SOURCE, "oriro-code-mode:controller.js").dispose();
...
function takeOutput(vm: QuickJS): unknown[] {
  const take = vm.global.getProp("__oriroTakeOutput");
  try {
...
function getResultHandle(vm: QuickJS): JSValueHandle {
  return vm.global.getProp("__oriroResult");
}
...
async function runResume(input: Extract<CodeModeWorkerInput, { kind: "resume" }>) {
  ...
  let output: unknown[] = [];
  try {
    const settle = vm.global.getProp("__oriroSettleBridge");
    try {
