...
struct VoiceWakeRuntimeTests {
    ...
    @Test func `matched trigger finds trigger not at transcript start`() {
        let triggers = ["oriro"]
        let text = "uh oriro"
        #expect(VoiceWakeRuntime._testMatchedTriggerWord(text, triggers: triggers) == "oriro")
    }
    ...
    @Test func `matched trigger prefers most specific overlapping phrase`() {
        let triggers = ["oriro", "hey oriro"]
        let text = "hey oriro"
        #expect(VoiceWakeRuntime._testMatchedTriggerWord(text, triggers: triggers) == "hey oriro")
    }
    ...
    @Test func `text only fallback populates matched trigger`() {
        let transcript = "hey oriro do thing"
        let config = WakeWordGateConfig(triggers: ["oriro"], minCommandLength: 1)
        let match = VoiceWakeRecognitionDebugSupport.textOnlyFallbackMatch(
            transcript: transcript,
            triggers: ["oriro"],
            config: config,
            trimWake: VoiceWakeRuntime._testTrimmedAfterTrigger)
        #expect(match?.trigger == "oriro")
    }

    @Test func `text only fallback keeps the first trigger phrase when later words match another trigger`() {
        let transcript = "oriro tell me about computer vision"
        let config = WakeWordGateConfig(triggers: ["oriro", "computer"], minCommandLength: 1)
        let match = VoiceWakeRecognitionDebugSupport.textOnlyFallbackMatch(
            transcript: transcript,
            triggers: ["oriro", "computer"],
            config: config,
            trimWake: VoiceWakeRuntime._testTrimmedAfterTrigger)
        #expect(match?.trigger == "oriro")
    }
    ...
    @Test func `gate requires gap between trigger and command`() {
        ...
        let config = WakeWordGateConfig(triggers: ["oriro"], minPostTriggerGap: 0.3)
    ...
    @Test func `gate command text handles foreign string ranges`() {
        ...
        let segments = [
            ...
            WakeWordSegment(text: "oriro", start: 0.2, duration: 0.1, range: transcript.range(of: "oriro")),
