/** Tests projecting Oriro user MCP servers into Codex app-server config. */
...
describe("buildCodexUserMcpServersThreadConfigPatch", () => {
  it("returns undefined when cfg has no mcp.servers (regression: #80814)", () => {
    ...
    expect(buildCodexUserMcpServersThreadConfigPatch({} as OriroConfig)).toBeUndefined();
    expect(
      buildCodexUserMcpServersThreadConfigPatch({ mcp: {} } as OriroConfig),
    ).toBeUndefined();
    expect(
      buildCodexUserMcpServersThreadConfigPatch({ mcp: { servers: {} } } as OriroConfig),
    ).toBeUndefined();
  });
  ...
  it("filters Codex-scoped user MCP servers by Oriro agent id", () => {
    // Agent-scoped MCP servers should follow the active Oriro agent, while
  ...
  it("omits scoped Codex MCP servers when no Oriro agent id is available", () => {
    const patch = buildCodexUserMcpServersThreadConfigPatch({
      mcp: {
        servers: {
          atlas: {
