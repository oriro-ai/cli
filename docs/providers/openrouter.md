---
summary: "Use OpenRouter's unified API to access many models in Oriro"
read_when:
  - You want a single API key for many LLMs
  - You want to run models via OpenRouter in Oriro
  ...
  <Tab title="OAuth">
    <Steps>
      <Step title="Run OAuth onboarding">
        ```bash
        oriro onboard --auth-choice openrouter-oauth
        ```

        Oriro opens OpenRouter's browser sign-in flow, exchanges the PKCE
        code for an OpenRouter API key, and stores that key in the default
        OpenRouter auth profile. On remote/headless hosts, Oriro prints the
      ...
      <Step title="(Optional) Switch to a specific model">
        ...
        ```bash
        oriro models set openrouter/<provider>/<model>
        ```
  ...
  <Tab title="API key">
    <Steps>
      ...
      <Step title="Run API-key onboarding">
        ```bash
        oriro onboard --auth-choice openrouter-api-key
        ```
      </Step>
      <Step title="(Optional) Switch to a specific model">
        Onboarding defaults to `openrouter/auto`. Pick a concrete model later:
...
Oriro submits text-to-video and image-to-video jobs to OpenRouter, polls
...
`google/lyria-3-clip-preview`. Oriro sends `modalities: ["text",
...
Oriro sends OpenRouter STT requests as JSON with base64 audio under
`input_audio` (OpenRouter STT contract), not as multipart OpenAI form uploads.

## Fusion router

Use OpenRouter Fusion when you want one Oriro model ref to ask several
...
the upstream model slug is `openrouter/fusion`, the Oriro model ref includes
both the Oriro provider prefix and the upstream OpenRouter namespace:

```bash
oriro models set openrouter/openrouter/fusion
```
...
`"required"` in normal Oriro agent/chat turns to try to force Fusion;
Oriro turns may include Oriro tool definitions, and a top-level required
tool choice can require one of those tools instead of the Fusion router. When
this Fusion plugin config is present, Oriro also adds a sanitized
...
Fusion is slower by design. OpenRouter may send the same Oriro prompt to
...
OAuth is a PKCE login flow that issues an OpenRouter API key, so Oriro stores
...
```bash
oriro models auth login --provider openrouter --method oauth
```

Use `oriro models auth login --provider openrouter --method api-key` when
you want to paste a key you created manually at OpenRouter.

On real OpenRouter requests (`https://openrouter.ai/api/v1`), Oriro also adds
...
<Warning>
If you repoint the OpenRouter provider at some other proxy or base URL, Oriro
...
<AccordionGroup>
  <Accordion title="Response caching">
    ...
    Oriro sends `X-OpenRouter-Cache: true` and, when configured,
  ...
  <Accordion title="Anthropic cache markers">
    ...
    OpenRouter-specific Anthropic `cache_control` markers that Oriro uses for
  ...
  <Accordion title="Thinking / reasoning injection">
    On supported non-`auto` routes, Oriro maps the selected thinking level to
  ...
  <Accordion title="DeepSeek V4 reasoning replay">
    ...
    required follow-up shape. Oriro sends OpenRouter-supported
  ...
  <Accordion title="Gemini-backed routes">
    Gemini-backed OpenRouter refs stay on the proxy-Gemini path: Oriro keeps
  ...
  <Accordion title="Provider routing metadata">
    ...
    Oriro forwards that object to OpenRouter as the request `provider`
