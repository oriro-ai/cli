---
summary: "CLI reference for `oriro agents` (list/add/delete/bindings/bind/unbind/set identity)"
read_when:
...
oriro agents add work --workspace ~/.oriro/workspace-work
oriro agents add work --workspace ~/.oriro/workspace-work --bind telegram:*
oriro agents add ops --workspace ~/.oriro/workspace-ops --bind telegram:ops --non-interactive
oriro agents bindings
oriro agents bind --agent work --bind telegram:ops
oriro agents unbind --agent work --bind telegram:ops
oriro agents set-identity --workspace ~/.oriro/workspace --from-identity
oriro agents set-identity --agent main --avatar avatars/oriro.png
oriro agents delete work
```
...
If you also want different visible skills per agent, configure `agents.defaults.skills` and `agents.list[].skills` in `oriro.json`. See [Skills config](/tools/skills-config) and [Configuration reference](/gateway/config-agents#agents-defaults-skills).

List bindings:

```bash
oriro agents bindings
oriro agents bindings --agent work
oriro agents bindings --json
```

Add bindings:

```bash
oriro agents bind --agent work --bind telegram:ops --bind discord:guild-a
```

You can also add bindings when creating an agent:

```bash
oriro agents add work --workspace ~/.oriro/workspace-work --bind telegram:* --bind discord:*
```

If you omit `accountId` (`--bind <channel>`), Oriro resolves it from plugin setup hooks, forced account binding, or the channel's configured account count.

If you omit `--agent` for `bind` or `unbind`, Oriro targets the current default agent.
...
- If the same agent already has a matching channel binding without `accountId`, and you later bind with an explicit or resolved `accountId`, Oriro upgrades that existing binding in place instead of adding a duplicate.

Examples:

```bash
# match all accounts on the channel
oriro agents bind --agent work --bind telegram:*
...
oriro agents bind --agent work --bind telegram:alerts
```
...
```bash
oriro agents unbind --agent work --bind telegram:ops
oriro agents unbind --agent work --all
```
...
Running `oriro agents` with no subcommand is equivalent to `oriro agents list`.
...
- Example path: `~/.oriro/workspace/IDENTITY.md`
...
```bash
oriro agents set-identity --agent main --name "Oriro" --emoji "ORIRO" --avatar avatars/oriro.png
```
