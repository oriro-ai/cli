...
describe("mcp loopback server", () => {
  it("passes session, account, message channel, and inbound event headers into shared tool resolution", async () => {
    ...
    const response = await sendRaw({
      ...
      headers: jsonHeaders({
        ...
        "x-oriro-session-id": "session-123",
        "x-oriro-account-id": "work",
        "x-oriro-message-channel": "telegram",
        "x-oriro-current-channel-id": "telegram:chat123",
        "x-oriro-current-thread-ts": "42",
        "x-oriro-current-message-id": "reply-message-1",
        "x-oriro-current-inbound-audio": "true",
        "x-oriro-inbound-event-kind": "room_event",
        "x-oriro-source-reply-delivery-mode": "message_tool_only",
        "x-oriro-require-explicit-message-target": "true",
      }),
      body: mcpToolsListBody(),
    });
    ...
    const sendYield = async (
      ...
      return await sendLoopbackToolCall({
        ...
        headers: {
          ...
          "x-oriro-session-id": "session-reused",
          "x-oriro-cli-capture-key": captureKey,
        },
      });
    };
    ...
    const sendToolsList = async (
      ...
      await sendLoopbackToolsList({
        ...
        headers: {
          "x-session-key": "agent:main:telegram:group:chat123",
          "x-oriro-message-channel": "telegram",
          "x-oriro-inbound-event-kind": inboundEventKind,
          ...(sourceReplyDeliveryMode
            ? { "x-oriro-source-reply-delivery-mode": sourceReplyDeliveryMode }
            : {}),
          ...(currentInboundAudio ? { "x-oriro-current-inbound-audio": "true" } : {}),
    ...
    const sendToolsList = async (token?: string) =>
      await sendLoopbackToolsList({
        ...
        headers: {
          ...
          "x-oriro-message-channel": "matrix",
        },
      });
    ...
    expect(
      (
        await sendLoopbackToolCall({
          ...
          headers: { "x-oriro-cli-capture-key": captureKey },
        })
      ).status,
    ).toBe(200);
  ...
  it("times out stalled request bodies and closes uploads after flushing 408", async () => {
    const previousTimeout = process.env.ORIRO_MCP_LOOPBACK_BODY_TIMEOUT_MS;
    process.env.ORIRO_MCP_LOOPBACK_BODY_TIMEOUT_MS = "20";
    try {
      server = await startMcpLoopbackServer(0);
      const runtime = getActiveMcpLoopbackRuntime();
      if (!runtime) {
      ...
      if (previousTimeout === undefined) {
        delete process.env.ORIRO_MCP_LOOPBACK_BODY_TIMEOUT_MS;
      } else {
        process.env.ORIRO_MCP_LOOPBACK_BODY_TIMEOUT_MS = previousTimeout;
      }
    }
  });
...
describe("createMcpLoopbackServerConfig", () => {
  it("builds a server entry with env-driven headers", () => {
    ...
    expect(config.mcpServers?.oriro?.url).toBe("http://127.0.0.1:23119/mcp");
    expect(config.mcpServers?.oriro?.alwaysLoad).toBe(true);
    expect(config.mcpServers?.oriro?.headers?.Authorization).toBe(
      "Bearer ${ORIRO_MCP_TOKEN}",
    );
    expect(config.mcpServers?.oriro?.headers?.["x-oriro-session-id"]).toBe(
      "${ORIRO_MCP_SESSION_ID}",
    );
    expect(config.mcpServers?.oriro?.headers?.["x-oriro-message-channel"]).toBe(
      "${ORIRO_MCP_MESSAGE_CHANNEL}",
    );
    expect(config.mcpServers?.oriro?.headers?.["x-oriro-current-channel-id"]).toBe(
      "${ORIRO_MCP_CURRENT_CHANNEL_ID}",
    );
    expect(config.mcpServers?.oriro?.headers?.["x-oriro-current-thread-ts"]).toBe(
      "${ORIRO_MCP_CURRENT_THREAD_TS}",
    );
    expect(config.mcpServers?.oriro?.headers?.["x-oriro-current-message-id"]).toBe(
      "${ORIRO_MCP_CURRENT_MESSAGE_ID}",
    );
    expect(config.mcpServers?.oriro?.headers?.["x-oriro-current-inbound-audio"]).toBe(
      "${ORIRO_MCP_CURRENT_INBOUND_AUDIO}",
    );
    expect(config.mcpServers?.oriro?.headers?.["x-oriro-source-reply-delivery-mode"]).toBe(
      "${ORIRO_MCP_SOURCE_REPLY_DELIVERY_MODE}",
    );
    expect(
      config.mcpServers?.oriro?.headers?.["x-oriro-require-explicit-message-target"],
    ).toBe("${ORIRO_MCP_REQUIRE_EXPLICIT_MESSAGE_TARGET}");
    expect(config.mcpServers?.oriro?.headers?.["x-oriro-cli-capture-key"]).toBe(
      "${ORIRO_MCP_CLI_CAPTURE_KEY}",
    );
    expect(config.mcpServers?.oriro?.headers).not.toHaveProperty("x-oriro-sender-is-owner");
  });
