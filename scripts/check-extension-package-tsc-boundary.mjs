...
export function resolveCompileConcurrency(
  ...
  const raw = env.ORIRO_EXTENSION_BOUNDARY_CONCURRENCY?.trim();
  if (raw) {
    return parsePositiveInt(raw, "ORIRO_EXTENSION_BOUNDARY_CONCURRENCY");
  }
...
async function runCompileCheck(extensionIds) {
  ...
  const verboseFreshLogs = process.env.ORIRO_EXTENSION_BOUNDARY_VERBOSE_FRESH === "1";
