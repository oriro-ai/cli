...
import type { PdfDocument, PdfEngine, PdfImage } from "oriropdf";
import type {
  DocumentExtractedImage,
  DocumentExtractionRequest,
  DocumentExtractionResult,
  DocumentExtractorPlugin,
} from "oriro/plugin-sdk/document-extractor";
...
async function loadPdfEngine(): Promise<PdfEngine> {
  if (!pdfEnginePromise) {
    pdfEnginePromise = import("oriropdf")
      .then(({ createEngine }) => createEngine())
      .catch((err: unknown) => {
        pdfEnginePromise = null;
        throw new Error("Dependency oriropdf is required for PDF extraction", {
          cause: err,
        });
      });
  }
