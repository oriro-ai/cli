...
const SKIP_DELETE_CONFIRM_KEY = "oriro:skipDeleteConfirm";
...
function isLocalAssistantAttachmentSource(source: string): boolean {
  ...
  if (/^\/(?:__oriro__|media|api\/chat\/media\/outgoing)\//.test(trimmed)) {
    return false;
  }
  return (
...
function buildAssistantAttachmentUrl(
  ...
  return `${normalizedBasePath}/__oriro__/assistant-media?${params.toString()}`;
}
  ...
  if (!pending) {
    pending = (async () => {
      ...
      if (requesterSessionKey) {
        headers.set("x-oriro-requester-session-key", requesterSessionKey);
      }
...
function renderGroupedMessage(
  ...
  const transcriptMeta =
    m["__oriro"] && typeof m["__oriro"] === "object" && !Array.isArray(m["__oriro"])
      ? (m["__oriro"] as Record<string, unknown>)
      : null;
