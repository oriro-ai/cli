...
struct OriroProBackground: View {
...
struct ProCard<Content: View>: View {
    ...
    var radius: CGFloat = OriroProMetric.cardRadius
    ...
    var body: some View {
        ...
        shape
            ...
            .overlay {
                if self.isProminent {
                    shape.strokeBorder(
                        OriroBrand.accent.opacity(self.colorScheme == .dark ? 0.12 : 0.07),
                        lineWidth: 1)
                        .padding(1)
                }
            }
...
extension View {
    func proPanelSurface(
        tint: Color? = nil,
        radius: CGFloat = OriroProMetric.cardRadius,
...
struct OriroSidebarHeaderAction {
    let systemName: String
...
struct OriroSidebarRevealButton: View {
    let headerAction: OriroSidebarHeaderAction

    init(action: OriroSidebarHeaderAction) {
...
struct OriroSidebarHeaderLeadingSlot: View {
    let action: OriroSidebarHeaderAction

    var body: some View {
        OriroSidebarRevealButton(action: self.action)
            .frame(width: 44, height: 44, alignment: .center)
    }
}

struct OriroAdaptiveHeaderRow<Leading: View, Accessory: View>: View {
    let title: String
    let subtitle: String
...
struct OriroProMark: View {
    ...
    var body: some View {
        Image("OriroIcon")
            ...
            .shadow(color: OriroBrand.accent.opacity(0.18), radius: self.shadowRadius, y: self.shadowRadius / 3)
            .accessibilityLabel("Oriro")
    }
}

struct ProProgressBar: View {
    let progress: Double
    var color: Color = OriroBrand.accentHot
...
struct OriroGatewayCompactPill: View {
