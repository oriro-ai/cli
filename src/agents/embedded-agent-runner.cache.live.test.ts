...
import type { AssistantMessage, Message, Tool } from "oriro/plugin-sdk/llm";
...
describeCacheLive("embedded agent runner prompt caching (live)", () => {
  beforeAll(async () => {
    liveRunnerRootDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-live-cache-"));
    ...
    previousCacheTraceEnv = {
      enabled: process.env.ORIRO_CACHE_TRACE,
      file: process.env.ORIRO_CACHE_TRACE_FILE,
      messages: process.env.ORIRO_CACHE_TRACE_MESSAGES,
      prompt: process.env.ORIRO_CACHE_TRACE_PROMPT,
      system: process.env.ORIRO_CACHE_TRACE_SYSTEM,
    };
    setTestEnvValue("ORIRO_CACHE_TRACE", "1");
    setTestEnvValue("ORIRO_CACHE_TRACE_FILE", liveCacheTraceFile);
    setTestEnvValue("ORIRO_CACHE_TRACE_MESSAGES", "0");
    setTestEnvValue("ORIRO_CACHE_TRACE_PROMPT", "0");
    setTestEnvValue("ORIRO_CACHE_TRACE_SYSTEM", "0");
  }, 120_000);
  ...
  afterAll(async () => {
    if (previousCacheTraceEnv) {
      ...
      restore("ORIRO_CACHE_TRACE", previousCacheTraceEnv.enabled);
      restore("ORIRO_CACHE_TRACE_FILE", previousCacheTraceEnv.file);
      restore("ORIRO_CACHE_TRACE_MESSAGES", previousCacheTraceEnv.messages);
      restore("ORIRO_CACHE_TRACE_PROMPT", previousCacheTraceEnv.prompt);
      restore("ORIRO_CACHE_TRACE_SYSTEM", previousCacheTraceEnv.system);
    }
  ...
  describe("openai", () => {
    ...
    beforeAll(async () => {
      fixture = await resolveLiveDirectModel({
        ...
        envVar: "ORIRO_LIVE_OPENAI_CACHE_MODEL",
  ...
  describe("anthropic", () => {
    ...
    beforeAll(async () => {
      fixture = await resolveLiveDirectModel({
        ...
        envVar: "ORIRO_LIVE_ANTHROPIC_CACHE_MODEL",
