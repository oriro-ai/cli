...
export function compareOriroVersions(leftVersion, rightVersion) {
...
function normalizePublishedVersions(publishedVersions) {
  return [...new Set(publishedVersions.map((version) => String(version).trim()).filter(Boolean))]
    ...
    .toSorted((left, right) => compareOriroVersions(right, left));
}
...
export function resolveDefaultReleaseUpgradeBaseline(candidateVersion, publishedVersions) {
  ...
  const older = versions.find((version) => compareOriroVersions(version, candidate.version) < 0);
  if (older) {
    return `oriro@${older}`;
  }

  const same = versions.find(
    (version) => compareOriroVersions(version, candidate.version) === 0,
  );
  if (same) {
    return `oriro@${same}`;
  }
...
function readPublishedVersions(args) {
  ...
  const raw = execFileSync("npm", ["view", "oriro", "versions", "--json", "--silent"], {
    encoding: "utf8",
    stdio: ["ignore", "pipe", "inherit"],
  });
  const parsed = JSON.parse(raw);
  if (!Array.isArray(parsed)) {
    throw new Error("npm returned a non-array oriro versions payload");
  }
  return parsed;
}
