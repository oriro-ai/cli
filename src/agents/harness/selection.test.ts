...
import type { Model } from "oriro/plugin-sdk/llm";
...
vi.mock("./builtin-oriro.js", () => ({
  createOriroAgentHarness: (): AgentHarness => ({
    id: "oriro",
...
const originalRuntime = process.env.ORIRO_AGENT_RUNTIME;
...
afterEach(() => {
  ...
  if (originalRuntime == null) {
    delete process.env.ORIRO_AGENT_RUNTIME;
  } else {
    process.env.ORIRO_AGENT_RUNTIME = originalRuntime;
  }
});

function createAttemptParams(config?: OriroConfig): EmbeddedRunAttemptParams {
  return {
    prompt: "hello",
    sessionId: "session-1",
    runId: "run-1",
...
function groupSenderDenyAllConfig(): OriroConfig {
...
function groupDenyAllConfig(): OriroConfig {
  return {
    channels: {
      telegram: {
        groups: {
...
function providerRuntimeConfig(provider: string, runtime: string): OriroConfig {
  return {
    models: {
      providers: {
        [provider]: {
...
describe("runAgentHarnessAttempt", () => {
  it("fails when a forced plugin harness is unavailable and fallback is omitted", async () => {
    process.env.ORIRO_AGENT_RUNTIME = "codex";
  ...
  it("falls back to the Oriro harness in auto mode when no plugin harness matches", async () => {
    const result = await runAgentHarnessAttempt(createAttemptParams());

    expect(result.sessionIdUsed).toBe("oriro");
    expect(agentRunAttempt).toHaveBeenCalledTimes(1);
  });

  it("allows the selected Oriro harness to satisfy context-engine pre-prompt assembly", async () => {
    const result = await runAgentHarnessAttempt({
      ...createAttemptParams(providerRuntimeConfig("codex", "oriro")),
  ...
  it("surfaces an auto-selected plugin harness failure instead of replaying through Oriro", async () => {
  ...
  it("surfaces a forced plugin harness failure instead of replaying through Oriro", async () => {
    registerFailingCodexHarness();
  ...
  it("falls back to Oriro when the implicit OpenAI Codex harness is unavailable", async () => {
    expect(resolveAgentHarnessPolicy({ provider: "openai", modelId: "gpt-5.4" })).toEqual({
      runtime: "codex",
      runtimeSource: "implicit",
    });
  ...
  it("honors explicit Oriro runtime for OpenAI agent model runs", async () => {
    const result = await runAgentHarnessAttempt({
      ...createAttemptParams(providerRuntimeConfig("openai", "oriro")),
      provider: "openai",
      modelId: "gpt-5.4",
    });
    expect(result.sessionIdUsed).toBe("oriro");
  ...
  it("honors provider wildcard Oriro runtime policy for OpenAI agent model runs", async () => {
    registerSuccessfulCodexHarness();

    const result = await runAgentHarnessAttempt({
      ...createAttemptParams(agentModelRuntimeConfig("openai/*", "oriro")),
      provider: "openai",
      modelId: "gpt-5.4",
    });
    expect(result.sessionIdUsed).toBe("oriro");
  ...
  it.each([
    {
      ...
      config: { tools: { allow: ["message"] } } as OriroConfig,
    },
    {
      name: "specific denylist",
      config: { tools: { deny: ["exec"] } } as OriroConfig,
    },
    {
      name: "narrow profile",
      config: { tools: { profile: "coding" } } as OriroConfig,
    },
  ...
  it.each([
    { name: "full tool profile", config: { tools: { profile: "full" } } as OriroConfig },
    { name: "explicit empty allowlist", config: { tools: { allow: [] } } as OriroConfig },
  ...
  it("leaves Oriro harness params unchanged for channel group sender deny-all policy", async () => {
    await runAgentHarnessAttempt({
      ...createAttemptParams(groupSenderDenyAllConfig()),
      sessionKey: "agent:main:telegram:group:test-deny-room",
      messageProvider: "telegram",
  ...
  it("does not let a strict agent model plugin runtime fall back to Oriro", async () => {
    await expect(
      runAgentHarnessAttempt({
...
describe("selectAgentHarness", () => {
  ...
  it("ignores session-level Oriro pins when selecting a harness", () => {
    const supports = vi.fn(() => ({ supported: true as const, priority: 100 }));
    registerAgentHarness({
      id: "codex",
      label: "Codex",
  ...
  it("honors explicit Oriro runtime overrides when selecting a harness", async () => {
    registerSuccessfulCodexHarness();

    const harness = selectAgentHarness({
      provider: "openai",
      modelId: "gpt-5.4",
      agentHarnessRuntimeOverride: "oriro",
    });

    expect(harness.id).toBe("oriro");
  ...
  it("treats legacy PI runtime overrides as the built-in Oriro harness", async () => {
    registerSuccessfulCodexHarness();
  ...
  it("selects Oriro when the implicit OpenAI Codex harness is unavailable", () => {
    expect(selectAgentHarness({ provider: "openai", modelId: "gpt-5.4" }).id).toBe("oriro");
  });
  ...
  it("ignores existing session Oriro pins when provider policy forces a plugin harness", () => {
    registerFailingCodexHarness();
  ...
  it("ignores env-forced Oriro for OpenAI default runtime selection", () => {
    process.env.ORIRO_AGENT_RUNTIME = "oriro";
    registerFailingCodexHarness();
      ...
      maybeCompactAgentHarnessSession({
        ...
        config: {
          agents: {
            ...
            defaults: {
              models: {
                "openai/gpt-5.5": { agentRuntime: { id: "oriro" } },
              },
            },
          },
        } as OriroConfig,
  ...
  it("keeps compaction recoverable when auth profile lookup fails", async () => {
    ...
    await expect(
      maybeCompactAgentHarnessSession({
        ...
        agentHarnessId: "codex",
        config: agentModelRuntimeConfig("openai/gpt-5.5", "oriro"),
      }),
    ).resolves.toEqual({ ok: true, compacted: false });
    expect(compact).toHaveBeenCalledTimes(1);
  ...
  it("does not compact a selected plugin harness through Oriro when the plugin has no compactor", async () => {
    registerFailingCodexHarness();
  ...
  it.each([
    ...
    "returns Oriro for explicit CLI runtime alias $alias on $provider instead of throwing MissingAgentHarnessError",
