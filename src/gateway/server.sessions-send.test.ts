...
const { createOriroTools } = await import("../agents/oriro-tools.js");
...
type SessionSendTool = ReturnType<typeof createOriroTools>[number];
...
function getSessionsSendTool(): SessionSendTool {
  ...
  const tool = createOriroTools().find((candidate) => candidate.name === "sessions_send");
  if (!tool) {
...
beforeAll(async () => {
  envSnapshot = captureEnv(["ORIRO_GATEWAY_PORT", "ORIRO_GATEWAY_TOKEN"]);
  ...
  process.env.ORIRO_GATEWAY_PORT = String(gatewayPort);
  process.env.ORIRO_GATEWAY_TOKEN = gatewayToken;
...
describe("sessions_send gateway loopback", () => {
  ...
  it(
    ...
    async () => {
      const dir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-sessions-send-route-"));
