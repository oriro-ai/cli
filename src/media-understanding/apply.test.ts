...
const TEMP_MEDIA_PREFIX = "oriro-media-";
...
function createGroqAudioConfig(): OriroConfig {
  return {
    tools: {
      media: {
        audio: {
...
function createMediaDisabledConfig(): OriroConfig {
  return {
    tools: {
      media: {
        audio: { enabled: false },
...
function createMediaDisabledConfigWithAllowedMimes(allowedMimes: string[]): OriroConfig {
  return {
  ...
  return await withEnvAsync(
    {
      ...
      ORIRO_ANTIGRAVITY_CLI: undefined,
...
async function setupAudioAutoDetectCase(stdout: string): Promise<{
  ...
  const cfg: OriroConfig = { tools: { media: { audio: {} } } };
...
describe("applyMediaUnderstanding", () => {
  beforeAll(async () => {
    ...
    const baseDir = resolvePreferredOriroTmpDir();
  ...
  it("uses Antigravity CLI as the last auto image fallback", async () => {
    ...
    const cfg: OriroConfig = { tools: { media: { image: {} } } };
    mockedResolveApiKey.mockResolvedValue({
      source: "none",
      mode: "api-key",
    });
    ...
    expect(probeOptions).toEqual({
      timeoutMs: 3000,
      cwd: expect.stringContaining("oriro-antigravity-probe-"),
    });
