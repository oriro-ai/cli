...
describe("sessions_history redaction", () => {
  beforeAll(async () => {
    previousConfigPath = process.env.ORIRO_CONFIG_PATH;
    tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-sessions-history-redact-"));
  ...
  afterAll(() => {
    if (previousConfigPath === undefined) {
      ...
      process.env.ORIRO_CONFIG_PATH = previousConfigPath;
    }
    if (tempDir) {
