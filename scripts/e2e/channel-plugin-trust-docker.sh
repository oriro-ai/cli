...
#   mode against the installed Oriro package.
#
# Parameters:
#   --container: run the in-container scenario. Host mode is the default.
#   ORIRO_CHANNEL_PLUGIN_TRUST_E2E_IMAGE: override the Docker image name.
#   ORIRO_CHANNEL_PLUGIN_TRUST_E2E_SKIP_BUILD=1: reuse/pull the image.
#
# Outputs:
...
Description:
  Proves the packaged Oriro CLI enforces local channel plugin trust for
...
Environment:
  ORIRO_CHANNEL_PLUGIN_TRUST_E2E_IMAGE       Override Docker image name.
  ORIRO_CHANNEL_PLUGIN_TRUST_E2E_SKIP_BUILD  Reuse/pull image instead of building.
  ORIRO_TEST_STATE_SCRIPT_B64                Required in --container mode.
...
Examples:
  ...
  ORIRO_CHANNEL_PLUGIN_TRUST_E2E_SKIP_BUILD=1 bash scripts/e2e/channel-plugin-trust-docker.sh
EOF
}
...
run_oriro() {
  ...
  if [ -f /app/oriro.mjs ]; then
    node /app/oriro.mjs "$@"
    return
  fi
  echo "oriro CLI not found in Docker image" >&2
  exit 1
}
...
EOF
  ...
  cat >"$plugin_dir/oriro.plugin.json" <<EOF
{
  "id": "$plugin_id",
...
run_case() {
  ...
  local trusted="${2:?missing trusted flag}"
  local scratch
  scratch="$(mktemp -d "/tmp/oriro-channel-plugin-trust-$case_id.XXXXXX")"
  ...
  PLUGINTRUST_IMPORT_MARKER="$marker_dir/import.marker" \
    ...
    run_oriro channels add --channel e2e-load-paths --token "$canary" \
...
run_container() {
  ...
  export ORIRO_WORKSPACE_DIR="$HOME/.oriro/workspace"
...
run_host() {
  ...
  image_name="$(
    docker_e2e_resolve_image \
      "oriro-channel-plugin-trust-e2e:local" \
      ORIRO_CHANNEL_PLUGIN_TRUST_E2E_IMAGE
  )"
  local skip_build="${ORIRO_CHANNEL_PLUGIN_TRUST_E2E_SKIP_BUILD:-0}"
  ...
  docker_e2e_run_logged_print_with_harness \
    ...
    -e "ORIRO_TEST_STATE_SCRIPT_B64=$state_script_b64" \
    "$image_name" \
    bash scripts/e2e/channel-plugin-trust-docker.sh --container
}
