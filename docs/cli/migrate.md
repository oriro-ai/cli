---
summary: "CLI reference for `oriro migrate` (import state from another agent system)"
read_when:
  - You want to migrate from Hermes or another agent system into Oriro
...
```bash
oriro migrate list
oriro migrate claude --dry-run
oriro migrate codex --dry-run
oriro migrate codex --skill gog-vault77-google-workspace
oriro migrate codex --plugin google-calendar --dry-run
oriro migrate codex --plugin google-calendar --verify-plugin-apps --dry-run
oriro migrate hermes --dry-run
oriro migrate hermes
oriro migrate apply codex --yes --skill gog-vault77-google-workspace
oriro migrate apply codex --yes --plugin google-calendar
oriro migrate apply codex --yes
oriro migrate apply claude --yes
oriro migrate apply hermes --yes
oriro migrate apply hermes --include-secrets --yes
oriro onboard --flow import
oriro onboard --import-from claude --import-source ~/.claude
oriro onboard --import-from hermes --import-source ~/.hermes
```

<ParamField path="<provider>" type="string">
  Name of a registered migration provider, for example `hermes`. Run `oriro migrate list` to see installed providers.
</ParamField>
...
<ParamField path="--no-backup" type="boolean">
  Skip the pre-apply backup. Requires `--force` when local Oriro state exists.
</ParamField>
...
`oriro migrate` is preview-first.
...
<AccordionGroup>
  <Accordion title="Preview before apply">
    ...
    `oriro migrate apply <provider>` previews the plan and prompts before changing state unless `--yes` is set. In non-interactive mode, apply requires `--yes`.

  </Accordion>
  <Accordion title="Backups">
    Apply creates and verifies an Oriro backup before applying the migration. If no local Oriro state exists yet, the backup step is skipped and the migration can continue. To skip a backup when state exists, pass both `--no-backup` and `--force`.
  </Accordion>
...
- Project `CLAUDE.md` and `.claude/CLAUDE.md` into the Oriro agent workspace.
...
- Claude command Markdown files converted into Oriro skills with manual invocation only.

### Archive and manual-review state

Claude hooks, permissions, environment defaults, local memory, path-scoped rules, subagents, caches, plans, and project history are preserved in the migration report or reported as manual-review items. Oriro does not execute hooks, copy broad allowlists, or import OAuth/Desktop credential state automatically.
...
Use this provider when moving to the Oriro Codex harness and you want to
...
Running `oriro migrate codex` in an interactive terminal previews the full
...
start checked unless the target Oriro Codex plugin config already has that
...
```bash
oriro migrate codex --dry-run --skill gog-vault77-google-workspace
oriro migrate apply codex --yes --skill gog-vault77-google-workspace
```
...
```bash
oriro migrate codex --dry-run --plugin google-calendar
oriro migrate apply codex --yes --plugin google-calendar
```
...
- Personal AgentSkills under `$HOME/.agents/skills`, copied into the current
  Oriro agent workspace when you want per-agent ownership.
- Source-installed `openai-curated` Codex plugins discovered through Codex
  ...
  native Codex harness; they are not exposed to Oriro provider runs,
...
- `SOUL.md` and `AGENTS.md` into the Oriro agent workspace.
- `memories/MEMORY.md` and `memories/USER.md` appended to workspace memory files.
- Memory config defaults for Oriro file memory, plus archive or manual-review items for external memory providers such as Honcho.
...
Hermes state that Oriro cannot safely interpret is copied into the migration report for manual review, but it is not loaded into live Oriro config or credentials. This preserves opaque or unsafe state without pretending Oriro can execute or trust it automatically:
...
Migration sources are plugins. A plugin declares its provider ids in `oriro.plugin.json`:
...
Provider plugins can use `oriro/plugin-sdk/migration` for item construction and summary counts, plus `oriro/plugin-sdk/migration-runtime` for conflict-aware file copies, archive-only report copies, cached config-runtime wrappers, and migration reports.

## Onboarding integration

Onboarding can offer migration when a provider detects a known source. Both `oriro onboard --flow import` and `oriro setup --wizard --import-from hermes` use the same plugin migration provider and still show a preview before applying.

<Note>
Onboarding imports require a fresh Oriro setup. Reset config, credentials, sessions, and the workspace first if you already have local state. Backup-plus-overwrite or merge imports are feature-gated for existing setups.
</Note>
...
- [Migrating](/install/migrating): move Oriro to a new machine.
