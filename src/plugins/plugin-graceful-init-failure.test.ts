...
const fixtureRoot = makeTrackedTempDir("oriro-plugin-graceful", fixtureTempDirs);
let tempDirIndex = 0;
const { loadOriroPlugins, clearPluginLoaderCache } = await import("./loader.js");
...
function readPluginId(pluginPath: string): string {
  const manifestPath = path.join(path.dirname(pluginPath), "oriro.plugin.json");
...
describe("graceful plugin initialization failure", () => {
  ...
  it("logs a startup summary grouped by failure phase", async () => {
    ...
    expect(summary).toContain("oriro plugins inspect <id> --runtime --json");
    expect(summary).toContain("oriro plugins list");
  });
});
