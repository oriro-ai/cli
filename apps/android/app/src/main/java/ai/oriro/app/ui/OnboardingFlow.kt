...
import ai.oriro.app.GatewayConnectionProblem
import ai.oriro.app.GatewayNodeApprovalState
import ai.oriro.app.LocationMode
import ai.oriro.app.MainViewModel
import ai.oriro.app.R
import ai.oriro.app.SensitiveFeatureConfig
import ai.oriro.app.node.DeviceNotificationListenerService
import ai.oriro.app.ui.design.OriroDesignTheme
import ai.oriro.app.ui.design.OriroErrorState
import ai.oriro.app.ui.design.OriroListItem
...
fun OnboardingFlow(
  ...
  OriroDesignTheme(dark = onboardingDark) {
    ...
    OriroSystemBarAppearance(lightAppearance = !onboardingDark && step != OnboardingStep.Welcome)
    ...
    pendingTrust?.let { prompt ->
      AlertDialog(
        ...
        containerColor = OriroTheme.colors.surfaceRaised,
        title = { Text("Trust this gateway?", style = OriroTheme.type.section, color = OriroTheme.colors.text) },
        text = {
          Text(
  ...
  Box(
    ...
    Column(
      ...
      Column(horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.spacedBy(18.dp)) {
        ...
        Text(
          text = "ORIRO",
          style = OriroTheme.type.display.copy(fontSize = 34.sp, lineHeight = 38.sp, fontWeight = FontWeight.Black),
...
private fun WelcomeLogo() {
  Surface(
    ...
    Box(modifier = Modifier.fillMaxSize().padding(12.dp), contentAlignment = Alignment.Center) {
      Image(painter = painterResource(id = R.drawable.oriro_logo), contentDescription = "Oriro logo", modifier = Modifier.fillMaxSize())
    }
  }
}
...
private fun GatewaySetupScreen(
  ...
  OriroScaffold(modifier = modifier, contentPadding = PaddingValues(horizontal = 18.dp, vertical = 16.dp)) {
          ...
          Column(verticalArrangement = Arrangement.spacedBy(8.dp)) {
            Surface(
              ...
              Row(
                ...
                Text(text = "Advanced", style = OriroTheme.type.section, color = OriroTheme.colors.text)
                Icon(
            ...
            if (advancedOpen) {
              OriroTextField(value = setupCode, onValueChange = onSetupCodeChange, placeholder = "Setup code")
              Row(horizontalArrangement = Arrangement.spacedBy(10.dp)) {
                OriroTextField(value = manualHost, onValueChange = onManualHostChange, placeholder = "Host", modifier = Modifier.weight(1f))
                OriroTextField(value = manualPort, onValueChange = onManualPortChange, placeholder = "Port", modifier = Modifier.width(104.dp))
              }
              Row(horizontalArrangement = Arrangement.spacedBy(10.dp)) {
              ...
              OriroTextField(value = token, onValueChange = onTokenChange, placeholder = "Token optional")
              OriroTextField(value = password, onValueChange = onPasswordChange, placeholder = "Password optional")
            }
          }
        }
      }
      OriroPrimaryButton(text = "Pair with Gateway", icon = Icons.Default.Security, onClick = onPair, modifier = Modifier.fillMaxWidth())
    }
  }
}
    ...
    Column(modifier = Modifier.fillMaxSize(), verticalArrangement = Arrangement.spacedBy(18.dp)) {
      ...
      Column(modifier = Modifier.fillMaxWidth(), horizontalAlignment = Alignment.CenterHorizontally, verticalArrangement = Arrangement.spacedBy(12.dp)) {
        Icon(
          ...
          tint =
            when (recoveryState) {
              GatewayRecoveryUiState.Connected -> OriroTheme.colors.success
              GatewayRecoveryUiState.NodeCapabilityApprovalPending -> OriroTheme.colors.warning
              GatewayRecoveryUiState.ApprovalRequired -> OriroTheme.colors.warning
              GatewayRecoveryUiState.Pairing -> OriroTheme.colors.text
              GatewayRecoveryUiState.Finishing -> OriroTheme.colors.text
              GatewayRecoveryUiState.Failed -> OriroTheme.colors.warning
            },
        )
        Text(text = recoveryState.title, style = OriroTheme.type.display, color = OriroTheme.colors.text)
        Text(
      ...
      OriroPanel {
        Column(verticalArrangement = Arrangement.spacedBy(10.dp)) {
          Text(text = "Last gateway", style = OriroTheme.type.caption, color = OriroTheme.colors.textMuted)
          Text(text = recoveryGatewayName(serverName = serverName, attemptedGatewayName = attemptedGatewayName), style = OriroTheme.type.section, color = OriroTheme.colors.text)
          Text(
            text =
          ...
          OriroStatusPill(
            ...
            status =
              when (recoveryState) {
                GatewayRecoveryUiState.Connected -> OriroStatus.Success
                GatewayRecoveryUiState.NodeCapabilityApprovalPending -> OriroStatus.Warning
                GatewayRecoveryUiState.ApprovalRequired -> OriroStatus.Warning
                GatewayRecoveryUiState.Pairing -> OriroStatus.Neutral
                GatewayRecoveryUiState.Finishing -> OriroStatus.Neutral
                GatewayRecoveryUiState.Failed -> OriroStatus.Warning
              },
          )
        }
      }
  ...
  Surface(
    ...
    Row(
      ...
      SelectionContainer(modifier = Modifier.weight(1f)) {
        Text(text = command, style = OriroTheme.type.body.copy(fontFamily = FontFamily.Monospace), color = OriroTheme.colors.text)
      }
      Surface(
        onClick = onCopy,
        ...
        item {
          Column(modifier = Modifier.padding(top = 10.dp, bottom = 18.dp), verticalArrangement = Arrangement.spacedBy(6.dp)) {
            Text(
              ...
              style = OriroTheme.type.title.copy(fontSize = 20.sp, lineHeight = 25.sp, fontWeight = FontWeight.Bold),
              color = OriroTheme.colors.text,
            )
            Text(
              text = "These permissions keep Oriro secure\nand useful.",
              style = OriroTheme.type.body,
              color = OriroTheme.colors.textMuted,
            )
          }
...
private fun HeroPrimaryAction(
  ...
  Surface(
    ...
    Row(
      ...
      Text(text = title, style = OriroTheme.type.title.copy(fontSize = 14.5.sp, lineHeight = 18.sp), modifier = Modifier.weight(1f), textAlign = TextAlign.Center)
...
private fun OnboardingHeader(
  ...
  Row(modifier = modifier.fillMaxWidth(), verticalAlignment = Alignment.CenterVertically, horizontalArrangement = Arrangement.spacedBy(12.dp)) {
    onBack?.let {
      Surface(onClick = it, modifier = Modifier.size(34.dp), color = Color.Transparent, contentColor = OriroTheme.colors.text) {
        Box(contentAlignment = Alignment.Center) {
    ...
    Column(verticalArrangement = Arrangement.spacedBy(4.dp)) {
      Text(text = title, style = OriroTheme.type.display, color = OriroTheme.colors.text)
      subtitle?.let { Text(text = it, style = OriroTheme.type.body, color = OriroTheme.colors.textMuted) }
    }
  }
}
...
private fun GatewayOption(
  ...
  OriroPanel(contentPadding = PaddingValues(horizontal = 12.dp, vertical = 8.dp)) {
    OriroListItem(
      title = title,
      subtitle = subtitle,
      metadata = status,
      leading = { Icon(imageVector = icon, contentDescription = null, modifier = Modifier.size(22.dp), tint = OriroTheme.colors.text) },
      trailing =
        onClick?.let {
          {
            Icon(imageVector = Icons.AutoMirrored.Filled.KeyboardArrowRight, contentDescription = "Open $title", modifier = Modifier.size(20.dp), tint = OriroTheme.colors.text)
          }
        },
      onClick = onClick,
    )
...
private fun OutlinedAction(
  ...
  Surface(
    ...
    Row(
      ...
      Text(text = title, style = OriroTheme.type.section, modifier = Modifier.weight(1f))
    }
  }
}
...
private fun TogglePill(
  ...
  Surface(
    ...
    Text(text = text, modifier = Modifier.padding(horizontal = 12.dp, vertical = 7.dp), style = OriroTheme.type.label)
  }
}
...
private fun PermissionTopBar(onBack: () -> Unit) {
  ...
  if (showHelp) {
    AlertDialog(
      ...
      containerColor = OriroTheme.colors.surfaceRaised,
      title = { Text("Permissions", style = OriroTheme.type.section, color = OriroTheme.colors.text) },
      text = {
        Text(
          "Choose what this phone can share with Oriro. You can change these later in Settings.",
          style = OriroTheme.type.body,
          color = OriroTheme.colors.textMuted,
        )
      },
  ...
  Box(modifier = Modifier.fillMaxWidth().height(38.dp), contentAlignment = Alignment.Center) {
    ...
    Text(
      ...
      style = OriroTheme.type.title.copy(fontSize = 15.2.sp, lineHeight = 19.sp),
      color = OriroTheme.colors.text,
      maxLines = 1,
    )
    Surface(
      ...
      color = Color.Transparent,
      contentColor = OriroTheme.colors.text,
      border = BorderStroke(1.dp, OriroTheme.colors.text),
    ) {
      Box(contentAlignment = Alignment.Center) {
        Text(text = "?", style = OriroTheme.type.label.copy(fontSize = 13.sp, lineHeight = 16.sp), color = OriroTheme.colors.text)
      }
    }
  }
}
...
private fun PermissionRow(row: PermissionRowModel) {
  Surface(
    ...
    Row(
      ...
      Surface(modifier = Modifier.size(30.dp), shape = CircleShape, color = OriroTheme.colors.surfacePressed, border = BorderStroke(1.dp, OriroTheme.colors.border)) {
        Box(contentAlignment = Alignment.Center) {
          Icon(imageVector = row.icon, contentDescription = null, modifier = Modifier.size(17.dp), tint = OriroTheme.colors.text)
        }
      }
      ...
      Text(
        ...
        style = OriroTheme.type.body,
        color = if (row.granted) OriroTheme.colors.success else OriroTheme.colors.textMuted,
        maxLines = 1,
      )
      Icon(
...
private fun PermissionContinueButton(onClick: () -> Unit) {
  Surface(
    ...
    Box(modifier = Modifier.fillMaxWidth(), contentAlignment = Alignment.Center) {
      Text(text = "Continue", style = OriroTheme.type.title.copy(fontSize = 18.sp, lineHeight = 23.sp), color = OriroTheme.colors.primaryText)
      Icon(
        imageVector = Icons.AutoMirrored.Filled.KeyboardArrowRight,
        contentDescription = null,
        modifier = Modifier.align(Alignment.CenterEnd).padding(end = 20.dp).size(19.dp),
        tint = OriroTheme.colors.primaryText,
      )
    }
  }
}
...
internal enum class GatewayRecoveryUiState(
  ...
  Pairing(
    title = "Pairing Gateway",
    message = "Approval is in progress.\nOriro will reconnect automatically.",
  ),
  Finishing(
    title = "Connecting Gateway",
    message = "Oriro is checking gateway and node access.",
  ),
  Failed(
  ...
  remoteAddress
    ...
    ?: if (ready) {
      ...
      recoveryGatewayApprovalCommand(gatewayConnectionProblem)
        ...
        ?: "Gateway approval is pending. Run oriro devices list on the gateway host, approve this phone, then retry."
      ...
      "Gateway approval is in progress. Oriro will retry automatically."
    } else {
      "Gateway unreachable"
    }
...
private fun copyApprovalCommand(
  ...
  clipboard.setPrimaryClip(ClipData.newPlainText("Oriro pairing approval command", command))
...
private fun copyGatewayDiagnostic(
  ...
  val diagnostic =
    listOfNotNull(
      "Oriro Android gateway diagnostic",
      "Status: $statusText",
  ...
  val clipboard = context.getSystemService(Context.CLIPBOARD_SERVICE) as ClipboardManager
  clipboard.setPrimaryClip(ClipData.newPlainText("Oriro gateway diagnostic", diagnostic))
