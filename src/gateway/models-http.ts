// OpenAI-compatible `/v1/models` HTTP route backed by configured Oriro agents.
...
function loadAgentModelIds(): string[] {
  ...
  const ids = new Set<string>([ORIRO_MODEL_ID, ORIRO_DEFAULT_MODEL_ID]);
  ids.add(`oriro/${defaultAgentId}`);
  for (const agentId of listAgentIds(cfg)) {
    ids.add(`oriro/${agentId}`);
  }
...
export async function handleOpenAiModelsHttpRequest(
  ...
  if (decodedId !== ORIRO_MODEL_ID && !resolveAgentIdFromModel(decodedId)) {
