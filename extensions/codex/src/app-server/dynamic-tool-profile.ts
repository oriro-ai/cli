...
/** Tool names owned by Codex app-server and normally excluded from Oriro dynamic tools. */
...
type CodexDynamicToolProfileEnv = {
  ORIRO_BUILD_PRIVATE_QA?: string;
  ORIRO_QA_FORCE_RUNTIME?: string;
};

/** Normalizes Oriro/Codex tool names before filtering and allowlist checks. */
...
export function isForcedPrivateQaCodexRuntime(
  ...
  return (
    ...
    env.ORIRO_QA_FORCE_RUNTIME?.trim().toLowerCase() === "codex"
  );
}
...
/** Filters Oriro tools that Codex owns natively or config explicitly excludes. */
