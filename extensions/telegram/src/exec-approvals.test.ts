...
function createTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-telegram-exec-approvals-"));
  tempDirs.push(dir);
  return dir;
}

function buildConfig(
  execApprovals?: NonNullable<NonNullable<OriroConfig["channels"]>["telegram"]>["execApprovals"],
  channelOverrides?: Partial<NonNullable<NonNullable<OriroConfig["channels"]>["telegram"]>>,
): OriroConfig {
  return {
    channels: {
      telegram: {
