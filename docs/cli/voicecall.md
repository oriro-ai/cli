---
summary: "CLI reference for `oriro voicecall` (voice-call plugin command surface)"
read_when:
...
oriro voicecall smoke    [-t <phone>] [--message <text>] [--mode <m>] [--yes] [--json]
oriro voicecall call     -m <text> [-t <phone>] [--mode <m>]
oriro voicecall start    --to <phone> [--message <text>] [--mode <m>]
oriro voicecall continue --call-id <id> --message <text>
oriro voicecall speak    --call-id <id> --message <text>
oriro voicecall dtmf     --call-id <id> --digits <digits>
oriro voicecall end      --call-id <id>
oriro voicecall status   [--call-id <id>] [--json]
oriro voicecall tail     [--file <path>] [--since <n>] [--poll <ms>]
oriro voicecall latency  [--file <path>] [--last <n>]
oriro voicecall expose   [--mode <m>] [--path <p>] [--port <port>] [--serve-path <p>]
```
...
| `--message <text>` | `Oriro voice call smoke test.` | Message to speak during the smoke call. |
...
oriro voicecall smoke --to "+15555550123"        # dry run
oriro voicecall smoke --to "+15555550123" --yes  # live notify call
```
...
```bash
oriro voicecall call --to "+15555550123" --message "Hello"
oriro voicecall call -m "Heads up" --mode notify
```
...
```bash
oriro voicecall status
oriro voicecall status --json
...
```bash
oriro voicecall expose --mode serve
oriro voicecall expose --mode funnel
oriro voicecall expose --mode off
```
