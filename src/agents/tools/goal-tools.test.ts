...
async function createStoreConfig(): Promise<{ config: OriroConfig; template: string }> {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-goal-tools-"));
  const template = path.join(dir, "{agentId}", "sessions.json");
  return {
    config: { session: { store: template } } as OriroConfig,
    template,
  };
}
