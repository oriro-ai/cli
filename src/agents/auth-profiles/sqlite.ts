...
import type { DB as OriroAgentKyselyDatabase } from "../../state/oriro-agent-db.generated.js";
import {
  runOriroAgentWriteTransaction,
  type OriroAgentDatabase,
} from "../../state/oriro-agent-db.js";
import { ORIRO_SQLITE_BUSY_TIMEOUT_MS } from "../../state/oriro-state-db.js";
import { resolveUserPath } from "../../utils.js";
import { resolveRegisteredAgentIdForDir } from "../agent-dir-registry.js";
import { resolveDefaultAgentDir } from "../agent-scope-config.js";

type AuthProfileDatabase = Pick<
  OriroAgentKyselyDatabase,
...
// The auth database lives in the agent dir and shares the oriro-agent schema
...
function resolveAuthProfileDatabaseOptions(agentDir?: string) {
  ...
  return {
    ...
    path: path.join(dir, "oriro-agent.sqlite"),
  };
}
...
export function writePersistedAuthProfileStoreRaw(
  ...
  const write = (target: OriroAgentDatabase) => {
  ...
  runOriroAgentWriteTransaction(write, resolveAuthProfileDatabaseOptions(agentDir));
}
...
export function deletePersistedAuthProfileStoreRaw(
  ...
  const remove = (target: OriroAgentDatabase) => {
    const db = getAuthProfileKysely(target.db);
    executeSqliteQuerySync(
      target.db,
  ...
  runOriroAgentWriteTransaction(remove, resolveAuthProfileDatabaseOptions(agentDir));
}
...
export function runAuthProfileWriteTransaction<T>(
  ...
  return runOriroAgentWriteTransaction(operation, resolveAuthProfileDatabaseOptions(agentDir));
}
