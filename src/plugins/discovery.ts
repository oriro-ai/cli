...
function deriveIdHint(params: {
  ...
  // package is scoped (example: @oriro/voice-call -> voice-call).
...
function pushInvalidPackageExtensionDiagnostic(params: {
  ...
  if (params.resolution.status === "empty") {
    params.diagnostics.push({
      level: "error",
      source: params.source,
      message: "package.json oriro.extensions is empty",
    });
    return true;
  }
  return false;
...
function addLegacyNpmDeclarationDiagnostic(params: {
  ...
  params.diagnostics.push({
    ...
    message: `legacy npm plugin declaration ignored for "${declaration.pluginId}"; run "oriro doctor --fix" to install ${declaration.npmSpec} into the managed plugin root`,
  });
  return true;
}
...
export function discoverOriroPlugins(params: {
  workspaceDir?: string;
  extraPaths?: string[];
  ...
  const scopedResult = tracePluginLifecyclePhase(
    ...
    () => {
      ...
      if (roots.workspace && workspaceRoot && !workspaceMatchesBundledRoot) {
        // Keep workspace auto-discovery constrained to the Oriro extensions root.
