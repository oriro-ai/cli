...
async function expectVertexAdcEnvApiKey(params: {
  ...
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), params.tempPrefix ?? "oriro-adc-"));
...
vi.mock("./model-auth-env-vars.js", () => {
  ...
  const resolveMockProviderAuthEvidence = (params?: { config?: OriroConfig }) => {
    const evidence = {
      "google-vertex": [
        {
  ...
  return {
    ...
    resolveProviderEnvAuthLookupMaps: (params?: { config?: OriroConfig }) => ({
      aliasMap,
...
function buildDemoLocalProviderCfg(apiKey: string): OriroConfig {
  return {
    models: {
      providers: {
        "demo-local": {
...
describe("getApiKeyForModel", () => {
  it("reads oauth auth-profiles entries from auth-profiles.json via explicit profile", async () => {
    await withOriroTestState(
      ...
      async (state) => {
        ...
        const store = ensureAuthProfileStore(process.env.ORIRO_AGENT_DIR, {
  ...
  it("reports the config default agent dir when provider auth is missing", async () => {
    await withOriroTestState(
      {
        layout: "state-only",
        prefix: "oriro-auth-missing-agent-dir-",
        agentEnv: "clear",
        env: {
          XAI_API_KEY: undefined,
        },
  ...
  it("uses trusted workspace manifest auth evidence in runtime auth checks", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-workspace-cloud-auth-"));
  ...
  it("resolveEnvApiKey('google-vertex') accepts Unicode explicit ADC credential paths", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-google-adc-unicode-"));
  ...
  it("resolveEnvApiKey('google-vertex') accepts Unicode ADC fallback home paths", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-google-adc-home-"));
  ...
  it("resolveEnvApiKey('google-vertex') rejects GOOGLE_CLOUD_PROJECT_ID-only ADC auth evidence", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-google-adc-project-id-"));
    const credentialsPath = path.join(tempDir, "adc.json");
    await fs.writeFile(credentialsPath, "{}", "utf8");
  ...
  it("resolveEnvApiKey('google-vertex') accepts Windows APPDATA ADC fallback evidence", async () => {
    const appDataDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-google-adc-appdata-"));
  ...
  it("resolveEnvApiKey('google-vertex') does not synthesize APPDATA from USERPROFILE", async () => {
    const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-google-adc-home-"));
    const userProfileDir = await fs.mkdtemp(
      path.join(os.tmpdir(), "oriro-google-adc-userprofile-"),
    );
  ...
  it("resolveEnvApiKey('google-vertex') keeps ADC fallback when manifest env candidates are empty", async () => {
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-google-adc-candidates-"));
    const credentialsPath = path.join(tempDir, "adc.json");
    await fs.writeFile(credentialsPath, "{}", "utf8");
...
describe("resolveApiKeyForProvider — per-entry apiKey as profile ID reference", () => {
  ...
  it("resolves profile reference even when provider sets auth: api-key explicitly (regression for orirosweeper P3)", async () => {
  ...
  it("throws when matched profile is an OAuth credential routed to an api-key provider (orirosweeper P1)", async () => {
    await expect(
      resolveApiKeyForProvider({
        provider: "openrouter-minimax",
        cfg: {
  ...
  it("throws (does not fall through to literal bearer) when matched profile resolution fails (orirosweeper P2)", async () => {
