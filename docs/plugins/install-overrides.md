...
[`oriro plugins install`](/cli/plugins).
...
```bash
export ORIRO_ALLOW_PLUGIN_INSTALL_OVERRIDES=1
export ORIRO_PLUGIN_INSTALL_OVERRIDES='{
  "codex": "npm-pack:/tmp/oriro-codex-2026.5.8.tgz",
  "oriro-web-search": "npm:@oriro/web-search@2026.5.8"
}'
```
...
Oriro uses the override source instead of the catalog, bundled, or default
...
entry normally represents an Oriro-owned package, an override is treated as
operator-supplied test input.

Workspace `.env` files cannot enable install overrides. Set these variables in
the trusted shell, CI job, or remote test command that launches Oriro.

## Package E2E

Use an isolated state directory so package installs and install records do not
touch your normal Oriro state:

```bash
npm pack extensions/codex --pack-destination /tmp

ORIRO_STATE_DIR="$(mktemp -d)" \
ORIRO_ALLOW_PLUGIN_INSTALL_OVERRIDES=1 \
ORIRO_PLUGIN_INSTALL_OVERRIDES='{"codex":"npm-pack:/tmp/oriro-codex-2026.5.8.tgz"}' \
pnpm oriro onboard --mode local
```

Verify the installed package under the state directory:

```bash
find "$ORIRO_STATE_DIR/npm/projects" -path '*/node_modules/@oriro/codex/package.json' -print
grep -R '"@oriro/codex"' "$ORIRO_STATE_DIR/npm/projects"/*/package-lock.json
```
