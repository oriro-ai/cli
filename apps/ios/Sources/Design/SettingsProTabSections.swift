...
extension SettingsProTab {
    ...
    var settingsListSection: some View {
        VStack(spacing: 10) {
            self.settingsListRow(
                ...
                route: .approvals,
                color: self.pendingApproval == nil ? .secondary : OriroBrand.warn,
    ...
    func destination(for route: SettingsRoute) -> some View {
        ZStack {
            ...
            ScrollView {
                ...
                .padding(.top, 18)
                .padding(.bottom, OriroProMetric.bottomScrollInset)
            }
        }
                ...
                if let pendingApproval {
                    ...
                    if let errorText = self.appModel.pendingExecApprovalPromptErrorText {
                        Text(errorText)
                            ...
                            .foregroundStyle(OriroBrand.danger)
                    }
                    ...
                    HStack(spacing: 12) {
                        ProIconBadge(systemName: "checkmark.shield.fill", color: OriroBrand.ok)
                        VStack(alignment: .leading, spacing: 3) {
    ...
    var permissionsDestination: some View {
        VStack(alignment: .leading, spacing: 14) {
            self.toggleCard(
                icon: "camera",
                title: "Camera",
                detail: "Allow the gateway to request photos or video while Oriro is foregrounded.",
            ...
            self.toggleCard(
                icon: "lock.display",
                title: "Keep Awake",
                detail: "Keep the screen awake while Oriro is open.",
    ...
    var voiceDestination: some View {
        VStack(alignment: .leading, spacing: 14) {
            self.detailStatusCard(
                ...
                color: self.talkEnabled || self.voiceWakeEnabled ? OriroBrand.accent : .secondary)
    ...
    var diagnosticsDestination: some View {
        VStack(alignment: .leading, spacing: 14) {
            self.detailStatusCard(
                ...
                color: self.gatewayDiagnosticConnected ? OriroBrand.ok : OriroBrand.warn)
    ...
    var privacyDestination: some View {
        VStack(alignment: .leading, spacing: 14) {
            self.detailStatusCard(
                icon: "hand.raised",
                title: "Privacy",
                detail: "Control what device context Oriro can expose to the gateway.",
        ...
        isOn: Binding<Bool>) -> some View
    {
        ProCard(radius: SettingsLayout.cardRadius) {
            Toggle(isOn: isOn) {
                HStack(spacing: 12) {
                    ProIconBadge(systemName: icon, color: isOn.wrappedValue ? OriroBrand.accent : .secondary)
                    VStack(alignment: .leading, spacing: 3) {
                        Text(title)
                            .font(.subheadline.weight(.semibold))
                        Text(detail)
        ...
        ProCard(radius: SettingsLayout.cardRadius) {
            VStack(alignment: .leading, spacing: 12) {
                HStack(spacing: 12) {
                    ProIconBadge(
                        systemName: "location",
                        color: self.locationModeRaw == OriroLocationMode.off.rawValue ? .secondary : OriroBrand
                            .accent)
                    VStack(alignment: .leading, spacing: 3) {
                        Text("Location")
                            .font(.subheadline.weight(.semibold))
                ...
                Picker("Location", selection: self.$locationModeRaw) {
                    Text("Off").tag(OriroLocationMode.off.rawValue)
                    Text("While Using").tag(OriroLocationMode.whileUsing.rawValue)
                    Text("Always").tag(OriroLocationMode.always.rawValue)
                }
                .pickerStyle(.segmented)
