---
summary: "Create your first Oriro plugin in minutes"
title: "Building plugins"
sidebarTitle: "Getting Started"
doc-schema-version: 1
read_when:
  - You want to create a new Oriro plugin
  - You need a quick-start for plugin development
  - You are choosing between channel, provider, CLI backend, tool, or hook docs
---

Plugins extend Oriro without changing core. A plugin can add a messaging
channel, model provider, local CLI backend, agent tool, hook, media provider,
or another plugin-owned capability.

You do not need to add an external plugin to the Oriro repository. Publish
the package to [OriroHub](/orirohub) and users install it with:
...
`orirohub:` prefix when you want OriroHub resolution.
...
- For in-repo bundled plugin work, clone the repository and run `pnpm install`.
  Source-checkout plugin development is pnpm-only because Oriro loads bundled
...
<CardGroup cols={2}>
  <Card title="Channel plugin" icon="messages-square" href="/plugins/sdk-channel-plugins">
    Connect Oriro to a messaging platform.
  </Card>
  ...
  <Card title="CLI backend plugin" icon="terminal" href="/plugins/cli-backend-plugins">
    Run a local AI CLI through Oriro model fallback.
  </Card>
...
```json oriro.plugin.json
{
  "id": "my-plugin",
  "name": "My Plugin",
  "description": "Adds a custom tool to Oriro",
  "contracts": {
    "tools": ["my_tool"]
  },
  "activation": {
...
```
    ...
    must appear in `contracts.tools` so Oriro can discover ownership without
  ...
  <Step title="Test the runtime">
    ...
    ```bash
    oriro plugins inspect my-plugin --runtime --json
    ```
    ...
    `oriro demo-plugin ping`.

    For a bundled plugin in this repository, Oriro discovers source-checkout
  ...
  <Step title="Publish">
    ...
    The canonical OriroHub snippets live in `docs/snippets/plugin-publish/`.

  </Step>

  <Step title="Install">
    Install the published package through OriroHub:

    ```bash
    oriro plugins install orirohub:your-org/your-plugin
    ```
...
operator, installed plugin code, or a modified Oriro runtime. Sensitive local
...
aligned with `api.registerTool(..., { optional: true })` so Oriro can avoid
...
```typescript
import { definePluginEntry } from "oriro/plugin-sdk";
```
...
`oriro/plugin-sdk/gateway-method-runtime` bridge is reserved for plugin HTTP
...
<Check>**package.json** has correct `oriro` metadata</Check>
<Check>**oriro.plugin.json** manifest is present and valid</Check>
...
1. Watch for GitHub release tags on [oriro-ai/cli](https://github.com/oriro-ai/cli/releases) and subscribe via `Watch` > `Releases`. Beta tags look like `v2026.3.N-beta.1`. You can also turn on notifications for the official Oriro X account [@oriro](https://x.com/oriro) for release announcements.
