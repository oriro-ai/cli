...
const ORIRO_CLI_GATEWAY_TIMEOUT_MS = 30_000;
const ORIRO_CLI_CHILD_TIMEOUT_MS = ORIRO_CLI_GATEWAY_TIMEOUT_MS + 45_000;
...
export async function runOriroCliJson<T>(args: string[], env: NodeJS.ProcessEnv): Promise<T> {
  const childEnv = { ...env };
  ...
  const cliArgs = args.includes("--timeout")
    ? args
    : [...args, "--timeout", String(ORIRO_CLI_GATEWAY_TIMEOUT_MS)];
  const { stdout, stderr } = await execFileAsync(process.execPath, ["oriro.mjs", ...cliArgs], {
    cwd: process.cwd(),
    env: childEnv,
    timeout: ORIRO_CLI_CHILD_TIMEOUT_MS,
    maxBuffer: 1024 * 1024,
  });
  ...
  if (!trimmed) {
    throw new Error(
      [
        `oriro ${args.join(" ")} produced no JSON stdout`,
  ...
  try {
    ...
    throw new Error(
      [
        `oriro ${args.join(" ")} returned invalid JSON`,
        `stdout: ${trimmed}`,
        stderr.trim() ? `stderr: ${stderr.trim()}` : undefined,
...
export async function assertCronJobVisibleViaCli(params: {
  ...
  const cronList = await runOriroCliJson<CronListCliResult>(
    [
      "cron",
      "list",
      "--all",
