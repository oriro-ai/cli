...
read_when:
  - You want Oriro to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Oriro to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Bundled plugin

Current Oriro releases ship Nostr as a bundled plugin, so normal packaged
builds do not need a separate install.

### Older/custom installs

- Onboarding (`oriro onboard`) and `oriro channels add` still surface
  Nostr from the shared channel catalog.
- If your build excludes bundled Nostr, install the npm package directly.

```bash
oriro plugins install @oriro/nostr
```
...
```bash
oriro plugins install --link <path-to-local-nostr-plugin>
```
...
oriro channels add --channel nostr --private-key "$NOSTR_PRIVATE_KEY" --relay-urls "wss://relay.damus.io,wss://relay.primal.net"
```
  ...
  channels: {
    nostr: {
      ...
      profile: {
        ...
        nip05: "oriro@example.com",
        lud16: "oriro@example.com",
      },
    },
  },
}
