...
function resolveSystemdUnit(env: NodeJS.ProcessEnv): string {
  const override = normalizeOptionalString(env.ORIRO_SYSTEMD_UNIT);
  if (override) {
    return override.endsWith(".service") ? override : `${override}.service`;
  }
  return `${resolveGatewaySystemdServiceName(env.ORIRO_PROFILE)}.service`;
}

function resolveLaunchdLabel(env: NodeJS.ProcessEnv): string {
  const override = normalizeOptionalString(env.ORIRO_LAUNCHD_LABEL);
  if (override) {
    return override;
  }
  return resolveGatewayLaunchAgentLabel(env.ORIRO_PROFILE);
}

function resolveWindowsTaskName(env: NodeJS.ProcessEnv): string {
  const override = env.ORIRO_WINDOWS_TASK_NAME?.trim();
  if (override) {
    return override;
  }
  return resolveGatewayWindowsTaskName(env.ORIRO_PROFILE);
}
...
export async function prepareRestartScript(
  ...
  try {
    if (platform === "linux") {
      ...
      filename = `oriro-restart-${timestamp}.sh`;
...
exec 3>&2
${logSetup}
printf '[%s] oriro restart attempt source=update target=%s\\n' "$(date -u +%FT%TZ)" '${escaped}' >&2
if systemctl --user is-active --quiet '${escaped}' || systemctl --user is-enabled --quiet '${escaped}'; then
  if systemctl --user restart '${escaped}'; then
    status=0
    printf '[%s] oriro restart done source=update\\n' "$(date -u +%FT%TZ)" >&2
  else
    status=$?
  ...
  status=78
  printf '[%s] system-scoped oriro gateway unit detected; update cannot restart it without sudo. Run: sudo systemctl restart %s\\n' "$(date -u +%FT%TZ)" '${escaped}' >&2
...
`;
      ...
      filename = `oriro-restart-${timestamp}.cmd`;
...
setlocal
set "ORIRO_RESTART_SCRIPT=%~f0"
set "ORIRO_RESTART_SCRIPT_DIR=%~dp0."
powershell -NoProfile -ExecutionPolicy Bypass -Command "$p=$env:ORIRO_RESTART_SCRIPT; $s=Get-Content -Raw -LiteralPath $p; $m='# POWERSHELL'; $i=$s.IndexOf($m); if ($i -lt 0) { exit 1 }; Invoke-Expression $s.Substring($i)"
set "status=%ERRORLEVEL%"
del "%~f0" >nul 2>&1
rmdir "%ORIRO_RESTART_SCRIPT_DIR%" >nul 2>&1
exit /b %status%
# POWERSHELL
# Wait briefly to ensure file locks are released after update.
...
try {
  ...
  Add-Content -LiteralPath $logPath -Value "[$(Get-Date -Format o)] oriro restart log initialized"
} catch {
...
function Join-OriroProcessArguments {
...
function Invoke-OriroSchtasksWithTimeout {
  param(
    [string[]]$Arguments,
    [int]$TimeoutSeconds
  )
  ...
  try {
    ...
    $startInfo.Arguments = Join-OriroProcessArguments -Arguments $Arguments
    ...
    if (-not $process.WaitForExit($TimeoutSeconds * 1000)) {
      ...
      Write-RestartLog "oriro restart schtasks timeout source=update args=$($Arguments -join ' ')"
      return 124
    }
    ...
    Write-RestartLog "oriro restart schtasks failed source=update args=$($Arguments -join ' ') error=$($_.Exception.Message)"
    return 1
  }
}

function Get-OriroScheduledTaskState {
...
function Get-OriroListenerPids {
  param([int]$Port)
  $listenerPids = @()
...
function Invoke-OriroStartupLauncher {
  $launcherPath = Join-Path $env:USERPROFILE ".oriro\\gateway.cmd"
  if (-not (Test-Path -LiteralPath $launcherPath)) {
    Write-RestartLog "oriro restart startup launcher missing source=update path=$launcherPath"
    return 1
  }

  try {
    Start-Process -FilePath $launcherPath -WindowStyle Hidden | Out-Null
    Write-RestartLog "oriro restart launched startup fallback source=update path=$launcherPath"
    return 0
  } catch {
    Write-RestartLog "oriro restart startup fallback failed source=update error=$($_.Exception.Message)"
    return 1
  }
}
...
$port = ${port}
Write-RestartLog "oriro restart attempt source=update target=$taskName"

$taskState = Get-OriroScheduledTaskState -TaskName $taskName
if ($taskState -eq "Running") {
  $endStatus = Invoke-OriroSchtasksWithTimeout -Arguments @("/End", "/TN", $taskName) -TimeoutSeconds 10
  if ($endStatus -ne 0) {
    Write-RestartLog "oriro restart schtasks end did not complete cleanly source=update status=$endStatus"
  }
} else {
  Write-RestartLog "oriro restart skipped schtasks end source=update state=$taskState"
}

for ($attempt = 1; $attempt -le 10; $attempt++) {
  $listeners = @(Get-OriroListenerPids -Port $port)
  ...
  if ($attempt -eq 10) {
    foreach ($listenerPid in $listeners) {
      try {
        ...
        Write-RestartLog "oriro restart killed stale listener source=update pid=$listenerPid"
      } catch {
        Write-RestartLog "oriro restart failed to kill stale listener source=update pid=$listenerPid error=$($_.Exception.Message)"
      }
    }
    break
  }
...
$status = Invoke-OriroSchtasksWithTimeout -Arguments @("/Run", "/TN", $taskName) -TimeoutSeconds 30
if ($status -ne 0) {
  $status = Invoke-OriroStartupLauncher
}
if ($status -eq 0) {
  Write-RestartLog "oriro restart done source=update"
} else {
  Write-RestartLog "oriro restart failed source=update status=$status"
}
...
`;
    ...
    const scriptDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-restart-"));
