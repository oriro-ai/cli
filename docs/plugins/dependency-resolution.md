---
summary: "How Oriro installs plugin packages and resolves plugin dependencies"
read_when:
  - You are debugging plugin package installs
  - You are changing plugin startup, doctor, or package-manager install behavior
  - You are maintaining packaged Oriro installs or bundled plugin manifests
title: "Plugin dependency resolution"
sidebarTitle: "Dependencies"
---

Oriro keeps plugin dependency work at install/update time. Runtime loading
does not run package managers, repair dependency trees, or mutate the Oriro
package directory.
...
- SDK/core imports are peer or supplied Oriro imports
- local development plugins bring their own already-installed dependencies
- npm and git plugins are installed into Oriro-owned package roots

Oriro owns only the plugin lifecycle:
...
Oriro uses stable per-source roots:

- npm packages install into per-plugin projects under
  `~/.oriro/npm/projects/<encoded-package>`
- git packages clone under `~/.oriro/git`
...
```bash
cd ~/.oriro/npm/projects/<encoded-package>
npm install --omit=dev --omit=peer --legacy-peer-deps --ignore-scripts --no-audit --no-fund
```

`oriro plugins install npm-pack:<path.tgz>` uses that same per-plugin npm
project root for a local npm-pack tarball. Oriro reads the tarball's npm
...
`node_modules` beside the plugin package. Oriro scans the managed project
...
publishable lockfile during install, and Oriro's managed npm project root
supports it through the normal npm install path. Oriro-owned publishable
...
Oriro does not require it for community packages, but npm will respect it
when present.

Oriro-owned npm plugin packages can also publish with explicit
...
with `oriro.release.bundleRuntimeDependencies: false`; those packages still
...
`oriro` package does not bundle its full dependency tree.

Plugins that import `oriro/plugin-sdk/*` declare `oriro` as a peer
dependency. Oriro does not let npm install a separate registry copy of the
...
resolution/materialization and Oriro reasserts plugin-local
`node_modules/oriro` links for installed packages that declare the host peer
...
Local plugins are treated as developer-controlled directories. Oriro does not
...
oriro plugins install <source>
oriro doctor --fix
```

`doctor --fix` can clean legacy Oriro-generated dependency state and recover
...
Lightweight and core-critical bundled plugins are shipped as part of Oriro.
They should either have no heavy runtime dependency tree or be moved out to a
downloadable package on OriroHub/npm.
...
the same npm/git/OriroHub path as third-party plugins.

In source checkouts, Oriro treats the repository as a pnpm monorepo. After
...
| -------------------------------- | ------------------------------------- | -------------------------------------------------------------------- |
| `npm install -g oriro`        | Built runtime tree inside the package | Oriro package and explicit plugin install/update/doctor flows     |
...
| `oriro plugins install ...`   | Managed npm project/git/OriroHub root  | The plugin install/update flow                                       |

## Legacy cleanup

Older Oriro versions generated bundled-plugin dependency roots at startup or
...
`.oriro-runtime-deps*` manifests, generated plugin `node_modules`, install
...
Older npm installs also used a shared `~/.oriro/npm/node_modules` root.
