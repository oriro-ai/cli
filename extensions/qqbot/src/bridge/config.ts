...
import { coerceSecretRef, normalizeSecretInputString } from "oriro/plugin-sdk/secret-input";
...
export function listQQBotAccountIds(cfg: OriroConfig): string[] {
  return listAccountIds(cfg as unknown as Record<string, unknown>);
}

/** Resolve the default QQBot account ID. */
export function resolveDefaultQQBotAccountId(cfg: OriroConfig): string {
...
/** Apply account config updates back into the Oriro config object. */
