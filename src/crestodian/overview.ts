...
type OriroReferencePaths = Awaited<ReturnType<typeof resolveOriroReferencePaths>>;
...
type CrestodianOverviewDependencies = {
  ...
  resolveOriroReferencePaths?: typeof resolveOriroReferencePaths;
};
...
function buildAgentSummaries(cfg: OriroConfig): CrestodianAgentSummary[] {
...
function resolveFastTestReferences(env: NodeJS.ProcessEnv): OriroReferencePaths | undefined {
...
export async function loadCrestodianOverview(
  ...
  const resolveReferences = deps.resolveOriroReferencePaths ?? resolveOriroReferencePaths;
  ...
  return {
    ...
    references: {
      ...
      docsUrl: ORIRO_DOCS_URL,
      sourcePath: references.sourcePath ?? undefined,
      sourceUrl: ORIRO_SOURCE_URL,
    },
  };
}
...
export function formatCrestodianOverview(overview: CrestodianOverview): string {
  ...
  const issueLines =
    overview.config.issues.length > 0
      ...
      : [];
  return [
    "Crestodian online. Little oriros, typed tools.",
    "",
    `Config: ${configStatus}`,
