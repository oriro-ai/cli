...
const mocks = vi.hoisted(() => ({
  ...
  readConfigFileSnapshot: vi.fn(async () => ({ path: "/tmp/oriro.json" })),
  ...
  resolveMissingOfficialExternalChannelPluginRepairHint: ({ channelId }: { channelId: string }) =>
    mocks.missingOfficialExternalChannels.has(channelId)
      ? {
          ...
          channelId,
          label: "Feishu",
          installSpec: "@oriro/feishu",
          installCommand: "oriro plugins install @oriro/feishu",
          doctorFixCommand: "oriro doctor --fix",
          repairHint:
            "Install the official external plugin with: oriro plugins install @oriro/feishu, or run: oriro doctor --fix.",
        }
      : null,
}));
...
describe("channelsStatusCommand SecretRef fallback flow", () => {
  ...
  it("shows missing official external plugin repair hints in config-only output", async () => {
    ...
    expect(joined).toContain(
      "Feishu: Install the official external plugin with: oriro plugins install @oriro/feishu, or run: oriro doctor --fix.",
    );
  });
