...
import type { ConfigFileSnapshot, OriroConfig } from "../config/types.js";
...
/**
 * Test for issue #6070:
 * `oriro config set/unset` must update snapshot.resolved (user config after $include/${ENV},
...
function setSnapshot(resolved: OriroConfig, config: OriroConfig) {
...
function withRuntimeDefaults(resolved: OriroConfig): OriroConfig {
  return {
    ...resolved,
    agents: {
      ...resolved.agents,
...
function createPluginManifestRecord(
  ...
  return {
    ...
    cliBackends: [],
    hooks: [],
    manifestPath: `/tmp/${overrides.id}/oriro.plugin.json`,
    origin: "bundled",
    providers: [],
...
function makeInvalidSnapshot(params: {
  ...
  return {
    path: params.path ?? "/tmp/custom-oriro.json",
    exists: true,
    raw: "{}",
    parsed: {},
    sourceConfig: {},
...
describe("config cli", () => {
  ...
  describe("config set - issue #6070", () => {
    it("preserves existing config keys when setting a new value", async () => {
      ...
      const runtimeMerged: OriroConfig = {
    ...
    it("rejects plugin install record config updates", async () => {
      await expect(
        runConfigCommand([
          "config",
          "set",
          'plugins.installs["oriro-web-search"].spec',
          '"@ollama/oriro-web-search@0.2.2"',
          "--strict-json",
          "--dry-run",
        ]),
      ...
      expectErrorIncludes("oriro plugins install <spec>");
      expectErrorIncludes("oriro plugins update <plugin-id>");
    });
  ...
  describe("config get", () => {
    it("redacts sensitive values", async () => {
      ...
      expect(mockLog).toHaveBeenCalledWith("__ORIRO_REDACTED__");
    });
  ...
  describe("config validate", () => {
    ...
    it("returns machine-readable JSON with --json for invalid config", async () => {
      ...
      expect(payload.path).toBe("/tmp/custom-oriro.json");
  ...
  describe("config set builders and dry-run", () => {
    ...
    it("dry-runs config patch channel fields against plugin-owned schemas", async () => {
      ...
      setSnapshot(resolved, resolved);
      const pathname = writeTempJson5File("oriro-config-plugin-channel-schema", {
        channels: {
          feishu: {
            appId: "app-id",
            appSecret: "secret",
    ...
    it("rejects --allow-exec without --dry-run", async () => {
      const nonexistentBatchPath = path.join(
        os.tmpdir(),
        `oriro-config-batch-nonexistent-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
      );
      await expect(
    ...
    it("supports batch-file mode", async () => {
      ...
      const pathname = path.join(
        os.tmpdir(),
        `oriro-config-batch-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
      );
    ...
    it("batch-file nested leaf updates preserve agents defaults and list siblings", async () => {
      ...
      const pathname = path.join(
        os.tmpdir(),
        `oriro-config-memory-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
      );
      fs.writeFileSync(
        pathname,
        JSON.stringify([
    ...
    it("rejects malformed batch-file payloads", async () => {
      const pathname = path.join(
        os.tmpdir(),
        `oriro-config-batch-invalid-${Date.now()}-${Math.random().toString(16).slice(2)}.json`,
      );
      fs.writeFileSync(pathname, '{"path":"gateway.auth.mode","value":"token"}', "utf8");
      try {
    ...
    it("patches config from one object in one write", async () => {
      ...
      const pathname = path.join(
        os.tmpdir(),
        `oriro-config-patch-${Date.now()}-${Math.random().toString(16).slice(2)}.json5`,
      );
      fs.writeFileSync(
        pathname,
        JSON.stringify({
    ...
    it("preserves empty object values in config patch", async () => {
      ...
      const pathname = writeTempJson5File("oriro-config-patch-empty-object", {
        agents: {
          defaults: {
            models: {
              "openai/gpt-5.5": {},
    ...
    it("treats empty object config patches as recursive merges", async () => {
      ...
      const pathname = writeTempJson5File("oriro-config-patch-empty-merge", {
        channels: {
          slack: {},
        },
      });
    ...
    it("keeps numeric config patch object keys as object keys", async () => {
      ...
      const pathname = writeTempJson5File("oriro-config-patch-numeric-object-key", {
        channels: {
          discord: {
            guilds: {
              "123456789012345678": {
    ...
    it("dry-runs config patch and resolves changed SecretRefs", async () => {
      ...
      const pathname = path.join(
        os.tmpdir(),
        `oriro-config-patch-dry-${Date.now()}-${Math.random().toString(16).slice(2)}.json5`,
      );
      fs.writeFileSync(
        pathname,
        JSON.stringify({
    ...
    it("dry-runs pluginIntegration provider patches against manifest integration metadata", async () => {
      ...
      const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-config-plugin-provider-"));
      try {
          ...
          createPluginMetadataSnapshot({
            ...
            plugins: [
              createPluginManifestRecord({
                ...
                manifestPath: path.join(rootDir, "oriro.plugin.json"),
        ...
        setSnapshot(resolved, resolved);
        const validPatch = writeTempJson5File("oriro-config-plugin-provider-valid", {
          secrets: {
            providers: {
              team: {
                source: "exec",
        ...
        setSnapshot(resolved, resolved);
        const invalidPatch = writeTempJson5File("oriro-config-plugin-provider-invalid", {
          secrets: {
            providers: {
              team: {
                source: "exec",
    ...
    it("does not revalidate untouched pluginIntegration providers when disabling plugins", async () => {
      ...
      const patch = writeTempJson5File("oriro-config-plugin-disable", {
        plugins: {
          entries: {
    ...
    it("validates pluginIntegration providers referenced by newly assigned SecretRefs", async () => {
      ...
      const patch = writeTempJson5File("oriro-config-plugin-provider-ref", {
        gateway: {
          auth: {
    ...
    it("schema-validates SecretRef-only config patch operations", async () => {
      ...
      const pathname = path.join(
        os.tmpdir(),
        `oriro-config-patch-ref-schema-${Date.now()}-${Math.random()
          .toString(16)
          .slice(2)}.json5`,
      );
      fs.writeFileSync(
    ...
    it("dry-runs nested SecretRefs inside config patch replacements", async () => {
      ...
      const pathname = path.join(
        os.tmpdir(),
        `oriro-config-patch-nested-ref-${Date.now()}-${Math.random()
          .toString(16)
          .slice(2)}.json5`,
      );
      fs.writeFileSync(
    ...
    it("supports replace-path and null deletes in config patch", async () => {
      ...
      const pathname = path.join(
        os.tmpdir(),
        `oriro-config-patch-replace-${Date.now()}-${Math.random().toString(16).slice(2)}.json5`,
      );
      fs.writeFileSync(
        pathname,
        JSON.stringify({
    ...
    it("rejects unused config patch replace paths", async () => {
      const pathname = path.join(
        os.tmpdir(),
        `oriro-config-patch-unused-replace-${Date.now()}-${Math.random()
          .toString(16)
          .slice(2)}.json5`,
      );
      fs.writeFileSync(
  ...
  describe("path hardening", () => {
    ...
    it("rejects impractical array indexes for config set", async () => {
      const resolved = { agents: { list: [] } } as unknown as OriroConfig;
      setSnapshot(resolved, resolved);
    ...
    it("rejects signed array indexes for config set", async () => {
      const resolved = { agents: { list: [{ id: "main" }] } } as unknown as OriroConfig;
      setSnapshot(resolved, resolved);
  ...
  describe("config unset - issue #6070", () => {
    ...
    it("dry-runs an unset without writing the config file", async () => {
      ...
      expect(mockWriteConfigFile).not.toHaveBeenCalled();
      expectLogIncludes("Dry run successful: 1 update(s) validated against /tmp/oriro.json.");
  ...
  describe("config file", () => {
    it("prints the active config file path", async () => {
      ...
      expect(mockLog).toHaveBeenCalledWith("/tmp/oriro.json");
      expect(mockWriteConfigFile).not.toHaveBeenCalled();
    });
