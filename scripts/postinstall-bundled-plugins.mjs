...
const DISABLE_POSTINSTALL_ENV = "ORIRO_DISABLE_BUNDLED_PLUGIN_POSTINSTALL";
const DISABLE_PLUGIN_REGISTRY_MIGRATION_ENV = "ORIRO_DISABLE_PLUGIN_REGISTRY_MIGRATION";
...
function resolvePostinstallOriroHomeDir(env, getHomedir = homedir) {
  const osHome = resolvePostinstallOsHomeDir(env, getHomedir);
  const override = env?.ORIRO_HOME?.trim();
...
function isLegacyInstalledPluginDependencyDirName(name) {
  return name === "node_modules" || /^\.oriro-install-stage(?:-[^/]+)?$/iu.test(name);
}
  ...
  const createTempPath =
    ...
    ((unsafeTargetPath) =>
      join(
        ...
        `.${basename(unsafeTargetPath)}.oriro-hotfix-${randomUUID()}`,
      ));
  const writeFile =
...
export function pruneOriroCompileCache(params = {}) {
  const env = params.env ?? process.env;
  const pathExists = params.existsSync ?? existsSync;
  const readDir = params.readdirSync ?? readdirSync;
