...
import { resolveOriroMetadata, resolveSkillInvocationPolicy } from "./frontmatter.js";
...
const SKILL_SOURCE_ORIGIN_RELATIVE_PATH = path.join(".oriro", "source-origin.json");
...
function isContainerStateHomeWherePromptTildeEscapes(home: string): boolean {
  ...
  return home === "/data" && (configDir === "/data/.oriro" || isPathInside("/data/.oriro", configDir));
}
...
function resolveSkillsLimits(config?: OriroConfig, agentId?: string): ResolvedSkillsLimits {
...
function buildEscapedSkillPathReason(params: { source: string; candidatePath: string }): {
  ...
  if (params.source === "oriro-bundled" && candidateIsSymlink) {
    return {
  ...
  if (params.source === "oriro-bundled") {
    return {
...
function resolveSkillEntryMetadata(params: {
  frontmatter: ParsedSkillFrontmatter;
  skillDir: string;
}): OriroSkillMetadata | undefined {
  const metadata = resolveOriroMetadata(params.frontmatter);
...
function shouldEnforceConfiguredSkillRootContainment(source: string): boolean {
  return source !== "oriro-managed" && source !== "agents-skills-personal";
}
...
function loadGeneratedPluginSkillRecords(params: {
  ...
  for (const name of childDirs) {
    ...
    // Plugin skills live as symlinks under ~/.oriro/plugin-skills/, so
...
function buildSkillsLimitNote(params: {
  ...
  if (params.truncated) {
    return `⚠️ Skills truncated: included ${params.included} of ${params.total}${params.compact ? " (compact format, descriptions omitted)" : ""}. Run \`oriro skills check\` to audit.`;
  }
  if (params.compact) {
    return `⚠️ Skills catalog using compact format (descriptions omitted). Run \`oriro skills check\` to audit.`;
  }
  return "";
}
