...
Paste this into `oriro.json` for a safe-default setup — plugin on, scoped to
...
With those enabled, Oriro can show:
...
case-insensitive. If `allowedChatIds` is non-empty and Oriro cannot resolve a
...
Lossless Oriro is a context-engine plugin with its own recall tools. Install and
configure it as a context engine first; see [Context engine](/concepts/context-engine).
Then let Active Memory use the Lossless Oriro recall tools:
...
Lossless Oriro uses that as a lower-level delegated expansion tool.
...
`config.promptOverride` replaces the default Active Memory prompt. Oriro
...
5. Verify memory search itself works with `oriro memory status --deep`.
...
<AccordionGroup>
  <Accordion title="Embedding provider switched or stopped working">
    If `memorySearch.provider` is unset, Oriro uses OpenAI embeddings. Set
  ...
  <Accordion title="Recall feels slow, empty, or inconsistent">
    ...
    - Run `oriro memory status --deep` to inspect the memory-search backend
      and index health.
