...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["ORIRO_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/oriro")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("oriro-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("oriro-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/oriro/ (rolling or stdout), or nil if none exist.
