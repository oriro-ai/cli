// Builds PATH values for Oriro child processes.
...
function candidateBinDirs(
  ...
  // subprocesses keep using the same Node/Bun the current Oriro process is on.
  try {
  ...
  // Bundled macOS app: `oriro` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "oriro");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.ORIRO_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "oriro"))) {
  ...
  // This includes Brew/Homebrew dirs, which are useful for finding `oriro`
...
/**
 * Best-effort PATH bootstrap so skills that require the `oriro` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureOriroCliOnPath(opts: EnsureOriroPathOpts = {}) {
  if (isTruthyEnvValue(process.env.ORIRO_PATH_BOOTSTRAPPED)) {
    return;
  }
  // Mark before filesystem probing so repeated calls from nested bootstraps do
  // not keep reshuffling PATH.
  process.env.ORIRO_PATH_BOOTSTRAPPED = "1";
