...
export function resolveModelRuntimePolicy(params: {
  ...
  if (process.env.ORIRO_BUILD_PRIVATE_QA === "1") {
    const forcedRuntime = process.env.ORIRO_QA_FORCE_RUNTIME?.trim().toLowerCase();
    if (forcedRuntime === "oriro" || forcedRuntime === "codex") {
