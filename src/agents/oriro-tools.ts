/**
 * Oriro built-in and plugin tool assembly.
 *
...
import { resolveOriroPluginToolsForOptions } from "./oriro-plugin-tools.js";
import {
  isToolExplicitlyAllowedByFactoryPolicy,
  mergeFactoryPolicyList,
  resolveImageToolFactoryAvailable,
  resolveOptionalMediaToolFactoryPlan,
} from "./oriro-tools.media-factory-plan.js";
import { applyNodesToolWorkspaceGuard } from "./oriro-tools.nodes-workspace-guard.js";
import {
  collectPresentOriroTools,
  shouldIncludeUpdatePlanToolForOriroTools,
} from "./oriro-tools.registration.js";
...
const defaultOriroToolsDeps: OriroToolsDeps = {
  callGateway,
};

let openOriroToolsDeps: OriroToolsDeps = defaultOriroToolsDeps;

export function createOriroTools(
  options?: {
  ...
  const resolvedConfig = options?.config ?? openOriroToolsDeps.config;
  ...
  options?.recordToolPrepStage?.("oriro-tools:session-workspace");
  ...
  const imageTool = resolveImageToolFactoryAvailable({
    ...
    : null;
  options?.recordToolPrepStage?.("oriro-tools:image-tool");
  const imageGenerateTool = optionalMediaTools.imageGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("oriro-tools:image-generate-tool");
  const videoGenerateTool = optionalMediaTools.videoGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("oriro-tools:video-generate-tool");
  const musicGenerateTool = optionalMediaTools.musicGenerate
    ...
    : null;
  options?.recordToolPrepStage?.("oriro-tools:music-generate-tool");
  const pdfTool =
    optionalMediaTools.pdf && options?.agentDir?.trim()
      ...
      : null;
  options?.recordToolPrepStage?.("oriro-tools:pdf-tool");
  ...
  options?.recordToolPrepStage?.("oriro-tools:web-search-tool");
  ...
  options?.recordToolPrepStage?.("oriro-tools:web-fetch-tool");
  ...
  options?.recordToolPrepStage?.("oriro-tools:message-tool");
  ...
  options?.recordToolPrepStage?.("oriro-tools:nodes-tool");
  ...
  const effectiveCallGateway = embedded
    ...
    : openOriroToolsDeps.callGateway;
  ...
  const tools: AnyAgentTool[] = [
    ...
    ...collectPresentOriroTools([heartbeatTool]),
    createTtsTool({
      agentChannel: options?.agentChannel,
      config: resolvedConfig,
      agentId: sessionAgentId,
    ...
    ...collectPresentOriroTools([imageGenerateTool, musicGenerateTool, videoGenerateTool]),
    ...(embedded
      ? []
      : [
          createGatewayTool({
    ...
    ...(embedded
      ...
      : [
          createSessionsSendTool({
            ...
            sandboxed: options?.sandboxed,
            config: resolvedConfig,
            callGateway: openOriroToolsDeps.callGateway,
          }),
        ]),
    ...
    ...collectPresentOriroTools([webSearchTool, webFetchTool, imageTool, pdfTool]),
  ];
  options?.recordToolPrepStage?.("oriro-tools:core-tool-list");
  let allTools = tools;
  if (!options?.disablePluginTools) {
    ...
    allTools = [
      ...tools,
      ...resolveOriroPluginToolsForOptions({
        options,
        resolvedConfig,
        existingToolNames,
      }),
    ];
    options?.recordToolPrepStage?.("oriro-tools:plugin-tools");
  }
  ...
  options?.recordToolPrepStage?.("oriro-tools:tool-hooks");
...
export const testing = {
  resolveOptionalMediaToolFactoryPlan,
  setDepsForTest(overrides?: Partial<OriroToolsDeps>) {
    openOriroToolsDeps = overrides
      ? {
          ...defaultOriroToolsDeps,
          ...overrides,
        }
      : defaultOriroToolsDeps;
  },
