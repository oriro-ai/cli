...
On the first agent run, Oriro bootstraps the workspace (default
`~/.oriro/workspace`):
...
For embedded/local model runs, Oriro keeps `BOOTSTRAP.md` out of the
...
To skip this for a pre-seeded workspace, run `oriro onboard --skip-bootstrap`.
...
host (for example, `user@gateway-host:~/.oriro/workspace`).
</Note>
