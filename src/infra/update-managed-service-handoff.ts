...
function resolveGatewayServiceRecovery(
  ...
  if (supervisor === "systemd") {
    const override = env.ORIRO_SYSTEMD_UNIT?.trim();
    const unit = override
      ? override.endsWith(".service")
        ? override
        : `${override}.service`
      : `${resolveGatewaySystemdServiceName(env.ORIRO_PROFILE)}.service`;
    return { kind: "systemd", unit };
  }
  if (supervisor === "launchd") {
    const label =
      env.ORIRO_LAUNCHD_LABEL?.trim() || resolveGatewayLaunchAgentLabel(env.ORIRO_PROFILE);
  ...
  if (supervisor === "schtasks") {
    const taskName =
      env.ORIRO_WINDOWS_TASK_NAME?.trim() || resolveGatewayWindowsTaskName(env.ORIRO_PROFILE);
...
function buildSystemdHandoffUnitName(handoffId: string | undefined): string {
  const suffix =
    ...
    "handoff";
  return `oriro-update-${suffix}.scope`;
}
...
async function resolveHandoffSpawn(params: {
  ...
  if (!systemdRunPath) {
    throw new Error(
      "systemd-run is required to start the managed update handoff outside oriro-gateway.service",
    );
  }
...
export async function startManagedServiceUpdateHandoff(params: {
  ...
  const helperParams = {
    ...
    logPath,
    metaPath,
    stateDatabasePath: resolveOriroStateSqlitePath(params.env ?? process.env),
