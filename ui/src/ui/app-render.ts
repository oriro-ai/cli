...
const SKILL_WORKSHOP_MODE_KEY = "oriro:control-ui:skill-workshop-mode:v1";
const SKILL_WORKSHOP_CURRENT_CHAT_REVISIONS_KEY =
  "oriro:control-ui:skill-workshop-current-chat-revisions:v1";
...
let orirohubSearchTimer: ReturnType<typeof setTimeout> | null = null;

const UPDATE_BANNER_DISMISS_KEY = "oriro:control-ui:update-banner-dismissed:v1";
                ...
                ${navCollapsed
                  ...
                  : html`
                      ...
                      <span class="sidebar-brand__copy">
                        ...
                        <span class="sidebar-brand__title">Oriro</span>
                      </span>
                    `}
              </div>
              <button
        ...
        ${state.tab === "skills"
          ? renderLazyView(lazySkills, (m) =>
              m.renderSkills({
                ...
                orirohubVerdicts: state.orirohubVerdicts,
                orirohubVerdictsLoading: state.orirohubVerdictsLoading,
                orirohubVerdictsError: state.orirohubVerdictsError,
                ...
                orirohubQuery: state.orirohubSearchQuery,
                orirohubResults: state.orirohubSearchResults,
                orirohubSearchLoading: state.orirohubSearchLoading,
                orirohubSearchError: state.orirohubSearchError,
                orirohubDetail: state.orirohubDetail,
                orirohubDetailSlug: state.orirohubDetailSlug,
                orirohubDetailLoading: state.orirohubDetailLoading,
                orirohubDetailError: state.orirohubDetailError,
                orirohubInstallSlug: state.orirohubInstallSlug,
                orirohubInstallMessage: state.orirohubInstallMessage,
                ...
                onOriroHubQueryChange: (query) => {
                  setOriroHubSearchQuery(state, query);
                  if (orirohubSearchTimer) {
                    clearTimeout(orirohubSearchTimer);
                  }
                  orirohubSearchTimer = setTimeout(() => {
                    void searchOriroHub(state, query);
                  }, 300);
                },
                onOriroHubDetailOpen: (slug) => void loadOriroHubDetail(state, slug),
                onOriroHubDetailClose: () => closeOriroHubDetail(state),
                onOriroHubInstall: (slug) => void installFromOriroHub(state, slug),
              }),
            )
          : nothing}
        ${state.tab === "skillWorkshop"
