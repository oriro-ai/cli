...
import { isRecord } from "oriro/plugin-sdk/string-coerce-runtime";
...
export function createCodexCliSessionNodeHostCommands(): OriroPluginNodeHostCommand[] {
  return [
    {
...
export function createCodexCliSessionNodeInvokePolicies(): OriroPluginNodeInvokePolicy[] {
  return [
    {
...
async function runCodexExecResume(params: {
  ...
  const outputPath = path.join(
    await fs.mkdtemp(path.join(resolvePreferredOriroTmpDir(), "oriro-codex-cli-")),
    "last-message.txt",
  );
  try {
    const args = [
