// Validates normalized Oriro config and reports user-facing errors.
import path from "node:path";
import { collectConfiguredModelRefs } from "@oriro/model-catalog-core/configured-model-refs";
import { isCanonicalDottedDecimalIPv4, isLoopbackIpAddress } from "@oriro/net-policy/ip";
...
import type { OriroConfig, ConfigValidationIssue } from "./types.js";
...
function materializeBundledModelProviderOverlays(config: OriroConfig): OriroConfig {
...
const SECRETREF_POLICY_DOC_URL = "https://docs.oriro.ai/reference/secretref-credential-surface";
...
function formatMissingOfficialExternalPluginWarning(
  ...
  const installSpec =
    install?.defaultChoice === "orirohub" ? (orirohubSpec ?? npmSpec) : (npmSpec ?? orirohubSpec);
  if (!installSpec) {
    return null;
  }
  if (pluginId === "memory-lancedb" && opts?.selectedMissingMemorySlot) {
    return `plugin not installed: ${pluginId} — gateway will run without persistent memory until installed; install the official external plugin with: oriro plugins install ${installSpec}`;
  }
  return `plugin not installed: ${pluginId} — install the official external plugin with: oriro plugins install ${installSpec}`;
}
...
function hasChannelDmPolicyDependencyWarningCandidates(config: OriroConfig): boolean {
...
function collectRawBundledChannelConfigIssues(config: OriroConfig): ConfigValidationIssue[] {
  if (!config.channels || !isRecord(config.channels)) {
    return [];
  }
...
function validateIdentityAvatar(config: OriroConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleBind(config: OriroConfig): ConfigValidationIssue[] {
...
function validateGatewayTailscaleAuth(config: OriroConfig): ConfigValidationIssue[] {
  const tailscaleMode = config.gateway?.tailscale?.mode ?? "off";
...
export function validateConfigObjectRaw(
...
): { ok: true; config: OriroConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = OriroSchema.safeParse(normalizedRaw);
  ...
  const validatedConfig = materializeBundledModelProviderOverlays(validated.data as OriroConfig);
  const channelIssues =
...
function validateConfigObjectWithPluginsBase(
  ...
  let compatConfig: OriroConfig | null | undefined;
  ...
  const ensureCompatConfig = (): OriroConfig => {
  ...
  const validateWebSearchProvider = () => {
    ...
    if (installCatalogEntry) {
      const issue = {
        path: pathEntry,
        message: `web_search provider is not available: ${trimmed} (install or enable plugin "${installCatalogEntry.pluginId}", then run oriro doctor --fix)`,
      ...
      if (hasPluginEvidenceForWebSearchProvider(trimmed, installCatalogEntry.pluginId)) {
        warnings.push({
          ...issue,
          message: `web_search provider is not available: ${trimmed} (configured plugin "${installCatalogEntry.pluginId}" is unavailable; Gateway will ignore this optional provider until the plugin is installed/enabled or oriro doctor --fix repairs the config)`,
        });
        return;
      }
      issues.push(issue);
    ...
    if (hasStalePluginEvidenceForUnknownWebSearchProvider(trimmed)) {
      warnings.push({
        ...issue,
        message: `${issue.message} (stale web search plugin config ignored; run oriro doctor --fix to remove stale config, or install the plugin)`,
      });
      return;
    }
    issues.push(issue);
    ...
    for (const key of Object.keys(config.channels)) {
      ...
      if (!allowedChannels.has(trimmed)) {
        ...
        if (hasStalePluginEvidenceForUnknownChannel(trimmed)) {
          warnings.push({
            ...issue,
            message: `${issue.message} (stale channel plugin config ignored; run oriro doctor --fix to remove stale config, or install the plugin)`,
          });
        } else {
          issues.push(issue);
        }
  ...
  for (const record of registry.plugins) {
    ...
    if (shouldValidate) {
      if (record.configSchema) {
        ...
        // Compatible bundles currently expose no native Oriro config schema.
