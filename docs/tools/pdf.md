...
The tool is only registered when Oriro can resolve a PDF-capable model config for the agent:
...
- The fallback chain is auth-aware. A configured `provider/model` only counts if
  Oriro can actually authenticate that provider for the agent.
...
- `file://` URL
- `http://` and `https://` URL
- Oriro-managed inbound refs such as `media://inbound/<id>`
...
- Managed inbound refs and replayed paths under Oriro's inbound media store are allowed with workspace-only file policy.
...
- If text extraction succeeds but image extraction would require vision on a
  text-only model, Oriro drops the rendered images and continues with the
  extracted text.
- Extraction fallback uses the bundled `document-extract` plugin. The plugin owns
  `oriropdf`, which provides text extraction and image rendering through PDFium
  WebAssembly.
