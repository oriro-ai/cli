---
summary: "Optional Docker-based setup and onboarding for Oriro"
read_when:
...
- **Yes**: you want an isolated, throwaway gateway environment or to run Oriro on a host without local installs.
...
<Steps>
  <Step title="Build the image">
    ...
    ```bash
    export ORIRO_IMAGE="ghcr.io/oriro-ai/cli:latest"
    ./scripts/docker/setup.sh
    ```

    Pre-built images are published at the
    [GitHub Container Registry](https://github.com/oriro-ai/cli/pkgs/container/oriro).
  ...
  <Step title="Airgapped rerun">
    ...
    ```bash
    docker load -i oriro-image.tar
    export ORIRO_IMAGE="ghcr.io/oriro-ai/cli:latest"
    ./scripts/docker/setup.sh --offline
    ```

    `--offline` verifies that `ORIRO_IMAGE` already exists locally, disables
    ...
    If `ORIRO_SANDBOX=1`, offline setup also checks the configured default
    and active per-agent sandbox images on the daemon behind
    `ORIRO_DOCKER_SOCKET`. Docker-backed browser images must also carry the
    current Oriro browser contract label. When a required image is missing or
  ...
  <Step title="Complete onboarding">
    ...
    `oriro-gateway` directly. `oriro-cli` is for commands you run after
  ...
  <Step title="Open the Control UI">
    ...
    ```bash
    docker compose run --rm oriro-cli dashboard --no-open
    ```
  ...
  <Step title="Configure channels (optional)">
    ...
    ```bash
    # WhatsApp (QR)
    docker compose run --rm oriro-cli channels login

    # Telegram
    docker compose run --rm oriro-cli channels add --channel telegram --token "<token>"

    # Discord
    docker compose run --rm oriro-cli channels add --channel discord --token "<token>"
    ```
...
```bash
docker build -t oriro:local -f Dockerfile .
docker compose run --rm --no-deps --entrypoint node oriro-gateway \
  dist/index.js onboard --mode local --no-install-daemon
docker compose run --rm --no-deps --entrypoint node oriro-gateway \
  dist/index.js config set --batch-json '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"},{"path":"gateway.controlUi.allowedOrigins","value":["http://localhost:18789","http://127.0.0.1:18789"]}]'
docker compose up -d oriro-gateway
```

<Note>
Run `docker compose` from the repo root. If you enabled `ORIRO_EXTRA_MOUNTS`
or `ORIRO_HOME_VOLUME`, the setup script writes `docker-compose.extra.yml`;
...
<Note>
Because `oriro-cli` shares `oriro-gateway`'s network namespace, it is a
post-start tool. Before `docker compose up -d oriro-gateway`, run onboarding
and setup-time config writes through `oriro-gateway` with
...
| Variable                                   | Purpose                                                               |
| ------------------------------------------ | --------------------------------------------------------------------- |
| `ORIRO_IMAGE`                           | Use a remote image instead of building locally                        |
| `ORIRO_IMAGE_APT_PACKAGES`              | Install extra apt packages during build (space-separated)             |
| `ORIRO_IMAGE_PIP_PACKAGES`              | Install extra Python packages during build (space-separated)          |
| `ORIRO_EXTENSIONS`                      | Pre-install plugin dependencies at build time (space-separated names) |
| `ORIRO_EXTRA_MOUNTS`                    | Extra host bind mounts (comma-separated `source:target[:opts]`)       |
| `ORIRO_HOME_VOLUME`                     | Persist `/home/node` in a named Docker volume                         |
| `ORIRO_SANDBOX`                         | Opt in to sandbox bootstrap (`1`, `true`, `yes`, `on`)                |
| `ORIRO_SKIP_ONBOARDING`                 | Skip the interactive onboarding step (`1`, `true`, `yes`, `on`)       |
| `ORIRO_DOCKER_SOCKET`                   | Override Docker socket path                                           |
| `ORIRO_DISABLE_BONJOUR`                 | Disable Bonjour/mDNS advertising (defaults to `1` for Docker)         |
| `ORIRO_DISABLE_BUNDLED_SOURCE_OVERLAYS` | Disable bundled plugin source bind-mount overlays                     |
...
| `ORIRO_OTEL_PRELOADED`                  | Skip starting a second OpenTelemetry SDK when one is preloaded        |

The official Docker image does not ship Homebrew. During onboarding, Oriro
...
`ORIRO_IMAGE_APT_PACKAGES` during image build. The legacy
`ORIRO_DOCKER_APT_PACKAGES` name is still accepted.
For Python dependencies, use `ORIRO_IMAGE_PIP_PACKAGES`. This runs
...
`ORIRO_EXTRA_MOUNTS=/path/to/fork/extensions/synology-chat:/app/extensions/synology-chat:ro`.
...
```bash
export ORIRO_EXTENSIONS="diagnostics-otel"
export OTEL_EXPORTER_OTLP_ENDPOINT="http://otel-collector:4318"
export OTEL_SERVICE_NAME="oriro-gateway"
./scripts/docker/setup.sh
```

Install the official `@oriro/diagnostics-otel` plugin from OriroHub in
packaged Docker installs before enabling export. Custom source-built images can
still include the local plugin source with
`ORIRO_EXTENSIONS=diagnostics-otel`. To enable export, allow and enable the
...
`orirohub:@oriro/diagnostics-prometheus`, enable the
...
```bash
docker compose exec oriro-gateway node dist/index.js health --token "$ORIRO_GATEWAY_TOKEN"
```

### LAN vs loopback

`scripts/docker/setup.sh` defaults `ORIRO_GATEWAY_BIND=lan` so host access to
...
When Oriro runs in Docker, `127.0.0.1` inside the container is the container
...
`ORIRO_DISABLE_BONJOUR=1` so the Gateway does not crash-loop or repeatedly
...
Set `ORIRO_DISABLE_BONJOUR=0` only when running with host networking, macvlan,
...
Docker Compose bind-mounts `ORIRO_CONFIG_DIR` to `/home/node/.oriro`,
`ORIRO_WORKSPACE_DIR` to `/home/node/.oriro/workspace`, and
`ORIRO_AUTH_PROFILE_SECRET_DIR` to `/home/node/.config/oriro`, so those
...
That mounted config directory is where Oriro keeps:

- `oriro.json` for behavior config
- `agents/<agentId>/agent/auth-profiles.json` for stored provider OAuth/API-key auth
- `.env` for env-backed runtime secrets such as `ORIRO_GATEWAY_TOKEN`
...
but separate from `ORIRO_CONFIG_DIR`.

Installed downloadable plugins store their package state under the mounted
Oriro home, so plugin install records and package roots survive container
...
For easier day-to-day Docker management, install `OriroDock`:
...
If you installed OriroDock from the older `scripts/shell-helpers/orirodock-helpers.sh` raw path, rerun the install command above so your local helper file tracks the new location.

Then use `orirodock-start`, `orirodock-stop`, `orirodock-dashboard`, etc. Run
`orirodock-help` for all commands.
See [OriroDock](/install/orirodock) for the full helper guide.

<AccordionGroup>
  <Accordion title="Enable agent sandbox for Docker gateway">
    ```bash
    export ORIRO_SANDBOX=1
    ./scripts/docker/setup.sh
    ```
    ...
    ```bash
    export ORIRO_SANDBOX=1
    export ORIRO_DOCKER_SOCKET=/run/user/1000/docker.sock
    ./scripts/docker/setup.sh
    ```
    ...
    `workspace-write` while the Oriro sandbox is active; do not mount the
  ...
  <Accordion title="Automation / CI (non-interactive)">
    ...
    ```bash
    docker compose run -T --rm oriro-cli gateway probe
    docker compose run -T --rm oriro-cli devices list --json
    ```

  </Accordion>

  <Accordion title="Shared-network security note">
    `oriro-cli` uses `network_mode: "service:oriro-gateway"` so CLI
    ...
    `no-new-privileges` on both `oriro-gateway` and `oriro-cli`.
  </Accordion>

  <Accordion title="Docker Desktop DNS failures in oriro-cli">
    Some Docker Desktop setups fail DNS lookups from the shared-network
    `oriro-cli` sidecar after `NET_RAW` is dropped, which shows up as
    `EAI_AGAIN` during npm-backed commands such as `oriro plugins install`.
    ...
    docker compose -f docker-compose.yml -f docker-compose.cli-no-dropped-caps.local.yml run --rm oriro-cli plugins install <package>
    ```

    If you already created a long-running `oriro-cli` container, recreate it
  ...
  <Accordion title="Permissions and EACCES">
    ...
    `/home/node/.oriro`, make sure your host bind mounts are owned by uid 1000:

    ```bash
    sudo chown -R 1000:1000 /path/to/oriro-config /path/to/oriro-workspace
    ```
    ...
    `/path/to/oriro-config/npm` to `root:root` if you intentionally run
    Oriro as root long term.
  ...
  <Accordion title="Power-user container options">
    ...
    1. **Persist `/home/node`**: `export ORIRO_HOME_VOLUME="oriro_home"`
    2. **Bake system deps**: `export ORIRO_IMAGE_APT_PACKAGES="git curl jq"`
    3. **Bake Python deps**: `export ORIRO_IMAGE_PIP_PACKAGES="requests==2.32.5 humanize==4.14.0"`
    4. **Bake Playwright Chromium**: `export ORIRO_INSTALL_BROWSER=1`
    5. **Or install Playwright browsers into a persisted volume**:
       ```bash
       docker compose run --rm oriro-cli \
         node /app/node_modules/playwright-core/cli.js install chromium
       ```
    6. **Persist browser downloads**: use `ORIRO_HOME_VOLUME` or
       `ORIRO_EXTRA_MOUNTS`. Oriro auto-detects the Docker image's
...
<AccordionGroup>
  <Accordion title="Image missing or sandbox container not starting">
    ...
    [`scripts/sandbox-setup.sh`](https://github.com/oriro-ai/cli/blob/main/scripts/sandbox-setup.sh)
  ...
  <Accordion title="Custom tools not found in sandbox">
    Oriro runs commands with `sh -lc` (login shell), which sources
  ...
  <Accordion title="Unauthorized or pairing required in Control UI">
    ...
    ```bash
    docker compose run --rm oriro-cli dashboard --no-open
    docker compose run --rm oriro-cli devices list
    docker compose run --rm oriro-cli devices approve <requestId>
    ```
  ...
  <Accordion title="Gateway target shows ws://172.x.x.x or pairing errors from Docker CLI">
    ...
    ```bash
    docker compose run --rm oriro-cli config set --batch-json '[{"path":"gateway.mode","value":"local"},{"path":"gateway.bind","value":"lan"}]'
    docker compose run --rm oriro-cli devices list --url ws://127.0.0.1:18789
    ```
...
- [OriroDock](/install/orirodock) — Docker Compose community setup
- [Updating](/install/updating) — keeping Oriro up to date
