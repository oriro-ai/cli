...
import type { PluginLogger } from "oriro/plugin-sdk/plugin-entry";
import { isTruthyEnvValue } from "oriro/plugin-sdk/runtime-env";
...
/** Input data used to publish Oriro gateway Bonjour records. */
...
function readBonjourDisableOverride(): boolean | null {
  const raw = process.env.ORIRO_DISABLE_BONJOUR;
...
function prettifyInstanceName(name: string) {
  ...
  return normalized.replace(/\s+\(Oriro\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  try {
    ...
    const hostnameRaw =
      process.env.ORIRO_MDNS_HOSTNAME?.trim() || resolveSystemMdnsHostname() || "oriro";
    ...
    const recreateAdvertiser = async (reason: string, optsLocal?: { stuckState?: boolean }) => {
      ...
      recreatePromise = (async () => {
        ...
        if (tooManyConsecutive || tooManyStuckStates || tooManyInWindow) {
          ...
          logger.warn(
            `bonjour: disabling advertiser after ${detail} (${reason}); set discovery.mdns.mode="off" or ORIRO_DISABLE_BONJOUR=1 to disable mDNS discovery`,
          );
