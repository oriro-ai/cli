...
import { loadSecretFileSync as loadSecretFileSyncFromCore } from "oriro/plugin-sdk/core";
import { readFileWithinRoot, writeFileWithinRoot } from "oriro/plugin-sdk/file-access-runtime";
import {
  loadSecretFileSync,
  type SecretFileReadResult,
} from "oriro/plugin-sdk/secret-file-runtime";
...
describe("plugin SDK fs-safe compatibility exports", () => {
  it("keeps deprecated secret-file result helpers on public SDK subpaths", async () => {
    await withTempDir({ prefix: "oriro-sdk-secret-compat-" }, async (root) => {
  ...
  it("keeps deprecated root-bounded read/write helpers on file-access-runtime", async () => {
    await withTempDir({ prefix: "oriro-sdk-file-access-compat-" }, async (root) => {
