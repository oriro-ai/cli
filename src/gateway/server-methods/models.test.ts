...
function requestModelsList(params: {
  ...
  const request = modelsHandlers["models.list"]({
    ...
    context: {
      getRuntimeConfig: () => params.runtimeConfig ?? ({} as OriroConfig),
...
describe("models.list", () => {
  ...
  it("marks legacy OpenAI Codex aliases available through ChatGPT OAuth", async () => {
    await withOriroTestState(
      {
        layout: "state-only",
        prefix: "oriro-models-list-codex-alias-",
        agentEnv: "main",
      },
      async (state) => {
  ...
  it("does not mark catalog rows available from expired OAuth profiles", async () => {
    await withOriroTestState(
      {
        layout: "state-only",
        prefix: "oriro-models-list-expired-profile-",
        agentEnv: "main",
      },
      async (state) => {
        await state.writeAuthProfiles({
  ...
  it("marks env SecretRef-backed auth profiles available", async () => {
    await withOriroTestState(
      {
        layout: "state-only",
        prefix: "oriro-models-list-env-profile-",
        agentEnv: "main",
        env: {
  ...
  it("keeps non-env SecretRef-backed auth profile availability unknown", async () => {
    await withOriroTestState(
      {
        layout: "state-only",
        prefix: "oriro-models-list-file-profile-",
        agentEnv: "main",
      },
      async (state) => {
        await state.writeAuthProfiles({
  ...
  it("marks auth profiles available even when provider config uses non-env SecretRef markers", async () => {
    for (const fixture of [
      ...
      await withOriroTestState(
        {
          layout: "state-only",
          prefix: `oriro-models-list-provider-${fixture.name}-profile-`,
          agentEnv: "main",
          env: {
            ORIRO_TEST_PROFILE_API_KEY: "test-token",
