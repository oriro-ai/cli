 ...
 *
 * Presents large Oriro/MCP/client tool inventories through search, describe, call, and optional code-mode tools.
 */
...
type CatalogSource = "oriro" | "mcp" | "client";
...
async function runModelCode(code, timeoutMs) {
  ...
  Object.defineProperties(sandbox, {
    ...
    oriro: { value: controller.oriro, enumerable: true },
  });
...
const SESSION_CATALOGS_KEY = Symbol.for("oriro.toolSearch.sessionCatalogs");
...
function readToolSearchConfig(config?: OriroConfig): Record<string, unknown> {
...
export function resolveToolSearchConfig(config?: OriroConfig): ToolSearchConfig {
...
function classifyTool(tool: CatalogTool): {
  ...
  if (pluginId) {
    return { source: "oriro", sourceName: pluginId };
  }
  return { source: "oriro", sourceName: "core" };
}
  ...
  return [
    {
      ...
      description:
        "Run JavaScript in an isolated Node subprocess with oriro.tools.search, oriro.tools.describe, and oriro.tools.call for large tool catalogs.",
      parameters: Type.Object({
        code: Type.String({
          description:
            "JavaScript body for an async function. Use return to return the final value. The oriro.tools bridge is available.",
        }),
      }),
      execute: async (
        toolCallId: string,
