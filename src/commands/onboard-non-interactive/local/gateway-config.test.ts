...
const SAMPLE_SECRET_REF = {
  source: "env" as const,
  provider: "default",
  id: "ORIRO_GATEWAY_TOKEN_REF",
};

function createTokenConfig(token: unknown): OriroConfig {
  return {
...
describe("applyNonInteractiveGatewayConfig token resolution chain", () => {
  ...
  it("prefers existing plaintext token over ambient ORIRO_GATEWAY_TOKEN on re-onboard", () => {
    // A stale shell/launchd ORIRO_GATEWAY_TOKEN must not rotate a
    // persisted token — that would break already-paired clients.
    const nextConfig = createTokenConfig("existing-user-token");

    const result = applyGatewayConfig({
      nextConfig,
      env: { ORIRO_GATEWAY_TOKEN: "stale-env-token" },
    });
  ...
  it("uses ORIRO_GATEWAY_TOKEN to fill an empty config on first-run", () => {
    const result = applyGatewayConfig({ env: { ORIRO_GATEWAY_TOKEN: "env-token" } });
  ...
  it("preserves an existing SecretRef even when ambient ORIRO_GATEWAY_TOKEN is set", () => {
  ...
  it("overrides an existing SecretRef when --gateway-token-ref-env is provided", () => {
    const newRefId = "ORIRO_GATEWAY_TOKEN_NEW_REF";
    const nextConfig = createTokenConfig(SAMPLE_SECRET_REF);
  ...
  it("fails when --gateway-token-ref-env points to a missing env var", () => {
    ...
    expect(runtime.error).toHaveBeenCalledWith(
      'Environment variable "MISSING_GATEWAY_TOKEN_ENV" is missing or empty. Export it first, then rerun oriro onboard --non-interactive.',
    );
