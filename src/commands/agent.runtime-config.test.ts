...
type ConfigSnapshotForWrite = {
  snapshot: { valid: boolean; resolved: OriroConfig };
...
const loadConfigMock = vi.hoisted(() => vi.fn<() => OriroConfig>());
...
const setRuntimeConfigSnapshotMock = vi.hoisted(() =>
  vi.fn<(cfg: OriroConfig, sourceConfig: OriroConfig) => void>(),
);
...
const resolveCommandConfigWithSecretsMock = vi.hoisted(() =>
  vi.fn<
    (params: ResolveCommandConfigParams) => Promise<{
      resolvedConfig: OriroConfig;
      effectiveConfig: OriroConfig;
      diagnostics: never[];
    }>
  >(),
);
...
async function withTempHome<T>(fn: (home: string) => Promise<T>): Promise<T> {
  return withTempHomeBase(fn, { prefix: "oriro-agent-" });
}
...
function mockConfig(home: string, storePath: string): OriroConfig {
  const cfg = {
    agents: {
      defaults: {
