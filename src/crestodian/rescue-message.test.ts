...
async function withRescueStateDir(
  ...
  await withEnvAsync({ ORIRO_STATE_DIR: stateDir }, async () => await run(stateDir));
}
...
describe("Crestodian rescue message", () => {
  ...
  it("refuses TUI handoff from remote rescue", async () => {
    const cfg: OriroConfig = { crestodian: { rescue: { enabled: true } } };
    const deps = {
      runTui: vi.fn(async () => {
  ...
  it("refuses plugin install from remote rescue", async () => {
    ...
    await expect(
      runRescue("/crestodian plugin install orirohub:oriro-demo", cfg, commandContext(), deps),
