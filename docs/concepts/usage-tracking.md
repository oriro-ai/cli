...
- `/usage cost` in chats: local cost summary aggregated from Oriro session logs.
- CLI: `oriro status --usage` prints a full per-provider breakdown.
- CLI: `oriro channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
...
{
  "messages": {
    "usageTemplate": "~/.oriro/usage-footer.json"
  }
}
```
...
- **MiniMax**: API key or MiniMax OAuth auth profile. Oriro treats
  ...
  quota, so Oriro inverts them before display; count-based fields win when
  present.
  ...
  - If the coding-plan endpoint returns `model_remains`, Oriro prefers the
...
- **DeepSeek**: API key via env/config/auth store (`DEEPSEEK_API_KEY`).
  Oriro calls DeepSeek's balance endpoint and shows the provider-reported
...
can supply plugin-specific usage auth logic; otherwise Oriro falls back to
