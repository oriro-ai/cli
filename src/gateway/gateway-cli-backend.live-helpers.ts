...
export const CLI_BACKEND_LIVE_PROVIDER_SKIP_ENV = "ORIRO_LIVE_CLI_BACKEND_ALLOW_PROVIDER_SKIP";
export const CLI_BACKEND_LIVE_ADVISORY_ENV = "ORIRO_LIVE_CLI_BACKEND_ADVISORY";
...
export function resolveCliBackendLiveModelSelection(params: {
  ...
  if (!parsed) {
    throw new Error(
      `ORIRO_LIVE_CLI_BACKEND_MODEL must resolve to a CLI backend model. Got: ${params.rawModel}`,
    );
  }

  if (parsed.provider === "codex-cli") {
    throw new Error(
      "ORIRO_LIVE_CLI_BACKEND_MODEL=codex-cli/... is no longer supported. Use a supported CLI backend such as claude-cli or google-gemini-cli.",
    );
  }
...
export function parseImageMode(raw?: string): "list" | "repeat" | undefined {
  ...
  throw new Error("ORIRO_LIVE_CLI_BACKEND_IMAGE_MODE must be 'list' or 'repeat'.");
}

export function shouldRunCliImageProbe(providerId: string): boolean {
  const raw = process.env.ORIRO_LIVE_CLI_BACKEND_IMAGE_PROBE?.trim();
  if (raw) {
...
export function shouldRunCliMcpProbe(providerId: string): boolean {
  const raw = process.env.ORIRO_LIVE_CLI_BACKEND_MCP_PROBE?.trim();
  if (raw) {
    return isTruthyEnvValue(raw);
  }
...
export function resolveCliBackendLiveArgs(params: {
  ...
  const args =
    parseJsonStringArray(
      "ORIRO_LIVE_CLI_BACKEND_ARGS",
      process.env.ORIRO_LIVE_CLI_BACKEND_ARGS,
    ) ?? params.defaultArgs;
  if (!args || args.length === 0) {
    throw new Error(
      `ORIRO_LIVE_CLI_BACKEND_ARGS is required for provider "${params.providerId}".`,
    );
  }
  const resumeArgs =
    parseJsonStringArray(
      "ORIRO_LIVE_CLI_BACKEND_RESUME_ARGS",
      process.env.ORIRO_LIVE_CLI_BACKEND_RESUME_ARGS,
...
export function shouldRunCliModelSwitchProbe(providerId: string, modelRef: string): boolean {
  const raw = process.env.ORIRO_LIVE_CLI_BACKEND_MODEL_SWITCH_PROBE?.trim();
  if (raw) {
    return isTruthyEnvValue(raw);
  }
...
export function snapshotCliBackendLiveEnv(): CliBackendLiveEnvSnapshot {
  return {
    ...
    skipProviders: process.env.ORIRO_SKIP_PROVIDERS,
    ...
    skipBrowserControl: process.env.ORIRO_SKIP_BROWSER_CONTROL_SERVER,
    bundledPluginsDir: process.env.ORIRO_BUNDLED_PLUGINS_DIR,
    minimalGateway: process.env.ORIRO_TEST_MINIMAL_GATEWAY,
...
export function applyCliBackendLiveEnv(preservedEnv: ReadonlySet<string>): void {
  ...
  process.env.ORIRO_SKIP_PROVIDERS = "1";
  process.env.ORIRO_SKIP_GMAIL_WATCHER = "1";
  process.env.ORIRO_SKIP_CRON = "1";
  process.env.ORIRO_SKIP_CANVAS_HOST = "1";
  process.env.ORIRO_SKIP_BROWSER_CONTROL_SERVER = "1";
  process.env.ORIRO_TEST_MINIMAL_GATEWAY = "1";
...
export function restoreCliBackendLiveEnv(snapshot: CliBackendLiveEnvSnapshot): void {
  restoreEnvVar("ORIRO_CONFIG_PATH", snapshot.configPath);
  restoreEnvVar("ORIRO_STATE_DIR", snapshot.stateDir);
  restoreEnvVar("ORIRO_GATEWAY_TOKEN", snapshot.token);
  restoreEnvVar("ORIRO_SKIP_CHANNELS", snapshot.skipChannels);
  restoreEnvVar("ORIRO_SKIP_PROVIDERS", snapshot.skipProviders);
  restoreEnvVar("ORIRO_SKIP_GMAIL_WATCHER", snapshot.skipGmail);
  restoreEnvVar("ORIRO_SKIP_CRON", snapshot.skipCron);
  restoreEnvVar("ORIRO_SKIP_CANVAS_HOST", snapshot.skipCanvas);
  restoreEnvVar("ORIRO_SKIP_BROWSER_CONTROL_SERVER", snapshot.skipBrowserControl);
  restoreEnvVar("ORIRO_BUNDLED_PLUGINS_DIR", snapshot.bundledPluginsDir);
  restoreEnvVar("ORIRO_TEST_MINIMAL_GATEWAY", snapshot.minimalGateway);
