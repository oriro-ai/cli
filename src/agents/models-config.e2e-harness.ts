...
export function installModelsConfigTestHooks(opts?: {
  ...
  let previousOriroAgentDir: string | undefined;
  const originalFetch = globalThis.fetch;
  const shouldResetPluginLoaderState = opts?.resetPluginLoaderState !== false;

  beforeEach(() => {
    previousHome = process.env.HOME;
    previousOriroAgentDir = process.env.ORIRO_AGENT_DIR;
  ...
  afterEach(() => {
    ...
    if (previousOriroAgentDir === undefined) {
      delete process.env.ORIRO_AGENT_DIR;
    } else {
      process.env.ORIRO_AGENT_DIR = previousOriroAgentDir;
    }
    clearRuntimeConfigSnapshot();
    clearConfigCache();
    if (shouldResetPluginLoaderState) {
...
export const MODELS_CONFIG_IMPLICIT_ENV_VARS = [
  "ORIRO_TEST_ONLY_PROVIDER_PLUGIN_IDS",
  "VITEST",
  "NODE_ENV",
  "AI_GATEWAY_API_KEY",
...
export const CUSTOM_PROXY_MODELS_CONFIG: OriroConfig = {
  models: {
    providers: {
      "custom-proxy": {
