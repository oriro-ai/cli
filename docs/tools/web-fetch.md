...
- If `provider` is omitted, Oriro auto-detects the first ready web-fetch
...
- Sandboxed `web_fetch` calls allow bundled providers plus installed providers
  whose official npm or OriroHub provenance is verified. Today that permits the
...
In this mode, Oriro still applies hostname-based SSRF checks before sending
