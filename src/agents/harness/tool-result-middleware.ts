...
type MiddlewareContentBlock = OriroAgentToolResult["content"][number];
...
function isValidMiddlewareToolResult(value: unknown): value is OriroAgentToolResult {
...
function coerceMiddlewareToolResult(
  value: unknown,
  options: MiddlewareToolResultCoerceOptions = {},
): OriroAgentToolResult | undefined {
  ...
  const content: OriroAgentToolResult["content"] = [];
 ...
 */
function sanitizeToolResultForMiddleware(result: OriroAgentToolResult): OriroAgentToolResult {
...
function buildMiddlewareFailureResult(): OriroAgentToolResult {
  return {
    content: [
      {
        type: "text",
...
function reconcileDeliveredMessagingFailure(
  result: OriroAgentToolResult,
  fallback: OriroAgentToolResult | undefined,
): OriroAgentToolResult {
