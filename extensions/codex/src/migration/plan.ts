...
import { asBoolean, isRecord } from "oriro/plugin-sdk/string-coerce-runtime";
    ...
    items.push(
      createMigrationManualItem({
        ...
        message:
          ...
          `Codex native plugin "${plugin.name}" was found but not activated automatically.`,
        recommendation:
          "Review the plugin bundle first, then install trusted compatible plugins with oriro plugins install <path>.",
      }),
    );
  }
  return items;
