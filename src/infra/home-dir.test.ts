// Tests Oriro home directory resolution.
...
describe("resolveEffectiveHomeDir", () => {
  it.each([
    {
      name: "prefers ORIRO_HOME over HOME and USERPROFILE",
      env: {
  ...
  it("expands ORIRO_HOME when set to ~", () => {
    const env = {
      ORIRO_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  it.each([
    ...
    {
      name: "returns a fully resolved path for ORIRO_HOME",
      env: { ORIRO_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      homedir: () => "/fallback",
      expected: path.resolve("/custom/home"),
    },
    {
      name: "returns cwd when ORIRO_HOME is tilde-only and no fallback home exists",
      env: { ORIRO_HOME: "~" } as NodeJS.ProcessEnv,
      homedir: () => {
        throw new Error("no home");
      },
      expected: process.cwd(),
...
describe("resolveOsHomeDir", () => {
  it("ignores ORIRO_HOME and uses HOME", () => {
    expect(
      resolveOsHomeDir(
        {
          ORIRO_HOME: "/srv/oriro-home",
  ...
  it.each([
    {
      ...
      expected: `${path.resolve("/srv/oriro-home")}/x`,
    },
    {
      name: "expands exact ~ using explicit home",
      input: "~",
      opts: { home: " /srv/oriro-home " },
      expected: "/srv/oriro-home",
    },
    {
...
describe("resolveHomeRelativePath", () => {
  it.each([
    ...
    },
    {
      ...
      expected: path.resolve("/srv/oriro-home/docs"),
    },
    {
...
describe("resolveOsHomeRelativePath", () => {
  it("expands tilde paths using the OS home instead of ORIRO_HOME", () => {
    expect(
