...
read_when:
  - You want Oriro to remember natural follow-ups
  - You want to understand how inferred check-ins differ from reminders
  - You want to review or dismiss follow-up commitments
---

Commitments are short-lived follow-up memories. When enabled, Oriro can
notice that a conversation created a future check-in opportunity and remember
to bring it back later.

Examples:

- You mention an interview tomorrow. Oriro may check in afterward.
- You say you are exhausted. Oriro may ask later whether you slept.
- The agent says it will follow up after something changes. Oriro may track
  that open loop.

Commitments are not durable facts like `MEMORY.md`, and they are not exact
reminders. They sit between memory and automation: Oriro remembers a
...
```bash
oriro config set commitments.enabled true
oriro config set commitments.maxPerDay 3
```

Equivalent `oriro.json`:
...
After an agent reply, Oriro may run a hidden background extraction pass in a
...
When it finds a high-confidence candidate, Oriro stores a commitment with:
...
Oriro never delivers an inferred commitment immediately after writing it.
...
oriro commitments dismiss cm_abc123
```

See [`oriro commitments`](/cli/commitments) for the command reference.
...
Stored commitments are local Oriro state. They are operational memory, not
long-term memory. Disable the feature with:

```bash
oriro config set commitments.enabled false
```
...
- Check `oriro commitments --all` for pending, dismissed, snoozed, or expired
  records.
...
- [`oriro commitments`](/cli/commitments)
