...
import { clearPluginLoaderCache, loadOriroPlugins } from "./loader.js";
import { resetPluginRuntimeStateForTest } from "./runtime.js";

export type TempPlugin = { dir: string; file: string; id: string };
export type PluginLoadConfig = NonNullable<Parameters<typeof loadOriroPlugins>[0]>["config"];
export type PluginRegistry = ReturnType<typeof loadOriroPlugins>;
...
const fixtureRoot = mkdtempSafe(path.join(os.tmpdir(), "oriro-plugin-"));
let tempDirIndex = 0;
const prevBundledDir = process.env.ORIRO_BUNDLED_PLUGINS_DIR;
const prevDisableBundledPlugins = process.env.ORIRO_DISABLE_BUNDLED_PLUGINS;
...
export function loadBundleFixture(params: {
  ...
  const bundleRoot = path.join(workspaceDir, ".oriro", "extensions", params.pluginId);
  params.build(bundleRoot);
  return withEnv({ ORIRO_STATE_DIR: stateDir, ...params.env }, () =>
    loadOriroPlugins({
      workspaceDir,
...
export function resetPluginLoaderTestStateForTest() {
  ...
  if (prevBundledDir === undefined) {
    ...
    process.env.ORIRO_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
  if (prevDisableBundledPlugins === undefined) {
    delete process.env.ORIRO_DISABLE_BUNDLED_PLUGINS;
  } else {
    process.env.ORIRO_DISABLE_BUNDLED_PLUGINS = prevDisableBundledPlugins;
  }
}
