// Creates isolated Oriro state directories for integration-style tests.
...
type OriroTestStateLayout = "home" | "state-only" | "split";

type OriroTestStateScenario =
  | "empty"
  | "minimal"
  | "update-stable"
  | "upgrade-survivor"
...
export type OriroTestStateOptions = {
  prefix?: string;
  label?: string;
  layout?: OriroTestStateLayout;
  scenario?: OriroTestStateScenario;
  agentEnv?: "clear" | "main";
  applyEnv?: boolean;
...
const DEFAULT_PREFIX = "oriro-test-state-";
const ENV_KEYS = [
  "HOME",
  "USERPROFILE",
  "HOMEDRIVE",
...
function resolveLayout(
  ...
  if (layout === "home") {
    ...
    return {
      home,
      stateDir,
      configPath: path.join(stateDir, "oriro.json"),
  ...
  if (layout === "split") {
    ...
    return {
      home,
      stateDir,
      configPath: path.join(root, "config", "oriro.json"),
...
function scenarioConfig(options: OriroTestStateOptions): Record<string, unknown> | undefined {
  ...
  if (scenario === "upgrade-survivor") {
    return {
      ...
      gateway: {
        ...
        auth: {
          mode: "token",
          token: options.gateway?.token ?? "oriro-test-token",
        },
        controlUi: {
          enabled: false,
        },
...
function scenarioEnv(options: OriroTestStateOptions): Record<string, string | undefined> {
...
export async function createOriroTestState(
  options: OriroTestStateOptions = {},
): Promise<OriroTestState> {
  ...
  const state: OriroTestState = {
    ...
    applyEnv: () => {
      ...
      for (const [key, value] of Object.entries(envVars)) {
        // Test fixtures apply a fixed Oriro env set, not plugin-provided host env.
        if (value === undefined) {
...
export async function withOriroTestState<T>(
  options: OriroTestStateOptions,
  fn: (state: OriroTestState) => Promise<T>,
): Promise<T> {
  const state = await createOriroTestState(options);
  try {
