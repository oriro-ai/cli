...
export async function prepareGatewayStartupConfig(params: {
  ...
  const canReusePreflightPreparedSnapshot = (config: OriroConfig): boolean =>
    Boolean(
      preflightPrepared &&
  ...
  const activateStartupSecrets = async (config: OriroConfig) => {
...
function hasActiveGatewayAuthSecretRef(config: OriroConfig): boolean {
...
function pruneSkippedStartupSecretSurfaces(config: OriroConfig): OriroConfig {
  const skipChannels =
...
function assertRuntimeGatewayAuthNotKnownWeak(config: OriroConfig): void {
