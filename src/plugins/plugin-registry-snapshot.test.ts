...
function makeTempDir() {
  return makeTrackedTempDir("oriro-plugin-registry-snapshot", tempDirs);
}

function createHermeticEnv(rootDir: string): NodeJS.ProcessEnv {
  return {
    ORIRO_BUNDLED_PLUGINS_DIR: path.join(rootDir, "bundled"),
    ORIRO_STATE_DIR: path.join(rootDir, "state"),
...
describe("loadPluginRegistrySnapshotWithMetadata", () => {
  ...
  it("recovers managed npm plugins missing from a stale persisted registry", () => {
    ...
    expect(result.snapshot.installRecords.whatsapp).toEqual({
      source: "npm",
      spec: "@oriro/whatsapp@2026.5.2",
      installPath: whatsappDir,
      version: "2026.5.2",
      resolvedName: "@oriro/whatsapp",
      resolvedVersion: "2026.5.2",
      resolvedSpec: "@oriro/whatsapp@2026.5.2",
    });
  ...
  it("keeps persisted manifestless Claude bundles on the fast path", () => {
    ...
    const stateDir = path.join(tempRoot, "state");
    const env = { ...createHermeticEnv(tempRoot), ORIRO_DISABLE_BUNDLED_PLUGINS: "1" };
    const config = {
      plugins: {
        load: { paths: [rootDir] },
      },
  ...
  it("refreshes a memoized derived snapshot when workspace plugins are installed", () => {
    ...
    writePackagePlugin(path.join(workspaceDir, ".oriro", "extensions", "demo"));
  ...
  it("keeps persisted package plugins when file hashes match", () => {
    ...
    expect(record.manifestFile.size).toBe(
      fs.statSync(path.join(rootDir, "oriro.plugin.json")).size,
    );
  ...
  it("keeps mixed source-checkout bundled roots from the same checkout", () => {
    const tempRoot = makeTempDir();
    const packageRoot = path.join(tempRoot, "oriro");
  ...
  it("treats persisted registry as stale when a plugin diagnostic source path no longer exists", () => {
    ...
    const config = {};
    const ghostDir = path.join(tempRoot, "extensions", "lossless-oriro");
    const npmPluginDir = writeManagedNpmPlugin({
      stateDir,
      packageName: "@martian-engineering/lossless-oriro",
    ...
    const losslessPlugin = requirePluginRecord(result.snapshot.plugins, "lossless-oriro");
