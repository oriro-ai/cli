...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/oriro`)
...
If the page fails to load, start the Gateway first: `oriro gateway`.
...
If the browser retries pairing with changed auth details (role/scopes/public key), the previous pending request is superseded and a new `requestId` is created. Re-run `oriro devices list` before approval.

If the browser is already paired and you change it from read access to write/admin access, this is treated as an approval upgrade, not a silent reconnect. Oriro keeps the old approval active, blocks the broader reconnect, and asks you to approve the new scope set explicitly.

Once approved, the device is remembered and won't require re-approval unless you revoke it with `oriro devices revoke --device <id> --role <role>`. See [Devices CLI](/cli/devices) for token rotation and revocation.

Paperclip agents that connect through the `oriro_gateway` adapter use the same first-run approval flow. After the initial connection attempt, run `oriro devices approve --latest` to preview the pending request, then rerun the printed `oriro devices approve <requestId>` command to approve it. Pass explicit `--url` and `--token` values for a remote gateway. To keep approvals stable across restarts, configure a persistent `adapterConfig.devicePrivateKeyPem` in Paperclip instead of letting it generate a new ephemeral device identity each run.
...
The Control UI fetches its runtime settings from `/control-ui-config.json`, resolved relative to the gateway's Control UI base path (for example `/__oriro__/control-ui-config.json` when the UI is served under `/__oriro__/`). That endpoint is gated by the same gateway auth as the rest of the HTTP surface: unauthenticated browsers cannot fetch it, and a successful fetch requires either an already valid gateway token/password, Tailscale Serve identity, or a trusted-proxy identity.
...
Imported themes are stored only in the current browser profile. They are not written to gateway config and do not sync across devices. Replacing the imported theme updates the one local slot; clearing it switches the active theme back to Oriro if the imported theme was selected.
...
<AccordionGroup>
  ...
  <Accordion title="Config">
    - View/edit `~/.oriro-ai/cli.json` (`config.get`, `config.set`).
  ...
  <Accordion title="Cron jobs panel notes">
    ...
    - Deprecated fallback: run `oriro doctor --fix` to migrate stored legacy jobs with `notify: true` from `cron.webhook` to explicit per-job webhook or completion delivery.
...
The dedicated MCP page is an operator view for Oriro-managed MCP servers under `mcp.servers`. It does not start MCP transports by itself; use it to inspect and edit saved config, then use `oriro mcp doctor --probe` when you need live server proof.
...
7. Run `oriro mcp status --verbose`, `oriro mcp doctor --probe`, or `oriro mcp reload` from a terminal when the edited process needs static diagnostics, live proof, or cached-runtime disposal.
...
<AccordionGroup>
  ...
  <Accordion title="Stop and abort">
    ...
    - Type `/stop` (or standalone abort phrases like `stop`, `stop action`, `stop run`, `stop oriro`, `please stop`) to abort out-of-band.
...
If the page shows **Protocol mismatch** right after an Oriro update, first reopen the dashboard with `oriro dashboard` and hard-refresh the page. If it still fails, clear site data for the dashboard origin or test in a private browser window; an old tab or browser service-worker cache can keep running a pre-update Control UI bundle against the newer Gateway.
...
| `push/vapid-keys.json` (under the Oriro state dir) | Auto-generated VAPID keypair used to sign Web Push payloads.       |
...
- `ORIRO_VAPID_PUBLIC_KEY`
- `ORIRO_VAPID_PRIVATE_KEY`
- `ORIRO_VAPID_SUBJECT` (defaults to `https://oriro.ai`)
...
<Tabs>
  ...
  <Tab title="Bind to tailnet + token">
    ```bash
    oriro gateway --bind tailnet --token "$(openssl rand -hex 32)"
    ```
...
If you open the dashboard over plain HTTP (`http://<lan-ip>` or `http://<tailscale-ip>`), the browser runs in a **non-secure context** and blocks WebCrypto. By default, Oriro **blocks** Control UI connections without device identity.
...
- `GET /__oriro__/assistant-media?meta=1&source=<path>` requires the normal Control UI operator auth. The browser sends the gateway token as a bearer header when checking availability.
...
```bash
ORIRO_CONTROL_UI_BASE_PATH=/oriro/ pnpm ui:build
```
...
If the browser loads a blank dashboard and DevTools shows no useful error, an extension or early content script may have prevented the JavaScript module app from evaluating. The static page includes a plain HTML recovery panel that appears when `<oriro-app>` is not registered after startup.
