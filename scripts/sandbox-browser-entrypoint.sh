...
export DISPLAY=:1
export HOME=/tmp/oriro-home
export XDG_CONFIG_HOME="${HOME}/.config"
export XDG_CACHE_HOME="${HOME}/.cache"

CDP_PORT="${ORIRO_BROWSER_CDP_PORT:-9222}"
CDP_SOURCE_RANGE="${ORIRO_BROWSER_CDP_SOURCE_RANGE:-}"
CDP_AUTH_TOKEN="${ORIRO_BROWSER_CDP_AUTH_TOKEN:-}"
VNC_PORT="${ORIRO_BROWSER_VNC_PORT:-5900}"
NOVNC_PORT="${ORIRO_BROWSER_NOVNC_PORT:-6080}"
ENABLE_NOVNC="${ORIRO_BROWSER_ENABLE_NOVNC:-1}"
HEADLESS="${ORIRO_BROWSER_HEADLESS:-0}"
ALLOW_NO_SANDBOX="${ORIRO_BROWSER_NO_SANDBOX:-0}"
NOVNC_PASSWORD="${ORIRO_BROWSER_NOVNC_PASSWORD:-}"

DISABLE_GRAPHICS_FLAGS="${ORIRO_BROWSER_DISABLE_GRAPHICS_FLAGS:-1}"
DISABLE_EXTENSIONS="${ORIRO_BROWSER_DISABLE_EXTENSIONS:-1}"
RENDERER_PROCESS_LIMIT="${ORIRO_BROWSER_RENDERER_PROCESS_LIMIT:-2}"
AUTO_START_TIMEOUT_MS="${ORIRO_BROWSER_AUTO_START_TIMEOUT_MS:-12000}"
...
if [[ -z "${CDP_AUTH_TOKEN}" ]]; then
  ...
  ORIRO_BROWSER_CHROME_CDP_PORT="${CHROME_CDP_PORT}" python3 - <<'PY' &
import base64
import hmac
import ipaddress
import os
...
LISTEN_PORT = int(os.environ["ORIRO_BROWSER_CDP_PORT"])
UPSTREAM_PORT = int(os.environ["ORIRO_BROWSER_CHROME_CDP_PORT"])
AUTH_TOKEN = os.environ["ORIRO_BROWSER_CDP_AUTH_TOKEN"]
SOURCE_RANGE = os.environ.get("ORIRO_BROWSER_CDP_SOURCE_RANGE", "").strip()
...
EXPECTED_BASIC = "Basic " + base64.b64encode(f"oriro:{AUTH_TOKEN}".encode()).decode()
