...
import { resolveStateDir } from "oriro/plugin-sdk/state-paths";
import { resolvePreferredOriroTmpDir } from "oriro/plugin-sdk/temp-path";
import { captureEnv } from "oriro/plugin-sdk/test-env";
...
import { withMockedWindowsPlatform, withRestoredMocks } from "oriro/plugin-sdk/test-node-mocks";
import { optimizeImageToPng } from "oriro/plugin-sdk/web-media";
...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(
    path.join(resolvePreferredOriroTmpDir(), "oriro-media-test-"),
  );
...
describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub ORIRO_STATE_DIR.
    // Also keep it outside the Oriro temp root so default localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["ORIRO_STATE_DIR"]);
    process.env.ORIRO_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "oriro-media-state-test",
    );
  });
...
describe("local media root guard", () => {
  ...
  it("allows local paths under an explicit root", async () => {
    const result = await loadWebMedia(tinyPngFile, 1024 * 1024, {
      localRoots: [resolvePreferredOriroTmpDir()],
    });
    expect(result.kind).toBe("image");
  });
  ...
  it("accepts win32 dev=0 stat mismatch for local file loads", async () => {
    ...
    // os.tmpdir() fallback rather than the POSIX `/tmp/oriro` root that
    // actually holds `tinyPngFile` on this Linux test runner (#60713).
    const realTmpRoot = resolvePreferredOriroTmpDir();
  ...
  it("allows default Oriro state workspace and sandbox roots", async () => {
  ...
  it("rejects default Oriro state per-agent workspace-* roots without explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));

    await expectLocalMediaAccessCode(
      loadWebMedia(path.join(stateDir, "workspace-orirody", "tmp", "render.bin"), {
        maxBytes: 1024 * 1024,
        readFile,
      }),
      "path-not-allowed",
  ...
  it("allows per-agent workspace-* paths with explicit local roots", async () => {
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
    const agentWorkspaceDir = path.join(stateDir, "workspace-orirody");
