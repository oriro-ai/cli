...
export function resolveVitestNodeArgs(env = process.env) {
  if (isTruthyEnvValue(env.ORIRO_VITEST_ENABLE_MAGLEV)) {
    return [];
  }
...
export function resolveVitestCliEntry({
  ...
  try {
    ...
    if (isMissingVitestResolveError(error)) {
      ...
      wrappedError.code = "ORIRO_MISSING_VITEST";
      throw wrappedError;
    }
    throw error;
  }
...
function shouldApplyNativeWorkerBudget(env) {
  ...
  return (
    env.ORIRO_TEST_PROJECTS_SERIAL === "1" || resolveExplicitVitestWorkerBudget(env) !== null
  );
}
...
function resolveExplicitVitestWorkerBudget(env) {
  return parsePositiveInt(env.ORIRO_VITEST_MAX_WORKERS ?? env.ORIRO_TEST_WORKERS);
}
  ...
  try {
    return fsImpl.readdirSync(extensionsDir, { withFileTypes: true }).some((entry) => {
      ...
      return (
        ...
        fsImpl.existsSync(path.join(pluginDir, "oriro.plugin.json"))
      );
    });
  } catch {
    return false;
...
export function resolveLinkedSourceBundledPluginsEnv(
  ...
  if (env.ORIRO_BUNDLED_PLUGINS_DIR?.trim()) {
    return {};
  }
  ...
  return {
    ORIRO_BUNDLED_PLUGINS_DIR: extensionsDir,
    ORIRO_TEST_TRUST_BUNDLED_PLUGINS_DIR:
      env.ORIRO_TEST_TRUST_BUNDLED_PLUGINS_DIR?.trim() || "1",
  };
}
...
function main(argv = process.argv.slice(2), env = process.env) {
  ...
  try {
    ...
    if (error instanceof Error && error.code === "ORIRO_MISSING_VITEST") {
