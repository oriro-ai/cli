...
type GatewayRestartSentinelDatabase = Pick<OriroStateKyselyDatabase, "gateway_restart_sentinel">;
...
async function runHelperWithExistingSentinel(params: {
  ...
  const helperParams = JSON.parse(await fs.readFile(args[1] ?? "", "utf-8")) as Record<
    ...
    unknown
  >;
  const env = { ORIRO_STATE_DIR: tmpDir } as NodeJS.ProcessEnv;
    ...
    `${JSON.stringify(
      {
        ...
        stateDatabasePath: resolveOriroStateSqlitePath(env),
...
async function createLegacyRestartSentinelTable(env: NodeJS.ProcessEnv): Promise<void> {
  ...
  const stateDatabasePath = resolveOriroStateSqlitePath(env);
    ...
    `${JSON.stringify(
      {
        ...
        stateDatabasePath: resolveOriroStateSqlitePath({ ORIRO_STATE_DIR: tmpDir }),
        logPath: path.join(tmpDir, "handoff.log"),
        sensitivePaths: [],
...
describe("managed service update handoff", () => {
  it("strips process supervisor hints while preserving service identity for the CLI handoff", async () => {
    ...
    expect(helperParams.stateDatabasePath).toMatch(/oriro\.sqlite$/u);
  ...
  it("writes a fallback update failure when no restart sentinel row exists", async () => {
    ...
    if (process.platform !== "win32") {
      const mode = (await fs.stat(resolveOriroStateSqlitePath(env))).mode & 0o777;
