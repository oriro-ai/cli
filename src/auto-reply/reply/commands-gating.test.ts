...
vi.mock("../../config/config.js", () => ({
  ...
  transformConfigFileWithRetry: async (params: {
    ...
    transform: (
      ...
      | Promise<{ nextConfig: OriroConfig; result?: unknown }>
      | {
    ...
    return {
      path: snapshot.path ?? "/tmp/oriro.json",
      previousHash,
...
function buildParams(commandBody: string, cfg: OriroConfig): HandleCommandsParams {
  const ctx = {
    Body: commandBody,
...
describe("command gating", () => {
  ...
  it("blocks disabled bash", async () => {
    const result = await handleBashChatCommand({
      ...
      cfg: { commands: { bash: false } } as OriroConfig,
      sessionKey: "agent:main:main",
      isGroup: false,
      elevated: { enabled: true, allowed: true, failures: [] },
    });
  ...
  it("blocks bash when elevated access is unavailable", async () => {
    const result = await handleBashChatCommand({
      ...
      cfg: { commands: { bash: true } } as OriroConfig,
      sessionKey: "agent:main:main",
      isGroup: false,
      elevated: {
        enabled: true,
  ...
  it("redacts secret-shaped fields from full /config show replies", async () => {
    readConfigFileSnapshotMock.mockResolvedValueOnce({
      ...
      parsed: {
        gateway: {
          auth: {
            mode: "token",
            token: "ORIRO_CONFIG_SHOW_CANARY_TOKEN_65623",
            password: "ORIRO_CONFIG_SHOW_CANARY_PASSWORD_65623",
          },
          bind: "127.0.0.1",
          port: 3210,
        },
        models: {
          providers: {
            openai: {
              apiKey: "ORIRO_CONFIG_SHOW_CANARY_API_KEY_65623",
        ...
        browser: {
          cdpUrl:
            "wss://chrome.example.test/devtools?token=ORIRO_CONFIG_SHOW_CANARY_CDP_TOKEN_65623&apiKey=ORIRO_CONFIG_SHOW_CANARY_CDP_API_KEY_65623",
          profiles: {
            local: {
            ...
            remote: {
              cdpUrl:
                "wss://chrome.remote.example.test/devtools?apiKey=ORIRO_CONFIG_SHOW_CANARY_CDP_PROFILE_API_KEY_65623",
            },
          },
        },
        talk: {
    ...
    expect(output).not.toContain("ORIRO_CONFIG_SHOW_CANARY_TOKEN_65623");
    expect(output).not.toContain("ORIRO_CONFIG_SHOW_CANARY_PASSWORD_65623");
    expect(output).not.toContain("ORIRO_CONFIG_SHOW_CANARY_API_KEY_65623");
    expect(output).not.toContain("ORIRO_CONFIG_SHOW_CANARY_CDP_TOKEN_65623");
    expect(output).not.toContain("ORIRO_CONFIG_SHOW_CANARY_CDP_API_KEY_65623");
    expect(output).not.toContain("ORIRO_CONFIG_SHOW_CANARY_CDP_PROFILE_API_KEY_65623");
  ...
  it("redacts secret-shaped values from /config set acknowledgements", async () => {
    ...
    const params = buildParams(
      '/config set gateway.auth.token="ORIRO_CONFIG_SET_CANARY_TOKEN_65623"',
      {
        commands: { config: true, text: true },
        channels: { whatsapp: { allowFrom: ["*"] } },
      } as OriroConfig,
    ...
    expect(output).toContain(REDACTED_SENTINEL);
    expect(output).not.toContain("ORIRO_CONFIG_SET_CANARY_TOKEN_65623");
  });
  ...
  it("redacts secret-shaped fields from /debug show replies", async () => {
    getConfigOverridesMock.mockReturnValueOnce({
      gateway: {
        auth: {
          token: "ORIRO_DEBUG_SHOW_CANARY_TOKEN_65623",
        },
      },
      channels: {
        telegram: {
          botToken: "ORIRO_DEBUG_SHOW_CANARY_BOT_TOKEN_65623",
        },
      },
      messages: {
        ackReaction: ":)",
    ...
    expect(output).not.toContain("ORIRO_DEBUG_SHOW_CANARY_TOKEN_65623");
    expect(output).not.toContain("ORIRO_DEBUG_SHOW_CANARY_BOT_TOKEN_65623");
  });

  it("redacts secret-shaped values from /debug set acknowledgements", async () => {
    const params = buildParams(
      '/debug set gateway.auth.token="ORIRO_DEBUG_SET_CANARY_TOKEN_65623"',
      {
        commands: { debug: true, text: true },
        channels: { whatsapp: { allowFrom: ["*"] } },
      } as OriroConfig,
    ...
    expect(output).toContain(REDACTED_SENTINEL);
    expect(output).not.toContain("ORIRO_DEBUG_SET_CANARY_TOKEN_65623");
  });
  ...
  it("ignores inherited command flags", () => {
    ...
    const cfg = { commands: inheritedCommands as never } as OriroConfig;
  ...
  it("enforces gateway client permissions for /config commands", async () => {
    const baseCfg = { commands: { config: true, text: true } } as OriroConfig;
