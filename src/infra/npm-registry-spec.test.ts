...
import {
  ...
  isOriroOrgNpmSpec,
  isOriroStableCorrectionVersion,
...
describe("npm registry spec validation", () => {
  it.each([
    "@oriro/voice-call",
    "@oriro/voice-call@1.2.3",
    "@oriro/voice-call@1.2.3-beta.4",
    "@oriro/voice-call@latest",
    "@oriro/voice-call@beta",
  ])("accepts %s", (spec) => {
...
describe("npm registry spec parsing helpers", () => {
  it.each([
    ...
    {
      spec: "@oriro/voice-call@2026.5.3-1",
      expected: {
        name: "@oriro/voice-call",
        raw: "@oriro/voice-call@2026.5.3-1",
        selector: "2026.5.3-1",
    ...
    {
      spec: "@oriro/voice-call@1.2.3-beta.1",
      expected: {
        name: "@oriro/voice-call",
        raw: "@oriro/voice-call@1.2.3-beta.1",
  ...
  it.each([
    { spec: "@oriro/voice-call", expected: true },
    { spec: "@oriro/voice-call@1.2.3", expected: true },
    { spec: "@other/voice-call", expected: false },
    { spec: "voice-call", expected: false },
    { spec: "npm:@oriro/voice-call", expected: false },
    { spec: undefined, expected: false },
  ])("detects Oriro-org npm specs for %s", ({ spec, expected }) => {
    expect(isOriroOrgNpmSpec(spec)).toBe(expected);
  });
  ...
  ])("detects Oriro stable correction versions for %s", ({ value, expected }) => {
    expect(isOriroStableCorrectionVersion(value)).toBe(expected);
  });
  ...
  ])("compares Oriro release versions for %s and %s", ({ left, right, expected }) => {
    expect(compareOriroReleaseVersions(left, right)).toBe(expected);
  });
});
  ...
  it.each([
    {
      spec: "@oriro/voice-call",
      resolvedVersion: "1.2.3-beta.1",
      expected: `Use "@oriro/voice-call@beta"`,
    },
    {
      spec: "@oriro/voice-call@beta",
      resolvedVersion: "1.2.3-rc.1",
