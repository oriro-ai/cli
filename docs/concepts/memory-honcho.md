...
[Honcho](https://honcho.dev) adds AI-native memory to Oriro. It persists
...
```bash
oriro plugins install @honcho-ai/oriro-honcho
oriro honcho setup
oriro gateway --force
```
...
Settings live under `plugins.entries["oriro-honcho"].config`:
    ...
    entries: {
      "oriro-honcho": {
        config: {
          ...
          workspaceId: "oriro", // memory isolation
...
`IDENTITY.md`, `memory/`, `canvas/`), `oriro honcho setup` detects and
...
```bash
oriro honcho setup                        # Configure API key and migrate files
oriro honcho status                       # Check connection status
oriro honcho ask <question>               # Query Honcho about the user
oriro honcho search <query> [-k N] [-d D] # Semantic search over memory
```

## Further reading

- [Plugin source code](https://github.com/plastic-labs/oriro-honcho)
- [Honcho documentation](https://docs.honcho.dev)
- [Honcho Oriro integration guide](https://docs.honcho.dev/v3/guides/integrations/oriro)
- [Memory](/concepts/memory) -- Oriro memory overview
