...
import type { Api, Model } from "oriro/plugin-sdk/llm";
...
import {
  ...
  createOriroTransportStreamFnForModel,
...
describe("openai transport stream", () => {
  ...
  it("tags Responses encrypted reasoning with replay provenance while streaming", async () => {
    ...
    expect(replayItem).not.toHaveProperty("__oriro_replay");
    expect(thinkingBlock.oriroReasoningReplay).toEqual(expectedReplayMetadata);
  });
  ...
  it("summarizes model payload tools with full names when requested", () => {
    const previous = process.env.ORIRO_DEBUG_MODEL_PAYLOAD;
    process.env.ORIRO_DEBUG_MODEL_PAYLOAD = "tools";
    try {
      expect(
      ...
      if (previous === undefined) {
        delete process.env.ORIRO_DEBUG_MODEL_PAYLOAD;
      } else {
        process.env.ORIRO_DEBUG_MODEL_PAYLOAD = previous;
      }
    }
  });
  ...
  it("redacts full model payload debug summaries", () => {
    const previous = process.env.ORIRO_DEBUG_MODEL_PAYLOAD;
    process.env.ORIRO_DEBUG_MODEL_PAYLOAD = "full-redacted";
    try {
  ...
  it("enforces the code mode responses tool surface before requests leave Oriro", () => {
    const payload = {
      tools: [
        { type: "function", name: "exec" },
  ...
  it("adds Oriro attribution to native OpenAI transport headers and protects it from provider overrides", () => {
  ...
  it("adds Oriro attribution to native OpenAI Codex transport headers", () => {
    vi.stubEnv("ORIRO_VERSION", "2026.3.22");
    const headers = testing.buildOpenAIClientHeaders(
      {
        id: "gpt-5.4-codex",
  ...
  it("adds SSE Accept only to native ChatGPT/Codex Responses stream requests", () => {
    ...
    const transportAliasModel = {
      ...codexModel,
      api: "oriro-openai-responses-transport" as Api,
    } satisfies Model;
  ...
  it("prepares a custom simple-completion api alias when transport overrides are attached", () => {
    ...
    expect(resolveTransportAwareSimpleApi(model.api)).toBe("oriro-openai-responses-transport");
    expectRecordFields(prepared, {
      api: "oriro-openai-responses-transport",
      provider: "openai",
      id: "gpt-5.4",
    });
  ...
  it("prepares an Anthropic simple-completion api alias when transport overrides are attached", () => {
    ...
    expect(resolveTransportAwareSimpleApi(model.api)).toBe("oriro-anthropic-messages-transport");
    expectRecordFields(prepared, {
      api: "oriro-anthropic-messages-transport",
      provider: "anthropic",
      id: "claude-sonnet-4-6",
    });
    expect(buildTransportAwareSimpleStreamFn(model)).toBeTypeOf("function");
  ...
  it("reports the Google simple-completion api alias without loading provider runtime", () => {
    ...
    expect(resolveTransportAwareSimpleApi(model.api)).toBe(
      "oriro-google-generative-ai-transport",
    );
  });
  ...
  it("uses top-level instructions for Codex responses and preserves prompt cache identity", () => {
    const params = buildOpenAIResponsesParams(
      ...
      {
        oriro_session_id: "session-123",
        oriro_turn_id: "turn-123",
      },
    ) as Record<string, unknown> & {
  ...
  it("keeps Codex response shaping when simple completions use the Oriro transport alias", () => {
    const params = buildOpenAIResponsesParams(
      {
        id: "gpt-5.5",
        name: "GPT-5.5",
  ...
  it("sanitizes Codex responses params after payload hooks mutate them without stripping cache identity", () => {
    const payload = {
      ...
      metadata: { oriro_session_id: "session-123" },
  ...
  it("omits prior Responses replay item ids when store is disabled for custom Codex-compatible responses", () => {
            ...
            content: [
              {
                ...
                oriroReasoningReplay: testing.buildOpenAIResponsesReasoningReplayMetadata(
                  model,
                  {
                    authProfileId: "openai:oauth",
                    sessionId: "session-123",
    ...
    expect(reasoningItem?.id).toBeUndefined();
    expect(reasoningItem).not.toHaveProperty("__oriro_replay");
    const assistantMessage = params.input?.find(
      (item) => item.type === "message" && item.role === "assistant",
    );
    expectRecordFields(assistantMessage, {
  ...
  it("adds native OpenAI turn metadata on direct Responses routes", () => {
    const params = buildOpenAIResponsesParams(
      ...
      { sessionId: "session-123" } as never,
      {
        oriro_session_id: "session-123",
        oriro_turn_id: "turn-123",
        oriro_turn_attempt: "1",
        oriro_transport: "stream",
      },
