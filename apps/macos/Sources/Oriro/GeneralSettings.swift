...
struct GeneralSettings: View {
        ...
        VStack(alignment: .leading, spacing: 20) {
            ...
            SettingsCardGroup("App") {
                SettingsCardToggleRow(
                    ...
                    subtitle: "Automatically start Oriro after you sign in.",
                    binding: self.$state.launchAtLogin)

                SettingsCardToggleRow(
                    title: "Show Dock icon",
                    subtitle: "Keep Oriro visible in the Dock. When off, windows still show the Dock icon while open.",
    ...
    private var openOriroStatusPanel: some View {
        HStack(alignment: .center, spacing: 14) {
            ...
            VStack(alignment: .leading, spacing: 4) {
                Text(self.state.isPaused ? "Oriro paused" : "Oriro active")
                    .font(.headline)
            ...
            Toggle("Oriro active", isOn: self.activeBinding)
                .labelsHidden()
                .toggleStyle(.switch)
        }
    ...
    private var connectionStatusSubtitle: String {
        switch self.state.connectionMode {
        case .local:
            return "Oriro starts and monitors the Gateway on this Mac."
        case .remote:
        ...
        SettingsCardGroup("Advanced") {
            DisclosureGroup(isExpanded: self.$showRemoteAdvanced) {
                VStack(alignment: .leading, spacing: 12) {
                    ...
                    self.advancedTextField(
                        "Project root",
                        placeholder: "/home/you/Projects/oriro",
                        text: self.$state.remoteProjectRoot)
                    self.advancedTextField(
                        "CLI path",
                        placeholder: "/Applications/Oriro.app/.../oriro",
...
extension GeneralSettings {
    ...
    private func revealLogs() {
        ...
        Looked for oriro logs in /tmp/oriro/.
...
extension GeneralSettings {
    static func exerciseForTesting() {
        ...
        state.remoteProjectRoot = "/tmp/oriro"
        state.remoteCliPath = "/tmp/oriro"
