...
function makeTempRepo() {
  const dir = mkdtempSync(path.join(tmpdir(), "oriro-root-deps-audit-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("collectRootDependencyOwnershipCheckErrors", () => {
  ...
  it("classifies root dependencies referenced through constant dynamic imports", () => {
    ...
    writeRepoFile(
      repoRoot,
      "package.json",
      JSON.stringify({ dependencies: { oriropdf: "^0.2.0", "sqlite-vec": "0.1.9" } }),
    );
    writeRepoFile(
      repoRoot,
  ...
  it("allows runtime deps for bundled plugins that are still packaged in core", () => {
    ...
    writeRepoFile(repoRoot, "extensions/internal/oriro.plugin.json", JSON.stringify({}));
    writeRepoFile(
      repoRoot,
  ...
  it("keeps excluded bundled plugin deps localizable", () => {
    ...
    writeRepoFile(repoRoot, "extensions/externalized/oriro.plugin.json", JSON.stringify({}));
    writeRepoFile(
      repoRoot,
