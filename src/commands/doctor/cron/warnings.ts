...
export function noteCronModelOverrides(params: {
  ...
  lines.push(
    `Review with ${formatCliCommand("oriro cron list")} and ${formatCliCommand("oriro cron show <job-id>")}; remove \`payload.model\` from jobs that should inherit the default.`,
  );
...
export async function collectLegacyWhatsAppCrontabHealthWarning(
  ...
  return [
    ...
    "`~/.oriro/bin/ensure-whatsapp.sh` is not maintained by current Oriro and can misreport `Gateway inactive` from cron when the systemd user bus environment is missing.",
    `Remove the stale crontab entry with ${formatCliCommand("crontab -e")}; use ${formatCliCommand("oriro channels status --probe")}, ${formatCliCommand("oriro doctor")}, and ${formatCliCommand("oriro gateway status")} for current health checks.`,
