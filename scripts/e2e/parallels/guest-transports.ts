// Guest Transports script supports Oriro repository automation.
...
function guestScriptName(extension: string): string {
  return `oriro-parallels-${randomUUID()}.${extension}`;
}
...
export async function runWindowsBackgroundPowerShell(
  ...
  const fileBase = `oriro-parallels-${nonce}`;
  const logLengthPrefix = `__ORIRO_LOG_LENGTH__:${nonce}:`;
  const logOffsetPrefix = `__ORIRO_LOG_OFFSET__:${nonce}:`;
  const backgroundExitPrefix = `__ORIRO_BACKGROUND_EXIT__:${nonce}:`;
  const backgroundDoneMarker = `__ORIRO_BACKGROUND_DONE__:${nonce}`;
...
$exitPath = "$base.exit"
$pidPath = "$base.pid"
function Write-OriroUtf8File([string]$Path, [string]$Value) {
  [System.IO.File]::WriteAllText($Path, $Value, [System.Text.UTF8Encoding]::new($false))
}`;
  const payload = `$ErrorActionPreference = 'Stop'
$PSNativeCommandUseErrorActionPreference = $false
${pathsScript}
function Add-OriroBackgroundLog {
...
try {
  & {
${options.script}
  } *>&1 | Add-OriroBackgroundLog
  Write-OriroUtf8File $exitPath '0'
} catch {
  $_ | Add-OriroBackgroundLog
  Write-OriroUtf8File $exitPath '1'
} finally {
  Write-OriroUtf8File $donePath 'done'
}`;
...
Write-OriroUtf8File $pidPath ([string]$process.Id)
'started'`),
        ],
...
export class LinuxGuest {
  ...
  private transportArgs(args: string[]): string[] {
    return ["exec", this.vmName, "/usr/bin/env", "HOME=/root", "ORIRO_ALLOW_ROOT=1", ...args];
  }
