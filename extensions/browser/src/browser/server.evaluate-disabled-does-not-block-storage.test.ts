...
vi.mock("../config/config.js", async () => {
  ...
  const loadConfig = () => ({
    browser: {
      ...
      profiles: {
        oriro: { cdpPort: testPort + 1, color: "#FF4500" },
      },
    },
  });
  return {
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.ORIRO_GATEWAY_PORT;
    process.env.ORIRO_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.ORIRO_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.ORIRO_GATEWAY_PASSWORD;
