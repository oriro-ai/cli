...
Oriro can run **local AI CLIs** as a **text-only fallback** when API providers are down,
rate-limited, or temporarily misbehaving. This is intentionally conservative:

- **Oriro tools are not injected directly**, but backends with `bundleMcp: true`
...
```bash
oriro agent --agent main --message "hi" --model claude-cli/claude-sonnet-4-6
```
...
gateway host, Oriro now auto-loads the owning bundled plugin when your config
...
- If the primary provider fails (auth, rate limits, timeouts), Oriro will
    ...
    defaults: {
      cliBackends: {
        "my-cli": {
          ...
          // from bounded raw Oriro transcript history before compaction.
...
2. **Builds a system prompt** using the same Oriro prompt + workspace context.
3. **Executes the CLI** with a session id (if supported) so history stays consistent.
   The bundled `claude-cli` backend keeps a Claude stdio process alive per
   Oriro session and sends follow-up turns over stream-json stdin.
4. **Parses output** (JSON or plain text) and returns the final text.
5. **Persists session ids** per backend, so follow-ups reuse the same CLI session.

<Note>
The bundled Anthropic `claude-cli` backend is supported again. Anthropic staff
told us Oriro-style Claude CLI usage is allowed again, so Oriro treats
...
resolver for Oriro skills. When the current skills snapshot includes at least
one selected skill with a materialized path, Oriro passes a temporary Claude
Code plugin with `--plugin-dir` and omits the duplicate Oriro skills catalog
from the appended system prompt. If the snapshot has no materialized plugin
skill, Oriro keeps the prompt catalog as a fallback. Skill env/API key
overrides are still applied by Oriro to the child process environment for the
run.

Claude CLI also has its own noninteractive permission mode. Oriro maps that
to the existing exec policy instead of adding Claude-specific policy config.
For Oriro-managed Claude live sessions, the effective Oriro exec policy is
...
Claude launches normalize that flag to match the effective Oriro exec policy.

The bundled Anthropic `claude-cli` backend also maps Oriro `/think` levels
...
Before Oriro can use the bundled `claude-cli` backend, Claude Code itself
...
- `claude-cli` defaults to `liveSession: "claude-stdio"`, `output: "jsonl"`,
  ...
  exits, Oriro resumes from the stored Claude session id. Stored session
...
- Claude live sessions keep bounded JSONL output guards. Defaults allow up to
  ...
  and `maxTurnLines`; Oriro clamps those settings to 64 MiB and 100,000
  lines.
- Stored CLI sessions are provider-owned continuity. The implicit daily session
  ...
  do.
- Fresh CLI sessions normally reseed only from Oriro's compaction summary
  plus post-compaction tail. To recover short sessions that are invalidated
  before compaction, a backend can opt in with
  `reseedFromRawTranscriptWhenUncompacted: true`. Oriro still keeps raw
...
- Oriro drops stored CLI session reuse when the selected auth identity changes,
  ...
  stable OAuth account id, Oriro lets that CLI enforce resume permissions.

## Fallback prelude from claude-cli sessions

When a `claude-cli` attempt fails over to a non-CLI candidate in
[`agents.defaults.model.fallbacks`](/concepts/model-failover), Oriro seeds
...
provider would start cold because Oriro's own session transcript is empty
for `claude-cli` runs.
...
Oriro will write base64 images to temp files. If `imageArg` is set, those
paths are passed as CLI args. If `imageArg` is missing, Oriro appends the
...
- For Gemini CLI JSON output, Oriro reads reply text from `response` and usage
...
agent runs use the Codex app-server harness through `openai/*`; Oriro no
...
- If you override Gemini args to `--output-format json`, Oriro normalizes that
...
- `stats.cached` is normalized into Oriro `cacheRead`.
- If `stats.input` is missing, Oriro derives input tokens from
...
rewrites streamed assistant deltas and parsed final text before Oriro handles
...
Some CLI backends run an agent that compacts its **own** transcript, so Oriro must
...
`ownsNativeCompaction: true`, and Oriro returns a no-op from the compaction path.
...
`contextTokens: 1_000_000` purely to keep Oriro's safeguard from firing on a
...
CLI backends do **not** receive Oriro tool calls directly, but a backend can
...
When bundle MCP is enabled, Oriro:

- spawns a loopback HTTP MCP server that exposes gateway tools to the CLI process
- authenticates the bridge with a per-session token (`ORIRO_MCP_TOKEN`)
...
If no MCP servers are enabled, Oriro still injects a strict config when a
...
When a fresh CLI session is seeded from a prior Oriro transcript (for
...
- **No direct Oriro tool calls.** Oriro does not inject tool calls into
