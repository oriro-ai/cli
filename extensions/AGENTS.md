...
- Extension production code should import from `oriro/plugin-sdk/*` and its
...
- Keep plugin metadata accurate in `oriro.plugin.json` and the package
  `oriro` block so discovery and setup work without executing plugin code.
...
- Before adding a new provider-local `wrapStreamFn`, `buildReplayPolicy`,
  ...
  whether the same behavior already exists through `oriro/plugin-sdk/*`.
