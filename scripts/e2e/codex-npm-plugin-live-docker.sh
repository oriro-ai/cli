#!/usr/bin/env bash
# Installs Oriro from a prepared package tarball, installs @oriro/codex
...
CANDIDATE_ROOT="${ORIRO_LIVE_DOCKER_REPO_ROOT:-$SCRIPT_ROOT_DIR}"
...
IMAGE_NAME="$(docker_e2e_resolve_image "oriro-codex-npm-plugin-live-e2e" ORIRO_CODEX_NPM_PLUGIN_E2E_IMAGE)"
DOCKER_TARGET="${ORIRO_CODEX_NPM_PLUGIN_DOCKER_TARGET:-bare}"
HOST_BUILD="${ORIRO_CODEX_NPM_PLUGIN_HOST_BUILD:-1}"
PACKAGE_TGZ="${ORIRO_CURRENT_PACKAGE_TGZ:-}"
PROFILE_FILE="${ORIRO_CODEX_NPM_PLUGIN_PROFILE_FILE:-${ORIRO_TESTBOX_PROFILE_FILE:-$HOME/.oriro-testbox-live.profile}}"
CODEX_PLUGIN_SPEC="${ORIRO_CODEX_NPM_PLUGIN_SPEC:-}"
CODEX_PLUGIN_MOUNT=()
CODEX_PLUGIN_PACK_DIR=""
ASSERT_MAX_TEXT_FILE_BYTES="$(
  docker_e2e_read_positive_int_env ORIRO_CODEX_NPM_PLUGIN_ASSERT_MAX_TEXT_FILE_BYTES 1048576
)"
ASSERT_MAX_ERROR_TAIL_BYTES="$(
  docker_e2e_read_positive_int_env ORIRO_CODEX_NPM_PLUGIN_ASSERT_MAX_ERROR_TAIL_BYTES 65536
)"
ASSERT_MAX_TRANSCRIPT_FILES="$(
  docker_e2e_read_positive_int_env ORIRO_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_FILES 64
)"
ASSERT_MAX_TRANSCRIPT_WALK_ENTRIES="$(
  docker_e2e_read_positive_int_env ORIRO_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_WALK_ENTRIES 4096
)"
ASSERT_MAX_TRANSCRIPT_SCAN_BYTES="$(
  docker_e2e_read_positive_int_env ORIRO_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_SCAN_BYTES 2097152
)"
...
prepare_package_tgz() {
  ...
  if [ "$HOST_BUILD" = "0" ] && [ -z "${ORIRO_CURRENT_PACKAGE_TGZ:-}" ]; then
    echo "ORIRO_CODEX_NPM_PLUGIN_HOST_BUILD=0 requires ORIRO_CURRENT_PACKAGE_TGZ" >&2
    exit 1
  fi
...
prepare_codex_plugin_spec() {
  ...
  if [ -z "$CODEX_PLUGIN_SPEC" ]; then
    CODEX_PLUGIN_PACK_DIR="$(mktemp -d "${TMPDIR:-/tmp}/oriro-codex-plugin-pack.XXXXXX")"
    (
      cd "$CANDIDATE_ROOT"
    ...
    ) >/tmp/oriro-codex-plugin-pack.log 2>&1
    pack_output=()
    ...
    if [ "${#pack_output[@]}" -ne 1 ]; then
      ...
      docker_e2e_print_log /tmp/oriro-codex-plugin-pack.log >&2
      exit 1
    fi
...
ORIRO_TEST_STATE_SCRIPT_B64="$(docker_e2e_test_state_shell_b64 codex-npm-plugin-live empty)"
...
if ! docker_e2e_run_with_harness \
  ...
  -e ORIRO_CODEX_NPM_PLUGIN_ALLOW_BETA_COMPAT_DIAGNOSTICS="${ORIRO_CODEX_NPM_PLUGIN_ALLOW_BETA_COMPAT_DIAGNOSTICS:-0}" \
  -e ORIRO_CODEX_NPM_PLUGIN_FORCE_UNSAFE_INSTALL="${ORIRO_CODEX_NPM_PLUGIN_FORCE_UNSAFE_INSTALL:-1}" \
  -e ORIRO_CODEX_NPM_PLUGIN_MODEL="${ORIRO_CODEX_NPM_PLUGIN_MODEL:-codex/gpt-5.4}" \
  -e ORIRO_CODEX_NPM_PLUGIN_SPEC="$CODEX_PLUGIN_SPEC" \
  -e "ORIRO_CODEX_NPM_PLUGIN_ASSERT_MAX_TEXT_FILE_BYTES=$ASSERT_MAX_TEXT_FILE_BYTES" \
  -e "ORIRO_CODEX_NPM_PLUGIN_ASSERT_MAX_ERROR_TAIL_BYTES=$ASSERT_MAX_ERROR_TAIL_BYTES" \
  -e "ORIRO_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_FILES=$ASSERT_MAX_TRANSCRIPT_FILES" \
  -e "ORIRO_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_WALK_ENTRIES=$ASSERT_MAX_TRANSCRIPT_WALK_ENTRIES" \
  -e "ORIRO_CODEX_NPM_PLUGIN_ASSERT_MAX_TRANSCRIPT_SCAN_BYTES=$ASSERT_MAX_TRANSCRIPT_SCAN_BYTES" \
  -e "ORIRO_CODEX_NPM_PLUGIN_AGENT_TIMEOUT_SECONDS=$AGENT_TURN_TIMEOUT_SECONDS" \
  -e "ORIRO_E2E_COMMAND_TIMEOUT=$COMMAND_TIMEOUT" \
  -e OPENAI_API_KEY \
  -e OPENAI_BASE_URL \
...
CODEX_PLUGIN_SPEC="${ORIRO_CODEX_NPM_PLUGIN_SPEC:?missing ORIRO_CODEX_NPM_PLUGIN_SPEC}"
MODEL_REF="${ORIRO_CODEX_NPM_PLUGIN_MODEL:?missing ORIRO_CODEX_NPM_PLUGIN_MODEL}"
POST_UNINSTALL_MODEL_REF="codex/${MODEL_REF#*/}"
SESSION_ID="codex-npm-plugin-live"
SUCCESS_MARKER="ORIRO-CODEX-NPM-PLUGIN-LIVE-OK"
AGENT_TURN_TIMEOUT_SECONDS="${ORIRO_CODEX_NPM_PLUGIN_AGENT_TIMEOUT_SECONDS:-420}"
PLUGIN_INSTALL_FLAGS=(--force)
if [ "${ORIRO_CODEX_NPM_PLUGIN_FORCE_UNSAFE_INSTALL:-0}" = "1" ]; then
...
dump_debug_logs() {
  ...
  oriro_e2e_dump_logs \
    /tmp/oriro-install.log \
    /tmp/oriro-codex-plugin-install.log \
    /tmp/oriro-codex-plugin-enable.log \
    /tmp/oriro-codex-plugins-list.json \
    /tmp/oriro-codex-plugin-inspect.json \
    /tmp/oriro-codex-preflight.log \
    /tmp/oriro-codex-agent.json \
    /tmp/oriro-codex-agent.err \
    /tmp/oriro-codex-agent-turn1.json \
    /tmp/oriro-codex-agent-turn1.err \
    /tmp/oriro-codex-agent-turn2.json \
    /tmp/oriro-codex-agent-turn2.err \
    /tmp/oriro-codex-plugin-uninstall.log \
    /tmp/oriro-codex-plugins-list-after-uninstall.json \
    /tmp/oriro-codex-agent-after-uninstall.json \
    /tmp/oriro-codex-agent-after-uninstall.err
}
...
oriro_e2e_install_package /tmp/oriro-install.log
command -v oriro >/dev/null
oriro_e2e_enable_oriro_cli_timeout

echo "Installing Codex plugin: $CODEX_PLUGIN_SPEC"
oriro plugins install "$CODEX_PLUGIN_SPEC" "${PLUGIN_INSTALL_FLAGS[@]}" >/tmp/oriro-codex-plugin-install.log 2>&1

node scripts/e2e/lib/codex-npm-plugin-live/assertions.mjs configure "$MODEL_REF"

echo "Enabling Codex plugin..."
oriro plugins enable codex >/tmp/oriro-codex-plugin-enable.log 2>&1

oriro plugins list --json >/tmp/oriro-codex-plugins-list.json
oriro plugins inspect codex --runtime --json >/tmp/oriro-codex-plugin-inspect.json
...
"$CODEX_BIN" exec \
  ...
  --color never \
  --skip-git-repo-check \
  "Reply exactly: ${SUCCESS_MARKER}-PREFLIGHT" >/tmp/oriro-codex-preflight.log 2>&1 </dev/null
...
oriro plugins uninstall codex --force >/tmp/oriro-codex-plugin-uninstall.log 2>&1
oriro plugins list --json >/tmp/oriro-codex-plugins-list-after-uninstall.json
...
if ! grep -Fq 'Requested agent harness "codex" is not registered' /tmp/oriro-codex-agent-after-uninstall.err &&
  ! grep -Fq 'Unknown model: codex/' /tmp/oriro-codex-agent-after-uninstall.err; then
  echo "Unexpected post-uninstall agent error:" >&2
  tail -n 120 /tmp/oriro-codex-agent-after-uninstall.err >&2 || true
  exit 1
fi
