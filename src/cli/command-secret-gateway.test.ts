...
describe("resolveCommandSecretRefsViaGateway", () => {
  function makeTalkProviderApiKeySecretRefConfig(envKey: string): OriroConfig {
  ...
  async function createExecProviderConfig(refId: string): Promise<{
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-command-secret-exec-"));
    tempRoots.add(root);
    ...
    const resolverScript = [
      ...
      "  fs.writeFileSync(process.env.ORIRO_EXEC_MARKER, 'executed');",
        ...
        secrets: {
          providers: {
            default: {
              ...
              env: { ORIRO_EXEC_MARKER: markerPath },
