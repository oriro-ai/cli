 ...
 * See: https://github.com/oriro/oriro/issues/60552
 */
...
async function runEmbeddedAgentInternal(
  ...
  return enqueueSession(async () => {
    ...
    return enqueueGlobal(async () => {
      ...
      const pluginHarnessOwnsTransport = agentHarness.id !== "oriro";
      ...
      for (const candidateProvider of modelResolutionProviders) {
        const candidateResolution = await resolveModelAsync(
          ...
          params.config,
          {
            ...
            // first generating Oriro models.json. This keeps one-shot model runs from
      ...
      if (!modelResolution) {
        await ensureOriroModelsJson(params.config, agentDir, {
          workspaceDir: resolvedWorkspace,
        });
        for (const candidateProvider of modelResolutionProviders) {
          const candidateResolution = await resolveModelAsync(
      ...
      const pluginHarnessNeedsOriroAuthBootstrap =
        pluginHarnessOwnsTransport &&
        provider === OPENAI_PROVIDER_ID &&
        effectiveModel.api === "openai-chatgpt-responses";
      const openOriroNativeCodexResponsesNeedsAuthBootstrap =
        !pluginHarnessOwnsTransport &&
        provider === OPENAI_PROVIDER_ID &&
        effectiveModel.api === "openai-chatgpt-responses";
      let piExternalCliAuthScope = pluginHarnessOwnsTransport
        ? { ignoreAutoPreferredProfile: false }
        : openOriroNativeCodexResponsesNeedsAuthBootstrap
          ? {
      ...
      if (
        !pluginHarnessOwnsTransport &&
        !pluginHarnessNeedsOriroAuthBootstrap &&
      ...
      const authStore =
        pluginHarnessOwnsTransport && !pluginHarnessNeedsOriroAuthBootstrap
          ? createEmptyAuthProfileStore()
          : pluginHarnessNeedsOriroAuthBootstrap
      ...
      // Plugin harnesses own their model transport/auth. Running Oriro's generic
      ...
      if (!pluginHarnessOwnsTransport || pluginHarnessNeedsOriroAuthBootstrap) {
      ...
      const maybeMarkAuthProfileFailure = async (failure: {
        ...
        if (pluginHarnessOwnsTransport && reason === "timeout") {
          ...
          // credential evidence. Do not poison Oriro auth cooldowns.
          return;
        }
      ...
      try {
        ...
        while (true) {
          ...
          const rawAttempt = await runEmbeddedAttemptWithBackend({
            ...
            // attempt too. Otherwise plugin-owned transports can skip Oriro auth
            // bootstrap but drift back to Oriro when the attempt is created.
            agentHarnessId: agentHarness.id,
            ...(params.sessionKey
              ? {
            ...
            // These harnesses build Oriro tools internally. Keep transport auth
            // scoped while letting tool construction see plugin/provider creds.
            toolAuthProfileStore: harnessBuildsOriroTools ? attemptAuthProfileStore : undefined,
            modelRegistry,
            agentId: workspaceResolution.agentId,
