 ...
 */
import { parseBrowserHttpUrl } from "oriro/plugin-sdk/browser-config";
...
function withLoopbackBrowserAuthImpl(
  ...
  if (headers.has("authorization") || headers.has("x-oriro-password")) {
    return { ...init, headers };
  }
  ...
  try {
    ...
    if (auth.password) {
      headers.set("x-oriro-password", auth.password);
      return { ...init, headers };
    }
  } catch {
  ...
  try {
    ...
    if (bridgeAuth?.token) {
      ...
      headers.set("x-oriro-password", bridgeAuth.password);
    }
  } catch {
    // ignore
  }
