...
export async function loadValidConfigOrThrow(): Promise<OriroConfig> {
...
export async function updateConfig(
  mutator: (cfg: OriroConfig, context: UpdateConfigContext) => OriroConfig,
): Promise<OriroConfig> {
  const snapshot = await readConfigFileSnapshot();
  if (!snapshot.valid) {
    const issues = formatConfigIssueLines(snapshot.issues, "-").join("\n");
...
export function resolveModelTarget(params: { raw: string; cfg: OriroConfig }): {
  provider: string;
  model: string;
} {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("oriro agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
