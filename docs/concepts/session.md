---
summary: "How Oriro manages conversation sessions"
read_when:
...
Oriro organizes conversations into **sessions**. Each message is routed to a
...
Verify your setup with `oriro security audit`.
...
Oriro automatically bounds session storage over time. By default, it runs
...
`oriro sessions cleanup --dry-run --fix-dm-scope`. Applying the same flag
retires those old direct-DM rows and keeps their transcripts as deleted
archives.

Preview with `oriro sessions cleanup --dry-run`.

## Inspecting sessions

- `oriro status` -- session store path and recent activity.
- `oriro sessions --json` -- all sessions (filter with `--active <minutes>`).
