...
describe("runDaemonRestart health checks", () => {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["ORIRO_CONTAINER_HINT", "ORIRO_PROFILE"]);
    delete process.env.ORIRO_CONTAINER_HINT;
    ...
    service.readCommand.mockResolvedValue({
      programArguments: ["oriro", "gateway", "--port", "18789"],
      environment: {},
    });
  ...
  it("repairs stale loaded service definitions from gateway start", async () => {
    ...
    runServiceStart.mockImplementation(
      async (params: {
        ...
        await params.repairLoadedService?.({
          json: true,
          stdout: process.stdout,
          state: { command: { environment: { ORIRO_SERVICE_VERSION: "2026.4.24" } } },
    ...
    expect(repairParams.state?.command?.environment).toEqual({
      ORIRO_SERVICE_VERSION: "2026.4.24",
    });
  ...
  it("does not fall back to unmanaged restart when launchd repair reports headless GUI bootstrap failure", async () => {
    ...
    recoverInstalledLaunchAgent.mockRejectedValue(
      new Error("LaunchAgent oriro gateway restart requires a logged-in macOS GUI session"),
    );
    findVerifiedGatewayListenerPidsOnPortSync.mockReturnValue([4200]);
  ...
  it("delegates system-scope restart to systemctl without unmanaged signaling when root (oriro#87577)", async () => {
  ...
  it("surfaces systemd sudo guidance and never signals when restarting a system-scope unit as non-root (oriro#87577)", async () => {
    vi.spyOn(process, "platform", "get").mockReturnValue("linux");
    findInstalledSystemdGatewayScope.mockResolvedValue({
      scope: "system",
      unitName: "oriro.service",
    ...
    restartSystemdService.mockRejectedValue(
      new Error(
        "oriro.service is a system-scope unit (/etc/systemd/system/oriro.service); run `sudo systemctl restart oriro.service` to restart it",
      ),
    );
    findVerifiedGatewayListenerPidsOnPortSync.mockReturnValue([4200]);
    mockUnmanagedRestart();
    ...
    await expect(runDaemonRestart({ json: true })).rejects.toThrow(
      /sudo systemctl restart oriro\.service/,
    );
  ...
  it("delegates system-scope stop to systemctl without unmanaged signaling when root (oriro#87577)", async () => {
    vi.spyOn(process, "platform", "get").mockReturnValue("linux");
    findInstalledSystemdGatewayScope.mockResolvedValue({
      scope: "system",
      unitName: "oriro-gateway.service",
  ...
  it("surfaces systemd sudo guidance and never signals when stopping a system-scope unit as non-root (oriro#87577)", async () => {
    vi.spyOn(process, "platform", "get").mockReturnValue("linux");
    findInstalledSystemdGatewayScope.mockResolvedValue({
      scope: "system",
      unitName: "oriro-gateway.service",
    ...
    stopSystemdService.mockRejectedValue(
      new Error(
        "oriro-gateway.service is a system-scope unit (/etc/systemd/system/oriro-gateway.service); run `sudo systemctl stop oriro-gateway.service` to stop it",
      ),
    );
    findVerifiedGatewayListenerPidsOnPortSync.mockReturnValue([4200]);
    runServiceStop.mockImplementation(async (params: { onNotLoaded?: () => Promise<unknown> }) => {
