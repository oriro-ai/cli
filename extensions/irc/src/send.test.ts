...
import { createSendCfgThreadingRuntime } from "oriro/plugin-sdk/channel-test-helpers";
...
vi.mock("oriro/plugin-sdk/plugin-config-runtime", async () => {
  const original = (await vi.importActual("oriro/plugin-sdk/plugin-config-runtime")) as Record<
    string,
    unknown
  >;
  return {
...
vi.mock("oriro/plugin-sdk/text-chunking", async () => {
  const original = (await vi.importActual("oriro/plugin-sdk/text-chunking")) as Record<
    string,
    unknown
  >;
  return {
...
afterAll(() => {
  ...
  vi.doUnmock("oriro/plugin-sdk/text-chunking");
  vi.resetModules();
});
