...
describe("channel registry helpers", () => {
  ...
  it("formats Telegram selection lines without a docs prefix and with website extras", () => {
    ...
    expect(line).toContain("https://oriro.ai");
  });

  it("prefers the pinned channel registry when resolving registered plugin channels", () => {
    const startupRegistry = createRegistryWithRegisteredChannel("oriro-weixin", ["weixin"]);
    ...
    expect(listRegisteredChannelPluginIds()).toEqual(["oriro-weixin"]);
    expect(normalizeAnyChannelId("weixin")).toBe("oriro-weixin");
    expect(getRegisteredChannelPluginMeta("ORIRO-WEIXIN")?.aliases).toEqual(["weixin"]);
  });
