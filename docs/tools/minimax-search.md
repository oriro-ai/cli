...
Oriro supports MiniMax as a `web_search` provider through the MiniMax
...
Oriro also accepts `MINIMAX_CODING_API_KEY`, `MINIMAX_OAUTH_TOKEN`, and
...
If `plugins.entries.minimax.config.webSearch.region` is unset, Oriro resolves
...
| `count`   | integer | 1-10        | Number of results to return. Oriro trims the returned list to this size. |
