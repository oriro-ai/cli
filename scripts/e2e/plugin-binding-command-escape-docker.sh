...
IMAGE_NAME="${ORIRO_PLUGIN_BINDING_COMMAND_ESCAPE_E2E_IMAGE:-oriro-plugin-binding-command-escape-e2e}"
CONTAINER_NAME="oriro-plugin-binding-command-escape-e2e-$$"
DOCKER_RUN_TIMEOUT="${ORIRO_PLUGIN_BINDING_COMMAND_ESCAPE_DOCKER_RUN_TIMEOUT:-900s}"
RUN_LOG="$(mktemp -t oriro-plugin-binding-command-escape-log.XXXXXX)"
...
DOCKER_COMMAND_TIMEOUT="$DOCKER_RUN_TIMEOUT" docker_e2e_docker_run_cmd run --rm \
  ...
  -e ORIRO_VITEST_FS_MODULE_CACHE_PATH=/tmp/oriro-vitest-cache \
  "$IMAGE_NAME" \
