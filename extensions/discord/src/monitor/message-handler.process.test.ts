...
import { DEFAULT_EMOJIS, DEFAULT_TIMING } from "oriro/plugin-sdk/channel-feedback";
import {
  recordChannelBotPairLoopAndCheckSuppression,
  type ChannelBotLoopProtectionFacts,
} from "oriro/plugin-sdk/channel-inbound";
import type { ReplyPayload } from "oriro/plugin-sdk/reply-dispatch-runtime";
import { setReplyPayloadMetadata } from "oriro/plugin-sdk/reply-payload-testing";
import * as runtimeEnvModule from "oriro/plugin-sdk/runtime-env";
...
function createNonTerminalToolWarningPayload(): ReplyPayload {
  return setReplyPayloadMetadata(
    {
      text: "⚠️ 🛠️ `run oriro definitely-not-a-real-subcommand (agent)` failed",
      isError: true,
    },
...
const configSessionsMocks = vi.hoisted(() => ({
  ...
  resolveAndPersistSessionFile: vi.fn<(params?: unknown) => Promise<{ sessionFile: string }>>(
    async () => ({ sessionFile: "/tmp/oriro-discord-process-test-session.jsonl" }),
  ),
  ...
  resolveStorePath: vi.fn<(path?: unknown, opts?: unknown) => string>(
    () => "/tmp/oriro-discord-process-test-sessions.json",
  ),
}));
...
vi.mock("oriro/plugin-sdk/reply-runtime", () => ({
...
vi.mock("oriro/plugin-sdk/conversation-runtime", () => ({
...
vi.mock("oriro/plugin-sdk/session-store-runtime", () => ({
...
beforeEach(() => {
  ...
  resolveAndPersistSessionFile.mockResolvedValue({
    sessionFile: "/tmp/oriro-discord-process-test-session.jsonl",
  });
  resolveSessionStoreEntry.mockImplementation((params) => ({
    existing: params.sessionKey ? params.store[params.sessionKey] : undefined,
  }));
  resolveStorePath.mockReturnValue("/tmp/oriro-discord-process-test-sessions.json");
  ...
  it("applies status reaction emoji/timing overrides from config", async () => {
    ...
    const ctx = await createAutomaticSourceDeliveryContext({
      cfg: {
        ...
        session: { store: "/tmp/oriro-discord-process-test-sessions.json" },
      },
    });
...
describe("processDiscordMessage session routing", () => {
  ...
  it("pins Discord text DM main-route updates to the single configured DM owner", async () => {
    const ctx = await createBaseContext({
      ...
      cfg: {
        messages: { ackReaction: "👀" },
        session: {
          store: "/tmp/oriro-discord-process-test-sessions.json",
          dmScope: "main",
        },
      },
  ...
  it("prefers bound session keys and sets MessageThreadId for bound thread messages", async () => {
    const threadBindings = createThreadBindingManager({
      cfg: {} as import("oriro/plugin-sdk/config-contracts").OriroConfig,
      accountId: "default",
      persist: false,
      enableSweeper: false,
    });
...
describe("processDiscordMessage draft streaming", () => {
  ...
  it("keeps Discord progress lines below the configured label", async () => {
    ...
    expect(draftStream.update).toHaveBeenCalledWith("Oriroing...\n\n🧩 First\n🧩 Second\n🧩 Third");
  });
