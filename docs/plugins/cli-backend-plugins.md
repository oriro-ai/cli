...
read_when:
  ...
  - You need to map a third-party CLI into Oriro's text fallback runner
---

CLI backend plugins let Oriro call a local AI CLI as a text inference
...
| -------------------- | ---------------------- | --------------------------------------------------------- |
| Package entry        | `package.json`         | Points Oriro at the plugin runtime module              |
| Manifest ownership   | `oriro.plugin.json` | Declares the backend id before runtime loads              |
...
<Steps>
  <Step title="Create package metadata">
    ...
    entry is `./src/index.ts`, add `oriro.runtimeExtensions` that points at
  ...
  <Step title="Declare backend ownership">
    ```json oriro.plugin.json
    {
      "id": "acme-cli",
      "name": "Acme CLI",
      "description": "Run Acme's local AI CLI through Oriro",
    ...
    `cliBackends` is the runtime ownership list. It lets Oriro auto-load the
  ...
  <Step title="Register the backend">
    ...
    } from "oriro/plugin-sdk/cli-backend";
...
`CliBackendConfig` describes how Oriro should launch and parse the CLI:
...
| `modelAliases`                            | Map Oriro model ids to CLI-native ids                    |
| `sessionArg` / `sessionArgs`              | How to pass a session id                                    |
| `sessionMode`                             | `always`, `existing`, or `none`                             |
| `sessionIdFields`                         | JSON fields Oriro reads from CLI output                  |
...
| `defaultAuthProfileId`             | Prefer a specific Oriro auth profile                                     |
...
| `bundleMcp` / `bundleMcpMode`      | Opt into Oriro's loopback MCP tool bridge                                |
| `ownsNativeCompaction`             | Backend owns its own compaction - Oriro defers                           |
...
otherwise Oriro fails closed when BTW requires a no-tools CLI run.

### `ownsNativeCompaction`: opting out of Oriro compaction

If your backend runs an agent that compacts its **own** transcript, set
`ownsNativeCompaction: true` so Oriro's safeguard summarizer never runs against its
...
stay over budget / go stale (Oriro no longer rescues it):
...
CLI backends do not receive Oriro tools by default. If the CLI can consume an
...
"always-on"` so Oriro can fail closed when a caller requires no native tools.
...
```bash
oriro plugins inspect acme-cli --runtime --json
oriro agent --message "reply exactly: backend ok" --model acme-cli/acme-large
```
...
<Check>`package.json` has `oriro.extensions` and built runtime entries for published packages</Check>
<Check>`oriro.plugin.json` declares `cliBackends` and intentional `activation.onStartup`</Check>
