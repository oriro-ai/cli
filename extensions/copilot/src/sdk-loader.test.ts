...
describe("sdk-loader", () => {
  ...
  it("default fallback importer resolves and imports the installed SDK entry", async () => {
    ...
    try {
      ...
      writeFileSync(
        ...
        "module.exports = { oriroDefaultImporterSentinel: true };",
      );
      ...
      })) as unknown as { oriroDefaultImporterSentinel?: boolean };

      expect(sdk.oriroDefaultImporterSentinel).toBe(true);
      expect(primaryImport).toHaveBeenCalledTimes(1);
    } finally {
      rmSync(tmp, { recursive: true, force: true });
    }
  ...
  it("throws an actionable error with plugin install instructions when both probes fail", async () => {
    ...
    await expect(
      ...
      message: expect.stringContaining("oriro plugins install @oriro/copilot"),
    });
  ...
  it("error message includes the fallback path and underlying primary error", async () => {
    ...
    expect(message).toContain("oriro plugins install @oriro/copilot");
  });
  ...
  it("resolves the fallback dir from ORIRO_STATE_DIR for relocated profiles", () => {
    expect(
      resolveCopilotSdkFallbackDir({
        ...process.env,
        ORIRO_STATE_DIR: "/tmp/oriro-state",
      }),
    ).toBe(path.join("/tmp/oriro-state", "npm-runtime", "copilot"));
  });
