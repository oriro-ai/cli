...
function loadProfileEnv(homeDir = os.homedir()): void {
  ...
  try {
    ...
    if (applied > 0 && !isTruthyEnvValue(process.env.ORIRO_LIVE_TEST_QUIET)) {
...
function resolveRestoreEntries(): RestoreEntry[] {
  return [
    { key: "ORIRO_TEST_FAST", value: process.env.ORIRO_TEST_FAST },
    {
      key: "ORIRO_STRICT_FAST_REPLY_CONFIG",
      value: process.env.ORIRO_STRICT_FAST_REPLY_CONFIG,
    },
    {
      key: "ORIRO_ALLOW_SLOW_REPLY_TESTS",
      value: process.env.ORIRO_ALLOW_SLOW_REPLY_TESTS,
    },
    {
      key: "ORIRO_LIVE_TEST_NORMALIZE_CONFIG",
      value: process.env.ORIRO_LIVE_TEST_NORMALIZE_CONFIG,
    },
    ...
    { key: "ORIRO_STATE_DIR", value: process.env.ORIRO_STATE_DIR },
    { key: "ORIRO_CONFIG_PATH", value: process.env.ORIRO_CONFIG_PATH },
    { key: "ORIRO_GATEWAY_PORT", value: process.env.ORIRO_GATEWAY_PORT },
    { key: "ORIRO_BRIDGE_ENABLED", value: process.env.ORIRO_BRIDGE_ENABLED },
    { key: "ORIRO_BRIDGE_HOST", value: process.env.ORIRO_BRIDGE_HOST },
    { key: "ORIRO_BRIDGE_PORT", value: process.env.ORIRO_BRIDGE_PORT },
    { key: "ORIRO_CANVAS_HOST_PORT", value: process.env.ORIRO_CANVAS_HOST_PORT },
    { key: "ORIRO_TEST_HOME", value: process.env.ORIRO_TEST_HOME },
    { key: "ORIRO_AGENT_DIR", value: process.env.ORIRO_AGENT_DIR },
...
function createIsolatedTestHome(restore: RestoreEntry[]): {
  ...
  const tempHome = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-test-home-"));

  process.env.HOME = tempHome;
  process.env.USERPROFILE = tempHome;
  process.env.ORIRO_TEST_HOME = tempHome;
  process.env.ORIRO_TEST_FAST = "1";
  process.env.ORIRO_STRICT_FAST_REPLY_CONFIG = "1";
  delete process.env.ORIRO_ALLOW_SLOW_REPLY_TESTS;
  ...
  delete process.env.ORIRO_GATEWAY_PORT;
  delete process.env.ORIRO_BRIDGE_ENABLED;
  delete process.env.ORIRO_BRIDGE_HOST;
  delete process.env.ORIRO_BRIDGE_PORT;
  delete process.env.ORIRO_CANVAS_HOST_PORT;
...
function sanitizeLiveConfig(raw: string): string {
  try {
    ...
    if (!isTruthyEnvValue(process.env.ORIRO_LIVE_TEST_NORMALIZE_CONFIG)) {
...
function stageLiveTestState(params: {
  ...
  const rawStateDir = params.env.ORIRO_STATE_DIR?.trim();
  let realStateDir = rawStateDir
    ? resolveHomeRelativePath(rawStateDir, params.realHome)
    : path.join(params.realHome, ".oriro");
  const priorIsolatedHome = params.env.ORIRO_TEST_HOME?.trim();
  const snapshotHome = params.env.HOME?.trim();
  if (
    priorIsolatedHome &&
    snapshotHome &&
    snapshotHome !== priorIsolatedHome &&
    realStateDir === path.join(priorIsolatedHome, ".oriro")
  ) {
    realStateDir = path.join(params.realHome, ".oriro");
  }
  const tempStateDir = path.join(params.tempHome, ".oriro");
  fs.mkdirSync(tempStateDir, { recursive: true });
  fs.mkdirSync(path.join(params.tempHome, ".gemini"), { recursive: true });

  const realConfigPath = params.env.ORIRO_CONFIG_PATH?.trim()
    ? resolveHomeRelativePath(params.env.ORIRO_CONFIG_PATH, params.realHome)
    : path.join(realStateDir, "oriro.json");
  if (fs.existsSync(realConfigPath)) {
    const rawConfig = fs.readFileSync(realConfigPath, "utf8");
    fs.writeFileSync(
      path.join(tempStateDir, "oriro.json"),
...
export function installTestEnv(options?: { loadProfileEnv?: boolean }): {
  ...
  const live =
    process.env.LIVE === "1" ||
    process.env.ORIRO_LIVE_TEST === "1" ||
    process.env.ORIRO_LIVE_GATEWAY === "1";
  const allowRealHome = isTruthyEnvValue(process.env.ORIRO_LIVE_USE_REAL_HOME);
