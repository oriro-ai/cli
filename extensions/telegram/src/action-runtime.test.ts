...
import { resolveStorePath } from "oriro/plugin-sdk/session-store-runtime";
...
describe("handleTelegramAction", () => {
  ...
  function reactionConfig(reactionLevel: "minimal" | "extensive" | "off" | "ack"): OriroConfig {
    return {
      channels: { telegram: { botToken: "tok", reactionLevel } },
    } as OriroConfig;
  }

  function telegramConfig(overrides?: Record<string, unknown>): OriroConfig {
    return {
      channels: {
        telegram: {
          botToken: "tok",
  ...
  function topicCacheScopeFor(cfg: OriroConfig, accountId: string): string {
  ...
  beforeEach(() => {
    envSnapshot = captureEnv(["ORIRO_STATE_DIR", "TELEGRAM_BOT_TOKEN"]);
    resetTopicNameCacheForTest();
  ...
  it("rejects sticker actions when disabled by default", async () => {
    const cfg = { channels: { telegram: { botToken: "tok" } } } as OriroConfig;
    await expect(
  ...
  it("persists sendMessage action deliveries before Telegram platform send", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-telegram-action-durable-"));
    const {
      createOutboundTestPlugin,
      createTestRegistry,
      readQueuedDeliveryEntriesForTest,
      setActivePluginRegistry,
    } = await import("oriro/plugin-sdk/plugin-test-runtime");
  ...
  it("stores created forum topic names in the account-scoped cache", async () => {
    ...
    const cfg = {
      ...
      session: { store: path.join(os.tmpdir(), "oriro-telegram-action-sessions.json") },
    } as OriroConfig;
...
describe("handleTelegramAction per-account gating", () => {
  ...
  async function expectAccountStickerSend(cfg: OriroConfig, accountId = "media") {
    await handleTelegramAction(
