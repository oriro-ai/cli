...
// oriro/plugin-sdk/agent-harness-runtime declares auth, messages,
// onAssistantDelta, and initialReplayState.sdkSessionId fields. Tracked by
// project oriro-copilot-harness; reviewer-attempt-bridge note.
...
type AttemptParamsLike = AgentHarnessAttemptParams & {
  ...
  // current-turn user message for the Oriro audit transcript so
...
export interface CopilotAttemptDeps {
   ...
   * `oriro/plugin-sdk/agent-harness-runtime#resolveSandboxContext`, which is
   ...
   * harness uses this to track the oriroSessionId -> sdkSessionId
...
export async function runCopilotAttempt(
  ...
  try {
    ...
    try {
      const toolBridge = await createToolBridge({
        ...
        // tool-bridge.ts buildOriroCodingToolsOptions().
        attemptParams: input,
        sessionRef,
        onYieldDetected: () => {
          yieldDetected = true;
    ...
    // Load Oriro workspace bootstrap files (SOUL.md, IDENTITY.md,
    ...
    // Oriro bootstrap block (SDK still loads AGENTS.md natively).
    ...
    const emitLlmInput = (prompt: string, additionalContext?: string) => {
      runAgentHarnessLlmInputHook({
        event: {
          ...
          // Copilot SDK sessions own their own transcript. Oriro's
    ...
    if (abortRequested || params.abortSignal?.aborted) {
      ...
      if (!bridge.recordSendResult(result) && !aborted) {
        ...
        // capability inventory. Do not call session.abort() here: Oriro may
  ...
  //
  // Without this synthesis the Oriro audit transcript never sees
  ...
  // (user/assistant/toolResult) into the Oriro audit transcript at
  // params.sessionFile, alongside the SDK's own session storage. The
  // Oriro shell (attempt-execution.ts) writes only the user prompt
  ...
  if (sessionFileForMirror && messagesSnapshot.length > 0) {
    ...
    await dualWriteCopilotTranscriptBestEffort({
      ...
      // authoritative; only the Oriro audit transcript would be
...
function createSessionConfig(
  ...
  return {
    ...
    // tool-bridge.ts) so 100% of tool calls go through Oriro's
    ...
    ...(resolvedAuth.authMode === "gitHubToken" && resolvedAuth.gitHubToken
      ...
      : {}),
    // Oriro workspace bootstrap plus per-turn runtime guidance
    // injected via the SDK's `systemMessage` field in append mode:
    // SDK foundation + Oriro context. Append keeps every SDK
    ...
    // L1036). Omitted when there is no Oriro-owned context so the
...
function hasMirrorIdentity(message: AgentMessage): boolean {
  const record = message as unknown as { __oriro?: unknown };
