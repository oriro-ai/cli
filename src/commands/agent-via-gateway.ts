...
let replyPayloadModulePromise:
  | Promise<typeof import("oriro/plugin-sdk/reply-payload")>
  | undefined;
...
async function loadRuntimeConfig(): Promise<OriroConfig> {
...
function loadReplyPayloadModule() {
  replyPayloadModulePromise ??= import("oriro/plugin-sdk/reply-payload");
...
function parseTimeoutSeconds(opts: { cfg: OriroConfig; timeout?: string }) {
  const raw =
...
async function agentViaGatewayCommand(
  ...
  if (!body) {
    throw new Error(
      `Missing message. Use ${formatCliCommand('oriro agent --message "..." --agent <id>')} or pass --to/--session-key/--session-id for an existing conversation.`,
    );
  }
  if (!opts.to && !opts.sessionId && !opts.agent && !explicitSessionKey) {
    throw new Error(
      `No target session selected. Use --agent <id>, --session-key <key>, --session-id <id>, or --to <E.164>. Run ${formatCliCommand("oriro agents list")} to see agents.`,
    );
  }
  ...
  if (agentId) {
    ...
    if (!knownAgents.includes(agentId)) {
      throw new Error(
        `Unknown agent id "${agentIdRaw}". Use "${formatCliCommand("oriro agents list")}" to see configured agents.`,
      );
    }
  }
  ...
  const dispatchGatewayAgentCall = async (activeCfg: OriroConfig) =>
    await withProgress(
      {
...
export async function agentCliCommand(
  ...
  if (isCompactControlCommand(opts.message)) {
    runtime.error?.(
      "Slash commands cannot be executed via --message from the CLI. Use: oriro sessions compact <key>",
    );
    runtime.exit(1);
    return undefined;
  }
