...
function resetGatewayCallMocks() {
  ...
  connectError = null;
  const loadConfigForTests = getRuntimeConfig as unknown as () => OriroConfig;
...
describe("callGateway url resolution", () => {
  ...
  beforeEach(() => {
    envSnapshot.restore();
    delete process.env.ORIRO_ALLOW_INSECURE_PRIVATE_WS;
    delete process.env.ORIRO_CONFIG_PATH;
    delete process.env.ORIRO_GATEWAY_PORT;
    delete process.env.ORIRO_GATEWAY_URL;
  ...
  it("allows Tailscale-authenticated backend calls without client-side credentials", async () => {
    getRuntimeConfig.mockReturnValue({
      gateway: {
        mode: "remote",
        remote: { url: "wss://oriro.example.test" },
    ...
    expect(lastClientOptions?.url).toBe("wss://oriro.example.test");
    expect(lastClientOptions?.token).toBeUndefined();
  ...
  it("uses ORIRO_GATEWAY_URL env override in remote mode when remote URL is missing", async () => {
    getRuntimeConfig.mockReturnValue({
      gateway: { mode: "remote", bind: "loopback", remote: {} },
    });
    resolveGatewayPort.mockReturnValue(18789);
    pickPrimaryTailnetIPv4.mockReturnValue(undefined);
    process.env.ORIRO_GATEWAY_URL = "wss://gateway-in-container.internal:9443/ws";
    process.env.ORIRO_GATEWAY_TOKEN = "env-token";
...
describe("buildGatewayConnectionDetails", () => {
  ...
  it("uses env ORIRO_GATEWAY_URL when set", () => {
    getRuntimeConfig.mockReturnValue({ gateway: { mode: "local", bind: "loopback" } });
    resolveGatewayPort.mockReturnValue(18800);
    pickPrimaryTailnetIPv4.mockReturnValue(undefined);
    const prevUrl = process.env.ORIRO_GATEWAY_URL;
    try {
      process.env.ORIRO_GATEWAY_URL = "wss://browser-gateway.local:9443/ws";

      const details = buildGatewayConnectionDetails();

      expect(details.url).toBe("wss://browser-gateway.local:9443/ws");
      expect(details.urlSource).toBe("env ORIRO_GATEWAY_URL");
      expect(details.bindDetail).toBeUndefined();
    } finally {
      if (prevUrl === undefined) {
        delete process.env.ORIRO_GATEWAY_URL;
      } else {
        process.env.ORIRO_GATEWAY_URL = prevUrl;
      }
    }
  });

  it("falls back to the default config loader when test deps drift", () => {
    const tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-gateway-call-"));
    process.env.ORIRO_STATE_DIR = tempStateDir;
    process.env.ORIRO_CONFIG_PATH = path.join(tempStateDir, "missing-config.json");
    try {
      getRuntimeConfig.mockReturnValue({ gateway: { mode: "local", bind: "loopback" } });
      resolveGatewayPort.mockReturnValue(18800);
      testing.setDepsForTests({
  ...
  it("throws for insecure ws:// remote URLs (CWE-319)", () => {
    ...
    expect((thrown as Error).message).toContain("oriro doctor --fix");
  });
  ...
  it("allows ws:// hostname remote URLs when ORIRO_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.ORIRO_ALLOW_INSECURE_PRIVATE_WS = "1";
    getRuntimeConfig.mockReturnValue({
      gateway: {
        mode: "remote",
        bind: "loopback",
        remote: { url: "ws://oriro-gateway.ai:18789" },
      },
    });
    resolveGatewayPort.mockReturnValue(18789);
    ...
    expect(details.url).toBe("ws://oriro-gateway.ai:18789");
    expect(details.urlSource).toBe("config gateway.remote.url");
  });
...
describe("callGateway error details", () => {
  ...
  it("keeps the default wrapper timeout aligned with env handshake timeout", async () => {
    const envSnapshot = captureEnv(["ORIRO_HANDSHAKE_TIMEOUT_MS"]);
    try {
      process.env.ORIRO_HANDSHAKE_TIMEOUT_MS = "30000";
      startMode = "silent";
      setLocalLoopbackGatewayConfig();
  ...
  it("runs the signal abort hook on the active gateway connection before teardown", async () => {
    ...
    testing.setDepsForTests({
      ...
      getRuntimeConfig: getRuntimeConfig as unknown as () => OriroConfig,
...
describe("callGateway url override auth requirements", () => {
  ...
  it("throws when url override is set without explicit credentials", async () => {
    process.env.ORIRO_GATEWAY_TOKEN = "env-token";
    process.env.ORIRO_GATEWAY_PASSWORD = "env-password";
    getRuntimeConfig.mockReturnValue({
      gateway: {
        mode: "local",
  ...
  it("throws when env URL override is set without env credentials", async () => {
    process.env.ORIRO_GATEWAY_URL = "wss://override.example/ws";
    getRuntimeConfig.mockReturnValue({
      gateway: {
        mode: "local",
        auth: { token: "local-token", password: "local-password" },
...
describe("callGateway password resolution", () => {
  ...
  it.each([
    ...
    if (envPassword !== undefined) {
      process.env.ORIRO_GATEWAY_PASSWORD = envPassword;
    }
  ...
  it("does not resolve local password ref when env password takes precedence", async () => {
    process.env.ORIRO_GATEWAY_PASSWORD = "from-env";
    getRuntimeConfig.mockReturnValue({
      gateway: {
        mode: "local",
        bind: "loopback",
