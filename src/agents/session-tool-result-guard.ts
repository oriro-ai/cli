 ...
 */
import { resolveIntegerOption } from "@oriro/normalization-core/number-coercion";
...
const PERSISTED_DETAIL_REDACTION_BOUNDARY = "\u0000ORIRO_PERSISTED_DETAIL_BOUNDARY\u0000";
...
function isTranscriptOnlyOriroAssistantMessage(message: AgentMessage): boolean {
  ...
  return isTranscriptOnlyOriroAssistantModel(provider, model);
}
...
export function installSessionToolResultGuard(
  ...
  const guardedAppend = (message: AgentMessage, callerOptions?: AppendMessageOptions) => {
    ...
    const transcriptOnlyAssistant =
      nextRole === "assistant" &&
      toolCalls.length === 0 &&
      isTranscriptOnlyOriroAssistantMessage(nextMessage);
    if (
