...
function resolveCompactionReserveTokensFloor(cfg?: OriroConfig): number {
...
export function resolveEffectiveCompactionMode(cfg?: OriroConfig): AgentCompactionMode {
 ...
 * fires on a successful turn and triggers Oriro runtime's `_runAutoCompaction` from
 * inside `Session.prompt()`, collapsing `agent.state.messages` before the
 * provider call (oriro#75799).
 *
...
/**
 * Disable Oriro runtime's `_checkCompaction → _runAutoCompaction` (which would otherwise
 * fire from inside `Session.prompt()` and reassign `agent.state.messages`
 * before the provider call) when Oriro or a plugin owns compaction:
 * `contextEngineInfo.ownsCompaction === true`, effective safeguard compaction,
 * or an active model that is silent-overflow-prone (oriro#75799).
 * Default-mode runs against ordinary providers keep Oriro runtime's auto-compaction as
 ...
 * rehydrates `compaction.enabled` from disk and silently restores Oriro runtime's
