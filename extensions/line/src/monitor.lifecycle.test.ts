...
import { createMockIncomingRequest } from "oriro/plugin-sdk/test-env";
import { WEBHOOK_IN_FLIGHT_DEFAULTS } from "oriro/plugin-sdk/webhook-request-guards";
...
vi.mock("oriro/plugin-sdk/webhook-ingress", async () => {
  const actual = await vi.importActual<typeof import("oriro/plugin-sdk/webhook-ingress")>(
    "oriro/plugin-sdk/webhook-ingress",
  );
  return {
    ...actual,
...
describe("monitorLineProvider lifecycle", () => {
  ...
  afterAll(() => {
    vi.doUnmock("./bot.js");
    vi.doUnmock("oriro/plugin-sdk/reply-runtime");
    vi.doUnmock("oriro/plugin-sdk/runtime-env");
    vi.doUnmock("oriro/plugin-sdk/webhook-ingress");
