...
import { resolveMemoryDreamingConfig } from "oriro/plugin-sdk/memory-core-host-status";
import type { OriroPluginApi, PluginCommandContext } from "oriro/plugin-sdk/plugin-entry";
import { normalizeLowercaseStringOrEmpty } from "oriro/plugin-sdk/string-coerce-runtime";
import { asRecord } from "./dreaming-shared.js";
import { resolveShortTermPromotionDreamingConfig } from "./dreaming.js";

function resolveMemoryCorePluginConfig(cfg: OriroConfig): Record<string, unknown> {
  const entry = asRecord(cfg.plugins?.entries?.["memory-core"]);
  return asRecord(entry?.config) ?? {};
}

function updateDreamingEnabledInConfig(cfg: OriroConfig, enabled: boolean): OriroConfig {
...
function formatStatus(cfg: OriroConfig): string {
...
export async function handleDreamingCommand(api: OriroPluginApi, ctx: PluginCommandContext) {
