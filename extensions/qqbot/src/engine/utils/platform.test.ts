...
describe("qqbot local media path remapping", () => {
  ...
  function createOriroTestRoot() {
    const actualHome = getHomeDir();
    const oriroDir = path.join(actualHome, ".oriro");
    fs.mkdirSync(oriroDir, { recursive: true });
    const testRoot = fs.mkdtempSync(path.join(oriroDir, "qqbot-platform-test-"));
  ...
  function createQqbotMediaFile(fileName: string) {
    const { actualHome, testRootName } = createOriroTestRoot();
  ...
  it("allows structured payload files inside sibling Oriro media subdirectories", () => {
    ...
    // trust the shared `~/.oriro/media` root so auto-routed sends can access
    // those files without the path-outside-storage guard firing.
    const actualHome = getHomeDir();
    const outboundDir = path.join(actualHome, ".oriro", "media", "outbound");
...
// home as the baseline and only varies `process.env.ORIRO_HOME`.
describe("qqbot media path resolution honors ORIRO_HOME (#83562)", () => {
  ...
  function makeFakeOriroHome(): string {
  ...
  it("accepts files under $ORIRO_HOME/.oriro/media/qqbot when ORIRO_HOME differs from HOME", () => {
    const fakeOriroHome = makeFakeOriroHome();
    vi.stubEnv("ORIRO_HOME", fakeOriroHome);

    const mediaFile = path.join(fakeOriroHome, ".oriro", "media", "qqbot", "repro.png");
    ...
    const oldHomeMediaRoot = path.join(realOsHome, ".oriro", "media", "qqbot");
    expect(isPathInsideOrEqual(mediaFile, oldHomeMediaRoot)).toBe(false);
    fs.mkdirSync(path.dirname(mediaFile), { recursive: true });
    fs.writeFileSync(mediaFile, "image", "utf8");

    expect(getQQBotMediaPath()).toBe(path.join(fakeOriroHome, ".oriro", "media", "qqbot"));
    expect(resolveQQBotPayloadLocalFilePath(mediaFile)).toBe(fs.realpathSync(mediaFile));
  });

  it("expands tilde-prefixed ORIRO_HOME against the OS home", () => {
    ...
    vi.stubEnv("ORIRO_HOME", `~/${sub}`);

    expect(getQQBotMediaPath()).toBe(path.join(expectedHome, ".oriro", "media", "qqbot"));

    const mediaFile = path.join(expectedHome, ".oriro", "media", "qqbot", "tilde.png");
    fs.mkdirSync(path.dirname(mediaFile), { recursive: true });
    fs.writeFileSync(mediaFile, "image", "utf8");
  ...
  it("falls back to OS home when ORIRO_HOME is unset (no regression)", () => {
    vi.stubEnv("ORIRO_HOME", "");

    expect(getQQBotMediaPath()).toBe(path.join(realOsHome, ".oriro", "media", "qqbot"));
  });

  it("treats sentinel strings 'undefined' and 'null' as unset", () => {
    for (const sentinel of ["undefined", "null"]) {
      vi.stubEnv("ORIRO_HOME", sentinel);
      expect(getQQBotMediaPath()).toBe(path.join(realOsHome, ".oriro", "media", "qqbot"));
    }
  });
  ...
  it("keeps persisted QQ Bot data anchored on the OS home (compatibility)", () => {
    ...
    // operator adds ORIRO_HOME — otherwise existing deployments would lose
    // their session state. Only the media root follows ORIRO_HOME.
    expect(getQQBotDataPath()).toBe(path.join(realOsHome, ".oriro", "qqbot"));
  });

  it("rejects files that live under HOME tree when ORIRO_HOME is the active root", () => {
    const fakeOriroHome = makeFakeOriroHome();
    vi.stubEnv("ORIRO_HOME", fakeOriroHome);

    // File under the HOME-side mirror — exactly the path that *worked* on
    // current main and *broke* the ORIRO_HOME setup. After the fix the
    // active media root is ORIRO_HOME, so a file under HOME is no longer
    ...
    const homeOnlyFile = path.join(realOsHome, ".oriro", "media", "qqbot", stale);
  ...
  it("remaps workspace paths under either HOME or ORIRO_HOME to the ORIRO_HOME media root", () => {
    const fakeOriroHome = makeFakeOriroHome();
    vi.stubEnv("ORIRO_HOME", fakeOriroHome);

    const baseName = `remap-${process.pid}-${Date.now()}`;

    // Real file lives under the ORIRO_HOME media tree.
    const mediaFile = path.join(
      fakeOriroHome,
      ".oriro",
      "media",
    ...
    const homeWorkspaceDir = path.join(realOsHome, ".oriro", "workspace", "qqbot");
    ...
    // HOME-side path, so nothing to clean up there beyond the ORIRO_HOME tree.
    expect(resolveQQBotLocalMediaPath(homeWorkspacePath)).toBe(mediaFile);

    // Same path but under ORIRO_HOME should also remap.
    ...
    expect(resolveQQBotLocalMediaPath(oriroWorkspacePath)).toBe(mediaFile);
  });
});
