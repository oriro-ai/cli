...
import { resolveChannelConfigWrites } from "oriro/plugin-sdk/channel-config-writes";
import { normalizeAccountId, resolveAgentRoute } from "oriro/plugin-sdk/routing";
...
class DynamicAgentMutationSkipped extends Error {
  constructor(readonly cfg: OriroConfig) {
...
export async function maybeCreateDynamicAgent(params: {
  ...
  accountId: string;
  senderOpenId: string;
  canCreateForConfig: (cfg: OriroConfig) => Promise<boolean>;
  log: (msg: string) => void;
  ...
  const currentCfg = runtime.config.current() as OriroConfig;
  ...
  let skippedCfg: OriroConfig | undefined;
      ...
      mutate: async (draft) => {
        ...
        if (!agentExists) {
          const workspaceTemplate =
            dynamicCfg.workspaceTemplate ?? "~/.oriro/workspace-{agentId}";
          const agentDirTemplate =
            dynamicCfg.agentDirTemplate ?? "~/.oriro/agents/{agentId}/agent";
  ...
  return {
    ...
    updatedCfg: runtime.config.current() as OriroConfig,
