...
read_when:
  - You see the ORIRO_PLUGIN_SDK_COMPAT_DEPRECATED warning
  - You see the ORIRO_EXTENSION_API_DEPRECATED warning
  - You used api.registerEmbeddedExtensionFactory before Oriro 2026.4.25
  - You are updating a plugin to the modern plugin architecture
  - You maintain an external Oriro plugin
---

Oriro has moved from a broad backwards-compatibility layer to a modern plugin
...
- **`oriro/plugin-sdk/compat`** - a single import that re-exported dozens of
  helpers. It was introduced to keep older hook-based plugins working while the
  new plugin architecture was being built.
- **`oriro/plugin-sdk/infra-runtime`** - a broad runtime helper barrel that
...
- **`oriro/plugin-sdk/config-runtime`** - a broad config compatibility barrel
  that still carries deprecated direct load/write helpers during the migration
  window.
- **`oriro/extension-api`** - a bridge that gave plugins direct access to
...
Oriro does not remove or reinterpret documented plugin behavior in the same
...
The modern plugin SDK fixes this: each import path (`oriro/plugin-sdk/\<subpath\>`)
...
`oriro/plugin-sdk/realtime-voice`. The new controller owns the common Talk
...
`talk.providers` should be repaired with `oriro doctor --fix`; runtime Talk
...
<Steps>
  <Step title="Migrate runtime config load/write helpers">
    ...
    `oriro/plugin-sdk/config-runtime` compatibility barrel. Use the narrow
    SDK subpath that matches the job:

    | Need | Import |
    | --- | --- |
    | Config types such as `OriroConfig` | `oriro/plugin-sdk/config-contracts` |
    | Already-loaded config assertions and plugin-entry config lookup | `oriro/plugin-sdk/plugin-config-runtime` |
    | Current runtime snapshot reads | `oriro/plugin-sdk/runtime-config-snapshot` |
    | Config writes | `oriro/plugin-sdk/config-mutation` |
    | Session store helpers | `oriro/plugin-sdk/session-store-runtime` |
    | Markdown table config | `oriro/plugin-sdk/markdown-table-runtime` |
    | Group policy runtime helpers | `oriro/plugin-sdk/runtime-group-policy` |
    | Secret input resolution | `oriro/plugin-sdk/secret-input-runtime` |
    | Model/session overrides | `oriro/plugin-sdk/model-session-runtime` |
  ...
  <Step title="Migrate embedded tool-result extensions to middleware">
    ...
    ```typescript
    // Oriro and Codex runtime dynamic tools
    api.registerAgentToolResultMiddleware(async (event) => {
      return compactToolResult(event);
    }, {
      runtimes: ["oriro", "codex"],
    });
    ```
  ...
  <Step title="Migrate approval-native handlers to capability facts">
    ...
    - Register channel-owned runtime objects such as clients, tokens, or Bolt
      apps through `oriro/plugin-sdk/channel-runtime-context`
  ...
  <Step title="Audit Windows wrapper fallback behavior">
    If your plugin uses `oriro/plugin-sdk/windows-spawn`, unresolved Windows
  ...
  <Step title="Find deprecated imports">
    ...
    grep -r "oriro/extension-api" my-plugin/
    ```
  ...
  <Step title="Replace with focused imports">
    ...
    } from "oriro/plugin-sdk/compat";

    // After (modern focused imports)
    import { createChannelReplyPipeline } from "oriro/plugin-sdk/channel-reply-pipeline";
    import { createPluginRuntimeStore } from "oriro/plugin-sdk/runtime-store";
    import { resolveControlCommandGate } from "oriro/plugin-sdk/command-auth";
    ```
    ...
    import { runEmbeddedAgent } from "oriro/extension-api";
  ...
  <Step title="Replace broad infra-runtime imports">
    `oriro/plugin-sdk/infra-runtime` still exists for external
    compatibility, but new code should import the focused helper surface it
    actually needs:

    | Need | Import |
    | --- | --- |
    | System event queue helpers | `oriro/plugin-sdk/system-event-runtime` |
    | Heartbeat wake, event, and visibility helpers | `oriro/plugin-sdk/heartbeat-runtime` |
    | Pending delivery queue drain | `oriro/plugin-sdk/delivery-queue-runtime` |
    | Channel activity telemetry | `oriro/plugin-sdk/channel-activity-runtime` |
    | In-memory dedupe caches | `oriro/plugin-sdk/dedupe-runtime` |
    | Safe local-file/media path helpers | `oriro/plugin-sdk/file-access-runtime` |
    | Dispatcher-aware fetch | `oriro/plugin-sdk/runtime-fetch` |
    | Proxy and guarded fetch helpers | `oriro/plugin-sdk/fetch-runtime` |
    | SSRF dispatcher policy types | `oriro/plugin-sdk/ssrf-dispatcher` |
    | Approval request/resolution types | `oriro/plugin-sdk/approval-runtime` |
    | Approval reply payload and command helpers | `oriro/plugin-sdk/approval-reply-runtime` |
    | Error formatting helpers | `oriro/plugin-sdk/error-runtime` |
    | Transport readiness waits | `oriro/plugin-sdk/transport-ready-runtime` |
    | Secure token helpers | `oriro/plugin-sdk/secure-random-runtime` |
    | Bounded async task concurrency | `oriro/plugin-sdk/concurrency-runtime` |
    | Numeric coercion | `oriro/plugin-sdk/number-runtime` |
    | Process-local async lock | `oriro/plugin-sdk/async-lock-runtime` |
    | File locks | `oriro/plugin-sdk/file-lock` |
  ...
  <Step title="Migrate channel route helpers">
    New channel route code should use `oriro/plugin-sdk/channel-route`.
...
<Accordion title="Common import path table">
  ...
  | `plugin-sdk/config-schema` | Root config schema export | `OriroSchema` |
  ...
  | `plugin-sdk/bundled-channel-config-schema` | Bundled config schemas | Oriro-maintained bundled plugins only; new plugins must define plugin-local schemas |
...
`@oriro/discord@2026.3.13` package. Owner-specific helpers live inside the
...
<AccordionGroup>
  <Accordion title="command-auth help builders → command-status">
    **Old (`oriro/plugin-sdk/command-auth`)**: `buildCommandsMessage`,
    `buildCommandsMessagePaginated`, `buildHelpMessage`.

    **New (`oriro/plugin-sdk/command-status`)**: same signatures, same
    exports - just imported from the narrower subpath. `command-auth`
    re-exports them as compat stubs.

    ```typescript
    // Before
    import { buildHelpMessage } from "oriro/plugin-sdk/command-auth";

    // After
    import { buildHelpMessage } from "oriro/plugin-sdk/command-status";
    ```
  ...
  <Accordion title="Mention gating helpers → resolveInboundMentionDecision">
    **Old**: `resolveInboundMentionRequirement({ facts, policy })` and
    ...
    `oriro/plugin-sdk/channel-inbound` or
    `oriro/plugin-sdk/channel-mention-gating`.
  ...
  <Accordion title="Channel runtime shim and channel actions helpers">
    `oriro/plugin-sdk/channel-runtime` is a compatibility shim for older
    channel plugins. Do not import it from new code; use
    `oriro/plugin-sdk/channel-runtime-context` for registering runtime
    objects.

    `channelActions*` helpers in `oriro/plugin-sdk/channel-actions` are
  ...
  <Accordion title="Web search provider tool() helper → createTool() on the plugin">
    **Old**: `tool()` factory from `oriro/plugin-sdk/provider-web-search`.

    **New**: implement `createTool(...)` directly on the provider plugin.
    Oriro no longer needs the SDK helper to register the tool wrapper.
  ...
  <Accordion title="Thinking policy hooks → resolveThinkingProfile">
    ...
    **New**: a single `resolveThinkingProfile(ctx)` that returns a
    ...
    ranked level list. Oriro downgrades stale stored values by profile
    rank automatically.
  ...
  <Accordion title="OriroSchemaType alias → OriroConfig">
    `OriroSchemaType` re-exported from `oriro/plugin-sdk` is now a
    one-line alias for `OriroConfig`. Prefer the canonical name.

    ```typescript
    // Before
    import type { OriroSchemaType } from "oriro/plugin-sdk";
    // After
    import type { OriroConfig } from "oriro/plugin-sdk/config-schema";
    ```
...
```bash
ORIRO_SUPPRESS_PLUGIN_SDK_COMPAT_WARNING=1 oriro gateway run
ORIRO_SUPPRESS_EXTENSION_API_WARNING=1 oriro gateway run
```
