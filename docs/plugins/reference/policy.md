...
The Policy plugin contributes doctor health checks for policy-managed Oriro
...
posture, ingress/channel access posture, data-handling posture, and Oriro config secret
provider/auth profile posture.

Policy stores authored requirements in `policy.jsonc`, observes existing
Oriro settings and workspace declarations as evidence, and reports drift
through `oriro policy check` and `oriro doctor --lint`. A clean policy
check emits policy, evidence, findings, and attestation hashes that operators
can record for audit.

`oriro policy compare --baseline <file>` compares one policy file to another
