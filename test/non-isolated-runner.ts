...
const SHARED_TEST_SETUP = Symbol.for("oriro.sharedTestSetup");
const EMBEDDED_RUN_STATE = Symbol.for("oriro.embeddedRunState");
const REPLY_RUN_REGISTRY = Symbol.for("oriro.replyRunRegistry");
...
function getSharedTestHome(): string | undefined {
  ...
  return globalState[SHARED_TEST_SETUP]?.tempHome ?? process.env.ORIRO_TEST_HOME;
}
...
function restoreSharedTestHomeAfterEnvUnstub(testHomeRaw: string | undefined): void {
  ...
  process.env.ORIRO_TEST_HOME = testHome;
...
function resetOriroGlobalRunState(): void {
...
export default class OriroNonIsolatedRunner extends TestRunner {
  override onCollectStart(file: { filepath: string }) {
    super.onCollectStart(file);
    restoreRealTimers();
    restoreNativeTimerGlobals();
    restoreSharedTestHomeAfterEnvUnstub(getSharedTestHome());
    const orderLogPath = process.env.ORIRO_VITEST_FILE_ORDER_LOG?.trim();
    if (orderLogPath) {
