...
function createManifestPlugin(id: string): PluginManifestRecord {
  return {
    ...
    rootDir: `/tmp/${id}`,
    source: "bundled",
    manifestPath: `/tmp/${id}/oriro.plugin.json`,
...
describe("resolvePluginDiscoveryProvidersRuntime", () => {
  ...
  it("clears bundled dist discovery chunks from the dist root", () => {
    ...
    mocks.loadPluginMetadataSnapshot.mockReturnValue({
      ...
      manifestRegistry: {
        plugins: [
          {
            ...createManifestPlugin("deepseek"),
            rootDir: "/tmp/oriro/dist/extensions/deepseek",
            manifestPath: "/tmp/oriro/dist/extensions/deepseek/oriro.plugin.json",
            providerDiscoverySource: "/tmp/oriro/dist/extensions/deepseek/provider-discovery.js",
          },
        ],
        diagnostics: [],
      },
    ...
    expect(mocks.clearNativeRequireJavaScriptModuleCache).toHaveBeenCalledWith(
      "/tmp/oriro/dist/extensions/deepseek/provider-discovery.js",
      { dependencyRoot: "/tmp/oriro/dist" },
    );
  });
