...
export function createColdPluginFixture(options: ColdPluginFixtureOptions): ColdPluginFixture {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(
      {
        name: options.packageName ?? "@example/oriro-cold-control-plane",
  ...
  fs.writeFileSync(
    path.join(options.rootDir, "oriro.plugin.json"),
    JSON.stringify(
      {
        id: pluginId,
...
export function createColdPluginConfig(pluginDir: string, pluginId: string): OriroConfig {
  return {
    plugins: {
...
export function createColdPluginHermeticEnv(
  ...
  return {
    ...process.env,
    ORIRO_HOME: path.join(homeDir, "home"),
    ORIRO_BUNDLED_PLUGINS_DIR: options.bundledPluginsDir,
    ORIRO_DISABLE_PERSISTED_PLUGIN_REGISTRY:
