...
import type { ResolvedTtsConfig, SpeechProviderPlugin } from "oriro/plugin-sdk/speech-core";
import { withEnv, withEnvAsync } from "oriro/plugin-sdk/test-env";
...
type TtsRuntimeModule = typeof import("oriro/plugin-sdk/tts-runtime");
type TtsCoreModule = typeof import("oriro/plugin-sdk/speech-core");
...
let completeSimple: typeof import("oriro/plugin-sdk/llm").completeSimple;
...
function asLegacyTtsConfig(value: unknown): OriroConfig {
  return value as OriroConfig;
}

function asLegacyOriroConfig(value: Record<string, unknown>): OriroConfig {
  return value as unknown as OriroConfig;
}
...
function createOpenAiTelephonyCfg(model: "tts-1" | "gpt-4o-mini-tts"): OriroConfig {
...
async function loadTtsCore(): Promise<TtsCoreModule> {
  ttsCorePromise ??= import("oriro/plugin-sdk/speech-core");
...
function createResolvedSummarizationConfig(cfg: OriroConfig): ResolvedTtsConfig {
  const rawConfig =
...
async function setupSummarizationMocks() {
  ...
  ({ completeSimple } = await import("oriro/plugin-sdk/llm"));
...
export function describeTtsAutoApplyContract() {
  describe("tts auto-apply contract", () => {
    ...
    const baseCfg: OriroConfig = asLegacyOriroConfig({
      agents: { defaults: { model: { primary: "openai/gpt-4o-mini" } } },
      messages: {
        tts: {
          auto: "inbound",
    ...
    const withMockedAutoTtsFetch = async (
      ...
      const prevPrefs = process.env.ORIRO_TTS_PREFS;
      process.env.ORIRO_TTS_PREFS = `/tmp/tts-test-${Date.now()}.json`;
      try {
        await withMockedSpeechFetch(run, 1);
      } finally {
        process.env.ORIRO_TTS_PREFS = prevPrefs;
      }
    };

    const taggedCfg: OriroConfig = {
      ...baseCfg,
      messages: {
        ...baseCfg.messages!,
