...
export function isPublishablePluginPackage(packageJson) {
  return (
    packageJson.oriro?.release?.publishToNpm === true ||
    packageJson.oriro?.release?.publishToOriroHub === true
  );
}
...
function createNeverBundleDependencyMatcher(packageJson) {
  ...
  return (id) => {
    if (id === "oriro" || id.startsWith("oriro/")) {
      return true;
    }
...
export function resolvePluginNpmRuntimePackageFiles(plan) {
  ...
  merged.add("dist/**");
  if (packageRelativePathExists(plan.packageDir, "oriro.plugin.json")) {
    merged.add("oriro.plugin.json");
  }
...
function normalizeOriroPeerRange(value) {
...
function resolveOriroPeerRange(packageJson, rootPackageJson) {
  return (
    normalizeOriroPeerRange(packageJson.oriro?.compat?.pluginApi) ||
    normalizeOriroPeerRange(packageJson.peerDependencies?.oriro) ||
    normalizeOriroPeerRange(packageJson.oriro?.build?.oriroVersion) ||
    normalizeOriroPeerRange(rootPackageJson?.version) ||
    normalizeOriroPeerRange(packageJson.version)
  );
}

/** Resolve package peer dependency metadata for the Oriro plugin API. */
export function resolvePluginNpmRuntimePackagePeerMetadata(plan) {
  const oriroPeerRange = resolveOriroPeerRange(plan.packageJson, plan.rootPackageJson);
  if (!oriroPeerRange) {
    throw new Error(
      `cannot infer oriro peerDependency range for ${plan.pluginDir}; set oriro.compat.pluginApi or package version`,
    );
  }
  ...
  const existingOriroMeta = getRecord(existingPeerDependenciesMeta.oriro);
  return {
    peerDependencies: {
      ...existingPeerDependencies,
      oriro: oriroPeerRange,
    },
...
export function resolvePluginNpmRuntimeBuildPlan(params) {
  ...
  const plan = {
    ...
    runtimeExtensions: (Array.isArray(packageJson.oriro?.extensions)
      ? packageJson.oriro.extensions
      : []
    )
      .map(normalizePackageEntry)
      .filter(Boolean)
      .map(toPackageRuntimeEntry),
    runtimeSetupEntry: normalizePackageEntry(packageJson.oriro?.setupEntry)
      ? toPackageRuntimeEntry(packageJson.oriro.setupEntry)
      : undefined,
  };
  return {
    ...plan,
