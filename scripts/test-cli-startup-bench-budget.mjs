...
if (process.argv.slice(2).includes("--help")) {
  console.log(
    [
      ...
      "ORIRO_STARTUP_BENCH_ENFORCE_NONCANONICAL_ARCH=1 to force them.",
  ...
  opts = parseFlagArgs(
    process.argv.slice(2),
    {
      ...
      maxDurationRegressionPct:
        readBudgetEnvNumber("ORIRO_STARTUP_BENCH_MAX_DURATION_REGRESSION_PCT") ?? 20,
      maxFirstOutputRegressionPct:
        readBudgetEnvNumber("ORIRO_STARTUP_BENCH_MAX_FIRST_OUTPUT_REGRESSION_PCT") ?? 20,
      maxRssRegressionPct:
        readBudgetEnvNumber("ORIRO_STARTUP_BENCH_MAX_RSS_REGRESSION_PCT") ?? 20,
      skipBaseline: false,
...
const shouldAutoSkipNonCanonicalBaselineChecks =
  process.arch !== "x64" && process.env.ORIRO_STARTUP_BENCH_ENFORCE_NONCANONICAL_ARCH !== "1";
if (shouldAutoSkipNonCanonicalBaselineChecks && !opts.skipBaseline) {
  console.warn(
    `[test-cli-startup-bench-budget] skipping x64 startup fixture budgets on ${process.arch}; response contracts and sample output validation still ran. Set ORIRO_STARTUP_BENCH_ENFORCE_NONCANONICAL_ARCH=1 to force fixture checks.`,
  );
  opts = {
    ...opts,
    skipBaseline: true,
