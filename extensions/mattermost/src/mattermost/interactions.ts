...
const SIGNED_CHANNEL_ID_CONTEXT_KEY = "__oriro_channel_id";
...
type InteractionCallbackConfig = Pick<OriroConfig, "gateway" | "channels"> & {
  interactions?: {
...
function deriveInteractionSecret(botToken: string): string {
  return createHmac("sha256", "oriro-mattermost-interactions").update(botToken).digest("hex");
}
