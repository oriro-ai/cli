...
import { OriroStdioClientTransport } from "./mcp-stdio-transport.js";
...
describe("OriroStdioClientTransport", () => {
  afterEach(() => {
    vi.useRealTimers();
    spawnMock.mockReset();
  ...
  it("starts stdio MCP servers in a disposable process group on POSIX", async () => {
    // Detached POSIX process groups let Oriro clean up child tool servers
    // without relying on shell-specific process trees.
    const child = new MockChildProcess();
    spawnMock.mockReturnValue(child);

    const transport = new OriroStdioClientTransport({
      command: "npx",
      args: ["-y", "example-mcp"],
      env: { EXAMPLE: "1" },
      cwd: "/tmp/example",
  ...
  it("kills the process tree when graceful stdio close does not exit", async () => {
    ...
    const transport = new OriroStdioClientTransport({ command: "npx" });
    const started = transport.start();
    child.emit("spawn");
    await started;
