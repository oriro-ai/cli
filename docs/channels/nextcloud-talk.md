...
Nextcloud Talk ships as a bundled plugin in current Oriro releases, so
...
```bash
oriro plugins install @oriro/nextcloud-talk
```
...
```bash
oriro plugins install ./path/to/local/nextcloud-talk-plugin
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Oriro" "<shared-secret>" "<webhook-url>" --feature webhook --feature response --feature reaction
   ```
...
4. Configure Oriro:
   ...
   ```bash
   oriro channels add --channel nextcloud-talk \
...
- Approve via:
  - `oriro pairing list nextcloud-talk`
  - `oriro pairing approve nextcloud-talk <CODE>`
