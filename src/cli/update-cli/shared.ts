...
export function normalizeTag(value?: string | null): string | null {
  return normalizePackageTagInput(value, ["oriro", DEFAULT_PACKAGE_NAME]);
}
...
export function resolveGitInstallDir(): string {
  const override = process.env.ORIRO_GIT_DIR?.trim();
  if (override) {
...
function resolveDefaultGitDir(): string {
  ...
  if (home.startsWith("/")) {
    return path.posix.join(home, "oriro");
  }
...
/** Locate the installed Oriro package root that should receive update operations. */
export async function resolveUpdateRoot(): Promise<string> {
  return (
    (await resolveOriroPackageRoot({
...
/** Ensure the configured source-update directory exists and points at an Oriro checkout. */
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    ...
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", ORIRO_REPO_URL, params.dir],
      env: gitEnv,
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `ORIRO_GIT_DIR points at a non-git directory: ${params.dir}. Set ORIRO_GIT_DIR to an empty folder or an oriro checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`ORIRO_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }

  return null;
}

/** Detect the package manager that owns a global/package Oriro install. */
...
const COMPLETION_CACHE_MANUAL_REFRESH_HINT =
  "Shell tab-completion may be stale; refresh manually with: oriro completion --write-state";

/** Best-effort refresh of shell completion state after a successful update. */
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "oriro.mjs");
