...
} from "@oriro/whatsapp/api.js";
import { normalizeE164 } from "oriro/plugin-sdk/account-resolution";
...
import { normalizeStringEntries, uniqueStrings } from "oriro/plugin-sdk/string-coerce-runtime";
...
const WHATSAPP_QA_CAPTURE_CONTENT_ENV = "ORIRO_QA_WHATSAPP_CAPTURE_CONTENT";
...
function buildWhatsAppQuoteReplyRun(target: "dm" | "group"): WhatsAppQaMessageScenarioRun {
  ...
  const input =
    target === "group"
      ? `oriroqa reply with only this exact marker: ${token}`
...
const WHATSAPP_QA_SCENARIOS: WhatsAppQaScenarioDefinition[] = [
  ...
  },
  {
    ...
    buildRun: () => ({
      ...
      matchText: /Oriro: access not configured|Pairing code:/iu,
      target: "dm",
    }),
  },
  {
    ...
    buildRun: () => {
      ...
      return {
        configMode: "allowlist",
        expectReply: true,
        input: `oriroqa reply with only this exact marker: ${replyToken}`,
  ...
  },
  {
    ...
    buildRun: () => ({
      ...
      expectReply: true,
      input: "/help",
      matchText: /Oriro|commands|status|\/new/iu,
      target: "dm",
    }),
  },
  {
    ...
    buildRun: () => ({
      ...
      expectReply: true,
      input: "/status",
      matchText: /Oriro|status|session|agent/iu,
      target: "dm",
    }),
  },
  {
  ...
  },
  {
    ...
    buildRun: () => {
      ...
      return {
        configMode: "allowlist",
        expectReply: true,
        input: `oriroqa reply with only this exact marker under groupPolicy open: ${token}`,
        matchText: token,
        target: "group",
      };
    },
  },
  {
    ...
    buildRun: () => {
      ...
      return {
        configMode: "allowlist",
        expectReply: false,
        input: `oriroqa groupPolicy disabled must not reply with ${token}`,
        matchText: token,
        target: "group",
      };
    },
  ...
  },
  {
    ...
    buildRun: () => {
      ...
      return {
        configMode: "allowlist",
        expectReply: false,
        input: `oriroqa blocked group should not reply with ${quietToken}`,
...
function resolveWhatsAppQaRuntimeEnv(env: NodeJS.ProcessEnv = process.env): WhatsAppQaRuntimeEnv {
  return validateWhatsAppQaRuntimeEnv(
    {
      driverPhoneE164: resolveEnvValue(env, "ORIRO_QA_WHATSAPP_DRIVER_PHONE_E164"),
      sutPhoneE164: resolveEnvValue(env, "ORIRO_QA_WHATSAPP_SUT_PHONE_E164"),
      driverAuthArchiveBase64: resolveEnvValue(
        env,
        "ORIRO_QA_WHATSAPP_DRIVER_AUTH_ARCHIVE_BASE64",
      ),
      sutAuthArchiveBase64: resolveEnvValue(env, "ORIRO_QA_WHATSAPP_SUT_AUTH_ARCHIVE_BASE64"),
      groupJid: env.ORIRO_QA_WHATSAPP_GROUP_JID?.trim() || undefined,
    },
    "ORIRO_QA_WHATSAPP",
  );
}
...
async function writeWhatsAppQaWorkspaceFixture(
  ...
  const fixtureDir = path.join(context.gatewayWorkspaceDir, ".oriro", "qa-whatsapp-media");
...
export async function runWhatsAppQaLive(params: {
  ...
  try {
    ...
    tempAuthRoot = await fs.mkdtemp(
      path.join(resolvePreferredOriroTmpDir(), "oriro-whatsapp-qa-"),
    );
