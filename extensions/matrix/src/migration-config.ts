...
import { DEFAULT_ACCOUNT_ID, normalizeAccountId } from "oriro/plugin-sdk/account-id";
  ...
  if (!channel) {
    return {
      warning:
        ...
        'Configure Matrix, then rerun "oriro doctor --fix" or restart the gateway.',
    };
  }
  if (requiresExplicitMatrixDefaultAccount(params.cfg)) {
    return {
      warning:
        ...
        'Set "channels.matrix.defaultAccount" to the intended target account before rerunning "oriro doctor --fix" or restarting the gateway.',
    };
  }
  ...
  if (!target) {
    ...
    return {
      warning:
        ...
        'Start the gateway once with a working Matrix login, or rerun "oriro doctor --fix" after cached credentials are available.',
    };
  }
