...
const COMPILE_CACHE_DISABLED_RESPAWNED_ENV = "ORIRO_COMPILE_CACHE_DISABLED_RESPAWNED";
...
export function shouldEnableOriroCompileCache(params: {
...
export function resolveOriroCompileCacheDirectory(params: {
  env?: NodeJS.ProcessEnv;
  installRoot: string;
}): string {
...
type OriroCompileCacheRespawnPlan = {
  command: string;
  args: string[];
  env: NodeJS.ProcessEnv;
...
type OriroCompileCacheRespawnRuntime = RespawnChildRuntime & {
  writeError: (message: string) => void;
};

export function buildOriroCompileCacheRespawnPlan(params: {
  currentFile: string;
  env?: NodeJS.ProcessEnv;
  execArgv?: string[];
  execPath?: string;
  ...
  nodeVersion?: string;
  platform?: NodeJS.Platform;
}): OriroCompileCacheRespawnPlan | undefined {
  const env = params.env ?? process.env;
...
export function respawnWithoutOriroCompileCacheIfNeeded(params: {
  currentFile: string;
  installRoot: string;
}): boolean {
  ...
  runOriroCompileCacheRespawnPlan(plan);
  return true;
}

export function runOriroCompileCacheRespawnPlan(
  plan: OriroCompileCacheRespawnPlan,
  runtime: OriroCompileCacheRespawnRuntime = {
    spawn,
  ...
  return runRespawnChildWithSignalBridge({
    ...
    onError: (error) => {
      runtime.writeError(
        `[oriro] Failed to respawn CLI without compile cache: ${
...
export function enableOriroCompileCache(params: {
  env?: NodeJS.ProcessEnv;
  installRoot: string;
}): void {
  if (!shouldEnableOriroCompileCache(params)) {
    return;
  }
  try {
    enableCompileCache(resolveOriroCompileCacheDirectory(params));
  } catch {
