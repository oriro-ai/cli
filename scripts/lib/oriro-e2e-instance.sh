...
oriro_e2e_read_positive_int_env() {
...
oriro_e2e_read_nonnegative_int_env() {
  local name="${1:?missing environment variable name}"
  local fallback="${2:?missing fallback value}"
  local value="${!name-}"
  if [ -z "${!name+x}" ]; then
...
const killGraceMs = Number.parseInt(
  process.env.ORIRO_E2E_TIMEOUT_KILL_GRACE_MS || "30000",
  10,
);
...
oriro_e2e_print_log() {
  ...
  max_bytes="$(oriro_e2e_read_nonnegative_int_env ORIRO_E2E_LOG_TAIL_BYTES 262144)" || return $?
  max_lines="$(oriro_e2e_read_nonnegative_int_env ORIRO_E2E_LOG_TAIL_LINES 120)" || return $?
  [ -f "$path" ] || return 0
  echo "--- $path ---"
...
oriro_e2e_install_package() {
  ...
  set +e
  oriro_e2e_maybe_timeout "$timeout_value" npm install "${args[@]}" "$package_tgz" --no-fund --no-audit >"$log_file" 2>&1
...
oriro_e2e_assert_package_extensions() {
  local root="$1"
  shift
  local extension
  for extension in "$@"; do
...
oriro_e2e_write_state_env() {
  local target="${1:-/tmp/oriro-test-state-env}"
  {
    printf 'export HOME=%q\n' "$HOME"
    printf 'export ORIRO_HOME=%q\n' "$ORIRO_HOME"
    printf 'export ORIRO_STATE_DIR=%q\n' "$ORIRO_STATE_DIR"
    printf 'export ORIRO_CONFIG_PATH=%q\n' "$ORIRO_CONFIG_PATH"
    printf 'export ORIRO_AGENT_DIR=%q\n' "${ORIRO_AGENT_DIR-}"
  } >"$target"
}
oriro_e2e_install_trash_shim() {
  local shim_dir="${ORIRO_E2E_BIN_DIR:-}"
  if [ -z "$shim_dir" ]; then
    if [ -n "${ORIRO_STATE_DIR:-}" ]; then
      shim_dir="$ORIRO_STATE_DIR/e2e-bin"
    else
      shim_dir="$(mktemp -d "${TMPDIR:-/tmp}/oriro-bin.XXXXXX")"
    fi
    ORIRO_E2E_BIN_DIR="$shim_dir"
    export ORIRO_E2E_BIN_DIR
  fi
  case ":$PATH:" in
    *":$shim_dir:"*) ;;
...
oriro_e2e_run_script_with_pty() {
  local command="$1"
  local log_path="$2"
  local timeout_value="${ORIRO_E2E_COMMAND_TIMEOUT:-300s}"
  if script --version >/dev/null 2>&1; then
    oriro_e2e_maybe_timeout "$timeout_value" script -q -f -c "$command" "$log_path"
  elif node -e 'import("@lydell/node-pty")' >/dev/null 2>&1; then
    oriro_e2e_maybe_timeout "$timeout_value" node scripts/e2e/lib/run-with-pty.mjs "$log_path" /bin/bash -lc "$command"
  else
    oriro_e2e_maybe_timeout "$timeout_value" script -q -F "$log_path" /bin/bash -lc "$command"
  fi
}
...
oriro_e2e_process_alive() {
  local pid="${1:-}"
  [ -n "$pid" ] || return 1
  kill -0 "$pid" >/dev/null 2>&1 || kill -0 -- "-$pid" >/dev/null 2>&1
}
oriro_e2e_stop_process() {
  local pid="${1:-}" _
  [ -n "$pid" ] || return 0
  oriro_e2e_signal_process "$pid" TERM
  for _ in $(seq 1 40); do
    ! oriro_e2e_process_alive "$pid" && { wait "$pid" >/dev/null 2>&1 || true; return 0; }
    sleep 0.25
  done
  oriro_e2e_signal_process "$pid" KILL
  wait "$pid" >/dev/null 2>&1 || true
}
oriro_e2e_terminate_gateways() {
  oriro_e2e_stop_process "${1:-}"
}
oriro_e2e_start_mock_openai() { oriro_e2e_start_tracked_process "$2" env "MOCK_PORT=$1" node scripts/e2e/mock-openai-server.mjs; }
oriro_e2e_wait_mock_openai() {
  local port="$1" attempts="${2:-80}" timeout_ms="${3:-400}" _
  for _ in $(seq 1 "$attempts"); do
    oriro_e2e_probe_http "http://127.0.0.1:${port}/health" ok "$timeout_ms" && return 0
    sleep 0.1
  done
  oriro_e2e_probe_http "http://127.0.0.1:${port}/health" ok "$timeout_ms"
}
oriro_e2e_start_gateway() { oriro_e2e_start_tracked_process "$3" node "$1" gateway --port "$2" --bind loopback --allow-unconfigured; }
oriro_e2e_exec_gateway() { exec node "$1" gateway --port "$2" --bind "${3:-loopback}" --allow-unconfigured >"$4" 2>&1; }
oriro_e2e_gateway_log_port_from_text() {
  sed -nE 's/.*(127\.0\.0\.1|localhost):([0-9]+).*/\2/p' | tail -n 1
}
oriro_e2e_gateway_log_port() {
  grep '\[gateway\] ready' "$1" 2>/dev/null | oriro_e2e_gateway_log_port_from_text
}
oriro_e2e_wait_gateway_ready() {
    ...
    if [ "$saw_ready_log" != "true" ] && [ -f "$log" ]; then
      ...
      if [ "$log_bytes" -gt "$ready_scan_offset" ]; then
        ...
        if [ -n "$ready_log_lines" ]; then
          saw_ready_log=true
          [ -n "$ready_port" ] || ready_port="$(printf "%s" "$ready_log_lines" | oriro_e2e_gateway_log_port_from_text)"
        fi
      fi
    fi
    if [ "$saw_ready_log" = "true" ]; then
      [ "$readiness_mode" = "legacy-ready-log-ok" ] && return 0
      [ -n "$ready_port" ] || ready_port="${ORIRO_E2E_GATEWAY_READY_PORT:-18789}"
      oriro_e2e_probe_http "http://127.0.0.1:${ready_port}/readyz" ok 400 && return 0
    fi
    sleep 0.25
  done
  if [ "$saw_ready_log" = "true" ]; then
...
oriro_e2e_probe_http_status() {
  oriro_e2e_probe_http "$1" "${2:-200}" "${3:-400}"
}
oriro_e2e_assert_file() { [ -f "$1" ] || { echo "Missing file: $1"; exit 1; }; }
oriro_e2e_assert_dir() { [ -d "$1" ] || { echo "Missing dir: $1"; exit 1; }; }
oriro_e2e_assert_log_not_contains() {
  ! grep -q "$2" "$1" || { echo "Unexpected log output: $2"; exit 1; }
}
oriro_e2e_run_logged() {
  local label="$1" log_root="${ORIRO_E2E_LOG_DIR:-${TMPDIR:-/tmp}}" log_path safe_label
  shift
  safe_label="${label//[^A-Za-z0-9_.-]/-}"
  [ -n "$safe_label" ] || safe_label="command"
  mkdir -p "$log_root"
  log_path="$(mktemp "$log_root/oriro-${safe_label}.XXXXXX.log")"
  ORIRO_E2E_LAST_LOG_PATH="$log_path"
  export ORIRO_E2E_LAST_LOG_PATH
  oriro_e2e_run_command "$@" >"$log_path" 2>&1 || { oriro_e2e_print_log "$log_path"; exit 1; }
}
oriro_e2e_run_command() {
  local timeout_value="${ORIRO_E2E_COMMAND_TIMEOUT:-300s}"
  oriro_e2e_maybe_timeout "$timeout_value" "$@"
}
oriro_e2e_enable_oriro_cli_timeout() {
  ...
  oriro() {
    oriro_e2e_run_command "$ORIRO_E2E_CLI_BIN" "$@"
  }
}
oriro_e2e_dump_logs() {
  local path
  for path in "$@"; do
    oriro_e2e_print_log "$path"
  done
}
