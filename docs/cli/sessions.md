---
summary: "CLI reference for `oriro sessions` (list stored sessions + usage)"
read_when:
...
until a message is processed. Use `oriro channels status --probe`,
`oriro status --deep`, or `oriro health --verbose` when you need live
channel connectivity.

`oriro sessions` and Gateway `sessions.list` responses are bounded by
...
```bash
oriro sessions
oriro sessions --agent work
oriro sessions --all-agents
oriro sessions --active 120
oriro sessions --limit 25
oriro sessions --verbose
oriro sessions --json
```
...
```bash
oriro sessions export-trajectory --session-key "agent:main:telegram:direct:123" --workspace .
oriro sessions export-trajectory --session-key "agent:main:telegram:direct:123" --output bug-123 --json
```
...
inside `.oriro/trajectory-exports/` under the selected workspace.

`oriro sessions --all-agents` reads configured agent stores. Gateway and ACP
...
`oriro sessions --all-agents --json`:

```json
{
  "path": null,
  "stores": [
    { "agentId": "main", "path": "/home/user/.oriro/agents/main/sessions/sessions.json" },
    { "agentId": "work", "path": "/home/user/.oriro/agents/work/sessions/sessions.json" }
  ],
  "allAgents": true,
  "count": 2,
  "totalCount": 2,
...
oriro sessions cleanup --agent work --dry-run
oriro sessions cleanup --all-agents --dry-run
oriro sessions cleanup --enforce
oriro sessions cleanup --enforce --active-key "agent:main:telegram:direct:123"
oriro sessions cleanup --dry-run --fix-dm-scope
oriro sessions cleanup --json
```

`oriro sessions cleanup` uses `session.maintenance` settings from config:

- Scope note: `oriro sessions cleanup` maintains session stores, transcripts, and trajectory sidecars. It does not prune cron run history, which is managed by `cron.runLog.keepLines` in [Cron configuration](/automation/cron-jobs#configuration) and explained in [Cron maintenance](/automation/cron-jobs#maintenance).
...
`oriro sessions cleanup --all-agents --dry-run --json`:
...
```json
{
  ...
  "stores": [
    {
      "agentId": "main",
      "storePath": "/home/user/.oriro/agents/main/sessions/sessions.json",
      "beforeCount": 120,
      "afterCount": 80,
      "missing": 0,
      "dmScopeRetired": 0,
    ...
    {
      "agentId": "work",
      "storePath": "/home/user/.oriro/agents/work/sessions/sessions.json",
      "beforeCount": 18,
      "afterCount": 18,
      "missing": 0,
      "dmScopeRetired": 0,
...
Reclaim context budget for a wedged or oversized session. `oriro sessions compact <key>` is the first-class wrapper around the `sessions.compact` gateway RPC and requires a running gateway.

```bash
oriro sessions compact "agent:main:main"
oriro sessions compact "agent:main:main" --max-lines 200
oriro sessions compact "agent:work:main" --agent work --json
```
...
> Note: `oriro agent --message '/compact ...'` is **not** a compaction path. Slash commands from the CLI are rejected by the authorized-sender check; that invocation exits non-zero with guidance pointing here instead of silently no-opping.

### sessions.compact RPC

`oriro gateway call sessions.compact --params '<json>'` accepts:
...
```json
{
  ...
  "key": "agent:main:main",
  "compacted": true,
  "archived": "/home/user/.oriro/agents/main/sessions/transcripts/<id>.jsonl.bak",
  "kept": 200
}
```
