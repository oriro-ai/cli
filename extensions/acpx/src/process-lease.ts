/**
 * Persistent lease store for ACPX wrapper processes. Leases let Oriro attach
...
export const ORIRO_ACPX_LEASE_ID_ENV = "ORIRO_ACPX_LEASE_ID";
/** Environment variable carrying the owning gateway instance id. */
export const ORIRO_GATEWAY_INSTANCE_ID_ENV = "ORIRO_GATEWAY_INSTANCE_ID";
/** CLI argument carrying the ACPX process lease id for platforms without env wrapping. */
export const ORIRO_ACPX_LEASE_ID_ARG = "--oriro-acpx-lease-id";
/** CLI argument carrying the owning gateway instance id. */
export const ORIRO_GATEWAY_INSTANCE_ID_ARG = "--oriro-gateway-instance-id";
...
export function withAcpxLeaseEnvironment(params: {
  ...
  return [
    "env",
    `${ORIRO_ACPX_LEASE_ID_ENV}=${quoteEnvValue(params.leaseId)}`,
    `${ORIRO_GATEWAY_INSTANCE_ID_ENV}=${quoteEnvValue(params.gatewayInstanceId)}`,
