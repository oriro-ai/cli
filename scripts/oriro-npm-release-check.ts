...
// Oriro Npm Release Check script supports Oriro repository automation.
...
const skipPackValidationEnv = "ORIRO_NPM_RELEASE_SKIP_PACK_CHECK";
...
export function resolveNpmReleaseCheckCommandTimeoutMs(
  ...
  return positiveEnvInt(
    "ORIRO_NPM_RELEASE_CHECK_COMMAND_TIMEOUT_MS",
    env,
...
export function collectReleasePackageMetadataErrors(pkg: PackageJson): string[] {
  ...
  if (pkg.name !== "oriro") {
    errors.push(`package.json name must be "oriro"; found "${pkg.name ?? ""}".`);
  }
  ...
  if (pkg.bin?.oriro !== "oriro.mjs") {
    errors.push(
      `package.json bin.oriro must be "oriro.mjs"; found "${pkg.bin?.oriro ?? ""}".`,
    );
  }
...
async function main(): Promise<number> {
  ...
  if (errors.length > 0) {
    for (const error of errors) {
      console.error(`oriro-npm-release-check: ${error}`);
    }
    return 1;
  }
  ...
  console.log(
    `oriro-npm-release-check: validated ${channel} release ${pkg.version} (monthly patch version${skipPackValidation ? "; metadata-only" : ""}).`,
  );
  return 0;
}
