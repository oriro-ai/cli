...
describe("environment docs", () => {
  it("documents the trusted sources for provider credentials", async () => {
    ...
    expect(markdown).toContain(
      "Oriro ignores provider credential environment variables from workspace `.env` files",
    );
    expect(markdown).toContain("~/.oriro/.env");
    expect(markdown).toContain("$ORIRO_STATE_DIR/.env");
    expect(markdown).toContain("The config `env` block");
    expect(markdown).toContain("ORIRO_LOAD_SHELL_ENV=1");
