...
const pluginApiMocks = vi.hoisted(() => ({
  ...
  resolvePreferredOriroTmpDir: vi.fn(() => path.join(os.tmpdir(), "oriro-device-pair-tests")),
...
vi.mock("./api.js", () => {
  return {
    ...
    resolvePreferredOriroTmpDir: pluginApiMocks.resolvePreferredOriroTmpDir,
...
function createApi(params?: {
  ...
  runtime?: OriroPluginApi["runtime"];
  ...
  return createTestPluginApi({
    ...
    runtime: (params?.runtime ?? {}) as OriroPluginApi["runtime"],
...
function registerPairCommand(params?: {
  ...
  let command: OriroPluginCommandDefinition | undefined;
...
describe("device-pair /pair qr", () => {
  beforeEach(async () => {
    ...
    await fs.mkdir(pluginApiMocks.resolvePreferredOriroTmpDir(), { recursive: true });
  });

  afterEach(async () => {
    await fs.rm(pluginApiMocks.resolvePreferredOriroTmpDir(), { recursive: true, force: true });
  });
  ...
  it("returns an inline QR image for webchat surfaces", async () => {
    ...
    expect(text).toContain("Scan this QR code with the Oriro iOS app:");
    ...
    expect(text).not.toContain("![Oriro pairing QR]");
  });
  ...
  it.each([
    ...
    expect(caption).toContain("Scan this QR code with the Oriro iOS app:");
...
describe("device-pair /pair default setup code", () => {
  ...
  it("allows mdns cleartext setup urls", async () => {
    const command = registerPairCommand({
      pluginConfig: {
        publicUrl: "ws://oriro.local:18789",
      },
    });
    const result = await command.handler(
      createCommandContext({
    ...
    expect(pluginApiMocks.issueDeviceBootstrapToken).toHaveBeenCalledTimes(1);
    expect(requireText(result)).toContain("Gateway: ws://oriro.local:18789");
  });
