...
describe("ACP session metadata SQLite store", () => {
  ...
  it("persists ACP metadata in SQLite without writing sessions.json acp blocks", async () => {
    await withTempDir({ prefix: "oriro-acp-meta-" }, async (dir) => {
      const storePath = path.join(dir, "sessions.json");
      const databasePath = path.join(dir, "state", "oriro.sqlite");
  ...
  it("honors ORIRO_STATE_DIR when joining listed SQLite rows to session stores", async () => {
    await withTempDir({ prefix: "oriro-acp-meta-" }, async (dir) => {
      const env = { ...process.env, ORIRO_STATE_DIR: dir } as NodeJS.ProcessEnv;
