...
const ORIRO_UPDATE_LAUNCHD_LABEL_PREFIX = "ai.oriro.update.";
const ORIRO_MANUAL_UPDATE_LAUNCHD_LABEL_PATTERN = /^ai\.oriro\.manual-update\.\d+$/;

export type StaleOriroUpdateLaunchdJob = {
  label: string;
  pid?: number;
  lastExitStatus?: number;
};

function normalizeOriroUpdateLaunchdLabel(label: unknown): string | null {
  if (typeof label !== "string") {
    return null;
  }
  const trimmed = label.trim();
  if (trimmed.startsWith(ORIRO_UPDATE_LAUNCHD_LABEL_PREFIX)) {
    return trimmed;
  }
  // Manual update jobs include a timestamp-like suffix and should be cleaned up
  // without matching arbitrary ai.oriro labels.
  return ORIRO_MANUAL_UPDATE_LAUNCHD_LABEL_PATTERN.test(trimmed) ? trimmed : null;
}

function isCurrentGatewayLaunchdLabel(label: string, env: NodeJS.ProcessEnv): boolean {
  const gatewayProfileLabel = resolveGatewayLaunchAgentLabel(env.ORIRO_PROFILE);
  if (label === gatewayProfileLabel) {
    return true;
  }
  if (
    env.ORIRO_SERVICE_MARKER?.trim() !== GATEWAY_SERVICE_MARKER ||
    env.ORIRO_SERVICE_KIND?.trim() !== GATEWAY_SERVICE_KIND
  ) {
    return false;
  }
  const configuredLabel = env.ORIRO_LAUNCHD_LABEL?.trim();
  return Boolean(configuredLabel && label === configuredLabel);
}

function resolveCurrentOriroUpdateLaunchdJobLabel(
  ...
  for (const label of [
    ...
    env.ORIRO_LAUNCHD_LABEL,
  ]) {
    const normalized = normalizeOriroUpdateLaunchdLabel(label);
    if (normalized) {
...
function resolveLaunchAgentLabel(args?: { env?: Record<string, string | undefined> }): string {
  const envLabel = args?.env?.ORIRO_LAUNCHD_LABEL?.trim();
  if (envLabel) {
    return assertValidLaunchAgentLabel(envLabel);
  }
  return assertValidLaunchAgentLabel(resolveGatewayLaunchAgentLabel(args?.env?.ORIRO_PROFILE));
}
...
function buildLaunchAgentEnvironmentFile(entries: Array<[string, string]>): string {
  return [
    "# Generated by Oriro. Do not edit while the gateway service is installed.",
...
export function parseLaunchctlListOriroUpdateJobs(
  output: string,
): StaleOriroUpdateLaunchdJob[] {
  const jobs: StaleOriroUpdateLaunchdJob[] = [];
  for (const rawLine of output.split(/\r?\n/)) {
    ...
    const label = normalizeOriroUpdateLaunchdLabel(labelParts.join(" "));
    if (!label) {
      continue;
    }
...
export async function findStaleOriroUpdateLaunchdJobs(
  env: NodeJS.ProcessEnv = process.env,
): Promise<StaleOriroUpdateLaunchdJob[]> {
  ...
  return parseLaunchctlListOriroUpdateJobs(result.stdout).filter(
    (job) => !isCurrentGatewayLaunchdLabel(job.label, env),
  );
}

export async function disableOriroUpdateLaunchdJob(label: string): Promise<boolean> {
  const normalizedLabel = normalizeOriroUpdateLaunchdLabel(label);
...
export async function disableCurrentOriroUpdateLaunchdJob(
  env: NodeJS.ProcessEnv = process.env,
): Promise<boolean> {
  const label = resolveCurrentOriroUpdateLaunchdJobLabel(env);
  if (!label) {
    return false;
  }
  return await disableOriroUpdateLaunchdJob(label);
}
...
async function resolveLaunchAgentGatewayPort(env: GatewayServiceEnv): Promise<number | null> {
  ...
  const fromServiceEnv = parseTcpPort(command?.environment?.ORIRO_GATEWAY_PORT ?? "");
  if (fromServiceEnv !== null) {
    return fromServiceEnv;
  }
  return parseTcpPort(env.ORIRO_GATEWAY_PORT ?? "");
}
...
export function formatLaunchAgentGuiSessionError(params: {
  ...
  return [
    ...
    "Headless deployments should use a dedicated logged-in user session or a custom LaunchDaemon (not shipped): https://docs.oriro.ai/gateway",
  ].join("\n");
}
...
export async function stopLaunchAgent({
  ...
  if (!persistDisable) {
    ...
    // `oriro gateway start` re-enables cleanly without a manual `launchctl enable`.
...
async function writeLaunchAgentPlist({
  ...
  const domain = resolveGuiDomain();
  const label = resolveLaunchAgentLabel({ env });
  for (const legacyLabel of resolveLegacyGatewayLaunchAgentLabels(env.ORIRO_PROFILE)) {
...
async function activateLaunchAgent(params: { env: GatewayServiceEnv; plistPath: string }) {
  ...
  await bootstrapLaunchAgentOrThrow({
    ...
    actionHint: "oriro gateway install --force",
  });
}
...
async function ensureLaunchAgentLoadedAfterFailure(params: {
  ...
  try {
    await bootstrapLaunchAgentOrThrow({
      ...
      plistPath: params.plistPath,
      actionHint: "oriro gateway start",
    });
  } catch {
...
export async function restartLaunchAgent({
  ...
  // `oriro gateway restart` is an explicit operator request to bring the
  ...
  if (plistReloadNeeded) {
    ...
    await bootstrapLaunchAgentOrThrow({
      ...
      serviceTarget,
      plistPath,
      actionHint: "oriro gateway restart",
    });
