...
Oriro sources most Go catalog rows from the bundled Oriro model registry and
supplements current upstream rows while the registry catches up. Run
`oriro models list --provider opencode-go` for the current model list.
...
<AccordionGroup>
  <Accordion title="Routing behavior">
    Oriro handles per-model routing automatically when the model ref uses
