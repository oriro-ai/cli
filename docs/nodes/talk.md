...
- `consultThinkingLevel`: optional thinking level override for the full Oriro agent run behind realtime `oriro_agent_consult` calls.
- `consultFastMode`: optional fast-mode override for realtime `oriro_agent_consult` calls.
...
- `realtime.consultRouting`: `provider-direct` preserves the provider's direct reply when it skips `oriro_agent_consult`; `force-agent-consult` makes Gateway relay route finalized user transcripts through Oriro instead.
- `realtime.instructions`: appends provider-facing system instructions to Oriro's built-in realtime prompt. Use it for voice style and tone; Oriro keeps the default `oriro_agent_consult` guidance.
...
- Browser realtime Talk uses `talk.client.toolCall` for `oriro_agent_consult` instead of exposing `chat.send` to provider-owned browser sessions.
...
- macOS local MLX playback uses the bundled `oriro-mlx-tts` helper when present, or an executable on `PATH`. Set `ORIRO_MLX_TTS_BIN` to point at a custom helper binary during development.
