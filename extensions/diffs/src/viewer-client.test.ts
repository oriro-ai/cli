...
const disableAutoStartKey = Symbol.for("oriro.diffs.disableAutoStart");
...
function renderCard(payloadOverride?: string): void {
  ...
  document.body.insertAdjacentHTML(
    ...
    `<section class="oc-diff-card">
      <div data-oriro-diff-host></div>
      <script type="application/json" data-oriro-diff-payload>${payload}</script>
    </section>`,
  );
}
...
describe("hydrateViewer", () => {
  beforeEach(() => {
    ...
    delete document.documentElement.dataset.oriroDiffsError;
    delete document.documentElement.dataset.oriroDiffsReady;
    vi.clearAllMocks();
  });
  ...
  it("continues hydrating later cards when one card throws", async () => {
    ...
    expect(document.documentElement.dataset.oriroDiffsError).toBeUndefined();
    warn.mockRestore();
  });
...
describe("ensureShadowRoot", () => {
  ...
  it("attaches shadow root from template and removes template element", async () => {
    renderCard();
    const host = document.querySelector<HTMLElement>("[data-oriro-diff-host]")!;
