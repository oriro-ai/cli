...
export function registerSecurityCli(program: Command) {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["oriro security audit", "Run a local security audit."],
          [
            "oriro security audit --deep",
            "Include best-effort live Gateway probes and plugin-owned security audit collectors.",
          ],
          ["oriro security audit --deep --token <token>", "Use explicit token for deep probe."],
          [
            "oriro security audit --deep --password <password>",
            "Use explicit password for deep probe.",
          ],
          [
            "oriro security audit --auth password --password <password>",
            "Audit a runtime-only password-mode Gateway secret.",
          ],
          ["oriro security audit --fix", "Apply safe remediations and file-permission fixes."],
          ["oriro security audit --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/security", "docs.oriro.ai/cli/security")}\n`,
    );
  ...
  security
    ...
    .action(async (opts: SecurityAuditOptions) => {
      ...
      const lines: string[] = [];
      lines.push(heading("ORIRO security audit"));
      ...
      lines.push(muted(`Run deeper: ${formatCliCommand("oriro security audit --deep")}`));
      for (const diagnostic of secretDiagnostics) {
        lines.push(muted(`[secrets] ${diagnostic}`));
      }

      if (opts.fix) {
        lines.push(muted(`Fix: ${formatCliCommand("oriro security audit --fix")}`));
        if (!fixResult) {
