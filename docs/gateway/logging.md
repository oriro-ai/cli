...
- Default rolling log file is under `/tmp/oriro/` (one file per day): `oriro-YYYY-MM-DD.log`
...
- The log file path and level can be configured via `~/.oriro-ai/cli.json`:
  - `logging.file`
  - `logging.level`
...
Oriro can mask sensitive tokens before log or transcript output leaves the
...
`oriro gateway` supports a per-gateway style switch:
