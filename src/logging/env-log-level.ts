...
/** Resolves ORIRO_LOG_LEVEL once per value, warning only when the invalid value changes. */
export function resolveEnvLogLevelOverride(): LogLevel | undefined {
  const trimmed = normalizeOptionalString(process.env.ORIRO_LOG_LEVEL) ?? "";
  if (!trimmed) {
  ...
  if (loggingState.invalidEnvLogLevelValue !== trimmed) {
    ...
    process.stderr.write(
      `[oriro] Ignoring invalid ORIRO_LOG_LEVEL="${trimmed}" (allowed: ${ALLOWED_LOG_LEVELS.join("|")}).\n`,
    );
  }
  return undefined;
}
