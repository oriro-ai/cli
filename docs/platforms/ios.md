...
- Network path:
  ...
  - Tailnet via unicast DNS-SD (example domain: `oriro.internal.`), **or**
...
Run `oriro devices list` again before approval.
...
```bash
oriro nodes status
oriro gateway call node.list --params "{}"
```
...
By default, official/TestFlight builds and gateways use the hosted relay at `https://ios-push-relay.oriro.ai`.
...
- `ORIRO_APNS_RELAY_BASE_URL` still works as a temporary env override for the gateway.
- `ORIRO_PUSH_RELAY_BASE_URL` still works as a temporary env override for official/TestFlight iOS builds.
...
- Only genuine Oriro iOS builds distributed through Apple can use the hosted relay.
...
- To allow hosted relay usage only for official/TestFlight Oriro builds.
...
```bash
export ORIRO_APNS_TEAM_ID="TEAMID"
export ORIRO_APNS_KEY_ID="KEYID"
export ORIRO_APNS_PRIVATE_KEY_P8="$(cat /path/to/AuthKey_KEYID.p8)"
```
...
```bash
mkdir -p ~/.oriro/credentials/apns
chmod 700 ~/.oriro/credentials/apns
mv /path/to/AuthKey_KEYID.p8 ~/.oriro/credentials/apns/AuthKey_KEYID.p8
chmod 600 ~/.oriro/credentials/apns/AuthKey_KEYID.p8
export ORIRO_APNS_PRIVATE_KEY_PATH="$HOME/.oriro/credentials/apns/AuthKey_KEYID.p8"
```
...
The iOS app browses `_oriro-gw._tcp` on `local.` and, when configured, the same
...
`oriro.internal.`) and Tailscale split DNS.
...
```bash
oriro nodes invoke --node "iOS Node" --command canvas.navigate --params '{"url":"http://<gateway-host>:18789/__oriro__/canvas/"}'
```

Notes:

- The Gateway canvas host serves `/__oriro__/canvas/` and `/__oriro__/a2ui/`.
...
tools; the iOS app exposes iPhone capabilities through Oriro node commands
such as `canvas.*`, `camera.*`, `screen.*`, `location.*`, and `talk.*`.

Agents can still operate the iOS app through Oriro by invoking node
...
```bash
oriro nodes invoke --node "iOS Node" --command canvas.eval --params '{"javaScript":"(() => { const {ctx} = window.__oriro; ctx.clearRect(0,0,innerWidth,innerHeight); ctx.lineWidth=6; ctx.strokeStyle=\"#ff2d55\"; ctx.beginPath(); ctx.moveTo(40,40); ctx.lineTo(innerWidth-40, innerHeight-40); ctx.stroke(); return \"ok\"; })()"}'
```

```bash
oriro nodes invoke --node "iOS Node" --command canvas.snapshot --params '{"maxWidth":900,"format":"jpeg"}'
```
...
- Pairing prompt never appears: run `oriro devices list` and approve manually.
