...
Designed to be called from workflow engines (for example, Oriro via
`oriro.invoke --each`) without adding new Oriro code per workflow.
...
- Side effects should be handled outside this tool (for example, approvals in
  Oriro) before calling tools that send messages/emails.

## Bundled extension note

This extension depends on Oriro internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Oriro extension (like `oriro`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.oriro/extensions` as a standalone plugin directory.
