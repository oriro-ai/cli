...
An **agent harness** is the low level executor for one prepared Oriro agent
...
runtime and the normal Oriro provider transport is the wrong abstraction.
...
- a model runtime that needs its own resume id in addition to the Oriro
  session transcript
...
Before a harness is selected, Oriro has already resolved:

- provider and model
- runtime auth state
- thinking level and context budget
- the Oriro transcript/session file
...
The prepared attempt also includes `params.runtimePlan`, an Oriro-owned
policy bundle for runtime decisions that must stay shared across Oriro and native
harnesses:
...
Harnesses may use the plan for decisions that need to match Oriro behavior, but
should still treat it as host-owned attempt state. Do not mutate it or use it to
switch providers/models inside a turn.

## Register a harness

**Import:** `oriro/plugin-sdk/agent-harness`

```typescript
import type { AgentHarness } from "oriro/plugin-sdk/agent-harness";
...
Oriro chooses a harness after provider/model resolution:
...
3. `auto` asks registered harnesses if they support the resolved
   provider/model.
4. If no registered harness matches, Oriro uses its embedded runtime.
...
provider/model. Once a plugin harness has claimed a run, Oriro does not
...
`agents.list[].agentRuntime`, and `ORIRO_AGENT_RUNTIME`. `/status` shows the
...
Oriro. The harness then claims that provider in `supports(...)`.
...
- app-server request: Oriro sends the bare model id to Codex and lets the
...
Oriro requires Codex app-server `0.125.0` or newer. The Codex plugin checks
the app-server initialize handshake and blocks older or unversioned servers so
Oriro only runs against the protocol surface it has been tested with. The
`0.125.0` floor includes the native MCP hook payload support that landed in
Codex `0.124.0`, while pinning Oriro to the newer tested stable line.
...
seam is for async tool-result transforms that must run before Oriro or Codex
...
`oriro/plugin-sdk/agent-harness-runtime` when a completed turn produced no
visible assistant text. The helper returns `empty`, `reasoning-only`, or
`planning-only` so Oriro's fallback policy can decide whether to retry on a
different model. `planning-only` requires the harness's explicit `planText`
field; Oriro does not infer it from assistant prose. The helper intentionally
leaves prompt errors, in-flight turns, and intentional silent replies such as
`NO_REPLY` unclassified.

### Native Codex harness mode

The bundled `codex` harness is the native Codex mode for embedded Oriro
...
`oriro doctor --fix`, and legacy `codex/*` model refs remain compatibility
aliases for the native harness.

When this mode runs, Codex owns the native thread id, resume behavior,
compaction, and app-server execution. Oriro still owns the chat channel,
...
By default, Oriro uses `auto` provider/model runtime policy: registered
...
fail hard. This does not block an explicit provider/model `agentRuntime.id: "oriro"`.
...
Keep that binding explicitly associated with the Oriro session, and keep
mirroring user-visible assistant/tool output into the Oriro transcript.

The Oriro transcript remains the compatibility layer for:

- channel-visible session history
- transcript search and indexing
- switching back to the built-in Oriro harness on a later turn
- generic `/new`, `/reset`, and session deletion behavior

If your harness stores a sidecar binding, implement `reset(...)` so Oriro can
clear it when the owning Oriro session is reset.

## Tool and media results

Core constructs the Oriro tool list and passes it into the prepared attempt.
...
on the same delivery path as Oriro-backed runs.
