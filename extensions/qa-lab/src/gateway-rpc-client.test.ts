...
vi.mock("oriro/plugin-sdk/gateway-runtime", () => ({
...
describe("startQaGatewayRpcClient", () => {
  ...
  it("calls the in-process gateway cli helper without mutating process.env", async () => {
    const originalHome = process.env.ORIRO_HOME;
    ...
    try {
      ...
      if (originalHome === undefined) {
        ...
        process.env.ORIRO_HOME = originalHome;
      }
    }

    expect(process.env.ORIRO_HOME).toBe(originalHome);
  });
