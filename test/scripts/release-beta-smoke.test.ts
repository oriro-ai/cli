...
describe("release-beta-smoke", () => {
  ...
  it("rejects malformed positive integer environment limits", () => {
    expect(readPositiveInt(undefined, 60, "ORIRO_RELEASE_BETA_SMOKE_COMMAND_MS")).toBe(60);
    expect(readPositiveInt("", 60, "ORIRO_RELEASE_BETA_SMOKE_COMMAND_MS")).toBe(60);
    expect(readPositiveInt("25", 60, "ORIRO_RELEASE_BETA_SMOKE_COMMAND_MS")).toBe(25);

    for (const raw of ["1e3", "25ms", "1.5", "0", "-1", String(Number.MAX_SAFE_INTEGER + 1)]) {
      expect(() => readPositiveInt(raw, 60, "ORIRO_RELEASE_BETA_SMOKE_COMMAND_MS")).toThrow(
        "ORIRO_RELEASE_BETA_SMOKE_COMMAND_MS must be a positive integer",
      );
    }
  });
  ...
  it("replaces stale Telegram proof placeholders", () => {
    const body = [
      ...
      "",
      "- npm package: https://www.npmjs.com/package/oriro/v/2026.5.20-beta.1",
