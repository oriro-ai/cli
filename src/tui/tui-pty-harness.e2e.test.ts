...
async function writeTuiPtyFixtureScript(dir: string) {
  ...
  await writeFile(
    ...
    `
      ...
      const actionLogPath = process.env.ORIRO_TUI_PTY_LOG_PATH;
      const gatewayStatus = process.env.ORIRO_TUI_PTY_GATEWAY_STATUS ?? "fixture gateway ok";
      ...
      let fastMode = process.env.ORIRO_TUI_PTY_FAST_MODE === "true";
...
async function startTuiFixture(opts: { env?: NodeJS.ProcessEnv } = {}) {
  const tempDir = await mkdtemp(path.join(tmpdir(), "oriro-tui-pty-"));
  ...
  const run = startPty(process.execPath, ["--import", "tsx", scriptPath], {
    ...
    env: {
      ORIRO_THEME: "dark",
      ORIRO_TUI_PTY_LOG_PATH: logPath,
      NO_COLOR: undefined,
      ...opts.env,
    },
