...
function makeOriroDevSourceRoot(): string {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-loader-dev-source-"));
...
describe("resolveRuntimePluginRegistry", () => {
  ...
  it("keeps the full workspace registry warm when scoped cron registries churn", () => {
    ...
    try {
      ...
      const fullRegistry = loadOriroPlugins(loadOptions);

      loadOriroPlugins({ ...loadOptions, onlyPluginIds: ["alpha"] });
      loadOriroPlugins({ ...loadOptions, onlyPluginIds: ["bravo"] });
...
describe("loadOriroPlugins active runtime clearing", () => {
  it("clears plugin-owned global providers before activating a new registry", () => {
    ...
    loadOriroPlugins({ onlyPluginIds: [] });
...
describe("clearPluginRegistryLoadCache", () => {
  ...
  it("invalidates full-workspace load snapshots", () => {
    ...
    const registry = loadOriroPlugins(loadOptions);

    clearPluginRegistryLoadCache();

    expect(loadOriroPlugins(loadOptions)).not.toBe(registry);
  });
});
