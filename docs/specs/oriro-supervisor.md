---
title: Oriro Supervisor
description: Fleet supervision plan for Codex app-server sessions controlled by Oriro.
readWhen:
  - Designing Codex fleet supervision
  - Building Oriro tools that read, steer, or spawn Codex sessions
...
Oriro Supervisor lets one always-on Oriro instance monitor and drive a fleet of Codex sessions without changing the normal Codex user experience. A user can SSH into a host, start Codex, work in the TUI, and still have the supervisor read the session, steer it, interrupt it, spawn related sessions, and accept handoffs. Codex sessions can also call back into Oriro through MCP.

## Product Model

Codex remains the primary work surface. Oriro supervises Codex rather than hiding Codex inside an opaque Oriro subagent.

The Oriro plugin is named `codex-supervisor`. `crabfleet` remains the deployment
...
- Supervisor Oriro: an always-on Oriro agent with tools for fleet state, transcript reads, steering, interruption, spawning, and handoff.

Oriro may use its existing subagent machinery internally, but the external contract is an attachable Codex session with a Codex thread id.
...
- MCP server for Codex-to-Oriro calls.
- Oriro tools for Oriro-to-Codex control.
...
Every supervised Codex gets an MCP server named `oriro-codex-supervisor`.
...
- `oriro_report_progress`: publish current task state to the supervisor.
- `oriro_ask`: ask the supervisor for help or delegation.
...
## Oriro Control Surface

The always-on Oriro gets the same primitives as internal tools:
...
6. Supervisor Oriro can read and steer immediately.
...
- Supervisor state lives in Oriro Gateway storage.
...
- Transcript reads redact secrets at Oriro boundary.
...
- Add escalation between Oriro groups.
...
- Durable state owner: Oriro Gateway DB, Cloudflare Durable Object, or VPS database.
- Approval policy granularity for supervisor-originated turns.
- How much transcript summary should be injected into the always-on Oriro context versus kept as a tool/resource.
