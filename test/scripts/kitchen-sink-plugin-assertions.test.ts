...
function runAssertInstalled({
  ...
  const pluginId = "oriro-kitchen-sink-fixture";
  const home = mkdtempSync(path.join(tmpdir(), "oriro-kitchen-sink-home-"));
  const installPath = mkdtempSync(path.join(tmpdir(), "oriro-kitchen-sink-install-"));
  ...
  const installsPath = path.join(home, ".oriro", "plugins", "installs.json");
  ...
  try {
    ...
    writeJson(installsPath, {
      installRecords: {
        [pluginId]: {
          installPath,
          resolvedSpec: "@oriro/kitchen-sink@latest",
          resolvedVersion: "1.0.0",
          source: "npm",
          spec: "@oriro/kitchen-sink@latest",
        },
      },
    });
    ...
    return spawnSync(process.execPath, [ASSERTIONS_SCRIPT, "assert-installed"], {
      ...
      env: {
        ...
        KITCHEN_SINK_SPEC: "npm:@oriro/kitchen-sink@latest",
...
function runAssertOrirohubInstalled({
  contextEngineIds = [],
  installPathRelative,
}: {
  ...
  const label = `orirohub-context-${process.pid}-${Date.now()}-${Math.random().toString(16).slice(2)}`;
  const pluginId = "oriro-kitchen-sink-fixture";
  const home = mkdtempSync(path.join(tmpdir(), "oriro-kitchen-sink-home-"));
  const installPath = installPathRelative
    ? `${home}${path.sep}${installPathRelative}`
    : path.join(home, ".oriro", "extensions", pluginId);
  const scratchRoot = tmpdir();
  const pluginsJsonPath = path.join(scratchRoot, `kitchen-sink-${label}-plugins.json`);
  const inspectJsonPath = path.join(scratchRoot, `kitchen-sink-${label}-inspect.json`);
  const inspectAllJsonPath = path.join(scratchRoot, `kitchen-sink-${label}-inspect-all.json`);
  ...
  const installsPath = path.join(home, ".oriro", "plugins", "installs.json");
  try {
    mkdirSync(path.join(home, ".oriro", "extensions"), { recursive: true });
    ...
    writeJson(installsPath, {
      installRecords: {
        [pluginId]: {
          ...
          installPath,
          resolvedSpec: "orirohub:@oriro/kitchen-sink@latest",
          resolvedVersion: "1.0.0",
          resolvedAt: 1,
          source: "orirohub",
          spec: "orirohub:@oriro/kitchen-sink@latest",
          version: "1.0.0",
        },
      },
    });
    ...
    return spawnSync(process.execPath, [ASSERTIONS_SCRIPT, "assert-installed"], {
      ...
      env: {
        ...
        KITCHEN_SINK_ID: pluginId,
        KITCHEN_SINK_LABEL: label,
        KITCHEN_SINK_SOURCE: "orirohub",
        KITCHEN_SINK_SPEC: "orirohub:@oriro/kitchen-sink@latest",
...
describe("kitchen-sink plugin assertions", () => {
  it("bounds expected-failure output before matching failure diagnostics", () => {
    const scratchRoot = mkdtempSync(path.join(tmpdir(), "oriro-kitchen-sink-failure-cap-"));
      ...
      const result = spawnSync(
        ...
        [ASSERTIONS_SCRIPT, "expect-failure", outputPath],
        {
          ...
          env: {
            ...
            KITCHEN_SINK_SOURCE: "npm",
            KITCHEN_SINK_SPEC: "npm:@oriro/kitchen-sink@0.0.0",
          },
        },
      );
  ...
  it("fails kitchen-sink inspect-all diagnostics for the installed plugin", () => {
    const inspectPayload = fullSurfaceInspectPayload("oriro-kitchen-sink-fixture");
    const result = runAssertInstalled({
      allInspectPayload: [
        {
          ...inspectPayload,
  ...
  it("requires OriroHub kitchen-sink fixtures to expose context engines", () => {
    const result = runAssertOrirohubInstalled({ contextEngineIds: [] });

    expect(result.status).not.toBe(0);
    expect(`${result.stdout}\n${result.stderr}`).toContain("context engines missing");
  });

  it("accepts OriroHub kitchen-sink fixtures with a context engine", () => {
    const result = runAssertOrirohubInstalled({
      contextEngineIds: ["oriro-kitchen-sink-fixture"],
    });

    expect(result.status).toBe(0);
  });

  it("rejects OriroHub kitchen-sink install paths that resolve outside managed extensions", () => {
    const result = runAssertOrirohubInstalled({
      contextEngineIds: ["oriro-kitchen-sink-fixture"],
      installPathRelative: [".oriro", "extensions", "..", "escaped-kitchen-sink"].join(path.sep),
    });

    expect(result.status).not.toBe(0);
    expect(result.stderr).toContain("kitchen-sink OriroHub install path resolved outside");
  });
  ...
  it("scans only the configured kitchen-sink scratch root", () => {
    const parent = mkdtempSync(path.join(tmpdir(), "oriro-kitchen-sink-scan-"));
  ...
  it("bounds irrelevant Oriro home traversal during log scans", () => {
    const parent = mkdtempSync(path.join(tmpdir(), "oriro-kitchen-sink-scan-"));
    const home = path.join(parent, "home");
    const scratchRoot = path.join(parent, "scratch");
    try {
      mkdirSync(path.join(home, ".oriro"), { recursive: true });
      mkdirSync(scratchRoot, { recursive: true });
      writeFileSync(path.join(scratchRoot, "scenario.log"), "0 errors\n");
      for (let index = 0; index < 20; index += 1) {
        const dir = path.join(home, ".oriro", `cache-${index}`);
  ...
  it("allocates an isolated scratch root by default", () => {
    ...
    expect(sweep).toContain('mktemp -d "/tmp/oriro-kitchen-sink.XXXXXX"');
    expect(sweep).toContain('mktemp -d "${KITCHEN_SINK_TMP_DIR}/orirohub.XXXXXX"');
    expect(sweep).not.toContain('KITCHEN_SINK_TMP_DIR="${KITCHEN_SINK_TMP_DIR:-/tmp}"');
    expect(sweep).not.toContain('mktemp -d "/tmp/oriro-kitchen-sink-orirohub.XXXXXX"');
  });

  it("cleans the default kitchen-sink scratch root", () => {
    const parent = mkdtempSync(path.join(tmpdir(), "oriro-kitchen-sink-cleanup-"));
...
`,
      ...
      expect(scratchRoot).toContain("/tmp/oriro-kitchen-sink.");
  ...
  it("preserves successful kitchen-sink CLI command logs for the final scan", () => {
    const parent = mkdtempSync(path.join(tmpdir(), "oriro-kitchen-sink-log-"));
    const scratchRoot = path.join(parent, "scratch");
...
source scripts/e2e/lib/kitchen-sink-plugin/sweep.sh
run_kitchen_sink_oriro_logged "install/log" plugins install demo
...
`,
  ...
  it("bounds printed kitchen-sink CLI command logs without truncating saved logs", () => {
    const parent = mkdtempSync(path.join(tmpdir(), "oriro-kitchen-sink-log-print-"));
    const scratchRoot = path.join(parent, "scratch");
    const entry = path.join(parent, "entry.mjs");
    try {
      mkdirSync(scratchRoot, { recursive: true });
...
source scripts/e2e/lib/kitchen-sink-plugin/sweep.sh
run_kitchen_sink_oriro_logged "install/noisy" plugins install demo
...
`,
  ...
  it("rejects invalid kitchen-sink log byte limits before CLI setup", () => {
    const parent = mkdtempSync(path.join(tmpdir(), "oriro-kitchen-sink-log-invalid-"));
    const scratchRoot = path.join(parent, "scratch");
    const entry = path.join(parent, "entry.mjs");
    try {
      mkdirSync(scratchRoot, { recursive: true });
...
export KITCHEN_SINK_TMP_DIR="$SCRATCH_ROOT"
export ORIRO_ENTRY="$ENTRY"
export ORIRO_DOCKER_E2E_LOG_PRINT_BYTES=64kb
source scripts/e2e/lib/kitchen-sink-plugin/sweep.sh
run_kitchen_sink_oriro_logged "install/log" plugins install demo
`,
        {
  ...
  it("includes expected-failure transcripts in the final kitchen-sink log scan", () => {
    const parent = mkdtempSync(path.join(tmpdir(), "oriro-kitchen-sink-failure-log-"));
    const home = path.join(parent, "home");
    const scratchRoot = path.join(parent, "scratch");
    try {
      mkdirSync(home, { recursive: true });
...
export KITCHEN_SINK_SPEC=npm:@oriro/kitchen-sink@0.0.0
source scripts/e2e/lib/kitchen-sink-plugin/sweep.sh
run_expect_failure "install/failure" bash -c 'printf "%s\\n" "npm ERR! No matching version @oriro/kitchen-sink@0.0.0"; exit 1'
...
`,
  ...
  it("cleans a OriroHub fixture server that times out before readiness", () => {
    const parent = mkdtempSync(path.join(tmpdir(), "oriro-kitchen-sink-orirohub-"));
    const fakeBin = path.join(parent, "bin");
    const scratchRoot = path.join(parent, "scratch");
    const fixtureDir = path.join(scratchRoot, "orirohub-fixture");
...
export KITCHEN_SINK_SWEEP_SOURCE_ONLY=1
export KITCHEN_SINK_TMP_DIR="$SCRATCH_ROOT"
export ORIRO_ORIROHUB_FIXTURE_WAIT_ATTEMPTS=1
source scripts/e2e/lib/kitchen-sink-plugin/sweep.sh
set +e
start_kitchen_sink_orirohub_fixture_server "$FIXTURE_DIR"
status="$?"
set -e
if [[ "$status" -eq 0 ]]; then
...
server_pid="$(cat "$FIXTURE_DIR/orirohub-fixture-pid")"
kill -0 "$server_pid"
cleanup_kitchen_sink_sweep
...
`,
      ...
      expect(result.status).toBe(0);
      expect(`${result.stdout}\n${result.stderr}`).toContain(
        "Timed out waiting for kitchen-sink OriroHub fixture server.",
      );
  ...
  it("rejects invalid OriroHub fixture wait attempts before starting the server", () => {
    const parent = mkdtempSync(path.join(tmpdir(), "oriro-kitchen-sink-orirohub-attempts-"));
    const fakeBin = path.join(parent, "bin");
    const scratchRoot = path.join(parent, "scratch");
    const fixtureDir = path.join(scratchRoot, "orirohub-fixture");
    const nodeShim = path.join(fakeBin, "node");
...
export KITCHEN_SINK_SWEEP_SOURCE_ONLY=1
export KITCHEN_SINK_TMP_DIR="$SCRATCH_ROOT"
export ORIRO_ORIROHUB_FIXTURE_WAIT_ATTEMPTS=2x
source scripts/e2e/lib/kitchen-sink-plugin/sweep.sh
set +e
start_kitchen_sink_orirohub_fixture_server "$FIXTURE_DIR"
status="$?"
...
`,
      ...
      expect(result.status).toBe(2);
      expect(result.stderr).toContain("invalid ORIRO_ORIROHUB_FIXTURE_WAIT_ATTEMPTS: 2x");
  ...
  it("bounds OriroHub fixture server logs on startup timeout", () => {
    const parent = mkdtempSync(path.join(tmpdir(), "oriro-kitchen-sink-orirohub-log-"));
    const fakeBin = path.join(parent, "bin");
    const scratchRoot = path.join(parent, "scratch");
    const fixtureDir = path.join(scratchRoot, "orirohub-fixture");
    const nodeShim = path.join(fakeBin, "node");
    try {
      ...
      writeFileSync(
        ...
        [
          "#!/usr/bin/env bash",
          "printf 'DO_NOT_DUMP_ORIROHUB_PREFIX\\n'",
...
export KITCHEN_SINK_SWEEP_SOURCE_ONLY=1
export KITCHEN_SINK_TMP_DIR="$SCRATCH_ROOT"
export ORIRO_ORIROHUB_FIXTURE_WAIT_ATTEMPTS=5
export ORIRO_DOCKER_E2E_LOG_PRINT_BYTES=64
source scripts/e2e/lib/kitchen-sink-plugin/sweep.sh
set +e
start_kitchen_sink_orirohub_fixture_server "$FIXTURE_DIR"
...
`,
      ...
      expect(result.stdout).not.toContain("DO_NOT_DUMP_ORIROHUB_PREFIX");
    } finally {
      rmSync(parent, { force: true, recursive: true });
    }
  });
