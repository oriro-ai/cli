---
summary: "Run multiple Oriro Gateways on one host (isolation, ports, and profiles)"
read_when:
...
oriro --profile rescue onboard
oriro --profile rescue gateway install --port 19789
```

If your main bot is already running, that is usually all you need.

During `oriro --profile rescue onboard`:
...
`oriro --profile rescue onboard` uses the normal onboarding flow, but it
...
- config file
- state directory
- workspace (by default `~/.oriro/workspace-rescue`)
...
# extra gateway
oriro --profile ops setup
oriro --profile ops gateway --port 19789
```

If you want both Gateways to use named profiles, that also works:

```bash
oriro --profile main setup
oriro --profile main gateway --port 18789
...
oriro --profile ops gateway install --port 19789
```
...
- `ORIRO_CONFIG_PATH` — per-instance config file
- `ORIRO_STATE_DIR` — per-instance sessions, creds, caches
...
Base port = `gateway.port` (or `ORIRO_GATEWAY_PORT` / `--port`).
...
```bash
ORIRO_CONFIG_PATH=~/.oriro/main.json \
ORIRO_STATE_DIR=~/.oriro \
oriro gateway --port 18789

ORIRO_CONFIG_PATH=~/.oriro/rescue.json \
ORIRO_STATE_DIR=~/.oriro-rescue \
oriro gateway --port 19789
```
...
oriro --profile rescue gateway status --deep
oriro --profile rescue gateway probe
oriro status
oriro --profile rescue status
oriro --profile rescue browser status
```

Interpretation:

- `gateway status --deep` helps catch stale launchd/systemd/schtasks services from older installs.
- `gateway probe` warning text such as `multiple reachable gateway identities detected` is expected only when you intentionally run more than one isolated gateway, or when Oriro cannot prove reachable probe targets are the same gateway. An SSH tunnel, proxy URL, or configured remote URL to the same gateway is one gateway with multiple transports, even when transport ports differ.
