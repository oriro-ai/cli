...
describe("managed npm root", () => {
  ...
  it("syncs Oriro-owned overrides without dropping unrelated local overrides", async () => {
    const npmRoot = await makeTempRoot();
    await fs.writeFile(
      path.join(npmRoot, "package.json"),
      `${JSON.stringify(
  ...
  it("reads workspace pnpm overrides for managed plugin installs", async () => {
    ...
    await expect(readOriroManagedNpmRootOverrides()).resolves.toEqual(expectedOverrides);
  });
  ...
  it("resolves npm override dependency references from the host package manifest", async () => {
    ...
    await expect(readOriroManagedNpmRootOverrides({ packageRoot })).resolves.toEqual({
      "managed-runtime": "3.1024.0",
      nested: {
        "optional-runtime": "2.0.0",
    ...
    expect(
      resolveManagedNpmRootDependencySpec({
        ...
        resolution: {
          name: "@oriro/discord",
          version: "2026.5.2",
          resolvedSpec: "@oriro/discord@2026.5.2",
      ...
      `${JSON.stringify(
        {
          lockfileVersion: 3,
          packages: {
            "node_modules/@oriro/discord": {
              version: "2026.5.2",
              resolved: "https://registry.npmjs.org/@oriro/discord/-/discord-2026.5.2.tgz",
    ...
    const runCommand = vi.fn(async (_args: string[], optionsOrTimeout: number | CommandOptions) => {
      ...
      if (runCommand.mock.calls.length === 1) {
        return {
          code: 1,
          stdout: "",
          stderr: "npm ERR! notarget No matching version found for oriro@2026.5.99-beta.1",
          signal: null,
          killed: false,
          termination: "exit" as const,
        };
  ...
  it("repairs stale managed oriro peer state without dropping plugin packages", async () => {
    const npmRoot = await makeTempRoot();
    ...
    await fs.writeFile(
      path.join(npmRoot, "node_modules", "oriro", "package.json"),
      `${JSON.stringify({ name: "oriro", version: "2026.5.4" })}\n`,
    );
    await fs.mkdir(path.join(npmRoot, "node_modules", ".bin"), { recursive: true });
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "oriro"), "shim");
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "oriro.cmd"), "cmd shim");
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "oriro.ps1"), "ps1 shim");
    await fs.writeFile(
    ...
    await expect(repairManagedNpmRootOriroPeer({ npmRoot, runCommand })).resolves.toBe(true);
    expect(runCommand).toHaveBeenCalledTimes(1);
    ...
    expect(lockfile.packages?.["node_modules/@oriro/discord"]?.version).toBe("2026.5.4");
    expect(lockfile.dependencies?.oriro).toBeUndefined();
    await expectPathMissing(path.join(npmRoot, "node_modules", "oriro"));
    for (const binName of ["oriro", "oriro.cmd", "oriro.ps1"]) {
  ...
  it("does not repair the active Oriro host package in a root-managed install", async () => {
    const npmRoot = await makeTempRoot();
      ...
      `${JSON.stringify(
        {
          ...
          dependencies: {
            ...
            "@xdarkicex/oriro-memory-libravdb": "1.4.69",
          },
        },
        null,
        2,
    ...
    await fs.writeFile(
      ...
      `${JSON.stringify({ name: "oriro", version: "2026.5.12-beta.6" })}\n`,
    );
  ...
  it("scrubs managed ownership metadata without deleting a linked active host package", async () => {
    const npmRoot = await makeTempRoot();
    const hostPackageRoot = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-host-package-"));
    ...
    await fs.symlink(hostPackageRoot, path.join(npmRoot, "node_modules", "oriro"), "dir");
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "oriro"), "shim");
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "oriro.cmd"), "cmd shim");
    await fs.writeFile(path.join(npmRoot, "node_modules", ".bin", "oriro.ps1"), "ps1 shim");
    await fs.writeFile(
      ...
      `${JSON.stringify(
        {
          ...
          packages: {
            ...
            "node_modules/@xdarkicex/oriro-memory-libravdb": {
              version: "1.4.69",
            },
          },
          dependencies: {
    ...
    expect(runCommand).not.toHaveBeenCalled();
    await expect(fs.realpath(path.join(npmRoot, "node_modules", "oriro"))).resolves.toBe(
    ...
    expect(lockfile.packages?.["node_modules/oriro"]).toBeUndefined();
    expect(lockfile.packages?.["node_modules/@xdarkicex/oriro-memory-libravdb"]?.version).toBe(
      "1.4.69",
    );
    expect(lockfile.dependencies?.oriro).toBeUndefined();
    for (const binName of ["oriro", "oriro.cmd", "oriro.ps1"]) {
