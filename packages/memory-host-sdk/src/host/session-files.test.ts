...
function captureStateDirEnv() {
  const value = process.env.ORIRO_STATE_DIR;
  return {
    restore() {
      if (value === undefined) {
        Reflect.deleteProperty(process.env, "ORIRO_STATE_DIR");
      } else {
        Reflect.set(process.env, "ORIRO_STATE_DIR", value);
      }
    },
  };
}
...
beforeEach(() => {
  ...
  Reflect.set(process.env, "ORIRO_STATE_DIR", tmpDir);
});
...
describe("buildSessionEntry", () => {
  it("returns lineMap tracking original JSONL line numbers", async () => {
    ...
    const jsonlLines = [
      ...
      JSON.stringify({ type: "custom", customType: "oriro.cache-ttl", data: {} }),
