...
describe("plugin authoring commands", () => {
  beforeAll(async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-plugin-source-warm-"));
    try {
      const entryPath = writeSourceToolPluginProject({
        tmpDir,
        packageName: "oriro-plugin-source-warm",
  ...
  it("preserves manifest-owned metadata while updating generated fields", () => {
    ...
    expect(
      validateToolPluginProject({
        ...
        entry: "./src/index.ts",
        manifest,
        packageManifest: { version: "1.2.3", oriro: { extensions: ["./src/index.ts"] } },
      }),
    ).toEqual([]);
  });
  ...
  it("drops stale manifest-owned tool metadata when no generated metadata remains", () => {
    const metadata = createDemoMetadata();
    const packageManifest = { version: "1.2.3", oriro: { extensions: ["./src/index.ts"] } };
    const manifest = buildToolPluginManifest({
      metadata,
      packageManifest,
      existingManifest: {
  ...
  it("aligns package metadata with the selected runtime extension entry", () => {
    expect(
      buildToolPluginPackageManifest({
        packageManifest: {
          name: "demo",
          oriro: { setupEntry: "./setup.ts", extensions: ["./src/other.ts"] },
        },
        entry: "./src/index.ts",
      }),
    ).toEqual({
  ...
  it("reports stale manifest contracts", () => {
    ...
    expect(
      validateToolPluginProject({
        ...
        packageManifest: { oriro: { extensions: ["./src/index.ts"] } },
      }),
    ).toEqual([
      "oriro.plugin.json generated metadata is stale. Run oriro plugins build.",
      "oriro.plugin.json contracts.tools is missing: demo_echo",
      "oriro.plugin.json contracts.tools has no matching defineToolPlugin tool: other_tool",
    ]);
  });

  it("reports missing entries with an author-facing path", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-plugin-missing-"));
  ...
  it("loads source entries that import the Oriro plugin SDK package subpath", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-plugin-source-"));
    const entryPath = writeSourceToolPluginProject({
      tmpDir,
      packageName: "oriro-plugin-source-demo",
  ...
  it("scaffolds a dist-entry tool plugin project", async () => {
    const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-plugin-init-"));
    ...
    expect(
      JSON.parse(fs.readFileSync(path.join(projectDir, "oriro.plugin.json"), "utf8")),
    ).toMatchObject({
      id: "stock-quotes",
      name: 'Stock "Quotes"',
      configSchema: {
