  ...
  if (typeof window === "undefined") {
    vi.stubGlobal(
      ...
      value == null
        ...
        : ({ __ORIRO_CONTROL_UI_BASE_PATH__: value } as Window & typeof globalThis),
    );
    return;
  }
  if (value == null) {
...
describe("loadSettings default gateway URL derivation", () => {
  ...
  it("uses configured base path and normalizes trailing slash", () => {
    ...
    setControlUiBasePath(" /oriro/ ");

    expect(loadSettings().gatewayUrl).toBe(expectedGatewayUrl("/oriro"));
  });
  ...
  it("infers base path from nested pathname when configured base path is not set", () => {
    setTestLocation({
      ...
      pathname: "/apps/oriro/chat",
    });

    expect(loadSettings().gatewayUrl).toBe(expectedGatewayUrl("/apps/oriro"));
  });
  ...
  it("ignores and scrubs legacy persisted tokens", () => {
    ...
    sessionStorage.setItem("oriro.control.token.v1", "legacy-session-token");
    localStorage.setItem(
      "oriro.control.settings.v1",
      JSON.stringify({
        gatewayUrl: "wss://gateway.example:8443/oriro",
    ...
    const settings = loadSettings();
    expect(settings.gatewayUrl).toBe("wss://gateway.example:8443/oriro");
    expect(settings.token).toBe("");
    expect(settings.sessionKey).toBe("agent");
    const scopedKey = "oriro.control.settings.v1:wss://gateway.example:8443/oriro";
    expect(JSON.parse(localStorage.getItem(scopedKey) ?? "{}")).toEqual({
      ...
      textScale: 100,
      sessionsByGateway: {
        "wss://gateway.example:8443/oriro": {
          sessionKey: "agent",
  ...
  it("does not persist gateway tokens when saving settings", () => {
    ...
    const scopedKey = `oriro.control.settings.v1:${gwUrl}`;
    expect(JSON.parse(localStorage.getItem(scopedKey) ?? "{}")).toEqual({
      gatewayUrl: gwUrl,
      theme: "oriro",
      themeMode: "system",
  ...
  it("normalizes persisted text scale to the nearest supported stop", () => {
    ...
    const gwUrl = expectedGatewayUrl("");
    localStorage.setItem(
      `oriro.control.settings.v1:${gwUrl}`,
      JSON.stringify({
        gatewayUrl: gwUrl,
        textScale: 123,
      }),
  ...
  it("falls back to oriro when persisted custom theme data is invalid", () => {
    setTestLocation({
      protocol: "https:",
      host: "gateway.example:8443",
      pathname: "/",
    ...
    const settings = loadSettings();
    expect(settings.theme).toBe("oriro");
  ...
  it("caps persisted session scopes to the most recent gateways", () => {
    ...
    const gwUrl = expectedGatewayUrl("");
    const scopedKey = `oriro.control.settings.v1:wss://gateway.example:8443`;
  ...
  it("removes the persisted local user identity when cleared", () => {
    ...
    expect(localStorage.getItem("oriro.control.user.v1")).toBeNull();
  });
});
