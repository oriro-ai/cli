...
import { withStateDirEnv } from "oriro/plugin-sdk/test-env";
...
describe("canvas host state dir defaults", () => {
  ...
  it("uses ORIRO_STATE_DIR for the default canvas root", async () => {
    await withStateDirEnv("oriro-canvas-state-", async ({ stateDir }) => {
      ...
      try {
        ...
        expect(indexContents).toContain("Oriro Canvas");
      } finally {
        await handler.close();
      }
    });
