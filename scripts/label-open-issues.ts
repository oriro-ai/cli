// Label Open Issues script supports Oriro repository automation.
...
const STATE_FILE_PATH = join(CONFIG_BASE_DIR, "oriro", STATE_FILE_NAME);
...
function resolveOpenAITimeoutMs(raw = process.env.ORIRO_LABEL_OPEN_ISSUES_OPENAI_TIMEOUT_MS) {
  return parseStrictIntegerOption({
    fallback: DEFAULT_OPENAI_TIMEOUT_MS,
    label: "ORIRO_LABEL_OPEN_ISSUES_OPENAI_TIMEOUT_MS",
    min: 1,
    raw,
  });
}
