// Persists managed task-flow records through the Oriro SQLite state database.
...
type FlowRunsTable = OriroStateKyselyDatabase["flow_runs"];
type FlowRegistryStoreDatabase = Pick<OriroStateKyselyDatabase, "flow_runs">;
...
// SQLite-backed task-flow store mirrors the in-process registry into oriro-state.db.
...
function pruneFlowsNotInSnapshot(params: { db: DatabaseSync; ids: readonly string[] }) {
  const tempTableName = "oriro_live_flow_ids";
