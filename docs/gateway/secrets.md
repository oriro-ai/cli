...
Oriro supports additive SecretRefs so supported credentials do not need to be stored as plaintext in configuration.
...
agent can inspect, including `oriro.json`, `auth-profiles.json`, `.env`, or
...
`oriro secrets audit --check` reports no plaintext secret residue.
</Warning>
...
- legacy plaintext residue has been scrubbed from `oriro.json`,
  `auth-profiles.json`, `.env`, and generated `models.json` files
- `oriro secrets audit --check` is clean after the migration
...
<AccordionGroup>
  <Accordion title="Examples of inactive surfaces">
    ...
    - `gateway.auth.token` SecretRef is inactive for startup auth resolution when `ORIRO_GATEWAY_TOKEN` is set, because env token input wins for that runtime.
...
When onboarding runs in interactive mode and you choose SecretRef storage, Oriro runs preflight validation before saving:
...
<AccordionGroup>
  ...
  <Accordion title="Exec provider">
    ...
    - Set `allowSymlinkCommand: true` to allow symlink command paths (for example Homebrew shims). Oriro validates the resolved target path.
    ...
    - Plugin-managed exec providers can use `pluginIntegration` instead of
      copied `command`/`args`. Oriro resolves the current command details
...
<AccordionGroup>
      ...
      secrets: {
        providers: {
          onepassword_openai: {
            ...
            args: ["read", "op://Personal/Oriro QA API Key/password"],
            passEnv: ["HOME"],
            jsonOnly: false,
          },
        },
  ...
  <Accordion title="Bitwarden Secrets Manager (`bws`)">
    ...
    `scripts/secrets/oriro-bws-resolver.mjs`; install or copy it to an absolute
    ...
    ```json5
    {
      secrets: {
        providers: {
          bws: {
            source: "exec",
            command: "/usr/local/bin/oriro-bws-resolver.mjs",
        ...
        providers: {
          openai: {
            ...
            apiKey: {
              source: "exec",
              provider: "bws",
              id: "oriro/providers/openai/apiKey",
            },
          },
        },
      },
    ...
    SecretRef id contract, such as `oriro/providers/openai/apiKey`; env-var
    ...
    ```bash
    oriro secrets audit --allow-exec
    ```
      ...
      secrets: {
        providers: {
          vault_openai: {
            ...
            allowSymlinkCommand: true, // required for Homebrew symlinked binaries
            trustedDirs: ["/opt/homebrew"],
            args: ["kv", "get", "-field=OPENAI_API_KEY", "secret/oriro"],
  ...
  <Accordion title="password-store (`pass`)">
    ...
    `/usr/local/bin/oriro-pass-resolver`. The `#!/usr/bin/env node` shebang
    ...
    ```json5
    {
      secrets: {
        providers: {
          pass_store: {
            source: "exec",
            command: "/usr/local/bin/oriro-pass-resolver",
...
- Oriro resolves these refs during sandbox activation, not lazily during each SSH call.
...
- The redaction sentinel `__ORIRO_REDACTED__` is reserved for internal config redaction/restore and is rejected as literal submitted config data.

Warning and audit signals:

- `SECRETS_REF_OVERRIDES_PLAINTEXT` (runtime warning)
- `REF_SHADOWED` (audit finding when `auth-profiles.json` credentials take precedence over `oriro.json` refs)
...
When reload-time activation fails after a healthy state, Oriro enters degraded secrets state.
...
<Tabs>
  <Tab title="Strict command paths">
    For example `oriro memory` remote-memory paths and `oriro qr --remote` when it needs remote shared-secret refs. They read from the active snapshot and fail fast when a required SecretRef is unavailable.
  </Tab>
  <Tab title="Read-only command paths">
    For example `oriro status`, `oriro status --all`, `oriro channels status`, `oriro channels resolve`, `oriro security audit`, and read-only doctor/config repair flows. They also prefer the active snapshot, but degrade instead of aborting when a targeted SecretRef is unavailable in that command path.
...
- Snapshot refresh after backend secret rotation is handled by `oriro secrets reload`.
...
with `oriro secrets apply --from <plan-path>` before the re-audit.

<AccordionGroup>
  <Accordion title="secrets audit">
    Findings include:

    - plaintext values at rest (`oriro.json`, `auth-profiles.json`, `.env`, and generated `agents/*/agent/models.json`)
    - plaintext sensitive provider header residues in generated `models.json` entries
    - unresolved refs
    - precedence shadowing (`auth-profiles.json` taking priority over `oriro.json` refs)
    - legacy residues (`auth.json`, OAuth reminders)

    Exec note:

    - By default, audit skips exec SecretRef resolvability checks to avoid command side effects.
    - Use `oriro secrets audit --allow-exec` to execute exec providers during audit.
  ...
  <Accordion title="secrets configure">
    ...
    - lets you select supported secret-bearing fields in `oriro.json` plus `auth-profiles.json` for one agent scope
    ...
    - `oriro secrets configure --providers-only`
    - `oriro secrets configure --skip-provider-setup`
    - `oriro secrets configure --agent <id>`
  ...
  <Accordion title="secrets apply">
    ...
    ```bash
    oriro secrets apply --from /tmp/oriro-secrets-plan.json
    oriro secrets apply --from /tmp/oriro-secrets-plan.json --allow-exec
    oriro secrets apply --from /tmp/oriro-secrets-plan.json --dry-run
    oriro secrets apply --from /tmp/oriro-secrets-plan.json --dry-run --allow-exec
    ```
...
<Warning>
Oriro intentionally does not write rollback backups containing historical plaintext secret values.
</Warning>
