...
Oriro routes replies **back to the channel where a message came from**. The
...
Oriro infers a pinned owner from `allowFrom` when all of these are true:
...
In that mismatch case, Oriro still records inbound session metadata, but it
...
when a guarded path must not create a new Oriro session. In that mode,
Oriro may update metadata and `lastRoute` for an existing session, but it
...
Broadcast groups let you run **multiple agents** for the same peer **when Oriro would normally reply** (for example: in WhatsApp groups, after mention/activation gating).
...
{
  agents: {
    list: [{ id: "support", name: "Support", workspace: "~/.oriro/workspace-support" }],
  },
  bindings: [
...
Session stores live under the state directory (default `~/.oriro`):

- `~/.oriro/agents/<agentId>/sessions/sessions.json`
