...
const DEFAULT_CLI_NAME = "oriro";

const KNOWN_CLI_NAMES = new Set([DEFAULT_CLI_NAME]);
const CLI_PREFIX_RE = /^(?:((?:pnpm|npm|bunx|npx)\s+))?(oriro)\b/;

/** Resolve the displayed CLI binary name from argv, falling back to `oriro`. */
...
/** Replace a leading `oriro` command prefix with the active CLI name. */
