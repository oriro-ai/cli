...
import { normalizeProviderId } from "oriro/plugin-sdk/provider-model-shared";
import { normalizeAgentId } from "oriro/plugin-sdk/routing";
import { coerceSecretRef } from "oriro/plugin-sdk/secret-input";
import {
...
const DEFAULT_POLICY_SANDBOX_BROWSER_NETWORK = "oriro-sandbox-browser";
  ...
  return Object.entries(configuredChannels(cfg))
    ...
    .map(([id, value]) => {
      const entry: {
        ...
        id,
        provider: id,
        source: `oc://oriro.config/channels/${id}`,
      };
  ...
  return Object.entries(configuredMcpServers(cfg))
    ...
    .map(([id, value]) => {
      const entry: {
        ...
        source: `oc://oriro.config/mcp/servers/${ocPathSegment(id)}`,
      };
      if (isRecord(value)) {
...
export function scanPolicyModelProviders(
  ...
  return Object.keys(configuredModelProviders(cfg))
    ...
    .map((id) => ({
      ...
      source: `oc://oriro.config/models/providers/${id}`,
    }));
}
...
export function scanPolicyModelRefs(
  ...
  if (isRecord(cfg.agents)) {
    collectModelRefsFromRecord(refs, cfg.agents, "oc://oriro.config/agents");
...
export function scanPolicyNetwork(cfg: Record<string, unknown>): readonly PolicyNetworkEvidence[] {
  return [
    networkBooleanEvidence(
      ...
      "oc://oriro.config/browser/ssrfPolicy/dangerouslyAllowPrivateNetwork",
    ),
    networkBooleanEvidence(
      cfg,
      "browser-private-network-legacy",
      ["browser", "ssrfPolicy", "allowPrivateNetwork"],
      "oc://oriro.config/browser/ssrfPolicy/allowPrivateNetwork",
    ),
    networkBooleanEvidence(
      cfg,
      "web-fetch-private-network",
      ["tools", "web", "fetch", "ssrfPolicy", "dangerouslyAllowPrivateNetwork"],
      "oc://oriro.config/tools/web/fetch/ssrfPolicy/dangerouslyAllowPrivateNetwork",
    ),
    networkBooleanEvidence(
      cfg,
      "web-fetch-private-network-legacy",
      ["tools", "web", "fetch", "ssrfPolicy", "allowPrivateNetwork"],
      "oc://oriro.config/tools/web/fetch/ssrfPolicy/allowPrivateNetwork",
    ),
    networkBooleanEvidence(
      cfg,
      "web-fetch-rfc2544-benchmark-range",
      ["tools", "web", "fetch", "ssrfPolicy", "allowRfc2544BenchmarkRange"],
      "oc://oriro.config/tools/web/fetch/ssrfPolicy/allowRfc2544BenchmarkRange",
    ),
    networkBooleanEvidence(
      cfg,
      "web-fetch-ipv6-unique-local-range",
      ["tools", "web", "fetch", "ssrfPolicy", "allowIpv6UniqueLocalRange"],
      "oc://oriro.config/tools/web/fetch/ssrfPolicy/allowIpv6UniqueLocalRange",
    ),
...
export function scanPolicyIngress(cfg: Record<string, unknown>): readonly PolicyIngressEvidence[] {
  ...
  const channelDefaultsSource = "oc://oriro.config/channels/defaults";
  ...
  entries.push({
    id: "session-dm-scope",
    kind: "sessionDmScope",
    source: "oc://oriro.config/session/dmScope",
    value: dmScope ?? "main",
  ...
  for (const [channel, value] of Object.entries(channels)) {
    ...
    const channelSource = `oc://oriro.config/channels/${ocPathSegment(channel)}`;
...
export function scanPolicyGatewayExposure(
  ...
  if (bind === "custom" && hasCustomBindHost) {
    entries.push({
      ...
      kind: "bind",
      source: "oc://oriro.config/gateway/customBindHost",
  ...
  entries.push({
    ...
    kind: "auth",
    source: "oc://oriro.config/gateway/auth/mode",
  ...
  entries.push({
    ...
    kind: "authRateLimit",
    source: "oc://oriro.config/gateway/auth/rateLimit",
  ...
  pushGatewayBooleanEvidence(
    ...
    "controlUi",
    controlUi.enabled,
    "oc://oriro.config/gateway/controlUi/enabled",
  );
  pushGatewayBooleanEvidence(
    entries,
    ...
    "oc://oriro.config/gateway/controlUi/allowInsecureAuth",
  );
  pushGatewayBooleanEvidence(
    entries,
    ...
    "oc://oriro.config/gateway/controlUi/dangerouslyDisableDeviceAuth",
  );
  pushGatewayBooleanEvidence(
    entries,
    ...
    "oc://oriro.config/gateway/controlUi/dangerouslyAllowHostHeaderOriginFallback",
  );
  ...
  if (typeof tailscale.mode === "string") {
    entries.push({
      ...
      kind: "tailscale",
      source: "oc://oriro.config/gateway/tailscale/mode",
  ...
  if (tailscale.mode === "serve" && tailscale.preserveFunnel === true) {
    entries.push({
      ...
      kind: "tailscale",
      source: "oc://oriro.config/gateway/tailscale/preserveFunnel",
      value: "funnel",
    });
  }
  ...
  if (gateway.mode === "remote") {
    entries.push({
      ...
      kind: "remote",
      source: "oc://oriro.config/gateway/mode",
      value: "remote",
    });
    if (typeof remote.url === "string" && remote.url.trim() !== "") {
      entries.push({
        ...
        kind: "remote",
        source: "oc://oriro.config/gateway/remote/url",
        value: true,
      });
    }
  }
...
export function scanPolicyAgentWorkspace(
  ...
  pushAgentWorkspaceEvidence(entries, {
    ...
    tools: defaultTools,
    inheritedTools: {},
    workspaceSourceBase: "oc://oriro.config/agents/defaults",
    inheritedWorkspaceSourceBase: "oc://oriro.config/agents/defaults",
    toolsSourceBase: "oc://oriro.config/tools",
    inheritedToolsSourceBase: "oc://oriro.config/tools",
  });
  ...
  list.forEach((agent, index) => {
    ...
    pushAgentWorkspaceEvidence(entries, {
      ...
      workspaceSourceBase: `oc://oriro.config/agents/list/#${index}`,
      inheritedWorkspaceSourceBase: "oc://oriro.config/agents/defaults",
      toolsSourceBase: `oc://oriro.config/agents/list/#${index}/tools`,
      inheritedToolsSourceBase: "oc://oriro.config/tools",
    });
  });
  return entries.toSorted((a, b) => a.source.localeCompare(b.source) || a.id.localeCompare(b.id));
...
export function scanPolicySandboxPosture(
  ...
  pushSandboxPostureEvidence(entries, {
    ...
    sandbox: defaultSandbox,
    inheritedSandbox: {},
    sourceBase: "oc://oriro.config/agents/defaults/sandbox",
    inheritedSourceBase: "oc://oriro.config/agents/defaults/sandbox",
  });
  ...
  list.forEach((agent, index) => {
    ...
    pushSandboxPostureEvidence(entries, {
      ...
      sourceBase: `oc://oriro.config/agents/list/#${index}/sandbox`,
      inheritedSourceBase: "oc://oriro.config/agents/defaults/sandbox",
    });
  });
...
export function scanPolicyToolPosture(
  ...
  pushToolPostureEvidence(entries, {
    ...
    sandbox: defaultSandbox,
    inheritedSandbox: {},
    sourceBase: "oc://oriro.config/tools",
    inheritedSourceBase: "oc://oriro.config/tools",
  });
  ...
  list.forEach((agent, index) => {
    ...
    pushToolPostureEvidence(entries, {
      ...
      inheritedSandbox: defaultSandbox,
      sourceBase: `oc://oriro.config/agents/list/#${index}/tools`,
      inheritedSourceBase: "oc://oriro.config/tools",
    });
  });
  ...
  return Object.entries(profiles)
    ...
    .map(([id, value]) => {
      const entry: {
        ...
        id,
        source: `oc://oriro.config/auth/profiles/${ocPathSegment(id)}`,
...
function pushMemorySessionTranscriptIndexing(
  ...
  if (defaultSessionMemory !== undefined) {
    entries.push({
      ...
      kind: "memorySessionTranscriptIndexing",
      source: "oc://oriro.config/agents/defaults/memorySearch/experimental/sessionMemory",
      scope: "global",
  ...
  agents.list.forEach((rawAgent, index) => {
    ...
    entries.push({
      ...
      kind: "memorySessionTranscriptIndexing",
      source: explicit
        ? `oc://oriro.config/agents/list/#${index}/memorySearch/experimental/sessionMemory`
        : "oc://oriro.config/agents/defaults/memorySearch/experimental/sessionMemory",
      scope: "agent",
...
function scanPolicySecretProviders(cfg: Record<string, unknown>): readonly PolicySecretEvidence[] {
  ...
  return Object.entries(providers).map(([id, value]) => {
    ...
    const entry: {
      ...
      id,
      kind: "provider",
      source: `oc://oriro.config/secrets/providers/${ocPathSegment(id)}`,
    };
...
function configPathSource(path: readonly string[]): string {
  return `oc://oriro.config/${path.map(ocPathSegment).join("/")}`;
}
  ...
  const sandboxModeSource =
    explicitSandboxMode !== undefined
      ...
      : inheritedSandboxMode !== undefined
        ...
        : "oc://oriro.config/agents/defaults/sandbox/mode";
    ...
    source:
      explicitWorkspaceAccess !== undefined
        ...
        : inheritedWorkspaceAccess !== undefined
          ...
          : "oc://oriro.config/agents/defaults/sandbox/workspaceAccess",
    scope: params.scope,
...
function pushGatewayHttpEndpointEvidence(
  ...
  const source = `oc://oriro.config/gateway/http/endpoints/${endpoint}`;
...
function collectModelRefsFromAgentAllowlist(
  ...
  if (isRecord(defaults) && isRecord(defaults.models)) {
    collectModelRefsFromModelMap(
      refs,
      defaults.models,
      "oc://oriro.config/agents/defaults/models",
    );
  }
  ...
  for (const [index, agent] of list.entries()) {
    ...
    collectModelRefsFromModelMap(
      refs,
      agent.models,
      `oc://oriro.config/agents/list/#${index}/models`,
    );
  }
}
