---
summary: "Step-by-step guide to building a messaging channel plugin for Oriro"
title: "Building channel plugins"
sidebarTitle: "Channel Plugins"
read_when:
  - You are building a new messaging channel plugin
  - You want to connect Oriro to a messaging platform
  - You need to understand the ChannelPlugin adapter surface
---

This guide walks through building a channel plugin that connects Oriro to a
...
<Info>
  If you have not built any Oriro plugin before, read
...
Channel plugins do not need their own send/edit/react tools. Oriro keeps one
...
`defineChannelMessageAdapter` from `oriro/plugin-sdk/channel-outbound`. The
...
receive/send lifecycle helpers on `oriro/plugin-sdk/channel-outbound`.

Channels migrating inbound authorization can use the experimental
`oriro/plugin-sdk/channel-ingress-runtime` subpath from runtime receive
...
`oriro/plugin-sdk/outbound-media` with plugin state stores. Keep platform
...
Use `oriro/plugin-sdk/channel-route` when plugin code needs to normalize
...
- If a channel can infer stable owner-like DM identities from existing config, use `createResolvedApproverActionAuthAdapter` from `oriro/plugin-sdk/approval-runtime` to restrict same-chat `/approve` without adding approval-specific core logic.
- If custom approval auth intentionally allows only same-chat fallback, return `markImplicitSameChatApprovalAuthorization({ authorized: true })` from `oriro/plugin-sdk/approval-auth-runtime`; otherwise core treats the result as explicit approver authorization.
...
- `oriro/plugin-sdk/approval-auth-runtime`
- `oriro/plugin-sdk/approval-client-runtime`
- `oriro/plugin-sdk/approval-delivery-runtime`
- `oriro/plugin-sdk/approval-gateway-runtime`
- `oriro/plugin-sdk/approval-handler-adapter-runtime`
- `oriro/plugin-sdk/approval-handler-runtime`
- `oriro/plugin-sdk/approval-native-runtime`
- `oriro/plugin-sdk/approval-reply-runtime`
- `oriro/plugin-sdk/channel-runtime-context`

Likewise, prefer `oriro/plugin-sdk/setup-runtime`,
`oriro/plugin-sdk/setup-runtime`,
`oriro/plugin-sdk/reply-runtime`,
`oriro/plugin-sdk/reply-dispatch-runtime`,
`oriro/plugin-sdk/reply-reference`, and
`oriro/plugin-sdk/reply-chunking` when you do not need the broader umbrella
surface.

For setup specifically:

- `oriro/plugin-sdk/setup-runtime` covers the runtime-safe setup helpers:
...
- `oriro/plugin-sdk/setup-runtime` includes the env-aware adapter seam for
  `createEnvPatchedAccountSetupAdapter`
- `oriro/plugin-sdk/channel-setup` covers the optional-install setup
...
SecretRef scans before the plugin runtime starts, add `oriro.setupEntry` in
...
- use the broader `oriro/plugin-sdk/setup` seam only when you also need the
...
- `oriro/plugin-sdk/account-core`,
  `oriro/plugin-sdk/account-id`,
  `oriro/plugin-sdk/account-resolution`, and
  `oriro/plugin-sdk/account-helpers` for multi-account config and
  default-account fallback
- `oriro/plugin-sdk/inbound-envelope` and
  `oriro/plugin-sdk/channel-inbound` for inbound route/envelope and
  record-and-dispatch wiring
- `oriro/plugin-sdk/channel-targets` for target parsing helpers
- `oriro/plugin-sdk/outbound-media` for media loading and
  `oriro/plugin-sdk/channel-outbound` for outbound identity/send delegates
  and payload planning
- `buildThreadAwareOutboundSessionRoute(...)` from
  `oriro/plugin-sdk/channel-core` when an outbound route should preserve an
...
- `oriro/plugin-sdk/thread-bindings-runtime` for thread-binding lifecycle
  and adapter registration
- `oriro/plugin-sdk/agent-media-payload` only when a legacy agent/media
  payload field layout is still required
- `oriro/plugin-sdk/telegram-command-config` for Telegram custom-command
...
Use `oriro/plugin-sdk/channel-mention-gating` for mention-policy decisions.
Use `oriro/plugin-sdk/channel-inbound` only when you need the broader inbound
helper barrel.
...
`oriro/plugin-sdk/channel-mention-gating` to avoid loading unrelated inbound
runtime helpers.
...
<Steps>
  ...
  <Step title="Package and manifest">
    ...
    see [Plugin Setup and Config](/plugins/sdk-setup#oriro-channel):

    <CodeGroup>
    ```json package.json
    {
      "name": "@myorg/oriro-acme-chat",
      "version": "1.0.0",
      "type": "module",
      "oriro": {
        "extensions": ["./index.ts"],
        ...
        "channel": {
          "id": "acme-chat",
          "label": "Acme Chat",
          "blurb": "Connect Oriro to Acme Chat."
        }
      }
    }
    ```

    ```json oriro.plugin.json
    {
      "id": "acme-chat",
      "kind": "channel",
      "channels": ["acme-chat"],
  ...
  <Step title="Build the channel plugin object">
    ...
    import type { OriroConfig } from "oriro/plugin-sdk/channel-core";
  ...
  <Step title="Wire the entry point">
    ...
    ```typescript index.ts
    import { defineChannelPluginEntry } from "oriro/plugin-sdk/channel-core";
    ...
    Put channel-owned CLI descriptors in `registerCliMetadata(...)` so Oriro
  ...
  <Step title="Add a setup entry">
    ...
    Oriro loads this instead of the full entry when the channel is disabled
    ...
    `oriro/plugin-sdk/channel-entry-contract` when they also need an
  ...
  <Step title="Handle inbound messages">
    ...
    Oriro. The typical pattern is a webhook that verifies the request and
    ...
    registerFull(api) {
      api.registerHttpRoute({
        ...
        handler: async (req, res) => {
          ...
          // Your inbound handler dispatches the message to Oriro.
...
├── package.json              # oriro.channel metadata
├── oriro.plugin.json      # Manifest with config schema
