...
describe("codex plugin lifecycle: pinned-old codex plugin with new Oriro", () => {
  it("blocks with a precise update remediation when the plugin is older than the host", async () => {
    ...
    expect(result.remediation).toBe(
      'Codex plugin version 2026.5.19 is older than Oriro 2026.5.21. Run "oriro plugins update codex" or unpin codex, then rerun "oriro doctor --fix".',
    );
  });
});

describe("codex plugin lifecycle: pinned-new codex plugin with old Oriro", () => {
  it("blocks with a precise host-upgrade remediation when the plugin is newer than the host", async () => {
    ...
    expect(result.status).toBe("blocked");
    expect(result.remediation).toBe(
      "Codex plugin version 2026.5.22 requires a newer Oriro host than 2026.5.21. Upgrade Oriro or install a codex plugin version pinned to 2026.5.21.",
    );
  });
});
...
describe("codex plugin lifecycle: doctor migration safety matrix", () => {
  it.each([
    ...
    {
      name: "mixed profile with defaults Oriro pin",
      profileShape: "mixed" as const,
      config: { agents: { defaults: { agentRuntime: { id: "oriro" } } } },
      expectedRemovedRuntimePins: ["agentRuntime.id=oriro"],
    },
    {
      name: "mixed profile with main-agent Oriro pin",
      profileShape: "mixed" as const,
      config: { agents: { list: { main: { agentRuntime: { id: "oriro" } } } } },
      expectedRemovedRuntimePins: ["agentRuntime.id=oriro"],
    },
  ])(
    "keeps codex auth and strips stale Oriro runtime pins for $name",
