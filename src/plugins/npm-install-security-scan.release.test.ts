...
const REQUIRED_REVIEWED_PUBLISHABLE_CRITICAL_FINDINGS = new Set([
  "@oriro/acpx:dangerous-exec:src/codex-auth-bridge.ts",
  "@oriro/acpx:dangerous-exec:src/runtime-internals/mcp-proxy.mjs",
  "@oriro/codex:dangerous-exec:src/app-server/sandbox-exec-server/http.ts",
  "@oriro/codex:dangerous-exec:src/app-server/sandbox-exec-server/processes.ts",
  "@oriro/codex:dangerous-exec:src/app-server/transport-stdio.ts",
  "@oriro/codex:dangerous-exec:src/node-cli-sessions.ts",
  "@oriro/discord:dangerous-exec:src/voice/audio.ts",
  "@oriro/google-meet:dangerous-exec:src/node-host.ts",
  "@oriro/google-meet:dangerous-exec:src/realtime.ts",
  "@oriro/matrix:dangerous-exec:src/matrix/deps.ts",
  "@oriro/voice-call:dangerous-exec:src/tunnel.ts",
  "@oriro/voice-call:dangerous-exec:src/webhook/tailscale.ts",
]);

const OPTIONAL_REVIEWED_PUBLISHABLE_DIST_CRITICAL_FINDINGS = new Set([
  "@oriro/acpx:dangerous-exec:dist/mcp-proxy.mjs",
  "@oriro/acpx:dangerous-exec:dist/service-<hash>.js",
  "@oriro/codex:dangerous-exec:dist/client-<hash>.js",
  "@oriro/google-meet:dangerous-exec:dist/index.js",
  "@oriro/slack:dynamic-code-execution:dist/outbound-payload.test-harness-<hash>.js",
  "@oriro/voice-call:dangerous-exec:dist/runtime-entry-<hash>.js",
]);
...
function stageScannerRelevantPackedFiles(
  ...
  const stageDir = mkdtempSync(join(tmpdir(), "oriro-plugin-npm-scan-"));
...
function collectPublishablePluginPackages(): PublishablePluginPackage[] {
  return listPublishablePluginPackageDirs()
    .flatMap((packageDir) => {
      ...
      let packageJson: {
        name?: unknown;
        oriro?: { release?: { publishToNpm?: unknown } };
      };
      try {
      ...
      if (packageJson.oriro?.release?.publishToNpm !== true) {
        return [];
      }
