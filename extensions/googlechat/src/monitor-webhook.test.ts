...
import type { FixedWindowRateLimiter } from "oriro/plugin-sdk/webhook-ingress";
...
vi.mock("oriro/plugin-sdk/webhook-request-guards", () => ({
  readJsonWebhookBodyOrReject,
}));

vi.mock("oriro/plugin-sdk/webhook-targets", () => ({
...
describe("googlechat monitor webhook", () => {
  ...
  afterAll(() => {
    vi.doUnmock("oriro/plugin-sdk/webhook-request-guards");
    vi.doUnmock("oriro/plugin-sdk/webhook-targets");
    vi.doUnmock("./auth.js");
    vi.resetModules();
  });
    ...
    expect(processEvent).toHaveBeenCalledWith(
      {
        ...
        action: {
          parameters: [
            { key: "oriro_action", value: "approval" },
