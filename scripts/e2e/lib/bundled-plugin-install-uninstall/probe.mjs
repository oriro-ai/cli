...
function resolveStateDir() {
  if (process.env.ORIRO_STATE_DIR) {
    return process.env.ORIRO_STATE_DIR;
  }
  return path.join(process.env.HOME || os.homedir(), ".oriro");
}
...
function resolveOriroEntry() {
  if (process.env.ORIRO_ENTRY) {
    return process.env.ORIRO_ENTRY;
  }
  ...
  throw new Error("Missing ORIRO_ENTRY and dist/index.(m)js");
}

function readPluginsList() {
  const entry = resolveOriroEntry();
...
async function loadManifestEntries() {
  const explicit = (process.env.ORIRO_BUNDLED_PLUGIN_SWEEP_IDS || "")
    .split(/[,\s]+/u)
  ...
  return explicit.map((lookup) => {
    ...
    if (!found) {
      throw new Error(
        `ORIRO_BUNDLED_PLUGIN_SWEEP_IDS entry is not an installable bundled plugin in this package: ${lookup}. Available: ${available}`,
      );
    }
    return found;
  });
...
async function selectedManifestEntries() {
  ...
  const total = readPositiveIntEnv("ORIRO_BUNDLED_PLUGIN_SWEEP_TOTAL", 1);
  const index = readNonNegativeIntEnv("ORIRO_BUNDLED_PLUGIN_SWEEP_INDEX", 0);
  if (index >= total) {
    throw new Error(
      `ORIRO_BUNDLED_PLUGIN_SWEEP_INDEX must be in [0, ${total - 1}], got ${process.env.ORIRO_BUNDLED_PLUGIN_SWEEP_INDEX}`,
    );
  }
