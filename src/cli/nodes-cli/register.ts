    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["oriro nodes status", "List known nodes with live status."],
          ["oriro nodes pairing pending", "Show pending node pairing requests."],
          ["oriro nodes remove --node <id|name|ip>", "Remove a stale paired node entry."],
          [
            'oriro nodes invoke --node <id> --command system.which --params \'{"name":"uname"}\'',
            "Invoke a node command directly.",
          ],
          ["oriro nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.oriro.ai/cli/nodes")}\n`,
    );
