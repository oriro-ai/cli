...
private let webChatSwiftLogger = Logger(subsystem: "ai.oriro", category: "WebChatSwiftUI")
private let webChatThinkingLevelDefaultsKey = "oriro.webchat.thinkingLevel"
...
struct MacGatewayChatTransport: OriroChatTransport {
    ...
    func listSessions(limit: Int?) async throws -> OriroChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        let decoded = try JSONDecoder().decode(OriroChatSessionsListResponse.self, from: data)
        let mainSessionKey = await GatewayConnection.shared.cachedMainSessionKey()
        let defaults = decoded.defaults.map {
            OriroChatSessionsDefaults(
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> OriroChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                OriroGatewayHealthOK.self,
        ...
        case let .event(evt):
            ...
            case "session.message":
                ...
                guard let message = try? JSONDecoder().decode(
                    OriroSessionMessageEventPayload.self,
                    from: JSONEncoder().encode(payload))
                else {
                    return nil
                }
    ...
    private static func mapModelChoice(_ model: OriroProtocol.ModelChoice) -> OriroChatModelChoice {
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<OriroChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any OriroChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = OriroChatViewModel(
            sessionKey: sessionKey,
            transport: transport,
        ...
        self.hosting = NSHostingController(rootView: OriroChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<OriroChatView>) -> NSViewController
    {
