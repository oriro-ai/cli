...
describe("diagnostic session context", () => {
  beforeEach(async () => {
    testState = await createOriroTestState({
      layout: "state-only",
      prefix: "oriro-diagnostic-session-",
    });
  ...
  it("parses cron run session keys", () => {
    expect(parseCronRunSessionKey("agent:oriroblocker:cron:job-123:run:run-456")).toEqual({
      agentId: "oriroblocker",
      cronJobId: "job-123",
      cronRunId: "run-456",
    });
  });
  ...
  it("formats cron job and last assistant context for stalled session logs", async () => {
    ...
    writeJsonl(path.join(stateDir, "agents", "oriroblocker", "sessions", "run-456.jsonl"), [
