...
import { setLoggerOverride } from "oriro/plugin-sdk/runtime-env";
...
describe("web auto-reply connection", () => {
  ...
  it("treats status 440 as non-retryable and stops without retrying", async () => {
    ...
    expectErrorContaining(runtime.error, "oriro channels logout --channel whatsapp");
    ...
    await withEnvAsync({ TZ: "Europe/Vienna" }, async () => {
      ...
      try {
        ...
        expect(firstArgs.Body).toMatch(
          new RegExp(
            `\\[WhatsApp \\+1 (\\+\\d+[smhd] )?${firstPattern}\\] \\+1: \\[oriro\\] first`,
          ),
        );
        expect(firstArgs.Body).not.toContain("second");
        expect(secondArgs.Body).toMatch(
          new RegExp(
            `\\[WhatsApp \\+1 (\\+\\d+[smhd] )?${secondPattern}\\] \\+1: \\[oriro\\] second`,
          ),
        );
  ...
  it("emits heartbeat logs with connection metadata", async () => {
    vi.useFakeTimers();
    const logPath = `/tmp/oriro-heartbeat-${crypto.randomUUID()}.log`;
  ...
  it("logs outbound replies to file", async () => {
    const logPath = `/tmp/oriro-log-test-${crypto.randomUUID()}.log`;
    setLoggerOverride({ level: "trace", file: logPath });
