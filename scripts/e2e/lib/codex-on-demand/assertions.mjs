...
const inspect = readJson("/tmp/oriro-codex-inspect.json");
...
if (!String(codexRecord.spec || "").includes("@oriro/codex")) {
  throw new Error(`expected @oriro/codex install spec, got ${codexRecord.spec}`);
}
...
if (!fs.existsSync(codexPackageJson)) {
  throw new Error(`missing npm-installed @oriro/codex package: ${codexPackageJson}`);
}
const codexPackage = readJson(codexPackageJson);
if (codexPackage.name !== "@oriro/codex") {
  throw new Error(`unexpected codex package name: ${codexPackage.name}`);
}

const npmProjectRoot = npmProjectRootForInstalledPackage(installPath, "@oriro/codex");
...
function readAuthProfileStoreText(agentDir) {
  const dbPath = path.join(agentDir, "oriro-agent.sqlite");
...
assertOpenAiEnvAuthProfileStore(authRaw, {
  ...
  rawKeyNeedle: "sk-oriro-codex-on-demand-e2e",
});
