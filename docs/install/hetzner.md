---
summary: "Run Oriro Gateway 24/7 on a cheap Hetzner VPS (Docker) with durable state and baked-in binaries"
read_when:
  - You want Oriro running 24/7 on a cloud VPS (not your laptop)
  - You want a production-grade, always-on Gateway on your own VPS
  - You want full control over persistence, binaries, and restart behavior
  - You are running Oriro in Docker on Hetzner or a similar provider
title: "Hetzner"
---

## Goal

Run a persistent Oriro Gateway on a Hetzner VPS using Docker, with durable state, baked-in binaries, and safe restart behavior.

If you want "Oriro 24/7 for ~$5", this is the simplest reliable setup.
...
<Steps>
  ...
  <Step title="Create persistent host directories">
    ...
    ```bash
    mkdir -p /root/.oriro/workspace

    # Set ownership to the container user (uid 1000):
    chown -R 1000:1000 /root/.oriro
    ```
  ...
  <Step title="Configure environment variables">
    ...
    ORIRO_CONFIG_DIR=/root/.oriro
    ORIRO_WORKSPACE_DIR=/root/.oriro/workspace
...
- Infrastructure: [oriro-terraform-hetzner](https://github.com/andreesg/oriro-terraform-hetzner)
- Docker config: [oriro-docker-config](https://github.com/andreesg/oriro-docker-config)
