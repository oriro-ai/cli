---
summary: "Oriro macOS companion app (menu bar + gateway broker)"
read_when:
...
The macOS app is the **menu-bar companion** for Oriro. It owns permissions,
...
- Installs the global CLI (`oriro`) on request via npm, pnpm, or bun (the app prefers npm, then pnpm, then bun; Node remains the recommended Gateway runtime).

## Local vs remote mode

- **Local** (default): the app attaches to a running local Gateway if present;
  otherwise it enables the launchd service via `oriro gateway install`.
...
The app manages a per-user LaunchAgent labeled `ai.oriro.gateway`
(or `ai.oriro.<profile>` when using `--profile`/`ORIRO_PROFILE`; legacy `com.oriro.*` still unloads).
...
Replace the label with `ai.oriro.<profile>` when running a named profile.
...
The app registers the `oriro://` URL scheme for local actions.
...
```bash
open 'oriro://agent?message=Hello%20from%20deep%20link'
```
...
1. Install and launch **Oriro.app**.
...
Avoid putting your Oriro state dir in iCloud or other cloud-synced folders.
...
If `oriro doctor` detects state under:
...
```bash
cd apps/macos
swift run oriro-mac connect --json
swift run oriro-mac discover --timeout 3000 --json
```
...
<Tip>
Compare against `oriro gateway discover --json` to see whether the macOS app's discovery pipeline (`local.` plus the configured wide-area domain, with wide-area and Tailscale Serve fallbacks) differs from the Node CLI's `dns-sd` based discovery.
</Tip>
