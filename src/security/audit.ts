...
import { asNullableRecord } from "@oriro/normalization-core/record-coerce";
import {
...
import type { ConfigFileSnapshot, OriroConfig } from "../config/config.js";
import { resolveConfigPath, resolveStateDir } from "../config/paths.js";
import type { CliBackendConfig } from "../config/types.agent-defaults.js";
import type { GatewayAuthConfig } from "../config/types.gateway.js";
import type { SecurityAuditSuppression } from "../config/types.oriro.js";
import {
...
async function materializeAuditGatewayAuthRefs(params: {
  ...
  const materializeParams = {
    ...
    hasTokenCandidate: Boolean(normalizeOptionalString(params.env.ORIRO_GATEWAY_TOKEN)),
    hasPasswordCandidate: Boolean(normalizeOptionalString(params.env.ORIRO_GATEWAY_PASSWORD)),
  };
...
function shouldMaterializeHooksGatewayAuthRefs(cfg: OriroConfig): boolean {
...
export function collectLoggingFindings(cfg: OriroConfig): SecurityAuditFinding[] {
...
export function collectElevatedFindings(cfg: OriroConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectYoloExecScopeIds(cfg: OriroConfig, approvals: ExecApprovalsFile): string[] {
...
export function collectExecRuntimeFindings(cfg: OriroConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
function collectOpenExecSurfacePaths(cfg: OriroConfig): string[] {
