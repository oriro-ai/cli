// Sync Plugin Versions script supports Oriro repository automation.
...
const ORIRO_VERSION_RANGE_RE = /^>=\d{4}\.\d{1,2}\.\d{1,2}(?:[-.][^"\s]+)?$/u;

function syncOriroDependencyRange(
  deps: Record<string, string> | undefined,
  targetVersion: string,
): boolean {
  const current = deps?.oriro;
  if (!current || current === "workspace:*" || !ORIRO_VERSION_RANGE_RE.test(current)) {
    return false;
  }
...
function syncPluginApiVersion(pkg: PackageJson, targetVersion: string): boolean {
  const compat = pkg.oriro?.compat;
  const current = compat?.pluginApi;
  if (!current || !ORIRO_VERSION_RANGE_RE.test(current)) {
    return false;
  }
  const next = `>=${targetVersion}`;
  if (current === next) {
...
function syncBuildOriroVersion(pkg: PackageJson, targetVersion: string): boolean {
  const build = pkg.oriro?.build;
  const current = build?.oriroVersion;
  if (!current) {
    return false;
  }
  ...
  build.oriroVersion = targetVersion;
  return true;
}
...
export function syncPluginVersions(
  ...
  for (const dir of dirs) {
    ...
    const devDependencyChanged = syncOriroDependencyRange(pkg.devDependencies, targetVersion);
    const peerDependencyChanged = syncOriroDependencyRange(pkg.peerDependencies, targetVersion);
    ...
    const buildOriroVersionChanged = syncBuildOriroVersion(pkg, targetVersion);
    const packageChanged =
      versionChanged ||
      devDependencyChanged ||
      peerDependencyChanged ||
      pluginApiChanged ||
      buildOriroVersionChanged;
    if (!packageChanged) {
