...
2. `npm i -g oriro@latest`
3. `oriro onboard --install-daemon`
...
Oriro installs a systemd **user** service by default. Use a **system**
service for shared or always-on servers. `oriro gateway install` and
`oriro onboard --install-daemon` already render the current canonical unit
...
Create `~/.config/systemd/user/oriro-gateway[-<profile>].service`:

```
[Unit]
Description=Oriro Gateway (profile: <profile>, v<version>)
...
sessions and channel connections. Oriro therefore biases transient child
processes to be killed before the Gateway when possible.

For eligible Linux child spawns, Oriro starts the child through a short
...
- Oriro-launched browser/Chrome processes.

The wrapper is Linux-only and is skipped when `/bin/sh` is unavailable. It is
also skipped if the child env sets `ORIRO_CHILD_OOM_SCORE_ADJ=0`, `false`,
`no`, or `off`.
