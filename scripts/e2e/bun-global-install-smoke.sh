...
BUN_BIN="${BUN_BIN:-bun}"
HOST_BUILD="${ORIRO_BUN_GLOBAL_SMOKE_HOST_BUILD:-1}"
DIST_IMAGE="${ORIRO_BUN_GLOBAL_SMOKE_DIST_IMAGE:-}"
PACKAGE_TGZ="${ORIRO_BUN_GLOBAL_SMOKE_PACKAGE_TGZ:-}"
...
resolve_package_tgz() {
  if [ -n "$PACKAGE_TGZ" ]; then
    if [ ! -f "$PACKAGE_TGZ" ]; then
      echo "ORIRO_BUN_GLOBAL_SMOKE_PACKAGE_TGZ does not exist: $PACKAGE_TGZ" >&2
      exit 1
    fi
  ...
  if [ -n "$DIST_IMAGE" ]; then
    ...
    echo "==> Skipping host build (ORIRO_BUN_GLOBAL_SMOKE_HOST_BUILD=0)"
  fi

  if [ ! -d "$ROOT_DIR/dist" ]; then
    echo "dist/ is missing; run pnpm build or set ORIRO_BUN_GLOBAL_SMOKE_DIST_IMAGE" >&2
    exit 1
  fi
...
main() {
  ...
  if [ ! -x "$oriro_bin" ]; then
    oriro_bin="$(command -v oriro || true)"
  fi
  if [ -z "$oriro_bin" ] || [ ! -x "$oriro_bin" ]; then
    echo "Bun global install did not create an executable oriro binary" >&2
    exit 1
  fi
