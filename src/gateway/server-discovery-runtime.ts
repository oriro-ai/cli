...
function resolveDiscoveryAdvertiseTimeoutMs(env: NodeJS.ProcessEnv): number {
  const raw = env.ORIRO_GATEWAY_DISCOVERY_ADVERTISE_TIMEOUT_MS?.trim();
  if (!raw) {
...
export async function startGatewayDiscovery(params: {
  ...
  const localDiscoveryEnabled =
    mdnsMode !== "off" &&
    !isTruthyEnvValue(process.env.ORIRO_DISABLE_BONJOUR) &&
  ...
  const sshPort = mdnsMinimal
    ? undefined
    : (parseTcpPort(process.env.ORIRO_SSH_PORT) ?? undefined);
