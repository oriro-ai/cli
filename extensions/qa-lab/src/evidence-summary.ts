...
export const QA_EVIDENCE_SUMMARY_KIND = "oriro.qa.evidence-summary";
...
export function resolveQaEvidenceProfile(params: {
  ...
  const envProfiles = [
    ["ORIRO_E2E_PROFILE", params.env?.ORIRO_E2E_PROFILE],
    ["ORIRO_QA_PROFILE", params.env?.ORIRO_QA_PROFILE],
  ] as const;
...
function resolveQaEvidenceRunner(params: { env?: NodeJS.ProcessEnv; fallback?: string }) {
  return params.env?.ORIRO_QA_RUNNER?.trim() || params.fallback || "host";
}

function resolveQaEvidenceChannelDriver(params: { env?: NodeJS.ProcessEnv; fallback?: string }) {
  const id =
    params.fallback?.trim() ||
    params.env?.ORIRO_QA_CHANNEL_DRIVER?.trim() ||
    params.env?.ORIRO_E2E_CHANNEL_DRIVER?.trim();
  return id ? { id } : undefined;
}

function resolveQaEvidenceEnvironment(env: NodeJS.ProcessEnv | undefined) {
  return {
    ref: env?.ORIRO_QA_REF?.trim() || env?.GITHUB_SHA?.trim() || null,
    os: process.platform,
...
function resolveQaEvidencePackageSource(env: NodeJS.ProcessEnv | undefined) {
  const spec = env?.ORIRO_QA_PACKAGE_SOURCE?.trim() || undefined;
  const sha = env?.ORIRO_QA_PACKAGE_SOURCE_SHA?.trim() || undefined;
  const explicitKind = env?.ORIRO_QA_PACKAGE_SOURCE_KIND?.trim();
  const kind =
    explicitKind ||
