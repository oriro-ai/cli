...
import type { AgentHarnessAttemptParams } from "oriro/plugin-sdk/agent-harness-runtime";
import { isLiveTestEnabled } from "oriro/plugin-sdk/test-env";
...
vi.mock("oriro/plugin-sdk/agent-harness", async (importOriginal) => {
  const actual = await importOriginal<typeof import("oriro/plugin-sdk/agent-harness")>();

  return {
    ...actual,
    createOriroCodingTools: vi.fn(() => [
      {
...
const LIVE = isLiveTestEnabled(["ORIRO_COPILOT_AGENT_LIVE_TEST"]);
const TOKEN =
  process.env.ORIRO_COPILOT_AGENT_LIVE_TOKEN ||
...
describeLive("copilot agent runtime live smoke", () => {
  it("runs one turn on gpt-4.1 with one custom tool", async () => {
    ...
    const copilotHome = await mkdtemp(join(tmpdir(), "oriro-copilot-live-"));
