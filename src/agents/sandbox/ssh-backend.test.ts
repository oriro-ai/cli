...
function createSession() {
  return {
    command: "ssh",
    configPath: path.join(os.tmpdir(), "oriro-test-ssh-config"),
    host: "oriro-sandbox",
  };
}
...
describe("ssh sandbox backend", () => {
  ...
  it("describes runtimes via the configured ssh target", async () => {
    const result = await sshSandboxBackendManager.describeRuntime({
      entry: {
        containerName: "oriro-ssh-worker-abcd1234",
        backendId: "ssh",
        runtimeLabel: "oriro-ssh-worker-abcd1234",
  ...
  it("creates a remote-canonical backend that seeds once and reuses ssh exec", async () => {
    ...
    const skillsWorkspaceDir = await createTempDir("oriro-ssh-skills-");
    ...
    expect(skillsUploadParams.remoteDir).toContain("/workspace/.oriro/sandbox-skills");
  ...
  it("clears stale remote materialized skills when the local copy is missing", async () => {
    const tmpDir = await createTempDir("oriro-ssh-skills-");
    ...
    expect(commandParams.remoteCommand).toContain("oriro-sandbox-clear");
    expect(commandParams.remoteCommand).toContain("/workspace/.oriro/sandbox-skills");
    await backend.finalizeExec?.({
      status: "completed",
      exitCode: 0,
      timedOut: false,
