...
describe("sessionsTailCommand", () => {
  ...
  beforeEach(() => {
    ...
    previousTrajectoryDir = process.env.ORIRO_TRAJECTORY_DIR;
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-sessions-tail-"));
    process.env.ORIRO_STATE_DIR = path.join(tmpDir, "state");
    delete process.env.ORIRO_TRAJECTORY_DIR;
  ...
  afterEach(() => {
    ...
    if (previousTrajectoryDir === undefined) {
      ...
      process.env.ORIRO_TRAJECTORY_DIR = previousTrajectoryDir;
    }
  ...
  it("uses a session trajectory pointer for relocated runtime files", async () => {
    ...
    fs.writeFileSync(
      ...
      `${JSON.stringify({
        traceSchema: "oriro-trajectory-pointer",
        schemaVersion: 1,
        sessionId: "session-one",
  ...
  it("resolves the target store from a fully qualified non-default agent session key", async () => {
    ...
    const opsSessionsDir = path.join(process.env.ORIRO_STATE_DIR!, "agents", "ops", "sessions");
