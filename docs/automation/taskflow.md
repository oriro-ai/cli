...
read_when:
  ...
  - You encounter Task Flow or oriro tasks flow in release notes or docs
...
3. Use [Oriro](/tools/oriro) for deterministic steps, approval gates, and resume tokens.
...
oriro cron add \
  ...
  --message "Run the market-intel Oriro workflow. Verify source freshness before summarizing." \
  --announce \
  --channel slack \
...
- Browser availability and profile choice, for example `oriro` for managed state or `user` when a signed-in Chrome session is required. See [Browser](/tools/browser).
- API credentials and quota for each source.
- Network reachability for required endpoints.
- Required tools enabled for the agent, such as `oriro`, `browser`, and `llm-task`.
...
For reusable team or community workflows, package the CLI, `.oriro` files, and any setup notes as a skill or plugin and publish it through [OriroHub](/orirohub). Keep workflow-specific guardrails in that package unless the plugin API is missing a needed generic capability.
...
`oriro tasks flow cancel` sets a sticky cancel intent on the flow. Active tasks within the flow are cancelled, and no new steps are started. The cancel intent persists across restarts, so a cancelled flow stays cancelled even if the gateway restarts before all child tasks have terminated.
...
| Command                           | Description                                   |
| --------------------------------- | --------------------------------------------- |
| `oriro tasks flow list`        | Shows tracked flows with status and sync mode |
| `oriro tasks flow show <id>`   | Inspect one flow by flow id or lookup key     |
| `oriro tasks flow cancel <id>` | Cancel a running flow and its active tasks    |

## How flows relate to tasks

Flows coordinate tasks, not replace them. A single flow may drive multiple background tasks over its lifetime. Use `oriro tasks` to inspect individual task records and `oriro tasks flow` to inspect the orchestrating flow.

## Related

- [Background Tasks](/automation/tasks) — the detached work ledger that flows coordinate
- [CLI: tasks](/cli/tasks) — CLI command reference for `oriro tasks flow`
