...
export function listAgentEntries(cfg: OriroConfig): AgentEntry[] {
...
export function listAgentIds(cfg: OriroConfig): string[] {
...
export function resolveDefaultAgentId(cfg: OriroConfig): string {
  const agents = listAgentEntries(cfg);
  if (agents.length === 0) {
    return DEFAULT_AGENT_ID;
  }
...
function resolveAgentEntry(cfg: OriroConfig, agentId: string): AgentEntry | undefined {
