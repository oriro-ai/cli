...
import { resolveOriroPackageRoot } from "./oriro-root.js";
...
function resolveAutoUpdatePolicy(cfg: OriroConfig): AutoUpdatePolicy {
...
function resolveCheckIntervalMs(cfg: OriroConfig): number {
...
export async function runGatewayUpdateCheck(params: {
  ...
  const autoDisabledByEnv = isTruthyEnvValue(process.env.ORIRO_NO_AUTO_UPDATE);
  ...
  if (cmp != null && cmp < 0) {
    ...
    if (shouldRunUpdateHints && shouldNotify) {
      params.log.info(
        `update available (${tag}): v${resolved.version} (current v${VERSION}). Run: ${formatCliCommand("oriro update")}`,
      );
    ...
    if (auto.enabled && autoDisabledByEnv) {
      params.log.info("auto-update disabled by ORIRO_NO_AUTO_UPDATE", {
