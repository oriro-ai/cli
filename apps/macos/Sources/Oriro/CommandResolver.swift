...
enum CommandResolver {
    private static let projectRootDefaultsKey = "oriro.gatewayProjectRootPath"
    private static let helperName = "oriro"
    ...
    static func gatewayEntrypoint(in root: URL) -> String? {
        ...
        let oriroEntry = root.appendingPathComponent("oriro.mjs").path
        if FileManager().isReadableFile(atPath: oriroEntry) { return oriroEntry }
        let binEntry = root.appendingPathComponent("bin/oriro.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/oriro")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let oriroPaths = self.oriroManagedPaths(home: home)
        if !oriroPaths.isEmpty {
            extras.insert(contentsOf: oriroPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + oriroPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func oriroManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".oriro"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func oriroExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectOriroExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("oriro.mjs").path,
            root.appendingPathComponent("bin/oriro.js").path,
        ]
    ...
    static func hasAnyOriroInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.oriroExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        projectRoot: URL? = nil) -> [String]
    {
        ...
        let root = projectRoot ?? self.projectRoot()
        if let oriroPath = self.projectOriroExecutable(projectRoot: root) {
            return [oriroPath, subcommand] + extraArgs
        }
        if let oriroPath = self.oriroExecutable(searchPaths: searchPaths) {
            return [oriroPath, subcommand] + extraArgs
        }
        ...
        if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
            ...
            return [pnpm, "--silent", "oriro", subcommand] + extraArgs
        }

        switch runtimeResult {
        case .success:
            let missingEntry = """
            oriro entrypoint missing (looked for dist/index.js or oriro.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real oriro CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/oriro"
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/oriro.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/oriro.mjs"
            node "$PRJ/oriro.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/oriro.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/oriro.js"
            node "$PRJ/bin/oriro.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent oriro"
          pnpm --silent oriro \(quotedArgs);
        else
          echo "oriro CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? OriroConfigFile.loadDict()
