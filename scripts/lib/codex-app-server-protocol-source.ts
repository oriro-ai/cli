// Codex App Server Protocol Source script supports Oriro repository automation.
...
export function resolveCodexProtocolMinFreeBytes(env: NodeJS.ProcessEnv = process.env): number {
  const raw = env.ORIRO_CODEX_PROTOCOL_MIN_FREE_BYTES;
  ...
  if (!Number.isFinite(parsed) || parsed < 0) {
    throw new Error(
      `ORIRO_CODEX_PROTOCOL_MIN_FREE_BYTES must be a non-negative byte count, got ${raw}`,
    );
  }
...
export function validateCodexProtocolGenerationHeadroom(params: {
  ...
  throw new Error(
    [
      ...
      "Run this check on Crabbox/Testbox, free local disk, or set ORIRO_CODEX_PROTOCOL_MIN_FREE_BYTES=0 to override intentionally.",
    ].join("\n"),
  );
}
