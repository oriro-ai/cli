...
function applyNativeControlAuth(host: SettingsHost) {
  const nativeAuth = window["__ORIRO_NATIVE_CONTROL_AUTH__"];
  if (!nativeAuth) {
    return;
  }
  try {
    delete window["__ORIRO_NATIVE_CONTROL_AUTH__"];
  } catch {
    window["__ORIRO_NATIVE_CONTROL_AUTH__"] = undefined;
  }
...
export function applySettingsFromUrl(host: SettingsHost) {
  ...
  if (hasTokenParam) {
    if (queryToken != null) {
      warnQueryToken = true;
      console.warn(
        "[oriro] Auth token passed as query parameter (?token=). Use URL fragment instead: #token=<token>. Query parameters may appear in server logs.",
      );
    }
...
export function inferBasePath() {
  ...
  const configured = window["__ORIRO_CONTROL_UI_BASE_PATH__"];
