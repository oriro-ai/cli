...
For session-backed agent runs, Oriro starts music generation as a
...
from the completion reply, Oriro sends an idempotent direct fallback with
...
<Note>
Not all providers support all parameters. Oriro still validates hard
limits such as input counts before submission. When a provider supports
duration but uses a shorter maximum than the requested value, Oriro
...
Provider request timeouts are operator configuration only. Oriro uses
...
- **Status lookup:** `oriro tasks list` or `oriro tasks show <taskId>`
  inspects queued, running, and terminal status.
- **Completion wake:** Oriro injects an internal completion event back
...
```bash
ORIRO_LIVE_TEST=1 pnpm test:live -- extensions/music-generation-providers.live.test.ts
```
