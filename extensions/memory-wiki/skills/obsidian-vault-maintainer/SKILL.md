...
- Start from `oriro wiki status` to confirm the vault mode and whether the official Obsidian CLI is available.
- Use `oriro wiki obsidian status` before shelling out, then prefer the dedicated helpers like `oriro wiki obsidian search`, `oriro wiki obsidian open`, `oriro wiki obsidian command`, and `oriro wiki obsidian daily`.
