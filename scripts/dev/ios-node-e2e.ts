// Ios Node E2E script supports Oriro repository automation.
...
function usage(): string {
  return [
    ...
    "Or set env: ORIRO_GATEWAY_URL / ORIRO_GATEWAY_TOKEN",
    "",
    "Options:",
...
const urlRaw = getArg("--url") ?? process.env.ORIRO_GATEWAY_URL;
const token = getArg("--token") ?? process.env.ORIRO_GATEWAY_TOKEN;
const nodeHint = getArg("--node");
const dangerous = hasFlag("--dangerous") || process.env.ORIRO_RUN_DANGEROUS === "1";
...
async function main() {
  ...
  const connectRes = await request("connect", {
    ...
    client: {
      id: "cli",
      displayName: "oriro ios node e2e",
      version: "dev",
      platform: "dev",
      mode: "cli",
      instanceId: "oriro-dev-ios-node-e2e",
    },
    locale: "en-US",
  ...
  const tests: TestCase[] = [
    ...
    { id: "device.status", command: "device.status" },
    {
      ...
      params: { title: "ORIRO E2E", body: `ios-node-e2e @ ${isoNow()}`, delivery: "system" },
    },
    {
      id: "contacts.search",
