...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.oriro", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> OriroSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: OriroSessionPreviewEntry,
    ...
    private static func cache(payload: OriroSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: OriroChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [OriroKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [OriroKit.AnyCodable]) -> [OriroChatMessage] {
    ...
    private static func previewText(for message: OriroChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: OriroChatMessage) -> Bool {
    ...
    private static func toolNames(for message: OriroChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: OriroChatMessage) -> String? {
