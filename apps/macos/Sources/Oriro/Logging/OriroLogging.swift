...
enum OriroLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = OriroOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = OriroFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        OriroLogging.bootstrapIfNeeded()
        let label = OriroLogging.makeLabel(subsystem: subsystem, category: category)
...
struct OriroOSLogHandler: AppLogLevelBackedHandler {
...
struct OriroFileLogHandler: AppLogLevelBackedHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = OriroLogging.parseLabel(self.label)
