...
function makeTempDir(): string {
  return tempDirs.make("oriro-plugin-lifecycle-probe-");
}
...
describe("plugin lifecycle matrix probe", () => {
  it("accepts inspect JSON for an enabled loaded plugin", async () => {
    ...
    writeFileSync(
      inspectPath,
      `${JSON.stringify({ plugin: { enabled: true, id: "lifecycle-oriro", status: "loaded" } })}\n`,
      "utf8",
    );

    expect(() => assertInspectLoaded("lifecycle-oriro", inspectPath)).not.toThrow();
  });
  ...
  it("rejects inspect JSON that does not prove the runtime loaded", async () => {
    ...
    writeFileSync(
      inspectPath,
      `${JSON.stringify({ plugin: { enabled: true, id: "lifecycle-oriro", status: "pending" } })}\n`,
      "utf8",
    );

    expect(() => assertInspectLoaded("lifecycle-oriro", inspectPath)).toThrow(
      "expected lifecycle-oriro inspect status loaded, got pending",
    );
  });
  ...
  it("rejects unreadable config during uninstall proof", async () => {
    const dir = makeTempDir();
    const configFile = path.join(dir, ".oriro", "oriro.json");
    mkdirSync(path.dirname(configFile), { recursive: true });
    writeFileSync(configFile, "{ malformed\n", "utf8");

    expect(() =>
      assertUninstalled("lifecycle-oriro", {
        HOME: dir,
  ...
  it("keeps fallback SIGKILL armed for ignored-stdio descendants", async () => {
    ...
    try {
      ...
      const parentScript = [
        ...
        "child.unref();",
        "writeFileSync(process.env.ORIRO_TEST_DESCENDANT_PID, String(child.pid));",
      ...
      const run = probeTesting.runCommand(
        ...
        {
          env: { ...process.env, ORIRO_TEST_DESCENDANT_PID: descendantPidPath },
