/** Installs native Node resolution aliases so plugins can import the Oriro SDK in dev and tests. */
...
export type InstallOriroPluginSdkNativeResolverOptions = {
  modulePath?: string;
...
const PLUGIN_SDK_PACKAGE_PREFIXES = ["oriro/plugin-sdk", "@oriro/plugin-sdk"] as const;
...
function resolveLoaderModulePath(options: InstallOriroPluginSdkNativeResolverOptions): string {
  ...
  for (let i = 0; i < 12; i += 1) {
    ...
    if (fs.existsSync(packageJsonPath)) {
      try {
        ...
        if (
          packageJson.name === "oriro" ||
          (typeof packageJson.bin === "object" &&
            packageJson.bin !== null &&
            typeof (packageJson.bin as { oriro?: unknown }).oriro === "string")
        ) {
          return cursor;
        }
      } catch {
...
function resolveAllowedParentRoots(
  options: InstallOriroPluginSdkNativeResolverOptions,
): string[] {
...
export function installOriroPluginSdkNativeResolver(
  options: InstallOriroPluginSdkNativeResolverOptions = {},
): string[] {
...
export function installOriroInternalCorePackageNativeResolver(
  options: Pick<InstallOriroPluginSdkNativeResolverOptions, "moduleUrl"> = {},
): string[] {
  for (const alias of listInternalCorePackageNativeAliases(options)) {
    registerNativeAlias(alias);
  }
...
export function resetOriroPluginSdkNativeResolverForTest(): void {
