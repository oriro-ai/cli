...
const ORIRO_MCP_TOOL_PREFIX = "mcp__oriro__";

function normalizeCliMessagingToolName(toolName: string): string {
  return toolName.startsWith(ORIRO_MCP_TOOL_PREFIX)
    ? toolName.slice(ORIRO_MCP_TOOL_PREFIX.length)
    : toolName;
}
...
const CLI_BACKEND_PRESERVE_ENV = "ORIRO_LIVE_CLI_BACKEND_PRESERVE_ENV";
...
function buildCliEnvMcpLog(childEnv: Record<string, string>): string {
  return [
    `token=${childEnv.ORIRO_MCP_TOKEN ? "set" : "missing"}`,
    `sessionKey=${childEnv.ORIRO_MCP_SESSION_KEY ? "set" : "<empty>"}`,
    `agentId=${childEnv.ORIRO_MCP_AGENT_ID || "<empty>"}`,
    `accountId=${childEnv.ORIRO_MCP_ACCOUNT_ID || "<empty>"}`,
    `messageChannel=${childEnv.ORIRO_MCP_MESSAGE_CHANNEL || "<empty>"}`,
  ].join(" ");
}
    ...
    completedOutput = await enqueueCliRun(queueKey, async () => {
      ...
      try {
        ...
        if (logOutputText) {
          ...
          if (env.ORIRO_MCP_TOKEN || env.ORIRO_MCP_SESSION_KEY || env.ORIRO_MCP_AGENT_ID) {
