...
<AccordionGroup>
  <Accordion title="Non-blocking, push-based completion">
    ...
    - On completion, Oriro best-effort closes tracked browser tabs/processes opened by that sub-agent session before the announce cleanup flow continues.

  </Accordion>
  <Accordion title="Completion delivery">
    - Oriro hands completions back to the requester session through an `agent` turn with a stable idempotency key.
    - If the requester run is still active, Oriro first tries to wake/steer that run instead of starting a second visible reply path.
    - If an active requester cannot be woken, Oriro falls back to a requester-agent handoff with the same completion context instead of dropping the announce.
  ...
  <Accordion title="Modes and ACP runtime">
    ...
    - Oriro hides `runtime: "acp"` until ACP is enabled, the requester is not sandboxed, and a backend plugin such as `acpx` is loaded. `runtime: "acp"` expects an external ACP harness id, or an `agents.list[]` entry with `runtime.type="acp"`; use the default sub-agent runtime for normal Oriro config agents from `agents_list`.
...
- **Run timeout:** Oriro uses `agents.defaults.subagents.runTimeoutSeconds` when set; otherwise it falls back to `0` (no timeout). `sessions_spawn` does not accept per-call timeout overrides.
...
When active children exist, Oriro injects a compact runtime-generated
...
<Steps>
  ...
  <Step title="Bind">
    Oriro creates or binds a thread to that session target in the active channel.
  </Step>
...
model and embedded runtime metadata so callers can distinguish Oriro, Codex
...
and `agents_list` omits it. Run `oriro doctor --fix` to clean stale
...
- If a nested requester subagent session is gone, Oriro falls back to that session's requester when available.
...
target agent first. After that, Oriro applies the sub-agent restriction
layer.
...
Oriro does not treat `endedAt` absence as permanent proof that a
...
rapid re-wedge window, Oriro persists a recovery tombstone on that
session and stops auto-resuming it on later restarts. Run
`oriro tasks maintenance --apply` to reconcile the task record, or
`oriro doctor --fix` to clear stale aborted recovery flags on
tombstoned sessions.
