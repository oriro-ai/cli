...
describe("registerQrCli", () => {
  ...
  beforeEach(() => {
    vi.clearAllMocks();
    resetRuntimeCapture();
    vi.stubEnv("ORIRO_GATEWAY_TOKEN", "");
    vi.stubEnv("ORIRO_GATEWAY_PASSWORD", "");
  ...
  it("renders ASCII QR by default", async () => {
    ...
    expect(output).toContain("oriro devices approve <requestId>");
  });
  ...
  it("uses ORIRO_GATEWAY_PASSWORD without resolving local password SecretRef", async () => {
    vi.stubEnv("ORIRO_GATEWAY_PASSWORD", "password-from-env");
    loadConfig.mockReturnValue(
      createLocalGatewayConfigWithAuth(
        createLocalGatewayPasswordRefAuth("MISSING_LOCAL_GATEWAY_PASSWORD"),
      ),
