...
describe("reparseProgramFromActionArgs", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    buildParseArgvMock.mockReturnValue(["node", "oriro", "status"]);
  ...
  it("uses action command name + args as fallback argv", async () => {
    ...
    setRawArgs(program, ["node", "oriro", "status", "--json"]);
    ...
    expect(buildParseArgvMock).toHaveBeenCalledWith({
      programName: "oriro",
      rawArgs: ["node", "oriro", "status", "--json"],
      fallbackArgv: ["status", "--json"],
    });
    expect(parseAsync).toHaveBeenCalledWith(["node", "oriro", "status"]);
  });

  it("falls back to action args without command name when action has no name", async () => {
    const program = new Command().name("oriro");
    setRawArgs(program, ["node", "oriro"]);
    const parseAsync = vi.spyOn(program, "parseAsync").mockResolvedValue(program);
    const actionCommand = {
      name: () => "",
      parent: program,
  ...
  it("uses root raw args and reparses the root for nested lazy commands", async () => {
    const root = new Command().name("oriro");
    setRawArgs(root, ["node", "oriro", "workspaces", "audit", "export", "--since", "1"]);
    ...
    expect(buildParseArgvMock).toHaveBeenCalledWith({
      programName: "oriro",
      rawArgs: ["node", "oriro", "workspaces", "audit", "export", "--since", "1"],
