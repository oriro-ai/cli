...
type ChannelIngressTestDatabase = Pick<OriroStateKyselyDatabase, "channel_ingress_events">;

async function withTempState<T>(fn: (stateDir: string) => Promise<T>): Promise<T> {
  const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-ingress-queue-"));
  try {
