...
/**
 * Runtime bridge between MCP tools and the Oriro Gateway channel APIs.
 *
...
export class OriroChannelBridge {
  ...
  constructor(
    private readonly cfg: OriroConfig,
  ...
  async handleClaudePermissionRequest(params: {
    ...
    if (this.verbose) {
      process.stderr.write(`oriro mcp: pending Claude permission ${params.requestId}\n`);
    }
  }
  ...
  private async sendNotification(notification: ServerNotification): Promise<void> {
    ...
    try {
      ...
      process.stderr.write(`oriro mcp: notification ${notification.method} failed\n`);
      if (this.verbose) {
        process.stderr.write(
          `oriro mcp: notification ${notification.method} error: ${String(error)}\n`,
        );
      }
    }
  }
