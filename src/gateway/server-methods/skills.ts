...
import { fetchOriroHubSkillDetail } from "../../infra/orirohub.js";
...
import {
  collectOriroHubVerdictTargets,
  fetchOriroSkillSecurityVerdicts,
} from "../../skills/security/orirohub-verdicts.js";
import {
  applySkillProposal,
...
export const skillsHandlers: GatewayRequestHandlers = {
  ...
  "skills.securityVerdicts": async ({ params, respond, context }) => {
    ...
    try {
      const report = buildRemoteAwareWorkspaceSkillStatus(resolved);
      const targets = collectOriroHubVerdictTargets(report);
      if (targets.length === 0) {
        respond(true, { schema: "oriro.skills.security-verdicts.v1", items: [] }, undefined);
        return;
      }
      const items = await fetchOriroSkillSecurityVerdicts(targets);
      respond(true, { schema: "oriro.skills.security-verdicts.v1", items }, undefined);
    } catch (err) {
  ...
  "skills.install": async ({ params, respond, context }) => {
    ...
    if (params && typeof params === "object" && "source" in params && params.source === "orirohub") {
      const p = params as {
        source: "orirohub";
        slug: string;
        version?: string;
    ...
    if (params && typeof params === "object" && "source" in params && params.source === "orirohub") {
      ...
      if (!p.slug && !p.all) {
        respond(
          false,
          undefined,
          errorShape(ErrorCodes.INVALID_REQUEST, 'orirohub skills.update requires "slug" or "all"'),
        );
        return;
      }
      if (p.slug && p.all) {
        respond(
          ...
          errorShape(
            ErrorCodes.INVALID_REQUEST,
            'orirohub skills.update accepts either "slug" or "all", not both',
          ),
        );
        return;
      }
