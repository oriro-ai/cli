...
import { oriroBannerBlock } from "../cli/brand-banner.js";
...
export function summarizeExistingConfig(config: OriroConfig): string {
  const rows: string[] = [];
...
function summarizeGatewayConfig(config: OriroConfig): string | null {
...
/** Prints the onboarding banner — the ORIRO wordmark in multi-bright color. */
export function printWizardHeader(runtime: RuntimeEnv) {
  runtime.log(`\n${oriroBannerBlock(isRich())}\n`);
}
...
export function formatControlUiSshHint(params: {
  ...
  return [
    ...
    "Docs:",
    "https://docs.oriro.ai/gateway/remote",
    "https://docs.oriro.ai/web/control-ui",
  ]
    .filter(Boolean)
    .join("\n");
}
