...
Create a Google Play service account JSON key with Google Play Developer API access, then grant that service account access to the Oriro app in Play Console.
...
```bash
GOOGLE_PLAY_PACKAGE_NAME=ai.oriro.app
```
...
ORIRO_ANDROID_UPLOAD_KEYSTORE=<path-to-upload-keystore.jks> \
ORIRO_ANDROID_SIGNING_PROPERTIES=<path-to-android-signing.properties> \
...
```properties
ORIRO_ANDROID_STORE_PASSWORD=<store-password>
ORIRO_ANDROID_KEY_ALIAS=<upload-key-alias>
ORIRO_ANDROID_KEY_PASSWORD=<key-password>
```
