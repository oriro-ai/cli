...
Linux desktop, VNC rescue, Slack Web, a real Oriro gateway, screenshots,
...
- Mantis artifacts: owned by the Oriro run. They live under
...
- remote logs such as `slack-desktop-command.log`, `oriro-gateway.log`,
...
- `crabbox.remote_run`: sync, browser launch, Oriro install/build or
...
remote status after Mantis has copied metadata proving that either the Oriro
