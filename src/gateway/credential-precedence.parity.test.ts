...
const gatewayEnv = {
  ORIRO_GATEWAY_TOKEN: "env-token", // pragma: allowlist secret
  ORIRO_GATEWAY_PASSWORD: "env-password", // pragma: allowlist secret
} as NodeJS.ProcessEnv;

function makeRemoteGatewayConfig(remote: { token?: string; password?: string }): OriroConfig {
  return {
    gateway: {
      mode: "remote",
      remote,
...
function withGatewayAuthEnv<T>(env: NodeJS.ProcessEnv, fn: () => T): T {
  return withEnv(
    {
      ORIRO_GATEWAY_TOKEN: env.ORIRO_GATEWAY_TOKEN,
      ORIRO_GATEWAY_PASSWORD: env.ORIRO_GATEWAY_PASSWORD,
      ORIRO_SERVICE_KIND: env.ORIRO_SERVICE_KIND,
    },
    fn,
  );
}
