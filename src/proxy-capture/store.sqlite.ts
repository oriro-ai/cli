...
import { normalizeNullableString as normalizeObservedValue } from "@oriro/normalization-core/string-coerce";
...
class DebugProxyCaptureStoreImpl {
  ...
  constructor(
    ...
    const database = openOriroStateDatabase({ env: optionsOrDbPath.env });
    this.db = database.db;
...
function resolveDebugProxyCaptureStoreKey(
  ...
  return typeof optionsOrDbPath === "string"
    ...
    : `shared:${openOriroStateDatabase({ env: optionsOrDbPath.env }).path}`;
}
