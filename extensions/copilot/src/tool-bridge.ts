...
type CreateOriroCodingTools =
  (typeof import("oriro/plugin-sdk/agent-harness"))["createOriroCodingTools"];
type OriroCodingToolsOptions = NonNullable<Parameters<CreateOriroCodingTools>[0]>;
 ...
 * authoritative `createOriroCodingTools({...})` call shape).
 *
 * Declared as `Partial<EmbeddedRunAttemptParams>` (imported from the
 * `oriro/plugin-sdk/agent-harness-runtime` boundary, *not* from
...
export interface CopilotToolBridgeInput {
   ...
   * model, and run-trace fields to `createOriroCodingTools` so the
  ...
  createOriroCodingTools?: (opts: unknown) => AnyAgentTool[] | Promise<AnyAgentTool[]>;
  beforeExecute?: (ctx: {
    toolName: string;
    toolCallId: string;
    args: unknown;
...
export async function createCopilotToolBridge(
  ...
  const createOriroCodingTools =
    input.createOriroCodingTools ??
    (await import("oriro/plugin-sdk/agent-harness")).createOriroCodingTools;

  const toolOptions = buildOriroCodingToolsOptions(input, effectiveToolPlan);

  let sourceTools: unknown;
  try {
    sourceTools = await createOriroCodingTools(toolOptions);
  } catch (error: unknown) {
    throw createError(
      `[copilot-tool-bridge] createOriroCodingTools failed: ${toError(error).message}`,
      error,
    );
  }
  ...
  if (!Array.isArray(sourceTools)) {
    throw new Error(
      "[copilot-tool-bridge] createOriroCodingTools must return an array of tools",
    );
  }
  ...
  return {
    sdkTools: filteredTools.map((sourceTool) =>
      convertOriroToolToSdkTool(sourceTool, {
...
/**
 * Builds the full `createOriroCodingTools` options bag mirroring the
 * PI in-tree call at `src/agents/pi-embedded-runner/run/attempt.ts:1029-1117`.
 *
 * Why PI parity matters: bridged Oriro tools register with the SDK
 * as `overridesBuiltInTool: true, skipPermission: true` (see
 * `convertOriroToolToSdkTool` below). That means the wrapped-tool
 * enforcement layer
...
function buildOriroCodingToolsOptions(
  ...
  const modelCompat =
    ...
    typeof model.compat === "object"
      ? (model.compat as OriroCodingToolsOptions["modelCompat"])
      : undefined;
...
export function convertOriroToolToSdkTool(
  sourceTool: AnyAgentTool,
  ctx: {
    abortSignal?: AbortSignal;
  ...
  return {
    ...
    // Oriro owns its bridged tools by design (the harness docs:
    // "Oriro still owns ... Oriro dynamic tools (bridged)"). The bundled
    // Copilot CLI ships built-in tools whose names (edit, read, write, bash,
    // ...) collide with Oriro's coding-tool set. Mark every bridged tool as
    ...
    // same name." Oriro's tool layer is the source of truth for these
    // names within a copilot attempt.
    overridesBuiltInTool: true,
    parameters: sourceTool.parameters as Record<string, unknown> | undefined,
    // Bridged Oriro tools enforce their own permission/policy decisions
    ...
    // `kind: "custom-tool"` would either short-circuit Oriro's richer
    // enforcement (if we allow-all) or block every call (if we
    // reject-all) — neither matches PI parity. The in-tree codex harness
    // takes the same approach: bridged Oriro tools are wrapped with
