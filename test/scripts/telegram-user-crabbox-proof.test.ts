...
function makeTempDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-telegram-proof-"));
  tempDirs.push(dir);
  return dir;
}
...
describe("telegram user Crabbox proof log polling", () => {
  it("starts the local gateway through the repo pnpm runner", () => {
    ...
    const spec = createOriroGatewaySpawnSpec({
      env: { ...process.env, ORIRO_TELEGRAM_PROOF_SENTINEL: "1" },
      gatewayPort: 19042,
    ...
    expect(spec.args).toEqual([fakePnpm, "oriro", "gateway", "--port", "19042"]);
    expect(spec.options.cwd).toBe(root);
    expect(spec.options.env?.ORIRO_TELEGRAM_PROOF_SENTINEL).toBe("1");
  ...
  it("rejects loose numeric log tail limits instead of parsing prefixes", () => {
    expect(() =>
      readTelegramUserProofLogTailBytes({
        ORIRO_TELEGRAM_USER_PROOF_LOG_TAIL_BYTES: "1e3",
      }),
    ).toThrow("invalid ORIRO_TELEGRAM_USER_PROOF_LOG_TAIL_BYTES: 1e3");
    expect(() =>
      readTelegramUserProofLogTailBytes({
        ORIRO_TELEGRAM_USER_PROOF_LOG_TAIL_BYTES: "1000bytes",
      }),
    ).toThrow("invalid ORIRO_TELEGRAM_USER_PROOF_LOG_TAIL_BYTES: 1000bytes");
    expect(
      readTelegramUserProofLogTailBytes({
        ORIRO_TELEGRAM_USER_PROOF_LOG_TAIL_BYTES: "4096",
      }),
    ).toBe(4096);
  });
  ...
  it("shell-quotes generated remote setup and chat literals", () => {
    const payload = "name $(touch /tmp/oriro-proof-injected) `touch /tmp/also-injected`";
  ...
  it("stages full publish artifacts without session control files", () => {
    ...
    fs.writeFileSync(path.join(outputDir, "session.json"), '{"sshKey":"/private/tmp/oriro/key"}');
  ...
  posixIt("does not expand generated remote probe arguments in the shell", () => {
    ...
    writeExecutable(
      ...
      `#!/usr/bin/env node
import fs from "node:fs";
fs.writeFileSync(process.env.ORIRO_TEST_ARGV_PATH, JSON.stringify(process.argv.slice(1)));
`,
    );
    writeExecutable(
      scriptPath,
