 ...
 * `__ORIRO_REDACTED__` literal anywhere in the output throws
...
export function emitMd(ast: MdAst, opts: EmitOptions = {}): string {
  ...
  if (mode === "roundtrip") {
    ...
    if (!acceptPreExisting && ast.raw.includes("__ORIRO_REDACTED__")) {
      guardSentinel("__ORIRO_REDACTED__", `${guardPath}/[raw]`);
    }
    return ast.raw;
  }
