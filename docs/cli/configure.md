---
summary: "CLI reference for `oriro configure` (interactive configuration prompts)"
read_when:
...
Use `oriro onboard` for the full guided first-run journey, `oriro setup` for the baseline config/workspace only, and `oriro channels add` when you only need channel account setup.
...
<Tip>
`oriro config` without a subcommand opens the same wizard. Use `oriro config get|set|unset` for non-interactive edits.
</Tip>

For web search, `oriro configure --section web` lets you choose a provider
...
oriro configure --section model --section channels
oriro configure --section gateway --section daemon
```
