...
describe("onboard-hooks", () => {
  ...
  const createMockHookReport = (eligible = true): HookStatusReport => ({
    ...
    managedHooksDir: "/mock/.oriro/hooks",
    hooks: [
      createMockHook(
        {
          name: "session-memory",
  ...
  describe("setupInternalHooks", () => {
    ...
    it("localizes built-in hook prompts when ORIRO_LOCALE is set", async () => {
      ...
      expect(noteCalls).toEqual([
        [
          [
            ...
            "",
            "Learn more: https://docs.oriro.ai/automation/hooks",
          ].join("\n"),
          "Hooks",
        ],
        [
