...
import { resolveClaudeFable5ModelIdentity } from "@oriro/llm-core";
...
let manifestModelCatalogCache = new WeakMap<OriroConfig, ManifestModelCatalogCacheEntry>();
...
function shouldLogModelCatalogTiming(): boolean {
  return process.env.ORIRO_DEBUG_INGRESS_TIMING === "1";
}
...
function hasConfiguredProviderRowsNeedingManifestLookup(cfg: OriroConfig): boolean {
...
export async function loadModelCatalog(params?: {
  ...
  const loadCatalog = async () => {
    ...
    try {
      ...
      if (!readOnly) {
        const preparedSource = await prepareOriroModelsJsonSource(cfg, agentDir, {
          pluginMetadataSnapshot: params?.metadataSnapshot,
          workspaceDir,
        });
