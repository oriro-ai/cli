...
function hasTrustedOriroRootIndicator(packageRoot, packageJson) {
  ...
  const hasOriroBin =
    (typeof packageJson?.bin === "string" &&
      normalizeLowercaseStringOrEmpty(packageJson.bin).includes("oriro")) ||
    (typeof packageJson?.bin === "object" &&
      packageJson.bin !== null &&
      typeof packageJson.bin.oriro === "string");
  const hasOriroEntrypoint = fs.existsSync(path.join(packageRoot, "oriro.mjs"));
  return hasCliEntryExport || hasOriroBin || hasOriroEntrypoint;
}

function findOriroPackageRoot(startDir) {
  let cursor = path.resolve(startDir);
  for (let i = 0; i < 12; i += 1) {
    const pkg = readPackageJson(cursor);
    if (pkg?.name === "oriro" && hasTrustedOriroRootIndicator(cursor, pkg)) {
...
function resolveBundledPluginRuntimeModulePath(moduleUrl, params) {
  ...
  const location = findOriroPackageRoot(moduleDir);
  if (location) {
