...
const mocks = vi.hoisted(() => ({
  loadOriroPlugins: vi.fn<typeof import("../loader.js").loadOriroPlugins>(),
...
function loadOptions(index = 0) {
  return requireRecord(mocks.loadOriroPlugins.mock.calls[index]?.[0], `load options ${index}`);
}
...
vi.mock("../loader.js", () => ({
  loadOriroPlugins: (...args: Parameters<typeof mocks.loadOriroPlugins>) =>
    mocks.loadOriroPlugins(...args),
  resolveCompatibleRuntimePluginRegistry: (
...
describe("ensurePluginRegistryLoaded", () => {
  ...
  beforeEach(() => {
    ...
    mocks.loadOriroPlugins.mockReturnValue(createEmptyPluginRegistry());
    mocks.resolveRuntimePluginRegistry.mockImplementation(
      (...args: Parameters<typeof mocks.loadOriroPlugins>) => mocks.loadOriroPlugins(...args),
    );
  ...
  it("uses the shared runtime load context for configured-channel loads", () => {
    ...
    const env = { HOME: "/tmp/oriro-home" } as NodeJS.ProcessEnv;
  ...
  it("does not reuse non-empty all-scope registries without loader compatibility", () => {
    ...
    mocks.loadOriroPlugins.mockClear();
