---
summary: "Oriro code mode: an opt-in exec/wait tool surface backed by QuickJS-WASI and a hidden run-scoped tool catalog"
title: "Code mode"
sidebarTitle: "Code mode"
read_when:
  - You want to enable Oriro code mode for an agent run
...
Code mode is an experimental Oriro agent-runtime feature. It is off by
default. When you enable it, Oriro changes what the model sees for one run:
instead of exposing every enabled tool schema directly, the model sees only
`exec` and `wait`.

This page documents Oriro code mode. It is not Codex Code mode. The two
...
- Oriro code mode is disabled unless `tools.codeMode.enabled: true` is
  configured. It runs in the Oriro generic agent runtime, where the model
...
surfaces. Oriro code mode is an Oriro-owned experimental tool-surface
adapter for generic Oriro runs. It uses `quickjs-wasi`, a hidden Oriro
tool catalog, and the normal Oriro tool executor.

## What is this?

Oriro code mode lets the model write a small JavaScript or TypeScript program
...
- `exec` evaluates model-generated JavaScript or TypeScript in a constrained
  QuickJS-WASI worker.
- Normal Oriro tools are hidden from the model prompt and exposed inside the
  guest program through `ALL_TOOLS` and `tools`.
- Guest code can search the hidden catalog, describe a tool, and call a tool
  through the same Oriro execution path used by normal agent turns.
...
surface. It does not replace Oriro tools, plugin tools, MCP tools, auth,
...
- Provider neutral: it works for Oriro, plugin, MCP, and client tools without
  depending on provider-native code execution.
- Existing policy stays in force: nested tool calls still go through Oriro
...
- Clear failure mode: when code mode is explicitly enabled and the runtime is
  unavailable, Oriro fails closed instead of falling back to broad direct tool
  exposure.
...
```bash
ORIRO_DEBUG_CODE_MODE=1 \
ORIRO_DEBUG_MODEL_TRANSPORT=1 \
ORIRO_DEBUG_MODEL_PAYLOAD=tools \
oriro gateway
```
...
`ORIRO_DEBUG_MODEL_PAYLOAD=full-redacted` for a short debugging session.
...
- Stability: experimental Oriro surface; Codex Code mode is a separate stable
  Codex harness surface.
- Target surface: generic Oriro agent runs.
...
**Code mode** is the Oriro runtime mode that hides normal model tools and
...
If code mode is enabled but QuickJS-WASI cannot load, Oriro fails closed for
...
2. Build the effective Oriro tool list.
...
- If `language` is `"typescript"`, Oriro transpiles before evaluation.
...
final value is JSON-compatible after Oriro's output adapter runs.
...
`wait` exists because nested Oriro tools can be slow, interactive, approval
...
2. On suspension, Oriro snapshots the QuickJS VM and records pending host
   work.
3. When pending work settles, `wait` restores the VM snapshot.
4. Oriro re-registers host callbacks by stable names.
5. Oriro delivers nested tool results into the restored VM.
6. Oriro drains QuickJS pending jobs.
...
type ToolCatalogEntry = {
  ...
  label?: string;
  description: string;
  source: "oriro" | "plugin" | "mcp" | "client";
  sourceName?: string;
};
```
...
const hits = await tools.web_search({ query: "Oriro code mode" });
```
...
state directory. For each code-mode `exec` call, Oriro builds the run-scoped
...
inside the QuickJS program while Oriro still shows only `exec` and `wait` to
the model.
...
7. Oriro auto-drains ready namespace bridge calls inside the active
...
- `globalName` collisions with built-in code-mode globals such as `tools`,
  `namespaces`, `text`, `json`, `yield_control`, or `__oriro*`
...
catalog for arbitrary enabled Oriro, plugin, and client tools; use `MCP` for
...
- images and files travel through ordinary Oriro tools, not through the
  code-mode bridge
...
Code mode supersedes the Oriro Tool Search model surface for runs where it is
active.

When `tools.codeMode.enabled` is true and code mode activates:

- Oriro does not expose `tool_search_code`, `tool_search`, `tool_describe`,
...
- MCP calls use the generated `MCP` namespace and its `$api()` headers instead
  of `tools.call(...)`.
- Nested calls dispatch through the same Oriro executor path that Tool Search
  uses.

The existing [Tool Search](/tools/tool-search) page describes the Oriro compact
catalog bridge. Code mode is the generic Oriro alternative for runs that can
use `exec` and `wait`.

## Tool names and collisions

The model-visible `exec` tool is the code-mode tool. If the normal Oriro
shell `exec` tool is enabled, it is hidden from the model and cataloged like any
other tool.

Inside the guest runtime:

- `tools.call("oriro:core:exec", input)` can call the shell exec tool if
  policy allows it.
...
If two tools normalize to the same safe convenience name, Oriro omits the
convenience function and requires `tools.call(id, input)`.

## Nested tool execution

Every nested tool call crosses the host bridge and re-enters Oriro.
...
Oriro loads `quickjs-wasi` as a direct dependency in the owning package. The
...
The runtime executes outside Oriro's main event loop in a worker. A guest
...
inputs beyond existing Oriro trajectory policy.
...
ORIRO_DEBUG_MODEL_TRANSPORT=1 \
ORIRO_DEBUG_MODEL_PAYLOAD=tools \
ORIRO_DEBUG_SSE=events \
oriro gateway
```

For payload-shape debugging, use `ORIRO_DEBUG_MODEL_PAYLOAD=full-redacted`.
...
For stream debugging, use `ORIRO_DEBUG_SSE=peek` to log the first five
...
- `tools.search`, `tools.describe`, and `tools.call` work for Oriro tools
...
5. Send an agent turn with Oriro, plugin, MCP, and client test tools.
...
8. In `exec`, call Oriro/plugin/client tools through `tools.search`,
