...
import { createOriroCodingTools } from "../../agents/agent-tools.js";
...
const { createOriroCodingToolsMock } = vi.hoisted(() => ({
  createOriroCodingToolsMock: vi.fn(() => []),
}));
...
vi.mock("../../agents/agent-tools.js", () => ({
  createOriroCodingTools: createOriroCodingToolsMock,
}));
...
describe("resolveCommandsSystemPromptBundle", () => {
  beforeEach(() => {
    vi.clearAllMocks();
    createOriroCodingToolsMock.mockClear();
  ...
  it("opts command tool builds into gateway subagent binding", async () => {
    ...
    const toolParams = requireFirstArg(
      vi.mocked(createOriroCodingTools),
  ...
  it("uses materialized sandbox skill paths for sandbox command prompts", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-command-sandbox-skills-"));
    try {
      ...
      vi.mocked(resolveReusableWorkspaceSkillSnapshot).mockReturnValue({
        snapshot: {
          prompt:
            "<available_skills>~/.npm-global/lib/node_modules/oriro/skills/gog/SKILL.md</available_skills>",
          skills: [],
          resolvedSkills: [],
        },
        shouldRefresh: false,
  ...
  it("uses config-backed prompt settings for the target agent", async () => {
    ...
    createOriroCodingToolsMock.mockReturnValue([{ name: "sessions_spawn" }] as never);
    const params = makeParams();
    params.cfg = {
      agents: {
        defaults: {
