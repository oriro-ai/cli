---
summary: "CLI reference for `oriro devices` (device pairing + token rotation/revocation)"
read_when:
...
```
oriro devices remove <deviceId>
oriro devices remove <deviceId> --json
```

### `oriro devices clear --yes [--pending]`

Clear paired devices in bulk.

```
oriro devices clear --yes
oriro devices clear --yes --pending
oriro devices clear --yes --pending --json
```

### `oriro devices approve [requestId] [--latest]`

Approve a pending device pairing request by exact `requestId`. If `requestId`
is omitted or `--latest` is passed, Oriro only prints the selected pending
request and exits; rerun approval with the exact request ID after verifying
the details.

<Note>
If a device retries pairing with changed auth details (role, scopes, or public key), Oriro supersedes the previous pending entry and issues a new `requestId`. Run `oriro devices list` right before approval to use the current ID.
</Note>

If the device is already paired and asks for broader scopes or a broader role,
Oriro keeps the existing approval in place and creates a new pending upgrade
request. Review the `Requested` vs `Approved` columns in `oriro devices list`
or use `oriro devices approve --latest` to preview the exact upgrade before
approving it.
...
```
oriro devices approve
oriro devices approve <requestId>
oriro devices approve --latest
```

## Paperclip / `oriro_gateway` first-run approval

When a new Paperclip agent connects through the `oriro_gateway` adapter for the first time, the Gateway may require a one-time device pairing approval before runs can succeed. If Paperclip reports `oriro_gateway_pairing_required`, approve the pending device and retry.
...
The preview prints the exact `oriro devices approve <requestId>` command. Verify the request details, then rerun that command with the request ID to approve it.

For remote gateways or explicit credentials, pass the same options while previewing and approving:

```bash
oriro devices approve --latest --url <gateway-ws-url> --token <gateway-token>
```
...
If approval keeps failing, run `oriro devices list` first to confirm a pending request exists.
...
### `oriro devices rotate --device <id> --role <role> [--scope <scope...>]`
...
```
oriro devices rotate --device <deviceId> --role operator --scope operator.read --scope operator.write
```
...
### `oriro devices revoke --device <id> --role <role>`
...
```
oriro devices revoke --device <deviceId> --role node
```
...
```bash
oriro devices rotate --device <deviceId> --role operator
```
