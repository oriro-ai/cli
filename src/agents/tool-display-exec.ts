 ...
 */
import { asOptionalObjectRecord as asRecord } from "@oriro/normalization-core/record-coerce";
...
function summarizeKnownExec(words: string[]): string {
  ...
  if (bin === "oriro") {
    ...
    return sub ? `run oriro ${sub}` : "run oriro";
  }
...
function classifyWorkspacePath(
  ...
  for (let index = 0; index < segments.length; index += 1) {
    ...
    if (segment === ".oriro" && segments[index + 1] === "workspace") {
      return "agent";
    }
    if (segment === ".oriro" && segments[index + 1] === "sandboxes") {
      return "sandbox";
    }
