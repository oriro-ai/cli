...
const CODEX_HARNESS_LIVE = process.env.ORIRO_LIVE_CODEX_HARNESS === "1";
const CODEX_HARNESS_DEBUG = process.env.ORIRO_LIVE_CODEX_HARNESS_DEBUG === "1";
...
function snapshotEnv(): LiveEnvSnapshot {
  return snapshotLiveEnv(["ORIRO_TRAJECTORY", "ORIRO_TRAJECTORY_DIR"]);
}
...
describeLive("gateway live trajectory export", () => {
  ...
  it(
    ...
    async () => {
      ...
      const tempDir = await fs.mkdtemp(path.join(process.cwd(), ".tmp-oriro-trajectory-live-"));
      cleanup.push(async () => {
      ...
      process.env.ORIRO_TRAJECTORY = "1";
      process.env.ORIRO_TRAJECTORY_DIR = trajectoryDir;
      ...
      const bundleDir = path.join(workspaceDir, ".oriro", "trajectory-exports", "bundle");
