...
import {
  ...
  normalizeProviderId,
} from "@oriro/model-catalog-core/provider-id";
...
function isOpenAICompatibleMemoryProvider(providerId: string, cfg: OriroConfig): boolean {
...
function isKeyOptionalMemoryProvider(providerId: string, cfg: OriroConfig): boolean {
  return (
    providerId === "local" ||
    providerId === "ollama" ||
    providerId === "lmstudio" ||
...
function buildMemoryRecallIssueNote(audit: ShortTermAuditSummary): string | null {
  ...
  const guidance = hasFixableIssue
    ? `Fix: ${formatCliCommand("oriro doctor --fix")} or ${formatCliCommand("oriro memory status --fix")}`
    : `Verify: ${formatCliCommand("oriro memory status --deep")}`;
  return [
...
export async function noteMemoryRecallHealth(cfg: OriroConfig): Promise<void> {
  try {
...
function hasActiveAlternateMemoryPluginSlot(cfg: OriroConfig): boolean {
 ...
 * Runs as part of `oriro doctor` — config-only checks where possible;
...
export async function noteMemorySearchHealth(
      ...
      note(
        [
          ...
          workspaceProbeFailed
            ...
            : `- Set an explicit binary path: ${formatCliCommand("oriro config set memory.qmd.command /absolute/path/to/qmd")}`,
          `- Or switch back to builtin memory: ${formatCliCommand("oriro config set memory.backend builtin")}`,
          "",
          `Verify: ${formatCliCommand("oriro memory status --deep")}`,
        ]
          .filter(Boolean)
  ...
  if (provider === "local") {
    ...
    note(
      [
        ...
        "",
        "Fix (pick one):",
        `- Install the llama.cpp provider plugin: ${formatCliCommand("oriro plugins install @oriro/llama-cpp-provider")}`,
        `- Set a local GGUF model path in config`,
        suggestedRemoteProvider
          ? `- Switch to a remote provider: ${formatCliCommand(`oriro config set agents.defaults.memorySearch.provider ${suggestedRemoteProvider}`)}`
  ...
  if (
    ...
    note(
      [
        ...
        "",
        "Fix:",
        `- ${formatCliCommand("oriro config set agents.defaults.memorySearch.remote.baseUrl http://127.0.0.1:1234/v1")}`,
        "",
        `Verify: ${formatCliCommand("oriro memory status --deep")}`,
      ].join("\n"),
      "Memory search",
  ...
  if (isOpenAICompatibleMemoryProvider(provider, cfg) && !normalizeOptionalString(resolved.model)) {
    note(
      [
        ...
        "",
        "Fix:",
        `- ${formatCliCommand("oriro config set agents.defaults.memorySearch.model text-embedding-bge-m3")}`,
        "",
        `Verify: ${formatCliCommand("oriro memory status --deep")}`,
      ].join("\n"),
      "Memory search",
  ...
  if (isKeyOptionalMemoryProvider(provider, cfg)) {
    ...
    // not warn. A skipped probe means the user ran `oriro doctor` without
  ...
  note(
    [
      ...
      `- Configure credentials: ${formatCliCommand("oriro configure --section model")}`,
      `- To disable: ${formatCliCommand("oriro config set agents.defaults.memorySearch.enabled false")}`,
      "",
      `Verify: ${formatCliCommand("oriro memory status --deep")}`,
    ].join("\n"),
    "Memory search",
