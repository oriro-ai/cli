...
beforeAll(() => {
  envSnapshot = captureEnv(["ORIRO_PROFILE"]);
  process.env.ORIRO_PROFILE = "isolated";
});
...
async function createStatusServiceSummary(
  ...
  return {
    ...
    loaded,
    managedByOriro: Boolean(command),
  ...
  return {
    ...
    update: {
      ...
      deps: {
        manager: "pnpm",
        status: "ok",
        lockfilePath: "/tmp/oriro/pnpm-lock.yaml",
        markerPath: "/tmp/oriro/node_modules/.modules.yaml",
      },
    ...
    gatewayProbeAuth: process.env.ORIRO_GATEWAY_TOKEN
...
const mocks = vi.hoisted(() => ({
  ...
  resolveGatewayService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.oriro.gateway.plist",
    }),
  }),
  resolveNodeService: vi.fn().mockReturnValue({
    ...
    readCommand: async () => ({
      ...
      sourcePath: "/tmp/Library/LaunchAgents/ai.oriro.node.plist",
    }),
  }),
}));
...
vi.mock("../gateway/call.js", () => ({
  ...
  resolveGatewayCredentialsWithSecretInputs: vi.fn(
    async (params: {
      ...
      const envToken = process.env.ORIRO_GATEWAY_TOKEN?.trim();
...
vi.mock("../infra/oriro-root.js", () => ({
  resolveOriroPackageRoot: vi.fn().mockResolvedValue("/tmp/oriro"),
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines with verbose cache details", async () => {
    ...
    expectLogsMatch(logs, /oriro (?:--profile isolated )?status --all/);
  ...
  it("shows node-only gateway info when no local gateway service is installed", async () => {
    ...
    expect(joined).toContain("oriro --profile isolated node status");
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    await withEnvVar("ORIRO_GATEWAY_TOKEN", "abcd1234", async () => {
