---
name: oriro-testing
description: Choose, run, rerun, or debug Oriro tests, CI checks, Docker E2E lanes, release validation, and the cheapest safe verification path.
---
...
--provider blacksmith-testbox ... -- env ORIRO_CHECK_CHANGED_REMOTE_CHILD=1 ORIRO_CHANGED_LANES_RAW_SYNC=1 corepack pnpm check:changed`. For direct AWS
...
ORIRO_TEST_CHANGED_BROAD=1 pnpm test:changed
pnpm test <path-or-filter> -- --reporter=verbose
ORIRO_VITEST_MAX_WORKERS=1 pnpm test <path-or-filter>
```
...
- `ORIRO_TEST_CHANGED_BROAD=1 pnpm test:changed` is the explicit broad
...
- Use `ORIRO_TEST_CHANGED_BROAD=1 pnpm test:changed` only when a harness,
...
`pnpm oriro qa parity-report`. For parity failures, inspect the failed lane
first; inspect the report job when both lane summaries exist but the comparison
fails.

### QA Lab Matrix Profiles

`pnpm oriro qa matrix` defaults to `--profile all`. Do not assume the CLI
...
- `--profile transport|media|e2ee-smoke|e2ee-deep|e2ee-cli`: sharded full
  Matrix proof
- `ORIRO_QA_MATRIX_NO_REPLY_WINDOW_MS=3000`: CI-friendly no-reply quiet
...
- `native-live-src-gateway-profiles` (release CI runs this with provider
  filters such as `ORIRO_LIVE_GATEWAY_PROVIDERS=anthropic`)
...
`ORIRO_LIVE_PROVIDERS` and selected model ids before declaring proof.
...
```bash
ORIRO_DOCKER_ALL_DRY_RUN=1 pnpm test:docker:all
ORIRO_DOCKER_ALL_DRY_RUN=1 ORIRO_DOCKER_ALL_LANES=install-e2e pnpm test:docker:all
ORIRO_DOCKER_ALL_LANES=install-e2e node scripts/test-docker-all.mjs --plan-json
```
...
```bash
ORIRO_DOCKER_ALL_LANES=<lane> \
ORIRO_DOCKER_ALL_BUILD=0 \
ORIRO_DOCKER_ALL_PREFLIGHT=0 \
ORIRO_SKIP_DOCKER_BUILD=1 \
ORIRO_DOCKER_E2E_BARE_IMAGE='<prepared-bare-image>' \
ORIRO_DOCKER_E2E_FUNCTIONAL_IMAGE='<prepared-functional-image>' \
...
- Resolve the registry immediately before dispatch:
  `npm view oriro dist-tags --json --prefer-online --cache /tmp/oriro-npm-cache-verify-$$`
  and `npm view oriro@beta version dist.tarball dist.integrity --json --prefer-online --cache /tmp/oriro-npm-cache-verify-$$`.
- If Peter asks for "latest beta", use `source=npm` with
  `package_spec=oriro@beta`, then record the resolved version from `npm view`
  or the workflow summary.
- For reruns, release proof, or comparing one known package, prefer the exact
  immutable spec: `package_spec=oriro@YYYY.M.D-beta.N` or
  `package_spec=oriro@YYYY.M.D`.
- For stable package proof, use `package_spec=oriro@latest` only when the
  question is explicitly the current stable dist-tag; otherwise pin the exact
  version.
- `source=npm` only accepts registry specs for `oriro@beta`,
  `oriro@latest`, or exact Oriro release versions. Do not pass semver
...
- Package acceptance tests exactly the selected package candidate. Do not apply
  `oriro update --channel beta` fallback semantics here; if `beta` is absent,
...
- `source=npm`: `oriro@beta`, `oriro@latest`, or an exact release version.
...
- `package_ref` is the source ref to build when `source=ref`. It can be an
  older branch/tag/SHA as long as it is reachable from an Oriro branch or
  release tag.
...
tarball that bare lanes mount. `scripts/package-oriro-for-docker.mjs` is the
...
`docker_lanes=skill-install` for live OriroHub skill-install validation. The
...
from `oriro skills search`, installs it, and verifies `.orirohub` origin/lock
