...
describe("shouldSkipRespawnForArgv", () => {
  it.each([
    ...
    { argv: ["node", "oriro", "-V"] },
    { argv: ["node", "oriro", "tui"] },
    { argv: ["node", "oriro", "terminal"] },
    { argv: ["node", "oriro", "chat"] },
    { argv: ["node", "oriro", "gateway"] },
    { argv: ["node", "oriro", "gateway", "--port", "14720", "--bind", "loopback"] },
    { argv: ["node", "oriro", "gateway", "run", "--port=14720", "--bind", "loopback"] },
    {
      argv: ["node", "oriro", "--profile", "server", "gateway", "run", "--allow-unconfigured"],
    },
  ...
  it.each([
    ...
    { argv: ["node", "oriro", "gateway", "status"] },
    { argv: ["node", "oriro", "gateway", "call", "health"] },
...
describe("shouldSkipStartupEnvironmentRespawnForArgv", () => {
  it.each([
    { argv: ["node", "oriro", "--help"] },
    { argv: ["node", "oriro", "gateway"] },
    { argv: ["node", "oriro", "gateway", "run", "--port=14720"] },
...
describe("dns cli", () => {
  it("prints setup info (no apply)", async () => {
    ...
    try {
      ...
      await program.parseAsync(["dns", "setup", "--domain", "oriro.internal"], { from: "user" });
      const output = log.mock.calls.map((call) => call.join(" ")).join("\\n");
      expect(output).toContain("DNS setup");
      expect(output).toContain("oriro.internal");
    } finally {
      log.mockRestore();
    }
  });
