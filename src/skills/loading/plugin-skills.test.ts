    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.acpxRoot, "oriro.plugin.json"),
      },
      {
        id: "helper",
        name: "Helper",
        ...
        manifestPath: path.join(params.helperRoot, "oriro.plugin.json"),
      },
    ],
  };
}
    ...
    plugins: [
      {
        ...
        manifestPath: path.join(params.pluginRoot, "oriro.plugin.json"),
      },
    ],
  };
}
...
async function setupAcpxAndHelperRegistry() {
  const workspaceDir = await tempDirs.make("oriro-");
  const acpxRoot = await tempDirs.make("oriro-acpx-plugin-");
  const helperRoot = await tempDirs.make("oriro-helper-plugin-");
...
async function setupPluginOutsideSkills() {
  const workspaceDir = await tempDirs.make("oriro-");
  const pluginRoot = await tempDirs.make("oriro-plugin-");
  const outsideDir = await tempDirs.make("oriro-outside-");
...
describe("resolvePluginSkillDirs", () => {
  ...
  it("resolves Claude bundle command roots through the normal plugin skill path", async () => {
    const workspaceDir = await tempDirs.make("oriro-");
    const pluginRoot = await tempDirs.make("oriro-claude-bundle-");
  ...
  it("resolves enabled plugin skills through legacy manifest aliases", async () => {
    const workspaceDir = await tempDirs.make("oriro-");
    const pluginRoot = await tempDirs.make("oriro-legacy-plugin-");
    await fs.mkdir(path.join(pluginRoot, "skills"), { recursive: true });
