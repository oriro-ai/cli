...
describe("live transport artifacts", () => {
  ...
  it("preserves cleanup phase labels while redacting details", () => {
    expect(
      ...
      "credential lease release: details redacted (ORIRO_QA_REDACT_PUBLIC_METADATA=1)",
      "live gateway cleanup: details redacted (ORIRO_QA_REDACT_PUBLIC_METADATA=1)",
    ]);
  });
    ...
    expect(
      redactQaLiveLaneIssues([
        [
          ...
          "Artifacts:",
          "- gatewayDebug: /tmp/oriro-whatsapp-qa/gateway-debug",
        ].join("\n"),
      ]),
    ).toEqual(["details redacted (ORIRO_QA_REDACT_PUBLIC_METADATA=1)"]);
  });
});
