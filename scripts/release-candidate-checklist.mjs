...
function githubApiTimeoutMs() {
  const raw = process.env.ORIRO_RELEASE_CANDIDATE_GITHUB_API_TIMEOUT_MS;
  if (!raw) {
    return DEFAULT_GITHUB_API_TIMEOUT_MS;
  }
  const value = Number(raw);
  if (!Number.isFinite(value) || value <= 0) {
    throw new Error("ORIRO_RELEASE_CANDIDATE_GITHUB_API_TIMEOUT_MS must be a positive number");
  }
...
export function buildPublishCommand(options) {
  const fields = [
    ...
    ["publish_oriro_npm", "true"],
    ["release_profile", "from-validation"],
    ["wait_for_orirohub", "false"],
  ];
...
async function runTelegramIfNeeded(options, artifactName) {
  ...
  const runId = dispatchWorkflow(options.repo, workflowFile, options.workflowRef, {
    package_spec: `oriro@${options.tag.replace(/^v/u, "")}`,
...
async function main() {
  ...
  if (!options.npmPreflightRunId && !options.skipDispatch) {
    const workflowFile = "oriro-npm-release.yml";
  ...
  const npmArtifactName = await downloadResolvedArtifact(
    ...
    `oriro-npm-preflight-${options.tag}`,
    "oriro-npm-preflight-",
    npmDir,
  );
  ...
  const pluginOriroHubPlan = await collectPluginPlanWithRetry(
  ...
  writeFileSync(
    ...
    [
      ...
      `- OriroHub plan: ${pluginOriroHubPlan.packages?.length ?? 0} packages`,
