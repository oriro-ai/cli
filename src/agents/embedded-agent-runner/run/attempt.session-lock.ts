...
type PromptReleaseStreamFn = ((...args: unknown[]) => unknown) & {
  __oriroSessionLockPromptReleaseInstalled?: boolean;
};
...
function parsePromptReleasedMessageLine(
  ...
  try {
    ...
    const isOriroTranscriptOnlyAssistant = isTranscriptOnlyOriroAssistantMessage(message);
    if (
      typeof message.role !== "string" ||
      (!options?.allowAnyMessage && !isOriroTranscriptOnlyAssistant)
    ) {
      return undefined;
    }
    return {
...
// Controllers in the same Oriro process can legitimately take turns writing
// the same session file while another attempt is released for model I/O. Track
// only fingerprints that changed while Oriro held the write lock so the
...
const EMBEDDED_ATTEMPT_SESSION_FILE_OWNER_STATE_KEY = Symbol.for(
  "oriro.embeddedAttemptSessionFileOwnerState",
);
...
export function installPromptSubmissionLockRelease(params: {
  ...
  if (currentStreamFn["__oriroSessionLockPromptReleaseInstalled"] === true) {
    return;
  }
  ...
  wrappedStreamFn["__oriroSessionLockPromptReleaseInstalled"] = true;
