...
export function testLiveUsage() {
  return [
    ...
    "",
    "Runs live Vitest suites with ORIRO_LIVE_TEST=1.",
    "",
    "Options:",
...
export function buildTestLiveEnv(args, baseEnv = process.env) {
  return {
    ...
    pnpm_config_verify_deps_before_run: baseEnv.pnpm_config_verify_deps_before_run || "false",
    ORIRO_LIVE_TEST: baseEnv.ORIRO_LIVE_TEST || "1",
    ORIRO_LIVE_TEST_QUIET: args.quietOverride ?? baseEnv.ORIRO_LIVE_TEST_QUIET ?? "1",
    ...(args.forceCodexHarness ? { ORIRO_LIVE_CODEX_HARNESS: "1" } : {}),
  };
}
...
export function resolveTestLiveHeartbeatMs(baseEnv = process.env) {
  const value = baseEnv.ORIRO_LIVE_WRAPPER_HEARTBEAT_MS;
  if (value === undefined || value === "") {
    return 20_000;
  }
  const text = value.trim();
  if (!/^\d+$/u.test(text)) {
    throw new Error(`invalid ORIRO_LIVE_WRAPPER_HEARTBEAT_MS: ${text}`);
  }
