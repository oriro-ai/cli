...
final class DeviceStatusService: DeviceStatusServicing {
    ...
    func status() async throws -> OriroDeviceStatusPayload {
        ...
        return OriroDeviceStatusPayload(
            battery: battery,
            thermal: thermal,
            storage: storage,
            network: network,
    ...
    private func batteryStatus() -> OriroBatteryStatusPayload {
        let device = UIDevice.current
        device.isBatteryMonitoringEnabled = true
        let level = device.batteryLevel >= 0 ? Double(device.batteryLevel) : nil
        let state: OriroBatteryState = switch device.batteryState {
        ...
        return OriroBatteryStatusPayload(
            level: level,
            state: state,
            lowPowerModeEnabled: ProcessInfo.processInfo.isLowPowerModeEnabled)
    }

    private func thermalStatus() -> OriroThermalStatusPayload {
        let state: OriroThermalState = switch ProcessInfo.processInfo.thermalState {
        ...
        return OriroThermalStatusPayload(state: state)
    }

    private func storageStatus() -> OriroStorageStatusPayload {
        let attrs = (try? FileManager.default.attributesOfFileSystem(forPath: NSHomeDirectory())) ?? [:]
        let total = (attrs[.systemSize] as? NSNumber)?.int64Value ?? 0
        let free = (attrs[.systemFreeSize] as? NSNumber)?.int64Value ?? 0
        let used = max(0, total - free)
        return OriroStorageStatusPayload(totalBytes: total, freeBytes: free, usedBytes: used)
    }
