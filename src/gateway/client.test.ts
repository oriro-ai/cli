...
function expectSecurityConnectError(
  ...
  expect(error.message).toContain("oriro doctor --fix");
...
describe("GatewayClient security checks", () => {
  ...
  beforeEach(async () => {
    ...
    delete process.env.ORIRO_PROXY_ACTIVE;
    delete process.env.ORIRO_PROXY_LOOPBACK_MODE;
  ...
  it("allows ws:// hostnames with ORIRO_ALLOW_INSECURE_PRIVATE_WS=1", () => {
    process.env.ORIRO_ALLOW_INSECURE_PRIVATE_WS = "1";
    const onConnectError = vi.fn();
    const client = new GatewayClient({
      url: "ws://oriro-gateway.ai:18789",
      onConnectError,
    });
...
describe("GatewayClient close handling", () => {
  ...
  it("clears stale token on device token mismatch close", () => {
    const onClose = vi.fn();
    const env = { ORIRO_HOME: "/tmp/custom-oriro-home" };
...
describe("GatewayClient connect auth payload", () => {
  ...
  it("loads stored device auth from the provided env", () => {
    ...
    const env = {
      ...process.env,
      ORIRO_STATE_DIR: "/tmp/oriro-client-service-state",
