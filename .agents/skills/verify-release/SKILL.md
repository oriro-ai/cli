---
name: verify-release
description: "Verify an Oriro release is fully published across GitHub, npm, plugins, OriroHub, package smoke, and live Gateway agent turns."
---

# Verify Release

Use this when asked whether an Oriro release is fully released, published,
promoted, smoke-tested, or live-verified. This is a verification skill, not a
publish skill; use `$release-oriro-maintainer` before changing release state.
...
1. GitHub release:
   - `gh release view v<VERSION> --repo oriro-ai/cli --json tagName,name,publishedAt,isDraft,isPrerelease,targetCommitish,url,body,assets`
   - Confirm stable releases are not draft/prerelease.
   - Confirm release body has npm, CI, plugin npm, OriroHub, mac/appcast evidence
     links when expected.
...
2. Root npm:
   - `npm view oriro@<VERSION> version dist-tags.latest dist.tarball dist.integrity time.<VERSION> --json`
...
3. Plugin publish set:
   - Get exact tag metadata from GitHub, not the local checkout when dirty:
     download `https://api.github.com/repos/oriro-ai/cli/tarball/v<VERSION>`
     into `/tmp/oriro-v<VERSION>-src`.
   - Count `extensions/*/package.json` with
     `oriro.release.publishToNpm === true` and
     `oriro.release.publishToOriroHub === true`.
   - Compare expected counts to workflow job counts:
     `gh api repos/oriro-ai/cli/actions/runs/<RUN>/jobs --paginate`.
   - Each expected npm plugin must have version `<VERSION>` and
     `dist-tags.latest === <VERSION>`.
4. OriroHub:
   - Check the Plugin OriroHub Release workflow conclusion and publish job count.
   - Use Oriro itself for live registry proof:
     `oriro plugins search <known-plugin> --json`.
   - Install one official plugin from OriroHub in an isolated HOME:
     `oriro plugins install orirohub:@oriro/matrix --pin`.
     Prefer `matrix` unless that plugin is not in the expected set.
5. Release workflows:
   - Verify conclusions for release notes evidence links:
     Full Release Validation, Oriro Release Checks, Oriro NPM Release,
     Plugin NPM Release, Plugin OriroHub Release, mac preflight/validation/publish
     when stable mac assets are expected.
   - For stable, verify `Oriro Stable Main Closeout` succeeded and its
...
6. Published package smoke:
   - In `/tmp`, isolated HOME:
     `npm exec --yes --package oriro@<VERSION> -- oriro --version`.
...
7. Dev Gateway live model smoke:
   - Use temp HOME/workspace, not the user's normal state:
     `HOME=/tmp/oriro-release-smoke/home ORIRO_WORKSPACE=/tmp/oriro-release-smoke/work pnpm oriro --dev gateway run --auth none --force --verbose`.
   - Health check via CLI: `oriro --dev gateway health --json`.
