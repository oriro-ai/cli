...
struct ConfigureRemoteCommandTests {
    @Test @MainActor func `configure remote writes ssh config and app defaults`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("oriro-configure-remote-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }

        let defaultSuites = ["ai.oriro.mac", "ai.oriro.mac.debug"]
        let keys = [
            "oriro.connectionMode",
            "oriro.remoteTarget",
            "oriro.onboardingSeen",
            "oriro.onboardingVersion",
            "oriro.remoteCliPath",
        ]
        ...
        try await TestIsolation.withIsolatedState(env: ["ORIRO_CONFIG_PATH": configURL.path]) {
            ...
            for (_, defaults) in defaultsBySuite {
                #expect(defaults.string(forKey: "oriro.connectionMode") == "remote")
                #expect(defaults.string(forKey: "oriro.remoteTarget") == "alice@gateway.example")
                #expect(defaults.bool(forKey: "oriro.onboardingSeen") == true)
                #expect(defaults.string(forKey: "oriro.remoteCliPath") == "/opt/homebrew/bin/oriro")
            }
        }
    }
    ...
    @Test @MainActor func `configure remote preserves existing optional credentials when flags omitted`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("oriro-configure-remote-preserve-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }
    ...
    @Test @MainActor func `configure remote can write direct private url`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("oriro-configure-direct-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }
    ...
    @Test @MainActor func `configure remote rejects plaintext public prefix bypass`() async throws {
        let configURL = FileManager().temporaryDirectory
            .appendingPathComponent("oriro-configure-direct-reject-\(UUID().uuidString).json")
        defer { try? FileManager().removeItem(at: configURL) }

        _ = await TestIsolation.withIsolatedState(env: ["ORIRO_CONFIG_PATH": configURL.path]) {
            #expect(throws: Error.self) {
