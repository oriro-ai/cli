...
read_when:
  ...
  - You want a reference for all registration methods on OriroPluginApi
...
<Note>
  This page is for plugin authors using `oriro/plugin-sdk/*` inside
  Oriro. For external apps, scripts, dashboards, CI jobs, and IDE extensions
...
prefer `oriro/plugin-sdk/channel-core`; keep `oriro/plugin-sdk/core` for
...
`oriro.plugin.json#channelConfigs`. The `plugin-sdk/channel-config-schema`
subpath is for shared schema primitives and the generic builder. Oriro's
...
<Warning>
  ...
  `oriro/plugin-sdk/slack`, `.../discord`, `.../signal`, `.../whatsapp`).
...
`oriro/plugin-sdk/discord` and `oriro/plugin-sdk/telegram-account` are
...
The `register(api)` callback receives an `OriroPluginApi` object with these
methods:
...
agentPromptGuidance: [
  "Global command hint.",
  { text: "Only show this in the main Oriro prompt.", surfaces: ["oriro_main"] },
];
```

Structured `surfaces` may include `oriro_main`, `codex_app_server`,
`cli_backend`, `acp_backend`, or `subagent`. `pi_main` remains a deprecated alias
for `oriro_main`. Omit `surfaces` for intentional all-surface guidance. Do
...
| `api.registerNodeCliFeature(registrar, opts?)` | Node feature CLI under `oriro nodes` |
...
runtime, for example `["oriro", "codex"]`. Installed plugins without that
contract, or without explicit enablement, cannot register this middleware; keep
normal Oriro plugin hooks for work that does not need pre-model tool-result
timing. The old
...
Gateway on a local discovery transport such as mDNS/Bonjour. Oriro calls the
...
`oriro nodes canvas` explicit plugin-owned node features.
...
- User config still wins. Oriro merges `agents.defaults.cliBackends.<id>` over the
...
- Use `resolveExecutionArgs` for request-scoped argv rewrites that belong to
  the CLI dialect, such as mapping Oriro thinking levels to a native effort
...
- `registerMemoryCapability` may also expose `publicArtifacts.listArtifacts(...)`
  ...
  `oriro/plugin-sdk/memory-host-core` instead of reaching into a specific
...
- `cron_changed`: observe gateway-owned cron lifecycle changes. Use `event.job?.state?.nextRunAtMs` and `ctx.getCron?.()` when syncing external wake schedulers, and keep Oriro as the source of truth for due checks and execution.
...
| `api.config`             | `OriroConfig`          | Current config snapshot (active in-memory runtime snapshot when available)                  |
...
<Warning>
  Never import your own plugin through `oriro/plugin-sdk/<your-plugin>`
...
active runtime config snapshot when Oriro is already running. If no runtime
snapshot exists yet, they fall back to the resolved config file on disk.
Packaged bundled plugin facades should be loaded through Oriro's plugin
...
- **`@oriro/openai-provider`**: `api.ts` exports provider builders,
  default-model helpers, and realtime provider builders.
- **`@oriro/openrouter-provider`**: `api.ts` exports the provider builder
  plus onboarding/config helpers.

<Warning>
  Extension production code should also avoid `oriro/plugin-sdk/<other-plugin>`
  imports. If a helper is truly shared, promote it to a neutral SDK subpath
  such as `oriro/plugin-sdk/speech`, `.../provider-model-shared`, or another
