...
import { clearMemoryEmbeddingProviders as clearRegistry } from "oriro/plugin-sdk/memory-core-host-engine-embeddings";
import { hashText } from "oriro/plugin-sdk/memory-core-host-engine-storage";
import { resolveSessionTranscriptsDirForAgent } from "oriro/plugin-sdk/memory-core-host-runtime-core";
import { resolveOriroAgentSqlitePath } from "oriro/plugin-sdk/sqlite-runtime";
import {
  closeOriroAgentDatabasesForTest,
  closeOriroStateDatabaseForTest,
  openOriroAgentDatabase,
} from "oriro/plugin-sdk/sqlite-runtime-testing";
...
describe("memory index", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-mem-fixtures-"));
  ...
  beforeEach(async () => {
    ...
    vi.stubEnv("ORIRO_STATE_DIR", path.join(workspaceDir, ".state-memory-index"));
    await fs.writeFile(
  ...
  async function getFtsSessionManager(params: {
    ...
    vi.stubEnv("ORIRO_STATE_DIR", path.join(workspaceDir, params.stateDirName));
    const cfg = createCfg({
  ...
  it("reindexes memory tables in place without deleting unrelated agent rows", async () => {
    ...
    const agentDbPath = resolveOriroAgentSqlitePath({ agentId: "main" });
    const agentDb = openOriroAgentDatabase({ agentId: "main" });
    agentDb.db
    ...
    const reopened = openOriroAgentDatabase({ agentId: "main" });
    expect(
      reopened.db
  ...
  it("clears dirty after sessions-only identity reindex", async () => {
    try {
      vi.stubEnv("ORIRO_STATE_DIR", path.join(workspaceDir, ".state-sessions-only-reindex"));
      const sessionsDir = resolveSessionTranscriptsDirForAgent("main");
      await fs.mkdir(sessionsDir, { recursive: true });
      await fs.writeFile(
  ...
  it("marks sessions-only indexes dirty when metadata is missing but chunks exist", async () => {
    try {
      vi.stubEnv("ORIRO_STATE_DIR", path.join(workspaceDir, ".state-sessions-missing-meta"));
      const sessionsDir = resolveSessionTranscriptsDirForAgent("main");
      await fs.mkdir(sessionsDir, { recursive: true });
      await fs.writeFile(
  ...
  it("keeps provider cutover vector search paused during targeted session sync", async () => {
    try {
      vi.stubEnv("ORIRO_STATE_DIR", path.join(workspaceDir, ".state-targeted-cutover"));
      const sessionsDir = resolveSessionTranscriptsDirForAgent("main");
      await fs.mkdir(sessionsDir, { recursive: true });
  ...
  it("preserves memory dirty events raised during session identity reindex", async () => {
    try {
      vi.stubEnv("ORIRO_STATE_DIR", path.join(workspaceDir, ".state-dirty-during-session"));
      const sessionsDir = resolveSessionTranscriptsDirForAgent("main");
      await fs.mkdir(sessionsDir, { recursive: true });
      await fs.writeFile(
