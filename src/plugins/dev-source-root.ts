...
/** Env var that points bundled-plugin lookup at an Oriro source checkout. */
export const ORIRO_DEV_SOURCE_ROOT_ENV = "ORIRO_DEV_SOURCE_ROOT";
...
/** Resolves and validates the configured Oriro development source root. */
export function resolveOriroDevSourceRoot(env: NodeJS.ProcessEnv = process.env): string | null {
  const rawRoot = env[ORIRO_DEV_SOURCE_ROOT_ENV]?.trim();
  if (!rawRoot) {
    return null;
  }
  ...
  if (readPackageName(path.join(realRoot, "package.json")) !== "oriro") {
    return null;
  }
...
export function isBundledPluginInsideDevSourceRoot(params: {
  ...
  const devSourceRoot = resolveOriroDevSourceRoot(params.env);
  if (!devSourceRoot) {
    return false;
  }
