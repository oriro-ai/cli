---
summary: "Export redacted trajectory bundles for debugging an Oriro agent session"
read_when:
  - Debugging why an agent answered, failed, or called tools a certain way
  - Exporting a support bundle for an Oriro session
...
Trajectory capture is Oriro's per-session flight recorder. It records a
...
Oriro writes the bundle under the workspace:

```text
.oriro/trajectory-exports/oriro-trajectory-<session>-<timestamp>/
```
...
The custom path is resolved inside `.oriro/trajectory-exports/`. Absolute
...
create the bundle; do not use allow-all. In group chats, Oriro sends the
...
Trajectory capture is on by default for Oriro agent runs.
...
| `metadata.json`       | Oriro version, OS/runtime, model, config snapshot, plugins, skills, and prompt metadata     |
...
Oriro also writes a best-effort pointer file beside the session:

```text
<session>.trajectory-path.json
```

Set `ORIRO_TRAJECTORY_DIR` to store runtime trajectory sidecars in a
dedicated directory:

```bash
export ORIRO_TRAJECTORY_DIR=/var/lib/oriro/trajectories
```

When this variable is set, Oriro writes one JSONL file per session id in that
directory.
...
Set `ORIRO_TRAJECTORY=0` before starting Oriro:

```bash
export ORIRO_TRAJECTORY=0
```
...
Oriro flushes runtime trajectory sidecars during agent cleanup. The default
cleanup timeout is 10,000 ms. On slow disks or large stores, set
`ORIRO_TRAJECTORY_FLUSH_TIMEOUT_MS` before starting Oriro:

```bash
export ORIRO_TRAJECTORY_FLUSH_TIMEOUT_MS=30000
```

This controls when Oriro logs an `oriro-trajectory-flush` timeout and continues.
It does not change the trajectory size caps. To tune all agent cleanup steps
that do not pass an explicit timeout, set `ORIRO_AGENT_CLEANUP_TIMEOUT_MS`.

## Privacy and limits

Trajectory bundles are designed for support and debugging, not public posting.
Oriro redacts sensitive values before writing export files:
...
- confirm Oriro was started without `ORIRO_TRAJECTORY=0`
- check whether `ORIRO_TRAJECTORY_DIR` points to a writable directory
...
- do not pass `/tmp/...` or `~/...`
- keep the export inside `.oriro/trajectory-exports/`
