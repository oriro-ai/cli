...
when Oriro loads an installed package and let npm packages avoid runtime
...
an installed package only declares a TypeScript source entry, Oriro will use a
...
**Import:** `oriro/plugin-sdk/tool-plugin`
...
schemas, wraps plain return values in the Oriro tool-result format, and
exposes static metadata that `oriro plugins build` writes into the plugin
manifest.
...
- `configSchema` is optional. When omitted, Oriro uses a strict empty object
...
- Tool names are static. `oriro plugins build` derives `contracts.tools`
...
- Runtime loading stays strict. Installed plugins still need
  `oriro.plugin.json` and `package.json` `oriro.extensions`; Oriro does
  not execute plugin code to infer missing manifest data.

## `definePluginEntry`

**Import:** `oriro/plugin-sdk/plugin-entry`
...
| `kind`         | `string`                                                         | No       | -                   |
| `configSchema` | `OriroPluginConfigSchema \| () => OriroPluginConfigSchema` | No       | Empty object schema |
| `register`     | `(api: OriroPluginApi) => void`                               | Yes      | -                   |

- `id` must match your `oriro.plugin.json` manifest.
- `kind` is for exclusive slots: `"memory"` or `"context-engine"`.
- `configSchema` can be a function for lazy evaluation.
- Oriro resolves and memoizes that schema on first access, so expensive schema
  builders only run once.

## `defineChannelPluginEntry`

**Import:** `oriro/plugin-sdk/channel-core`
...
| `registerCliMetadata` | `(api: OriroPluginApi) => void`                               | No       | -                   |
| `registerFull`        | `(api: OriroPluginApi) => void`                               | No       | -                   |
...
- Discovery registration is non-activating, not import-free. Oriro may
...
- Like `definePluginEntry`, `configSchema` can be a lazy factory and Oriro
...
- For plugin-owned root CLI commands, prefer `api.registerCli(..., { descriptors: [...] })`
  ...
  `api.registerNodeCliFeature(...)` so the command lands under `oriro nodes`.
  For other nested plugin commands, add `parentPath` and register commands on
  the `program` object passed to the registrar; Oriro resolves it to the
...
Oriro loads this instead of the full entry when a channel is disabled,
...
- `oriro/plugin-sdk/setup-runtime` for runtime-safe setup helpers such as
...
- `oriro/plugin-sdk/channel-setup` for optional-install setup surfaces
- `oriro/plugin-sdk/setup-tools` for setup/install CLI/archive/docs helpers
...
`oriro/plugin-sdk/channel-entry-contract` instead. That contract lets the
setup entry keep setup-safe plugin/secrets exports while still exposing a
runtime setter:

```typescript
import { defineBundledChannelSetupEntry } from "oriro/plugin-sdk/channel-entry-contract";
...
the plugin entry and the channel plugin object so Oriro can register channel
...
- use `descriptors` when the registrar owns one or more root commands and you
  want Oriro to lazy-load the real CLI module on first invocation
...
- keep descriptor command names to letters, numbers, hyphen, and underscore,
  starting with a letter or number; Oriro rejects descriptor names outside
...
Oriro classifies loaded plugins by their registration behavior:
...
Use `oriro plugins inspect <id>` to see a plugin's shape.
