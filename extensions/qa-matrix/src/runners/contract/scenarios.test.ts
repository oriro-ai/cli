...
const {
  ...
  resolveMatrixQaOriroCliEntryPath,
  runMatrixQaOriroCli,
  startMatrixQaOriroCli,
} = vi.hoisted(() => ({
  formatMatrixQaCliCommand: (args: string[]) => `oriro ${args.join(" ")}`,
  redactMatrixQaCliOutput: (text: string) => text,
  resolveMatrixQaOriroCliEntryPath: (cwd: string) => `${cwd}/dist/index.js`,
  runMatrixQaOriroCli: vi.fn(),
  startMatrixQaOriroCli: vi.fn(),
}));
...
async function writeMatrixInboundDedupePluginStateEntry(params: {
  ...
  const databasePath = path.join(params.stateRoot, "state", "oriro.sqlite");
...
describe("matrix live qa scenarios", () => {
  beforeEach(() => {
    ...
    runMatrixQaOriroCli.mockReset();
    startMatrixQaOriroCli.mockReset();
  ...
  it("merges default and scenario-requested Matrix topology once per run", () => {
    expect(
      scenarioTesting.buildMatrixQaTopologyForScenarios({
        defaultRoomName: "Oriro Matrix QA run",
        scenarios: [
  ...
  it("runs the Matrix E2EE state_after regression scenario through the gateway fault proxy", async () => {
    ...
    try {
      ...
      const result = await runMatrixQaScenario(scenario, {
        ...
        driverDeviceId: "DRIVERDEVICE",
        gatewayRuntimeEnv: {
          ORIRO_CONFIG_PATH: gatewayConfigPath,
  ...
  it("runs Matrix self-verification through the interactive CLI command", async () => {
    ...
    try {
      ...
      startMatrixQaOriroCli.mockReturnValue({
        args: ["matrix", "verify", "self", "--account", "cli"],
        endStdin,
        kill,
      ...
      runMatrixQaOriroCli.mockImplementation(async ({ args, env, stdin }) => {
        if (!cliAccountConfigDuringRun && env.ORIRO_CONFIG_PATH) {
          const cliConfig = JSON.parse(
            await readFile(String(env.ORIRO_CONFIG_PATH), "utf8"),
          ) as {
            channels?: {
              matrix?: {
      ...
      const result = await runMatrixQaScenario(scenario, {
        ...
        driverPassword: "driver-password",
        gatewayRuntimeEnv: {
          ORIRO_CONFIG_PATH: "/tmp/gateway-config.json",
          ORIRO_STATE_DIR: "/tmp/gateway-state",
          PATH: process.env.PATH,
        },
        outputDir,
      });
      ...
      expect(startMatrixQaOriroCli).toHaveBeenCalledTimes(1);
      expect(mockObjectArg(startMatrixQaOriroCli, "startMatrixQaOriroCli").args).toEqual([
        "matrix",
        "verify",
        "self",
        "--account",
      ...
      expect(mockObjectArg(startMatrixQaOriroCli, "startMatrixQaOriroCli").timeoutMs).toBe(
        16_000,
      );
      ...
      expect(registrationRequest?.deviceName).toBe(
        "Oriro Matrix QA CLI Self Verification Owner",
      );
      if (
      ...
      expect(runMatrixQaOriroCli).toHaveBeenCalledTimes(2);
      expect(runMatrixQaOriroCli.mock.calls.map(([params]) => params.args)).toEqual([
        [
          "matrix",
          "verify",
          "backup",
      ...
      expect(mockObjectArg(runMatrixQaOriroCli, "runMatrixQaOriroCli").stdin).toBe(
        "encoded-recovery-key\n",
      );
      const cliEnv = mockObjectArg(startMatrixQaOriroCli, "startMatrixQaOriroCli")
        .env as Record<string, unknown>;
      expect(cliEnv?.ORIRO_STATE_DIR).toContain("oriro-matrix-cli-qa-");
      expect(cliEnv?.ORIRO_CONFIG_PATH).toContain("oriro-matrix-cli-qa-");
      const configPath = String(cliEnv?.ORIRO_CONFIG_PATH);
      ...
      await expectPathMissing(String(cliEnv?.ORIRO_STATE_DIR));
  ...
  it("runs Matrix account add --enable-e2ee through the CLI QA scenario", async () => {
    ...
    try {
      ...
      runMatrixQaOriroCli.mockImplementation(async ({ args, env }) => {
        if (env.ORIRO_CONFIG_PATH) {
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "Oriro Matrix QA CLI Account Add Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix encryption setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      runMatrixQaOriroCli.mockImplementation(async ({ args, env }) => {
        if (!initialAccountConfig && env.ORIRO_CONFIG_PATH) {
          const initialConfig = JSON.parse(
            await readFile(String(env.ORIRO_CONFIG_PATH), "utf8"),
          ) as {
            channels?: {
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "Oriro Matrix QA CLI Encryption Setup Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix encryption setup idempotency through the CLI QA scenario", async () => {
    ...
    try {
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "Oriro Matrix QA CLI Encryption Idempotent Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix encryption setup bootstrap failure through the CLI QA scenario", async () => {
    ...
    try {
      ...
      const wait = vi
        .fn()
        .mockRejectedValue(new Error("oriro matrix encryption setup exited 1"));
      const kill = vi.fn();
      startMatrixQaOriroCli.mockReturnValue({
      ...
      expect(
        (
          mockObjectArg(startMatrixQaOriroCli, "startMatrixQaOriroCli").env as Record<
            string,
            unknown
          >
        ).ORIRO_CONFIG_PATH,
      ).toContain("oriro-matrix-e2ee-setup-qa-");
      expect(output).toHaveBeenCalledTimes(1);
      expect(wait).toHaveBeenCalledTimes(1);
      expect(kill).toHaveBeenCalledTimes(1);
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "Oriro Matrix QA CLI Encryption Failure Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix recovery-key setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "Oriro Matrix QA CLI Recovery Key Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix invalid recovery-key setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      expect(kill).toHaveBeenCalledTimes(1);
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "Oriro Matrix QA CLI Invalid Recovery Key Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
  ...
  it("runs Matrix multi-account encryption setup through the CLI QA scenario", async () => {
    ...
    try {
      ...
      runMatrixQaOriroCli.mockImplementation(async ({ args, env }) => {
        const configPath = String(env.ORIRO_CONFIG_PATH);
      ...
      expect(mockObjectArg(registerWithToken, "registerWithToken").deviceName).toBe(
        "Oriro Matrix QA CLI Multi Account Owner",
      );
      expect(mockObjectArg(registerWithToken, "registerWithToken").registrationToken).toBe(
        "registration-token",
      );
    ...
    try {
      ...
      expect(
        registrationRequests.some(
          (request) =>
            request.deviceName === "Oriro Matrix QA CLI Setup Gateway" &&
      ...
      expect(
        registrationRequests.some(
          (request) =>
            request.deviceName === "Oriro Matrix QA CLI Setup Driver" &&
            request.registrationToken === "registration-token",
        ),
      ).toBe(true);
      expect(createPrivateRoom).toHaveBeenCalledWith({
