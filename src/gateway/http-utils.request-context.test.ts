...
describe("resolveGatewayRequestContext", () => {
  it("uses normalized x-oriro-message-channel when enabled", () => {
    const result = resolveGatewayRequestContext({
      req: createReq({ "x-oriro-message-channel": " Custom-Channel " }),
      model: "oriro",
  ...
  it("preserves normal explicit session-key overrides", () => {
    const result = resolveGatewayRequestContext({
      req: createReq({ "x-oriro-session-key": "customer-case-42" }),
      model: "oriro",
      sessionPrefix: "openai",
      defaultMessageChannel: "webchat",
    });
  ...
  it.each([
    ...
    expect(() =>
      resolveGatewayRequestContext({
        req: createReq({ "x-oriro-session-key": sessionKey }),
        model: "oriro",
        sessionPrefix: "openai",
        defaultMessageChannel: "webchat",
      }),
  ...
  it("does not build session state for explicit unknown agent ids", () => {
    expect(() =>
      resolveGatewayRequestContext({
        req: createReq({ "x-oriro-agent-id": "missing-agent" }),
        model: "oriro",
        sessionPrefix: "openai",
        defaultMessageChannel: "webchat",
      }),
    ...
    expect(() =>
      resolveGatewayRequestContext({
        req: createReq({ "x-oriro-agent-id": "!!!" }),
        model: "oriro",
        sessionPrefix: "openai",
        defaultMessageChannel: "webchat",
      }),
...
describe("resolveHttpSenderIsOwner", () => {
  it("requires operator.admin on a trusted HTTP scope-bearing request", () => {
    expect(
      resolveHttpSenderIsOwner(createReq({ "x-oriro-scopes": "operator.admin" }), noneAuth),
    ).toBe(true);
    expect(
      resolveHttpSenderIsOwner(createReq({ "x-oriro-scopes": "operator.write" }), noneAuth),
    ).toBe(false);
  });
...
describe("resolveOpenAiCompatibleHttpSenderIsOwner", () => {
  ...
  it("still requires operator.admin for trusted scope-bearing requests", () => {
    expect(
      resolveOpenAiCompatibleHttpSenderIsOwner(
        createReq({ "x-oriro-scopes": "operator.write" }),
        { authMethod: "trusted-proxy", trustDeclaredOperatorScopes: true },
      ),
    ).toBe(false);
    expect(
      resolveOpenAiCompatibleHttpSenderIsOwner(
        createReq({ "x-oriro-scopes": "operator.admin" }),
        { authMethod: "trusted-proxy", trustDeclaredOperatorScopes: true },
      ),
    ).toBe(true);
  });
...
describe("authorizeOpenAiCompatibleHttpModelOverride", () => {
  it("allows shared-secret bearer callers to use x-oriro-model", () => {
    expect(
      authorizeOpenAiCompatibleHttpModelOverride(
        createReq({ authorization: "Bearer secret", "x-oriro-model": "openai/gpt-5.4" }),
        { authMethod: "token", trustDeclaredOperatorScopes: false },
      ),
    ).toEqual({ allowed: true });
  });

  it("allows trusted admin callers to use x-oriro-model", () => {
    expect(
      authorizeOpenAiCompatibleHttpModelOverride(
        createReq({
          "x-oriro-scopes": "operator.admin, operator.write",
  ...
  it("rejects trusted write-only callers that try to use x-oriro-model", () => {
    expect(
      authorizeOpenAiCompatibleHttpModelOverride(
        createReq({
          "x-oriro-scopes": "operator.write",
