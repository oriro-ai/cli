    ...
    mcpServers: {
      oriro: {
        ...
        headers: {
          ...
          "x-oriro-session-id": "${ORIRO_MCP_SESSION_ID}",
          "x-oriro-agent-id": "${ORIRO_MCP_AGENT_ID}",
          "x-oriro-account-id": "${ORIRO_MCP_ACCOUNT_ID}",
          "x-oriro-message-channel": "${ORIRO_MCP_MESSAGE_CHANNEL}",
          "x-oriro-current-channel-id": "${ORIRO_MCP_CURRENT_CHANNEL_ID}",
          "x-oriro-current-thread-ts": "${ORIRO_MCP_CURRENT_THREAD_TS}",
          "x-oriro-current-message-id": "${ORIRO_MCP_CURRENT_MESSAGE_ID}",
          "x-oriro-current-inbound-audio": "${ORIRO_MCP_CURRENT_INBOUND_AUDIO}",
          "x-oriro-inbound-event-kind": "${ORIRO_MCP_INBOUND_EVENT_KIND}",
          "x-oriro-source-reply-delivery-mode": "${ORIRO_MCP_SOURCE_REPLY_DELIVERY_MODE}",
          "x-oriro-require-explicit-message-target":
            "${ORIRO_MCP_REQUIRE_EXPLICIT_MESSAGE_TARGET}",
...
function createSessionFile() {
  // Prepare tests use canonical Oriro session paths because several cases
  // assert that external or stale transcript paths are ignored.
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-cli-prepare-"));
  sessionFileEnvSnapshot ??= captureEnv(["ORIRO_STATE_DIR"]);
  setTestEnvValue("ORIRO_STATE_DIR", dir);
...
describe("shouldSkipLocalCliCredentialEpoch", () => {
  beforeEach(() => {
    ...
    setCliRunnerPrepareTestDeps({
      ...
      resolveOriroReferencePaths: vi.fn(async () => ({ docsPath: null, sourcePath: null })),
  ...
  it("lets Gemini CLI preparation override generated MCP system settings auth", async () => {
    ...
    try {
      ...
      expect(generatedSettings.mcp?.allowed).toEqual(["oriro"]);
      expect(generatedSettings.mcpServers?.oriro?.url).toBe("http://127.0.0.1:31783/mcp");
  ...
  it("prepares side questions without agent-turn context, tools, hooks, or reusable sessions", async () => {
    ...
    setCliRunnerPrepareTestDeps({
      ...
      resolveOriroReferencePaths: vi.fn(async () => ({ docsPath: "docs", sourcePath: "src" })),
    });
  ...
  it("uses compact current-turn context when a room event resumes a CLI session", async () => {
    ...
    try {
      ...
      // richer room context in Oriro history for reseed and audits.
      const context = await prepareCliRunContext({
        sessionId: "session-test",
        sessionKey: "agent:main:test",
        agentId: "main",
  ...
  it("does not allocate a non-legacy context engine before fallible CLI preparation finishes", async () => {
    ...
    setCliRunnerPrepareTestDeps({
      resolveOriroReferencePaths: vi.fn(async () => {
        ...
        info: {
          ...
          hostRequirements: {
            "agent-run": {
              ...
              unsupportedMessage: "Use the native Codex or Oriro embedded runtime.",
            },
          },
        },
        ingest: vi.fn(async () => ({ ingested: true })),
  ...
  it("uses cwd for CLI system prompt workspace guidance", async () => {
    const { dir, sessionFile } = createSessionFile();
    const taskDir = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-cli-task-"));
    try {
      const context = await prepareCliRunContext({
        sessionId: "session-test",
        sessionFile,
  ...
  it("passes current turn kind into bundle MCP loopback env", async () => {
    ...
    try {
      ...
      expect(context.preparedBackend.env).toMatchObject({
        ORIRO_MCP_SESSION_ID: "session-test",
        ORIRO_MCP_MESSAGE_CHANNEL: "telegram",
        ORIRO_MCP_CURRENT_CHANNEL_ID: "telegram:-100123:topic:42",
        ORIRO_MCP_CURRENT_THREAD_TS: "42",
        ORIRO_MCP_CURRENT_MESSAGE_ID: "reply-message-1",
        ORIRO_MCP_CURRENT_INBOUND_AUDIO: "true",
        ORIRO_MCP_INBOUND_EVENT_KIND: "room_event",
        ORIRO_MCP_SOURCE_REPLY_DELIVERY_MODE: "message_tool_only",
        ORIRO_MCP_REQUIRE_EXPLICIT_MESSAGE_TARGET: "true",
  ...
  it("renders CLI skills from sandbox-readable paths instead of persisted host snapshots", async () => {
    const { dir, sessionFile } = createSessionFile();
    const hostSkillDir = "/home/tzdai/.npm-global/lib/node_modules/oriro/skills/gog";
    ...
    try {
      ...
      expect(context.systemPrompt).toContain(
        "/workspace/.oriro/sandbox-skills/skills/gog/SKILL.md",
      );
  ...
  it("omits Claude CLI prompt skills when the native skills plugin can carry them", async () => {
    ...
    try {
      ...
      setCliRunnerPrepareTestDeps({
        prepareClaudeCliSkillsPlugin: vi.fn(async () => ({
          args: ["--plugin-dir", path.join(dir, "oriro-skills")],
          cleanup: vi.fn(async () => undefined),
          pluginDir: path.join(dir, "oriro-skills"),
        })),
      });
