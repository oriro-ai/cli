...
function makeTempDir() {
  return makeTrackedTempDir("oriro-doctor-plugin-registry", tempDirs);
}
...
function createBundledCandidate(params: {
  ...
  fs.writeFileSync(
    path.join(params.rootDir, "oriro.plugin.json"),
    JSON.stringify({
      id: params.id,
      name: params.id,
      configSchema: { type: "object" },
...
function expectedPluginIndexRecord(params: {
  ...
  return {
    ...
    manifestPath: path.join(params.rootDir, "oriro.plugin.json"),
...
describe("maybeRepairPluginRegistryState", () => {
  ...
  it("warns about stale managed npm packages that shadow bundled plugins", async () => {
    ...
    expect(vi.mocked(note).mock.calls.join("\n")).toContain("@oriro/google-meet@2026.5.2");
  ...
  it("removes stale managed npm packages from the package lock during repair", async () => {
    ...
    expect(packageLock.packages).not.toHaveProperty("node_modules/@oriro/google-meet");
    expect(packageLock.dependencies).not.toHaveProperty("@oriro/google-meet");
    expect(packageLock.dependencies).toHaveProperty("other-plugin");
  });

  it("repairs managed npm oriro peer links during registry repair", async () => {
    const stateDir = makeTempDir();
    const managed = createManagedNpmPlugin({
      stateDir,
      id: "codex",
    ...
    const linkPath = path.join(managed.packageDir, "node_modules", "oriro");
    expect(fs.lstatSync(linkPath).isSymbolicLink()).toBe(true);
    expect(fs.realpathSync(linkPath)).toBe(fs.realpathSync(process.cwd()));
    expect(vi.mocked(note).mock.calls.join("\n")).toContain("Repaired Oriro host peer link");
  });

  it("warns about broken managed npm oriro peer links without repairing them", async () => {
    const stateDir = makeTempDir();
    const managed = createManagedNpmPlugin({
      stateDir,
      id: "codex",
    ...
    const linkPath = path.join(managed.packageDir, "node_modules", "oriro");
    const notes = vi.mocked(note).mock.calls.join("\n");
    expect(notes).toContain("Managed npm Oriro host peer links need repair");
    expect(notes).toContain("codex-plugin");
    expect(notes).toContain("oriro doctor --fix");
