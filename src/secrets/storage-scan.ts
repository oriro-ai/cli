...
import { isRecord as isJsonObject } from "@oriro/normalization-core/record-coerce";
...
export function listAuthProfileStoreAgentDirs(config: OriroConfig, stateDir: string): string[] {
...
function resolveActiveAgentDir(stateDir: string, env: NodeJS.ProcessEnv = process.env): string {
  const override = env.ORIRO_AGENT_DIR?.trim() || env.PI_CODING_AGENT_DIR?.trim();
  if (override) {
