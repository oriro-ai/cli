...
import { runOriroStateWriteTransaction } from "../state/oriro-state-db.js";
import {
...
function tempStateDir(): string {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-metadata-memo-"));
  tempDirs.push(dir);
  return dir;
}
...
function writePersistedIndex(params: {
  ...
  const manifestPath = params.manifestPath ?? path.join(pluginDir, "oriro.plugin.json");
...
function writeRecoverableNpmPlugin(params: {
  ...
  writeJson(path.join(packageDir, "oriro.plugin.json"), { id: params.pluginId });
}
...
function makeIndex(
  ...
  const manifestPath = options.manifestPath ?? path.join(rootDir, "oriro.plugin.json");
  return {
    version: 1,
    hostContractVersion: "test",
    compatRegistryVersion: "test",
...
function makeManifestRegistry(pluginId = "demo"): PluginManifestRegistry {
  const plugin: PluginManifestRecord = {
    ...
    manifestPath: `/plugins/${pluginId}/oriro.plugin.json`,
    origin: "global",
  };
...
describe("loadPluginMetadataSnapshot process memo", () => {
  ...
  it("reuses the expanded freshness fingerprint on hot cache hits", () => {
    const stateDir = tempStateDir();
    const manifestPath = path.join(stateDir, "extensions", "demo", "oriro.plugin.json");
  ...
  it.each([
    ["manifest", "oriro.plugin.json", "manifestPath"],
  ...
  it("does not fingerprint persisted plugin paths outside the plugin root", () => {
    const stateDir = tempStateDir();
    const outsideManifestPath = path.join(stateDir, "outside", "oriro.plugin.json");
