...
// Macos Smoke script supports Oriro repository automation.
...
const defaultOptions = (): MacosOptions => ({
  ...
  installUrl: "https://oriro.ai/install.sh",
  installVersion: "",
  json: false,
  keepServer: false,
  latestVersion: "",
...
Options:
  ...
  --install-url <url>        Installer URL for latest release. Default: https://oriro.ai/install.sh
...
class MacosSmoke {
  ...
  async run(): Promise<void> {
    ...
    this.runDir = await makeTempDir("oriro-parallels-macos.");
    ...
    this.tgzDir = await makeTempDir("oriro-parallels-macos-tgz.");
    try {
  ...
  private async runFreshLane(): Promise<void> {
    ...
    await this.phase("fresh.install-main", this.targetInstallsDirectly() ? 420 : 420, () =>
      this.installMain("oriro-main-fresh.tgz"),
    );
  ...
  private async runUpgradeLane(): Promise<void> {
    ...
    if (this.options.targetPackageSpec) {
      await this.phase("upgrade.install-main", this.targetInstallsDirectly() ? 420 : 420, () =>
        this.installMain("oriro-main-upgrade.tgz"),
      );
...
exec node "$entry" ${argv}`,
  ...
  private resetState(): void {
    this.guestSh(String.raw`/usr/bin/pkill -f 'oriro.*gateway run' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'oriro-gateway' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'oriro.mjs gateway' >/dev/null 2>&1 || true
...
${guestNpm} uninstall -g oriro >/dev/null 2>&1 || true
rm -rf "$HOME/.oriro"
...
curl -fsSL --connect-timeout 10 --max-time 120 --retry 2 --retry-delay 2 ${shellQuote(
      ...
      )} -o /tmp/oriro-install.sh
bash /tmp/oriro-install.sh --version ${shellQuote(this.installVersion)}
${guestOriro} --version`,
    );
  }
...
${guestOriro} --version`);
  ...
  private verifyVersionContains(needle: string): void {
    const version = this.guestExec([guestOriro, "--version"]);
...
check_path "$root/oriro"
check_path "$root/oriro/extensions"
if [ -d "$root/oriro/extensions" ]; then
  while IFS= read -r -d '' extension_dir; do
    check_path "$extension_dir"
  done < <(/usr/bin/find "$root/oriro/extensions" -mindepth 1 -maxdepth 1 -type d -print0)
fi`);
  }
  ...
  private ensureGuestPnpm(): void {
    this.guestSh(String.raw`set -eu
bootstrap_root=/tmp/oriro-smoke-pnpm-bootstrap
...
"$bootstrap_bin/pnpm" --version`);
  ...
  private runDevChannelUpdate(): void {
    ...
    this.guestSh(
      `set -eu
rm -rf ${shellQuote(`${home}/oriro`)}
export PATH=${shellQuote(`/tmp/oriro-smoke-pnpm-bootstrap/node_modules/.bin:${guestPath}`)}
${guestNode} - <<'JS'
const fs = require("node:fs");
const path = require("node:path");
const configPath = path.join(process.env.HOME || ${JSON.stringify(home)}, ".oriro", "oriro.json");
const config = JSON.parse(fs.readFileSync(configPath, "utf8"));
config.update = { ...(config.update || {}), channel: "dev" };
fs.writeFileSync(configPath, JSON.stringify(config, null, 2) + "\\n");
JS
/usr/bin/env NODE_OPTIONS=--max-old-space-size=8192 ORIRO_ALLOW_OLDER_BINARY_DESTRUCTIVE_ACTIONS=1 ORIRO_DISABLE_BUNDLED_PLUGINS=1 ${guestOriroEntryRunner} update --channel dev --yes --json
${guestOriroEntryRunner} --version
${guestOriroEntryRunner} update status --json`,
    );
  }

  private verifyDevChannelUpdate(): void {
    const status = this.guestOriroEntryExec(["update", "status", "--json"]);
  ...
  private startManualGatewayIfNeeded(): void {
    ...
    this.guestSh(
      `set -euo pipefail
trap '' HUP
/usr/bin/pkill -f 'oriro.*gateway run' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'oriro-gateway' >/dev/null 2>&1 || true
/usr/bin/pkill -f 'oriro.mjs gateway' >/dev/null 2>&1 || true
...
sleep 1`,
  ...
  private showGatewayStatusCompat(): void {
    const help = this.guestExec([guestOriro, "gateway", "status", "--help"], { check: false });
    const args = help.includes("--require-rpc")
      ? ["gateway", "status", "--deep", "--require-rpc"]
      : ["gateway", "status", "--deep"];
    if (!this.guestOriro(args, false)) {
      throw new Error("gateway status failed");
    }
  }

  private guestOriro(args: string[], check: boolean): boolean {
    const result = run(
      "prlctl",
      [
        "exec",
    ...
    if (check && result.status !== 0) {
      throw new Error(`oriro ${args.join(" ")} failed`);
    }
...
while [ $SECONDS -lt $deadline ]; do
  if curl -fsSL --connect-timeout 2 --max-time 5 http://127.0.0.1:18789/ >/tmp/oriro-dashboard-smoke.html 2>/dev/null; then
    if grep -F '<title>Oriro Control</title>' /tmp/oriro-dashboard-smoke.html >/dev/null &&
      grep -F '<oriro-app></oriro-app>' /tmp/oriro-dashboard-smoke.html >/dev/null; then
      asset_paths="$(
        sed -nE 's/.*<(script|link)[^>]*(src|href)=["'"'"']([^"'"'"']+)["'"'"'].*/\3/p' /tmp/oriro-dashboard-smoke.html |
          grep -E '(^|/)assets/' |
          grep -Ev '^(https?:)?//' |
          sort -u
      )"
...
exit 1`);
  ...
  private verifyTurn(): void {
    this.guestOriroEntryExec(["models", "set", this.auth.modelId]);
...
JSON
${guestOriroEntryRunner} config set --batch-file "$provider_config_batch" --strict-json
rm -f "$provider_config_batch"`);
    }
    this.guestOriroEntryExec([
      "config",
      "set",
    ...
    this.guestOriroEntryExec(["config", "set", "tools.profile", "minimal"]);
...
for attempt in 1 2; do
  ...
  output_file="$(mktemp)"
  set +e
  /usr/bin/env ${shellQuote(`${this.auth.apiKeyEnv}=${this.auth.apiKeyValue}`)} ${guestOriroEntryRunner} agent --local --agent main --session-id "$session_id" --message ${shellQuote(
...
if [ "$agent_ok" != true ]; then
  ...
  private ensureDiscordGatewayReady(): void {
    ...
    this.verifyGateway();
    const status = this.guestOriroEntryExec(["channels", "status", "--probe", "--json"]);
  ...
  private async extractLastVersion(phaseName: string): Promise<string> {
    return await extractLastOriroVersionFromLog(path.join(this.runDir, `${phaseName}.log`));
  }
