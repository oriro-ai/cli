...
At startup, Oriro does roughly this:
...
The manifest is the control-plane source of truth. Oriro uses it to:
...
channels, slots, and auto-enable rules. If that derived set is empty, Oriro
...
Oriro does not cache plugin discovery results or direct manifest registry
...
Oriro still owns the generic agent loop, failover, transcript handling, and
...
For model/provider plugins, Oriro calls hooks in this rough order.
The "When to use" column is the quick decision guide.
Compatibility-only provider fields that Oriro no longer calls, such as
...
| --  | _(built-in model lookup)_         | Oriro tries the normal registry/catalog path first                                                          | _(not a plugin hook)_                                                                                                                         |
...
| 24  | `refreshOAuth`                    | OAuth refresh override for custom refresh endpoints or refresh-failure policy                                  | Provider does not fit the shared Oriro refreshers                                                                                          |
...
that still runs on Oriro's normal inference loop.

`resolveUsageAuth` decides whether Oriro should call `fetchUsageSnapshot` or
...
<AccordionGroup>
  <Accordion title="Pass-through catalog providers">
    ...
    model ids ahead of Oriro's static catalog.
  </Accordion>
...
- The preferred ownership model is company-oriented: one vendor plugin can own
  text, speech, image, and future media providers as Oriro adds those
...
- `extractStructuredWithModel(...)` is the plugin-facing seam for bounded
  ...
  product plugins own their routes and schemas while Oriro owns the
...
- Oriro only honors those override fields for trusted callers.
...
const result = await api.runtime.webSearch.search({
  config: api.config,
  args: {
    query: "Oriro plugin runtime helpers",
    count: 5,
  },
});
```
...
const result = await api.runtime.imageGeneration.generate({
  config: api.config,
  args: { prompt: "A friendly oriro mascot", size: "1024x1024" },
});
...
- `auth: "gateway"` routes run inside a Gateway request runtime scope, but that scope is intentionally conservative:
  - shared-secret bearer auth (`gateway.auth.mode = "token"` / `"password"`) keeps plugin-route runtime scopes pinned to `operator.write`, even if the caller sends `x-oriro-scopes`
  - trusted identity-bearing HTTP modes (for example `trusted-proxy` or `gateway.auth.mode = "none"` on a private ingress) honor `x-oriro-scopes` only when the header is explicitly present
  - if `x-oriro-scopes` is absent on those identity-bearing plugin-route requests, runtime scope falls back to `operator.write`
- Practical rule: do not assume a gateway-auth plugin route is an implicit admin surface. If your route needs admin-only behavior, require an identity-bearing auth mode and document the explicit `x-oriro-scopes` header contract.

## Plugin SDK import paths

Use narrow SDK subpaths instead of the monolithic `oriro/plugin-sdk` root
barrel when authoring new plugins. Core subpaths:

| Subpath                             | Purpose                                            |
| ----------------------------------- | -------------------------------------------------- |
| `oriro/plugin-sdk/plugin-entry`  | Plugin registration primitives                     |
| `oriro/plugin-sdk/channel-core`  | Channel entry/build helpers                        |
| `oriro/plugin-sdk/core`          | Generic shared helpers and umbrella contract       |
| `oriro/plugin-sdk/config-schema` | Root `oriro.json` Zod schema (`OriroSchema`) |
...
<Info>
`oriro/plugin-sdk/channel-runtime`, `oriro/plugin-sdk/channel-lifecycle`,
small channel helper facades, `oriro/plugin-sdk/outbound-runtime`,
`oriro/plugin-sdk/outbound-send-deps`, `oriro/plugin-sdk/config-runtime`,
and `oriro/plugin-sdk/infra-runtime` are deprecated compatibility shims for
...
External plugins should only import `oriro/plugin-sdk/*` subpaths. Never
...
`oriro/plugin-sdk/directory-runtime`.
...
`catalog.run(...)` returns the same shape Oriro writes into
`models.providers`:
...
`catalog.order` controls when a plugin's catalog merges relative to Oriro's
...
- if both `catalog` and `discovery` are registered, Oriro uses `catalog`
...
- Read-only command paths such as `oriro status`, `oriro status --all`,
  `oriro channels status`, `oriro channels resolve`, and doctor/config
...
A plugin directory may include a `package.json` with `oriro.extensions`:
...
Security guardrail: every `oriro.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.

Security note: `oriro plugins install` installs plugin dependencies with a
...
Optional: `oriro.setupEntry` can point at a lightweight setup-only module.
When Oriro needs setup surfaces for a disabled channel plugin, or
...
Optional: `oriro.startup.deferConfiguredChannelFullLoadUntilAfterListen`
...
this flag. Keep the plugin on the default behavior and let Oriro load the
...
Channel plugins can advertise setup/discovery metadata via `oriro.channel` and
install hints via `oriro.install`. This keeps the core catalog data-free.

Example:

```json
{
  "name": "@oriro/nextcloud-talk",
  "oriro": {
    "extensions": ["./index.ts"],
    "channel": {
      "id": "nextcloud-talk",
...
Useful `oriro.channel` fields beyond the minimal example:
...
Oriro can also merge **external channel catalogs** (for example, an MPM
registry export). Drop a JSON file at one of:

- `~/.oriro/mpm/plugins.json`
- `~/.oriro/mpm/catalog.json`
- `~/.oriro/plugins/catalog.json`

Or point `ORIRO_PLUGIN_CATALOG_PATHS` (or `ORIRO_MPM_CATALOG_PATHS`) at
one or more JSON files (comma/semicolon/`PATH`-delimited). Each file should
contain `{ "entries": [ { "name": "@scope/pkg", "oriro": { "channel": {...}, "install": {...} } } ] }`. The parser also accepts `"packages"` or `"plugins"` as legacy aliases for the `"entries"` key.

Generated channel catalog entries and provider install catalog entries expose
normalized install-source facts next to the raw `oriro.install` block. The
...
export default function (api) {
  api.registerContextEngine("lossless-oriro", (ctx) => ({
    info: { id: "lossless-oriro", name: "Lossless Oriro", ownsCompaction: true },
    async ingest() {
...
2. add typed plugin registration/runtime surfaces
   Extend `OriroPluginApi` and/or `api.runtime` with the smallest useful
...
This is how Oriro stays opinionated without becoming hardcoded to one
