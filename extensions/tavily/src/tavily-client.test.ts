...
vi.mock("oriro/plugin-sdk/provider-web-search", () => ({
...
describe("tavily client X-Client-Source header", () => {
  ...
  it("runTavilySearch sends X-Client-Source: oriro", async () => {
    ...
    expect(params.extraHeaders).toEqual({ "X-Client-Source": "oriro" });
  });
  ...
  it("runTavilyExtract sends X-Client-Source: oriro", async () => {
