...
async function createParams(): Promise<EmbeddedRunAttemptParams> {
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-codex-projector-"));
...
describe("CodexAppServerEventProjector", () => {
  ...
  it("saves raw Codex image-generation results as reply media", async () => {
    const stateDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-codex-media-state-"));
  ...
  it("records dynamic Oriro tool calls in mirrored transcript snapshots", async () => {
    const projector = await createProjector();
  ...
  it("does not keep side-effect evidence for pre-execution dynamic tool errors", async () => {
    ...
    projector.recordDynamicToolResult({
      ...
      success: false,
      terminalType: "error",
      contentItems: [{ type: "inputText", text: "Unknown Oriro tool: message" }],
    });
  ...
  it("does not parse shell command text to suppress transcript progress", async () => {
    ...
    projector.recordDynamicToolCall({
      ...
      arguments: {
        command:
          '/bin/bash -lc \'/home/oriro/.oriro/workspace/bin/log_activity.sh "web_search" "Grilled salmon research"\'',
        cwd: "/workspace",
      },
    });
    projector.recordDynamicToolResult({
