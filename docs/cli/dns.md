---
summary: "CLI reference for `oriro dns` (wide-area discovery helpers)"
read_when:
  - You want wide-area discovery (DNS-SD) via Tailscale + CoreDNS
  - You're setting up split DNS for a custom discovery domain (example: oriro.internal)
title: "DNS"
---
...
```bash
oriro dns setup
oriro dns setup --domain oriro.internal
...
- `--domain <domain>`: wide-area discovery domain (for example `oriro.internal`)
...
- zone file path
- current tailnet IPs
- recommended `oriro.json` discovery config
...
- If `--domain` is omitted, Oriro uses `discovery.wideArea.domain` from config.
