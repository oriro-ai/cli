    ...
    response: {
      ...
      response: allowed
        ...
        : {
            ...
            message: `Oriro exec policy denied Claude native tool use (security=${turn.execPermission.security}, ask=${turn.execPermission.ask}).`,
          },
    },
  });
}
...
async function createClaudeLiveSession(params: {
  ...
  const managedRun = await params.supervisor.spawn({
    ...
    env: params.mcpCaptureKey
      ? { ...params.env, ORIRO_MCP_CLI_CAPTURE_KEY: params.mcpCaptureKey }
      : params.env,
    stdinMode: "pipe-open",
    captureOutput: false,
    onStdout: (chunk) => {
