...
function withStableOwnerDisplaySecretForTest(cfg: unknown): unknown {
  ...
  return {
    ...
    commands: {
      ...commands,
      ownerDisplaySecret: "oriro-test-owner-display-secret",
    },
  };
}

/** Writes a temp Oriro config, installs it as runtime state, then restores globals. */
export async function withTempConfig(params: {
  cfg: unknown;
  run: () => Promise<void>;
  prefix?: string;
}): Promise<void> {
  const prevConfigPath = process.env.ORIRO_CONFIG_PATH;

  const testConfig = withStableOwnerDisplaySecretForTest(params.cfg) as OriroConfig;
  const dir = await mkdtemp(path.join(os.tmpdir(), params.prefix ?? "oriro-test-config-"));
  ...
  try {
    ...
    if (prevConfigPath === undefined) {
      ...
      process.env.ORIRO_CONFIG_PATH = prevConfigPath;
    }
    clearConfigCache();
    resetConfigRuntimeState();
    clearSecretsRuntimeSnapshot();
