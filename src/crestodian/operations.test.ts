...
vi.mock("./overview.js", () => ({
  ...
  loadCrestodianOverview: vi.fn(async () => ({
    ...
    config: { path: "/tmp/oriro.json", exists: true, valid: true, issues: [], hash: null },
    tools: {
...
describe("parseCrestodianOperation", () => {
  ...
  it("parses plugin management operations", () => {
    ...
    expect(parseCrestodianOperation("install npm plugin @oriro/demo")).toEqual({
      kind: "plugin-install",
      spec: "npm:@oriro/demo",
    });
    expect(parseCrestodianOperation("plugin install orirohub:oriro-demo")).toEqual({
      kind: "plugin-install",
      spec: "orirohub:oriro-demo",
    });
    expect(parseCrestodianOperation("plugin uninstall oriro-demo")).toEqual({
  ...
  it("validates missing config without exiting the process", async () => {
    mockConfig.missing("/tmp/oriro.json");
    const { runtime, lines } = createCrestodianTestRuntime();
  ...
  it("installs plugins only after approval and audits the write", async () => {
    ...
    const plan = await executeCrestodianOperation(
      { kind: "plugin-install", spec: "orirohub:oriro-demo" },
      runtime,
      { deps: { runPluginInstall } },
    );
    expectRecordFields(plan as unknown as Record<string, unknown>, {
      applied: false,
      message: "Plan: install plugin orirohub:oriro-demo. Say yes to apply.",
    });
    ...
    expect(installCall[0]).toBe("orirohub:oriro-demo");
    ...
    expectAuditRecord(
      audit,
      {
        ...
        summary: "Installed plugin orirohub:oriro-demo",
      },
      { rescue: true, spec: "orirohub:oriro-demo" },
    );
  });
  ...
  it("uninstalls plugins only after approval and audits the write", async () => {
    ...
    const plan = await executeCrestodianOperation(
      { kind: "plugin-uninstall", pluginId: "oriro-demo" },
      runtime,
      { deps: { runPluginUninstall } },
    );
    expectRecordFields(plan as unknown as Record<string, unknown>, {
      applied: false,
      message: "Plan: uninstall plugin oriro-demo. Say yes to apply.",
    });
    ...
    expect(uninstallCall[0]).toBe("oriro-demo");
    ...
    expectAuditRecord(
      audit,
      {
        ...
        summary: "Uninstalled plugin oriro-demo",
      },
      { rescue: true, pluginId: "oriro-demo" },
    );
  });
