...
struct OriroHubSearchParams: Encodable {
    let query: String?
    let limit: Int
}

struct OriroHubSearchResponseLite: Decodable {
    let results: [OriroHubSearchResultLite]
}

struct OriroHubSearchResultLite: Decodable {
    let slug: String
...
struct OriroHubInstallParams: Encodable {
    let source = "orirohub"
    let slug: String
}
