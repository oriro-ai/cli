# Oriro iOS (Super Alpha)

This iOS app is super-alpha and internal-use only. The first public App Store release targets iPhone and connects to an Oriro Gateway as a `role: node`.
...
- `xcodegen`
- `fastlane`
- Apple account signed into Xcode for the canonical Oriro team (`FWJYW4S8P8`)
- Fastlane Apple Developer Portal session for the canonical Oriro team when creating bundle IDs or enabling services
- Release-owner access to the encrypted signing repo password (`MATCH_PASSWORD`)
- App Store Connect app already created for `ai.orirofoundation.app`
...
- Local development uses the canonical `ai.orirofoundation.app*` bundle IDs when the Oriro team is available, and unique `ai.orirofoundation.app.test.*` bundle IDs only for non-canonical fallback teams.
- App Store release uses canonical `ai.orirofoundation.app*` bundle IDs through a temporary generated xcconfig in `apps/ios/build/AppStoreRelease.xcconfig`.
...
- App Store release also switches the app to `OriroPushTransport=relay`, `OriroPushDistribution=official`, `OriroPushAPNsEnvironment=production`, and a production `aps-environment` entitlement.
...
- Release builds default to `https://ios-push-relay.oriro.ai`.
- Optional custom relay override: `ORIRO_PUSH_RELAY_BASE_URL=https://relay.example.com`
...
3. Confirm the App Store Connect app and Apple Developer identifiers/capabilities exist for:
   - `ai.orirofoundation.app`
   - `ai.orirofoundation.app.share`
   - `ai.orirofoundation.app.activitywidget`
   - `ai.orirofoundation.app.watchkitapp`
...
4. Optional: set a custom official relay URL for the build. If unset, the release flow uses `https://ios-push-relay.oriro.ai`.

```bash
export ORIRO_PUSH_RELAY_BASE_URL=https://relay.example.com
```
...
8. Expected outputs after a successful run:
   - `apps/ios/build/app-store/Oriro-<version>.ipa`
   - `apps/ios/build/app-store/Oriro-<version>.app.dSYM.zip`
...
- `apps/ios/Sources/Oriro.entitlements` derives `aps-environment` from the active build configuration/signing override.
- APNs token registration to gateway happens only after gateway connection (`push.apns.register`).
- Local/manual builds default to `OriroPushTransport=direct`, `OriroPushDistribution=local`, and a development `aps-environment` entitlement.
- Your selected team/profile must support Push Notifications for the app bundle ID you are signing.
- If push capability or provisioning is wrong, APNs registration fails at runtime (check Xcode logs for `APNs registration failed`).
- The gateway host also needs direct APNs auth configured separately with `ORIRO_APNS_TEAM_ID`, `ORIRO_APNS_KEY_ID`, and either `ORIRO_APNS_PRIVATE_KEY_P8` or `ORIRO_APNS_PRIVATE_KEY_PATH`.
- Recommended gateway-host storage for the APNs `.p8` file is `~/.oriro/credentials/apns/AuthKey_<KEYID>.p8` with restrictive permissions, then point `ORIRO_APNS_PRIVATE_KEY_PATH` at that file.
...
- Debug builds default to `OriroPushAPNsEnvironment=sandbox`; Release builds default to `production`.
...
- Gateway-side relay sending is configured through `gateway.push.apns.relay.baseUrl` in `oriro.json`. `ORIRO_APNS_RELAY_BASE_URL` remains a temporary env override only.
...
This exists to keep the hosted relay limited to genuine Oriro official builds and to ensure a
...
The iOS app is not a Codex Computer Use backend. Computer Use and `cua-driver mcp` are macOS desktop-control paths; iOS exposes device capabilities as Oriro node commands through the gateway. Agents can drive the iPhone canvas, camera, screen, location, voice, and other node capabilities with `node.invoke`, subject to iOS foreground/background limits.
