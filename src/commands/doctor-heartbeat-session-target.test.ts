...
describe("describeHeartbeatSessionTargetIssues", () => {
  ...
  beforeEach(() => {
    tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-heartbeat-doctor-"));
  });
  ...
  function cfgWithSession(session: string, target: string | null = "slack"): OriroConfig {
  ...
  function writeStore(cfg: OriroConfig, entries: Record<string, unknown>) {
