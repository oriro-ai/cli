...
function resolvePluginContractApiPath(rootDir: string): string | null {
  ...
  // (per package.json `oriro.runtimeExtensions`), while flat-layout plugins keep
  // them at <rootDir>/. Search both, preferring dist/ when running from built oriro
...
function loadExternalChannelSecretContractFromRecord(
  ...
  try {
    ...
    if (process.env.ORIRO_DEBUG_CHANNEL_CONTRACT_API === "1") {
