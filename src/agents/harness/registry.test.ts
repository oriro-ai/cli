...
describe("agent harness registry", () => {
  ...
  it("keeps model-specific harnesses behind plugin registration in auto mode", () => {
    ...
    process.env.ORIRO_AGENT_RUNTIME = "auto";
  ...
  it("falls back to Oriro for other models", () => {
    process.env.ORIRO_AGENT_RUNTIME = "auto";
  ...
  it("honors explicit provider Oriro runtime policy", () => {
    registerAgentHarness(makeHarness("plugin-harness", { priority: 200 }), {
      ownerPluginId: "plugin-a",
    });
    ...
    expect(
      selectAgentHarness({
        provider: "codex",
        modelId: "gpt-5.4",
        config: providerRuntimeConfig("codex", "oriro"),
      }).id,
    ).toBe("oriro");
  });
