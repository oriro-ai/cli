...
describe("context engine quarantine health", () => {
  it("lists persisted runtime quarantines when local process state is empty", async () => {
    await withStateDirEnv("oriro-context-engine-quarantine-", async () => {
      clearContextEngineRuntimeQuarantine();
      recordPersistedContextEngineQuarantine({
        engineId: "lossless-oriro",
        owner: "plugin:lossless-oriro",
        operation: "bootstrap",
    ...
    async () => {
      await withStateDirEnv("oriro-context-engine-quarantine-", async () => {
        await withLiveSiblingProcess(async (siblingProcessId) => {
          ...
          clearPersistedContextEngineQuarantineForProcess("lossless-oriro", process.pid);
  ...
  it("drops records from a previous incarnation of this PID", async () => {
    await withStateDirEnv("oriro-context-engine-quarantine-incarnation-", async () => {
      clearContextEngineRuntimeQuarantine();
      seedPersistedContextEngineQuarantineForTest({
        engineId: "lossless-oriro",
        owner: "plugin:lossless-oriro",
  ...
  it.runIf(hasProcessStartTimes)(
    ...
    async () => {
      await withStateDirEnv("oriro-context-engine-quarantine-pid-reuse-", async () => {
        await withLiveSiblingProcess(async (siblingProcessId) => {
          clearContextEngineRuntimeQuarantine();
  ...
  it("drops sibling records whose process identity cannot be verified", async () => {
    await withStateDirEnv("oriro-context-engine-quarantine-unverified-", async () => {
      await withLiveSiblingProcess(async (siblingProcessId) => {
        clearContextEngineRuntimeQuarantine();
  ...
  it("clears persisted quarantine records when owner engines unload", async () => {
    await withStateDirEnv("oriro-context-engine-quarantine-owner-", async () => {
      const owner = "plugin:lossless-oriro";
      registerContextEngineForOwner(
        "lossless-oriro",
        () => ({
          info: { id: "lossless-oriro", name: "Lossless Oriro", version: "1" },
          async ingest() {
