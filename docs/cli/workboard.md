---
summary: "CLI reference for `oriro workboard` cards, dispatch, and worker runs"
read_when:
...
`oriro workboard` is the terminal surface for the bundled
...
```bash
oriro plugins enable workboard
...
```bash
oriro workboard list [--board <id>] [--status <status>] [--json]
oriro workboard create <title...> [--notes <text>] [--status <status>] [--priority <priority>] [--agent <id>] [--board <id>] [--labels <items>] [--json]
oriro workboard show <id> [--json]
oriro workboard dispatch [--url <url>] [--token <token>] [--timeout <ms>] [--json]
```
...
```bash
oriro workboard list
oriro workboard list --board default --status ready
oriro workboard list --json
```
...
```bash
oriro workboard create "Fix stale worker heartbeat" --priority high --labels bug,workboard
oriro workboard create "Write Workboard docs" --status ready --agent docs-agent --board docs --notes "Cover CLI, slash command, dispatch, and SQLite state."
```
...
```bash
oriro workboard show 7f4a2c10
oriro workboard show 7f4a2c10 --json
```
...
```bash
oriro workboard dispatch
oriro workboard dispatch --json
oriro workboard dispatch --url http://127.0.0.1:18789 --token "$ORIRO_GATEWAY_TOKEN"
```
...
Local `list`, `create`, and `show` commands operate on the local Oriro state
directory used by the current profile. Use `--dev` or `--profile <name>` on the
top-level `oriro` command when you need a different state root.
...
```bash
oriro plugins inspect workboard --runtime --json
```
...
Then retry `oriro workboard dispatch`. Data-only fallback is useful for local
...
```bash
oriro workboard list --status ready
```
