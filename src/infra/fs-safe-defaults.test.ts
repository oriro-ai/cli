// Covers Oriro's default fs-safe Python helper configuration.
...
describe("fs-safe defaults", () => {
  afterEach(() => {
    ...
    delete process.env.ORIRO_FS_SAFE_PYTHON_MODE;
  });

  it("disables the Python helper by default in Oriro", async () => {
  ...
  it("honors the Oriro-specific env mode override", async () => {
    process.env.ORIRO_FS_SAFE_PYTHON_MODE = "auto";
