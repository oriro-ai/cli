...
read_when:
  - Setting up Oriro for the first time
  - Looking for common configuration patterns
  - Navigating to specific config sections
title: "Configuration"
---

Oriro reads an optional <Tooltip tip="JSON5 supports comments and trailing commas">**JSON5**</Tooltip> config from `~/.oriro-ai/cli.json`.
The active config path must be a regular file. Symlinked `oriro.json`
layouts are unsupported for Oriro-owned writes; an atomic write may replace
the path instead of preserving the symlink. If you keep config outside the
default state directory, point `ORIRO_CONFIG_PATH` directly at the real file.

If the file is missing, Oriro uses safe defaults. Common reasons to add a config:
...
<Tip>
**New to configuration?** Start with `oriro onboard` for interactive setup, or check out the [Configuration Examples](/gateway/configuration-examples) guide for complete copy-paste configs.
</Tip>
...
<Tabs>
  <Tab title="Interactive wizard">
    ```bash
    oriro onboard       # full onboarding flow
    oriro configure     # config wizard
    ```
  </Tab>
  ...
  <Tab title="Direct edit">
    Edit `~/.oriro-ai/cli.json` directly. The Gateway watches the file and applies changes automatically (see [hot reload](#config-hot-reload)).
  </Tab>
</Tabs>
...
<Warning>
Oriro only accepts configurations that fully match the schema. Unknown keys, malformed types, or invalid values cause the Gateway to **refuse to start**. The only root-level exception is `$schema` (string), so editors can attach JSON Schema metadata.
</Warning>

`oriro config schema` prints the canonical JSON Schema used by Control UI
...
- Only diagnostic commands work (`oriro doctor`, `oriro logs`, `oriro health`, `oriro status`)
- Run `oriro doctor` to see exact issues
- Run `oriro doctor --fix` (or `--yes`) to apply repairs

The Gateway keeps a trusted last-known-good copy after each successful startup,
but startup and hot reload do not restore it automatically. If `oriro.json`
...
Run `oriro doctor --fix` (or `--yes`) to repair prefixed/clobbered config or
...
<AccordionGroup>
  ...
  <Accordion title="Choose and configure models">
    ...
    - Use `oriro config set agents.defaults.models '<json>' --strict-json --merge` to add allowlist entries without removing existing models. Plain replacements that would remove entries are rejected unless you pass `--replace`.
  ...
  <Accordion title="Tune gateway WebSocket handshake timeout">
    ...
    - `ORIRO_HANDSHAKE_TIMEOUT_MS` still takes precedence for one-off service or shell overrides.
  ...
  <Accordion title="Enable relay-backed push for official iOS builds">
    Relay-backed push uses the hosted Oriro relay by default: `https://ios-push-relay.oriro.ai`.
    ...
    ```bash
    oriro config set gateway.push.apns.relay.baseUrl https://relay.example.com
    ```
    ...
    - `ORIRO_APNS_RELAY_BASE_URL` and `ORIRO_APNS_RELAY_TIMEOUT_MS` still work as temporary env overrides.
    - Custom gateway relay URLs must match the relay base URL baked into the official/TestFlight iOS build.
    - `ORIRO_APNS_RELAY_ALLOW_HTTP=true` remains a loopback-only development escape hatch; do not persist HTTP relay URLs in config.
  ...
  <Accordion title="Set up webhooks (hooks)">
    ...
    - Use a dedicated `hooks.token`; do not reuse active Gateway auth secrets (`gateway.auth.token` / `ORIRO_GATEWAY_TOKEN` or `gateway.auth.password` / `ORIRO_GATEWAY_PASSWORD`).
    - Hook auth is header-only (`Authorization: Bearer ...` or `x-oriro-token`); query-string tokens are rejected.
  ...
  <Accordion title="Split config into multiple files ($include)">
    ...
    - **Oriro-owned writes**: when a write changes only one top-level section
      backed by a single-file include such as `plugins: { $include: "./plugins.json5" }`,
      Oriro updates that included file and leaves `oriro.json` intact
    - **Unsupported write-through**: root includes, include arrays, and includes
      with sibling overrides fail closed for Oriro-owned writes instead of
      flattening the config
    - **Confinement**: `$include` paths must resolve under the directory holding
      `oriro.json`. To share a tree across machines or users, set
      `ORIRO_INCLUDE_ROOTS` to a path-list (`:` on POSIX, `;` on Windows) of
...
The Gateway watches `~/.oriro-ai/cli.json` and applies changes automatically - no manual restart needed for most settings.

Direct file edits are treated as untrusted until they validate. The watcher waits
for editor temp-write/rename churn to settle, reads the final file, and rejects
invalid external edits without rewriting `oriro.json`. Oriro-owned config
...
config`, inspect the config, run `oriro config validate`, then run `oriro
...
When you edit a source file that is referenced through `$include`, Oriro plans
...
```bash
oriro gateway call config.get --params '{}'  # capture payload.hash
oriro gateway call config.patch --params '{
...
Oriro reads env vars from the parent process plus:

- `.env` from the current working directory (if present)
- `~/.oriro/.env` (global fallback)
...
<Accordion title="Shell env import (optional)">
  If enabled and expected keys aren't set, Oriro runs your login shell and imports only the missing keys:
...
Env var equivalent: `ORIRO_LOAD_SHELL_ENV=1`
</Accordion>
...
```json5
{
  gateway: { auth: { token: "${ORIRO_GATEWAY_TOKEN}" } },
