...
import { isRecord as isPlainRecord } from "@oriro/normalization-core/record-coerce";
...
function normalizeModelProviderRefsForConfigMutation(
  providers: NonNullable<OriroConfig["models"]>["providers"] | undefined,
): unknown {
...
function normalizeConfigMutationModelRefs(cfg: OriroConfig): OriroConfig {
...
const CONFIG_SET_EXAMPLE_PROVIDER = formatCliCommand(
  "oriro config set secrets.providers.vault --provider-source file --provider-path /etc/oriro/secrets.json --provider-mode json",
);
...
const CONFIG_PATCH_EXAMPLE_STDIN = formatCliCommand("oriro config patch --stdin");
...
function formatDoctorHint(message: string): string {
  return `Run \`${formatCliCommand("oriro doctor --fix")}\` ${message}`;
}
...
function formatPluginInstallConfigSetError(): string {
  return [
    ...
    `  ${formatCliCommand("oriro plugins install <spec>")}`,
    `  ${formatCliCommand("oriro plugins update <plugin-id>")}`,
    `  ${formatCliCommand("oriro plugins uninstall <plugin-id>")}`,
  ].join("\n");
}
...
function collectDryRunSchemaErrors(params: { config: OriroConfig }): ConfigSetDryRunError[] {
...
async function runConfigOperations(params: {
  ...
  const nextConfig = normalizeConfigMutationModelRefs(next as OriroConfig);
...
export async function runConfigGet(opts: { path: string; json?: boolean; runtime?: RuntimeEnv }) {
  ...
  try {
    ...
    if (!res.found) {
      runtime.error(
        danger(
          `Config path not found: ${opts.path}. Run ${formatCliCommand("oriro config validate")} to inspect config shape.`,
        ),
      );
      runtime.exit(1);
      return;
...
export async function runConfigUnset(opts: {
  ...
  try {
    ...
    if (!unsetResult.removed) {
      ...
      runtime.error(
        danger(
          `Config path not found: ${opts.path}. Nothing was changed. Run ${formatCliCommand("oriro config get <path>")} first if you are unsure of the path.`,
        ),
      );
      runtime.exit(1);
      return;
...
export async function runConfigValidate(opts: { json?: boolean; runtime?: RuntimeEnv } = {}) {
  const runtime = opts.runtime ?? defaultRuntime;
  let outputPath = CONFIG_PATH ?? "oriro.json";
  ...
  try {
    ...
    if (!snapshot.exists) {
      if (opts.json) {
        ...
        runtime.error(
          `Create one with ${formatCliCommand("oriro onboard")} or run ${formatCliCommand("oriro doctor --fix")}.`,
        );
      }
      runtime.exit(1);
      return;
    ...
    if (!snapshot.valid) {
      ...
      if (opts.json) {
        ...
        runtime.error(`Inspect with ${formatCliCommand("oriro config validate")}.`);
      }
      runtime.exit(1);
      return;
    }
...
export function registerConfigCli(program: Command) {
  const cmd = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/config", "docs.oriro.ai/cli/config")}\n`,
    )
    .option(
      "--section <section>",
  ...
  setCommandJsonMode(cmd.command("set"), "parse-only")
    ...
    .option(
      "--dry-run",
      "Validate changes without writing oriro.json (checks run in builder/json/batch modes; exec SecretRefs are skipped unless --allow-exec is set)",
      false,
    )
    .option(
      "--allow-exec",
  ...
  cmd
    ...
    .option(
      "--dry-run",
      "Validate changes without writing oriro.json (checks schema and SecretRef resolvability; exec SecretRefs are skipped unless --allow-exec is set)",
      false,
    )
    .option(
      "--allow-exec",
  ...
  cmd
    .command("schema")
    .description("Print the JSON schema for oriro.json")
    .action(async () => {
