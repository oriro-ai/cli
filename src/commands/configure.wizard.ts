...
async function runGatewayHealthCheck(params: {
  ...
  const token = process.env.ORIRO_GATEWAY_TOKEN ?? configuredToken;
  const password = process.env.ORIRO_GATEWAY_PASSWORD ?? configuredPassword;
  ...
  try {
    ...
    note(
      [
        "Docs:",
        "https://docs.oriro.ai/gateway/health",
        "https://docs.oriro.ai/gateway/troubleshooting",
      ].join("\n"),
      "Health check help",
    );
  }
    ...
    await select({
      ...
      options: [
        ...
        },
        {
          ...
          hint: "Delete channel tokens/settings from oriro.json",
        },
      ],
...
async function promptWebToolsConfig(
  ...
  type WebSearchConfig = NonNullable<NonNullable<OriroConfig["tools"]>["web"]>["search"];
...
export async function runConfigureWizard(
  ...
  try {
    ...
    const baseConfig: OriroConfig = snapshot.valid
    ...
    if (snapshot.exists) {
      ...
      if (!snapshot.valid && snapshot.issues.length > 0) {
        note(
          [
            ...
            "",
            "Docs: https://docs.oriro.ai/gateway/configuration",
          ].join("\n"),
          "Config issues",
        );
      }
      if (!snapshot.valid) {
        outro(
          `Config invalid. Run \`${formatCliCommand("oriro doctor")}\` to repair it, then re-run configure.`,
        );
        runtime.exit(1);
        return;
      }
    ...
    const promptGatewayRunMode = async (): Promise<OnboardMode> => {
      ...
      const localProbePromise = (async () => {
        ...
        return probeGatewayReachable({
          url: localUrl,
          token: process.env.ORIRO_GATEWAY_TOKEN ?? baseLocalProbeToken,
          password: process.env.ORIRO_GATEWAY_PASSWORD ?? baseLocalProbePassword,
    ...
    if (shouldSkipGatewaySummary) {
      ...
      if (remoteUrl) {
        note(
          ["Remote Gateway:", remoteUrl, "Docs: https://docs.oriro.ai/gateway/remote"].join(
            "\n",
          ),
          "Gateway",
        );
    ...
    note(
      [
        ...
        gatewayStatusLine,
        "Docs: https://docs.oriro.ai/web/control-ui",
      ].join("\n"),
      "Control UI",
    );
