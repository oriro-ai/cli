...
import { normalizeTrimmedStringList } from "@oriro/normalization-core/string-normalization";
import { listOriroPluginManifestMetadata } from "../plugins/manifest-metadata-scan.js";
...
function formatOriroUserAgent(version: string): string {
  return `${ORIRO_ATTRIBUTION_ORIGINATOR}/${version}`;
}
...
function collectManifestProviderEndpoints(): ManifestProviderEndpointCacheEntry[] {
  const entries: ManifestProviderEndpointCacheEntry[] = [];
  for (const { manifest } of listOriroPluginManifestMetadata()) {
...
export function resolveProviderAttributionIdentity(
  ...
  return {
    product: ORIRO_ATTRIBUTION_PRODUCT,
...
function buildNvidiaAttributionPolicy(
  ...
  return {
    ...
    headers: {
      "X-BILLING-INVOKE-ORIGIN": ORIRO_ATTRIBUTION_PRODUCT,
    },
  };
}
...
function buildOpenAIAttributionPolicy(
  ...
  return {
    ...
    ...identity,
    headers: {
      originator: ORIRO_ATTRIBUTION_ORIGINATOR,
      version: identity.version,
      "User-Agent": formatOriroUserAgent(identity.version),
    },
  };
}
