...
describe("json output mode", () => {
  ...
  it("detects json output flags before argv terminators", () => {
    expect(hasJsonOutputFlag(["node", "oriro", "nodes", "list", "--json"])).toBe(true);
    expect(hasJsonOutputFlag(["node", "oriro", "nodes", "list", "--json=true"])).toBe(true);
    expect(hasJsonOutputFlag(["node", "oriro", "nodes", "--", "--json"])).toBe(false);
  });
