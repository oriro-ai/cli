// Runtime implementations for `oriro plugins` subcommands. Heavy plugin modules stay
...
function formatConfiguredRuntimePluginInstallSpec(params: {
  ...
  const orirohubSpec = params.orirohubSpec?.trim();
  const npmSpec = params.npmSpec?.trim();
  if (orirohubSpec && params.defaultChoice !== "npm") {
    return orirohubSpec;
  }
  return npmSpec ?? orirohubSpec ?? params.pluginId;
}
...
function formatBlockedRuntimePluginGuidance(params: {
  ...
  const alternative =
    pluginId === "acpx"
      ...
      : 'change the runtime policy to "oriro"';
...
function collectConfiguredRuntimePluginWarnings(params: {
  ...
  return collectConfiguredRuntimePluginIds(params.cfg, {
    ...
    return [
      `- Configured runtime "${runtimeId}" requires the ${candidate.label} plugin, but no enabled "${runtimeId}" plugin was found. Run "oriro doctor --fix" to install ${installSpec}, or install it manually with "oriro plugins install ${installSpec}".`,
    ];
  });
}
...
export async function runPluginsEnableCommand(idInput: string): Promise<void> {
  ...
  const cfg = (snapshot.sourceConfig ?? snapshot.config) as OriroConfig;
  ...
  let next: OriroConfig = enableResult.config;
...
export async function runPluginsRegistryCommand(opts: PluginRegistryOptions): Promise<void> {
  ...
  if (inspection.refreshReasons.length > 0) {
    ...
    lines.push(`${theme.muted("Repair:")} ${theme.command("oriro plugins registry --refresh")}`);
  }
...
export async function runPluginsDoctorCommand(): Promise<void> {
  ...
  if (shadowed.length > 0) {
    ...
    for (const diag of shadowed) {
      ...
      lines.push("  repair:");
      lines.push("    oriro plugins inspect " + (diag.pluginId ?? "<plugin-id>"));
      lines.push("    edit or remove the config-selected plugin source");
      lines.push("    oriro plugins registry --refresh");
      lines.push("    oriro gateway restart --force");
    }
  }
  ...
  const docs = formatDocsLink("/plugin", "docs.oriro.ai/plugin");
  lines.push("");
