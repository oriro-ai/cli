// QA OTEL Smoke runtime supports Oriro repository automation.
...
const DEFAULT_DOCKER_COLLECTOR_IMAGE =
  process.env.ORIRO_QA_OTEL_COLLECTOR_IMAGE || "otel/opentelemetry-collector:0.104.0";
...
const REQUIRED_METRIC_NAMES = ["oriro.harness.duration_ms"] as const;
...
const MAX_OTLP_COMPRESSED_BODY_BYTES = readPositiveIntegerEnv(
  "ORIRO_QA_OTEL_MAX_COMPRESSED_BODY_BYTES",
  2 * 1024 * 1024,
);
const MAX_OTLP_DECODED_BODY_BYTES = readPositiveIntegerEnv(
  "ORIRO_QA_OTEL_MAX_DECODED_BODY_BYTES",
  8 * 1024 * 1024,
);
const MAX_CAPTURED_BODY_TEXT_BYTES = readPositiveIntegerEnv(
  "ORIRO_QA_OTEL_MAX_CAPTURED_BODY_TEXT_BYTES",
  512 * 1024,
);
const QA_SUITE_TIMEOUT_MS = readPositiveIntegerEnv(
  "ORIRO_QA_OTEL_SUITE_TIMEOUT_MS",
  10 * 60 * 1000,
);
const QA_SUITE_KILL_GRACE_MS = readPositiveIntegerEnv("ORIRO_QA_OTEL_SUITE_KILL_GRACE_MS", 5000);
const MAX_STDOUT_DIAGNOSTIC_LINE_BYTES = readPositiveIntegerEnv(
  "ORIRO_QA_OTEL_MAX_STDOUT_DIAGNOSTIC_LINE_BYTES",
  512 * 1024,
);
...
function isStdoutDiagnosticLogRecord(value: unknown): value is StdoutDiagnosticLogRecord {
  return (
    ...
    value !== null &&
    !Array.isArray(value) &&
    objectValue(value, "signal") === "oriro.diagnostic.log"
  );
}
...
async function startDockerOtelCollector(
  ...
  const tempDir = await makeTempDir(path.join(osTmpdir(), "oriro-otel-collector-"));
  const configPath = path.join(tempDir, "collector.yaml");
  const containerName = `oriro-otel-smoke-${makeUuid()}`;
...
function buildQaEnv(port: number): NodeJS.ProcessEnv {
  ...
  env.OTEL_SERVICE_NAME = "oriro-qa-lab-otel-smoke";
  env.OTEL_SEMCONV_STABILITY_OPT_IN = "gen_ai_latest_experimental";
  env.ORIRO_QA_SUITE_PROGRESS = env.ORIRO_QA_SUITE_PROGRESS ?? "1";
  return env;
}
...
function isLatestGenAiModelCallSpan(span: CapturedSpan): boolean {
  ...
  return (
    ...
    typeof span.attributes["oriro.provider"] === "string" &&
    typeof span.attributes["oriro.model"] === "string"
  );
}
...
function assertSmoke(params: {
  ...
  if (spanNames.has("oriro.model.call")) {
    failures.push("legacy oriro.model.call span exported with GenAI semconv opt-in");
  }
  ...
  const contentKeys = [...attributeKeys].filter((key) => key.startsWith("oriro.content."));
  ...
  const modelErrorSpans = modelSpans.filter((span) => {
    ...
    return (
      ...
      Object.hasOwn(span.attributes, "oriro.errorCategory") ||
...
async function main() {
  ...
  try {
    ...
    const child = spawnOriro(buildQaArgs(options), buildQaEnv(exportPort));
