...
read_when:
  - You run oriro with no command after setup and want to understand Crestodian
  - You need a configless-safe way to inspect or repair Oriro
...
Crestodian is Oriro's local setup, repair, and configuration helper. It is
designed to stay reachable when the normal agent path is broken.

Running `oriro` with no command starts classic onboarding first when the
active config file is missing or has no authored settings (empty or
metadata-only). After a config file has authored settings, running `oriro`
with no command starts Crestodian in an interactive terminal. Running
`oriro crestodian` starts the same helper explicitly.

## What Crestodian shows

On startup, interactive Crestodian opens the same TUI shell used by
`oriro tui`, with a Crestodian chat backend. The chat log starts with a short
greeting:
...
Crestodian uses the same Oriro reference discovery as regular agents. In a Git checkout,
...
[https://github.com/oriro-ai/cli](https://github.com/oriro-ai/cli), with explicit
...
oriro
oriro crestodian
oriro crestodian --json
oriro crestodian --message "models"
oriro crestodian --message "validate config"
oriro crestodian --message "setup workspace ~/Projects/work model openai/gpt-5.5" --yes
oriro crestodian --message "set default model openai/gpt-5.5" --yes
oriro onboard --modern
```
...
config set-ref gateway.auth.token env ORIRO_GATEWAY_TOKEN
gateway status
restart gateway
agents
...
plugin install orirohub:oriro-codex-app-server
plugin uninstall oriro-codex-app-server
talk to work agent
...
- `oriro.json` is missing
- `oriro.json` is invalid
- the Gateway is down
- plugin command registration is unavailable
- no agent has been configured yet

`oriro --help` and `oriro --version` still use the normal fast paths.
Noninteractive bare `oriro` exits with a short message instead of printing
...
- create agents
- install plugins from OriroHub or npm
- uninstall plugins
...
```text
~/.oriro/audit/crestodian.jsonl
```

Discovery is not audited. Only applied operations and writes are logged.

`oriro onboard --modern` starts Crestodian as the modern onboarding preview.
Plain `oriro onboard` still runs classic onboarding.
...
planner turn through Oriro's normal runtime paths. It first uses the
configured Oriro model. If no configured model is usable yet, it can fall
...
`oriro tui`, `oriro chat`, and `oriro terminal` still open the normal
...
Oriro: Crestodian rescue mode. Gateway reachable: no. Config valid: no.
You: /crestodian restart gateway
Oriro: Plan: restart the Gateway. Reply /crestodian yes to apply.
You: /crestodian yes
Oriro: Applied. Audit entry written.
```
...
- Remote rescue cannot open the local TUI or switch into an interactive agent
  session. Use local `oriro` for agent handoff.
