...
private let appDefaultsSuites = ["ai.oriro.mac", "ai.oriro.mac.debug"]
...
func runConfigureRemote(_ args: [String]) {
    do {
        ...
        if opts.help {
            ...
            Usage:
              oriro-mac configure-remote --ssh-target <user@host[:port]> [--local-port <port>]
                                          [--remote-port <port>] [--token <token>] [--password <password>]
                                          [--identity <path>] [--project-root <path>] [--cli-path <path>] [--json]
              oriro-mac configure-remote --direct-url <ws://host:port|wss://host> [--token <token>]
            ...
            Options:
              ...
              --project-root <p>  Remote Oriro checkout for CLI commands.
              --cli-path <path>   Remote oriro executable or entrypoint.
              --json              Emit JSON.
              -h, --help          Show help.
            """)
            return
...
private func openOriroConfigURL() -> URL {
    ...
    return FileManager().homeDirectoryForCurrentUser.appendingPathComponent(".oriro/oriro.json")
}
...
private func writeAppDefaults(opts: ConfigureRemoteOptions, target: String) {
    for suite in appDefaultsSuites {
        ...
        defaults.set("remote", forKey: "oriro.connectionMode")
        setDefaultString(defaults, key: "oriro.remoteTarget", value: target)
        defaults.set(true, forKey: "oriro.onboardingSeen")
        defaults.set(appOnboardingVersion, forKey: "oriro.onboardingVersion")
        setDefaultStringIfProvided(defaults, key: "oriro.remoteIdentity", value: opts.identity)
        setDefaultStringIfProvided(defaults, key: "oriro.remoteProjectRoot", value: opts.projectRoot)
        setDefaultStringIfProvided(defaults, key: "oriro.remoteCliPath", value: opts.cliPath)
...
private func printConfigureRemoteOutput(_ output: ConfigureRemoteOutput, json: Bool) {
    ...
    print("Oriro macOS Remote Config")
