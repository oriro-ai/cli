...
```bash
oriro nodes describe --node <idOrNameOrIp>
oriro nodes canvas snapshot --node <idOrNameOrIp>
oriro logs --follow
```
...
oriro nodes status
oriro approvals get --node <idOrNameOrIp>
oriro approvals allowlist add --node <idOrNameOrIp> "/usr/bin/uname"
```
...
Node pairing is an identity/trust gate, not a per-command approval surface. For `system.run`, the per-node policy lives in that node's exec approvals file (`oriro approvals get --node ...`), not in the gateway pairing record.
