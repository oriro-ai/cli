...
service via the external `oriro` CLI (no embedded runtime). This gives you
...
- The app installs a per-user LaunchAgent labeled `ai.oriro.gateway`
  (or `ai.oriro.<profile>` when using `--profile`/`ORIRO_PROFILE`; legacy `com.oriro.*` is supported).
