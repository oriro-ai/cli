...
import { asFiniteNumber } from "@oriro/normalization-core/number-coercion";
import { asOptionalRecord as readRecord } from "@oriro/normalization-core/record-coerce";
import { normalizeOptionalString } from "@oriro/normalization-core/string-coerce";
import { ORIRO_RUNTIME_CONTEXT_CUSTOM_TYPE } from "../agents/internal-runtime-context.js";
...
import { isOriroDeliveryMirrorAssistantMessage } from "../shared/transcript-only-oriro-assistant.js";
...
function sanitizeChatHistoryContentBlock(
  ...
  if ("oriroReasoningReplay" in entry) {
    delete entry.oriroReasoningReplay;
    changed = true;
  }
...
function buildMessageToolVisibleReplyMirror(
  ...
  const transcriptMeta = readRecord((pending.completionAnchor ?? pending.anchor)["__oriro"]);
  if (transcriptMeta) {
    mirror["__oriro"] = { ...transcriptMeta };
  }
  return mirror;
}
...
function readMessageToolDeliveryMirrorText(message: Record<string, unknown>): string | undefined {
  ...
  if (!isOriroDeliveryMirrorAssistantMessage(message)) {
    return undefined;
  }
...
function readTtsSupplementMarker(
  ...
  const marker = message.oriroTtsSupplement;
...
function readChatHistoryRecordTimestampMs(message: unknown): number | undefined {
  const meta = readRecord(readRecord(message)?.["__oriro"]);
...
function isDisplayHiddenProjectedMessage(message: Record<string, unknown>): boolean {
  ...
  return message.role === "custom" && message.customType === ORIRO_RUNTIME_CONTEXT_CUSTOM_TYPE;
}
...
function oriroAssistantModel(message: Record<string, unknown>): string | undefined {
  return message.role === "assistant" &&
    message.provider === "oriro" &&
...
function isDuplicateAcpGatewayInjectedMessage(
  ...
  if (
    oriroAssistantModel(previousVisible) !== "acp-runtime" ||
    oriroAssistantModel(current) !== "gateway-injected"
  ) {
    return false;
  }
