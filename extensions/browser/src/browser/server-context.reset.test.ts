...
function localOriroProfile(): Parameters<typeof createProfileResetOps>[0]["profile"] {
  return {
    name: "oriro",
...
function createLocalOriroResetOps(
  params: Omit<Parameters<typeof createProfileResetOps>[0], "profile">,
) {
  return createProfileResetOps({ profile: localOriroProfile(), ...params });
}
...
function createStatelessResetOps(profile: Parameters<typeof createProfileResetOps>[0]["profile"]) {
  return createProfileResetOps({
    ...
    resolveOriroUserDataDir: (name: string) => `/tmp/${name}`,
  });
}
...
describe("createProfileResetOps", () => {
  ...
  it("stops local browser, closes playwright connection, and trashes profile dir", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-reset-"));
    const profileDir = path.join(tempRoot, "oriro");
    ...
    const ops = createLocalOriroResetOps({
      getProfileState,
      stopRunningBrowser,
      isHttpReachable,
      resolveOriroUserDataDir: () => profileDir,
    });
  ...
  it("forces playwright disconnect when loopback cdp is occupied by non-owned process", async () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-reset-no-own-"));
    const profileDir = path.join(tempRoot, "oriro");
    fs.mkdirSync(profileDir, { recursive: true });
