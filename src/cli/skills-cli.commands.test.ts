...
const mocks = vi.hoisted(() => {
  ...
  return {
    ...
    searchSkillsFromOriroHubMock: vi.fn(),
    installSkillFromOriroHubMock: vi.fn(),
    installSkillFromSourceMock: vi.fn(),
    updateSkillsFromOriroHubMock: vi.fn(),
    readTrackedOriroHubSkillSlugsMock: vi.fn(),
    readVerifiedOriroHubSkillSourceUrlMock: vi.fn(),
    resolveOriroHubSkillVerificationTargetMock: vi.fn(),
    readOriroHubSkillsLockfileStatusSyncMock: vi.fn((..._args: unknown[]) => ({ kind: "missing" })),
    resolveOriroHubSkillStatusLinkSyncMock: vi.fn(),
    resolveLocalSkillCardStatusSyncMock: vi.fn(),
    fetchOriroHubSkillVerificationMock: vi.fn(),
    fetchOriroHubSkillCardMock: vi.fn(),
...
const {
  ...
  searchSkillsFromOriroHubMock,
  installSkillFromOriroHubMock,
  installSkillFromSourceMock,
  updateSkillsFromOriroHubMock,
  readTrackedOriroHubSkillSlugsMock,
  readVerifiedOriroHubSkillSourceUrlMock,
  resolveOriroHubSkillVerificationTargetMock,
  readOriroHubSkillsLockfileStatusSyncMock,
  resolveOriroHubSkillStatusLinkSyncMock,
  resolveLocalSkillCardStatusSyncMock,
  fetchOriroHubSkillVerificationMock,
  fetchOriroHubSkillCardMock,
  buildWorkspaceSkillStatusMock,
  skillStatusReportFixture,
  defaultRuntime,
  runtimeLogs,
...
vi.mock("../skills/lifecycle/orirohub.js", () => ({
  searchSkillsFromOriroHub: (...args: unknown[]) => mocks.searchSkillsFromOriroHubMock(...args),
  installSkillFromOriroHub: (...args: unknown[]) => mocks.installSkillFromOriroHubMock(...args),
  updateSkillsFromOriroHub: (...args: unknown[]) => mocks.updateSkillsFromOriroHubMock(...args),
  readTrackedOriroHubSkillSlugs: (...args: unknown[]) =>
    mocks.readTrackedOriroHubSkillSlugsMock(...args),
  readVerifiedOriroHubSkillSourceUrl: (...args: unknown[]) =>
    mocks.readVerifiedOriroHubSkillSourceUrlMock(...args),
  resolveOriroHubSkillVerificationTarget: (...args: unknown[]) =>
    mocks.resolveOriroHubSkillVerificationTargetMock(...args),
  readOriroHubSkillsLockfileStatusSync: (...args: unknown[]) =>
    mocks.readOriroHubSkillsLockfileStatusSyncMock(...args),
  resolveOriroHubSkillStatusLinkSync: (...args: unknown[]) =>
    mocks.resolveOriroHubSkillStatusLinkSyncMock(...args),
  resolveLocalSkillCardStatusSync: (...args: unknown[]) =>
    mocks.resolveLocalSkillCardStatusSyncMock(...args),
}));

vi.mock("../infra/orirohub.js", () => ({
  fetchOriroHubSkillVerification: (...args: unknown[]) =>
    mocks.fetchOriroHubSkillVerificationMock(...args),
  fetchOriroHubSkillCard: (...args: unknown[]) => mocks.fetchOriroHubSkillCardMock(...args),
}));
...
describe("skills cli commands", () => {
  ...
  beforeEach(() => {
    ...
    readTrackedOriroHubSkillSlugsMock.mockReset();
    readVerifiedOriroHubSkillSourceUrlMock.mockReset();
    resolveOriroHubSkillVerificationTargetMock.mockReset();
    readOriroHubSkillsLockfileStatusSyncMock.mockReset();
    resolveOriroHubSkillStatusLinkSyncMock.mockReset();
    resolveLocalSkillCardStatusSyncMock.mockReset();
    fetchOriroHubSkillVerificationMock.mockReset();
    fetchOriroHubSkillCardMock.mockReset();
    ...
    readTrackedOriroHubSkillSlugsMock.mockResolvedValue([]);
    readVerifiedOriroHubSkillSourceUrlMock.mockReturnValue(undefined);
    readOriroHubSkillsLockfileStatusSyncMock.mockReturnValue({ kind: "missing" });
    resolveOriroHubSkillStatusLinkSyncMock.mockReturnValue(undefined);
    resolveLocalSkillCardStatusSyncMock.mockReturnValue(undefined);
    resolveOriroHubSkillVerificationTargetMock.mockResolvedValue({
      ok: true,
      slug: "agentreceipt",
    ...
    fetchOriroHubSkillVerificationMock.mockResolvedValue({
      ...
      publisher: { handle: "oriro" },
      version: { version: "1.2.3" },
      card: {
        available: true,
        url: "https://private.example.com/orirohub/api/v1/skills/agentreceipt/card?version=1.2.3",
      },
      artifact: {
    ...
    fetchOriroHubSkillCardMock.mockResolvedValue("# Agent Receipt\n\nGenerated by OriroHub.\n");
  ...
  it("searches OriroHub skills from the native CLI", async () => {
    searchSkillsFromOriroHubMock.mockResolvedValue([
      {
        slug: "calendar",
  ...
  it("installs a skill from OriroHub into the active workspace", async () => {
    installSkillFromOriroHubMock.mockResolvedValue({
      ok: true,
      slug: "calendar",
      version: "1.2.3",
    ...
    const installArgs = mockFirstObjectArg(installSkillFromOriroHubMock);
  ...
  it("passes owner-qualified OriroHub skill refs through to the installer", async () => {
    installSkillFromOriroHubMock.mockResolvedValue({
      ok: true,
      slug: "calendar",
      version: "1.2.3",
  ...
  it("documents owner-qualified OriroHub install refs in command help", () => {
    ...
    expect(help).toContain("oriro skills install @owner/weather");
    expect(help).not.toContain("oriro skills install weather");
  });
  ...
  it("installs a skill from a git source into the active workspace", async () => {
    ...
    expectLogger(installArgs.logger);
    expect(installSkillFromOriroHubMock).not.toHaveBeenCalled();
    expect(
      runtimeLogs.some((line) =>
  ...
  it("rejects --version for git and local source installs", async () => {
    ...
    expect(runtimeErrors).toContain("--version is only supported for OriroHub skill installs.");
    expect(installSkillFromOriroHubMock).not.toHaveBeenCalled();
    expect(installSkillFromSourceMock).not.toHaveBeenCalled();
  });
  ...
  it("installs a skill into the cwd-inferred agent workspace", async () => {
    ...
    expect(mockFirstObjectArg(installSkillFromOriroHubMock).workspaceDir).toBe(
  ...
  it("installs a skill into the shared global skills directory", async () => {
    installSkillFromOriroHubMock.mockResolvedValue({
      ...
      slug: "calendar",
      version: "1.2.3",
      targetDir: "/tmp/oriro-config/skills/calendar",
    });
    ...
    expect(installSkillFromOriroHubMock).toHaveBeenCalledWith(
      expect.objectContaining({
        workspaceDir: "/tmp/oriro-config",
      }),
    );
  });

  it("passes --force-install through for OriroHub skill installs", async () => {
    installSkillFromOriroHubMock.mockResolvedValue({
      ok: true,
      slug: "calendar",
      version: "1.2.3",
  ...
  it("updates all tracked OriroHub skills", async () => {
    readTrackedOriroHubSkillSlugsMock.mockResolvedValue(["calendar"]);
    updateSkillsFromOriroHubMock.mockResolvedValue([
      {
        ok: true,
        slug: "calendar",
    ...
    expect(readTrackedOriroHubSkillSlugsMock).toHaveBeenCalledWith("/tmp/workspace");
    const updateAllArgs = mockFirstObjectArg(updateSkillsFromOriroHubMock);
  ...
  it("passes --force-install through for OriroHub skill updates", async () => {
    readTrackedOriroHubSkillSlugsMock.mockResolvedValue(["calendar"]);
    updateSkillsFromOriroHubMock.mockResolvedValue([
      {
        ok: true,
  ...
  it("updates tracked OriroHub skills in the cwd-inferred agent workspace", async () => {
    routeWorkspaceByAgent();
    resolveAgentIdByWorkspacePathMock.mockReturnValue("writer");
    readTrackedOriroHubSkillSlugsMock.mockResolvedValue(["calendar"]);
    updateSkillsFromOriroHubMock.mockResolvedValue([
    ...
    expect(readTrackedOriroHubSkillSlugsMock).toHaveBeenCalledWith("/tmp/workspace-writer");
    const updateInferredArgs = mockFirstObjectArg(updateSkillsFromOriroHubMock);
  ...
  it("lets --agent override cwd-inferred workspace for updates", async () => {
    ...
    expect(resolveAgentIdByWorkspacePathMock).not.toHaveBeenCalled();
    const updateOverrideArgs = mockFirstObjectArg(updateSkillsFromOriroHubMock);
  ...
  it("updates tracked OriroHub skills in the shared global skills directory", async () => {
    readTrackedOriroHubSkillSlugsMock.mockResolvedValue(["calendar"]);
    updateSkillsFromOriroHubMock.mockResolvedValue([
      {
        ok: true,
    ...
    expect(resolveDefaultAgentIdMock).not.toHaveBeenCalled();
    expect(resolveAgentWorkspaceDirMock).not.toHaveBeenCalled();
    expect(readTrackedOriroHubSkillSlugsMock).toHaveBeenCalledWith("/tmp/oriro-config");
    expect(updateSkillsFromOriroHubMock).toHaveBeenCalledWith({
      workspaceDir: "/tmp/oriro-config",
      slug: undefined,
  ...
  it("updates a single tracked OriroHub skill in the shared global skills directory", async () => {
    readTrackedOriroHubSkillSlugsMock.mockResolvedValue(["calendar"]);
    updateSkillsFromOriroHubMock.mockResolvedValue([
      {
        ok: true,
  ...
  it("exits nonzero when a tracked OriroHub skill update fails", async () => {
    readTrackedOriroHubSkillSlugsMock.mockResolvedValue(["calendar"]);
    updateSkillsFromOriroHubMock.mockResolvedValue([
      {
        ok: false,
  ...
  it("rejects using --global and --agent together for updates", async () => {
    ...
    expect(runtimeErrors).toContain("Use either --global or --agent, not both.");
    expect(readTrackedOriroHubSkillSlugsMock).not.toHaveBeenCalled();
  ...
  it("verifies OriroHub skills with JSON output by default", async () => {
    await runCommand(["skills", "verify", "agentreceipt"]);

    expect(resolveOriroHubSkillVerificationTargetMock).toHaveBeenCalledWith({
      workspaceDir: "/tmp/workspace",
      slug: "agentreceipt",
      version: undefined,
      tag: undefined,
    ...
    expect(payload.schema).toBe("orirohub.skill.verify.v1");
    expect(payload.ok).toBe(true);
    expect(payload.signature).toEqual({ status: "unsigned" });
    expect(payload.oriro).toEqual({
      resolution: {
        source: "installed",
        selector: "installed-version",
        registry: "https://private.example.com/orirohub",
  ...
  it("passes owner-qualified installed verification targets to OriroHub verification", async () => {
    resolveOriroHubSkillVerificationTargetMock.mockResolvedValueOnce({
      ok: true,
      slug: "weather",
  ...
  it("includes verified OriroHub source URLs in verify JSON output", async () => {
    const provenance = {
    ...
    readVerifiedOriroHubSkillSourceUrlMock.mockReturnValueOnce(verifiedSourceUrl);
    ...
    expect(readVerifiedOriroHubSkillSourceUrlMock).toHaveBeenCalledWith(provenance);
    const payload = JSON.parse(runtimeStdout.at(-1) ?? "{}") as {
      oriro?: { verifiedSourceUrl?: string };
    };
    expect(payload.oriro?.verifiedSourceUrl).toBe(verifiedSourceUrl);
    expect(defaultRuntime.exit).not.toHaveBeenCalled();
  });
  ...
  it("fetches generated Skill Card markdown for --card", async () => {
    ...
    expect(fetchOriroHubSkillCardMock).toHaveBeenCalledWith({
      url: "https://cards.example.test/generated/agentreceipt.md",
      baseUrl: "https://private.example.com/orirohub",
    });
  ...
  it("fails --card when the verified Skill Card is unavailable", async () => {
    ...
    expect(fetchOriroHubSkillCardMock).not.toHaveBeenCalled();
    expect(defaultRuntime.writeJson).not.toHaveBeenCalled();
  });
  ...
  it.each([
    ...
    expect(runtimeErrors).toContain(
      "OriroHub verification response did not include a Skill Card URL.",
    );
    expect(fetchOriroHubSkillCardMock).not.toHaveBeenCalled();
    expect(defaultRuntime.writeJson).not.toHaveBeenCalled();
  });

  it("exits non-zero when the OriroHub verification envelope fails", async () => {
    fetchOriroHubSkillVerificationMock.mockResolvedValueOnce({
      schema: "orirohub.skill.verify.v1",
      ok: false,
      decision: "fail",
  ...
  it("fails before fetching when verification target resolution fails", async () => {
    ...
    expect(fetchOriroHubSkillVerificationMock).not.toHaveBeenCalled();
    expect(fetchOriroHubSkillCardMock).not.toHaveBeenCalled();
  });
  ...
  it("keeps non-JSON skills list output on stdout with human-readable formatting", async () => {
    ...
    expect(runtimeStdout.at(-1)).toContain("oriro skills search");
  });
});
