...
import type {
  ...
  RequestFrame,
  ResponseFrame,
} from "@oriro/gateway-protocol";
import {
  GATEWAY_CLIENT_MODES,
  GATEWAY_CLIENT_NAMES,
  type GatewayClientMode,
  type GatewayClientName,
} from "@oriro/gateway-protocol/client-info";
import {
...
} from "@oriro/gateway-protocol/connect-error-details";
import { resolveGatewayStartupRetryAfterMs } from "@oriro/gateway-protocol/startup-unavailable";
import { MIN_CLIENT_PROTOCOL_VERSION, PROTOCOL_VERSION } from "@oriro/gateway-protocol/version";
...
// The package stays reusable by depending on host callbacks for Oriro-owned
...
export class GatewayClient {
  ...
  constructor(opts: GatewayClientOptions) {
    this.deps = {
      // Defaults keep the package inert outside Oriro; device signing throws
  ...
  start() {
    ...
    const allowPrivateWs =
      (this.opts.env ?? process.env).ORIRO_ALLOW_INSECURE_PRIVATE_WS === "1";
    ...
    if (!isSecureWebSocketUrl(url, { allowPrivateWs })) {
      ...
      const error = new Error(
        `SECURITY ERROR: Cannot connect to "${displayHost}" over plaintext ws://. ` +
          ...
          (allowPrivateWs
            ? ""
            : "Break-glass (trusted private networks only): set ORIRO_ALLOW_INSECURE_PRIVATE_WS=1. ") +
          "Run `oriro doctor --fix` for guidance.",
      );
