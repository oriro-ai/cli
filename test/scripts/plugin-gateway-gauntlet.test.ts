...
describe("plugin gateway gauntlet helpers", () => {
  ...
  it("discovers bundled plugin manifests into lifecycle matrix rows", async () => {
    ...
    expect(matrix[0]).toEqual({
      ...
      id: "alpha",
      manifestPath: path.join("extensions", "alpha", "oriro.plugin.json"),
      name: "alpha",
  ...
  it("keeps manifest ids separate from bounded build entry ids", async () => {
    await writeManifest("kimi-coding", "oriro.plugin.json", JSON.stringify({ id: "kimi" }));
    ...
    expect(buildGauntletPrebuildEnv({}, { buildIds: [matrix[0].buildId] })).toEqual({
      ORIRO_BUNDLED_PLUGIN_BUILD_IDS: "kimi-coding",
      PNPM_CONFIG_VERIFY_DEPS_BEFORE_RUN: "false",
    });
  });

  it("skips source-only plugin dirs that are excluded from the built runtime", async () => {
    await writeManifest("qa-lab", "oriro.plugin.json", JSON.stringify({ id: "qa-lab" }));
    await writeManifest("qqbot", "oriro.plugin.json", JSON.stringify({ id: "qqbot" }));
    await writeManifest("telegram", "oriro.plugin.json", JSON.stringify({ id: "telegram" }));
  ...
  it("prebuilds private QA dist when QA chunks are enabled", () => {
    expect(buildGauntletPrebuildEnv({ EXISTING: "1" }, { includePrivateQa: true })).toEqual({
      ...
      ORIRO_BUNDLED_PLUGIN_BUILD_IDS: "qa-channel,qa-lab,qa-matrix",
  ...
  it("marks gauntlet prebuilds as runtime-only when requested", () => {
    expect(
      ...
      EXISTING: "1",
      ORIRO_BUNDLED_PLUGIN_BUILD_IDS: "acpx",
      ORIRO_RUN_NODE_SKIP_DTS_BUILD: "1",
      PNPM_CONFIG_VERIFY_DEPS_BEFORE_RUN: "false",
    });
  });
  ...
  it("prebuilds only selected plugin dist entries for bounded gauntlet runs", () => {
    expect(
      ...
      EXISTING: "1",
      ORIRO_BUILD_PRIVATE_QA: "1",
      ORIRO_BUNDLED_PLUGIN_BUILD_IDS: "acpx,active-memory,qa-channel,qa-lab,qa-matrix",
      ORIRO_ENABLE_PRIVATE_QA_CLI: "1",
      PNPM_CONFIG_VERIFY_DEPS_BEFORE_RUN: "false",
    });
  });
...
`,
  ...
  it("documents gauntlet guardrail options and env defaults in help", () => {
    ...
    for (const text of [
      ...
      "--build-timeout-ms",
      "--qa-timeout-ms",
      "ORIRO_PLUGIN_GATEWAY_GAUNTLET_IDS",
      "ORIRO_PLUGIN_GATEWAY_GAUNTLET_TOTAL",
      "ORIRO_PLUGIN_GATEWAY_GAUNTLET_INDEX",
      "ORIRO_PLUGIN_GATEWAY_GAUNTLET_FAIL_ON_OBSERVATION",
      "ORIRO_PLUGIN_GATEWAY_GAUNTLET_KEEP_RUN_ROOT",
      "ORIRO_PLUGIN_GATEWAY_GAUNTLET_QA_SUMMARY_MAX_BYTES",
    ]) {
  ...
  it("fails once when skip-prebuild leaves plugin lifecycle probes without a built entry", async () => {
    const outputDir = path.join(repoRoot, "artifacts");
    await writeManifest("acpx", "oriro.plugin.json", JSON.stringify({ id: "acpx" }));
  ...
  it("parses observation failure mode from CLI and env", () => {
    ...
    vi.stubEnv("ORIRO_PLUGIN_GATEWAY_GAUNTLET_FAIL_ON_OBSERVATION", "1");
  ...
  it("does not parse QA summary limit env when QA is skipped", () => {
    ...
    const result = spawnSync(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          ORIRO_PLUGIN_GATEWAY_GAUNTLET_QA_SUMMARY_MAX_BYTES: "not-a-number",
        },
      },
    );
  ...
  it("probes plugin-owned slash help while the plugin is installed", async () => {
    ...
    await fs.writeFile(
      ...
      [
        ...
        "const stateDir = process.env.ORIRO_STATE_DIR ?? process.cwd();",
        ...
        '    console.log("Usage: oriro workboard");',
        "    process.exit(0);",
        "  }",
  ...
  it("carries required plugin build ids and enables dependencies in QA chunks", async () => {
    ...
    await writeManifest("beta", "oriro.plugin.json", JSON.stringify({ id: "beta" }));
    ...
    await fs.writeFile(
      ...
      [
        ...
        'fs.writeFileSync(path.join(outputDir, "env.txt"), process.env.ORIRO_BUNDLED_PLUGIN_BUILD_IDS ?? "", "utf8");',
  ...
  it("fails successful QA chunks whose summary reports failed scenarios", async () => {
    ...
    await writeManifest("alpha", "oriro.plugin.json", JSON.stringify({ id: "alpha" }));
    await fs.writeFile(path.join(repoRoot, "extensions", "alpha", "index.ts"), "export {};\n");
    await fs.mkdir(path.join(repoRoot, "scripts"), { recursive: true });
    await fs.writeFile(
      path.join(repoRoot, "scripts", "run-node.mjs"),
  ...
  it("fails successful QA chunks that write oversized summary JSON", async () => {
    ...
    const result = spawnSync(
      ...
      ],
      {
        ...
        env: {
          ...process.env,
          ORIRO_PLUGIN_GATEWAY_GAUNTLET_QA_SUMMARY_MAX_BYTES: "64",
        },
      },
    );
