...
describe("scripts/test-live", () => {
  ...
  posixIt("signals the live pnpm child when the wrapper is terminated", async () => {
    const root = mkdtempSync(join(tmpdir(), "oriro-test-live-signal-"));
  ...
  it("rejects loose heartbeat intervals instead of parsing prefixes", () => {
    ...
    expect(resolveTestLiveHeartbeatMs({ ORIRO_LIVE_WRAPPER_HEARTBEAT_MS: "2500" })).toBe(2500);
    expect(() => resolveTestLiveHeartbeatMs({ ORIRO_LIVE_WRAPPER_HEARTBEAT_MS: "1e3" })).toThrow(
      "invalid ORIRO_LIVE_WRAPPER_HEARTBEAT_MS: 1e3",
    );
    expect(() =>
      resolveTestLiveHeartbeatMs({ ORIRO_LIVE_WRAPPER_HEARTBEAT_MS: "1000ms" }),
    ).toThrow("invalid ORIRO_LIVE_WRAPPER_HEARTBEAT_MS: 1000ms");
    expect(() => resolveTestLiveHeartbeatMs({ ORIRO_LIVE_WRAPPER_HEARTBEAT_MS: "0" })).toThrow(
      "invalid ORIRO_LIVE_WRAPPER_HEARTBEAT_MS: 0",
    );
  });
