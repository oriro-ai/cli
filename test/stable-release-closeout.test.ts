...
const release = {
  ...
  isPrerelease: false,
  assets: [
    { name: "Oriro-2026.6.8.zip", digest: `sha256:${"a".repeat(64)}` },
    { name: "Oriro-2026.6.8.dmg", digest: `sha256:${"b".repeat(64)}` },
    { name: "Oriro-2026.6.8.dSYM.zip", digest: `sha256:${"c".repeat(64)}` },
  ],
};
const changelog =
  "# Changelog\n\n## 2026.6.8\n\n### Fixes\n\n- Shipped fix.\n\n## 2026.6.7\n\n- Old.\n";
...
describe("stable release closeout", () => {
    ...
    const replay = verifyStableMainCloseout({
      ...
      release: {
        ...
        assets: [
          ...release.assets,
          {
            name: "oriro-2026.6.8-stable-main-closeout.json",
            digest: `sha256:${"d".repeat(64)}`,
          },
          {
            name: "oriro-2026.6.8-stable-main-closeout.json.sha256",
  ...
  it("requires the canonical macOS zip, dmg, and dSYM assets", () => {
    ...
    expect(result.errors).toContain(
      "GitHub release v2026.6.8 is missing required macOS asset(s): Oriro-2026.6.8.zip, Oriro-2026.6.8.dmg, Oriro-2026.6.8.dSYM.zip.",
    );
  });
  ...
  it("rejects speculative main state, appcast drift, and stale rollback drills", () => {
    ...
    expect(result.errors).toContain(
      "main appcast.xml does not point at Oriro-2026.6.8.zip from v2026.6.8.",
    );
    expect(result.errors).toContain(
