...
function isTempBackedOriroRoot(segments: readonly string[]): boolean {
  const lower = segments.map((segment) => segment.toLowerCase());
  const oriroIndex = lower.lastIndexOf("oriro");
  if (oriroIndex < 1) {
    return false;
  }
  return lower[oriroIndex - 1] === "tmp" || lower[oriroIndex - 1] === "temp";
}
...
function isBundledRuntimeSkillsPath(cachedPath: string, skillRootIndex: number): boolean {
  ...
  return (
    lower.some(
      (segment) =>
        segment === "dist-runtime" || segment === "node_modules" || segment.startsWith("oriro@"),
    ) || isTempBackedOriroRoot(beforeSkillRoot)
  );
}
