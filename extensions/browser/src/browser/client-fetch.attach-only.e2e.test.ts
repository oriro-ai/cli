...
import { clearRuntimeConfigSnapshot } from "oriro/plugin-sdk/runtime-config-snapshot";
...
describe("browser client fetch attachOnly diagnostics", () => {
  ...
  it("does not suggest gateway restart when an attachOnly CDP endpoint hangs", async () => {
    tempHome = await createTempHomeEnv("oriro-browser-client-fetch-live-");
    ...
    const configPath = path.join(tempHome.home, ".oriro", "oriro.json");
    await fs.writeFile(
      configPath,
      JSON.stringify(
        {
    ...
    try {
      ...
      expect(message).toContain("browser profile is external to Oriro");
      expect(message).toContain("Restarting the Oriro gateway will not launch it");
      expect(message).not.toContain("Restart the Oriro gateway");
