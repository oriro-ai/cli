...
# Oriro CLI installer (non-interactive, no onboarding)
# Usage: curl -fsSL --proto '=https' --tlsv1.2 https://oriro.ai/install-cli.sh | bash -s -- [--json] [--prefix <path>] [--version <ver>] [--node-version <ver>] [--onboard]
...
ORIRO_EFFECTIVE_HOME="$(resolve_oriro_effective_home)"
PREFIX="${ORIRO_PREFIX:-${HOME}/.oriro}"
ORIRO_VERSION="${ORIRO_VERSION:-latest}"
NODE_VERSION="${ORIRO_NODE_VERSION:-22.22.0}"
NODE_VERSION_REQUESTED=0
if [[ -n "${ORIRO_NODE_VERSION:-}" ]]; then
  NODE_VERSION_REQUESTED=1
fi
MIN_NODE_VERSION="22.19.0"
APK_NODE_BIN_DIR="/usr/bin"
NPM_LOGLEVEL="${ORIRO_NPM_LOGLEVEL:-error}"
INSTALL_METHOD="${ORIRO_INSTALL_METHOD:-npm}"
GIT_DIR="${ORIRO_GIT_DIR:-${ORIRO_EFFECTIVE_HOME}/oriro}"
...
Usage: install-cli.sh [options]
  ...
  --prefix <path>                     Install prefix (default: ~/.oriro; use \$ORIRO_PREFIX to override)
  --install-method, --method npm|git  Install via npm (default) or from a git checkout
  --npm                               Shortcut for --install-method npm
  --git, --github                     Shortcut for --install-method git
  --git-dir, --dir <path>             Checkout directory (default: ~/oriro, or \$ORIRO_HOME/oriro)
  --version <ver>                     Oriro version (default: latest)
...
Environment variables:
  ...
  ORIRO_HOME=...
  ORIRO_PREFIX=...
  ORIRO_VERSION=latest|next|<semver>
  ORIRO_GIT_DIR=...
...
cleanup_legacy_submodules() {
  local repo_dir="${1:-${ORIRO_GIT_DIR:-${ORIRO_EFFECTIVE_HOME}/oriro}}"
...
resolve_git_oriro_ref() {
  ...
  case "$requested" in
    ""|latest)
      resolved_version="$("$(npm_bin)" view "oriro" "dist-tags.${requested:-latest}" 2>/dev/null || true)"
...
install_oriro() {
  ...
  if [[ "${requested}" == "latest" ]]; then
    if ! env -u NPM_CONFIG_BEFORE -u npm_config_before -u NPM_CONFIG_MIN_RELEASE_AGE -u npm_config_min_release_age -u npm_config_min-release-age "$(npm_bin)" install -g --prefix "$(node_dir)" "${npm_args[@]}" "oriro@latest"; then
      log "npm install oriro@latest failed; retrying oriro@next"
      emit_json "{\"event\":\"step\",\"name\":\"oriro\",\"status\":\"retry\",\"version\":\"next\"}"
      env -u NPM_CONFIG_BEFORE -u npm_config_before -u NPM_CONFIG_MIN_RELEASE_AGE -u npm_config_min_release_age -u npm_config_min-release-age "$(npm_bin)" install -g --prefix "$(node_dir)" "${npm_args[@]}" "oriro@next"
      requested="next"
    fi
  else
    env -u NPM_CONFIG_BEFORE -u npm_config_before -u NPM_CONFIG_MIN_RELEASE_AGE -u npm_config_min_release_age -u npm_config_min-release-age "$(npm_bin)" install -g --prefix "$(node_dir)" "${npm_args[@]}" "oriro@${requested}"
  fi

  mkdir -p "${PREFIX}/bin"
  rm -f "${PREFIX}/bin/oriro"
  cat > "${PREFIX}/bin/oriro" <<EOF
#!/usr/bin/env bash
set -euo pipefail
exec "${PREFIX}/tools/node/bin/node" "$(node_dir)/lib/node_modules/oriro/dist/entry.js" "\$@"
EOF
  chmod +x "${PREFIX}/bin/oriro"
  emit_json "{\"event\":\"step\",\"name\":\"oriro\",\"status\":\"ok\",\"version\":\"${requested}\"}"
}
...
EOF
  chmod +x "${PREFIX}/bin/oriro"
  emit_json "{\"event\":\"step\",\"name\":\"oriro\",\"status\":\"ok\",\"method\":\"git\"}"
}

resolve_oriro_version() {
  local version=""
  if [[ -x "${PREFIX}/bin/oriro" ]]; then
    version="$("${PREFIX}/bin/oriro" --version 2>/dev/null | head -n 1 | tr -d '\r')"
  fi
  echo "$version"
}
...
is_gateway_daemon_loaded() {
  local oriro="$1"
  if [[ -z "$oriro" || ! -x "$oriro" ]]; then
    return 1
  fi

  local status_json=""
  status_json="$("$oriro" daemon status --json 2>/dev/null || true)"
...
refresh_gateway_service_if_loaded() {
  ...
  if ! "$oriro" gateway install --force >/dev/null 2>&1; then
  ...
  if ! "$oriro" gateway restart >/dev/null 2>&1; then
  ...
  "$oriro" gateway status --probe --json >/dev/null 2>&1 || true
...
main() {
  ...
  if [[ "$INSTALL_METHOD" == "git" ]]; then
    install_oriro_from_git "$GIT_DIR"
  ...
  if [[ "$RUN_ONBOARD" -eq 1 ]]; then
    "${PREFIX}/bin/oriro" onboard
  fi
}

if [[ "${ORIRO_INSTALL_CLI_SH_NO_RUN:-0}" != "1" ]]; then
  main "$@"
fi
