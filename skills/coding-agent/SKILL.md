...
Use for background feature builds, PR reviews, large refactors, and issue-to-PR loops. Do not use for simple edits, read-only lookup, ACP thread-bound work, or any run inside `~/.oriro`, `$ORIRO_STATE_DIR`, or active Oriro state dirs.
...
- Worker must send completion/failure via `oriro message send`.
...
- Never checkout branches or run background coding agents in `~/Projects/oriro`; use an isolated checkout.
...
oriro message send --channel <channel> --target '<target>' --message '<brief result>'
Add --account, --reply-to, or --thread-id only when present above.
Do not use oriro system event or heartbeat.
```
...
```bash
PROMPT=$(mktemp -t oriro-worker-prompt.XXXXXX)
cat >"$PROMPT" <<'EOF'
Task.
<notification block>
EOF
