...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.oriro.mac`) even if the shipped
    ...
    {
        if environment["ORIRO_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "oriro.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "oriro.nixMode") { return true }
