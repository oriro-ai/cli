...
function createOriroBusyResult(pid: number, overrides: Partial<MockLsofResult> = {}) {
  return createLsofResult({
    stdout: lsofOutput([{ pid, cmd: "oriro-gateway" }]),
    ...overrides,
  });
}
...
function installInitialBusyPoll(
  ...
  mockSpawnSync.mockImplementation((command: unknown) => {
    ...
    call += 1;
    if (call === 1) {
      return createOriroBusyResult(stalePid);
    }
...
describe.skipIf(isWindows)("restart-stale-pids", () => {
  ...
  describe("findGatewayPidsOnPortSync", () => {
    ...
    it("parses oriro-gateway pids and excludes the current process", () => {
      const stalePid = process.pid + 1;
      mockSpawnSync.mockReturnValue({
        error: null,
        status: 0,
        stdout: lsofOutput([
          { pid: stalePid, cmd: "oriro-gateway" },
          { pid: process.pid, cmd: "oriro-gateway" },
        ]),
        stderr: "",
      });
    ...
    it("verifies argv when lsof reports the node process name instead of oriro", () => {
      ...
      mockSpawnSync.mockImplementation((command: unknown) => {
        if (command === "ps") {
          return {
            error: null,
            status: 0,
            stdout: "node /opt/oriro/dist/entry.js gateway\n",
            stderr: "",
          };
        }
        return {
    ...
    it("excludes ancestor pids so a sidecar cannot kill its parent gateway — regression for #68451", () => {
      // Regression: oriro-weixin sidecar (child of the gateway) invoked
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stdout: lsofOutput([
          { pid: parentGatewayPid, cmd: "oriro-gateway" },
          { pid: unrelatedStalePid, cmd: "oriro-gateway" },
        ]),
        stderr: "",
      });
    ...
    it.skipIf(process.platform !== "linux")(
      ...
      () => {
        ...
        mockSpawnSync.mockReturnValue({
          ...
          status: 0,
          stdout: lsofOutput([
            { pid: directParentPid, cmd: "oriro-gateway" },
            { pid: grandparentPid, cmd: "oriro-gateway" },
            { pid: benignStalePid, cmd: "oriro-gateway" },
          ]),
          stderr: "",
        });
    ...
    it.skipIf(process.platform !== "linux")(
      ...
      () => {
        ...
        mockSpawnSync.mockReturnValue({
          ...
          status: 0,
          stdout: lsofOutput([
            { pid: pluginHostPid, cmd: "oriro-gateway" },
            { pid: gatewayGrandparentPid, cmd: "oriro-gateway" },
          ]),
          stderr: "",
        });
      ...
      try {
        mockSpawnSync.mockImplementation((command: unknown, args: unknown) => {
          ...
          return {
            ...
            status: 0,
            stdout: lsofOutput([
              { pid: toolHostPid, cmd: "oriro-gateway" },
              { pid: gatewayGrandparentPid, cmd: "oriro-gateway" },
              { pid: benignStalePid, cmd: "oriro-gateway" },
            ]),
            stderr: "",
    ...
    it("excludes pids whose command does not include 'oriro'", () => {
      ...
      try {
        mockSpawnSync.mockImplementation((command: unknown, args: unknown) => {
          ...
          return {
            error: null,
            status: 0,
            stdout: lsofOutput([{ pid: process.pid + 3152, cmd: "oriro-gateway" }]),
            stderr: "",
          };
        });
    ...
    it("returns verified gateway pids from Windows helpers on win32", () => {
      ...
      try {
        ...
        mockReadWindowsProcessArgs.mockReturnValue(["oriro", "gateway"]);
        expect(findGatewayPidsOnPortSync(18789)).toEqual([stalePid]);
        expect(mockReadWindowsListeningPids).toHaveBeenCalledWith(18789, undefined);
  ...
  describe("parsePidsFromLsofOutput (via findGatewayPidsOnPortSync stdout path)", () => {
    ...
    it("parses multiple oriro pids from a single lsof output block", () => {
      ...
      mockSpawnSync.mockReturnValue({
        ...
        status: 0,
        stdout: lsofOutput([
          { pid: pid1, cmd: "oriro-gateway" },
          { pid: pid2, cmd: "oriro-gateway" },
        ]),
        stderr: "",
      });
      const result = findGatewayPidsOnPortSync(18789);
    ...
    it("returns [] when status 0 but only non-oriro pids present", () => {
      // Port may be bound by an unrelated process. findGatewayPidsOnPortSync
      // only tracks oriro processes — non-oriro listeners are ignored.
  ...
  describe("pollPortOnce — no second lsof spawn (Codex P1 regression)", () => {
    ...
    it("lsof status 1 with non-empty oriro stdout is treated as busy, not free (Linux container edge case)", () => {
      ...
      // status 1 + non-empty oriro stdout must not be treated as port-free.
      const stalePid = process.pid + 601;
      const getCallCount = installInitialBusyPoll(stalePid, (call) => {
        if (call === 2) {
          // status 1 + oriro pid in stdout — container-restricted lsof reports partial results
          return createOriroBusyResult(stalePid, {
            status: 1,
    ...
    it("pollPortOnce outer catch returns { free: null, permanent: false } when resolveLsofCommandSync throws", () => {
      ...
      mockSpawnSync.mockImplementationOnce(() => {
        ...
        return {
          error: null,
          status: 0,
          stdout: lsofOutput([{ pid: stalePid, cmd: "oriro-gateway" }]),
          stderr: "",
        };
      });
    ...
    it("does not kill a protected gateway pid after reparenting", () => {
      ...
      mockSpawnSync.mockImplementation(() => {
        ...
        return lsofCall === 1
          ? createLsofResult({
              stdout: lsofOutput([
                { pid: protectedPid, cmd: "oriro-gateway" },
                { pid: stalePid, cmd: "oriro-gateway" },
              ]),
            })
  ...
  describe("parsePidsFromLsofOutput — branch coverage (lines 67-69)", () => {
    it("skips a mid-loop entry when the command does not include 'oriro'", () => {
      // Exercises the false branch of currentCmd.toLowerCase().includes("oriro")
      // inside the mid-loop flush: a non-oriro cmd between two entries must not
      // be pushed, but the following oriro entry still must be.
      const stalePid = process.pid + 700;
      // Mixed output: non-oriro entry first, then oriro entry
    ...
    it("ignores a p-line with an invalid (non-positive) PID — ternary false branch", () => {
      ...
      // p0 is invalid (not > 0); the following valid oriro entry must still be found.
      const stdout = `p0\ncoriro-gateway\np${stalePid}\ncoriro-gateway\n`;
      mockSpawnSync.mockReturnValue({ error: null, status: 0, stdout, stderr: "" });
      const result = findGatewayPidsOnPortSync(18789);
      expect(result).toContain(stalePid);
  ...
  // -------------------------------------------------------------------------
  // pollPortOnce branch — status 1 + non-empty stdout with zero oriro pids
  // -------------------------------------------------------------------------
  describe("pollPortOnce — status 1 + non-empty non-oriro stdout (line 145)", () => {
    it("treats status 1 + non-oriro stdout as port-free (not an oriro process)", () => {
      // status 1 + non-empty stdout where no oriro pids are present:
      // the port may be held by an unrelated process. From our perspective
      // (we only kill oriro pids) it is effectively free.
      const stalePid = process.pid + 800;
      const getCallCount = installInitialBusyPoll(stalePid, () => {
        // status 1 + non-oriro output — should be treated as free:true for our purposes
        return createLsofResult({
          status: 1,
      ...
      vi.spyOn(process, "kill").mockReturnValue(true);
      // No oriro pids in status-1 output means the port is free for this cleanup.
      expect(cleanStaleGatewayProcessesSync()).toContain(stalePid);
      ...
      try {
        mockSpawnSync.mockImplementation((command: unknown, args: unknown) => {
          ...
          if (command === "lsof") {
            ...
            return createLsofResult({
              stdout: lsofOutput([{ pid: gatewayParentPid, cmd: "oriro-gateway" }]),
            });
          }
          return createLsofResult();
        });
