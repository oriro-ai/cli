...
Message presentation is Oriro's shared contract for rich outbound chat UI.
...
```bash
oriro message send --channel slack \
  --target channel:C123 \
...
```bash
oriro message send --channel telegram \
...
Use helpers from `oriro/plugin-sdk/interactive-runtime` when bridging older
code:
