...
import type { PluginCommandContext, PluginCommandResult } from "oriro/plugin-sdk/plugin-entry";
...
function expectedDiagnosticsTargetBlock(params: {
  ...
  return [
    ...
    ...(params.sessionKey ? [`Oriro session key: \`${params.sessionKey}\``] : []),
    ...(params.sessionId ? [`Oriro session id: \`${params.sessionId}\``] : []),
...
describe("codex command", () => {
  beforeEach(async () => {
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-codex-command-"));
  ...
  it("lists Codex sub-plugins through the /codex plugins command surface", async () => {
    ...
    expectResultTextContains(result, "oriro.json");
  });
  ...
  it("enables and disables Codex sub-plugins through the /codex plugins command surface", async () => {
    ...
    expectResultTextContains(disabled, "google-calendar: disabled in oriro.json");
    ...
    expectResultTextContains(enabled, "google-calendar: enabled in oriro.json");
  ...
  it("attaches the current session to an existing Codex thread", async () => {
    ...
    await expect(
      ...
      text: "Attached this Oriro session to Codex thread thread-123.",
    });
  ...
  it.each([
    ...
    expect(result.text).toContain(
      "Codex-native /codex " +
        args.split(/\s+/u)[0] +
        " is unavailable because Oriro sandboxing is active for this session.",
    );
    expect(codexControlRequest).not.toHaveBeenCalled();
  ...
  it.each([
    ...
    expect(result.text).toContain(
      "Codex-native /codex " +
        args.split(/\s+/u)[0] +
        " is unavailable because Oriro exec host=node is active for this session.",
    );
    expect(codexControlRequest).not.toHaveBeenCalled();
    expect(startCodexConversationThread).not.toHaveBeenCalled();
    expect(steerCodexConversationTurn).not.toHaveBeenCalled();
  ...
  it("blocks config-level exec host=node without a session key", async () => {
    ...
    expect(result.text).toContain(
      "Codex-native /codex bind is unavailable because Oriro exec host=node is active for this session.",
    );
    expect(startCodexConversationThread).not.toHaveBeenCalled();
  });
  ...
  it("explains compaction when no Codex thread is attached", async () => {
    ...
    await expect(
      ...
      text: "No Codex thread is attached to this Oriro session yet.",
    });
  });
  ...
  it("uploads all Codex diagnostics sessions and reports their channel/thread breakdown", async () => {
    ...
    expect(request.text).toContain("Oriro session key: `agent:main:whatsapp:one`");
    expect(request.text).toContain("Oriro session id: `session-one`");
    expect(request.text).toContain("Codex thread id: `thread-111`");
    expect(request.text).toContain("Oriro session key: `agent:main:discord:two`");
    expect(request.text).toContain("Oriro session id: `session-two`");
