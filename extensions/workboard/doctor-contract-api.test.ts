...
import { createPluginStateKeyedStoreForTests as createPluginStateKeyedStore } from "oriro/plugin-sdk/plugin-state-test-runtime";
import type {
...
describe("workboard doctor contract", () => {
  it("migrates shipped .28 plugin-state workboard data into sqlite", async () => {
    const stateDir = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-workboard-doctor-"));
    const env = { ...process.env, ORIRO_STATE_DIR: stateDir };
    try {
