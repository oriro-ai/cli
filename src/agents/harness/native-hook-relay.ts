...
const NATIVE_HOOK_RELAY_STATE_SYMBOL = Symbol.for("oriro.nativeHookRelay.state");
...
export function buildNativeHookRelayCommand(params: {
  ...
  const executable = params.executable ?? resolveOriroCliExecutable();
  const argv =
    executable === "oriro"
      ? ["oriro"]
...
function registerNativeHookRelayBridge(registration: ActiveNativeHookRelayRegistration): void {
  ...
  // directory is scoped by OS user (uid) and is shared across all Oriro
...
function nativeHookRelayBridgeDir(): string {
  ...
  return path.join(tmpdir(), `oriro-native-hook-relays-${uid}`);
}
...
function nativeHookRelayPermissionAllowAlwaysKey(params: {
  ...
  hash.update("oriro:native-hook-relay:permission-allow-always:v2");
  hash.update("\0");
...
function readNativeHookRelayApprovalMode(rawPayload: JsonValue): "report" | undefined {
  const payload = isJsonObject(rawPayload) ? rawPayload : {};
  return payload.oriro_approval_mode === "report" ? "report" : undefined;
}
...
async function requestNativeHookRelayPermissionApproval(
  ...
  const requestResult: {
    ...
    {
      pluginId: `oriro-native-hook-relay-${request.provider}`,
      title: truncateText(
...
function resolveOriroCliExecutable(): string {
  const envPath = process.env.ORIRO_CLI_PATH?.trim();
  ...
  if (packageRoot) {
    for (const candidate of [
      path.join(packageRoot, "oriro.mjs"),
