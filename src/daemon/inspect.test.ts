...
// Real content from the oriro-gateway.service unit file (the canonical gateway unit).
const GATEWAY_SERVICE_CONTENTS = `\
[Unit]
Description=Oriro Gateway (v2026.3.8)
After=network-online.target
Wants=network-online.target

[Service]
ExecStart=/usr/bin/node /home/oriro/.npm-global/lib/node_modules/oriro/dist/entry.js gateway --port 18789
Restart=always
Environment=ORIRO_SERVICE_MARKER=oriro
Environment=ORIRO_SERVICE_KIND=gateway
Environment=ORIRO_SERVICE_VERSION=2026.3.8

[Install]
WantedBy=default.target
`;

// Real content from the oriro-test.service unit file (a non-gateway oriro service).
const TEST_SERVICE_CONTENTS = `\
[Unit]
Description=Oriro test service
...
[Unit]
Description=Oriro companion worker
After=oriro-gateway.service
Requires=oriro-gateway.service

[Service]
ExecStart=/usr/bin/node /opt/oriro-worker/dist/index.js worker
`;

const CUSTOM_ORIRO_GATEWAY_CONTENTS = `\
[Unit]
Description=Custom Oriro gateway

[Service]
ExecStart=/usr/bin/node /opt/oriro/dist/entry.js gateway --port 18888
`;

describe("detectMarkerLineWithGateway", () => {
  it("returns null for oriro-test.service (oriro only in description, no gateway on same line)", () => {
    expect(detectMarkerLineWithGateway(TEST_SERVICE_CONTENTS)).toBeNull();
  });

  it("returns oriro for the canonical gateway unit (ExecStart has both oriro and gateway)", () => {
  ...
  it("handles line continuations — marker and gateway split across physical lines", () => {
    const contents = `[Service]\nExecStart=/usr/bin/node /opt/oriro/dist/entry.js \\\n  gateway --port 18789\n`;
    expect(detectMarkerLineWithGateway(contents)).toBe("oriro");
  });
  ...
  it("ignores non-gateway ExecStart commands that only pass gateway-named options", () => {
    const contents = `[Service]\nExecStart=/usr/bin/oriro-helper --gateway-url http://127.0.0.1:18789 sync\n`;
...
describe("findExtraGatewayServices (linux / scanSystemdDir) — real filesystem", () => {
  ...
  it.skipIf(!isLinux)("does not report oriro-test.service as a gateway service", async () => {
    const tmpHome = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-test-"));
    const systemdDir = path.join(tmpHome, ".config", "systemd", "user");
    try {
      await fs.mkdir(systemdDir, { recursive: true });
      await fs.writeFile(path.join(systemdDir, "oriro-test.service"), TEST_SERVICE_CONTENTS);
  ...
  it.skipIf(!isLinux)(
    "does not report the canonical oriro-gateway.service as an extra service",
    async () => {
      const tmpHome = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-test-"));
      const systemdDir = path.join(tmpHome, ".config", "systemd", "user");
      try {
        await fs.mkdir(systemdDir, { recursive: true });
        await fs.writeFile(
          path.join(systemdDir, "oriro-gateway.service"),
  ...
  it.skipIf(!isLinux)(
    ...
    async () => {
      ...
      try {
        await fs.mkdir(systemdDir, { recursive: true });
        await fs.writeFile(
          path.join(systemdDir, "oriro-companion.service"),
  ...
  it.skipIf(!isLinux)(
    "reports custom-named gateway units that execute oriro gateway",
    async () => {
      const tmpHome = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-test-"));
      const systemdDir = path.join(tmpHome, ".config", "systemd", "user");
      const unitPath = path.join(systemdDir, "custom-oriro.service");
      try {
        await fs.mkdir(systemdDir, { recursive: true });
        await fs.writeFile(unitPath, CUSTOM_ORIRO_GATEWAY_CONTENTS);
        const result = await findExtraGatewayServices({ HOME: tmpHome });
        expect(result).toEqual([
          {
            platform: "linux",
            label: "custom-oriro.service",
            detail: `unit: ${unitPath}`,
            scope: "user",
            marker: "oriro",
            legacy: false,
...
<key>KeepAlive</key><dict><key>OtherJobEnabled</key><dict><key>ai.oriro.gateway</key><true/></dict></dict>
<key>ProgramArguments</key><array><string>/usr/local/bin/oriro-helper</string><string>sync</string></array>
</dict></plist>`,
      );
      const result = await findExtraGatewayServices({ HOME: tmpHome });
      expect(result).toStrictEqual([]);
...
<key>ProgramArguments</key><array><string>/usr/local/bin/oriro-helper</string><string>--gateway-url</string><string>http://127.0.0.1:18789</string><string>sync</string></array>
</dict></plist>`,
      );
      const result = await findExtraGatewayServices({ HOME: tmpHome });
      expect(result).toStrictEqual([]);
...
<key>ProgramArguments</key><array><string>/opt/homebrew/bin/watchman</string><string>--foreground</string></array>
  ...
  it("reports custom LaunchAgents that execute oriro gateway", async () => {
    const tmpHome = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-test-"));
    const launchdDir = path.join(tmpHome, "Library", "LaunchAgents");
    const plistPath = path.join(launchdDir, "com.example.oriro-gateway.plist");
    try {
      await fs.mkdir(launchdDir, { recursive: true });
      await fs.writeFile(
        plistPath,
        `<?xml version="1.0" encoding="UTF-8"?>
<plist version="1.0"><dict>
<key>Label</key><string>com.example.oriro-gateway</string>
<key>ProgramArguments</key><array><string>/usr/local/bin/oriro</string><string>gateway</string><string>--port</string><string>18888</string></array>
</dict></plist>`,
      );
      const result = await findExtraGatewayServices({ HOME: tmpHome });
      expect(result).toEqual([
        {
          platform: "darwin",
          label: "com.example.oriro-gateway",
...
describe("findExtraGatewayServices (win32)", () => {
  ...
  it("collects only non-oriro marker tasks from schtasks output", async () => {
    // Real schtasks /Query /FO LIST /V output prefixes root-folder task
    // names with a backslash (e.g. TaskName:\Oriro Gateway).
    execSchtasksMock.mockResolvedValueOnce({
      code: 0,
      stdout: [
  ...
  it("reports duplicate root tasks that only share the gateway task prefix", async () => {
    execSchtasksMock.mockResolvedValueOnce({
      ...
      stdout: [
        ...
        "Task To Run: C:\\Program Files\\Oriro\\oriro.exe gateway run",
        "",
        "TaskName:\\Oriro Gateway (dev)",
        "Task To Run: C:\\Program Files\\Oriro\\oriro.exe gateway run --profile dev",
        "",
        "TaskName:\\Oriro Gateway Backup",
        "Task To Run: C:\\Program Files\\Oriro\\oriro.exe gateway run",
        "",
      ].join("\n"),
      stderr: "",
    ...
    expect(result).toEqual([
      {
        platform: "win32",
        label: "\\Oriro Gateway Backup",
        detail:
          "task: \\Oriro Gateway Backup, run: C:\\Program Files\\Oriro\\oriro.exe gateway run",
        scope: "system",
        marker: "oriro",
        legacy: false,
      },
