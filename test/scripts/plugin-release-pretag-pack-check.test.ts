...
import { ORIRO_PLUGIN_NPM_REPOSITORY_URL } from "../../scripts/lib/plugin-npm-release.ts";
import {
...
function createDualPublishPluginRepo() {
  const repoDir = makeTempRepoRoot(tempDirs, "oriro-plugin-pretag-pack-");
  const packageDir = join(repoDir, "extensions", "demo-plugin");
  mkdirSync(packageDir, { recursive: true });
  writeJsonFile(join(repoDir, "package.json"), { name: "oriro-test-root", type: "module" });
...
describe("scripts/plugin-release-pretag-pack-check.ts", () => {
  it("collects dual-published plugin targets for npm and OriroHub pack checks", () => {
  ...
  it("runs runtime build, npm pack, and OriroHub pack commands for selected targets", () => {
    const repoDir = createDualPublishPluginRepo();
    ...
    expect(callOptions(1)).toMatchObject({
      cwd: repoDir,
      env: { ORIRO_PLUGIN_NPM_RUNTIME_BUILD: "0" },
      stdio: ["inherit", "ignore", "inherit"],
    });

    expect(execFileSyncMock.mock.calls[2]?.slice(0, 2)).toEqual([
      "bash",
      ["scripts/plugin-orirohub-publish.sh", "--pack", "extensions/demo-plugin"],
    ]);
    ...
    expect(callOptions(2).env?.ORIRO_ORIROHUB_PACK_OUTPUT_DIR).toContain("orirohub-0");
  });
});
