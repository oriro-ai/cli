...
const agentTurnTimeoutSeconds = readPositiveIntEnv(
  "ORIRO_LIVE_PLUGIN_TOOL_TIMEOUT_SECONDS",
  300,
);
...
const AGENT_OUTPUT_MAX_BYTES = readPositiveIntEnv(
  "ORIRO_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_MAX_BYTES",
  1024 * 1024,
);
const SESSION_FILE_LIST_LIMIT = 20;
const SESSION_SCAN_MAX_ENTRIES = readPositiveIntEnv(
  "ORIRO_LIVE_PLUGIN_TOOL_SESSION_SCAN_MAX_ENTRIES",
  50_000,
);
...
function stateDir() {
  return process.env.ORIRO_STATE_DIR || path.join(process.env.HOME, ".oriro");
}

function configPath() {
  return process.env.ORIRO_CONFIG_PATH || path.join(stateDir(), "oriro.json");
}

function agentOutputPath() {
  return process.env.ORIRO_LIVE_PLUGIN_TOOL_AGENT_OUTPUT_PATH || "/tmp/oriro-agent.json";
}

function agentErrorPath() {
  return process.env.ORIRO_LIVE_PLUGIN_TOOL_AGENT_ERROR_PATH || "/tmp/oriro-agent.err";
}
...
function pluginInstallPath() {
  ...
  const inspect = fs.existsSync("/tmp/oriro-plugin-inspect.json")
    ? readJson("/tmp/oriro-plugin-inspect.json")
    : {};
...
function writeFixture() {
  ...
  writeJson(path.join(dir, "oriro.plugin.json"), {
    id: pluginId,
    name: "E2E Slug Tool",
...
function assertInstalled() {
  ...
  const list = readJson("/tmp/oriro-plugins-list.json");
  ...
  const inspect = readJson("/tmp/oriro-plugin-inspect.json");
