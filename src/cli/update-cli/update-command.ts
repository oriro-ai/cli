...
import { disableCurrentOriroUpdateLaunchdJob } from "../../daemon/launchd.js";
...
const POST_CORE_UPDATE_ENV = "ORIRO_UPDATE_POST_CORE";
const POST_CORE_UPDATE_CHANNEL_ENV = "ORIRO_UPDATE_POST_CORE_CHANNEL";
const POST_CORE_UPDATE_REQUESTED_CHANNEL_ENV = "ORIRO_UPDATE_POST_CORE_REQUESTED_CHANNEL";
const POST_CORE_UPDATE_RESULT_PATH_ENV = "ORIRO_UPDATE_POST_CORE_RESULT_PATH";
const POST_CORE_UPDATE_INSTALL_RECORDS_PATH_ENV = "ORIRO_UPDATE_POST_CORE_INSTALL_RECORDS_PATH";
const POST_CORE_UPDATE_STARTED_AT_ENV = "ORIRO_UPDATE_POST_CORE_STARTED_AT_MS";
...
function restoreDroppedPreUpdateChannels(
  ...
  const authoredChannels = resolveRestoredAuthoredChannels({
    ...
    currentAuthoredChannels: isRecord(snapshot.parsed)
      ? (snapshot.parsed as OriroConfig).channels
...
function formatPostUpdatePluginInspectGuidance(pluginId: string): string {
  return `Run oriro plugins inspect ${pluginId} --runtime --json for details.`;
}
...
export function buildInvalidConfigPostCoreUpdateResult(): {
  ...
  const guidance = [
    "Run `oriro doctor` to inspect the config validation errors.",
    "Once the config parses, rerun `oriro update repair`.",
  ];
  const message =
...
function formatPostUpdateGatewayRecoveryLine(platform: NodeJS.Platform): string {
  const restartCommand = replaceCliName(formatCliCommand("oriro gateway restart"), CLI_NAME);
  const installCommand = replaceCliName(
    formatCliCommand("oriro gateway install --force"),
    CLI_NAME,
  );
...
function formatGatewayAncestryBlockMessage(pid: number): string {
  return `oriro update detected it is running inside the gateway process tree.
...
function isRunningInsideGatewayService(
  ...
  if (env.ORIRO_SERVICE_MARKER?.trim() !== GATEWAY_SERVICE_MARKER) {
    return false;
  }
  const serviceKind = env.ORIRO_SERVICE_KIND?.trim();
...
async function resolvePackageRuntimePreflightError(params: {
  ...
  return [
    `${runtimeLabel} is too old for oriro@${targetLabel}.`,
    `The requested package requires ${status.nodeEngine}.`,
    runtime.nodeRunner
      ? "Upgrade the Node runtime that owns the managed Gateway service, then rerun `oriro update`."
      : "Upgrade Node to 22.19+ or Node 24, then rerun `oriro update`.",
    "Bare `npm i -g oriro` can silently install an older compatible release.",
    "After upgrading Node, use `npm i -g oriro@latest`.",
  ].join("\n");
}
...
function stripGatewayServiceMarkerEnv(env: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  delete resolvedEnv.ORIRO_SERVICE_MARKER;
  delete resolvedEnv.ORIRO_SERVICE_KIND;
  ...
  if (!status.profileInstalled) {
    ...
    if (isCancel(shouldInstall) || !shouldInstall) {
      if (!opts.skipPrompt) {
        defaultRuntime.log(
          theme.muted(
            `Skipped. Run \`${replaceCliName(formatCliCommand("oriro completion --install"), CLI_NAME)}\` later to enable.`,
          ),
        );
      }
      return;
        ...
        const doctorStep = await runUpdateStep({
          ...
          env: {
            ...
            ...(candidateHostVersion === null
              ? {}
              : { ORIRO_COMPATIBILITY_HOST_VERSION: candidateHostVersion }),
          },
          timeoutMs: params.timeoutMs,
        });
...
export async function updatePluginsAfterCoreUpdate(params: {
  ...
  if (pluginsChanged) {
    ...
    if (params.restoredAuthoredChannels !== undefined) {
      nextConfig = {
        ...nextConfig,
        channels: structuredClone(params.restoredAuthoredChannels) as OriroConfig["channels"],
      };
    }
...
async function maybeRestartService(params: {
  ...
  const verifyRestartedGateway = async (
    ...
    const diagnosticLines = [
      ...
      `Run \`${replaceCliName(formatCliCommand("oriro gateway status --deep"), CLI_NAME)}\` for details.`,
  ...
  if (params.shouldRestart) {
    ...
    try {
      ...
      if (!params.opts.json && restarted) {
        ...
        defaultRuntime.log("");
        await createUpdateConfigSnapshot();
        process.env.ORIRO_UPDATE_IN_PROGRESS = "1";
        ...
        try {
          ...
          delete process.env.ORIRO_UPDATE_IN_PROGRESS;
      ...
      if (!params.opts.json) {
        ...
        defaultRuntime.log(
          theme.muted(
            `You may need to restart the service manually: ${replaceCliName(formatCliCommand("oriro gateway restart"), CLI_NAME)}`,
          ),
        );
      }
      if (
  ...
  if (!params.opts.json) {
    ...
    if (params.result.mode === "npm" || params.result.mode === "pnpm") {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("oriro doctor"), CLI_NAME)}\`, then \`${replaceCliName(formatCliCommand("oriro gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    } else {
      defaultRuntime.log(
        theme.muted(
          `Tip: Run \`${replaceCliName(formatCliCommand("oriro gateway restart"), CLI_NAME)}\` to apply updates to a running gateway.`,
        ),
      );
    }
  }
...
function withUpdateFinalizationEnv<T>(run: () => Promise<T>): Promise<T> {
  const previousUpdateInProgress = process.env.ORIRO_UPDATE_IN_PROGRESS;
  ...
  return run().finally(() => {
    if (previousUpdateInProgress === undefined) {
      ...
      process.env.ORIRO_UPDATE_IN_PROGRESS = previousUpdateInProgress;
    }
    ...
    (configSnapshot.valid
      ? {
          ...
          authoredConfig: isRecord(configSnapshot.parsed)
            ? (configSnapshot.parsed as OriroConfig)
...
function normalizePreUpdateConfigRestoreInput(
  ...
  if (isRecord(sourceConfig) && isRecord(authoredConfig)) {
    return {
      sourceConfig: sourceConfig as OriroConfig,
      authoredConfig: authoredConfig as OriroConfig,
    };
  }
  const authored = parsed as OriroConfig;
  return {
...
function resolvePreUpdateSourceConfigFromAuthored(
  ...
  try {
    ...
    return isRecord(resolved) ? (resolved as OriroConfig) : authoredConfig;
  } catch {
...
async function continuePostCoreUpdateInFreshProcess(params: {
  ...
  const resultDir = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-update-post-core-"));
    ...
    const child = spawn(params.nodeRunner ?? resolveNodeRunner(), argv, {
      ...
      env: {
        ...
        ...(postCoreHostVersion === null
          ? {}
          : { ORIRO_COMPATIBILITY_HOST_VERSION: postCoreHostVersion }),
...
async function updateCommandInternal(opts: UpdateCommandOptions): Promise<void> {
  ...
  if (opts.dryRun !== true) {
    await disableCurrentOriroUpdateLaunchdJob().catch(() => undefined);
    assertConfigWriteAllowedInCurrentMode();
  }
  ...
  if (postCoreUpdateResume) {
    ...
    process.env.ORIRO_COMPATIBILITY_HOST_VERSION = (await readPackageVersion(root)) ?? VERSION;
  ...
  const devTargetRef =
    channel === "dev" ? process.env.ORIRO_UPDATE_DEV_TARGET_REF?.trim() || undefined : undefined;
  ...
  if (result.status === "skipped") {
    ...
    if (result.reason === "dirty") {
      ...
      defaultRuntime.log(
        theme.muted("Commit, stash, or discard the local changes, then rerun `oriro update`."),
      );
    }
    if (result.reason === "not-git-install") {
      ...
      defaultRuntime.log(
        theme.muted(
          `Examples: \`${replaceCliName("npm i -g oriro@latest", CLI_NAME)}\` or \`${replaceCliName("pnpm add -g oriro@latest", CLI_NAME)}\``,
        ),
      );
    }
    defaultRuntime.exit(0);
  ...
  if (!pluginsUpdatedInFreshProcess) {
    ...
    const previousCompatibilityHostVersion = process.env.ORIRO_COMPATIBILITY_HOST_VERSION;
    if (compatibilityDowngradeTarget) {
      process.env.ORIRO_COMPATIBILITY_HOST_VERSION = compatibilityDowngradeTarget;
    }
    try {
      ...
      if (compatibilityDowngradeTarget) {
        if (previousCompatibilityHostVersion === undefined) {
          ...
          process.env.ORIRO_COMPATIBILITY_HOST_VERSION = previousCompatibilityHostVersion;
        }
      }
    }
  }
