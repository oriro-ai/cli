...
import { createOriroChannelMcpServer, OriroChannelBridge } from "./channel-server.js";
...
async function connectMcpWithoutGateway(params?: { claudeChannelMode?: "auto" | "on" | "off" }) {
  const serverHarness = await createOriroChannelMcpServer({
...
describe("oriro channel mcp server", () => {
  ...
  describe("gateway-backed flows", () => {
    describe("gateway integration", () => {
      ...
      test("lists conversations and reads messages", async () => {
        ...
        const bridge = new OriroChannelBridge({} as never, {
        ...
        expect((messages[1]?.["__oriro"] as { id?: string } | undefined)?.id).toBe(
          "msg-attachment",
        );
        expect(
