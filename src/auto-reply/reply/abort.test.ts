...
describe("abort detection", () => {
  ...
  async function createAbortConfig(params?: {
    ...
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-abort-"));
  ...
  it("isAbortRequestText aligns abort command semantics", () => {
    ...
    expect(isAbortRequestText("stop oriro!!!")).toBe(true);
    ...
    expect(isAbortRequestText("/stop@oriro_bot", { botUsername: "oriro_bot" })).toBe(true);
    expect(isAbortRequestText("/Stop@oriro_bot", { botUsername: "oriro_bot" })).toBe(true);
