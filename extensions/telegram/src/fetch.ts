...
} from "oriro/plugin-sdk/fetch-runtime";
import {
...
} from "oriro/plugin-sdk/proxy-capture";
import { resolveRequestUrl } from "oriro/plugin-sdk/request-url";
...
// fix for the leak observed in oriro#68128 is the transport lifecycle that
...
function resolveOriroProxyUrlForTelegram(
  env: NodeJS.ProcessEnv = process.env,
): string | undefined {
  const proxyUrl = env.ORIRO_PROXY_URL?.trim();
...
export function resolveTelegramTransport(
  ...
  const managedProxyUrl =
    !effectiveProxyFetch && !hasEnvProxy ? resolveOriroProxyUrlForTelegram() : undefined;
