...
import { parseStrictInteger } from "@oriro/normalization-core/number-coercion";
...
export function buildDockerExecArgs(params: {
  ...
  for (const [key, value] of Object.entries(params.env)) {
    ...
    // PATH is handled separately via ORIRO_PREPEND_PATH below.
    if (key === "PATH") {
      continue;
    }
  ...
  if (hasCustomPath) {
    ...
    args.push("-e", `ORIRO_PREPEND_PATH=${params.env.PATH}`);
  }
  ...
  const pathExport = hasCustomPath
    ? 'export PATH="${ORIRO_PREPEND_PATH}:$PATH"; unset ORIRO_PREPEND_PATH; '
    : "";
