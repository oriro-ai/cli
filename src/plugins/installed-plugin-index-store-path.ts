...
import type { OriroStateDatabaseOptions } from "../state/oriro-state-db.js";
...
function resolveStoreEnv(options: InstalledPluginIndexStoreOptions): NodeJS.ProcessEnv {
  return options.stateDir
    ? { ...(options.env ?? process.env), ORIRO_STATE_DIR: options.stateDir }
...
export function resolveInstalledPluginIndexStorePath(
  ...
  return resolveOriroStateSqlitePath(resolveStoreEnv(options));
}
