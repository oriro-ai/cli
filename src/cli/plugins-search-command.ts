// OriroHub-backed plugin search command; queries installable plugin families and merges scores.
import { normalizeOptionalString } from "@oriro/normalization-core/string-coerce";
import { theme } from "../../packages/terminal-core/src/theme.js";
import {
  searchOriroHubPackages,
  type OriroHubPackageFamily,
  type OriroHubPackageSearchResult,
} from "../infra/orirohub.js";
import { formatErrorMessage } from "../infra/errors.js";
import { defaultRuntime, writeRuntimeJson, type RuntimeEnv } from "../runtime.js";

/** Options accepted by `oriro plugins search`. */
export type PluginsSearchOptions = {
  json?: boolean;
  limit?: number;
};

const INSTALLABLE_PLUGIN_FAMILIES: OriroHubPackageFamily[] = ["code-plugin", "bundle-plugin"];
...
function mergePackageSearchResults(
  groups: readonly OriroHubPackageSearchResult[][],
  limit: number,
): OriroHubPackageSearchResult[] {
  const byName = new Map<string, OriroHubPackageSearchResult>();
  ...
  const selected: OriroHubPackageSearchResult[] = [];
...
function formatPackageSearchLine(entry: OriroHubPackageSearchResult): string {
  ...
  return `${pkg.name}  ${theme.muted(flags.join(" | "))}${summary}\n  ${theme.muted(`Install: oriro plugins install orirohub:${pkg.name}`)}`;
}

/** Search OriroHub for installable plugins and write JSON or terminal output. */
export async function runPluginsSearchCommand(
  ...
  if (!query) {
    runtime.error("Usage: oriro plugins search <query>");
  ...
  try {
    ...
    if (results.length === 0) {
      runtime.log("No OriroHub plugins found.");
      return;
    }
    runtime.log(`${theme.heading("OriroHub plugins")} ${theme.muted(`(${results.length})`)}`);
