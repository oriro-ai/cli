...
read_when:
  - You are building an external app, script, dashboard, CI job, or IDE extension that talks to Oriro
...
External apps should talk to Oriro through the Gateway protocol today. Use
...
<Warning>
  There is no public npm client package yet. Do not add Oriro client package
...
<Note>
  This page is for code outside the Oriro process. Plugin code that runs
  inside Oriro should use documented `oriro/plugin-sdk/*` subpaths instead.
</Note>
...
| [`oriro agent`](/cli/agent)          | Ready  | One-shot script integration when shelling out to the CLI is enough.                           |
| [`oriro message`](/cli/message)      | Ready  | Sending messages or channel actions from scripts.                                             |
...
4. Pin the Oriro version you test against.
5. Recheck the RPC reference when upgrading Oriro.
...
Use Gateway RPC when code lives outside Oriro:
...
Use the Plugin SDK when code runs inside Oriro:
...
External apps should not import `oriro/plugin-sdk/*`; those subpaths are for
plugins loaded by Oriro.
