...
const resolveShellEnvExpectedKeys = vi.fn((_env?: NodeJS.ProcessEnv) => ["ORIRO_GATEWAY_TOKEN"]);
...
const writeDiagnosticStabilityBundleForFailureSync = vi.fn((_reason: string, _error: unknown) => ({
  ...
  message: "wrote stability bundle: /tmp/oriro-stability.json",
  path: "/tmp/oriro-stability.json",
}));
const controlUiState = vi.hoisted(() => ({
  root: "/tmp/oriro-control-ui" as string | null,
}));
...
vi.mock("../../config/config.js", () => ({
  getConfigPath: () => "/tmp/oriro-test-missing-config.json",
...
vi.mock("../../config/paths.js", () => ({
  CONFIG_PATH: "/tmp/oriro-test-missing-config.json",
...
describe("gateway run option collisions", () => {
  ...
  beforeEach(() => {
    ...
    controlUiState.root = "/tmp/oriro-control-ui";
  ...
  it("loads configured shell env fallback before final proxy refresh and gateway startup", async () => {
    await withEnvAsync({ ORIRO_GATEWAY_TOKEN: undefined }, async () => {
      const finalConfig = {
        env: {
          shellEnv: { enabled: true, timeoutMs: 1234 },
          vars: { ORIRO_GATEWAY_TOKEN: "config-token" },
        },
        gateway: {
          auth: { mode: "token", token: "${ORIRO_GATEWAY_TOKEN}" },
          mode: "local",
        },
        proxy: { enabled: true, proxyUrl: "http://127.0.0.1:29876" },
      };
      ...
      readConfigFileSnapshotWithPluginMetadata
        ...
        .mockImplementationOnce(async (options) => {
          expect(options?.lowerPrecedenceEnv).toEqual({
            ORIRO_GATEWAY_TOKEN: "shell-token",
          });
          expect(process.env.ORIRO_GATEWAY_TOKEN).toBe("shell-token");
          return {
            snapshot: {
              ...configState.snapshot,
              config: {
      ...
      loadShellEnvFallback.mockImplementationOnce((opts?: unknown) => {
        callOrder.push("shell-env");
        (opts as { env: NodeJS.ProcessEnv }).env.ORIRO_GATEWAY_TOKEN = "shell-token";
      });
      const uninstall = installGatewayRunRuntimeHooks({ refreshManagedProxy });
      try {
        await runGatewayCli(["gateway"]);
      ...
      expect(loadShellEnvFallback).toHaveBeenCalledWith({
        enabled: true,
        env: process.env,
        expectedKeys: ["ORIRO_GATEWAY_TOKEN"],
        logger: expect.any(Object),
        timeoutMs: 1234,
      });
      expect(readConfigFileSnapshotWithPluginMetadata).toHaveBeenCalledWith(
        expect.objectContaining({
          lowerPrecedenceEnv: { ORIRO_GATEWAY_TOKEN: "shell-token" },
        }),
      );
      expect(readConfigFileSnapshotWithPluginMetadata).toHaveBeenCalledTimes(2);
      expect(process.env.ORIRO_GATEWAY_TOKEN).toBe("config-token");
      expect(clearShellEnvAppliedKeys).toHaveBeenCalledWith(["ORIRO_GATEWAY_TOKEN"]);
  ...
  it("uses config env shell fallback controls without mutating the live env during planning", async () => {
    await withEnvAsync(
      {
        ORIRO_GATEWAY_TOKEN: undefined,
        ORIRO_LOAD_SHELL_ENV: undefined,
        ORIRO_SHELL_ENV_TIMEOUT_MS: undefined,
      },
      async () => {
        const finalConfig = {
          env: {
            vars: {
              ORIRO_LOAD_SHELL_ENV: "1",
              ORIRO_SHELL_ENV_TIMEOUT_MS: "4321",
            },
          },
          gateway: { auth: { mode: "none" }, mode: "local" },
        };
        ...
        shouldEnableShellEnvFallback.mockImplementationOnce(
          (env?: NodeJS.ProcessEnv) => env?.ORIRO_LOAD_SHELL_ENV === "1",
        );
        resolveShellEnvFallbackTimeoutMs.mockImplementationOnce((env?: NodeJS.ProcessEnv) =>
          Number(env?.ORIRO_SHELL_ENV_TIMEOUT_MS),
        );
        ...
        expect(process.env.ORIRO_LOAD_SHELL_ENV).toBe("1");
        expect(process.env.ORIRO_SHELL_ENV_TIMEOUT_MS).toBe("4321");
      },
    );
  });
  ...
  it("honors config env shell fallback deferral", async () => {
    await withEnvAsync(
      {
        ORIRO_DEFER_SHELL_ENV_FALLBACK: undefined,
        ORIRO_LOAD_SHELL_ENV: undefined,
      },
      async () => {
        const finalConfig = {
          env: {
            vars: {
              ORIRO_DEFER_SHELL_ENV_FALLBACK: "1",
              ORIRO_LOAD_SHELL_ENV: "1",
            },
          },
          gateway: { auth: { mode: "none" }, mode: "local" },
        ...
        shouldDeferShellEnvFallback.mockImplementationOnce(
          (env?: NodeJS.ProcessEnv) => env?.ORIRO_DEFER_SHELL_ENV_FALLBACK === "1",
        );
  ...
  it("ignores shell fallback controls from invalid config", async () => {
    ...
    await withEnvAsync(
      ...
      async () => {
        const invalidConfig = {
          env: { vars: { ORIRO_DEFER_SHELL_ENV_FALLBACK: "1" } },
          gateway: { mode: "local" },
        };
        configState.snapshot = {
  ...
  it("rejects an invalid final config after a prepared config selected runtime paths", async () => {
    const selectedStateDir = "/tmp/oriro-prepared-selected-state";
    await withEnvAsync({ ORIRO_STATE_DIR: undefined }, async () => {
  ...
  it("replaces config-derived env when the final startup snapshot changes in place", async () => {
    await withEnvAsync(
      {
        ORIRO_GATEWAY_TOKEN: undefined,
        ORIRO_PROXY_URL: undefined,
        ORIRO_RAW_STREAM: undefined,
      },
      async () => {
        const oldConfig = {
          env: {
            vars: {
              ...
              ORIRO_PROXY_URL: "http://127.0.0.1:19876",
              ORIRO_RAW_STREAM: "1",
            },
          },
          gateway: { mode: "local" },
        ...
        const newConfig = {
          env: { vars: { ORIRO_GATEWAY_TOKEN: "new-token" } },
          gateway: { mode: "local" },
        };
        configState.snapshot = {
          config: oldConfig,
        ...
        expect(process.env.ORIRO_GATEWAY_TOKEN).toBe("old-token");
        expect(process.env.ORIRO_PROXY_URL).toBe("http://127.0.0.1:19876");
        ...
        readConfigFileSnapshotWithPluginMetadata.mockImplementationOnce(async () => {
          expect(process.env.ORIRO_GATEWAY_TOKEN).toBeUndefined();
          expect(process.env.ORIRO_PROXY_URL).toBeUndefined();
          return { snapshot: configState.snapshot };
        });
        await runGatewayCli(["gateway", "--raw-stream"]);

        expect(process.env.ORIRO_GATEWAY_TOKEN).toBe("new-token");
        expect(process.env.ORIRO_PROXY_URL).toBeUndefined();
        expect(process.env.ORIRO_RAW_STREAM).toBe("1");
      },
    );
  });
  ...
  it("marks descendants when the final config supplies the service marker", async () => {
    await withEnvAsync(
      ...
      async () => {
        const finalConfig = {
          env: { vars: { ORIRO_SERVICE_MARKER: "oriro" } },
          gateway: { mode: "local" },
        };
        configState.snapshot = {
          config: finalConfig,
        ...
        expect(process.env.ORIRO_SERVICE_MARKER).toBe("oriro");
        expect(process.env[GATEWAY_SERVICE_RUNTIME_PID_ENV]).toBe(String(process.pid));
      },
    );
  });
  ...
  it("blocks service-mode startup from an older binary with newer config", async () => {
    ...
    process.env.ORIRO_SERVICE_MARKER = "gateway";
    try {
    ...
    await withEnvAsync(
      ...
      async () => {
        ...
        ensureDevGatewayConfig.mockImplementationOnce(async () => {
          ...
          expect(process.env.ORIRO_HOME).toBeUndefined();
          expect(process.env.ORIRO_PROFILE).toBe("dev");
          expect(process.env.ORIRO_STATE_DIR).toBeUndefined();
          expect(process.env.ORIRO_GATEWAY_TOKEN).toBeUndefined();
          expect(process.env.ORIRO_WORKSPACE_DIR).toBe("/tmp/oriro-reset-workspace");
          configState.snapshot = {
            exists: true,
            valid: true,
            config: { gateway: { mode: "local" } },
        ...
        loadGlobalRuntimeDotEnvFiles.mockImplementation(() => {
          process.env.ORIRO_GATEWAY_TOKEN ??= "trusted-token";
          process.env.ORIRO_PROFILE ??= "dev";
          process.env.ORIRO_WORKSPACE_DIR ??= "/tmp/oriro-reset-workspace";
        });
        ...
        expect(process.env.ORIRO_GATEWAY_TOKEN).toBe("trusted-token");
  ...
  it("refuses dev reset if trusted dotenv retargets after pre-bootstrap", async () => {
    await withEnvAsync({ ORIRO_STATE_DIR: "/tmp/oriro-reset-original" }, async () => {
      configState.snapshot = {
        config: { gateway: { mode: "local" } },
        exists: true,
        path: "/tmp/oriro-reset-original/oriro.json",
        sourceConfig: { gateway: { mode: "local" } },
        valid: true,
      };
      await prepareGatewayReset();
      loadGlobalRuntimeDotEnvFiles.mockImplementation(() => {
        process.env.ORIRO_STATE_DIR = "/tmp/oriro-reset-retargeted";
        return {
          gatewayEnvAppliedKeys: [],
          stateEnvAppliedKeys: ["ORIRO_STATE_DIR"],
        };
      });
      ...
      expect(ensureDevGatewayConfig).not.toHaveBeenCalled();
      expect(process.env.ORIRO_STATE_DIR).toBe("/tmp/oriro-reset-original");
      expect(runtimeErrors.join("\n")).toContain(
  ...
  it.each([
    ...
    await withEnvAsync({ [selector]: "/tmp/oriro-reset-value" }, async () => {
      loadGlobalRuntimeDotEnvFiles.mockImplementation(() => {
        process.env[selector] = "/tmp/oriro-reset-retargeted";
      });
      ...
      expect(process.env[selector]).toBe("/tmp/oriro-reset-value");
      expect(runtimeErrors.join("\n")).toContain(
  ...
  it("blocks a final startup snapshot that changes guarded config selection", async () => {
    await withEnvAsync({ ORIRO_STATE_DIR: undefined }, async () => {
      configState.snapshot = {
        ...
        config: { gateway: { mode: "local" } },
        sourceConfig: {
          env: { vars: { ORIRO_STATE_DIR: "/tmp/oriro-late-selection" } },
          gateway: { mode: "local" },
        },
      };
  ...
  it("blocks a final startup snapshot that changes an already-selected config selector", async () => {
    await withEnvAsync({ ORIRO_STATE_DIR: undefined }, async () => {
      const guardedConfig = {
        env: { vars: { ORIRO_STATE_DIR: "/tmp/oriro-guarded-state" } },
        gateway: { mode: "local" },
      };
      configState.snapshot = {
      ...
      await selectGatewayRunEnvironment({ opts: {}, runtime: defaultRuntime });
      await prepareGatewayRunBootstrap({ opts: {}, runtime: defaultRuntime });
      expect(process.env.ORIRO_STATE_DIR).toBe("/tmp/oriro-guarded-state");

      const finalConfig = {
        env: { vars: { ORIRO_STATE_DIR: "/tmp/oriro-final-state" } },
        gateway: { mode: "local" },
      };
      configState.snapshot = {
        config: finalConfig,
  ...
  it.each([
    ...
    delete process.env.ORIRO_CLI_BACKEND_LOG_OUTPUT;
    ...
    expect(process.env.ORIRO_CLI_BACKEND_LOG_OUTPUT).toBe("1");
  });
  ...
  it("blocks startup when the observed snapshot loses gateway.mode", async () => {
    ...
    expect(runtimeErrors).toContain(
      "Gateway start blocked: existing config is missing gateway.mode. Treat this as suspicious or clobbered config. Re-run `oriro onboard --mode local` or `oriro setup`, set gateway.mode=local manually, or pass --allow-unconfigured.",
    );
    expect(runtimeErrors).toContain(
  ...
  it("blocks invalid startup config without automatic recovery", async () => {
    ...
    configState.snapshot = {
      exists: true,
      valid: false,
      path: "/tmp/oriro-test-missing-config.json",
      config: {},
      parsed: null,
  ...
  it("reads gateway password from --password-file", async () => {
    ...
    expect(runtimeErrors).not.toContain(
      "Warning: --password can be exposed via process listings. Prefer --password-file or ORIRO_GATEWAY_PASSWORD.",
    );
  });
