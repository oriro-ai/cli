...
function makeTempRoot(): string {
  const root = mkdtempSync(path.join(tmpdir(), "oriro-startup-memory-test-"));
  tempRoots.push(root);
  return root;
}
...
describe("check-cli-startup-memory", () => {
  ...
  it("keeps invalid startup memory env values from bypassing budgets", () => {
    expect(() =>
      testing.readPositiveNumberEnv("ORIRO_STARTUP_MEMORY_HELP_MB", 100, {
        ORIRO_STARTUP_MEMORY_HELP_MB: "abc",
      }),
    ).toThrow("ORIRO_STARTUP_MEMORY_HELP_MB must be a positive number");
    expect(() =>
      testing.readPositiveNumberEnv("ORIRO_STARTUP_MEMORY_HELP_MB", 100, {
        ORIRO_STARTUP_MEMORY_HELP_MB: "1e3",
      }),
    ).toThrow("ORIRO_STARTUP_MEMORY_HELP_MB must be a positive number");
    expect(() =>
      testing.readPositiveNumberEnv("ORIRO_STARTUP_MEMORY_HELP_MB", 100, {
        ORIRO_STARTUP_MEMORY_HELP_MB: "0x10",
      }),
    ).toThrow("ORIRO_STARTUP_MEMORY_HELP_MB must be a positive number");
    expect(() =>
      testing.readPositiveNumberEnv("ORIRO_STARTUP_MEMORY_HELP_MB", 100, {
        ORIRO_STARTUP_MEMORY_HELP_MB: "0",
      }),
    ).toThrow("ORIRO_STARTUP_MEMORY_HELP_MB must be a positive number");
    expect(
      testing.readPositiveNumberEnv("ORIRO_STARTUP_MEMORY_HELP_MB", 100, {
        ORIRO_STARTUP_MEMORY_HELP_MB: "125.5",
      }),
    ).toBe(125.5);
  });
  ...
  it("keeps invalid startup memory timeout env values from parsing loosely", () => {
    expect(() =>
      testing.readPositiveIntEnv("ORIRO_STARTUP_MEMORY_TIMEOUT_MS", 60_000, {
        ORIRO_STARTUP_MEMORY_TIMEOUT_MS: "1e3",
      }),
    ).toThrow("ORIRO_STARTUP_MEMORY_TIMEOUT_MS must be a positive number");
    expect(() =>
      testing.readPositiveIntEnv("ORIRO_STARTUP_MEMORY_TIMEOUT_MS", 60_000, {
        ORIRO_STARTUP_MEMORY_TIMEOUT_MS: "1000.5",
      }),
    ).toThrow("ORIRO_STARTUP_MEMORY_TIMEOUT_MS must be a positive integer");
    expect(() =>
      testing.readPositiveIntEnv("ORIRO_STARTUP_MEMORY_TIMEOUT_MS", 60_000, {
        ORIRO_STARTUP_MEMORY_TIMEOUT_MS: String(Number.MAX_SAFE_INTEGER + 1),
      }),
    ).toThrow("ORIRO_STARTUP_MEMORY_TIMEOUT_MS must be a positive integer");
    expect(
      testing.readPositiveIntEnv("ORIRO_STARTUP_MEMORY_TIMEOUT_MS", 60_000, {
        ORIRO_STARTUP_MEMORY_TIMEOUT_MS: "1000",
      }),
    ).toBe(1000);
  });
