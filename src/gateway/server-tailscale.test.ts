...
describe("startGatewayTailscaleExposure preserveFunnel", () => {
  ...
  it("passes serviceName through to Tailscale Serve setup and cleanup", async () => {
    ...
    expect(mocks.enableTailscaleServe).toHaveBeenCalledWith(18789, undefined, "svc:oriro");
    expect(logTailscale.info).toHaveBeenCalledWith(
      "serve enabled for svc:oriro: https://oriro.tailnet.ts.net/ (WS via wss://oriro.tailnet.ts.net)",
    );

    await cleanup?.();

    expect(mocks.disableTailscaleServe).toHaveBeenCalledWith(undefined, "svc:oriro");
  });
