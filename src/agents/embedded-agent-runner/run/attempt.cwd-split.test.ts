...
describe("runEmbeddedAttempt cwd/workspace split", () => {
  ...
  it("uses workspace for bootstrap and cwd for runtime tools", async () => {
    ...
    const taskRepo = await fs.mkdtemp(path.join(os.tmpdir(), "oriro-task-repo-"));
    ...
    const toolsCall = hoisted.createOriroCodingToolsMock.mock.calls[0]?.[0] as
  ...
  it("rejects cwd overrides for sandboxed runs instead of silently ignoring them", async () => {
    ...
    hoisted.resolveSandboxContextMock.mockResolvedValueOnce({
      enabled: true,
      workspaceAccess: "ro",
      workspaceDir: "/tmp/oriro-sandbox-copy",
    });
    ...
    expect(hoisted.createOriroCodingToolsMock).not.toHaveBeenCalled();
  });
});
