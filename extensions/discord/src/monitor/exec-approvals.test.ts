...
vi.mock("oriro/plugin-sdk/approval-gateway-runtime", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("oriro/plugin-sdk/approval-gateway-runtime")>();
  return {
    ...actual,
...
function buildConfig(
  execApprovals?: NonNullable<NonNullable<OriroConfig["channels"]>["discord"]>["execApprovals"],
): OriroConfig {
  return {
    channels: {
      discord: {
