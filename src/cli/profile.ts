...
function resolveProfileStateDir(
  ...
  return path.join(resolveRequiredHomeDir(env as NodeJS.ProcessEnv, homedir), `.oriro${suffix}`);
}
...
export function applyCliProfileEnv(params: {
  ...
  env.ORIRO_PROFILE = profile;

  const existingStateDir = normalizeOptionalString(env.ORIRO_STATE_DIR);
  const stateDir = existingStateDir || resolveProfileStateDir(profile, env, homedir);
  if (!existingStateDir) {
    env.ORIRO_STATE_DIR = stateDir;
  }

  if (!normalizeOptionalString(env.ORIRO_CONFIG_PATH)) {
    env.ORIRO_CONFIG_PATH = path.join(stateDir, "oriro.json");
  }

  if (profile === "dev" && !env.ORIRO_GATEWAY_PORT?.trim()) {
    env.ORIRO_GATEWAY_PORT = "19001";
  }
}
