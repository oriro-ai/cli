...
function mergePackageChannelMetaIntoChannelConfigs(params: {
  channelConfigs?: Record<string, PluginManifestChannelConfig>;
  packageChannel?: OriroPackageManifest["channel"];
}): Record<string, PluginManifestChannelConfig> | undefined {
...
function buildRecord(params: {
  ...
  return {
    ...
    format: params.candidate.format ?? "oriro",
...
function pushNonBundledChannelConfigDescriptorDiagnostic(params: {
  ...
  params.diagnostics.push({
    ...
    pluginId: sanitizeForLog(params.record.id),
    source: sanitizeForLog(params.record.manifestPath),
    message: `channel plugin manifest declares ${safeMissingChannels.join(", ")} without channelConfigs metadata; add oriro.plugin.json#channelConfigs so config schema and setup surfaces work before runtime loads. Channels without channelConfigs still appear in channel listings, but setup UI may be limited.`,
  });
}
...
function isTrustedOfficialPluginInstall(params: {
  ...
  if (
    installRecord.source === "orirohub" &&
    officialInstall?.orirohubSpec &&
    installRecord.orirohubChannel === "official" &&
    (installRecord.orirohubPackage === packageName ||
      installRecord.spec === officialInstall.orirohubSpec ||
      installRecord.resolvedSpec === officialInstall.orirohubSpec)
  ) {
    return true;
  }
  return false;
...
export function loadPluginManifestRegistry(
  ...
  for (const candidate of candidates) {
    ...
    const isBundleRecord = (candidate.format ?? "oriro") === "bundle";
    const manifestRes:
