...
describe("resolveAgentConfig", () => {
  ...
  it("resolves explicit and effective model primary separately", () => {
    ...
    const cfgWithObjectDefault: OriroConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgNoDefaults: OriroConfig = {
      agents: {
        list: [{ id: "main" }],
      },
    };
  ...
  it("supports per-agent model primary+fallbacks", () => {
    ...
    const cfgNoOverride: OriroConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgStringModel: OriroConfig = {
      agents: {
        list: [
          {
            id: "linus",
    ...
    const cfgStrictAgentWithDefaultFallbacks: OriroConfig = {
      agents: {
        defaults: {
          model: {
    ...
    const cfgInheritDefaultsWithoutAgentModel: OriroConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
  ...
  it("updates the effective model primary at the winning config layer", () => {
    ...
    const inheritedCfg: OriroConfig = {
      agents: {
        defaults: {
          model: {
            primary: "openai/gpt-5.4",
  ...
  it("computes whether any model fallbacks are configured via shared helper", () => {
    const cfgDefaultsOnly: OriroConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: ["openai/gpt-5.4"],
    ...
    const cfgAgentOverrideOnly: OriroConfig = {
      agents: {
        defaults: {
          model: {
            fallbacks: [],
  ...
  it("should normalize agent id", () => {
    ...
    expect(result?.workspace).toBe("~/oriro");
  });

  it("uses ORIRO_HOME for default agent workspace", () => {
    const home = path.join(path.sep, "srv", "oriro-home");
    withEnv({ ORIRO_HOME: home }, () => {
      const workspace = resolveAgentWorkspaceDir({} as OriroConfig, "main");
      expect(workspace).toBe(path.join(path.resolve(home), ".oriro", "workspace"));
    });
  });

  it("uses ORIRO_WORKSPACE_DIR for default agent workspace", () => {
  ...
  it("uses ORIRO_HOME for default agentDir", () => {
    const home = path.join(path.sep, "srv", "oriro-home");
    withEnv({ ORIRO_HOME: home, ORIRO_STATE_DIR: "" }, () => {
      const agentDir = resolveAgentDir({} as OriroConfig, "main");
      expect(agentDir).toBe(path.join(path.resolve(home), ".oriro", "agents", "main", "agent"));
    });
  });
  ...
  it("resolves default agentDir from the configured default agent", () => {
    ...
    const agentDir = withEnv({ ORIRO_STATE_DIR: stateDir }, () => resolveDefaultAgentDir(cfg));
  ...
  it("non-default agent without defaults.workspace falls back to stateDir", () => {
    ...
    const workspace = withEnv({ ORIRO_STATE_DIR: stateDir }, () =>
...
describe("resolveAgentIdByWorkspacePath", () => {
  it("returns the most specific workspace match for a directory", () => {
    const workspaceRoot = `/tmp/oriro-agent-scope-${Date.now()}-root`;
  ...
  it("returns undefined when directory has no matching workspace", () => {
    ...
    expect(
      resolveAgentIdByWorkspacePath(cfg, `/tmp/oriro-agent-scope-${Date.now()}-unrelated`),
    ).toBeUndefined();
  });

  it("matches workspace paths through symlink aliases", () => {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "oriro-agent-scope-"));
