...
# Checks both Claude Code and Oriro auth status

set -euo pipefail

CLAUDE_CREDS="$HOME/.claude/.credentials.json"
ORIRO_AUTH="$HOME/.oriro/agents/main/agent/auth-profiles.json"
...
fetch_models_status_json() {
    oriro models status --json 2>/dev/null || true
}
...
if [ "$OUTPUT_MODE" = "json" ]; then
    ...
    oriro_status=$(check_oriro_auth 2>/dev/null || true)

    claude_expires=0
    oriro_expires=0
    if [ "$USE_JSON" -eq 1 ]; then
        claude_expires=$(json_expires_for_claude_cli)
        oriro_expires=$(json_expires_for_anthropic_any)
    else
        claude_expires=$(jq -r '.claudeAiOauth.expiresAt // 0' "$CLAUDE_CREDS" 2>/dev/null || echo "0")
        oriro_expires=$(jq -r '.profiles["anthropic:default"].expires // 0' "$ORIRO_AUTH" 2>/dev/null || echo "0")
    fi
    ...
    jq -n \
        ...
        --arg bs "$oriro_status" \
        --arg be "$oriro_expires" \
        '{
            claude_code: {status: $cs, expires_at_ms: ($ce | tonumber)},
            oriro: {status: $bs, expires_at_ms: ($be | tonumber)},
...
if [ "$OUTPUT_MODE" = "simple" ]; then
    ...
    if [[ "$claude_status" == EXPIRED* ]] || [[ "$claude_status" == MISSING* ]]; then
        echo "CLAUDE_EXPIRED"
        exit 1
    elif [[ "$oriro_status" == EXPIRED* ]] || [[ "$oriro_status" == MISSING* ]]; then
        echo "ORIRO_EXPIRED"
        exit 1
    elif [[ "$claude_status" == EXPIRING* ]]; then
        echo "CLAUDE_EXPIRING"
        exit 2
    elif [[ "$oriro_status" == EXPIRING* ]]; then
        echo "ORIRO_EXPIRING"
        exit 2
    else
        echo "OK"
...
echo ""
echo "Oriro Auth (~/.oriro/agents/main/agent/auth-profiles.json):"
if [ "$USE_JSON" -eq 1 ]; then
    ...
    best_profile=$(jq -r '
        ...
        | .[0].key // "none"
    ' "$ORIRO_AUTH" 2>/dev/null || echo "none")
    expires=$(jq -r '
        [.profiles | to_entries[] | select(.value.provider == "anthropic") | .value.expires]
        | max // 0
    ' "$ORIRO_AUTH" 2>/dev/null || echo "0")
...
if [ "$expires" -le 0 ] && [ "$api_keys" -gt 0 ]; then
    ...
    echo -e "  Status: ${RED}NOT FOUND${NC}"
    echo "  Note: Run 'oriro doctor --yes' to sync from Claude Code"
else
    now_ms=$(( $(date +%s) * 1000 ))
