...
const persistedConfigResultMock = vi.fn((config: OriroConfig) => config);
...
vi.mock("../../config/config.js", async () => {
  ...
  return {
    ...actual,
    createConfigIO: () => ({ configPath: "/tmp/oriro.json" }),
    writeConfigFile: writeConfigFileMock,
    replaceConfigFile: async (params: { nextConfig: OriroConfig; writeOptions?: unknown }) => {
...
function tokenAuthConfig(token: string): OriroConfig {
  return {
    gateway: {
      auth: {
        mode: "token",
...
function hotReloadConfig(): OriroConfig {
  return {
    gateway: {
      reload: {
        mode: "hot",
...
function mockPreviousConfig(config: OriroConfig): void {
...
beforeEach(() => {
  validateConfigObjectWithPluginsMock.mockImplementation((config: OriroConfig) => ({
    ok: true,
    config,
  }));
  prepareSecretsRuntimeSnapshotMock.mockImplementation(
    async ({ config }: { config: OriroConfig }) => ({
      config,
    }),
  );
  restartSentinelMocks.writeRestartSentinel.mockClear();
  persistedConfigResultMock.mockImplementation((config: OriroConfig) => config);
});

describe("config shared auth disconnects", () => {
  it("returns the persisted config from config.set write results", async () => {
    const prevConfig: OriroConfig = {
      gateway: {
        port: 19000,
      },
    };
    const submittedConfig: OriroConfig = {
      gateway: {
        port: 19001,
      },
    };
    const persistedConfig: OriroConfig = {
      gateway: {
        port: 19001,
      },
      meta: {
