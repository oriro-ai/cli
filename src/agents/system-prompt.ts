...
import {
  buildOriroToolFallbackText,
  shouldRenderOriroToolWorkflowHints,
...
function buildWebchatCanvasSection(params: {
  ...
  return [
    ...
    '- You may also use an explicit hosted URL: `[embed url="/__oriro__/canvas/documents/cv_123/index.html" title="Status" height="320" /]`.',
    '- Never use local filesystem paths or `file://...` URLs in `[embed ...]`. Hosted embeds must point at `/__oriro__/canvas/...` URLs or use `ref="..."`.',
    ...
    "- Quote all attribute values. Prefer `ref` for hosted documents unless you already have the full `/__oriro__/canvas/documents/<id>/index.html` URL.",
    "",
  ];
}
...
function buildDocsSection(params: {
  ...
  const lines = [
    ...
    "Diagnosing issues: run `oriro status` when possible; ask user only if blocked.",
    "",
  ];
...
export function buildAgentSystemPrompt(params: {
  ...
  acpEnabled?: boolean;
  /** Prompt surface controls runtime-specific fallback fragments. Defaults to Oriro main. */
  ...
  const promptSurface = params.promptSurface ?? "oriro_main";
  ...
  const renderOriroToolWorkflowHints = shouldRenderOriroToolWorkflowHints({
  ...
  const workspaceOnlyGuidance =
    params.fsWorkspaceOnly === true
      ? "tools.fs.workspaceOnly is enabled: scratch/temp/meta files that file tools must later read/write/edit must stay inside the workspace, preferably as relative paths under `.oriro/tmp/`. Do not write files to `/tmp/...` with exec if a later read/write/edit/apply_patch tool needs them; use `.oriro/tmp/...` instead."
      : "";
  const safetySection = [
    "## Safety",
