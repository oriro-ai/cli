...
usage() {
  echo "usage: bash scripts/oriro-npm-publish.sh --publish [package.tgz]"
}
...
mapfile -t publish_plan < <(
  PACKAGE_VERSION="${package_version}" REQUESTED_PUBLISH_TAG="${ORIRO_NPM_PUBLISH_TAG:-}" \
    node --import tsx --input-type=module <<'EOF'
import { resolveNpmPublishPlan } from "./scripts/oriro-npm-release-check.ts";
