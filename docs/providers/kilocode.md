---
summary: "Use Kilo Gateway's unified API to access many models in Oriro"
title: "Kilo Gateway"
read_when:
  - You want a single API key for many LLMs
  - You want to run models via Kilo Gateway in Oriro
---
...
```bash
oriro plugins install @oriro/kilocode-provider
...
<Steps>
  ...
  <Step title="Run onboarding">
    ```bash
    oriro onboard --auth-choice kilocode-api-key
    ```
  ...
  <Step title="Verify the model is available">
    ```bash
    oriro models list --provider kilocode
    ```
  </Step>
</Steps>
...
<Note>
Oriro treats `kilocode/kilo/auto` as the stable default ref, but does not
...
Oriro dynamically discovers available models from the Kilo Gateway at startup. Use
...
<Tip>
At startup, Oriro queries `GET https://api.kilo.ai/api/gateway/models` and merges
...
<AccordionGroup>
  <Accordion title="Transport and compatibility">
    ...
    - Gemini-backed Kilo refs stay on the proxy-Gemini path, so Oriro keeps
  ...
  <Accordion title="Troubleshooting">
    - If model discovery fails at startup, Oriro falls back to the static catalog containing `kilocode/kilo/auto`.
    - Confirm your API key is valid and that your Kilo account has the desired models enabled.
    - When the Gateway runs as a daemon, ensure `KILOCODE_API_KEY` is available to that process (for example in `~/.oriro/.env` or via `env.shellEnv`).
