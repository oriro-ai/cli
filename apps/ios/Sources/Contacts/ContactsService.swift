...
final class ContactsService: ContactsServicing {
    ...
    func search(params: OriroContactsSearchParams) async throws -> OriroContactsSearchPayload {
        ...
        return OriroContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: OriroContactsAddParams) async throws -> OriroContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return OriroContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return OriroContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> OriroContactPayload {
        OriroContactPayload(
