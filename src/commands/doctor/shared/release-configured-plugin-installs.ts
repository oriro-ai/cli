...
import { normalizeNullableString as normalizeId } from "@oriro/normalization-core/string-coerce";
...
import { compareOriroVersions } from "../../../config/version.js";
import {
...
function isPluginsGloballyDisabled(cfg: OriroConfig): boolean {
  return cfg.plugins?.enabled === false;
}

function isDenied(cfg: OriroConfig, pluginId: string): boolean {
  const deny = cfg.plugins?.deny;
  return Array.isArray(deny) && deny.includes(pluginId);
}

function collectBlockedPluginIds(cfg: OriroConfig): string[] {
...
function isPluginEntryDisabled(cfg: OriroConfig, pluginId: string): boolean {
  return cfg.plugins?.entries?.[pluginId]?.enabled === false;
}

function isChannelDisabled(cfg: OriroConfig, channelId: string): boolean {
  const channels = asObjectRecord(cfg.channels);
  const entry = asObjectRecord(channels?.[channelId]);
  return entry?.enabled === false;
}

function isDisabled(cfg: OriroConfig, pluginId: string): boolean {
...
function collectMaterialPluginEntryIds(cfg: OriroConfig): string[] {
  const entries = asObjectRecord(cfg.plugins?.entries);
  if (!entries) {
    return [];
  }
...
function collectSlotPluginIds(cfg: OriroConfig): string[] {
...
function collectConfiguredChannelIds(cfg: OriroConfig, env: NodeJS.ProcessEnv): string[] {
  const ids = new Set<string>();
  const channels = asObjectRecord(cfg.channels);
  if (channels) {
...
function collectWebSearchPluginIds(cfg: OriroConfig): string[] {
...
function collectEnvWebSearchPluginIds(cfg: OriroConfig, env: NodeJS.ProcessEnv): string[] {
  if (cfg.tools?.web?.search?.enabled === false) {
    return [];
  }
...
function collectWebFetchPluginIds(cfg: OriroConfig): string[] {
...
function collectEnvWebFetchPluginIds(cfg: OriroConfig, env: NodeJS.ProcessEnv): string[] {
...
function collectSpeechPluginIds(cfg: OriroConfig): string[] {
  return resolveOfficialExternalProviderContractPluginIds({
...
function collectAcpRuntimePluginIds(cfg: OriroConfig): string[] {
...
function collectAllowOnlyOfficialPluginIds(cfg: OriroConfig): string[] {
...
function addEligiblePluginId(cfg: OriroConfig, pluginIds: Set<string>, pluginId: string): void {
...
export function shouldRunConfiguredPluginInstallReleaseStep(params: {
  ...
  const currentComparedToRelease = compareOriroVersions(
  ...
  const touchedComparedToRelease = compareOriroVersions(params.touchedVersion, releaseVersion);
