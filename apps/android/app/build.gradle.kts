...
val androidStoreFile = providers.gradleProperty("ORIRO_ANDROID_STORE_FILE").orNull?.takeIf { it.isNotBlank() }
val androidStorePassword = providers.gradleProperty("ORIRO_ANDROID_STORE_PASSWORD").orNull?.takeIf { it.isNotBlank() }
val androidKeyAlias = providers.gradleProperty("ORIRO_ANDROID_KEY_ALIAS").orNull?.takeIf { it.isNotBlank() }
val androidKeyPassword = providers.gradleProperty("ORIRO_ANDROID_KEY_PASSWORD").orNull?.takeIf { it.isNotBlank() }
...
if (wantsAndroidReleaseBuild && !hasAndroidReleaseSigning) {
  error(
    "Missing Android release signing properties. Set ORIRO_ANDROID_STORE_FILE, " +
      "ORIRO_ANDROID_STORE_PASSWORD, ORIRO_ANDROID_KEY_ALIAS, and " +
      "ORIRO_ANDROID_KEY_PASSWORD in ~/.gradle/gradle.properties.",
  )
}
...
android {
  ...
  sourceSets {
    getByName("main") {
      assets.directories.add("../../shared/OriroKit/Sources/OriroKit/Resources")
    }
  }
  ...
  onVariants { variant ->
    variant.outputs
      ...
      .forEach { output ->
        ...
        val outputFileName =
          if (flavorName == null) {
            "oriro-$versionName-$buildType.apk"
          } else {
            "oriro-$versionName-$flavorName-$buildType.apk"
          }
