...
import type { DmPolicy, GroupPolicy, ReplyToMode } from "oriro/plugin-sdk/config-contracts";
import { resolveOAuthDir } from "oriro/plugin-sdk/state-paths";
...
export function listWhatsAppAuthDirs(cfg: OriroConfig): string[] {
...
export function hasAnyWhatsAppAuth(cfg: OriroConfig): boolean {
...
export function resolveWhatsAppAuthDir(params: { cfg: OriroConfig; accountId: string }): {
  authDir: string;
  isLegacy: boolean;
} {
...
export function listEnabledWhatsAppAccounts(cfg: OriroConfig): ResolvedWhatsAppAccount[] {
