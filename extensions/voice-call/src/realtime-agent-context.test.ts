...
async function createWorkspace(): Promise<string> {
  const workspaceDir = await mkdtemp(path.join(tmpdir(), "oriro-voice-context-"));
...
describe("buildRealtimeVoiceInstructions", () => {
  it("injects bounded identity and workspace context", async () => {
    ...
    await writeFile(path.join(workspaceDir, "IDENTITY.md"), "Name: Oriro Voice\nVibe: snappy\n");
    ...
    expect(instructions).toContain("Oriro agent voice context:");
    expect(instructions).toContain("Consult behavior:");
    expect(instructions).toContain("Call oriro_agent_consult before answering requests");
    expect(instructions).toContain("- Agent id: voice");
    expect(instructions).toContain("- Name: Oriro Voice");
