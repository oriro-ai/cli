...
Oriro supports **camera capture** for agent workflows:
...
```bash
oriro nodes camera snap --node <id>               # default: both front + back (2 MEDIA lines)
oriro nodes camera snap --node <id> --facing front
oriro nodes camera clip --node <id> --duration 3000
oriro nodes camera clip --node <id> --no-audio
```
...
- **Settings → General → Allow Camera** (`oriro.cameraEnabled`)
  - Default: **off**
  - When off: camera requests return "Camera disabled by user".

### CLI helper (node invoke)

Use the main `oriro` CLI to invoke camera commands on the macOS node.

Examples:

```bash
oriro nodes camera list --node <id>            # list camera ids
oriro nodes camera snap --node <id>            # prints saved path
oriro nodes camera snap --node <id> --max-width 1280
oriro nodes camera snap --node <id> --delay-ms 2000
oriro nodes camera snap --node <id> --device-id <id>
oriro nodes camera clip --node <id> --duration 10s          # prints saved path
oriro nodes camera clip --node <id> --duration-ms 3000      # prints saved path (legacy flag)
oriro nodes camera clip --node <id> --device-id <id>
oriro nodes camera clip --node <id> --no-audio
```

Notes:

- `oriro nodes camera snap` defaults to `maxWidth=1600` unless overridden.
...
```bash
oriro nodes screen record --node <id> --duration 10s --fps 15   # prints saved path
```
