...
export function buildGatewayCronService(params: {
  ...
  const cronEnabled = process.env.ORIRO_SKIP_CRON !== "1" && params.cfg.cron?.enabled !== false;

  const findAgentEntry = (cfg: OriroConfig, agentId: string) =>
  ...
  const hasConfiguredAgent = (cfg: OriroConfig, agentId: string) =>
    Boolean(findAgentEntry(cfg, agentId));

  const mergeRuntimeAgentConfig = (runtimeConfig: OriroConfig, requestedAgentId: string) => {
