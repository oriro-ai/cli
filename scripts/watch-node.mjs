...
const shouldRunAutoDoctor = (deps, autoDoctorAttempted) =>
  ...
  !AUTO_DOCTOR_DISABLE_VALUES.has(
    String(deps.env.ORIRO_GATEWAY_WATCH_AUTO_DOCTOR ?? "").toLowerCase(),
  );
...
const logWatcher = (message, deps) => {
  deps.process.stderr?.write?.(`[oriro] ${message}\n`);
};
...
const printFriendlyWatchStartupError = (err) => {
  ...
  if (packageConfigPath) {
    console.error(`[oriro] Invalid package config: ${packageConfigPath}`);
  }
  console.error("[oriro] This usually means a file in node_modules is empty or truncated.");
  console.error("[oriro] Recommended recovery:");
  console.error("[oriro]   rm -rf node_modules");
  console.error("[oriro]   pnpm store prune");
  console.error("[oriro]   pnpm install");
  console.error("");
  console.error("[oriro] Original error:");
  console.error(err);
};
...
export async function runWatchMain(params = {}) {
  ...
  childEnv.ORIRO_WATCH_MODE = "1";
  childEnv.ORIRO_WATCH_SESSION = watchSession;
  ...
  childEnv.ORIRO_NO_RESPAWN = "1";
  if (deps.args.length > 0) {
    childEnv.ORIRO_WATCH_COMMAND = deps.args.join(" ");
  }
  ...
  return await new Promise((resolve, reject) => {
    ...
    const runAutoDoctorAndRestart = () => {
      ...
      logWatcher(
        "Gateway exited early; running `oriro doctor --fix --non-interactive` once.",
        deps,
      );
