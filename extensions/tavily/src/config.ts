...
import { resolvePositiveTimeoutSeconds } from "oriro/plugin-sdk/provider-web-search";
import {
...
export function resolveTavilySearchConfig(cfg?: OriroConfig): TavilySearchConfig {
...
export function resolveTavilyApiKey(cfg?: OriroConfig): string | undefined {
...
export function resolveTavilyBaseUrl(cfg?: OriroConfig): string {
  const search = resolveTavilySearchConfig(cfg);
  const configured =
