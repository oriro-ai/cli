...
final class LiveActivityManager {
    ...
    private let logger = Logger(subsystem: "ai.orirofoundation.app", category: "LiveActivity")
    private let connectingStaleSeconds: TimeInterval = 120
    private let hydrationStaleSeconds: TimeInterval = 300
    private var currentActivity: Activity<OriroActivityAttributes>?
    ...
    func showConnecting(statusText: String = "Connecting...", agentName: String, sessionKey: String) {
        ...
        let attributes = OriroActivityAttributes(agentName: agentName, sessionKey: sessionKey)
    ...
    private func hydrateCurrentAndPruneDuplicates() {
        let active = Activity<OriroActivityAttributes>.activities
    ...
    private func updateCurrent(state: OriroActivityAttributes.ContentState, staleDate: Date? = nil) {
    ...
    private func end(activity: Activity<OriroActivityAttributes>) {
        Task {
            await activity.end(
                ActivityContent(state: self.disconnectedState(), staleDate: nil),
                dismissalPolicy: .immediate)
    ...
    private func connectingState(statusText: String = "Connecting...") -> OriroActivityAttributes.ContentState {
        OriroActivityAttributes.ContentState(
    ...
    private func attentionState(statusText: String) -> OriroActivityAttributes.ContentState {
        OriroActivityAttributes.ContentState(
            statusText: statusText,
            isIdle: false,
            isDisconnected: false,
    ...
    private func disconnectedState() -> OriroActivityAttributes.ContentState {
        OriroActivityAttributes.ContentState(
