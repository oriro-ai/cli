...
# Output:
# - dist/Oriro.app
# - dist/Oriro-<version>.zip
# - dist/Oriro-<version>.dmg
...
export BUNDLE_ID="${BUNDLE_ID:-ai.oriro.mac}"
...
canonical_sparkle_build() {
  ...
  stderr_file="$(mktemp "${TMPDIR:-/tmp}/oriro-sparkle-build.XXXXXX")" || {
...
ZIP="$ROOT_DIR/dist/Oriro-$VERSION.zip"
DMG="$ROOT_DIR/dist/Oriro-$VERSION.dmg"
NOTARY_ZIP="$ROOT_DIR/dist/Oriro-$VERSION.notary.zip"
DSYM_ZIP="$ROOT_DIR/dist/Oriro-$VERSION.dSYM.zip"
