...
import type { OriroConfig } from "../../dist/config/types.oriro.js";
...
function makeParams(commandBody: string, cfg: OriroConfig, isGroup = false) {
  return {
    cfg,
    command: {
      surface: "whatsapp",
...
async function invoke(commandBody: string, cfg: OriroConfig, isGroup = false): Promise<string> {
...
async function main() {
  const tempState = await createE2eStateDir("oriro-crestodian-");
  tempState.registerExitCleanup();
  const stateDir = tempState.stateDir;
  const configPath = process.env.ORIRO_CONFIG_PATH ?? path.join(stateDir, "oriro.json");
  ...
  const refPlan = await invoke(
    "/crestodian config set-ref gateway.auth.token env ORIRO_GATEWAY_TOKEN",
    cfg,
  );
  assert(
  ...
  const agentPlan = await invoke("/crestodian create agent work workspace /tmp/oriro-work", cfg);
  assert(
  ...
  const setupPlan = await invoke(
    "/crestodian setup workspace /tmp/oriro-setup model openai/gpt-5.2",
    cfg,
  );
  ...
  const updatedConfig = JSON.parse(await fs.readFile(configPath, "utf8")) as OriroConfig;
  assert(
  ...
  assert(
    updatedConfig.gateway?.auth?.token &&
      ...
      updatedConfig.gateway.auth.token.id === "ORIRO_GATEWAY_TOKEN",
    "SecretRef set did not update gateway.auth.token",
  );
  assert(
    updatedConfig.agents?.defaults?.workspace === "/tmp/oriro-setup",
    "setup did not update default workspace",
  );
  assert(
    updatedConfig.agents?.list?.some(
      (agent) => agent.id === "work" && agent.workspace === "/tmp/oriro-work",
    ),
