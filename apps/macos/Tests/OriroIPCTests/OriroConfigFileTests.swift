...
@Suite(.serialized)
struct OriroConfigFileTests {
    private func makeConfigOverridePath() -> String {
        FileManager().temporaryDirectory
            .appendingPathComponent("oriro-config-\(UUID().uuidString)")
            .appendingPathComponent("oriro.json")
            .path
    }
    ...
    func `config path respects env override`() async {
        ...
        await TestIsolation.withEnvValues(["ORIRO_CONFIG_PATH": override]) {
            #expect(OriroConfigFile.url().path == override)
        }
    }
    ...
    func `remote gateway port parses and matches host`() async {
        ...
        await TestIsolation.withEnvValues(["ORIRO_CONFIG_PATH": override]) {
            ...
            #expect(OriroConfigFile.remoteGatewayPort() == 19999)
            #expect(OriroConfigFile.remoteGatewayPort(matchingHost: "gateway.ts.net") == 19999)
            #expect(OriroConfigFile.remoteGatewayPort(matchingHost: "GATEWAY.ts.net.") == 19999)
            #expect(OriroConfigFile.remoteGatewayPort(matchingHost: "gateway") == nil)
            #expect(OriroConfigFile.remoteGatewayPort(matchingHost: "other.ts.net") == nil)
            #expect(OriroConfigFile.remoteGatewayPort(matchingHost: "gateway.attacker.tld") == nil)
        }
    }
    ...
    func `set remote gateway url string replaces scheme`() async {
        ...
        await TestIsolation.withEnvValues(["ORIRO_CONFIG_PATH": override]) {
            ...
            OriroConfigFile.setRemoteGatewayUrlString("ws://127.0.0.1:18789")
            let root = OriroConfigFile.loadDict()
    ...
    func `set remote gateway url preserves scheme`() async {
        ...
        await TestIsolation.withEnvValues(["ORIRO_CONFIG_PATH": override]) {
            ...
            OriroConfigFile.setRemoteGatewayUrl(host: "new-host", port: 2222)
            let root = OriroConfigFile.loadDict()
            let url = ((root["gateway"] as? [String: Any])?["remote"] as? [String: Any])?["url"] as? String
    ...
    func `clear remote gateway url removes only url field`() async {
        ...
        await TestIsolation.withEnvValues(["ORIRO_CONFIG_PATH": override]) {
            ...
            OriroConfigFile.clearRemoteGatewayUrl()
            let root = OriroConfigFile.loadDict()
    ...
    func `state dir override sets config path`() async {
        ...
        await TestIsolation.withEnvValues([
            "ORIRO_CONFIG_PATH": nil,
            "ORIRO_STATE_DIR": dir,
        ]) {
            #expect(OriroConfigFile.stateDirURL().path == dir)
            #expect(OriroConfigFile.url().path == "\(dir)/oriro.json")
        }
    }
    ...
    func `save dict appends config audit log`() async throws {
        let stateDir = FileManager().temporaryDirectory
            .appendingPathComponent("oriro-state-\(UUID().uuidString)", isDirectory: true)
        let configPath = stateDir.appendingPathComponent("oriro.json")
        let auditPath = stateDir.appendingPathComponent("logs/config-audit.jsonl")

        defer { try? FileManager().removeItem(at: stateDir) }

        try await TestIsolation.withEnvValues([
            "ORIRO_STATE_DIR": stateDir.path,
            "ORIRO_CONFIG_PATH": configPath.path,
        ]) {
            OriroConfigFile.saveDict([
                "gateway": ["mode": "local"],
            ])
            ...
            #expect(auditRoot?["source"] as? String == "macos-oriro-config-file")
    ...
    func `save dict preserves gateway auth unless explicitly allowed`() async throws {
        ...
        await TestIsolation.withEnvValues([
            ...
            let allowedRoot = OriroConfigFile.loadDict()
    ...
    func `load dict audits suspicious out-of-band clobbers`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            try OriroConfigFile.withTestingFileLock {
                OriroConfigFile.saveDict([
                    "update": ["channel": "beta"],
                ...
                let loaded = OriroConfigFile.loadDict()
    ...
    func `save dict records preserved gateway auth in audit`() async throws {
        ...
        try await TestIsolation.withEnvValues([
            ...
            let saved = OriroConfigFile.saveDict([
                "gateway": [
                    "mode": "local",
                ],
                "browser": [
