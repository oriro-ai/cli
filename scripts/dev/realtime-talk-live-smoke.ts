// Realtime Talk Live Smoke script supports Oriro repository automation.
...
const OPENAI_REALTIME_MODEL =
  process.env.ORIRO_REALTIME_OPENAI_MODEL?.trim() || "gpt-realtime-2";
const OPENAI_REALTIME_VOICE = process.env.ORIRO_REALTIME_OPENAI_VOICE?.trim() || "alloy";
const DEFAULT_OPENAI_HTTP_TIMEOUT_MS = 30_000;
const OPENAI_HTTP_RESPONSE_MAX_BYTES = 256 * 1024;
const GOOGLE_REALTIME_MODEL =
  process.env.ORIRO_REALTIME_GOOGLE_MODEL?.trim() ||
  "gemini-2.5-flash-native-audio-preview-12-2025";
const GOOGLE_REALTIME_VOICE = process.env.ORIRO_REALTIME_GOOGLE_VOICE?.trim() || "Kore";
...
type OpenAIWebRtcSmokeGlobal = typeof globalThis & {
  oriroReadBoundedRealtimeResponseText?: OpenAIRealtimeBrowserResponseReader;
};
...
function resolveOpenAIHttpTimeoutMs(
  raw = process.env.ORIRO_REALTIME_OPENAI_HTTP_TIMEOUT_MS,
): number {
  return parseStrictIntegerOption({
    fallback: DEFAULT_OPENAI_HTTP_TIMEOUT_MS,
    label: "ORIRO_REALTIME_OPENAI_HTTP_TIMEOUT_MS",
    min: 1,
    raw,
  });
}
...
function openAIRealtimeBrowserResponseReaderInitScript(): string {
  return `globalThis.oriroReadBoundedRealtimeResponseText = ${readOpenAIRealtimeBrowserResponseText.toString()};`;
}
      ...
      const result = await page.evaluate(
        async ({ clientSecret: secret, sdpAnswerMaxBytes, timeoutMs }) => {
          const readBoundedTextLocal = (globalThis as OpenAIWebRtcSmokeGlobal)
            .oriroReadBoundedRealtimeResponseText;
...
async function smokeGatewayRelayBrowser(browser: Browser): Promise<SmokeResult> {
  ...
  const dir = await mkdtemp(path.join(tmpdir(), "oriro-realtime-talk-"));
  try {
