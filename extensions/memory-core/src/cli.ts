...
import {
  ...
  formatHelpExamples,
  theme,
} from "oriro/plugin-sdk/memory-core-host-runtime-cli";
import {
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["oriro memory status", "Show index and provider status."],
          [
            "oriro memory status --fix",
            "Repair stale recall locks and normalize promotion metadata.",
          ],
          ["oriro memory status --deep", "Probe embedding provider readiness."],
          ["oriro memory index --force", "Force a full reindex."],
          ['oriro memory search "meeting notes"', "Quick search using positional query."],
          [
            'oriro memory search --query "deployment" --max-results 20',
            "Limit results for focused troubleshooting.",
          ],
          [
            `oriro memory promote --limit 10 --min-score ${DEFAULT_PROMOTION_MIN_SCORE}`,
          ...
          [
            "oriro memory rem-backfill --path ./memory",
          ...
          ["oriro memory status --json", "Output machine-readable JSON (good for scripts)."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.oriro.ai/cli/memory")}\n`,
    );
