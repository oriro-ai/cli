...
read_when:
  - You want an Oriro agent to join a Google Meet call
  - You want an Oriro agent to create a new Google Meet call
  - You are configuring Chrome, Chrome node, or Twilio as a Google Meet transport
title: "Google Meet plugin"
---

Google Meet participant support for Oriro — the plugin is explicit by design:
...
- `agent` is the default talk-back mode: realtime transcription listens, the
  configured Oriro agent answers, and regular Oriro TTS speaks into Meet.
...
provider plus regular Oriro TTS. OpenAI is the default transcription
...
```bash
oriro googlemeet setup --transport chrome-node --mode transcribe
```
...
before asking an agent to join. Use `oriro googlemeet setup --json` for
...
```bash
oriro googlemeet setup --transport twilio
```
...
```bash
oriro googlemeet join https://meet.google.com/abc-defg-hij
```
...
```bash
oriro googlemeet create --transport chrome-node --mode agent
```
...
```bash
oriro googlemeet create --access-type OPEN --transport chrome-node --mode agent
```
...
- Browser fallback: used when OAuth credentials are absent. Oriro uses the
  ...
  the Oriro Chrome profile on the node to already be signed in to Google.
...
Oriro's microphone/camera permission grant and avoid the Meet **Use
...
Use `oriro googlemeet test-listen <meet-url> --transport chrome-node` when
...
Local Chrome joins through the signed-in Oriro browser profile. Realtime mode
requires `BlackHole 2ch` for the microphone/speaker path used by Oriro. For
...
You do **not** need a full Oriro Gateway or model API key inside a macOS VM
...
- Gateway host: Oriro Gateway, agent workspace, model/API keys, realtime
  provider, and the Google Meet plugin config.
- Parallels macOS VM: Oriro CLI/node host, Google Chrome, SoX, BlackHole 2ch,
...
Install or update Oriro in the VM, then enable the bundled plugin there:

```bash
oriro plugins enable google-meet
```

Start the node host in the VM:

```bash
oriro node run --host <gateway-host> --port 18789 --display-name parallels-macos
```
...
```bash
ORIRO_ALLOW_INSECURE_PRIVATE_WS=1 \
  oriro node run --host <gateway-lan-ip> --port 18789 --display-name parallels-macos
```
...
```bash
ORIRO_ALLOW_INSECURE_PRIVATE_WS=1 \
  oriro node install --host <gateway-lan-ip> --port 18789 --display-name parallels-macos --force
oriro node restart
```

`ORIRO_ALLOW_INSECURE_PRIVATE_WS=1` is process environment, not an
`oriro.json` setting. `oriro node install` stores it in the LaunchAgent
...
```bash
oriro googlemeet test-speech https://meet.google.com/abc-defg-hij
```

During realtime join, Oriro browser automation fills the guest name, clicks
...
If `chromeNode.node` is omitted, Oriro auto-selects only when exactly one
...
- `No connected Google Meet-capable node`: start `oriro node run` in the VM,
  approve pairing, and make sure `oriro plugins enable google-meet` and
  `oriro plugins enable browser` were run in the VM. Also confirm the
...
- Chrome opens but cannot join: sign in to the browser profile inside the VM, or
  keep `chrome.guestName` set for guest join. Guest auto-join uses Oriro
...
- Duplicate Meet tabs: leave `chrome.reuseExistingTab: true` enabled. Oriro
...
- No audio: in Meet, route microphone/speaker through the virtual audio device
  path used by Oriro; use separate virtual devices or Loopback-style routing
...
Oriro does not bundle or redistribute either package. The docs ask users to
...
installer or appliance that bundles BlackHole with Oriro, review BlackHole's
...
Chrome transport opens the Meet URL through Oriro browser control and joins
as the signed-in Oriro browser profile. On macOS, the plugin checks for
...
```bash
oriro googlemeet join https://meet.google.com/abc-defg-hij --transport chrome
oriro googlemeet join https://meet.google.com/abc-defg-hij --transport chrome-node
```

Route Chrome microphone and speaker audio through the local Oriro audio
...
meeting; Oriro does not discover those from the Meet page.
      ...
      "voice-call": {
        ...
        config: {
          ...
          realtime: {
            enabled: true,
            provider: "google",
            instructions: "Join this Google Meet as an Oriro agent. Be brief.",
...
oriro plugins list | grep -E 'google-meet|voice-call'
oriro googlemeet setup
```
...
`ORIRO_GOOGLE_MEET_*` environment variables.
...
4. Add the scopes Oriro requests:
...
`meetings.space.readonly` lets Oriro resolve Meet URLs/codes to spaces.
`meetings.space.settings` lets Oriro pass `SpaceConfig` settings such as
...
```bash
oriro googlemeet auth login --json
```
...
```bash
ORIRO_GOOGLE_MEET_CLIENT_ID="your-client-id" \
ORIRO_GOOGLE_MEET_CLIENT_SECRET="your-client-secret" \
oriro googlemeet auth login --json
```
...
ORIRO_GOOGLE_MEET_CLIENT_ID="your-client-id" \
ORIRO_GOOGLE_MEET_CLIENT_SECRET="your-client-secret" \
oriro googlemeet auth login --json --manual
```
...
support existed, rerun `oriro googlemeet auth login --json` so the refresh
...
```bash
oriro googlemeet doctor --oauth --json
```
...
```bash
oriro googlemeet doctor --oauth --create-space --json
oriro googlemeet create --no-join --json
```
...
```bash
oriro googlemeet doctor --oauth --meeting https://meet.google.com/abc-defg-hij --json
oriro googlemeet resolve-space --meeting https://meet.google.com/abc-defg-hij
```
...
space. A refresh-token error means rerun `oriro googlemeet auth login
...
- `ORIRO_GOOGLE_MEET_CLIENT_ID` or `GOOGLE_MEET_CLIENT_ID`
- `ORIRO_GOOGLE_MEET_CLIENT_SECRET` or `GOOGLE_MEET_CLIENT_SECRET`
- `ORIRO_GOOGLE_MEET_REFRESH_TOKEN` or `GOOGLE_MEET_REFRESH_TOKEN`
- `ORIRO_GOOGLE_MEET_ACCESS_TOKEN` or `GOOGLE_MEET_ACCESS_TOKEN`
- `ORIRO_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT` or
  `GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT`
- `ORIRO_GOOGLE_MEET_DEFAULT_MEETING` or `GOOGLE_MEET_DEFAULT_MEETING`
- `ORIRO_GOOGLE_MEET_PREVIEW_ACK` or `GOOGLE_MEET_PREVIEW_ACK`
...
```bash
oriro googlemeet preflight --meeting https://meet.google.com/abc-defg-hij
```

List meeting artifacts and attendance after Meet has created conference records:

```bash
oriro googlemeet artifacts --meeting https://meet.google.com/abc-defg-hij
oriro googlemeet attendance --meeting https://meet.google.com/abc-defg-hij
oriro googlemeet export --meeting https://meet.google.com/abc-defg-hij --output ./meet-export
```
...
```bash
oriro googlemeet latest --today
oriro googlemeet calendar-events --today --json
oriro googlemeet artifacts --event "Weekly sync"
oriro googlemeet attendance --today --format csv --output attendance.csv
```
...
```bash
oriro googlemeet latest --meeting https://meet.google.com/abc-defg-hij
oriro googlemeet artifacts --conference-record conferenceRecords/abc123 --json
oriro googlemeet attendance --conference-record conferenceRecords/abc123 --json
```
...
```bash
oriro googlemeet end-active-conference https://meet.google.com/abc-defg-hij
```
...
Oriro accepts a Meet URL, meeting code, or `spaces/{id}` input and resolves it
to the API space resource before ending the active conference.
It is separate from `googlemeet leave`: `leave` stops Oriro's local/session
...
```bash
oriro googlemeet artifacts --conference-record conferenceRecords/abc123 \
  --format markdown --output meet-artifacts.md
oriro googlemeet attendance --conference-record conferenceRecords/abc123 \
  --format markdown --output meet-attendance.md
oriro googlemeet attendance --conference-record conferenceRecords/abc123 \
  --format csv --output meet-attendance.csv
oriro googlemeet export --conference-record conferenceRecords/abc123 \
...
```bash
ORIRO_LIVE_TEST=1 \
ORIRO_GOOGLE_MEET_LIVE_MEETING=https://meet.google.com/abc-defg-hij \
...
```bash
oriro googlemeet setup --transport chrome-node --mode transcribe
oriro googlemeet test-listen https://meet.google.com/abc-defg-hij --transport chrome-node --timeout-ms 30000
```

Live smoke environment:

- `ORIRO_LIVE_TEST=1` enables guarded live tests.
- `ORIRO_GOOGLE_MEET_LIVE_MEETING` points at a retained Meet URL, code, or
  `spaces/{id}`.
- `ORIRO_GOOGLE_MEET_CLIENT_ID` or `GOOGLE_MEET_CLIENT_ID` provides the OAuth
  client id.
- `ORIRO_GOOGLE_MEET_REFRESH_TOKEN` or `GOOGLE_MEET_REFRESH_TOKEN` provides
  the refresh token.
- Optional: `ORIRO_GOOGLE_MEET_CLIENT_SECRET`,
  `ORIRO_GOOGLE_MEET_ACCESS_TOKEN`, and
  `ORIRO_GOOGLE_MEET_ACCESS_TOKEN_EXPIRES_AT` use the same fallback names
  without the `ORIRO_` prefix.
...
{
  "source": "browser",
  "error": "google-login-required: Sign in to Google in the Oriro browser profile, then retry meeting creation.",
  "manualActionRequired": true,
  "manualActionReason": "google-login-required",
  "manualActionMessage": "Sign in to Google in the Oriro browser profile, then retry meeting creation.",
  "browser": {
    "nodeId": "ba0f4e4bc...",
    "targetId": "tab-1",
...
profile is signed out, Oriro reports `manualActionRequired: true` or a
...
realtime transcription provider key, and a configured Oriro TTS provider.
...
- `chrome.guestName: "Oriro Agent"`: name used on the signed-out Meet guest
  screen
- `chrome.autoJoin: true`: best-effort guest-name fill and Join Now click
  through Oriro browser automation on `chrome-node`
...
- `mode: "agent"`: default talk-back mode. Participant speech is transcribed by
  ...
  Oriro agent in a per-meeting sub-agent session, and spoken back through the
  normal Oriro TTS runtime.
- `mode: "bidi"`: fallback direct bidirectional realtime model mode. The
  ...
  `oriro_agent_consult` for deeper/tool-backed answers.
...
- `realtime.instructions`: brief spoken replies, with
  `oriro_agent_consult` for deeper answers
- `realtime.introMessage`: short spoken readiness check when the realtime bridge
  connects; set it to `""` to join silently
- `realtime.agentId`: optional Oriro agent id for
  `oriro_agent_consult`; defaults to `main`
...
VM. In both cases the model providers and `oriro_agent_consult` run on the
Gateway host, so model credentials stay there. With the default `mode: "agent"`,
the realtime transcription provider handles listening, the configured Oriro
agent produces the answer, and regular Oriro TTS speaks it into Meet. Use
...
- `speechReady` / `speechBlockedReason` / `speechBlockedMessage`: whether
  managed Chrome speech is allowed now. `speechReady: false` means Oriro did
...
transcripts are routed through the configured Oriro agent, and the answer is
spoken through the normal Oriro TTS runtime. Set `mode: "bidi"` when you want
...
| ------- | ----------------------------- | -------------------------------------- | ----------------------------------------------------- |
| `agent` | The configured Oriro agent | Normal Oriro TTS runtime            | You want "my agent is in the meeting" behavior        |
...
information, or normal Oriro tools, it can call `oriro_agent_consult`.

The consult tool runs the regular Oriro agent behind the scenes with recent
meeting transcript context and returns a concise spoken answer. In `agent` mode,
Oriro sends that answer directly to the TTS runtime; in `bidi` mode, the
...
Meet lane should consult a dedicated Oriro agent workspace, model defaults,
...
```bash
oriro googlemeet speak meet_... "Say exactly: I'm here and listening."
```
...
- `oriro logs --follow` shows DTMF TwiML served before realtime TwiML, then a
...
```bash
oriro plugins list | grep google-meet
oriro googlemeet setup
```
...
hear you in the meeting?" That is Meet's audio-choice interstitial; Oriro
clicks **Use microphone** through browser automation when available and keeps
waiting for the real meeting state. For create-only browser fallback, Oriro
...
- For API creation: `oauth.clientId` and `oauth.refreshToken` are configured,
  or matching `ORIRO_GOOGLE_MEET_*` environment variables are present.
- For API creation: the refresh token was minted after create support was
  ...
  `oriro googlemeet auth login --json` and update plugin config.
- For browser fallback: `defaultTransport: "chrome-node"` and
  `chromeNode.node` point at a connected node with `browser.proxy` and
  `googlemeet.chrome`.
- For browser fallback: the Oriro Chrome profile on that node is signed in
...
- For browser fallback: if Meet shows "Do you want people to hear you in the
  meeting?", leave the tab open. Oriro should click **Use microphone** or, for
...
```bash
oriro googlemeet setup
oriro googlemeet doctor
```

Use `mode: "agent"` for the normal STT -> Oriro agent -> TTS talk-back path,
...
run `oriro googlemeet status --json <session-id>` after participants speak
...
utterance but Oriro did not see new output bytes reach the Chrome audio
bridge.
...
- Meet microphone and speaker are routed through the virtual audio path used by
  Oriro. `doctor` should show `meet output routed: yes` for local Chrome
  realtime joins.
...
```bash
oriro googlemeet recover-tab
oriro googlemeet recover-tab https://meet.google.com/abc-defg-hij
```
...
```bash
oriro googlemeet setup --transport twilio
oriro voicecall setup
oriro voicecall smoke
```

`voicecall smoke` is readiness-only by default. To dry-run a specific number:

```bash
oriro voicecall smoke --to "+15555550123"
```
...
```bash
oriro voicecall smoke --to "+15555550123" --yes
```
...
- Run `oriro googlemeet doctor <session-id>` to confirm the delegated Twilio
  call ID, whether DTMF was queued, and whether the intro greeting was requested.
- Run `oriro voicecall status --call-id <id>` and confirm the call is still
  active.
- Run `oriro voicecall tail` and check that Twilio webhooks are arriving at
  the Gateway.
- Run `oriro logs --follow` and look for the Twilio Meet sequence: Google
...
- Re-run `oriro googlemeet setup --transport twilio`; a green setup check is
...
- If the participant joins but you do not hear the greeting, check
  `oriro logs --follow` for the post-DTMF `voicecall.speak` request and
...
- `chrome.audioInputCommand` plus `chrome.audioOutputCommand`: Oriro owns the
