...
REMOTE_URL="${ORIRO_REF_REMOTE:-https://github.com/oriro/oriro.git}"
REF=""
EXPECTED_SHA=""
FALLBACK_OK=0
...
usage() {
  cat >&2 <<'EOF'
Usage: resolve-oriro-ref.sh --ref <ref> [--expected-sha <sha>] [--fallback-ok] [--github-output <file>]

Fast-resolves Oriro branch and tag refs with git ls-remote. Full commit SHAs
...
echo "Failed to resolve ORIRO ref: ${REF}" >&2
exit 1
