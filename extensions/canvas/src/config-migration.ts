...
import { asOptionalRecord as readRecord } from "oriro/plugin-sdk/string-coerce-runtime";
...
export function migrateLegacyCanvasHostConfig(config: OriroConfig): {
  config: OriroConfig;
  changes: string[];
} | null {
  ...
  const next = { ...config, plugins } as OriroConfig & { canvasHost?: unknown };
