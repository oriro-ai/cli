...
read_when:
  ...
  - Explaining how Oriro updates one channel message while work is in progress
...
When progress drafts are enabled, Oriro creates one visible work-in-progress
...
That is usually enough. Oriro will pick an automatic one-word label, wait
...
Oriro sends the final answer normally and cleans up or stops updating the
...
The default label is `auto`, which chooses from Oriro's built-in
...
Oriro renders only the `progress.text` in the channel progress UI. The
...
Oriro uses the same formatter for progress drafts and `/verbose`:
...
Oriro truncates long progress lines by default so repeated draft edits do not
...
With `toolProgress: false`, Oriro still suppresses the older standalone
...
When the final answer is ready, Oriro tries to keep the chat clean:

- If the draft can safely become the final answer, Oriro edits it in place.
- If the channel uses native progress streaming, Oriro finalizes that stream
...
- If the final answer has media, an approval prompt, an explicit reply target,
  too many chunks, or a failed edit/send, Oriro sends the final answer through
...
Check `streaming.progress.toolProgress`. If it is `false`, Oriro keeps the
