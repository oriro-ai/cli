...
function buildQaLabSearchResult(query: string, index: number) {
  const url = `https://docs.oriro.ai/qa-lab/search-fixture/${index + 1}`;
  return {
    ...
    siteName: resolveSiteName(url) || "docs.oriro.ai",
  };
}
...
export function createQaLabWebSearchProvider(): WebSearchProviderPlugin {
  return {
    ...
    signupUrl: "https://docs.oriro.ai/concepts/qa-e2e-automation",
    docsUrl: "https://docs.oriro.ai/concepts/qa-e2e-automation",
    credentialPath: "",
    inactiveSecretPaths: [],
