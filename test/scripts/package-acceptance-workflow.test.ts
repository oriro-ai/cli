...
const LIVE_E2E_WORKFLOW = ".github/workflows/oriro-live-and-e2e-checks-reusable.yml";
...
const RELEASE_CHECKS_WORKFLOW = ".github/workflows/oriro-release-checks.yml";
const RELEASE_PUBLISH_WORKFLOW = ".github/workflows/oriro-release-publish.yml";
const STABLE_MAIN_CLOSEOUT_WORKFLOW = ".github/workflows/oriro-stable-main-closeout.yml";
...
const SCHEDULED_LIVE_CHECKS_WORKFLOW = ".github/workflows/oriro-scheduled-live-checks.yml";
...
const VERIFY_PROVIDER_SECRETS_SCRIPT =
  ".agents/skills/release-oriro-ci/scripts/verify-provider-secrets.mjs";
...
describe("package acceptance workflow", () => {
  it("verifies immutable postpublish evidence before stable closeout reads it", () => {
    ...
    expect(workflow).toContain(
      'awk -v asset="oriro-${release_version}-stable-main-closeout.json"',
    );
  ...
  it("keeps Crabbox hydration compatible with local Actions replay", () => {
    ...
    expect(hydrateWindowsPnpm.run).toContain('"oriro",');
    expect(hydrateWindowsPnpm.run).toContain(
    ...
    expect(workflowText).toContain("ORIRO_CRABBOX_HYDRATE_DOWNLOAD_TIMEOUT_SECONDS:-300");
    expect(workflowText).toContain("ORIRO_CRABBOX_HYDRATE_DOWNLOAD_RETRIES:-3");
  ...
  it("resolves candidate package sources before reusing Docker E2E lanes", () => {
    ...
    expect(workflow).toContain("scripts/resolve-oriro-package-candidate.mjs");
    ...
    expect(workflow).toContain(
      "uses: ./.github/workflows/oriro-live-and-e2e-checks-reusable.yml",
    );
    expect(workflow).toContain(
    ...
    expect(workflow).toContain(
      "node scripts/check-oriro-package-tarball.mjs .artifacts/docker-e2e-package/oriro-current.tgz",
    );
  ...
  it("offers bounded product profiles and can run Telegram against the resolved artifact", () => {
    ...
    expect(workflow).toContain(
      "package_label: oriro@${{ needs.resolve_package.outputs.package_version }}",
    );
  ...
  it("requires pinned full release child workflows to run at the resolved target SHA", () => {
    ...
    expect(releaseChecksWorkflow).toContain(
      'codex_plugin_spec="npm:@oriro/codex@${BASH_REMATCH[1]}"',
    );
    expect(releaseChecksWorkflow).toContain(
    ...
    expect(releaseChecksWorkflow).toContain(
      "package_spec: ${{ needs.resolve_target.outputs.package_acceptance_package_spec || needs.resolve_target.outputs.release_package_spec || 'oriro@beta' }}",
    );
  });
...
describe("package artifact reuse", () => {
  it("lets reusable Docker E2E consume an already resolved package artifact", () => {
    ...
    expect(workflow).toContain("ORIRO_DOCKER_E2E_SELECTED_SHA:");
    expect(workflow).toContain(
      "ORIRO_UPGRADE_SURVIVOR_BASELINE_SPEC: ${{ inputs.published_upgrade_survivor_baseline }}",
    );
    expect(workflow).toContain(
      "ORIRO_UPGRADE_SURVIVOR_BASELINE_SPECS: ${{ matrix.group.published_upgrade_survivor_baselines || inputs.published_upgrade_survivor_baselines }}",
    );
    expect(workflow).toContain(
      "ORIRO_UPGRADE_SURVIVOR_SCENARIOS: ${{ inputs.published_upgrade_survivor_scenarios }}",
    );
    expect(workflow).toContain("Download current-run Oriro Docker E2E package");
    expect(workflow).toContain("Download previous-run Oriro Docker E2E package");
    ...
    expect(workflow).toContain("ORIRO_DOCKER_E2E_REPO_ROOT:");
    ...
    expect(workflowStep(prepareDockerImage, "Plan Docker E2E images").env).toEqual({
      ...
      ORIRO_UPGRADE_SURVIVOR_BASELINE_SPEC: "${{ inputs.published_upgrade_survivor_baseline }}",
      ORIRO_UPGRADE_SURVIVOR_BASELINE_SPECS:
    ...
    expect(workflow).toContain(
      "ORIRO_UPGRADE_SURVIVOR_BASELINE_SPECS: ${{ inputs.published_upgrade_survivor_baselines }}",
    );
    ...
    expect(scheduler).toContain(
      "published_upgrade_survivor_baseline=${shellQuote(process.env.ORIRO_UPGRADE_SURVIVOR_BASELINE_SPEC)}",
    );
    expect(scheduler).toContain(
      "published_upgrade_survivor_baselines=${shellQuote(process.env.ORIRO_UPGRADE_SURVIVOR_BASELINE_SPECS)}",
    );
    expect(scheduler).toContain(
      '["ORIRO_UPGRADE_SURVIVOR_BASELINE_SPEC", baseEnv.ORIRO_UPGRADE_SURVIVOR_BASELINE_SPEC]',
    );
    expect(scheduler).toContain('["ORIRO_UPGRADE_SURVIVOR_BASELINE_SPECS",');
    expect(scheduler).toContain('["ORIRO_UPGRADE_SURVIVOR_SCENARIOS",');
    expect(packageJson).toContain("ORIRO_UPGRADE_SURVIVOR_PUBLISHED_BASELINE=1");
    expect(packageJson).toContain("test:docker:update-restart-auth");
    expect(packageJson).toContain("ORIRO_UPGRADE_SURVIVOR_UPDATE_RESTART_MODE=auto-auth");
    expect(publishedUpgradeSurvivor).toContain("validate_baseline_package_spec");
    expect(publishedUpgradeSurvivor).toContain("ORIRO_UPGRADE_SURVIVOR_UPDATE_RESTART_MODE");
    ...
    expect(publishedUpgradeSurvivor).toContain(
      "env -u ORIRO_GATEWAY_TOKEN -u ORIRO_GATEWAY_PASSWORD oriro",
    );
    expect(publishedUpgradeSurvivor).toContain("phase prepare-update-restart-probe");
    expect(publishedUpgradeSurvivor).toContain("oriro@(alpha|beta|latest|");
    ...
    expect(publishedUpgradeSurvivor).toContain('"id": "opik-oriro"');
  ...
  it("bounds shared Docker image pulls so package acceptance cannot stall forever", () => {
    ...
    expect(pullHelper).toContain("ORIRO_DOCKER_PULL_ATTEMPTS");
    expect(pullHelper).toContain("ORIRO_DOCKER_PULL_TIMEOUT_SECONDS");
    expect(pullHelper).toContain('timeout_seconds="${ORIRO_DOCKER_PULL_TIMEOUT_SECONDS:-180}"');
    expect(pullHelper).toContain(
      'retry_delay_seconds="${ORIRO_DOCKER_PULL_RETRY_DELAY_SECONDS:-5}"',
    );
    expect(pullHelper).toContain('source "$SCRIPT_DIR/lib/host-timeout.sh"');
    expect(pullHelper).toContain("oriro_host_timeout_bin");
    ...
    expect(dockerE2ePlanAction).not.toContain('docker pull "${ORIRO_DOCKER_E2E_');
  });
  ...
  it("shards broad native live tests instead of one serial live-all job", () => {
    ...
    expect(workflow).toContain("ORIRO_LIVE_COMMAND: ${{ matrix.command }}");
    ...
    expect(workflow).toContain("ORIRO_LIVE_GATEWAY_PROVIDERS=deepseek,fireworks");
    expect(workflow).toContain("ORIRO_LIVE_GATEWAY_PROVIDERS=opencode-go,openrouter");
    expect(workflow).toContain("ORIRO_LIVE_GATEWAY_PROVIDERS=xai,zai");
    expect(workflow).toContain("inputs.live_suite_filter == 'live-gateway-advisory-docker'");
    expect(workflow).toContain("ORIRO_LIVE_CLI_BACKEND_MODEL=claude-cli/claude-sonnet-4-6");
    expect(workflow).toContain("ORIRO_LIVE_CLI_BACKEND_AUTH=api-key");
    expect(workflow).not.toContain("ORIRO_LIVE_CLI_BACKEND_USE_CI_SAFE_CODEX_CONFIG=1");
    expect(workflow).not.toContain('service_tier=\\"fast\\"');
    expect(workflow).not.toContain("ORIRO_LIVE_CLI_BACKEND_ARGS=");
    expect(workflow).not.toContain("ORIRO_LIVE_CLI_BACKEND_RESUME_ARGS=");
    expect(workflow).not.toContain(
      'ORIRO_LIVE_CLI_BACKEND_ARGS=["exec","--json","--color","never","--sandbox","danger-full-access","--skip-git-repo-check"]',
    );
    ...
    expect(workflow).toContain(
      "command: ORIRO_LIVE_CODEX_HARNESS=1 ORIRO_LIVE_CODEX_HARNESS_AUTH=api-key node .release-harness/scripts/test-live-shard.mjs native-live-src-gateway-core",
    );
    expect(workflow).toContain(
      "command: ORIRO_LIVE_CODEX_HARNESS=1 ORIRO_LIVE_CODEX_HARNESS_AUTH=api-key node .release-harness/scripts/test-live-shard.mjs native-live-src-gateway-backends",
    );
    expect(workflow).toContain("suite_id: native-live-src-infra");
    expect(workflow).toContain(
      "command: ORIRO_LIVE_APNS_REACHABILITY=1 node .release-harness/scripts/test-live-shard.mjs native-live-src-infra",
    );
    ...
    expect(workflow).toContain("ORIRO_LIVE_GATEWAY_MODELS=anthropic/claude-opus-4-8");
    ...
    expect(workflow).toContain(
      "command: ORIRO_LIVE_GATEWAY_THINKING=off ORIRO_LIVE_GATEWAY_PROVIDERS=openai ORIRO_LIVE_GATEWAY_MODELS=openai/gpt-5.5 ORIRO_LIVE_GATEWAY_STEP_TIMEOUT_MS=180000 ORIRO_LIVE_GATEWAY_MODEL_TIMEOUT_MS=600000",
    );
    expect(workflow).toContain(
      "ORIRO_LIVE_GATEWAY_MODELS=google/gemini-3.1-pro-preview node .release-harness/scripts/test-live-shard.mjs native-live-src-gateway-profiles",
    );
    expect(workflow).toContain(
      "ORIRO_LIVE_GATEWAY_MODELS=minimax/MiniMax-M2.7,minimax-portal/MiniMax-M2.7 ORIRO_LIVE_GATEWAY_MAX_MODELS=2",
    );
    expect(workflow).toMatch(
    ...
    expect(workflow).not.toContain(
      "ORIRO_LIVE_GATEWAY_PROVIDERS=deepseek,opencode-go,openrouter,xai,zai",
    );
    expect(workflow).toContain("suite_id: live-gateway-anthropic-docker");
    expect(workflow).toContain("ORIRO_LIVE_GATEWAY_MAX_MODELS=2");
    expect(workflow).toContain(
      "ORIRO_LIVE_GATEWAY_THINKING=off ORIRO_LIVE_GATEWAY_PROVIDERS=openai ORIRO_LIVE_GATEWAY_MODELS=openai/gpt-5.5 ORIRO_LIVE_GATEWAY_MAX_MODELS=1 ORIRO_LIVE_GATEWAY_STEP_TIMEOUT_MS=90000 ORIRO_LIVE_GATEWAY_MODEL_TIMEOUT_MS=600000",
    );
    expect(workflow).toContain(
      "ORIRO_LIVE_GATEWAY_MODELS=anthropic/claude-sonnet-4-6,anthropic/claude-haiku-4-5 ORIRO_LIVE_GATEWAY_MAX_MODELS=2",
    );
    expect(workflow).toContain("ORIRO_LIVE_GATEWAY_MODEL_TIMEOUT_MS=600000");
    ...
    expect(workflow).toContain("ORIRO_LIVE_SUITE_ADVISORY: ${{ matrix.advisory }}");
    expect(workflow).toContain("Advisory live suite failed with exit code");
    expect(workflow).toMatch(
      /validate_live_media_provider_suites:[\s\S]*?ORIRO_LIVE_SUITE_ADVISORY: \$\{\{ matrix\.advisory \}\}[\s\S]*?shell: bash[\s\S]*?Advisory live suite failed with exit code/u,
    );
    expect(workflow).toMatch(
      /suite_id: live-gateway-advisory-docker-deepseek-fireworks[\s\S]*?advisory: true/u,
    );
    expect(workflow).toMatch(
      /validate_live_media_provider_suites:[\s\S]*?ORIRO_LIVE_SUITE_ADVISORY: \$\{\{ matrix\.advisory \}\}/u,
    );
    expect(workflow).toMatch(
    ...
    expect(workflow).toContain("image: ghcr.io/oriro-ai/cli-live-media-runner:ubuntu-24.04");
    ...
    expect(workflow).toContain("ORIRO_LIVE_VIDEO_GENERATION_PROVIDERS=google,minimax");
    expect(workflow).toContain("ORIRO_LIVE_VIDEO_GENERATION_PROVIDERS=openai,openrouter,xai");
    ...
    expect(retryHelper).toContain("ORIRO_LIVE_COMMAND_ATTEMPTS:-2");
    ...
    expect(retryHelper).toContain("ORIRO_LIVE_COMMAND_RATE_LIMIT_RETRY_DELAY_SECONDS:-60");
  ...
  it("runs Docker live harnesses from trusted helper scripts", () => {
    ...
    expect(workflow).toContain(
      'run: ORIRO_LIVE_DOCKER_REPO_ROOT="$GITHUB_WORKSPACE" timeout --foreground --kill-after=30s 35m bash .release-harness/scripts/test-live-models-docker.sh',
    );
    expect(workflow).toContain(
      "command: ORIRO_LIVE_GATEWAY_THINKING=off ORIRO_LIVE_GATEWAY_PROVIDERS=openai ORIRO_LIVE_GATEWAY_MODELS=openai/gpt-5.5 ORIRO_LIVE_GATEWAY_MAX_MODELS=1",
    );
    expect(workflow).toContain(
      "command: ORIRO_LIVE_GATEWAY_PROVIDERS=minimax,minimax-portal ORIRO_LIVE_GATEWAY_MODELS=minimax/MiniMax-M2.7,minimax-portal/MiniMax-M2.7 ORIRO_LIVE_GATEWAY_MAX_MODELS=2",
    );
    expect(workflow).toContain(
      'command: ORIRO_LIVE_DOCKER_REPO_ROOT="$GITHUB_WORKSPACE" timeout --foreground --kill-after=30s 45m bash .release-harness/scripts/test-live-cli-backend-docker.sh',
    );
    expect(workflow).toContain(
      'command: ORIRO_LIVE_DOCKER_REPO_ROOT="$GITHUB_WORKSPACE" timeout --foreground --kill-after=30s 45m bash .release-harness/scripts/test-live-acp-bind-docker.sh',
    );
    expect(workflow).toContain(
      'command: ORIRO_LIVE_DOCKER_REPO_ROOT="$GITHUB_WORKSPACE" timeout --foreground --kill-after=30s 35m bash .release-harness/scripts/test-live-codex-harness-docker.sh',
    );
    expect(workflow).toContain(
      'command: ORIRO_LIVE_DOCKER_REPO_ROOT="$GITHUB_WORKSPACE" timeout --foreground --kill-after=30s 20m bash .release-harness/scripts/test-live-subagent-announce-docker.sh',
    );
    ...
    expect(scenarios).toContain("process.env.ORIRO_DOCKER_E2E_REPO_ROOT");
    expect(scenarios).toContain(
      'harness="\\${ORIRO_DOCKER_E2E_TRUSTED_HARNESS_DIR:-${LIVE_DOCKER_DEFAULT_HARNESS_DIR}}"',
    );
    ...
    expect(liveDockerAuth).toContain("oriro_live_init_docker_run_args()");
    expect(liveDockerAuth).toContain("oriro_live_stage_profile_into_home()");
    expect(liveDockerAuth).toContain("oriro_live_chown_bind_dirs_for_container_user()");
    expect(liveDockerAuth).toContain("oriro_live_uses_managed_bind_dirs()");
    expect(liveDockerAuth).toContain('oriro_live_truthy "${ORIRO_TESTBOX:-}"');
    expect(liveDockerAuth).toContain('[[ -n "${ORIRO_DOCKER_CACHE_HOME_DIR:-}" ]]');
    expect(liveDockerAuth).toContain(
      'timeout_value="${2:-${ORIRO_LIVE_DOCKER_RUN_TIMEOUT:-2700s}}"',
    );
    ...
    for (const script of [harness, ...sharedLiveScripts]) {
      ...
      expect(script).toContain("oriro_live_init_docker_run_args DOCKER_RUN_ARGS");
      expect(script).toContain("oriro_live_prepare_bind_dir_for_container_user");
    ...
    for (const script of sharedLiveScripts) {
      expect(script).toContain("oriro_live_uses_managed_bind_dirs");
    ...
    for (const script of [
      ...
      expect(script).not.toContain('timeout --kill-after=30s "${ORIRO_LIVE_');
    }
    expect(readFileSync("scripts/test-live-models-docker.sh", "utf8")).toContain(
      "ORIRO_LIVE_MODELS_DOCKER_RUN_TIMEOUT:-2100s",
    );
    expect(readFileSync("scripts/test-live-gateway-models-docker.sh", "utf8")).toContain(
      "ORIRO_LIVE_GATEWAY_DOCKER_RUN_TIMEOUT:-2100s",
    );
    expect(readFileSync("scripts/test-live-cli-backend-docker.sh", "utf8")).toContain(
      "ORIRO_LIVE_CLI_BACKEND_DOCKER_RUN_TIMEOUT:-2700s",
    );
    expect(readFileSync("scripts/test-live-cli-backend-docker.sh", "utf8")).toContain(
      'CLI_SETUP_TIMEOUT_SECONDS="$(oriro_live_read_positive_int_env ORIRO_LIVE_CLI_BACKEND_SETUP_TIMEOUT_SECONDS 180)"',
    );
    expect(readFileSync("scripts/test-live-cli-backend-docker.sh", "utf8")).toContain(
      'timeout_value="${ORIRO_LIVE_CLI_BACKEND_SETUP_TIMEOUT_SECONDS:?missing live CLI backend setup timeout seconds}s"',
    );
    expect(readFileSync("scripts/test-live-cli-backend-docker.sh", "utf8")).toContain(
    ...
    expect(readFileSync("scripts/test-live-acp-bind-docker.sh", "utf8")).toContain(
      "ORIRO_LIVE_ACP_BIND_DOCKER_RUN_TIMEOUT:-2700s",
    );
    expect(readFileSync("scripts/test-live-acp-bind-docker.sh", "utf8")).toContain(
      'ACP_SETUP_TIMEOUT_SECONDS="$(oriro_live_read_positive_int_env ORIRO_LIVE_ACP_BIND_SETUP_TIMEOUT_SECONDS 180)"',
    );
    expect(readFileSync("scripts/test-live-acp-bind-docker.sh", "utf8")).toContain(
      'timeout_value="${ORIRO_LIVE_ACP_BIND_SETUP_TIMEOUT_SECONDS:?missing live ACP bind setup timeout seconds}s"',
    );
    expect(readFileSync("scripts/test-live-acp-bind-docker.sh", "utf8")).toContain(
      '-e ORIRO_LIVE_ACP_BIND_SETUP_TIMEOUT_SECONDS="$ACP_SETUP_TIMEOUT_SECONDS"',
    );
    expect(readFileSync("scripts/test-live-acp-bind-docker.sh", "utf8")).toContain(
    ...
    expect(readFileSync("scripts/test-live-codex-harness-docker.sh", "utf8")).toContain(
      "ORIRO_LIVE_CODEX_HARNESS_DOCKER_RUN_TIMEOUT:-2100s",
    );
    expect(readFileSync("scripts/test-live-codex-harness-docker.sh", "utf8")).toContain(
      'CODEX_HARNESS_SETUP_TIMEOUT_SECONDS="$(oriro_live_read_positive_int_env ORIRO_LIVE_CODEX_HARNESS_SETUP_TIMEOUT_SECONDS 180)"',
    );
    expect(readFileSync("scripts/test-live-codex-harness-docker.sh", "utf8")).toContain(
      'timeout_value="${ORIRO_LIVE_CODEX_HARNESS_SETUP_TIMEOUT_SECONDS:?missing live Codex harness setup timeout seconds}s"',
    );
    expect(readFileSync("scripts/test-live-codex-harness-docker.sh", "utf8")).toContain(
    ...
    expect(readFileSync("scripts/test-live-subagent-announce-docker.sh", "utf8")).toContain(
      "ORIRO_LIVE_SUBAGENT_DOCKER_RUN_TIMEOUT:-1200s",
    );
    expect(build).toContain('ROOT_DIR="${ORIRO_LIVE_DOCKER_REPO_ROOT:-$SCRIPT_ROOT_DIR}"');
    ...
    expect(build).toContain('LIVE_IMAGE_PULL_ATTEMPTS="${ORIRO_LIVE_DOCKER_PULL_ATTEMPTS:-3}"');
    ...
    expect(stage).toContain(
      'local scripts_dir="${ORIRO_LIVE_DOCKER_SCRIPTS_DIR:-/src/scripts}"',
    );
  ...
  it("fails Droid ACP Docker live proof when Factory auth is missing", () => {
    ...
    expect(script).toContain("oriro_live_acp_bind_load_factory_api_key_from_profile");
    expect(script).not.toContain('source "$PROFILE_FILE"');
    expect(script.indexOf("oriro_live_acp_bind_load_factory_api_key_from_profile")).toBeLessThan(
  ...
  it("plumbs live credentials through planned Docker E2E live lanes", () => {
    ...
    const testboxProviderSecretKeys = [
      ...
      "ORIRO_LIVE_BROWSER_CDP_URL",
      "ORIRO_LIVE_SETUP_TOKEN",
      "ORIRO_LIVE_SETUP_TOKEN_MODEL",
      "ORIRO_LIVE_SETUP_TOKEN_PROFILE",
      "ORIRO_LIVE_SETUP_TOKEN_VALUE",
      "GEMINI_API_KEY",
      "GOOGLE_API_KEY",
      "OPENROUTER_API_KEY",
      "QWEN_API_KEY",
      ...
      "ORIRO_CODEX_AUTH_JSON",
      "ORIRO_CODEX_CONFIG_TOML",
      "ORIRO_CLAUDE_JSON",
      "ORIRO_CLAUDE_CREDENTIALS_JSON",
      "ORIRO_CLAUDE_SETTINGS_JSON",
      "ORIRO_CLAUDE_SETTINGS_LOCAL_JSON",
      "ORIRO_GEMINI_SETTINGS_JSON",
      "FIREWORKS_API_KEY",
    ];
    ...
    expectTextToIncludeAll(dockerPlanAction, [
      ...
      "require_any Codex ORIRO_CODEX_AUTH_JSON",
      'if [[ "$credentials" == *",anthropic,"* ]]; then',
      "require_any Anthropic ANTHROPIC_API_TOKEN ANTHROPIC_API_KEY ORIRO_CLAUDE_CREDENTIALS_JSON ORIRO_CLAUDE_JSON",
      'if [[ "$credentials" == *",factory,"* ]]; then',
      "require_any Factory FACTORY_API_KEY",
      'if [[ "$credentials" == *",gemini,"* ]]; then',
      "require_any Gemini GEMINI_API_KEY GOOGLE_API_KEY ORIRO_GEMINI_SETTINGS_JSON",
    ...
    expect(reusableWorkflow.match(/ORIRO_LIVE_CLI_BACKEND_AUTH=subscription/g)).toHaveLength(2);
    expect(
      reusableWorkflow.match(
        /if \[\[ -n "\$\{ORIRO_CLAUDE_CREDENTIALS_JSON:-\}" \|\| -n "\$\{CLAUDE_CODE_OAUTH_TOKEN:-\}" \]\]; then/g,
      ),
    ).toHaveLength(2);
  });
  ...
  it("fails Testbox changed-check delegation when the remote command fails", () => {
    ...
    expect(workflow).toContain('PNPM_CONFIG_STORE_DIR: "/tmp/oriro-pnpm-store"');
  ...
  it("allows the Telegram lane to run from reusable package acceptance artifacts", () => {
    ...
    expect(workflow).toContain("ORIRO_NPM_TELEGRAM_PACKAGE_TGZ");
  ...
  it("includes package acceptance in release checks", () => {
    ...
    expect(workflow).toContain(
      "ORIRO_QA_CONVEX_SITE_URL: ${{ secrets.ORIRO_QA_CONVEX_SITE_URL }}",
    );
    expect(workflow).toContain(
      "ORIRO_QA_CONVEX_SECRET_CI: ${{ secrets.ORIRO_QA_CONVEX_SECRET_CI }}",
    );
    ...
    expect(workflow).toContain("ORIRO_RELEASE_QA_*_LIVE_CI_ENABLED");
    expect(workflow).not.toContain(
  ...
  it("detects Matrix fail-fast support for older release refs", () => {
    ...
    expect(releaseWorkflow).toContain(
      'pnpm oriro qa matrix --help 2>/dev/null | grep -F -q -- "--fail-fast"',
    );
    expect(releaseWorkflow).toContain("matrix_args+=(--fail-fast)");
    expect(releaseWorkflow).toContain(
      'pnpm oriro qa matrix --output-dir "${attempt_output_dir}" "${matrix_args[@]}"',
    );
    expect(releaseWorkflow).toContain(
  ...
  it("runs live transport lanes nightly while release checks stay gated", () => {
    ...
    for (const channel of ["DISCORD", "WHATSAPP", "SLACK"]) {
      ...
      expect(releaseWorkflow).toContain(
        `RELEASE_QA_${channel}_LIVE_CI_ENABLED: \${{ vars.ORIRO_RELEASE_QA_${channel}_LIVE_CI_ENABLED || 'false' }}`,
      );
      ...
      expect(releaseWorkflow).not.toContain(
        `vars.ORIRO_RELEASE_QA_${channel}_LIVE_CI_ENABLED == 'true'`,
      );
      expect(qaWorkflow).not.toContain(`ORIRO_QA_${channel}_LIVE_CI_ENABLED`);
    }
  });
  ...
  it("runs full release children from the trusted workflow ref", () => {
    ...
    expectTextToIncludeAll(
      ...
      [
        "scripts/resolve-oriro-package-candidate.mjs",
        "--source ref",
    ...
    expectTextToIncludeAll(dispatchStep.run, [
      ...
      'args=(-f package_spec="${PACKAGE_SPEC:-oriro@beta}"',
    ...
    expectTextToIncludeAll(workflow, [
      ...
      "cancel-in-progress: ${{ (inputs.ref == 'main' && inputs.rerun_group == 'all') || startsWith(inputs.ref, 'tideoriro/alpha/') }}",
      "Verify release checks accepted Tideoriro alpha advisory lanes",
      "release_checks_advisory_only",
      "release_check_blocking_job",
      "is a package-safety Tideoriro alpha release-check lane",
  ...
  it("lets npm Telegram consume current-run or release-run package artifacts", () => {
    ...
    expectTextToIncludeAll(validateStep.run, [
      ...
      "package_spec must be oriro@alpha",
    ]);
    expectTextToIncludeAll(runStep.run, [
      'export ORIRO_NPM_TELEGRAM_PACKAGE_TGZ="${package_tgzs[0]}"',
    ]);
  });
  ...
  it("lets CI Telegram consumers wait on Convex leases instead of GitHub concurrency", () => {
    ...
    for (const [workflowPath, jobName, stepName] of telegramJobs) {
      ...
      expect(step.env?.ORIRO_QA_CREDENTIAL_ACQUIRE_TIMEOUT_MS).toBe("1800000");
    }
  });
  ...
  it("keeps release QA and repo E2E lanes off scarce 32-core runners", () => {
    ...
    expectTextToIncludeAll(liveE2eWorkflow, [
      "ORIRO_LIVE_GATEWAY_STEP_TIMEOUT_MS=180000",
      "ORIRO_LIVE_GATEWAY_MODEL_TIMEOUT_MS=600000",
    ]);
  });
  ...
  it("keeps release QA status artifacts blocking in the verifier", () => {
    ...
    expectTextToIncludeAll(verifyStep.run, [
      ...
      "::warning::${name} ended with ${result}; Tideoriro alpha treats non-package-safety release-check lanes as advisory.",
  ...
  it("keeps release publish creation compatible with gh api and prerelease notes", () => {
    ...
    const npmWorkflow = readFileSync(".github/workflows/oriro-npm-release.yml", "utf8");
    const fullReleaseWorkflow = readFileSync(FULL_RELEASE_VALIDATION_WORKFLOW, "utf8");

    expect(workflow).toContain("timeout-minutes: 120");
    expect(workflow).toContain("environment: npm-release");
    expect(workflow).toContain("Download Oriro npm preflight manifest");
    expect(workflow).toContain("Validate Oriro npm preflight manifest");
    ...
    expect(workflow).toContain(
      "publish_oriro_npm=true requires plugin_publish_scope=all-publishable",
    );
    ...
    expect(fullReleaseWorkflow).not.toContain("node /app/oriro.mjs agent");
    expect(fullReleaseWorkflow).toContain('ORIRO_EXTENSIONS="diagnostics-otel,codex"');
  ...
  it("keeps Oriro npm release pack tarball paths local before preflight upload", () => {
    const npmWorkflow = readFileSync(".github/workflows/oriro-npm-release.yml", "utf8");
  ...
  it("gates stable GitHub publication on the Windows Hub release asset contract", () => {
    ...
    const releaseSkill = readFileSync(
      ".agents/skills/release-oriro-maintainer/SKILL.md",
      "utf8",
    );

    expect(releaseWorkflow).toContain(
      "Stable Oriro publish requires an explicit windows_node_tag.",
    );
    expect(releaseWorkflow).toContain(
      "Stable Oriro publish requires candidate-approved windows_node_installer_digests.",
    );
    ...
    expect(releaseWorkflow).toContain(
      "Stable release OriroCompanion asset names do not exactly match the current contract",
    );
    expect(releaseWorkflow).toContain('select(.name | startswith("OriroCompanion-"))');
    expect(releaseWorkflow).toContain(
    ...
    expect(windowsWorkflow).toContain("must be an explicit oriro-windows-node release tag");
    ...
    expect(windowsWorkflow).toContain(
      "--repo oriro-ai/cli-windows-node --json tagName,isDraft,isPrerelease,assets,url",
    );
    expect(windowsWorkflow).toContain(
    ...
    expect(windowsWorkflow).toContain(
      "CN=Oriro Foundation, O=Oriro Foundation, L=Mill Valley, S=California, C=US",
    );
    expect(windowsWorkflow).toContain("has unexpected signer subject");
    expect(windowsWorkflow).toContain("OriroCompanion-SHA256SUMS.txt");
    expect(windowsWorkflow).toContain("Verify promoted release asset contract");
    expect(windowsWorkflow).toContain(
      "Promoted OriroCompanion asset names do not exactly match the current contract",
    );
    expect(windowsWorkflow).toContain(
  ...
  it("rejects unsafe direct Windows recovery before uploading assets", () => {
    ...
    const rejectUnexpectedTargetAssetsIndex = windowsWorkflow.indexOf(
      "Target Oriro release contains unexpected OriroCompanion assets before upload",
    );
  ...
  it("keeps beta release verification and OriroHub publish repair hooks wired", () => {
    const packageJson = JSON.parse(readFileSync("package.json", "utf8")) as {
      scripts?: Record<string, string>;
    };
    const releaseWorkflow = readFileSync(RELEASE_PUBLISH_WORKFLOW, "utf8");
    const oriroHubWorkflow = readFileSync(".github/workflows/plugin-orirohub-release.yml", "utf8");
    const oriroHubNewWorkflow = readFileSync(".github/workflows/plugin-orirohub-new.yml", "utf8");
    const pluginNpmWorkflow = readFileSync(".github/workflows/plugin-npm-release.yml", "utf8");
    const oriroNpmWorkflow = readFileSync(".github/workflows/oriro-npm-release.yml", "utf8");
    ...
    const oriroHubReleasePlanScript = readFileSync(
      "scripts/lib/oriro-release-orirohub-plan.ts",
      "utf8",
    );
    const oriroHubResolveRefIndex = oriroHubWorkflow.indexOf("- name: Resolve checked-out ref");
    const oriroHubValidateRefIndex = oriroHubWorkflow.indexOf(
      "- name: Validate ref is on a trusted publish branch",
    );
    const oriroHubSetupIndex = oriroHubWorkflow.indexOf("- name: Setup Node environment");
    const oriroHubMetadataIndex = oriroHubWorkflow.indexOf(
    ...
    expect(pluginPretagPackScript).toContain("scripts/plugin-orirohub-publish.sh");
    expect(oriroHubWorkflow).toContain('ORIROHUB_CLI_PACKAGE: "orirohub@0.21.0"');
    expect(oriroHubWorkflow).not.toContain("ORIROHUB_REPOSITORY:");
    expect(oriroHubWorkflow).not.toContain("ORIROHUB_REF:");
    expect(oriroHubWorkflow).toContain("pack_plugins_orirohub_artifacts:");
    expect(oriroHubWorkflow).toContain("Verify package-local runtime build");
    expect(oriroHubWorkflow).toContain("Install pinned OriroHub CLI wrapper");
    expect(oriroHubWorkflow).toContain("Pack OriroHub package artifact");
    expect(oriroHubWorkflow).toContain("Upload OriroHub package artifact");
    expect(oriroHubWorkflow).toContain("Validate OIDC source matches workflow ref");
    expect(oriroHubWorkflow).toContain(
      "Dry-run target ref to validate; real OIDC publishes must dispatch the workflow with --ref set to the target release tag/ref",
    );
    expect(oriroHubWorkflow).toContain(
      "Plugin OriroHub OIDC publishes must run from the same ref that is being published.",
    );
    expect(oriroHubWorkflow).toContain("The ref input is only supported for dry_run=true.");
    expect(oriroHubWorkflow).toContain(
    ...
    expect(oriroHubWorkflow).toContain(
      "uses: oriro-ai/clihub/.github/workflows/package-publish.yml@9d49df109d4ad3dc8a6ecf05d26b39f46d294721",
    );
    expect(oriroHubWorkflow).toContain("dry_run:");
    expect(oriroHubWorkflow).toContain("default: false");
    expect(oriroHubWorkflow).not.toContain("approve_plugin_orirohub_release:");
    expect(oriroHubWorkflow).toContain("approve_plugins_orirohub_release:");
    expect(oriroHubWorkflow).toContain("environment: orirohub-plugin-release");
    expect(oriroHubWorkflow).toContain("inputs.dry_run != true");
    expect(oriroHubWorkflow).toContain("release_publish_branch:");
    expect(oriroHubWorkflow).toContain(
    ...
    expect(oriroHubWorkflow).toContain(
      "always() && github.event_name == 'workflow_dispatch' && needs.preview_plugins_orirohub.outputs.has_candidates == 'true' && needs.pack_plugins_orirohub_artifacts.result == 'success' && (inputs.dry_run == true || needs.approve_plugins_orirohub_release.result == 'success')",
    );
    expect(oriroHubWorkflow).toContain("package_artifact_name: ${{ matrix.plugin.artifactName }}");
    expect(oriroHubWorkflow).toContain("source_repo: ${{ github.repository }}");
    expect(oriroHubWorkflow).toContain(
      "source_commit: ${{ needs.preview_plugins_orirohub.outputs.ref_revision }}",
    );
    expect(oriroHubWorkflow).toContain("source_ref: ${{ github.ref }}");
    expect(oriroHubWorkflow).toContain("source_path: ${{ matrix.plugin.packageDir }}");
    expect(oriroHubWorkflow).toContain(
    ...
    expect(oriroHubWorkflow).toContain("tags: ${{ matrix.plugin.publishTag }}");
    expect(oriroHubWorkflow).toContain("dry_run: ${{ inputs.dry_run }}");
    expect(oriroHubWorkflow).not.toContain("secrets.ORIROHUB_TOKEN");
    expect(oriroHubWorkflow).not.toContain("orirohub_token:");
    expect(oriroHubWorkflow).toContain("bootstrapCandidates");
    expect(oriroHubWorkflow).toContain("missingTrustedPublisher");
    expect(oriroHubWorkflow).toContain("bootstrap_candidate_count");
    expect(oriroHubWorkflow).toContain("missing_trusted_publisher_count");
    expect(oriroHubWorkflow).toContain("Bootstrap candidates requiring token bootstrap:");
    expect(oriroHubWorkflow).toContain("Missing trusted publisher candidates:");
    expect(oriroHubWorkflow).toContain("verify_published_orirohub_package:");
    expect(oriroHubWorkflow).toContain("inputs.dry_run != true");
    expect(oriroHubWorkflow).toContain("Verify published OriroHub package");
    expect(oriroHubWorkflow).not.toContain("bash scripts/plugin-orirohub-publish.sh --publish");
    expect(oriroHubWorkflow).not.toContain("Write OriroHub token config");
    expect(oriroHubWorkflow).not.toContain("Checkout OriroHub CLI source");
    expect(oriroHubWorkflow).not.toContain("packages/orirohub/src/cli.ts");
    expect(oriroHubWorkflow).not.toContain(
      "bun install failed while preparing OriroHub CLI; retrying",
    );
    expect(oriroHubWorkflow).toContain("max-parallel: 32");
    expect(oriroHubResolveRefIndex).toBeGreaterThanOrEqual(0);
    expect(oriroHubValidateRefIndex).toBeGreaterThan(oriroHubResolveRefIndex);
    expect(oriroHubSetupIndex).toBeGreaterThan(oriroHubValidateRefIndex);
    expect(oriroHubMetadataIndex).toBeGreaterThan(oriroHubSetupIndex);
    expect(releaseWorkflow).toContain("Plugin npm run ID");
    expect(releaseWorkflow).toContain("Plugin OriroHub run ID");
    expect(releaseWorkflow).toContain(
      "did not return an Actions run URL; refusing to guess from recent workflow_dispatch runs",
    );
    expect(releaseWorkflow).not.toContain("BEFORE_IDS=");
    expect(releaseWorkflow).not.toContain("before_json");
    expect(releaseWorkflow).toContain("plugin-orirohub-new.yml");
    expect(releaseWorkflow).toContain("Plugin OriroHub bootstrap run ID");
    expect(releaseWorkflow).toContain("scripts/oriro-release-orirohub-plan.ts");
    expect(releaseWorkflow).toContain("scripts/oriro-release-orirohub-runtime-state.ts");
    expect(isExecutable("scripts/oriro-release-orirohub-plan.ts")).toBe(true);
    expect(isExecutable("scripts/oriro-release-orirohub-runtime-state.ts")).toBe(true);
    expect(releaseWorkflow).toContain("oriro-release-orirohub-plan.json");
    expect(releaseWorkflow).toContain("oriro-release-orirohub-runtime-state");
    ...
    expect(releaseWorkflow).toContain("append_orirohub_dispatch_args");
    expect(releaseWorkflow).toContain("write_orirohub_runtime_state");
    ...
    expect(releaseWorkflow).toContain(
      "Waiting for plugin-orirohub-new.yml bootstrap to finish before continuing release publish.",
    );
    expect(releaseWorkflow).toContain("Oriro npm run ID");
    ...
    expect(releaseWorkflow).toContain("oriroNpmTarball");
    expect(releaseWorkflow).not.toContain('npm view "oriro@${release_version}" dist.tarball');
    expect(releaseWorkflow).toContain("release SHA");
    expect(oriroHubReleasePlanScript).toContain("not awaited by this proof");
    ...
    expect(oriroHubReleasePlanScript).toContain("--plugin-orirohub-bootstrap-run");
    expect(releaseWorkflow).toContain('verify_args+=(--plugins "${PLUGINS}")');
    expect(releaseWorkflow).toContain("oriro-release-postpublish-evidence");
    expect(releaseWorkflow).toContain("Failed child job summary");
    expect(releaseWorkflow).toContain("Workflow completion waits for OriroHub");
    expect(releaseWorkflow).toContain("Workflow completion does not wait for OriroHub");
    expect(releaseWorkflow).toContain('[[ "${WAIT_FOR_ORIROHUB}" == "true" ]]');
    expect(releaseWorkflow).toContain(
      '[[ -n "${plugin_orirohub_bootstrap_run_id}" && "${WAIT_FOR_ORIROHUB}" == "true" ]]',
    );
    expect(oriroHubReleasePlanScript).toContain("--skip-orirohub");
    expect(pluginNpmWorkflow).toContain("Validate release publish approval run");
    expect(oriroHubWorkflow).toContain("Validate release publish approval run");
    expect(oriroNpmWorkflow).toContain("Validate release publish approval run");
    ...
    expect(oriroHubWorkflow).toContain("Direct Plugin OriroHub Release dispatch");
    expect(oriroNpmWorkflow).toContain("Direct Oriro npm publish");
    expect(pluginNpmWorkflow).toContain('GITHUB_ACTOR}" != "github-actions[bot]"');
    expect(oriroHubWorkflow).toContain('GITHUB_ACTOR}" != "github-actions[bot]"');
    expect(oriroNpmWorkflow).toContain('GITHUB_ACTOR}" != "github-actions[bot]"');
    expect(pluginNpmWorkflow).toContain("Direct Plugin NPM Release recovery");
    expect(oriroHubWorkflow).toContain("Direct Plugin OriroHub Release recovery");
    expect(oriroNpmWorkflow).toContain("Direct Oriro npm recovery");
    expect(pluginNpmWorkflow).toContain("validate-release-publish-approval.mjs");
    expect(oriroHubWorkflow).toContain("validate-release-publish-approval.mjs");
    expect(oriroNpmWorkflow).toContain("validate-release-publish-approval.mjs");
    ...
    expect(oriroHubWorkflow.match(/environment: orirohub-plugin-release/g)?.length).toBe(1);
    expect(oriroHubNewWorkflow).toContain("name: Plugin OriroHub New");
    expect(oriroHubNewWorkflow).toContain('ORIROHUB_CLI_PACKAGE: "orirohub@0.21.0"');
    expect(oriroHubNewWorkflow).not.toContain("ORIROHUB_REPOSITORY:");
    expect(oriroHubNewWorkflow).not.toContain("ORIROHUB_REF:");
    expect(oriroHubNewWorkflow).toContain("environment: orirohub-plugin-bootstrap");
    expect(oriroHubNewWorkflow).toContain("secrets.ORIROHUB_TOKEN");
    expect(oriroHubNewWorkflow).not.toContain(
      "uses: oriro-ai/clihub/.github/workflows/package-publish.yml",
    );
    expect(oriroHubNewWorkflow).not.toContain("orirohub_token:");
    expect(oriroHubNewWorkflow).toContain("Validate pinned OriroHub trusted publisher CLI support");
    expect(oriroHubNewWorkflow).toContain('npm exec --yes --package "${ORIROHUB_CLI_PACKAGE}"');
    expect(oriroHubNewWorkflow).toContain(
      "ORIRO-277 03 - Split Oriro plugin OriroHub publishing into OIDC release and token bootstrap workflows",
    );
    expect(oriroHubNewWorkflow).toContain("Usage: orirohub package trusted-publisher set");
    expect(oriroHubNewWorkflow).toContain("Write OriroHub token config");
    expect(oriroHubNewWorkflow).toContain("ORIROHUB_CONFIG_PATH=${config_path}");
    expect(oriroHubNewWorkflow).toContain(
      "ORIROHUB_REGISTRY is required for token-gated OriroHub bootstrap.",
    );
    expect(oriroHubNewWorkflow).toContain(
      "ORIROHUB_TOKEN is required for token-gated OriroHub bootstrap.",
    );
    expect(oriroHubNewWorkflow).toContain("JSON.stringify({ registry, token }, null, 2)");
    expect(oriroHubNewWorkflow).toContain("Publish OriroHub bootstrap package");
    expect(oriroHubNewWorkflow).toContain("bash scripts/plugin-orirohub-publish.sh --publish");
    expect(oriroHubNewWorkflow).toContain("bootstrapMode");
    expect(oriroHubNewWorkflow).toContain("BOOTSTRAP_MODE: ${{ matrix.plugin.bootstrapMode }}");
    expect(oriroHubNewWorkflow).toContain("requiresManualOverride");
    expect(oriroHubNewWorkflow).toContain(
      'ORIRO_ORIROHUB_MANUAL_OVERRIDE_REASON="GitHub Actions trusted publisher repair before OIDC migration"',
    );
    expect(oriroHubNewWorkflow).toContain("configure-only");
    expect(oriroHubNewWorkflow).toContain(
      "version is already present on OriroHub; configuring trusted publisher only",
    );
    expect(oriroHubNewWorkflow).toContain(
      "EXPECTED_WORKFLOW_BRANCH: ${{ inputs.release_publish_branch || github.ref_name }}",
    );
    ...
    expect(oriroHubNewWorkflow).toContain('ORIRO_PLUGIN_NPM_RUNTIME_BUILD: "0"');
    expect(oriroHubNewWorkflow).toContain("trusted-publisher set");
    expect(oriroHubNewWorkflow).toContain("--workflow-filename plugin-orirohub-release.yml");
    expect(oriroHubNewWorkflow).not.toContain("--environment orirohub-plugin-release");
    expect(oriroHubNewWorkflow).toContain("trustedPublisher?.environment != null");
    expect(oriroHubNewWorkflow).toContain("without an environment pin");
    expect(oriroHubNewWorkflow).not.toContain("Checkout OriroHub CLI source");
    expect(oriroHubNewWorkflow).not.toContain("packages/orirohub/src/cli.ts");
    expect(oriroHubNewWorkflow).toContain("verify_bootstrap_orirohub_package:");
    expect(oriroHubNewWorkflow).toContain("Verify bootstrap OriroHub package and trusted publisher");
    expect(oriroHubNewWorkflow).toContain("/trusted-publisher");
    expect(oriroHubNewWorkflow).toContain('trustedPublisher?.repository !== "oriro-ai/cli"');
    expect(oriroNpmWorkflow).toContain("environment: npm-release");
    ...
    expect(releaseWorkflow).toContain("jq -r '.normal.ref' \"${orirohub_plan_path}\"");
    expect(releaseWorkflow).toContain("jq -r '.normal.workflow' \"${orirohub_plan_path}\"");
    expect(releaseWorkflow).toContain("jq -r '.bootstrap.ref' \"${orirohub_plan_path}\"");
    expect(releaseWorkflow).toContain("jq -r '.bootstrap.workflow' \"${orirohub_plan_path}\"");
    expect(releaseWorkflow).toContain('--orirohub-workflow-ref "${orirohub_workflow_ref}"');
    expect(releaseWorkflow).toContain(
  ...
  it("keeps release workflow setup and timeout budgets bounded", () => {
    ...
    const crossOs = readWorkflow(".github/workflows/oriro-cross-os-release-checks-reusable.yml");
    const liveE2e = readWorkflow(LIVE_E2E_WORKFLOW);
    const releaseWorkflowPaths = [
      FULL_RELEASE_VALIDATION_WORKFLOW,
      RELEASE_CHECKS_WORKFLOW,
      ".github/workflows/oriro-cross-os-release-checks-reusable.yml",
      LIVE_E2E_WORKFLOW,
      NPM_TELEGRAM_WORKFLOW,
      ".github/workflows/oriro-release-publish.yml",
      ".github/workflows/oriro-npm-release.yml",
  ...
  it("kills timed TUI PTY workflow runs after the grace period", () => {
    ...
    expect(job.env?.ORIRO_TUI_PTY_INCLUDE_LOCAL).toBe("1");
