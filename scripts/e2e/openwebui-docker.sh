#!/usr/bin/env bash
# Runs Open WebUI against a Dockerized Oriro Gateway and verifies the proxied
...
IMAGE_NAME="$(docker_e2e_resolve_image "oriro-openwebui-e2e" ORIRO_OPENWEBUI_E2E_IMAGE)"
OPENWEBUI_IMAGE="${OPENWEBUI_IMAGE:-ghcr.io/open-webui/open-webui:v0.8.10}"
MAX_MEMORY_MIB="$(docker_e2e_read_nonnegative_decimal_env ORIRO_OPENWEBUI_MAX_MEMORY_MIB 8192)"
MAX_CPU_PERCENT="$(docker_e2e_read_nonnegative_decimal_env ORIRO_OPENWEBUI_MAX_CPU_PERCENT 1600)"
...
MODEL="${ORIRO_OPENWEBUI_MODEL:-openai/gpt-5.5}"
PROMPT_NONCE="OPENWEBUI_DOCKER_E2E_$(date +%s)_$$"
PROMPT="${ORIRO_OPENWEBUI_PROMPT:-Reply with exactly this token and nothing else: ${PROMPT_NONCE}}"
PORT="$(docker_e2e_read_tcp_port_env ORIRO_OPENWEBUI_GATEWAY_PORT 18789)"
WEBUI_PORT="$(docker_e2e_read_tcp_port_env ORIRO_OPENWEBUI_PORT 8080)"
TOKEN="openwebui-e2e-$(date +%s)-$$"
ADMIN_EMAIL="${ORIRO_OPENWEBUI_ADMIN_EMAIL:-openwebui-e2e@example.com}"
ADMIN_PASSWORD="${ORIRO_OPENWEBUI_ADMIN_PASSWORD:-OpenWebUI-E2E-Password-$(date +%s)-$$}"
NET_NAME="oriro-openwebui-e2e-$$"
GW_NAME="oriro-openwebui-gateway-$$"
OW_NAME="oriro-openwebui-$$"
PROVIDER_TIMEOUT_SECONDS="${ORIRO_OPENWEBUI_PROVIDER_TIMEOUT_SECONDS:-900}"
DOCKER_PULL_TIMEOUT="${ORIRO_OPENWEBUI_DOCKER_PULL_TIMEOUT:-600s}"
...
PROBE_FETCH_TIMEOUT_MS="${ORIRO_OPENWEBUI_FETCH_TIMEOUT_MS:-$((PROVIDER_TIMEOUT_SECONDS_DECIMAL * 1000 + 60000))}"
validate_positive_int ORIRO_OPENWEBUI_FETCH_TIMEOUT_MS "$PROBE_FETCH_TIMEOUT_MS"
DOCKER_COMMAND_TIMEOUT="${ORIRO_OPENWEBUI_DOCKER_COMMAND_TIMEOUT:-$((PROVIDER_TIMEOUT_SECONDS_DECIMAL + 90))s}"
...
PROFILE_FILE="${ORIRO_TESTBOX_PROFILE_FILE:-$HOME/.oriro-testbox-live.profile}"
...
STATS_LOG="$(mktemp "${TMPDIR:-/tmp}/oriro-openwebui-stats.XXXXXX")"
PROBE_LOG="$(mktemp "${TMPDIR:-/tmp}/oriro-openwebui-probe.XXXXXX")"
STATS_STOP_FILE="$(mktemp "${TMPDIR:-/tmp}/oriro-openwebui-stats-stop.XXXXXX")"
STATS_PIDS=()
...
docker_e2e_docker_cmd run -d \
  ...
  --name "$GW_NAME" \
  --network "$NET_NAME" \
  -e "ORIRO_GATEWAY_TOKEN=$TOKEN" \
  -e "ORIRO_OPENWEBUI_MODEL=$MODEL" \
  -e "ORIRO_SKIP_CHANNELS=1" \
  -e "ORIRO_SKIP_GMAIL_WATCHER=1" \
  -e "ORIRO_SKIP_CRON=1" \
  -e "ORIRO_SKIP_CANVAS_HOST=1" \
  -e "ORIRO_OPENWEBUI_PROVIDER_TIMEOUT_SECONDS=$PROVIDER_TIMEOUT_SECONDS" \
  -e OPENAI_API_KEY \
  ...
  bash -lc '
    ...
    batch_file="$(mktemp /tmp/oriro-openwebui-config.XXXXXX.json)"
    ORIRO_CONFIG_BATCH_PATH="$batch_file" node scripts/e2e/lib/fixture.mjs openwebui-config "$openai_api_key"
    node "$entry" config set --batch-file "$batch_file" >/dev/null
    rm -f "$batch_file"
    node scripts/e2e/lib/fixture.mjs openwebui-workspace

    oriro_e2e_exec_gateway "$entry" '"$PORT"' lan /tmp/openwebui-gateway.log
  ' >/dev/null
start_openwebui_stats_sampler

echo "Waiting for gateway HTTP surface..."
if ! docker_e2e_wait_container_bash "$GW_NAME" 240 1 "ORIRO_HTTP_PROBE_BEARER='$TOKEN' node scripts/e2e/lib/openwebui/http-probe.mjs 'http://127.0.0.1:$PORT/v1/models' 200"; then
...
docker_e2e_docker_cmd run -d \
  ...
  --network "$NET_NAME" \
  -e ENV=prod \
  -e WEBUI_NAME="Oriro E2E" \
  -e WEBUI_SECRET_KEY="oriro-openwebui-e2e-secret-key-v1" \
  -e OFFLINE_MODE=True \
  ...
  -e WEBUI_ADMIN_NAME="Oriro E2E" \
  -e ENABLE_SIGNUP=False \
  -e DEFAULT_MODELS="oriro/default" \
...
if ! docker_e2e_wait_container_bash "$GW_NAME" 90 5 "ORIRO_HTTP_PROBE_BEARER='$TOKEN' ORIRO_HTTP_PROBE_TIMEOUT_MS=8000 node scripts/e2e/lib/openwebui/http-probe.mjs 'http://$GW_NAME:$PORT/v1/models' 200"; then
...
echo "Running Open WebUI -> Oriro smoke..."
set +e
