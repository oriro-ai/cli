...
import { isRecord as hasRecord } from "@oriro/normalization-core/record-coerce";
...
function listAgentRecords(cfg: OriroConfig): Record<string, unknown>[] {
  return Array.isArray(cfg.agents?.list) ? cfg.agents.list.filter(hasRecord) : [];
}

function hasChannels(cfg: OriroConfig): boolean {
  return hasRecord(cfg.channels);
}

function hasPlugins(cfg: OriroConfig): boolean {
  return hasRecord(cfg.plugins);
}

function hasPluginLoadPaths(cfg: OriroConfig): boolean {
...
function hasSubagentAllowlistConfig(cfg: OriroConfig): boolean {
...
function hasConfiguredSafeBins(cfg: OriroConfig): boolean {
...
function sourceReplyRuntimeMayAllowMessageTool(cfg: OriroConfig): boolean {
...
function resolveGroupVisibleReplyProvenance(cfg: OriroConfig): {
...
export function collectVisibleReplyToolPolicyWarnings(cfg: OriroConfig): string[] {
  const groupPolicy = resolveGroupVisibleReplyProvenance(cfg);
...
export function collectChannelBoundMessageToolPolicyWarnings(cfg: OriroConfig): string[] {
...
export function collectProfileConfiguredToolSectionWarnings(cfg: OriroConfig): string[] {
  const warnings: string[] = [];
...
async function resolveDoctorChannelPreviewConfig(params: {
  ...
  allowExec?: boolean;
}): Promise<{ cfg: OriroConfig; diagnostics: string[] }> {
