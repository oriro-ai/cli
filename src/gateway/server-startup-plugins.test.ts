...
const resolveOriroPackageRootSync = vi.hoisted(() => vi.fn((_params: unknown) => "/package"));
...
vi.mock("../infra/oriro-root.js", () => ({
  resolveOriroPackageRootSync: (params: unknown) => resolveOriroPackageRootSync(params),
}));
...
function slackConfig(): OriroConfig {
  return {
    channels: {
...
describe("prepareGatewayPluginBootstrap startup plugins", () => {
  beforeEach(() => {
    ...
    resolveOriroPackageRootSync.mockClear().mockReturnValue("/package");
...
describe("warnUnregisteredConfiguredMemoryEmbeddingProviders", () => {
  ...
  function customOllamaConfig(source: "provider" | "fallback" = "provider"): OriroConfig {
    const memorySearch =
      source === "provider"
