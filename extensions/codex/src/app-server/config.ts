...
} from "oriro/plugin-sdk/agent-runtime";
import {
  resolveExecApprovalsFromFile,
  type ExecApprovalsFile,
} from "oriro/plugin-sdk/exec-approvals-runtime";
import { resolvePositiveTimerTimeoutMs } from "oriro/plugin-sdk/number-runtime";
import { normalizeAgentId } from "oriro/plugin-sdk/routing";
import {
  buildSecretInputSchema,
  normalizeResolvedSecretInputString,
  type SecretInput,
} from "oriro/plugin-sdk/secret-input";
import { normalizeTrimmedStringList } from "oriro/plugin-sdk/string-coerce-runtime";
import { detectWindowsSpawnCommandInlineArgs } from "oriro/plugin-sdk/windows-spawn";
import { z } from "zod";
import type { CodexSandboxPolicy, CodexServiceTier, JsonObject, JsonValue } from "./protocol.js";

const START_OPTIONS_KEY_SECRET_SYMBOL = Symbol.for("oriro.codexAppServerStartOptionsKeySecret");
...
type OriroExecMode = "deny" | "allowlist" | "ask" | "auto" | "full";
type OriroExecSecurity = "deny" | "allowlist" | "full";
type OriroExecAsk = "off" | "on-miss" | "always";
type OriroExecApprovalFloorsForCodexAppServer = {
  security?: OriroExecSecurity;
  ask?: OriroExecAsk;
};
export type OriroExecPolicyForCodexAppServer = {
  mode?: OriroExecMode;
  security: OriroExecSecurity;
  ask: OriroExecAsk;
  touched: boolean;
};
type OriroExecPolicy = OriroExecPolicyForCodexAppServer;
...
const DEFAULT_CODEX_APP_SERVER_NETWORK_PROXY_PROFILE_PREFIX = "oriro-network";
...
export function resolveCodexAppServerRuntimeOptions(
  ...
  const envCommand = readNonEmptyString(env.ORIRO_CODEX_APP_SERVER_BIN);
  ...
  const args = resolveArgs(config.args, env.ORIRO_CODEX_APP_SERVER_ARGS);
  ...
  const execMode = resolveEffectiveOriroExecModeForCodexAppServer({
    execMode: params.execMode,
    execPolicy: params.execPolicy,
  });
  assertCodexAppServerAllowedForOriroExecMode(execMode);
  const explicitPolicyMode =
    resolvePolicyMode(config.mode) ?? resolvePolicyMode(env.ORIRO_CODEX_APP_SERVER_MODE);
  const configuredSandbox =
    resolveSandbox(config.sandbox) ?? resolveSandbox(env.ORIRO_CODEX_APP_SERVER_SANDBOX);
  const explicitApprovalsReviewer = resolveApprovalsReviewer(config.approvalsReviewer);
  const normalizedPolicyMode = resolveCodexPolicyModeForOriroExecMode(execMode);
  ...
  const execModeRequiringPromptingApprovals: Extract<OriroExecMode, "auto" | "ask"> | undefined =
  ...
  const envApprovalPolicy = resolveApprovalPolicy(env.ORIRO_CODEX_APP_SERVER_APPROVAL_POLICY);
...
export function resolveCodexComputerUseConfig(
  ...
  const marketplaceSource =
    ...
    readNonEmptyString(env.ORIRO_CODEX_COMPUTER_USE_MARKETPLACE_SOURCE);
  const marketplacePath =
    readNonEmptyString(params.overrides?.marketplacePath) ??
    readNonEmptyString(config.marketplacePath) ??
    readNonEmptyString(env.ORIRO_CODEX_COMPUTER_USE_MARKETPLACE_PATH);
  const marketplaceName =
    readNonEmptyString(params.overrides?.marketplaceName) ??
    readNonEmptyString(config.marketplaceName) ??
    readNonEmptyString(env.ORIRO_CODEX_COMPUTER_USE_MARKETPLACE_NAME);
  const autoInstall =
    params.overrides?.autoInstall ??
    config.autoInstall ??
    readBooleanEnv(env.ORIRO_CODEX_COMPUTER_USE_AUTO_INSTALL) ??
    false;
  const marketplaceDiscoveryTimeoutMs = normalizePositiveNumber(
    params.overrides?.marketplaceDiscoveryTimeoutMs ??
      ...
      readNumberEnv(env.ORIRO_CODEX_COMPUTER_USE_MARKETPLACE_DISCOVERY_TIMEOUT_MS),
    DEFAULT_CODEX_COMPUTER_USE_MARKETPLACE_DISCOVERY_TIMEOUT_MS,
  );
  const enabled =
    params.overrides?.enabled ??
    config.enabled ??
    readBooleanEnv(env.ORIRO_CODEX_COMPUTER_USE) ??
  ...
  return {
    ...
    pluginName:
      ...
      readNonEmptyString(env.ORIRO_CODEX_COMPUTER_USE_PLUGIN_NAME) ??
    ...
    mcpServerName:
      ...
      readNonEmptyString(env.ORIRO_CODEX_COMPUTER_USE_MCP_SERVER_NAME) ??
...
function resolveDefaultCodexAppServerPolicy(params: {
  ...
  execModeRequiringPromptingApprovals?: Extract<OriroExecMode, "auto" | "ask">;
  execModeRequiringUserReviewer?: OriroExecMode;
  env?: NodeJS.ProcessEnv;
  requirementsToml?: string | null;
  requirementsPath?: string;
  readRequirementsFile?: (path: string) => string | undefined;
...
function selectGuardianApprovalsReviewer(
  ...
  execModeRequiringAutoReviewer?: Extract<OriroExecMode, "auto">,
...
export function resolveOriroExecModeFromConfig(params: {
  config?: unknown;
  agentId?: string;
}): OriroExecMode | undefined {
  const policy = resolveOriroExecPolicyFromConfig(params);
  return policy.touched ? policy.mode : undefined;
}

function resolveOriroExecPolicyFromConfig(params: {
  config?: unknown;
  agentId?: string;
}): OriroExecPolicy {
  const root = readRecord(params.config);
  const globalExec = readRecord(readRecord(root?.tools)?.exec);
  const globalPolicy = applyOriroExecPolicyLayer(createDefaultOriroExecPolicy(), globalExec);
  ...
  return applyOriroExecPolicyLayer(globalPolicy, agentExec);
}

export function resolveOriroExecModeForCodexAppServer(params: {
  execOverrides?: {
    security?: unknown;
    ask?: unknown;
  };
  ...
  const policy = resolveOriroExecPolicyForCodexAppServer(params);
  return policy.touched ? policy.mode : undefined;
}

export function resolveOriroExecPolicyForCodexAppServer(params: {
  execOverrides?: {
    security?: unknown;
    ask?: unknown;
  };
  ...
  config?: unknown;
  agentId?: string;
}): OriroExecPolicyForCodexAppServer {
  const basePolicy = resolveOriroExecPolicyFromConfig({
    config: params.config,
    agentId: params.agentId,
  });
  const overridePolicy = applyOriroExecPolicyLayer(basePolicy, params.execOverrides);
  const approvalFloors = resolveOriroExecApprovalFloorsForCodexAppServer({
    approvals: params.approvals,
    agentId: params.agentId,
    policy: overridePolicy,
  });
  return applyOriroExecApprovalFloors(overridePolicy, approvalFloors);
}

function resolveEffectiveOriroExecModeForCodexAppServer(params: {
  execMode?: OriroExecMode;
  execPolicy?: OriroExecPolicyForCodexAppServer;
}): OriroExecMode | undefined {
...
function resolveCodexPolicyModeForOriroExecMode(
  mode: OriroExecMode | undefined,
...
function assertCodexAppServerAllowedForOriroExecMode(mode: OriroExecMode | undefined): void {
...
function createDefaultOriroExecPolicy(): OriroExecPolicy {
  return {
    security: "full",
    ask: "off",
    touched: false,
...
function applyOriroExecPolicyLayer(
  base: OriroExecPolicy,
  ...
  if (mode !== undefined) {
    return {
      ...resolveOriroExecPolicyForMode(mode),
      touched: true,
    };
  }
  ...
  return {
    mode: resolveOriroExecModeFromPolicy({ security: nextSecurity, ask: nextAsk }),
...
function resolveOriroExecApprovalFloorsForCodexAppServer(params: {
  approvals?: ExecApprovalsFile;
  agentId?: string;
  policy: OriroExecPolicy;
}): OriroExecApprovalFloorsForCodexAppServer | undefined {
  if (!params.approvals) {
    return undefined;
  }
...
function applyOriroExecApprovalFloors(
  base: OriroExecPolicy,
  approvalFloors?: OriroExecApprovalFloorsForCodexAppServer,
): OriroExecPolicy {
  if (!approvalFloors) {
    return base;
  }
  const nextSecurity = approvalFloors.security
    ? minOriroExecSecurity(base.security, approvalFloors.security)
    : base.security;
  const nextAsk = approvalFloors.ask ? maxOriroExecAsk(base.ask, approvalFloors.ask) : base.ask;
...
function resolveOriroExecPolicyForMode(
  mode: OriroExecMode,
): Omit<OriroExecPolicy, "touched"> {
  switch (mode) {
    case "deny":
...
function resolveOriroExecModeFromPolicy(params: {
  security: OriroExecSecurity;
  ask: OriroExecAsk;
}): OriroExecMode {
...
function minOriroExecSecurity(
  left: OriroExecSecurity,
  right: OriroExecSecurity,
): OriroExecSecurity {
  const order: Record<OriroExecSecurity, number> = { deny: 0, allowlist: 1, full: 2 };
  return order[left] <= order[right] ? left : right;
}

function maxOriroExecAsk(left: OriroExecAsk, right: OriroExecAsk): OriroExecAsk {
  const order: Record<OriroExecAsk, number> = { off: 0, "on-miss": 1, always: 2 };
  return order[left] >= order[right] ? left : right;
}

function readExecMode(value: unknown): OriroExecMode | undefined {
  return value === "deny" ||
    value === "allowlist" ||
    value === "ask" ||
    value === "auto" ||
...
function readExecSecurity(value: unknown): OriroExecSecurity | undefined {
  return value === "deny" || value === "allowlist" || value === "full" ? value : undefined;
}

function readExecAsk(value: unknown): OriroExecAsk | undefined {
