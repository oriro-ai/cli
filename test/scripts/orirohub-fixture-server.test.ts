// OriroHub Fixture Server tests cover the local package fixture HTTP contract.
...
const SCRIPT_PATH = "scripts/e2e/lib/orirohub-fixture-server.cjs";
const PACKAGE_NAME = "@oriro/kitchen-sink";
...
async function startFixtureServer(profile: string) {
  const root = makeTempDir(tempDirs, "oriro-orirohub-fixture-server-");
...
describe("OriroHub fixture server", () => {
  it("serves package metadata and npm-pack artifacts for kitchen-sink fixtures", async () => {
    ...
    expect(artifactResponse.headers.get("x-orirohub-artifact-type")).toBe("npm-pack-tarball");
    expect(artifactResponse.headers.get("x-orirohub-artifact-sha256")).toMatch(/^[a-f0-9]{64}$/u);
  ...
  it("rejects missing startup arguments before binding a fixture server", () => {
    ...
    expect(result.stderr).toContain(
      "usage: orirohub-fixture-server.cjs <kitchen-sink-plugin|plugins> <port-file>",
    );
  });
});
