...
export function maybeWarnAboutDebugProxyCoverage(
  ...
  warn(
    `[oriro proxy] debug proxy coverage: ${summary.captured}/${summary.total} captured, ${summary.proxyOnly} proxy-only, ${summary.uncovered} uncovered.`,
  );
  warn(
    `[oriro proxy] remaining gaps: ${partial.map((entry) => entry.id).join(", ")}. Run \`oriro proxy coverage\` for details.`,
  );
}
