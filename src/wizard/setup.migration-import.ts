...
function hasMeaningfulConfig(config: OriroConfig): boolean {
...
function assertFreshSetupMigrationTarget(freshness: {
  ...
  if (freshness.fresh || process.env.ORIRO_MIGRATION_EXISTING_IMPORT === "1") {
    return;
  }
  throw new Error(
    [
      "Migration import during onboarding requires a fresh Oriro setup.",
...
export async function runSetupMigrationImport(params: {
  ...
  prompter: WizardPrompter;
  runtime: RuntimeEnv;
  commitConfigFile: (config: OriroConfig) => Promise<OriroConfig>;
}): Promise<void> {
  const [
  ...
  // can reference a concrete Oriro config target.
