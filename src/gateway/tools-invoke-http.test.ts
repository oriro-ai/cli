...
let lastCreateOriroToolsContext: Record<string, unknown> | undefined;
...
vi.mock("../agents/oriro-tools.js", () => {
  ...
  const tools = [
    ...
    },
    {
      ...
      execute: async () => ({
        ok: true,
        route: {
          agentTo: lastCreateOriroToolsContext?.agentTo,
          agentThreadId: lastCreateOriroToolsContext?.agentThreadId,
        },
        inheritedToolDenylist: lastCreateOriroToolsContext?.inheritedToolDenylist,
      }),
    },
    {
      name: "sessions_send",
  ...
  return {
    createOriroTools: (ctx: Record<string, unknown>) => {
      lastCreateOriroToolsContext = ctx;
...
beforeEach(() => {
  ...
  pluginHttpHandlers = [];
  cfg = {};
  lastCreateOriroToolsContext = undefined;
...
const gatewayAuthHeaders = () => ({ "x-oriro-scopes": "operator.write" });
const gatewayAdminHeaders = () => ({ "x-oriro-scopes": "operator.admin" });
...
describe("POST /tools/invoke", () => {
  it("invokes a tool and returns {ok:true,result}", async () => {
    ...
    expect(lastCreateOriroToolsContext?.allowMediaInvokeCommands).toBe(true);
    expect(lastCreateOriroToolsContext?.disablePluginTools).toBe(true);
  ...
  it("opts direct gateway tool invocation into gateway subagent binding", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateOriroToolsContext?.allowGatewaySubagentBinding).toBe(true);
  });
  ...
  it("keeps plugin tools enabled for non-core tool invokes", async () => {
    ...
    expect(res.status).toBe(200);
    expect(lastCreateOriroToolsContext?.disablePluginTools).toBe(false);
  });
  ...
  it("allows the requested plugin tool through Gateway profile filtering", async () => {
    ...
    expect(lastCreateOriroToolsContext?.pluginToolAllowlist).toContain("plugin_doctor");
  });
  ...
  it("uses tools.alsoAllow for optional plugin discovery without loading every plugin tool", async () => {
    ...
    expect(body.result?.permissionFlow).toBe(true);
    expect(lastCreateOriroToolsContext?.pluginToolAllowlist).toContain("plugin_doctor");
    expect(lastCreateOriroToolsContext?.pluginToolAllowlist).not.toContain("*");
  });
  ...
  it("propagates message target/thread headers into tools context for sessions_spawn", async () => {
    ...
    const res = await invokeTool({
      ...
      headers: {
        ...gatewayAuthHeaders(),
        "x-oriro-message-to": "channel:24514",
        "x-oriro-thread-id": "thread-24514",
      },
      tool: "sessions_spawn",
      sessionKey: "main",
    });
  ...
  it("keeps shared-secret bearer auth as owner for explicitly allowed owner-only tools", async () => {
    ...
    expect(lastCreateOriroToolsContext?.senderIsOwner).toBe(true);
  });
  ...
  it("derives sender owner identity from HTTP auth instead of caller headers", async () => {
    ...
    expect(lastCreateOriroToolsContext?.senderIsOwner).toBe(false);
