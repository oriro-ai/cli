...
async function extractOrirohubSkillInstallVerifier(): Promise<string> {
  const script = await readFile("scripts/e2e/lib/skills/orirohub-install-proof.sh", "utf8");
  const marker =
    'node --input-type=module - "$ORIRO_CONFIG_PATH" "$skill_dir" "$origin_json" "$lock_json" "$info_json" "$slug" <<\'NODE\'\n';
  const start = script.indexOf(marker);
  if (start === -1) {
    throw new Error("OriroHub skill install verifier heredoc was not found");
  }
  const verifierStart = start + marker.length;
  const verifierEnd = script.indexOf("\nNODE", verifierStart);
  if (verifierEnd === -1) {
    throw new Error("OriroHub skill install verifier heredoc was not terminated");
  }
...
describe("e2e shell tempfile hygiene", () => {
  ...
  it("preserves wizard exit status when reporting failures", async () => {
    const tempRoot = await mkdtemp(path.join(tmpdir(), "oriro-onboard-status-test-"));
...
export ORIRO_ONBOARD_SCENARIO_SOURCE_ONLY=1
export ORIRO_ONBOARD_E2E_TMPDIR=${JSON.stringify(tempRoot)}
ORIRO_ENTRY=node
oriro_test_state_create() { :; }
...
`,
  ...
  it("checks local onboarding logs for systemd noise", async () => {
    ...
    expect(contents).toContain('ORIRO_E2E_LOG_DIR="$ONBOARD_TMP_DIR/logs"');
    ...
    expect(contents).toContain('validate_local_basic_log "$ORIRO_E2E_LAST_LOG_PATH"');
    expect(contents).not.toContain(
      "validate_local_basic_log /tmp/oriro-onboard-local-basic.log",
    );
    expect(contents).toContain(
  ...
  it("probes onboarding gateway readiness through TCP", async () => {
    const tempRoot = await mkdtemp(path.join(tmpdir(), "oriro-onboard-gateway-log-"));
...
`,
  ...
  it("rejects onboarding gateway readiness when the TCP probe fails", async () => {
    const tempRoot = await mkdtemp(path.join(tmpdir(), "oriro-onboard-gateway-tcp-"));
...
export ORIRO_ONBOARD_SCENARIO_SOURCE_ONLY=1
export ORIRO_ONBOARD_E2E_TMPDIR=${JSON.stringify(tempRoot)}
export ORIRO_ONBOARD_GATEWAY_WAIT_ATTEMPTS=2
export ORIRO_ONBOARD_GATEWAY_WAIT_INTERVAL_S=0.1
ORIRO_ENTRY=node
source scripts/e2e/lib/onboard/scenario.sh

oriro_e2e_probe_tcp() { return 1; }
sleep 30 &
GATEWAY_PID="$!"
printf 'listening on ws://127.0.0.1:18789\\n' >"$GATEWAY_LOG_PATH"
...
`,
  ...
  it("rejects invalid onboarding gateway wait attempts before probing", async () => {
    const tempRoot = await mkdtemp(path.join(tmpdir(), "oriro-onboard-gateway-attempts-"));
...
export ORIRO_ONBOARD_SCENARIO_SOURCE_ONLY=1
export ORIRO_ONBOARD_E2E_TMPDIR=${JSON.stringify(tempRoot)}
export ORIRO_ONBOARD_GATEWAY_WAIT_ATTEMPTS=2x
ORIRO_ENTRY=node
source scripts/e2e/lib/onboard/scenario.sh
...
`,
    ...
    try {
      ...
      expect(result.stderr).toContain("invalid ORIRO_ONBOARD_GATEWAY_WAIT_ATTEMPTS: 2x");
  ...
  it("removes fallback OriroHub skill install HOME on failure", async () => {
    const tempRoot = await mkdtemp(path.join(tmpdir(), "oriro-orirohub-home-test-"));
...
`,
    ...
    try {
      const result = spawnSync("bash", ["scripts/e2e/lib/skills/orirohub-install-proof.sh"], {
        cwd: process.cwd(),
        encoding: "utf8",
        env: {
          ...process.env,
          ORIRO_CURRENT_PACKAGE_TGZ: "",
          ORIRO_TEST_STATE_SCRIPT_B64: "",
      ...
      expect(
        scratchEntries.filter((entry) => entry.startsWith("oriro-skill-install-home.")),
      ).toEqual([]);
    } finally {
      await rm(tempRoot, { force: true, recursive: true });
    }
  ...
  it("rejects OriroHub skill info paths that only share a resolved prefix", async () => {
    const tempRoot = await mkdtemp(path.join(tmpdir(), "oriro-orirohub-info-path-"));
    ...
    try {
      ...
      const result = spawnSync(
        ...
        {
          encoding: "utf8",
          input: await extractOrirohubSkillInstallVerifier(),
        },
      );
