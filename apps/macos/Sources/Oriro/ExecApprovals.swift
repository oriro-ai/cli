...
enum ExecApprovalsStore {
    private static let logger = Logger(subsystem: "ai.oriro", category: "exec-approvals")
    ...
    static func fileURL() -> URL {
        OriroPaths.stateDirURL.appendingPathComponent("exec-approvals.json")
    }

    static func socketPath() -> String {
        OriroPaths.stateDirURL.appendingPathComponent("exec-approvals.sock").path
    }

    private static func legacyStateDirURLs() -> [URL] {
        if let home = OriroEnv.path("ORIRO_HOME") {
            var urls = [
                URL(fileURLWithPath: home, isDirectory: true)
                    .appendingPathComponent(".oriro", isDirectory: true),
            ]
    ...
    private static func legacyFileURLIfPending() -> URL? {
        guard OriroEnv.path("ORIRO_STATE_DIR") != nil else { return nil }
    ...
    private static func ensureSecureStateDirectory() {
        let url = OriroPaths.stateDirURL
        do {
